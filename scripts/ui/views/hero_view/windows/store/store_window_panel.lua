...
function StoreWindowPanel:on_enter(params, offset)
...
function StoreWindowPanel:_create_ui_elements(params, offset)
	...
	mark_all_seen_button.style.new_marker.offset = { -80, 8, 10 }
...
function StoreWindowPanel:on_exit(params)
	print("[HeroViewWindow] Exit Substate StoreWindowPanel")
	self._ui_animator = nil
end

function StoreWindowPanel:update(dt, t)
...
function StoreWindowPanel:post_update(dt, t)
	self:_handle_input(dt, t)
end

function StoreWindowPanel:_update_animations(dt)
...
function StoreWindowPanel:_is_stepper_button_pressed(widget)
	local content = widget.content
...
function StoreWindowPanel:_handle_input(dt, t)
...
function StoreWindowPanel:_on_panel_button_selected(index)
	local parent = self._parent

	local widget = self._title_button_widgets [index]
	local page_name = widget.content.page_name
	local path = { page_name }
	parent:go_to_store_path(path)
end

function StoreWindowPanel:_set_selected_option(index)
...
function StoreWindowPanel:_update_selected_option()
	local parent = self._parent
	local path = parent:get_store_path()
	...
	if path then
		...
		for i, widget in ipairs(title_button_widgets) do
			local page_name = widget.content.page_name
			if
			page_name == root_page and i ~= self._selected_index then
...
function StoreWindowPanel:_draw(dt)
...
function StoreWindowPanel:_play_sound(event)
	return self._parent:play_sound(event)
end

function StoreWindowPanel:_setup_input_buttons()
	local force = true
...
function StoreWindowPanel:_handle_back_button_visibility()
...
function StoreWindowPanel:_reset_back_button()
	local back_button = self._widgets_by_name.back_button
...
function StoreWindowPanel:_get_text_width(text_style, text)
...
function StoreWindowPanel:_set_text_button_horizontal_position(widget, x_position)
	local offset = widget.offset
	offset [1] = x_position
end

function StoreWindowPanel:_animate_title_entry(widget, dt)
	local content = widget.content
	local style = widget.style
...
function StoreWindowPanel:_animate_back_button(widget, dt)
	local content = widget.content
	local style = widget.style
	local hotspot = content.button_hotspot
...
function StoreWindowPanel:_sync_wallet_matchmaking_location()
...
function StoreWindowPanel:_sync_player_wallet()
...
function StoreWindowPanel:_start_panel_selection_animation(previous_selected_index, new_selected_index)
	local widgets_by_name = self._widgets_by_name
...
function StoreWindowPanel:_update_panel_selection_animation(dt)
