...
local RPCS = { "rpc_spawn_pickup_with_physics", "rpc_spawn_pickup", "rpc_spawn_linked_pickup", "rpc_force_use_pickup", "rpc_delete_pickup", "rpc_delete_limited_owned_pickup_unit", "rpc_delete_limited_owned_pickups", "rpc_delete_limited_owned_pickup_type" }
...
local extensions = { "LifeTimePickupUnitExtension", "LimitedOwnedPickupUnitExtension", "PlayerTeleportingPickupExtension", "PickupUnitExtension", "PickupSpawnerExtension" }
...
function PickupSystem:init(context, system_name)
	...
	if not DEDICATED_SERVER then --[[ Nothing --]]
	end
...
function PickupSystem:set_seed(seed)
	fassert(seed and type(seed) == "number", "Bad seed input!")
	self._seed = seed
	self._starting_seed = seed
end

function PickupSystem:on_add_extension(world, unit, extension_name, extension_init_data, ...)
	if extension_name ~= "PickupSpawnerExtension" then
		...
		if extension_name == "PlayerTeleportingPickupExtension" then
			...
			self._teleporting_pickups [unit] = { line_of_sight_fails = 0, init_data = extension_init_data, next_line_of_sight_check = t + pickup_settings.teleport_time }
		elseif extension_name == "LifeTimePickupUnitExtension" then
			local t = Managers.time:time("game")
			local pickup_settings = AllPickups [pickup_name]
			self._life_time_pickups [unit] = { init_data = extension_init_data, pickup_settings = pickup_settings, life_time = t + pickup_settings.life_time }
...
function PickupSystem:on_remove_extension(unit, extension_name, ...)
	if extension_name ~= "PickupSpawnerExtension" then
		...
		elseif
		extension_name == "LimitedOwnedPickupUnitExtension" and self.is_server then
...
function PickupSystem:get_pickups(position, radius, result)
	return Broadphase.query(self._broadphase, position, radius, result)
end

function PickupSystem:get_pickups_by_type(pickup_name)
	local pickup_units_by_type = self._pickup_units_by_type

	return pickup_units_by_type [pickup_name]
end

function PickupSystem:pickup_gizmo_spawned(unit)
...
function PickupSystem:specific_pickup_gizmo_spawned(unit)
	if not self.is_server and not LEVEL_EDITOR_TEST then
		return
	end
...
function PickupSystem:activate_triggered_pickup_spawners(triggered_spawn_id)
...
function PickupSystem:create_checkpoint_data()
	return { seed = self._starting_seed, taken = table.clone(self._taken) }
end

function PickupSystem:remove_pickups_due_to_crossroads(removed_path_distances, total_main_path_length)
	local to_remove = { }
	local num_removed_dist_pairs = #removed_path_distances


	local remove_tables = { self.primary_pickup_spawners, self.secondary_pickup_spawners, self.guaranteed_pickup_spawners, self.triggered_pickup_spawners }
...
function PickupSystem:setup_taken_pickups(checkpoint_data)
...
function PickupSystem:disable_spawners(spawner_types)
...
function PickupSystem:populate_pickups(checkpoint_data)
	if checkpoint_data then
...
function PickupSystem:populate_specified_pickups(checkpoint_data)
	if checkpoint_data then
		local checkpoint_seed = checkpoint_data.seed
		self:set_seed(checkpoint_seed)
	end
...
function PickupSystem:_spawn_spread_pickups(spawners, pickup_settings, comparator, priority, ignore_sections)
...
function PickupSystem:_debug_add_spread_pickup_spawner(pickup_type, section_index, spawner_unit, priority)
...
function PickupSystem:_debug_add_spread_pickup(spawner_unit, pickup_type)
...
function PickupSystem:debug_draw_spread_pickups()
	...
	local section_colors = {
		Colors.get("orange"), Colors.get("pink"), Colors.get("yellow"),
		Colors.get("red"), Colors.get("light_green"), Colors.get("blue"),
		Colors.get("cyan"), Colors.get("magenta"), Colors.get("white") }


	local drawer = Managers.state.debug:drawer({ mode = "retained", name = "debug_spread_pickups" })
	drawer:reset()
...
function PickupSystem:disable_teleporting_pickups()
	for unit, _ in pairs(self._teleporting_pickups) do
		self._teleporting_pickups [unit] = nil
	end
end

function PickupSystem:spawn_guarenteed_pickups()
...
function PickupSystem:_spawn_guaranteed_pickup(spawner_unit, spawn_type)
...
function PickupSystem:_spawn_specified_pickups()
...
function PickupSystem:update(context, t)
	local dt = context.dt
	if self.is_server then
...
function PickupSystem:get_and_delete_limited_owned_pickup_with_index(owner_peer_id, index)
...
function PickupSystem:delete_limited_owned_pickup_unit(owner_peer_id, pickup_unit)
	local limited_owned_pickups = self._limited_owned_pickups [owner_peer_id]
	if not limited_owned_pickups then
		return
	end
...
function PickupSystem:event_delete_limited_owned_pickups(peer_id)
	if self.is_server then
...
function PickupSystem:delete_limited_owned_pickup_type(peer_id, type)
	if self.is_server then
		local limited_owned_pickups = self._limited_owned_pickups [peer_id]
		if not limited_owned_pickups then
			return
...
function PickupSystem:_update_life_time_pickups(dt, t)
	for unit, data in pairs(self._life_time_pickups) do
		if data.life_time < t and
		data.pickup_settings.on_life_over_func then
...
function PickupSystem:_update_teleporting_pickups(dt, t)
...
function PickupSystem:_check_teleporting_pickup_line_of_sight(unit)
	...
	for _, player in pairs(Managers.player:players()) do
		...
		if HEALTH_ALIVE [player_unit] then
			...
			if MAX_RAY_DIST < length then --[[ Nothing --]]
...
function PickupSystem:_teleport_pickup(unit)
	local new_pos = nil
...
function PickupSystem:hot_join_sync(sender)
	return end

function PickupSystem:spawn_pickup(pickup_name, position, rotation, with_physics, spawn_type, velocity, override_unit_template_name)
	local pickup_settings = AllPickups [pickup_name]
...
function PickupSystem:buff_spawn_pickup(pickup_name, position, raycast_down)
...
function PickupSystem:_spawn_pickup(pickup_settings, pickup_name, position, rotation, with_physics, spawn_type, owner_peer_id, spawn_limit, velocity, override_unit_template_name)
...
function PickupSystem:_update_limited_limited_owned_pickups(pickup_settings, pickup_name, position, rotation, with_physics, spawn_type, owner_peer_id)
	local limited_owned_pickups = self._limited_owned_pickups [owner_peer_id]
	if not limited_owned_pickups then
		return
	end
...
function PickupSystem:_can_spawn(spawner_unit, pickup_name)
	return Unit.get_data(spawner_unit, pickup_name) or Managers.mechanism:can_spawn_pickup(spawner_unit, pickup_name)
end



function PickupSystem:rpc_spawn_pickup_with_physics(channel_id, pickup_name_id, position, rotation, spawn_type_id)
...
function PickupSystem:rpc_spawn_pickup(channel_id, pickup_name_id, position, rotation, spawn_type_id)
	local pickup_name = NetworkLookup.pickup_names [pickup_name_id]
	fassert(AllPickups [pickup_name], "pickup name %s does not exist in Pickups table", pickup_name)
...
function PickupSystem:rpc_spawn_linked_pickup(channel_id, pickup_name_id, link_position, link_rotation, spawn_type_id, hit_unit_go_id, node_index, is_level_unit, spawn_limit)
...
function PickupSystem:_delete_pickup(unit)
	local unit_spawner = Managers.state.unit_spawner
...
function PickupSystem:rpc_delete_pickup(channel_id, unit_id)
	local unit = Managers.state.network:game_object_or_level_unit(unit_id)
	self:_delete_pickup(unit)
end

function PickupSystem:rpc_delete_limited_owned_pickup_unit(channel_id, owner_peer_id, pickup_unit_id)
...
function PickupSystem:rpc_delete_limited_owned_pickups(channel_id, owner_peer_id)
	self:event_delete_limited_owned_pickups(owner_peer_id)
end

function PickupSystem:rpc_delete_limited_owned_pickup_type(channel_id, owner_peer_id, pickup_name_id)
...
function PickupSystem:rpc_force_use_pickup(channel_id, pickup_name_id)
...
function PickupSystem:explosive_barrel(pickup_settings, position, rotation)
...
function PickupSystem:wizards_barrel(pickup_settings, position, rotation)
	local network_position = AiAnimUtils.position_network_scale(position, true)
	local network_rotation = AiAnimUtils.rotation_network_scale(rotation, true)
	local network_velocity = AiAnimUtils.velocity_network_scale(Vector3(0, 0, 0), true)
	local network_angular_velocity = network_velocity
...
function PickupSystem:training_dummy(pickup_settings, position, rotation)
	local network_position = AiAnimUtils.position_network_scale(position, true)
	local network_rotation = AiAnimUtils.rotation_network_scale(rotation, true)
	local network_velocity = AiAnimUtils.velocity_network_scale(Vector3(0, 0, 0), true)
	local network_angular_velocity = network_velocity
...
function PickupSystem:set_taken(spawn_index)
	if self.is_server then
