...
local STATES = { INITIAL = "INITIAL", COOLDOWN_FROM_TARGETTING = "COOLDOWN_FROM_TARGETTING", FINDING_TARGET = "FINDING_TARGET", DONE = "DONE", WAITING_TO_SPAWN_SKULLS = "WAITING_TO_SPAWN_SKULLS", SPAWNING_SKULL = "SPAWNING_SKULL" }
...
function ShadowHomingSkullsSpawnerExtension:init(extension_init_context, unit, extension_init_data)
...
function ShadowHomingSkullsSpawnerExtension:destroy()
	return end

function ShadowHomingSkullsSpawnerExtension:on_remove_extension(unit, extension_name)
	return end

function ShadowHomingSkullsSpawnerExtension:update(unit, input, dt, context, t)
	if self._done then
		return
	end
	...
	not self._destroyed and Unit.alive(unit) then
