...
function BackendManagerPlayFab:init(signin_name, mirror_name, server_queue_name)
...
function BackendManagerPlayFab:reset()
	self._errors = { }
...
function BackendManagerPlayFab:signin(authentication_token)
	...
	if not available or not backend_plugin_loaded or not use_backend then
		if allow_local then
			if use_backend and not available then
				local error_data = { reason = BACKEND_LUA_ERRORS.ERR_PLATFORM_SPECIFIC_INTERFACE_MISSING }
				self:_post_error(error_data)
				do return end
			end
		elseif not backend_plugin_loaded then
			local error_data = { reason = BACKEND_LUA_ERRORS.ERR_LOADING_PLUGIN }
			self:_post_error(error_data)
			do return end
		elseif not use_backend then
			local error_data = { reason = BACKEND_LUA_ERRORS.ERR_USE_LOCAL_BACKEND_NOT_ALLOWED }
			self:_post_error(error_data)
			do return end
		else
...
function BackendManagerPlayFab:on_shutdown(external_callback)
...
function BackendManagerPlayFab:_backend_plugin_loaded()
...
function BackendManagerPlayFab:_create_interfaces()
	local settings = GameSettingsDevelopment.backend_settings
...
function BackendManagerPlayFab:_destroy_backend()
	if self._backend_signin then
...
function BackendManagerPlayFab:item_script_type()
	return "backend"
end

function BackendManagerPlayFab:get_interface(interface_name, player_id)
...
function BackendManagerPlayFab:dirtify_interfaces()
...
function BackendManagerPlayFab:get_data_server_queue()
	return self._data_server_queue
end

function BackendManagerPlayFab:is_disconnected()
	return self._is_disconnected
end

function BackendManagerPlayFab:is_waiting_for_user_input()
	return not not self._error_dialog
end


function BackendManagerPlayFab:get_title_data(key)
...
function BackendManagerPlayFab:get_read_only_data(key)
	return self._backend_mirror and self._backend_mirror:get_read_only_data(key) or nil
end

function BackendManagerPlayFab:start_tutorial()
...
function BackendManagerPlayFab:stop_tutorial()
...
function BackendManagerPlayFab:is_tutorial_backend()
	return self._is_tutorial_backend
end

function BackendManagerPlayFab:is_benchmark_backend()
	return self._benchmark_backend
end

function BackendManagerPlayFab:start_benchmark()
...
function BackendManagerPlayFab:stop_benchmark()
...
function BackendManagerPlayFab:add_loadout_interface_override(override_name, interface_name_by_slot)
	self._loadout_interface_overrides [override_name] = interface_name_by_slot
end

function BackendManagerPlayFab:set_loadout_interface_override(override_name)
...
function BackendManagerPlayFab:get_loadout_interface_by_slot(slot_name)
...
function BackendManagerPlayFab:add_talents_interface_override(override_name, interface_name)
	self._talents_interface_overrides [override_name] = interface_name
end

function BackendManagerPlayFab:set_talents_interface_override(override_name)
...
function BackendManagerPlayFab:get_talents_interface()
...
function BackendManagerPlayFab:set_total_power_level_interface_for_game_mode(game_mode, interface_name)
	self._total_power_level_interface_overrides [game_mode] = interface_name
end

function BackendManagerPlayFab:get_total_power_level(profile_name, career_name, game_mode_key)
...
function BackendManagerPlayFab:_update_state()


	local settings = GameSettingsDevelopment.backend_settings
	local signin = self._backend_signin

	if ( not settings.allow_backend or self._local_save_loaded or DEDICATED_SERVER ) and
	self._need_signin then
		...
		elseif self._signin_timeout < os.time() then
			...
			local error_data = { reason = BACKEND_LUA_ERRORS.ERR_SIGNIN_TIMEOUT }
...
function BackendManagerPlayFab:_update_error_handling(dt)
...
function BackendManagerPlayFab:_update_interface(interface_name, dt)
	local interface = self._interfaces [interface_name]
	local backend_mirror = self._backend_mirror
...
function BackendManagerPlayFab:update(dt, t)
...
function BackendManagerPlayFab:playfab_api_error(result, error_code)
	table.dump(result, nil, 10)

	local error_data = { reason = BACKEND_PLAYFAB_ERRORS.ERR_PLAYFAB_ERROR, details = error_code }

	self:_post_error(error_data)
end

function BackendManagerPlayFab:request_timeout()
	local error_data = { reason = BACKEND_LUA_ERRORS.ERR_REQUEST_TIMEOUT }

	self:_post_error(error_data, "backend_err_request_timeout")
end

function BackendManagerPlayFab:commit_error()
	local reason = BACKEND_PLAYFAB_ERRORS.ERR_PLAYFAB_COMMIT_TIMEOUT
	local details = nil
	local error_data = { reason = reason, details = details }

	self:_post_error(error_data)
end

function BackendManagerPlayFab:playfab_eac_error()
...
function BackendManagerPlayFab:playfab_error(reason, details)
	local error_data = { reason = reason, details = details }

	self:_post_error(error_data)
end

function BackendManagerPlayFab:missing_required_dlc_error(details, optional_error_topic, optional_url_button)
	local reason = BACKEND_PLAYFAB_ERRORS.ERR_PLAYFAB_MISSING_REQUIRED_DLC
	local error_data = { reason = reason, details = details, optional_error_topic = optional_error_topic, optional_url_button = optional_url_button }

	self:_post_error(error_data, nil, true)
end

function BackendManagerPlayFab:signed_in()
	local signin = self._backend_signin
...
function BackendManagerPlayFab:authenticated()
	local signin = self._backend_signin
	local mirror = self._backend_mirror
...
function BackendManagerPlayFab:has_error()
	return self._in_error_state
end



function BackendManagerPlayFab:error_string()
...
function BackendManagerPlayFab:_post_error(error_data, crashify_override, ignore_crashify)
...
function BackendManagerPlayFab:_is_fatal(reason)
	local harmless = reason == BACKEND_PLAYFAB_ERRORS.ERR_PLAYFAB_ACHIEVEMENT_REWARD_CLAIMED or
	reason == BACKEND_PLAYFAB_ERRORS.ERR_PLAYFAB_QUEST_REFRESH_UNAVAILABLE or
	reason == BACKEND_PLAYFAB_ERRORS.ERR_PLAYFAB_NON_FATAL_STORE_ERROR
	return not harmless
end


function BackendManagerPlayFab:_reason_localize_key(reason, error_code)
	...
	if IS_CONSOLE then
		if not self:profiles_loaded() then
			if rawget(_G, "Backend") and reason == Backend.ERR_AUTH then
				if IS_XB1 then
					do return "backend_err_auth_xb1" end
				else
					do return "backend_err_auth_ps4" end
				end
			elseif reason == BACKEND_LUA_ERRORS.ERR_SIGNIN_TIMEOUT then
				do return "backend_err_signin_timeout" end
			elseif reason == BACKEND_LUA_ERRORS.ERR_REQUEST_TIMEOUT then
				do return "connection_timeout" end
			elseif reason == BACKEND_PLAYFAB_ERRORS.ERR_PLAYFAB_ERROR then
				...
				do return "backend_err_network" end
			else
				do return "backend_err_connecting" end
			end
	...
	elseif reason == BACKEND_PLAYFAB_ERRORS.ERR_PLAYFAB_ERROR then
		...
		do return ERROR_CODES [reason] end
	elseif reason == BACKEND_PLAYFAB_ERRORS.ERR_PLAYFAB_EAC_ERROR or
	reason == BACKEND_PLAYFAB_ERRORS.ERR_PLAYFAB_COMMIT_TIMEOUT then

		do return ERROR_CODES [reason] end
	elseif reason == BACKEND_PLAYFAB_ERRORS.ERR_PLAYFAB_ACHIEVEMENT_REWARD_CLAIMED or
	reason == BACKEND_PLAYFAB_ERRORS.ERR_PLAYFAB_QUEST_REFRESH_UNAVAILABLE or
	reason == BACKEND_PLAYFAB_ERRORS.ERR_PLAYFAB_NON_FATAL_STORE_ERROR then
		do return ERROR_CODES [reason] end
	else
...
function BackendManagerPlayFab:_format_error_message_console(reason, error_code)
	local button = { result = self._button_retry, text = Localize("button_ok") }
	return self:_reason_localize_key(reason, error_code), button
end

function BackendManagerPlayFab:_format_error_message_windows(reason, error_code, optional_url_button)
	...
	if not self:profiles_loaded() then
		button_1 = { result = self._button_quit, text = Localize("menu_quit") }
	...
	elseif reason == BACKEND_PLAYFAB_ERRORS.ERR_PLAYFAB_ERROR or
	reason == BACKEND_PLAYFAB_ERRORS.ERR_PLAYFAB_EAC_ERROR or
	reason == BACKEND_PLAYFAB_ERRORS.ERR_PLAYFAB_COMMIT_TIMEOUT then
	...
	reason == BACKEND_PLAYFAB_ERRORS.ERR_PLAYFAB_QUEST_REFRESH_UNAVAILABLE or
	reason == BACKEND_PLAYFAB_ERRORS.ERR_PLAYFAB_NON_FATAL_STORE_ERROR then
		button_1 = { result = self._button_ok, text = Localize("button_ok") }
	else
		button_1 = { result = self._button_disconnected, text = Localize("button_ok") }
	end
...
function BackendManagerPlayFab:_show_error_dialog(reason, details_message, optional_error_topic, optional_url_button)
...
function BackendManagerPlayFab:get_stats()
	if self._backend_mirror then
...
function BackendManagerPlayFab:set_stats(stats)
	if self._backend_mirror then
...
function BackendManagerPlayFab:get_user_data(key)
	if self._backend_mirror then
...
function BackendManagerPlayFab:set_user_data(key, value)
	if self._backend_mirror then
...
function BackendManagerPlayFab:available()
	local settings = GameSettingsDevelopment.backend_settings

	if IS_WINDOWS or IS_LINUX then
		do return rawget(_G, "Steam") ~= nil or DEDICATED_SERVER end
	elseif IS_XB1 then
		do return true end
	elseif IS_PS4 then
		return true
...
function BackendManagerPlayFab:commit(skip_queue, commit_complete_callback)
...
function BackendManagerPlayFab:has_loaded()
	return self._local_save_loaded or DEDICATED_SERVER
end

function BackendManagerPlayFab:_are_profiles_loaded()
	local signin = self._backend_signin
	local mirror = self._backend_mirror
...
function BackendManagerPlayFab:profiles_loaded()
	return self._profiles_loaded
end

function BackendManagerPlayFab:interfaces_ready()
	return self:_interfaces_ready()
end

function BackendManagerPlayFab:_interfaces_ready()
...
function BackendManagerPlayFab:refresh_log_level()
	print("[BackendManagerPlayFab] No backend to set log level on!")
end

function BackendManagerPlayFab:logout()
	error("[BackendManagerPlayFab] Not implemented yet")
end

function BackendManagerPlayFab:disconnect()
	error("[BackendManagerPlayFab] Not implemented yet")
end

function BackendManagerPlayFab:destroy()
...
function BackendManagerPlayFab:implementation()
	return self._backend_implementation
end



function BackendManagerPlayFab:_create_items_interface(settings)
...
function BackendManagerPlayFab:_create_quests_interface(settings)
	local implementation = self._backend_implementation
...
function BackendManagerPlayFab:_create_crafting_interface(settings)
	local implementation = self._backend_implementation
...
function BackendManagerPlayFab:_create_talents_interface(settings)
	local implementation = self._backend_implementation
...
function BackendManagerPlayFab:_create_loot_interface(settings)
	local implementation = self._backend_implementation
...
function BackendManagerPlayFab:_create_common_interface(settings)
	self._interfaces.common = BackendInterfaceCommon:new(self._backend_mirror)
end

function BackendManagerPlayFab:_create_hero_attributes_interface(settings)
	local implementation = self._backend_implementation
...
function BackendManagerPlayFab:_create_cdn_resources_interface(settings)
...
function BackendManagerPlayFab:_cb_backend_localizations_loaded(localizations)
...
function BackendManagerPlayFab:player_id()
	local signin = self._backend_signin
...
function BackendManagerPlayFab:load_mechanism_loadout(mechanism_key)
	self._backend_mirror:request_characters(mechanism_key)
end

function BackendManagerPlayFab:is_pending_request()
	local mirror = self._backend_mirror
...
local EMPTY_TABLE = { }
function BackendManagerPlayFab:get_level_variation_data()
...
function BackendManagerPlayFab:get_deus_weapon_preload_settings()
	if not self._backend_mirror then
		return EMPTY_TABLE
	end
...
function BackendManagerPlayFab:get_title_settings()
	if not self._backend_mirror then
		return EMPTY_TABLE
	end
...
function BackendManagerPlayFab:dlc_unlocked_at_signin(dlc_name)
...
function BackendManagerPlayFab:get_metadata()
	return self._metadata
end

function BackendManagerPlayFab:get_backend_mirror()
	return self._backend_mirror
end

function BackendManagerPlayFab:get_twitch_app_access_token()
