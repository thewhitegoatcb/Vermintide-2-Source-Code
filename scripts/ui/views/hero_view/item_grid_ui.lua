...
function ItemGridUI:init(category_settings, widget, hero_name, career_index, params)
...
function ItemGridUI:_append_widget_content(widget, params)
...
function ItemGridUI:change_category(category_name, keep_page_index)
...
function ItemGridUI:set_item_page(page_index)
...
function ItemGridUI:items()
	return self._items
end

function ItemGridUI:get_page_info()
	return self._selected_page_index, self._total_item_pages
end

function ItemGridUI:get_equipped_items(hero_name, career_index)
...
function ItemGridUI:apply_item_sorting_function(item_sort_func)
	self._item_sort_func = item_sort_func
end

function ItemGridUI:set_locked_items_icon(locked_item_icon)
	self._locked_item_icon = locked_item_icon
	self:update_items_status()
end

function ItemGridUI:disable_locked_items(disable)
	self._disable_locked_items = disable
	self:mark_locked_items(self._mark_locked_items)
end

function ItemGridUI:lock_item_by_id(backend_id, reserve)
	self._locked_items [backend_id] = reserve
end

function ItemGridUI:clear_locked_items()
	self._locked_items = { }
end

function ItemGridUI:hide_slots(hide)
	self._hide_slots = hide
	self:update_items_status()
end
function ItemGridUI:mark_locked_items(mark)
	self._mark_locked_items = mark
	self:update_items_status()
end

function ItemGridUI:disable_unwieldable_items(disable)
	self._disable_unwieldable_items = disable
end

function ItemGridUI:disable_equipped_items(disable)
	self._disable_equipped_items = disable
	self:mark_equipped_items(self._mark_equipped_items)
end

function ItemGridUI:mark_equipped_items(mark)
	self._mark_equipped_items = mark
	self:update_items_status()
end

function ItemGridUI:disable_item_drag()
	self._item_drag_disabled = true
	self:update_items_status()
end

function ItemGridUI:update_items_status()
...
function ItemGridUI:has_item(item)
	local items = self._items
	if items then
...
function ItemGridUI:set_item_selected(item)
...
function ItemGridUI:is_item_wieldable(item)
	local widget = self._widget
	local content = widget.content
...
function ItemGridUI:handle_favorite_marking(input_service)
...
function ItemGridUI:handle_gamepad_selection(input_service)
...
function ItemGridUI:get_item_in_slot(row_number, column_number)
	local widget = self._widget
	local content = widget.content
...
function ItemGridUI:set_backend_id_selected(backend_id)
	local item_interface = Managers.backend:get_interface("items")
...
function ItemGridUI:selected_item()
	return self._selected_item, self._selected_item_equipped
end

function ItemGridUI:add_item_to_slot_index(slot_index, item, optional_amount)
	local widget = self._widget
	local content = widget.content
	local style = widget.style
...
function ItemGridUI:_set_color_values(target_color, p1, p2, p3)
	target_color [2] = p1
	target_color [3] = p2
	target_color [4] = p3
end

function ItemGridUI:repopulate_current_inventory_page()
	local widget = self._widget
	local content = widget.content
...
function ItemGridUI:_populate_inventory_page(items, start_read_index)
	local widget = self._widget
	local content = widget.content
	local style = widget.style
...
function ItemGridUI:clear_item_grid()
	local widget = self._widget
	local content = widget.content
	local style = widget.style
...
function ItemGridUI:_on_category_index_change(index, keep_page_index)
...
function ItemGridUI:_apply_search_query(items, search_query)
...
function ItemGridUI:change_item_filter(item_filter, change_page, optional_search_query)
...
function ItemGridUI:_sort_items(items, item_sort_func)
	if item_sort_func and #items > 1 then
		table.sort(items, item_sort_func)
	end
end

function ItemGridUI:_get_items_by_filter(item_filter)
	local backend_items = Managers.backend:get_interface("items")
...
function ItemGridUI:_get_slot_by_ui_index(index)
	local slots = InventorySettings.slots
	for _, slot in pairs(slots) do
...
function ItemGridUI:_handle_page_arrow_pressed()
...
function ItemGridUI:is_item_pressed(allow_single_press)
	local widget = self._widget
	local content = widget.content
	...
	for i = 1, rows do
		for k = 1, columns do
			...
			if
			not locked and not unwieldable_locked and (slot_hotspot.on_double_click or slot_hotspot.on_right_click or allow_single_press and slot_hotspot.on_pressed) then
				local item = content ["item" .. name_sufix]
...
function ItemGridUI:is_item_hovered()
	local widget = self._widget
	local content = widget.content
...
function ItemGridUI:get_item_hovered()
	local widget = self._widget
	local content = widget.content
...
function ItemGridUI:get_item_hovered_slot()
	local widget = self._widget
	local content = widget.content
...
function ItemGridUI:get_item_content(row, column)
	local widget = self._widget
	local content = widget.content
...
function ItemGridUI:is_slot_hovered()
	local widget = self._widget
	local content = widget.content
...
function ItemGridUI:highlight_slots(enabled, optional_alpha)
	local widget = self._widget
	local content = widget.content
	local style = widget.style
...
function ItemGridUI:highlight_drop_slots(enabled)
	local widget = self._widget
	local content = widget.content
	local style = widget.style
...
function ItemGridUI:is_item_dragged()
	local widget = self._widget
	local content = widget.content
...
function ItemGridUI:is_dragging_item()
	local widget = self._widget
	local content = widget.content
...
function ItemGridUI:update(dt, t)
	local input_pressed = self:_handle_page_arrow_pressed()


end

function ItemGridUI:destroy()

	return end

function ItemGridUI:get_selected_item_grid_slot()
