...
function StoreWindowPathTitle:on_enter(params, offset)
...
function StoreWindowPathTitle:_create_ui_elements(params, offset)
...
function StoreWindowPathTitle:_sync_layout_path()
...
function StoreWindowPathTitle:_get_selected_product_display_name()
	...
	elseif product_type == "dlc" then
		...
		do return dlc_settings.name end
	else
		return "n/a"
	end
end

function StoreWindowPathTitle:_setup_breadcrumb_widgets()
...
function StoreWindowPathTitle:_get_breadcrumb_text_width(widget)
...
function StoreWindowPathTitle:_create_breadcrumb_widget()
	local definition = create_breadcrumbs_definition()
	local widget = UIWidget.init(definition)
	return widget
end

function StoreWindowPathTitle:on_exit(params)
	print("[HeroViewWindow] Exit Substate StoreWindowPathTitle")
	self._ui_animator = nil
end

function StoreWindowPathTitle:update(dt, t)
...
function StoreWindowPathTitle:_get_layout_settings_by_name(name)
...
function StoreWindowPathTitle:post_update(dt, t)
	local input_service = self._parent:window_input_service()
	self:_handle_input(input_service, dt, t)
end

function StoreWindowPathTitle:_update_animations(dt)
...
function StoreWindowPathTitle:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_enter
end

function StoreWindowPathTitle:_is_button_pressed(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function StoreWindowPathTitle:_handle_input(input_service, dt, t)
	local breadcrumb_widgets = self._breadcrumb_widgets
	if breadcrumb_widgets then
...
function StoreWindowPathTitle:_on_path_index_pressed_pressed(index)
	local parent = self._parent
	local path = parent:get_store_path()
	local pages = StoreLayoutConfig.pages
...
function StoreWindowPathTitle:_draw(dt)
	local ui_renderer = self._ui_renderer
...
function StoreWindowPathTitle:_play_sound(event)
	self._parent:play_sound(event)
end

function StoreWindowPathTitle:_animate_breadcrumb_widget(widget, dt)
	local content = widget.content
	local style = widget.style
	local hotspot = content.button_hotspot
