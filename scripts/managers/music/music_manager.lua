...
MusicManager.bus_transition_functions = {
	linear = function (transition, t) return math.lerp(transition.start_value, transition.target_value, t) end,
	sine = function (transition, t) return math.lerp(transition.start_value, transition.target_value, math.sin(t * math.pi * 0.5)) end,
	smoothstep = function (transition, t) return math.lerp(transition.start_value, transition.target_value, math.smoothstep(t, 0, 1)) end }


MusicManager.panning_rules = { PANNING_RULE_SPEAKERS = 0, PANNING_RULE_HEADPHONES = 1 }
...
function MusicManager:duck_sounds()
...
function MusicManager:unduck_sounds()
...
function MusicManager:_update_window_focus()
	if not DEDICATED_SERVER then
...
function MusicManager:stop_event_id(event_id)
...
function MusicManager:trigger_event_queue(event_queue_name, event_queue, delay)
...
function MusicManager:_handle_event_queues(dt, t)
...
function MusicManager:stop_event_queue(event_queue_name)
...
function MusicManager:on_enter_level(network_event_delegate, is_server)
	...
	if is_server then
		...
		local init_data = { go_type = go_type, combat_intensity = intensity_state_id, boss_state = boss_state_id, override = override_state_id }
		...
		self._game_object_id = Managers.state.network:create_game_object("music_states", init_data, function (game_object_id) self:server_game_session_disconnect_music_states(game_object_id) end)
	end
...
function MusicManager:on_exit_level()
...
function MusicManager:_register_events()
...
function MusicManager:_unregister_events()
	local event_manager = Managers.state.event
	if not event_manager then
		return
	end
...
function MusicManager:client_game_session_disconnect_music_states(game_object_id)

	return end

function MusicManager:server_game_session_disconnect_music_states(game_object_id)
	self:game_object_destroyed(game_object_id, self._owner_id, self._go_template)
end

function MusicManager:game_object_created(game_object_id, owner_id, go_template)
...
function MusicManager:game_object_destroyed(game_object_id, owner_id, go_template)
...
function MusicManager:_update_flags()
...
function MusicManager:set_flag(flag, value)
...
function MusicManager:_setup_level_music_players()
...
function MusicManager:_reset_level_music_players()
...
function MusicManager:_number_of_aggroed_enemies()
...
function MusicManager:_update_flag_in_combat(conflict_director)
...
function MusicManager:_update_combat_intensity(conflict_director)
	local pacing = conflict_director.pacing
	local intensity = pacing.total_intensity
...
function MusicManager:_update_boss_state(conflict_director)
...
function MusicManager:_get_combat_music_state(conflict_director)
...
function MusicManager:_update_boss_music_intensity(conflict_director)
	...
	if local_player then
		...
		if Unit.alive(player_unit) then
			...
			for _, unit in pairs(boss_units) do
				...
				if distance_sq < min_distance_sq then min_distance_sq = distance_sq or min_distance_sq
				end end
...
function MusicManager:set_wwise_state(group_name, state_name)
...
function MusicManager:check_last_man_standing_music_state()
...
local HORDE_MUSIC_STATES = { pre_ambush = true, pre_horde = true, ambush = true, horde = true }
...
function MusicManager:_update_game_state(dt, t, conflict_director)
	local parties = Managers.party:parties()
...
function MusicManager:_get_game_state_for_player(dt, t, conflict_director, party_id, old_state, player)
	...
	if round_about_to_end then
		...
		elseif game_mode_manager:game_won(player) then
			...
			do return level_settings and level_settings.music_won_state or "won" end
...
local ai_units = { }
function MusicManager:_horde_done_spawning(horde)
...
function MusicManager:_update_player_state(dt, t)
	local music_player = self._music_players.combat_music
...
function MusicManager:_update_career_state(dt, t)
	local music_player = self._music_players.combat_music
	local player = self:_get_player()
...
function MusicManager:_update_enemy_aggro_state(dt, t)
	local music_player = self._music_players.combat_music
...
function MusicManager:_update_game_mode(dt, t)
	local music_player = self._music_players.combat_music
	if music_player then
		local mechanism_name = Managers.mechanism:current_mechanism_name()
		music_player:set_group_state("game_mode", mechanism_name)

		if mechanism_name == "adventure" then --[[ Nothing --]]
...
function MusicManager:_update_side_state(dt, t)
	local music_player = self._music_players.combat_music
...
function MusicManager:_update_versus_game_state(music_player, dt, t)
	local music_player = self._music_players.combat_music
	if not music_player or not self._active_local_player_id then
		return
	end
	...
	if side_close_to_winning then
		...
		side_name == side_close_to_winning and "close_to_win" or
...
function MusicManager:register_active_player(player_id)
...
function MusicManager:unregister_active_player(player_id)
...
function MusicManager:set_music_group_state(music_player, group, state)
	local game_object_id = self._game_object_id
...
function MusicManager:is_playing(wwise_playing_id)
	return WwiseWorld.is_playing(self._wwise_world, wwise_playing_id)
end


function MusicManager:delay_trigger_horde_dialogue(t, delay, horde_name)
	if delay ~= nil then
		self._horde_delay = delay
		self._horde_type = horde_name
	end
	if self._horde_delay ~= nil and

	self._horde_delay < t then
...
function MusicManager:trigger_horde_dialogue(horde_name)
...
function MusicManager:_get_player()
...
function MusicManager:_get_party()
	if self._party then
		return self._party
	end
...
function MusicManager:_get_side()
	if self._side then
		return self._side
	end
...
function MusicManager:on_player_party_changed(player, is_local_player, old_party_id, new_party_id)
