...
function StartGameStateWeaveLeaderboard:on_enter(params)
	...
	local leaderboard_tab_data = { { value = "global",

			text = Localize("menu_weave_leaderboard_filter_option_global") }, { value = "friends",
	...
	local filter_data = { { value = "personal",
			text = Localize("menu_weave_leaderboard_filter_option_you") }, { value = "top",
			text = Localize("menu_weave_leaderboard_filter_option_top") } }


	local team_size_data = { { value = 1,
			text = Localize("menu_weave_leaderboard_filter_option_players_1") }, { value = 2,
			text = Localize("menu_weave_leaderboard_filter_option_players_2") }, { value = 3,
			text = Localize("menu_weave_leaderboard_filter_option_players_3") }, { value = 4,
	...
	for i = 1, self._current_season_id do
		...
		season_data [i] = { text = season_text, value = i }
	end
...
function StartGameStateWeaveLeaderboard:_setup_poll_queue(leaderboard_type_data, filter_data, season_stat_data)
	self._poll_queues = { }
...
function StartGameStateWeaveLeaderboard:_restart_poll_queue(t)
...
function StartGameStateWeaveLeaderboard:_add_poll_queue(filter_value, leaderboard_type, stat_name, season_id)
...
function StartGameStateWeaveLeaderboard:_handle_next_poll_request(t)
	...
	if priority_index and #poll_queues [priority_index] > 0 then
		...
		for i = 1, #poll_queue do
			...
			if self._stat_name == stat_name and self._filter_value == filter_value and self._leaderboard_type == leaderboard_type then
				...
				break else
				next_poll_queue_index =
				next_poll_queue_index or i
			end
		end
...
function StartGameStateWeaveLeaderboard:_update_leaderboard_presentation()
...
function StartGameStateWeaveLeaderboard:_list_including_local_player(list_entries)
...
function StartGameStateWeaveLeaderboard:_get_cashed_list_data(filter_value, leaderboard_type, stat_name, current_season_id)
...
function StartGameStateWeaveLeaderboard:_cb_cashe_list_data(filter_value, leaderboard_type, stat_name, current_season_id, list_entries, request_failed)
	...
	cashed_list_data [filter_value] [leaderboard_type] [stat_name] = { list_entries, Managers.time:time("ui") }
...
function StartGameStateWeaveLeaderboard:_poll_list(dt, t)
	local weave_interface = Managers.backend:get_interface("weaves")
...
function StartGameStateWeaveLeaderboard:_set_refresh_time(t)
	self._refreshed_at_time = t
	self:_update_refresh_time(t)
end

function StartGameStateWeaveLeaderboard:_update_refresh_time(t)
...
function StartGameStateWeaveLeaderboard:_initialize_stepper(stepper_index, title_text, content, start_index)
...
function StartGameStateWeaveLeaderboard:_set_stepper_read_index(stepper_index, read_index)
	local stepper_settings = self._stepper_settings
...
function StartGameStateWeaveLeaderboard:_on_stepper_pressed(widget)
	local content = widget.content
...
function StartGameStateWeaveLeaderboard:_create_ui_elements(params)
...
function StartGameStateWeaveLeaderboard:_setup_tab_widget(options)
	local num_tabs = #options

	local widgets = self._widgets
	local widgets_by_name = self._widgets_by_name
	local option_tabs_segments = UIWidget.init(UIWidgets.create_simple_centered_texture_amount("menu_frame_09_divider_vertical", { 5, 35 }, "option_tabs_segments", num_tabs - 1))
	local option_tabs_segments_top = UIWidget.init(UIWidgets.create_simple_centered_texture_amount("menu_frame_09_divider_top", { 17, 9 }, "option_tabs_segments_top", num_tabs - 1))
	local option_tabs_segments_bottom = UIWidget.init(UIWidgets.create_simple_centered_texture_amount("menu_frame_09_divider_bottom", { 17, 9 }, "option_tabs_segments_bottom", num_tabs - 1))
...
function StartGameStateWeaveLeaderboard:disable_player_world()
...
function StartGameStateWeaveLeaderboard:enable_player_world()
...
function StartGameStateWeaveLeaderboard:close_on_exit()
	return self._close_on_exit
end

function StartGameStateWeaveLeaderboard:transitioning()
...
function StartGameStateWeaveLeaderboard:_wanted_state()
	local new_state = self.parent:wanted_state()
	return new_state
end

function StartGameStateWeaveLeaderboard:wanted_menu_state()
	return self._wanted_menu_state
end

function StartGameStateWeaveLeaderboard:clear_wanted_menu_state()
	self._wanted_menu_state = nil
end

function StartGameStateWeaveLeaderboard:hotkey_allowed()
	return true
end

function StartGameStateWeaveLeaderboard:on_exit(params)
...
function StartGameStateWeaveLeaderboard:_update_transition_timer(dt)
...
function StartGameStateWeaveLeaderboard:input_service()
	return self.parent:input_service()
end

function StartGameStateWeaveLeaderboard:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function StartGameStateWeaveLeaderboard:post_update(dt, t)
...
function StartGameStateWeaveLeaderboard:_update_animations(dt)
...
function StartGameStateWeaveLeaderboard:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_enter
end

function StartGameStateWeaveLeaderboard:_is_inventory_tab_pressed()
...
function StartGameStateWeaveLeaderboard:_select_tab_by_index(index)
	local widget = self._widgets_by_name.option_tabs
	local widget_content = widget.content
	local amount = widget_content.amount
	for i = 1, amount do
...
function StartGameStateWeaveLeaderboard:_handle_input(dt, t)
	local widgets_by_name = self._widgets_by_name
	local input_service = self.parent:input_service()
...
function StartGameStateWeaveLeaderboard:close_menu(ignore_sound_on_close_menu)
	self.parent:close_menu(nil, ignore_sound_on_close_menu)
end

function StartGameStateWeaveLeaderboard:set_input_description(input_description)
...
function StartGameStateWeaveLeaderboard:change_generic_actions(input_description)
	if not self._menu_input_description then
		return
	end
...
function StartGameStateWeaveLeaderboard:draw(input_service, dt)
		...
		if list_entries then
			...
			if list_entries and list_widget and list_draw_index then
				...
				for i = start_index, end_index do
					...
					content.button_hotspot.is_hover then
...
function StartGameStateWeaveLeaderboard:_open_profile(profile_id)
...
function StartGameStateWeaveLeaderboard:_is_button_pressed(widget)
	local content = widget.content
...
function StartGameStateWeaveLeaderboard:play_sound(event)
	self.parent:play_sound(event)
end

function StartGameStateWeaveLeaderboard:_start_transition_animation(key, animation_name)
	local params = {
...
function StartGameStateWeaveLeaderboard:set_fullscreen_effect_enable_state(enabled)
...
function StartGameStateWeaveLeaderboard:_setup_list_widget()
	local masked = true
...
function StartGameStateWeaveLeaderboard:_create_list_entries(entries)
	local list_entries = { }
	...
	for i = 1, num_entries do
		...
		list_entries [i] = { alpha_fade_in_delay = 0.4,
...
function StartGameStateWeaveLeaderboard:_populate_list(list_entries, show_no_placement)
...
function StartGameStateWeaveLeaderboard:_calculate_list_height(amount)
...
function StartGameStateWeaveLeaderboard:_initialize_scrollbar()
...
function StartGameStateWeaveLeaderboard:_update_scroll_position()
	local scrollbar_logic = self._scrollbar_logic
...
function StartGameStateWeaveLeaderboard:_update_visible_list_entries()
	local scrollbar_logic = self._scrollbar_logic
...
function StartGameStateWeaveLeaderboard:_get_scrollbar_percentage_by_index(index)
	local scrollbar_logic = self._scrollbar_logic
	local enabled = scrollbar_logic:enabled()
...
function StartGameStateWeaveLeaderboard:_animate_element_by_time(target, target_index, from, to, time)
	local new_animation = UIAnimation.init(UIAnimation.function_by_time, target, target_index,
	from, to, time, math.ease_out_quad)
	return new_animation
end

function StartGameStateWeaveLeaderboard:_animate_element_by_catmullrom(target, target_index, target_value, p0, p1, p2, p3, time)
