...
function EndViewStateSummary:on_enter(params)
	...
	local _ = level_start [1] local start_experience = level_start [2] local start_experience_pool = level_start [3]
...
function EndViewStateSummary:_wanted_state()
	local new_state = self.parent:wanted_menu_state()
	return new_state
end

function EndViewStateSummary:set_input_manager(input_manager)
	self.input_manager = input_manager
end

function EndViewStateSummary:on_exit(params)
	print("[EndViewState] Exit Substate EndViewStateSummary")
	self.ui_animator = nil
end

function EndViewStateSummary:_update_transition_timer(dt)
...
function EndViewStateSummary:update(dt, t)
	if DO_RELOAD then
...
function EndViewStateSummary:post_update(dt, t)

	return end

function EndViewStateSummary:_update_animations(dt)
...
function EndViewStateSummary:_handle_input(dt, t)
	local widgets_by_name = self._widgets_by_name
end

function EndViewStateSummary:draw(input_service, dt)
...
function EndViewStateSummary:_start_transition_animation(key, animation_name)
	local params = {
...
function EndViewStateSummary:_start_animation(key, animation_name)
	local params = {
		wwise_world = self.wwise_world,
		render_settings = self.render_settings }
...
function EndViewStateSummary:_animate_element_by_time(target, target_index, from, to, time)
	local new_animation = UIAnimation.init(UIAnimation.function_by_time, target, target_index,
	from, to, time, math.ease_out_quad)
	return new_animation
end

function EndViewStateSummary:_animate_element_by_catmullrom(target, target_index, target_value, p0, p1, p2, p3, time)
	local new_animation = UIAnimation.init(UIAnimation.catmullrom, target, target_index, target_value,
	p0, p1, p2, p3, time)
	return new_animation
end

function EndViewStateSummary:_initialize_entries()
...
function EndViewStateSummary:_get_summary_entries(game_won, game_mode_key)
	...
	for index, mission_reward in ipairs(mission_rewards) do
		...
		local entry = { spacing = 8, start_counter_sound = true,
			name = name,
...
function EndViewStateSummary:_animate_summary_entries(dt)
	local summary_entries = self._summary_entries
	if not summary_entries or summary_entries.complete then
		if summary_entries and summary_entries.complete and
		self._is_max_level and not self._experience_presentation_completed then
	...
	if animations_completed then --[[ Nothing --]]
	end

end

function EndViewStateSummary:_get_essence_earned()
...
function EndViewStateSummary:_setup_essence_presentation()
...
function EndViewStateSummary:_get_total_experience_progress_data(start_experience, start_experience_pool)
...
function EndViewStateSummary:_animate_experience_bar(dt, displaying_reward_presentation)
	...
	if has_reached_level then
		...
		self.level_up_anim_id = self.ui_animator:start_animation("level_up", self._widgets_by_name, self._scenegraph_definition, { wwise_world = self.wwise_world })
	end
...
function EndViewStateSummary:_set_current_experience(current_experience)
...
function EndViewStateSummary:done()
	return self._experience_presentation_completed and self._summary_entries.complete
end

function EndViewStateSummary:_play_sound(event)
	self.parent:play_sound(event)
end

function EndViewStateSummary:_set_player_count_presence(context)
