...
function StartGameWindowAreaSelection:on_enter(params, offset)
...
function StartGameWindowAreaSelection:create_ui_elements(params, offset)
...
function StartGameWindowAreaSelection:_setup_area_widgets()
...
function StartGameWindowAreaSelection:_select_area_by_name(area_name)
...
function StartGameWindowAreaSelection:_set_area_presentation_info(area_name)
	local title_text = ""
...
function StartGameWindowAreaSelection:on_exit(params)
...
function StartGameWindowAreaSelection:update(dt, t)
...
function StartGameWindowAreaSelection:post_update(dt, t)
	self:draw_video(dt)
end

function StartGameWindowAreaSelection:_update_animations(dt)
...
function StartGameWindowAreaSelection:_is_button_pressed(widget)
	local content = widget.content
...
function StartGameWindowAreaSelection:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function StartGameWindowAreaSelection:_update_area_option()
...
function StartGameWindowAreaSelection:_handle_input(dt, t)
	local active_area_widgets = self._active_area_widgets
	if active_area_widgets then
		for i = 1, #active_area_widgets do
			local widget = active_area_widgets [i]
...
function StartGameWindowAreaSelection:_on_select_button_pressed()
...
function StartGameWindowAreaSelection:draw(dt)
...
function StartGameWindowAreaSelection:draw_video(dt)
	local ui_renderer = self.ui_renderer
	local ui_scenegraph = self.ui_scenegraph
	local input_service = self.parent:window_input_service()
...
function StartGameWindowAreaSelection:_play_sound(event)
	self.parent:play_sound(event)
end

function StartGameWindowAreaSelection:_setup_video_player(material_name, resource)
	self:_destroy_video_player()
...
function StartGameWindowAreaSelection:_destroy_video_player()
	local ui_renderer = self.ui_renderer
...
function StartGameWindowAreaSelection:_animate_area_widget(widget, dt)
	local content = widget.content
...
function StartGameWindowAreaSelection:_show_storepage(url)
