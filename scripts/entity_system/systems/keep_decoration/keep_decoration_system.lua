...
local extensions = { "KeepDecorationPaintingExtension", "KeepDecorationTrophyExtension" }




local RPCS = { "rpc_request_painting", "rpc_send_painting" }




function KeepDecorationSystem:init(entity_system_creation_context, system_name)
...
function KeepDecorationSystem:destroy()
...
function KeepDecorationSystem:on_add_extension(world, unit, extension_name, extension_init_data, ...)
...
function KeepDecorationSystem:update(context, t)
...
function KeepDecorationSystem:on_painting_set(painting, asking_extension)
...
function KeepDecorationSystem:on_decoration_set(decoration, asking_extension, type)
	local extensions = self._extensions
...
function KeepDecorationSystem:is_decoration_in_use(decoration)
	local extensions = self._extensions
	for i = 1, #extensions do
		local extension = extensions [i]
...
function KeepDecorationSystem:_add_client_painting(player_id, painting)
	local client_paintings = self._client_paintings
	client_paintings [player_id] = painting
end

function KeepDecorationSystem:_sync_client_paintings()
	local client_paintings = self._client_paintings
	local players = Managers.player:human_players()
...
function KeepDecorationSystem:_refresh_client_paintings()
	local client_paintings = self._client_paintings
	local paintings = { }
	local count = 1
...
function KeepDecorationSystem:rpc_send_painting(channel_id, painting)
	local peer_id = CHANNEL_TO_PEER_ID [channel_id]
	self:_add_client_painting(peer_id, painting)
	self:_refresh_client_paintings()
end

function KeepDecorationSystem:rpc_request_painting(channel_id)
...
function KeepDecorationSystem:hot_join_sync(peer_id)
	local level_key = Managers.state.game_mode:level_key()
	local level_setting = LevelSettings [level_key]
