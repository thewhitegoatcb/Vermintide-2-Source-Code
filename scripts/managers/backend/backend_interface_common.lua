...
function BackendInterfaceCommon:init(backend_mirror)
	self._backend_mirror = backend_mirror
end

function BackendInterfaceCommon:ready()
	return true
end

function BackendInterfaceCommon:can_wield(career_name, item_data)
...
local filter_operators = {
	["not"] = { 4, 1, function (op1) return not op1 end },
	["<"] = { 3, 2, function (op1, op2) return op1 < op2 end },
	[">"] = { 3, 2, function (op1, op2) return op2 < op1 end },
	["<="] = { 3, 2, function (op1, op2) return op1 <= op2 end },
	[">="] = { 3, 2, function (op1, op2) return op2 <= op1 end },
	["~="] = { 3, 2, function (op1, op2) return op1 ~= op2 end },
	["=="] = { 3, 2, function (op1, op2) return op1 == op2 end },
	["and"] = { 2, 2, function (op1, op2) return op1 and op2 end },
	["or"] = { 1, 2, function (op1, op2) return op1 or op2 end } }
...
function filter_macros.item_rarity(item, backend_id)
	local item_data = item.data
...
function filter_macros.slot_type(item, backend_id)
	local item_data = item.data
	return item_data.slot_type
end
function filter_macros.item_type(item, backend_id)
	local item_data = item.data
	return item_data.item_type
end
function filter_macros.chest_category(item, backend_id)
	local item_data = item.data
	return item_data.chest_category
end
function filter_macros.discounted_items(item, backend_id)
	local item_data = item.data
...
function filter_macros.trinket_as_hero(item, backend_id)
	local item_data = item.data
...
function filter_macros.is_weapon(item, backend_id)
	local item_data = item.data
...
function filter_macros.equipped_by_current_career(item, backend_id, params)
	...
	if not profile_index or profile_index == 0 then return false
	end
	local career_index = player:career_index()
	if not career_index or career_index == 0 then return false
	end
...
function filter_macros.is_equipped(item, backend_id)
	local item_data = item.data
	local backend_items = Managers.backend:get_interface("items")
	local career_names = backend_items:equipped_by(backend_id)
...
function filter_macros.is_equipment_slot(item, backend_id)
	local item_data = item.data
...
function filter_macros.current_hero(item, backend_id)
	local item_data = item.data
	local profile_synchronizer = Managers.state.network.profile_synchronizer
...
function filter_macros.can_wield_by_current_career(item, backend_id, params)
	local item_data = item.data
	local profile_synchronizer = Managers.state.network.profile_synchronizer
	local player = Managers.player:local_player()
...
function filter_macros.can_wield_by_current_hero(item, backend_id, params)
	local item_data = item.data
	local profile_synchronizer = Managers.state.network.profile_synchronizer
	local player = Managers.player:local_player()
	local profile_index = params and params.profile_index or player:profile_index()
...
function filter_macros.is_new(item, backend_id)
	return PlayerData.new_item_ids [backend_id]
end
function filter_macros.is_plentiful(item, backend_id)
	local backend_items = Managers.backend:get_interface("items")
	local rarity = backend_items:get_item_rarity(backend_id)

	return rarity == "plentiful"
end
function filter_macros.is_common(item, backend_id)
	local backend_items = Managers.backend:get_interface("items")
	local rarity = backend_items:get_item_rarity(backend_id)

	return rarity == "common"
end
function filter_macros.is_rare(item, backend_id)
	local backend_items = Managers.backend:get_interface("items")
	local rarity = backend_items:get_item_rarity(backend_id)

	return rarity == "rare"
end
function filter_macros.is_exotic(item, backend_id)
	local backend_items = Managers.backend:get_interface("items")
	local rarity = backend_items:get_item_rarity(backend_id)

	return rarity == "exotic"
end
function filter_macros.is_unique(item, backend_id)
	local backend_items = Managers.backend:get_interface("items")
	local rarity = backend_items:get_item_rarity(backend_id)

	return rarity == "unique"
end
function filter_macros.is_promo(item, backend_id)
	local backend_items = Managers.backend:get_interface("items")
	local rarity = backend_items:get_item_rarity(backend_id)

	return rarity == "promo"
end
function filter_macros.is_default(item, backend_id)
	local backend_items = Managers.backend:get_interface("items")
	local rarity = backend_items:get_item_rarity(backend_id)

	return rarity == "default"
end
function filter_macros.is_magic(item, backend_id)
	local backend_items = Managers.backend:get_interface("items")
	local rarity = backend_items:get_item_rarity(backend_id)
...
function filter_macros.player_owns_item_key(item, backend_id)
	local item_data = item.data
	local backend_items = Managers.backend:get_interface("items")
...
function filter_macros.can_salvage(item, backend_id)
	local item_data = item.data
	local slot_type = item_data.slot_type
...
function filter_macros.has_properties(item, backend_id)
	if item.properties then
		return true
	end
	return false
end
function filter_macros.has_traits(item, backend_id)
	if item.traits then
		return true
	end
	return false
end
function filter_macros.has_applied_skin(item, backend_id)
	local item_data = item.data
	local slot_type = item_data.slot_type
...
function filter_macros.can_apply_skin(item, backend_id)
	local item_data = item.data
	local slot_type = item_data.slot_type
...
function filter_macros.can_upgrade(item, backend_id)
	local item_data = item.data
	local slot_type = item_data.slot_type
	if slot_type == "ranged" or slot_type == "melee" or slot_type == "ring" or slot_type == "necklace" or slot_type == "trinket" then
		local backend_items = Managers.backend:get_interface("items")
...
function filter_macros.can_craft_with(item, backend_id)
	local item_data = item.data
	local slot_type = item_data.slot_type
	if slot_type == "ranged" or slot_type == "melee" or slot_type == "ring" or slot_type == "necklace" or slot_type == "trinket" then
		local backend_items = Managers.backend:get_interface("items")
...
function filter_macros.available_in_mechanism_versus(item, backend_id)
	local item_data = item.data
	local mechanisms = item_data.mechanisms

	local is_cosmetic = table.contains({ "hat", "weapon_skin", "frame", "skin" }, item_data.slot_type)
	return is_cosmetic or mechanisms and table.contains(mechanisms, "versus")
end

function filter_macros.available_in_mechanism_adventure(item, backend_id)
	local item_data = item.data
	local mechanisms = item_data.mechanisms
...
function filter_macros.available_in_current_mechanism(item, backend_id)
...
function filter_macros.owned(item, backend_id)
...
function filter_macros.is_fake_item(item, backend_id)
...
function BackendInterfaceCommon:filter_items(items, filter_infix, params)
...
function BackendInterfaceCommon:_infix_to_postfix_item_filter(filter_infix)
	local output = { }
	local stack = { }
...
function BackendInterfaceCommon:serialize_traits(traits)
...
function BackendInterfaceCommon:serialize_runes(runes)
...
function BackendInterfaceCommon:commit_load_time_data(load_time_data)
	...
	local request = { FunctionName = "reportTimer",
	...
	request_queue:enqueue(request, function () print("Commit load time data") end, false)
end