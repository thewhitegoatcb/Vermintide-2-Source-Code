...
function flow_query_script_data(params)
	...
	local returns = { value = output_value }
	return returns
end
...
function flow_script_data_compare_bool(params)
	...
	if type(script_data_value) == "boolean" then
		if script_data_value == ref_value then
			returns = { equal = true, unequal = false }
		else
			returns = { equal = false, unequal = true }
		end
		return returns
	end
end
...
function flow_script_data_compare_number(params)
	...
	if type(script_data_value) == "number" then
		...
		elseif script_data_value <= ref_value then
			returns = { less_or_equal = true }
		elseif script_data_value == ref_value then
			returns = { equal = true }
		elseif script_data_value ~= ref_value then
			returns = { unequal = true }
		elseif ref_value <= script_data_value then
			returns = { more_or_equal = true }
		elseif ref_value < script_data_value then
			returns = { more = true }
		end
		return returns
...
function flow_callback_state_false(params)
	local returns = { updated = true, state = false }
	return returns
end

function flow_callback_state_true(params)
	local returns = { updated = true, state = true }
	return returns
end
...
function flow_callback_store_float(params)
	...
	local returns = { updated = true, state = true, outvalue = stored_value }
	return returns
end

function flow_callback_store_boolean(params)
	local stored_bool = params.inbool
	local returns = { updated = true, state = true, outbool = stored_bool }
	return returns
end
...
function flow_callback_math_addition(params)
	...
	return { value = term_one + term_two }
end
...
function flow_callback_math_multiplication(params)
	...
	return { value = factor_one * factor_two }
end
...
function flow_callback_math_floor(params)
	return { value = math.floor(params.float) }
end

function flow_callback_math_ceil(params)
	return { value = math.ceil(params.float) }
end
...
function flow_callback_distance_between(params)
	...
	local returns = { distance = distance_between }
	return returns
end
...
function flow_callback_attach_player_item(params)
	...
	if ItemMasterList ~= nil then
		...
		if item ~= nil then
			if params.career_filter and not table.is_empty(item.can_wield) and
			not table.find(item.can_wield, params.career_filter) then
			...
			if item.slot_type == "melee" or item.slot_type == "ranged" or item.slot_type == "weapon_skin" then
				if Weapons ~= nil then
					...
					if Unit.has_animation_state_machine(parent_unit) and
					weapon_template.wield_anim ~= nil and not params.skip_wield_anim then
			...
			elseif item.slot_type == "skin" then
				if Cosmetics ~= nil then
					...
					if skin_template.third_person_attachment ~= nil then
						...
						if Unit.has_animation_state_machine(item_unit) and
						Unit.has_animation_event(item_unit, "enable") then
...
function flow_callback_material_scalar_set_chr_inventory(params)
	...
	local unit = params.unit
	local items = { }
	for _, v in ipairs({ "outfit", "stump", "helmet", "other" }) do
...
function flow_callback_material_dissolve_chr(params)
	...
	local unit = params.unit
	local items = { }
	for _, v in ipairs({ "outfit", "stump", "helmet" }) do
		items = Unit.get_data(unit, v .. "_items") or { }
		for i = 1, #items do
			params.unit = items [i]
			flow_callback_material_dissolve(params)
...
function flow_callback_get_local_transform(params)
	...
	return { position = Unit.local_position(unit, nodeindex), rotation = Unit.local_rotation(unit, nodeindex), scale = Unit.local_scale(unit, nodeindex) }
end
...
function flow_callback_get_world_transform(params)
	...
	return { position = Unit.world_position(unit, nodeindex), rotation = Unit.world_rotation(unit, nodeindex) }
end
