...
function MainPathUtils.closest_pos_at_main_path(not_used, p, search_main_path_index)
	...
	if search_main_path_index then
		...
		start_node_index =
		search_main_path_index == 1 and 1 or

		breaks_order [search_main_path_index - 1] + 1
...
function MainPathUtils.closest_pos_at_main_path_lua(main_paths, p, search_main_path_index)
	...
	p =
	p or main_paths [1].nodes [1]:unbox()
...
function MainPathUtils.collapse_main_paths(main_paths)
	local unified_path = { } local unified_travel_dists = { } local breaks = { } local breaks_order = { } local segments = { }
	local k = 1
...
function MainPathUtils.point_on_mainpath(main_paths, wanted_distance)
	return EngineOptimized.point_on_mainpath(wanted_distance)
end

function MainPathUtils.point_on_mainpath_lua(main_paths, wanted_distance)
...
function MainPathUtils.zone_segment_on_mainpath(main_paths, p)
...
local index_list = { 0, 1, -1, 5, -5, 20, -20 }
local index_list_size = #index_list
function MainPathUtils.closest_pos_at_collapsed_main_path(collapsed_path, collapsed_dists, breaks_lookup, p, last_index)
...
function MainPathUtils.resolve_node_in_door(nav_world, node_position, door_unit)
...
function MainPathUtils.node_list_from_main_paths(nav_world, main_paths, max_node_distance, obstacles)
	local forward_list = { }
	local reversed_list = { }
	...
	if obstacles then
		...
		for i = 1, #obstacles do
			...
			local path_position, _, _, _, _, best_sub_index = MainPathUtils.closest_pos_at_main_path_lua({ { path_length = 1, nodes = forward_list } }, obstacle_position)
...
function MainPathUtils.closest_node_in_node_list(node_list, p)
	local best_dist = math.huge
...
function MainPathUtils.ray_along_node_list(nav_world, node_list, start_node_index, node_list_direction, wanted_distance)
...
function MainPathUtils.find_equidistant_points_in_node_list(node_list, start_node_index, node_list_direction, point_distance, num_wanted_points, points)
...
function MainPathUtils.get_main_path_point_between_players(main_paths, main_path_info, main_path_player_info)
