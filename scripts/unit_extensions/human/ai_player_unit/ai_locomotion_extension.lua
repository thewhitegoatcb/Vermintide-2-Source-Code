...
function AILocomotionExtension:init(extension_init_context, unit, extension_init_data)
...
function AILocomotionExtension:destroy()
	local system_data = self._system_data
	local unit = self._unit
	system_data.destroy_units [unit] = self
end

function AILocomotionExtension:ready(go_id, blackboard)
	return end


function AILocomotionExtension:hot_join_sync(sender)
	local unit = self._unit
...
function AILocomotionExtension:set_mover_displacement(displacement, duration)
...
function AILocomotionExtension:teleport_to(position, rotation)
	local unit = self._unit
...
function AILocomotionExtension:set_animation_driven(is_animation_driven, is_affected_by_gravity, script_driven_rotation, is_on_transport)
...
function AILocomotionExtension:set_animation_translation_scale(animation_translation_scale)
	self._animation_translation_scale_box:store(animation_translation_scale)
end


function AILocomotionExtension:set_animation_rotation_scale(animation_rotation_scale)
	self._animation_rotation_scale = animation_rotation_scale
end


function AILocomotionExtension:set_wanted_velocity_flat(wanted_velocity)
	wanted_velocity.z = self._velocity.z
	self._wanted_velocity = wanted_velocity
end


function AILocomotionExtension:set_wanted_velocity(wanted_velocity)
	self._wanted_velocity = wanted_velocity
	self._velocity:store(wanted_velocity)
end


function AILocomotionExtension:set_wanted_rotation(rotation)
	self._wanted_rotation = rotation
end


function AILocomotionExtension:use_lerp_rotation(active)

	self._lerp_rotation = active
end


function AILocomotionExtension:set_rotation_speed(rotation_speed)
...
function AILocomotionExtension:set_rotation_speed_modifier(rotation_speed_modifier, rotation_speed_modifier_lerp_time, start_time)
...
function AILocomotionExtension:set_affected_by_gravity(affected_by_gravity)
...
function AILocomotionExtension:set_gravity(gravity)
	self._gravity = gravity or LOCOMOTION_GRAVITY
end



function AILocomotionExtension:set_mover_disable_reason(reason, state)
	MoverHelper.set_disable_reason(self._unit, self._mover_state, reason, state)
end


function AILocomotionExtension:set_check_falling(state)
	self._check_falling = state
end


function AILocomotionExtension:set_collision_disabled(reason, state)
...
function AILocomotionExtension:set_movement_type(movement_type, override_mover_move_distance)
...
function AILocomotionExtension:set_disabled()
...
function AILocomotionExtension:current_velocity()
	return self._velocity:unbox()
end


function AILocomotionExtension:is_falling()
	return self._is_falling
end


function AILocomotionExtension:get_rotation_speed()
	return self._rotation_speed
end


function AILocomotionExtension:get_rotation_speed_modifier()
	return self._rotation_speed_modifier
end

function AILocomotionExtension:get_animation_rotation_scale()
	return self._animation_rotation_scale
end

function AILocomotionExtension:get_animation_translation_scale()
