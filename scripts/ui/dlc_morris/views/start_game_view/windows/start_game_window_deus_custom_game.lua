...
function StartGameWindowDeusCustomGame:on_enter(params, offset)
...
function StartGameWindowDeusCustomGame:_start_transition_animation(animation_name)
	local params = {
...
function StartGameWindowDeusCustomGame:_create_ui_elements(params, offset)
...
function StartGameWindowDeusCustomGame:on_exit(params)
...
function StartGameWindowDeusCustomGame:set_focus(focused)
	self._is_focused = focused
end

function StartGameWindowDeusCustomGame:post_update(dt, t)
	return end

function StartGameWindowDeusCustomGame:_can_play()
...
function StartGameWindowDeusCustomGame:_update_can_play()
	...
	if can_play then
		if self._is_offline then input_desc = "default_deus_custom_game_offline_play" else input_desc = "default_deus_custom_game_play" end
	elseif self._dlc_locked then
		if self._is_offline then input_desc = "default_deus_custom_game_offline_buy" else input_desc = "default_deus_custom_game_buy"
		end
	end
...
function StartGameWindowDeusCustomGame:_gather_unlocked_journeys()
...
function StartGameWindowDeusCustomGame:_setup_journey_widgets()
...
function StartGameWindowDeusCustomGame:_set_info_window(difficulty_key)
...
function StartGameWindowDeusCustomGame:_update_difficulty_option(difficulty_key)
...
function StartGameWindowDeusCustomGame:_option_selected(widget_name, button_name, t)
	local parent = self._parent
...
function StartGameWindowDeusCustomGame:_update_modifiers(current_time)
...
function StartGameWindowDeusCustomGame:_update_modifier_timer(current_time)
	local journey_cycle = self._journey_cycle
...
function StartGameWindowDeusCustomGame:_refresh_journey_cycle()
	self._journey_cycle = self._backend_deus:get_journey_cycle()
	self:_update_journey_god_icons()
end
function StartGameWindowDeusCustomGame:_update_additional_curse_frame(journey_name)
...
function StartGameWindowDeusCustomGame:_update_journey_god_icons()
	local journey_cycle = self._journey_cycle
	for _, widget in ipairs(self._expedition_widgets) do
		local content = widget.content
...
function StartGameWindowDeusCustomGame:update(dt, t)
...
function StartGameWindowDeusCustomGame:_verify_selection_index(input_index, input_change)
...
function StartGameWindowDeusCustomGame:_gamepad_selector_input_func(input_index, input_change)
...
function StartGameWindowDeusCustomGame:_update_animations(dt, t)
...
function StartGameWindowDeusCustomGame:_draw(dt)
...
function StartGameWindowDeusCustomGame:_animate_expedition_widget(widget, dt)
	local content = widget.content
...
function StartGameWindowDeusCustomGame:_handle_input(dt, t)
	local parent = self._parent
...
function StartGameWindowDeusCustomGame:handle_expedition_input(input_service, dt, t)

	return end

function StartGameWindowDeusCustomGame:handle_difficulty_input(input_service, dt, t)

	return end

function StartGameWindowDeusCustomGame:_play_sound(event)
	self._parent:play_sound(event)
end

function StartGameWindowDeusCustomGame:_update_expedition_option()
	local parent = self._parent
	local selected_level_id = parent:get_selected_level_id()
...
function StartGameWindowDeusCustomGame:_handle_gamepad_activity()
...
function StartGameWindowDeusCustomGame:_update_gamemode_info_text(input_service)
...
function StartGameWindowDeusCustomGame:_update_difficulty_lock()
...
function StartGameWindowDeusCustomGame:_calculate_difficulty_info_widget_size(diff_widget)
	local spacing = 20
...
function StartGameWindowDeusCustomGame:_resize_difficulty_info(new_size, new_offset)
	local difficulty_info_widget = self._widgets_by_name.difficulty_info
...
function StartGameWindowDeusCustomGame:_set_expedition_text_highlight_offset(ui_renderer)
