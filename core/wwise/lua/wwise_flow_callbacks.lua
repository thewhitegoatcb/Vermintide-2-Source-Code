...
function M.wwise_trigger_event(t)
	...
	return { playing_id = r1, source_id = r2, Playing_Id = r1, Source_Id = r2 }
end
...
function M.wwise_make_auto_source(t)
	...
	return { source_id = id, Source_Id = id }
end
...
function M.wwise_set_state(t)
	...
	if not group or not state then return
	end
...
function M.wwise_has_source(t)
	...
	if WwiseWorld.has_source(wwise_world, id) then
		do return { yes = true, Yes = true } end
	else
		return { No = true, no = true }
	end
end
...
function M.wwise_add_soundscape_source(t)
	...
	if unit then
		if name == "" then
			name = Unit.get_data(unit, "Wwise", "event_name") or ""
			if name == "" then
				return { ss_source_id = result_id, SS_Source_Id = result_id }
			end
		end
		shape =
		shape or Unit.get_data(unit, "Wwise", "shape") or "point"
		...
		positioning =
		positioning or string.lower(Unit.get_data(unit, "Wwise", "positioning")) or "closest"
