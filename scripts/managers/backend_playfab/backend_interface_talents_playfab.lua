...
function BackendInterfaceTalentsPlayfab:init(backend_mirror)
...
function BackendInterfaceTalentsPlayfab:_refresh()
...
function BackendInterfaceTalentsPlayfab:_validate_talents(career_name, career_talents)
...
function BackendInterfaceTalentsPlayfab:ready()
	return true
end

function BackendInterfaceTalentsPlayfab:update(dt)
	return end

function BackendInterfaceTalentsPlayfab:make_dirty()
	self._dirty = true
end

function BackendInterfaceTalentsPlayfab:get_talent_ids(career_name)
...
function BackendInterfaceTalentsPlayfab:get_talent_tree(career_name)
	local career_settings = CareerSettings [career_name]
	local profile_name = career_settings.profile_name
	local talent_tree_index = career_settings.talent_tree_index
	local talent_tree = talent_tree_index and TalentTrees [profile_name] [talent_tree_index]
...
function BackendInterfaceTalentsPlayfab:set_talents(career_name, talents)
	local talent_string = ""
...
function BackendInterfaceTalentsPlayfab:get_talents(career_name)
	if self._dirty then
		self:_refresh()
	end
