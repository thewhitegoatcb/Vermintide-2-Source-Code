...
function HexGrid:init(center, xy_extents, z_extents, x_cell_size, z_cell_size)
	...
	self._directions = { { 1, 0, angle = 0 },
		...
		{ 1, -1,
			angle = two_pi - pi_div_3 }, { 0, -1,
			angle = two_pi - pi_div_3 * 2 }, { -1, 0,
			angle = two_pi - pi_div_3 * 3 }, { -1, 1,
			angle = two_pi - pi_div_3 * 4 }, { 0, 1,
...
function HexGrid:find_index(position)
...
function HexGrid:real_index(i, j, k)
...
function HexGrid:ijk(real_index)
	local xy_extents = self._xy_extents
	local row_size = xy_extents * 2 + 1
...
function HexGrid:find_position(i, j, k)
	local root = self._root_position:unbox()
...
function HexGrid:sample_grid(samples, z, multiplier)
	local drawer = QuickDrawerStay
	drawer:reset()

	local xy_extents = self._xy_extents

	local min_i = 1 local min_j = 1 local max_i = xy_extents * 2 + 1 local max_j = xy_extents * 2 + 1
	local root = self._root_position:unbox()
