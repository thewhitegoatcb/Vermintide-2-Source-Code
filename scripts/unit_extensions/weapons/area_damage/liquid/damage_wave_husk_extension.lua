...
function DamageWaveHuskExtension:init(extension_init_context, unit, extension_init_data)
...
function DamageWaveHuskExtension:destroy()
...
function DamageWaveHuskExtension:update(unit, input, dt, context, t)
...
function DamageWaveHuskExtension:add_damage_wave_fx(position, rotation, fx_idx, name_idx)
	...
	if fx_idx == 0 or config.spawn_type == "effect" then
		...
		local fx_list = self.fx_list
		fx_list [#fx_list + 1] = { id = unit_or_id, position = Vector3Box(position), rotation = QuaternionBox(rotation), index = fx_idx }
...
function DamageWaveHuskExtension:set_running_wave(unit)
	local world = self.world
...
function DamageWaveHuskExtension:hide_wave(unit)
	local world = self.world
...
function DamageWaveHuskExtension:set_wave_arrived(unit)
	self:hide_wave(unit)
...
function DamageWaveHuskExtension:on_wavefront_impact(unit)
	local world = self.world
...
local wave_length = 1
function DamageWaveHuskExtension:debug_render_wave(t, dt, pos, travel_dir, height)
	local k = 0
