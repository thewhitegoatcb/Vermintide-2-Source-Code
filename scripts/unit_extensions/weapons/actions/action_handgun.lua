...
function ActionHandgun:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionHandgun:client_owner_start_action(new_action, t, chain_action_data, power_level)
...
function ActionHandgun:client_owner_post_update(dt, t, world, can_damage)
	local weapon_unit = self.weapon_unit
	local owner_unit = self.owner_unit
	...
	if self.state == "shooting" then
		...
		direction =
		direction or Quaternion.forward(rotation)
		...
		if current_action.projectile_info then
			...
			if current_action.ray_against_large_hitbox then
				result = PhysicsWorld.immediate_raycast_actors(physics_world,
				position,
				direction,
...
function ActionHandgun:finish(reason)
	local ammo_extension = self.ammo_extension
	local current_action = self.current_action
	local owner_unit = self.owner_unit
