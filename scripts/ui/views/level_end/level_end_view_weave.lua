...
function LevelEndViewWeave:init(context)
	self._team_heroes = { }
...
function LevelEndViewWeave:start()
...
function LevelEndViewWeave:destroy()
	LevelEndViewWeave.super.destroy(self)
	self:_destroy_team_previewer()
end

function LevelEndViewWeave:setup_pages(game_won, rewards)
...
function LevelEndViewWeave:_setup_pages_untrusted()
	return { EndViewStateWeave = 1 }


end

function LevelEndViewWeave:_setup_pages_victory(rewards)

	return { EndViewStateSummary = 2, EndViewStateWeave = 1 }
...
function LevelEndViewWeave:_setup_pages_defeat(rewards)
	return { EndViewStateSummary = 1 }


end

function LevelEndViewWeave:create_ui_elements()
...
function LevelEndViewWeave:update(dt, t)
...
function LevelEndViewWeave:set_input_description(input_desc)
...
function LevelEndViewWeave:active_input_service()
	return self.input_blocked and FAKE_INPUT_SERVICE or self:input_service()
end

function LevelEndViewWeave:_retry_level()
...
function LevelEndViewWeave:do_retry()
...
function LevelEndViewWeave:_setup_weave_data()
	local weave_manager = Managers.weave
...
local hero_locations = { {
			...
			unit_x, unit_y, unit_z } },
	{
		...
		{
			unit_x + unit_x_seperation * 0.5, unit_y + unit_y_seperation * 0.5, unit_z }, {
			unit_x + unit_x_seperation * -0.5, unit_y + unit_y_seperation * -0.5, unit_z } },
	{

		{
			unit_x + unit_x_seperation * 1, unit_y + unit_y_seperation * 1, unit_z }, {
			unit_x + unit_x_seperation * 0, unit_y + unit_y_seperation * 0, unit_z }, {
			unit_x + unit_x_seperation * -1, unit_y + unit_y_seperation * -1, unit_z } },
	{

		{
			unit_x + unit_x_seperation * 1.5, unit_y + unit_y_seperation * 1.5, unit_z }, {
			unit_x + unit_x_seperation * 0.5, unit_y + unit_y_seperation * 0.5, unit_z }, {
			unit_x + unit_x_seperation * -0.5, unit_y + unit_y_seperation * -0.5, unit_z }, {
			unit_x + unit_x_seperation * -1.5, unit_y + unit_y_seperation * -1.5, unit_z } } }
...
function LevelEndViewWeave:_destroy_team_previewer()
	if self._team_previewer then
...
function LevelEndViewWeave:_update_team_previewer(dt, t)
...
function LevelEndViewWeave:_setup_team_previewer(num_players)
	if self._team_previewer then
		return
	end
...
function LevelEndViewWeave:_setup_team_heroes(players_session_scores, num_players)
...
function LevelEndViewWeave:get_hero_from_score(player_data)
...
function LevelEndViewWeave:setup_camera()
...
function LevelEndViewWeave:start_camera_look_up(transition_delay, transition_duration, degrees)
...
function LevelEndViewWeave:_update_camera_look_up(dt, t)
...
function LevelEndViewWeave:spawn_level(context, world)
	local object_sets = { }
...
function LevelEndViewWeave:exit_to_game()
	self:play_sound(self._stop_music_event)
	self._exit_timer = 0.5
	self._started_exit = true
end

function LevelEndViewWeave:update_force_shutdown(dt)
...
function LevelEndViewWeave:get_all_signaled_done()
