...
FAKE_INPUT_SERVICE = { get = NOP, has = NOP }




ALL_INPUT_METHODS = { "keyboard", "gamepad", "mouse" }
...
local VALUE_LIST = { }
function UIUtils.format_localized_description(fmt_str, fmt_def)
	local fmt_localized = Localize(fmt_str)



	if not fmt_def or table.is_empty(fmt_def) then return fmt_localized
	end
...
function UIUtils.get_talent_description(talent_data)
...
function UIUtils.get_ability_description(ability_data)
...
function UIUtils.get_perk_description(perk_data)
	return UIUtils.format_localized_description(perk_data.description, perk_data.description_values)
end

function UIUtils.get_weave_property_description(property_name, property_data, mastery_costs, optional_amount)
...
function UIUtils.get_weave_property_value_text(property_name, property_data, mastery_costs, amount)
	local description_values = property_data.description_values
	local num_costs = #mastery_costs
	local text = "n/a"
...
function UIUtils.get_property_description(property_name, lerp_value, optional_property_data)
...
function UIUtils.get_trait_description(trait_name, optional_trait_data)
...
function UIUtils.get_ui_information_from_item(item)
...
function UIUtils.presentable_hero_power_level(power_level)
	return math.max(0, math.floor(power_level - PowerLevelFromLevelSettings.starting_power_level))
end

function UIUtils.presentable_hero_power_level_weaves(power_level)
	return math.max(0, math.floor(power_level - PowerLevelFromMagicLevel.starting_power_level))
end

function UIUtils.get_item_tooltip_value(unit, item, stat_descriptor)
...
function UIUtils.get_hero_statistics_by_template(template)
	local layout = { }
...
function UIUtils.get_text_height(ui_renderer, size, text_style, text)
...
function UIUtils.get_text_width(ui_renderer, text_style, text)
...
function UIUtils.is_button_pressed(widget, hotspot_name, keyboard_input)
	if widget then
...
function UIUtils.is_button_held(widget, hotspot_name)
	if widget then
		local content = widget.content
		local hotspot = content [hotspot_name] or content.button_hotspot or content.hotspot
...
function UIUtils.is_button_hover_enter(widget, hotspot_name)
	if widget then
		local content = widget.content
		local hotspot = content [hotspot_name] or content.button_hotspot or content.hotspot
...
function UIUtils.is_button_hover(widget, hotspot_name)
	if widget then
		local content = widget.content
		local hotspot = content [hotspot_name] or content.button_hotspot or content.hotspot
...
function UIUtils.is_button_selected(widget, hotspot_name)
	if widget then
		local content = widget.content
		local hotspot = content [hotspot_name] or content.button_hotspot or content.hotspot
...
function UIUtils.is_left_button_released(widget, hotspot_name)
	if widget then
		local content = widget.content
		local hotspot = content [hotspot_name] or content.button_hotspot or content.hotspot
...
function UIUtils.animate_value(x, dx, dir)
	if dir then
...
function UIUtils.comma_value(amount, comma)
	local formatted = amount local k = nil
...
function UIUtils.get_portrait_image_by_profile_index(profile_index, career_index)
...
function UIUtils.create_widgets(widget_definitions, widgets, widgets_by_name)
	widgets = widgets or { }
...
function UIUtils.destroy_widgets(ui_renderer, widget_list)
...
function UIUtils.mark_dirty(widget_list)
	for _, widget in pairs(widget_list) do
...
function UIUtils.align_box_inplace(ui_style, position, size, child_size)
...
function UIUtils.format_time(t)
	local sec = t % 60
	local min = ( t - sec ) / 60
	return string.format("%02d:%02d", min, sec)
end

function UIUtils.format_time_long(t)
...
function UIUtils.format_duration(t, done_string)
	if t > 172800 then
...
function UIUtils.get_color_for_consumable_item(item_key)
...
function UIUtils.sort_items_power_level_ascending(item_1, item_2)
...
function UIUtils.sort_items_power_level_descending(item_1, item_2)
	local item_1_power_level = item_1.power_level or math.huge
	local item_2_power_level = item_2.power_level or math.huge
...
function UIUtils.sort_items_rarity_ascending(item_1, item_2)
...
function UIUtils.sort_items_rarity_descending(item_1, item_2)
	local item_data_1 = item_1.data
	local item_data_2 = item_2.data
