...
function LevelTransitionHandler:init()
	...
	level_key, environment_variation_id, level_seed, mechanism, game_mode, conflict_director, locked_director_functions, difficulty, difficulty_tweak, extra_packages = LevelTransitionHandler.apply_defaults_to_level_data(
	level_key,
	level_seed,
	...
	local default_level_data = { level_transition_type = "load_next_level",
		level_key = level_key,
		mechanism = mechanism,
		game_mode = game_mode,
...
function LevelTransitionHandler:register_network_state(network_state)
	...
	if network_state:is_server() then
		network_state:set_level_data(
		offline_level_data.level_key,
		offline_level_data.environment_variation_id,
		offline_level_data.level_seed,
		offline_level_data.mechanism,
		offline_level_data.game_mode,
		offline_level_data.conflict_director,
		offline_level_data.locked_director_functions,
		offline_level_data.difficulty,
		offline_level_data.difficulty_tweak,
		offline_level_data.level_session_id,
		offline_level_data.level_transition_type,


		offline_level_data.extra_packages)
	end
...
function LevelTransitionHandler:deregister_network_state()
...
function LevelTransitionHandler:register_rpcs(network_event_delegate)
...
function LevelTransitionHandler:unregister_rpcs()
	self.enemy_package_loader:unregister_rpcs()
	self.transient_package_loader:unregister_rpcs()
end


function LevelTransitionHandler:reload_level(optional_checkpoint_data, optional_level_seed)
	...
	self:get_current_level_key(),
	self:get_current_environment_variation_id(),
	optional_level_seed or self:get_current_level_seed(),
	self:get_current_mechanism(),
	self:get_current_game_mode(),
	self:get_current_conflict_director(),
	self:get_current_locked_director_functions(),
	self:get_current_difficulty(),
	self:get_current_difficulty_tweak(),
	self:get_current_extra_packages())

end


function LevelTransitionHandler:get_checkpoint_data()
...
function LevelTransitionHandler:get_current_environment_variation_name()
...
function LevelTransitionHandler:update()
...
function LevelTransitionHandler:promote_next_level_data()
	...
	if self._network_state then
		self._network_state:set_level_data(
		self._next_level_data.level_key,
		self._next_level_data.environment_variation_id,
		self._next_level_data.level_seed,
		self._next_level_data.mechanism,
		self._next_level_data.game_mode,
		self._next_level_data.conflict_director,
		self._next_level_data.locked_director_functions,
		self._next_level_data.difficulty,
		self._next_level_data.difficulty_tweak,
		self._next_level_data.level_session_id,
		self._next_level_data.level_transition_type,
		self._next_level_data.extra_packages)
	else
...
function LevelTransitionHandler:needs_level_load()
	return self:get_current_level_session_id() ~= self._currently_loaded_level_session_id
end

function LevelTransitionHandler:load_current_level()
...
function LevelTransitionHandler:release_level_resources()
...
function LevelTransitionHandler:_release_level_resources(reference_name)
...
function LevelTransitionHandler:_load_extra_packages(level_key, extra_packages)
...
function LevelTransitionHandler:_release_extra_packages(reference_name)
...
function LevelTransitionHandler:has_next_level()
...
function LevelTransitionHandler:clear_next_level()
	fassert(not self._network_state or self._network_state:is_server(), "only the server handles next level logic")
...
function LevelTransitionHandler:set_next_level(optional_level_key, optional_environment_variation_id, optional_level_seed, optional_mechanism, optional_game_mode, optional_conflict_director, optional_locked_director_functions, optional_difficulty, optional_difficulty_tweak, optional_extra_packages)
...
function LevelTransitionHandler:get_current_locked_director_functions()
...
function LevelTransitionHandler:_set_next_level(level_transition_type, optional_level_key, optional_environment_variation_id, optional_level_seed, optional_mechanism, optional_game_mode, optional_conflict_director, optional_locked_director_functions, optional_difficulty, optional_difficulty_tweak, optional_extra_packages)
	...
	local level_key, environment_variation_id, level_seed, mechanism, game_mode, conflict_director, locked_director_functions, difficulty, difficulty_tweak, extra_packages = LevelTransitionHandler.apply_defaults_to_level_data(
	optional_level_key,
	optional_environment_variation_id,
	optional_level_seed,
	optional_mechanism,
	...
	print("set_next_level( lvl:%s, mc:%s, gm:%s, env:%s, seed:%d, conflict:%s, lckd_director_funcs:{%s}, diff:%s, diff_tweak:%d, id:%d, lt:%s, extra_packages:%s)",
	tostring(level_key),
	mechanism,
	game_mode,
	tostring(environment_variation_id),
	level_seed,
	conflict_director,
	table.concat(locked_director_functions, ","),
	difficulty,
	difficulty_tweak,
	new_level_session_id,
	level_transition_type,
...
function LevelTransitionHandler:_load_level_packages(level_key)
	local async = true
	local package_manager = Managers.package
	local reference_name = level_key
	local settings = LevelSettings [level_key]
...
function LevelTransitionHandler:_unload_level_packages(level_key)
	local reference_name = level_key
	local package_manager = Managers.package
	local settings = LevelSettings [level_key]
	local packages = settings.packages
...
function LevelTransitionHandler:_level_packages_loaded(level_key)
	local reference_name = level_key
	local package_manager = Managers.package
	local settings = LevelSettings [level_key]
	local packages = settings.packages
...
function LevelTransitionHandler.apply_defaults_to_level_data(level_key, environment_variation_id, level_seed, mechanism, game_mode, conflict_director, locked_director_functions, difficulty, difficulty_tweak, optional_extra_packages, is_server)
...
function LevelTransitionHandler:_update_debug()
...
function LevelTransitionHandler:in_hub_level()
	local level_key = self:get_current_level_key()
	if level_key then
		local level_settings = LevelSettings [level_key]
