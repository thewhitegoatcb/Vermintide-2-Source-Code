...
function ActionSoulDrain:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionSoulDrain:client_owner_start_action(new_action, t, chain_action_data, power_level)
...
function ActionSoulDrain:_start_charge_sound()
...
function ActionSoulDrain:_stop_charge_sound()
	local current_action = self.current_action
	local owner_unit = self.owner_unit
	local owner_player = self.owner_player
	local is_bot = owner_player and owner_player.bot_player
...
function ActionSoulDrain:client_owner_post_update(dt, t, world, can_damage)
				...
				if health_extension then
					...
					if self.damage_timer == 0 then
						...
						if not Managers.player:owner(self.owner_unit).bot_player then
							Managers.state.controller_features:add_effect("rumble", { rumble_effect = "hit_character_light" })
						end
					...
					elseif self.last_alive_target_hit == hit_unit and not health_extension:is_alive() and
					self.ammo_extension and self.ammo_extension:current_ammo() < self.ammo_extension:clip_size() then
						self.last_alive_target_hit = nil
...
function ActionSoulDrain:_stop_fx()
	local world = self.world
...
function ActionSoulDrain:_stop_client_vfx()
...
function ActionSoulDrain:finish(reason)
	...
	return { beam_consecutive_hits = math.max(self.consecutive_hits - 1, 0) }
end

function ActionSoulDrain:destroy()
	self:_stop_client_vfx()
	self:_stop_fx()
end