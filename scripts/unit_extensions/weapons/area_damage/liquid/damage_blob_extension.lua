...
function DamageBlobExtension:init(extension_init_context, unit, extension_init_data)
...
function DamageBlobExtension:start_placing_blobs(wait_time, t)
	self.state = "waiting"
...
function DamageBlobExtension:stop_placing_blobs(t)
...
function DamageBlobExtension:_remove_blob(blob, blob_index, blobs)
...
function DamageBlobExtension:destroy()
	local unit = self.unit
	local buff_system = self.buff_system
...
function DamageBlobExtension:place_blobs(unit, t)
	...
	if blob_separation_dist_sq <= blob_dist_sq then
		...
		if success then
			...
			local sfx_list = self.sfx_list
			sfx_list [#sfx_list + 1] = { source = source, time = t + self.blob_life_time }
		end
	end
...
function DamageBlobExtension:update(unit, input, dt, context, t)
	...
	elseif
	state == "lingering" and self.linger_time < t then
...
function DamageBlobExtension:insert_blob(position, radius, rotation, t, nav_world)
	...
	blobs [blob_index] = { position [1], position [2], position [3], radius, { }, t + self.blob_life_time, nav_cost_map_volume_id }
...
function DamageBlobExtension:insert_fx(position, rot, t)
	local world = self.world
	local fx_list = self.fx_list
	...
	fx_list [#fx_list + 1] = { position = Vector3Box(position), id = fx_id_filled, time = blob_life_time, size = Vector3Box(0.6, 1.2, 0) }
	...
	fx_list [#fx_list + 1] = { position = Vector3Box(position), id = fx_id_rim, time = blob_life_time }
...
function DamageBlobExtension:update_blobs_fx_and_sfx(t, dt)
	local world = self.world
...
function DamageBlobExtension:update_blob_overlaps(t)
	local blobs = self.blobs
	local num_blobs = #blobs
	if num_blobs < 1 then
		return
...
function DamageBlobExtension:check_overlap(unit, target_unit, blob_radius, p1, p2, buff_system, num_blobs)
	local player_units_inside = self.player_units_inside
...
function DamageBlobExtension:get_rim_nodes()
	return self.rim_nodes, true
end

function DamageBlobExtension:is_position_inside(position, nav_cost_map_table)
	local blobs = self.blobs
	local num_blobs = #blobs
	if num_blobs == 0 then
		return false
...
function DamageBlobExtension:hot_join_sync(peer_id)
	local fx_list = self.fx_list
	local unit_id = self.unit_id
...
function DamageBlobExtension:_debug_render_blobs()
	local blobs = self.blobs
	for i = 1, #blobs do
		local blob = blobs [i]
...
function DamageBlobExtension:get_source_attacker_unit()
