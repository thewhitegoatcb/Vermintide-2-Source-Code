...
function PlayerCharacterStateInVortex:init(character_state_init_context)
	PlayerCharacterState.init(self, character_state_init_context, "in_vortex")
end

function PlayerCharacterStateInVortex:on_enter(unit, input, dt, context, t, previous_state)
...
function PlayerCharacterStateInVortex:on_exit(unit, input, dt, context, t, next_state)
...
function PlayerCharacterStateInVortex:update_spin_velocity(unit, vortex_unit, vortex_unit_go_id, dt)
...
function PlayerCharacterStateInVortex:update(unit, input, dt, context, t)
	local csm = self.csm
	local status_extension = self.status_extension
	local first_person_extension = self.first_person_extension
	...
	if is_catapulted then
		local params = { sound_event = "Play_enemy_sorcerer_vortex_throw_player", direction = direction }
	...
	if player_actions_allowed and CharacterStateHelper.is_starting_interaction(input_extension, interactor_extension) and
	interactor_extension:allow_movement_during_interaction() then
		interactor_extension:start_interaction("interacting")
	end


	local spin_direction = Unit.alive(self.vortex_unit) and
