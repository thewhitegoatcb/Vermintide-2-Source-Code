...
local BACKGROUND_PROFILE_UNITS = {
	[WITCH_HUNTER] = { unit_package = "units/props/deus_idol/deus_sigmar_01", unit_name = "units/props/deus_idol/deus_sigmar_01" },



	[BRIGHT_WIZARD] = { unit_package = "units/props/deus_idol/deus_myrmidia_01", unit_name = "units/props/deus_idol/deus_myrmidia_01" },



	[DWARF_RANGER] = { unit_package = "units/props/deus_idol/deus_valaya_01", unit_name = "units/props/deus_idol/deus_valaya_01" },



	[WOOD_ELF] = { unit_package = "units/props/deus_idol/deus_lileath_01", unit_name = "units/props/deus_idol/deus_lileath_01" },



	[EMPIRE_SOLDIER] = { unit_package = "units/props/deus_idol/deus_taal_01", unit_name = "units/props/deus_idol/deus_taal_01" } }





local SOUND_EVENTS = { blessing_bought = "hud_morris_map_shrine_buy_blessing", power_up_bought = "hud_morris_map_shrine_buy_power_up", button_hover = "hud_morris_hover", ready_pressed = "hud_morris_close" }
...
local states = { FINISHED = 5, SELECTING = 3, INITIALIZED = 1, FINISHING = 4, STARTING = 2 }
...
local peer_states = { READY_TO_BUY = 1, DONE_BUYING = 2 }
...
local shared_state_spec = {
	server = {
		shop_state = { default_value = 0, type = "number",
	...
	peer = {
		peer_state = { default_value = 0, type = "number",
...
function DeusShopView:init(context)
	...
	"deus_shop_" .. self._deus_run_controller:get_run_id(),
	shared_state_spec,
	self._is_server,
	context.network_server,
	server_peer_id,
...
function DeusShopView:_set_camera_node(camera_node)
...
function DeusShopView:start(params)
...
function DeusShopView:register_rpcs(network_event_delegate, network_transmit)
	self._shared_state:register_rpcs(network_event_delegate)
end

function DeusShopView:unregister_rpcs()
	self._shared_state:unregister_rpcs()
end

function DeusShopView:_create_ui_elements(shop_settings, power_ups, blessings, background_unit_settings)
	...
	for i = 1, num_power_ups do
		...
		widget.offset = {
			...
			init_pos_y - ( spacing_y + widget_size [2] ) * i, 0 }
	...
	for i = 1, num_blessings do
		...
		local relative_offset = { 541, 75, 10 }
		local frame_offset = {
...
function DeusShopView:update(dt, t)
	...
	elseif state == states.FINISHED and
	not self._finished then
		self:_finish()
		self._finished = true
	end
...
function DeusShopView:post_update(dt, t)
	if self._unit_previewer then
		self._unit_previewer:post_update(dt, t)
	end
end

function DeusShopView:_update_animations(dt)
	local ui_animator = self._ui_animator
	ui_animator:update(dt)
end

function DeusShopView:destroy_idol()
...
function DeusShopView:input_service()
	return self._input_manager:get_service(self._input_service_name)
end

function DeusShopView:_finish(data)
...
function DeusShopView:_init_power_up_widget(widget, power_up_instance, discount, current_value, max_value, profile_index, career_index)
...
function DeusShopView:_init_blessing_widget(widget, blessing_name)
	local content = widget.content
...
function DeusShopView:_update_countdowns(state, dt, t)
...
function DeusShopView:_check_transition(state)
	if state == states.STARTING then
		if self:_are_all_peers_ready() then
			do return states.SELECTING end
		end
	elseif state == states.SELECTING then
...
function DeusShopView:_update_during_starting(dt, t)
...
function DeusShopView:_update_during_selecting(dt, t)
	local widgets_by_name = self._widgets_by_name
	local bottom_text_widget_content = widgets_by_name.bottom_text.content
...
function DeusShopView:_update_during_finishing(dt, t)
	local widgets_by_name = self._widgets_by_name
	local bottom_text_widget_content = widgets_by_name.bottom_text.content
...
function DeusShopView:_update_shop_widgets()
	local local_peer_id = self._deus_run_controller:get_own_peer_id()
...
function DeusShopView:_acquire_input(ignore_cursor_stack)
	self:_release_input(true)
	local input_manager = self._input_manager
	local input_service_name = self._input_service_name
	input_manager:capture_input({ "keyboard", "gamepad", "mouse" }, 1, input_service_name, "DeusShopView")
...
function DeusShopView:_release_input(ignore_cursor_stack)
	local input_manager = self._input_manager
	input_manager:release_input({ "keyboard", "gamepad", "mouse" }, 1, self._input_service_name, "DeusShopView")
...
function DeusShopView:_update_button_hover_sound(widget)
	if UIUtils.is_button_hover_enter(widget) then
		self:_play_sound(SOUND_EVENTS.button_hover)
	end
end

function DeusShopView:_on_blessing_bought(blessing_name)
	self._deus_run_controller:shop_buy_blessing(blessing_name)
end

function DeusShopView:_on_power_up_bought(power_up, discount)
	self._deus_run_controller:shop_buy_power_up(power_up, discount)
end

function DeusShopView:_handle_input(dt, t)
	local shop_items = self._shop_items
	for _, power_up_data in ipairs(shop_items.power_ups) do
		local widget = power_up_data.widget
...
function DeusShopView:_get_power_up_costs(rarity, discount)
...
function DeusShopView:_play_sound(event)
	WwiseWorld.trigger_event(self._wwise_world, event)
end

function DeusShopView:_update_player_data()
	local player_data = { { } }
...
function DeusShopView:_update_portrait_frame(frame_name, level_text, index)
...
function DeusShopView:_update_blessing_portrait_frame(frame_name, level_text, blessing_name, index, offset, is_bought)
...
function DeusShopView:_update_player_portraits(player_data)
	local widgets_by_name = self._widgets_by_name
...
function DeusShopView:_are_all_peers_ready()
	local local_peer_id = self._deus_run_controller:get_own_peer_id()
...
function DeusShopView:_are_all_peers_done()
	for _, peer_id in ipairs(self._deus_run_controller:get_peers()) do
		local key = self._shared_state:get_key("peer_state")
...
function DeusShopView:_did_someone_vote()
	for _, peer_id in ipairs(self._deus_run_controller:get_peers()) do
		local key = self._shared_state:get_key("peer_state")
		local ready_state = self._shared_state:get_peer(peer_id, key) == peer_states.DONE_BUYING
...
function DeusShopView:_animate_shop_item_widget(dt, widget)
	local content = widget.content
	local style = widget.style
...
function DeusShopView:_draw(dt)
...
function DeusShopView:_create_unit_previewer(widget, unit_name, package_name)
	...
	local preview_position = { 0.15, 2.5, -0.5 }
...
function DeusShopView:_create_background_unit_definition()
	local shading_environment = "environment/ui_weave_forge_preview"
	return { scenegraph_id = "background_unit",
...
function DeusShopView:_update_hold_text()
	local widgets_by_name = self._widgets_by_name
...
function DeusShopView:_update_background_animations(dt)
	local widgets_by_name = self._widgets_by_name
...
function DeusShopView:on_ingame_menu_opened()
	self._render_top_widgets = false
	Managers.input:disable_gamepad_cursor()
end

function DeusShopView:on_ingame_menu_closed()
	self._render_top_widgets = true
