...
function RewardsPopupUI:init(parent, in_game_ui_context)
	self._parent = parent
...
function RewardsPopupUI:destroy()
	Managers.state.event:unregister("present_rewards", self)
end

function RewardsPopupUI:update(dt, t)
...
function RewardsPopupUI:present_rewards(rewards)
		...
		for _, data in ipairs(rewards) do
			...
			elseif reward_type == "item_tooltip" then
				...
				local entry = {
					...
					[#entry + 1] = { widget_type = "item_tooltip",
						value = reward_item },



					[#entry + 1] = { widget_type = "item",
						value = reward_item } }
			...
			elseif reward_type == "deus_item_tooltip" then
				...
				local entry = {
					...
					[#entry + 1] = { widget_type = "deus_item_tooltip",
						value = reward_item },



					[#entry + 1] = { widget_type = "deus_item",
						value = reward_item } }



				local animation_data = { end_animation = "deus_close", start_animation = "deus_open" }
			...
			elseif reward_type == "deus_power_up" then
				...
				local entry = {
					...
					[#entry + 1] = { widget_type = "deus_power_up",
						value = deus_power_up },



					[#entry + 1] = { widget_type = "deus_icon",
						value = deus_power_up } }
			...
			elseif reward_type == "deus_power_up_end_of_level" then
				...
				local animation_data = { end_animation = "deus_close", start_animation = "deus_open", animation_wait_time = 6 }
...
function RewardsPopupUI:_displaying_reward_presentation()
	return self._reward_popup:is_presentation_active()
end

function RewardsPopupUI:_is_reward_presentation_complete()
	return self._reward_popup:is_presentation_complete()
end

function RewardsPopupUI:all_presentations_done()
...
function RewardsPopupUI:_handle_queued_presentations()
...
function RewardsPopupUI:_play_sounds(sounds)
	if not sounds then
		return
	end
...
function RewardsPopupUI:_present_reward(data)
