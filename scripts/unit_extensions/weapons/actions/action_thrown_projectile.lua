...
function ActionThrownProjectile:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionThrownProjectile:client_owner_start_action(new_action, t, chain_action_data, power_level)
...
function ActionThrownProjectile:_use_ammo()
...
function ActionThrownProjectile:_reload()
	local ammo_extension = self._ammo_extension
...
function ActionThrownProjectile:client_owner_post_update(dt, t, world, can_damage)
	local current_action = self._current_action
	...
	if self.state == "shooting" then
		...
		if not Managers.player:owner(self.owner_unit).bot_player then
			Managers.state.controller_features:add_effect("rumble", { rumble_effect = "bow_fire" })
		end
...
function ActionThrownProjectile:finish(reason, data)
...
function ActionThrownProjectile:_fire(add_spread)
	local current_action = self._current_action
	local owner_unit = self.owner_unit
	local first_person_extension = self._first_person_extension
