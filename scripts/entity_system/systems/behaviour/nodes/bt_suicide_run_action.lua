...
function BTSuicideRunAction:init(...)
...
function BTSuicideRunAction:enter(unit, blackboard, t)
	...
	local params = { unit = unit, blackboard = blackboard, action = action }
...
function BTSuicideRunAction:leave(unit, blackboard, t, reason, destroy)
...
function BTSuicideRunAction:update_target_position(unit, blackboard, ai_navigation, stop)
...
function BTSuicideRunAction:play_unit_audio(unit, blackboard, sound_name)
	Managers.state.entity:system("audio_system"):play_audio_unit_event(sound_name, unit)
end

function BTSuicideRunAction:run(unit, blackboard, t, dt)
	local suicide_run = blackboard.suicide_run
...
function BTSuicideRunAction.StateInit:on_enter(params)
...
function BTSuicideRunAction.StateInit:update(dt, t)
...
function BTSuicideRunAction.StateMove:on_enter(params)
	local unit = params.unit
	local blackboard = params.blackboard
...
function BTSuicideRunAction.StateMove:update(dt, t)
	local unit = self.unit
	local blackboard = self.blackboard
	local suicide_run = blackboard.suicide_run
...
function BTSuicideRunAction.StateExplode:on_enter(params)
	local unit = params.unit
	local blackboard = params.blackboard
	local suicide_run = blackboard.suicide_run
...
function BTSuicideRunAction.StateExplode:update(dt, t)
	local unit = self.unit
	local blackboard = self.blackboard
	local suicide_run = blackboard.suicide_run
