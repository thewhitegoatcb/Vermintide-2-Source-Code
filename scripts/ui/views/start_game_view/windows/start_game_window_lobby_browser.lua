...
local MATCHMAKING_TYPE_LOOKUP_STRINGS = { deus = "area_selection_morris_name", deed = "lb_game_type_deed", weave = "menu_weave_area_no_wom_title", event = "lb_game_type_event", custom = "lb_game_type_custom", demo = "lb_game_type_none", adventure = "area_selection_campaign", tutorial = "lb_game_type_prologue", versus = "vs_ui_versus_tag", ["n/a"] = "lb_game_type_none", any = "lobby_browser_mission" }
...
function StartGameWindowLobbyBrowser:on_enter(params, offset)
	...
	local lobby_list_offset = { 0, 0, 0 }
	local settings = { use_top_renderer = false, num_list_items = 15, input_service_name = input_service_name, offset = lobby_list_offset }
...
function StartGameWindowLobbyBrowser:create_ui_elements(params, offset)
...
function StartGameWindowLobbyBrowser:_assign_hero_portraits()
...
function StartGameWindowLobbyBrowser:on_exit(params)
...
function StartGameWindowLobbyBrowser:update(dt, t)
...
function StartGameWindowLobbyBrowser:post_update(dt, t)
	return end

function StartGameWindowLobbyBrowser:_handle_weave_data(lobby_data)
	...
	if weave_name ~= "false" then
		...
		if weave_template then
			...
			wind_icon_style.texture_size = { thumbnail_icon_size [1] * 0.8, thumbnail_icon_size [2] * 0.8 }
...
function StartGameWindowLobbyBrowser:_handle_lobby_data(game_type, lobby_data)
	local info_box_widgets = self._lobby_info_box_base_widgets_by_name
...
function StartGameWindowLobbyBrowser:_gather_unlocked_journeys()
...
function StartGameWindowLobbyBrowser:_handle_deus_data(lobby_data)
...
function StartGameWindowLobbyBrowser:_assign_objective(index, text, icon, spacing)
...
function StartGameWindowLobbyBrowser:_setup_lobby_info_box(lobby_data)
...
function StartGameWindowLobbyBrowser:_update_animations(dt)
...
function StartGameWindowLobbyBrowser:_is_refreshing()
	local current_lobby_type = self._current_lobby_type
	if current_lobby_type == "lobbies" then
...
function StartGameWindowLobbyBrowser:_handle_input(dt, t)
...
function StartGameWindowLobbyBrowser:_handle_name_input_box(dt, t)
	local input_service = self.parent:window_input_service()
...
function StartGameWindowLobbyBrowser:_handle_selected_lobby_input()
...
function StartGameWindowLobbyBrowser:_add_server_to_favorites(lobby_data)
	local server_info = lobby_data.server_info
...
function StartGameWindowLobbyBrowser:_remove_server_from_favorites(lobby_data)
	local server_info = lobby_data.server_info
	local ip = server_info.ip_address
	local connection_port = server_info.connection_port
	local query_port = server_info.query_port
...
function StartGameWindowLobbyBrowser:draw(dt)
...
function StartGameWindowLobbyBrowser:_play_sound(event)
...
function StartGameWindowLobbyBrowser:cancel_join_lobby(status_message)
...
function StartGameWindowLobbyBrowser:_populate_lobby_list(auto_update)
...
function StartGameWindowLobbyBrowser:_get_lobbies()
	local current_lobby_type = self._current_lobby_type
	if current_lobby_type == "lobbies" then
		local lobby_finder = self.lobby_finder
		do return lobby_finder:lobbies() or empty_lobby_list end
	elseif current_lobby_type == "servers" then
		local game_server_finder = self.game_server_finder
		do return game_server_finder:servers() or empty_lobby_list end
	else
		ferror("Unknown lobby type (%s)", current_lobby_type)
	end
end

function StartGameWindowLobbyBrowser:_valid_lobby(lobby_data)
	...
	if is_server then
		...
		if
		wanted_server_name ~= "" and string.find(lobby_data.server_info.name, wanted_server_name) == nil then
			do return false end
		end
	else
...
function StartGameWindowLobbyBrowser:_update_auto_refresh(dt)
...
function StartGameWindowLobbyBrowser:_update_join_button(lobby_data)
...
function StartGameWindowLobbyBrowser:_reset_filters()
...
function StartGameWindowLobbyBrowser:_reset_level_filter()
	local levels_table = self:_get_levels()
	local any_level_index = #levels_table
	self:_on_level_stepper_input(0, any_level_index)
end

function StartGameWindowLobbyBrowser:_reset_difficulty_filter()
...
function StartGameWindowLobbyBrowser:_switch_lobby_type(new_lobby_type)
	...
	if new_lobby_type == "lobbies" then --[[ Nothing --]]
...
function StartGameWindowLobbyBrowser:_create_filter_requirements()
	local lobby_finder = self.lobby_finder
	...
	if IS_PS4 then
		...
		if distance_filter == "close" then
			requirements.filters.primary_region = { comparison = "equal",
				value = MatchmakingRegionLookup.primary [user_region] }


		elseif distance_filter == "medium" then
			requirements.filters.secondary_region = { comparison = "equal",
	...
	requirements.filters.eac_authorized = { comparison = "equal",
		value = eac_authorized and "true" or "false" }



	if difficulty_key ~= "any" and difficulty_key then
		requirements.filters.difficulty = { comparison = "equal",
	...
	if level_key ~= "any" and level_key then
		requirements.filters.selected_mission_id = { comparison = "equal",
			value = level_key }
	end
	...
	if mechanism ~= "any" then
		requirements.filters.mechanism = { comparison = "equal",
			value = mechanism }
	end
	...
	if only_show_valid_lobbies then
		requirements.filters.network_hash = { comparison = "equal",
	...
	if matchmaking and current_lobby_type == "lobbies" then
		requirements.filters.matchmaking = { value = "false", comparison = "not_equal" }
	end
...
function StartGameWindowLobbyBrowser:_join(lobby_data, join_params)
	Managers.matchmaking:request_join_lobby(lobby_data, join_params)
	self.join_lobby_data_id = lobby_data.id
end

function StartGameWindowLobbyBrowser:_search()
	...
	elseif self._current_lobby_type == "servers" then
		...
		local game_server_requirements = {
			server_browser_filters = { dedicated = "valuenotused", full = "valuenotused",
...
function StartGameWindowLobbyBrowser:_get_levels()
	local game_mode_data = self._game_mode_data
	...
	local levels = data and data.levels or { "any" }
	return levels
end

function StartGameWindowLobbyBrowser:_get_difficulties()
	local game_mode_data = self._game_mode_data
	local game_modes = game_mode_data.game_modes
	local game_mode_index = self.selected_game_mode_index or game_modes.any
	local data = game_mode_data [game_mode_index]
	local difficulties = data and data.difficulties or { "any" }
	return difficulties
end

function StartGameWindowLobbyBrowser:_on_game_type_stepper_input(index_change, specific_index)
...
function StartGameWindowLobbyBrowser:_on_level_stepper_input(index_change, specific_index)
...
function StartGameWindowLobbyBrowser:_on_difficulty_stepper_input(index_change, specific_index)
...
function StartGameWindowLobbyBrowser:_on_show_lobbies_stepper_input(index_change, specific_index)
...
function StartGameWindowLobbyBrowser:_on_distance_stepper_input(index_change, specific_index)
...
function StartGameWindowLobbyBrowser:_on_search_type_stepper_input(index_change, specific_index)
...
function StartGameWindowLobbyBrowser:_on_stepper_input(stepper, data_table, current_index, index_change, specific_index)
...
function StartGameWindowLobbyBrowser:_handle_stepper_input(stepper_name, stepper, step_function)
...
function StartGameWindowLobbyBrowser:_on_stepper_arrow_pressed(widget, name, style_id)
	local ui_animations = self._ui_animations
...
function StartGameWindowLobbyBrowser:_on_stepper_arrow_hover(widget, name, style_id)
	local ui_animations = self._ui_animations
	local animation_name = "stepper_widget_arrow_" .. name .. style_id
...
function StartGameWindowLobbyBrowser:_on_stepper_arrow_dehover(widget, name, style_id)
	local ui_animations = self._ui_animations
	local animation_name = "stepper_widget_arrow_" .. name .. style_id
...
function StartGameWindowLobbyBrowser:_animate_element_by_time(target, target_index, from, to, time)
	local new_animation = UIAnimation.init(UIAnimation.function_by_time, target, target_index, from, to, time, math.ease_out_quad)
	return new_animation
end

function StartGameWindowLobbyBrowser:_animate_element_by_catmullrom(target, target_index, target_value, p0, p1, p2, p3, time)
