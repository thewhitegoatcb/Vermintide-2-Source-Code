...
function ProjectilePhysicsUnitLocomotionExtension:init(extension_init_context, unit, extension_init_data)
	self.unit = unit
...
function ProjectilePhysicsUnitLocomotionExtension:destroy()
	return end

local STOP_VELOCITY_THRESHOLD = 0.1
local STOP_TIME_THRESHOLD = 0.5
function ProjectilePhysicsUnitLocomotionExtension:update(unit, input, dt, context, t)
...
function ProjectilePhysicsUnitLocomotionExtension:bounce(touching_unit, position, normal, separation_distance, impulse_force)
	local length = Vector3.length(impulse_force)
	if BOUNCE_FORCE_THRESHOLD < length then --[[ Nothing --]]
	end

end

function ProjectilePhysicsUnitLocomotionExtension:stop()
	self.stopped = true
...
function ProjectilePhysicsUnitLocomotionExtension:drop()
	self.dropped = true
...
function ProjectilePhysicsUnitLocomotionExtension:has_stopped()
	return self.stopped
end