...
function BTInterestPointApproachAction:init(...)
	BTInterestPointApproachAction.super.init(self, ...)
end

function BTInterestPointApproachAction:enter(unit, blackboard, t)
...
function BTInterestPointApproachAction:leave(unit, blackboard, t, reason, destroy)
	...
	elseif reason == "aborted" then --[[ Nothing --]]
	end
...
function BTInterestPointApproachAction:run(unit, blackboard, t, dt)
	...
	local navigation_extension = blackboard.navigation_extension
	if blackboard.ip_state == "moving_to_target" and
	navigation_extension:has_reached_destination() then
