...
function DeusMapUI:init(context)
...
function DeusMapUI:event_ingame_player_list_enabled(enabled, override_disable_cursor)
...
function DeusMapUI:_create_ui_elements()
	local scenegraph_definition = definitions.scenegraph_definition
	local ui_scenegraph = UISceneGraph.init_scenegraph(scenegraph_definition)

	local _, widgets_by_name = UIUtils.create_widgets(definitions.widget_definitions)

	local render_settings = { alpha_multiplier = 1, snap_pixel_positions = true }
	local anim_data = { alpha_multiplier = 1 }
	local widgets = { }
...
function DeusMapUI:on_enter(input_service)
	self._input_service = input_service
end

function DeusMapUI:on_exit()
	return end

function DeusMapUI:update(dt, t)
...
function DeusMapUI:_update_animations(dt, t)
	...
	progress = interpolation_time <= 0.001 and
	1 or

	math.clamp(( t - anim_data.alpha_multiplier_animation_start_time ) / interpolation_time, 0, 1)
...
function DeusMapUI:enable_hover_text(screen_pos, type, level, theme, minor_modifier_group, director_name, terror_event_power_up, grant_random_power_up_count, terror_event_power_up_rarity, selected, selectable, profile_index, career_index)
...
function DeusMapUI:_update_portrait_frame(frame_name, level_text, index)
...
function DeusMapUI:update_player_data(player_data)
...
function DeusMapUI:update_timer(time_left, optional_override_text)
	local widget = self._widgets_by_name.general_info
...
function DeusMapUI:hide_timer()
	local widget = self._widgets_by_name.general_info
	widget.content.time = ""
end

function DeusMapUI:disable_hover_text()
	local widget = self._widgets_by_name.node_info
	widget.content.visible = false
end

function DeusMapUI:set_alpha_multiplier(alpha)
	self._anim_data.alpha_multiplier = alpha
end

function DeusMapUI:show_full_screen_rect()

	return self:set_full_screen_rect_visibility(true)
end

function DeusMapUI:hide_full_screen_rect()

	return self:set_full_screen_rect_visibility(false)
end

function DeusMapUI:set_full_screen_rect_visibility(is_visible)
	self._render_full_screen_rect = is_visible
end

function DeusMapUI:show_content()
	self:_set_content_visibility(true)
end

function DeusMapUI:hide_content()
	self:_set_content_visibility(false)
end

function DeusMapUI:_set_content_visibility(is_visible)
...
function DeusMapUI:fade_out(duration)
	local anim_data = self._anim_data
...
function DeusMapUI:fade_in(duration)
	local anim_data = self._anim_data
	anim_data.source_alpha_multiplier = anim_data.alpha_multiplier
