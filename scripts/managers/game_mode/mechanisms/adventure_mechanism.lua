...
local vote_requests = {
	...
	weave_quick_play = function (params)
		print_vote_request(params)
		local vote_data = { quick_game = true,
			difficulty = params.difficulty,
	...
	weave = function (params)
		print_vote_request(params)
		local vote_data = { quick_game = false,
			mission_id = params.mission_id,
			difficulty = params.difficulty,
...
local RPCS = { "rpc_sync_adventure_data_to_peer" }



function AdventureMechanism:init(settings)
	self:_reset(settings)
end

function AdventureMechanism:register_rpcs(network_event_delegate)
...
function AdventureMechanism:unregister_rpcs()
...
function AdventureMechanism:on_venture_start()
	return end

function AdventureMechanism:on_venture_end()
...
function AdventureMechanism:is_venture_over()
	...
	if self._state == WEAVE_STATE then
		...
		do return game_mode_ended and (reason == "lost" or final_round) end
	else
		return game_mode_ended
	end
end

function AdventureMechanism:handle_ingame_exit(exit_type)
...
function AdventureMechanism:_reset(settings, prior_state)
...
function AdventureMechanism:network_context_destroyed()
	self:_reset()
end

function AdventureMechanism:choose_next_state(next_state)
	local state = self._state

	if state == HUB_STATE or state == TUTORIAL_STATE then
		local acceptable_states = { weave = true, ingame = true, tutorial = true }
...
function AdventureMechanism:reset_choose_next_state()
	self._next_state = nil
end

function AdventureMechanism:progress_state()
...
function AdventureMechanism:get_prior_state()
	return self._prior_state
end

function AdventureMechanism:set_current_state(state)
	self._prior_state = self._state
	self._state = state
end

function AdventureMechanism:get_hub_level_key()
	return self._debug_hub_level_key or AdventureMechanism.get_starting_level()
end

function AdventureMechanism:allocate_slot(sender, profile_index)
...
function AdventureMechanism:get_level_seed(level_seed, optional_system)
	local weave_manager = Managers.weave
...
function AdventureMechanism:get_end_of_level_rewards_arguments(game_won, quickplay, statistics_db, stats_id)
...
function AdventureMechanism:get_end_of_level_extra_mission_results()
...
function AdventureMechanism:is_final_round()
...
function AdventureMechanism:get_level_end_view()
...
function AdventureMechanism:game_round_ended(t, dt, reason)
...
function AdventureMechanism:should_run_tutorial()
	return true, TUTORIAL_STATE
end


function AdventureMechanism:_get_next_game_mode_key()
...
function AdventureMechanism:start_next_round()
...
function AdventureMechanism:_build_side_compositions(state)
...
function AdventureMechanism:profile_available(profile_synchronizer, profile_name, career_name)
...
function AdventureMechanism:get_state()
	return self._state
end


function AdventureMechanism:sync_mechanism_data(peer_id, mechanism_newly_initialized)
	local weave_manager = Managers.weave
...
function AdventureMechanism:rpc_sync_adventure_data_to_peer(channel_id, next_weave_name_id, next_weave_objective_index)
...
function AdventureMechanism:profile_available_for_peer(profile_synchronizer, peer_id, local_player_id, profile_name, career_name)
	local profile_index = FindProfileIndex(profile_name)
...
function AdventureMechanism:should_play_level_introduction()
	return true
end

function AdventureMechanism:uses_random_directors()
...
function AdventureMechanism:debug_load_level(level_name, environment_variation_id)
...
function AdventureMechanism:request_vote(params)
...
function AdventureMechanism:override_hub_level(new_hub_level_key)
...
function AdventureMechanism:left_mechanism_due_to_switch()
	self:_unload_packages()
end

function AdventureMechanism:is_packages_loaded()
...
function AdventureMechanism:load_packages()
...
function AdventureMechanism:_load_live_event_packages()
...
function AdventureMechanism:_unload_packages()
	local additional_packages = self._additional_packages
