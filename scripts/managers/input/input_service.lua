...
function InputService:init(input_service_name, keymaps_name, filters_name, block_reasons)
...
function InputService:map_device(input_device_type, input_device, input_device_data)
...
function InputService:unmap_device(input_device_type, input_device)
	local input_device_type_list = self.mapped_devices [input_device_type]
...
function InputService:get(input_data_name, consume)
...
function InputService:get_controller_cursor_position()
	return self.controller_select:unbox()
end

function InputService:set_controller_cursor_position(x, y, z)
...
function InputService:get_active_filters(optional_platform, optional_input_name)
	...
	if not optional_platform and IS_XB1 and (self.input_manager:is_device_active("keyboard") or self.input_manager:is_device_active("mouse")) then
		...
		if not filters [optional_input_name] then
			do return filters_data [platform] end
		else
			return filters
		end
	end
...
function InputService:get_keymapping(keymap_name, optional_platform)
	local keymaps = self:get_active_keymaps(optional_platform, keymap_name)
	return keymaps [keymap_name]
end

function InputService:add_keymap(keymap_name)
...
function InputService:remove_keymap(keymap_name)
	local keymaps = self:get_active_keymaps()
...
function InputService:generate_keybinding_setting()
	local new_keymaps = { }
	local keymaps = self:get_active_keymaps()
...
function InputService:generate_filters_setting()
	local new_filters = { }
...
function InputService:has(keymap_name)
...
function InputService:is_blocked()
	return self.service_is_blocked or self.disabled_input_group
end

function InputService:set_blocked(is_blocked, disabled_input_group)
	self.service_is_blocked = is_blocked
end

function InputService:set_disabled_input_group(disabled_input_group)
	self.disabled_input_group = disabled_input_group
end

function InputService:set_input_blocked(input_data_name, blocked)
	self.blocked_input [input_data_name] = blocked or nil
end

function InputService:set_hover(hover)
	self.hovering = self.hovering or hover
end

function InputService:is_hovering()
