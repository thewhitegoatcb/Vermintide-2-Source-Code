...
function CareerAbilityWHZealot:init(extension_init_context, unit, extension_init_data)
...
function CareerAbilityWHZealot:extensions_ready(world, unit)
...
function CareerAbilityWHZealot:destroy()
	return end

function CareerAbilityWHZealot:update(unit, input, dt, context, t)
...
function CareerAbilityWHZealot:stop(reason)
	if
...
function CareerAbilityWHZealot:_start_priming()
	...
	local buff_params = { external_optional_multiplier = 0.3 }
	self._buff_id = buff_extension:add_buff(buff_template_name, buff_params)

	self._is_priming = true
end

function CareerAbilityWHZealot:_update_priming(dt)
	if self._local_player then
...
function CareerAbilityWHZealot:_stop_priming()
...
function CareerAbilityWHZealot:_run_ability()
	self:_stop_priming()
	...
	local buff_names = { "victor_zealot_activated_ability" }
	...
	if talent_extension:has_talent("victor_zealot_activated_ability_cooldown_stack_on_hit", "witch_hunter", true) then
		buff_extension:add_buff("victor_zealot_activated_ability_cooldown_stack_on_hit", { attacker_unit = owner_unit })
	end
	...
	status_extension.do_lunge = { animation_end_event = "zealot_active_ability_charge_hit", allow_rotation = false, first_person_animation_end_event = "dodge_bwd", first_person_hit_animation_event = "charge_react", falloff_to_speed = 8, dodge = true, first_person_animation_event = "shade_stealth_ability", first_person_animation_end_event_hit = "dodge_bwd", duration = 0.75, initial_speed = 25, animation_event = "zealot_active_ability_charge_start",
		...
		damage = { depth_padding = 0.4, height = 1.8, collision_filter = "filter_explosion_overlap_no_player", hit_zone_hit_name = "full", ignore_shield = true, interrupt_on_max_hit_mass = true, power_level_multiplier = 0.8, interrupt_on_first_hit = false, damage_profile = "heavy_slashing_linesman", width = 1.5, allow_backstab = true,
			...
			stagger_angles = { max = 90, min = 45 },




			on_interrupt_blast = { allow_backstab = false, radius = 3, power_level_multiplier = 1, hit_zone_hit_name = "full", damage_profile = "heavy_slashing_linesman", ignore_shield = false, collision_filter = "filter_explosion_overlap_no_player" } } }
...
function CareerAbilityWHZealot:_play_vo()
	local owner_unit = self._owner_unit
