...
function NavTagVolumeHandler:init(world, nav_world)
	self.world = world
...
function NavTagVolumeHandler:create_tag_volume_from_mappings(level_volume_name)
...
function NavTagVolumeHandler:create_mapping(pos, size, layer_name)
	...
	mapping.bottom_points = { {
			...
			left_down [1], left_down [2], left_down [3] }, {
			down [1], down [2], down [3] }, {
			down_right [1], down_right [2], down_right [3] }, {
			right [1], right [2], right [3] }, {
			up_right [1], up_right [2], up_right [3] }, {
			up [1], up [2], up [3] }, {
			up_left [1], up_left [2], up_left [3] } }


	mapping.color = { 255, 255, 255, 255 }
...
function NavTagVolumeHandler:get_mapping_from_lookup_id(lookup_id)
	local volume_name = self.mapping_lookup_table [lookup_id]
	return volume_name and self.mappings [volume_name]
end

function NavTagVolumeHandler:destroy_nav_tag_volume(volume_name)
...
function NavTagVolumeHandler:set_mapping_layer_name(level_volume_name, layer_name)
	fassert(self.mappings_available, "[NavTagVolumeHandler] Current level requires world_nav_tag_volumes.lua to be located in the level directory. Run SpawnGenerator in the level editor to export it!")
	local mapping = self.mappings [level_volume_name]
	fassert(mapping, "[NavTagVolumeHandler] Level volume %q could not be found in world_nav_tag_volumes.lua. Run SpawnGenerator in the level editor to export it!", level_volume_name)
...
function NavTagVolumeHandler:destroy()
