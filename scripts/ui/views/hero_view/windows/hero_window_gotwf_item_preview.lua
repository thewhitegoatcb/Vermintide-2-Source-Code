...
function HeroWindowGotwfItemPreview:on_enter(params, offset)
...
function HeroWindowGotwfItemPreview:_start_transition_animation(animation_name)
	local params = {
...
function HeroWindowGotwfItemPreview:_create_viewport_definition()
	local shading_environment = "environment/ui_store_preview"
	return { scenegraph_id = "viewport",
		element = UIElements.Viewport,
		style = {
			viewport = { viewport_type = "default_forward", layer = 990, viewport_name = "item_preview_viewport", world_name = "item_preview", horizontal_alignment = "center", vertical_alignment = "center", level_name = "levels/ui_store_preview/world", enable_sub_gui = false, fov = 65,
...
function HeroWindowGotwfItemPreview:_create_ui_elements(params, offset)
...
function HeroWindowGotwfItemPreview:on_exit(params, force_unload)
...
function HeroWindowGotwfItemPreview:update(dt, t)
...
function HeroWindowGotwfItemPreview:_update_previewers(dt, t)
	if self._selected_product then
		local input_service = self._parent:window_input_service()
		local input_handled = false local input_hovered = false
...
function HeroWindowGotwfItemPreview:_register_object_sets(viewport_widget, viewport_definition)
...
function HeroWindowGotwfItemPreview:_show_object_set(object_set_name, force_disable)
...
function HeroWindowGotwfItemPreview:_update_environment(item_preview_environment, force_default)
	if not self._viewport_widget then
		return
	end
...
function HeroWindowGotwfItemPreview:_destroy_viewport_gui()
...
function HeroWindowGotwfItemPreview:_create_viewport_gui()
	local world = Managers.world:world("item_preview")
	...
	self._gui_resolution = { w, h }
end

function HeroWindowGotwfItemPreview:post_update(dt, t)
...
function HeroWindowGotwfItemPreview:_update_animations(dt)
...
function HeroWindowGotwfItemPreview:_exit()
	self.exit = true
end

function HeroWindowGotwfItemPreview:_get_alpha_multiplier(widget, alpha_multiplier)
...
function HeroWindowGotwfItemPreview:draw(dt)
...
local EMPTY_TABLE = { }
function HeroWindowGotwfItemPreview:_render_viewport_mask()
	...
	local x = viewport_size_x * w * 0.285 local y = viewport_size_y * h * 0.26
	...
	local x = x local y = viewport_size_y * h * 0.2
	...
	local x = viewport_size_x * w * 0.09 local y = viewport_size_y * h
	Gui.bitmap(viewport_gui, "gui_lock_test_viewport_mask", Vector3(0, 0, 2), Vector2(x, y))
	Gui.bitmap(viewport_gui, "gui_lock_test_viewport_mask", Vector3(w * viewport_size_x - x, 0, 2), Vector2(x, y))
...
function HeroWindowGotwfItemPreview:_play_sound(event)
	self._parent:play_sound(event)
end

function HeroWindowGotwfItemPreview:_start_loading_overlay()
...
function HeroWindowGotwfItemPreview:_update_loading_overlay_fadeout_animation(dt)
...
function HeroWindowGotwfItemPreview:_destroy_previewers()
...
function HeroWindowGotwfItemPreview:_sync_presentation_item(force_update)
...
local EMPTY_TABLE = { }
function HeroWindowGotwfItemPreview:_present_item(item)
...
function HeroWindowGotwfItemPreview:_update_claimed_status()
...
function HeroWindowGotwfItemPreview:_create_material_instance(gui, new_material_name, template_material_name, reference_name)
...
function HeroWindowGotwfItemPreview:_set_material_diffuse(gui, material_name, texture_path)
...
function HeroWindowGotwfItemPreview:_load_texture_package(package_name, reference_name, callback)
...
function HeroWindowGotwfItemPreview:_is_unique_reference_to_material(reference_name)
	local cloned_materials_by_reference = self._cloned_materials_by_reference
...
function HeroWindowGotwfItemPreview:_unload_texture_by_reference(reference_name)
	local loaded_package_names = self._loaded_package_names
	local cloned_materials_by_reference = self._cloned_materials_by_reference
...
function HeroWindowGotwfItemPreview:_delayed_item_unit_presentation(item)
	local reward_type = item.reward_type
	if reward_type == "keep_decoration_painting" then
...
function HeroWindowGotwfItemPreview:_setup_painting_presentation(item)
	...
	local function callback()
		...
		if painting_data.orientation == "horizontal" then
			content.painting = {
				...
				uvs = { { 0, padding }, { 1, 1 - padding } } }


			style.painting.texture_size = { base_size, base_size * (1 - 2 * padding) }
			style.painting_frame.area_size = { base_size, base_size * (1 - 2 * padding) }
		else
			content.painting = {
				texture_id = texture_name,
				uvs = { { padding, 0 }, { 1 - padding, 1 } } }


			style.painting.texture_size = { base_size * (1 - 2 * padding), base_size }
			style.painting_frame.area_size = { base_size * (1 - 2 * padding), base_size }
		end
...
function HeroWindowGotwfItemPreview:_setup_item_presentation(item)
	...
	if slot_type == "melee" or slot_type == "ranged" or slot_type == "weapon_skin" then
		...
		local item = { data = masterlist_item }
...
function HeroWindowGotwfItemPreview:_update_delayed_item_unit_presentation(dt)
...
function HeroWindowGotwfItemPreview:_set_title_name(text)
	local widget = self._top_widgets_by_name.title_text
	widget.content.text = text
end

function HeroWindowGotwfItemPreview:_set_sub_title_name(text)
	local widget = self._top_widgets_by_name.sub_title_text
	widget.content.text = text
end

function HeroWindowGotwfItemPreview:_set_description_text(text)
	local widget = self._top_widgets_by_name.description_text
	widget.content.text = text
end

function HeroWindowGotwfItemPreview:_set_sub_title_alpha_multiplier(alpha_multiplier)
	local widget = self._top_widgets_by_name.sub_title_text
	widget.alpha_multiplier = alpha_multiplier
end

function HeroWindowGotwfItemPreview:_set_type_title_name(text)
	local widget = self._top_widgets_by_name.type_title_text
	widget.content.text = text
end

function HeroWindowGotwfItemPreview:_set_career_title_name(text)
	local widget = self._top_widgets_by_name.career_title_text
	widget.content.text = text
end

function HeroWindowGotwfItemPreview:_set_disclaimer_text(text)
...
function HeroWindowGotwfItemPreview:_set_amount_text(text)
	local widget = self._top_widgets_by_name.amount_text
	widget.content.text = text
end

function HeroWindowGotwfItemPreview:_update_info_text_alignment()
...
function HeroWindowGotwfItemPreview:cb_unit_spawned_item_preview(item_previewer, item_key)
	local ignore_spin = true
	item_previewer:present_item(item_key, ignore_spin)
	self._fadeout_loading_overlay = true
end

function HeroWindowGotwfItemPreview:_spawn_hero_skin(world_previewer, hero_name, career_index, optional_skin)
...
function HeroWindowGotwfItemPreview:_spawn_hero_with_hat(world_previewer, hero_name, career_index, optional_skin, item_name)
	local callback = callback(self, "cb_hero_unit_spawned_hat_preview", world_previewer, hero_name, career_index, item_name)
	world_previewer:request_spawn_hero_unit(hero_name, career_index, false, callback, 1, nil, optional_skin)
end

function HeroWindowGotwfItemPreview:cb_hero_unit_spawned_skin_preview(world_previewer, hero_name, career_index)
...
function HeroWindowGotwfItemPreview:cb_hero_unit_spawned_hat_preview(world_previewer, hero_name, career_index, hat_item_name)
	local profile_index = FindProfileIndex(hero_name)
	local profile = SPProfiles [profile_index]
	local careers = profile.careers
	local career_settings = careers [career_index]
...
function HeroWindowGotwfItemPreview:_get_can_wield_display_text(can_wield)
	local hero_text = ""
	local career_text = ""
...
function HeroWindowGotwfItemPreview:_get_hero_wield_info_by_item(item)
...
function HeroWindowGotwfItemPreview:_sync_layout_path()
	local parent = self._parent
...
function HeroWindowGotwfItemPreview:_update_title_edge_animation(dt)
