...
local Gui_rect = Gui.rect local Gui_text = Gui.text local V2 = Vector2 local V3 = Vector3 local Color = Color
local Mouse = Mouse
...
function ImguiFlamegraph:is_persistent()
	return false
end

function ImguiFlamegraph:clear_data()
	ImguiFlamegraph._root = { [false] = 0 }
	self:reset_zoom()
end
...
function ImguiFlamegraph:toggle_recording(bool)
	if bool == nil then bool = not self._recording end
...
function ImguiFlamegraph:toggle_rendering(bool)
	if bool == nil then bool = not self._rendering end
...
function ImguiFlamegraph:draw()
	local do_close = Imgui.begin_window("Flamegraph")

	local recording = Imgui.checkbox("Recording", self._recording)
	if recording ~= self._recording then self:toggle_recording(recording)
	end
	local rendering = Imgui.checkbox("Draw flamegraph", self._rendering)
	if rendering ~= self._rendering then self:toggle_rendering(rendering)
	end
	local invert = Imgui.checkbox("Invert", self._invert)
	if not recording and not next(ImguiFlamegraph._root, false) then self._invert = invert
	end
