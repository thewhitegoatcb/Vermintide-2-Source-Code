...
function TransientPackageLoader:init()
	self._unload_package_queue = { }
	self._load_package_queue = { }

	self._tracked_projectiles = { units = { }, refs = { } }
	self._tracked_units = { units = { }, refs = { } }
end

local rpcs = { "rpc_sync_transient_projectile_packages", "rpc_sync_transient_unit_packages", "rpc_sync_transient_ready" }





function TransientPackageLoader:register_rpcs(network_event_delegate)
	self.network_event_delegate = network_event_delegate
	network_event_delegate:register(self, unpack(rpcs))
end

function TransientPackageLoader:unregister_rpcs()
	self.network_event_delegate:unregister(self)
	self.network_event_delegate = nil
end


function TransientPackageLoader:network_context_created(lobby, server_peer_id, own_peer_id)
...
function TransientPackageLoader:network_context_destroyed()
...
function TransientPackageLoader:update()
	if self._should_unload_packages_t and self._should_unload_packages_t < Managers.time:time("game") then
		...
		package_name and (package_manager:num_references(package_name) > 1 or package_manager:can_unload(package_name)) then
...
function TransientPackageLoader:signal_in_game()
	self._should_unload_packages_t = Managers.time:time("game") + UNLOAD_GRACE_PERIOD
end

function TransientPackageLoader:unload_all_packages()
...
function TransientPackageLoader:loading_completed()
	return self._loaded
end

function TransientPackageLoader:add_projectile(unit)
...
function TransientPackageLoader:add_unit(unit, unit_name)
	add_ref(unit, unit_name or Unit_get_data(unit, "unit_name"), self._tracked_units)
end

function TransientPackageLoader:remove_projectile(unit)
	remove_ref(unit, self._tracked_projectiles)
end

function TransientPackageLoader:remove_unit(unit)
	remove_ref(unit, self._tracked_units)
end

function TransientPackageLoader:hot_join_sync(peer_id)
...
function TransientPackageLoader:rpc_sync_transient_projectile_packages(channel_id, projectile_unit_ids)
...
function TransientPackageLoader:rpc_sync_transient_unit_packages(channel_id, husk_unit_name_ids)
	local packages = self._load_package_queue
...
function TransientPackageLoader:rpc_sync_transient_ready(channel_id)
