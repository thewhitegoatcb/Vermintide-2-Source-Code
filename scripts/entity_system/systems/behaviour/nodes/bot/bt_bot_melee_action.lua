...
function BTBotMeleeAction:init(...)
...
function BTBotMeleeAction:enter(unit, blackboard, t)
	blackboard.node_timer = t
	blackboard.melee = { engage_update_time = 0, engage_position_set = false, engage_change_time = 0, engaging = false,
...
function BTBotMeleeAction:leave(unit, blackboard, t, reason, destroy)
	local input_ext = blackboard.input_extension
...
function BTBotMeleeAction:run(unit, blackboard, t, dt)
...
function BTBotMeleeAction:_update_engage_position(unit, target_unit, blackboard, t, melee_range)
...
local DEFAULT_ATTACK_META_DATA = {
	tap_attack = { arc = 0, penetrating = false,

		max_range = DEFAULT_MAXIMAL_MELEE_RANGE },


	hold_attack = { arc = 2, penetrating = true,
...
function BTBotMeleeAction:_choose_attack(blackboard, target_unit)
...
function BTBotMeleeAction:_is_in_melee_range(current_position, aim_position, melee_range, attack_input, t, blackboard, target_unit)
	local target_locomotion_extension = ScriptUnit.has_extension(target_unit, "locomotion_system")
...
function BTBotMeleeAction:_is_in_engage_range(self_unit, target_unit, nav_world, action_data, follow_pos)
...
function BTBotMeleeAction:_aim_position(target_unit, blackboard)
	local node = 0
...
function BTBotMeleeAction:_target_unit_position(self_position, target_unit, nav_world)
...
function BTBotMeleeAction:_allow_engage(self_unit, target_unit, blackboard, action_data, already_engaged, aim_position, follow_pos)
	...
	if
	in_total_darkness and target_unit ~= blackboard.breakable_object and target_unit ~= priority_target and

	target_unit ~= blackboard.urgent_target_enemy and
	target_unit ~= blackboard.opportunity_target_enemy and
	not blackboard.aggressive_mode and
	not blackboard.target_ally_needs_aid then
...
function BTBotMeleeAction:_calculate_melee_range(target_unit, attack_meta_data)
...
function BTBotMeleeAction:_update_melee(unit, blackboard, dt, t)
...
local DEFAULT_DEFENSE_META_DATA = { push = "medium" }



function BTBotMeleeAction:_defend(unit, blackboard, target_unit, input_ext, t, in_melee_range)
	if self:_is_attacking_me(unit, target_unit) then
		...
		local breed = Unit.get_data(target_unit, "breed")
		if
		not in_melee_range or not breed or breed.boss or
		breed.armor_category == 2 and push_type ~= "heavy" or
		push_type == "light" and num_enemies > 2 or low_stamina then
...
function BTBotMeleeAction:_time_to_next_attack(attack_input, blackboard, t)
...
function BTBotMeleeAction:_attack(attack_input, blackboard)
	local weapon_extension = AiUtils.get_bot_weapon_extension(blackboard)
	if weapon_extension then
		local wielded_item_template = blackboard.wielded_item_template
		local weapon_meta_data = wielded_item_template.attack_meta_data or DEFAULT_ATTACK_META_DATA
...
function BTBotMeleeAction:_should_stop_attack_on_leave(blackboard)
	local weapon_extension = AiUtils.get_bot_weapon_extension(blackboard)
	if weapon_extension then
...
function BTBotMeleeAction:_stop_attack(blackboard)
	local weapon_extension = AiUtils.get_bot_weapon_extension(blackboard)
	if weapon_extension then
...
function BTBotMeleeAction:_clear_pending_attack(blackboard)
	local weapon_extension = AiUtils.get_bot_weapon_extension(blackboard)
	if weapon_extension then
...
function BTBotMeleeAction:_is_starting_attack(blackboard)
	local weapon_extension = AiUtils.get_bot_weapon_extension(blackboard)
	if weapon_extension then
...
function BTBotMeleeAction:_evaluation_timer(blackboard, t, timer_value)
...
function BTBotMeleeAction:_disengage(unit, t, blackboard, position)
...
function BTBotMeleeAction:_engage(t, blackboard)
	local bb = blackboard.melee
	bb.engaging = true
	bb.engage_change_time = t
end
...
function BTBotMeleeAction:_debug_draw_melee_range(unit, target_unit, blackboard, current_position, aim_position, attack_input, attack_meta_data, attack_performed, t)
