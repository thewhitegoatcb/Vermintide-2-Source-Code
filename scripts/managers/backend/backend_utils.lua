...
local placeholder_icon_textures = { melee = "icons_placeholder_melee_01", ranged = "icons_placeholder_ranged_01", hat = "icons_placeholder_hat_01", trinket = "icons_placeholder_trinket_01" }
...
function BackendUtils.get_loadout_item_id(career_name, slot_name)
...
function BackendUtils.set_loadout_item(backend_id, career_name, slot_name)
	local loadout_interface = Managers.backend:get_loadout_interface_by_slot(slot_name)
	if loadout_interface then
...
function BackendUtils.get_loadout_item(career_name, slot)
...
function BackendUtils.try_set_loadout_item(career_name, slot_name, item_key)
	local backend_items = Managers.backend:get_interface("items")
...
function BackendUtils.get_item_from_masterlist(backend_id)
	local backend_items = Managers.backend:get_interface("items")
...
function BackendUtils.get_hero_power_level_from_level(profile_name)
...
function BackendUtils.get_hero_power_level(profile_name)
	local settings = PowerLevelFromLevelSettings
	local experience = ExperienceSettings.get_experience(profile_name)
	local level = ExperienceSettings.get_level(experience)
...
function BackendUtils.get_average_item_power_level(career_name)
	local backend_items = Managers.backend:get_interface("items")
...
function BackendUtils.get_total_power_level(profile_name, career_name, optional_game_mode_key)
...
function BackendUtils.get_item_template(item_data, backend_id)
	local backend_items = Managers.backend:get_interface("items")
...
function BackendUtils.get_item_units(item_data, backend_id, skin, career_name)
...
function BackendUtils.format_profile_hash(hash, num_chars, block_length, block_divider)
	if not hash then
		return "n/a"
	end
...
function BackendUtils.calculate_weave_score(tier, score, career_name)
...
function BackendUtils.convert_weave_score(weave_score)
...
function BackendUtils.commit_load_time_data(load_time_data)
