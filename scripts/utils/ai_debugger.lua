...
function AIDebugger:init(world, nav_world, group_blackboard, is_server, free_flight_manager)
...
function AIDebugger:lazy_create_drawer()
	if self.drawer then return end
	self.drawer = Managers.state.debug:drawer({ mode = "immediate", name = "AIDebugger" })
end

function AIDebugger:destroy()
	return end

function AIDebugger:update(t, dt)
	...
	if DebugKeyHandler.key_pressed("c", "spawn bot player", "ai debugger", nil, "FreeFlight") then --[[ Nothing --]]
	end
...
function AIDebugger:perlin_path(t, x, y, xsize, ysize)
	local resx, resy = Application.resolution()
	local layer = 500
	local line_width = 3
	local colors = { Color(255, 30, 240, 70), Color(255, 130, 40, 170), Color(255, 130, 240, 70), Color(255, 0, 40, 170), Color(255, 230, 40, 230) }
...
function AIDebugger:update_selection(input, dt)
...
function AIDebugger:update_ingame_selection(in_free_flight)
	if not Unit.alive(self.active_unit) then
		self.active_unit = nil
	end
	...
	if ( select_target or DebugKeyHandler.key_pressed("v", "select bot", "ai debugger") ) and
	self:closest_unit_in_aim_dir(in_free_flight) then
...
function AIDebugger:closest_unit_in_aim_dir(in_free_flight)
	if in_free_flight then
		return true
	end
...
function AIDebugger:mouse_raycast(input)
	...
	local hit, hit_position, distance, normal, actor = PhysicsWorld.immediate_raycast(
	physics_world,
	position,
	raycast_dir,
	100,
...
local damage_direction = { z = -1, x = 0, y = 0 }
function AIDebugger:update_mouse_input(input)
...
function AIDebugger:draw_nearby_navmesh(ai_unit)
	if not self.show_navmesh then return
	end
	local drawer = self.drawer
...
function AIDebugger:draw_blackboard(ai_unit)
	if not self.show_blackboard then return
	end
	local gui = self.screen_gui
	local blackboard = BLACKBOARDS [ai_unit]

	local key_dest_root = { } local value_dest_root = { }
	local key_dest_subtree = { } local value_dest_subtree = { }
...
function AIDebugger:draw_behavior_tree(ai_unit, t, dt)
	if not self.show_behavior_tree then return
	end
...
function AIDebugger:draw_reticule()
	do return end
...
function AIDebugger:debug_player_intensity(t, dt)
	local xcol = { Color(200, 160, 145, 0), Color(200, 90, 150, 170), Color(200, 10, 200, 100), Color(200, 190, 50, 190) }
	local gui = self.screen_gui
	...
	local bar_width = 0.15 local bar_height = 0.02 local wedge = 0.0025
	...
	elseif pacing:ignore_pacing_intensity_decay_delay() then
		...
		if status_extension then --[[ Nothing --]]
		end
	end
...
function AIDebugger:debug_pacing(t, dt)
	local gui = self.screen_gui
	local cm = Managers.state.conflict

	local res_x, res_y = Application.resolution()
	local text_height = 0.02
	local width = 0.3 local height = 0.2 local wedge = 0.0025
	local win_x = 0.45
	local win_y = 0.01
	local row = win_y
...
function AIDebugger:draw_hint(t)
	if script_data and script_data.disable_debug_draw then
		return
	end

	if hint_shown then return end
	local gui = self.screen_gui
	local res_x, res_y = Application.resolution()
...
function AIDebugger:create_fake_players()
	local player_manager = Managers.player
	local player = player_manager:player_from_peer_id(Network.peer_id())
	local player_unit = player.player_unit
...
function AIDebugger:fake_players()
	return self._fake_players
end


function AIDebugger:draw_extensions(ai_unit)
	if not self.show_extensions then return
	end
