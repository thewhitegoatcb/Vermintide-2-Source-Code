...
function PlayerHuskAttachmentExtension:init(extension_init_context, unit, extension_init_data)
...
function PlayerHuskAttachmentExtension:extensions_ready(world, unit)
...
function PlayerHuskAttachmentExtension:destroy()
...
function PlayerHuskAttachmentExtension:update(unit, input, dt, context, t)
	return end

function PlayerHuskAttachmentExtension:hot_join_sync(sender)
	AttachmentUtils.hot_join_sync(sender, self._unit, self._attachments.slots, self._synced_slot_buffs)
end

function PlayerHuskAttachmentExtension:create_attachment(slot_name, item_data)
...
function PlayerHuskAttachmentExtension:remove_attachment(slot_name)
...
function PlayerHuskAttachmentExtension:attachments()
	return self._attachments
end

function PlayerHuskAttachmentExtension:get_slot_data(slot_id)
	local attachments = self._attachments
...
function PlayerHuskAttachmentExtension:show_attachments(show)
...
function PlayerHuskAttachmentExtension:_show_attachment(slot_name, slot_data, show)
...
function PlayerHuskAttachmentExtension:_apply_buffs(buffs, slot_name)
...
function PlayerHuskAttachmentExtension:_remove_buffs(slot_name)
	local buff_extension = ScriptUnit.extension(self._unit, "buff_system")
...
function PlayerHuskAttachmentExtension:set_buffs_to_slot(slot_name, buffs)
