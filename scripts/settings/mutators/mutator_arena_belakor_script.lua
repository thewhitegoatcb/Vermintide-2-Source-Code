...
local BaseStates = { }
BaseStates.tower = { mission_name = "arena_belakor_overload_statue",
	...
	setup = function (state_template, data)
		if data.is_server then
			...
			for locus_unit, extension in pairs(locus_entities) do
				data.active_locus [#data.active_locus + 1] = { locus_unit, extension }
			end
		end
	end,
...
ArenaStates = {
	...
	approaching_the_tower = { mission_name = "arena_belakor_go_tower", exit_volume_id = "trigger_approach_tower_done", id = 1,
		...
		on_server_enter = function (state_template, data)
			...
			volume_system:register_volume(exit_volume_id, "trigger_volume", { sub_type = "players_inside",
	...
	escape = { mission_name = "arena_belakor_escape", exit_volume_id = "trigger_escape_done", id = 4,
...
local shared_state_spec = {
	server = {
		...
		socketed_count = { default_value = 0, type = "number",
...
return { hide_from_player_ui = true,
	...
	client_start_function = function (context, data)
		...
		data.shared_state = SharedState:new(
		"mutator_arena_belakor_script",
		shared_state_spec,
		is_server,
		network_server,
		server_peer_id,
