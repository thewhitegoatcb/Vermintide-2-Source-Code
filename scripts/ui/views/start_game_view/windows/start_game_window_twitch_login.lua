...
function StartGameWindowTwitchLogin:on_enter(params, offset)
...
function StartGameWindowTwitchLogin:create_ui_elements(params, offset)
...
function StartGameWindowTwitchLogin:on_exit(params)
	print("[StartGameWindow] Exit Substate StartGameWindowTwitchLogin")
	self:set_active(false)
end

function StartGameWindowTwitchLogin:update(dt, t)
...
function StartGameWindowTwitchLogin:set_active(active, skip_block)
	self._active = active
	if IS_WINDOWS then
		if active then
...
function StartGameWindowTwitchLogin:_update_popup()
...
function StartGameWindowTwitchLogin:_handle_input(dt, t)
...
function StartGameWindowTwitchLogin:_update_game_options(dt, t)
	local is_connected = Managers.twitch:is_connected()
	local is_connecting = Managers.twitch:is_connecting()
...
function StartGameWindowTwitchLogin:cb_connection_success_callback(user_data)
	self:_set_disconnect_button_text()
end

function StartGameWindowTwitchLogin:_set_disconnect_button_text()
...
function StartGameWindowTwitchLogin:cb_on_message_received(key, message_type, user_name, message, parameter)
	local chat_output_widget = self._widgets_by_name.chat_output_widget
	local chat_output_content = chat_output_widget.content
...
function StartGameWindowTwitchLogin:_is_button_pressed(widget)
...
function StartGameWindowTwitchLogin:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_enter
end

function StartGameWindowTwitchLogin:post_update(dt, t)
	return end

function StartGameWindowTwitchLogin:_update_animations(dt)
	self:_update_button_animations(dt)
end

function StartGameWindowTwitchLogin:_update_button_animations(dt)
...
function StartGameWindowTwitchLogin:_animate_button(widget, dt)
	local content = widget.content
...
function StartGameWindowTwitchLogin:draw(dt)
...
function StartGameWindowTwitchLogin:_play_sound(event)
