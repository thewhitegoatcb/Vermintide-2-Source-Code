return { description = "weaves_beasts_mutator_desc", display_name = "weaves_beasts_mutator_name", icon = "mutator_icon_beast_totems",
	...
	update_totems = function (context, data, dt, t)
		...
		if data.update_timer > 1 then
			...
			for ai_unit, buff_id in pairs(data.old_ai_units_inside) do
				if ( not data.ai_units_inside [ai_unit] or not HEALTH_ALIVE [ai_unit] ) and
				Unit.alive(ai_unit) and data.buff_system:has_server_controlled_buff(ai_unit, buff_id) then
	...
	server_start_function = function (context, data)
		...
		data.extension_init_data = { health_system = { damage_cap_per_hit = 1, health = 5 }, hit_reaction_system = { hit_reaction_template = "level_object" } }
