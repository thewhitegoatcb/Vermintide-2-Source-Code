...
function StateTitleScreen:on_enter(params)
	...
	if IS_CONSOLE then
		...
		if rawget(_G, "LobbyInternal") and LobbyInternal.network_initialized() and (
		IS_PS4 or Managers.account:offline_mode()) then
...
function StateTitleScreen:_demo_hack_state_managers()
	local STATE_TABLE = { }
	local FUNCTION_TABLE = { }
	local SYSTEM_TABLE = { }
	local mt = {
		__index = function () return SYSTEM_TABLE end }

	setmetatable(STATE_TABLE, mt)
	local mt = {
		__index = function () return FUNCTION_TABLE end }
...
function StateTitleScreen:_fade_out()
	if IS_XB1 then
...
function StateTitleScreen:_setup_leak_prevention()
...
function StateTitleScreen:_setup_world()
...
function StateTitleScreen:_init_input()
...
function StateTitleScreen:_init_ui()
...
function StateTitleScreen:_setup_state_machine()
	local loading_context = self.parent.loading_context
	if loading_context.skip_signin then
		loading_context.skip_signin = nil
		self._machine = GameStateMachine:new(self, StateTitleScreenMainMenu, { skip_signin = true, world = self._world, ui = self._title_start_ui, viewport = self._viewport, auto_start = self._auto_start, auto_sign_in = self._auto_sign_in }, true)
	else
		self._machine = GameStateMachine:new(self, StateTitleScreenMain, { world = self._world, ui = self._title_start_ui, viewport = self._viewport, auto_start = self._auto_start, auto_sign_in = self._auto_sign_in }, true)
	end
end

function StateTitleScreen:_init_popup_manager()
...
function StateTitleScreen:_init_chat_manager()
...
function StateTitleScreen:update(dt, t)
...
function StateTitleScreen:post_update(dt, t)
	self._machine:post_update(dt, t)
end

function StateTitleScreen:_next_state()
...
function StateTitleScreen:_handle_delayed_fade_in()
	if IS_XB1 and
	self._wait_for_xboxlive_teardown and
	not Managers.account:should_teardown_xboxlive() and
	not self._auto_start then
		Managers.transition:fade_out(1)
...
function StateTitleScreen:_update_play_go_progress(dt, t)
...
function StateTitleScreen:enter_attract_mode(enter)
	self._attract_mode_active = enter
end

function StateTitleScreen:_render(dt, render_only_background)

	return end

function StateTitleScreen:show_menu(show, force)
	self._title_start_ui:show_menu(show, force)
end

function StateTitleScreen:on_exit(application_shutdown)
