...
function PassiveAbilityNecromancerCharges:init(extension_init_context, unit, extension_init_data, ability_init_data)
	...
	self._ping_explosion_params = { source_attacker_unit = unit }
	self._dual_wield_params = { source_attacker_unit = unit }

	self._achv_staff_gandalf_data = { }
end

function PassiveAbilityNecromancerCharges:warm_up_skeletons(breeds)
...
function PassiveAbilityNecromancerCharges:extensions_ready(world, unit)
...
function PassiveAbilityNecromancerCharges:_on_talents_changed(unit, talent_extension)
...
function PassiveAbilityNecromancerCharges:_register_events()
	Managers.state.event:register(self, "on_talents_changed", "_on_talents_changed")
end

function PassiveAbilityNecromancerCharges:_unregister_events()
...
function PassiveAbilityNecromancerCharges:destroy()
...
function PassiveAbilityNecromancerCharges:update(dt, t)
...
function PassiveAbilityNecromancerCharges:spawn_pets(num_pets, template_name, breed_name)
	for i = 1, num_pets do
		self:spawn_pet(template_name, breed_name)
	end
end

function PassiveAbilityNecromancerCharges:_queue_pet(breed_name, position, position_mode, template_name)
...
function PassiveAbilityNecromancerCharges:store_buff_unit(buff_unit)
	self._buff_unit = buff_unit
end

function PassiveAbilityNecromancerCharges:is_ready()
...
function PassiveAbilityNecromancerCharges:rpc_necromancer_passive_spawn_pet(channel_id, template_id, breed_id, position, position_mode_id)
...
function PassiveAbilityNecromancerCharges:stragglify_pets(peer_id)
...
function PassiveAbilityNecromancerCharges:rpc_necromancer_passive_stragglify_pets(channel_id)
...
function PassiveAbilityNecromancerCharges:rpc_necromancer_respawn_all_pets(channel_id)
...
function PassiveAbilityNecromancerCharges:_update_pets_server()
	...
	if ( status_extension:is_dead() or status_extension:is_ready_for_assisted_respawn() ) and
	not self._was_dead then
		self._was_dead = true
...
function PassiveAbilityNecromancerCharges:invalid_spawn_position()
	return Vector3(0, 0, -500)
end

function PassiveAbilityNecromancerCharges:is_invalid_spawn_position(position)
	return not position or position [3] < -400
end

function PassiveAbilityNecromancerCharges:_spawn_pet_server(breed_name, position, position_mode, template_name)
	local commander_ext = self._commander_extension
	...
	local optional_data = { ignore_event_counter = true, ignore_breed_limits = true,
		side_id = side_id,
...
function PassiveAbilityNecromancerCharges:_kill_all_pets_server(is_destroy)
...
function PassiveAbilityNecromancerCharges:resummon_pet(controlled_unit)
...
local EMPTY_TABLE = { }
function PassiveAbilityNecromancerCharges:_gather_resummon_data(controlled_unit, template_name)
	if not self._is_server then
		return
	end
...
function PassiveAbilityNecromancerCharges:_extract_resummon_data(unit, template_name)
...
function PassiveAbilityNecromancerCharges:_remove_unit(controlled_unit)
	self._spawned_pets [controlled_unit] = nil
	self._commander_extension:remove_controlled_unit(controlled_unit)
end

function PassiveAbilityNecromancerCharges:add_pet_charge(removed_unit, override_duration)
...
function PassiveAbilityNecromancerCharges:consume_pet_charge(buff_id)
	assert(self._is_server, "[PassiveAbilityNecromancerCharges] Local only function")
...
function PassiveAbilityNecromancerCharges:_remove_pet_charges()
	assert(self._is_server, "[PassiveAbilityNecromancerCharges] Local only function")
...
function PassiveAbilityNecromancerCharges:_update_spawning(t)
...
function PassiveAbilityNecromancerCharges:_update_achievements(t)
...
function PassiveAbilityNecromancerCharges:achievement_staff_gandalf_trigger(target_unit, t, check_delay)
	self._achv_staff_gandalf_data [target_unit] = t + check_delay
end

function PassiveAbilityNecromancerCharges:_achievement_staff_gandalf_update(t)
