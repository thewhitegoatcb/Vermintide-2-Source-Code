...
local RPCS = { "rpc_request_profile", "rpc_request_profile_reply" }
...
ProfileRequester.REQUEST_RESULTS = { "success", "failure", success = 1, failure = 2 }
...
function ProfileRequester:init(is_server, network_server, profile_synchronizer)
...
function ProfileRequester:destroy()
	return end

function ProfileRequester:register_rpcs(network_event_delegate, network_transmit)
...
function ProfileRequester:unregister_rpcs()
...
function ProfileRequester:profile_is_specator(profile_index)
	return profile_index == FindProfileIndex("spectator")
end

function ProfileRequester:request_profile(peer_id, local_player_id, profile_name, career_name, force_respawn)
...
function ProfileRequester:_request_profile(peer_id, local_player_id, request_id, profile_index, career_index, force_respawn)
...
function ProfileRequester:_despawn_player_unit(player)
...
function ProfileRequester:update(dt)
	if self._despawning_player_unit and
	not Unit.alive(self._despawning_player_unit) then
		self._despawning_player_unit = nil
	end

end

function ProfileRequester:result()
	return self._request_result
end

function ProfileRequester:rpc_request_profile(channel_id, peer_id, local_player_id, request_id, profile_index, career_index, force_respawn)
...
function ProfileRequester:rpc_request_profile_reply(channel_id, local_player_id, request_id, profile_index, career_index, force_respawn, result_id)
