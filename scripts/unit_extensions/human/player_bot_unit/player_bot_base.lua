...
local SELF_UNIT = nil
local function dprint(...) if SELF_UNIT == script_data.debug_unit and script_data.ai_bots_debug then print(...) end end
local function dtext(...) if SELF_UNIT == script_data.debug_unit and script_data.ai_bots_debug then Debug.text(...) end end
...
function PlayerBotBase:init(extension_init_context, unit, extension_init_data)
	...
	self._blackboard = { target_ally_needs_aid = false, using_navigation_destination_override = false, is_passive = true,
	...
	local up = Vector3.up()
	local vortex_escape_directions = { { weight = 0.5,
			direction = Vector3Box(Quaternion.forward(Quaternion(up, -math.pi * 4 / 8))) }, { weight = 0.75,
			direction = Vector3Box(Quaternion.forward(Quaternion(up, -math.pi * 3 / 8))) }, { weight = 1,
			direction = Vector3Box(Quaternion.forward(Quaternion(up, -math.pi * 2 / 8))) }, { weight = 0.7,
			direction = Vector3Box(Quaternion.forward(Quaternion(up, -math.pi * 1 / 8))) }, { weight = 0.6,
			direction = Vector3Box(Quaternion.forward(Quaternion(up, math.pi * 0 / 8))) }, { weight = 0.7,
			direction = Vector3Box(Quaternion.forward(Quaternion(up, math.pi * 1 / 8))) }, { weight = 1,
			direction = Vector3Box(Quaternion.forward(Quaternion(up, math.pi * 2 / 8))) }, { weight = 0.75,
			direction = Vector3Box(Quaternion.forward(Quaternion(up, math.pi * 3 / 8))) }, { weight = 0.5,
	...
	self._last_health_pickup_attempt = { blacklist = false, distance = 0, index = 1, path_failed = false,
	...
	self._last_mule_pickup_attempt = { blacklist = false, distance = 0, index = 1, path_failed = false,
...
function PlayerBotBase:ranged_attack_started(attacking_unit, victim_unit, attack_type)
...
function PlayerBotBase:ranged_attack_ended(attacking_unit, victim_unit, attack_type)
	local blackboard = self._blackboard
	if attack_type == "ratling_gun_fire" then
		local targets = blackboard.taking_cover.threats
...
function PlayerBotBase:hit_by_projectile(attacking_unit)
	local blackboard = self._blackboard
	blackboard.hit_by_projectile [attacking_unit] = self._t
end

local DEFAULT_STAY_NEAR_PLAYER_RANGE = 5
function PlayerBotBase:set_stay_near_player(stay_near_player, max_range)
...
function PlayerBotBase:should_stay_near_player()
	return self._stay_near_player, self._stay_near_player_range
end

function PlayerBotBase:set_seen_by_player(seen, player_unit, t)
...
function PlayerBotBase:extensions_ready(world, unit)
	local blackboard = self._blackboard
...
function PlayerBotBase:_init_brain()
	self._brain = AIBrain:new(self._world, self._unit, self._blackboard, self._bot_profile, self._bot_profile.behavior)
end

function PlayerBotBase:brain()
	return self._brain
end

function PlayerBotBase:profile()
	return self._bot_profile
end

function PlayerBotBase:blackboard()
	return self._blackboard
end

function PlayerBotBase:update(unit, input, dt, context, t)
...
function PlayerBotBase:_update_blackboard(dt, t)
...
function PlayerBotBase:_update_target_enemy(dt, t)
...
function PlayerBotBase:_update_proximity_target(dt, t, self_position)
	local blackboard = self._blackboard
	...
	if self._proximity_target_update_timer < t then
		...
		local num_hits = Broadphase.query(self._enemy_broadphase, search_position, check_range,
		BROADPHASE_QUERY_TEMP,
...
function PlayerBotBase:_target_valid(unit, enemy_offset)
...
function PlayerBotBase:_update_slot_target(dt, t, self_position)
	local bb = self._blackboard
	local unit = self._unit
...
function PlayerBotBase:_get_closest_target_in_slot(position, unit, current_enemy_unit, is_self)
...
function PlayerBotBase:_alter_target_position(nav_world, self_position, target_unit, target_position, reason)
...
function PlayerBotBase:_find_target_position_on_nav_mesh(nav_world, wanted_position)
	local above = 0.5 local below = 0.5
	local success, z = GwNavQueries.triangle_from_position(nav_world, wanted_position, above, below)
...
function PlayerBotBase:_update_target_ally(dt, t)
...
function PlayerBotBase:_player_needs_attention(self_unit, player_unit, blackboard, player_inventory_extension, player_locomotion_extension, t)
	...
	if stop_threshold < current_seen_time and not has_pickup_order then
		...
		do return "stop", extra_utility end
...
function PlayerBotBase:_calculate_healing_item_utility(permanent_health_percent, is_wounded, is_quick_use)
	if is_quick_use then
		do return 1 - (is_wounded and permanent_health_percent - 0.5 or permanent_health_percent) end
	else
		return 1 - (is_wounded and permanent_health_percent * 0.33 or permanent_health_percent)
	end
end

function PlayerBotBase:_select_ally_by_utility(unit, blackboard, breed, t)
...
function PlayerBotBase:within_aid_range(blackboard)
...
function PlayerBotBase:_update_liquid_escape()
	local unit = self._unit
	local blackboard = self._blackboard
	local status_extension = self._status_extension
...
function PlayerBotBase:_should_re_evaluate_vortex_escape(current_position, previous_check_position, navigation_extension, vortex_unit)
	local re_evaluate_destination = false local escape_completed = nil
...
function PlayerBotBase:_find_vortex_escape_destination(start_position, rotation, nav_world, traverse_logic, best_weighted_distance_sq, largest_weighted_distance_sq)
...
function PlayerBotBase:_update_vortex_escape()
	local unit = self._unit
	local blackboard = self._blackboard
	...
	local re_evaluate_destination = false local escape_completed = false
...
function PlayerBotBase:_update_attack_request(t)
	local blackboard = self._blackboard
...
function PlayerBotBase:_update_pickups(dt, t)
	local unit = self._unit
	local blackboard = self._blackboard
...
function PlayerBotBase:_update_interactables(dt, t)
	local blackboard = self._blackboard
...
function PlayerBotBase:_find_cover(take_cover_targets, self_pos, max_radius, allow_forward_distance)
...
function PlayerBotBase:_in_line_of_fire(self_unit, self_pos, take_cover_targets, taking_cover_from)
	local changed = false
...
function PlayerBotBase:cb_cover_point_path_result(hash, success, destination)
	if not success then
...
function PlayerBotBase:_update_cover(unit, self_pos, blackboard, cover_bb, follow_bb)
...
function PlayerBotBase:new_destination_distance_check(self_position, previous_destination, new_destination, navigation_extension)
	...
	if navigation_extension:destination_reached() then
		...
		do return ( previous_pos_offset_ok or destination_offset_ok ) and new_destination_offset_ok end
	else
...
function PlayerBotBase:_update_movement_target(dt, t)
	local unit = self._unit
	local self_pos = POSITION_LOOKUP [unit]
	local blackboard = self._blackboard
	...
	elseif override_vortex_escape or override_liquid_escape or cover_position or override_melee or override_ranged or override_ability then
		...
		if follow_bb.needs_target_position_refresh and (target_ally_has_moved_from_start_position or has_pickup_order) then
			...
			elseif unit_alive(blackboard.health_pickup) and blackboard.allowed_to_take_health_pickup and t < blackboard.health_pickup_valid_until and (
			self._last_health_pickup_attempt.unit ~= blackboard.health_pickup or not self._last_health_pickup_attempt.blacklist or
			health_slot_pickup_order_unit == blackboard.health_pickup) then
			...
			elseif unit_alive(blackboard.mule_pickup) and (self._last_mule_pickup_attempt.unit ~= blackboard.mule_pickup or not self._last_mule_pickup_attempt.blacklist or
			potion_slot_pickup_order_unit == blackboard.mule_pickup) then
			...
			if not target_position and unit_alive(blackboard.ammo_pickup) and blackboard.has_ammo_missing and t < blackboard.ammo_pickup_valid_until then
				...
				local above = 0.5 local below = 1.5 local lateral = INTERACT_RAY_DISTANCE - 0.3 local distance = 0
...
function PlayerBotBase:_find_pickup_position_on_navmesh(nav_world, self_pos, pickup_unit, pickup_attempt)
	local above = 1.5 local below = 2.2
	local STEP = 0.1
...
function PlayerBotBase:_find_position_on_navmesh(nav_world, original_position, offset_position, above, below, lateral, distance)
...
function PlayerBotBase:unit_removed_from_game()
	return end

function PlayerBotBase:destroy()
	self._brain:destroy()
...
function PlayerBotBase:_debug_draw_update(dt)
	...
	local drawer = Managers.state.debug:drawer({ mode = "immediate", name = drawer_name })
...
function PlayerBotBase:clear_failed_paths()
	table.clear(self._attempted_ally_paths)
	table.clear(self._attempted_enemy_paths)
end

function PlayerBotBase:cb_enemy_path_result(enemy_unit, success, destination, forced_callback)
	...
	if not path_status then
		path_status = { last_path_destination = Vector3Box() }
...
function PlayerBotBase:_enemy_path_allowed(enemy_unit)
...
function PlayerBotBase:cb_health_pickup_path_result(pickup_unit, success, destination, forced_callback)
	if forced_callback then
		return
	end
...
function PlayerBotBase:cb_mule_pickup_path_result(pickup_unit, success, destination, forced_callback)
	if forced_callback then
		return
	end
...
function PlayerBotBase:cb_ally_path_result(ally_unit, success, destination, forced_callback)
...
function PlayerBotBase:_ally_path_allowed(self_unit, ally_unit, t)
	...
	if path_status and path_status.failed then
		...
		if no_longer_ignored then
			...
			do return true, has_moved or forced_callback end
		else
...
local function find_chain_times(allowed_chain_actions)
	...
	local min = math.huge local max = -math.huge
	local min_idx = 1 local max_idx = 1
...
function PlayerBotBase:_update_weapon_metadata(template)
	...
	if metadata and not template._precalculated_metadata then
		...
		if used_actions.action_one then
			...
			local data_count = 0
			local armor_types = 6
			local light_attack_data = { total_chain_time = 0, armor_mods = { 0, 0, 0, 0, 0, 0 } }
			local heavy_attack_data = { total_chain_time = 0, armor_mods = { 0, 0, 0, 0, 0, 0 } }
...
function PlayerBotBase:_update_weapon_loadout_data()
	local blackboard = self._blackboard
...
function PlayerBotBase:_update_best_weapon()
	local blackboard = self._blackboard
...
function PlayerBotBase:_update_reload()
	local blackboard = self._blackboard
	local inventory = blackboard.inventory_extension
	local ranged_slot_data = inventory:get_slot_data("slot_ranged")
	...
	if wanted_slot_to_reload == nil then
		...
		if career_extension then
			...
			if
			career_name == "dr_engineer" and career_extension:current_ability_cooldown() > 0 and #blackboard.proximite_enemies == 0 then
