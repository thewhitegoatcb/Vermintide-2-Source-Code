...
local SOUND_EVENTS = { close_ui = "hud_morris_weapon_chest_close", button_hover = "hud_morris_hover", power_up_unlocked = "hud_morris_cursed_chest_activate_powerup", open_ui = "hud_morris_cursed_chest_open" }
...
function DeusCursedChestView:init(ingame_ui_context)
...
function DeusCursedChestView:destroy()
	return end

function DeusCursedChestView:on_enter(params)
	...
	if not self._power_up_data then
		...
		local seed = HashUtils.fnv32_hash(
		position.x .. "_" ..
		position.y .. "_" ..
		position.z)

		local power_ups = self._deus_run_controller:generate_random_power_ups(
		DeusPowerUpSettings.cursed_chest_choice_amount,
		...
		for i, power_up in ipairs(power_ups) do
			self._power_up_data [i] = { selected = false,
				power_up = power_up }
		end
...
function DeusCursedChestView:create_ui_elements()
	...
	for i = 1, num_power_ups do
		...
		widget.offset = {
			...
			init_pos_y - ( POWER_UP_SPACING_Y + widget_size [2] ) * i, 0 }
...
function DeusCursedChestView:post_update_on_enter()
	return end

function DeusCursedChestView:on_exit()
...
function DeusCursedChestView:post_update_on_exit()
	return end

function DeusCursedChestView:_init_power_up_widget(widget, power_up_instance, discount, current_value, max_value, profile_index, career_index)
...
function DeusCursedChestView:draw(dt)
...
function DeusCursedChestView:_get_selected_power_up_count()
	local counter = 0
...
function DeusCursedChestView:update(dt, t)
...
function DeusCursedChestView:_handle_input(dt)
	for _, power_up_data in ipairs(self._power_up_data) do
		local widget = power_up_data.widget
		...
		if self:_is_button_pressed(widget) then
			...
			run_controller:add_power_ups({ power_up_data.power_up }, REAL_PLAYER_LOCAL_ID)
			...
			Managers.state.event:trigger("present_rewards", { { type = "deus_power_up", power_up = power_up } })
...
function DeusCursedChestView:disable_toggle_menu()
	return true
end

function DeusCursedChestView:input_service()
	return self._input_manager:get_service(self._input_service_name)
end

function DeusCursedChestView:_close()
	self:_play_sound(SOUND_EVENTS.close_ui)
	self.ingame_ui:handle_transition("exit_menu")
end

function DeusCursedChestView:_acquire_input(ignore_cursor_stack)
	self:_release_input(true)
	local input_manager = self._input_manager
	local input_service_name = self._input_service_name
	input_manager:capture_input({ "keyboard", "gamepad", "mouse" }, 1, input_service_name, "DeusCursedChestView")
...
function DeusCursedChestView:_release_input(ignore_cursor_stack)
	local input_manager = self._input_manager
	input_manager:release_input({ "keyboard", "gamepad", "mouse" }, 1, self._input_service_name, "DeusCursedChestView")
	if not ignore_cursor_stack then
		ShowCursorStack.pop()
...
function DeusCursedChestView:_is_button_pressed(widget)
	local content = widget.content
...
function DeusCursedChestView:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function DeusCursedChestView:_update_button_hover_sound(widget)
	if self:_is_button_hovered(widget) then
		self:_play_sound(SOUND_EVENTS.button_hover)
	end
end

function DeusCursedChestView:_animate_power_up_widget(dt, widget)
	local content = widget.content
	local style = widget.style
...
function DeusCursedChestView:_play_sound(event)
	WwiseWorld.trigger_event(self._wwise_world, event)
end

function DeusCursedChestView:_update_background_animations(dt)
	local widgets_by_name = self._widgets_by_name
