...
BreedFreezerSettings = {
	freezer_pos = { 0, 0, -600 },
	freezer_offset = { 0, 0.05, 0.05 },
	freezer_pos_debug = { 0, 0, 10 },
	freezer_offset_debug = { 0, 2, 3 },
	freezer_size = { 0, 0, 0 },


	breeds = {
		skaven_clan_rat = { pool_size = 32 },


		skaven_slave = { pool_size = 50 },


		skaven_storm_vermin = { pool_size = 16 },


		skaven_plague_monk = { pool_size = 8 },


		chaos_marauder = { pool_size = 32 },


		chaos_fanatic = { pool_size = 50 },


		chaos_berzerker = { pool_size = 8 },


		chaos_raider = { pool_size = 8 },


		chaos_warrior = { pool_size = 6 },


		beastmen_ungor = { pool_size = 50 },


		beastmen_ungor_archer = { pool_size = 16 },


		beastmen_gor = { pool_size = 32 },


		beastmen_bestigor = { pool_size = 16 } },
...
function BreedFreezer:init(world, entity_manager, network_event_delegate, enemy_package_loader)
	...
	if network_event_delegate then
		...
		"rpc_breed_freeze_units", "rpc_breed_unfreeze_breed",
		"rpc_breed_freezer_hot_join", "rpc_breed_freezer_sync_breeds")
	end
...
function BreedFreezer:_setup_freezable_breeds(enemy_package_loader)
...
function BreedFreezer:_setup_freeze_box()
	local offset_z = 0
...
function BreedFreezer:_spawn_template_unit(world, unit_name, pos)
...
function BreedFreezer:try_mark_unit_for_freeze(breed, unit)
...
function BreedFreezer:rpc_breed_freeze_units(channel_id, unit_go_ids)
...
function BreedFreezer:commit_freezes()
...
function BreedFreezer:try_unfreeze_breed(breed, data)
	local breed_name = breed.name
	if self._breed_freezer_settings.breeds [breed_name] == nil then
		return nil
	end
...
function BreedFreezer:rpc_breed_unfreeze_breed(channel_id, breed_id, pos, rot, side_id, go_id)
...
function BreedFreezer:unfreeze_unit(unit, breed_name, data)
...
function BreedFreezer:rpc_breed_freezer_hot_join(channel_id, debug_breed_freeze)
...
function BreedFreezer:hot_join_sync(peer_id)
...
function BreedFreezer:rpc_breed_freezer_sync_breeds(channel_id, starts, unit_go_ids)
