...
function AILocomotionExtensionC:init(extension_init_context, unit, extension_init_data)
	self._unit = unit
...
function AILocomotionExtensionC:freeze()
	if self._engine_extension_id then
		EngineOptimizedExtensions.ai_locomotion_destroy_extension(self._engine_extension_id)
		self._engine_extension_id = nil
	end
...
function AILocomotionExtensionC:unfreeze(unit)
...
function AILocomotionExtensionC:hot_join_sync(sender)
...
function AILocomotionExtensionC:set_mover_displacement(displacement, duration)
	if self._engine_extension_id then
...
function AILocomotionExtensionC:teleport_to(position, rotation)
	if self._engine_extension_id then
...
function AILocomotionExtensionC:set_animation_driven(is_animation_driven, is_affected_by_gravity, script_driven_rotation, is_on_transport)
...
function AILocomotionExtensionC:set_animation_translation_scale(animation_translation_scale)
	if not self._engine_extension_id then
		return
	end
...
function AILocomotionExtensionC:set_animation_rotation_scale(animation_rotation_scale)
	if not self._engine_extension_id then
		return
	end
...
function AILocomotionExtensionC:set_wanted_velocity_flat(wanted_velocity)
	if not self._engine_extension_id then
		return
	end
...
function AILocomotionExtensionC:set_wanted_velocity(wanted_velocity)
	if not self._engine_extension_id then
		return
	end
...
function AILocomotionExtensionC:set_external_velocity(external_velocity)
	if not self._engine_extension_id then
		return
	end
...
function AILocomotionExtensionC:set_animation_external_velocity(external_velocity)
	if not self._engine_extension_id then
		return
	end
...
function AILocomotionExtensionC:set_wanted_rotation(rotation)
	if not self._engine_extension_id then
		return
	end
...
function AILocomotionExtensionC:use_lerp_rotation(active)
	if not self._engine_extension_id then
		return
	end
...
function AILocomotionExtensionC:set_rotation_speed(rotation_speed)
	if not self._engine_extension_id then
		return
	end
...
function AILocomotionExtensionC:set_rotation_speed_modifier(rotation_speed_modifier, rotation_speed_modifier_lerp_time, start_time)
	if not self._engine_extension_id then
		return
	end
...
function AILocomotionExtensionC:set_affected_by_gravity(affected_by_gravity)
	if not self._engine_extension_id then
		return
	end
...
function AILocomotionExtensionC:set_gravity(gravity)
	if not self._engine_extension_id then
		return
	end
...
function AILocomotionExtensionC:set_check_falling(state)
	if not self._engine_extension_id then
		return
	end
...
local movement_types = { script_driven = 0, snap_to_navmesh = 1, constrained_by_mover = 2, disabled = 3 }





function AILocomotionExtensionC:set_movement_type(movement_type, override_mover_move_distance, ignore_forced_mover_kill)
	if not self._engine_extension_id then
		return
	end
...
function AILocomotionExtensionC:current_velocity()
	if not self._engine_extension_id then
		return
	end
...
function AILocomotionExtensionC:is_falling()
	if not self._engine_extension_id then
		return
	end
...
function AILocomotionExtensionC:get_rotation_speed()
	if not self._engine_extension_id then
		return
	end
...
function AILocomotionExtensionC:get_rotation_speed_modifier()
	if not self._engine_extension_id then
		return
	end
...
function AILocomotionExtensionC:get_animation_rotation_scale()
	return self._animation_rotation_scale
end

function AILocomotionExtensionC:get_animation_translation_scale()
	return self._animation_translation_scale_box:unbox()
end


function AILocomotionExtensionC:set_disabled()
	if not self._engine_extension_id then
		return
	end
...
function AILocomotionExtensionC:set_mover_disable_reason(reason, state)
	MoverHelper.set_disable_reason(self._unit, self._mover_state, reason, state)
end


function AILocomotionExtensionC:set_collision_disabled(reason, state)
