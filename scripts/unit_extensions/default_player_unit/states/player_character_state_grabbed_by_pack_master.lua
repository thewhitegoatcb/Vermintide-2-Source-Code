...
function PlayerCharacterStateGrabbedByPackMaster:init(character_state_init_context)
...
function PlayerCharacterStateGrabbedByPackMaster:on_enter(unit, input, dt, context, t, previous_state, params)
	...
	self._pole = { pole_length = 2, apos = Vector3Box(start_pos), bpos = Vector3Box(target_pos) }
...
function PlayerCharacterStateGrabbedByPackMaster:on_exit(unit, input, dt, context, t, next_state)
	local first_person_extension = self.first_person_extension
	local status_extension = self.status_extension
...
function PlayerCharacterStateGrabbedByPackMaster:update(unit, input, dt, context, t)
	local csm = self.csm
