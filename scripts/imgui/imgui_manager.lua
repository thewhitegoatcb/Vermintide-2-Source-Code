...
ImguiKeymaps = { win32 = { toggle_imgui = { "keyboard", "f3", "pressed" } } }
...
function ImguiManager:_call_on_guis(func_name, ...)
	for _, category in pairs(self._guis_by_category) do
		for _, menu_item in pairs(category.list) do
			...
			if func then func(gui, ...)
			end
		end end
end

function ImguiManager:add_gui(gui_class, category_name, name, enabled)
	...
	table.insert(category_list, gui_index, { gui = gui, name = name, enabled = enabled })

	local keybind_settings = self._key_bindings [category_name] [name]
	if not keybind_settings then
		self._key_bindings [category_name] [name] = { id = 0, keybind = { } }
	end
...
function ImguiManager:set_open(bool)
...
function ImguiManager:update(t, dt)
...
function ImguiManager:post_update(t, dt)
	self:post_update_guis(t, dt)
end

function ImguiManager:post_update_guis(t, dt)
...
function ImguiManager:update_main_menu()
...
function ImguiManager:update_guis(t, dt)
	local prev_persistant = self._persistant_windows
	self._persistant_windows = 0
	local is_open = self._open
	for _, category in pairs(self._guis_by_category) do
...
function ImguiManager:on_round_start(...)
	return self:_call_on_guis("on_round_start", ...)
end

function ImguiManager:on_round_end(...)
	return self:_call_on_guis("on_round_end", ...)
end

function ImguiManager:on_venture_start(...)
	return self:_call_on_guis("on_venture_start", ...)
end

function ImguiManager:on_venture_end(...)
	return self:_call_on_guis("on_venture_end", ...)
end

function ImguiManager:_input_manager_do(method)
...
function ImguiManager:_set_gui_enabled(menu_item, enabled, capture_input)
	if enabled then
...
function ImguiManager:_update_keybinds()
...
function ImguiManager:_draw_keybind_settings()
...
function ImguiManager:_get_keybind_text(category_name, menu_item)
...
function ImguiManager:_save_settings()
	Development.set_setting("ImguiManager_keybinds", self._key_bindings)
	Application.save_user_settings()
end

function ImguiManager:_load_settings()
...
function ImguiManager:_capture_input()
...
function ImguiManager:_release_input()
