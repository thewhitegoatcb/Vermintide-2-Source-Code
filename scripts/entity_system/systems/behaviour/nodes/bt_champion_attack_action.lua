...
function BTChampionAttackAction:init(...)
...
function BTChampionAttackAction:enter(unit, blackboard, t)
...
function BTChampionAttackAction:_init_attack(unit, blackboard, action, t)
	...
	elseif action.mode == "nav_mesh_wave" then
		...
		if success then
			...
			local points = { Vector3Box(last_pos) }
			...
			blackboard.overlap_wave_points = { Vector3Box(wanted_start_pos) }
...
function BTChampionAttackAction:_attack_threat_over(unit, blackboard, action)
...
function BTChampionAttackAction:leave(unit, blackboard, t, reason, destroy)
	...
	elseif action.mode == "radial_cylinder" then --[[ Nothing --]]
	end
...
function BTChampionAttackAction:run(unit, blackboard, t, dt)
	local target_unit = blackboard.attacking_target
...
function BTChampionAttackAction:_next_in_sequence(blackboard, t, sequence_data, sequence_index)
	...
	if next_sequence_step then
		...
		blackboard.attack_next_sequence_ready = at and function (unit, blackboard, t) return at <= t - start_time end or next_sequence_step.ready_function
...
function BTChampionAttackAction:_update_rotation(unit, t, dt, blackboard)
...
local debug_drawer_info = { mode = "retained", name = "BTChampionAttackAction" }

function BTChampionAttackAction:_update_overlap(unit, blackboard, action, dt, t)
	...
	if start_t < t and last_t < end_t then
		...
		local hit_actors, actor_count = PhysicsWorld.immediate_overlap(pw,
		"position", oobb_pos,
		"rotation", rot,
		"size", size,
		"shape", "oobb",
		...
		if blackboard.overlap_walls_check_time < t then
			local above = 0.3 local below = 0.3
...
function BTChampionAttackAction:_update_nav_mesh_wave(unit, blackboard, action, dt, t)
	local world = blackboard.world
	local start_t = blackboard.overlap_start_time
	local end_t = blackboard.overlap_end_time
...
function BTChampionAttackAction:anim_cb_damage(unit, blackboard)
	local action = blackboard.action
	...
	elseif action.collision_type == "cylinder" then
		...
		"shape", shape,
		"types", "dynamics",
		"collision_filter", action.collision_filter or "filter_player_hit_box_check")
...
function BTChampionAttackAction:_update_radial_cylinder(unit, blackboard, action, dt, t)

	if blackboard.overlap_end_time < t and blackboard.attacking_target then
		self:_attack_threat_over(unit, blackboard, action)
		do return end
	...
	"shape", shape,
	"types", "dynamics",
	"collision_filter", action.collision_filter)
	...
	for i = 1, actor_count do
		...
		if hit_unit ~= unit and not blackboard.hit_players [hit_unit] then
			...
			local hit = min_angle < 0 and relative_angle < max_angle or
			min_angle > 0 and min_angle < relative_angle or
			max_angle > 0 and max_angle < relative_angle
...
function BTChampionAttackAction:_calculate_cylinder_collision(action, self_pos, self_rot)
...
function BTChampionAttackAction:_calculate_oobb_collision(action, self_pos, self_rot)
...
function BTChampionAttackAction:_deal_damage(unit, blackboard, action, self_pos, hit_actors, actor_count, is_animation_callback)
		...
		if is_a_character and Unit_alive(target_unit) and not hit_players [target_unit] and unit ~= target_unit then
			...
			if shove_speed and is_player_unit then
				...
				if is_animation_callback then
					...
					catapulted [#catapulted + 1] = { target_unit = target_unit, blocked = blocked, direction = Vector3Box(direction) }
				else
...
function BTChampionAttackAction:_catapult_players(unit, blackboard, action, catapulted_players)
	local shove_speed = action.shove_speed local shove_z_speed = action.shove_z_speed
...
function BTChampionAttackAction:_catapult_player(unit, shove_speed, shove_z_speed, target_unit, blocked, direction)
