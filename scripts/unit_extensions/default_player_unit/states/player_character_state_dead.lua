...
function PlayerCharacterStateDead:on_enter(unit, input, dt, context, t, previous_state, params)
...
function PlayerCharacterStateDead:on_exit(unit, input, dt, context, t, next_state)
	self.override_item_drop_position = nil
	self.override_item_drop_direction = nil
end

function PlayerCharacterStateDead:update(unit, input, dt, context, t)
	...
	local should_go_to_observer = not self.switched_to_observer_camera and (marked_for_despawn or self.dead_player_destroy_time < time_since_death + 1)
