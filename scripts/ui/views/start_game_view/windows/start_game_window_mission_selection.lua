...
function StartGameWindowMissionSelection:on_enter(params, offset)
...
function StartGameWindowMissionSelection:create_ui_elements(params, offset)
...
function StartGameWindowMissionSelection:_setup_levels_by_area(area_name)
...
function StartGameWindowMissionSelection:_setup_level_acts()
	local levels_by_act = { }
...
function StartGameWindowMissionSelection:_present_acts(acts)
...
function StartGameWindowMissionSelection:_setup_required_act_connections()
	local statistics_db = self.statistics_db
	local stats_id = self._stats_id
	local ui_scenegraph = self.ui_scenegraph
	...
	for i = 1, #assigned_widgets do
		...
		if required_acts then
			...
			local offset = widget.offset
			local pos_x = position [1] + offset [1] local pos_y = position [2] + offset [2]
			for j = 1, #required_acts do
				...
				for k = 1, #assigned_widgets do
					...
					if level_widget.content.level_key == required_level_key then
						...
						local level_pos_x = level_position [1] + level_offset [1] local level_pos_y = level_position [2] + level_offset [2]
...
function StartGameWindowMissionSelection:_get_last_level_in_act(act_key)
	local act_levels = GameActs [act_key]
	local best_level_id = nil local best_sort_order = 0
...
function StartGameWindowMissionSelection:_get_first_level_id()
...
function StartGameWindowMissionSelection:_is_level_presented(level_id)
	local active_node_widgets = self._active_node_widgets
	if active_node_widgets then
...
function StartGameWindowMissionSelection:_select_level(level_id)
	local active_node_widgets = self._active_node_widgets
	if active_node_widgets then
		for i = 1, #active_node_widgets do
			local widget = active_node_widgets [i]
...
function StartGameWindowMissionSelection:_set_presentation_info(level_id)
	local level_text = ""
...
function StartGameWindowMissionSelection:on_exit(params)
	print("[StartGameWindow] Exit Substate StartGameWindowMissionSelection")
	self.ui_animator = nil
	self.parent:set_input_description(nil)
end

function StartGameWindowMissionSelection:update(dt, t)
...
function StartGameWindowMissionSelection:post_update(dt, t)
	self:_handle_input(dt, t)
end

function StartGameWindowMissionSelection:_update_animations(dt)
...
function StartGameWindowMissionSelection:_is_button_pressed(widget)
	local content = widget.content
...
function StartGameWindowMissionSelection:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function StartGameWindowMissionSelection:_update_level_option()
...
function StartGameWindowMissionSelection:_handle_input(dt, t)
	local active_node_widgets = self._active_node_widgets
	if active_node_widgets then
		for i = 1, #active_node_widgets do
			local widget = active_node_widgets [i]
...
function StartGameWindowMissionSelection:draw(dt)
...
function StartGameWindowMissionSelection:_play_sound(event)
