...
local RPCS = { "rpc_to_client_spawn_player", "rpc_set_observed_player_id", "rpc_sync_loadout_slot" }





function PlayerManager:set_is_server(is_server, network_event_delegate, network_manager)
...
function PlayerManager:set_statistics_db(statistics_db)
	self._statistics_db = statistics_db
end

function PlayerManager:statistics_db()
	return self._statistics_db
end

function PlayerManager:player_loadouts()
	return self._player_loadouts
end

function PlayerManager:rpc_sync_loadout_slot(channel_id, peer_id, local_player_id, slot_id, item_id, rarity_id, power_level, buff_ids, buff_value_type_ids, buff_values)
...
function PlayerManager:rpc_to_client_spawn_player(channel_id, local_player_id, profile_index, career_index, position, rotation, is_initial_spawn, ammo_melee_percent_int, ammo_ranged_percent_int, ability_cooldown_percent_int, healthkit_id, potion_id, grenade_id, network_additional_items, network_buff_ids)
	...
	if player_unit == nil or not Unit.alive(player_unit) then
		...
		local position = Vector3Box(position) local rotation = QuaternionBox(rotation)
...
function PlayerManager:exit_ingame()
...
function PlayerManager:assign_unit_ownership(unit, player, is_player_unit)
	if script_data.network_debug_connections then
...
function PlayerManager:unit_destroy_callback(unit)
	self:relinquish_unit_ownership(unit)
end

function PlayerManager:unit_owner(unit)
...
function PlayerManager:player_from_stats_id(stats_id)
	return self:player_from_unique_id(stats_id)
end

function PlayerManager:player_from_game_object_id(game_object_id)
	for _, player in pairs(self._players) do
...
function PlayerManager:add_player(input_source, viewport_name, viewport_world_name, local_player_id)
	if script_data.network_debug_connections then
...
function PlayerManager:add_remote_player(peer_id, player_controlled, local_player_id, clan_tag, account_id)
	if script_data.network_debug_connections then
...
function PlayerManager:is_player_unit(unit)
...
function PlayerManager:_create_ui_id()
	self._ui_id_increment = self._ui_id_increment % 1000 + 1
	return self._ui_id_increment
end

function PlayerManager:add_bot_player(player_name, bot_player_peer_id, bot_profile_index, profile_index, career_index, local_player_id)
	local peer_id = Network.peer_id()
	local unique_id = PlayerUtils.unique_player_id(peer_id, local_player_id)
...
function PlayerManager:clear_all_players()
	for unique_id, player in pairs(self._players) do
		self:remove_player(player:network_id(), player:local_player_id())
	end
end

function PlayerManager:remove_all_players_from_peer(peer_id)
	local peer_table = self._players_by_peer [peer_id]
	if peer_table then
...
function PlayerManager:set_stats_backend(player)
...
function PlayerManager:player_from_peer_id(peer_id, local_player_id)
...
function PlayerManager:human_and_bot_players()
...
function PlayerManager:num_alive_allies(player)
...
function PlayerManager:server_player()
	local network_manager = Managers.state.network
	local network_transmit = network_manager.network_transmit
...
function PlayerManager:party_leader_player()
...
function PlayerManager:next_available_local_player_id(peer_id, profile_index)
	local i = 2
	local player_table = self._players_by_peer [peer_id]
...
function PlayerManager:local_human_player()
	return self._local_human_player
end

function PlayerManager:bots()
	local player_bots = { }
	for _, player in pairs(self._players) do
...
function PlayerManager:rpc_set_observed_player_id(sender, observer_player_go_id, player_to_observe_go_id)
