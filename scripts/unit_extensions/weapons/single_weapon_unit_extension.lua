...
function SingleWeaponUnitExtension:init(extension_init_context, unit, extension_init_data)
...
function SingleWeaponUnitExtension:extensions_ready(world, unit)
	return end


function SingleWeaponUnitExtension:has_current_action()
	return false
end

function SingleWeaponUnitExtension:change_state(state)
	self.state = state
	self.weapon_template [state](self.world, self.unit, self.owner_unit, self.data)
end

function SingleWeaponUnitExtension:destroy()

	self.weapon_template.destroy(self.world, self.unit, self.owner_unit, self.data)
end

function SingleWeaponUnitExtension:update(unit, input, dt, context, t)
	self.weapon_template.update(self.world, self.unit, self.owner_unit, self.data, t, dt)
end

function SingleWeaponUnitExtension:on_wield(hand_name)
	if self._weapon_wield then
		self:_weapon_wield(hand_name)
	end
end

function SingleWeaponUnitExtension:on_unwield(hand_name)
