...
function LiquidAreaDamageExtension:init(extension_init_context, unit, extension_init_data)
...
function LiquidAreaDamageExtension:ready()
...
function LiquidAreaDamageExtension:_set_active(real_index)
...
function LiquidAreaDamageExtension:stop_fx()
	if script_data.debug_liquid_system then
...
function LiquidAreaDamageExtension:_create_liquid(real_index, angle)
	local grid = self._grid
	...
	for index = 1, #dirs do
		...
		local new_i = i + dir [1] local new_j = j + dir [2] local new_k = k
	...
	local liquid = { full = false, amount = 0, neighbours = neighbours, position = Vector3Box(from), rotation = QuaternionBox(rotation), fx_id = fx_id, angle = angle or 0 }
	self._flow [real_index] = liquid
	self._inactive_flow [real_index] = liquid
end

function LiquidAreaDamageExtension:_find_point(position, above, below)
	local nav_world = self._nav_world
...
function LiquidAreaDamageExtension:destroy()
...
local remove_list = { }
local add_list = { }
local fill_list = { { index = 0, weight = 0, angle = 0 },
	{ index = 0, weight = 0, angle = 0 },
	{ index = 0, weight = 0, angle = 0 },
	{ index = 0, weight = 0, angle = 0 },
	{ index = 0, weight = 0, angle = 0 },
	{ index = 0, weight = 0, angle = 0 } }
...
function LiquidAreaDamageExtension:update(unit, input, dt, context, t)
...
function LiquidAreaDamageExtension:_add_buff_helper_function(unit, liquid_unit, buff_name, buff_condition, buff_system)
...
function LiquidAreaDamageExtension:_update_collision_detection(dt, t)
...
function LiquidAreaDamageExtension:_is_unit_colliding(grid, unit)
...
local buff_params = { }
function LiquidAreaDamageExtension:_pulse_damage()
	local remove_i = 0
...
function LiquidAreaDamageExtension:is_position_inside(position, nav_cost_map_table)
	local grid = self._grid
	local i, j, k = grid:find_index(position)
	if grid:is_out_of_bounds(i, j, k) then
		return false
...
function LiquidAreaDamageExtension:get_rim_nodes()
	return self._inactive_flow, false
end

function LiquidAreaDamageExtension:hot_join_sync(peer_id)
	local flow = self._flow
...
function LiquidAreaDamageExtension:get_source_attacker_unit()
