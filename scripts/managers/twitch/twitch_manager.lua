...
local RPCS = { "rpc_update_twitch_vote" }



TwitchManager = class(TwitchManager)

function TwitchManager:init()
...
local twitch_difficulty_override = { cataclysm = true, cataclysm_3 = true, hardest = true, cataclysm_2 = true }
...
function TwitchManager:game_mode_supported(game_mode, difficulty)
...
function TwitchManager:stream_type() return "twitch" end

function TwitchManager:_load_sound_bank()
...
function TwitchManager:_unload_sound_bank()
...
function TwitchManager:connect(twitch_user_name, optional_connection_failure_callback, optional_connection_success_callback, is_retry)
	...
	if IS_CONSOLE then
		headers = { "Content-Type", "application/json", "Client-ID",
			...
			self._twitch_settings.client_id, "Authorization",
	...
	self._rest_interface:get(url, self._headers, callback(self, "cb_on_user_info_received"), { "User Data", twitch_user_name }, options)
end

function TwitchManager:cb_on_user_info_received(success, code, headers, data, userdata)
	...
	if success then
		...
		if result_data then
			...
			elseif result_data.data and #result_data.data > 0 then
				...
				self._rest_interface:get(url, self._headers, callback(self, "cb_on_user_streams_received"), { "User Data", twitch_user_name }, options)
				do return end
			else
...
function TwitchManager:cb_request_twitch_access_token(token)
...
function TwitchManager:cb_on_user_streams_received(success, code, headers, data, userdata)
	self:_show_result_info(success, code, headers, data, userdata)
		...
		if result_data then
			...
			if stream and type(stream) == "table" then
				...
				if channel then
					...
					local settings = { port = 6667, allow_send = false, address = "irc.chat.twitch.tv",
...
function TwitchManager:cb_on_notify_connected(connected)
...
function TwitchManager:_show_result_info(success, code, headers, data, userdata)
...
function TwitchManager:is_connected()
	return self._connected
end

function TwitchManager:is_connecting()
	return self._connecting
end

function TwitchManager:user_name()
	return self._twitch_user_name
end

function TwitchManager:cb_game_session_disconnect()
	return end

function TwitchManager:cb_connection_error_callback(message)
...
function TwitchManager:add_game_object_id(game_object_id)
...
function TwitchManager:remove_game_object_id(game_object_id)
	local game = Managers.state.network and Managers.state.network:game()
	if game then
...
function TwitchManager:_update_game_object(vote_key, vote_data)
	local game = Managers.state.network and Managers.state.network:game()
	if game then
...
function TwitchManager:_register_networked_vote(game_object_id)
	local game = Managers.state.network and Managers.state.network:game()
	...
	self._votes [#self._votes + 1] = { activated = true,
		timer = time,
...
function TwitchManager:register_vote(time, vote_type, validation_func, vote_templates, show_vote_ui, cb)
	local network_manager = Managers.state.network
	...
	self._votes [#self._votes + 1] = { activated = false,
		timer = time,
		option_strings = option_strings,
...
function TwitchManager:unregister_vote(vote_key)
	local network_manager = Managers.state.network
...
function TwitchManager:_activate_next_vote()
...
function TwitchManager:get_vote_data(vote_key)
	return self._votes_lookup_table [vote_key]
end

function TwitchManager:on_client_message_received(vote_key, message_type, user_name, message, parameter)
...
function TwitchManager:rpc_update_twitch_vote(channel_id, twitch_rpc_type, vote_key, user_name, vote_index, vote_template_id)
...
function TwitchManager:rpc_add_client_twitch_vote(channel_id, vote_key, user_name, vote_index, vote_template_id)
	local vote_data = self._votes_lookup_table [vote_key]
	if not vote_data then
		Application.error("TwitchManager] Something went wrong. There is no vote with the vote_key: " .. tostring(vote_key))
		self:unregister_vote(vote_key)
...
function TwitchManager:rpc_finish_twitch_vote(channel_id, vote_key, user_name, vote_index, vote_template_id)
	local is_server = Managers.state.network and Managers.state.network.is_server
...
function TwitchManager:rpc_disconnected_from_twitch(channel_id, vote_key, user_name, vote_index, vote_template_id)
	self._loading_popup_message = "twitch_connection_failed"
end

function TwitchManager:get_twitch_popup_message()
	local message = self._loading_popup_message
	self._loading_popup_message = nil
	return message
end

function TwitchManager:on_message_received(vote_key, message_type, user_name, message, parameter)
	if string.find(message, "@") then
		return
	end
	...
	if not Development.parameter("twitch_allow_multiple_votes") and
	vote_data.user_names [user_name] then
		return
	end
...
function TwitchManager:disconnect()
...
function TwitchManager:update(dt, t)
...
function TwitchManager:_handle_popup()
...
function TwitchManager:_handle_disconnect_popup()
...
function TwitchManager:_update_vote_data(dt, t)
	local network_manager = Managers.state.network
	local is_server = network_manager and network_manager.is_server
	if is_server then
...
function TwitchManager:_handle_results(vote_results)
	...
	for i = 1, num_options do repeat
			...
			elseif option == best_option then
				...
				if random_winner == 1 then
					best_option_index = i
					best_option = option end end until true
	end
...
function TwitchManager:_valid_player_index(index)
...
function TwitchManager:_validate_data(dt, t)
	local current_vote = self._current_vote
	if current_vote then
...
function TwitchManager:is_activated()
	return self._activated
end

function TwitchManager:reset()
	self:destroy()
end

function TwitchManager:destroy()
...
function TwitchManager:activate_twitch_game_mode(network_event_delegate, game_mode_key)
...
function TwitchManager:debug_activate_twitch_game_mode()
...
function TwitchManager:deactivate_twitch_game_mode()
	if self._current_vote then
...
function TwitchManager:_update_twitch_game_mode(dt, t)
...
local debug_messages = { "default_vote_a_str", "default_vote_b_str", "default_vote_c_str", "default_vote_d_str", "default_vote_e_str" }
function TwitchManager:_update_debug_voting(dt)
...
function TwitchGameMode:init(parent)
...
function TwitchGameMode:update(dt, t)
...
function TwitchGameMode:_update_used_votes()
...
function TwitchGameMode:_clear_used_votes(force_clear)
	local used_vote_templates = self._used_vote_templates
...
function TwitchGameMode:_check_breed_package_loading(wanted_template, previous_template)
	...
	local request_success = true local replacement_breed_name = nil
	...
	elseif is_special then
		...
		local replacement_template = templates [replacement_breed_name]
		if not used_vote_templates [replacement_template.name] and (
		previous_template == nil or previous_template and previous_template.name ~= replacement_template.name) and
		wanted_template.name ~= replacement_template.name then
			override_template = replacement_template
		end
	end
...
function TwitchGameMode:_get_game_mode_whitelist()
	local game_mode_key = Managers.state.game_mode:game_mode_key()
	return TwitchVoteWhitelists [game_mode_key]
end

function TwitchGameMode:_in_whitelist(template_name)
	local game_mode_whitelist = self:_get_game_mode_whitelist()
...
function TwitchGameMode:_get_next_vote()
...
function TwitchGameMode:_next_multiple_choice_vote(template)
	local vote_templates = { }
	for i = 1, 5 do
...
function TwitchGameMode:_next_standard_vote(template_a)
	local used_vote_templates = self._used_vote_templates
...
function TwitchGameMode:_trigger_new_vote()
...
function TwitchGameMode:cb_on_vote_complete(current_vote)
...
function TwitchGameMode:destroy()
