...
function WorldMarkerExtension:init(extension_init_context, unit, extension_init_data)
...
function WorldMarkerExtension:extensions_ready()
	if self._extensions_ready then
		self:_extensions_ready()
	end
end

function WorldMarkerExtension:destroy()
...
function WorldMarkerExtension:add_marker(cb_func)
...
function WorldMarkerExtension:remove_marker()
	local id = self._id
	if id then
...
function WorldMarkerExtension:hot_join_sync(sender)
	if self._hot_join_sync then
		self:_hot_join_sync(sender)
	end
end

function WorldMarkerExtension:cb_add_marker(cb_func, id)
	self._id = id
