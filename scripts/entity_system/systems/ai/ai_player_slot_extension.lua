...
local SLOT_POSITION_CHECK_INDEX = { CHECK_LEFT = 0, CHECK_RIGHT = 2, CHECK_MIDDLE = 1 }
...
local SLOT_COLORS = { { "aqua_marine", "cadet_blue", "corn_flower_blue", "dodger_blue", "sky_blue", "midnight_blue", "medium_purple", "blue_violet", "dark_slate_blue" },

	{ "dark_green", "green", "lime", "light_green", "dark_sea_green", "spring_green", "sea_green", "medium_aqua_marine", "light_sea_green" },
	{ "maroon", "dark_red", "brown", "firebrick", "crimson", "red", "tomato", "coral", "indian_red", "light_coral" },
	{ "orange", "gold", "dark_golden_rod", "golden_rod", "pale_golden_rod", "dark_khaki", "khaki", "olive", "yellow" },
	{ "orange", "gold", "dark_golden_rod", "golden_rod", "pale_golden_rod", "dark_khaki", "khaki", "olive", "yellow" },
	{ "orange", "gold", "dark_golden_rod", "golden_rod", "pale_golden_rod", "dark_khaki", "khaki", "olive", "yellow" },
	{ "orange", "gold", "dark_golden_rod", "golden_rod", "pale_golden_rod", "dark_khaki", "khaki", "olive", "yellow" },
	{ "orange", "gold", "dark_golden_rod", "golden_rod", "pale_golden_rod", "dark_khaki", "khaki", "olive", "yellow" },
...
function AIPlayerSlotExtension:init(extension_init_context, unit, extension_init_data)
	self.unit = unit
	self.all_slots = { }
...
function AIPlayerSlotExtension:_create_target_slots(self_unit, color_index)
...
function AIPlayerSlotExtension:_update_assigned_player(player, unit, unique_id)
...
function AIPlayerSlotExtension:extensions_ready(world, unit)
...
function AIPlayerSlotExtension:cleanup_extension(unit, update_slots_ai_units, update_slots_ai_units_n, all_extensions)
	if self.slots then
...
function AIPlayerSlotExtension:update_total_slots_count(t)
	local all_slots = self.all_slots
	local num_slots = 0
...
function AIPlayerSlotExtension:update_disabled_slots_count(t)
	local all_slots = self.all_slots
...
function AIPlayerSlotExtension:update_slot_sound(t)
...
local function get_anchor_slot(slot_data)
	...
	for i = 1, total_slots_count do repeat
			local slot = target_slots [i]
			...
			if best_anchor_weight < slot_anchor_weight or slot_anchor_weight == best_anchor_weight and slot.index < best_slot.index then
				best_slot = slot
				best_anchor_weight = slot_anchor_weight end until true
	end
...
local function overlap_with_other_target_slot(slot, all_slot_proviers)
	...
	for i = 1, target_units_n do repeat
			...
			for j = 1, num_slot_types do
				...
				for k = 1, total_slots_count do repeat
						...
						if distance_squared < overlap_distance_sq then
							return target_slot end until true end end until true
	end
...
local function overlap_with_own_slots(slot)
	...
	for i = 1, num_slot_types do repeat
			...
			for j = 1, num_slots do repeat
					...
					if distance_squared < overlap_distance_sq then
						return slot_b end until true end until true
	end
...
local function offset_slot(locomotion_ext, slot_absolute_position, target_unit_position)
	...
	if Vector3_length(target_velocity) > 0.1 then
		...
		do return slot_offset_position end
	else
...
function AIPlayerSlotExtension:update_target_slots(t, all_slot_proviers, nav_world, traverse_logic)
...
function AIPlayerSlotExtension:_set_slot_absolute_position(slot, position)
...
function AIPlayerSlotExtension:_update_target_slots_status(all_slot_provier_units, nav_world, traverse_logic, outside_navmesh, t)
...
function AIPlayerSlotExtension:_update_target_slots_positions(all_slot_proviers, should_offset_slot, nav_world, traverse_logic, is_on_ladder, ladder_unit, bottom, top, target_outside_navmesh)
	...
	if target_outside_navmesh then
		below = SLOT_Z_MAX_DOWN above = SLOT_Z_MAX_UP
	else
		below = Z_MAX_DIFFERENCE_BELOW above = Z_MAX_DIFFERENCE_ABOVE
	end
...
function AIPlayerSlotExtension:_update_slot_position(slot, slot_position, should_offset_slot, nav_world, traverse_logic, above, below, target_outside_navmesh)
	...
	if position_on_navmesh then
		...
		do return true, position_on_navmesh end
	else
...
function AIPlayerSlotExtension:_update_anchor_slot_position(slot, should_offset_slot, nav_world, traverse_logic, above, below, target_outside_navmesh)
	local locomotion_ext = self._locomotion_ext
	local target_position = self.position:unbox()
	local ai_unit = slot.ai_unit
...
function AIPlayerSlotExtension:_update_target_slots_positions_on_ladder(all_slot_proviers, should_offset_slot, nav_world, traverse_logic, ladder_unit, bottom, top)
	local all_slots = self.all_slots
	...
	if not slot_queue_position_on_navmesh or not can_go then
		...
		if slot_template.restricted_queue_distance then
			...
			if min_wait_queue_distance_sq <= slot_queue_distance_from_target_sq then
				do return slot_queue_position, penalty_term end
			else
				...
				if fallback_queue_position_on_navmesh then
					do return fallback_queue_position_on_navmesh, 0 end
				else
					do return slot_queue_position, penalty_term end
				end
			end
...
function AIPlayerSlotExtension:debug_draw(drawer, t, nav_world, i_target)
	...
	for slot_type, slot_data in pairs(all_slots) do
		...
		for i = 1, target_slots_n do repeat
				local slot = target_slots [i]
				local anchor_slot = get_anchor_slot(slot_data)
				...
				if slot.absolute_position then
					...
					local category = "wait_slot_index_" .. slot_type .. "_" .. slot.index .. "_" .. i
					Managers.state.debug_text:clear_world_text(category) end until true
		end
	end
...
TEST_SLOT = 0
function AIPlayerSlotExtension:_get_best_slot(attacker_unit, slot_type, skip_slots_behind_target, avoid_slots_behind_overwhelmed_target)
...
function AIPlayerSlotExtension:_get_best_slot_to_wait_on(attacker_unit, slot_type, skip_slots_behind_target, avoid_slots_behind_overwhelmed_target, nav_world, t)
	local ai_unit_position = POSITION_LOOKUP [attacker_unit]
	local target_unit_position = self.position:unbox()
	local all_slots = self.all_slots
	local slot_data = all_slots [slot_type]
	...
	for i = 1, num_slot_types do
		...
		for j = 1, count do
			...
			if not slot_queue_position then --[[ Nothing --]]
...
function AIPlayerSlotExtension:_update_slot(slot, attacker_unit, nav_world, traverse_logic)
	check_to_release_slot(slot)
...
function AIPlayerSlotExtension:_assign_slot(slot, slot_consumer_ext)
...
function AIPlayerSlotExtension:request_best_slot(slot_consumer_ext, skip_slots_behind_target, avoid_slots_behind_overwhelmed_target, nav_world, traverse_logic, t)
...
function AIPlayerSlotExtension:get_destination(slot_consumer_ext, slot, in_queue, nav_world, t)
	local position = nil
...
function AIPlayerSlotExtension:free_slot(slot_consumer_ext, slot, in_queue)
