...
table.merge_recursive(BTConditions.ability_check_categories, {
	activate_ability = { we_thornsister = true } })
...
function BTConditions.can_activate.we_thornsister(blackboard)
	...
	if target_unit then
		...
		if wall_target_distance_sq <= wall_max_distance_sq and wall_target_distance_sq >= 4 then
			if is_smiter_ability then
				...
				if target_unit == blackboard.priority_target_enemy or
				target_unit == blackboard.urgent_target_enemy or
				target_unit == blackboard.opportunity_target_enemy or
				target_threat_value >= 8 then
