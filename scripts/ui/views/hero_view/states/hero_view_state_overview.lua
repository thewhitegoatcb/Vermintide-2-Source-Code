...
function HeroViewStateOverview:on_enter(params)
...
function HeroViewStateOverview:_setup_menu_layout(params)
...
function HeroViewStateOverview:can_add(window_layout_name)
...
function HeroViewStateOverview:create_ui_elements(params)
...
function HeroViewStateOverview:disable_player_world()
...
function HeroViewStateOverview:enable_player_world()
...
function HeroViewStateOverview:enable_ingame_overlay()
...
function HeroViewStateOverview:disable_ingame_overlay()
...
function HeroViewStateOverview:_initial_windows_setups(params)
...
function HeroViewStateOverview:window_input_service()
	return self._input_blocked and FAKE_INPUT_SERVICE or self:input_service()
end

function HeroViewStateOverview:change_profile(profile_index, career_index)
...
function HeroViewStateOverview:currently_selected_profile()
	return self.profile_index, self.career_index, self.hero_name
end

function HeroViewStateOverview:_close_window_at_index(window_index)
...
function HeroViewStateOverview:_change_window(window_index, window_name)
	local active_windows = self._active_windows
...
function HeroViewStateOverview:get_layout_name()
...
function HeroViewStateOverview:set_layout_by_name(name)
...
function HeroViewStateOverview:close_on_exit()
	return self._close_on_exit
end

function HeroViewStateOverview:set_layout(index)
...
function HeroViewStateOverview:set_window_input_focus(window_name)
...
function HeroViewStateOverview:get_selected_game_mode_index()
	return self._selected_game_mode_index
end

function HeroViewStateOverview:get_previous_selected_game_mode_index()
	return self._previous_selected_game_mode_index
end

function HeroViewStateOverview:_get_layout_setting(index)
	return self._window_layouts [index]
end

function HeroViewStateOverview:_windows_update(dt, t)
	local active_windows = self._active_windows
	for _, window in pairs(active_windows) do
		window:update(dt, t)
	end
...
function HeroViewStateOverview:_windows_post_update(dt, t)
	local active_windows = self._active_windows
	for _, window in pairs(active_windows) do
...
function HeroViewStateOverview:enable_widget(active_window_index, widget_name, enable)
	local active_windows = self._active_windows
...
function HeroViewStateOverview:transitioning()
...
function HeroViewStateOverview:_wanted_state()
	local new_state = self.parent:wanted_state()
	return new_state
end

function HeroViewStateOverview:wanted_menu_state()
	return self._wanted_menu_state
end

function HeroViewStateOverview:clear_wanted_menu_state()
	self._wanted_menu_state = nil
end

function HeroViewStateOverview:requested_screen_change_by_name(state)
	self._on_close_next_state = state
	self:close_menu()
end

function HeroViewStateOverview:on_exit(params)
...
function HeroViewStateOverview:_close_active_windows()
	local active_windows = self._active_windows
	local params = self._window_params
...
function HeroViewStateOverview:_update_transition_timer(dt)
...
function HeroViewStateOverview:input_service()
	return self.parent:input_service()
end

function HeroViewStateOverview:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function HeroViewStateOverview:is_friends_list_active()
	local friends_component_ui = self._friends_component_ui
...
function HeroViewStateOverview:_handle_friend_joining()
	local friends_component_ui = self._friends_component_ui
	if friends_component_ui then
...
function HeroViewStateOverview:_has_active_level_vote()
...
function HeroViewStateOverview:post_update(dt, t)
...
function HeroViewStateOverview:_update_animations(dt)
...
function HeroViewStateOverview:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_enter
end

function HeroViewStateOverview:_handle_input(dt, t)
...
function HeroViewStateOverview:close_menu(ignore_sound_on_close_menu)
...
function HeroViewStateOverview:draw(input_service, dt)
...
function HeroViewStateOverview:_is_button_pressed(widget)
	local content = widget.content
...
function HeroViewStateOverview:play_sound(event)
	self.parent:play_sound(event)
end

function HeroViewStateOverview:_start_transition_animation(key, animation_name)
	local params = {
		wwise_world = self.wwise_world,
...
function HeroViewStateOverview:set_auto_fill_rarity(rarity)
	self._auto_fill_rarity = rarity
end

function HeroViewStateOverview:get_auto_fill_rarity()
	local auto_fill_rarity = self._auto_fill_rarity
	self._auto_fill_rarity = nil
	return auto_fill_rarity
end

function HeroViewStateOverview:set_filter_selected(selected)
	self._filter_selected = selected
end

function HeroViewStateOverview:filter_selected()
	return self._filter_selected
end

function HeroViewStateOverview:set_filter_active(enable)
	self._filter_active = enable
end

function HeroViewStateOverview:filter_active()
	return self._filter_active
end

function HeroViewStateOverview:reset_filter()
	self._filter_reset = true
end

function HeroViewStateOverview:filter_reset()
...
function HeroViewStateOverview:disable_filter(disable)
	self._filter_disabled = disable
end

function HeroViewStateOverview:disable_search(disable)
	self._search_disabled = disable
end

function HeroViewStateOverview:filter_search_disabled()
	return self._filter_disabled, self._search_disabled
end

function HeroViewStateOverview:set_selected_items_backend_ids(backend_ids)
	self._selected_items_backend_ids = backend_ids
end

function HeroViewStateOverview:get_selected_items_backend_ids()
	return self._selected_items_backend_ids
end

function HeroViewStateOverview:set_pressed_item_backend_id(backend_id, is_drag_item)
	self._pressed_item_backend_id = backend_id
	self._pressed_item_by_drag = backend_id and is_drag_item or nil
end

function HeroViewStateOverview:get_disabled_backend_ids()
	return self._disabled_backend_ids
end

function HeroViewStateOverview:clear_disabled_backend_ids()
	self._disabled_backend_ids = { }
	self.disabled_backend_ids_sync_id = self.disabled_backend_ids_sync_id + 1
end

function HeroViewStateOverview:disabled_item_icon()
	return self._disabled_item_icon
end

function HeroViewStateOverview:set_disabled_item_icon(icon)
	self._disabled_item_icon = icon
end

function HeroViewStateOverview:set_disabled_backend_id(backend_id, is_disabled)
	if is_disabled then
...
function HeroViewStateOverview:get_pressed_item_backend_id()
	return self._pressed_item_backend_id, self._pressed_item_by_drag
end

function HeroViewStateOverview:get_inventory_grid()
	return self._current_inventory_grid
end

function HeroViewStateOverview:set_inventory_grid(inventory_grid)
	self._current_inventory_grid = inventory_grid
end

function HeroViewStateOverview:set_fullscreen_effect_enable_state(enabled)
...
function HeroViewStateOverview:block_input()
	self._input_blocked = true
end

function HeroViewStateOverview:unblock_input()
	self._input_blocked = false
end

function HeroViewStateOverview:input_blocked()
	return self._input_blocked
end

function HeroViewStateOverview:set_selected_craft_page(name)
	self._selected_craft_page_name = name
end

function HeroViewStateOverview:get_selected_craft_page()
	return self._selected_craft_page_name
end

function HeroViewStateOverview:set_craft_optional_item_filter(item_filter)
	self._craft_optional_item_filter = item_filter
end

function HeroViewStateOverview:get_craft_optional_item_filter()
	return self._craft_optional_item_filter
end

function HeroViewStateOverview:set_selected_loadout_slot_index(index)
	self._selected_loadout_slot_index = index
end

function HeroViewStateOverview:get_selected_loadout_slot_index()
	return self._selected_loadout_slot_index or 1
end

function HeroViewStateOverview:set_selected_cosmetic_slot_index(index)
	self._selected_cosmetic_slot_index = index
end

function HeroViewStateOverview:get_selected_cosmetic_slot_index()
	return self._selected_cosmetic_slot_index or 1
end

function HeroViewStateOverview:_set_loadout_item(item, strict_slot_name)
...
function HeroViewStateOverview:is_bot_career()
...
function HeroViewStateOverview:get_career_data()
	return self.profile_index, self.career_index
end

function HeroViewStateOverview:update_talent_sync()
	self.talent_sync_id = self.talent_sync_id + 1
end

function HeroViewStateOverview:update_skin_sync()
	self.skin_sync_id = self.skin_sync_id + 1
	self.ingame_ui:respawn()
end

function HeroViewStateOverview:unequip_item_in_slot(slot_type)
	local hero_name = self.hero_name
	local career_index = self.career_index
...
function HeroViewStateOverview:update_inventory_items()
	self.inventory_sync_id = self.inventory_sync_id + 1
end

function HeroViewStateOverview:_get_slot_by_type(slot_type)
