...
if script_data.honduras_demo or Development.parameter("attract_mode") then
	...
	for name, func in pairs(ChatGui) do
		ChatGuiNull [name] = function () return end
	end
end

ChatManager = class(ChatManager)

if not MESSAGE_TYPES then local MESSAGE_TYPES = {
...
local CHAT_VIEWS = { "All", "Channels", "Party", "Private" }
...
function ChatManager:update_ignore_list()
	self.peer_ignore_list = SaveData.chat_ignore_list or self.peer_ignore_list
end

function ChatManager:cb_encrypted_app_ticket_recieved(info)
	local password = nil
	...
	local irc_settings = { port = 6667, allow_send = true, channel_name = "#vermintide_se", address = "172.16.2.24" }
...
function ChatManager:cb_notify_connected(connected)
	if connected then
...
function ChatManager:cb_channel_msg_received(key, message_type, username, message, parameter)
...
function ChatManager:cb_private_msg_received(key, message_type, username, message, parameter)
	local message, link_data = self:check_meta(message, username, parameter)
	if message then
		Managers.chat:add_irc_message(message_type, username, message, parameter, link_data)
	end
...
function ChatManager:cb_system_msg_received(key, message_type, username, message, parameter)
	Managers.chat:add_irc_message(message_type, username, message, parameter)
end

function ChatManager:cb_join_msg_received(key, message_type, username, message, parameter)
	message = username .. " " .. message .. parameter
	Managers.chat:add_irc_message(message_type, username, message, parameter)
end

function ChatManager:cb_leave_msg_received(key, message_type, username, message, parameter)
	message = username .. " " .. message .. parameter
	Managers.chat:add_irc_message(message_type, username, message, parameter)
end

function ChatManager:cb_names_msg_received(key, message_type, username, message, parameter)
	Managers.chat:add_irc_message(message_type, username, message, parameter)
end

function ChatManager:check_meta(message, username, parameter)
	if string.find(message, "$LINK;") then
		...
		if end_index then
			...
			if lobby_data then
				do return new_message, { lobby_id = lobby_id } end
			else
				return new_message
			end
		end
...
function ChatManager:add_message_target(message_target, message_target_type, message_target_key)
...
function ChatManager:set_message_target_type(message_target_type)
...
function ChatManager:_verify_new_target(message_target, message_target_type)
...
function ChatManager:remove_message_target(message_target)
...
function ChatManager:current_message_target()
	return self.message_targets [self.current_message_target_index]
end

function ChatManager:gui_should_clear()
	local clear = self.clear_messages
	self.clear_messages = nil
	return clear
end

function ChatManager:create_chat_gui()
...
function ChatManager:set_profile_synchronizer(profile_synchronizer)
...
function ChatManager:set_wwise_world(wwise_world)
...
function ChatManager:set_input_manager(input_manager)
...
function ChatManager:block_chat_input_for_one_frame()
...
function ChatManager:register_network_event_delegate(network_event_delegate)
	network_event_delegate:register(self, "rpc_chat_message")
	self.network_event_delegate = network_event_delegate
end

function ChatManager:unregister_network_event_delegate()
	self.network_event_delegate:unregister(self)
	self.network_event_delegate = nil
end

function ChatManager:setup_network_context(network_context)
...
function ChatManager:ignoring_peer_id(peer_id)
	return self.peer_ignore_list [peer_id]
end

function ChatManager:ignore_peer_id(peer_id)
...
function ChatManager:remove_ignore_peer_id(peer_id)
...
function ChatManager:set_font_size(font_size)
	if self.chat_gui then
		self.chat_gui:set_font_size(font_size)
	end
end

function ChatManager:set_chat_enabled(chat_enabled)
	self._chat_enabled = chat_enabled
end

function ChatManager:is_chat_enabled()
...
function ChatManager:chat_is_focused()
	return self.chat_gui.chat_focused
end

function ChatManager:enable_gui(enable)
	self.gui_enabled = enable
end

function ChatManager:update(dt, t, menu_active, menu_input_service, no_unblock)
...
function ChatManager:_get_localized_message(message, localize, localization_parameters, localize_parameters)
...
function ChatManager:_get_message_target(message_target)
...
function ChatManager:send_chat_message(channel_id, local_player_id, message, localize, localization_parameters, localize_parameters, recent_message_index, optional_message_target, optional_message_type, optional_message_target_key, sender_peer_id)
	...
	if message_type == Irc.PARTY_MSG or message_type == Irc.TEAM_MSG or message_type == Irc.ALL_MSG then
		if self.is_server then
			if sender_peer_id then peer_id = sender_peer_id or peer_id end
...
function ChatManager:send_system_chat_message(channel_id, message_id, localization_parameters, localize_parameters, pop_chat)
	fassert(self:has_channel(channel_id), "Haven't registered channel: %s", tostring(channel_id))
...
function ChatManager:add_local_system_message(channel_id, message, pop_chat)
...
function ChatManager:add_irc_message(message_type, username, message, parameter, context)
	local channel_id = 1

	local data = { username = username, message = message, parameter = parameter }
...
function ChatManager:rpc_chat_message(sender_channel_id, channel_id, message_sender, local_player_id, message, localization_parameters, localize, localize_parameters, is_system_message, pop_chat, is_dev, message_type)
...
function ChatManager:_profanity_check(message)
...
function ChatManager:_add_message_to_list(channel_id, message_sender, local_player_id, message, is_system_message, pop_chat, is_dev, message_type, link, data)
	if not IS_WINDOWS and
	not self._chat_enabled then
		return
	end
...
function ChatManager:get_chat_messages(destination_table, filter_name)
...
function ChatManager:_switch_view_internally(view_index)
...
function ChatManager:switch_view(view_index)
...
COMMAND_LUT = { ["/w"] = "send_message", ["/t"] = "send_message", ["/away"] = "away", ["/cls"] = "clear_chat", ["/reply"] = "reply", ["/clear"] = "clear_chat", ["/j"] = "join_channel", ["/leave"] = "leave", ["/invite"] = "game_invite", ["/msg"] = "send_message", ["/join"] = "join_channel", ["/send"] = "send_message", ["/r"] = "reply", ["/who"] = "who", ["/part"] = "leave" }
...
function ChatManager:_handle_command(message, recent_message_index, optional_message_target)
...
function ChatManager:join_channel(parameters)
...
function ChatManager:game_invite(parameters, message, recent_message_index, optional_message_target)
	if #parameters > 0 then
		...
		local link_data = { lobby_id = lobby_id }
...
function ChatManager:send_message(parameters, message, recent_message_index)
	if parameters [2] then
...
function ChatManager:leave(parameters)
...
function ChatManager:who(parameters)
	if parameters [2] and string.find(parameters [2], "#") == 1 then
		local channel_name = string.lower(parameters [2])
...
function ChatManager:reply(parameters, message)
...
function ChatManager:clear_chat()
	self.global_messages = { }
	self.chat_messages = { }
