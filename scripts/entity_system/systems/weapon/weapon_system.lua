...
local RPCS = { "rpc_attack_hit", "rpc_alert_enemy", "rpc_ai_weapon_shoot_start", "rpc_ai_weapon_shoot", "rpc_ai_weapon_shoot_end", "rpc_start_beam", "rpc_end_beam", "rpc_start_flamethrower", "rpc_end_flamethrower", "rpc_set_stormfiend_beam", "rpc_start_geiser", "rpc_end_geiser", "rpc_weapon_blood", "rpc_play_fx", "rpc_change_single_weapon_state", "rpc_summon_vortex", "rpc_start_soul_rip", "rpc_stop_soul_rip", "rpc_soul_rip_burst" }
...
local extensions = { "WeaponUnitExtension", "AiWeaponUnitExtension", "SingleWeaponUnitExtension" }





function WeaponSystem:init(entity_system_creation_context, system_name)
...
function WeaponSystem:rpc_alert_enemy(channel_id, alert_unit_id, attacker_unit_id)
...
local ARGS = { { default = 0, name = "power_level",
		min = MIN_POWER_LEVEL, max = MAX_POWER_LEVEL }, { default = 0, name = "hit_target_index" },
	{ default = 0, name = "boost_curve_multiplier" },
	{ default = false, name = "is_critical_strike" },
	{ default = true, name = "can_damage" },
	{ default = true, name = "can_stagger" },
	{ default = 1, name = "hit_ragdoll_actor" },
	{ default = false, name = "blocking" },
	{ default = false, name = "shield_break_procced" },
	{ default = 1, name = "backstab_multiplier" },
	{ default = false, name = "attacker_is_level_unit" },
	{ default = false, name = "first_hit" },
	{ default = 0, name = "total_hits" } }
...
function WeaponSystem:send_rpc_attack_hit(damage_source_id, attacker_unit_id, hit_unit_id, hit_zone_id, hit_position, attack_direction, damage_profile_id, ...)
...
function WeaponSystem:rpc_attack_hit(channel_id, damage_source_id, attacker_unit_id, hit_unit_id, hit_zone_id, hit_position, attack_direction, damage_profile_id, power_level, target_index, boost_curve_multiplier, is_critical_strike, can_damage, can_stagger, hit_ragdoll_actor_id, blocking, shield_break_procced, backstab_multiplier, attacker_is_level_unit, first_hit, total_hits)
	...
	power_level, damage_profile, target_index, boost_curve_multiplier, is_critical_strike, can_damage, can_stagger,
	blocking, shield_breaking_hit, backstab_multiplier, first_hit, total_hits)
...
function WeaponSystem:update(context, t)
...
function WeaponSystem:update_synced_beam_particle_effects()
	...
	for unit, data in pairs(self._beam_particle_effects) do
		...
		if not unit_id or not Unit.alive(weapon_unit) then
			...
			"dynamic_collision_filter", "filter_player_ray_projectile_ai_only",
...
local function ballistic_raycast(physics_world, max_steps, max_time, position, velocity, gravity, collision_filter, visualize)
	...
	for i = 1, max_steps do
		...
		local result, hit_position, hit_distance, normal, actor = PhysicsWorld.immediate_raycast(physics_world,
		position,
		direction,
		distance,
		"closest",
...
function WeaponSystem:update_synced_geiser_particle_effects(context, t)
	local game = self.game
	local network_manager = self.network_manager
	local world = self.world
...
function WeaponSystem:update_synced_flamethrower_particle_effects()
	local network_manager = self.network_manager
...
function WeaponSystem:rpc_ai_weapon_shoot_start(channel_id, owner_unit_id, shoot_time)
...
function WeaponSystem:rpc_ai_weapon_shoot(channel_id, owner_unit_id)
	local owner_unit = Managers.state.unit_storage:unit(owner_unit_id)
	if not owner_unit then
		return
	end
...
function WeaponSystem:rpc_ai_weapon_shoot_end(channel_id, owner_unit_id)
	local owner_unit = Managers.state.unit_storage:unit(owner_unit_id)
	if not owner_unit then
		return
	end
...
function WeaponSystem:rpc_change_single_weapon_state(channel_id, owner_unit_id, state_id)
	local owner_unit = Managers.state.unit_storage:unit(owner_unit_id)
	if not owner_unit then
		return
	end
...
function WeaponSystem:change_single_weapon_state(owner_unit, state, except_peer, received_via_network)
...
function WeaponSystem:rpc_start_beam(channel_id, unit_id, beam_effect_id, beam_end_effect_id, range)
...
function WeaponSystem:rpc_end_beam(channel_id, unit_id)
	if not LEVEL_EDITOR_TEST then
		local world = self.world
		local unit = self.unit_storage:unit(unit_id)
...
function WeaponSystem:rpc_start_geiser(channel_id, unit_id, geiser_effect_id, min_radius, max_radius, charge_time, angle)
	if not LEVEL_EDITOR_TEST then
...
function WeaponSystem:rpc_end_geiser(channel_id, unit_id)
	if not LEVEL_EDITOR_TEST then
		local world = self.world
		local unit = self.unit_storage:unit(unit_id)
...
function WeaponSystem:rpc_start_flamethrower(channel_id, unit_id, flamethrower_effect_id)
	if not LEVEL_EDITOR_TEST then
		local unit = self.unit_storage:unit(unit_id)
...
function WeaponSystem:rpc_end_flamethrower(channel_id, unit_id)
	if not LEVEL_EDITOR_TEST then
		local world = self.world
		local unit = self.unit_storage:unit(unit_id)
...
function WeaponSystem:rpc_summon_vortex(channel_id, owner_unit_id, target_unit_id)
	if not LEVEL_EDITOR_TEST then
		local world = self.world
...
function WeaponSystem:rpc_set_stormfiend_beam(channel_id, unit_id, arm_id, active)
	local unit = self.unit_storage:unit(unit_id)
	if ALIVE [unit] then
...
function WeaponSystem:rpc_weapon_blood(channel_id, attacker_unit_id, attack_template_damage_type_id)
	local attacker_unit = self.unit_storage:unit(attacker_unit_id)
...
function WeaponSystem:rpc_play_fx(channel_id, vfx_array, sfx_array, position_array)
	local world = self.world
...
function WeaponSystem:hot_join_sync(peer_id)
...
function WeaponSystem:start_soul_rip(owner_unit, target_unit, target_node_id, seed, net_sync)
	local world = self.world
...
function WeaponSystem:update_synced_soul_rip_particle_effects()
...
function WeaponSystem:stop_soul_rip(owner_unit, net_sync)
	local current_data = self._soul_rip_particle_effects [owner_unit]
	if current_data then
		self:cleanup_soul_rip(owner_unit)
...
function WeaponSystem:cleanup_soul_rip(owner_unit)
	local current_data = self._soul_rip_particle_effects [owner_unit]
	if current_data then
		local world = self.world
...
function WeaponSystem:soul_rip_burst(owner_unit, target_unit, target_node_id, fx_name, seed, net_sync)
	local world = self.world
...
function WeaponSystem:rpc_start_soul_rip(channel_id, owner_unit_id, target_unit_id, target_node_id, seed)
...
function WeaponSystem:rpc_stop_soul_rip(channel_id, owner_unit_id)
	local owner_unit = self.unit_storage:unit(owner_unit_id)
...
function WeaponSystem:rpc_soul_rip_burst(channel_id, owner_unit_id, target_unit_id, target_node_id, fx_name_id, seed)
	local target_unit = self.unit_storage:unit(target_unit_id)
	local owner_unit = self.unit_storage:unit(owner_unit_id)
...
function WeaponSystem:_update_chained_projectiles(t)
...
function WeaponSystem:is_chained_projectile_active(projectile)
	return not not self._chained_projectiles [projectile]
end

local BROADPHASE_QUERY_TEMP = { }
function WeaponSystem:_select_next_chained_projectile_target(chain_data, ai_broadphase)
	...
	for target_id = 1, num_enemies do
		...
		if not hit_units [target_unit] and HEALTH_ALIVE [target_unit] then
			...
			local mid_offset = Vector3(
			math.lerp(-0.5, 0.5, math.random()),
			math.lerp(-0.5, 0.5, math.random()),
			math.lerp(-0.5, 0.5, math.random()))
...
function WeaponSystem:_play_chained_projectile_fx(fx_name, fx_spline_ids, point_1, point_2, point_3)
	local fx_name_id = NetworkLookup.effects [fx_name]
	local spline_points = {
		point_1,
		point_2,
	...
	if self._fire_sound_event and
	self.first_person_extension then
...
function WeaponSystem:try_fire_chained_projectile(chain_hit_settings, damage_source, is_critical_strike, power_level, boost_curve_multiplier, t, owner_unit, source_pos, optional_target_unit, optional_ignore_unit, target_index)
	local chain_data = { chain_count = 0,
...
function WeaponSystem:_apply_chained_projectile_damage(chain_data)
	local settings = chain_data.settings
	...
	if HEALTH_ALIVE [target_unit] then
		...
		if is_level_unit then
			...
			self:send_rpc_attack_hit(damage_source_id,
			attacker_unit_id,
			target_unit_id,
			hit_zone_id,
			hit_position,
