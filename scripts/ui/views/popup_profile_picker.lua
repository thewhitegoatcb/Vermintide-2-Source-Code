...
function PopupProfilePicker:init(ingame_ui_context, ...)
...
function PopupProfilePicker:_create_ui_elements()
...
function PopupProfilePicker:update(dt, t)
...
PopupProfilePicker._INPUT_DEVICES = { "keyboard", "gamepad", "mouse" }

function PopupProfilePicker:show(current_profile_index, current_career_index, time_until_cancel, join_by_lobby_browser, _difficulty, lobby_data)
...
function PopupProfilePicker:hide()
	local input_manager = self._input_manager
...
function PopupProfilePicker:input_service()
	return self._input_manager:get_service("popup_profile_picker")
end

function PopupProfilePicker:draw(ui_top_renderer, input_service, dt)
...
function PopupProfilePicker:set_result(accepted, reason)
...
function PopupProfilePicker:query_result()
	return self._join_lobby_result
end

function PopupProfilePicker:destroy()
	return end

function PopupProfilePicker:_handle_input(dt, t)
...
function PopupProfilePicker:_handle_mouse_selection()
...
function PopupProfilePicker:_handle_gamepad_selection(input_service)
...
function PopupProfilePicker:_select_hero(profile_index, career_index, ignore_sound)
	local profile_settings = SPProfiles [profile_index]
...
function PopupProfilePicker:_set_hero_icon_selected(index)
	for icon_index, widget in ipairs(self._hero_icon_widgets) do
		widget.content.button_hotspot.is_selected = icon_index == index
	end
end

function PopupProfilePicker:_set_hero_info(hero_name, career_name, level)
	local widgets_by_name = self._widgets_by_name
...
function PopupProfilePicker:_set_timer_text(timer_text)
	local widgets_by_name = self._widgets_by_name
	widgets_by_name.timer_text.content.text = timer_text
end

function PopupProfilePicker:set_unavailable_heroes(occupied_heroes)
	self._occupied_heroes = occupied_heroes
	self:_update_occupied_profiles()
end

function PopupProfilePicker:set_difficulty(difficulty)
	self._difficulty = difficulty
end

function PopupProfilePicker:_update_occupied_profiles()
...
function PopupProfilePicker:_animate_element_by_time(target, target_index, from, to, time)
	local new_animation = UIAnimation.init(UIAnimation.function_by_time, target, target_index,
	from, to, time, math.ease_out_quad)
	return new_animation
end

function PopupProfilePicker:set_select_button_enable_state(enabled)
...
function PopupProfilePicker:_play_sound(event)
