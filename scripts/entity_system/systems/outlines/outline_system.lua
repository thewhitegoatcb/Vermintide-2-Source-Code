...
OutlineSystem.system_extensions = { "AIOutlineExtension", "PickupOutlineExtension", "PlayerHuskOutlineExtension", "PlayerOutlineExtension", "MinionOutlineExtension", "DoorOutlineExtension", "ObjectiveOutlineExtension", "ObjectiveLightOutlineExtension", "ElevatorOutlineExtension", "ConditionalInteractOutlineExtension", "ConditionalPickupOutlineExtension", "EnemyOutlineExtension", "GenericOutlineExtension", "SmallPickupOutlineExtension", "SmallDoorOutlineExtension",
...
function OutlineSystem:init(context, system_name)
...
OutlineSystem.add_ext_functions = {
	PlayerOutlineExtension = function (extension)
		extension:add_outline({ method = "never",
	...
	PlayerHuskOutlineExtension = function (extension)
		extension:add_outline({ method = "outside_distance_or_not_visible",
			outline_color = OutlineSettings.colors.ally,
		...
		function extension:update_override_method_player_setting()
	...
	MinionOutlineExtension = function (extension)
		...
		function extension:update_override_method_minion_setting()
			local user_outline_method = nil
	...
	PickupOutlineExtension = function (extension)
		extension:add_outline({ method = "within_distance_and_not_in_dark",
	...
	ObjectiveOutlineExtension = function (extension)
		extension:add_outline({ method = "within_distance",
			outline_color = OutlineSettings.colors.interactable,
	...
	ConditionalInteractOutlineExtension = function (extension)
		extension:add_outline({ method = "conditional_within_distance",
			outline_color = OutlineSettings.colors.interactable,
			distance = OutlineSettings.ranges.doors,
	...
	DarkPactPlayerHuskOutlineExtension = function (extension)
		...
		extension:add_outline({ method = "always_same_side",
...
function OutlineSystem:on_add_extension(world, unit, extension_name)
...
function OutlineSystem:on_freeze_extension(unit, extension_name)
...
function OutlineSystem:_cleanup_extension(unit, extension_name)
	local extension = self.unit_extension_data [unit]
...
function OutlineSystem:freeze(unit, extension_name, reason)
...
function OutlineSystem:unfreeze(unit)
...
function OutlineSystem:local_player_created(player)
	self._local_player = player
	self.camera_unit = player.camera_follow_unit
end

function OutlineSystem:_is_cutscene_active()
...
function OutlineSystem:_is_photomode_active()
	local game_mode = self._game_mode
	return game_mode and game_mode:photomode_enabled()
end

function OutlineSystem:set_disabled(disabled)
...
function OutlineSystem:update(context, t)
	...
	for i = 1, num_to_check_per_frame do
		...
		if extension then
			...
			local do_outline = false local slow_check = false
...
local PULSE_METHODS = {
	flash = function (t) return math.round(t * 3 % 1) end,
	pulse = function (t) return math.round(t * 3 % 1.5) end }



function OutlineSystem:set_pulsing(unit, enable, method)
	if enable then
...
function OutlineSystem:_update_pulsing(dt, t)
...
function OutlineSystem:outline_unit(unit, flag, channel, do_outline, apply_method)
...
function OutlineSystem:raycast_result(unit_center)
...
function OutlineSystem:distance_sq_to_unit(unit)
	local camera_position = Unit.local_position(self.camera_unit, 0)
...
function OutlineSystem:never(unit, extension)
	return false
end

function OutlineSystem:ai_alive(unit, extension)
	return not not HEALTH_ALIVE [unit]
end

function OutlineSystem:always(unit, extension)
	return true
end

function OutlineSystem:always_same_side(unit, extension)
...
function OutlineSystem:same_side_in_ghost_mode(unit, extension)
...
function OutlineSystem:visible(unit, extension)
	local pose, _ = Unit.box(unit)
	local unit_center = Matrix4x4.translation(pose)
...
function OutlineSystem:not_in_dark(unit, extension)
	local pose, _ = Unit.box(unit)
	local unit_center = Matrix4x4.translation(pose)
	local in_darkness = self.darkness_system:is_in_darkness(unit_center)
...
function OutlineSystem:not_visible(unit, extension)
	return not self:visible(unit, extension), IS_SLOW_CHECK
end

function OutlineSystem:within_distance_and_not_in_dark(unit, extension)
...
function OutlineSystem:within_distance(unit, extension)
	return self:distance_sq_to_unit(unit) <= extension.distance * extension.distance
end

function OutlineSystem:outside_distance(unit, extension)
	return self:distance_sq_to_unit(unit) > extension.distance * extension.distance
end

function OutlineSystem:outside_distance_or_not_visible(unit, extension)
...
function OutlineSystem:within_distance_and_visible(unit, extension)
	if self:within_distance(unit, extension) and
	self:visible(unit, extension) then
		return true, IS_SLOW_CHECK
	end
...
function OutlineSystem:conditional_within_distance(unit, extension)
