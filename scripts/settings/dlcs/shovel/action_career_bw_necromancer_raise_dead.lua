...
local MIN_ANGLE = math.degrees_to_radians(-60) local MAX_ANGLE = math.degrees_to_radians(60)

ActionCareerBWNecromancerRaiseDead = class(ActionCareerBWNecromancerRaiseDead, ActionBase)

function ActionCareerBWNecromancerRaiseDead:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionCareerBWNecromancerRaiseDead:client_owner_start_action(new_action, t, chain_action_data, power_level, action_init_data)
...
function ActionCareerBWNecromancerRaiseDead:_trigger_spawn()
	...
	if position then
		...
		local spline_points = { start_pos, mid_point, position }
...
function ActionCareerBWNecromancerRaiseDead:client_owner_post_update(dt, t, world, can_damage, current_time_in_action)
	self._ai_navigation_system:add_safe_navigation_callback(self._nav_callback)
end

function ActionCareerBWNecromancerRaiseDead:_update_spawning(t)
...
function ActionCareerBWNecromancerRaiseDead:_play_vo()
...
function ActionCareerBWNecromancerRaiseDead:_generate_position()
	local source_pos = self._position:unbox()

	local above = 1 local below = 3
