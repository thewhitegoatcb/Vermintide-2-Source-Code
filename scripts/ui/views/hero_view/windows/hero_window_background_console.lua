...
local object_sets_per_layout = {
	equipment = { equipment_view = true },


	talents = { talents_view = true },


	forge = { crafting_view = true },


	cosmetics = { cosmetics_view = true },


	crafting_recipe = { crafting_view = true },


	equipment_selection = { equipment_view = true },


	cosmetics_selection = { cosmetics_view = true },


	system = { main_menu = true },


	character_selection = { keep_current_object_set = true },


	item_customization = { keep_current_object_set = true } }
...
local level_events_per_layout = {
	...
	crafting_recipe = { "crafting_view" },


	equipment_selection = { "equipment_view" },


	cosmetics_selection = { "cosmetics_view" },


	system = { "main_menu" },


	character_selection = { "equipment_view", "main_menu", "cosmetics_view", "crafting_view" } }
...
local character_visibility_per_layout = { cosmetics_selection = true, equipment_selection = true, forge = false, cosmetics = true, system = false, equipment = true, character_selection = true, crafting_recipe = false, talents = false }
...
function HeroWindowBackgroundConsole:on_enter(params, offset)
...
function HeroWindowBackgroundConsole:_create_viewport_definition()
	return { scenegraph_id = "screen",
		element = UIElements.Viewport,
		style = {
			viewport = { layer = 960, viewport_name = "character_preview_viewport", clear_screen_on_create = true, level_name = "levels/ui_keep_menu/world", enable_sub_gui = false, fov = 50, world_name = "character_preview",
...
function HeroWindowBackgroundConsole:create_ui_elements(params, offset)
...
function HeroWindowBackgroundConsole:_setup_object_sets()
...
function HeroWindowBackgroundConsole:on_exit(params)
...
function HeroWindowBackgroundConsole:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function HeroWindowBackgroundConsole:_update_character_visibility(layout_name)
...
local EMPTY_TABLE = { }
function HeroWindowBackgroundConsole:_update_level_events(layout_name)
...
function HeroWindowBackgroundConsole:_update_object_sets(layout_name)
...
function HeroWindowBackgroundConsole:post_update(dt, t)
	...
	if not self.initialized and self._viewport_widget then
		...
		local function callback() self.hero_unit_spawned = true end
...
function HeroWindowBackgroundConsole:respawn_hero(optional_params, override_camera_movement)
...
function HeroWindowBackgroundConsole:despawn_hero()
	local world_previewer = self.world_previewer
	if not world_previewer then
		return
	end
...
function HeroWindowBackgroundConsole:_update_animations(dt)
...
function HeroWindowBackgroundConsole:_update_loadout_sync()
...
function HeroWindowBackgroundConsole:_update_skin_sync()
	local parent = self.parent
...
function HeroWindowBackgroundConsole:_update_wielded_slot()
	local parent = self.parent
...
function HeroWindowBackgroundConsole:_populate_loadout()
	local world_previewer = self.world_previewer
...
function HeroWindowBackgroundConsole:_is_button_pressed(widget)
...
function HeroWindowBackgroundConsole:_is_stepper_button_pressed(widget)
	local content = widget.content
...
function HeroWindowBackgroundConsole:_handle_input(input_service, dt, t)
	local widgets_by_name = self._widgets_by_name
	local detailed_widget = widgets_by_name.detailed

	if self._draw_character then --[[ Nothing --]]
	end
...
function HeroWindowBackgroundConsole:_exit(selected_level)
	self.exit = true
	self.exit_level_id = selected_level
end

function HeroWindowBackgroundConsole:draw(dt)
...
function HeroWindowBackgroundConsole:_play_sound(event)
	self.parent:play_sound(event)
end

function HeroWindowBackgroundConsole:_update_loading_overlay_fadeout_animation(dt)
...
function HeroWindowBackgroundConsole:_handle_statistics_pressed()
	local statistics_activate = self:_statistics_activate()
...
function HeroWindowBackgroundConsole:_statistics_activate()
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.detailed
	return widget.content.active
end

function HeroWindowBackgroundConsole:_activate_statistics()
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.detailed
...
function HeroWindowBackgroundConsole:_sync_statistics()
...
function HeroWindowBackgroundConsole:_deactivate_statistics()
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.detailed
...
function HeroWindowBackgroundConsole:_update_statistics_widget(input_service, dt)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.detailed
	...
	if
	gamepad_input_axis and Vector3.length(gamepad_input_axis) > 0.01 then
...
function HeroWindowBackgroundConsole:_populate_statistics(layout)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.detailed
...
function HeroWindowBackgroundConsole:_setup_tab_scrollbar(widget)
	local detailed_button_size = scenegraph_definition.detailed_button.size
	local detailed_list_size = scenegraph_definition.detailed_list.size
