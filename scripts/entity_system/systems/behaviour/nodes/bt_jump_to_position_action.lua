...
function BTJumpToPositionAction:init(...)
	BTJumpToPositionAction.super.init(self, ...)
end

BTJumpToPositionAction.name = "BTJumpToPositionAction"

function BTJumpToPositionAction:enter(unit, blackboard, t)
	local action = self._tree_node.action_data
	blackboard.action = action

	local drawer = Managers.state.debug:drawer({ mode = "retained", name = "BTJumpToPositionAction" })
	drawer:reset()
...
function BTJumpToPositionAction:leave(unit, blackboard, t, reason, destroy)
...
function BTJumpToPositionAction:run(unit, blackboard, t, dt)
	local navigation_extension = blackboard.navigation_extension
	local locomotion_extension = blackboard.locomotion_extension
	...
	Vector3.distance_squared(entrance_pos, unit_position) < 1 then
	...
	if blackboard.jump_state == "waiting_for_landing_finished" and
	blackboard.landing_finished then
