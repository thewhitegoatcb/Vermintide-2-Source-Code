...
function ObserverUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function ObserverUI:create_ui_elements()
...
function ObserverUI:get_player_camera_extension()
...
function ObserverUI:handle_observer_player_changed()
...
function ObserverUI:set_observer_player(player_id)
...
function ObserverUI:stop_draw_observer_ui()
...
function ObserverUI:update(dt, t)
	if RELOAD_UI then
		self:create_ui_elements()
	end
...
function ObserverUI:destroy()

	return end

function ObserverUI:set_visible(visible)
...
function ObserverUI:is_visible()
	return self._is_visible
end

function ObserverUI:update_follow_player_health_bar(peer_id)
...
function ObserverUI:on_player_health_changed(name, widget, health_percent)
	if not self.bar_animations_data then
		self.bar_animations_data = { }
	end
	local unit_frames_settings = UISettings.unit_frames

	if not self.bar_animations_data [name] then local widget_animation_data = {
			low_health_animation = UIAnimation.init(UIAnimation.pulse_animation,
			widget.style.hp_bar.color, 1, unit_frames_settings.low_health_animation_alpha_from,
			unit_frames_settings.low_health_animation_alpha_to, unit_frames_settings.low_health_animation_time) }
	end
...
function ObserverUI:on_num_grimoires_changed(name, widget, health_debuff_percent)
	if not self.bar_animations_data then
		self.bar_animations_data = { }
	end
...
function ObserverUI:update_health_animations(dt)
...
function ObserverUI:update_player_bar_animation(widget, bar, time, total_time, anim_start_health, anim_end_health, dt)
	time = time + dt
...
function ObserverUI:update_damage_highlight(widget, time, dt)
