...
function PickupUnitExtension:init(extension_init_context, unit, extension_init_data)
...
function PickupUnitExtension:extensions_ready()
	...
	if outline_extension then
		...
		if pickup_settings.outline_available_func then
			...
			if not available then
				outline_extension:update_outline({ method = "never" },
...
function PickupUnitExtension:update(unit, input, dt, context, t)
	return end

function PickupUnitExtension:hide()
	local unit = self.unit
...
function PickupUnitExtension:get_pickup_settings()
	return AllPickups [self.pickup_name]
end

function PickupUnitExtension:destroy()
...
function PickupUnitExtension:get_dropped_by_breed()
	return self.dropped_by_breed
end

function PickupUnitExtension:can_interact()
...
function PickupUnitExtension:set_physics_enabled(has_physics)
	local unit = self.unit
