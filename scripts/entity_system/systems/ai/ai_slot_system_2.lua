...
function AISlotSystem2:init(context, system_name)
...
function AISlotSystem2:destroy()
...
function AISlotSystem2:hot_join_sync(peer_id, player)
	return end

local AI_UPDATES_PER_FRAME = 1

function AISlotSystem2:do_slot_search(ai_unit, set)
...
function AISlotSystem2:ai_unit_have_slot(ai_unit)
	local ai_unit_extension = self.unit_extension_data [ai_unit]
...
function AISlotSystem2:ai_unit_have_wait_slot(ai_unit)
	local ai_unit_extension = self.unit_extension_data [ai_unit]
	if not ai_unit_extension then
		return false
	end
...
function AISlotSystem2:ai_unit_wait_slot_distance(ai_unit)
	local ai_unit_extension = self.unit_extension_data [ai_unit]
	if not ai_unit_extension then
		return math.huge
	end
...
function AISlotSystem2:ai_unit_slot_position(ai_unit)
	local ai_unit_extension = self.unit_extension_data [ai_unit]
	if not ai_unit_extension then
		return nil
	end
...
function AISlotSystem2:ai_unit_blocked_attack(ai_unit)
	local ai_unit_extension = self.unit_extension_data [ai_unit]
...
function AISlotSystem2:ai_unit_staggered(ai_unit)
	local ai_unit_extension = self.unit_extension_data [ai_unit]
...
function AISlotSystem2:get_target_unit_slot_data(target_unit, slot_type)
...
function AISlotSystem2:slots_count(unit, slot_type)
...
function AISlotSystem2:total_slots_count(unit, slot_type)
	local unit_extension = self.unit_extension_data [unit]
	slot_type = slot_type or DEFAULT_SLOT_TYPE
	local slot_data = unit_extension.all_slots [slot_type]
...
function AISlotSystem2:disabled_slots_count(unit, slot_type)
	local unit_extension = self.unit_extension_data [unit]
	slot_type = slot_type or DEFAULT_SLOT_TYPE
	local slot_data = unit_extension.all_slots [slot_type]
...
function AISlotSystem2:set_release_slot_lock(unit, release_slot_lock)
	local unit_extension = self.unit_extension_data [unit]
...
function AISlotSystem2:update_target_slots(t)
...
function AISlotSystem2:update_disabled_slots_count(t)
	local target_units = self.target_units
	for i = 1, #target_units do
		local target_unit_extension = target_units [i]
...
function AISlotSystem2:update_slot_sound(t)
	local target_units = self.target_units
...
function AISlotSystem2:update(context, t, dt)
...
function AISlotSystem2:update_slot_providers(t)
	local target_units = self.target_units
...
function AISlotSystem2:traverse_logic()
	return self._traverse_logic
end

function AISlotSystem2:update_slot_consumers(t)
	local nav_world = self.nav_world
...
function AISlotSystem2:physics_async_update(context, t)
	self.t = t
	local target_units = self.target_units
	local target_units_n = #target_units
	if target_units_n == 0 then
...
function AISlotSystem2:update_total_slots_count(t)
	local target_units = self.target_units
	local num_slots = 0
...
function AISlotSystem2:register_prioritized_ai_unit_update(unit)
	self.update_slots_ai_units_prioritized [unit] = true
end

function AISlotSystem2:prioritize_queued_units_on_slot(slot)
...
function AISlotSystem2:on_add_extension(world, unit, extension_name, extension_init_data)
	local extension = nil
...
function AISlotSystem2:on_remove_extension(unit, extension_name)
	self.frozen_unit_extension_data [unit] = nil
	self:_cleanup_extension(unit, extension_name)
	ScriptUnit.remove_extension(unit, self.NAME)
end

function AISlotSystem2:on_freeze_extension(unit, extension_name)
...
function AISlotSystem2:_cleanup_extension(unit, extension_name)
	local extension = self.unit_extension_data [unit]
...
function AISlotSystem2:freeze(unit, extension_name, reason)
...
function AISlotSystem2:unfreeze(unit)
...
function debug_draw_slots(unit_extension_data, nav_world, t)
	local drawer = Managers.state.debug:drawer({ mode = "immediate", name = "AISlotSystem2_immediate" })
...
function AISlotSystem2:set_allowed_layer(layer_name, allowed)
