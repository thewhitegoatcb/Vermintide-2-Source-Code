local definitions = local_require("scripts/ui/views/start_game_view/windows/definitions/start_game_window_event_settings_definitions")
...
function StartGameWindowEventSettings:on_enter(params, offset)
...
function StartGameWindowEventSettings:create_ui_elements(params, offset)
...
function StartGameWindowEventSettings:_setup_content_from_backend()
...
function StartGameWindowEventSettings:on_exit(params)
	print("[StartGameWindow] Exit Substate StartGameWindowEventSettings")
	self.ui_animator = nil
end

function StartGameWindowEventSettings:update(dt, t)
...
function StartGameWindowEventSettings:post_update(dt, t)
	return end

function StartGameWindowEventSettings:_update_animations(dt)
...
function StartGameWindowEventSettings:_is_button_released(widget)
...
function StartGameWindowEventSettings:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_enter
end

function StartGameWindowEventSettings:_is_button_hover_exit(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_exit
end

function StartGameWindowEventSettings:_handle_input(dt, t)
	local widgets_by_name = self._widgets_by_name
...
function StartGameWindowEventSettings:_play_sound(event)
	self.parent:play_sound(event)
end

function StartGameWindowEventSettings:_update_difficulty_option()
	local parent = self.parent
...
function StartGameWindowEventSettings:_set_difficulty_option(difficulty_key)
...
function StartGameWindowEventSettings:draw(dt)
...
function StartGameWindowEventSettings:_update_game_options_hover_effect()
	local widgets_by_name = self._widgets_by_name
...
function StartGameWindowEventSettings:_on_option_button_hover_enter(widget, index, instant)
...
function StartGameWindowEventSettings:_on_option_button_hover_exit(widget, index, instant)
...
function StartGameWindowEventSettings:_create_style_animation_enter(widget, target_value, style_id, widget_index, instant)
...
function StartGameWindowEventSettings:_create_style_animation_exit(widget, target_value, style_id, widget_index, instant)
	local widget_style = widget.style
	local pass_style = widget_style [style_id]
	if not pass_style then
		return
...
function StartGameWindowEventSettings:_animate_pulse(target, target_index, from, to, speed)
	local new_animation = UIAnimation.init(UIAnimation.pulse_animation, target, target_index, from, to, speed)
	return new_animation
end

function StartGameWindowEventSettings:_animate_element_by_time(target, target_index, from, to, time)
