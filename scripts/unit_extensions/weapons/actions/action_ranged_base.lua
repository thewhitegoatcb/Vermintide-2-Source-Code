...
function ActionRangedBase:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionRangedBase:client_owner_start_action(new_action, t, chain_action_data, power_level, action_init_data)
...
function ActionRangedBase:client_owner_post_update(dt, t, world, can_damage, current_time_in_action)
...
function ActionRangedBase:finish(reason)
...
function ActionRangedBase:_waiting_to_shoot(dt, t)
	if self._time_to_shoot <= t then
		self._state = "start_shooting"
	end
end

function ActionRangedBase:_start_shooting(t)
	local owner_unit = self.owner_unit
	...
	if current_action.fire_at_gaze_setting and eyetracking_extension and
	eyetracking_extension:get_is_feature_enabled("tobii_fire_at_gaze") then
...
function ActionRangedBase:_shooting(t, action_ended)
...
function ActionRangedBase:_finished_shooting(t)
...
function ActionRangedBase:shoot(num_shots_this_frame, shots_fired, num_shots_total)
...
function ActionRangedBase:reload(reason)
	local ammo_extension = self.ammo_extension
	...
	local current_action = self.current_action
	if current_action.reload_when_out_of_ammo and
	ammo_extension:ammo_count() == 0 and ammo_extension:can_reload() then
		local owner_unit = self.owner_unit
...
function ActionRangedBase:spawn_projectile(position, rotation)
	local current_action = self.current_action
...
function ActionRangedBase:fire_projectile(position, rotation)
	local owner_unit = self.owner_unit
	local current_action = self.current_action
	...
	ActionUtils.spawn_player_projectile(owner_unit,
	position,
	rotation,
	0,
	angle,
	...
	speed,
	self.item_name,
	lookup_data.item_template_name,
	lookup_data.action_name,
	lookup_data.sub_action_name,
	self._is_critical_strike,
	self._power_level)
end

function ActionRangedBase:fire_hitscan(position, direction, range)
	local result = nil
	if self.current_action.ray_against_large_hitbox then
		result = PhysicsWorld.immediate_raycast_actors(self.physics_world,
		position,
		direction,
		range,
		...
		"dynamic_collision_filter", "filter_enemy_trigger")
	else
		result = PhysicsWorld.immediate_raycast_actors(self.physics_world,
		position,
		direction,
...
function ActionRangedBase:proc_extra_shot(t)
...
function ActionRangedBase:gen_num_shots()
	local current_action = self.current_action
	local ammo_extension = self.ammo_extension
...
function ActionRangedBase:apply_shot_cost(t)
	self:_use_ammo()
	self:_add_overcharge()
end

function ActionRangedBase:_use_ammo()
	local ammo_extension = self.ammo_extension
...
function ActionRangedBase:_add_overcharge()
	local current_action = self.current_action
