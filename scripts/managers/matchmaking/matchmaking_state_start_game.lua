...
function MatchmakingStateStartGame:init(params)
...
function MatchmakingStateStartGame:on_enter(state_context)
...
DLCS_TO_CHECK = { }
ADDED_DLCS = { }
function MatchmakingStateStartGame:_verify_requirements()
...
function MatchmakingStateStartGame:_initiate_start_game()
...
function MatchmakingStateStartGame:update(dt, t)
...
function MatchmakingStateStartGame:_setup_lobby_data()
	...
	if quick_game or mission_id == nil then
		...
		elseif mechanism == "deus" then
			...
			local vote_data = { private_game = false, quick_game = true, strict_matchmaking = false,
	...
	if IS_XB1 then
		...
		local DIFFICULTY_LUT = { "easy", "normal", "hard", "harder", "hardest", "cataclysm", "cataclysm_2", "cataclysm_3" }
		...
		if matchmaking_type == "event" then
			matchmaking_types = { "event" }
		elseif mechanism == "weave" then
			if quick_game then
				ticket_mission_id = "weave_any"
				matchmaking_types = { "weave_quick_game" }
			else
				hopper_name = LobbyInternal.WEAVE_HOPPER_NAME
				matchmaking_types = { "weave", mission_id }
			end
		elseif mechanism == "deus" then
			matchmaking_types = { "deus_quick_game", "deus_custom_game" }
		else
			matchmaking_types = { "quick_game", "custom_game" }
		end
		...
		local ticket_params = { level = { ticket_mission_id }, matchmaking_types = matchmaking_types, difficulty = difficulty_id, powerlevel = powerlevel, strict_matchmaking = strict_matchmaking, profiles = profiles, network_hash = network_hash, weave_index = weave_index }
...
function MatchmakingStateStartGame:_start_game()
	...
	if game_server_lobby_client then
		...
		self.start_lobby_data = { lobby_client = game_server_lobby_client }
...
function MatchmakingStateStartGame:_capture_telemetry()
	local lobby_members = self._lobby:members()
	local members = lobby_members:get_members()
	local nr_friends = 0
...
function MatchmakingStateStartGame:_handle_verify_dlcs(dt, t)
...
function MatchmakingStateStartGame:_handle_results(verify_dlc_data)
	local is_done = true
	local success = true
...
function MatchmakingStateStartGame:rpc_matchmaking_verify_dlc_reply(channel_id, success)
	local peer_id = CHANNEL_TO_PEER_ID [channel_id]
	self._verify_dlc_data.results [peer_id] = success
end

local removed_peers = { }
function MatchmakingStateStartGame:_update_voter_list_by_active_peers(active_peers, voter_list, results)
...
local peers_local = { }
function MatchmakingStateStartGame:_active_peers()
