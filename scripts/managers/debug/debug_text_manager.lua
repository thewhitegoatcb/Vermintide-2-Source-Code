...
function DebugTextManager:init(world, gui, is_server, network_event_delegate)
	self._world = world
	self._gui = gui
...
function DebugTextManager:_update_unit_texts(viewport_name, dt)
...
function DebugTextManager:output_unit_text(text, text_size, unit, node_index, offset, time, category, color, viewport_name, floating, fade)
	if script_data and script_data.disable_debug_draw then
		return
	end
	...
	local new_text = { alpha = 255,
		id = Gui.text_3d(gui, text, font, text_size, material, tm, text_offset, 0, Color(color.x, color.y, color.z)),
		text = text,
		text_size = text_size,
		node_index = node_index,
		offset = { x = offset.x, y = offset.y, z = offset.z },
		text_offset = { x = text_offset.x, y = text_offset.y, z = text_offset.z },
		color = { r = color.x, g = color.y, b = color.z },
...
function DebugTextManager:output_world_text(text, text_size, position, time, category, color, viewport_name)
	if script_data and script_data.disable_debug_draw then
		return
	end
