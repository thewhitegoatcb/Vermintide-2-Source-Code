...
local RPCS = { "rpc_add_liquid_damage_blob", "rpc_area_damage", "rpc_create_explosion", "rpc_enable_area_damage", "rpc_update_liquid_damage_blob", "rpc_create_liquid_damage_area", "rpc_add_damage_wave_fx", "rpc_add_damage_blob_fx", "rpc_abort_damage_blob", "rpc_damage_wave_set_state", "rpc_create_damage_wave", "rpc_create_thornsister_push_wave", "rpc_necromancer_create_curse_weave", "rpc_necromancer_create_curse_area" }
...
local extensions = { "AreaDamageExtension", "TimedExplosionExtension", "LiquidAreaDamageExtension", "LiquidAreaDamageHuskExtension", "DamageWaveExtension", "DamageWaveHuskExtension", "DamageBlobExtension", "DamageBlobHuskExtension", "ProximityMineExtension" }
...
function AreaDamageSystem:init(entity_system_creation_context, system_name)
...
local LIQUID_EXTENSIONS = { "DamageBlobExtension", "DamageWaveExtension", "LiquidAreaDamageExtension" }





function AreaDamageSystem:on_add_extension(world, unit, extension_name, extension_init_data)
...
function AreaDamageSystem:on_remove_extension(unit, extension_name)
...
function AreaDamageSystem:destroy()
	self.network_event_delegate:unregister(self)
end

function AreaDamageSystem:update(context, t)
...
function AreaDamageSystem:create_explosion(attacker_unit, position, rotation, explosion_template_name, scale, damage_source, attacker_power_level, is_critical_strike, source_attacker_unit)
...
function AreaDamageSystem:enable_area_damage(unit, enable)
...
function AreaDamageSystem:is_position_in_liquid(position, nav_cost_map_table)
...
function AreaDamageSystem:_create_aoe_damage_buffer()
	local buffer_size = AOE_DAMAGE_RING_BUFFER_SIZE
	self._aoe_damage_ring_buffer = { write_index = 1, read_index = 1, size = 0, buffer = Script.new_array(buffer_size), max_size = buffer_size }
	for index = 1, buffer_size do
		self._aoe_damage_ring_buffer.buffer [index] = { radius = 0, radius_max = 0, max_damage_radius = 0, shield_blocked = false, do_damage = false, radius_min = 0, full_power_level = 0, hit_distance = 0, hit_zone_name = "n/a", actual_power_level = 0, damage_source = "n/a", explosion_template_name = "n/a", push_speed = 0,
...
function AreaDamageSystem:_update_aoe_damage_buffer()
...
function AreaDamageSystem:_damage_unit(aoe_damage_data)
	...
	if not is_immune then
		...
		if
		blackboard and radius < hit_distance and blackboard.shield_user then
		...
		if
		not blocking and breed and breed.hitbox_ragdoll_translation then
...
function AreaDamageSystem:rpc_area_damage(channel_id, go_id, position)
...
function AreaDamageSystem:rpc_create_explosion(channel_id, attacker_unit_id, attacker_is_level_unit, position, rotation, explosion_template_name_id, scale, damage_source_id, attacker_power_level, is_critical_strike, source_attacker_unit_id)
	if self.is_server then
...
function AreaDamageSystem:rpc_enable_area_damage(channel_id, level_index, enable)
...
function AreaDamageSystem:rpc_create_liquid_damage_area(channel_id, source_unit_go_id, position, flow_direction, liquid_template_id)
...
function AreaDamageSystem:rpc_add_liquid_damage_blob(channel_id, liquid_unit_id, blob_id, position, is_filled)
...
function AreaDamageSystem:rpc_update_liquid_damage_blob(channel_id, liquid_unit_id, blob_id, state)
	local unit = self.unit_storage:unit(liquid_unit_id)
	if not unit then
		return
	end
...
function AreaDamageSystem:rpc_damage_wave_set_state(channel_id, unit_id, state)
...
function AreaDamageSystem:_create_damage_wave(source_unit, position, damage_wave_template, unit_name, optional_extension_init_data)
...
function AreaDamageSystem:rpc_create_damage_wave(channel_id, source_unit_go_id, position, optional_target_position, damage_wave_template_id)
	fassert(self.is_server, "Error! Only the server should create Damage Waves!")
...
function AreaDamageSystem:rpc_create_thornsister_push_wave(channel_id, source_unit_go_id, position, optional_target_position, damage_wave_template_id, power_level, segments, wall_index)
...
function AreaDamageSystem:rpc_necromancer_create_curse_weave(channel_id, source_unit_id, position, wave_direction, num_instances)
...
function AreaDamageSystem:rpc_necromancer_create_curse_area(channel_id, source_unit_id, position, wave_direction, num_instances)
	local source_unit = self.unit_storage:unit(source_unit_id)
	if not source_unit then
		return
	end
...
function AreaDamageSystem:rpc_add_damage_wave_fx(channel_id, damage_wave_unit_id, position, rotation, fx_idx, name_index)
...
function AreaDamageSystem:rpc_add_damage_blob_fx(channel_id, damage_blob_unit_id, position, life_time_percentage)
...
function AreaDamageSystem:rpc_abort_damage_blob(channel_id, damage_blob_unit_id)
	local unit = self.unit_storage:unit(damage_blob_unit_id)
