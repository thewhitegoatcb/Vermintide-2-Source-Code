...
function PlayFabRequestQueue:init()
	self._queue = { }
...
function PlayFabRequestQueue:is_pending_request()
	return self._active_entry or #self._queue > 0
end


function PlayFabRequestQueue:enqueue(request, success_callback, send_eac_challenge, error_callback)
	local id = self._id + 1

	local parameters = request.FunctionParameter
	if not parameters then
		request.FunctionParameter = { metadata = self._metadata }
	else
		parameters.metadata = self._metadata
	end

	local entry = { resends = 0, eac_challenge_success = false, api_function_name = "ExecuteCloudScript",
...
function PlayFabRequestQueue:enqueue_api_request(api_function_name, request, success_callback, optional_error_callback)
	local id = self._id + 1

	local entry = { resends = 0, send_eac_challenge = false,
...
function PlayFabRequestQueue:_need_throttle(func_name, t)
...
function PlayFabRequestQueue:_update_throttling(t, entry)
	...
	if entry.send_eac_challenge and
	self:_need_throttle("generateChallenge", t) then
		return false
	end
...
function PlayFabRequestQueue:update(dt, t)
	...
	if entry.send_eac_challenge then
		...
		local generate_challenge_request = { FunctionName = "generateChallenge",
...
function PlayFabRequestQueue:eac_challenge_success_cb(result)
...
function PlayFabRequestQueue:_challenge_response_received(response)
	local entry = self._active_entry
...
function PlayFabRequestQueue:_send_request(entry)
...
function PlayFabRequestQueue:playfab_request_success_cb(success_callback, id, result)
	local entry = self._active_entry
	local function_result = result.FunctionResult
	...
	if script_data.testify then
		...
		if function_to_wait_for and function_to_wait_for == request.FunctionName then
			Testify:respond_to_request("wait_for_playfab_response", { request.FunctionName }, 1)
		end
	end
end

function PlayFabRequestQueue:playfab_request_error_cb(error_callback, id, result)
	local entry = self._active_entry
	local request = entry.request
	if not entry or id and id ~= entry.id then
...
function PlayFabRequestQueue:_get_eac_response(challenge)
	local i = 0
	local str = ""
