...
function BenchmarkHandler:init(ingame_ui, world)
	...
	if BenchmarkSettings.bot_power_level_override then
		function BackendUtils.get_total_power_level(backend_id, player_unit)
	...
	if BenchmarkSettings.bot_damage_multiplier then
		...
		function GenericHealthExtension:add_damage(attacker_unit, damage_amount, hit_zone_name, damage_type, damage_direction, damage_source_name, hit_ragdoll_actor, damaging_unit, hit_react_type)
	...
	function PlayerBotUnitFirstPerson.animation_event(s, event)
...
function BenchmarkHandler:story_spawn_and_animate_troll(element, t)
...
function BenchmarkHandler:story_destroy_close_units(element, t)
	local radius_squared = element.radius_squared or 900
	Managers.state.conflict:destroy_close_units(nil, nil, radius_squared)
end


function BenchmarkHandler:story_teleport_party(element, t)
...
function BenchmarkHandler:recycler_spawn_at(element, t)
...
function BenchmarkHandler:story_troll_sound(element, t)
	WwiseUtils.trigger_position_event(self._world, "Play_military_benchmark_troll", Vector3(0, 0, 0))
end



function BenchmarkHandler:story_end_benchmark(element, t)
...
function BenchmarkHandler:_setup_initial_values(t)
...
function BenchmarkHandler:run_func_on_bots(f, ...)
...
function BenchmarkHandler:gather_performance_data(dt, t)
	self._performance_data [#self._performance_data + 1] = { dt, t }


end

function BenchmarkHandler:write_data()
...
function BenchmarkHandler:update(dt, t)
...
function BenchmarkHandler:_handle_early_out(t)
...
function BenchmarkHandler:_disable_third_person(override)
...
function BenchmarkHandler:_camera_follow_bot()
...
function BenchmarkHandler:_set_overview_camera(t)
...
function BenchmarkHandler:_disable_overview_camera()
...
function BenchmarkHandler:_update_overview(dt, t)
...
function BenchmarkHandler:_update_selected_bot(dt, t)
...
function BenchmarkHandler:_update_bot_view(dt, t)
...
function BenchmarkHandler:_update_main_path(dt, t, total_proximate_enemies)
...
function BenchmarkHandler:destroy()
...
function BenchmarkHandler:_get_teleporter_portals()
...
function BenchmarkHandler:_update_info()
...
function BenchmarkHandler:_handle_views(dt, t)
...
function BenchmarkHandler:_update_input(dt, t)
	self:_update_info()
...
function BenchmarkHandler:_handle_teleport(dt, t)
...
function BenchmarkHandler:cb_fade_in_done(portal_data)
...
function BenchmarkHandler:cb_fade_out_done()
