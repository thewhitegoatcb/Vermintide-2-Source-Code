...
function PlayerUnitAttachmentExtension:init(extension_init_context, unit, extension_init_data)
...
function PlayerUnitAttachmentExtension:extensions_ready(world, unit)
	...
	for i = 1, slots_n do repeat
			...
			if item then
				...
				self:create_attachment(slot_name, item_data) end until true
	end
...
function PlayerUnitAttachmentExtension:game_object_initialized(unit, unit_go_id)
	local attachments = self._attachments
...
function PlayerUnitAttachmentExtension:destroy()
...
function PlayerUnitAttachmentExtension:update(unit, input, dt, context, t)
	self:update_resync_loadout()
end

function PlayerUnitAttachmentExtension:hot_join_sync(sender)
	AttachmentUtils.hot_join_sync(sender, self._unit, self._attachments.slots, self._synced_slot_buffs)
end

function PlayerUnitAttachmentExtension:create_attachment(slot_name, item_data)
	local attachments = self._attachments
...
function PlayerUnitAttachmentExtension:remove_attachment(slot_name)
...
function PlayerUnitAttachmentExtension:attachments()
	return self._attachments
end

function PlayerUnitAttachmentExtension:get_slot_data(slot_id)
	local attachments = self._attachments
	local slots = attachments.slots
...
function PlayerUnitAttachmentExtension:_show_attachment(slot_name, slot_data, show)
...
function PlayerUnitAttachmentExtension:show_attachments(show)
...
function PlayerUnitAttachmentExtension:create_attachment_in_slot(slot_name, backend_id)
	...
	self._item_to_spawn = { slot_id = slot_name, item_data = item_data }

	self.resync_loadout_needed = true
end

function PlayerUnitAttachmentExtension:update_resync_loadout()
...
function PlayerUnitAttachmentExtension:spawn_resynced_loadout(item_to_spawn)
...
function PlayerUnitAttachmentExtension:_send_rpc_add_attachment_buffs(unit_object_id, slot_id, synced_buffs)
	...
	if num_buffs > 0 then
		...
		if self._is_server then
			network_transmit:send_rpc_clients(
			"rpc_add_attachment_buffs",
			unit_object_id, slot_id,
...
function PlayerUnitAttachmentExtension:_get_property_and_trait_buffs(backend_id)
...
function PlayerUnitAttachmentExtension:_apply_buffs(buffs_by_buffer, item_name, slot_name)
...
function PlayerUnitAttachmentExtension:_remove_buffs(slot_name)
	local buff_extension = self.buff_extension
