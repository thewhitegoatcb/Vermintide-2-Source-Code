...
function BotNavTransitionExtension:init(extension_init_context, unit)
	self._unit = unit
...
function BotNavTransitionExtension.try_create_transition(unit, index_offset, bot_nav_transition_manager, drawer, called_from_editor)
	...
	if jump_transition and Vector3.distance_squared(from, via) > 0.25 then
		...
		do return nil, error_message end
	else
...
function BotNavTransitionExtension:destroy()
