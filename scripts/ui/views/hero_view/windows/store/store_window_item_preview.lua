...
function StoreWindowItemPreview:on_enter(params, offset)
...
function StoreWindowItemPreview:_set_window_expanded(expand)
	local title_edge_length = scenegraph_definition.title_text.size [1] + 50
...
function StoreWindowItemPreview:_start_transition_animation(animation_name)
	local params = {
...
function StoreWindowItemPreview:_create_viewport_definition()
	local shading_environment = "environment/ui_store_preview"
	return { scenegraph_id = "viewport",
		element = UIElements.Viewport,
		style = {
			viewport = { viewport_type = "default_forward", layer = 990, viewport_name = "item_preview_viewport", world_name = "item_preview", level_name = "levels/ui_store_preview/world", enable_sub_gui = false, fov = 65,
...
function StoreWindowItemPreview:_create_ui_elements(params, offset)
...
function StoreWindowItemPreview:on_exit(params, force_unload)
...
function StoreWindowItemPreview:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function StoreWindowItemPreview:_register_object_sets(viewport_widget, viewport_definition)
...
function StoreWindowItemPreview:_show_object_set(object_set_name, force_disable)
...
function StoreWindowItemPreview:_update_environment(item_preview_environment, force_default)
	if not self._viewport_widget then
		return
	end
...
function StoreWindowItemPreview:post_update(dt, t)
...
function StoreWindowItemPreview:_sync_products_version()
	local parent = self._parent
...
function StoreWindowItemPreview:_update_animations(dt)
...
function StoreWindowItemPreview:_animate_dlc_list_entries(dt)
	local parent = self._parent
...
function StoreWindowItemPreview:_is_dlc_list_hovered()
	local list_mask = self._dlc_top_widgets_by_name.list
	return list_mask.content.list_hotspot.is_hover or false
end

function StoreWindowItemPreview:_is_button_hover(widget)
	local content = widget.content
	local hotspot = content.button_hotspot or content.hotspot
	return hotspot.is_hover
end

function StoreWindowItemPreview:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_enter
end

function StoreWindowItemPreview:_is_button_pressed(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function StoreWindowItemPreview:_dlc_list_index_pressed()
...
function StoreWindowItemPreview:_bundle_item_pressed()
	local list_widgets = self._dlc_list_widgets
	if list_widgets then
		for index, widget in ipairs(list_widgets) do
			local content = widget.content
...
function StoreWindowItemPreview:_handle_input(input_service, dt, t)
	local parent = self._parent
			...
			if self:_is_dlc_list_hovered() then
				...
				if list_index then
					...
					elseif product_type == "item" then
						...
						parent:go_to_product(product_id, nil, { acquire_hidden = true, part_of_bundle = true, acquire_disabled = true })
					end
				end
			end
	...
	if ( self:_is_button_pressed(unlock_button) or confirm_press ) and not self:_owns_product() then
		...
		if self._show_dupe_warning then
			self._dupe_warning_popup_id = Managers.popup:queue_popup(
			Localize("bundle_party_owned_description"),
			Localize("bundle_partly_owned_title"),
			"yes", Localize("popup_choice_yes"),
			"no", Localize("popup_choice_no"))
		...
		elseif self._should_show_required_dlcs_popup then
			...
			self._missing_required_dlc_popup_id = Managers.popup:queue_popup(
			Localize("menu_store_missing_dlc_error") .. "\n\n" .. string.format(Localize("menu_store_missing_dlc_careers"), missing_dlcs_text),
			Localize("twitch_disconnect_warning"),
			"yes", Localize("popup_choice_yes"),
			"no", Localize("popup_choice_no"))
		else
...
function StoreWindowItemPreview:_exit()
	self.exit = true
end

function StoreWindowItemPreview:_get_alpha_multiplier(widget, alpha_multiplier)
...
function StoreWindowItemPreview:draw(dt)
...
function StoreWindowItemPreview:_play_sound(event)
	self._parent:play_sound(event)
end

function StoreWindowItemPreview:_start_loading_overlay()
...
function StoreWindowItemPreview:_update_loading_overlay_fadeout_animation(dt)
...
function StoreWindowItemPreview:_destroy_previewers()
...
function StoreWindowItemPreview:_sync_presentation_item(force_update)
...
function StoreWindowItemPreview:_create_dlc_bundle_layout(settings, product_id)
	...
	if settings.store_bundle_big_image then
		layout [#layout + 1] = { id = "dlc_feature_1", type = "big_image", settings = { text = "", localize = false, show_frame = true, texture_path = settings.store_bundle_big_image, texture_package = settings.store_texture_package, image_size = { 800, 592 } } }
	end

	layout [#layout + 1] = { type = "spacing" }
	layout [#layout + 1] = { type = "divider_horizontal" }
	layout [#layout + 1] = { type = "spacing" }
	layout [#layout + 1] = { type = "header_text", settings = { text = "menu_store_dlc_title_including", localize = true } }

	local item_row = #layout + 1
	local num_items = #bundle_contains
	if num_items == 2 then
		layout [item_row] = { type = "spacing", settings = { size = { 130, 0 } } }
		item_row = item_row + 1
	end

	for i = 1, #bundle_contains do
		local item_key = bundle_contains [i]
		layout [item_row + i - 1] = { type = "bundle_item", id = item_key, settings = { hide_price = true, hide_new = true } }
	end


	layout [#layout + 1] = { type = "body_text", settings = { localize = true, text = bundle_desc } }
	layout [#layout + 1] = { type = "spacing" }
	layout [#layout + 1] = { type = "divider_horizontal" }
	layout [#layout + 1] = { type = "spacing" }
...
function StoreWindowItemPreview:_create_item_bundle_layout_with_items(item_preview_layout, steam_itemdefid, item_data)
	...
	for i = 1, #item_preview_layout do
		...
		if original_layout.type == "inject_bundle_items" then
			...
			for i = 1, #bundle_contains do
				...
				layout [item_row + i - 1] = { type = "item", id = item_key, settings = { hide_price = true, hide_new = true } }
			end
		else
...
function StoreWindowItemPreview:_create_item_bundle_layout(steam_itemdefid, item_data)
	...
	if item_data.store_bundle_big_image then
		layout [#layout + 1] = { id = "dlc_feature_1", type = "big_image", settings = { text = "", localize = false, show_frame = true, texture_path = item_data.store_bundle_big_image, texture_package = item_data.store_texture_package, image_size = { 800, 592 } } }
	end
	...
	for i = 1, #bundle_contains do
		local steam_itemdefid = bundle_contains [i]
		local item_key = SteamitemdefidToMasterList [steam_itemdefid]
		layout [item_row + i - 1] = { type = "item", id = item_key, settings = { hide_price = false, hide_new = true } }
	end
...
function StoreWindowItemPreview:_present_dlc(settings, product_id)
...
function StoreWindowItemPreview:_present_item(item, product)
...
function StoreWindowItemPreview:_delayed_item_unit_presentation(item)
	local item_data = item.data
	...
	if slot_type == "melee" or slot_type == "ranged" or slot_type == "weapon_skin" then
		local preview_position = { 0, 0, 0 }
		local unique_id = nil local invert_start_rotation = true local display_unit_key = nil
...
function StoreWindowItemPreview:_update_delayed_item_unit_presentation(dt)
...
function StoreWindowItemPreview:_set_price(price, dlc_name, steam_itemdefid)
...
function StoreWindowItemPreview:_handle_platform_price_data(widget, dlc_name)
	local backend_store = Managers.backend:get_interface("peddler")
...
function StoreWindowItemPreview:_setup_ps4_price_data(widget, price_data)
	local content = widget.content
	local style = widget.style
	local spacing = 20
...
function StoreWindowItemPreview:_setup_xb1_price_data(widget, price_data)
	local content = widget.content
	local style = widget.style
	local spacing = 20
	local size = content.size
	...
	console_secondary_price_stroke_style.offset [1] = -starting_point
	console_secondary_price_stroke_style.texture_size = { console_secondary_price_text_length, 2 }
...
function StoreWindowItemPreview:_set_unlock_button_states(already_owned, can_afford, dlc_unlocked, acquire_disabled, acquire_hidden, owns_required_dlc)
...
function StoreWindowItemPreview:_owns_product()
	local widget = self._top_widgets_by_name.unlock_button
	local disable_button = widget.content.button_hotspot.disable_button
	return disable_button
end

function StoreWindowItemPreview:_detailed_view_available()
	local visible = self._item_widgets_by_name.details_button.content.visible
	return visible
end

function StoreWindowItemPreview:_set_title_name(text)
	local widget = self._top_widgets_by_name.title_text
	widget.content.text = text
end

function StoreWindowItemPreview:_set_sub_title_name(text)
	local widget = self._top_widgets_by_name.sub_title_text
	widget.content.text = text
end

function StoreWindowItemPreview:_set_sub_title_alpha_multiplier(alpha_multiplier)
	local widget = self._top_widgets_by_name.sub_title_text
	widget.alpha_multiplier = alpha_multiplier
end

function StoreWindowItemPreview:_set_type_title_name(text)
	local widget = self._top_widgets_by_name.type_title_text
	widget.content.text = text
end

function StoreWindowItemPreview:_set_career_title_name(text)
	local widget = self._top_widgets_by_name.career_title_text
	widget.content.text = text
end

function StoreWindowItemPreview:_set_disclaimer_text(text)
...
function StoreWindowItemPreview:_set_expire_timer_text(text)
...
function StoreWindowItemPreview:_update_info_text_alignment()
...
local month_lookup = { "datetime_january", "datetime_february", "datetime_march", "datetime_april", "datetime_may", "datetime_june", "datetime_july", "datetime_august", "datetime_september", "datetime_october", "datetime_november", "datetime_december" }
...
function StoreWindowItemPreview:_calculate_expire_timer_text(end_time)
...
function StoreWindowItemPreview:cb_unit_spawned_item_preview(item_previewer, item_key)
	local ignore_spin = true
	item_previewer:present_item(item_key, ignore_spin)
	self._fadeout_loading_overlay = true
end

function StoreWindowItemPreview:_spawn_hero_with_hat(world_previewer, hero_name, career_index, optional_skin, hat_item_name)
...
function StoreWindowItemPreview:cb_hero_unit_spawned_hat_preview(world_previewer, hero_name, career_index, hat_item_name)
...
function StoreWindowItemPreview:_get_can_wield_display_text(can_wield)
	local hero_text = ""
	local career_text = ""
...
function StoreWindowItemPreview:_get_hero_wield_info_by_item(item)
	local item_data = item.data
	local can_wield = item_data.can_wield
...
function StoreWindowItemPreview:_sync_layout_path()
	local parent = self._parent
...
function StoreWindowItemPreview:_hide_detail_button_assets()
	local item_widgets_by_name = self._item_widgets_by_name
...
function StoreWindowItemPreview:_animate_detail_button(dt)
	local item_widgets_by_name = self._item_widgets_by_name
...
function StoreWindowItemPreview:_set_title_edge_length(length, animation_duration)
	local ui_scenegraph = self._ui_scenegraph
...
function StoreWindowItemPreview:_update_title_edge_animation(dt)
...
function StoreWindowItemPreview:_update_unlock_button_width(width_offset, already_owned, dlc_name)
	local top_widgets_by_name = self._top_widgets_by_name
...
function StoreWindowItemPreview:_get_text_width(text_style, text)
...
function StoreWindowItemPreview:_initialize_dlc_scrollbar()
...
function StoreWindowItemPreview:_update_dlc_scroll_position()
	local dlc_scrollbar_logic = self._dlc_scrollbar_logic
...
function StoreWindowItemPreview:_update_visible_list_entries()
	local dlc_scrollbar_logic = self._dlc_scrollbar_logic
...
function StoreWindowItemPreview:_scroll_to_list_index(index)
	local dlc_scrollbar_logic = self._dlc_scrollbar_logic
	local enabled = dlc_scrollbar_logic:enabled()
...
function StoreWindowItemPreview:_get_scrollbar_percentage_by_index(index)
	local dlc_scrollbar_logic = self._dlc_scrollbar_logic
	local enabled = dlc_scrollbar_logic:enabled()
...
function StoreWindowItemPreview:_dlc_component_layout(layout)
	...
	for i = 1, #layout do
		...
		if product_type == "dlc" then
			...
			if dlc_settings then
				product = { dlc_settings = dlc_settings, type = product_type, product_id = product_id }
			end
		elseif product_type == "item" then
			local item = self._parent:get_item_by_key(product_id)
			if item then
				product = { item = item, type = product_type, product_id = product_id, settings = product_data.settings }
			end
		elseif product_type == "bundle_item" then
			local item = ItemMasterList [product_id]
			if item then
				product = { item = { data = item }, type = product_type, product_id = product_id, settings = product_data.settings }
			end
		else
			product = { type = product_type, settings = product_data.settings, product_id = product_id }
		end
...
function StoreWindowItemPreview:_create_dlc_product_widgets(layout)
	local widgets = { }
...
function StoreWindowItemPreview:_destroy_dlc_product_widgets(force_unload)
	local parent = self._parent
...
function StoreWindowItemPreview:_align_dlc_widgets()
