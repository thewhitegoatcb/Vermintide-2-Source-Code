...
function DamageBlobHuskExtension:init(extension_init_context, unit, extension_init_data)
...
function DamageBlobHuskExtension:destroy()
...
function DamageBlobHuskExtension:update(unit, input, dt, context, t)
...
function DamageBlobHuskExtension:update_blobs_fx_and_sfx(t, dt)
	local world = self.world
...
function DamageBlobHuskExtension:add_damage_blob_fx(position, life_time_percentage)
	local unit = self.unit
	local world = self.world
	...
	fx_list [#fx_list + 1] = { id = fx_id_filled, time = blob_death_time, size = particle_size }

	local fx_id_rim = World.create_particles(world, self.fx_name_rim, position, rotation)
	fx_list [#fx_list + 1] = { id = fx_id_rim, time = blob_death_time }

	local id, source = WwiseUtils.trigger_position_event(world, self._sfx_name_start_remains, position)
	local sfx_list = self.sfx_list
	sfx_list [#sfx_list + 1] = { source = source, time = blob_death_time }
end

function DamageBlobHuskExtension:abort()
	local unit = self.unit
	local sfx_name_stop = self._sfx_name_stop
	if sfx_name_stop and Unit.alive(unit) then
...
function DamageBlobHuskExtension:get_source_attacker_unit()
