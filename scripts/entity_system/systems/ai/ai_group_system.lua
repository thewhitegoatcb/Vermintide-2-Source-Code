...
local extensions = { "AIGroupMember" }



function AIGroupSystem:init(context, system_name)
...
function AIGroupSystem:add_ready_splines(waypoint_list, spline_type)
	...
	for i = 1, #waypoint_list do
		...
		if spline_points then
			...
			local spline_data = { start_position = start_position_boxed, start_direction = Vector3Box(start_direction), spline_points = spline_points }
...
function AIGroupSystem:destroy()
	return end

function AIGroupSystem:ai_ready(patrol_analysis)
	self.patrol_analysis = patrol_analysis
end

function AIGroupSystem:on_add_extension(world, unit, extension_name, extension_init_data)
	local extension = { }
...
function AIGroupSystem:init_extension(unit, extension, extension_init_data)
	...
	if group == nil then
		group = { members_n = 0, start_forward = true, num_spawned_members = 0,
			id = id,
			members = { },
...
function AIGroupSystem:extensions_ready(world, unit, extension_name)
...
function AIGroupSystem:on_remove_extension(unit, extension_name)
	self.frozen_unit_extension_data [unit] = nil
	self:_cleanup_extension(unit, extension_name)
	ScriptUnit.remove_extension(unit, self.NAME)
end

function AIGroupSystem:on_freeze_extension(unit, extension_name)
	local extension = self.unit_extension_data [unit]
...
function AIGroupSystem:freeze(unit, extension_name, reason)
...
function AIGroupSystem:unfreeze(unit, extension_name, data)
...
function AIGroupSystem:_cleanup_extension(unit, extension_name)
	local extension = self.unit_extension_data [unit]
	if extension == nil then return end
	self.unit_extension_data [unit] = nil
...
function AIGroupSystem:set_level(level)
	self._level = level
	...
	for i = 1, MAX_PATROL_SPLINES do repeat
			...
			local data = { start_position = Vector3Box(start_position), start_direction = Vector3Box(start_direction) }
			self._patrol_splines [spline_name] = data
			self._spline_lookup [spline_name] = data until true
	end

	for i = 1, MAX_ROAMING_SPLINES do repeat
	...
	for i = 1, MAX_EVENT_SPLINES do repeat
...
function AIGroupSystem:get_best_spline(position, spline_type)
	...
	for spline_name, spline_data in pairs(splines) do repeat
			...
			best_spline_data = spline_data until true
	end

	return best_spline, best_spline_data
end

function AIGroupSystem:spline_start_position(spline_name)
	local spline = self._spline_lookup [spline_name]
	local start_position = spline.start_position:unbox()
	return start_position
end

function AIGroupSystem:spline_start_direction(spline_name)
	local spline = self._spline_lookup [spline_name]
	local start_direction = spline.start_direction:unbox()
	return start_direction
end

function AIGroupSystem:spline(spline_name)
...
function AIGroupSystem:level_has_splines(spline_type)
	local splines = nil
	if spline_type == "patrol" then
		splines = self._patrol_splines
	elseif spline_type == "roaming" then
...
function AIGroupSystem:get_available_spline_type()
...
function AIGroupSystem:hot_join_sync(peer_id, player)
	return end

local position_lookup = POSITION_LOOKUP
function AIGroupSystem:check_recycler_despawn(player_positions, player_zones, use_player_zones)
...
local debug_drawer_info = { mode = "retained", name = "AIGroupTemplates_retained" }
function AIGroupSystem:update(context, t)
	...
	if self.patrol_analysis and self._computing_splines then
		...
		for spline_name, spline_type in pairs(self._computing_splines) do repeat
				...
				local spline_data = { start_position = start_position_boxed, start_direction = Vector3Box(start_direction), spline_points = spline_points, failed = spline.failed }

				self:_add_spline(spline_name, spline_data, spline_type)
				self._computing_splines [spline_name] = nil until true
		end
	end
...
function AIGroupSystem:prepare_update_recycler(player_positions, player_areas, use_player_areas)
...
function AIGroupSystem:get_ai_group(id)
	local groups = self.groups
	return groups [id]
end

function AIGroupSystem:run_func_on_all_members(group, func, ...)
...
function AIGroupSystem:generate_group_id()
	self.group_uid = self.group_uid + 1
	return self.group_uid
end

function AIGroupSystem:set_allowed_layer(layer_name, allowed)
...
function AIGroupSystem:create_spline_from_way_points(spline_name, spline_way_points, spline_type)
...
function AIGroupSystem:spline_ready(spline_name)
	return self._spline_lookup [spline_name]
end

function AIGroupSystem:_spline_ready(spline_name)
...
function AIGroupSystem:_add_spline(spline_name, spline_data, spline_type)
...
function AIGroupSystem:_calculate_splines(spline_name, spline_data)
...
function AIGroupSystem:draw_active_spline_paths()
...
function AIGroupSystem:draw_spline(spline, drawer, color)
...
function AIGroupSystem:create_formation_data(position, formation, spline_name, spawn_all_at_same_position, group_data)
	...
	for row, columns in ipairs(formation) do
		...
		for column, breed_name in ipairs(columns) do
			...
			if Breeds [breed_name] then
				...
				spawn_pos =
				spawn_pos or LocomotionUtils.pos_on_mesh(nav_world, position_on_spline, above, below)


				spawn_pos =
				spawn_pos or inside_position_from_outside_position(nav_world, wanted_spawn_position, above, below, 0.5, 0.2)


				if spawn_pos then
					formation_data [current_row] [column] = { breed_name = breed_name, start_position = Vector3Box(spawn_pos), start_direction = Vector3Box(flat_start_direction) }
					...
					formation_data [current_row] [column] = { start_position = Vector3Box(wanted_spawn_position), start_direction = Vector3Box(flat_start_direction) }
				end
			else
...
function AIGroupSystem:_get_position_on_spline_by_distance(distance, spline_curve, start_spline_index)
...
function AIGroupSystem:register_spline_properties(spline_name, properties)
