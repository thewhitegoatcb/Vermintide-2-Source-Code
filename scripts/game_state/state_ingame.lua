...
function StateIngame:on_enter()
	...
	self.world = self.parent.loading_context.ingame_world_object self.parent.loading_context.ingame_world_object = self.world


	self.level = self.parent.loading_context.ingame_level_object self.parent.loading_context.ingame_level_object = self.level
	...
	self,
	"event_play_particle_effect",
	"event_play_particle_effect",
	"event_start_network_timer",
	"event_start_network_timer",
	"xbox_one_hack_start_game",
	"event_xbox_one_hack_start_game",
	"gm_event_end_conditions_met",
...
function StateIngame:peer_type()
	if DEDICATED_SERVER then
		do return "dedicated-server" end
	elseif self.is_server then
...
function StateIngame:event_xbox_one_hack_start_game(level_key, difficulty)
	print(level_key, difficulty)
	Managers.level_transition_handler:set_next_level(
	level_key,
	nil, nil, nil, nil, nil,
...
function StateIngame:cb_save_data()
	print("saved data")
end
...
function StateIngame:_setup_world()
	...
	Managers.world:set_scene_update_callback(
	self.world,
	function ()
...
function StateIngame:_safe_to_do_entity_update()
...
function StateIngame:physics_async_update(dt)
	local t = Managers.time:time("game")
...
function StateIngame:shading_callback(world, shading_env, viewport)
	Managers.state.camera:shading_callback(world, shading_env, viewport)
end

function StateIngame:_teardown_level()
...
function StateIngame:unspawn_unit(unit)
	if not Managers.state.entity then
...
function StateIngame:_gather_backend_flow_events()
...
function StateIngame:pre_update(dt)
	local t = Managers.time:time("game")
	local network_manager = Managers.state.network
...
local TIMER = 0
local REMAP_INDEX = 0
local EVENTS = { "charge_end", "spark_muzzlefx_right", "spark_muzzlefx_left", "above_overcharge_threshold", "sfx_ranged_weapon_foley", "beam_muzzlefx", "fx_show_fire_trail", "below_overcharge_threshold", "send_spear", "fireball_charged_shoot", "fireball_shoot", "staff_charge_cancel", "fx_hide_fire_trail", "sfx_ranged_weapon_equip", "lua_wield", "geiser_muzzlefx" }
...
function StateIngame:update(dt, main_t)
	...
	if is_server then
		...
		if self._lobby_host:is_joined() and

		Managers.state.network:game() then
...
function StateIngame:_update_onclose_check(dt, t)
	if self._onclose_called and not self._onclose_popup_id then
		...
		self._onclose_popup_id = Managers.popup:queue_popup(
		popup_text,
		Localize("popup_exit_game_topic"),
		"end_game",
		Localize("popup_choice_yes"),
		"cancel_popup",
		Localize("popup_choice_no"))
	end
...
function StateIngame:_update_deed_manager(dt)
	local deed_manager = Managers.deed

	deed_manager:update(dt)

	if self.is_server and
	deed_manager:has_deed() and deed_manager:is_session_faulty() then
		if self.is_in_inn then
			deed_manager:reset()
		else
...
function StateIngame:event_start_network_timer(time)
...
function StateIngame:_check_exit(t)
	...
	local waiting_for_item_poll = ( backend_items:num_current_item_server_requests() ~= 0 or UISettings.waiting_for_response ) and
	not backend_manager:is_disconnected()
	...
	if not self.exit_type and not waiting_user_input and not waiting_for_item_poll then
		...
		transition =
		transition or Managers.state.game_mode:wanted_transition()
		...
		elseif self.network_client and self.network_client.state == "lost_connection_to_host" and lobby and
		lobby:lost_connection_to_lobby() then
		...
		elseif transition == "restart_game_server" then
			...
			Managers.transition:fade_in(GameSettings.transition_fade_in_speed, nil)
			Managers.transition:show_loading_icon()
		elseif transition == "complete_level" then --[[ Nothing --]]
		end
		...
		if self.exit_type then
			...
			if self.is_server and not self.is_in_inn and self.exit_type ~= "reload_level" and Managers.matchmaking and
			Managers.matchmaking:game_mode_event_data() then
			...
			printf(
			"StateIngame: Got transition %s, set exit type to %s. Will exit at t=%.2f",
			tostring(transition),
			self.exit_type,
	...
	if self.exit_time and self.exit_time <= t then
		...
		elseif exit_type == "join_lobby_failed" or exit_type == "left_game" or exit_type == "lobby_state_failed" or exit_type == "kicked_by_server" or exit_type == "afk_kick" then
			...
			elseif
			( exit_type == "return_to_pc_menu" or exit_type == "left_game" ) and network_manager:in_game_session() then
				local force_diconnect = true
				network_manager:leave_game(force_diconnect)
			end
			...
			if exit_type == "lobby_state_failed" then
				if IS_WINDOWS or IS_LINUX then
					do return StateTitleScreen end
				else
					do return StateLoading end
				end
			else
				do return StateLoading end
			end
		...
		elseif exit_type == "restart_game_server" then
			...
			do return StateDedicatedServer end
...
function StateIngame:wanted_profile_index()
	local peer_id = Network.peer_id()
...
function StateIngame:wanted_party_index()
...
function StateIngame:pre_render()
...
function StateIngame:post_render()
	if not self.machines then
		return
	end
...
function StateIngame:on_exit(application_shutdown)
	...
	if application_shutdown or self.leave_lobby then
		...
		if self.is_server and not self.is_in_inn and not self.is_in_tutorial and
		IS_XB1 and not script_data.honduras_demo then
			local level_key = level_transition_handler:get_current_level_key()
			local level_setting = LevelSettings [level_key]
...
function StateIngame:on_close()


	if self.network_server and self.network_server:num_active_peers() > 1 and
	not Development.parameter("disable_exit_popup_warning") then
...
function StateIngame:_commit_playfab_stats()
	local backend_manager = Managers.backend
...
function StateIngame:_check_and_add_end_game_telemetry(application_shutdown)
...
function StateIngame:_setup_state_context(world, is_server, network_event_delegate)
	...
	local go_context = { world = world }
	local unit_go_sync_functions = require("scripts/network/game_object_initializers_extractors")
	unit_spawner:set_gameobject_initializer_data(
	unit_go_sync_functions.initializers,
	unit_go_sync_functions.extractors,
	...
	Managers.state.debug =
	DebugManager:new(world, self.free_flight_manager, self.input_manager, network_event_delegate, is_server)
	Managers.state.spawn =
	SpawnManager:new(
	world,
	is_server,
	network_event_delegate,
	...
	local entity_systems_init_context = {
		...
		world = self.world,
		startup_data = { level_key = level_key, environment_variation_name = environment_variation_name },
		is_server = is_server,
		free_flight_manager = self.free_flight_manager,
		network_event_delegate = network_event_delegate,
	...
	Managers.state.bot_nav_transition =
	BotNavTransitionManager:new(world, physics_world, nav_world, is_server, network_event_delegate)
...
function StateIngame:rpc_kick_peer(channel_id)
...
function StateIngame:event_play_particle_effect(effect_name, unit, node, offset, rotation_offset, linked)
	if linked then
		ScriptWorld.create_particles_linked(
		self.world,
		effect_name,
		unit,
		node,
		"destroy",
		Matrix4x4.from_quaternion_position(rotation_offset, offset))
	else
...
function StateIngame:gm_event_end_conditions_met(reason, checkpoint_available, percentages_completed)
	...
	if game_mode_key == "survival" then
		...
		game_won and self.is_server then
	...
	elseif
	game_lost and self.is_server and checkpoint_available then
...
function StateIngame:_generate_ingame_clock()
...
function StateIngame:_remove_ingame_clock()
...
function StateIngame:_handle_onclose_warning_result()
