...
function ActionAim:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionAim:client_owner_start_action(new_action, t)
...
function ActionAim:_start_charge_sound()
...
function ActionAim:_stop_charge_sound()
	local current_action = self.current_action
	local owner_unit = self.owner_unit
	local owner_player = self.owner_player
	local is_bot = owner_player and owner_player.bot_player
...
function ActionAim:client_owner_post_update(dt, t, world, can_damage)
	local current_action = self.current_action
	local owner_unit = self.owner_unit
	...
	if not self.played_aim_sound and self.aim_sound_time <= t and
	not Managers.player:owner(self.owner_unit).bot_player then
		Managers.state.controller_features:add_effect("rumble", { rumble_effect = "aim_start" })
	...
	if not self.heavy_aim_flow_done and self.heavy_aim_flow_time <= t and
	not Managers.player:owner(self.owner_unit).bot_player then
		Managers.state.controller_features:add_effect("rumble", { rumble_effect = "aim_start" })
...
function ActionAim:finish(reason)
	local current_action = self.current_action
	...
	if not owner.bot_player then
		Managers.state.controller_features:add_effect("rumble", { rumble_effect = "full_stop" })
	end
