...
local DAMAGE_TYPE_BLACKLIST = { temporary_health_degen = true, sync_health = true, knockdown_bleed = true, death_zone = true, volume_insta_kill = true, heal = true, inside_forbidden_tag_volume = true, forced = true }
...
local function add_damage(data, unit, damage, damaging_unit)
	if not data.damage_buffer [unit] then local damage_data = { damage = 0,
...
local function create_missing_beam_effects(world, beam_effects, player_unit)
	if not beam_effects [player_unit] then
		...
		beam_effects [player_unit] = { blinking_enabled = false, beam_softness = 0,
			ids = {
...
return { description = "curse_empathy_desc", display_name = "curse_empathy_name", icon = "deus_curse_slaanesh_01",
