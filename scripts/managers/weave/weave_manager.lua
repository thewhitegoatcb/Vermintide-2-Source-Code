...
local RPCS = { "rpc_set_active_weave", "rpc_weave_objective_completed", "rpc_weave_final_objective_completed", "rpc_sync_end_of_weave_data", "rpc_sync_player_count", "rpc_bar_cutoff_reached" }
...
local EVENTS = { "conflict_director_setup_done", "event_conflict_director_setup_done" }




function WeaveManager:init()
	self:_reset()
end

function WeaveManager:initiate(world, network_event_delegate, is_server, game_mode_key)
...
function WeaveManager:clear_weave_data()
	self._bar_score = 0
	self._bar_filled = false
	self._remaining_time = WeaveSettings.starting_time
	self._damage_taken = 0
...
function WeaveManager:clear_weave_name()
...
function WeaveManager:_setup_data(world, is_server)
	self._world = world
...
function WeaveManager:weave_spawner()
	return self._weave_spawner
end

function WeaveManager:_setup_weave_data(is_server)
...
function WeaveManager:_register_events()
	local event_manager = Managers.state.event
	event_manager:register(self, unpack(EVENTS))
end

function WeaveManager:_unregister_events()
	local event_manager = Managers.state.event
...
function WeaveManager:_register_rpcs(network_event_delegate)
	self._network_event_delegate = network_event_delegate
	network_event_delegate:register(self, unpack(RPCS))
end

function WeaveManager:_unregister_rpcs()
...
function WeaveManager:reset_statistics_for_challenges()
...
function WeaveManager:teardown()
...
function WeaveManager:destroy()
	self:_unregister_rpcs()
	self:_unregister_events()
end

function WeaveManager:update(dt, t)
...
function WeaveManager:event_conflict_director_setup_done()
...
function WeaveManager:_set_player_ids(player_ids)
...
function WeaveManager:store_player_ids()
...
function WeaveManager:get_saved_game_mode_data()
...
function WeaveManager:store_saved_game_mode_data()
	if not self._is_server then
		return
	end
...
function WeaveManager:get_player_ids()
	return self._player_ids
end

function WeaveManager:set_next_weave(weave_name)
	self._next_weave_name = weave_name
end

function WeaveManager:set_next_objective(objective_index)
	self._next_objective_index = objective_index
end

function WeaveManager:get_next_weave()
	return self._next_weave_name
end

function WeaveManager:get_next_objective()
	return self._next_objective_index
end

function WeaveManager:get_time_left()
	return self._remaining_time
end

function WeaveManager:get_damage_taken()
	return self._damage_taken
end

function WeaveManager:_set_active_weave(weave_name)
	self._active_weave_name = weave_name
end

function WeaveManager:_report_telemetry()
...
function WeaveManager:_set_active_objective(objective_index)
	self._active_objective_index = objective_index
end

function WeaveManager:get_active_objective()
	return self._active_objective_index
end

function WeaveManager:_set_time_left(remaining_time)
	self._remaining_time = remaining_time
end

function WeaveManager:_set_damage_taken(damage_taken)
	self._damage_taken = damage_taken
end

function WeaveManager:get_active_weave()
	return self._active_weave_name
end

function WeaveManager:get_active_weave_phase()
	return self._active_weave_phase
end

function WeaveManager:set_active_weave_phase(phase)
	self._active_weave_phase = phase
end

function WeaveManager:get_active_wind()
...
function WeaveManager:get_active_wind_settings()
	if not self._active_weave_name then
		return
	end
...
function WeaveManager:get_scaling_value(scaling_value_key)
	...
	if scaling_setting then
		for _, increase_data in ipairs(WeaveSettings.difficulty_increases) do
			if tier <= increase_data.breakpoint then
				...
				do return final_scaling_value end
			else
...
function WeaveManager:start_timer()
	self._pause_timer = false
end

function WeaveManager:calculate_next_objective_index()
	if not self._active_weave_name then
		return
	end
...
function WeaveManager:sync_end_of_weave_data()
...
function WeaveManager:hot_join_sync(peer_id)
...
function WeaveManager:mutators()
...
function WeaveManager:start_objective()
	local objective_template = self:get_active_objective_template()
...
function WeaveManager:increase_bar_score(amount)
...
function WeaveManager:show_bar()
	local objective_template = self:get_active_objective_template()
...
function WeaveManager:get_active_objective_template()
...
function WeaveManager:get_scaling_difficulty_index()

	return end

function WeaveManager:get_active_weave_template()
	if not self._active_weave_name then
		return
	end
...
function WeaveManager:start_terror_event(event_template_name, spawner_id)
...
function WeaveManager:stop_terror_event(event_template_name, spawner_id)
	local weave_settings = self:get_active_weave_template()
	local objective_settings = self:get_active_objective_template()
	local objective_index = self._active_objective_index
...
function WeaveManager:get_wind_strength()
	local weave_template = WeaveSettings.templates [self._active_weave_name]

	return weave_template and weave_template.wind_strength or 1
end

function WeaveManager:_create_game_object()
	local go_data_table = {
...
function WeaveManager:game_object_created(go_id)
	self._go_id = go_id
end

function WeaveManager:game_object_destroyed()
	self._go_id = nil
end

function WeaveManager:cb_game_session_disconnect()
	self._go_id = nil
end

function WeaveManager:final_objective_completed()
...
function WeaveManager:_objective_completed()
...
function WeaveManager:_calculate_score()
...
function WeaveManager:get_bar_score()
	return self._bar_score
end

function WeaveManager:get_score()
	return self._score
end



function WeaveManager:get_time_score()
	return math.floor(math.max(self:get_score() - self:get_damage_score(), 0))
end

function WeaveManager:get_damage_score()
	return math.floor(( WeaveSettings.max_damage_taken - self._damage_taken ) * 10)
end

function WeaveManager:get_weave_tier()
	local weave_template = WeaveSettings.templates [self._active_weave_name]

	return weave_template.tier
end

function WeaveManager:get_num_players()

	return self._num_players
end

function WeaveManager:is_tracking_kills()
	return self._track_kills
end


function WeaveManager:ai_killed(killed_unit, killer_unit, death_data, killing_blow)
...
function WeaveManager:_track_ai_killed(breed_name)
	if self._is_server then
...
function WeaveManager:rpc_bar_cutoff_reached(channel_id)
	local objective_template = self:get_active_objective_template()
	local bar_cutoff = objective_template.bar_cutoff
	self._bar_score = bar_cutoff
end

function WeaveManager:rpc_set_active_weave(channel_id, weave_name_id, objective_index)
...
function WeaveManager:rpc_weave_objective_completed(channel_id)
	self:_objective_completed()
end

function WeaveManager:rpc_sync_end_of_weave_data(channel_id, score, remaining_time, num_players, damage_taken)
	self._score = score
	self._remaining_time = remaining_time
...
function WeaveManager:rpc_sync_player_count(channel_id, num_players)
	self._num_players = num_players
end

function WeaveManager:rpc_weave_final_objective_completed(channel_id)
