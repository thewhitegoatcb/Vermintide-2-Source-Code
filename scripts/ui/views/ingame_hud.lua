...
function IngameHud:init(parent, ingame_ui_context)
	self._parent = parent
...
function IngameHud:_setup_components()
...
function IngameHud:reset_components()
	self:destroy()
	self:_setup_components()
end

function IngameHud:event_player_party_changed(player, is_local, old_party_id, new_party_id)
...
function IngameHud:_setup_component_definitions(hud_component_list_path)
...
function IngameHud:_compile_component_list(ingame_ui_context, component_definitions)
...
function IngameHud:_add_component(component_list, components, components_array, components_array_id_lookup, class_name)
...
function IngameHud:_remove_component(component_list, components, components_array, components_array_id_lookup, class_name)
...
function IngameHud:remove_components(components_to_remove)
...
function IngameHud:component(component_name)
	local components = self._components
	local component = components [component_name]
	return component
end

function IngameHud:_update_components_post_visibility()
...
function IngameHud:_update_components_visibility()
	local definitions = self._definitions
	local visibility_groups = definitions.visibility_groups
...
function IngameHud:get_hud_component(hud_component_name)
	return self._components [hud_component_name]
end

function IngameHud:_update_hud_scale()
...
function IngameHud:_apply_hud_scale()
...
function IngameHud:_abort_hud_scale()
	local scale_modified = self._scale_modified
	local resolution_modified = self._resolution_modified
	local force_update = scale_modified or resolution_modified
...
function IngameHud:update(dt, t)
...
function IngameHud:post_update(dt, t)
...
function IngameHud:input_service()
	return false
end
...
function IngameHud:is_in_inn()
	return self._ingame_ui_context.is_in_inn
end

function IngameHud:_reset_hud_frame_variables()
...
function IngameHud:is_own_player_dead()
	return self._is_own_player_dead
end

function IngameHud:get_crosshair_position()
	if not self._crosshair_position_x or not self._crosshair_position_y then
		...
		if ALIVE [player_unit] then
			...
			if
			eyetracking_extension and eyetracking_extension:get_is_feature_enabled("tobii_extended_view") then
...
function IngameHud:enable_clean_ui(enable)
	self._tobii_clean_ui_is_enabled = enable
end

function IngameHud:_update_clean_ui(dt, t)
