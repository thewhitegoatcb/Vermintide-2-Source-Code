...
local function cmp_setting_name(a, b) return a.setting_name < b.setting_name end
local function cmp_name(a, b) return a.name < b.name end

function ImguiDebugMenu:init()
	self._needle = ""
	...
	for name, list in pairs(settings_by_category) do
		...
		table.insert(self._settings_categories, { name = name, list = list })
	end
...
function ImguiDebugMenu:update()
	return end

local find = string.find local lower = string.lower
local function ifind(h, n) return find(lower(h), lower(n)) end

function ImguiDebugMenu:_find_needle(setting, needle)

	return ifind(setting.setting_name, needle) or
	ifind(setting.description, needle) or
	ifind(setting.category, needle)

end

function ImguiDebugMenu:_find_needle_list(setting_list, needle)
...
function ImguiDebugMenu:draw()
	...
	for _, sc in pairs(self._settings_categories) do
		local category = sc.name local setting_list = sc.list
	...
	if no_matches then Imgui.text("No matches.") end
...
function ImguiDebugMenu:_set_setting(key, val)
	Development.set_setting(key, val)
	script_data [key] = val
	Development.clear_param_cache(key)
end

function ImguiDebugMenu:_show_debug_setting(setting)
	...
	if setting.is_boolean then
		...
		if Imgui.radio_button("false##" .. setting_name, val == false) then val = false end Imgui.same_line()
		if Imgui.radio_button("true##" .. setting_name, val == true) then val = true end Imgui.same_line()
		if Imgui.small_button("Reset") then val = nil end
	...
	elseif setting.load_items_source_func or setting.item_source then
		...
		Imgui.same_line()
		if Imgui.small_button("Reset") then index = 0 end
		self:_set_setting(setting_name, options [index])
		if setting.func then
			Imgui.same_line()
			if Imgui.small_button("Execute") then setting.func(options, index)
			end
		end
	...
	elseif setting.func then
		Imgui.same_line()
		if Imgui.small_button("Execute") then setting.func()
		end end
end

function ImguiDebugMenu:is_persistent()
	return false
end