...
local scenegraph_definition = {
	...
	text_root = { vertical_alignment = "bottom", parent = "screen", horizontal_alignment = "left",
	...
	damage_text = { vertical_alignment = "center", parent = "text_root", horizontal_alignment = "center",
...
local default_text_style = { word_wrap = false, font_size = 40, localize = false, use_shadow = true, horizontal_alignment = "center", vertical_alignment = "center", font_type = "hell_shark_header",
...
function DamageNumbersUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function DamageNumbersUI:update(dt, viewport_name)
...
function DamageNumbersUI:event_add_damage_number(damage, size, unit, time, color, is_critical_strike)
	...
	if is_infront then
		...
		local new_text = { floating_speed = 150, alpha = 255,
			size = size,
			text = damage * 100,
			color = { 255, color.x, color.y, color.z },
...
function DamageNumbersUI:destroy()
...
function DamageNumbersUI:_destroy_unit_texts(unit)
	self._unit_texts [unit] = nil
end

function DamageNumbersUI:create_ui_elements()
	self.ui_scenegraph = UISceneGraph.init_scenegraph(scenegraph_definition)
	self.damage_text = UIWidget.init(widget_definitions.damage_text)
	UIRenderer.clear_scenegraph_queue(self.ui_renderer)
end

function DamageNumbersUI:draw(dt)
