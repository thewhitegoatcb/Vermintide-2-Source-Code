...
function PlayerCharacterStateInHangingCage:init(character_state_init_context)
	PlayerCharacterState.init(self, character_state_init_context, "in_hanging_cage")
end

function PlayerCharacterStateInHangingCage:on_enter(unit, input, dt, context, t, previous_state, params)
...
function PlayerCharacterStateInHangingCage:on_exit(unit, input, dt, context, t, next_state)
	local status_extension = self.status_extension
	status_extension:set_in_hanging_cage(false)
end

function PlayerCharacterStateInHangingCage:update(unit, input, dt, context, t)
	local csm = self.csm
	local status_extension = self.status_extension
	...
	if current_state ~= new_state then
		...
		elseif new_state == "landed" then
			...
			csm:change_state("knocked_down", { already_in_ko_anim = true })
		end
