...
local states = {
	item_setting = { setup_function = "_state_setup_overview", craft_complete_func_name = "_apply_weapon_skin_craft_complete", gamepad_input_func = "_update_skin_gamepad_input", transition_time = 0.3, fov = 30, draw_function = "_state_draw_overview",
	...
	item_properties = { setup_function = "_state_setup_property_reroll", craft_complete_func_name = "_state_setup_property_reroll", transition_time = 0.3, fov = 30, draw_function = "_state_draw_property_reroll",
		...
		recipe_by_slot_type = { trinket = "reroll_jewellery_properties", ranged = "reroll_weapon_properties", necklace = "reroll_jewellery_properties", ring = "reroll_jewellery_properties", melee = "reroll_weapon_properties" } },
	...
	item_trait = { setup_function = "_state_setup_trait_reroll", craft_complete_func_name = "_state_setup_trait_reroll", transition_time = 0.3, fov = 30, draw_function = "_state_draw_trait_reroll",




		camera_position = { 0, -1, 0 },
		recipe_by_slot_type = { trinket = "reroll_jewellery_traits", ranged = "reroll_weapon_traits", necklace = "reroll_jewellery_traits", ring = "reroll_jewellery_traits", melee = "reroll_weapon_traits" } },
	...
	item_upgrade = { setup_function = "_state_setup_upgrade", craft_complete_func_name = "_upgrade_item_craft_complete", transition_time = 0.3, fov = 30, draw_function = "_state_draw_upgrade",




		camera_position = { 0, -1, 0 },
		recipe_by_rarity = { common = "upgrade_item_rarity_rare", plentiful = "upgrade_item_rarity_common", rare = "upgrade_item_rarity_exotic", exotic = "upgrade_item_rarity_unique" } } }
...
function HeroWindowItemCustomization:on_enter(params)
...
function HeroWindowItemCustomization:_setup_menu_input_description()
...
function HeroWindowItemCustomization:_find_equipment_slot()
...
function HeroWindowItemCustomization:_setup_availble_states(item)
	local item_data = item.data
	...
	if item_rarity == "default" or item_rarity == "promo" then
		self._available_states = { "item_setting" }


	elseif rarity_rating <= rarity_rating_table.unique then
		self._available_states = { "item_setting", "item_properties", "item_trait" }




	elseif rarity_rating <= rarity_rating_table.exotic then
		self._available_states = { "item_setting", "item_properties", "item_trait", "item_upgrade" }
	...
	elseif rarity_rating <= rarity_rating_table.common then
		self._available_states = { "item_setting", "item_properties", "item_upgrade" }




	elseif rarity_rating <= rarity_rating_table.plentiful then
		self._available_states = { "item_setting", "item_upgrade" }
	end
...
function HeroWindowItemCustomization:_set_camera_position(camera_position)
...
function HeroWindowItemCustomization:_camera_position()
	local preview_widget = self._preview_widget
	local previewer_pass_data = preview_widget.element.pass_data [1]
	local viewport = previewer_pass_data.viewport
	local camera = ScriptViewport.camera(viewport)
...
function HeroWindowItemCustomization:_set_camera_fov(fov)
	local preview_widget = self._preview_widget
	local previewer_pass_data = preview_widget.element.pass_data [1]
	local viewport = previewer_pass_data.viewport
	local camera = ScriptViewport.camera(viewport)
...
function HeroWindowItemCustomization:_camera_fov()
	local preview_widget = self._preview_widget
	local previewer_pass_data = preview_widget.element.pass_data [1]
	local viewport = previewer_pass_data.viewport
	local camera = ScriptViewport.camera(viewport)
...
function HeroWindowItemCustomization:_change_state(new_state)
	...
	self._state_start_camera_position = { camera_position.x, camera_position.y, camera_position.z }
...
function HeroWindowItemCustomization:_get_item(backend_id)
	self._item_backend_id = backend_id
	local backend_items = Managers.backend:get_interface("items")
	return backend_items:get_item_from_id(backend_id)
end

function HeroWindowItemCustomization:_start_transition_animation(animation_name)
	local params = {
...
function HeroWindowItemCustomization:_create_ui_elements()
...
function HeroWindowItemCustomization:_create_preview_widget()
...
function HeroWindowItemCustomization:_create_item_preview_widget_definition()
	...
	local style = {
		viewport = { viewport_type = "default_forward", layer = 962, shading_environment = "environment/ui_store_preview", viewport_name = "item_preview", level_name = "levels/ui_store_preview/world", enable_sub_gui = true, fov = 65, world_name = "item_preview",
...
function HeroWindowItemCustomization:on_exit(params)
...
function HeroWindowItemCustomization:play_sound(sound_event)
...
function HeroWindowItemCustomization:_register_object_sets(viewport_widget, viewport_definition)
	...
	for _, set_name in ipairs(available_level_sets) do
		object_sets [set_name] = { set_enabled = true, units = LevelResource.unit_indices_in_object_set(level_name, set_name) }
	end
...
function HeroWindowItemCustomization:_show_object_set(object_set_name, force_disable)
...
function HeroWindowItemCustomization:_update_environment(item_preview_environment, force_default)
	if not self._preview_widget then
		return
	end
...
function HeroWindowItemCustomization:_is_button_hover(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.is_hover
end

function HeroWindowItemCustomization:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_enter
end

function HeroWindowItemCustomization:_is_button_pressed(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function HeroWindowItemCustomization:_navigation_menu_disabled()
	local disabled = self._mission_selection_grid ~= nil
	return disabled
end

function HeroWindowItemCustomization:_handle_gamepad_input(input_service, dt, t)
	local parent = self._parent
...
function HeroWindowItemCustomization:_handle_input(input_service, dt, t)
	local parent = self._parent
	local input_handled = false
	local widgets_by_name = self._widgets_by_name
...
function HeroWindowItemCustomization:_update_active_preview()
...
function HeroWindowItemCustomization:_option_selected(input_index, ignore_sound)
	local parent = self._parent
...
function HeroWindowItemCustomization:_setting_option_pressed(widget)
	local content = widget.content
...
function HeroWindowItemCustomization:_set_setting_option_selected(widget, index, select_all)
	local content = widget.content
	local num_options = content.num_options
	if num_options then
		for i = 1, num_options do
...
function HeroWindowItemCustomization:_handle_new_selection(input_index)
	local num_inputs = #self._available_states
	input_index =
	input_index and math.clamp(input_index, 1, num_inputs)
...
function HeroWindowItemCustomization:_update_animations(dt)
...
function HeroWindowItemCustomization:_animate_state_transition(dt)
...
function HeroWindowItemCustomization:_draw(input_service, dt)
...
function HeroWindowItemCustomization:_state_draw_overview(ui_renderer, dt)
...
function HeroWindowItemCustomization:_state_draw_property_reroll(ui_renderer, dt)
...
function HeroWindowItemCustomization:_handle_gamepad_activity()
...
function HeroWindowItemCustomization:_update_item_rarity()
	local item = self:_get_item(self._item_backend_id)
	local item_data = item.data
	...
	local active_color = { 255, math.floor(rarity_color [2] * color_multiplier), math.floor(rarity_color [3] * color_multiplier), math.floor(rarity_color [4] * color_multiplier) }
...
function HeroWindowItemCustomization:_update_property_option()
	local item = self:_get_item(self._item_backend_id)
...
function HeroWindowItemCustomization:_update_upgrade_option()
	...
	if rarity_name_color then
		...
		local active_color = { 255, math.floor(rarity_name_color [1] * color_multiplier), math.floor(rarity_name_color [2] * color_multiplier), math.floor(rarity_name_color [3] * color_multiplier) }
...
function HeroWindowItemCustomization:_update_trait_option()
	local ui_renderer = self._ui_top_renderer
	local ui_scenegraph = self._ui_scenegraph
	local item = self:_get_item(self._item_backend_id)
	...
	if traits then
		...
		for index, trait_key in ipairs(traits) do
			...
			new_height = new_height + text_height
			if new_height ~= content.size [2] then --[[ Nothing --]]
			end
...
function HeroWindowItemCustomization:_present_item(item, ignore_spin, custom_spawn_position)
...
function HeroWindowItemCustomization:_spawn_item_unit(item, ignore_spin, custom_spawn_position)
	if self._previewer then
		self._previewer:destroy()
	end
	...
	local preview_position = custom_spawn_position or { 0, 1, 0 }
...
function HeroWindowItemCustomization:cb_on_item_loaded(item_key, ignore_spin)
	print("cb_on_item_loaded", item_key)
	local previewer = self._previewer
	previewer:present_item(item_key, ignore_spin)
end

function HeroWindowItemCustomization:_select_illusion_by_key(key, ignore_item_spawn, mark_as_equipped)
	if not key then
		return
	end
...
function HeroWindowItemCustomization:_on_illusion_index_pressed(index, ignore_item_spawn, mark_as_equipped)
	local illusion_widgets = self._illusion_widgets
	...
	if not ignore_item_spawn then
		...
		local item = { data = item_data, skin = skin_key }
...
local EMPTY_TABLE = { }
function HeroWindowItemCustomization:_setup_illusions(item)
	local item_key = item.key
	local item_data = item.data
			...
			for _, skin in ipairs(weapon_skins) do
				if not used_skins [skin] then
					if not rarity_settings [weapon_skins_rarity] then
						...
						if weapon_skin_data then weapon_skins_rarity = weapon_skin_data.rarity or weapon_skins_rarity
						end
					end
...
function HeroWindowItemCustomization:_state_setup_overview()
	...
	if is_weapon then
		...
		if keywords then
			...
			local text_style = { word_wrap = true, font_size = 28, localize = false, use_shadow = true, horizontal_alignment = "center", vertical_alignment = "center", font_type = "hell_shark",
	...
	self:_update_state_craft_button(self._current_recipe_name, Localize("input_description_apply"), nil, nil, { 0, -40, 0 })
...
function HeroWindowItemCustomization:_state_setup_property_reroll()
...
function HeroWindowItemCustomization:_enable_craft_button(enable, disable_edges)
...
function HeroWindowItemCustomization:_update_state_craft_button(recipe_name, button_text, glow_color, force_disable, offset)
	...
	self._ui_scenegraph.craft_button.local_position = offset or { 0, 0, 0 }
end

local FALLBACK_WEAPON_DIAGRAM = { 0, 0, 0, 0, 0 }
function HeroWindowItemCustomization:_create_weapon_diagram_widget(item_template)
...
function HeroWindowItemCustomization:_create_item_feature_widget(title_text, value_text, value_texture)
	local masked = false
...
function HeroWindowItemCustomization:_create_description_widget(scenegraph_id, text, text_style)
	local masked = false

	text_style = text_style or { word_wrap = true, font_size = 20, localize = false, use_shadow = true, horizontal_alignment = "left", vertical_alignment = "top",
...
function HeroWindowItemCustomization:_create_property_option_entry(text, value_range_text)
...
function HeroWindowItemCustomization:_set_scroll_area_height(widget, height)
	self._ui_scenegraph.scroll_area.size [2] = height
	widget.style.mask.size [2] = height
end

function HeroWindowItemCustomization:_destroy_scrollbar()
...
function HeroWindowItemCustomization:_initialize_scrollbar(content_length, step_size)
	local widgets_by_name = self._widgets_by_name
	local ui_scenegraph = self._ui_scenegraph
...
function HeroWindowItemCustomization:_update_scroll_position()
	local scrollbar_logic = self._scrollbar_logic
...
function HeroWindowItemCustomization:_create_material_requirement_widgets(recipe_name)
	...
	for index, data in ipairs(ingredients) do
		if not data.catergory then
			...
			content.item = { data = table.clone(ItemMasterList [item_key]) }
...
function HeroWindowItemCustomization:_play_sound(event)
	self._parent:play_sound(event)
end

function HeroWindowItemCustomization:_state_setup_trait_reroll()
...
function HeroWindowItemCustomization:_create_trait_option_entry(title_text, description_text, icon)
	local ui_top_renderer = self._ui_top_renderer
...
function HeroWindowItemCustomization:_state_draw_trait_reroll(ui_renderer, dt)
...
function HeroWindowItemCustomization:_state_setup_upgrade()
	self:_destroy_scrollbar()
...
function HeroWindowItemCustomization:_state_draw_upgrade(ui_renderer, dt)
...
function HeroWindowItemCustomization:_craft(ingredients, recipe_override)
	local item = self:_get_item(self._item_backend_id)
	local backend_id = item.backend_id
	...
	if craft_id then
		...
		self._current_crafting_data = { craft_id = craft_id, state_name = self._state }
...
function HeroWindowItemCustomization:_update_craft_response()
...
function HeroWindowItemCustomization:_craft_completed(result)
	...
	function self._animation_callbacks.on_crafting_exit() widget.content.active = false end
	self._item_dirty = true
...
function HeroWindowItemCustomization:_apply_weapon_skin_craft_complete(result)
	local item = self:_get_item(self._item_backend_id)
	local item_key = item.key
...
function HeroWindowItemCustomization:_update_skin_gamepad_input(input_service, dt, t)
...
function HeroWindowItemCustomization:_upgrade_item_craft_complete(result)
