...
function HeroWindowWeaveForgeOverview:on_enter(params, offset)
	...
	self.weapon_crafting_tutorial = not WeaveOnboardingUtils.tutorial_completed(ui_onboarding_state, WeaveUITutorials.equip_weapon) and
	WeaveOnboardingUtils.reached_requirements(onboarding_step, WeaveUITutorials.equip_weapon)
	self.forge_upgrade_tutorial = not WeaveOnboardingUtils.tutorial_completed(ui_onboarding_state, WeaveUITutorials.forge_upgrade) and
	WeaveOnboardingUtils.reached_requirements(onboarding_step, WeaveUITutorials.forge_upgrade)
...
function HeroWindowWeaveForgeOverview:_setup_definitions()
...
function HeroWindowWeaveForgeOverview:_start_transition_animation(animation_name)
	local params = {
		parent = self._parent,
...
function HeroWindowWeaveForgeOverview:create_ui_elements(params, offset)
...
function HeroWindowWeaveForgeOverview:_play_sound(event)
	self._parent:play_sound(event)
end

function HeroWindowWeaveForgeOverview:_initialize_viewports()
	local weapon_crafting_tutorial = self.weapon_crafting_tutorial
	local amulet_introduced = self.amulet_introduced
	local slots = { { slot_name = "slot_melee", no_item_sub_title_text = "inventory_screen_melee_weapon_title",


			no_item_for_tutorial = weapon_crafting_tutorial },

		{ unit_name = "units/ui/pup_weave_amulet/pup_weave_amulet", package_name = "units/ui/pup_weave_amulet/pup_weave_amulet",



			optional_title = Localize("weave_amulet_name"),
			hidden_for_tutorial = not amulet_introduced },
		{ slot_name = "slot_ranged", no_item_sub_title_text = "inventory_screen_ranged_weapon_title",
	...
	for i, slot_data in ipairs(slots) do
		...
		if change_button and weapon_crafting_tutorial then
			...
			self._ui_animations ["change_button_pulse" .. i] = UIAnimation.init(UIAnimation.pulse_animation,
			change_button.style.texture_highlight.color, 1, 100, 255, 2)
		end
...
function HeroWindowWeaveForgeOverview:_create_item_previewer(viewport_widget, item, x_offset, invert_start_rotation)
	local item_data = item.data
	...
	local preview_position = { x_offset, 3, 0 }
...
function HeroWindowWeaveForgeOverview:_create_unit_previewer(viewport_widget, unit_name, package_name)
	local viewport_pass_data = viewport_widget.element.pass_data [1]
	local viewport = viewport_pass_data.viewport
	local world = viewport_pass_data.world

	local preview_position = { 0, 2.8, 0 }
...
function HeroWindowWeaveForgeOverview:cb_unit_spawned_unit_preview(unit_previewer, unit_name)
	print("cb_unit_spawned_unit_preview", unit_previewer, unit_name)
end

function HeroWindowWeaveForgeOverview:cb_unit_spawned_item_preview(item_previewer, item_key)
	local ignore_spin = true
	item_previewer:present_item(item_key, ignore_spin)
end


function HeroWindowWeaveForgeOverview:_create_viewport_definition(scenegraph_id, invert_rendering)
	...
	return {
		...
		style = {
			viewport = { layer = 801, viewport_type = "default_forward", enable_sub_gui = false, fov = 20,
...
function HeroWindowWeaveForgeOverview:on_exit(params)
...
function HeroWindowWeaveForgeOverview:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function HeroWindowWeaveForgeOverview:post_update(dt, t)
...
function HeroWindowWeaveForgeOverview:_update_animations(dt)
	...
	if self._viewports_data then
		for _, data in ipairs(self._viewports_data) do
			...
			if customize_button then --[[ Nothing --]]
			end
...
function HeroWindowWeaveForgeOverview:_is_button_pressed(widget)
...
function HeroWindowWeaveForgeOverview:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot or content.hotspot
	return hotspot.on_hover_enter and not hotspot.is_selected
end

function HeroWindowWeaveForgeOverview:_is_button_hover(widget)
	local content = widget.content
	local hotspot = content.button_hotspot or content.hotspot
	return hotspot.is_hover
end

function HeroWindowWeaveForgeOverview:_is_button_hover_exit(widget)
	local content = widget.content
	local hotspot = content.button_hotspot or content.hotspot
	return hotspot.on_hover_exit and not hotspot.is_selected
end

function HeroWindowWeaveForgeOverview:_is_button_selected(widget)
	local content = widget.content
	local hotspot = content.button_hotspot or content.hotspot
	return hotspot.is_selected
end

function HeroWindowWeaveForgeOverview:_sync_backend_loadout()
	local backend_manger = Managers.backend
	local backend_interface_items = backend_manger:get_interface("items")
	local backend_interface_weaves = backend_manger:get_interface("weaves")
...
function HeroWindowWeaveForgeOverview:_handle_input(dt, t)
	local parent = self._parent
	local widgets_by_name = self._widgets_by_name
	local gamepad_active = Managers.input:is_device_active("gamepad")
...
function HeroWindowWeaveForgeOverview:_draw(dt)
	local parent = self._parent
...
function HeroWindowWeaveForgeOverview:_set_forge_upgrade_price_by_level(forge_level, current_essence_amount)
...
function HeroWindowWeaveForgeOverview:_upgrade_forge_cb(success)
	self._upgrade_forge_response = success
end

function HeroWindowWeaveForgeOverview:_upgrade_forge_done(success)
...
function HeroWindowWeaveForgeOverview:_upgrade_forge()
...
function HeroWindowWeaveForgeOverview:_setup_upgrade_tooltip(athanor_level, max_athanor_level)
	local widgets_by_name = self._widgets_by_name
	local upgrade_button = widgets_by_name.upgrade_button
...
function HeroWindowWeaveForgeOverview:_set_essence_upgrade_cost(essence_amount, can_afford)
	local widgets_by_name = self._widgets_by_name
...
function HeroWindowWeaveForgeOverview:_set_forge_level(level)
	local widgets_by_name = self._widgets_by_name
