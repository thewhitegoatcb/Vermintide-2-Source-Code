...
function ImguiCareerDebug:init()
...
function ImguiCareerDebug:_get_profile_requester()
...
function ImguiCareerDebug:_get_profile_synchronizer()
...
function ImguiCareerDebug:destroy()
	self:unregister_events()
end

function ImguiCareerDebug:register_events()
	local event_manager = Managers.state.event
	if event_manager then --[[ Nothing --]]
	end

end

function ImguiCareerDebug:unregister_events()
	local event_manager = Managers.state.event
	if event_manager then --[[ Nothing --]]
	end
...
function ImguiCareerDebug:is_persistent()
	return self._is_persistent
end

function ImguiCareerDebug:update()
...
function ImguiCareerDebug:_update_profiles_and_careers()
	self._profiles = { }
	self._careers = { }
...
function ImguiCareerDebug:_update_players()
	self._players = Managers.player:players()
end

function ImguiCareerDebug:draw()
...
local header = { "Name", "Profile", "Career", "Is Bot", "Is Server" }
...
function ImguiCareerDebug:_draw_players()
...
function ImguiCareerDebug:_draw_profile_combo(player)
...
function ImguiCareerDebug:_draw_career_combo(player)
	local profile_index = player:profile_index()
...
function ImguiCareerDebug:_find_who_uses_profile(profile_index)
...
function ImguiCareerDebug:_set_columns(num_columns, border, columns_width)
...
function ImguiCareerDebug:_reset_columns()
	self:_set_columns(1)
end

local indent_width = 8
function ImguiCareerDebug:_indent()
	self._indent_counter = self._indent_counter + 1
	Imgui.indent(indent_width)
end

function ImguiCareerDebug:_unindent()
	self._indent_counter = self._indent_counter - 1
	Imgui.unindent(indent_width)
end

function ImguiCareerDebug:_verify_indent()
