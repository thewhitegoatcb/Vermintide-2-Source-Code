...
function ControllerFeaturesImplementation:init(is_in_inn)
	self:_reset()
...
function ControllerFeaturesImplementation:_reset()
	self._effects = { }
...
function ControllerFeaturesImplementation:event_end_conditions_met()
	self._game_mode_ended = true
end

local EFFECTS_TO_REMOVE = { }

function ControllerFeaturesImplementation:update(dt, t)
...
function ControllerFeaturesImplementation:add_effect(effect_name, params, user_id)
	...
	if ControllerFeaturesSettings [effect_name] then
		...
		self._effects [user_id] [self._current_effect_id] = { state_data = state_data, effect = effect }
...
function ControllerFeaturesImplementation:stop_effect(effect_id)
...
function ControllerFeaturesImplementation:destroy()
	for user, effects in pairs(self._effects) do
		for id, effect_data in pairs(effects) do
			effect_data.effect.destroy(effect_data.state_data)
		end
