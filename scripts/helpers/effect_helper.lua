...
function EffectHelper.play_surface_material_effects(effect_name, world, hit_unit, position, rotation, normal, sound_character, husk, unit, hit_actor)
	...
	if not has_material then
		...
		if not material and script_data.debug_material_effects then
			local drawer = Managers.state.debug:drawer({ mode = "retained", name = "DEBUG_DRAW_IMPACT_DECAL_HIT" })
...
function EffectHelper.play_skinned_surface_material_effects(effect_name, world, hit_unit, position, rotation, normal, husk, enemy_type, damage_sound, no_damage, hit_zone_name, shield_blocked, breed)
	local effect_settings = MaterialEffectMappings [effect_name]

	if not effect_settings then return
	end
	local material = nil
...
function EffectHelper.player_critical_hit(world, is_critical_hit, attacker_unit, target_unit, hit_position)
...
function EffectHelper.player_melee_hit_particles(world, particles_name, hit_position, hit_direction, damage_type, hit_unit)
...
function EffectHelper.play_melee_hit_effects(sound_event, world, hit_position, sound_type, husk, hit_unit)
...
function EffectHelper.play_melee_hit_effects_enemy(sound_event, enemy_hit_sound, world, victim_unit, damage_type, is_husk)
...
function EffectHelper.remote_play_surface_material_effects(effect_name, world, unit, position, rotation, normal, is_server, hit_actor)
...
function EffectHelper.remote_play_skinned_surface_material_effects(effect_name, world, position, rotation, normal, enemy_type, damage_sound, no_damage, hit_zone_name, is_server)
	local network_manager = Managers.state.network
	local effect_name_id = NetworkLookup.surface_material_effects [effect_name]
	local network_safe_position = NetworkUtils.network_safe_position(position)
	if not network_safe_position then
...
function EffectHelper.flow_cb_play_surface_material_effect(effect_name, unit, position, rotation, normal, sound_character, husk, offset, range)
	...
	local hit, hit_position, _, hit_normal, actor = PhysicsWorld.immediate_raycast(physics_world,
	raycast_position,
	raycast_direction,
	raycast_range,
	"closest",
...
function EffectHelper.flow_cb_play_footstep_surface_material_effects(effect_name, unit, object, foot_direction, use_occlusion)
	...
	local hit, position, _, normal, actor = PhysicsWorld.immediate_raycast(physics_world,
	raycast_position,
	raycast_direction,
	raycast_range,
	"closest",
...
local material = { "surface_material" }
local output = { }
function EffectHelper.query_material_surface(hit_unit, position, normal)
