...
function EnergyBarUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function EnergyBarUI:_update_energy(player, dt)
...
function EnergyBarUI:create_ui_elements()
...
function EnergyBarUI:update(dt, t, player)
...
local colors = {
	normal = { 255, 0, 255, 255 } }


function EnergyBarUI:_set_charge_bar_fraction(energy_fraction, is_drainable)
...
function EnergyBarUI:destroy()
	return end

function EnergyBarUI:set_alpha(alpha)
	self.render_settings.alpha_multiplier = alpha
end

function EnergyBarUI:_apply_crosshair_position(x, y)
