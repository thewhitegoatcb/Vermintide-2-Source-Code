...
local BACKFILL_ITEM_ORDER = { "dlc", "bundles", "items", "shilling_items" }
...
local PRODUCT_SORT_ORDER = { default = 15, discounted_item = 7, item = 11, bundle = 2, premium_hat = 5, skin = 12, premium_weapon_skin = 6, premium_item = 3, weapon_skin = 14, discounted_hat = 9, dlc = 1, discounted_skin = 8, premium_skin = 4, hat = 13, discounted_weapon_skin = 10 }
...
function StoreWindowFeatured:on_enter(params, offset)
	...
	self._render_settings = { alpha_multiplier = 0, content_alpha_multiplier = 0, snap_pixel_positions = true }
	...
	Managers.telemetry_events:store_breadcrumbs_changed({ {
			...
			content = { page_name = "featured", text = "Featured" } } })
...
function StoreWindowFeatured:_start_transition_animation(animation_name)
	local params = {
...
function StoreWindowFeatured:_create_ui_elements(params, offset)
...
function StoreWindowFeatured:on_exit(params, force_unload)
...
function StoreWindowFeatured:update(dt, t)
...
function StoreWindowFeatured:post_update(dt, t)
	self:_handle_input(dt, t)
end

function StoreWindowFeatured:_update_animations(dt)
...
function StoreWindowFeatured:_update_rotation_timer(dt)
	...
	if time_until_rotation then
		...
		Utf8.upper(Localize("menu_store_featured_page_banner_title_default")),
		Localize("timer_prefix_time_left"),
		UIUtils.format_duration(math.ceil(time_until_rotation)))
...
function StoreWindowFeatured:_refresh_cb()
	self._refresh_counter = self._refresh_counter - 1
	if self._refresh_counter <= 0 then
		self._parent:go_to_store_path({ "featured" })
	end
end

function StoreWindowFeatured:_initialize_time_until_rotation()
...
function StoreWindowFeatured:_handle_input(dt, t)
	local parent = self._parent
	local content_widgets_by_name = self._content_widgets_by_name
...
function StoreWindowFeatured:_draw(dt)
	local parent = self._parent
...
function StoreWindowFeatured:_play_sound(event)
	self._parent:play_sound(event)
end

function StoreWindowFeatured:_handle_gamepad_activity()
...
function StoreWindowFeatured:_list_index_pressed()
	local grid_widgets = self._grid_widgets
	if grid_widgets then
...
function StoreWindowFeatured:_animate_grid_entries(dt)
	local parent = self._parent
	local grid_widgets = self._grid_widgets
	for _, widget in ipairs(grid_widgets) do
		local content = widget.content
...
function StoreWindowFeatured:_create_product_widgets(layout)
	local widgets = { }
...
function StoreWindowFeatured:_destroy_product_widgets(force_unload)
	local parent = self._parent
...
function StoreWindowFeatured:_align_grid_widgets()
	local max_width = 800
	local spacing = 10
...
function StoreWindowFeatured:_sort_slideshow(slideshow_content)
...
function StoreWindowFeatured:_trim_slideshow(slideshow_content)
...
function StoreWindowFeatured:_append_filtered_slideshow_content(slideshow_content, product_type, settings_table, check_function)
...
function StoreWindowFeatured:_get_default_featured_slideshow_content()
...
function StoreWindowFeatured:_get_default_featured_grid_content(optional_num_items, optional_used_ids)
...
function StoreWindowFeatured:_setup_grid_products(grid_content)
	local layout = { }
	local layout_ids = { }
...
function StoreWindowFeatured:_add_product(product_data)
	...
	elseif product_type == "item" then
		...
		if item then
			product = { item = item, type = product_type, product_id = product_id, settings = { mask_price_strike_through_hack = true } }
...
function StoreWindowFeatured:_setup_slideshow(widget, data)
	local slideshow_content = { }
...
function StoreWindowFeatured:_setup_backend_image_material(reference_name, texture_name, masked)
...
function StoreWindowFeatured:_cb_on_backend_url_loaded(gui, reference_name, texture_name, material_name, result)
...
function StoreWindowFeatured:_cb_on_backend_image_loaded(gui, reference_name, material_name, texture_resource)
...
function StoreWindowFeatured:_create_material_instance(gui, new_material_name, template_material_name, reference_name)
...
function StoreWindowFeatured:_set_material_diffuse_by_resource(gui, material_name, texture_resource)
...
function StoreWindowFeatured:_set_material_diffuse_by_path(gui, material_name, texture_path)
	local material = Gui.material(gui, material_name)
	if material then
...
function StoreWindowFeatured:_handle_slideshow_logic(widget, dt, input_service)
	local content = widget.content
	local style = widget.style
...
function StoreWindowFeatured:_set_slideshow_selected_read_index(widget, index)
	local content = widget.content
	local slideshow_content = content.slideshow_content
...
function StoreWindowFeatured:_on_list_index_selected(index)
...
function StoreWindowFeatured:_on_list_index_pressed(index)
	local layout = self._layout
...
function StoreWindowFeatured:_set_slideshow_animation_progress(widget, progress)
	local content = widget.content
	local style = widget.style
...
function StoreWindowFeatured:_update_hotspot_progress(hotspot, dt)
...
function StoreWindowFeatured:_is_unique_reference_to_material(reference_name)
	local cloned_materials_by_reference = self._cloned_materials_by_reference
...
function StoreWindowFeatured:_reset_cloned_materials()
	local ui_top_renderer = self._ui_top_renderer
	local gui = ui_top_renderer.gui
...
function StoreWindowFeatured:_handle_gamepad_grid_selection(input_service)
		...
		for column, index in ipairs(columns) do
			if index == current_index then
				...
				if input_service:get("move_right_hold_continuous") then
					new_index = columns [column + 1] break end
				if input_service:get("move_up_hold_continuous") then
					if row > 1 and num_columns_on_row == 1 and previous_selected_row and previous_selected_column then
						...
						new_index = gamepad_navigation [row - 1] [previous_selected_column] break
					end
				...
				if input_service:get("move_down_hold_continuous") and row < num_rows and num_columns_on_row == 1 and previous_selected_row and previous_selected_column then
					...
					new_index = next_row_columns [next_column] break
				end
				local next_row_columns = gamepad_navigation [row + 1]
				local next_column = math.min(column, #next_row_columns)
				new_index = next_row_columns [next_column]
...
function StoreWindowFeatured:_select_slideshow_widget(is_selected)
	local content_widgets_by_name = self._content_widgets_by_name
