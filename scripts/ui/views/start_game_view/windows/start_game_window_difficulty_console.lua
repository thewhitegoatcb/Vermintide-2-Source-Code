...
function StartGameWindowDifficultyConsole:on_enter(params, offset)
...
function StartGameWindowDifficultyConsole:_verify_difficulty(difficulty_key)
...
function StartGameWindowDifficultyConsole:_start_transition_animation(animation_name)
	local params = {
...
function StartGameWindowDifficultyConsole:create_ui_elements(params, offset)
...
function StartGameWindowDifficultyConsole:_setup_difficulties()
...
function StartGameWindowDifficultyConsole:_get_difficulty_options()
	return Managers.state.difficulty:get_level_difficulties()
end

function StartGameWindowDifficultyConsole:on_exit(params)
	print("[StartGameWindow] Exit Substate StartGameWindowDifficultyConsole")
	self.ui_animator = nil
	self.parent:set_input_description(nil)
end

function StartGameWindowDifficultyConsole:update(dt, t)
...
function StartGameWindowDifficultyConsole:post_update(dt, t)
	return end

function StartGameWindowDifficultyConsole:_update_animations(dt)
...
function StartGameWindowDifficultyConsole:_is_button_pressed(widget)
	local content = widget.content
...
function StartGameWindowDifficultyConsole:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_enter and not hotspot.is_selected
end

function StartGameWindowDifficultyConsole:_handle_input(dt, t)
	...
	for i = 1, #difficulty_widgets do
		...
		if not content.is_selected and
		self:_is_button_hover_enter(widget) then
...
function StartGameWindowDifficultyConsole:_on_difficulty_selection_confirmed()
...
function StartGameWindowDifficultyConsole:_update_difficulty_selection(navigation_id_change, new_navigation_id)
	local difficulties = self:_get_difficulty_options()
...
function StartGameWindowDifficultyConsole:_get_difficulty_navigation_id_from_difficulty_key(difficulty_key)
	local difficulties = self:_get_difficulty_options()
...
function StartGameWindowDifficultyConsole:_set_selected_difficulty_option(new_difficulty_key)
	local difficulty_widgets = self._difficulty_widgets
	for i = 1, #difficulty_widgets do
		local widget = difficulty_widgets [i]
		local content = widget.content
...
function StartGameWindowDifficultyConsole:_set_info_window(difficulty_key)
	local difficulty_settings = DifficultySettings [difficulty_key]
...
function StartGameWindowDifficultyConsole:_update_difficulty_lock()
	local widgets_by_name = self._widgets_by_name
	local buy_button = widgets_by_name.buy_button
...
function StartGameWindowDifficultyConsole:_update_selected_difficulty_option(difficulty_key)
...
function StartGameWindowDifficultyConsole:draw(dt)
...
function StartGameWindowDifficultyConsole:_play_sound(event)
	self.parent:play_sound(event)
end

function StartGameWindowDifficultyConsole:_is_button_released(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function StartGameWindowDifficultyConsole:_show_storepage(store_page_url, dlc_name)
