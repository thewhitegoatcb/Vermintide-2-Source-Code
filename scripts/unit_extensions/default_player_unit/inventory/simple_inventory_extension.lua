...
SwapFromStorageType = SwapFromStorageType or CreateStrictEnumTable("First", "Unique", "Same",
"SameOrAny",
"UnwieldPrio",
"LowestUnwieldPrio")
...
local consumable_slots = { "slot_potion", "slot_grenade", "slot_healthkit" }
...
function SimpleInventoryExtension:init(extension_init_context, unit, extension_init_data)
...
function SimpleInventoryExtension:update_career_skill_weapon_slot()
...
function SimpleInventoryExtension:update_career_skill_weapon_slot_safe()
	self._queue_update_career_skill_weapon_slot = true
end

function SimpleInventoryExtension:game_object_initialized(unit, unit_go_id)
...
function SimpleInventoryExtension:_send_rpc_add_equipment_buffs(unit_go_id, slot_id, backend_id)
	local function send_equipment_buffs(rpc_name, buffs)
		...
		if num_buffs > 0 then
			...
			rpc_name,
			unit_go_id, slot_id,
			num_buffs, buff_ids, buff_value_type_ids,
			buff_values)
		end
	end
...
function SimpleInventoryExtension:_override_career_skill_item_template(item_data)
...
function SimpleInventoryExtension:add_equipment_by_category(category)
	...
	for i = 1, num_slots do repeat
			local slot = category_slots [i]
			local slot_name = slot.name
			local item = BackendUtils.get_loadout_item(career_name, slot_name)
			local item_data = nil until true
	end
...
function SimpleInventoryExtension:destroy()
...
function SimpleInventoryExtension:_unlink_unit(unit, reason, attachment_node_linking)
...
function SimpleInventoryExtension:drop_equipped_weapons(reason, unit)
	return end

function SimpleInventoryExtension:equipment()
	return self._equipment
end

function SimpleInventoryExtension:update(unit, input, dt, context, t)
...
function SimpleInventoryExtension:recently_acquired(slot_name)
	local slot_data = self.recently_acquired_list [slot_name]
	self.recently_acquired_list [slot_name] = nil
	return slot_data
end

function SimpleInventoryExtension:_update_resync_loadout()
...
function SimpleInventoryExtension:can_wield()
	local equipment = self._equipment
...
function SimpleInventoryExtension:_spawn_attached_units(attached_units)
...
function SimpleInventoryExtension:apply_buffs(buffs_by_buffer, reason, item_name, slot_name)
...
function SimpleInventoryExtension:has_inventory_item(slot_name, item_name)
	local slot_data = self:get_slot_data(slot_name)
...
function SimpleInventoryExtension:add_equipment(slot_name, item_name, unit_template, extra_extension_data, ammo_percent)
	local item_data = nil
...
function SimpleInventoryExtension:show_first_person_inventory_lights(show)
	self._show_first_person_lights = show

	local right_hand_wielded_unit = self._equipment.right_hand_wielded_unit
	if right_hand_wielded_unit and Unit.alive(right_hand_wielded_unit) and
	Unit.has_visibility_group(right_hand_wielded_unit, "normal") then
	...
	if left_hand_wielded_unit and Unit.alive(left_hand_wielded_unit) and
	Unit.has_visibility_group(left_hand_wielded_unit, "normal") then
...
function SimpleInventoryExtension:show_first_person_inventory(show)
...
function SimpleInventoryExtension:show_first_person_ammo(show)
	local equipment = self._equipment
...
function SimpleInventoryExtension:show_third_person_inventory(show)
...
function SimpleInventoryExtension:is_showing_third_person_inventory()
	return self._show_third_person
end

function SimpleInventoryExtension:hot_join_sync(sender)
...
function SimpleInventoryExtension:current_ammo_status(slot_name)
...
function SimpleInventoryExtension:ammo_percentage()
	local current_ammo, max_ammo = self:current_ammo_status("slot_ranged")
	local ammo_percentage = 1
...
function SimpleInventoryExtension:ammo_status()
	local current_ammo, max_ammo = self:current_ammo_status("slot_ranged")
	return current_ammo, max_ammo
end

function SimpleInventoryExtension:current_ammo_kind(slot_name)
	local slot_data = self._equipment.slots [slot_name]
	if not slot_data then
		return
	end
...
function SimpleInventoryExtension:add_ammo_from_pickup(pickup_settings)
	local equipment = self._equipment
	local slots = equipment.slots
...
function SimpleInventoryExtension:_add_ammo_to_slot(slot_name, slot_data, refill_percentage, refill_amount)
	local left_hand_unit = slot_data.left_unit_1p
	local right_hand_unit = slot_data.right_unit_1p
	local ammo_extension = nil

	if
	left_hand_unit and ScriptUnit.has_extension(left_hand_unit, "ammo_system") then
...
function SimpleInventoryExtension:get_item_template(slot_data)
	if slot_data then
		local item_data = slot_data.item_data
		local item_template = slot_data.item_template or BackendUtils.get_item_template(item_data)
...
function SimpleInventoryExtension:get_wielded_slot_item_template()
	local slot_name = self:get_wielded_slot_name()
	local slot_data = self:get_slot_data(slot_name)
	return self:get_item_template(slot_data)
end

function SimpleInventoryExtension:get_wielded_slot_name()
	local equipment = self._equipment
	local wielded_slot = equipment.wielded_slot
...
function SimpleInventoryExtension:get_slot_data(slot_id)
	local equipment = self._equipment
	local slots = equipment.slots
...
function SimpleInventoryExtension:get_wielded_slot_data()
	local slot_name = self:get_wielded_slot_name()
	local slot_data = self:get_slot_data(slot_name)
...
function SimpleInventoryExtension:get_item_name(slot_name)
	local slot_data = self:get_slot_data(slot_name)
...
function SimpleInventoryExtension:get_item_data(slot_name)
	local slot_data = self:get_slot_data(slot_name)
	local item_data = slot_data and slot_data.item_data
...
function SimpleInventoryExtension:create_equipment_in_slot(slot_id, backend_id, ammo_percent)
	local item_data = BackendUtils.get_item_from_masterlist(backend_id)
...
function SimpleInventoryExtension:_queue_item_spawn(slot_name, item_data, skin, ammo_percent)
	if not slot_name or not item_data then
		return
	end

	self._items_to_spawn [slot_name] = { slot_id = slot_name, item_data = item_data, skin = skin, ammo_percent = ammo_percent }
	self.resync_loadout_needed = true
end

function SimpleInventoryExtension:_spawn_resynced_loadout(equipment_to_spawn, skip_wield)
...
local slots_to_check = { slot_ranged = true, slot_melee = true }




function SimpleInventoryExtension:has_unique_ammo_type_weapon_equipped()
	local equipment = self._equipment
...
function SimpleInventoryExtension:has_ammo_consuming_weapon_equipped(ammo_type)
	local equipment = self._equipment
	local inventory_slots = equipment.slots
...
function SimpleInventoryExtension:has_infinite_ammo()
	local equipment = self._equipment
	local inventory_slots = equipment.slots
...
function SimpleInventoryExtension:reset_ammo(slot_name)
	local slot_data = self._equipment.slots [slot_name]
...
function SimpleInventoryExtension:has_full_ammo()
	local equipment = self._equipment
	local inventory_slots = equipment.slots
	...
	for slot_name, slot_data in pairs(inventory_slots) do
		if slots_to_check [slot_name] then
			...
			ammo_extension =
			ammo_extension or ScriptUnit.has_extension(left_hand_unit, "ammo_system") and ScriptUnit.extension(left_hand_unit, "ammo_system")
...
function SimpleInventoryExtension:is_ammo_blocked()
	local equipment = self._equipment
	local inventory_slots = equipment.slots
...
function SimpleInventoryExtension:apply_buffs_to_ammo()
	local equipment = self._equipment
	local inventory_slots = equipment.slots
...
function SimpleInventoryExtension:refresh_buffs_on_ammo()
	local equipment = self._equipment
	local inventory_slots = equipment.slots
...
function SimpleInventoryExtension:drop_level_event_item(slot_data)
	...
	if NetworkUtils.network_safe_position(position) then
		...
		ActionUtils.spawn_pickup_projectile(
		self._world,
		weapon_unit,
		projectile_info.projectile_unit_name,
		projectile_info.projectile_unit_template_name,
		action,
		unit,
		position,
		proj_rotation,
...
function SimpleInventoryExtension:check_and_drop_pickups(drop_reason, override_pos, override_dir)
	local unit = self._unit
	local equipment = self._equipment
	local inventory_slots = equipment.slots
...
function SimpleInventoryExtension:set_loaded_projectile_override(settings)

	self._loaded_projectile_settings_override = settings
end

function SimpleInventoryExtension:_update_loaded_projectile_settings()
...
function SimpleInventoryExtension:get_loaded_projectile_settings()
	return self._loaded_projectile_settings
end


function SimpleInventoryExtension:_update_selected_consumable_slot()
	...
	if self._selected_consumable_slot then
		...
		for _, slot_input_data in pairs(InventorySettings.slots_by_wield_input) do
			if not slot_input_data.loadout_slot and
			input_extension:get(slot_input_data.wield_input) and slots [slot_input_data.name] then
...
function SimpleInventoryExtension:get_selected_consumable_slot_template()
...
function SimpleInventoryExtension:get_selected_consumable_slot_name()
	return self._selected_consumable_slot
end

function SimpleInventoryExtension:resyncing_loadout()
...
function SimpleInventoryExtension:get_item_slot_extension(slot_name, system_name)
	local slot_data = self:get_slot_data(slot_name)
...
function SimpleInventoryExtension:get_num_grimoires()
...
function SimpleInventoryExtension:_get_property_and_trait_buffs(backend_id)
...
function SimpleInventoryExtension:_get_no_wield_required_property_and_trait_buffs(backend_id)
	local backend_items = self._backend_items
...
function SimpleInventoryExtension:_wield_slot(equipment, slot_data, unit_1p, unit_3p, buff_extension)
	...
	if slot_data.right_unit_1p or slot_data.left_unit_1p then
		...
		state_machine or self._profile.default_state_machine
...
function SimpleInventoryExtension:get_equipped_item_names()
	local equipped = { }
...
function SimpleInventoryExtension:testify_wield_weapon(weapon)
...
function SimpleInventoryExtension:start_weapon_fx(fx_name, network_sync)
	local equipment = self._equipment
...
function SimpleInventoryExtension:stop_weapon_fx(fx_name, network_sync)
...
function SimpleInventoryExtension:_stop_all_weapon_fx()
	local world = self._world
...
function SimpleInventoryExtension:has_additional_item_slots(slot_name)
	local additional_items_slot = self._additional_items [slot_name]
	return additional_items_slot ~= nil
end

function SimpleInventoryExtension:can_store_additional_item(slot_name)
	local additional_items_slot = self._additional_items [slot_name]
	return additional_items_slot and #additional_items_slot.items < additional_items_slot.max_slots
end

function SimpleInventoryExtension:has_additional_items(slot_name)
	local additional_items_slot = self._additional_items [slot_name]
	return additional_items_slot and #additional_items_slot.items > 0
end

function SimpleInventoryExtension:get_additional_items(slot_name)
	local additional_items_slot = self._additional_items [slot_name]
	return additional_items_slot and additional_items_slot.items
end

function SimpleInventoryExtension:get_additional_items_table()
	return self._additional_items
end

function SimpleInventoryExtension:get_total_item_count(slot_name)
	local count = 0
...
function SimpleInventoryExtension:store_additional_item(slot_name, item_data, skip_resync)
...
function SimpleInventoryExtension:remove_additional_item(slot_name, item_data, skip_resync)
	local stored_items = self:get_additional_items(slot_name)
...
function SimpleInventoryExtension:has_droppable_item(slot_name, filter_func)
	local has_droppable = false local is_stored = false
...
function SimpleInventoryExtension:get_additional_item_swap_id(stored_items, swap_type, current_item)
	local item_id = nil
	if stored_items then
...
function SimpleInventoryExtension:can_swap_from_storage(slot_name, swap_type, optional_compare_item)
...
function SimpleInventoryExtension:swap_equipment_from_storage(slot_name, swap_type, optional_compare_item)
	...
	if can_swap then
		...
		local resync_data = { slot_id = slot_name, item_data = stored_item }
...
local temp_table = { }
function SimpleInventoryExtension:_resync_stored_items(slot_name)
	local items = self:get_additional_items(slot_name)
	if items then
		local unit_go_id = Managers.state.unit_storage:go_id(self._unit)
		if unit_go_id then
