...
local RPCS = { "rpc_shared_state_set_server_int", "rpc_shared_state_set_server_string", "rpc_shared_state_set_server_bool", "rpc_shared_state_set_int", "rpc_shared_state_set_string", "rpc_shared_state_set_bool", "rpc_shared_state_client_left", "rpc_shared_state_request_sync", "rpc_shared_state_full_sync_complete", "rpc_shared_state_start_atomic_set_server", "rpc_shared_state_end_atomic_set_server" }
...
function SharedState:init(context, spec, is_server, network_server, server_peer_id, own_peer_id)
...
function SharedState:network_context_created(lobby, server_peer_id, own_peer_id, is_server, network_server)
	self._peer_id = own_peer_id
	self._server_peer_id = server_peer_id
	self._is_server = is_server
...
function SharedState:register_rpcs(network_event_delegate)
...
function SharedState:get_revision()
	return self._revision
end



function SharedState:clear_peer_data(peer_id)
...
function SharedState:full_sync()
	if self:_is_destroyed() then
		return
	end
...
function SharedState:unregister_rpcs()
	if self._network_event_delegate then
		self._network_event_delegate:unregister(self)
	end
...
function SharedState:is_peer_fully_synced(peer_id)
	if self._is_server then
		if not self._network_server then
			return false
		end
		if peer_id ~= self._server_peer_id then
			do return self._server_full_sync_complete_mapping [peer_id] end
		else
			do return true end
		end
	else
...
function SharedState:get_key(key_type, peer_id, local_player_id, profile_index, career_index)
...
function SharedState:set_peer(owner, key, value)
	if self:_is_destroyed() then
		return
	end
...
function SharedState:start_atomic_set_server(name)
...
function SharedState:end_atomic_set_server(name)
...
function SharedState:set_server(key, value)
	if self:_is_destroyed() then
		return
	end
...
function SharedState:set_own(key, value)
	self:set_peer(self._peer_id, key, value)
end


function SharedState:get_peer(owner, key)
...
function SharedState:get_own(key)
	return self:get_peer(self._peer_id, key)
end


function SharedState:get_server(key)
...
function SharedState:rpc_shared_state_request_sync(channel_id, context)
	if self:_is_destroyed() then
		return nil
	end
...
function SharedState:rpc_shared_state_full_sync_complete(channel_id, context)
	if self:_is_destroyed() then
		return
	end
...
function SharedState:rpc_shared_state_set_int(channel_id, context, owner, key_type_lookup, peer_id, local_player_id, profile_index, career_index, encoded_value)
	if self:_is_destroyed() then
		return
	end
...
function SharedState:rpc_shared_state_set_string(channel_id, context, owner, key_type_lookup, peer_id, local_player_id, profile_index, career_index, encoded_value, complete)
	if self:_is_destroyed() then
		return
	end
	...
	elseif complete then
		...
		self._batched_string_buffer = { encoded_value }
	end

end

function SharedState:rpc_shared_state_set_bool(channel_id, context, owner, key_type_lookup, peer_id, local_player_id, profile_index, career_index, encoded_value)
	if self:_is_destroyed() then
		return
	end
...
function SharedState:rpc_shared_state_set_server_int(channel_id, context, key_type_lookup, peer_id, local_player_id, profile_index, career_index, encoded_value)
	if self:_is_destroyed() then
		return
	end
...
function SharedState:rpc_shared_state_set_server_string(channel_id, context, key_type_lookup, peer_id, local_player_id, profile_index, career_index, encoded_value, complete)
	if self:_is_destroyed() then
		return
	end
...
function SharedState:rpc_shared_state_set_server_bool(channel_id, context, key_type_lookup, peer_id, local_player_id, profile_index, career_index, encoded_value)
	if self:_is_destroyed() then
		return
	end
...
function SharedState:rpc_shared_state_client_left(channel_id, context, peer_id)
	if self:_is_destroyed() then
		return
	end
...
function SharedState:_set_rpc(sender_channel_id, owner, key_type_lookup, peer_id, local_player_id, profile_index, career_index, encoded_value)
...
function SharedState:_set_server_rpc(sender_channel_id, key_type_lookup, peer_id, local_player_id, profile_index, career_index, encoded_value)
...
function SharedState:_send_all(channel_id, player_id, player_state)
...
function SharedState:_clear_peer_id_data(peer_id)
...
function SharedState:_is_destroyed()
	return self._server_state == nil
end

function SharedState:_increment_revision()
...
function SharedState:rpc_shared_state_start_atomic_set_server(context)
	if self:_is_destroyed() then
		return
	end
...
function SharedState:rpc_shared_state_end_atomic_set_server(context)
	if self:_is_destroyed() then
		return
	end
...
function SharedState.validate_spec(spec)
