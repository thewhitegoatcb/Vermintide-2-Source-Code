...
function ProfileSynchronizer:init(is_server, lobby_object, network_state)
...
local RPCS = { "rpc_assign_peer_to_profile" }



function ProfileSynchronizer:register_rpcs(network_event_delegate, network_transmit)

	self._network_event_delegate = network_event_delegate
	self._network_event_delegate:register(self, unpack(RPCS))
end

function ProfileSynchronizer:unregister_network_events()

	self._network_event_delegate:unregister(self)
end

function ProfileSynchronizer:destroy()

	unload_all_packages(self._state)
end

function ProfileSynchronizer:update()
	update_local_packages(self._state)
end

function ProfileSynchronizer:hot_join_sync(peer_id)
...
function ProfileSynchronizer:clear_peer_data(peer_id)
	fassert(self._state:is_server(), "only for the server")
...
function ProfileSynchronizer:get_profile_index_reservation(profile_index)
	local peer_id = self._state:get_profile_index_reservation(profile_index)
	return peer_id
end


function ProfileSynchronizer:try_reserve_profile_for_peer(peer_id, profile_index)
...
function ProfileSynchronizer:clear_profile_index_reservation(peer_id)
...
function ProfileSynchronizer:reset_profile_index_reservation(profile_index)
	self._state:set_profile_index_reservation(profile_index, "")
end

function ProfileSynchronizer:profile_by_peer(peer_id, local_player_id)
	local peers_with_full_profiles = self._state:get_peers_with_full_profiles()
...
function ProfileSynchronizer:get_peers_with_full_profiles()
	return self._state:get_peers_with_full_profiles()
end


function ProfileSynchronizer:assign_full_profile(peer_id, local_player_id, profile_index, career_index, is_bot)
...
function ProfileSynchronizer:unassign_profiles_of_peer(peer_id, optional_local_player_id)

	self:_unassign_profiles_of_peer(peer_id, optional_local_player_id)
end

function ProfileSynchronizer:get_first_free_profile()
...
function ProfileSynchronizer:is_profile_in_use(profile_index)
	local peers_with_full_profiles = self._state:get_peers_with_full_profiles()
...
function ProfileSynchronizer:all_synced()
...
function ProfileSynchronizer:all_synced_for_peer(peer_id, local_player_id)
	local ignore_loading_peers = false
	return self:_all_synced_for_peer(peer_id, local_player_id, ignore_loading_peers)
end


function ProfileSynchronizer:all_ingame_synced_for_peer(peer_id, local_player_id)
	local ignore_loading_peers = true
	return self:_all_synced_for_peer(peer_id, local_player_id, ignore_loading_peers)
end

function ProfileSynchronizer:_all_synced_for_peer(peer_id, local_player_id, ignore_loading_peers)
	local current_revision = self._state:get_revision()
	...
	if not cached_data or cached_data [cached_data_revision_key] ~= current_revision then
		...
		cached_data = { current_revision, result }
...
function ProfileSynchronizer:is_peer_all_synced(peer_id)
	local peers_with_full_profiles = self._state:get_peers_with_full_profiles()
...
function ProfileSynchronizer:resync_loadout(peer_id, local_player_id)
...
function ProfileSynchronizer:rpc_assign_peer_to_profile(channel_id, peer_id, local_player_id, profile_index, career_index, is_bot)
...
function ProfileSynchronizer:_clear_profile_index_reservation(peer_id)
	for profile_index = 1, NUM_HERO_PROFILES do
		local profile_reserver_peer_id = self._state:get_profile_index_reservation(profile_index)
...
function ProfileSynchronizer:_unassign_profiles_of_peer(peer_id, optional_local_player_id)
	local peers_with_full_profiles = self._state:get_peers_with_full_profiles()
...
function ProfileSynchronizer:_assign_peer_to_profile(peer_id, local_player_id, profile_index, career_index, is_bot)
...
function ProfileSynchronizer:_sync_lobby_data()
	fassert(self._state:is_server(), "Should only be called on server.")
...
function ProfileSynchronizer:get_packed_lobby_profile_slots()
	local peers = { }
...
function ProfileSynchronizer:set_own_actually_ingame(actually_ingame)
	self._state:set_own_actually_ingame(actually_ingame)
end

function ProfileSynchronizer:others_actually_ingame()
	...
	for _, peer in ipairs(peers_with_full_profiles) do
		...
		local peer_id = peer.peer_id
		if
		own_peer_id ~= peer_id and not network_state:get_actually_ingame(peer_id) then
			return false
		end
	end
...
function ProfileSynchronizer.is_free_in_lobby(profile_index, lobby_data)
...
function ProfileSynchronizer.unpack_lobby_profile_slots(peer_ids, player_indices, lobby_data)
...
function ProfileSynchronizer.owner_in_lobby(profile_index, lobby_data)
	local slot_name = lobby_slot_name(profile_index)
	local owner = lobby_data [slot_name]
	if owner == nil then
		return
