...
local input_actions_by_slot = { slot_potion = "wield_4", slot_grenade = "wield_5", slot_healthkit = "wield_3", slot_career_skill_weapon = "weapon_reload", slot_melee = "wield_1", slot_ranged = "wield_2" }
...
local allowed_equipment_slots = { slot_grenade = true, slot_healthkit = true, slot_potion = true, slot_career_skill_weapon = true, slot_melee = true, slot_ranged = true }
...
function EquipmentUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function EquipmentUI:_create_ui_elements()
...
function EquipmentUI:event_input_changed()
...
function EquipmentUI:on_spectator_target_changed(spectated_player_unit)
...
function EquipmentUI:event_swap_equipment_from_storage(slot_name, additional_items)
...
function EquipmentUI:_set_slot_input(widget, slot_name)
	...
	input_text =
	input_text and UIRenderer.crop_text_width(ui_renderer, input_text, max_length, input_style)
...
function EquipmentUI:_get_input_texture_data(input_action)
	...
	if device_type == "keyboard" then
		if is_button_unassigned then button_name = "" else button_name = Keyboard.button_locale_name(key_index) end
...
function EquipmentUI:_update_widgets()
...
function EquipmentUI:_get_wield_scroll_input()
...
function EquipmentUI:_set_wielded_item(wielded_slot_name)
...
function EquipmentUI:_sync_player_equipment()
	local player = self._is_spectator and self._spectated_player or self.player
	local player_unit = player.player_unit
	...
	for i = 1, num_inventory_slots do
		...
		if allowed_equipment_slots [slot_name] then
			...
			if slot_name ~= "slot_career_skill_weapon" or is_cog then
				...
				if slot_name == "slot_career_skill_weapon" and item_data and widget_id > 0 then
					...
					if widget then
						...
						local can_reload = equipment.wielded_slot == "slot_career_skill_weapon" and
						heat > 0 and
						not is_exhausted
...
function EquipmentUI:_update_ammo_count(item_data, slot_data, player_unit)
...
function EquipmentUI:_animate_ammo_counter(dt)
...
function EquipmentUI:_set_ammo_counter_alpha(alpha)
	local ammo_widgets_by_name = self._ammo_widgets_by_name
...
function EquipmentUI:_set_ammo_counter_color(color)
...
function EquipmentUI:_set_ammo_text_focus(focus)
...
function EquipmentUI:_get_ammunition_count(left_hand_wielded_unit, right_hand_wielded_unit, item_template)
...
function EquipmentUI:_get_overcharge_amount(player_unit)
...
function EquipmentUI:_add_animation(name, widget, style, func_name)
...
function EquipmentUI:_update_animations(dt, t)
...
function EquipmentUI:_animate_slot_wield(animation_data, dt)
	local widget = animation_data.widget
...
function EquipmentUI:_animate_slot_unwield(animation_data, dt)
	local widget = animation_data.widget
	local total_time = animation_data.total_time
	local time = animation_data.time
...
function EquipmentUI:_animate_slot_equip(animation_data, dt)
...
function EquipmentUI:_add_item(slot_data, data)
...
function EquipmentUI:_remove_item(index)
	local num_added_items = self._num_added_items or 0
...
function EquipmentUI:set_position(x, y)
...
function EquipmentUI:set_visible(visible)
	self._is_visible = visible

	self:_set_elements_visible(visible)
end

function EquipmentUI:_set_elements_visible(visible)
	local ui_renderer = self.ui_renderer
...
local customizer_data_player_status = { lock_y = true, registry_key = "player_status", drag_scenegraph_id = "background_panel", root_scenegraph_id = "root", label = "Player status", lock_x = false }
...
local customizer_data_ammo = { root_scenegraph_id = "ammo_background", label = "Ammo", registry_key = "ammo", drag_scenegraph_id = "ammo_background" }
...
function EquipmentUI:update(dt, t)
...
function EquipmentUI:_handle_resolution_modified()
	if RESOLUTION_LOOKUP.modified then
		self:_on_resolution_modified()
	end
end

function EquipmentUI:_on_resolution_modified()
	for _, widget in ipairs(self._widgets) do
		self:_set_widget_dirty(widget)
	end
	for _, widget in ipairs(self._static_widgets) do
...
function EquipmentUI:_handle_gamepad()
...
function EquipmentUI:_set_color(color, new_color, ignore_alpha)
...
function EquipmentUI:set_dirty()
	self._dirty = true
...
function EquipmentUI:_set_widget_dirty(widget)
...
function EquipmentUI:on_gamepad_activated()
	self:_update_widgets()
end

function EquipmentUI:on_gamepad_deactivated()
	self:_update_widgets()
end

function EquipmentUI:_set_overheat_fraction(fraction)
...
function EquipmentUI:_show_overheat_meter(visible)
	local widgets_by_name = self._widgets_by_name
	local ammo_widgets_by_name = self._ammo_widgets_by_name
...
function EquipmentUI:_set_widget_visibility(widget, visible)
	widget.content.visible = visible
	self:_set_widget_dirty(widget)
end

function EquipmentUI:set_alpha(alpha)
...
function EquipmentUI:set_ammo_alpha(alpha)
...
function EquipmentUI:set_panel_alpha(alpha)
...
function EquipmentUI:_apply_crosshair_position(x, y)
...
function EquipmentUI:_show_hold_to_reload(t)
	local player = self._is_spectator and self._spectated_player or self.player
	local player_unit = player.player_unit
	if not player_unit then return
	end
	local inventory_extension = ScriptUnit.extension(player_unit, "inventory_system")
	local equipment = inventory_extension:equipment()
...
function EquipmentUI:_update_reload_ui_state(t, item_template)
	local reload_tip_widget = self._ammo_widgets_by_name.reload_tip_text
