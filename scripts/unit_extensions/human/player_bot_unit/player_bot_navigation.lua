...
function PlayerBotNavigation:init(extension_init_context, unit, extension_init_data)
	self._unit = unit
...
function PlayerBotNavigation:destroy()
...
function PlayerBotNavigation:reset()
	return end

function PlayerBotNavigation:update(unit, input, dt, context, t)
...
function PlayerBotNavigation:move_to(target_position, callback)
	...
	local above = 0.75 local below = 0.5
...
function PlayerBotNavigation:teleport(destination)
...
function PlayerBotNavigation:stop()
	local current_position = POSITION_LOOKUP [self._unit]
	self:teleport(current_position)
end

function PlayerBotNavigation:is_path_safe_from_vortex(path_check_distance, min_allowed_vortex_distance)
	...
	local should_end = false local check_node = true
...
function PlayerBotNavigation:_update_path(t)
	local path = self._path
	if not path or self._final_goal_reached then
		self._current_transition = nil
		return
...
function PlayerBotNavigation:_reevaluate_current_nav_transition(self_unit, self_position, current_goal, new_goal)
...
function PlayerBotNavigation:_goal_reached(position, goal, previous_goal, t)
...
function PlayerBotNavigation:current_goal()
...
function PlayerBotNavigation:is_following_last_goal()
	if self._final_goal_reached then
		do return false end
	elseif self._path then
		do return self._path_index == #self._path end
	elseif self._last_path then
		do return self._last_path_index == #self._last_path end
	else
		return false
	end
end

function PlayerBotNavigation:destination_reached()
	return self._final_goal_reached
end

function PlayerBotNavigation:_update_astar(t)
...
function PlayerBotNavigation:path_callback()
	return self._path_callback
end

function PlayerBotNavigation:_path_failed(t)
...
function PlayerBotNavigation:_path_successful(t)
...
function PlayerBotNavigation:successive_failed_paths()
	return self._successive_failed_paths, self._last_successful_path
end

function PlayerBotNavigation:destination()
	if self._has_queued_target then
...
function PlayerBotNavigation:position_when_destination_reached()
	if self._final_goal_reached then
...
function PlayerBotNavigation:_debug_draw_path(position, previous_goal, current_goal)
...
function PlayerBotNavigation:is_in_transition()
	return self._current_transition ~= nil
end

function PlayerBotNavigation:transition_type()
	local transition = self._current_transition
	return transition.type
end

function PlayerBotNavigation:transition_requires_jump(position, direction)
...
function PlayerBotNavigation:flow_cb_entered_nav_transition(transition_unit, actor)
	...
	local transition = { type = type, from = Vector3Box(from), to = Vector3Box(to) }
	if type ~= "ladder" then
...
function PlayerBotNavigation:flow_cb_left_nav_transition(transition_unit, actor)
	local transitions = self._available_nav_transitions
	local index = Unit.get_data(transition_unit, "bot_nav_transition_manager_index")
...
function PlayerBotNavigation:traverse_logic()
	return self._traverse_data
end

function PlayerBotNavigation:add_transition(transition_unit, type, from, to)
	local transition = { unit = transition_unit, type = type, from = Vector3Box(from), to = Vector3Box(to) }
	local transitions = self._available_nav_transitions
	transitions [transition_unit] = transition
end

function PlayerBotNavigation:remove_transition(transition_unit)
	local transitions = self._available_nav_transitions
	transitions [transition_unit] = nil
end