...
function GenericHitReactionExtension:init(extension_init_context, unit, extension_init_data)
...
function GenericHitReactionExtension:set_hit_effect_template_id(template_id)

	self.hit_effect_template = template_id
end

function GenericHitReactionExtension:extensions_ready(world, unit)
...
function GenericHitReactionExtension:destroy()
	return end

function GenericHitReactionExtension:unfreeze()
	self._delayed_animation = nil
	self._delayed_flow = nil
	self._delayed_push = nil
end

function GenericHitReactionExtension:reset()
	return end
...
function GenericHitReactionExtension:update(unit, input, dt, context, t)
...
function GenericHitReactionExtension:_resolve_effects(effect_conditions, results)
...
function GenericHitReactionExtension:_can_wall_nail(effect_template)
...
function GenericHitReactionExtension:set_death_sound_event_id(playing_id)
	self._death_sound_event_id = playing_id
end

function GenericHitReactionExtension:death_sound_event_id()
	return self._death_sound_event_id
end

local allowed_diagonal_hit_zones = { left_arm = true, right_arm = true, torso = true }





function GenericHitReactionExtension:_check_for_diagonal_dismemberment(unit, actor_name, hit_direction, hit_zone)
...
local restricted_regions = { at = true, de = true }




function GenericHitReactionExtension:_is_dismembering_allowed(parameters)
	if IS_CONSOLE then
...
local status_effect_overrides = {
	bw_necromancer = {
		[StatusEffectNames.burning] = {
			...
			damage_types = table.set({ "burning_stab_fencer", "burning_tank", "heavy_burning_tank", "burn", "burn_sniper", "burn_shotgun", "burn_machinegun", "burn_carbine", "burning_smiter", "light_burning_linesman", "burning_linesman", "drakegun", "drakegun_glance" }) },
...
function GenericHitReactionExtension:_execute_effect(unit, effect_template, effect_biggest_hit, parameters, t, dt)
	...
	if timed_status then
		...
		if overrides_by_status then
			...
			if overrides_by_damage_type and overrides_by_damage_type.damage_types [damage_type] then timed_status = overrides_by_damage_type.override or timed_status
			end
		end
	end
	...
	elseif effect_template.animations and Unit.has_animation_state_machine(unit) then
		...
		if
		animation_event and (has_flow_event or parameters.death) then
	...
	if sound_event then
		...
		map_function(sound_event,
		play_sound,
		wwise_world,
		wwise_source_id,
		WWISE_PARAMETERS.damage_type,
		WWISE_PARAMETERS.enemy_type,
		WWISE_PARAMETERS.weapon_type,
		WWISE_PARAMETERS.hit_zone,
		WWISE_PARAMETERS.husk)
	end
...
function GenericHitReactionExtension:_do_push(unit, dt)
