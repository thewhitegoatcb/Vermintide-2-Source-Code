...
local default_color = { 170, 255, 255, 255 }
...
function ContractLogUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function ContractLogUI:_create_ui_elements()
...
function ContractLogUI:_align_widgets()
...
function ContractLogUI:destroy()
	self:set_visible(false)
end

function ContractLogUI:set_visible(visible)
...
function ContractLogUI:_sync_active_contracts()
	local dirty = false
	...
	if active_contract_ids then
		...
		for _, contract_id in pairs(active_contract_ids) do
			if not log_entries_by_contract_id [contract_id] and
			self.quest_manager:is_contract_able_to_progress(contract_id) then
...
function ContractLogUI:_sync_contract_progression()
	local log_entries = self._log_entries
...
function ContractLogUI:_update_contract_goal(entry_data)
	local contract_id = entry_data.contract_id
	...
	if task then
		...
		if not widget_content.tasks_complete and tasks_complete then
			...
			do return make_dirty, tasks_complete end
		else
			widget_content.tasks_complete = tasks_complete
		end
...
function ContractLogUI:_add_contract(contract_id)
...
function ContractLogUI:_remove_contract(contract_id)
	local num_added_contracts = self.num_added_contracts or 0
...
function ContractLogUI:_get_text_size(text_style, text)
	local ui_renderer = self.ui_renderer
...
function ContractLogUI:update(dt, t)
	local dirty = false
	local realign = false
...
function ContractLogUI:_handle_resolution_modified()
...
function ContractLogUI:_on_resolution_modified()
...
function ContractLogUI:draw(dt)
...
function ContractLogUI:set_dirty()
	self._dirty = true
end

function ContractLogUI:_set_widget_dirty(widget)
	widget.element.dirty = true
end

function ContractLogUI:play_sound(event)
