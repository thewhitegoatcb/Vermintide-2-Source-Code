...
local state = { idle = 0, aggressive = 1 }




local RPCS = { "rpc_client_ward_hot_join_sync" }



function WardExtension:init(extension_init_context, unit, extension_init_data)
...
function WardExtension:destroy()
...
function WardExtension:update(unit, input, dt, context, t)
...
function WardExtension:get_closest_player(ward_pos)
...
function WardExtension:update_state(ward_pos, closest_player)
	if not ward_pos then return
	end
	local player_pos = POSITION_LOOKUP [closest_player]
	if not player_pos then return
	end
	local dist_sq = Vector3.distance_squared(ward_pos, player_pos)
...
function WardExtension:set_defenders_aggressive(closest_player)
...
function WardExtension:spawn_defenders(num_defenders)
	self._spawned = true
	...
	function defender_optional_data.spawned_func(unit, breed, optional_data)
...
function WardExtension:toggle_health_bar_by_proximity(ward_unit)
...
function WardExtension:player_party_changed()
...
function WardExtension:rpc_client_ward_hot_join_sync(spawned)
