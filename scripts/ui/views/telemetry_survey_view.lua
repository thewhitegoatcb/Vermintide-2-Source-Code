...
function TelemetrySurveyView:init(ingame_ui_context)
...
function TelemetrySurveyView:input_service()
	return self.input_manager:get_service("telemetry_survey")
end

function TelemetrySurveyView:set_transition(transition)
...
function TelemetrySurveyView:set_survey_context(survey_context)
	self.survey_context = survey_context
end

function TelemetrySurveyView:get_survey_context()
	return self.survey_context
end

function TelemetrySurveyView:is_survey_answered()
	return self.survey_answered and self.survey_confirmed
end

function TelemetrySurveyView:is_survey_timed_out()
	return self.timed_out
end

function TelemetrySurveyView:create_ui_elements()
...
function TelemetrySurveyView:destroy()
	if self.active then
		self:set_active(false)
	end
end

function TelemetrySurveyView:play_sound(event)
	WwiseWorld.trigger_event(self.wwise_world, event)
end

function TelemetrySurveyView:on_enter()
	self.timed_out = false
	self.survey_confirmed = false
	self:set_active(not self.active)
end

function TelemetrySurveyView:on_exit()
	self:set_active(not self.active)
...
function TelemetrySurveyView:transition()
...
function TelemetrySurveyView:record_telemetry_survey()
...
function TelemetrySurveyView:update(dt)
...
function TelemetrySurveyView:update_time_text(time_left)
...
function TelemetrySurveyView:update_rating_buttons()
...
function TelemetrySurveyView:update_button_disabled()
...
function TelemetrySurveyView:set_active(active)
	self.active = active
...
function TelemetrySurveyView:handle_interaction(dt)
	if self.opened then
		local is_disabled = self.continue_button.content.disabled
...
function TelemetrySurveyView:draw(dt)
