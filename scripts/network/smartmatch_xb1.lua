...
local OPTIONAL_HOPPER_PARAM_LUT = {
	...
	new_stage_hopper = { strict_matchmaking = true },


	safe_profiles_hopper = { strict_matchmaking = true },


	weave_find_group_hopper = { profiles = true, weave_index = true, difficulty = true, powerlevel = true } }
...
function SmartMatch:init(hopper_name, is_host, ticket_params, timeout)
...
function SmartMatch:_create_smartmatch_session()
...
function SmartMatch:_handle_smartmatch_session()
...
function SmartMatch:_start_smartmatch(dt)
...
function SmartMatch:_check_smartmatch_result(dt)
	if not self._ready then
		return
	end
...
function SmartMatch:_smartmatch_done(dt) return end

function SmartMatch:_convert_to_json(hopper_name, params)
...
function SmartMatch:is_search_done()
	return self._done
end

function SmartMatch:results()
	return self._found_session_name, self._found_session_template
end

function SmartMatch:success()
	return not self._failed
end

function SmartMatch:destroy()
