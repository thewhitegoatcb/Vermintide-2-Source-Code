...
local OVERRIDABLE_ACTION_DATA_KEYS = { "damage_profile", "impact_sound_event", "no_damage_impact_sound_event", "slide_armour_hit", "hit_mass_count", "hit_effect", "use_precision_sweep", "invert_attack_direction", "additional_critical_strike_chance", "hit_stop_anim" }
...
local function get_baked_data_name(action_hand)
	if action_hand then
		do return "baked_sweep_" .. action_hand end
	else
		return "baked_sweep"
	end
end

function ActionSweep:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionSweep:check_precision_target(owner_unit, owner_player, dedicated_target_range, check_distance, weapon_furthest_point)
...
function ActionSweep:client_owner_start_action(new_action, t, chain_action_data, power_level, action_init_data)
	...
	if not Managers.player:owner(self.owner_unit).bot_player and damage_profile.charge_value == "heavy_attack" then
		Managers.state.controller_features:add_effect("rumble", { rumble_effect = "light_swing" })
	end
		...
		if results then
			...
			for i = 1, num_results do
				...
				if breed and not friendly_fire then
					...
					if
					hit_zone_name ~= "afro" and HEALTH_ALIVE [hit_unit] then
...
function ActionSweep:client_owner_post_update(dt, t, world, _, current_time_in_action)
	local owner_unit = self.owner_unit
	...
	local aborted = false
	if ( aborted or self._attack_aborted ) and current_action.reset_aim_on_attack and
	not self._auto_aim_reset then
		local first_person_extension = ScriptUnit.extension(owner_unit, "first_person_system")
...
function ActionSweep:_update_sweep(dt, t, current_action, current_time_in_action)
...
function ActionSweep:_update_sweep_baked(dt, t, current_action, current_time_in_action)
	local is_within_damage_window = nil
...
function ActionSweep:_update_sweep_runtime(dt, t, current_action, current_time_in_action)
	local owner_unit = self.owner_unit
	local weapon_unit = self.weapon_unit
	local physics_world = self.physics_world
...
function ActionSweep:_get_power_boost()
	local has_melee_boost = self._has_starting_melee_boost local melee_boost_curve_multiplier = self._starting_melee_boost_curve_multiplier
	if not has_melee_boost then
		...
		self._starting_melee_boost_curve_multiplier = melee_boost_curve_multiplier self._has_starting_melee_boost = has_melee_boost
	end

	return has_melee_boost, melee_boost_curve_multiplier
end

function ActionSweep:_is_within_damage_window(current_time_in_action, action, owner_unit)
...
function ActionSweep:_get_target_hit_mass(difficulty_rank, shield_blocked, current_action, breed, hit_unit_id, hit_unit)
...
function ActionSweep:_calculate_hit_mass(difficulty_rank, actual_hit_target_index, shield_blocked, current_action, breed, hit_unit_id, hit_unit)
...
function ActionSweep:_calculate_hit_mass_level_object(unit, actual_hit_target_index, current_action)
...
function ActionSweep:_calculate_attack_direction(action, weapon_rotation)
...
function ActionSweep:_check_backstab(breed, hit_unit, owner_unit, buff_extension, first_person_extension)
...
function ActionSweep:_send_attack_hit(t, damage_source_id, attacker_unit_id, hit_unit_id, hit_zone_id, hit_position, attack_direction, damage_profile_id, ...)
...
function ActionSweep:_do_overlap(dt, t, unit, owner_unit, current_action, physics_world, is_within_damage_window, current_position, current_rotation)
		...
		if unit_alive(hit_unit) and Vector3.is_valid(hit_position) then
			...
			if is_character and not is_friendly_fire and not hit_self and in_view and (has_hit_precision_target_and_has_last_hit_result or self._hit_units [hit_unit] == nil) then
				...
				if target_settings then
					...
					self.owner.local_player and not self._has_played_rumble_effect then
						if hit_armor then
							Managers.state.controller_features:add_effect("rumble", { rumble_effect = "hit_armor" })
						else
							local hit_rumble_effect = current_action.hit_rumble_effect or "hit_character"
							Managers.state.controller_features:add_effect("rumble", { rumble_effect = hit_rumble_effect })
						end
					...
					if buff_result ~= "killing_blow" then
						self:_send_attack_hit(t, damage_source_id, attacker_unit_id, hit_unit_id, hit_zone_id, hit_position, attack_direction, damage_profile_id,
						"power_level", power_level,
						"hit_target_index", actual_hit_target_index,
						"blocking", shield_blocked,
						"shield_break_procced", shield_break_procc,
						"boost_curve_multiplier", melee_boost_curve_multiplier,
			...
			elseif not is_character and in_view then
				if ScriptUnit.has_extension(hit_unit, "ai_inventory_item_system") then
					...
					if Managers.state.controller_features and
					self.owner.local_player and not self._has_played_rumble_effect then
						Managers.state.controller_features:add_effect("rumble", { rumble_effect = "hit_shield" })
						self._has_played_rumble_effect = true
					end
	...
	if environment_unit_hit and not self._has_hit_environment and
	num_results1 + num_results2 > 0 then
		...
		if unit ~= hit_unit then
			...
			if Managers.state.controller_features and global_is_inside_inn and
			self.owner.local_player and not self._has_played_rumble_effect then
				Managers.state.controller_features:add_effect("rumble", { rumble_effect = "hit_environment" })
				self._has_played_rumble_effect = true
			end
...
function ActionSweep:_play_environmental_effect(weapon_rotation, current_action, hit_unit, hit_position, hit_normal, hit_actor)
...
local sound_events = { javelin_stab_hit = "stab_hit", slashing_hit = "slashing_hit", stab_hit = "stab_hit", Play_weapon_fire_torch_flesh_hit = "burning_hit", hammer_2h_hit = "blunt_hit", axe_2h_hit = "slashing_hit", crowbill_stab_hit = "stab_hit", axe_1h_hit = "slashing_hit", blunt_hit = "blunt_hit" }
...
function ActionSweep:_play_character_impact(is_server, attacker_unit, hit_unit, breed, hit_position, hit_zone_name, current_action, damage_profile, target_index, power_level, attack_direction, blocking, boost_curve_multiplier, is_critical_strike, backstab_multiplier)
	...
	if target_settings then
		...
		predicted_damage, target_invulerable = DamageUtils.calculate_damage(DamageOutput,
		hit_unit,
		attacker_unit,
		hit_zone_name,
		power_level,
	...
	if
	( multiplier_type == "headshot" or multiplier_type == "weakspot" and not blocking ) and not current_action.no_headshot_sound and HEALTH_ALIVE [hit_unit] then
		local first_person_extension = ScriptUnit.extension(owner_unit, "first_person_system")
...
function ActionSweep:hit_level_object(hit_units, hit_unit, owner_unit, current_action, hit_position, attack_direction, level_index, hit_actor)
	hit_units [hit_unit] = true
	self._has_hit_environment = true
...
function ActionSweep:finish(reason, data)
	local current_action = self._current_action
...
function ActionSweep:destroy()
	return end

function ActionSweep:_play_hit_animations(owner_unit, current_action, abort_attack, hit_zone_name, armor_type, blocking, killed_unit)
	...
	local first_person_hit_anim = abort_attack and killed_unit and current_action.hit_stop_kill_anim or
	hit_zone_name ~= "head" and armor_type == 2 and abort_attack and current_action.hit_armor_anim or
	abort_attack and blocking and current_action.hit_shield_stop_anim or abort_attack and hit_stop_anim or
	current_action.first_person_hit_anim
...
function ActionSweep:_get_damage_profile_name(action_hand, action)
	return action_hand and action ["damage_profile_" .. action_hand] or self._overridable_settings.damage_profile or "default"
end

function ActionSweep:_populate_sweep_action_data(action, overrides)
