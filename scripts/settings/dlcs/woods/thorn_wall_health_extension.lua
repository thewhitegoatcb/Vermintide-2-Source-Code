...
function ThornWallHealthExtension:init(extension_init_context, unit, extension_init_data)
	ThornWallHealthExtension.super.init(self, extension_init_context, unit, extension_init_data)
end

function ThornWallHealthExtension:extensions_ready(world, unit, extension_name)
	return end

function ThornWallHealthExtension:destroy()
	ThornWallHealthExtension.super.destroy(self)
end

function ThornWallHealthExtension:apply_client_predicted_damage(predicted_damage)

	return end

local allowed_damage_sources = { chaos_exalted_champion_norsca = true, chaos_exalted_champion_warcamp = true, skaven_storm_vermin_warlord = true }





function ThornWallHealthExtension:add_damage(attacker_unit, damage_amount, hit_zone_name, damage_type, hit_position, damage_direction, damage_source_name, hit_ragdoll_actor, damaging_unit, hit_react_type, is_critical_strike, added_dot, first_hit, total_hits, attack_type)
