...
function Gathering:init(nav_world, traverse_logic, start_pos)
...
local DEBUG_SIDE_COLORS = { "red", "blue", "green", "yellow" }
function Gathering:write_dogpiled_attackers(dogpiled_attackers_on_unit)
	local t = ""
...
function Gathering:respawn_balls(pos, target_unit)
...
function Gathering:add_static_ball(pos, rad, unit)
	local ball = self:add_ball(pos, rad, unit, nil, true)
	self.static_units [unit] = ball
end

function Gathering:remove_static_ball(unit)
	local id = self.static_units [unit].id
	self:remove_ball(id)
	self.static_units [unit] = nil
end

function Gathering:add_ball(pos, rad, owner_unit, target_unit, is_static)
	...
	if not dogpiled_attackers then
		self.dogpiled_attackers_on_unit [target_unit] = { [owner_unit] = ball }
		return ball
	end
...
function Gathering:remove_ball(ball)
...
function Gathering:release_attacking_balls(unit)

	return end

function Gathering:notify_attackers(unit)
...
local result_table = { }
function Gathering:overlap_update(t, dt)
...
function Gathering:slot_vs_slot_overlap(pos, rad, pos2, rad2, ball1, ball2)
...
function Gathering:slot_vs_breed_overlap(pos, rad, pos2, rad2, ball1)
...
function Gathering:update_efficient(t, dt)
	...
	for i = start_index, end_index do repeat
			local ball = balls [i]
			...
			for j = 1, num_units do
				...
				if side_id ~= other_blackboard.side.side_id then
					...
					num_unit_checks = num_unit_checks + 1 end end until true
	end
	...
	for i = 1, self.num_balls do
		...
		if dirty then
			...
			pos [3] = position [3] pos [2] = position [2] pos [1] = position [1]
			last_pos [3] = position [3] last_pos [2] = position [2] last_pos [1] = position [1]
		end
	end
...
function Gathering:update_brute_force(t, dt)
...
function Gathering:update(t, dt)
