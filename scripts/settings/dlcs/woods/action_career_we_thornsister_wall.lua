...
function ActionCareerWEThornsisterWall:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionCareerWEThornsisterWall:client_owner_start_action(new_action, t, chain_action_data, power_level, action_init_data)
	...
	if num_segments > 0 then
		...
		if explosion_template then
			...
			for i = 1, num_segments do
				segment_arr [i] = segments [i]:unbox() + forward * (math.random() * WALL_FORWARD_OFFSET_RANGE * 2 - WALL_FORWARD_OFFSET_RANGE) +
				right * (math.random() * WALL_RIGHT_OFFSET_RANGE * 2 - WALL_RIGHT_OFFSET_RANGE)
			end

			local wall_index = self:_get_next_wall_index()
			network_manager.network_transmit:send_rpc_server("rpc_create_thornsister_push_wave", source_unit_id, POSITION_LOOKUP [self.owner_unit], position, damage_wave_template_id,
			career_power_level, segment_arr, wall_index)
		end
...
function ActionCareerWEThornsisterWall:client_owner_post_update(dt, t, world, can_damage, current_time_in_action)
	return end

function ActionCareerWEThornsisterWall:finish(reason)
	self.inventory_extension:wield_previous_non_level_slot()
end

function ActionCareerWEThornsisterWall:_play_vo()
...
function ActionCareerWEThornsisterWall:_get_next_wall_index()
	local wall_index = self._wall_index % 16 + 1
	self._wall_index = wall_index
	return wall_index
end

function ActionCareerWEThornsisterWall:_spawn_wall(num_segments, segments, wall_rotation)
	local wall_index = self:_get_next_wall_index()
	local owner_unit = self.owner_unit
	...
	for i = 1, num_segments do
		...
		local spawn_position = position + forward * (math.random() * WALL_FORWARD_OFFSET_RANGE * 2 - WALL_FORWARD_OFFSET_RANGE) +
		right * (math.random() * WALL_RIGHT_OFFSET_RANGE * 2 - WALL_RIGHT_OFFSET_RANGE)
