...
function ActionCareerWEThornsisterTargetStagger:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionCareerWEThornsisterTargetStagger:client_owner_start_action(new_action, t, chain_action_data, power_level, action_init_data)
...
function ActionCareerWEThornsisterTargetStagger:client_owner_post_update(dt, t, world, can_damage, current_time_in_action)
...
function ActionCareerWEThornsisterTargetStagger:finish(reason)
	if self._decal_unit then
	...
	if reason == "new_interupting_action" then
		...
		local targeting_data = { direction = Vector3Box(final_target_direction) }

		return targeting_data
	end
end

function ActionCareerWEThornsisterTargetStagger:_get_direction(force_flat)
