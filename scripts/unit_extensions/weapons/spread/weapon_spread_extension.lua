...
function WeaponSpreadExtension:init(extension_init_context, unit, extension_init_data)
	self.unit = unit
...
function WeaponSpreadExtension:extensions_ready(world, unit)
...
function WeaponSpreadExtension:destroy()

	return end

local ignored_damage_types = { temporary_health_degen = true, buff_shared_medpack_temp_health = true, buff_shared_medpack = true, buff = true, warpfire_ground = true, life_tap = true, health_degen = true, vomit_ground = true, wounded_dot = true, heal = true, life_drain = true }
...
function WeaponSpreadExtension:update(unit, input, dt, context, t)
...
function WeaponSpreadExtension:set_shooting()
	self.shooting = true
end

function WeaponSpreadExtension:combine_spread_rotations(roll, pitch, current_rot)
...
function WeaponSpreadExtension:get_max_pitch_rotation(roll_rotation)
	local current_pitch = self.current_pitch
	local current_yaw = self.current_yaw
...
function WeaponSpreadExtension:get_current_pitch_and_yaw()
	return self.current_pitch, self.current_yaw
end

function WeaponSpreadExtension:override_spread_template(spread_template_name)
...
function WeaponSpreadExtension:reset_spread_template()
	self.spread_settings = SpreadTemplates [self.default_spread_template_name]
end
...
function WeaponSpreadExtension:get_randomised_spread(current_rotation)
...
function WeaponSpreadExtension:get_target_style_spread(original_current_shot, original_max_shots, current_rotation, num_layers_spread, bullseye, spread_pitch)
