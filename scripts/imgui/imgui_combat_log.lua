...
function ImguiCombatLog:init()
	self._log = { }
	self._max_lines = 1000
	self._start_time = os.time() - os.clock()
	self.categories = { { name = "Damage", enabled = true, type = "damage" },
		{ name = "Heal", enabled = true, type = "heal" },
		{ name = "Buff", enabled = true, type = "buff" },
		{ name = "Buff Proc", enabled = true, type = "buff_proc" },
		{ name = "Action", enabled = true, type = "action" } }


	self._type_ids = { }
	self._settings = { auto_start_recording = true, show_timestamp = true, show_type = true }
...
function ImguiCombatLog:register_events()
...
function ImguiCombatLog:unregister_events()
	local event_manager = Managers.state.event
	if event_manager then
...
function ImguiCombatLog:on_round_start()
...
function ImguiCombatLog:on_round_end()
	self:unregister_events()
	self:_save_settings()
end

function ImguiCombatLog:destroy()
	self:unregister_events()
	self:_save_settings()
end

function ImguiCombatLog:update()
...
function ImguiCombatLog:is_persistent()
	return true
end

function ImguiCombatLog:draw(is_open)
...
function ImguiCombatLog:log_damage(attacker_unit, victim_unit, networkified_damage_amount, hit_zone_name, damage_type, damage_source, is_critical_strike, backstab_multiplier, added_dot, target_index, first_hit, total_hits, power_level)
	...
	self:_add_colored_segment(line,
	string.format("%s -> %s (%d) (%.2f %s), Power(%.2f), hit (%s) using (%s) Crit: %s, Backstab Mult: %.2f, Target Index: %d",
	tostring(attacker_unit_breed and attacker_unit_breed.name or attacker_unit),
	tostring(victim_unit_breed and victim_unit_breed.name or victim_unit),
	unit_id or 0,
	networkified_damage_amount or 0,
	tostring(damage_type),
	power_level or 0,
	tostring(hit_zone_name),
	tostring(damage_source),
	tostring(is_critical_strike),
	backstab_multiplier or 1,

	target_index or 0), Colors.get_table("orange"))
end

function ImguiCombatLog:log_heal(healer_unit, unit, buffed_heal_amount, heal_type)
	...
	tostring(healer_unit_breed and healer_unit_breed.name or healer_unit),
	tostring(unit_breed and unit_breed.name or unit),
	buffed_heal_amount or 0,
	tostring(heal_type)),
	Colors.get_table("lime"))
end

function ImguiCombatLog:log_action(unit, item_name, kind, action_name, sub_action_name, power_level, started, reason)
	local unit_breed = Unit.alive(unit) and Unit.get_data(unit, "breed")
	local line = self:_add_line("action")
	if started then
		self:_add_colored_segment(line,
		string.format("[Start] %s (%s - power %.2f) - %s/%s/%s",
		tostring(unit_breed and unit_breed.name or unit),
		tostring(kind),
		power_level or 0,
		tostring(item_name),
		tostring(action_name),
		tostring(sub_action_name)),
		Colors.get_table("white"))
	else
		self:_add_colored_segment(line,
		string.format("[End] %s (%s - power %.2f), Reason: %s - %s/%s/%s ",
		tostring(unit_breed and unit_breed.name or unit),
		tostring(kind),
		power_level or 0,
		tostring(reason),
...
function ImguiCombatLog:log_proc(player, event, buff, params, success)
	...
	tostring(unit_breed and unit_breed.name or unit),
	event or "-",
	tostring(buff.buff_type)),
	Colors.get_table("silver"))
end

function ImguiCombatLog:log_buff(unit, buff, added, stack_count, max_stacks)
	...
	if added then
		self:_add_colored_segment(line,
		string.format("[Added] %s -> %s (mult: %.2f)",
		tostring(owner_breed and owner_breed.name or unit),
		tostring(buff.buff_type),
		buff.multiplier or
		1), Colors.get_table("lime"))

		if stack_count and max_stacks then
			self:_add_colored_segment(line,
			string.format("(stacks: %d/%d)",
			stack_count,

			max_stacks), Colors.get_table("lime"))
		end

		if attacker_unit then
			self:_add_colored_segment(line,
			string.format("(%s)",
			tostring(attacker_breed and attacker_breed.name or attacker_unit)),
			Colors.get_table("lime"))
		end
	else
		self:_add_colored_segment(line,
		string.format("[Removed] %s -> %s",
		tostring(owner_breed and owner_breed.name or unit),
		tostring(buff.buff_type)),
		Colors.get_table("yellow"))
...
function ImguiCombatLog:_make_log_type_lookup()
	local categories = self.categories
	for i = 1, #categories do
...
function ImguiCombatLog:_get_type_name(type)
	if type then
...
function ImguiCombatLog:_add_line(type)
	local new_line = {
...
function ImguiCombatLog:_add_colored_segment(line, text, color)
	local text_to_add = text or ""
	local color_to_add = color or Colors.get_table("white")
	table.insert(line.content, { text_to_add, color_to_add })
end

function ImguiCombatLog:clear()
	self._log = { }
end

function ImguiCombatLog:copy_to_clipboard(copy_all)
	local output = ""
	local categories = self.categories
	local show_type = self._settings.show_type
	for line_id = 1, #self._log do
...
function ImguiCombatLog:_save_settings()
	local categories = self.categories
...
function ImguiCombatLog:_load_settings()
