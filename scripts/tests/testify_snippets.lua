...
function TestifySnippets.disable_level_intro_dialogue()
	TestifySnippets.set_script_data({ disable_level_intro_dialogue = true })
...
function TestifySnippets.set_player_profile(profile_name, career_name)
	Testify:make_request(
	"set_player_profile", {
...
function TestifySnippets.set_bot_profile(profile_name, career_name)
	Testify:make_request(
	"set_bot_profile", {
...
function TestifySnippets.set_script_data(options)
...
function TestifySnippets.load_weave(weave_name)
	Testify:make_request("set_next_weave", weave_name)
	Testify:make_request("load_weave", weave_name)
	Testify:make_request("wait_for_level_to_be_loaded")
end

function TestifySnippets.disable_ai()
	TestifySnippets.set_script_data({ ai_mini_patrol_disabled = true, disable_plague_sorcerer = true, ai_roaming_spawning_disabled = true, disable_gutter_runner = true, ai_boss_spawning_disabled = true, ai_bots_disabled = true, ai_roaming_patrols_disabled = true, ai_terror_events_disabled = true, ai_critter_spawning_disabled = true, disable_globadier = true, disable_warpfire_thrower = true, ai_pacing_disabled = true, disable_pack_master = true, ai_rush_intervention_disabled = true, disable_ratling_gunner = true, disable_vortex_sorcerer = true, ai_horde_spawning_disabled = true, ai_specials_spawning_disabled = true, ai_champion_spawn_debug = true })
...
function TestifySnippets.open_hero_view()
	local params = { transition = "hero_view_force",

		transition_params = { menu_state_name = "overview" } }

	Testify:make_request("transition_with_fade", params)
	Testify:make_request("wait_for_hero_view")
end

function TestifySnippets.open_cosmetics_inventory()
	Testify:make_request("set_hero_window_layout", 4)
	Testify:make_request("wait_for_cosmetics_inventory_window")
end

function TestifySnippets.equip_hats()
