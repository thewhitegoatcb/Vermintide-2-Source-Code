...
function BigBoyDestructibleExtension:init(extension_init_context, unit, extension_init_data)
	self.unit = unit
...
function BigBoyDestructibleExtension:extensions_ready()
	self.health_extension = ScriptUnit.extension(self.unit, "health_system")
end

function BigBoyDestructibleExtension:animation_played(frames, speed)
	local animation_length = frames / SIMPLE_ANIMATION_FPS / speed
	local t = Managers.time:time("game")
	self.animation_stop_time = t + animation_length
end

function BigBoyDestructibleExtension:update_nav_obstacles()
...
function BigBoyDestructibleExtension:_get_animation_flow_event(current_state, new_state)
...
function BigBoyDestructibleExtension:update_nav_graphs()
	local unit = self.unit
...
function BigBoyDestructibleExtension:update(unit, input, dt, context, t)
...
function BigBoyDestructibleExtension:register_breed_failed_leaving_smart_object(unit)
	if self.breeds_failed_leaving_smart_object == nil then return
	end
	self.breeds_failed_leaving_smart_object [unit] = true
end

function BigBoyDestructibleExtension:handle_breeds_failed_leaving_smart_object()
	if self.breeds_failed_leaving_smart_object == nil then return
	end
...
function BigBoyDestructibleExtension:destroy()
	self:destroy_box_obstacles()
...
function BigBoyDestructibleExtension:destroy_box_obstacles()
...
function BigBoyDestructibleExtension:is_open()
	return self.dead
end

function BigBoyDestructibleExtension:is_opening()
	return false
end