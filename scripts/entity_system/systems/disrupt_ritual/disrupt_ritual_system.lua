...
function DisruptRitualSystem:init(entity_system_creation_context, ...)
...
function DisruptRitualSystem:on_add_extension(world, unit, extension_name, extension_init_data)
...
function DisruptRitualSystem:hot_join_sync(peer_id)
	for extension_name, _ in pairs(self.extensions) do
		self:_hot_join_sync_extension(extension_name, peer_id)
	end
end

function DisruptRitualSystem:_hot_join_sync_extension(extension_name, peer_id)
