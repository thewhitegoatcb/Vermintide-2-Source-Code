...
if IS_WINDOWS then
	SplitScreenTesterKeymaps = {
		toggle_splitscreen = {
			input_mappings = { { "keyboard", "j", "pressed" } } } }
...
elseif IS_XB1 then
	SplitScreenTesterKeymaps = {
		toggle_splitscreen = { combination_type = "and",
			input_mappings = { { "gamepad", "left_thumb", "held" },
				{ "gamepad", "right_thumb", "pressed" } } } }
...
elseif IS_PS4 then
	SplitScreenTesterKeymaps = {
		toggle_splitscreen = { combination_type = "and",
			input_mappings = { { "gamepad", "l3", "held" },
				{ "gamepad", "r3", "pressed" } } } }
end
...
function SplitscreenTester:init()
	self:_setup_names()
...
function SplitscreenTester:_setup_names()
	self._world_name = "splitscreen_background"
	self._viewport_name = "splitscreen_viewport"
end

function SplitscreenTester:_setup_background()
...
function SplitscreenTester:_setup_input()
	...
	if not IS_CONSOLE then --[[ Nothing --]] end
...
function SplitscreenTester:add_splitscreen_viewport(world)
...
function SplitscreenTester:remove_splitscreen_viewport()
	self._splitscreen_viewport = nil
	self._splitscreen_world = nil
end

function SplitscreenTester:update(dt, t)
...
function SplitscreenTester:_fill_background(dt, t)
	local w, h = Application.screen_resolution()
	Gui.rect(self._gui, Vector3(0, 0, 0), Vector2(w, h), Color(0, 0, 0))
end

function SplitscreenTester:_update_splitscreen_camera(dt, t)
...
function SplitscreenTester:_update_input(dt, t)
...
function SplitscreenTester:_resize_viewports()
...
function SplitscreenTester:active()
	return self._splitscreen_active
end

function SplitscreenTester:destroy()
...
function Viewport.set_rect(viewport, offset_x, offset_y, size_x, size_y, extra_offset_x, extra_offset_y)
	local extra_offset_x = extra_offset_x or 0
	local extra_offset_y = extra_offset_y or 0
	Viewport.set_data(viewport, "rect", { offset_x, offset_y, size_x, size_y })
