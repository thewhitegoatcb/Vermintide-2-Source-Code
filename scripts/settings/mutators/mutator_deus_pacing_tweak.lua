...
local conflict_settings = {
	deus_skaven_chaos = { breed2 = "deus_chaos", breed1 = "deus_skaven" },



	deus_skaven_beastmen = { breed2 = "deus_beastmen", breed1 = "deus_skaven" } }
...
local EVENT_WEIGHTS = { { run_progress = 0,


		weights = { event_boss = 0, nothing = 30, event_patrol = 70 } },
	...
	{ run_progress = 0.4,
		...
		weights = { event_boss = 20, nothing = 10, event_patrol = 70 } } }
...
local Sequencer = { }
function Sequencer.apply_travel_dist_to_sequence_with_peaks_and_events(sequence, total_travel_dist, peaks, possible_events, event_weight_table, seed)
...
function Sequencer.tweak_zones_with_sequence(breed_a, breed_b, both_breeds, zones, num_zones, sequence_with_travel_dist)
	...
	for index, sequence_node in ipairs(sequence_with_travel_dist) do
		...
		for i = seek_start_index, num_zones do
			...
			if zone.mutators then
				for name in string.gmatch(zone.mutators, "([^[%s,]+)%s*,?%s*") do mutator_list [#mutator_list + 1] = name
				end
			end
			...
			if sequence_node.travel_dist < zone.travel_dist then
				if not zone.roaming_set then
					...
					zone.roaming_set = sequence_node.breeds == "a" and breed_a or
					sequence_node.breeds == "b" and breed_b or
					both_breeds
				end
...
return { hide_from_player_ui = true,
	...
	tweak_zones = function (mutator_context, data, conflict_director_name, zones, num_zones)
		...
		local sequence_with_travel_dist, best_event = Sequencer.apply_travel_dist_to_sequence_with_peaks_and_events(
		sequence, last_zone_travel_dist, peaks, possible_events, event_weight_table, seed)
		...
		Sequencer.tweak_zones_with_sequence(
		breed_a,
		breed_b,
	...
	server_start_function = function (mutator_context, data, dt, t)
		...
		elseif data.event.kind == "event_patrol" then
			...
			local event_data = { spline_type = "patrol", event_kind = "event_spline_patrol",
