...
function ActionChainedProjectile:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionChainedProjectile:client_owner_start_action(new_action, t, chain_action_data, power_level)
...
function ActionChainedProjectile:client_owner_post_update(dt, t, world, can_damage)
	if self.state == "waiting_to_shoot" and
	self._fire_t <= t then
		self.state = "shooting"
	end
...
function ActionChainedProjectile:finish(reason)
	ActionChainedProjectile.super.finish(self, reason)
end

function ActionChainedProjectile:_shoot(t)
	local length = 100
	...
	local static_hit, _, ray_length, hit_surface_normal, static_hit_actor = PhysicsWorld.immediate_raycast(
	physics_world,
	player_position,
	direction,
	length, "closest",
	"collision_filter", "filter_player_ray_projectile_static_only")
	...
	local results = PhysicsWorld.linear_sphere_sweep(
	physics_world,
	player_position + direction * hit_radius / 2,
	player_position + direction * max_length,
	hit_radius,
	100,
	"types", "both",
	...
	if hit_unit then
		...
		active_projectiles [active_projectiles_n] = { chain_count = 0,
	...
	local start_offset = Quaternion.rotate(player_rotation, Vector3(
	math.lerp(start_offset_limits.min [1], start_offset_limits.max [1], math.random()),
	math.lerp(start_offset_limits.min [2], start_offset_limits.max [2], math.random()),
	math.lerp(start_offset_limits.min [3], start_offset_limits.max [3], math.random())))


	local curve_offset_limits = self.current_action.curve_offset
	local curve_offset = Quaternion.rotate(player_rotation, Vector3(
	math.lerp(curve_offset_limits.min [1], curve_offset_limits.max [1], math.random()),
	math.lerp(curve_offset_limits.min [2], curve_offset_limits.max [2], math.random()),
	math.lerp(curve_offset_limits.min [3], curve_offset_limits.max [3], math.random())))
...
function ActionChainedProjectile:_apply_damage(hit_unit, hit_target_index, is_critical_strike, power_level, boost_curve_multiplier, damage_profile, damage_source_position)
	...
	if is_level_unit then
		...
		self.weapon_system:send_rpc_attack_hit(damage_source_id,
		attacker_unit_id,
		hit_unit_id,
		hit_zone_id,
		hit_position,
		...
		damage_profile_id,
		"power_level", power_level,
		"hit_target_index", hit_target_index,
		"blocking", false,
...
function ActionChainedProjectile:destroy()

	return end

function ActionChainedProjectile:passive_update(dt, t)
...
function ActionChainedProjectile:_select_next_target(chain_data, ai_broadphase, enemy_categories)
...
function ActionChainedProjectile:_play_fx(fx_name, point_1, point_2, point_3, is_chain)
	...
	elseif self._fire_sound_event and
...
function ActionChainedProjectile:_apply_chain_damage(chain_data, ai_broadphase, enemy_categories)
	local settings = chain_data.settings
	...
	if HEALTH_ALIVE [target_unit] then
		...
		target_unit,
		chain_count + 1,
		chain_data.is_critical_strike,
		chain_data.power_level,
		chain_data.boost_curve_multiplier,
		settings.damage_profile,
