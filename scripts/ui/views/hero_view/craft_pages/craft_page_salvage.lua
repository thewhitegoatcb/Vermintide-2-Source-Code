...
function CraftPageSalvage:on_enter(params, settings)
...
function CraftPageSalvage:create_ui_elements(params)
...
function CraftPageSalvage:on_exit(params)
...
function CraftPageSalvage:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function CraftPageSalvage:post_update(dt, t)

	return end

function CraftPageSalvage:_update_animations(dt)
...
function CraftPageSalvage:_is_button_pressed(widget)
...
function CraftPageSalvage:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageSalvage:_is_button_held(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageSalvage:_handle_input(dt, t)
...
function CraftPageSalvage:_fill_by_rarity(rarity)
...
function CraftPageSalvage:_handle_craft_input_progress(progress)
...
function CraftPageSalvage:craft_result(result, error, reset_slots)
	if not error then
		self._craft_result = result
	end
end

function CraftPageSalvage:clear_craft_items()
	local audio_triggered = false
...
function CraftPageSalvage:reset()
	local item_grid = self._item_grid
	item_grid:clear_locked_items()
	item_grid:update_items_status()
end

function CraftPageSalvage:on_craft_completed()
...
function CraftPageSalvage:_update_craft_items()
	local super_parent = self.super_parent
	local item_grid = self._item_grid
...
function CraftPageSalvage:_remove_craft_item(backend_id, slot_index, ignore_sound)
	local craft_items = self._craft_items
...
function CraftPageSalvage:_add_craft_item(backend_id, slot_index, ignore_sound, specific_amount)
	if self._num_craft_items == 0 then
...
function CraftPageSalvage:_set_craft_button_disabled(disabled)
	self._widgets_by_name.craft_button.content.button_hotspot.disable_button = disabled
end

function CraftPageSalvage:_exit(selected_level)
	self.exit = true
	self.exit_level_id = selected_level
end

function CraftPageSalvage:draw(dt)
...
function CraftPageSalvage:_play_sound(event)
	self.super_parent:play_sound(event)
end

function CraftPageSalvage:_set_craft_button_text(text, localize)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.craft_button
