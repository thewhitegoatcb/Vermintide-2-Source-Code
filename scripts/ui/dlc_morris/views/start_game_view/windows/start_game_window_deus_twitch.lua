...
function StartGameWindowDeusTwitch:on_enter(params, offset)
...
function StartGameWindowDeusTwitch:_create_ui_elements(params, offset)
...
function StartGameWindowDeusTwitch:set_focus(focused)
	self._is_focused = focused
end



function StartGameWindowDeusTwitch:_set_active(active)
	if active then
...
function StartGameWindowDeusTwitch:_set_disconnect_button_text()
...
function StartGameWindowDeusTwitch:cb_on_message_received(key, message_type, user_name, message, parameter)
	local chat_output_widget = self._widgets_by_name.chat_output_widget
	local chat_output_content = chat_output_widget.content
...
function StartGameWindowDeusTwitch:set_input_blocked(blocked)
...
function StartGameWindowDeusTwitch:_handle_twitch_login_input(dt, t, input_service)
...
function StartGameWindowDeusTwitch:cb_connection_success_callback(user_data)
	self:_set_disconnect_button_text()
	self:_setup_connected_status()
	self:_set_active(true)
end

function StartGameWindowDeusTwitch:_setup_connected_status()
	local user_name = Managers.twitch and Managers.twitch:user_name() or "N/A"
...
function StartGameWindowDeusTwitch:_gather_unlocked_journeys()
...
function StartGameWindowDeusTwitch:_setup_journey_widgets()
...
function StartGameWindowDeusTwitch:_refresh_journey_cycle()
	self._journey_cycle = self._backend_deus:get_journey_cycle()
	self:_on_new_journey_cycle()
end

function StartGameWindowDeusTwitch:_on_new_journey_cycle()
	self:_update_journey_god_icons()
end

function StartGameWindowDeusTwitch:_update_journey_god_icons()
...
function StartGameWindowDeusTwitch:update(dt, t)
...
function StartGameWindowDeusTwitch:post_update(dt, t)
	return end

function StartGameWindowDeusTwitch:_handle_input(dt, t)
...
function StartGameWindowDeusTwitch:_update_input_description(dt, t)
	local connected = Managers.twitch and Managers.twitch:is_connected()
	self:_set_input_description(connected)
end

function StartGameWindowDeusTwitch:_update_modifiers(dt, t)
...
function StartGameWindowDeusTwitch:_update_expedition_option()
	local parent = self._parent
	local selected_level_id = parent:get_selected_level_id()

	if not selected_level_id then return
	end
...
function StartGameWindowDeusTwitch:_update_button_animations(dt)
	local widgets_by_name = self._widgets_by_name
	self:_animate_button(widgets_by_name.button_1, dt)
	self:_animate_button(widgets_by_name.button_2, dt)
end

function StartGameWindowDeusTwitch:_update_animations(dt, t)
...
function StartGameWindowDeusTwitch:_draw(dt, t)
...
function StartGameWindowDeusTwitch:_start_transition_animation(animation_name)
	local params = {
...
function StartGameWindowDeusTwitch:_animate_button(widget, dt)
	local content = widget.content
...
function StartGameWindowDeusTwitch:_animate_expedition_widget(widget, dt)
	local content = widget.content
...
function StartGameWindowDeusTwitch:_play_sound(event)
	self._parent:play_sound(event)
end


function StartGameWindowDeusTwitch:_handle_virtual_keyboard(dt, t)
...
function StartGameWindowDeusTwitch:_handle_gamepad_activity(dt, t)
...
function StartGameWindowDeusTwitch:_verify_selection_index(input_index, input_change)
...
function StartGameWindowDeusTwitch:_gamepad_selector_input_func(input_index, input_change)
	local mouse_active = Managers.input:is_device_active("mouse")
...
function StartGameWindowDeusTwitch:_update_difficulty_option(difficulty_key)
...
function StartGameWindowDeusTwitch:_option_selected(widget_name, button_name, t)
	local parent = self._parent
	local twitch_settings = parent:get_twitch_settings(self._mechanism_name) or parent:get_twitch_settings("adventure")
	...
	if widget_name == "difficulty_stepper" then
		...
		elseif
		button_name == "right_arrow" and current_difficulty_index + 1 <= #difficulty_list then
			new_current_index = current_difficulty_index + 1
			self._parent:play_sound("hud_morris_start_menu_set")
		end
...
function StartGameWindowDeusTwitch:_set_input_description(connected)
	if self._is_server then
		if connected then
...
function StartGameWindowDeusTwitch:_update_can_play(dt, t)
	if self._is_server then
...
function StartGameWindowDeusTwitch:_can_play()
...
function StartGameWindowDeusTwitch:on_exit(params)
...
function StartGameWindowDeusTwitch:_set_info_window(difficulty_key)
	local difficulty_settings = DifficultySettings [difficulty_key]
...
function StartGameWindowDeusTwitch:_update_difficulty_lock()
	...
	self:_resize_difficulty_info({ math.floor(scenegraph_definition.difficulty_info.size [1]), math.floor(widget_height) }, { 0, -offset_y, 1 })
	upsell_button.offset [2] = -math.floor(widget_height) / 2 + 24
end


function StartGameWindowDeusTwitch:_calculate_difficulty_info_widget_size(diff_widget)
	local spacing = 20
...
function StartGameWindowDeusTwitch:_resize_difficulty_info(new_size, new_offset)
	local difficulty_info_widget = self._widgets_by_name.difficulty_info
...
function StartGameWindowDeusTwitch:_update_gamemode_info_text(input_service)
...
function StartGameWindowDeusTwitch:_set_expedition_text_highlight_offset(ui_renderer)
...
function StartGameWindowDeusTwitch:_update_additional_curse_frame(journey_name)
	for _, widget in ipairs(self._expedition_widgets) do
		local content = widget.content
