...
function PackageManager:load(package_name, reference_name, callback, asynchronous, prioritize)
	...
	if self._references [package_name] then
		...
		self._references [package_name] = { [reference_name] = 1 }

		if next(self._asynch_packages) and asynchronous then
			self._queued_async_packages [package_name] = { callbacks = { callback } }
			if prioritize then
				table.insert(self._queue_order, 1, package_name)
			else
		...
		elseif not asynchronous then
			...
			self._asynch_packages [package_name] = { handle = resource_handle, callbacks = { callback } }
		end
	end
end

function PackageManager:force_load(package_name)
...
function PackageManager:force_load_queued_package(package_name)
...
function PackageManager:_pop_queue()
	...
	if queued_package_name and self._queued_async_packages [queued_package_name] then
		...
		self._asynch_packages [queued_package_name] = { handle = resource_handle, callbacks = data.callbacks }
...
function PackageManager:unload(package_name, reference_name)
...
function PackageManager:can_unload(package_name)
	...
	if
	resource_handle and self._packages [package_name] then
...
function PackageManager:is_loading(package, optional_reference_name)
...
function PackageManager:has_loaded(package, reference_name)
	...
	if reference_name then
		do return loaded and self._references [package] [reference_name] ~= nil end
	else
		return loaded
	end
end
...
function PackageManager:num_references(package_name)
...
function PackageManager:unload_dangling_painting_materials()
...
function PackageManager:_force_unload(package_name)
...
function PackageManager:dump_reference_counter(reference_name)
