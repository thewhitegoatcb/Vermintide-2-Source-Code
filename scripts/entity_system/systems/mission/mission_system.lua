...
local RPCS = { "rpc_start_mission", "rpc_start_mission_with_unit", "rpc_end_mission", "rpc_request_mission", "rpc_request_mission_with_unit", "rpc_update_mission", "rpc_request_mission_update" }
...
function MissionSystem:init(entity_system_creation_context, system_name)
...
function MissionSystem:create_checkpoint_data()
...
function MissionSystem:load_checkpoint_data(checkpoint_data)
	local world = self.world
...
function MissionSystem:destroy()
...
function MissionSystem:update(context, t)
	local dt = context.dt
...
function MissionSystem:request_mission(mission_name, unit)
...
function MissionSystem:start_mission(mission_name, unit, sync_data)
...
function MissionSystem:block_mission_ui(ui_blocked)
	Managers.state.event:trigger("ui_event_block_mission_ui", ui_blocked)
end


function MissionSystem:trigger_active_mission_ui_events()
...
function MissionSystem:end_mission(mission_name, sync)
...
function MissionSystem:reset_mission(mission_name, sync)
	fassert(self.active_missions [mission_name], "No active mission with passed mission_name %q", mission_name)
...
function MissionSystem:update_mission(mission_name, positive, dt, sync)
	fassert(self.active_missions [mission_name], "No active mission with passed mission_name %q", mission_name)
...
function MissionSystem:evaluate_level_end_missions()
...
function MissionSystem:debug_draw(dt)
	for mission_name, data in pairs(self.active_missions) do
		Debug.text(data.text)
	end
end

function MissionSystem:hot_join_sync(sender)
	for name, data in pairs(self.active_missions) do
...
function MissionSystem:flow_callback_start_mission(mission_name, unit)
...
function MissionSystem:flow_callback_reset_mission(mission_name)
	if not self.is_server then
		return
	end
...
function MissionSystem:flow_callback_update_mission(mission_name)
	if not self.is_server then
		return
	end
	fassert(self.active_missions [mission_name], "No active mission with passed mission_name %q", mission_name)
...
function MissionSystem:flow_callback_end_mission(mission_name)
	if not self.is_server then
		return
	end
...
function MissionSystem:rpc_start_mission(channel_id, mission_name_id, sync_data)
	local mission_name = NetworkLookup.mission_names [mission_name_id]
	self:start_mission(mission_name, nil, sync_data)
end

function MissionSystem:rpc_start_mission_with_unit(channel_id, mission_name_id, level_unit_id, sync_data)
	local mission_name = NetworkLookup.mission_names [mission_name_id]
...
function MissionSystem:rpc_request_mission(channel_id, mission_name_id)
	fassert(self.is_server, "[MissionSystem] Request mission ended up on a client")
	local mission_name = NetworkLookup.mission_names [mission_name_id]
	self:request_mission(mission_name)
end

function MissionSystem:rpc_request_mission_with_unit(channel_id, mission_name_id, level_unit_id)
	fassert(self.is_server, "[MissionSystem] Request mission ended up on a client")
	local mission_name = NetworkLookup.mission_names [mission_name_id]
	local unit = Level.unit_by_index(LevelHelper:current_level(self.world), level_unit_id)
	self:request_mission(mission_name, unit)
...
function MissionSystem:rpc_request_mission_update(channel_id, mission_name_id, positive)
...
function MissionSystem:rpc_end_mission(channel_id, mission_name_id)
	local mission_name = NetworkLookup.mission_names [mission_name_id]
	self:end_mission(mission_name)
end

function MissionSystem:rpc_update_mission(channel_id, mission_name_id, sync_data)
	local mission_name = NetworkLookup.mission_names [mission_name_id]
	local data = self.active_missions [mission_name]
...
function MissionSystem:get_missions()
	return self.active_missions, self.completed_missions
end

function MissionSystem:has_active_mission(mission_name)
	return self.active_missions [mission_name] ~= nil
end

function MissionSystem:get_level_end_mission_data(mission_template_name)
	return self.level_end_missions [mission_template_name]
end

function MissionSystem:set_percentage_completed(percentage_completed)
	self._percentage_completed = percentage_completed
end


function MissionSystem:_update_level_progress(dt)
	if self.is_server then
...
function MissionSystem:override_percentage_completed(progression)
	if self.is_server then
		self._percentage_completed_override = progression
	end
end

function MissionSystem:percentages_completed()
