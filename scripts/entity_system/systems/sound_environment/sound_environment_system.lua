...
function SoundEnvironmentSystem:init(entity_system_creation_context, system_name)
...
function SoundEnvironmentSystem:destroy()
	EngineOptimized.highest_prio_environment_destroy(self._highest_prio_system)
end

local environment_base = { aux_bus_name = "", prio = 0, fade_time = 0, volume_name = "", ambient_sound_event_stop = "", ambient_sound_event_start = "",
...
function SoundEnvironmentSystem:register_sound_environment(volume_name, prio, ambient_sound_event, fade_time, aux_bus_name, environment_state)
	...
	for volume_name, data in pairs(self._environments) do
		...
		array_to_sort [count] = { p = prio, n = volume_name }
		count = count + 1
	end
...
function SoundEnvironmentSystem:_highest_prio_environment_at_position(position)
...
function SoundEnvironmentSystem:set_source_environment(source, position)
...
function SoundEnvironmentSystem:register_source_environment_update(source, unit, object)
	self._updated_sources [#self._updated_sources + 1] = { unit = unit, source = source, node = object and Unit.node(unit, object) or 0 }
	self._num_sources = self._num_sources + 1
end

function SoundEnvironmentSystem:unregister_source_environment_update(source)
...
function SoundEnvironmentSystem:_update_source_environments()
	local num_sources = self._num_sources
...
function SoundEnvironmentSystem:local_player_created(player)
	self.player = player
end

function SoundEnvironmentSystem:update(context, t)
...
function SoundEnvironmentSystem:_update_fade(t)
	local wwise_world = self.wwise_world
	local environments = self._environments
	local fade_environments = self._fade_environments
	for volume_name, _ in pairs(fade_environments) do
...
function SoundEnvironmentSystem:_add_fade_environment(t, volume_name, fade_time, target_value)
...
function SoundEnvironmentSystem:_clamp_num_fade_environments()
...
function SoundEnvironmentSystem:enter_environment(t, volume_name, current_environment_name)
	local environment = self._environments [volume_name]
...
function SoundEnvironmentSystem:_set_environment(volume_name)
	local wwise_world = self.wwise_world
	local environments = self._environments
	local environment = environments [volume_name]
