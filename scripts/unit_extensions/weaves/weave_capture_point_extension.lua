...
function WeaveCapturePointExtension:init(extension_init_context, unit, extension_init_data)
...
function WeaveCapturePointExtension:get_objective_settings()
	return WeaveObjectiveSettings [WeaveCapturePointExtension.NAME]
end

function WeaveCapturePointExtension:score()
	return self._score
end

function WeaveCapturePointExtension:_calculate_size()
...
function WeaveCapturePointExtension:activate(game_object_id, objective_data)
	...
	if self._is_server then
		local game_object_data_table = { value = 0,
		...
		self._game_object_id = Managers.state.network:create_game_object(
		"weave_objective", game_object_data_table,
		callback)
	else
	...
	if self._is_server then
		...
		if num_extra_players == 0 then num_extra_players = 1 end
		self._capture_rate_multiplier = 1 / num_extra_players
	end
end

function WeaveCapturePointExtension:complete()
...
function WeaveCapturePointExtension:deactivate()
	if self._is_server then
...
function WeaveCapturePointExtension:cb_game_session_disconnect()

	return end

function WeaveCapturePointExtension:objective_name()
	return self._objective_name
end

function WeaveCapturePointExtension:update(dt, t)
...
function WeaveCapturePointExtension:_update_num_players_required(num_players)
...
function WeaveCapturePointExtension:_server_update(dt, t)
...
function WeaveCapturePointExtension:_client_update(dt, t)
	local game_session = Network.game_session()
	...
	if self._new_value < value then value = ( value + 2 ) * 0.01
	end
...
function WeaveCapturePointExtension:is_done()
	return self._timer <= 0
end

function WeaveCapturePointExtension:get_percentage_done()
	return math.clamp(1 - self._timer / self._max_time, 0, 1)
end

function WeaveCapturePointExtension:get_score()
	return self._score
end