...
function PlayerBotInput:init(extension_init_context, unit, extension_init_data)
	self.unit = unit
	self.move = { x = 0, y = 0 }
	self.look = { x = 0, y = 0 }
...
function PlayerBotInput:extensions_ready(world, unit)
...
function PlayerBotInput:destroy()
	return end

function PlayerBotInput:reset()
	return end

function PlayerBotInput:pre_update(unit, input, dt, context, t)
...
function PlayerBotInput:update(unit, input, dt, context, t)
...
function PlayerBotInput:_update_actions()
...
function PlayerBotInput:_update_debug_text(unit, input)
...
function PlayerBotInput:set_aim_position(position)
	self._aim_target:store(position)
end

function PlayerBotInput:set_aim_rotation(rotation)
	self._aim_rotation:store(rotation)
end

function PlayerBotInput:set_aiming(aiming, soft, use_rotation)
	self._aiming = aiming
...
function PlayerBotInput:set_look_at_player(player_unit, rotation_allowed)
	self._look_at_player = player_unit
	self._look_at_player_rotation_allowed = not not rotation_allowed
end

function PlayerBotInput:defend()
	self._defend = true
end

function PlayerBotInput:activate_ability()
	self._activate_ability = true
	self._cancel_held_ability = false
end

function PlayerBotInput:cancel_ability()
	self._cancel_held_ability = true
	self._activate_ability = false
	self._activate_ability_held = false
end

function PlayerBotInput:release_ability_hold()
	self._activate_ability_held = true
end

function PlayerBotInput:melee_push()
	self._melee_push = true
end

function PlayerBotInput:hold_attack()
	self._hold_attack = true
end

function PlayerBotInput:tap_attack()
	self._tap_attack = true
end

function PlayerBotInput:charge_shot()
	self._charge_shot = true
end

function PlayerBotInput:fire()
	self._fire = true
end

function PlayerBotInput:fire_hold()
	self._fire_hold = true
end

function PlayerBotInput:interact()
	self._interact = true
end

function PlayerBotInput:weapon_reload()
	self._weapon_reload = true
end

function PlayerBotInput:dodge()
	self._dodge = true
end

function PlayerBotInput:wield(slot)
...
function PlayerBotInput:_update_wanted_rotation_for_attract_mode(dt, rotation, current_goal, on_ladder, ladder_unit, camera_position)
...
function PlayerBotInput:set_bot_in_attract_mode_focus(is_in_focus)
...
function PlayerBotInput:_obstacle_check(position, current_speed_sq, goal_vector, goal_direction, up)
	...
	local _, num_low_hit_actors = PhysicsWorld.immediate_overlap(physics_world,
	"shape", "oobb",
	"position", lower_check_pos,
	"rotation", rotation,
	"size", lower_extents,
	"types", "statics",
	...
	local _, num_high_hit_actors = PhysicsWorld.immediate_overlap(physics_world,
	"shape", "oobb",
	"position", upper_check_pos,
	"rotation", rotation,
	"size", upper_extents,
	"types", "statics",
	"collision_filter", collision_filter)
...
function PlayerBotInput:_update_movement(dt, t)
...
function PlayerBotInput:is_input_blocked()
	return false
end

function PlayerBotInput:get(input_key)
...
function PlayerBotInput:set_enabled(enabled)
	return end

function PlayerBotInput:get_buffer(input_key)
	return end

function PlayerBotInput:add_buffer(input_key)
	return end

function PlayerBotInput:reset_input_buffer(input_key)
	return end

function PlayerBotInput:clear_input_buffer(input_key)
	return end

function PlayerBotInput:reset_wield_switch_buffer()
	return end

function PlayerBotInput:set_last_scroll_value()
	return end

function PlayerBotInput:get_last_scroll_value()
	return end

function PlayerBotInput:set_input_key_scale(input_key, scale, lerp_time)
	return end

function PlayerBotInput:move(vector)
	self.move.x = vector.x
	self.move.y = vector.y
end

function PlayerBotInput:look(vector)
	self.look.x = vector.x
	self.look.y = vector.y
end

function PlayerBotInput:move_forward()
	self.move.x = 0
	self.move.y = 1
end

function PlayerBotInput:rotate_right()
	self.look.x = 0.1
	self.look.y = 0
end

function PlayerBotInput:not_moving()
	return self.move.x == 0 and self.move.y == 0
end

function PlayerBotInput:move_towards(target_position)
	self.target_position = target_position and Vector3Box(target_position) or nil
end

function PlayerBotInput:get_wield_cooldown()
	return false
end

function PlayerBotInput:add_wield_cooldown(cooldown_time)
	return
end

function PlayerBotInput:released_input(input)
	return not self._input [input]
end

function PlayerBotInput:add_stun_buffer(input_key)
	return end

function PlayerBotInput:reset_release_input()
	return true
end

function PlayerBotInput:reset_release_input_with_delay()
	return true
end

function PlayerBotInput:force_release_input()
	return true
end