...
function ActionUtils.get_power_level_percentage(power_level)
	local min_power_level = MIN_POWER_LEVEL local max_power_level = MAX_POWER_LEVEL
	local percentage = ( power_level - min_power_level ) / (max_power_level - min_power_level)

	return percentage
end

function ActionUtils.get_max_targets(damage_profile, cleave_power_level)
...
function ActionUtils.get_target_armor(hit_zone_name, breed, armor_override)
...
function ActionUtils.get_range_scalar_multiplier(damage_profile, target_settings, attacker_unit, target_unit)
	...
	if scaling_steps then
		...
		elseif scaling_steps [#scaling_steps].distance < distance then
			...
			do return range_multiplier end
		else
...
function ActionUtils.get_armor_power_modifier(power_type, damage_profile, target_settings, target_unit_armor, target_unit_primary_armor, critical_strike_settings, range_scalar_multiplier)
...
function ActionUtils.scale_power_levels(power_level, power_type, attacker_unit, difficulty_level)
...
function ActionUtils.get_power_multiplier(power_type, damage_profile, target_settings, range_scalar_multiplier)
...
function ActionUtils.get_power_level(power_type, power_level, damage_profile, target_settings, critical_strike_settings, range_scalar_multiplier, attacker_unit, difficulty_level)
...
function ActionUtils.get_power_level_for_target(optional_target_unit, original_power_level, damage_profile, target_index, is_critical_strike, attacker_unit, hit_zone_name, armor_type_override, damage_source, breed, range_scalar_multiplier, difficulty_level, target_unit_armor, target_unit_primary_armor)
...
function ActionUtils.apply_buffs_to_power_level(unit, power_level)
...
function ActionUtils.apply_buffs_to_power_level_on_hit(unit, power_level, breed, damage_source, is_critical_strike, armor_override)
...
function ActionUtils.apply_buffs_to_armor_power_on_hit(attacker_unit, target_unit, attack_armor_power_modifer, armor_category)
...
function ActionUtils.scale_charged_projectile_power_level(power_level, action, charge_level)
...
function ActionUtils.scale_geiser_power_level(power_level, charge_value)
	local scaled_power_level = ( 0.5 + 0.5 * charge_value ) * power_level
	return scaled_power_level
end

function ActionUtils.get_melee_boost(unit, melee_boost_override)
	local career_extension = ScriptUnit.has_extension(unit, "career_system")
	local has_melee_boost = false local boost_curve_multiplier = 0
...
function ActionUtils.get_ranged_boost(unit)
	local career_extension = ScriptUnit.has_extension(unit, "career_system")
	local has_ranged_boost = false local boost_curve_multiplier = 0
	if career_extension then
...
function ActionUtils.spawn_player_projectile(owner_unit, position, rotation, scale, angle, target_vector, speed, item_name, item_template_name, action_name, sub_action_name, is_critical_strike, power_level, gaze_settings, charge_level)
	scale = scale or 100
...
function ActionUtils.spawn_pickup_projectile(world, weapon_unit, projectile_unit_name, projectile_unit_template_name, current_action, owner_unit, position, rotation, velocity, angular_velocity, item_name, spawn_type)
	...
	if has_death_extension then
		...
		if ScriptUnit.has_extension(weapon_unit, "limited_item_track_system") then
			...
			Managers.state.network.network_transmit:send_rpc_server("rpc_spawn_explosive_pickup_projectile_limited",
			projectile_unit_name_id,
			projectile_unit_template_name_id,
			network_position,
			network_rotation,
			network_velocity,
			...
			Managers.state.network.network_transmit:send_rpc_server("rpc_spawn_explosive_pickup_projectile",
			projectile_unit_name_id,
			projectile_unit_template_name_id,
			network_position,
			network_rotation,
	...
	elseif ScriptUnit.has_extension(weapon_unit, "limited_item_track_system") then
		...
		Managers.state.network.network_transmit:send_rpc_server("rpc_spawn_pickup_projectile_limited",
		projectile_unit_name_id,
		projectile_unit_template_name_id,
		network_position,
		network_rotation,
		...
		Managers.state.network.network_transmit:send_rpc_server("rpc_spawn_pickup_projectile",
		projectile_unit_name_id,
		projectile_unit_template_name_id,
		network_position,
		network_rotation,
...
function ActionUtils.spawn_true_flight_projectile(owner_unit, target_unit, true_flight_template_id, position, rotation, angle, target_vector, speed, item_name, item_template_name, action_name, sub_action_name, scale, is_critical_strike, power_level)
	local projectile_system = Managers.state.entity:system("projectile_system")
...
function ActionUtils.get_action_time_scale(unit, action_settings, is_animation, custom_value)
...
function ActionUtils.init_action_buff_data(action_buff_data, buff_data, t)
...
function ActionUtils.update_action_buff_data(action_buff_data, buff_data, owner_unit, t)
	local start_times = action_buff_data.buff_start_times
	local end_times = action_buff_data.buff_end_times
	local buff_identifiers = action_buff_data.buff_identifiers
	local action_buffs_in_progress = action_buff_data.action_buffs_in_progress
...
function ActionUtils.remove_action_buff_data(action_buff_data, buff_data, owner_unit)
...
function ActionUtils.start_charge_sound(wwise_world, weapon_unit, player_unit, action_settings)
...
function ActionUtils.stop_charge_sound(wwise_world, wwise_playing_id, wwise_source_id, action_settings)
...
function ActionUtils.play_husk_sound_event(wwise_world, sound_event, player_unit, is_bot)
...
function ActionUtils.get_critical_strike_chance(unit, action, overrides)
...
function ActionUtils.is_critical_strike(unit, action, t, overrides)
	local buff_extension = ScriptUnit.extension(unit, "buff_system")
...
function ActionUtils.pitch_from_rotation(rotation)
...
function ActionUtils.redirect_shield_hit(hit_unit, hit_actor)
...
function ActionUtils.resolve_action_selector(action, talent_extension, buff_extension, weapon_extension, owner_unit)
	if not action then
		return nil
	end
...
function ActionUtils.get_push_damage_profile(sub_action)
	if sub_action then
...
function ActionUtils.get_damage_profile_name(sub_action, action_hand)
	if sub_action then
...
function ActionUtils.get_damage_profile_performance_scores(damage_profile_name)
	local results = { 0, 0, 0, 0, 0, 0 }
...
function ActionUtils.get_performance_scores_for_sub_action(sub_action)
	local results = nil
...
function ActionUtils.is_melee_start_sub_action(sub_action)
...
function ActionUtils.is_backstab(unit, hit_unit)
