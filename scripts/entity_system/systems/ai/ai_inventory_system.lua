...
local RPCS = { "rpc_ai_inventory_wield", "rpc_ai_drop_single_item", "rpc_ai_show_single_item" }





local extensions = { "AIInventoryExtension" }



AIInventorySystem = class(AIInventorySystem, ExtensionSystemBase)

function AIInventorySystem:init(context, system_name)
...
function AIInventorySystem:destroy()
	self.network_event_delegate:unregister(self)
end

function AIInventorySystem:drop_item(unit)
...
local dummy_input = { }
function AIInventorySystem:on_add_extension(world, unit, extension_name, extension_init_data)
	local extension = nil
...
function AIInventorySystem:on_remove_extension(unit, extension_name)
	self:_cleanup_extension(unit, extension_name)
	ScriptUnit.remove_extension(unit, self.NAME)
end

function AIInventorySystem:on_freeze_extension(unit, extension_name)
...
function AIInventorySystem:_cleanup_extension(unit, extension_name)
...
function AIInventorySystem:freeze(unit, extension_name, reason)
...
function AIInventorySystem:unfreeze(unit)
...
function AIInventorySystem:update(context, t, dt)
	...
	for i = 1, units_to_wield_n do
		...
		if item_sets then
			...
			end_index = extension.dropped and 0 or item_set.end_index start_index = item_set.start_index
		else
			...
			end_index = extension.dropped and 0 or extension.inventory_items_n start_index = 1
		end
		...
		if script_data.ai_debug_inventory then --[[ Nothing --]]
		end
...
function AIInventorySystem:rpc_ai_inventory_wield(channel_id, go_id, item_set_index)
	local unit = self.unit_storage:unit(go_id)
	if unit == nil then return end
	if self.frozen_unit_extension_data [unit] then return end
...
function AIInventorySystem:rpc_ai_drop_single_item(channel_id, unit_id, item_inventory_index, item_drop_reason_id)
...
function AIInventorySystem:rpc_ai_show_single_item(channel_id, unit_id, item_inventory_index, show)
	local unit = self.unit_storage:unit(unit_id)
	if unit == nil then return end
	if self.frozen_unit_extension_data [unit] then return end
	local ai_inventory_extension = ScriptUnit.extension(unit, "ai_inventory_system")
...
function AIInventorySystem:hot_join_sync(peer_id)
