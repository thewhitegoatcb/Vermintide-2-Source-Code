...
function PerlinNoise:init(world)
...
function PerlinNoise:draw_height(height, x, y, z, rad)
	local drawer = Managers.state.debug:drawer({ mode = "lel", name = "perlin noise" })
...
function PerlinNoise:filter_list_using_noise(list, height_threshold)
	local a, b, c = Script.temp_count()

	local lowest = 0 local highest = 0
	for i = #list, 1, -1 do
...
function PerlinNoise:normalize(gradient_x, gradient_y)
	local s = nil
...
function PerlinNoise:setup()
	for i = 1, self._n do
		self._permutations [i] = i
		self._gradients [i] = { false, false }
		local check = true

		repeat
			for j = 1, 2 do
				self._gradients [i] [j] = Math.random(-10, 10) * 0.1
				check = check and self._gradients [i] [j] == 0 end until not check
	...
	for i = 1, self._n + 2 do
		...
		self._gradients [self._n + i] = { false, false }
...
function PerlinNoise:get_height(x, y)
	local point_x = x local point_y = y
	local tx = x + 4096
	local ty = y + 4096
	...
	if p0_x == 0 then p0_x = 1 end
	if p0_y == 0 then p0_y = 1 end
	if p1_x == 0 then p1_x = 1 end
	if p1_y == 0 then p1_y = 1
	end
	...
	local p00 = self._permutations [i + p0_y] local p10 = self._permutations [j + p0_y]
	local p01 = self._permutations [i + p1_y] local p11 = self._permutations [j + p1_y]

	local s_curve_x = self:getSCurve(rx0)
	local s_curve_y = self:getSCurve(ry0)

	local gradient, s, t, u, v = nil
	gradient = self._gradients [p00] s = self:at2(gradient, rx0, ry0)
	gradient = self._gradients [p10] t = self:at2(gradient, rx1, ry0)
	gradient = self._gradients [p01] u = self:at2(gradient, rx0, ry1)
	gradient = self._gradients [p11] v = self:at2(gradient, rx1, ry1)
...
function PerlinNoise:at2(gradient, vec_x, vec_y)
	local arctang = gradient [1] * vec_x + gradient [2] * vec_y
	return arctang
end

function PerlinNoise:getSCurve(p)
	local s_curve = 6 * p ^ 5 - 15 * p ^ 4 + 10 * p ^ 3
	return s_curve
end

function PerlinNoise:simulate_points()
	local drawer = Managers.state.debug:drawer({ mode = "lel", name = "perlin noise" })
