...
local generic_input_actions = { { input_action = "confirm", priority = 2, description_text = "input_description_open" },
...
function IngameView:init(ingame_ui_context)
	...
	self.controller_grid_index = { x = 1, y = 1 }
...
function IngameView:on_enter(params)
...
function IngameView:input_service()
	return self.input_manager:get_service("ingame_menu")
end

function IngameView:create_ui_elements()
...
function IngameView:_update_presentation()
...
function IngameView:destroy()
	self.menu_input_description:destroy()
	self.menu_input_description = nil
end

function IngameView:set_background_height(num_buttons)
...
function IngameView:update(dt)
	...
	if ( input_service:get("toggle_menu", true) or input_service:get("back", true) ) and
	not ingame_ui:pending_transition() then
		ingame_ui:handle_transition("exit_menu")
	end

end

function IngameView:setup_controller_selection()
	local selection_index = 1
	self:controller_select_button_index(selection_index, true)
end

function IngameView:controller_select_button_index(index, ignore_sound)
...
function IngameView:clear_controller_selection()
	local layout_logic = self.layout_logic
	local stored_buttons = self.stored_buttons
...
function IngameView:update_controller_input(input_service, dt)
	local layout_logic = self.layout_logic
	local layout_data = layout_logic:layout_data()
...
function IngameView:get_transition()
	if self.leave_game then return "leave_game" end
end

function IngameView:play_sound(event)
