...
function SpawnManager:init(world, is_server, network_event_delegate, unit_spawner, profile_synchronizer, network_server, checkpoint_data)
	self.world = world
	self.spawn_points = { }
...
function SpawnManager:_default_player_statuses()
	...
	for i = 1, num_slots do
		local status = { temporary_health_percentage = 0, spawn_state = "not_spawned", health_percentage = 1, health_state = "alive",
			...
			consumables = { },
			ammo = { slot_ranged = 1, slot_melee = 1 } }
...
function SpawnManager:_spawn_pos_rot_from_index(index)
...
function SpawnManager:flow_callback_set_checkpoint(no_spawn_volume, safe_zone_volume_name, ...)
...
function SpawnManager:load_checkpoint_data(data)
...
function SpawnManager:checkpoint_data()
	return self._checkpoint_data
end

function SpawnManager:_clone_player_status(t)
	local clone = { }
...
function SpawnManager:_pack_spawn_unit_level_indices(...)
	local return_table = { }
	local level = LevelHelper:current_level(self.world)
...
function SpawnManager:pre_update(dt, t)
	if self._despawn_queue_size > 0 then
		self:_update_despawns()
	end
end

function SpawnManager:delayed_despawn(player)
...
function SpawnManager:_update_despawns()
	local despawn_queue = self._despawn_queue
