local function array_new() return { { }, 0 } end
local function array_data(a) return a [1], a [2] end
local function array(a) return a [1] end
local function array_n(a) return a [2] end

local function array_copy(a)
	local at, an = array_data(a)
	local t = { }
	local b = { t, an }
	for i = 1, an do t [i] = at [i] end
	return b
end

local function array_resize(a, new_n)
	local t = a [1] local n = a [2]
	while n < new_n do
		n = n + 1
		t [n] = nil
	end
...
local function array_item_index(a, item)
	local t = a [1] local n = a [2]
	local found_i = nil
	for i = 1, n do if t [i] == item then found_i = i break end end
	return found_i
end
...
local function array_pop_item(a, item)
	...
	if not found_i then return nil
	end
...
local function comp_default(a, b) return a < b end
...
local function array_binary_insert(a, value, comp_function)
	...
	local iStart = 1 local iEnd = n local iMid = 1 local iState = 0
	...
	while iStart <= iEnd do
		...
		if comp_function(value, at [iMid]) then
			iState = 0 iEnd = iMid - 1
		else
			iState = 1 iStart = iMid + 1
		end
	end
