...
function StartGameWindowSettings:on_enter(params, offset)
...
function StartGameWindowSettings:create_ui_elements(params, offset)
...
function StartGameWindowSettings:_set_additional_options_enabled_state(enabled)
...
function StartGameWindowSettings:on_exit(params)
	print("[StartGameWindow] Exit Substate StartGameWindowSettings")
	self.ui_animator = nil
end

function StartGameWindowSettings:update(dt, t)
...
function StartGameWindowSettings:post_update(dt, t)
	self:draw(dt)
end

function StartGameWindowSettings:_update_animations(dt)
...
function StartGameWindowSettings:_is_button_released(widget)
...
function StartGameWindowSettings:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_enter
end

function StartGameWindowSettings:_is_button_hover_exit(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_exit
end

function StartGameWindowSettings:_is_other_option_button_selected(widget, current_option)
...
function StartGameWindowSettings:_handle_input(dt, t)
...
function StartGameWindowSettings:_play_sound(event)
	self.parent:play_sound(event)
end

function StartGameWindowSettings:_update_game_options_hover_effect()
	local widgets_by_name = self._widgets_by_name
...
function StartGameWindowSettings:_on_option_button_hover_enter(index, instant)
	local widgets_by_name = self._widgets_by_name
...
function StartGameWindowSettings:_on_option_button_hover_exit(index, instant)
	local widgets_by_name = self._widgets_by_name
	local widget_name = "game_option_" .. index
	local widget = widgets_by_name [widget_name]
...
function StartGameWindowSettings:_update_additional_options()
	local parent = self.parent
...
function StartGameWindowSettings:_update_difficulty_option()
	local parent = self.parent
...
function StartGameWindowSettings:_set_difficulty_option(difficulty_key)
...
function StartGameWindowSettings:_update_mission_selection()
	local parent = self.parent
...
function StartGameWindowSettings:_set_selected_level(level_id)
...
function StartGameWindowSettings:draw(dt)
...
function StartGameWindowSettings:_create_style_animation_enter(widget, target_value, style_id, widget_index, instant)
...
function StartGameWindowSettings:_create_style_animation_exit(widget, target_value, style_id, widget_index, instant)
	local widget_style = widget.style
	local pass_style = widget_style [style_id]
	if not pass_style then
		return
...
function StartGameWindowSettings:_animate_pulse(target, target_index, from, to, speed)
	local new_animation = UIAnimation.init(UIAnimation.pulse_animation, target, target_index, from, to, speed)
	return new_animation
end

function StartGameWindowSettings:_animate_element_by_time(target, target_index, from, to, time)
