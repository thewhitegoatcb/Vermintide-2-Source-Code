...
function ProjectileStickyLocomotion:init(extension_init_context, unit, extension_init_data)
	self.unit = unit
...
function ProjectileStickyLocomotion:_init_from_seed(seed)
	seed = seed or 0
	self._seed = seed
...
function ProjectileStickyLocomotion:destroy()
	return end

function ProjectileStickyLocomotion:update(unit, input, dt, context, t)
	self.moved = false
	...
	if self.stopped and self.stop_time then
		...
		if target_unit_pos then
			...
			local p0 = self.initial_position_boxed:unbox()
			local p1 = p0 + Vector3.up() * (0.1 + math.sin(time_lived) * 0.1) +
			forward * math.sin(time_lived * 1.4) * 0.1 +
			right * math.sin(time_lived * 1.8) * 0.1
		...
		local wobble_offset = Vector3(
		math.sin(time_lived * wobble_speed - math.rad(115)) * horizontal_wobble_amount,
		0,
		math.cos(time_lived * wobble_speed - math.rad(115)) * vertical_wobble_amount)
...
function ProjectileStickyLocomotion:moved_this_frame()
	return self.moved
end

function ProjectileStickyLocomotion:current_velocity()
	return self.velocity:unbox()
end

function ProjectileStickyLocomotion:current_position()
	return self.position_boxed:unbox()
end

function ProjectileStickyLocomotion:current_rotation()
	return self._rotation:unbox()
end

function ProjectileStickyLocomotion:last_position()
	return self._last_position:unbox()
end

function ProjectileStickyLocomotion:stop(hit_unit, hit_zone_name, hit_normal)
...
function ProjectileStickyLocomotion:stick_to_unit(unit)
	self.stopped = true
...
function ProjectileStickyLocomotion:stick_to_position(position, hit_normal)
	self.stopped = true
	self.stop_time = Managers.time:time("game")
...
function ProjectileStickyLocomotion:has_stopped()
	return self.stopped
end

function ProjectileStickyLocomotion:hot_join_sync(peer_id)
...
function ProjectileStickyLocomotion:hot_join_sync_projectile_sticky(time_lived, time_stopped)
