...
MatchmakingSettings = { TIME_BETWEEN_EACH_SEARCH = 3.4, MAX_NUM_LOBBIES = 100, START_GAME_TIME = 5, REQUEST_JOIN_LOBBY_REPLY_TIME = 30, MIN_STATUS_MESSAGE_TIME = 2, TOTAL_GAME_SEARCH_TIME = 5, afk_force_stop_mm_timer = 180, afk_warn_timer = 150, MAX_NUMBER_OF_PLAYERS = 4, host_games = "auto", restart_search_after_host_cancel = true, auto_ready = false, REQUEST_PROFILES_REPLY_TIME = 10, JOIN_LOBBY_TIME_UNTIL_AUTO_CANCEL = 20, LOBBY_FINDER_UPDATE_INTERVAL = 1,
	...
	allowed_profiles = { true, true, true, true, true },


	hero_search_filter = { true, true, true, true, true },




	quickplay_level_select_settings = { loss_multiplier = 1, win_multiplier = 1, base_level_weight = 1, amount_of_relevant_games = 20, progression_multiplier = 10 } }
...
local RPCS = { "rpc_matchmaking_request_profiles_data", "rpc_matchmaking_request_join_lobby", "rpc_matchmaking_request_profile", "rpc_set_matchmaking", "rpc_cancel_matchmaking", "rpc_matchmaking_request_join_lobby_reply", "rpc_notify_connected", "rpc_matchmaking_join_game", "rpc_matchmaking_request_profile_reply", "rpc_matchmaking_request_profiles_data_reply", "rpc_matchmaking_request_selected_level", "rpc_matchmaking_request_selected_level_reply", "rpc_matchmaking_request_selected_difficulty", "rpc_matchmaking_request_selected_difficulty_reply", "rpc_matchmaking_request_status_message", "rpc_matchmaking_status_message", "rpc_set_client_game_privacy", "rpc_game_server_reserve_slots", "rpc_game_server_set_group_leader", "rpc_matchmaking_broadcast_game_server_ip_address", "rpc_set_quick_game", "rpc_start_game_countdown_finished", "rpc_set_game_mode_event_data", "rpc_clear_game_mode_event_data", "rpc_matchmaking_sync_quickplay_data", "rpc_matchmaking_request_quickplay_data", "rpc_matchmaking_verify_dlc", "rpc_matchmaking_verify_dlc_reply", "rpc_join_reserved_game_server" }
...
function MatchmakingManager:init(params)
	self.params = params
	...
	self.debug = { text = "", progression = "", lobby_timer = 0, state = "", hero = "", difficulty = "", level = "" }
...
function MatchmakingManager:reset_lobby_filters()
...
function MatchmakingManager:game_mode_event_data()
	return self._game_mode_event_data
end

function MatchmakingManager:set_game_mode_event_data(event_data)
...
function MatchmakingManager:clear_game_mode_event_data()
...
function MatchmakingManager:set_statistics_db(statistics_db)
	self.statistics_db = statistics_db
	self.params.statistics_db = statistics_db
end

function MatchmakingManager:set_active_lobby_browser(lobby_browser)
	self._lobby_browser = lobby_browser
end

function MatchmakingManager:setup_post_init_data(post_init_data)
...
function MatchmakingManager:waystone_is_active()
	return self._waystone_is_active or false, self._waystone_type or 0
end

function MatchmakingManager:activate_waystone_portal(waystone_type)
...
function MatchmakingManager:destroy()
...
function MatchmakingManager:register_rpcs(network_event_delegate)
...
function MatchmakingManager:unregister_rpcs()
...
function MatchmakingManager:_change_state(new_state, params, state_context, reason)
...
function MatchmakingManager:_remove_old_broken_lobbies(t)
...
function MatchmakingManager:update(dt, t)
	if self._state then
	...
	if not DEDICATED_SERVER and
	self._joining_this_host_peer_id and PEER_ID_TO_CHANNEL [self._joining_this_host_peer_id] == nil then
...
function MatchmakingManager:_update_afk_logic(dt, t)
...
function MatchmakingManager:_update_power_level(t)
...
function MatchmakingManager:get_average_power_level()
...
function MatchmakingManager:get_average_weave_progression()
	return 1
end


function MatchmakingManager:has_required_power_level(lobby_data, profile_name, career_name)
...
function MatchmakingManager:_set_power_level()
...
function MatchmakingManager:state()
	return self._state
end

function MatchmakingManager:gather_party_unlocked_journeys()
...
function MatchmakingManager:party_has_level_unlocked(level_key, ignore_dlc_check, ommit_dlc_levels, event_mode)
...
function MatchmakingManager:_get_unlocked_levels_by_party(ignore_dlc_check, ommit_dlc_levels, event_mode, excluded_level_keys)
	local unlocked_levels = { }

	excluded_level_keys =
	excluded_level_keys or { }


	local level_keys = UnlockableLevelsByGameMode.adventure
	for _, level_key in ipairs(level_keys) do
		if self:party_has_level_unlocked(level_key, ignore_dlc_check, ommit_dlc_levels, event_mode) and
		not table.contains(excluded_level_keys, level_key) then
...
function MatchmakingManager:_get_unlocked_levels(ignore_dlc_check)
	local unlocked_levels = { }
...
function MatchmakingManager:_get_level_key_from_level_weights(level_keys, event_mode)
...
function MatchmakingManager:_calculate_level_weights(level_keys, recent_games_played)
	fassert(#level_keys > 0, "Empty level_keys list")
	...
	for i = 1, #last_played_games do
		...
		if
		level_key and table.contains(lookup_level_keys, level_key) then
			local level_index = lookup_level_keys [level_key]
			if level_index then
...
function MatchmakingManager:_add_level_weight(peer_id, weight_array)
	local level_weights = self._level_weights
...
function MatchmakingManager:_remove_irrelevant_level_weights()
...
function MatchmakingManager:get_weighed_random_unlocked_level(ignore_dlc_check, custom_game, excluded_level_keys)
...
function MatchmakingManager:_party_has_completed_act(act_name)
	local players = Managers.player:human_players()
	local statistics_db = self.statistics_db
	for _, player in pairs(players) do
		local stats_id = player:stats_id()
...
function MatchmakingManager:set_matchmaking_data(next_mission_id, difficulty, act_key, matchmaking_type, private_game, quick_game, eac_authorized, environment_id, mechanism)
...
function MatchmakingManager:on_dedicated_server()
	return self.lobby:is_dedicated_server()
end

function MatchmakingManager:weave_vote_result(accepted)
...
function MatchmakingManager:find_game(search_config)
	if self.is_server then
...
function MatchmakingManager:cancel_matchmaking()
...
function MatchmakingManager:is_matchmaking_in_inn()
...
function MatchmakingManager:is_game_matchmaking()
	local name = self._state.NAME
	local is_matchmaking = name ~= "MatchmakingStateIdle"
	local search_config = self.state_context.search_config
...
function MatchmakingManager:active_game_mode()
	local name = self._state.NAME
	local is_matchmaking = name ~= "MatchmakingStateIdle"
...
function MatchmakingManager:rpc_set_matchmaking(channel_id, is_matchmaking, private_game, mission_id, difficulty_id, quick_game, mechanism_id)
...
function MatchmakingManager:rpc_set_game_mode_event_data(channel_id, mutator_lookup_array)
	local mutators = { }
	...
	self._game_mode_event_data = { mutators = mutators }
end

function MatchmakingManager:rpc_clear_game_mode_event_data(channel_id)
	self:clear_game_mode_event_data()
end



function MatchmakingManager:rpc_cancel_matchmaking(channel_id)
...
function MatchmakingManager:rpc_matchmaking_request_profiles_data(channel_id)
	local peer_id = CHANNEL_TO_PEER_ID [channel_id]
...
function MatchmakingManager:_extract_dlcs(client_dlc_unlocked_array)
...
function MatchmakingManager:_missing_required_dlc(mechanism, difficulty_key, client_unlocked_dlcs)
	local mechanism_settings = mechanism and MechanismSettings [mechanism]
	if mechanism_settings and mechanism_settings.required_dlc and
	not client_unlocked_dlcs [mechanism_settings.required_dlc] then
		return mechanism_settings.required_dlc
	end


	local difficulty_settings = difficulty_key and DifficultySettings [difficulty_key]
	if difficulty_settings and difficulty_settings.dlc_requirement and
	not client_unlocked_dlcs [difficulty_settings.dlc_requirement] then
...
function MatchmakingManager:rpc_matchmaking_request_join_lobby(channel_id, lobby_id, friend_join, client_dlc_unlocked_array)
	...
	local is_searching_for_players = self._state.NAME == "MatchmakingStateHostGame" or
	self._state.NAME == "MatchmakingStateIngame" or
	self._state.NAME == "MatchmakingStateWaitForCountdown" or
	self._state.NAME == "MatchmakingStateHostFindWeaveGroup"
...
function MatchmakingManager:rpc_matchmaking_request_profile(channel_id, profile)
	local peer_id = CHANNEL_TO_PEER_ID [channel_id]
...
function MatchmakingManager:current_state()
	return self._state and self._state.NAME or "none"
end

function MatchmakingManager:get_transition()
...
function MatchmakingManager:hero_available_in_lobby_data(hero_index, lobby_data)
...
function MatchmakingManager:lobby_match(lobby_data, act_key, mission_id, difficulty, matchmaking_type, player_peer_id, weave_name, mechanism)
	...
	if IS_WINDOWS then
		local party_members = { lobby_data.player_slot_1, lobby_data.player_slot_2, lobby_data.player_slot_3, lobby_data.player_slot_4, lobby_data.player_slot_5 }
	...
	if
	act_key and lobby_data.act_key ~= act_key then
	...
	if mechanism == "weave" then
		if weave_name ~= "false" then
			...
			if weave_name ~= lobby_weave_name then
				do return false, "wrong weave name" end
			end
		else
...
function MatchmakingManager:add_broken_lobby_client(lobby_client, t, is_bad_connection_or_otherwise_not_nice)
...
function MatchmakingManager:lobby_listed_as_broken(lobby_id)
	return MatchmakingManager._broken_lobbies [lobby_id]
end


function MatchmakingManager:server_listed_as_broken(ip_port)
	return MatchmakingManager._broken_servers [ip_port]
end


function MatchmakingManager:broken_server_map()
	return MatchmakingManager._broken_servers
end


function MatchmakingManager:rpc_matchmaking_request_join_lobby_reply(channel_id, reply_id, reply_variable)
...
function MatchmakingManager:rpc_notify_connected(channel_id)
	local state_name = self._state and self._state.NAME or "none"
...
function MatchmakingManager:rpc_matchmaking_join_game(channel_id)
...
function MatchmakingManager:rpc_matchmaking_request_profile_reply(channel_id, profile, reply)
...
function MatchmakingManager:rpc_matchmaking_request_profiles_data_reply(channel_id, profile_array, player_id_array)
...
function MatchmakingManager:rpc_matchmaking_request_selected_level(channel_id)
...
function MatchmakingManager:rpc_matchmaking_request_selected_level_reply(channel_id, selected_level_id)
...
function MatchmakingManager:rpc_matchmaking_request_selected_difficulty(channel_id)
	if self._state and self._state.NAME == "MatchmakingStateHostGame" then
		local lobby_data = self.lobby:get_stored_lobby_data()
		local difficulty = lobby_data.difficulty
...
function MatchmakingManager:rpc_matchmaking_request_selected_difficulty_reply(channel_id, difficulty_id)
	if self._state and self._state.NAME == "MatchmakingStateFriendClient" then
...
function MatchmakingManager:rpc_matchmaking_request_status_message(channel_id)
	if self._state and self._state.NAME == "MatchmakingStateHostGame" then
...
function MatchmakingManager:rpc_matchmaking_status_message(channel_id, status_message)
	if self._state and self._state.NAME == "MatchmakingStateFriendClient" then
...
function MatchmakingManager:rpc_game_server_set_group_leader(channel_id, peer_id)
...
function MatchmakingManager:rpc_matchmaking_broadcast_game_server_ip_address(channel_id, ip_address)
	if self._state and self._state.NAME == "MatchmakingStateFriendClient" then
...
function MatchmakingManager:rpc_game_server_reserve_slots(channel_id, peer_table)
	for _, peer_id in ipairs(peer_table) do
		self.network_server:add_reserve_slot(peer_id)
	end
end

function MatchmakingManager:rpc_set_quick_game(channel_id, quick_game)
	self:set_quick_game(quick_game)
end

function MatchmakingManager:rpc_start_game_countdown_finished(channel_id)
	self:countdown_completed()
end

function MatchmakingManager:rpc_matchmaking_sync_quickplay_data(channel_id, weight_array)
	local peer_id = CHANNEL_TO_PEER_ID [channel_id]
	self:_add_level_weight(peer_id, weight_array)
end

function MatchmakingManager:rpc_matchmaking_request_quickplay_data(channel_id)
	local unlocked_levels = self:_get_unlocked_levels()
	local recent_games_played_json = Managers.backend:get_read_only_data("recent_quickplay_games")
	local recent_games_played = recent_games_played_json and cjson.decode(recent_games_played_json) or { }
	local ignore_dlc_check = false
...
function MatchmakingManager:rpc_matchmaking_verify_dlc(channel_id, dlc_name_ids)
	if self._state then
		local success = true
...
function MatchmakingManager:rpc_matchmaking_verify_dlc_reply(channel_id, success)
...
function MatchmakingManager:rpc_join_reserved_game_server(channel_id)
	local state_name = self._state and self._state.NAME or "none"
...
function MatchmakingManager:hot_join_sync(peer_id)
...
function MatchmakingManager:countdown_completed()


	local tell_server = not self.countdown_has_finished and
	not self.is_server and
	Managers.party:is_leader(self.peer_id) and
	self:on_dedicated_server()
...
function MatchmakingManager:set_status_message(status_message)
...
function MatchmakingManager:setup_filter_requirements(number_of_players, distance_filter, filters, near_filters)
	filters.network_hash = { comparison = "equal",
		value = self._network_hash }


	filters.matchmaking = { value = "true", comparison = "equal" }
...
function MatchmakingManager:request_join_lobby(lobby, state_context_params)
	...
	local state_context = { join_by_lobby_browser = true,
...
function MatchmakingManager:is_joining_friend()
	return self.state_context.friend_join
end


function MatchmakingManager:cancel_join_lobby(reason, reason_variable)
...
function MatchmakingManager:allowed_to_initiate_join_lobby()
	local matchmaking_status = self:_matchmaking_status()
	return matchmaking_status == "idle"
end

function MatchmakingManager:allow_cancel_matchmaking()
...
function MatchmakingManager:send_system_chat_message(message, localization_param)
	local channel_id = 1
...
function MatchmakingManager:rpc_set_client_game_privacy(channel_id, is_private)
	local lobby = self.lobby
	if not self.is_server then
...
function MatchmakingManager:set_game_privacy(is_private)
	local lobby = self.lobby
	if self.is_server and lobby:is_joined() then
...
function MatchmakingManager:set_in_progress_game_privacy(is_private)
	local lobby = self.lobby
	if self.is_server and lobby:is_joined() then
...
function MatchmakingManager:_set_lobby_data(lobby, field, value)
	local stored_lobby_data = lobby:get_stored_lobby_data()
	stored_lobby_data [field] = value
	lobby:set_lobby_data(stored_lobby_data)
end

function MatchmakingManager:is_game_private()
	local lobby = self.lobby
	local stored_lobby_data = lobby:get_stored_lobby_data()
...
function MatchmakingManager:set_local_quick_game(quick_game)
	self._local_quick_game = quick_game or false
end

function MatchmakingManager:is_local_quick_game()
	return self._local_quick_game
end

function MatchmakingManager:set_quick_game(quick_game)
...
function MatchmakingManager:is_quick_game()
	return self._quick_game
end

function MatchmakingManager:_matchmaking_status()
	local state_name = self._state.NAME
	if state_name == "MatchmakingStateIdle" then
		do return "idle" end
	elseif state_name == "MatchmakingStateSearchGame" then
		do return "searching_for_game" end
	...
	elseif state_name == "MatchmakingStateRequestJoinGame" or state_name == "MatchmakingStateRequestProfiles" or state_name == "MatchmakingStateReserveLobby" or state_name == "MatchmakingStateJoinGame" then
		...
		do return "joining_game" end
	elseif state_name == "MatchmakingStateFriendClient" then
		do return "waiting_for_game_start" end
	else
		return state_name
	end
end

function MatchmakingManager:are_all_players_spawned()
	local lobby = self.lobby
...
function MatchmakingManager:get_reserved_slots()
...
local SLOTS = { }
local RESERVATIONS = { }
function MatchmakingManager:_decode_reserved_slots_mask(reserved_slots_mask, mechanism_name)
	table.clear(SLOTS)
...
local info = { }
function MatchmakingManager:search_info()
	table.clear(info)
...
function MatchmakingManager:setup_weave_filters(state_context, filter_table)
	...
	for filter_name, filter_data in pairs(current_filter_rules) do
		...
		if filter_data.transform_data_function then value = filter_data.transform_data_function(value) or value end
...
function MatchmakingManager:reset_joining()
	self._joining_this_host_peer_id = nil
end

function MatchmakingManager:setup_weave_near_filters(state_context, filter_table)
	local expansion_rule_index = math.min(self.state_context.expansion_rule_index or 1, WeaveMatchmakingSettings.num_expansion_rules)
	local expansion_rules = WeaveMatchmakingSettings.expansion_rules [expansion_rule_index]
...
function MatchmakingManager:debug_weave_matchmaking(state_context, state)
