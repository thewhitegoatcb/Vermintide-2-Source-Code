...
function EventManager:register(object, ...)
	for i = 1, select("#", ...), 2 do
		...
		self._events [event_name] = self._events [event_name] or setmetatable({ }, { __mode = "v" })
...
function EventManager:register_referenced(reference, object, ...)
	...
	for i = 1, select("#", ...), 2 do
		...
		registered_events [event_name] = registered_events [event_name] or setmetatable({ }, { __mode = "v" })
...
function EventManager:unregister_referenced(event_name, reference, object)
...
function EventManager:unregister_referenced_all(reference)
	self._referenced_events [reference] = nil
end
...
function EventManager:trigger_referenced(reference, event_name, ...)
