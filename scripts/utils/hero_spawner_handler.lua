...
function HeroSpawnerHandler:init(is_server, profile_synchronizer, network_event_delegate)
...
function HeroSpawnerHandler:destroy()
	self.network_event_delegate:unregister(self)
end

function HeroSpawnerHandler:spawn_hero_request(player, hero_name)
...
function HeroSpawnerHandler:start()
...
function HeroSpawnerHandler:update(dt)
...
function HeroSpawnerHandler:save_selected_profile(index)
	local save_manager = Managers.save
	SaveData.wanted_profile_index = index
	save_manager:auto_save(SaveFileName, SaveData, nil)
end

function HeroSpawnerHandler:query_result(request_id)
	fassert(request_id == self.request_id, "HeroSpawnerHandler:query_result with invalid request_id")
	return self.result
end

function HeroSpawnerHandler:rpc_to_client_spawn_player(channel_id, local_player_id, profile_index, career_index, position, rotation, is_initial_spawn)
