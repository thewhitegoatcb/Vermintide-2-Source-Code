...
local rpcs = { "rpc_request_join_party", "rpc_reset_party_data", "rpc_peer_assigned_to_party", "rpc_remove_peer_from_party" }
...
function PartyManager:_free_lobby()
...
function PartyManager:set_leader(peer_id)
	if peer_id == nil then
...
function PartyManager:is_leader(peer_id)
	return peer_id == self._leader
end

function PartyManager:has_party_lobby()
	return self._party_lobby_or_data ~= nil
end


function PartyManager:store_lobby(lobby_or_data)
...
function PartyManager:steal_lobby()
...
function PartyManager:clear(sync_to_clients)
	...
	self._undecided_party = self:_register_party({ party_id = 0, name = "undecided", num_open_slots = 0, game_participating = false, num_slots = 64, tags = { } })
...
function PartyManager:gather_party_members()
	local members = { }
...
function PartyManager:_register_party(def)
	...
	for j = 1, num_slots do
		...
		slots_data [j] = { slot_id = j }
	end

	local party = { num_bots = 0, num_used_slots = 0,
...
function PartyManager:register_parties(party_definitions)
...
function PartyManager:cleared()
	return self._cleared
end

function PartyManager:_create_player_status(peer_id, local_player_id, is_bot)
...
function PartyManager:register_player(player, unique_id)
...
function PartyManager:set_selected_profile(peer_id, local_player_id, profile_index, career_index)
...
function PartyManager:cleanup_game_mode_data()
	for unique_id, status in pairs(self._player_statuses) do
		status.game_mode_data = { }
	end
end

function PartyManager:register_rpcs(network_event_delegate)
	self._network_event_delegate = network_event_delegate
	network_event_delegate:register(self, unpack(rpcs))
end

function PartyManager:unregister_rpcs()
	self._network_event_delegate:unregister(self)
	self._network_event_delegate = nil
end

function PartyManager:update(t, dt)
...
function PartyManager:get_local_player_party()
	return self:get_party(self._local_player_party_id)
end

function PartyManager:get_party(party_id)
	return self._parties [party_id]
end

function PartyManager:parties()
	return self._parties
end

function PartyManager:_slot_empty_in_party(party_id, slot_id)
...
function PartyManager:request_join_party(peer_id, local_player_id, party_id, optional_slot_id, optional_bot_player)
...
function PartyManager:get_player_status(peer_id, local_player_id)
	local unique_id = PlayerUtils.unique_player_id(peer_id, local_player_id)
	local status = self._player_statuses [unique_id]
	return status
end

function PartyManager:get_status_from_unique_id(unique_id)
	return self._player_statuses [unique_id]
end

function PartyManager:get_party_from_player_id(peer_id, local_player_id)
	local unique_id = PlayerUtils.unique_player_id(peer_id, local_player_id)
	local status = self._player_statuses [unique_id]
	if status then
...
function PartyManager:get_party_from_unique_id(unique_id)
	local status = self._player_statuses [unique_id]
	if status then
		local party_id = status.party_id
		return self._parties [party_id], party_id
...
function PartyManager:get_party_from_name(party_name)
...
function update_status_profile_index(player_status)
	...
	if not profile_synchronizer then return
	end
...
function PartyManager:get_num_parties()
	return self._num_parties
end

function PartyManager:get_num_game_participating_parties()
	return self._num_game_participating_parties
end

function PartyManager:assign_peer_to_party(peer_id, local_player_id, wanted_party_id, optional_slot_id, is_bot)
...
function PartyManager:remove_peer_from_party(peer_id, local_player_id, party_id)
...
function PartyManager:get_players_in_party(party_id)
...
function PartyManager._find_slot_index(party, slot_id)
...
function PartyManager:_clear_slot_in_party(party, slot_id, is_bot)
	party.slots [slot_id] = { }
...
function PartyManager:_find_first_empty_slot_id(party)
...
function PartyManager:get_least_filled_party(ignore_bots, ignore_none_game_participating_party)
	local parties = self._parties
...
function PartyManager:is_party_full(party_id)
	local party = self._parties [party_id]
	local num_open_slots = party.num_open_slots + party.num_bots
	return num_open_slots == 0
end

function PartyManager:is_player_in_party(unique_id, party_id)
	local party = self._parties [party_id]
	local player_status = self._player_statuses [unique_id]
	return party_id == player_status.party_id
end

function PartyManager:_get_last_added_bot_for_party(party_id)
	local party = self._parties [party_id]
...
function PartyManager:hot_join_sync(peer_id)
	local parties = self._parties
...
function PartyManager:_send_rpc_to_clients(rpc_name, ...)
...
function PartyManager:network_context_created(lobby, server_peer_id, own_peer_id)
...
function PartyManager:parties_by_name()
	return self._party_by_name
end


function PartyManager:network_context_destroyed()
...
function PartyManager:server_peer_hot_join_synced(peer_id)
	self._hot_join_synced_peers [peer_id] = true
end



function PartyManager:server_peer_left_session(peer_id)
...
function PartyManager:rpc_request_join_party(channel_id, peer_id, local_player_id, party_id, optional_slot_id)
...
function PartyManager:rpc_peer_assigned_to_party(channel_id, peer_id, local_player_id, party_id, slot_id, is_bot)
...
function PartyManager:rpc_remove_peer_from_party(channel_id, peer_id, local_player_id, party_id)
...
function PartyManager:rpc_reset_party_data()
	self:clear()
	Managers.mechanism:setup_mechanism_parties()
end

function PartyManager:_draw_debug(t)
	...
	for i = 0, #parties do
		...
		for i = 1, #occupied_slots do
			...
			if player then
				if player:is_player_controlled() then player_controlled = "P" else player_controlled = "B" end
				info = "1"
				...
				elseif next(player.owned_units) then info = "P" else info = "?"
				end
			end
...
function PartyManager:any_party_has_free_slots(num_slots)
