...
local RPCS = { "rpc_link_pickup", "rpc_spawn_and_link_units" }




local extensions = { "ProjectileLinkerExtension" }



local LINKED_PROJECTILE_LIFETIME = 30

function ProjectileLinkerSystem:init(entity_system_creation_context, system_name)
	...
	function self.cb_linked_projectile_owner_destroyed(destroyed_linked_projectile_owner_unit)
	...
	function self.cb_linked_pickup_projectile_owner_destroyed(destroyed_linked_projectile_owner_unit)
		for owner_unit, linked_projectiles in pairs(self.linked_projectile_units) do
			if owner_unit == destroyed_linked_projectile_owner_unit then
				for linked_projectile_unit, _ in pairs(linked_projectiles) do
					if self:_has_reference(linked_projectile_unit) then
	...
	function self.cb_linked_projectile_timeout(linked_projectile_owner_unit, linked_projectile_unit)
		if self:_has_reference(linked_projectile_unit) then
			self:_remove_linked_projectile_reference(linked_projectile_unit)
		end
	...
	function self.cb_linked_pickup_projectile_timeout(linked_projectile_owner_unit, linked_projectile_unit)
		if self:_has_reference(linked_projectile_unit) then
			self:_remove_linked_projectile_reference(linked_projectile_unit)
		end
...
function ProjectileLinkerSystem:on_remove_extension(unit, extension_name)
	self:clear_linked_projectiles(unit)
	return ProjectileLinkerSystem.super.on_remove_extension(self, unit, extension_name)
end

function ProjectileLinkerSystem:freeze(unit, extension_name, reason)
	self:clear_linked_projectiles(unit)
end

function ProjectileLinkerSystem:clear_linked_projectiles(unit)
...
function ProjectileLinkerSystem:update(context, t)
...
function ProjectileLinkerSystem:destroy()
	self.network_event_delegate:unregister(self)
end

function ProjectileLinkerSystem:add_linked_projectile_reference(owner_unit, linked_projectile_unit, destroy_cb_name, timeout_cb_name, add_destroy_listener)
...
function ProjectileLinkerSystem:_remove_linked_projectile_reference(linked_projectile_unit)
	for _, linked_projectiles in pairs(self.linked_projectile_units) do
		linked_projectiles [linked_projectile_unit] = nil
	end
end

function ProjectileLinkerSystem:_has_reference(linked_projectile_unit)
	for _, linked_projectiles in pairs(self.linked_projectile_units) do
...
function ProjectileLinkerSystem:link_pickup(pickup_unit, link_position, link_rotation, hit_unit, node_index)
...
function ProjectileLinkerSystem:rpc_link_pickup(channel_id, pickup_unit_go_id, link_position, link_rotation, hit_unit_go_id, node_index, is_level_unit)
...
function ProjectileLinkerSystem:spawn_and_link_units(linked_unit_name, link_position, link_rotation, hit_unit, node_index)
	local unit_spawner = Managers.state.unit_spawner
	if ScriptUnit.has_extension(hit_unit, "projectile_linker_system") then
...
function ProjectileLinkerSystem:rpc_spawn_and_link_units(channel_id, linked_unit_name_id, link_position, link_rotation, hit_unit_go_id, node_index, is_level_unit)
	local hit_unit = Managers.state.network:game_object_or_level_unit(hit_unit_go_id, is_level_unit)
