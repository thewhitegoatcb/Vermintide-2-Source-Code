...
function PlayerCharacterStateJumping:init(character_state_init_context)
...
function PlayerCharacterStateJumping:on_enter(unit, input, dt, context, t, previous_state, params)
	...
	if script_data.use_super_jumps then jump_speed = jump_speed * 2
	end
	...
	move_anim = CharacterStateHelper.has_move_input(input_extension) and
	"jump_fwd" or
...
function PlayerCharacterStateJumping:on_exit(unit, input, dt, context, t, next_state)
	local input_extension = self.input_extension
...
function PlayerCharacterStateJumping:update(unit, input, dt, context, t)
	local csm = self.csm
	local movement_settings_table = PlayerUnitMovementSettings.get_movement_settings_table(unit)
