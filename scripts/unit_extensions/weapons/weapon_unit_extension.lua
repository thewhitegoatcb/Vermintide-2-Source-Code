...
function WeaponUnitExtension:init(extension_init_context, unit, extension_init_data)
...
function WeaponUnitExtension:update_game_options()
	local weapon_trails = Application.user_setting("weapon_trails")
	Unit.set_data(self.unit, "trails_enabled", weapon_trails ~= "none")
end


function WeaponUnitExtension:cb_game_session_disconnect()
	self.sync_data_game_object_id = nil
end

function WeaponUnitExtension:extensions_ready(world, unit)
...
function WeaponUnitExtension:destroy()
...
function WeaponUnitExtension:get_action(action_name, sub_action_name, actions)
...
function WeaponUnitExtension:start_action(action_name, sub_action_name, actions, t, power_level, action_init_data)
	local owner_unit = self.owner_unit
		...
		if not self.actions [action_kind] then
			...
			if new_action_instance.passive_update then
				if not self._passive_update_actions then
					self._passive_update_actions = { new_action_instance }
	...
	if ammo_extension ~= nil and new_action then
		...
		elseif ammo_count < ammo_requirement then
			if ammo_extension:total_remaining_ammo() == 0 and (not self.reload_failed_timer or self.reload_failed_timer < t) and (


			not action.interaction_type or action.interaction_type ~= "heal") and not action.no_out_of_ammo_vo then
...
function WeaponUnitExtension:stop_action(reason, data)
	if self:has_current_action() and

	not self._currently_stopping_action then
...
function WeaponUnitExtension:_finish_action(reason, data)
	local current_action_settings = self.current_action_settings
	local action_kind = current_action_settings.kind
	local action = self.actions [action_kind]
...
function WeaponUnitExtension:anim_end_event(reason, current_action_settings)
...
function WeaponUnitExtension:trigger_anim_event(event)
	if event then
		local event_id = NetworkLookup.anims [event]
...
function WeaponUnitExtension:update(unit, input, dt, context, t)
	local current_action_settings = self.current_action_settings
...
function WeaponUnitExtension:is_streak_action_available(streak_action, t, time_offset)
	...
	if action.streak_available and
	action:streak_available(current_time_in_action, streak_action) and
	self:is_chain_action_available(streak_action, t, time_offset) then
		return true
	end
...
function WeaponUnitExtension:is_chain_action_available(next_chain_action, t, time_offset)
	local current_action_settings = self.current_action_settings or self.temporary_action_settings
	local current_time_in_action = t - self.action_time_started
	...
	if next_chain_action.auto_chain then
		do return current_time_in_action >= ( next_chain_action.start_time and next_chain_action.start_time / chain_time_scale or max_time ) + time_offset end
	else
...
function WeaponUnitExtension:time_to_next_chain_action(next_chain_action, t, time_offset, action_settings)
...
function WeaponUnitExtension:get_scaled_min_hold_time(action)
...
function WeaponUnitExtension:can_stop_hold_action(t)
	local current_time_in_action = t - self.action_time_started
	local current_action_settings = self.current_action_settings
...
function WeaponUnitExtension:get_action_cooldown(action)
	local action_cooldown = self.cooldown_timer [action]
	return action_cooldown
end

function WeaponUnitExtension:get_current_action()
	return self.actions [self.current_action_settings.kind]
end

function WeaponUnitExtension:has_current_action()
	return self.current_action_settings ~= nil
end

function WeaponUnitExtension:get_current_action_settings()
	return self.current_action_settings
end


function WeaponUnitExtension:is_after_damage_window()
...
function WeaponUnitExtension:bot_should_stop_attack_on_leave()
	local current_action_settings = self.current_action_settings
	if current_action_settings then
...
function WeaponUnitExtension:_is_before_end_time(next_chain_action, t)
	local current_action_settings = self.current_action_settings or self.temporary_action_settings
	local current_time_in_action = t - self.action_time_started
	local max_time = current_action_settings.total_time + 2
...
function WeaponUnitExtension:_find_chain_action(actions, allowed_chain_actions, t, wanted_input, wanted_occurrence_number)
...
function WeaponUnitExtension:_get_attack_chain_data(actions, attack_chain, t)
	...
	if current_action_settings then
		...
		local start_action_name = attack_chain.start_action_name local start_sub_action_name = attack_chain.start_sub_action_name
		action_settings = actions [start_action_name] [start_sub_action_name]
	end
	local lookup_data = action_settings.lookup_data
	local action_name = lookup_data.action_name local sub_action_name = lookup_data.sub_action_name
...
function WeaponUnitExtension:_process_bot_attack_request(attack_type, actions, weapon_name, t, attack_chain)
...
function WeaponUnitExtension:update_bot_attack_request(t)
...
function WeaponUnitExtension:request_bot_attack_action(attack_type, actions, weapon_name, attack_chain)
	local bot_attack_data = self.bot_attack_data
...
function WeaponUnitExtension:clear_bot_attack_request()
	local bot_attack_data = self.bot_attack_data
	local attack_request = bot_attack_data.request
...
function WeaponUnitExtension:is_starting_attack()
	local current_action_settings = self.current_action_settings
	return ActionUtils.is_melee_start_sub_action(current_action_settings)
end

function WeaponUnitExtension:time_to_next_attack(wanted_attack_type, current_actions, current_weapon_name, t, attack_chain)
	local bot_attack_data = self.bot_attack_data
	...
	if chain_action then
		...
		do return chain_action_time end
	else
		return nil
	end
end

function WeaponUnitExtension:set_mode(new_mode)
	self.weapon_mode = new_mode
end

function WeaponUnitExtension:get_mode()
	return self.weapon_mode
end

function WeaponUnitExtension:get_custom_data(key)
...
function WeaponUnitExtension:set_custom_data(key, value)
	fassert(self._custom_data [key] ~= nil, "Custom data key '%s' does not exist, add it to the weapon template", key)
	self._custom_data [key] = value
end

function WeaponUnitExtension:set_weapon_buffs(buffs)
	local owner_unit = self.owner_unit
	local buff_extension = ScriptUnit.extension(owner_unit, "buff_system")
...
function WeaponUnitExtension:add_looping_audio(id, start_event_id, end_event_id, start_event_husk_id, end_event_husk_id, auto_start)
...
function WeaponUnitExtension:start_looping_audio(id)
	local audio_data = self.looping_audio_events [id]
	if not audio_data or audio_data.is_playing then
		return
	end

	if self.is_local and not self.is_bot and
	not audio_data.wwise_playing_id then
...
function WeaponUnitExtension:stop_looping_audio(id)
	local audio_data = self.looping_audio_events [id]
...
function WeaponUnitExtension:is_playing_looping_audio(id)
	local audio_data = self.looping_audio_events [id]
	if audio_data then
...
function WeaponUnitExtension:set_looping_audio_switch(id, group, state)
	local audio_data = self.looping_audio_events [id]
...
function WeaponUnitExtension:update_looping_audio_parameter(id, parameter_name, parameter_value)
	local audio_data = self.looping_audio_events [id]
...
function WeaponUnitExtension:on_wield(hand_name)
	local first_person_extension = self.first_person_extension
	if first_person_extension then
		local weapon_template = self._weapon_template
		local sway_settings = weapon_template and weapon_template.weapon_sway_settings
...
function WeaponUnitExtension:on_unwield(hand_name)
