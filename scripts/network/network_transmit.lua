...
function NetworkTransmit:init(is_server, server_peer_id)
	self.is_server = is_server
	self.peer_id = Network.peer_id()
	self.server_peer_id = server_peer_id

	self.local_rpc_queue = { { }, { } }
	self.local_rpc_queue_n = { 0, 0 }
...
function NetworkTransmit:update_receive()
	self._pack_temp_types = false
end

function NetworkTransmit:set_game_session(session)
	self.game_session = session
end

function NetworkTransmit:add_peer_ignore(peer_id)
	self.peer_ignore_list [peer_id] = true
end

function NetworkTransmit:remove_peer_ignore(peer_id)
	self.peer_ignore_list [peer_id] = nil
end

function NetworkTransmit:destroy()
	GarbageLeakDetector.register_object(self, "NetworkTransmit")
end

function NetworkTransmit:queue_local_rpc(rpc_name, ...)
...
function NetworkTransmit:transmit_local_rpcs()
...
function NetworkTransmit:set_network_event_delegate(network_event_delegate)
	self.network_event_delegate = network_event_delegate
end

function NetworkTransmit:send_rpc(rpc_name, peer_id, ...)
...
function NetworkTransmit:send_rpc_server(rpc_name, ...)
	local rpc = RPC [rpc_name]
...
function NetworkTransmit:send_rpc_party_clients(rpc_name, party, include_spectators, ...)
...
function NetworkTransmit:send_rpc_party(rpc_name, party, include_spectators, ...)
	fassert(self.is_server, "Trying to send rpc %q on client to clients which is wrong. Only servers should use this function.", rpc_name)
...
function NetworkTransmit:send_rpc_side_clients(rpc_name, side, include_spectators, ...)
	fassert(self.is_server, "Trying to send rpc %q on client to clients which is wrong. Only servers should use this function.", rpc_name)
...
function NetworkTransmit:send_rpc_clients(rpc_name, ...)
	fassert(self.is_server, "Trying to send rpc %q on client to clients which is wrong. Only servers should use this function.", rpc_name)
...
function NetworkTransmit:send_rpc_clients_except(rpc_name, except, ...)
	fassert(self.is_server, "Trying to send rpc %q on client to clients which is wrong. Only servers should use this function.", rpc_name)
...
function NetworkTransmit:send_rpc_side_clients_except(rpc_name, side, include_spectators, except, ...)
	fassert(self.is_server, "Trying to send rpc %q on client to clients which is wrong. Only servers should use this function.", rpc_name)
...
function NetworkTransmit:send_rpc_all(rpc_name, ...)
	fassert(self.is_server, "Trying to send rpc %q on client to clients which is wrong. Only servers should use this function.", rpc_name)
...
function NetworkTransmit:send_rpc_all_except(rpc_name, except, ...)
	fassert(self.is_server, "Trying to send rpc %q on client to clients which is wrong. Only servers should use this function.", rpc_name)
