...
function DeusSpawning:init(profile_synchronizer, side, is_server, network_server, deus_run_controller)
...
function DeusSpawning:register_rpcs(network_event_delegate, network_transmit)
	self._respawn_handler:register_rpcs(network_event_delegate, network_transmit)
end

function DeusSpawning:unregister_rpcs()
	self._respawn_handler:unregister_rpcs()
end

function DeusSpawning:_restore_player_game_mode_data(peer_id, local_player_id, profile_index, career_index)
...
function DeusSpawning:_check_observer_camera(peer_id, local_player_id)
...
function DeusSpawning:_unassign_data_from_slot(slot, data)
	slot.game_mode_data = { }
end

function DeusSpawning:player_entered_game_session(peer_id, local_player_id)
...
function DeusSpawning:player_joined_party(peer_id, local_player_id, party_id, slot_id)
	return end

function DeusSpawning:player_left_party(peer_id, local_player_id, party_id, slot_id)
	return end

function DeusSpawning:update(t, dt)
	if Managers.state.network:game() then
		self._respawn_handler:update(dt, t)
	end
end

function DeusSpawning:server_update(t, dt)
...
function DeusSpawning:profile_changed(peer_id, local_player_id, profile_index, career_index)
	local status = Managers.party:get_player_status(peer_id, local_player_id)
	status.game_mode_data = self:_restore_player_game_mode_data(peer_id, local_player_id, status.profile_index, status.career_index)
end

local working_persistent_buff_array = { }
function DeusSpawning:_update_player_status(t, dt, occupied_slots)
...
function DeusSpawning:_apply_initial_buffs(player)
	local player_unit = player.player_unit
...
function DeusSpawning:_update_spawning(dt, t, occupied_slots)
...
function DeusSpawning:add_delayed_client(peer_id, local_player_id)
...
function DeusSpawning:remove_delayed_client(peer_id, local_player_id)
...
function DeusSpawning:_update_joining_clients(dt, t)
...
function DeusSpawning:_add_client_to_party(peer_id, local_player_id)
...
function DeusSpawning:_spawn_player(status)
	local data = status.game_mode_data
...
function DeusSpawning:_spawn_bot(status)
	local data = status.game_mode_data
...
function DeusSpawning:_find_spawn_point(status)
...
function DeusSpawning:force_update_spawn_positions(safe_position, safe_rotation)
	local party = self._side.party
	local occupied_slots = party.occupied_slots
...
function DeusSpawning:set_respawning_enabled(enabled)
	fassert(self._respawns_enabled ~= enabled, "Respawns already enabled=%s", tostring(enabled))
	self._respawns_enabled = enabled
end

function DeusSpawning:set_spawning_disabled(disabled)
	self._spawning = not disabled
end

function DeusSpawning:add_spawn_point(unit)
...
function DeusSpawning:get_spawn_point()
...
function DeusSpawning:respawn_unit_spawned(unit)
	self._respawn_handler:respawn_unit_spawned(unit)
end

function DeusSpawning:respawn_gate_unit_spawned(unit)
	self._respawn_handler:respawn_gate_unit_spawned(unit)
end

function DeusSpawning:remove_respawn_units_due_to_crossroads(removed_path_distances, total_main_path_length)
	self._respawn_handler:remove_respawn_units_due_to_crossroads(removed_path_distances, total_main_path_length)
end

function DeusSpawning:recalc_respawner_dist_due_to_crossroads()
	self._respawn_handler:recalc_respawner_dist_due_to_crossroads()
end

function DeusSpawning:disable_status_updates()
	self._status_updates_active = false
end

function DeusSpawning:teleport_despawned_players(position)
	local party = self._side.party
	local occupied_slots = party.occupied_slots
	local player_manager = Managers.player
	for i = 1, #occupied_slots do
...
function DeusSpawning:force_respawn(peer_id, local_player_id)
	local status = Managers.party:get_player_status(peer_id, local_player_id)
	local data = status.game_mode_data
	data.spawn_state = "force_respawn"
end

function DeusSpawning:force_respawn_dead_players()
	local party = self._side.party
	self._respawn_handler:force_respawn_dead_players(party)
end

function DeusSpawning:set_override_respawn_group(respawn_group_name, active)
	self._respawn_handler:set_override_respawn_group(respawn_group_name, active)
end

function DeusSpawning:set_respawn_group_enabled(respawn_group_name, enabled)
	self._respawn_handler:set_respawn_group_enabled(respawn_group_name, enabled)
end

function DeusSpawning:set_respawn_gate_enabled(respawn_gate_unit, enabled)
	self._respawn_handler:set_respawn_gate_enabled(respawn_gate_unit, enabled)
end

function DeusSpawning:get_active_respawn_units()
	return self._respawn_handler:get_active_respawn_units()
end

function DeusSpawning:get_respawn_handler()
