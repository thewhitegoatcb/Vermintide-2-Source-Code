...
local extensions = { "PlayGoTutorialExtension" }



PlayGoTutorialSystem = class(PlayGoTutorialSystem, ExtensionSystemBase)

function PlayGoTutorialSystem:init(entity_system_creation_context, system_name)
...
function PlayGoTutorialSystem:destroy()
...
function PlayGoTutorialSystem:active()
	return self._active
end

local dummy_input = { }
function PlayGoTutorialSystem:on_add_extension(world, unit, extension_name, ...)
	...
	self._saved_definition.position = { 0, -440, 1 }
...
function PlayGoTutorialSystem:trigger_pause_event(pause_event, look_position)
	self._current_pause_event = nil
...
function PlayGoTutorialSystem:add_animation_hook(animation_hook)
	self._animation_hooks [#self._animation_hooks + 1] = animation_hook

	self:_add_next_animation_hook()
end

function PlayGoTutorialSystem:_add_next_animation_hook()
	...
	if animation_hook then
		...
		function Unit.animation_event(unit, animation_event)
			local breed = Unit.get_data(unit, "breed")
			if breed and breed.name == animation_hook.breed and
			not animation_hook.activated and
			table.find(animation_hook.animations, animation_event) and
			animation_hook.check_prerequisites() then
...
function PlayGoTutorialSystem:on_remove_extension(unit, extension_name)
...
function PlayGoTutorialSystem:set_bot_ready_for_assisted_respawn(unit, respawn_unit)
	local status_extension = ScriptUnit.extension(unit, "status_system")
	status_extension:set_ready_for_assisted_respawn(true, respawn_unit)
end

function PlayGoTutorialSystem:remove_player_ammo()
...
function PlayGoTutorialSystem:check_player_ammo()
	local player = Managers.player:local_player()
	local inventory_extension = ScriptUnit.extension(player.player_unit, "inventory_system")
	local current, _ = inventory_extension:current_ammo_status("slot_ranged")
	if current > 0 then
...
function PlayGoTutorialSystem:enable_player_ammo_refill()
	self.player_ammo_refill = true
end


function PlayGoTutorialSystem:give_player_potion_from_bot(player_unit, bot_unit)
...
function PlayGoTutorialSystem:update(context, t)
...
function PlayGoTutorialSystem:_update_animation_hooks(player, t)
	if self._current_animation_hook and self._current_animation_hook.activated and
	self._current_animation_hook.update(self._current_animation_hook, t) then
		self._current_animation_hook.on_exit(self._current_animation_hook)
...
function PlayGoTutorialSystem:_update_pause_events(t)
	if self._current_pause_event and
	self._current_pause_event.update(self._current_pause_event, t) then
		self._current_pause_event.on_exit(self._current_pause_event)
		self._current_pause_event = nil
	end
...
function PlayGoTutorialSystem:_update_player_health(player)
...
function PlayGoTutorialSystem:_capture_wield_switch(player)
	local player_unit = player.player_unit
...
function PlayGoTutorialSystem:_capture_attacks(player)
	local player_unit = player.player_unit
	local inventory_extension = ScriptUnit.extension(player_unit, "inventory_system")
...
function PlayGoTutorialSystem:_update_player_ammo(player)
...
function PlayGoTutorialSystem:_update_ai_units()
...
function PlayGoTutorialSystem:clear_hooks()
	if self._unit_animation_event then
		Unit.animation_event = self._unit_animation_event
		self._unit_animation_event = nil
	end
...
function PlayGoTutorialSystem:_load_profile_packages()
	local profiles_to_load = { 3, 4 }
	local career_index = 1
	local is_first_person = { ["4"] = true, ["3"] = false }
		...
		for i = 1, num_slots do repeat
				...
				if slot_category == "weapon" then
					...
					if ammo_unit_name then
						...
						if not item_units.ammo_unit_3p then local actions = ammo_unit_name .. "_3p" end profile_packages [actions] = true
					end
				...
				elseif slot_category == "attachment" then
					...
					error("InventoryPackageSynchronizerClient unknown slot_category: " .. slot_category) end until true
		end
...
function PlayGoTutorialSystem:_unload_profile_packages()
	local profile_packages = self._profile_packages
	for package_name, _ in pairs(profile_packages) do
...
function PlayGoTutorialSystem:register_dodge(dodge_direction)
...
function PlayGoTutorialSystem:register_push(hit_unit)
	if self._tutorial_started and
	HEALTH_ALIVE [hit_unit] then
		Unit.flow_event(self._tutorial_unit, "lua_pushed_enemy")
	end

end

function PlayGoTutorialSystem:register_block()
	if self._tutorial_started then
		Unit.flow_event(self._tutorial_unit, "lua_blocked_attack")
	end
end

function PlayGoTutorialSystem:register_killing_blow(damage_type, attacker)
	if self._tutorial_started then
...
function PlayGoTutorialSystem:register_unit(spawner_unit, ai_unit)
	if not self._tutorial_started then
		return
	end
	local data = { }
...
function PlayGoTutorialSystem:teleport_unit(unit, position, rotation)
...
function PlayGoTutorialSystem:enable_bot_loot(enable)
	self._bot_loot_enabled = enable
end

function PlayGoTutorialSystem:bot_loot_enabled()
	return self._bot_loot_enabled
end

function PlayGoTutorialSystem:set_bot_portrait_enabled(bot_display_name)
	self._bot_portraits_enabled [bot_display_name] = true
end

function PlayGoTutorialSystem:bot_portrait_enabled(player)
