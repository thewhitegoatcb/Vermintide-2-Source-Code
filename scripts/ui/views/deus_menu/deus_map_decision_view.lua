...
local VISIBILITY_CONFIG = {
	[DeusMapVisibilitySettings.WEAK_FOG_LEVEL] = { conflict_settings = true, shop = true, terror_event_power_up = true, theme = true, minor_modifier = true, level = true },
	...
	[DeusMapVisibilitySettings.WEAK_FOG_LEVEL + 1] = { conflict_settings = false, shop = true, terror_event_power_up = true, theme = true, minor_modifier = true, level = true },
	...
	[DeusMapVisibilitySettings.WEAK_FOG_LEVEL + 2] = { conflict_settings = false, shop = true, terror_event_power_up = false, theme = true, minor_modifier = false, level = false },
	...
	[DeusMapVisibilitySettings.WEAK_FOG_LEVEL + 3] = { conflict_settings = false, shop = false, terror_event_power_up = false, theme = false, minor_modifier = false, level = false } }
...
local SOUND_EVENTS = { ingame_final_node_selected = "hud_morris_world_map_level_chosen", token_move = "hud_morris_world_map_token_move", node_hover = "hud_morris_world_map_hover", node_pressed = "hud_morris_world_map_chose_level", shrine_final_node_selected = "hud_morris_map_shrine_open" }
...
local states = { TWITCH_STARTING = "TWITCH_STARTING", VOTING = "VOTING", FINISHED = "FINISHED", WAITING = "WAITING", VOTING_FINISHING = "VOTING_FINISHING", TWITCH_WAITING = "TWITCH_WAITING", FINISHING = "FINISHING", STARTING = "STARTING" }
...
local shared_state_spec = {
	server = {
		map_state = { default_value = "", type = "string",


			composite_keys = { } },

		final_node_selected = { default_value = "", type = "string",
	...
	peer = {
		ready = { default_value = false, type = "boolean",

			composite_keys = { } },


		vote = { default_value = "", type = "string",
...
function DeusMapDecisionView:init(context)
...
function DeusMapDecisionView:_start()
	self._state = states.IDLE

	local current_node_key = self._deus_run_controller:get_current_node_key()
	self._shared_state = SharedState:new(
	"deus_map_" .. self._deus_run_controller:get_run_id() .. "_" .. current_node_key,
	shared_state_spec,
	self._is_server,
	self._network_server,
	self._server_peer_id,
	...
	if self._is_server then
		...
		start_dialogue_event = table.contains(next_node_types, "shop") and
		"deus_before_shrine_tutorial" or
...
function DeusMapDecisionView:register_rpcs(network_event_delegate, network_transmit)
	DeusMapDecisionView.super.register_rpcs(self, network_event_delegate, network_transmit)
	self._network_event_delegate = network_event_delegate
end

function DeusMapDecisionView:unregister_rpcs()
	DeusMapDecisionView.super.unregister_rpcs(self)

	self._network_event_delegate = nil
end

function DeusMapDecisionView:destroy()
...
function DeusMapDecisionView:_update(dt, t)
...
function DeusMapDecisionView:_get_rpcs()
	return nil
end

function DeusMapDecisionView:_node_pressed(node_key)
...
function DeusMapDecisionView:_node_hovered(node_key)
	self:_enable_hover(node_key)
end

function DeusMapDecisionView:_node_unhovered()
	self:_disable_hover()
end
...
function DeusMapDecisionView:_check_transition(state)
	if state == states.TWITCH_STARTING then
		if self._starting_countdown == 0 or self:_are_all_peers_ready() then
			do return states.TWITCH_WAITING end
		end
	elseif state == states.STARTING then
		if self._starting_countdown == 0 or self:_are_all_peers_ready() then
			do return states.WAITING end
		end
	elseif state == states.TWITCH_WAITING then
		if self:_get_twitch_vote() then
			self:_handle_twitch_waiting_end()
			do return states.FINISHING end
		end
	elseif state == states.WAITING then
		if self:_did_someone_vote() then
			do return states.VOTING end
		end
	elseif state == states.VOTING then
	...
	elseif state == states.FINISHING and
	self._final_countdown == 0 then
...
function DeusMapDecisionView:_enable_hover(node_key)
	...
	self._scene:get_screen_pos_of_node(node_key),
	node.level_type,
	reveal_base_level and node.base_level or nil,
	reveal_theme and node.theme or nil,
	reveal_minor_modifier and node.minor_modifier_group or nil,
	reveal_conflict_settings and node.conflict_settings or nil,
	reveal_terror_event_power_up and node.terror_event_power_up or nil,
	reveal_terror_event_power_up and node.grant_random_power_up_count or nil,
	reveal_terror_event_power_up and node.terror_event_power_up_rarity or nil,
	self._shared_state:get_own(self._shared_state:get_key("vote")) == node_key,
	table.contains(current_node.next, node_key),
	profile_index,
...
function DeusMapDecisionView:_disable_hover()
	self._ui:disable_hover_text()
	self._scene:unhover_node(self._hovered_node)
	self._hovered_node = nil
end

function DeusMapDecisionView:_on_enter_starting(dt, t)
	self._ui:set_general_info(
	Localize("deus_map_info_waiting_title"),
	Localize("deus_map_info_waiting_desc"))

	self._starting_countdown = START_COUNTDOWN
end

function DeusMapDecisionView:_update_during_starting(dt, t)
	self._starting_countdown = math.max(0, self._starting_countdown - dt)
	self._ui:update_timer(self._starting_countdown)
end

function DeusMapDecisionView:_on_enter_waiting(dt, t)
	self._ui:set_general_info(
	Localize("deus_map_info_voting_title"),
	Localize("deus_map_info_voting_desc"))
...
function DeusMapDecisionView:_update_during_waiting(dt, t)

	return end

function DeusMapDecisionView:_on_enter_voting(dt, t)
	local current_node = self._deus_run_controller:get_current_node()
...
function DeusMapDecisionView:_update_during_voting(dt, t)
	self._voting_countdown = math.max(0, self._voting_countdown - dt)
	self._ui:update_timer(self._voting_countdown)
end

function DeusMapDecisionView:_on_enter_voting_finishing(dt, t)
	local current_node = self._deus_run_controller:get_current_node()
...
function DeusMapDecisionView:_update_during_voting_finishing(dt, t)
	self._voting_countdown = math.max(0, self._voting_countdown - dt)
	self._ui:update_timer(self._voting_countdown)
end

function DeusMapDecisionView:_on_enter_finishing(dt, t)
	...
	self._ui:set_general_info(
	Localize("deus_map_info_finishing_title"),
	string.format(
	Localize("deus_map_info_finishing_desc"),
	Localize(final_node.base_level .. "_" .. "title")))
...
function DeusMapDecisionView:_on_enter_finished(dt, t)
...
function DeusMapDecisionView:_finish()
...
function DeusMapDecisionView:_update_during_finishing(dt, t)
...
function DeusMapDecisionView:_update_player_state()
	local player_data = { }
	...
	local profile_indexes_missing = { true, true, true, true, true }
...
function DeusMapDecisionView:_handle_voting_end()
...
function DeusMapDecisionView:_handle_twitch_waiting_end()
...
function DeusMapDecisionView:_are_all_peers_ready()
...
function DeusMapDecisionView:_did_someone_vote()
	for _, peer_id in ipairs(self._deus_run_controller:get_peers()) do
		local vote = self._shared_state:get_peer(peer_id, self._shared_state:get_key("vote"))
...
function DeusMapDecisionView:_did_everyone_vote()
	for _, peer_id in ipairs(self._deus_run_controller:get_peers()) do
		local vote = self._shared_state:get_peer(peer_id, self._shared_state:get_key("vote")) or ""
...
function DeusMapDecisionView:_play_networked_2d_sound(event)
	local audio_system = Managers.state.entity:system("audio_system")
	audio_system:play_2d_audio_event(event)
end

function DeusMapDecisionView:on_ingame_menu_opened()
	Managers.input:disable_gamepad_cursor()
end

function DeusMapDecisionView:on_ingame_menu_closed()
