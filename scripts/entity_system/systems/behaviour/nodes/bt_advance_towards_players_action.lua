...
function BTAdvanceTowardsPlayersAction:init(...)
	BTAdvanceTowardsPlayersAction.super.init(self, ...)
end

local EVALUATE_TIME = 1

function BTAdvanceTowardsPlayersAction:enter(unit, blackboard, t)
...
function BTAdvanceTowardsPlayersAction:leave(unit, blackboard, t, reason, destroy)
...
function BTAdvanceTowardsPlayersAction:run(unit, blackboard, t, dt)
...
function BTAdvanceTowardsPlayersAction:_calculate_trajectory_to_target(unit, world, blackboard, action)
	...
	if hit then
		blackboard.throw_globe_data = blackboard.throw_globe_data or { throw_pos = Vector3Box(), target_direction = Vector3Box() }
...
function BTAdvanceTowardsPlayersAction:has_valid_target(target_unit, blackboard)
	local side = blackboard.side
	return side.VALID_ENEMY_TARGETS_PLAYERS_AND_BOTS [target_unit]
end

function BTAdvanceTowardsPlayersAction:want_to_throw(unit, blackboard, t)
	local action = blackboard.action
...
function BTAdvanceTowardsPlayersAction:can_throw(unit, blackboard, t)
	...
	World.get_data(blackboard.world, "physics_world"),
	start_pos,
	direction,
	length,
	"closest",
...
function BTAdvanceTowardsPlayersAction:get_new_goal(unit, blackboard)
	local action = blackboard.action
...
function BTAdvanceTowardsPlayersAction:start_idle_animation(unit, blackboard)
	Managers.state.network:anim_event(unit, "idle")
	blackboard.move_state = "idle"
end

function BTAdvanceTowardsPlayersAction:start_move_animation(unit, blackboard)
