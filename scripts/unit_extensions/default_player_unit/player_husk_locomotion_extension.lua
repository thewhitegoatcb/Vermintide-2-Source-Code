...
function PlayerHuskLocomotionExtension:init(extension_init_context, unit, extension_init_data)
...
function PlayerHuskLocomotionExtension:average_velocity()
	return GameSession.game_object_field(self.game, self.id, "average_velocity")
end

function PlayerHuskLocomotionExtension:small_sample_size_average_velocity()
	return GameSession.game_object_field(self.game, self.id, "small_sample_size_average_velocity")
end

function PlayerHuskLocomotionExtension:get_script_driven_gravity_scale()
	return 1
end
...
function PlayerHuskLocomotionExtension:add_external_velocity(velocity, upper_limit)
...
function PlayerHuskLocomotionExtension:set_forced_velocity(velocity_forced)
...
function PlayerHuskLocomotionExtension:set_disabled(disabled, run_func, master_unit)
...
function PlayerHuskLocomotionExtension:update(unit, input, dt, context, t)
...
function PlayerHuskLocomotionExtension:last_position_on_navmesh()
	assert(self.is_server, "last position on nav mesh is only saved on server")
	return self._latest_position_on_navmesh:unbox()
end

function PlayerHuskLocomotionExtension:_update_last_position_on_navmesh()
	if self.is_server then
...
function PlayerHuskLocomotionExtension:update_movement(dt, unit, movement_state)
...
function PlayerHuskLocomotionExtension:update_ladder_animation_position(ladder_unit)
	local unit = self.unit
...
function PlayerHuskLocomotionExtension:_extrapolation_movement(unit, dt, old_pos, new_pos, new_rot, movement_state, velocity, linked_movement)
...
function PlayerHuskLocomotionExtension:_update_speed_variable(dt)
...
function PlayerHuskLocomotionExtension:_calculate_move_speed_var_from_mps(move_speed)
	local speed_var = nil
...
function PlayerHuskLocomotionExtension:rpc_animation_set_variable(index, variable)
	Unit.animation_set_variable(self.unit, index, variable)
end

function PlayerHuskLocomotionExtension:hot_join_sync(sender)
...
function PlayerHuskLocomotionExtension:current_rotation()
	return self._current_rotation:unbox()
end

local ALLOWED_MOVER_MOVE_DISTANCE = 1
function PlayerHuskLocomotionExtension:move_to_non_intersecting_position()
	local unit = self.unit
	local mover = Unit.mover(unit)
...
function PlayerHuskLocomotionExtension:teleport_to(pos, rot)
	local unit = self.unit
	local mover = Unit.mover(unit)
	Mover.set_position(mover, pos)
	Unit.set_local_position(unit, 0, pos)
