...
function ConsoleFriendsView:init(ingame_ui_context)
...
function ConsoleFriendsView:on_enter()
...
function ConsoleFriendsView:_join_game()
		...
		if friend_widget then
			...
			if room_id then
				...
				if not self._is_server or not self._is_in_inn then
					...
					Managers.matchmaking:request_join_lobby(lobby_data, { friend_join = true })
				end
			end
		end
	end
...
function ConsoleFriendsView:_refresh_friends()
...
function ConsoleFriendsView:cb_friends_collected(friend_data)
...
function ConsoleFriendsView:on_exit()
...
function ConsoleFriendsView:exit()
...
function ConsoleFriendsView:transitioning()
...
function ConsoleFriendsView:_setup_input(ingame_ui_context)
...
function ConsoleFriendsView:_create_ui_elements()
	...
	friends_bg.style.background.color = { 255, 128, 128, 128 }
...
function ConsoleFriendsView:_sorted_players()
...
function ConsoleFriendsView:_setup_party_entries()
	self._party_entries = { }
...
function ConsoleFriendsView:update(dt, t)
	if DO_RELOAD then
		self:_create_ui_elements()
	end
...
function ConsoleFriendsView:_update_input_descriptions(dt, t)
	local friends_widget = self._friend_list_widgets [self._current_friend_index]
	local enable_profile_button = false local enable_invite_button = false
...
function ConsoleFriendsView:_handle_refresh(dt, t)
	if IS_PS4 then
...
function ConsoleFriendsView:_animate_default_buttons(dt, t)
...
function ConsoleFriendsView:_handle_input(dt, t)
	if self._exiting then
		return
	end
...
function ConsoleFriendsView:_update_animations(dt, t)
...
function ConsoleFriendsView:_draw(dt, t)
...
function ConsoleFriendsView:_handle_popup()
...
function ConsoleFriendsView:destroy()

	return end

function ConsoleFriendsView:input_service()
	return self._input_manager:get_service("console_friends_view")
end

function ConsoleFriendsView:_open_profile(widget)
...
function ConsoleFriendsView:_send_invite(widget, t)
	local content = widget.content
...
function ConsoleFriendsView:cleanup_popups()
	if self._popup_id then
		Managers.popup:cancel_popup(self._popup_id)
		self._popup_id = nil
	end
