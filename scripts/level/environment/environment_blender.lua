...
function EnvironmentBlender:init(world, viewport)
	self.world = world
	...
	local blend_data = { volume_name = "world", environment = "default", always_inside = true, override_sun_snap = false, particle_light_intensity = 1,
...
function EnvironmentBlender:event_register_environment_volume(volume_name, environment_name, priority, blend_time, override_sun_snap, particle_light_intensity, sphere_pos, sphere_radius, specified_id)



	local blend_data = { always_inside = false,
...
function EnvironmentBlender:event_unregister_environment_volume(id)
	self.environment_handler:remove_blend(id)
end

function EnvironmentBlender:update(dt, t)
...
function EnvironmentBlender:update_shading_settings()
...
function EnvironmentBlender:destroy()
...
local debug_colors = { { 255, 100, 100, 200 },
	{ 255, 100, 200, 100 },
	{ 255, 200, 100, 100 },
	{ 255, 200, 200, 100 } }


function EnvironmentBlender:debug_color()
	return table.remove(debug_colors)
end

function EnvironmentBlender:debug_draw(shading_settings)
