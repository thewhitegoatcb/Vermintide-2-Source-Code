...
function StateDedicatedServer:on_enter(params)
...
function StateDedicatedServer:_setup_garbage_collection()
...
function StateDedicatedServer:_init_input()
...
function StateDedicatedServer:_unload_packages()
	local package_manager = Managers.package
	for i, name in ipairs(StateDedicatedServer.packages_to_load) do
...
function StateDedicatedServer:_packages_loaded()
	local package_manager = Managers.package
...
function StateDedicatedServer:update(dt, t)
...
function StateDedicatedServer:setup_network_server(game_server)
	...
	local matchmaking_params = { is_server = true,
		network_transmit = self._network_transmit,
		lobby = game_server,
...
function StateDedicatedServer:setup_chat_manager(game_server)
	local peer_id = Network.peer_id()

	local network_context = { is_server = true,
	...
	local function member_func() return game_server:members():get_members() end
	Managers.chat:register_channel(1, member_func)

end

function StateDedicatedServer:setup_enemy_package_loader(game_server)
	local peer_id = Network.peer_id()
...
function StateDedicatedServer:setup_global_managers(game_server)
	local peer_id = Network.peer_id()
...
function StateDedicatedServer:_update_network(dt)
	if self._network_server then
		self._network_server:update(dt)
	end
end

function StateDedicatedServer:_update_wanted_state()
...
function StateDedicatedServer:_destroy_network()
	if self._network_server then
...
function StateDedicatedServer:on_exit(application_shutdown)
	if self._network_server then
