...
function NavigationGroupManager:init(using_editor)
...
function NavigationGroupManager:setup(world, nav_world)
	self._world = world
...
function NavigationGroupManager:form_groups(radius, finish_point, optional_level_name)
...
function NavigationGroupManager:form_groups_start(radius, finish_point, optional_level_name)
...
function NavigationGroupManager:form_groups_update()
...
function NavigationGroupManager:form_groups_end()
	local time1 = os.clock()
...
function NavigationGroupManager:_breadth_first_fill_main_path_index(main_path_index, starting_nav_group)
	local added_to_queue = { starting_nav_group = true }
	local queue = { starting_nav_group }
...
function NavigationGroupManager:assign_main_path_indexes(main_paths)
	local time1 = os.clock()
...
function NavigationGroupManager:assign_group(group, in_group_queue, rejected_queue)
...
function NavigationGroupManager:next_poly_in_queue(in_group_queue, rejected_queue)
	...
	if not poly_is_valid then
		...
		repeat
			...
			if poly_is_valid == nil then
				...
				return false, false end until poly_is_valid
	end
...
function NavigationGroupManager:poly_is_valid(poly)
...
function NavigationGroupManager:unmark_polys(rejected_queue)
...
function NavigationGroupManager:add_neighbours_to_queue(poly, group, in_group_queue)
...
function NavigationGroupManager:refine_groups()
...
function NavigationGroupManager:make_sure_group_centers_are_on_mesh()
	for group, _ in pairs(self._navigation_groups) do
...
function NavigationGroupManager:find_smallest_neighbour_group(group)
	local group_neighbours = group:get_group_neighbours()
...
function NavigationGroupManager:calc_distances_from_finish_for_all(in_group_queue)
	local a, b, c = Script.temp_count()
	...
	for i, poly in ipairs(in_group_queue) do repeat
			Script.set_temp_count(a, b, c)
			...
			group:set_distance_from_finish(distance_from_finish) until true
	end

end

function NavigationGroupManager:get_neighbours(poly)
	local neighbours = { GwNavTraversal.get_neighboring_triangles(poly) }
	return neighbours
end

function NavigationGroupManager:create_group(world, poly_hash, poly)
...
function NavigationGroupManager:join_group(poly, poly_hash, group)
	local poly_area = self:calc_polygon_area(poly)
	local poly_center = self:calc_polygon_center(poly)
...
function NavigationGroupManager:in_range(poly, group)
	local poly_center = self:calc_polygon_center(poly)
...
function NavigationGroupManager:calc_distance_from_finish(group, first_group)
...
function NavigationGroupManager:calc_polygon_center(poly)
...
function NavigationGroupManager:calc_polygon_area(poly)
...
function NavigationGroupManager:get_polygon_sides(poly)
	local p1, p2, p3 = GwNavTraversal.get_triangle_vertices(self.nav_world, poly)
...
function NavigationGroupManager:destroy(world)
...
function NavigationGroupManager:get_group_from_position(position)
...
function NavigationGroupManager:get_polygon_group(triangle, dont_clear)
...
function NavigationGroupManager:draw_tri(triangle, h, col)
	h = h or 0.1
	local p1, p2, p3 = GwNavTraversal.get_triangle_vertices(self.nav_world, triangle)
	p1 = p1 + Vector3(0, 0, h)
	p2 = p2 + Vector3(0, 0, h)
...
function NavigationGroupManager:breadth_first_search_neighbours(root_triangle)
	...
	while b_start <= b_last do
		...
		local neighbours = { GwNavTraversal.get_neighboring_triangles(node_tri) }
...
function NavigationGroupManager:get_group_polygons_centers(list)
	for group, _ in pairs(self._navigation_groups) do
		local a, b, c = Script.temp_count()
...
function NavigationGroupManager:print_groups(world, nav_world)
	local a, b, c = Script.temp_count()
	...
	if do_print_groups then
		...
		self._drawer = self._drawer or Managers.state.debug:drawer({ mode = "perm", name = "nav_group" })
...
function NavigationGroupManager:destroy_gui(world)
...
function NavigationGroupManager:a_star_cached(group1, group2)
	return LuaAStar.a_star_cached(self._navigation_groups, group1, group2)
end

function NavigationGroupManager:a_star_cached_between_positions(p1, p2)
...
function NavigationGroupManager:draw_group_path(path)
...
function NavigationGroupManager:draw_group_connections()
...
function NavigationGroupManager:knit_groups_with_ledges()
		...
		for i = 1, #smart_object_data do
			...
			if group1 then
				...
				if group2 and group1 ~= group2 then
					...
					if smart_object.data.is_bidirectional and
					not group2._group_neighbours [group1] then
...
function NavigationGroupManager:breadth_first_search_all_triangles(triangle)
	local time1 = os.clock()
	...
	local tri_lookup = { source_tri_hash = unique_id }

	while i < num_triangles do
		i = i + 1
		triangle = triangles [i]

		local neighbors = { get_neighboring_triangles(triangle) }
