...
function TwitchVoteUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function TwitchVoteUI:event_add_vote_ui(vote_key)
...
function TwitchVoteUI:event_finish_vote_ui(vote_key, winning_index)
	local vote_data = Managers.twitch:get_vote_data(vote_key)
	if not vote_data then
		return
	end
	...
	self._vote_result = { vote_key = vote_key, winning_index = winning_index, winning_template_name = winning_template_name, vote_template = vote_template }
...
function TwitchVoteUI:event_reset_vote_ui(vote_key)
	if vote_key then
...
function TwitchVoteUI:start_standard_vote(vote_template_a_name, vote_template_b_name, vote_inputs, vote_key)
...
function TwitchVoteUI:start_multiple_choice_vote(vote_template_name, vote_inputs, vote_key)
	...
	local vote_data = Managers.twitch:get_vote_data(vote_key)
	local vote = { vote_type = "multiple_choice",
		vote_template = table.clone(vote_template),
		inputs = vote_inputs or { "#a", "#b", "#c", "#d", "#e" },
		vote_key = vote_key,
...
function TwitchVoteUI:set_visible(visible)
	self._visible = visible
end

function TwitchVoteUI:_create_elements()
	local scenegraph_definition = definitions.scenegraph_definition
	self._ui_scenegraph = UISceneGraph.init_scenegraph(scenegraph_definition)

	self._widgets = { }

	self._vote_count = { 0, 0, 0, 0, 0 }
...
local customizer_data = { root_scenegraph_id = "pivot", label = "Twitch", registry_key = "twitch", drag_scenegraph_id = "pivot_dragger" }
...
function TwitchVoteUI:update(dt, t)
	...
	if DEBUG_VOTE_UI and
	self._active_vote then
		for i = 1, 5 do
...
function TwitchVoteUI:_update_transition(dt)
...
function TwitchVoteUI:show_ui(ui)
	self._next_ui = ui
	if self._ui then
		self._fade_out = true
	else
...
function TwitchVoteUI:hide_ui()
	self._fade_out = true
end

function TwitchVoteUI:_show_next_ui()
...
function TwitchVoteUI:_create_vote_icon(vote_index)
	...
	scenegraph_definition [base_name] = { parent = "vote_icon",
...
function TwitchVoteUI:cb_destroy_vote_icon(vote_icon_name)
	self._widgets [vote_icon_name] = nil
end

function TwitchVoteUI:_update_active_vote(dt, t)
	...
	self._vote_count = self._vote_count or { 0, 0, 0, 0, 0 }
	...
	self._active_vote.vote_percentages = self._active_vote.vote_percentages or { 0, 0, 0, 0, 0 }
	for i = 1, 5 do
...
function TwitchVoteUI:_draw(dt, t)
...
function TwitchVoteUI:_show_multiple_choice_vote()
	local active_vote = self._active_vote
	...
	for index, player in pairs(players) do repeat
			...
			if player_profile and index <= PlayerManager.MAX_PLAYERS then
				...
				if widget then
					...
					vote_widget.content.text = option_strings [profile_index] end end until true
	end
...
function TwitchVoteUI:_update_multiple_votes_ui(dt)
	local active_vote = self._active_vote
	if not active_vote then
		return
	end
...
function TwitchVoteUI:_show_multiple_choice_result()
...
function TwitchVoteUI:_update_result(dt)
...
function TwitchVoteUI:_show_standard_vote()
	local active_vote = self._active_vote
	if not active_vote then
		return
	end
...
function TwitchVoteUI:_update_standard_vote()
	local active_vote = self._active_vote
	if not active_vote then
		return
	end
...
function TwitchVoteUI:_show_standard_vote_result()
	self._fade_out = false
	local vote_result = self._vote_result
	assert(vote_result)
...
function TwitchVoteUI:_sorted_player_list()
	local human_and_bot_players = Managers.player:human_and_bot_players()
	local players = { }
	for peer_id, player in pairs(human_and_bot_players) do
...
function TwitchVoteUI:_play_winning_sfx(cost)
	if cost == nil then
		return
	end
	if cost <= 0 then
...
function TwitchVoteUI:_play_timer_sfx(time_left)
...
function TwitchVoteUI:_play_multiple_vote_start()
	WwiseWorld.trigger_event(self.wwise_world, "Play_twitch_vote_multiple_start")
end

function TwitchVoteUI:_play_standard_vote_start()
