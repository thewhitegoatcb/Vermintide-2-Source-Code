...
function HeroPreviewer:init(ingame_ui_context, unique_id, delayed_spawn)
...
function HeroPreviewer:activate(activate, world)
...
function HeroPreviewer:prepare_exit()
	self:clear_units()
end

function HeroPreviewer:on_exit()
...
function HeroPreviewer:update(dt, t)
	return end

function HeroPreviewer:post_update(dt)
...
function HeroPreviewer:_update_unit_mip_streaming()
...
function HeroPreviewer:_update_delayed_material_changes()
...
function HeroPreviewer:_request_mip_streaming_for_unit(unit)
	local requested_mip_streaming_units = self._requested_mip_streaming_units
...
function HeroPreviewer:_update_units_visibility(dt)
	if not self._activated then
		return
	end
	...
	if self.character_unit_hidden_after_spawn then
		...
		if Unit.has_animation_state_machine(self.mesh_unit) and
		Unit.has_animation_event(self.mesh_unit, "enable") then
...
function HeroPreviewer:_set_character_visibility(visible)
...
function HeroPreviewer:character_visible()
	return self.character_unit_visible and Unit.alive(self.character_unit)
end

function HeroPreviewer:play_character_animation(animation_event, force_play_animation)
	local character_unit = self.character_unit
...
function HeroPreviewer:request_spawn_hero_unit(profile_name, career_index, callback, optional_skin)
...
function HeroPreviewer:_handle_hero_spawn_request()
...
function HeroPreviewer:_load_hero_unit(profile_name, career_index, callback, optional_skin, optional_scale)
	if not self._delayed_spawn then
		self:_unload_all_packages()
	end
...
function HeroPreviewer:_poll_hero_package_loading()
...
function HeroPreviewer:_spawn_hero_unit(skin_data, optional_scale, career_index)
...
function HeroPreviewer:respawn_hero_unit(profile_name, career_index, callback)
	self:request_spawn_hero_unit(profile_name, career_index, callback, nil, nil)
end

function HeroPreviewer:get_equipped_item_info(slot)
...
function HeroPreviewer:spawn_all_props(prop_spawn_settings_list)
	for _, prop_spawn_settings in pairs(prop_spawn_settings_list) do
		self:spawn_prop(prop_spawn_settings)
	end
end
function HeroPreviewer:spawn_prop(prop_spawn_settings)
	self._props_data [#self._props_data + 1] = { visible = false, loaded = false,
...
function HeroPreviewer:equip_item(item_name, slot, backend_id, skin)
	...
	if item_slot_type == "melee" or item_slot_type == "ranged" then
		...
		if left_hand_unit then
			...
			spawn_data [#spawn_data + 1] = { left_hand = true,
		...
		if right_hand_unit then
			...
			spawn_data [#spawn_data + 1] = { right_hand = true,
				despawn_both_hands_units = despawn_both_hands_units,
	...
	if #package_names > 0 then
		...
		item_info_by_slot [item_slot_type] = { name = item_name, backend_id = backend_id, skin_name = skin, package_names = package_names, spawn_data = spawn_data }
		self:_load_packages(package_names)
	end
end

function HeroPreviewer:_poll_item_package_loading()
	local character_unit = self.character_unit
	if not Unit.alive(character_unit) then
		return
	end
...
function HeroPreviewer:_is_all_items_loaded()
	local item_info_by_slot = self._item_info_by_slot
...
function HeroPreviewer:_spawn_prop(data)
	local settings = data.settings
	local world = self.world
...
function HeroPreviewer:_spawn_item(item_name, spawn_data)
	local world = self.world
	local character_unit = self.character_unit
	local mesh_unit = self.mesh_unit
...
function HeroPreviewer:_spawn_item_unit(unit, item_slot_type, item_template, unit_attachment_node_linking, scene_graph_links, material_settings)
	local world = self.world
	local character_unit = self.character_unit
	...
	if item_slot_type == "melee" or item_slot_type == "ranged" then
		if self._wielded_slot_type == item_slot_type then
			...
			if not script_data.disable_third_person_weapon_animation_events then
				...
				if wield_anim then
					local wield_anim = get_wield_anim(nil, item_template.wield_anim_career_3p, self._current_career_name) or
					get_wield_anim(wield_anim, item_template.wield_anim_career, self._current_career_name)
...
function HeroPreviewer:_destroy_item_units_by_slot(slot_type)
	local world = self.world
...
function HeroPreviewer:wield_weapon_slot(slot_type)
...
function HeroPreviewer:item_name_by_slot_type(item_slot_type)
	local item_info = self._item_info_by_slot [item_slot_type]

	return item_info and item_info.name
end

function HeroPreviewer:wielded_slot_type()
	return self._wielded_slot_type
end

function HeroPreviewer:_reference_name()
...
function HeroPreviewer:_trigger_equip_events()
...
function HeroPreviewer:_load_packages(package_names)
	local reference_name = self:_reference_name()
...
function HeroPreviewer:_unload_all_packages()
...
function HeroPreviewer:_unload_all_prop_packages()
...
function HeroPreviewer:_unload_hero_packages()
	local data = self._hero_loading_package_data
	if not data then
		return
	end
...
function HeroPreviewer:_unload_all_items()
	local item_info_by_slot = self._item_info_by_slot
...
function HeroPreviewer:_unload_prop_packages(data)
	local package_manager = Managers.package
	local reference_name = self:_reference_name()
...
function HeroPreviewer:_unload_item_packages_by_slot(slot_type)
	local item_info_by_slot = self._item_info_by_slot
...
function HeroPreviewer:clear_units()
	table.clear(self._requested_mip_streaming_units)
...
function HeroPreviewer:set_hero_location(location)
	if location then
...
function HeroPreviewer:set_hero_rotation(angle)
	if angle then
...
function HeroPreviewer:set_hero_look_target(look_target)
	if look_target then
