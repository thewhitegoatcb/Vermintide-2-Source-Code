...
function PlayerCharacterStateInteracting:on_enter(unit, input, dt, context, t, previous_state, params)
...
function PlayerCharacterStateInteracting:on_exit(unit, input, dt, context, t, next_state)
...
function PlayerCharacterStateInteracting:update(unit, input, dt, context, t)
	local csm = self.csm
	...
	if self.activate_block then
		if not status_extension:is_blocking() and
		not LEVEL_EDITOR_TEST and Managers.state.network:game() then
			local game_object_id = Managers.state.unit_storage:go_id(unit)
			if self.is_server then
				Managers.state.network.network_transmit:send_rpc_clients("rpc_set_blocking", game_object_id, true)
			else
