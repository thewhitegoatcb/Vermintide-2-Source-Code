...
function BTRatlingGunnerApproachAction:leave(unit, blackboard, t, reason, destroy)
	if reason ~= "done" then
...
function BTRatlingGunnerApproachAction:run(unit, blackboard, t, dt)
...
function BTRatlingGunnerApproachAction:is_within_check_distance(unit, blackboard)
	local action = blackboard.action
...
function BTRatlingGunnerApproachAction:move_to(position, blackboard)
	local navigation_extension = blackboard.navigation_extension
	navigation_extension:move_to(position)
	blackboard.move_pos = Vector3Box(position)
end

function BTRatlingGunnerApproachAction:calculate_move_position(unit, blackboard)
	local action = blackboard.action
