...
function InputDebugger:setup(world, input_manager)
...
function InputDebugger:pre_update_device(input_device, device_data, dt)
	if script_data.input_debug_device_state then --[[ Nothing --]]
	end
end

local x_spacing = 100
function InputDebugger:post_update_device(input_device, device_data, dt)
	if not self.input_manager then return end
	if not script_data.debug_enabled then return end
	if script_data.input_debug_device_state then
		...
		local s_w = RESOLUTION_LOOKUP.res_w local s_h = RESOLUTION_LOOKUP.res_h
...
function InputDebugger:debug_input_filters()
	local blue_color = Color(128, 128, 255)
	local green_color = Color(128, 255, 128)
	local i = 0
	local s_w = RESOLUTION_LOOKUP.res_w local s_h = RESOLUTION_LOOKUP.res_h
...
function InputDebugger:update_input_service_data(input_service, t)
	local selected_color = Color(128, 128, 192) local normal_color = Color(255, 255, 255) local current_color = Color(192, 192, 64) local title_color = Color(64, 192, 64)
	...
	local s_w = RESOLUTION_LOOKUP.res_w local s_h = RESOLUTION_LOOKUP.res_h
	local top = s_h - 20 - font_size local x_pos = s_w / 2
	local i = 0
...
function InputDebugger:update_selected_device(input_service, x_pos, top, t)
	local selected_color = Color(128, 128, 192) local normal_color = Color(255, 255, 255) local current_color = Color(192, 192, 64) local title_color = Color(64, 192, 64)
	local s_w = RESOLUTION_LOOKUP.res_w local s_h = RESOLUTION_LOOKUP.res_h
	local current_selection = self.current_selection
	...
	elseif selected_map_type == "axis" then --[[ Nothing --]]
	end

end

function InputDebugger:update_input_modify_type(input_service, t, x_pos)
	local selected_color = Color(128, 128, 192) local normal_color = Color(255, 255, 255) local current_color = Color(192, 192, 64) local title_color = Color(64, 192, 64)
	local s_w = RESOLUTION_LOOKUP.res_w local s_h = RESOLUTION_LOOKUP.res_h
...
function InputDebugger:update_selected_keymap_edit(input_service, dt, t, x_pos, current_input_service_data)
	local selected_color = Color(128, 128, 192) local normal_color = Color(255, 255, 255) local current_color = Color(192, 192, 64) local title_color = Color(64, 192, 64)
	local s_w = RESOLUTION_LOOKUP.res_w local s_h = RESOLUTION_LOOKUP.res_h
	local current_selection = self.current_selection
	...
	if added_keymap then
		...
		if self.current_keybinds then
			...
			for i = 1, #current_keybinds / 3 do
				...
				render_text(string.format("%s:%s[%d] (%s)",
				device_type, button_name, current_keybinds [i * 3 - 1], current_keybinds [i * 3]), Vector3(x_pos, top, 900), selected_color)
				top = top - font_size
			end
		...
		if not edit_mode then
			local index = self.edit_index or 1 local mode = self.edit_mode
		...
		if edit_mode == "device" then
			...
			for device_type, device_list in pairs(current_input_service_data.mapped_devices) do
				if #device_list > 0 then num_device_types = num_device_types + 1
				end end
			for device_type, device_list in pairs(current_input_service_data.mapped_devices) do
	...
	for j, keymap in ipairs(current_keymap_data.input_mappings) do
		for k = 1, keymap.n, 3 do
			...
			local text = string.format("[%d.%d] %s [%d] %s (%s)", j, math.ceil(k / 3), device_type, button_index,
			input_device.button_name(button_index), keymap [k + 2])
...
function InputDebugger:update_selected_filter_edit(input_service, dt, t, x_pos, current_input_service_data)
	local selected_color = Color(128, 128, 192) local normal_color = Color(255, 255, 255) local current_color = Color(192, 192, 64) local title_color = Color(64, 192, 64)
	local s_w = RESOLUTION_LOOKUP.res_w local s_h = RESOLUTION_LOOKUP.res_h
	local current_selection = self.current_selection
	...
	elseif selected_edit_type [2] == "number" then
		...
		local index = self.number_edit_index or 1 local mode = self.number_edit_mode or ""
		local keystrokes = Keyboard.keystrokes()
		for i = #keystrokes, 1, -1 do
			local k = keystrokes [i]
			if not tonumber(k) and k ~= "." or
			string.find(current_number_text, "%.") and k == "." then
...
function InputDebugger:finalize_update(input_services, dt, t)
	if not self.input_manager then return end
	if not script_data.debug_enabled then return end
	self.num_updated_devices = 0
	local gui = self.gui
	...
	elseif self.input_device_data and not IS_LINUX and
	DebugKeyHandler.key_pressed("f4", "Enable keymap editor.", "Input", "left ctrl") then
...
function InputDebugger:handle_edit_debug_keys(input_service, current_choice_name, choice_store, back_clear, max_nr_choices, t)
	local current_selection = self.current_selection
	if input_service:get("enter_key") then
