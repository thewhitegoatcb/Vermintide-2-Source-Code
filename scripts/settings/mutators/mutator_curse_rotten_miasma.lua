...
local function update_positions(rotten_miasma_safe_area, target_to_follow)
	...
	if not Unit.alive(rotten_miasma_safe_area) or
	not Unit.alive(target_to_follow) or not game_session then
...
local function create_default_target(position, rotation)
	local extension_init_data = {
		pickup_system = { has_physics = true, pickup_name = "deus_relic_01", spawn_type = "dropped" },
	...
	return Managers.state.unit_spawner:spawn_network_unit(
...
local function setup_rotten_miasma(buff_name)
	...
	target_to_follow =
	target_to_follow or create_default_target(position, rotation)
	...
	local rotten_miasma_safe_area = Managers.state.unit_spawner:spawn_network_unit(
...
return { description = "curse_rotten_miasma_desc", curse_package_name = "resource_packages/mutators/mutator_curse_rotten_miasma", display_name = "curse_rotten_miasma_name", icon = "deus_curse_nurgle_01",
