...
function ScriptBackendPlayFab:init()
	if HAS_STEAM then
		self._steam_ticket_id = Steam.retrieve_auth_session_ticket("AzurePlayFab")
	elseif GameSettingsDevelopment.use_offline_backend then --[[ Nothing --]]
	end

	self._metadata = Managers.backend:get_metadata()
end

function ScriptBackendPlayFab:update_state()

	return end

function ScriptBackendPlayFab:update_signin()
	if self._steam_ticket_id then
		...
		if ticket then
			local login_request = { TicketIsServiceSpecific = true, CreateAccount = true,
				TitleId = PlayFabClientApi.settings.titleId,
				SteamTicket = ticket,


				InfoRequestParameters = { GetUserReadOnlyData = true, GetTitleData = true, GetUserData = true } }
	...
	if signin_result then
		...
		return { reason = error_code, details = error_message }
	end
...
function ScriptBackendPlayFab:login_request_cb(result)
...
function ScriptBackendPlayFab:_update_telemetry_settings()
...
function ScriptBackendPlayFab:_validate_version()
	local request = { FunctionName = "validateVersion",
...
function ScriptBackendPlayFab:_validate_version_cb(result)
...
function ScriptBackendPlayFab:_set_up_initial_account()
	local initial_account_set_up = { FunctionName = "initialAccountSetUp",
...
function ScriptBackendPlayFab:initial_setup_request_cb(result)
...
function ScriptBackendPlayFab:_set_up_initial_inventory(start_index)
	local initial_account_data_set_up = { FunctionName = "initialInventorySetup",
...
function ScriptBackendPlayFab:initial_inventory_setup_request_cb(result)
...
function ScriptBackendPlayFab:authenticated()
...
function ScriptBackendPlayFab:get_signin_result()
	return self._signin_result
end

function ScriptBackendPlayFab:destroy()
	return end