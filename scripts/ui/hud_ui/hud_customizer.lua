...
function HudCustomizer.is_active()
	...
	return hud_customizer_enabled and
	Managers.chat.chat_gui.chat_focused and
	Keyboard.button(Keyboard.button_id("left alt")) > 0.5

end

function HudCustomizer.reset_button(ui_renderer)
	if not HudCustomizer.is_active() then return
	end

end

function HudCustomizer.run(ui_renderer, ui_scenegraph, customizer_data)
	if not HudCustomizer.is_active() then return end
	drag_hover = false
	...
	if not customizer_data.is_child then
		...
		if drag_active == customizer_data then
			...
			if not customizer_data.lock_x then offset [1] = cursor [1] - drag_base [1] end
			if not customizer_data.lock_y then offset [2] = cursor [2] - drag_base [2] end
			if Mouse.released(Mouse.button_id("left")) then
				drag_active = false
			end
		elseif not drag_active and
		not drag_hover and not customizer_data.is_child and is_hover then
		...
		if drag_active == customizer_data then color = COLOR_ACTIVE
		elseif drag_hover == customizer_data then color = COLOR_HOVER color [1] = 200 + 55 * math.sin(5 * Managers.time:time("ui"))
		end
...
function HudCustomizer.debug_temp(scenegraph, scenegraph_id)
