...
InputAux.input_map_types = { soft_button = "number", released = "boolean", axis = "Vector3", pressed = "boolean", held = "boolean" }
...
function InputAux.get_device_type(device)
...
function InputAux.remove_device(input_device_type, input_device)
...
function InputAux.add_device(input_device_type, input_device)
...
InputAux.combination_functions = {
	...
	avg = function (lhs, rhs) return ( lhs + rhs ) / 2 end,
	["or"] = function (lhs, rhs) return lhs or rhs end,
	["and"] = function (lhs, rhs) return lhs and rhs end }


InputAux.default_values_for_types = { boolean = false, number = 0 }
...
TestKeyMap = {
	...
	super_attack = {
		input_mappings = { { "keyboard", "left shift", "held", "mouse", "right", "pressed" } } },
	...
	weak_attack = {
		...
		input_mappings = { { "keyboard", "k", "held" } } },

	move_up = { combination_type = "max", input_mappings = { { "keyboard", "oem_comma (< ,)", "soft_button" },
			{ "mouse", "right", "soft_button" } } },





	move_down = { input_mappings = { { "keyboard", "o", "soft_button" } } },
	move_left = { input_mappings = { { "keyboard", "e", "soft_button" } } },
	move_right = { input_mappings = { { "keyboard", "a", "soft_button" } } } }


TestFilters = {
	move = { filter_type = "virtual_axis",
		input_mappings = { down = "move_down", up = "move_up", left = "move_left", right = "move_right" } } }