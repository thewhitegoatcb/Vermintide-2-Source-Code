...
local APP_IDS = { [1.0] = "795750", [2.0] = "552500" }




local SEARCH_TYPES = table.enum(
"slideshow",
"featured",
...
function ImguiStoreRotation:init()
	self._fp = nil
	self._save_file = nil
	...
	self._tabs = { "Feature Page Rotation", "Store Discounts", "Create Items and Bundles" }
	self._selected_tab = self._tabs [1]
	self._save_successful_discount = ""
	self._save_successful_featured = ""
end

function ImguiStoreRotation:_cleanup_slideshow()
	...
	for i = 1, #self._item_keys_list do
		...
		if item then if item.item_type ~= "bundle" and not is_dlc and not item.store_bundle_big_image then --[[ Nothing --]]
...
function ImguiStoreRotation:_filter_item_keys_list()
	local slideshow_items = { }
	local featured_items = { }
	...
	for i = 1, #self._item_keys_list do
		...
		local is_dlc = self:_is_a_dlc(key)
		local item = is_dlc and StoreDlcSettingsByName [key] or ItemMasterList [key]
		if item then if item.item_type ~= "deed" then
...
function ImguiStoreRotation:_load_saved_data()
	self._save_data = { }
...
function ImguiStoreRotation:_save_settings()
...
function ImguiStoreRotation:_setup_timpestamp_fields()
...
function ImguiStoreRotation:_setup_discount_begin_end_date()
...
function ImguiStoreRotation:_setup_layout_template()
...
function ImguiStoreRotation:is_persistent()
	return false
end

function ImguiStoreRotation:update()
	if DO_RELOAD then
		self:init()
		DO_RELOAD = false
	end
...
function ImguiStoreRotation:draw(is_open)
...
function ImguiStoreRotation:_featured_page_tab()
	self:_do_new_file_name()
	self:_do_timestamp_settings()
	Imgui.text("Timestamp: ") Imgui.same_line() Imgui.text_colored(self._timestamp, 44, 192, 133, 255)
...
function ImguiStoreRotation:_do_edit_buttons()
...
function ImguiStoreRotation:_do_item_selection()
...
function ImguiStoreRotation:_do_save_file_button()
	Imgui.dummy(2, 10)
...
function ImguiStoreRotation:_preview_changes()
...
function ImguiStoreRotation:_draw_layout_slideshow_preview()
	Imgui.dummy(2, 10)
...
function ImguiStoreRotation:_do_new_file_name()
...
function ImguiStoreRotation:_is_a_dlc(key)
	local is_dlc = table.find(self._dlc_list, key)
	return is_dlc
end

function ImguiStoreRotation:_get_layout_item(key)
	local layout_item = { }
	local is_dlc = self:_is_a_dlc(key)
	if is_dlc then
		layout_item.id = key
...
function ImguiStoreRotation:_get_slideshow_item(key)
...
function ImguiStoreRotation:_draw_item_selection()
	Imgui.text("Select Item")
	local index, results, text = ImguiX.combo_search(
	self._selected_item_index,
	self._item_search_results,
	self._item_search_text,
	self._searcheable_item_keys [self._search_type])
...
function ImguiStoreRotation:_draw_selcted_layout_items(items_list)
...
function ImguiStoreRotation:_draw_selcted_slideshow_items(items_list)
	for i = 1, #items_list do
		local item = items_list [i]
...
function ImguiStoreRotation:_do_timestamp_settings()
...
function ImguiStoreRotation:_calculate_timestamp(year, month, day, hour, minutes, seconds)
	...
	local timestamp = os.time({ day = day, month = month, year = year, hour = hour, min = minutes, sec = seconds, isdst = is_daylight_saving_time })
	self._timestamp_error = false
	return timestamp, true
end

function ImguiStoreRotation:_save_layout_items(layout_items)
...
function ImguiStoreRotation:_save_slideshow_items(slideshow_items)
...
function ImguiStoreRotation:_remove_last_added_item(table)
	table [#table] = nil
end

function ImguiStoreRotation:_do_clear_edit_buttons()
	Imgui.dummy(2, 10)
...
function ImguiStoreRotation:_save_to_file()
	local has_error = false
...
function ImguiStoreRotation:_calculate_discount(price_string, discount)
...
function ImguiStoreRotation:_make_item_def(key, item, discount)
	local steam_id = item.steam_itemdefid
	local original_price = SteamInventory.get_item_definition_property(steam_id, "price")
	return { item_quality = 2, type = "item", purchase_limit = 1, tradable = false, marketable = false, store_hidden = false, hidden = false,
...
function ImguiStoreRotation:_make_bundle_def(key, item, discount)
	local steam_id = item.steam_itemdefid
	local original_price = SteamInventory.get_item_definition_property(steam_id, "price")
	return { item_quality = 2, use_bundle_price = true, type = "bundle", tradable = false, marketable = false, hidden = false, store_hidden = false,
		itemdefid = item.steam_itemdefid,
...
function ImguiStoreRotation:_generate_discounted_item(key, item, discount)
...
function ImguiStoreRotation:_draw_dicount_begin_and_end_fields()
...
function ImguiStoreRotation:_store_rotation_discounts_tab()
...
function ImguiStoreRotation:_do_discount_rotation_file_name()
	Imgui.dummy(2, 3)
...
function ImguiStoreRotation:_do_edit_discounts_button()
	Imgui.dummy(2, 10)
	Imgui.text("Edit Discounts")
...
function ImguiStoreRotation:_on_search_type_changed(search_type)
	self._search_type = search_type
	self._item_search_results = table.clone(self._searcheable_item_keys [search_type])
end

function ImguiStoreRotation:_do_discount_item_selection(is_valid_end_date)
	if self._is_selecting_discount_item then
		Imgui.dummy(2, 5)
		Imgui.text_colored("OBS! PRESS ENTER", 255, 0, 0, 255) Imgui.same_line() Imgui.text("after inputting the discoiunt to apply it")
...
function ImguiStoreRotation:_handle_discount_page_errors(is_valid_end_date)
...
function ImguiStoreRotation:_do_clear_discount_edit_buttons()
	Imgui.dummy(2, 10)
...
function ImguiStoreRotation:_do_save_discounted_items_button()
	Imgui.dummy(2, 10)
...
function ImguiStoreRotation:_do_preview_discounted_items()
	Imgui.dummy(2, 10)
...
function ImguiStoreRotation:_get_from_to_discount_price(steam_itemdefid)
...
function ImguiStoreRotation:_draw_discounted_items(items_list)
	for i = 1, #items_list do
...
function ImguiStoreRotation:_get_rotation_items()
	local items = { }
...
function ImguiStoreRotation:_save_discounts_to_file()
	if not self._has_error_discount then
		local items = self:_get_rotation_items()
		local json_object = cjson.encode({ appid = self._appid, items = items })
...
function ImguiStoreRotation:_item_and_bundle_creator()
...
function ImguiStoreRotation:_handle_error_messages()
