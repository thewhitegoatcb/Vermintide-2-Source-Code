...
local RPCS = { "rpc_to_client_respawn_player", "rpc_respawn_confirmed" }




function RespawnHandler:init(profile_synchronizer, is_server)
...
function RespawnHandler:register_rpcs(network_event_delegate, network_transmit)
	network_event_delegate:register(self, unpack(RPCS))
	self._network_event_delegate = network_event_delegate
	self._network_transmit = network_transmit
end

function RespawnHandler:unregister_rpcs()
	self._network_event_delegate:unregister(self)
	self._network_event_delegate = nil
	self._network_transmit = nil
end

function RespawnHandler:set_respawn_unit_available(unit)
...
function RespawnHandler:find_respawn_data_from_unit(unit)
...
function RespawnHandler:set_override_respawn_group(group_id, enable)
...
function RespawnHandler:set_respawn_group_enabled(respawn_group_name, enabled)
...
function RespawnHandler:set_respawn_gate_enabled(respawn_gate_unit, enabled)
...
function RespawnHandler:respawn_gate_unit_spawned(unit)
	local distance_through_level = Unit.get_data(unit, "distance_through_level")
...
function RespawnHandler:remove_respawn_units_due_to_crossroads(removed_path_distances, total_main_path_length)
...
function RespawnHandler:recalc_respawner_dist_due_to_crossroads()
...
function RespawnHandler:update(dt, t)
...
function RespawnHandler:_check_all_synced()
...
function RespawnHandler:server_update(dt, t, slots)
...
function RespawnHandler:set_move_dead_players_to_next_respawn(value)
	self._move_players = value
end

function RespawnHandler:queue_force_move_dead_players()
	self._move_players = true
	self._force_move = true
end

local BOSS_TERROR_EVENT_LOOKUP = { boss_event_chaos_spawn = true, boss_event_storm_fiend = true, boss_event_chaos_troll = true, boss_event_minotaur = true, boss_event_rat_ogre = true }
...
function RespawnHandler:destroy()
	return end

function RespawnHandler:get_active_respawn_units()
	local respawn_units = self._respawn_units
...
function RespawnHandler:rpc_to_client_respawn_player(channel_id, local_player_id, profile_index, career_index, respawn_unit_id, slot_index, health_kit_id, potion_id, grenade_id, network_additional_items)
...
function RespawnHandler:_respawn_player(player, profile_index, career_index, respawn_unit, health_kit, potion, grenade, additional_items)
...
function RespawnHandler:rpc_respawn_confirmed(channel_id, local_player_id)
	local peer_id = CHANNEL_TO_PEER_ID [channel_id]
	local status = Managers.party:get_player_status(peer_id, local_player_id)
	status.game_mode_data.ready_for_respawn = false
end

function RespawnHandler:force_respawn_dead_players(party)
...
function RespawnHandler:_delayed_respawn_player(player, profile_index, career_index, respawn_unit, health_kit, potion, grenade, additional_items, status)
	local respawn_entry = {
		player, profile_index, career_index, respawn_unit, health_kit, potion, grenade, additional_items,
		status }
	table.insert(self._delayed_respawn_queue, respawn_entry)
end

function RespawnHandler:is_respawn_enabled(respawn_point)
	local active_overridden = self._active_overridden_units
...
function RespawnHandler:is_spawn_group_override_active(group_id)
	return self._active_overrides [group_id]
end

function RespawnHandler:get_boss_door_dist(main_paths, door_unit)
...
function RespawnHandler:get_next_boss_door_dist(main_path_info, ahead_unit_travel_dist)
...
function RespawnHandler:get_next_respawn_gate_dist(ahead_unit_travel_dist)
	local respawn_gate_units = self._respawn_gate_units
	for i = 1, self._respawn_gate_units_n do
		local gate = respawn_gate_units [i]
...
function RespawnHandler:get_main_path_segment_start(main_path_info)
...
function RespawnHandler:get_behind_unit_segment_start(main_path_info)
...
function RespawnHandler:get_respawn_dist_range(main_path_info, ahead_unit_travel_dist)
...
function RespawnHandler:_is_respawn_reachable(respawn_data)
...
function RespawnHandler:find_best_respawn_point(reserve_best, evaluate_all)
	local main_path_info = Managers.state.conflict.main_path_info
	...
	for i = 1, #respawn_units do
		...
		not best_respawn or best_score < score or best_score == score and (score < 3 and best_respawn.distance_through_level < respawn_dist and respawn_dist < preferred_spawn_travel_dist or
		score >= 3 and respawn_dist < best_respawn.distance_through_level) then
