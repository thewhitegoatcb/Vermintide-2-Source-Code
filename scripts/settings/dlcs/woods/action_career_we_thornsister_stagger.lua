...
function ActionCareerWEThornsisterStagger:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionCareerWEThornsisterStagger:client_owner_start_action(new_action, t, chain_action_data, power_level, action_init_data)
...
function ActionCareerWEThornsisterStagger:client_owner_post_update(dt, t, world, can_damage, current_time_in_action)
	return end

function ActionCareerWEThornsisterStagger:finish(reason)
	self.inventory_extension:wield_previous_non_level_slot()
end

function ActionCareerWEThornsisterStagger:_play_vo()
	local owner_unit = self.owner_unit
