...
function BTEnterHooks.crouch_on_enter(unit, blackboard, t)
...
function BTEnterHooks.upright_on_enter(unit, blackboard, t)
...
function BTEnterHooks.drop_items(unit, blackboard, t)
...
function BTEnterHooks.crouch_or_upright_on_enter(unit, blackboard, t)
...
function BTEnterHooks.rage_on_enter(unit, blackboard, t)
	blackboard.next_rage_time = t + 7
end

function BTEnterHooks.attack_grabbed_smash(unit, blackboard, t)
...
function BTEnterHooks.on_warlord_dual_wield(unit, blackboard, t)
...
function BTEnterHooks.on_warlord_halberd(unit, blackboard, t)
...
function BTEnterHooks.on_warlord_disable_blocking(unit, blackboard, t)
...
function BTEnterHooks.on_grey_seer_intro_enter(unit, blackboard, t)
...
function BTEnterHooks.grey_seer_death_sequence_teleport(unit, blackboard, t)
...
function BTEnterHooks.grey_seer_call_stormfiend_enter(unit, blackboard, t)
	local dialogue_input = ScriptUnit.extension_input(unit, "dialogue_system")
...
function BTEnterHooks.stormfiend_boss_charge_enter(unit, blackboard, t)
...
function BTEnterHooks.to_combat(unit, blackboard, t)
	Managers.state.network:anim_event(unit, "to_combat")
end

function BTEnterHooks.on_chaos_exalted_champion_intro_enter(unit, blackboard, t)
...
function BTEnterHooks.on_chaos_exalted_sorcerer_intro_enter(unit, blackboard, t)
	local level_analysis = Managers.state.conflict.level_analysis
...
function BTEnterHooks.on_skaven_warlord_intro_enter(unit, blackboard, t)
	local level_analysis = Managers.state.conflict.level_analysis
...
function BTEnterHooks.sorcerer_dummy_idle(unit, blackboard, t)
...
function BTEnterHooks.corruptor_enter(unit, blackboard, t)
	Managers.state.network:anim_event(unit, "to_corruptor")
end


function BTEnterHooks.grey_seer_stagger_enter(unit, blackboard, t)
...
local function setup_sorcerer_boss_spawning(unit, blackboard, data)
	...
	if spawner then
		...
		local dummy_action = { spawn_group = "default", use_fallback_spawners = true }
...
function BTEnterHooks.sorcerer_begin_defensive_mode(unit, blackboard, t)

	local data = { stay_still = true, end_time = math.huge }
...
function BTEnterHooks.dont_face_target_while_summoning(unit, blackboard, t)
	blackboard.face_target_while_summoning = false
end

function BTEnterHooks.sorcerer_re_enter_defensive_mode(unit, blackboard, t)
...
function BTEnterHooks.target_furthest_player_in_sight(unit, blackboard, t)
...
function BTEnterHooks.sorcerer_spawn_horde(unit, blackboard, t)
	local data = { stay_still = true, end_time = math.huge }
	setup_sorcerer_boss_spawning(unit, blackboard, data)
	blackboard.spawning_allies = data
end

function BTEnterHooks.sorcerer_defensive_seeking_bomb(unit, blackboard, t)
	local dialogue_input = ScriptUnit.extension_input(unit, "dialogue_system")
	local event_data = FrameTable.alloc_table()
	dialogue_input:trigger_networked_dialogue_event("ebh_insect_spell", event_data)
end

function BTEnterHooks.teleport_to_center(unit, blackboard, t)
...
function BTEnterHooks.quick_teleport(unit, blackboard, t)
	blackboard.quick_teleport = true
end

function BTEnterHooks.summoning_starts(unit, blackboard, t)
	blackboard.is_summoning = true
end

function BTEnterHooks.wave_summoning_starts(unit, blackboard, t)
	blackboard.is_summoning = true

end

function BTEnterHooks.block_stagger_start(unit, blackboard, t)
...
function BTEnterHooks.sorcerer_evade(unit, blackboard, t)
	blackboard.quick_teleport = true
...
function BTEnterHooks.warlord_defensive_on_enter(unit, blackboard, t)
	...
	wanted_pos =
	wanted_pos or self_position


	blackboard.override_spawn_allies_call_position = Vector3Box(wanted_pos)
end

function BTEnterHooks.keep_target(unit, blackboard, t)
	blackboard.keep_target = true
end

function BTEnterHooks.add_invincibility(unit, blackboard, t)
	local health_extension = ScriptUnit.extension(unit, "health_system")
	health_extension.is_invincible = true
end

function BTEnterHooks.remove_invincibility(unit, blackboard, t)
	local health_extension = ScriptUnit.extension(unit, "health_system")
	health_extension.is_invincible = false
end

function BTEnterHooks.activate_slot_system(unit, blackboard, t)
