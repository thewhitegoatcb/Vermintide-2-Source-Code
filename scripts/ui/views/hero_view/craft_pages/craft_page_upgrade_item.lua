...
local definitions = local_require("scripts/ui/views/hero_view/craft_pages/definitions/craft_page_upgrade_item_definitions")
...
function CraftPageUpgradeItem:on_enter(params, settings)
...
function CraftPageUpgradeItem:setup_recipe_requirements()
	...
	for index, data in ipairs(ingredients) do
		if not data.catergory then
			...
			local fake_item = { data = table.clone(ItemMasterList [item_key]), amount = presentation_amount, insufficient_amount = not has_required_amount }
...
function CraftPageUpgradeItem:create_ui_elements(params)
...
function CraftPageUpgradeItem:on_exit(params)
...
function CraftPageUpgradeItem:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function CraftPageUpgradeItem:post_update(dt, t)

	return end

function CraftPageUpgradeItem:_update_animations(dt)
...
function CraftPageUpgradeItem:_is_button_pressed(widget)
...
function CraftPageUpgradeItem:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageUpgradeItem:_is_button_held(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageUpgradeItem:_handle_input(dt, t)
...
function CraftPageUpgradeItem:_handle_craft_input_progress(progress)
...
function CraftPageUpgradeItem:craft_result(result, error, reset_slots)
	if not error then
		self._craft_result = result
	end
end

function CraftPageUpgradeItem:reset()
	local item_grid = self._item_grid
	item_grid:clear_locked_items()
	item_grid:update_items_status()
end

function CraftPageUpgradeItem:on_craft_completed()
...
function CraftPageUpgradeItem:_update_craft_items()
	local super_parent = self.super_parent
	local item_grid = self._item_grid
...
function CraftPageUpgradeItem:_remove_craft_item(backend_id, slot_index)
	local craft_items = self._craft_items
...
function CraftPageUpgradeItem:_add_craft_item(backend_id, slot_index, ignore_sound)
	if self._num_craft_items == 0 then
...
function CraftPageUpgradeItem:_set_craft_button_disabled(disabled)
	self._widgets_by_name.craft_button.content.button_hotspot.disable_button = disabled
end

function CraftPageUpgradeItem:_exit(selected_level)
	self.exit = true
	self.exit_level_id = selected_level
end

function CraftPageUpgradeItem:draw(dt)
...
function CraftPageUpgradeItem:_play_sound(event)
	self.super_parent:play_sound(event)
end

function CraftPageUpgradeItem:_set_craft_button_text(text, localize)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.craft_button
