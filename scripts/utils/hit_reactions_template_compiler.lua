...
DismemberFlowEvents = { explode_head = true }


AdditionalHitReactions = { "HitEffectsSkavenGreySeerMounted" }
...
local function compile_effects_templates(template)
	...
	for name, template_rule in pairs(templates) do
		local new_template_rule = compile_template_rule(template_rule, templates, { name })
