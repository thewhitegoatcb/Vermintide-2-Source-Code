...
function ImguiDeusWeapons:init()
	self._next_weapon_time = 0
end

function ImguiDeusWeapons:update()
	...
	if self._equip_random_weapon then
		...
		if self._next_weapon_time < t then
			...
			if inventory_extension and not inventory_extension:resyncing_loadout() then
				...
				deus_run_controller,
				inventory_extension,
				rarities [self._selected_rarity_index or 1],
				self._run_progress or 0,
				difficulties [self._difficulty_index or 1],
...
function ImguiDeusWeapons:on_round_end(...)
	self._equip_random_weapon = false
end

function ImguiDeusWeapons:on_venture_end(...)
	self._equip_random_weapon = false
end

function ImguiDeusWeapons:is_persistent()
	return false
end

function ImguiDeusWeapons:draw(is_open)
	if not Managers.state or not Managers.state.game_mode or Managers.state.game_mode:game_mode_key() ~= "deus" then
		local do_close = Imgui.begin_window("DeusWeapons", "always_auto_resize")
	...
	self._selected_weapon_group_index, self._selected_rarity_index, self._difficulty_index, self._run_progress = render_weapon_group_selection(
	self._available_weapon_groups, self._selected_weapon_group_index,
	rarities, self._selected_rarity_index,
	difficulties, self._difficulty_index,
	...
	if not items_for_rarity or #items_for_rarity <= 0 or not items_for_rarity then local available_items = { group.default }
	end
	Imgui.spacing()
	...
	if self._prev_item_key ~= item_key or
	self._prev_difficulty_index ~= self._difficulty_index or
	self._prev_rarity ~= rarity or
	self._prev_run_progress ~= self._run_progress then
	...
	if generate then
		self._weapon = DeusWeaponGeneration.create_weapon(
		item_key,
		self._properties and table.clone(self._properties),
		self._traits and table.clone(self._traits),
		self._skin,
		self._powerlevel,
...
function ImguiDeusWeapons:_reset_base_weapon_selection_data()
...
function ImguiDeusWeapons:_reset_weapon_setting_data(item_key, difficulty, run_progress, rarity)
...
function ImguiDeusWeapons:_get_inventory_extension()
