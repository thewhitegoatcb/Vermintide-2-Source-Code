...
local MAX_RETRIES = 2
local retry_codes = { 1199, 1342, 1133, 1287, 1127, 1131, 1214, 1123, 1101 }
...
local function on_error(request_data, result, id, error_override)
	...
	if retry and request_data.retries < MAX_RETRIES then
		...
		if error_override then error_code = error_override or error_code end
...
function curl_callback(success, code, headers, data, id)
	...
	if success then
		...
		if response and type(response) == "table" then
			...
			local error_data = { error = "ServiceUnavailable", errorCode = 1123, status = "",
				code = code }
...
local PlayFabHttpsCurl = {
	...
	MakePlayFabApiCall = function (url_path, request, auth_key, auth_value, on_success_callback, optional_on_fail_callback)
		...
		local headers = { "X-ReportErrorAsSuccess: true",

			"X-PlayFabSDK: " .. PlayFabSettings._internalSettings.sdkVersionString, "Content-Type: application/json",
