...
function FreeFlightManager:register_input_manager(input_manager)
...
function FreeFlightManager:unregister_input_manager()
...
function FreeFlightManager:set_teleport_override(cb)
	self._teleport_override = cb
end

function FreeFlightManager:_get_camera(local_player_id)
...
function FreeFlightManager:teleport_camera(local_player_id, pos, rot)
...
function FreeFlightManager:camera_position_rotation(local_player_id)
	local cam = self:_get_camera(local_player_id)
...
function FreeFlightManager:_resolve_input_service()
...
function FreeFlightManager:_update_global_free_flight(dt, data, input_service)
	local world = Managers.world:world(data.viewport_world_name)
	local viewport = ScriptWorld.global_free_flight_viewport(world)
...
function FreeFlightManager:cleanup_free_flight()
...
function FreeFlightManager:_clear_free_flight(data)
	data.active = false
	data.viewport_world_name = nil
	data.viewport_name = nil
end

function FreeFlightManager:register_player(local_player_id)

	local input_service = self.input_manager:get_service("FreeFlight")
	self.data [local_player_id] = { mode = "paused", current_translation_max_speed = 10, dof_focal_distance = 10, acceleration = 10, dof_focal_region_end = 4, dof_focal_near_scale = 1, rotation_speed = 0.003, dof_focal_far_scale = 1, dof_focal_region_start = 4, dof_enabled = 0, active = false, dof_focal_region = 8, input_service = input_service, rotation_accumulation = Vector3Box(), current_translation_speed = Vector3Box() }
end

function FreeFlightManager:unregister_player(local_player_id)
	local data = self.data [local_player_id]
...
function FreeFlightManager:_setup_data(data)
	data.global = { translation_speed = 0.05, rotation_speed = 0.003, mode = "paused", active = false, projection_type = Camera.PERSPECTIVE, orthographic_data = { size = 100 } }
end
...
function FreeFlightManager:_update_free_flight(dt, player, data)
	...
	if not Vector3.equal(new_speed, current_speed) then --[[ Nothing --]]
	end
...
function FreeFlightManager:drop_player_at_camera_pos(cam, player)
