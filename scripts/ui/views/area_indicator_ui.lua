...
function AreaIndicatorUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function AreaIndicatorUI:create_ui_elements()
...
function AreaIndicatorUI:destroy()
	return end

function AreaIndicatorUI:update(dt)
	...
	if player_unit and Unit.alive(player_unit) then
		...
		if not location_ui_blocked and current_location ~= nil and current_location ~= saved_location then
			...
			self.area_text_box_animation = UIAnimation.init(UIAnimation.function_by_time, widget.style.text.text_color, 1, 0, 255, ui_settings.fade_time, math.easeInCubic,
			UIAnimation.wait, ui_settings.wait_time,
			UIAnimation.function_by_time, widget.style.text.text_color, 1, 255, 0, ui_settings.fade_time, math.easeInCubic)
			self.area_text_box_shadow_animation = UIAnimation.init(UIAnimation.function_by_time, widget.style.text_shadow.text_color, 1, 0, 255, ui_settings.fade_time, math.easeInCubic,
			UIAnimation.wait, ui_settings.wait_time,
			UIAnimation.function_by_time, widget.style.text_shadow.text_color, 1, 255, 0, ui_settings.fade_time, math.easeInCubic)
...
function AreaIndicatorUI:update_animation(animation, dt)
	if animation then
...
function AreaIndicatorUI:draw(dt)
