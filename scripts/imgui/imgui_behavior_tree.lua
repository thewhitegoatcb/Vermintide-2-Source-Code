...
function ImguiBehaviorTree:init(world, ...)
	...
	self._scrolling = { x = 0, y = 0 }
	self._node_size = { width = 150, height = 0 }
	self._grid_size = 64
	self._offset = { x = 0, y = 0 }
	self._zoom = 1
	self._node_inner_padding = { x = 5, y = 5 }
	...
	self._curve_in_offset = { x = -50, y = 0 }
	self._curve_out_offset = { x = 50, y = 0 }
...
function ImguiBehaviorTree:_update_leaf_history(running_node_name)
...
function ImguiBehaviorTree:_calculate_rect_box(txt_1, txt_2, txt_3)
...
function ImguiBehaviorTree:_draw_node(x, y, node, link_out)
...
function ImguiBehaviorTree:_draw_nodes(node, x, y, link_out)
...
function ImguiBehaviorTree:_get_blackboard_value_type(key, entry, out)
...
function ImguiBehaviorTree:_draw_blackboard_element(key, value)
...
function ImguiBehaviorTree:_draw_blackboard_value(data)
...
function ImguiBehaviorTree:_save_history(brain, running_node_name)
...
function ImguiBehaviorTree:update()
	return end

function ImguiBehaviorTree:draw()
...
function ImguiBehaviorTree:_draw_graph(consideration_key, consideration_val, blackboard, action_data)
	...
	local margin_x = 10 local margin_y = 10
	local size_x = 300 local size_y = 150
	local axis_margin_x = 50 local axis_margin_y = 20
	local axis_offset_x = -20 local axis_offset_y = 0
...
function ImguiBehaviorTree:_draw_action_data(action_data, blackboard)
	...
	for key, val in pairs(action_data) do
		...
		if
		consideration and Imgui.tree_node(key) then
...
function ImguiBehaviorTree:is_persistent()
	return true
end
