...
PeerStates.Connecting = { approved_for_joining = false,
			...
			if mechanism == "weave" and matchmaking == "false" then
				...
				if player_ids then
					if not player_ids [self.peer_id] then
						...
						do return PeerStates.Disconnecting end
					end
				else
					self.server:disconnect_peer(self.peer_id, "cannot_join_weave")
					return PeerStates.Disconnecting
		...
		if self._has_been_notfied_of_post_game_state then
			if not server_in_post_game then
				...
				elseif self.has_received_rpc_notify_lobby_joined then
					...
					do return PeerStates.Loading end
				end
			end
		else
...
PeerStates.Loading = { approved_for_joining = true,
...
PeerStates.LoadingProfilePackages = { approved_for_joining = true,
...
PeerStates.WaitingForEnterGame = { approved_for_joining = true,
...
PeerStates.WaitingForGameObjectSync = { approved_for_joining = true,
...
PeerStates.WaitingForPlayers = { approved_for_joining = true,
	...
	update = function (self, dt)
		...
		if not cutscene_system.cutscene_started then
			...
			if server:are_all_peers_ready() then
				do return PeerStates.InGame end
			end
...
PeerStates.InGame = { approved_for_joining = true,
...
PeerStates.InPostGame = { approved_for_joining = true,
...
PeerStates.Disconnecting = { approved_for_joining = false,
	...
	on_enter = function (self, previous_state)
		...
		if
		game_session and server.peers_added_to_gamesession [peer_id] then
...
PeerStates.Disconnected = { approved_for_joining = false,
...
for state_name, state_table in pairs(PeerStates) do
	...
	setmetatable(state_table, { __tostring = function () return state_name end }) end