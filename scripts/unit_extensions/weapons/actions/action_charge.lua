...
function ActionCharge:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionCharge:client_owner_start_action(new_action, t)
...
function ActionCharge:_start_charge_sound()
...
function ActionCharge:_stop_charge_sound(reason)
...
function ActionCharge:client_owner_post_update(dt, t, world, can_damage)
	local current_action = self.current_action
	...
	if
	charge_effect_material_name and charge_effect_material_variable_name and particle_id and World.has_particles_material(world, particle_id, charge_effect_material_name) then
	...
	if
	left_charge_effect_material_name and left_charge_effect_material_variable_name and left_particle_id and World.has_particles_material(world, left_particle_id, left_charge_effect_material_name) then
	...
	if not is_bot then
		...
		if charge_level >= 1 and
		self.charge_flow_event_left_weapon and self.left_unit then
...
function ActionCharge:_clean_up(reason)
...
function ActionCharge:finish(reason)
	local owner_unit = self.owner_unit
	...
	return { charge_level = self.charge_level }
end

function ActionCharge:destroy()
	self:_clean_up()
end