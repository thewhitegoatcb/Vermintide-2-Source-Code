...
function PlayerUnitEnergyExtension:init(extension_init_context, unit, extension_init_data)
...
function PlayerUnitEnergyExtension:extensions_ready(world, unit)
	return end

function PlayerUnitEnergyExtension:destroy()
	return end

function PlayerUnitEnergyExtension:_update_game_object()
...
function PlayerUnitEnergyExtension:_update_events()
...
function PlayerUnitEnergyExtension:update(unit, input, dt, context, t)
...
function PlayerUnitEnergyExtension:drain(amount)
	assert(amount >= 0, "Use add_energy()")


	local buff_extension = ScriptUnit.has_extension(self.unit, "buff_system")
	if
	buff_extension and buff_extension:has_buff_perk("infinite_ammo") then
		amount = 0
	end
...
function PlayerUnitEnergyExtension:add_energy(amount)
...
function PlayerUnitEnergyExtension:get_max()
	return self._max_energy
end

function PlayerUnitEnergyExtension:is_drainable()
...
function PlayerUnitEnergyExtension:is_depleted()
	return self._energy <= 0
end

function PlayerUnitEnergyExtension:get_fraction()
	return math.clamp(self._energy / self._max_energy, 0, 1)
end

function PlayerUnitEnergyExtension:_start_depletion(dt, t)
	self._depletion_cooldown_timer = self._depletion_cooldown + t
end

function PlayerUnitEnergyExtension:_process_recharge(dt, t)
	self._energy = math.clamp(self._energy + self._recharge_rate * dt, 0, self._max_energy)
end

function PlayerUnitEnergyExtension:_is_on_depletion_cooldown()
	return Managers.time:time("game") < self._depletion_cooldown_timer
end

function PlayerUnitEnergyExtension:_is_recharging()
	return self._recharge_delay_timer <= Managers.time:time("game")
end

function PlayerUnitEnergyExtension:_broadcast_equipment_flow_event(event_name)
