...
local RPCS = { "rpc_social_wheel_event" }
...
if IS_WINDOWS then
	ANIMATION_TIMES = {
		OPEN = { MOVE_Y = 1.5, SIZE = 1.5, MOVE_X = 1.5, ALPHA = 0.45 },





		CLOSE = { MOVE_Y = 0.2, SIZE = 0.25, MOVE_X = 0.2, ALPHA = 0.1 } }
else
	...
	ANIMATION_TIMES = {
		OPEN = { MOVE_Y = 0.75, SIZE = 0.75, MOVE_X = 0.75, ALPHA = 0.45 },
...
local SFX_EVENTS = {
	default = { OPEN = "Play_hud_socialwheel_open", HOVER = "Play_hud_socialwheel_hover", SELECT = "Play_hud_socialwheel_select" },





	heroes = { OPEN = "Play_hud_socialwheel_open", HOVER = "Play_hud_socialwheel_hover", SELECT = "Play_hud_socialwheel_select" } }
...
function SocialWheelUI:init(parent, ingame_ui_context)
	self._parent = parent
	self._ui_top_renderer = ingame_ui_context.ui_top_renderer
	self._render_settings = { alpha_multiplier = 0 }
	self._is_visible = true
	self._state = "update_closed"
	self._states = { update_closed = true, update_open = true }
...
function SocialWheelUI:_create_ui_elements()
...
function SocialWheelUI:_register_rpcs()
...
function SocialWheelUI:_unregister_rpcs()
	local ingame_ui_context = self._ingame_ui_context
	local network_event_delegate = ingame_ui_context.network_event_delegate
	network_event_delegate:unregister(self)
end

function SocialWheelUI:destroy()
...
function SocialWheelUI:_widget_angle(total_angle, num_elements, i)
	local pi = math.pi

	local segment = total_angle / num_elements
	local radial_offset = pi * 0.5 + pi -
	total_angle * 0.5 +
	segment * 0.5
...
function SocialWheelUI:_select_widget(total_angle, num_elements, angle)
	local pi = math.pi
	local segment = total_angle / num_elements
...
local GARBAGE = { 0, 0, 0 }
function SocialWheelUI:_add_social_wheel_event(player, social_wheel_event, target_unit, play_notification_sound)
	...
	local event_text = text local localization_parameters = nil
	if text_func then
...
function SocialWheelUI:_add_social_wheel_event_animation(widget, is_local_player)
...
function SocialWheelUI:rpc_social_wheel_event(channel_id, peer_id, social_wheel_event_id, target_unit_id)
	if IS_XB1 and
	Managers.chat:ignoring_peer_id(peer_id) then
		return
	end
...
function SocialWheelUI:post_update(dt)
	self:_post_update_remove_icon(dt)
	self:_post_update_render(dt)
end

local TO_REMOVE = { }
function SocialWheelUI:_post_update_remove_icon(dt)
...
function SocialWheelUI:_post_update_render(dt)
...
function SocialWheelUI:update(dt, t)
...
function SocialWheelUI:_update_animations(dt)
...
function SocialWheelUI:_update_input(dt, t)
...
function SocialWheelUI:_draw(dt, t)
	if not self._is_visible then
		return
	end
...
function SocialWheelUI:set_visible(visible)
	self._is_visible = visible
end

function SocialWheelUI:_set_player_input_scale(scale, lerp_time)
...
function SocialWheelUI:_ping_unit_attempt(target_unit, ping_type, social_wheel_event_id)
	local player = self._player
	local player_unit = player.player_unit
...
function SocialWheelUI:_ping_world_position_attempt(position, ping_type, social_wheel_event_id)
	local player = self._player
	local player_unit = player.player_unit
	if Unit.alive(player_unit) then
		local game_time = Managers.time:time("game")
...
function SocialWheelUI:_social_message_attempt(social_wheel_event_id, target_unit)
	local player = self._player
	local player_unit = player.player_unit
	if Unit.alive(player_unit) then
		local game_time = Managers.time:time("game")
...
function SocialWheelUI:_local_ping_attempt(social_wheel_event_id, target_unit)
	local player = self._player
	local player_unit = player.player_unit
	if Unit.alive(player_unit) then
...
function SocialWheelUI:_play_sound(sound_event)
...
function SocialWheelUI:_set_pulsing(context, enable)
...
function SocialWheelUI:_set_current_context(new_context)
...
function SocialWheelUI:_open_menu(dt, t, input_service, increment_page)
	...
	if category_page then
		...
		elseif active_context.show_emotes then category_page = 2 else category_page = 1
		end
...
function SocialWheelUI:update_open(dt, t, input_service)
...
function SocialWheelUI:_update_pointer(input_service, enabled, t)
	local arrow_widget = self._arrow_widget
	local arrow_content = arrow_widget.content
	...
	if gamepad_enabled then
		...
		if axis_length < 0.5 then
			...
			arrow_style.offset = { 90 * gamepad_axis [1], 90 * gamepad_axis [2], 0 }
		...
		if new_length < 100 then
			...
			cursor_style.offset = { offset.x, offset.y }
			arrow_content.visible = enabled
			arrow_content.pointing_point:store(new_position)
		else
			angle = math.atan2(direction [2] * aspect_ratio, direction [1])
			arrow_style.angle = math.pi - angle
			arrow_style.offset = { 100 * direction [1], 100 * direction [2], 0 }
...
function SocialWheelUI:_update_selection(enabled, total_angle, angle)
	local selection_widgets = self._current_selection_widgets
...
function SocialWheelUI:_close_menu(dt, t, input_service, page_only)
...
function SocialWheelUI:is_active()
