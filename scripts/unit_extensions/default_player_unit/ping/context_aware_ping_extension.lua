...
function ContextAwarePingExtension:init(extension_init_context, unit, extension_init_data)
...
function ContextAwarePingExtension:extensions_ready(world, unit)
...
function ContextAwarePingExtension:ping_context()
	return self._ping_context
end

function ContextAwarePingExtension:social_wheel_context()
	return self._social_wheel_context
end

function ContextAwarePingExtension:destroy()
	return end

function ContextAwarePingExtension:update(unit, input, dt, context, t)
	...
	elseif self._social_wheel_context then
		...
		if ping or is_ping_only or social_wheel_only or photomode_only then
			...
			elseif social_wheel_only then
				self._social_wheel_context = { min_t = 0, unit = social_wheel_unit, distance = social_wheel_unit_distance, position = stored_ping_position }
			elseif ping then
				...
				self._ping_context = { unit = ping_unit, max_t = t + social_wheel_delay, distance = ping_unit_distance, position = stored_ping_position, ping_type = ping_type }
				self._social_wheel_context = { unit = social_wheel_unit, ping_context_unit = ping_unit, min_t = t + social_wheel_delay, distance = social_wheel_unit_distance, position = stored_ping_position }
			elseif photomode_only then
				self._social_wheel_context = { min_t = 0, show_emotes = true, unit = social_wheel_unit, distance = social_wheel_unit_distance, position = stored_ping_position }
			end
		end
	end
...
function ContextAwarePingExtension:_have_free_events()
	return self._num_free_events > 0
end

function ContextAwarePingExtension:_consume_ping_event()
	self._num_free_events = self._num_free_events - 1
end

function ContextAwarePingExtension:ping_attempt(unit, unit_to_ping, t, ping_type, social_wheel_event_id)
	...
	ping_type =
	ping_type or self._world_markers_enabled and PingTypes.CONTEXT or PingTypes.PING_ONLY
...
function ContextAwarePingExtension:ping_world_position_attempt(unit, position, t, ping_type, social_wheel_event_id)
	if t < self._ping_timer and not IgnoreCooldownPingTypes [ping_type] then
		return
	end
...
function ContextAwarePingExtension:social_message_attempt(unit, social_wheel_event_id, target_unit)
	if not self:_have_free_events() then
		local error_message = Localize("social_wheel_too_many_messages_warning")
		Managers.chat:add_local_system_message(1, error_message, true)
		return false
...
local INDEX_ACTOR = 4
function ContextAwarePingExtension:_check_raycast(unit)
			...
			if actor then
				...
				if hit_unit ~= unit then
					...
					if ping_ext then
						...
						if ( not ghost_mode_ext or not ghost_mode_ext:is_in_ghost_mode() ) and
						distance > 0.05 then
							...
							if ( ping_ext.always_pingable or is_pickup or is_alive and (is_enemy or is_incapacitated_player) ) and
							not darkness_system:is_in_darkness(hit_position) and best_ping_utility < utility then
