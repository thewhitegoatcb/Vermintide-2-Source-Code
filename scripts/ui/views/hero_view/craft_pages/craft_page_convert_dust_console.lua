...
local definitions = local_require("scripts/ui/views/hero_view/craft_pages/definitions/craft_page_convert_dust_console_definitions")
...
function CraftPageConvertDustConsole:on_enter(params, settings)
...
function CraftPageConvertDustConsole:_has_required_item_amount(backend_id)
...
function CraftPageConvertDustConsole:_get_recipe_by_backend_id(backend_id)
	local item_interface = Managers.backend:get_interface("items")
...
function CraftPageConvertDustConsole:setup_recipe_requirements()
...
function CraftPageConvertDustConsole:reset_requirements(num_required_ingredients)
...
function CraftPageConvertDustConsole:_add_crafting_material_requirement(index, item_key, amount_text, has_required_amount)
...
function CraftPageConvertDustConsole:create_ui_elements(params)
...
function CraftPageConvertDustConsole:on_exit(params)
...
function CraftPageConvertDustConsole:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function CraftPageConvertDustConsole:post_update(dt, t)

	return end

function CraftPageConvertDustConsole:_update_animations(dt)
...
function CraftPageConvertDustConsole:_is_button_pressed(widget)
	local content = widget.content
...
function CraftPageConvertDustConsole:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageConvertDustConsole:_is_button_held(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageConvertDustConsole:_handle_input(dt, t)
...
function CraftPageConvertDustConsole:_handle_craft_input_progress(progress)
	return self.parent:_set_input_progress(progress)
end

function CraftPageConvertDustConsole:craft_result(result, error, reset_slots)
	if not error then
		self._craft_result = result
	end
end

function CraftPageConvertDustConsole:reset()
	if self._craft_items [1] then
...
function CraftPageConvertDustConsole:present_results()
	local item_grid = self._item_grid
	item_grid:clear_locked_items()
	item_grid:update_items_status()
...
function CraftPageConvertDustConsole:on_craft_completed()
	local item_grid = self._item_grid
	...
	local item_interface = Managers.backend:get_interface("items")
	if item_interface:get_item_from_id(backend_id) then
		add_item = true
	end
	...
	if add_item then
		...
		self:_add_craft_item(backend_id, 1, ignore_sound)
	else
...
function CraftPageConvertDustConsole:_update_craft_items()
	local super_parent = self.super_parent
	local item_grid = self._item_grid
...
function CraftPageConvertDustConsole:_remove_craft_item(backend_id, slot_index)
	local craft_items = self._craft_items
...
function CraftPageConvertDustConsole:_add_craft_item(backend_id, slot_index, ignore_sound)
...
function CraftPageConvertDustConsole:_set_craft_button_disabled(disabled)
...
function CraftPageConvertDustConsole:_exit(selected_level)
	self.exit = true
	self.exit_level_id = selected_level
end

function CraftPageConvertDustConsole:draw(dt)
...
function CraftPageConvertDustConsole:_play_sound(event)
	self.super_parent:play_sound(event)
end

function CraftPageConvertDustConsole:_set_craft_button_text(text, localize)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.craft_button
	widget.content.button_text = localize and Localize(text) or text
end

function CraftPageConvertDustConsole:_has_added_item_by_id(backend_id)
	local craft_items = self._craft_items
...
function CraftPageConvertDustConsole:_clear_item_grid()
	local craft_items = self._craft_items
	local super_parent = self.super_parent
