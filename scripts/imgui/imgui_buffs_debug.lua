...
function ImguiBuffsDebug:_get_buff_templates()
...
function ImguiBuffsDebug:_apply_buff_filter(filter_text, buff_list)
	if filter_text == "" then
		return buff_list
	end
...
function ImguiBuffsDebug:update()
...
function ImguiBuffsDebug:on_round_start()
	self._current_unit = nil
	self:_refresh_unit_list()
end

function ImguiBuffsDebug:is_persistent()
	return true
end

function ImguiBuffsDebug:draw(is_open)
...
function ImguiBuffsDebug:_update_controls()
	...
	if self._buff_advanced_params_enabled then
		...
		Imgui.dummy(15, 15) Imgui.same_line() Imgui.text("Power Level")
		Imgui.same_line()
	...
	if Imgui.button("Add", 100, 20) then
		...
		if self._buff_advanced_params_enabled then local params = {
	...
	if sync_type == BuffSyncType.Client or sync_type == BuffSyncType.ClientAndServer then
		...
		local peer_ids = table.select_array(table.keys(Managers.player:human_players()),
		function (new_t, unique_id)
...
function ImguiBuffsDebug:_display_buffs(buffs)
			...
			for i = 1, #buffs do
				...
				if not buff.removed and Imgui.tree_node(buff.buff_type .. "(" .. buff.id .. ")") then
					for name, data in pairs(buff) do
						if
						name == "template" and Imgui.tree_node(name) then
...
function ImguiBuffsDebug:_display_perks(perks)
...
function ImguiBuffsDebug:_display_stat_buffs(stat_buffs)
...
function ImguiBuffsDebug:_display_event_buffs(event_buffs)
...
function ImguiBuffsDebug:_refresh_unit_list()
	self._unit_names = { }
	self._units = { }
...
function ImguiBuffsDebug:_initialize_unit(unit)
...
function ImguiBuffsDebug:_add_buff(buff_extension, buff_name, params)
	if self._buff_extension and buff_name then
		self._buff_extension:add_buff(buff_name, params)
	end
end

function ImguiBuffsDebug:_add_buff_with_buff_system(buff_name)
...
function ImguiBuffsDebug:_add_buff_with_buff_synced(buff_name, sync_type)
	if self._current_unit then
		local buff_system = Managers.state.entity:system("buff_system")
		if buff_system then
...
function ImguiBuffsDebug:_remove_buff(buff_extension, buff_id)
