...
local arm_to_nodename = { attack_right = "fx_right_muzzle", attack_left = "fx_left_muzzle" }



arm_to_nodename = table.mirror_array_inplace(arm_to_nodename)

function StormfiendBeamExtension:init(extension_init_context, unit, extension_init_data)
...
function StormfiendBeamExtension:destroy()
	for _, node_name in pairs(arm_to_nodename) do
		self:_remove_beam(node_name)
	end
end

function StormfiendBeamExtension:_remove_beam(node_name)
...
function StormfiendBeamExtension:set_beam(arm, active)
...
function StormfiendBeamExtension:_create_beam(node_name)
...
function StormfiendBeamExtension:get_target_position(unit, muzzle_node)
...
function StormfiendBeamExtension:update(unit, input, dt, context, t)
	local world = self.world
