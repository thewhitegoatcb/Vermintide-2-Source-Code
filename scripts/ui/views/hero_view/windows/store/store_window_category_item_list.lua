...
function StoreWindowCategoryItemList:on_enter(params, offset)
	...
	self._render_settings = { alpha_multiplier = 0, list_alpha_multiplier = 0, snap_pixel_positions = true }
...
function StoreWindowCategoryItemList:_start_transition_animation(animation_name)
	local params = {
...
function StoreWindowCategoryItemList:_create_ui_elements(params, offset)
...
function StoreWindowCategoryItemList:on_exit(params, force_unload)
...
function StoreWindowCategoryItemList:update(dt, t)
...
function StoreWindowCategoryItemList:_sync_products_version()
...
function StoreWindowCategoryItemList:post_update(dt, t)
...
function StoreWindowCategoryItemList:_update_animations(dt)
...
function StoreWindowCategoryItemList:_is_list_hovered()
	local list_mask = self._widgets_by_name.list
	return list_mask.content.list_hotspot.is_hover or false
end

function StoreWindowCategoryItemList:_handle_input(dt, t)
	local parent = self._parent
...
function StoreWindowCategoryItemList:_draw(dt)
...
function StoreWindowCategoryItemList:_play_sound(event)
	self._parent:play_sound(event)
end

function StoreWindowCategoryItemList:_update_gamepad_focus()
...
function StoreWindowCategoryItemList:_handle_gamepad_activity()
...
function StoreWindowCategoryItemList:_list_index_pressed()
	local list_widgets = self._list_widgets
	if list_widgets then
...
function StoreWindowCategoryItemList:_animate_list_entries(dt)
	local parent = self._parent
...
function StoreWindowCategoryItemList:_get_items_by_filter(item_filter)
...
function StoreWindowCategoryItemList:_get_all_items()
	local backend_store = Managers.backend:get_interface("peddler")
...
function StoreWindowCategoryItemList:_update_item_list()
...
function StoreWindowCategoryItemList:_set_title_texts(title_text)
	local widgets_by_name = self._widgets_by_name
...
function StoreWindowCategoryItemList:_get_list_index_by_product_id(product_id)
...
function StoreWindowCategoryItemList:_on_list_index_selected(index, scrollbar_animation_percentage)
	local layout = self._layout
...
function StoreWindowCategoryItemList:_on_list_index_pressed(index)
	local layout = self._layout
	local entry = layout [index]
...
function StoreWindowCategoryItemList:_create_product_widgets(layout)
...
function StoreWindowCategoryItemList:_destroy_product_widgets(force_unload)
	local parent = self._parent
...
function StoreWindowCategoryItemList:_align_item_widgets()
...
function StoreWindowCategoryItemList:_right_align_item_widgets()
	local widgets = self._list_widgets

	local previous_column_index, new_position_x = nil
...
function StoreWindowCategoryItemList:_sync_layout_path(force_update)
	local parent = self._parent
...
function StoreWindowCategoryItemList:_handle_gamepad_grid_selection(input_service)
	local current_index = self._selected_gamepad_grid_index
...
function StoreWindowCategoryItemList:_find_closest_neighbour(column_index_list, current_index)
	local list_widgets = self._list_widgets
...
function StoreWindowCategoryItemList:_initialize_scrollbar()
...
function StoreWindowCategoryItemList:_update_scroll_position()
	local scrollbar_logic = self._scrollbar_logic
...
function StoreWindowCategoryItemList:_update_visible_list_entries()
	local scrollbar_logic = self._scrollbar_logic
...
function StoreWindowCategoryItemList:_scroll_to_list_index(index)
	local scrollbar_logic = self._scrollbar_logic
	local enabled = scrollbar_logic:enabled()
...
function StoreWindowCategoryItemList:_get_scrollbar_percentage_by_index(index)
	local scrollbar_logic = self._scrollbar_logic
	local enabled = scrollbar_logic:enabled()
