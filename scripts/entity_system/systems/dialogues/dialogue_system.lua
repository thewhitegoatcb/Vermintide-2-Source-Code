...
local extensions = { "DialogueActorExtension" }
...
function DialogueSystem:_load_special_event_dialogues(package_name, mechanism_name)
...
function DialogueSystem:init(entity_system_creation_context, system_name)
...
function DialogueSystem:on_add_extension(world, unit, extension_name, extension_init_data)
	local extension = { is_silenced = false,
		user_memory = { },
		context = { health = 1 },
	...
	local input = MakeTableStrict({
		...
		trigger_dialogue_event = function (self, event_name, event_data, identifier)
			if not dialogue_system.is_server then return
			end
		...
		play_voice = function (self, sound_event, use_occlusion)
			if DEDICATED_SERVER then return
			end
...
function DialogueSystem:on_freeze_extension(unit, extension_name)
	local extension = self.unit_extension_data [unit]
...
function DialogueSystem:freeze(unit, extension_name, reason)
...
function DialogueSystem:unfreeze(unit)
...
function DialogueSystem:_update_currently_playing_dialogues(dt)
	...
	for unit, extension in pairs(playing_units) do repeat
				...
				if currently_playing_dialogue.dialogue_timer == nil then
					...
					if not is_currently_playing then
						...
						if result then
							...
							if currently_playing_dialogue.override_awareness then
								...
								self.entity_manager:system("surrounding_aware_system"):add_system_event(source,
								"heard_speak",
								sound_distance,
								"speaker",
								source,
								...
								speaker_name,
								"sound_event",
								extension.last_query_sound_event or "unknown",
								"dialogue_name",
								result,
								"dialogue_name_nopre",
								string.sub(result, 5))
					...
					elseif currently_playing_dialogue.dialogue_timer then
						if extension.input:is_silenced() then
							...
							currently_playing_dialogue.dialogue_timer = currently_playing_dialogue.dialogue_timer - dt end end end end until true
	end
...
function DialogueSystem:update(context, t)
	return end

function DialogueSystem:_handle_wwise_markers(dt, t)
...
function DialogueSystem:_trigger_marker(marker_data)
	...
	for _, player in pairs(players) do
		...
		if Unit.alive(unit) and
		ScriptUnit.has_extension(unit, "dialogue_system") then
	...
	elseif self.playing_units [source_player] then
		...
		if not extension then
			...
			source_id =
			source_id or WwiseWorld.trigger_event(wwise_world, sound_event, wwise_source_id)
...
local EMPTY_TABLE = { }
function DialogueSystem:physics_async_update(context, t)
	...
	if enabled and (
	DialogueSettings.dialogue_level_start_delay < self.global_context.level_time or DialogueSystem:has_local_player_moved_from_start_position()) then
		if query then
			...
			if result then
				...
				if will_play then
					...
					if query_context.identifier and query_context.identifier ~= "" then
						self.dialogue_state_handler:add_playing_dialogue(query_context.identifier,
						dialogue.sound_events [dialogue_index],
						t,
...
function DialogueSystem:_update_incapacitation(t)
...
function DialogueSystem:_update_new_events(t)
	...
	for i = 1, input_event_queue_n, 4 do repeat
			...
			if extension then if extension.is_incapacitated and t > extension.incapacitate_time + 0.1 and dialogue_category ~= "knocked_down_override" and input_event_queue [i + 2].is_ping ~= true then
					break
				end
				...
				input_event_queue [i + 2] = nil end until true
	end
	self.input_event_queue_n = 0
...
function DialogueSystem:hot_join_sync(peer_id)
...
function DialogueSystem:has_local_player_moved_from_start_position()
	...
	if Unit.alive(local_player_unit) and ScriptUnit.has_extension(local_player_unit, "dialogue_system") then
		...
		if extension.local_player_has_moved and not debug_vo_by_file then return true end
		local player_pos = POSITION_LOOKUP [local_player_unit]
		if
		player_pos ~= nil and Vector3.distance(player_pos, Vector3Aux.unbox(extension.local_player_start_pos)) > 2 then
...
function DialogueSystem:player_shield_check(unit, slot)
	local has_shield = 0
...
function DialogueSystem:trigger_targeted_by_ratling(player_unit)
	local player_manager = Managers.player
...
function DialogueSystem:trigger_backstab(player_unit, enemy_unit, blackboard)
	local player_manager = Managers.player
	local owner = player_manager:unit_owner(player_unit)
...
function DialogueSystem:trigger_flanking(player_unit, enemy_unit)
	local player_manager = Managers.player
	local owner = player_manager:unit_owner(player_unit)
...
function DialogueSystem:trigger_backstab_hit(player_unit, enemy_unit)
	local player_manager = Managers.player
	local owner = player_manager:unit_owner(player_unit)
	local game = Managers.state.network:game()
	if Unit.alive(player_unit) and owner and ALIVE [enemy_unit] and game and
	not owner.bot_player then
...
function DialogueSystem:get_random_player()
...
function DialogueSystem:freeze_story_trigger()
	self._story_trigger_freezes = self._story_trigger_freezes + 1
end

function DialogueSystem:unfreeze_story_trigger()
	self._story_trigger_freezes = math.max(0, self._story_trigger_freezes - 1)
end

function DialogueSystem:_is_story_trigger_frozen()
...
local end_time = 0
local end_delay = 5
function DialogueSystem:trigger_cutscene_subtitles(event_name, speaker, end_hangtime)
	enabled = false
...
function DialogueSystem:_update_cutscene_subtitles(t)
...
function DialogueSystem:trigger_sound_event_with_subtitles(sound_event, subtitle_event, speaker_name, source_unit, unit_node)
	if DEDICATED_SERVER then
...
function DialogueSystem:_update_sound_event_subtitles()
	if DEDICATED_SERVER then
...
function DialogueSystem:reset_memory_time(memory, name, unit)
...
function DialogueSystem:trigger_story_dialogue(unit)
...
function DialogueSystem:_update_player_jumping(t)
	if DEDICATED_SERVER then
		return
	end
...
function DialogueSystem:rpc_play_marker_event(channel_id, go_id, marker_id)
	local marker_unit = Managers.state.network:game_object_or_level_unit(go_id, false)
	if not marker_unit then return end
	if FROZEN [marker_unit] then return
	end
...
function DialogueSystem:_check_play_debug_sound(sound_event, subtitles_event)
...
function DialogueSystem:rpc_play_dialogue_event(channel_id, go_id, is_level_unit, dialogue_id, dialogue_index)
	local dialogue_actor_unit = Managers.state.network:game_object_or_level_unit(go_id, is_level_unit)
	if not dialogue_actor_unit then return end
	if FROZEN [dialogue_actor_unit] then return
	end
...
function DialogueSystem:rpc_interrupt_dialogue_event(channel_id, go_id, is_level_unit)
	local dialogue_actor_unit = Managers.state.network:game_object_or_level_unit(go_id, is_level_unit)
	if not dialogue_actor_unit then return end
	if self.frozen_unit_extension_data [dialogue_actor_unit] then return
	end
	local extension = self.unit_extension_data [dialogue_actor_unit]
...
function DialogueSystem:rpc_update_current_wind(channel_id, weave_name_id)
