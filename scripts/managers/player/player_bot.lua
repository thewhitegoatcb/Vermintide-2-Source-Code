...
function PlayerBot:init(network_manager, player_name, bot_profile_name, is_server, profile_index, career_index, local_player_id, unique_id, ui_id, account_id)
...
function PlayerBot:profile_index()
	return self._profile_index
end

function PlayerBot:career_index()
	return self._career_index
end

function PlayerBot:stats_id()
	return self._unique_id
end

function PlayerBot:ui_id()
	return self._ui_id
end

function PlayerBot:local_player_id()
	return self._local_player_id
end

function PlayerBot:unique_id()
	return self._unique_id
end

function PlayerBot:platform_id()
	ferror("Not implemented")
end

function PlayerBot:type()
	return "PlayerBot"
end

function PlayerBot:is_player_controlled()
	return false
end

function PlayerBot:set_player_unit(unit)
	self.player_unit = unit
end

function PlayerBot:profile_display_name()
	local profile = SPProfiles [self._profile_index]
	local display_name = profile and profile.display_name
	return display_name
end

function PlayerBot:despawn()
...
function PlayerBot:telemetry_id()
	return self._telemetry_id
end

function PlayerBot:spawn(position, rotation, is_initial_spawn, ammo_melee, ammo_ranged, healthkit, potion, grenade, ability_cooldown_percent_int, additional_items)
	...
	local extension_init_data = {
		...
		ai_system = { player = self, bot_profile = self.bot_profile, nav_world = nav_world },
		ai_bot_group_system = { initial_inventory = initial_inventory, side = side },
		input_system = { player = self },
		character_state_machine_system = { start_state = "standing", nav_world = nav_world, character_state_class_list = character_state_class_list, player = self },
		...
		aim_system = { is_husk = false, template = "player" },
		...
		buff_system = { is_husk = false },
		statistics_system = { template = "player", statistics_id = self.peer_id },
		ai_slot_system = { profile_index = profile_index },
		talent_system = { player = self, profile_index = profile_index },
...
function PlayerBot:create_game_object()
	local game_object_data_table = { ping = 0, player_controlled = false,
