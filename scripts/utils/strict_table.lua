...
local function sprintf(...) return format(...) end
...
if USE_ERROR then
	...
	local function console_print(level, message)
		if Application.console_send then
			Application.console_send({ system = "Lua", type = "message", level = level, message = message })
		else
			print(message)
		end
	end

	function error_func(...) console_print("error", sprintf(...)) end
end
...
function MakeTableStrict(t)
	...
	local meta = { __declared = declared_args }
		...
		if not meta.__declared [k] then
			if not rawget(t, k) then
				...
				if k ~= "to_console_line" and info and info.what ~= "main" and info.what ~= "C" then
					error_func("[ERROR] cannot assign undeclared member variable %q, %s",
					k, debug_info_string(info))
				end
			end
	...
	function meta.__index(t, k)
		if not meta.__declared [k] and
		not rawget(t, k) then
			local info = debug_getinfo(2, "Sl")
			if k ~= "to_console_line" and info and info.what ~= "main" and info.what ~= "C" then
				error_func("[ERROR] cannot index undeclared member variable %q, %s",
				tostring(k), debug_info_string(info))
			end
		end
