...
local CAREER_ID_LOOKUP = { "dr_ranger", "dr_slayer", "dr_ironbreaker", "dr_engineer", "we_waywatcher", "we_shade", "we_maidenguard", "es_huntsman", "es_mercenary", "es_knight", "es_questingknight", "bw_adept", "bw_scholar", "bw_unchained", "wh_captain", "wh_bountyhunter", "wh_zealot", "we_thornsister", "wh_priest", "bw_necromancer" }
...
local ALLOWED_DATA_TYPES = { string = true, boolean = true, number = true }
...
function PlayFabMirrorBase:init(signin_result)
...
function PlayFabMirrorBase:_parse_claimed_achievements()
...
function PlayFabMirrorBase:_parse_claimed_event_quests()
...
function PlayFabMirrorBase:_parse_unlocked_weapon_skins()
...
function PlayFabMirrorBase:_parse_unlocked_cosmetics()
...
function PlayFabMirrorBase:_parse_claimed_console_dlc_rewards()
...
function PlayFabMirrorBase:_verify_account_data()
	if DEDICATED_SERVER then
		return
	end

	local request = { FunctionName = "verifyAccountData" }
...
function PlayFabMirrorBase:verify_account_data_cb(result)
	self._num_items_to_load = self._num_items_to_load - 1
...
function PlayFabMirrorBase:_migrate_characters()
	...
	if not characters_data or characters_data == "{}" or characters_data == "" or type(characters_data) == "table" and table.is_empty(characters_data) then
		local request = { FunctionName = "migrateCharacters",
...
function PlayFabMirrorBase:migrate_characters_cb(result)
...
function PlayFabMirrorBase:_migrate_cosmetics()
	if DEDICATED_SERVER then
		return
	end

	local request = { FunctionName = "migrateCosmetics" }
...
function PlayFabMirrorBase:migrate_cosmetics_request_cb(result)
	self._num_items_to_load = self._num_items_to_load - 1
...
function PlayFabMirrorBase:_update_dlc_ownership()
	if DEDICATED_SERVER then
		return
	end
	...
	local request = { FunctionName = "updateDLCOwnership",
...
function PlayFabMirrorBase:dlc_unlocked_at_signin(dlc_name)
	return table.find(self._unlocked_dlcs, dlc_name) ~= false
end



function PlayFabMirrorBase:dlc_ownership_request_cb(result)
	self._num_items_to_load = self._num_items_to_load - 1
...
function PlayFabMirrorBase:_execute_dlc_specific_logic()
	local request = { FunctionName = "executeDLCLogic",
...
function PlayFabMirrorBase:_sync_unseen_rewards(new_rewards)
	...
	for i = 1, #new_rewards do
		...
		if item_type == "keep_decoration_painting" then
			local reward = { reward_type = "keep_decoration_painting",
		...
		elseif CosmeticUtils.is_cosmetic_item(item_type) then
			local backend_id = self:add_item(nil, { ItemId = item_id })
			if backend_id then
				local reward = {
			...
			if data and rewarded_from then
				if data.bundle then
					...
					for currency_type, currency_amount in pairs(bundled_currencies) do
						local reward = { reward_type = "currency",
					...
					ItemHelper.mark_backend_id_as_new(backend_id, { data = data })
				end
			end
		end
	end
...
function PlayFabMirrorBase:execute_dlc_logic_request_cb(result)
	self._num_items_to_load = self._num_items_to_load - 1
...
function PlayFabMirrorBase:_request_best_power_levels()
	local request = { FunctionName = "bestPowerLevels",
...
function PlayFabMirrorBase:best_power_levels_request_cb(result)
	self._num_items_to_load = self._num_items_to_load - 1
...
function PlayFabMirrorBase:_request_signin_reward()
	local request = { FunctionName = "signInRewards",
...
function PlayFabMirrorBase:sign_in_reward_request_cb(result)
	self._num_items_to_load = self._num_items_to_load - 1
	...
	for reward_id, reward_data in pairs(rewards) do
		...
		if unlocked_cosmetics then
			for i = 1, #unlocked_cosmetics do
				self:add_item(nil, { ItemId = unlocked_cosmetics [i] })
			end
		end
...
function PlayFabMirrorBase:_request_quests()
...
function PlayFabMirrorBase:get_quests_cb(result)
	self._num_items_to_load = self._num_items_to_load - 1
...
function PlayFabMirrorBase:_request_fix_inventory_data_1()
	local request = { FunctionName = "fixInventoryData1",
...
function PlayFabMirrorBase:fix_inventory_data_1_request_cb(result)
	self._num_items_to_load = self._num_items_to_load - 1
...
function PlayFabMirrorBase:_request_fix_inventory_data_2()
	local request = { FunctionName = "fixInventoryData2",
...
function PlayFabMirrorBase:fix_inventory_data_2_request_cb(result)
	self._num_items_to_load = self._num_items_to_load - 1
...
function PlayFabMirrorBase:_handle_fix_data_ids()
	local request = { FunctionName = "handleFixDataIds",
...
function PlayFabMirrorBase:handle_fix_data_ids_request_cb(result)
	self._num_items_to_load = self._num_items_to_load - 1
	local function_result = result.FunctionResult
	...
	if function_result.new_cosmetics then
		for i = 1, #function_result.new_cosmetics do
			self:add_item(nil, { ItemId = function_result.new_cosmetics [i] })
		end
	end
...
function PlayFabMirrorBase:_fix_excess_bogenhafen_chests()
	local request = { FunctionName = "removeExcessBogenhafenChests",
...
function PlayFabMirrorBase:_fix_excess_bogenhafen_chests_cb()
	self._num_items_to_load = self._num_items_to_load - 1
...
function PlayFabMirrorBase:_fix_excess_duplicate_bogenhafen_cosmetics()
	local request = { FunctionName = "removeDuplicateBogenhafenCosmetics",
...
function PlayFabMirrorBase:_fix_excess_duplicate_bogenhafen_cosmetics_cb()
	self._num_items_to_load = self._num_items_to_load - 1
...
function PlayFabMirrorBase:_request_read_only_data()
	local request = { FunctionName = "getReadOnlyData",
...
function PlayFabMirrorBase:read_only_data_request_cb(result)
	self._num_items_to_load = self._num_items_to_load - 1
...
function PlayFabMirrorBase:_request_user_data()
	local request = { }
...
function PlayFabMirrorBase:user_data_request_cb(result)
	self._num_items_to_load = self._num_items_to_load - 1
...
function PlayFabMirrorBase:_request_twitch_app_access_token()
...
function PlayFabMirrorBase:_request_twitch_app_access_token_cb(result)
	self._num_items_to_load = self._num_items_to_load - 1
...
function PlayFabMirrorBase:get_twitch_app_access_token()
	return self._twitch_app_access_token
end



function PlayFabMirrorBase:_weaves_player_setup()
	local request = { FunctionName = "weavesPlayerSetup",
...
function PlayFabMirrorBase:weaves_player_setup_request_cb(result)
	self._num_items_to_load = self._num_items_to_load - 1
...
function PlayFabMirrorBase:_fix_total_collected_essence()
	local request = { FunctionName = "fixTotalCollectedEssence",
...
function PlayFabMirrorBase:fix_total_collected_essence_cb(result)
	self._num_items_to_load = self._num_items_to_load - 1
...
function PlayFabMirrorBase:_request_win_tracks()
	local request = { FunctionName = "winTracksSetup",
...
function PlayFabMirrorBase:win_tracks_request_cb(result)
	self._num_items_to_load = self._num_items_to_load - 1
...
function PlayFabMirrorBase:get_win_tracks()
	return self._win_tracks
end



function PlayFabMirrorBase:_deus_player_setup()
	local request = { FunctionName = "deusPlayerSetup",
...
function PlayFabMirrorBase:deus_player_setup_request_cb(result)
	self._num_items_to_load = self._num_items_to_load - 1
...
function PlayFabMirrorBase:deus_refresh_belakor_data()
	self:_deus_setup_belakor_data()
end



function PlayFabMirrorBase:has_loaded_belakor_data()
	return self._belakor_data_loaded
end



function PlayFabMirrorBase:set_has_loaded_belakor_data(value)
	self._belakor_data_loaded = value
end



function PlayFabMirrorBase:_deus_setup_belakor_data()
	local request = { FunctionName = "deusSetBelakorCurse",
...
function PlayFabMirrorBase:deus_setup_belakor_data_request_cb(result)
...
function PlayFabMirrorBase:_set_up_additional_account_data(steps_completed)
	local request = { FunctionName = "additionalAccountDataSetUp",

		FunctionParameter = { steps_completed = steps_completed } }
...
function PlayFabMirrorBase:additional_data_setup_request_cb(result)
	self._num_items_to_load = self._num_items_to_load - 1
...
function PlayFabMirrorBase:_request_user_inventory()
	local request = { }
...
function PlayFabMirrorBase:inventory_request_cb(result)
	self._num_items_to_load = self._num_items_to_load - 1
...
function PlayFabMirrorBase:_request_steam_user_inventory()
...
function PlayFabMirrorBase:delete_playfab_characters_cb(result)
	self._num_items_to_load = self._num_items_to_load - 1
	self:request_characters()
end
...
function PlayFabMirrorBase:add_steam_items(item_list)
...
function PlayFabMirrorBase:_cb_steam_user_inventory(result, item_list, request_characters, skip_mark_as_new)
...
function PlayFabMirrorBase:_set_inital_career_data(career_name, character_data, slots_to_verify)
...
function PlayFabMirrorBase:_verify_items_are_usable(broken_slots, character_data, career_name, slots_to_verify)
...
function PlayFabMirrorBase:_update_data(item, backend_id)
	local custom_data = item.CustomData
...
function PlayFabMirrorBase:ready()
	return self._inventory_items and self._num_items_to_load == 0
end



function PlayFabMirrorBase:update(dt, t)
...
function PlayFabMirrorBase:_check_current_commit()
...
function PlayFabMirrorBase:_commit_status()
	local commit_current_id = self._commit_current_id
	...
	if commit_data.status == "commit_error" then
		do return "commit_error" end
	elseif commit_data.num_updates == commit_data.updates_to_make and
	not commit_data.wait_for_stats and
	not commit_data.wait_for_weave_user_data and
...
function PlayFabMirrorBase:request_queue()
...
function PlayFabMirrorBase:get_playfab_id()
...
function PlayFabMirrorBase:get_character_data(career_name, key)
...
function PlayFabMirrorBase:set_character_data(career_name, key, value, set_mirror)
	local career_data = self._career_data [career_name]
	career_data [key] = value
...
function PlayFabMirrorBase:get_title_data()
	return self._title_data
end



function PlayFabMirrorBase:set_title_data(key, value)
	if tonumber(value) then
		value = tonumber(value)
	end
...
function PlayFabMirrorBase:get_user_data(key)
	return self._user_data [key]
end



function PlayFabMirrorBase:set_user_data(key, value, set_mirror)
...
function PlayFabMirrorBase:log_player_exit(external_cb)
	local request = { FunctionName = "logPlayerExit",
...
function PlayFabMirrorBase:log_player_exit_cb(external_cb, result)
	external_cb(result)
end



function PlayFabMirrorBase:_commit_user_data(new_data, commit, commit_id)
...
function PlayFabMirrorBase:update_user_data_cb(commit_id, result)
...
function PlayFabMirrorBase:get_read_only_data(key)
...
function PlayFabMirrorBase:set_read_only_data(key, value, set_mirror)
	local value_type = type(value)
...
function PlayFabMirrorBase:merge_read_only_data(data, set_mirror)
...
function PlayFabMirrorBase:get_all_inventory_items()
	return self._inventory_items
end



function PlayFabMirrorBase:get_all_fake_inventory_items()
...
function PlayFabMirrorBase:get_stats()
	return self._stats
end



function PlayFabMirrorBase:set_stats(stats)
	self._stats = stats
end



function PlayFabMirrorBase:get_claimed_achievements()
	return self._claimed_achievements
end



function PlayFabMirrorBase:get_claimed_event_quests()
	return self._claimed_event_quests
end



function PlayFabMirrorBase:add_claimed_event_quest(quest_name)
	self._claimed_event_quests [quest_name] = true
end



function PlayFabMirrorBase:add_claimed_multiple_event_quests(quest_names)
...
function PlayFabMirrorBase:get_achievement_rewards()
	return self._achievement_rewards
end



function PlayFabMirrorBase:get_weaves_progression_settings()
	return self._weaves_progression_settings
end



function PlayFabMirrorBase:get_unlocked_weapon_skins()
...
function PlayFabMirrorBase:get_unlocked_cosmetics()
...
function PlayFabMirrorBase:get_unlocked_keep_decorations()
	return self._unlocked_keep_decorations
end



function PlayFabMirrorBase:get_owned_dlcs()
	return self._owned_dlcs
end



function PlayFabMirrorBase:get_platform_dlcs()
...
function PlayFabMirrorBase:set_owned_dlcs(owned_dlcs)
...
function PlayFabMirrorBase:set_platform_dlcs(platform_dlcs)
	self._platform_dlcs = platform_dlcs
end



function PlayFabMirrorBase:add_keep_decoration(decoration_name)
...
function PlayFabMirrorBase:_create_fake_inventory_items(fake_inventory_items, items_type)
...
function PlayFabMirrorBase:set_achievement_claimed(achievement_id)
...
function PlayFabMirrorBase:get_claimed_console_dlc_rewards()
	return self._claimed_console_dlc_rewards
end



function PlayFabMirrorBase:set_console_dlc_reward_claimed(reward_id, claimed)
...
function PlayFabMirrorBase:get_quest_data()
	return self._quest_data
end



function PlayFabMirrorBase:set_quest_data(key, value)
...
function PlayFabMirrorBase:check_for_errors()
	return end


local slot_type_to_best_power_level_key = { ranged = "best_ranged_pl", ring = "best_ring_pl", necklace = "best_necklace_pl", trinket = "best_trinket_pl", melee = "best_melee_pl" }
...
function PlayFabMirrorBase:_re_evaluate_best_power_level(item)
...
function PlayFabMirrorBase:_add_new_weapon_skin(item, generate_new_id, optional_skin_name)
...
function PlayFabMirrorBase:add_item(backend_id, item, skip_autosave, skip_mark_as_new)
	if not self._inventory_items then
		self._inventory_items = { }
	end
...
function PlayFabMirrorBase:remove_item(backend_id)
...
function PlayFabMirrorBase:update_item_field(backend_id, field, value)
	local inventory_items = self._inventory_items
...
function PlayFabMirrorBase:update_item(backend_id, new_item)
	local inventory_items = self._inventory_items
...
function PlayFabMirrorBase:add_unlocked_weapon_skin(weapon_skin, offline_backend_id)
	if self._unlocked_weapon_skins then
		...
		if existing_weapon_skin then
			return { existing_weapon_skin }
		end

		self._unlocked_weapon_skins [weapon_skin] = true
		return self:_create_fake_inventory_items({ [weapon_skin] = offline_backend_id or true }, "weapon_skins")
	else
...
function PlayFabMirrorBase:add_unlocked_cosmetic(cosmetic_name, offline_backend_id)
	if self._unlocked_cosmetics then
		local backend_id = self:_create_fake_inventory_items({ [cosmetic_name] = offline_backend_id or true }, "cosmetics")
		if #backend_id > 0 then
			self._unlocked_cosmetics [cosmetic_name] = backend_id [1]
			do return backend_id [1] end
		end
	else
...
function PlayFabMirrorBase:set_essence(amount)
	self._essence = amount
end



function PlayFabMirrorBase:get_essence()
	return self._essence
end



function PlayFabMirrorBase:set_total_essence(amount)
	self._total_essence = amount
end



function PlayFabMirrorBase:get_total_essence()
	return self._total_essence
end



function PlayFabMirrorBase:set_maximum_essence(amount)
	self._maximum_essence = amount
end



function PlayFabMirrorBase:get_maximum_essence()
...
function PlayFabMirrorBase:get_deus_rolled_over_soft_currency()
	return self._deus_rolled_over_soft_currency or 0
end



function PlayFabMirrorBase:get_deus_journey_cycle_data()
	return self._deus_journey_cycle_data
end



function PlayFabMirrorBase:get_deus_belakor_curse_data()
...
function PlayFabMirrorBase:handle_deus_result(result)
	local function_result = result.FunctionResult
...
function PlayFabMirrorBase:predict_deus_rolled_over_soft_currency(amount)
...
function PlayFabMirrorBase:predict_deus_run_started()
...
function PlayFabMirrorBase:predict_debug_clear_deus_meta_progression(amount)
	self._deus_rolled_over_soft_currency = 0
end
...
function PlayFabMirrorBase:commit(skip_queue, commit_complete_callback)
	local queued_commit = self._queued_commit
	local id = nil
...
function PlayFabMirrorBase:_new_id()
...
function PlayFabMirrorBase:_queue_commit(commit_complete_callback)
	local queued_commit = self._queued_commit
...
function PlayFabMirrorBase:_commit_internal(queue_id, commit_complete_callbacks)
	debug_printf("_commit_internal %q", queue_id)

	local commit_id = queue_id or self:_new_id()

	local commit = { num_updates = 0, status = "success", updates_to_make = 0,
	...
	if not script_data ["eac-untrusted"] then
		...
		if weave_user_data then
			local weave_user_data_request = { FunctionName = "updateWeaveUserData",
	...
	if win_tracks_interface then
		...
		if current_win_track_id ~= self._current_win_track_id then
			...
			local update_win_tracks_request = { FunctionName = "updateCurrentWinTrack",
	...
	if not table.is_empty(new_data) then
		local update_read_only_data_request = { FunctionName = "updateHeroAttributes",
...
function PlayFabMirrorBase:update_current_win_track_cb(commit_id, result)
	local commit = self._commits [commit_id]
...
function PlayFabMirrorBase:update_read_only_data_request_cb(commit_id, result)
	local commit = self._commits [commit_id]
	local function_result = result.FunctionResult
...
function PlayFabMirrorBase:save_statistics_cb(commit_id, stats, result)
	local commit = self._commits [commit_id]
	local stats_interface = Managers.backend:get_interface("statistics")
...
function PlayFabMirrorBase:update_weave_user_data_cb(commit_id, result)
	local commit = self._commits [commit_id]
...
function PlayFabMirrorBase:save_keep_decorations_cb(commit_id, on_complete, success)
	local commit = self._commits [commit_id]
...
function PlayFabMirrorBase:wait_for_shutdown(duration)
	return end



function PlayFabMirrorBase:destroy()
	return end



function PlayFabMirrorBase:_get_eac_response(challenge)
	local i = 0
	local str = ""
...
function PlayFabMirrorBase:_verify_dlc_careers()
	local request = { FunctionName = "verifyDlcCareers",
...
function PlayFabMirrorBase:verify_dlc_careers_cb(result)
	local function_result = result.FunctionResult
...
function PlayFabMirrorBase:_setup_careers()
...
function PlayFabMirrorBase:_fix_career_data(broken_slots_data, override_mechanism, override_cb_func)
	local request = { FunctionName = "fixCareerData",
...
function PlayFabMirrorBase:fix_career_data_request_cb(result)
...
local keys = { "slot_ranged", "slot_melee", "slot_skin", "slot_hat", "slot_necklace", "slot_ring", "slot_trinket_1", "slot_frame", "talents" }
...
function PlayFabMirrorBase:_check_career_data(careers_data, career_data_mirror)
	local characters_data = self._characters_data
	local characters_data_mirror = self._characters_data_mirror
	local dirty = false

	local ignore_keys = { "careers" }
...
function PlayFabMirrorBase:set_career_read_only_data(character, key, value, career, set_mirror)
	local characters_data = self._characters_data
...
function PlayFabMirrorBase:get_characters_data()
	return self._characters_data
end



function PlayFabMirrorBase:update_owned_dlcs(set_status_changed)
...
function PlayFabMirrorBase:handle_new_dlcs(new_dlcs)
...
function PlayFabMirrorBase:_snippet_clear_inventory()
	local function clear_inventory_cb(result)
		local broken_slots = { slot_necklace = true, slot_hat = true, slot_trinket_1 = true, slot_skin = true, slot_frame = true, slot_melee = true, slot_ring = true, slot_ranged = true }
	...
	self._request_queue [#self._request_queue + 1] = { eac_check = false, func = "devClearInventory",
...
function PlayFabMirrorBase:snippet_clear_inventory()
	self:_snippet_clear_inventory()
end



function PlayFabMirrorBase:set_twitch_app_access_token(access_token)
