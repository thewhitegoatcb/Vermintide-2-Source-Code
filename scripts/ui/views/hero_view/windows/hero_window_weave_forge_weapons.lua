...
function HeroWindowWeaveForgeWeapons:on_enter(params, offset)
	...
	if self._crafting_tutorial then
		...
		self._ui_animations.unlock_button_pulse = UIAnimation.init(UIAnimation.pulse_animation,
		unlock_button.style.texture_highlight.color, 1, 100, 255, 2)
	end
...
function HeroWindowWeaveForgeWeapons:_start_transition_animation(animation_name)
	local params = {
		parent = self._parent,
...
function HeroWindowWeaveForgeWeapons:_setup_weapon_list()
...
function HeroWindowWeaveForgeWeapons:_setup_definitions()
...
function HeroWindowWeaveForgeWeapons:create_ui_elements(params, offset)
...
function HeroWindowWeaveForgeWeapons:_initialize_viewports()
...
function HeroWindowWeaveForgeWeapons:_create_item_previewer(viewport_widget, item, activate_spin)
	...
	local preview_position = { 0, 2.5, 0 }
...
function HeroWindowWeaveForgeWeapons:cb_unit_spawned_item_preview(item_previewer, item_key, activate_spin)
	local ignore_spin = not activate_spin
	item_previewer:present_item(item_key, ignore_spin)
end


function HeroWindowWeaveForgeWeapons:_create_viewport_definition(scenegraph_id)
	local shading_environment = "environment/ui_weave_forge_preview"
	return {
		element = UIElements.Viewport,
		style = {
			viewport = { layer = 840, viewport_type = "default_forward", enable_sub_gui = false, fov = 20,
...
function HeroWindowWeaveForgeWeapons:on_exit(params)
...
function HeroWindowWeaveForgeWeapons:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function HeroWindowWeaveForgeWeapons:_update_button_visibility()
...
function HeroWindowWeaveForgeWeapons:post_update(dt, t)
...
function HeroWindowWeaveForgeWeapons:_update_animations(dt)
...
function HeroWindowWeaveForgeWeapons:_is_button_pressed(widget)
...
function HeroWindowWeaveForgeWeapons:_is_button_hover(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.is_hover
end

function HeroWindowWeaveForgeWeapons:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_enter and not hotspot.is_selected
end

function HeroWindowWeaveForgeWeapons:_is_button_hover_exit(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_exit and not hotspot.is_selected
end

function HeroWindowWeaveForgeWeapons:_is_button_selected(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.is_selected
end

function HeroWindowWeaveForgeWeapons:_list_index_pressed(widgets)
...
function HeroWindowWeaveForgeWeapons:_is_list_hovered(widget)
	return widget.content.hotspot.is_hover or false
end

function HeroWindowWeaveForgeWeapons:_sync_backend_loadout()
	local backend_manger = Managers.backend
	local backend_interface_items = backend_manger:get_interface("items")
	local backend_interface_weaves = backend_manger:get_interface("weaves")
...
function HeroWindowWeaveForgeWeapons:_play_sound(event)
	self._parent:play_sound(event)
end

function HeroWindowWeaveForgeWeapons:_handle_input(dt, t)
	local parent = self._parent
	local widgets_by_name = self._widgets_by_name
	local gamepad_active = Managers.input:is_device_active("gamepad")
...
function HeroWindowWeaveForgeWeapons:_on_list_index_selected(index)
	local scrollbars = self._scrollbars
	local scrollbar_data = scrollbars.weapons
	local list_widgets = scrollbar_data.list_widgets
...
function HeroWindowWeaveForgeWeapons:_update_equip_button_status(equipable_item, is_item_equipped)
	local viewport_data = self._viewport_data
	if viewport_data then
		local equip_button = viewport_data.equip_button
...
function HeroWindowWeaveForgeWeapons:_list_index_by_item_key(item_key)
	local scrollbars = self._scrollbars
	local scrollbar_data = scrollbars.weapons
	local list_widgets = scrollbar_data.list_widgets
	for i, widget in ipairs(list_widgets) do
...
function HeroWindowWeaveForgeWeapons:_present_item(item_key, activate_spin)
	local viewport_data = self._viewport_data
	...
	if not item then
		...
		fake_item = { data = item_data, key = item_key }
		locked = true
	end
...
function HeroWindowWeaveForgeWeapons:_set_presentation_locked_state(locked)
	local widgets_by_name = self._widgets_by_name
	local level_text_widget = widgets_by_name.viewport_level_value
...
function HeroWindowWeaveForgeWeapons:_set_essence_upgrade_cost(essence_cost, can_afford)
	local widgets_by_name = self._widgets_by_name
...
function HeroWindowWeaveForgeWeapons:_unlock_item(item_id)
...
function HeroWindowWeaveForgeWeapons:_unlock_item_cb(success)
...
function HeroWindowWeaveForgeWeapons:_on_unlock_item_done(success)
	local widgets_by_name = self._widgets_by_name
	local unlock_button = widgets_by_name.unlock_button
...
function HeroWindowWeaveForgeWeapons:_equip_item(backend_id)
	local career_name = self._career_name
	local backend_manger = Managers.backend
	local backend_interface_weaves = backend_manger:get_interface("weaves")
...
function HeroWindowWeaveForgeWeapons:_update_item_pulse_animation(dt)
...
function HeroWindowWeaveForgeWeapons:_draw(dt)
...
function HeroWindowWeaveForgeWeapons:_populate_list(layout)
...
function HeroWindowWeaveForgeWeapons:_align_list_widgets(widgets, spacing)
...
function HeroWindowWeaveForgeWeapons:_initialize_scrollbar(scrollbar_widget, content_length, list_scenegraph_id, spacing)
...
function HeroWindowWeaveForgeWeapons:_update_scrollbar_positions()
	local scrollbars = self._scrollbars
	if scrollbars then
		local ui_scenegraph = self._ui_scenegraph
...
function HeroWindowWeaveForgeWeapons:_update_visible_list_entries()
	local scrollbars = self._scrollbars
	if scrollbars then
		local ui_scenegraph = self._ui_scenegraph
		for _, data in pairs(scrollbars) do
...
function HeroWindowWeaveForgeWeapons:_get_scrollbar_percentage_by_index(scrollbar_key, index)
	local scrollbars = self._scrollbars
...
function HeroWindowWeaveForgeWeapons:_find_closest_neighbour(scrollbar_key, column_index_list, current_index)
	local scrollbars = self._scrollbars
	local scrollbar_data = scrollbars [scrollbar_key]
	local list_widgets = scrollbar_data.list_widgets
...
function HeroWindowWeaveForgeWeapons:_animate_weapon_lists_widgets(widgets, dt, is_list_hovered)
	for index, widget in ipairs(widgets) do
		self:_animate_list_widget(widget, dt, is_list_hovered)
	end
end

function HeroWindowWeaveForgeWeapons:_animate_list_widget(widget, dt, optional_hover)
	local offset = widget.offset
	local content = widget.content
	local style = widget.style
...
function HeroWindowWeaveForgeWeapons:_setup_weapon_stats(item)
	local career_name = self._career_name
	local item_data = item.data
	...
	local divider_size = { 0, entry_height }
	local item_option_size = { 0, entry_height }
	...
	if traits then
		...
		local trait_size = { 0, entry_height }
		for trait_key, _ in pairs(traits) do
	...
	if properties then
		...
		local property_size = { 0, entry_height }
...
function HeroWindowWeaveForgeWeapons:_create_item_keywords_option_entry(size, text)
	local masked = true
	local ui_renderer = self._ui_renderer
...
function HeroWindowWeaveForgeWeapons:_create_item_overheat_option_entry(size)
	local masked = true
	local ui_renderer = self._ui_renderer
	local scenegraph_id = "stat_option"
...
function HeroWindowWeaveForgeWeapons:_create_item_ammunition_option_entry(size, amount)
	local masked = true
	local ui_renderer = self._ui_renderer
	local scenegraph_id = "stat_option"
...
function HeroWindowWeaveForgeWeapons:_create_item_stamina_option_entry(size, amount)
	local masked = true
	local ui_renderer = self._ui_renderer
	local scenegraph_id = "stat_option"
...
function HeroWindowWeaveForgeWeapons:_create_item_block_option_entry(size, angle)
...
function HeroWindowWeaveForgeWeapons:_create_divider_option_entry(size, title_text)
	local masked = true
	local ui_renderer = self._ui_renderer
	local scenegraph_id = "stat_option"
...
function HeroWindowWeaveForgeWeapons:_create_trait_option_entry(size, title_text, description_text, icon)
	local masked = true
	local ui_renderer = self._ui_renderer
	local scenegraph_id = "stat_option"
...
function HeroWindowWeaveForgeWeapons:_create_property_option_entry(size, text, value_string, icon)
	local masked = true
	local scenegraph_id = "stat_option"
