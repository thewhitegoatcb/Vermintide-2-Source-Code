...
local EMPTY_LIST = { }
function ImguiTeleportTool:init()
	...
	self._key_bindings = { select_down = "down", teleport = "home", quick_teleport = "home", teleport_from_clipboard = "numpad enter", confirm = "enter", register_new = "insert", delete = "delete", select_up = "up", save_to_clipboatd = "numpad 0" }
	...
	self._custom_target_point = { 0, 0, 0, 0, 0, 0, 1 }
...
function ImguiTeleportTool:update()
...
function ImguiTeleportTool:is_persistent()
	return self._is_persistent
end

function ImguiTeleportTool:draw(is_open)
...
function ImguiTeleportTool:_update_input()
	for name, val in pairs(self._key_bindings) do
...
function ImguiTeleportTool:_handle_input()
...
function ImguiTeleportTool:_refresh_filter()
...
function ImguiTeleportTool:_apply_filter(filter_text, names)
	if filter_text == "" then
		return names
	end
...
function ImguiTeleportTool:_register_point(name)
	local level_key = self._current_level
...
function ImguiTeleportTool:_get_unit_location(unit)
	if Unit.alive(unit) then
		...
		local data_point = { pos_x, pos_y, pos_z, rot_x, rot_y, rot_z, rot_w }
		return data_point
	end
	return nil
end

function ImguiTeleportTool:_get_selected_teleport_coords()
	local level_key = self._current_level
...
function ImguiTeleportTool:_teleport_to_selected(unit)
	local coords = self:_get_selected_teleport_coords()
	self:_teleport_to_point(unit, coords)
end

function ImguiTeleportTool:_teleport_to_point(unit, point)
	if point and Unit.alive(unit) then
		...
		local ctp = self._custom_target_point
		ctp [3] = point [3] ctp [2] = point [2] ctp [1] = point [1]
		if locomotion then
...
function ImguiTeleportTool:_get_teleport_names(level_key)
	return level_key and self._teleport_name_map [level_key] or EMPTY_LIST
end

function ImguiTeleportTool:_get_selected_teleport_id()
...
function ImguiTeleportTool:_save_points()
...
function ImguiTeleportTool:_load_points()
...
function ImguiTeleportTool:_save_point_to_clipboard(data_point)
	if data_point then
...
function ImguiTeleportTool:_get_point_from_clipboard()
