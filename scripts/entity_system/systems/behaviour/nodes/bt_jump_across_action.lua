...
local function debug_graph()
	...
	if graph == nil then
		graph = Managers.state.debug.graph_drawer:create_graph("BTJumpAcrossAction", { "time", "unit altitude" })
	end
	return graph
end
...
function BTJumpAcrossAction:init(...)
	BTJumpAcrossAction.super.init(self, ...)
end

BTJumpAcrossAction.name = "BTJumpAcrossAction"

function BTJumpAcrossAction:enter(unit, blackboard, t)
	local drawer = Managers.state.debug:drawer({ mode = "retained", name = "BTJumpAcrossAction" })
	drawer:reset()
	...
	if script_data.ai_debug_smartobject then
		...
		debug_graph():add_annotation({ color = "green", x = t, y = unit_position.z, text = "starting BTJumpAcrossAction" .. tostring(unit_position) })
	else
		debug_graph():set_active(false)
	end
end

function BTJumpAcrossAction:leave(unit, blackboard, t, reason, destroy)
...
function BTJumpAcrossAction:run(unit, blackboard, t, dt)
	local navigation_extension = blackboard.navigation_extension
	local locomotion_extension = blackboard.locomotion_extension
	local unit_position = POSITION_LOOKUP [unit]
	...
	if blackboard.jump_state == "moving_to_ledge" and
	blackboard.is_in_smartobject_range then
	...
	if blackboard.jump_state == "moving_towards_smartobject_entrance" then
		...
		if distance_to_target > 0.1 then
			...
			if script_data.ai_debug_smartobject then
				local drawer = Managers.state.debug:drawer({ mode = "immediate", name = "BTJumpAcrossAction2" })
	...
	if blackboard.jump_state == "waiting_to_reach_end" and
	blackboard.jump_start_finished then
...
function BTJumpAcrossAction:_debug_draw_update(unit, blackboard, t)
	local drawer = Managers.state.debug:drawer({ mode = "immediate", name = "BTJumpAcrossAction2" })
	local unit_position = POSITION_LOOKUP [unit]
