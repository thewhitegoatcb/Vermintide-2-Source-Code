...
function ChallengeTrackerUI:init(parent, ingame_ui_context)
	self._ui_renderer = ingame_ui_context.ui_renderer
	self._wwise_world = ingame_ui_context.wwise_world
	self:_create_ui_elements()
end

function ChallengeTrackerUI:destroy()
...
function ChallengeTrackerUI:_play_sound(sound_event)
	return WwiseWorld.trigger_event(self._wwise_world, sound_event)
end


function ChallengeTrackerUI:_create_ui_elements()
	self:destroy()
...
function ChallengeTrackerUI:_refresh_challenge_data(data)
...
function ChallengeTrackerUI:_cb_on_done(widget, challenge)
...
function ChallengeTrackerUI:_play_animation(name, widget, initial_delay)
...
function ChallengeTrackerUI:_play_animation_queued(name, widget, initial_delay)
	local animator = self._ui_animator
	...
	if animator:is_animation_completed(current_animation_id) then
		...
		queue [#queue + 1] = { name = name, initial_delay = initial_delay }
		self._animation_queue [widget] = queue
	end
end

function ChallengeTrackerUI:_update_animation_queue()
	local animator = self._ui_animator
...
function ChallengeTrackerUI:_update_restacking(dt)
	local restack_targets = self._restack_targets
...
function ChallengeTrackerUI:_set_widget_dirty(widget)
	widget.element.dirty = true
	self._dirty = true
end

function ChallengeTrackerUI:_update_animations(dt, t)
...
function ChallengeTrackerUI:_handle_resolution_modified()
...
local customizer_data = { lock_y = false, registry_key = "questingknight", drag_scenegraph_id = "quest", root_scenegraph_id = "quest", label = "Duties", lock_x = false }
...
function ChallengeTrackerUI:update(dt, t)
...
function ChallengeTrackerUI:_draw(dt)
