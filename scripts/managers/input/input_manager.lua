...
local function dprint(...) if script_data.input_debug_filters then printf(...) end end
...
function InputManager:initialize_device(input_device_type, input_device_slot)
...
function InputManager:remove_all_devices(input_device_type)
	local device_list = InputAux.input_device_mapping [input_device_type]
...
function InputManager:set_exclusive_gamepad(input_device)
...
function InputManager:set_all_gamepads_available()
	local input_device_type = "gamepad"
...
function InputManager:block_device_except_service(service_exception, device_type, device_index, block_reason)
...
function InputManager:device_unblock_all_services(device_type, device_index)
...
function InputManager:device_block_service(device_type, device_index, service_name, block_reason)
...
function InputManager:device_unblock_service(device_type, device_index, service_name)
...
function InputManager:get_unblocked_services(device_type, device_index, services_dest)
	local services_n = 0
...
function InputManager:get_blocked_services(device_type, device_index, services_dest)
	local services_n = 0
	for service_name, service in pairs(self.input_services) do
...
function InputManager:device_block_services(device_type, device_index, services, services_n, block_reason)
	device_index = device_index or 1
...
function InputManager:device_unblock_services(device_type, device_index, services, services_n)
	device_index = device_index or 1
	for i = 1, services_n do
		local service_name = services [i]
...
function InputManager:capture_input(device_types, device_index, service_name, capture_owner)
...
function InputManager:release_input(device_types, device_index, service_name, capture_owner, block_reason)
	if not device_types then
		return
	end
	if not device_index then
...
function InputManager:_find_service_input_group(service_name)
...
function InputManager:_refresh_active_input_group()
	local device_input_groups = self._device_input_groups
...
function InputManager:_capture_input_group(active_input_group)
...
function InputManager:_update_service_input_group(service, active_input_group)
	if not service then
		return
	end
...
function InputManager:_find_active_input_group_id(device_input_groups)
...
function InputManager:create_input_service(input_service_name, keymaps_name, filters_name, block_reasons)
...
function InputManager:get_input_service(input_service_name)
	return self.input_services [input_service_name]
end

function InputManager:get_active_input_service_by_device(device_name)
	for service_name, service in pairs(self.input_services) do
		if not service:is_blocked() then
...
function InputManager:map_device_to_service(input_service_name, input_device_type, input_device_slot)
...
function InputManager:update_devices(dt, t)
	...
	for input_device, device_data in pairs(input_devices) do
		...
		for key = 0, device_data.num_axes - 1 do
			...
			if IS_PS4 or is_ps_pad then
				...
				local valid_axis = { left = true, right = true }
...
function InputManager:get_service(input_service_name)
	if self.input_services then
		do return self.input_services [input_service_name] end
	else
...
function InputManager:get_device(input_device_type, input_device_slot)
	if gamepad_disabled and input_device_type == "gamepad" then
...
function InputManager:any_input_pressed()
	return self.any_device_input_pressed
end
function InputManager:any_input_released()
	return self.any_device_input_released
end

function InputManager:any_input_axis_moved()
	return self.any_device_input_axis_moved
end

function InputManager:get_most_recent_device()
	return most_recent_input_device
end

function InputManager:get_most_recent_device_type()
	return most_recent_input_device_type
end

function InputManager:is_device_active(input_device_type)
	if
	...
	input_device_type == "gamepad" and gamepad_disabled then
		return false
	end
...
function InputManager:add_filters_data(filters, name)
...
function InputManager:update_filters_data(filters, name)
	local stored_filters_data = self.stored_filters_data
...
function InputManager:setup_filters(filters)
	local input_filters = { }
	if filters then
...
function InputManager:filters_data(name)
	local stored_filters_data = self.stored_filters_data
...
function InputManager:apply_saved_keymaps(specific_table_name)
	...
	if IS_WINDOWS or IS_XB1 or IS_LINUX then
		...
		for keybinding_table_name, keybinding_table in pairs(keymaps) do
			if
			( not specific_table_name or specific_table_name == keybinding_table_name ) and stored_keymaps_data [keybinding_table_name] then
...
function InputManager:set_hovering(is_hovering)
	if is_hovering and not self._hovering then --[[ Nothing --]]
	end
...
function InputManager:set_gamepad_cursor_pos(pos_x, pos_y)
	GAMEPAD_CURSOR_POS [1] = pos_x
	GAMEPAD_CURSOR_POS [2] = pos_y
end

function InputManager:center_gamepad_cursor_pos()
	GAMEPAD_CURSOR_POS [1] = 960
	GAMEPAD_CURSOR_POS [2] = 540
end

function InputManager:get_gamepad_cursor_pos()
...
function InputManager:disable_gamepad_cursor()
	self._gamepad_cursor_active = false
end

function InputManager:enable_gamepad_cursor()
	self._gamepad_cursor_active = true
end

function InputManager:gamepad_cursor_active()
	return self._gamepad_cursor_active
end

function InputManager:is_hovering()
	return self._hovering
end

function InputManager:is_frame_hovering()
	return self._frame_hovering
end

function InputManager:set_showing_tooltip(showing_tooltip)
	self._showing_tooltip = showing_tooltip
end

function InputManager:is_showing_tooltip()
	return self._showing_tooltip
end

function InputManager:add_keymaps_data(keymaps, name)
	local stored_keymaps_data = self.stored_keymaps_data
...
function InputManager:update_keymaps_data(keymaps, name)
	local stored_keymaps_data = self.stored_keymaps_data
...
function InputManager:keymaps_data(name)
	local stored_keymaps_data = self.stored_keymaps_data
...
function InputManager:setup_keymaps(keymaps)
...
function InputManager:clear_keybinding(keybinding_table_name, keybinding_table_key, keymap_name)
...
function InputManager:change_keybinding(keybinding_table_name, keybinding_table_key, keymap_name, ...)
	local keymaps_data = self:keymaps_data(keybinding_table_name)
	assert(keymaps_data, "No keymaps data found under table_name: %s", keybinding_table_name)
	local keymaps_sub_data = keymaps_data [keybinding_table_key]
	assert(keymaps_sub_data, "No keymaps data found under table_key: %s", keybinding_table_key)
...
function InputManager:add_keybinding(keybinding_table_name, keybinding_table_key, keymap_name, ...)
	assert(type(new_button_index) == "number", "New button index must be a number.")
	local keymaps_data = self:keymaps_data(keybinding_table_name)
	assert(keymaps_data, "No keymaps data found under table_name: %s", keybinding_table_name)
	local keymaps_sub_data = keymaps_data [keybinding_table_key]
