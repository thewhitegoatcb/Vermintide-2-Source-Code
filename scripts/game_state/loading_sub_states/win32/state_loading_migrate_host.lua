...
function StateLoadingMigrateHost:on_enter(params)
...
function StateLoadingMigrateHost:_init_params(params)
...
function StateLoadingMigrateHost:_init_network()
	...
	if host_peer_id == Network.peer_id() then
		...
		if host_migration_info.level_data then
			...
			level_data.environment_variation_id,
			level_data.level_seed,
			level_data.mechanism,
			level_data.game_mode_key,
			level_data.conflict_settings,
			level_data.locked_director_functions,
			level_data.difficulty,
			level_data.difficulty_tweak,


			level_data.extra_packages)
	...
	elseif IS_XB1 then
		...
		print("#################################")
		print(host_to_migrate_to.session_id, host_to_migrate_to.session_template_name)
		self.parent.parent.loading_context.join_lobby_data = { name = host_to_migrate_to.session_id, session_template_name = host_to_migrate_to.session_template_name }
		...
		local requirements = { free_slots = 1, distance_filter = "world",


			filters = {
				host = { comparison = "equal",
					value = host_peer_id } },
...
function StateLoadingMigrateHost:update(dt, t)
	...
	if self._server_created or self._lobby_joined then
		do return StateLoadingRunning end
	elseif IS_XB1 and self.parent:lobby_verified() then
		return StateLoadingRunning
	end
end

function StateLoadingMigrateHost:on_exit(application_shutdown)
	self.parent.parent.loading_context.host_migration_info = nil
end

function StateLoadingMigrateHost:cb_server_created()
...
function StateLoadingMigrateHost:cb_lobby_joined()
