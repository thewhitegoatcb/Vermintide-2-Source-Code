...
function ActionChargedSweep:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionChargedSweep:client_owner_start_action(new_action, t, chain_action_data, power_level, action_init_data)
...
function ActionChargedSweep:client_owner_post_update(dt, t, world, can_damage, current_time_in_action)
	ActionChargedSweep.super.client_owner_post_update(self, dt, t, world, can_damage, current_time_in_action)
end

function ActionChargedSweep:finish(reason)
	ActionChargedSweep.super.finish(self, reason)
end

function ActionChargedSweep:get_overcharge_level(value)
	local level = 1
...
function ActionChargedSweep:get_discharge_effect(action, overcharge_level)
...
function ActionChargedSweep:apply_overcharge(overcharge_type, consume)
...
function ActionChargedSweep:_send_attack_hit(t, damage_source_id, attacker_unit_id, hit_unit_id, hit_zone_id, hit_position, attack_direction, damage_profile_id, ...)
	local first_alive_hit = false
	local hit_unit = nil
	if self._time_to_hit < t and
	self._number_of_hit_enemies == 1 then
...
function ActionChargedSweep:_apply_discharge_effect(discharge_effect, damage_source_id, hit_unit, hit_position)
	if discharge_effect then
...
function ActionChargedSweep:_get_damage_profile_name(action_hand, action)
