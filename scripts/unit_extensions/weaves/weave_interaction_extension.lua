...
function WeaveInteractionExtension:init(extension_init_context, unit, extension_init_data)
...
function WeaveInteractionExtension:get_objective_settings()
	return WeaveObjectiveSettings [WeaveInteractionExtension.NAME]
end

function WeaveInteractionExtension:score()
	return self._score
end

function WeaveInteractionExtension:activate(game_object_id, objective_data)
...
function WeaveInteractionExtension:complete()
...
function WeaveInteractionExtension:deactivate()
	if self._is_server then
...
function WeaveInteractionExtension:objective_name()
	return self._objective_name
end

function WeaveInteractionExtension:cb_game_session_disconnect()

	return end

function WeaveInteractionExtension:update(dt, t)
...
function WeaveInteractionExtension:_server_update(dt, t)
	...
	elseif interaction_result ~= self._interactable_state and self._interactable_state ~= 0 then
		...
		elseif ( interaction_result == InteractionResult.FAILURE or interaction_result == InteractionResult.USER_ENDED ) and
		self._on_interact_interupt_func then
...
function WeaveInteractionExtension:_client_update(dt, t)
	return end

function WeaveInteractionExtension:is_done()
	return self._num_times_to_complete <= self._interactable_system.num_times_successfully_completed
end

function WeaveInteractionExtension:get_percentage_done()
	return math.clamp(self._value / self._max_value, 0, 1)
end

function WeaveInteractionExtension:get_score()
	return self._score
end