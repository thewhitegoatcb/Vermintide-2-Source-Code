...
local POISON_DAMAGE_TYPES = { aoe_poison_dot = true, poison = true, arrow_poison = true, arrow_poison_dot = true }
...
local POISON_DAMAGE_SOURCES = { skaven_poison_wind_globadier = true, poison_dot = true }
...
local function is_immune_to_damage(attacked_unit, damage_type, damage_source, charge_value)
	...
	if victim_buff_extension then
		if victim_buff_extension:has_buff_perk("invulnerable") or
		RangedAttackTypes [charge_value] and victim_buff_extension:has_buff_perk("invulnerable_ranged") then
			return true
		end
...
function DamageUtils.get_breed_damage_multiplier_type(breed, hit_zone_name)
...
local function get_clamped_curve_value(curve, index)
	if index < 1 then
		do return curve [1] end
	elseif index >= #curve then
		do return curve [#curve] end
	else
		return curve [index]
	end
end

function DamageUtils.get_boost_curve_multiplier(curve, percent)
...
local function do_damage_calculation(attacker_unit, damage_source, original_power_level, damage_output, hit_zone_name, damage_profile, target_index, boost_curve, boost_damage_multiplier, is_critical_strike, backstab_multiplier, breed, range_scalar_multiplier, static_base_damage, is_friendly_fire, has_power_boost, difficulty_level, target_unit_armor, target_unit_primary_armor, has_crit_head_shot_killing_blow_perk, has_crit_backstab_killing_blow_perk, target_max_health, target_unit)
	...
	if not static_base_damage then
		...
		if is_critical_strike then
			...
			elseif backstab_multiplier and backstab_multiplier > 1 and has_crit_backstab_killing_blow_perk and
			damage_profile.charge_value == "heavy_attack" then
				killing_blow_triggered = true
			end
		end
...
function DamageUtils.calculate_damage_tooltip(attacker_unit, damage_source, original_power_level, hit_zone_name, damage_profile, target_index, boost_curve, boost_damage_multiplier, is_critical_strike, backstab_multiplier, breed, range_scalar_multiplier, has_power_boost, difficulty_level, target_unit_armor, target_unit_primary_armor)
	...
	local calculated_damage = do_damage_calculation(attacker_unit, damage_source, original_power_level, damage_output, hit_zone_name, damage_profile, target_index,
	boost_curve, boost_damage_multiplier, is_critical_strike, backstab_multiplier, breed,
	range_scalar_multiplier, static_base_damage, is_friendly_fire, has_power_boost, difficulty_level, target_unit_armor, target_unit_primary_armor,
	has_crit_head_shot_killing_blow_perk, has_crit_backstab_killing_blow_perk)
	calculated_damage = DamageUtils.networkify_damage(calculated_damage)
	return calculated_damage
end

function DamageUtils.calculate_dot_buff_damage(target_unit, attacker_unit, hit_zone, damage_source, power_level, damage_profile_name, template)
	...
	return DamageUtils.calculate_damage(DamageOutput, target_unit, attacker_unit, hit_zone, power_level, boost_curve, boost_curve_multiplier,
	is_critical_strike, damage_profile, target_index, backstab_multiplier, damage_source)
end

function DamageUtils.calculate_damage(damage_output, target_unit, attacker_unit, hit_zone_name, original_power_level, boost_curve, boost_damage_multiplier, is_critical_strike, damage_profile, target_index, backstab_multiplier, damage_source)



	local difficulty_settings = Managers.state.difficulty:get_difficulty_settings()
	local breed, armor_override, unit_max_health = nil
	...
	local calculated_damage = do_damage_calculation(attacker_unit, damage_source, original_power_level, damage_output, hit_zone_name, damage_profile, target_index,
	boost_curve, boost_damage_multiplier, is_critical_strike, backstab_multiplier, breed, range_scalar_multiplier,
	static_base_damage, is_friendly_fire, has_power_boost, difficulty_level, target_unit_armor, target_unit_primary_armor,
	has_crit_head_shot_killing_blow_perk, has_crit_backstab_killing_blow_perk, unit_max_health, target_unit)
...
local function do_stagger_calculation(stagger_table, breed, blackboard, attacker_unit, target_unit, hit_zone_name, original_power_level, boost_curve_multiplier, is_critical_strike, damage_profile, target_index, blocked, damage_source, range_scalar_multiplier, ai_shield_extension, difficulty_level, shield_user, has_power_boost, override_target_armor)
	...
	elseif stagger_table and not breed.stagger_immune then
		...
		if breed then
			...
			local stagger_resistance = breed.diff_stagger_resist and breed.diff_stagger_resist [difficulty_rank] or
			is_ranged and breed.stagger_resistance_ranged or breed.stagger_resistance or 2
			if target_buff_extension then
	...
	if breed.boss_staggers and (stagger_type < stagger_types.explosion or stagger_type == stagger_types.pulling) or
	breed.small_boss_staggers and stagger_type == stagger_types.pulling then
		stagger_type = stagger_types.none
	end
	...
	if
	ignore_staggers and attacker_buff_extension and attacker_buff_extension:has_buff_type("push_increase") then
...
local EMPTY_TABLE = { }
function DamageUtils.calculate_stagger_player_tooltip(breed, attacker_unit, hit_zone_name, original_power_level, is_critical_strike, damage_profile, target_index, blocked, damage_source, difficulty_level, has_power_boost, range_scalar_multiplier, target_armor_type)
...
function DamageUtils.calculate_stagger_player(stagger_table, target_unit, attacker_unit, hit_zone_name, original_power_level, boost_curve_multiplier, is_critical_strike, damage_profile, target_index, blocked, damage_source)
...
function DamageUtils.calculate_stagger(impact_table, duration_table, target_unit, attacker_unit, attack_template, hit_zone_name, blocked)
	local blackboard = BLACKBOARDS [target_unit]
	local breed = blackboard.breed
...
function DamageUtils.is_player_unit(unit)
	return Managers.player:is_player_unit(unit)
end

function DamageUtils.stagger_player(unit, breed, stagger_direction, stagger_length, stagger_type, stagger_duration, stagger_animation_scale, t, stagger_value, always_stagger, is_push, should_play_push_sound)
...
function DamageUtils.hit_zone(unit, actor)
	local breed = AiUtils.unit_breed(unit)
	if breed then
		local node = actor_node(actor)
		local hit_zone = breed.hit_zones_lookup [node]
		local hit_zone_name = hit_zone.name
		do return hit_zone_name end
	else
		return "full"
	end
end

function DamageUtils.aoe_hit_zone(unit, actor)
	local breed = AiUtils.unit_breed(unit)
	if breed then
		local node = actor_node(actor)
		local hit_zone = breed.hit_zones_lookup [node]
		local hit_zone_name = hit_zone and hit_zone.name
		do return hit_zone_name == "afro" and "afro" or "torso" end
	else
		return "full"
	end
end

function DamageUtils.draw_aoe_size(target_unit, breed)
...
function DamageUtils.calculate_aoe_size(target_unit, breed)
...
function DamageUtils.create_explosion(world, attacker_unit, impact_position, rotation, explosion_template, scale, damage_source, is_server, is_husk, damaging_unit, attacker_power_level, is_critical_strike, source_attacker_unit)
	...
	if is_server and attacker_unit_alive then
		...
		explosion_angle =
		explosion_angle and math.cos(explosion_angle / 2)
		...
		for i = 1, num_hits do
			...
			if damage_unit then
				...
				push_speed =
				push_speed and math.auto_lerp(max_damage_radius, radius, push_speed, 1, math.clamp(hit_distance, max_damage_radius, radius))
...
function DamageUtils.create_taunt(world, owner_unit, projectile_unit, position, aoe_data)
	...
	local extension_init_data = {
		...
		death_system = { death_reaction_template = "lure_unit" } }
...
function DamageUtils.create_aoe(world, attacker_unit, position, damage_source, explosion_template, radius, duration)
	...
	local extension_init_data = {
		area_damage_system = { invisible_unit = true, aoe_dot_damage = 0,
...
function DamageUtils.networkify_damage(damage_amount)
...
function DamageUtils.networkify_health(health_amount)
...
function DamageUtils.create_hit_zone_lookup(unit, breed)
	...
	for zone_name, zone in pairs(hit_zones) do
		for k, actor_name in ipairs(zone.actors) do
			...
			local node = actor_node(actor)
			hit_zones_lookup [node] = { name = zone_name, prio = zone.prio, actor_name = actor_name }
...
function DamageUtils.add_damage_network(attacked_unit, attacker_unit, original_damage_amount, hit_zone_name, damage_type, hit_position, damage_direction, damage_source, hit_ragdoll_actor, source_attacker_unit, buff_attack_type, hit_react_type, is_critical_strike, added_dot, first_hit, total_hits, backstab_multiplier, skip_buffs)
	...
	if DamageUtils.is_in_inn then
		...
		if
		attacked_side and attacked_side.VALID_ENEMY_PLAYERS_AND_BOTS [attacked_unit] then
...
function DamageUtils.get_damage_type(damage_profile, target_index)
	local target_settings = damage_profile.targets and damage_profile.targets [target_index] or damage_profile.default_target
	local attack_template_name = target_settings.attack_template
	local attack_template = AttackTemplates [attack_template_name]
...
function DamageUtils.add_damage_network_player(damage_profile, target_index, power_level, hit_unit, attacker_unit, hit_zone_name, hit_position, attack_direction, damage_source, hit_ragdoll_actor, boost_curve_multiplier, is_critical_strike, added_dot, first_hit, total_hits, backstab_multiplier, source_attacker_unit)
...
local HIT_CRITICAL_MULTIPLIER_TYPES = { "headshot", "weakspot" }
function DamageUtils.handle_hit_indication(attacker_unit, victim_unit, damage_amount, hit_zone_name, added_dot, invulnerable, shield_break)

	local hud_extension = ScriptUnit.has_extension(attacker_unit, "hud_system")
	if

	hud_extension and HEALTH_ALIVE [victim_unit] and attacker_unit ~= victim_unit then
...
function DamageUtils.get_item_buff_type(damage_source)
	local item_data = rawget(ItemMasterList, damage_source)
...
function DamageUtils.buff_on_attack(unit, hit_unit, attack_type, is_critical, hit_zone_name, target_number, send_to_server, buff_type, unmodified, damage_source)
...
local IGNORED_SHARED_DAMAGE_TYPES = { wounded_dot = true, suicide = true, knockdown_bleed = true }





local INVALID_DAMAGE_TO_OVERHEAT_DAMAGE_SOURCES = { temporary_health_degen = true, overcharge = true, life_tap = true, ground_impact = true, life_drain = true }
...
local INVALID_GROMRIL_DAMAGE_SOURCE = { temporary_health_degen = true, overcharge = true, life_tap = true, ground_impact = true, life_drain = true }
...
function DamageUtils.apply_buffs_to_damage(current_damage, attacked_unit, attacker_unit, damage_source, victim_units, damage_type, buff_attack_type, first_hit)
	...
	if ScriptUnit.has_extension(attacked_unit, "buff_system") then
		...
		if attacker_unit_buff_extension then
			local has_burning_perk = attacker_unit_buff_extension:has_buff_perk("burning") or
			attacker_unit_buff_extension:has_buff_perk("burning_balefire") or
			attacker_unit_buff_extension:has_buff_perk("burning_elven_magic")
		...
		if breed and (breed.boss or breed.elite) then
			...
			min_damage_cap =
			boss_elite_damage_cap and all_damage_cap and math.min(boss_elite_damage_cap, all_damage_cap) or boss_elite_damage_cap and boss_elite_damage_cap or
		...
		if talent_extension and talent_extension:has_talent("bardin_ranger_reduced_damage_taken_headshot") then
			...
			if
			has_position and AiUtils.unit_is_flanking_player(attacker_unit, attacked_unit) and not buff_extension:has_buff_type("bardin_ranger_reduced_damage_taken_headshot_buff") then
...
function DamageUtils.apply_damage_to_overcharge(unit, damage)
...
function DamageUtils.assist_shield_network(shielded_unit, shielder_unit, shield_amount)
...
local HEALED_UNITS = { }
function DamageUtils.heal_network(healed_unit, healer_unit, heal_amount, heal_type)
...
function DamageUtils.apply_buffs_to_heal(healed_unit, healer_unit, heal_amount, heal_type, healed_units)
...
function DamageUtils.debug_heal(unit, amount)
...
function DamageUtils.debug_deal_damage(victim_unit, damage)
...
function DamageUtils.check_distance(action, blackboard, attacking_unit, target_unit)
	local breed = blackboard.breed
...
function DamageUtils.check_infront(attacking_unit, target_unit)
	local pos_attacker = POSITION_LOOKUP [attacking_unit]
	local pos_target = POSITION_LOOKUP [target_unit] or Unit.world_position(target_unit, 0)
...
function DamageUtils.check_block(attacking_unit, target_unit, fatigue_type, original_enemy_weapon_direction)
	...
	if status_extension then
		...
		if is_blocking and can_block and not is_invulnerable then
			...
			Managers.player.is_server then
				...
				local is_player = blackboard.is_player and not ai_extension
				local action = is_player and status_extension:breed_action() or blackboard.action
				if action and action.no_block_stagger then --[[ Nothing --]]
...
function DamageUtils.check_ranged_block(attacking_unit, target_unit, attack_direction, fatigue_type)
...
function DamageUtils.camera_shake_by_distance(shake_name, start_time, player_unit, source_unit, near_dist, far_dist, near_value, far_value)
	local local_player = Managers.player:local_player()
...
function DamageUtils.is_character(unit)
	local has_breed = Unit.has_data(unit, "breed")
	return has_breed or false
end

function DamageUtils.can_bots_damage(unit)
...
function DamageUtils.allow_friendly_fire_ranged(difficulty_settings, attacker_player)
	return difficulty_settings.friendly_fire_ranged and attacker_player and not attacker_player.bot_player
end

function DamageUtils.allow_friendly_fire_melee(difficulty_settings, attacker_player)
	return difficulty_settings.friendly_fire_melee and not attacker_player.bot_player
end

function DamageUtils.damage_level_unit(hit_unit, attacker_unit, hit_zone_name, power_level, boost_curve_multiplier, is_critical_strike, damage_profile, target_index, hit_normal, damage_source)
...
function DamageUtils._projectile_hit_object(current_action, owner_unit, owner_player, owner_buff_extension, target_settings, hit_unit, hit_actor, hit_position, hit_rotation, hit_normal, is_husk, breed, is_server, check_buffs, check_backstab, is_critical_strike, difficulty_rank, power_level, ranged_boost_curve_multiplier, damage_profile, damage_source, critical_hit_effect, world, hit_effect, attack_direction, damage_source_id, damage_profile_id, max_targets, num_penetrations, current_amount_of_mass_hit)
	...
	elseif not is_level_unit and allow_ranged_damage and has_health_extension and not owner then
		...
		"hit_target_index", nil,
		"blocking", false,
		"shield_break_procced", false,
		"boost_curve_multiplier", ranged_boost_curve_multiplier,
		"is_critical_strike", is_critical_strike,
		"first_hit", num_penetrations == 0)
...
function DamageUtils._projectile_hit_character(current_action, owner_unit, owner_player, owner_buff_extension, target_settings, hit_unit, hit_actor, hit_position, hit_rotation, hit_normal, is_husk, breed, is_server, check_buffs, is_critical_strike, difficulty_rank, power_level, ranged_boost_curve_multiplier, damage_profile, damage_source, critical_hit_effect, world, hit_effect, attack_direction, damage_source_id, damage_profile_id, max_targets, current_num_penetrations, current_amount_of_mass_hit, target_number)
	...
	if
	hit_zone_name ~= "head" and HEALTH_ALIVE [hit_unit] and breed and breed.hit_zones and breed.hit_zones.head then
	...
	elseif hit_unit_player and hit_actor == unit_actor(hit_unit, "c_afro") then
		...
		predicted_damage, invulnerable = DamageUtils.calculate_damage(DamageOutput,
		hit_unit,
		owner_unit,
		hit_zone_name,
		power_level,
		BoostCurves [target_settings.boost_curve_type],
		ranged_boost_curve_multiplier,
		is_critical_strike,
		damage_profile,
		actual_target_index,
		nil,
		...
		if deal_damage then
			...
			damage_profile_id,
			"power_level", power_level,
			"hit_target_index", actual_target_index,
			"blocking", shield_blocked,
			"shield_break_procced", false,
			"boost_curve_multiplier", ranged_boost_curve_multiplier,
			"is_critical_strike", is_critical_strike,
			"attacker_is_level_unit", attacker_is_level_unit,
			"first_hit", num_penetrations == 0)
...
function DamageUtils.process_projectile_hit(world, damage_source, owner_unit, is_server, raycast_result, current_action, direction, check_buffs, target, ignore_list, is_critical_strike, power_level, override_damage_profile_name, target_number)
	...
	for i = 1, num_hits do repeat
			...
			if breed then
				...
				if
				ignore_list and ignore_list [hit_unit] and hit_zone_name ~= "afro" then
					return hit_data
				end
			end
			...
			elseif not hit_units [hit_unit] and is_target and not block_processing then
				...
				if hit_data.stop then
					if num_additional_penetrations > 0 then
						...
						return hit_data end end end until true
	end
...
function DamageUtils.get_modified_boost_curve(boost_curve, boost_curve_coefficient)
...
local function action_ignores_stagger(blackboard, attack_template, stagger_type, target_unit, is_player)
	...
	if type(ignore_stagger) == "table" then
		...
		if type == "ignore_by_health" then
			...
			do return health_ignore_stagger end
...
function DamageUtils.stagger_ai(t, damage_profile, target_index, power_level, target_unit, attacker_unit, hit_zone_name, attack_direction, boost_curve_multiplier, is_critical_strike, blocked, damage_source, source_attacker_unit)


	local hazard = EnvironmentalHazards [damage_source]

	if not damage_profile.always_stagger_ai and
	not DamageUtils.is_enemy(source_attacker_unit or attacker_unit, target_unit) and (
	not hazard or not hazard.enemy.can_stagger) then
	...
	local stagger_type, stagger_duration, stagger_length, stagger_value = DamageUtils.calculate_stagger_player(ImpactTypeOutput, target_unit, attacker_unit,
	hit_zone_name, power_level, boost_curve_multiplier,
	is_critical_strike, damage_profile, target_index,
	blocked, damage_source)
...
function DamageUtils.apply_dot(damage_profile, target_index, power_level, target_unit, attacker_unit, hit_zone_name, damage_source, boost_curve_multiplier, is_critical_strike, explosion_or_aoe_data, source_attacker_unit, custom_dot)
	...
	if dot_balefire_variant then
		...
		if career_extension and career_extension:career_name() == "bw_necromancer" then
			dot_template_name = BalefireBurnDotLookup [dot_template_name] or dot_template_name
		end
	end
...
function DamageUtils.custom_calculate_damage(attacker_unit, damage_source, power_level, damage_profile, target_index, range_scalar_multiplier, is_critical_strike, backstab_multiplier, has_power_boost, boost_damage_multiplier, breed, hit_zone_name, stagger_level, difficulty_level)
	...
	boost_curve, boost_damage_multiplier, is_critical_strike, backstab_multiplier, breed,
	range_scalar_multiplier, static_base_damage, is_friendly_fire, has_power_boost, difficulty_level, armor_type, primary_armor_type,
	has_crit_head_shot_killing_blow_perk, has_crit_backstab_killing_blow_perk, target_max_health, target_unit)
...
function DamageUtils.calculate_stagger_multiplier(damage_profile, target_buff_extension, difficulty_settings, stagger_number)
