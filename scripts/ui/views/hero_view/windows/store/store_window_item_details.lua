...
function StoreWindowItemDetails:on_enter(params, offset)
...
function StoreWindowItemDetails:_start_transition_animation(animation_name)
	local params = {
...
function StoreWindowItemDetails:_create_ui_elements(params, offset)
...
function StoreWindowItemDetails:on_exit(params)
	print("[HeroViewWindow] Exit Substate StoreWindowItemDetails")
	self._ui_animator = nil
end

function StoreWindowItemDetails:update(dt, t)
	self:_sync_presentation_item()
	self:_update_animations(dt)
	self:_draw(dt)
end

function StoreWindowItemDetails:post_update(dt, t)
	self:_handle_input(dt, t)
end

function StoreWindowItemDetails:_update_animations(dt)
...
function StoreWindowItemDetails:_sync_presentation_item()
...
function StoreWindowItemDetails:_present_dlc(settings)
...
function StoreWindowItemDetails:_present_item(item)
...
function StoreWindowItemDetails:_get_hero_wield_info_by_item(item)
	local item_data = item.data
	local can_wield = item_data.can_wield
...
function StoreWindowItemDetails:_setup_career_icons(careers)
...
function StoreWindowItemDetails:_set_item_icon(texture)
	local widget = self._widgets_by_name.item_icon
	widget.content.texture_id = texture
end

function StoreWindowItemDetails:_set_title_text_color(text_color)
	local widget = self._widgets_by_name.title_text
	widget.style.text.text_color = text_color
end

function StoreWindowItemDetails:_set_title_text(text)
	local widget = self._widgets_by_name.title_text
...
function StoreWindowItemDetails:_set_hero_text(text)
	local widget = self._widgets_by_name.hero_text
	widget.content.text = text
end

function StoreWindowItemDetails:_set_sub_title_text(text)
...
function StoreWindowItemDetails:_set_description_text(text)
	local widget = self._widgets_by_name.description_text
	widget.content.text = text
end

function StoreWindowItemDetails:_is_button_pressed(widget)
...
function StoreWindowItemDetails:_is_stepper_button_pressed(widget)
	local content = widget.content
...
function StoreWindowItemDetails:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_enter
end

function StoreWindowItemDetails:_is_button_hover_exit(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_exit
end

function StoreWindowItemDetails:_is_button_selected(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.is_selected
end

function StoreWindowItemDetails:_handle_input(dt, t)
	local parent = self._parent
	local widgets_by_name = self._widgets_by_name
	local input_service = self._parent:window_input_service()
end

function StoreWindowItemDetails:_draw(dt)
	local ui_top_renderer = self._ui_top_renderer
	local ui_scenegraph = self._ui_scenegraph
	local input_service = self._parent:window_input_service()
...
function StoreWindowItemDetails:_play_sound(event)
	self._parent:play_sound(event)
end

function StoreWindowItemDetails:_handle_gamepad_activity()
