...
function flow_callback_switchcase_range(params)
	...
	if params.case ~= "" then
		for k, v in pairs(params) do
			if k ~= "case" then
				local number = { }
				for n in string.gmatch(v, "(%d+)") do table.insert(number, tonumber(n)) end
				if number [1] ~= nil and number [2] ~= nil and
				number [1] <= params.case and params.case <= number [2] then
					ret [outStr .. string.sub(k, -1)] = true
				end
			end
		end
...
function flow_callback_switchcase_unit(params)
	...
	if params.case ~= "" then
		for k, v in pairs(params) do
			if
			k ~= "case" and params.case == tonumber(string.sub(k, -1)) then
				ret_unit = params [k]
			end
		end
	end
...
function flow_callback_randomize_sequential_numbers(params)
	...
	for i = 1, 10 do
		...
		numbers [random2] = numbers [random1] numbers [random1] = numbers [random2]
	end
...
function flow_camera_shake(params)
	...
	DamageUtils.camera_shake_by_distance(params.shake_name,
	Managers.time:time("game"),
	params.player_unit,
	params.shake_unit,
	params.near_distance,
	params.far_distance,
	params.near_shake_scale,

	params.far_shake_scale)
...
function flow_register_unit_extensions(params)
	...
	local extension_init_data = {
		...
		navgraph_system = { nav_world = GLOBAL_AI_NAVWORLD } }
...
function flow_callback_projectile_impacts_stopped(params)
	...
	if
	projectile_extension and projectile_extension.are_impacts_stopped then
...
function flow_callback_ussingen_barrel_challenge(params)
	...
	if Managers.player.is_server then
		...
		if not is_event_spawned then
			...
			do return flow_return_table end
		end
	end
...
function flow_callback_ussingen_barrel_challenge_completed(params)
	...
	if Managers.player.is_server then
		local stat_names = { "ussingen_used_no_barrels", "ussingen_used_no_barrels_cata" }
...
function flow_callback_local_player_profile_switch(params)
	...
	local returns = { witch_hunter = profile_name == "witch_hunter", bright_wizard = profile_name == "bright_wizard", dwarf_ranger = profile_name == "dwarf_ranger", wood_elf = profile_name == "wood_elf", empire_soldier = profile_name == "empire_soldier" }
...
function flow_callback_local_player_profile_available(params)
	...
	return { is_available = profile_name ~= nil }
...
function flow_callback_compare_string(params)
	local a = params.a
	local b = params.b
	local returns = { equals = a == b, not_equals = a ~= b }
...
function flow_callback_register_spline_properties(params)
	...
	ai_group_system:register_spline_properties(spline_name, { despawn_patrol_at_end_of_spline = despawn_patrol_at_end_of_spline })
end
...
function flow_callback_get_health_player_bot_ai(params)
	...
	if unit_alive(unit) then
		...
		current_health = status_extension and (status_extension:is_knocked_down() or status_extension:is_ready_for_assisted_respawn()) and
		0 or

		health_extension:current_health()
	end
...
function flow_callback_blood_collision(params)
	if Managers.state.decal ~= nil then
		...
		if MAX_VELOCITY < velocity.x or velocity.x < -MAX_VELOCITY or
		MAX_VELOCITY < velocity.y or velocity.y < -MAX_VELOCITY or
		MAX_VELOCITY < velocity.z or velocity.z < -MAX_VELOCITY then
...
function flow_callback_objective_is_objective_completed(params)
	...
	return { out_value = objective:is_done() }
end
...
function flow_callback_objective_last_objective_by_name_completed(params)
	...
	return { out_value = objective:objective_name() == params.name }
end
...
function flow_callback_objective_last_completed_objective_name(params)
	...
	return { out_value = objective:objective_name() }
end
...
function flow_callback_force_death_end(params)
	...
	ScriptUnit.has_extension(params.unit, "death_system") then
...
function flow_callback_unit_spawner_mark_for_deletion(params)
	if not unit_alive(params.unit) then return
	end
...
function flow_callback_survival_handler(params)
	...
	if wave ~= nil then
		...
		if
		event_found and (Managers.player.is_server or LEVEL_EDITOR_TEST) then
	...
	local returns = { current_wave = return_wave, total_num_waves = total_waves, last_wave = is_last_wave }
	return returns
end
...
function flow_callback_survival_handler_reset(params)
	...
	local returns = { initial_wave = wave + 1 }
	return returns
end
...
function flow_callback_get_difficulty(params)
	...
	if getdifficulty == "easy" then difficulty_easy = true end
	if getdifficulty == "normal" then difficulty_normal = true end
	if getdifficulty == "hard" then difficulty_hard = true end
	if getdifficulty == "cataclysm" then difficulty_cataclysm = true end
	if getdifficulty == "harder" then difficulty_harder = true end
	if getdifficulty == "cataclysm_2" then difficulty_cataclysm_2 = true end
	if getdifficulty == "hardest" then difficulty_hardest = true end
	if getdifficulty == "cataclysm_3" then difficulty_cataclysm_3 = true
	end
...
function flow_callback_broadphase_deal_damage(params)
	...
	if params.hits_enemies then
		...
		for i = 1, num_hits do
			...
			DamageUtils.server_apply_hit(t, attacker_unit, hit_unit, hit_zone_name, nil, Vector3.normalize(attack_direction), hit_ragdoll_actor, damage_source,
			power_level, damage_profile, target_index, boost_curve_multiplier, is_critical_strike, can_damage, can_stagger, blocking, shield_breaking_hit)
		end
	end
...
function flow_callback_set_mutator_active(params)
	...
	if active then
		mutator_handler:initialize_mutators({ mutator_name })
...
function flow_callback_is_leader(params)
	...
	return { yes = is_leader, no = not is_leader }
end
...
function flow_callback_tutorial_enable_equipment(params)
	...
	local actions_to_allow = { action_two_release = true, action_inspect = true, action_three = true, action_one_hold = true, action_inspect_hold = true, action_one_release = true, action_three_hold = true, character_inspecting = true, action_three_release = true, action_two = true, action_one = true, action_two_hold = true }
...
function flow_callbacks_add_tutorial_animation_hook(params)
	...
	if animation_hook_free_text_name ~= "" then animation_hook_name = animation_hook_free_text_name or animation_hook_name end
...
function flow_callbacks_tutorial_inputs_enabled(params)
	...
	local move_actions = { move_back_pressed = true, move_forward_pressed = true, move_right = true, move_controller = true, move_right_pressed = true, move_left = true, move_forward = true, move_back = true, move_left_pressed = true }
	...
	local jump_dodge_actions = { jump_only = true, dodge = true, jump_1 = true, dodge_hold = true, jump_2 = true }
	...
	local attack_actions = { action_one_softbutton_gamepad = true, action_one_mouse = true, action_one_hold = true, action_one_release = true, action_one = true }
	...
	local block_actions = { action_two_hold = true, action_two = true }




	local ability_actions = { action_career_release = true, action_career = true, action_career_hold = true }





	local switch_actions = { wield_switch = true, wield_2 = true, wield_next = true, wield_5 = true, wield_prev = true, wield_0 = true, wield_8 = true, wield_3 = true, wield_switch_2 = true, wield_6 = true, wield_switch_1 = true, wield_1 = true, wield_9 = true, wield_4 = true, wield_scroll = true, wield_7 = true }
...
function flow_callback_set_player_in_hanging_cage(params)
	...
	if unit_alive(player_unit) then
		...
		if not status_extension.in_hanging_cage_animations then local animations = { idle = idle_animation, falling = falling_animation, landing = landing_animation } end
...
function flow_callback_stored_parent(params)
	...
	local returns = { parent_unit = parent }
	return returns
end
...
function flow_callback_predict_hitscan(params)
	...
	if game and unit_id then
		...
		local result = PhysicsWorld.immediate_raycast_actors(physics_world,
		aim_position,
		aim_direction,
		range,
		"static_collision_filter", "filter_player_ray_projectile_static_only",
		"dynamic_collision_filter", "filter_player_ray_projectile_hitbox_only")
...
function flow_callback_cog_collision(params)
	...
	if not trigger_actor then return end
	if Vector3.length(actor_velocity) <= 0.1 then return
	end
	local touching_unit = params.touching_unit
	...
	if Managers.state.network.is_server then
		...
		DamageUtils.server_apply_hit(t, cog_unit, hit_unit, hit_zone_name, nil, push_direction, hit_ragdoll_actor, damage_source,
		power_level, damage_profile, target_index, boost_curve_multiplier, is_critical_strike, can_damage, can_stagger, blocking, shield_breaking_hit)
	end
end
...
function flow_callback_environment_hazard_damage_collision(params)
	if Managers.state.network.is_server then
		...
		if health_extension:is_dead() then
			local gibbs = { "dismember_torso", "dismember_head", "explode_body" }
			local gibb = gibbs [math.random(1, 3)]
			Unit.flow_event(hit_unit, gibb)
		else
			DamageUtils.stagger_ai(t, damage_profile, target_index, power_level, hit_unit, hazard_unit, hit_zone_name, push_direction,
			boost_curve_multiplier, is_critical_strike, blocking, damage_source)
		end
	end
end
...
function flow_callback_start_disrupt_ritual(params)
	...
	"start_disrupt_ritual",
	params.unit,
	params.volume_name,
	params.volume_type,
	params.num_progression_events,
	params.tick_length,
	params.damage_per_tick,

	params.heal_per_tick)
end
...
function flow_callback_spawn_skulls_tower_end(params)
	if not Managers.player.is_server then return
	end
	...
	function optional_data.prepare_func(breed, extension_init_data)
		local is_husk = false
