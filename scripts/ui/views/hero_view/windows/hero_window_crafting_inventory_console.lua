...
function HeroWindowCraftingInventoryConsole:on_enter(params, offset)
...
function HeroWindowCraftingInventoryConsole:_start_transition_animation(animation_name)
	local params = {
...
function HeroWindowCraftingInventoryConsole:create_ui_elements(params, offset)
...
function HeroWindowCraftingInventoryConsole:on_exit(params)
...
function HeroWindowCraftingInventoryConsole:_input_service()
...
function HeroWindowCraftingInventoryConsole:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function HeroWindowCraftingInventoryConsole:_update_filter_status()
...
function HeroWindowCraftingInventoryConsole:_update_filter_reset(force_reset)
...
function HeroWindowCraftingInventoryConsole:post_update(dt, t)

	return end

function HeroWindowCraftingInventoryConsole:_update_animations(dt)
...
function HeroWindowCraftingInventoryConsole:_is_button_pressed(widget)
...
function HeroWindowCraftingInventoryConsole:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.button_hotspot or content.hotspot
...
function HeroWindowCraftingInventoryConsole:_handle_filter_input(input_service, dt, t)
...
function HeroWindowCraftingInventoryConsole:_handle_filer_widgets_sounds()
	local filter_widget = self._filter_widget
	local search_widget = self._widgets_by_name.input

	if

	search_widget and UIUtils.is_button_hover_enter(search_widget, "search_filters_hotspot") then
	...
	if filter_widget then
		...
		local sort_by_names = { [1.0] = "rarity", [2.0] = "power_level" }
...
function HeroWindowCraftingInventoryConsole:_handle_gamepad_filter_input(input_service, dt, t)
	local filter_active = self.parent:filter_active()
	...
	if filter_active then
		...
		local filter_widget = self._filter_widget
		local filter_content = filter_widget.content
		local current_gamepad_index = filter_content.current_gamepad_index or { 1, 1 }
...
function HeroWindowCraftingInventoryConsole:filter_selected()
	return self.parent:filter_selected()
end

function HeroWindowCraftingInventoryConsole:filter_active()
	return self.parent:filter_active()
end

function HeroWindowCraftingInventoryConsole:_set_filter_selected(selected)
...
function HeroWindowCraftingInventoryConsole:_handle_input(dt, t)
	...
	if not gamepad_active then
		...
		if self._pressed_backend_id and
		parent:get_pressed_item_backend_id() == self._pressed_backend_id then
...
function HeroWindowCraftingInventoryConsole:_handle_gamepad_input(dt, t)
	local parent = self.parent
	local input_service = self:_input_service()
	local item_grid = self._item_grid
...
function HeroWindowCraftingInventoryConsole:_set_input_blocked(blocked)
...
function HeroWindowCraftingInventoryConsole:_handle_search_input(dt, t)
	local gamepad_active = Managers.input:is_device_active("gamepad")
	local input_service = self:_input_service()
	local input_widget = self._widgets_by_name.input
	local input_content = input_widget.content
...
local EMPTY_TABLE = { }
function HeroWindowCraftingInventoryConsole:_do_search(search_query, filter_query)
...
function HeroWindowCraftingInventoryConsole:_handle_recipe_inputs(dt, t)
	local input_service = self:_input_service()
	local local_crafting_recipes_by_name = crafting_recipes_by_name
	local recipe = local_crafting_recipes_by_name [self._selected_craft_page_name]
...
function HeroWindowCraftingInventoryConsole:_update_page_info()
...
function HeroWindowCraftingInventoryConsole:_update_crafting_material_panel()
	...
	for index, item_key in ipairs(material_order) do
		...
		if not content.item then
			content.item = item or { data = table.clone(ItemMasterList [item_key]) }
		end
	end
end

function HeroWindowCraftingInventoryConsole:_update_inventory_items()
	local item_grid = self._item_grid
	local parent = self.parent
...
function HeroWindowCraftingInventoryConsole:_update_disabled_item_icon()
	local item_grid = self._item_grid
	local parent = self.parent
...
function HeroWindowCraftingInventoryConsole:_update_disabled_backend_ids()
	local item_grid = self._item_grid
	local parent = self.parent
...
function HeroWindowCraftingInventoryConsole:_exit(selected_level)
	self.exit = true
	self.exit_level_id = selected_level
end

function HeroWindowCraftingInventoryConsole:draw(dt)
	...
	if filter_active then input_service = FAKE_INPUT_SERVICE or input_service
	end
	UIRenderer.begin_pass(ui_top_renderer, ui_scenegraph, input_service, dt, nil, self.render_settings)
...
function HeroWindowCraftingInventoryConsole:_play_sound(event)
	self.parent:play_sound(event)
end

function HeroWindowCraftingInventoryConsole:_change_category_by_name(name)
...
function HeroWindowCraftingInventoryConsole:_change_category_by_index(index, force_update)
...
function HeroWindowCraftingInventoryConsole:change_item_filter(item_filter, change_page, optional_search_query)
...
function HeroWindowCraftingInventoryConsole:_update_selected_item_tooltip(forced_update)
...
function HeroWindowCraftingInventoryConsole:_setup_input_buttons()
...
function HeroWindowCraftingInventoryConsole:_set_gamepad_input_buttons_visibility(visible)
	local widgets_by_name = self._widgets_by_name
...
function HeroWindowCraftingInventoryConsole:_handle_gamepad_activity(forced_update)
