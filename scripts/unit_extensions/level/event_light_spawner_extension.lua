...
function EventLightSpawnerExtension:init(extension_init_context, unit, extension_init_data)
...
function EventLightSpawnerExtension:update(unit, input, dt, context, t)
...
function EventLightSpawnerExtension:_update_units(context, dt)
	local units = self._units
	...
	for _, light_unit in pairs(units) do
		...
		if player_unit and Unit.alive(player_unit) and unit and
		Unit.alive(unit) then
...
function EventLightSpawnerExtension:_update_spawn_pool(dt)
...
function EventLightSpawnerExtension:_add_to_spawn_pool(id)
	local spawn_pool = self._spawn_pool
	spawn_pool [self._spawn_pool_add_index] = id
	self._spawn_pool_add_index = self._spawn_pool_add_index + 1
end

function EventLightSpawnerExtension:_activate()
	self._active = true
...
function EventLightSpawnerExtension:_deactivate()
	self._active = false
	local units = self._units
	for i = 1, #units do
...
function EventLightSpawnerExtension:_explode_spirit(unit)
...
function EventLightSpawnerExtension:_sync_light_units()
	if not self.is_server then
		return
	end
