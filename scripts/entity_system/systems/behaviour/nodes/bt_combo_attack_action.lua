...
function BTComboAttackAction:init(...)
...
function BTComboAttackAction:enter(unit, blackboard, t)
	...
	if combo then
		...
		combo = { successful_hit = false, aborted = false, is_animation_driven = false, refresh_last_target_position = false, has_been_blocked = false, blocked = false,
...
local function randomize(event, blackboard)
	...
	if type(event) == "table" then
		do return event [combo.attack_variation] end
	else
...
function BTComboAttackAction:_start_attack(unit, blackboard, t, action, attack_name)
...
function BTComboAttackAction:leave(unit, blackboard, t, reason, destroy)
...
function BTComboAttackAction:run(unit, blackboard, t, dt)
	local combo = blackboard.combo_attack_data
	local attacking_target = combo.attacking_target
...
function BTComboAttackAction:_follow(dt, t, unit, blackboard, current_attack)
	local breed = blackboard.breed
	local combo = blackboard.combo_attack_data
	local attacking_target = combo.attacking_target
...
function BTComboAttackAction:attack_success(unit, blackboard)
	local breed = blackboard.breed
...
function BTComboAttackAction:_update_rotation_target(t, unit, blackboard, combo)
	local target_status_extension = blackboard.target_status_extension
...
function BTComboAttackAction:_set_target_position(blackboard, combo, position, t)
	local target_locomotion_extension = blackboard.target_locomotion_extension
...
function BTComboAttackAction:_update_rotation_target_lerped(t, unit, blackboard, combo, rotation_constraint)
	self:_update_rotation_target(t, unit, blackboard, combo)
...
function BTComboAttackAction:attack_cooldown(unit, blackboard)
...
function BTComboAttackAction:_push_non_targets(unit, self_pos, current_target, combo, forward_direction, close_impact_radius, far_impact_radius, forward_impact_speed, lateral_impact_speed)
...
function BTComboAttackAction:stagger_override(unit, blackboard, attacker_unit, stagger_direction, stagger_length, stagger_type, stagger_duration, stagger_animation_scale, t, is_push)
	local combo = blackboard.combo_attack_data
	local action = blackboard.action
	local current_attack = action.combo_attacks [combo.current_attack_name]
...
function BTComboAttackAction:anim_cb_frenzy_damage(unit, blackboard)
	local action = blackboard.action
	local combo = blackboard.combo_attack_data
	local attacking_target = combo.attacking_target
...
function BTComboAttackAction:get_attack_cooldown_finished_at(unit, blackboard, t)
	local combo = blackboard.combo_attack_data
	local attacking_target = combo.attacking_target
	if not Unit.alive(attacking_target) then
		return false, 0
...
function BTComboAttackAction:anim_cb_attack_vce(unit, blackboard)
	local network_manager = Managers.state.network
