...
local OVERCHARGE_LEVELS = table.enum(
"none",
"low",
"medium",
"high",
...
function PlayerUnitOverchargeExtension:init(extension_init_context, unit, extension_init_data)
	...
	self._overcharge_states = {
		...
		[OVERCHARGE_LEVELS.low] = {
			...
			controller_effect = { rumble_effect = "overcharge_rumble" } },

		[OVERCHARGE_LEVELS.medium] = { dialogue_event = "overcharge",
			sound_event = overcharge_data.overcharge_warning_med_sound_event,

			controller_effect = { rumble_effect = "overcharge_rumble_overcharged" } },

		[OVERCHARGE_LEVELS.high] = { dialogue_event = "overcharge_high",
			sound_event = overcharge_data.overcharge_warning_high_sound_event,

			controller_effect = { rumble_effect = "overcharge_rumble_crit" } },

		[OVERCHARGE_LEVELS.critical] = { dialogue_event = "overcharge_critical",
			sound_event = overcharge_data.overcharge_warning_critical_sound_event },



		[OVERCHARGE_LEVELS.exploding] = { dialogue_event = "overcharge_explode" } }
	...
	self._ignored_overcharge_types = { flamethrower = true, damage_to_overcharge = true, charging = true, drakegun_charging = true }
...
function PlayerUnitOverchargeExtension:extensions_ready(world, unit)
...
function PlayerUnitOverchargeExtension:_calculate_and_set_buffed_max_overcharge_values()
...
function PlayerUnitOverchargeExtension:set_screen_particle_opacity_modifier(value)
	self._screen_particle_opacity_modifier = value / 100
end

function PlayerUnitOverchargeExtension:reset()
...
function PlayerUnitOverchargeExtension:_destroy_all_screen_space_particles()
...
function PlayerUnitOverchargeExtension:_destroy_screen_space_particles(particle_id)
	if particle_id then
		World.destroy_particles(self.world, particle_id)
	end
end

function PlayerUnitOverchargeExtension:_update_vfx_sfx(owner_player)
...
function PlayerUnitOverchargeExtension:destroy()
	self:_destroy_all_screen_space_particles()
...
function PlayerUnitOverchargeExtension:set_animation_variable()
...
function PlayerUnitOverchargeExtension:_update_game_object()
...
function PlayerUnitOverchargeExtension:update(unit, input, dt, context, t)
	self:_calculate_and_set_buffed_max_overcharge_values()
	...
	if self.overcharge_value > 0 or buff_extension:has_buff_type("sienna_unchained_activated_ability") then
		...
		if not self.is_exploding and self.time_when_overcharge_start_decreasing < t or self.lockout == true then
			...
			if buff_extension:has_buff_type("sienna_unchained_activated_ability") and
			self.max_value <= value then
				self:add_charge(1)
			end
...
function PlayerUnitOverchargeExtension:add_charge(overcharge_amount, charge_level, overcharge_type)
...
function PlayerUnitOverchargeExtension:remove_charge(overcharge_amount)
...
function PlayerUnitOverchargeExtension:remove_charge_fraction(fraction)
...
function PlayerUnitOverchargeExtension:_check_overcharge_level_thresholds(new_overcharge_value)
	local buff_extension = self._buff_extension
	local max_value = self.max_value
...
function PlayerUnitOverchargeExtension:set_lockout(new_state)
	self.lockout = new_state
end

function PlayerUnitOverchargeExtension:get_overcharge_value()
	return self.overcharge_value
end

function PlayerUnitOverchargeExtension:is_above_critical_limit()
	return self.overcharge_critical_limit <= self.overcharge_value
end

function PlayerUnitOverchargeExtension:get_original_max_value()
	return self.original_max_value
end

function PlayerUnitOverchargeExtension:get_max_value()
	return self.max_value
end

function PlayerUnitOverchargeExtension:get_overcharge_threshold()
	return self.overcharge_threshold
end

function PlayerUnitOverchargeExtension:above_overcharge_threshold()
	return self.overcharge_threshold <= self.overcharge_value
end

function PlayerUnitOverchargeExtension:are_you_exploding()
	return self.is_exploding
end

function PlayerUnitOverchargeExtension:are_you_locked_out()
	return self.lockout
end

function PlayerUnitOverchargeExtension:overcharge_fraction()
	return math.clamp(self.overcharge_value / self.max_value, 0, 1)
end

function PlayerUnitOverchargeExtension:lerped_overcharge_fraction()
	return self._lerped_overcharge_fraction
end

function PlayerUnitOverchargeExtension:threshold_fraction()
	return self.overcharge_threshold / self.max_value
end

function PlayerUnitOverchargeExtension:current_overcharge_status()
...
function PlayerUnitOverchargeExtension:vent_overcharge()
...
function PlayerUnitOverchargeExtension:vent_overcharge_done()
	self.venting_overcharge = false
	self.venting_anim = "cooldown_end"
end

function PlayerUnitOverchargeExtension:get_anim_blend_overcharge()
...
function PlayerUnitOverchargeExtension:_trigger_hud_sound(event, fp_extension)
...
function PlayerUnitOverchargeExtension:_trigger_dialogue(event_name)
...
function PlayerUnitOverchargeExtension:_trigger_controller_effect(type, effect)
...
function PlayerUnitOverchargeExtension:_add_overcharge_buff(buff_name)
	local buff_extension = self._buff_extension
...
function PlayerUnitOverchargeExtension:_update_overcharge_buff(state)
	local buff_extension = self._buff_extension
...
function PlayerUnitOverchargeExtension:_update_lerped_overcharge(dt)
...
function PlayerUnitOverchargeExtension:_update_screen_effect()
...
function PlayerUnitOverchargeExtension:_update_overcharge_buff_state(old_overcharge_value, new_overcharge_value)
...
function PlayerUnitOverchargeExtension:_overcharge_value_state(value)
	if self.overcharge_critical_limit <= value then
		do return OVERCHARGE_LEVELS.high end
	elseif self.overcharge_limit <= value then
		do return OVERCHARGE_LEVELS.medium end
	elseif self.overcharge_threshold <= value then
		do return OVERCHARGE_LEVELS.low end
	else
