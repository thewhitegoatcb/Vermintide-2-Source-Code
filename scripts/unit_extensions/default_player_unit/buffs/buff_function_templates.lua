...
BuffFunctionTemplates.functions = {
	...
	health_regen_all_update = function (unit, buff, params)
		...
		buff.next_heal_time < params.t then
			local buff_template = buff.template
	...
	mutator_life_health_regeneration_update = function (unit, buff, params)
		if Managers.state.network.is_server and
		buff.next_buff_time < params.t then
			local buff_template = buff.template
	...
	remove_metal_mutator_gromril_armour = function (unit, buff, params)
		local buff_params = { attacker_unit = unit }
			...
			if Managers.state.network.is_server then
				...
				if used_attacker_unit then
					if buff.template.custom_dot_tick_func then
						...
						DamageUtils.server_apply_hit(t, used_attacker_unit, target_unit, hit_zone_name, nil, attack_direction, hit_ragdoll_actor, damage_source,
						power_level, damage_profile, target_index, boost_curve_multiplier, is_critical_strike, can_damage, can_stagger,
						blocking, shield_breaking_hit, backstab_multiplier, first_hit, total_hits, source_attacker_unit)
					end
				end
			end
		end
					...
					if talent_extension:has_talent("kerillian_waywatcher_group_regen", "wood_elf", true) then
						...
						for i = 1, #player_and_bot_units do
							...
							if HEALTH_ALIVE [heal_unit] then
								...
								if health_extension:current_permanent_health_percent() <= regen_cap and not status_extension:is_knocked_down() and not status_extension:is_assisted_respawning() then
		...
		if Unit.alive(unit) then
			...
			if current_stacks < max_stacks then
				...
				if not buff.server_buff_ids then
					buff.server_buff_ids = { server_buff_id }
				else
	...
	activate_buff_stacks_based_on_enemy_proximity = function (unit, buff, params)
		if not Managers.state.network.is_server then return
		end
		local side = Managers.state.side.side_by_unit [unit]
		if not side then
			return
	...
	activate_buff_on_distance = function (owner_unit, buff, params)
		...
		for i = 1, num_units do
			...
			if Unit.alive(unit) and (not disregard_self or unit ~= owner_unit) then
				...
				if
				distance_squared < range_squared and not buff_extension:has_buff_type(buff_to_add) then
	...
	side_buff_aura = function (owner_unit, buff, params)
		...
		if template.player_buff_name then
			...
			for i = 1, #friendly_players do
				...
				local inside = Vector3.distance_squared(owner_position, POSITION_LOOKUP [friendly_unit]) < range_squared
	...
	activate_buff_on_closest_distance = function (owner_unit, buff, params)
		...
		if current_unit then
			...
			if
			dist_squared < range_squared and not buff_extension:has_buff_type(buff_to_add) then
	...
	markus_knight_proximity_buff_update = function (owner_unit, buff, params)
		...
		for i = 1, num_units do
			...
			if Unit.alive(unit) then
				...
				if
				distance_squared < range_squared and not power_talent and not range_talent and not buff_extension:has_buff_type(buff_to_add) then
					local server_buff_id = buff_system:add_buff(unit, buff_to_add, owner_unit, true)
					local buff = buff_extension:get_non_stacking_buff(buff_to_add)
	...
	bardin_slayer_activate_buff_on_loadout = function (unit, buff, params)
		...
		if Unit.alive(unit) then
			...
			if melee_slot_data and ranged_slot_data then
				...
				if ranged_slot_buff_type == "RANGED" then ranged_slot_buff_type = "MELEE_1H" end
	...
	enter_sienna_unchained_activated_ability = function (unit, buff, params)
		...
		if not go_id or not game then return
		end
		...
		projected_start_pos =
		projected_start_pos or GwNavQueries.inside_position_from_outside_position(nav_world, unit_pos, above, below, 2, 0.5)
	...
	sienna_adept_double_trail_talent_start_ability_cooldown_add = function (unit, buff, params)
		if ALIVE [unit] and not buff.aborted and
		is_local(unit) then
			local buff_extension = ScriptUnit.extension(unit, "buff_system")
			local buff_to_add = buff.template.buff_to_add
			buff_extension:add_buff(buff_to_add)
	...
	sienna_adept_double_trail_talent_start_ability_cooldown = function (unit, buff, params)
		if ALIVE [unit] and not buff._already_removed and
		is_local(unit) then
			local career_extension = ScriptUnit.extension(unit, "career_system")
	...
	play_sound_synced = function (owner_unit, buff, params)
		if not ALIVE [owner_unit] then return false end
	...
	bardin_ranger_heal_smoke = function (unit, buff, params)
		...
		if t > next_heal_tick and HEALTH_ALIVE [unit] then
			local talent_extension = ScriptUnit.has_extension(unit, "talent_system")
	...
	update_volume_dot_damage = function (unit, buff, params)
		if buff.next_damage_time < params.t and
		HEALTH_ALIVE [unit] then
	...
	update_speed_scaled_dot_buff = function (unit, buff, params)
		if is_local(unit) then
			...
			if length > 0.5 and buff.next_damage_t < params.t and
			HEALTH_ALIVE [unit] then
				local template = buff.template
	...
	update_twitch_infinite_bombs = function (unit, buff, params)
		if is_local(unit) then
			...
			local additional_item_granted = nil
			repeat
				additional_item_granted = inventory_extension:store_additional_item(slot_name, item_data) until not additional_item_granted
...
function BuffFunctionTemplates.functions.update_charging_action_lerp_movement_buff(unit, buff, params)
	local multiplier = params.multiplier
	local time_into_buff = params.time_into_buff
	...
	multiplier =
	multiplier and 1 - buff_extension:apply_buffs_to_value(1 - multiplier, "increased_move_speed_while_aiming")
...
DLCUtils.merge("buff_function_templates", BuffFunctionTemplates.functions) function BuffFunctionTemplates.functions.ai_update_max_health(unit, buff, params) if is_server() then local buff_extension = ScriptUnit.extension(unit, "buff_system") local health_extension = ScriptUnit.has_extension(unit, "health_system") if buff_extension and health_extension then local base_health = health_extension.unmodified_max_health local max_health = math.max(buff_extension:apply_buffs_to_value(base_health, "max_health"), 0.25) health_extension:set_max_health(max_health) end end end