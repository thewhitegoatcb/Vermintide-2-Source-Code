...
function GraphDrawer:init(world, input_manager)
	self.world = world
	self.input_manager = input_manager

	self.gui = World.create_screen_gui(world,
...
function GraphDrawer:create_graph(graph_name, axis_names)
	local graph = Graph:new(graph_name, axis_names)
	self.graphs [graph_name] = graph
	return graph
end

function GraphDrawer:destroy_graph(graph)
	self.graphs [graph.name] = nil
end

function GraphDrawer:graph(graph_name)
	return self.graphs [graph_name]
end

function GraphDrawer:update(input_service, t)

	if input_service:get("f11") then
		if not self.active then
			self.input_manager:capture_input({ "mouse" }, 1, "Debug", "GraphDrawer")
			Window.set_show_cursor(true)
		else
			self.input_manager:release_input({ "mouse" }, 1, "Debug", "GraphDrawer")
...
function Graph:init(name, axis_names)
	self.name = name
	...
	self.range_x = { math.huge, -math.huge }
	self.range_y = { math.huge, -math.huge }
	self.visual_frame = { x_min = 0, x_max = 0, y_min = 0, y_max = 0 }
	...
	self.scroll_lock = { vertical = true, left = true, right = true }
	self.valid = false
...
function Graph:set_active(active)
	self.active = active
end

function Graph:set_plot_color(plot_name, point_color, line_color)
...
function Graph:add_point(x, y, plot_name)
...
function Graph:set_visual_range(x_min, x_max, y_min, y_max)
	self.visual_frame = { x_min = x_min, x_max = x_max, y_min = y_min, y_max = y_max }
end

function Graph:update(input_service, t)
	...
	local graph_size_x = 800 local graph_size_y = 400
	...
	if self.state == "drawing_zoom_window" then
		if input_service:get("mouse_left_held") then
			if self.zoom_window == nil then
				...
				self.zoom_window = { x_start = mouse.x, y_start = mouse.y }
			end
	...
	if self.state == "panning" then
		...
		if self.pan_previous == nil then
			self.pan_previous = { x = mouse.x, y = mouse.y }
		end
...
function Graph:draw(gui, input_service, t)
