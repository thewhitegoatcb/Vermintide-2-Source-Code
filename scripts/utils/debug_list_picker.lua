...
function DebugListPicker:init(list, save_data_name, max_cols_seen)
	self.pick_list = list
...
function DebugListPicker:destroy()
	return end

function DebugListPicker:setup()

	local save_data = SaveData [self.save_data_name]
	save_data = type(save_data) == "table" and save_data or { last_column_index = 1, columns = { } }
	...
	local max_width = 0 local max_height = 0
...
function DebugListPicker:activate()
...
function DebugListPicker:current_item()
	return self.item
end

function DebugListPicker:current_item_name()
	return self.item [1]
end

function DebugListPicker:update(t, dt)
	...
	if DebugKeyHandler.key_pressed("up_key", "switch spawn category", "ai") and
	self.move_cursor_timer < wall_time then
	...
	if DebugKeyHandler.key_pressed("down_key", "switch spawn category", "ai") and
	self.move_cursor_timer < wall_time then
		self.row_index = self.row_index + 1
		self.row_index = ( self.row_index - 1 ) % #column + 1
		self.item = self.column [self.row_index]
	...
	if not script_data.disable_debug_draw then
		...
		local c1 = col_in - 1 local c2 = col_in + 1
		if col_in == 1 then
			c1 = 1
