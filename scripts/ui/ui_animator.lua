...
function UIAnimator:init(ui_scenegraph, animation_definitions)
...
function UIAnimator:start_animation(anim_name, widget, scenegraph_def, params, speed, initial_delay)
	...
	for i = 1, #anim_def do
		...
		if anim.start_progress then
			...
			t1 = anim.end_progress t0 = anim.start_progress
		else
	...
	self._active_animations [animation_id] = { time = 0,
		anim_name = anim_name,
		anim_def = anim_def,
		widget = widget,
...
function UIAnimator:is_animation_completed(animation_id)
	return self._active_animations [animation_id] == nil
end



function UIAnimator:stop_animation(animation_id)
