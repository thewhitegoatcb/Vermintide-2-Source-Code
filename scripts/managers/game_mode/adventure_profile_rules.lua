...
function AdventureProfileRules:init(profile_synchronizer, network_server)
	self._profile_synchronizer = profile_synchronizer
	self._network_server = network_server
end

function AdventureProfileRules:_profile_career_exists(profile_index, career_index)
...
function AdventureProfileRules:_profile_career_unlocked(profile_index, career_index)
	local profile = SPProfiles [profile_index]
	local careers = profile and profile.careers
	local career = careers and careers [career_index]
...
function AdventureProfileRules:handle_profile_delegation_for_joining_player(peer_id, local_player_id)
	...
	if not current_profile_index then
		...
		if not current_reserver or current_reserver == peer_id then
			new_career_index = wanted_career_index new_profile_index = wanted_profile_index
		else
