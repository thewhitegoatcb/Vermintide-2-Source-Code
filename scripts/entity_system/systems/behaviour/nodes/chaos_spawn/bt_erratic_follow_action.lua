...
function BTErraticFollowAction:init(...)
...
function BTErraticFollowAction:enter(unit, blackboard, t)
	...
	if not blackboard.random_dirs then
		blackboard.random_dirs = { action.move_jump_fwd_anims, action.move_jump_right_anims, action.move_jump_fwd_anims }
	end
...
function BTErraticFollowAction:leave(unit, blackboard, t, reason, destroy)
...
function BTErraticFollowAction:run(unit, blackboard, t, dt)
	local locomotion_extension = blackboard.locomotion_extension
...
function BTErraticFollowAction:_go_idle(unit, blackboard, locomotion_extension)
...
function BTErraticFollowAction:_go_moving(unit, blackboard, move_anim)
	blackboard.move_state = "moving"
	Managers.state.network:anim_event(unit, move_anim)
end


function BTErraticFollowAction:_go_walking(unit, blackboard, walk_anim)
	blackboard.move_state = "walking"
	Managers.state.network:anim_event(unit, walk_anim)
end

function BTErraticFollowAction:follow(unit, t, dt, blackboard, locomotion_extension)
	local navigation_extension = blackboard.navigation_extension
...
function BTErraticFollowAction:check_for_high_jump(unit, blackboard)
...
function BTErraticFollowAction:debug_ray_casts(unit_position, move_dir, nav_world, traverse_logic, action)
...
function BTErraticFollowAction:investigate_jump(unit, t, blackboard, unit_position, locomotion_extension)
	...
	if moving_towards_target then
		...
		if right_of_to_goal then
			...
			jump_data =
			jump_data or self:check_dir(unit_position, move_dir, nav_world, traverse_logic, action.move_jump_only_fwd_right_anims)
	...
	if jump_data then
		...
		blackboard.jump_color = { math.random(100, 255), math.random(100, 255), math.random(100, 255) }
...
function BTErraticFollowAction:get_travel_dir(unit, blackboard, pos)
	local navigation_extension = blackboard.navigation_extension
	local nav_bot = navigation_extension._nav_bot
...
function BTErraticFollowAction:anim_cb_move_jump_finished(unit, blackboard)
