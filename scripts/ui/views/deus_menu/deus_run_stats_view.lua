...
function DeusRunStatsView:init(parent, ingame_ui_context)
...
function DeusRunStatsView:on_enter()
	self._input_manager:capture_input({ "keyboard", "gamepad", "mouse" }, 1, self._input_service_name, "DeusRunStatsView")
end

function DeusRunStatsView:on_exit()
	self._input_manager:release_input({ "keyboard", "gamepad", "mouse" }, 1, self._input_service_name, "DeusRunStatsView")
end

function DeusRunStatsView:update(dt, t)
	self._ui:update(dt, t)
	self:_handle_input(dt, t)
end

function DeusRunStatsView:input_service()
	return self._input_manager:get_service(self._input_service_name)
end

function DeusRunStatsView:is_ui_active()
	return self._ui:active()
end

function DeusRunStatsView:_handle_input(dt, t)
	...
	elseif ui_active ~= new_active and new_active == true then
		...
		local no_player_unit = local_player.player_unit == nil
		if no_player_unit or gamepad_active then
			self._ui:lock(true, no_player_unit)
		end
	end
...
function DeusRunStatsView:destroy()
	self._ui:destroy()
end

function DeusRunStatsView:_update_dynamic_values()
...
function DeusRunStatsView:_update_inventory()
	local run_controller = self._deus_run_controller
	...
	if self._melee ~= melee or
	self._ranged ~= ranged or
	self._potion_slot ~= potion_slot or
	self._grenade_slot ~= grenade_slot or
	self._healing_slot ~= healing_slot then
