...
function CraftPageApplySkinConsole:on_enter(params, settings)
...
function CraftPageApplySkinConsole:setup_recipe_requirements()
...
function CraftPageApplySkinConsole:reset_requirements(num_required_ingredients)
...
function CraftPageApplySkinConsole:create_ui_elements(params)
...
function CraftPageApplySkinConsole:_weapon_slot_updated()
	local item_interface = Managers.backend:get_interface("items")
...
function CraftPageApplySkinConsole:on_exit(params)
	self.parent.parent:set_craft_optional_item_filter(nil)
	self.parent.parent:disable_filter(false)
	self.parent.parent:disable_search(false)
...
function CraftPageApplySkinConsole:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function CraftPageApplySkinConsole:post_update(dt, t)

	return end

function CraftPageApplySkinConsole:_update_animations(dt)
...
function CraftPageApplySkinConsole:_is_button_pressed(widget)
...
function CraftPageApplySkinConsole:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageApplySkinConsole:_is_button_held(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageApplySkinConsole:_handle_input(dt, t)
	...
	if craft_input_accepted then
		...
		local items = { craft_item, skin_item }
...
function CraftPageApplySkinConsole:_handle_craft_input_progress(progress)
	return self.parent:_set_input_progress(progress)
end

function CraftPageApplySkinConsole:craft_result(result, error, reset_slots)
	if not error then
		self._craft_result = result
	end
end

function CraftPageApplySkinConsole:reset()
...
function CraftPageApplySkinConsole:present_results()
	self.super_parent:clear_disabled_backend_ids()
...
function CraftPageApplySkinConsole:on_craft_completed()
	local item_grid = self._item_grid
	local item_grid_2 = self._item_grid_2
...
function CraftPageApplySkinConsole:_update_craft_items()
	local super_parent = self.super_parent
	local item_grid = self._item_grid
	local item_grid_2 = self._item_grid_2
...
function CraftPageApplySkinConsole:_remove_item(item_grid, backend_id)
...
function CraftPageApplySkinConsole:_add_item(item_grid, backend_id, ignore_sound)
...
function CraftPageApplySkinConsole:_set_craft_button_disabled(disabled)
...
function CraftPageApplySkinConsole:_exit(selected_level)
	self.exit = true
	self.exit_level_id = selected_level
end

function CraftPageApplySkinConsole:draw(dt)
...
function CraftPageApplySkinConsole:_play_sound(event)
	self.super_parent:play_sound(event)
end

function CraftPageApplySkinConsole:_set_craft_button_text(text, localize)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.craft_button
	widget.content.button_text = localize and Localize(text) or text
end

function CraftPageApplySkinConsole:_add_crafting_material_requirement(index, item_key, amount_text, has_required_amount)
