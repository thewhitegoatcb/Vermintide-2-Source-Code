...
function ActionWield:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionWield:client_owner_start_action(new_action, t, chain_attack_data)
	...
	if new_slot == inventory_extension:get_wielded_slot_name() or swap_from_storage_type then
		...
		inventory_extension:swap_equipment_from_storage(

		new_slot,
		swap_from_storage_type)
...
function ActionWield:client_owner_post_update(dt, t, world, can_damage)
	return end

function ActionWield:finish(reason)
