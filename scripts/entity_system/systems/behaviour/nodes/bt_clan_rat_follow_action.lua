...
function BTClanRatFollowAction:init(...)
...
function BTClanRatFollowAction:enter(unit, blackboard, t)
...
function BTClanRatFollowAction:_should_walk(destination, self_pos, max_distance_sq, rotation_towards_target)
...
function BTClanRatFollowAction:_slow_approach(destination, self_pos, action_data, rotation_towards_target)
...
function BTClanRatFollowAction:leave(unit, blackboard, t, reason, destroy)
...
function BTClanRatFollowAction:run(unit, blackboard, t, dt)
...
function BTClanRatFollowAction:_update_walking(unit, blackboard, dt, t)
...
function BTClanRatFollowAction:_calculate_walk_animation(walk_dir, walk_anims)
	local anim = nil
...
function BTClanRatFollowAction:_calculate_walk_dir(right_vector, forward_vector, dir, pos, walk_anims)
...
function BTClanRatFollowAction:follow(unit, blackboard, t, dt)
	...
	elseif ( current_speed > breed.run_speed + 0.1 or blackboard.deacceleration_factor ) and target_distance < 2 * weapon_reach + CHASE_DEACCELERATION_DISTANCE then
		...
		if sign > 0 and current_speed < breed.run_speed and target_distance > ( breed.match_speed_distance or weapon_reach ) + 0.5 then current_speed = breed.run_speed or current_speed end
	...
	if blackboard.time_to_next_friend_alert < t then
		...
		if is_within_range then
			...
			if Vector3.length_squared(direction) > 0 then
				local result, hit_pos, hit_distance, normal = PhysicsWorld.immediate_raycast(
				physics_world,
				raycast_pos,
				direction,
				blackboard.target_dist,
				"closest",
				"types", "statics",
				"collision_filter",
...
function BTClanRatFollowAction:_calculate_run_speed(unit, target_unit, blackboard, target_locomotion)
	local target_distance = blackboard.target_dist
...
function BTClanRatFollowAction:start_move_animation(unit, blackboard)
...
function BTClanRatFollowAction:start_move_rotation(unit, blackboard, t, dt)
...
function BTClanRatFollowAction:set_start_move_animation_lock(unit, blackboard, should_lock_ani)
	local locomotion_extension = blackboard.locomotion_extension
...
local nearby_units = { }
function BTClanRatFollowAction:do_dialogue(unit, blackboard, t, dt)
