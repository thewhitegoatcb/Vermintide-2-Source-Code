...
local mission_settings = { { texture = "loot_objective_icon_02", mission_name = "tome_bonus_mission", key = "tome", widget_name = "tome_counter", title_text = "dlc1_3_1_tomes" },
	...
	{ texture = "loot_objective_icon_01", mission_name = "grimoire_hidden_mission", key = "grimoire", widget_name = "grimoire_counter", title_text = "dlc1_3_1_grimoires" },
	...
	{ texture = "loot_mutator_icon_05", mission_name = "bonus_dice_hidden_mission", key = "loot_die", widget_name = "loot_dice", title_text = "interaction_loot_dice" } }
...
function IngamePlayerListUI:init(parent, ingame_ui_context)
	self._parent = parent
	...
	self._render_settings = { alpha_multiplier = 0, snap_pixel_positions = true }
	self._active = false
...
function IngamePlayerListUI:_create_ui_elements()
	self._num_players = 0
	self._num_rewards = 0
	...
	mutator_summary1_widget.content.item = { mutators = { } }
	local mutator_summary2_widget = widgets_by_name.mutator_summary2
	mutator_summary2_widget.content.item = { mutators = { } }
	local mutator_summary3_widget = widgets_by_name.mutator_summary3
	mutator_summary3_widget.content.item = { mutators = { } }
...
function IngamePlayerListUI:_setup_mutator_data()
...
function IngamePlayerListUI:_get_deus_current_node()
	...
	if mechanism_name ~= "deus" then return
	end
...
function IngamePlayerListUI:_setup_chaos_wastes_info()
...
function IngamePlayerListUI:_setup_deed_reward_data(deed_reward_data)
...
function IngamePlayerListUI:_setup_mission_data(level_settings)
	...
	for _, setting in ipairs(mission_settings) do
		...
		if total_amount then
			...
			local size_w = texture_size [1] local size_h = texture_size [2]
			local size_scale = 1
...
function IngamePlayerListUI:_sync_missions()
...
function IngamePlayerListUI:_get_item_amount_by_mission_name(mission_name)
...
function IngamePlayerListUI:_create_player_portrait(portrait_frame, portrait_image, player_level_text)
...
function IngamePlayerListUI:_setup_weave_display_info()
...
function IngamePlayerListUI:_setup_weave_objectives(weave_template)
...
function IngamePlayerListUI:_assign_objective(widget, title_text, text, icon, spacing)
...
function IngamePlayerListUI:destroy()
...
function IngamePlayerListUI:_set_level_name(name)
	self:_set_widget_text("game_level", name)
end

function IngamePlayerListUI:_set_difficulty_name(name)
	self:_set_widget_text("game_difficulty", name)
end

function IngamePlayerListUI:_set_widget_text(widget_name, text)
	local widget = self._widgets_by_name [widget_name]
	widget.content.text = text
end
function IngamePlayerListUI:_set_simple_widget_texture(widget_name, texture)
	local widget = self._widgets_by_name [widget_name]
	widget.content.texture_id = texture
end

function IngamePlayerListUI:_add_player(player)
	...
	local player_data = { unit_spawned = false,
...
function IngamePlayerListUI:_update_widgets()
...
local slot_to_block_by_wield_slot = { slot_ranged = "slot_melee", slot_melee = "slot_ranged" }




function IngamePlayerListUI:_update_player_information(dt, t)
...
function IngamePlayerListUI:_create_portrait_frame_widget(frame_settings_name, portrait_texture, player_level_text)
...
function IngamePlayerListUI:_get_ping_texture_by_ping_value(ping_value)
	if ping_value <= 125 then

		do return "ping_icon_01", "low_ping_color" end
	elseif ping_value > 125 and ping_value <= 175 then

		do return "ping_icon_02", "medium_ping_color" end
...
function IngamePlayerListUI:_ignoring_chat_peer_id(peer_id)
	if IS_WINDOWS then
...
function IngamePlayerListUI:_ignore_chat_message_from_peer_id(peer_id)
	if IS_WINDOWS then
		local chat_gui = Managers.chat.chat_gui
...
function IngamePlayerListUI:_remove_ignore_chat_message_from_peer_id(peer_id)
	if IS_WINDOWS then
		local chat_gui = Managers.chat.chat_gui
...
function IngamePlayerListUI:_muted_peer_id(peer_id)
	if IS_XB1 then
...
function IngamePlayerListUI:_ignore_voice_message_from_peer_id(peer_id)
	if IS_XB1 then
		if Managers.voice_chat then
...
function IngamePlayerListUI:_remove_ignore_voice_message_from_peer_id(peer_id)
	if IS_XB1 then
		if Managers.voice_chat then
...
function IngamePlayerListUI:post_update(dt)
	return end

function IngamePlayerListUI:update(dt, t)
		...
		if not in_fade_active and input_service:get("ingame_player_list_toggle") and not is_matchmaking then
			if not self._active then
				...
				if not self._cursor_active then
					...
					input_manager:capture_input({ "keyboard", "gamepad", "mouse" }, 1, "player_list_input", "IngamePlayerListUI")
	...
	if self._active then
		if input_service:get("activate_ingame_player_list") and
		not self._cursor_active then
			ShowCursorStack.push()
		...
		if self._local_player.is_server and not self._is_in_inn and not private_checkbox_content.is_disabled then
			...
			if self._private_setting_enabled and
			private_checkbox_hotspot.on_release then
...
function IngamePlayerListUI:_on_resolution_changed()
...
function IngamePlayerListUI:_set_privacy_enabled(enabled, animate)
...
function IngamePlayerListUI:on_save_ended_callback()
	print("[IngamePlayerWiew] - settings saved")
end

function IngamePlayerListUI:is_active()
	return self._active
end

function IngamePlayerListUI:is_focused()
	return self._active and self._cursor_active
end

function IngamePlayerListUI:input_service()
	local input_manager = self._input_manager
	return input_manager:get_service("player_list_input")
end

function IngamePlayerListUI:set_visible(visible)
	if self._active and not visible then
		self:_set_active(false)
	end
end

function IngamePlayerListUI:_set_active(active)
	local chat_gui = Managers.chat.chat_gui
	if active then
		self:_on_resolution_changed()
		...
		local matchmaking_type_lookup = { deus = "area_selection_morris_name", deed = "start_game_window_mutator_title", tutorial = "lb_game_type_prologue", event = "start_game_window_event_title", ["n/a"] = "lb_game_type_none", custom = "lb_game_type_custom" }
	...
	if self._cursor_active then
		...
		local input_manager = self._input_manager
		input_manager:release_input({ "keyboard", "gamepad", "mouse" }, 1, "player_list_input", "IngamePlayerListUI", true)
...
function IngamePlayerListUI:_update_fade_in_duration(dt)
...
local temp_players = { }
function IngamePlayerListUI:_update_player_list(dt, t)
...
local EMPTY_TABLE = { }
function IngamePlayerListUI:_update_dynamic_widget_information(dt, t)
	local network_manager = Managers.state.network
...
local ITEM_TOOLTIP_RENDER_SETTINGS = { alpha_multiplier = 0 }



function IngamePlayerListUI:_update_item_tooltip_widget(item, offset)
	local ui_scenegraph = self._ui_scenegraph
	local ui_renderer = self._ui_renderer
...
function IngamePlayerListUI:_update_difficulty()
...
function IngamePlayerListUI:_draw(dt)
	local ui_renderer = self._ui_renderer
...
function IngamePlayerListUI:_can_host_solo_kick()
	return self._is_server and Managers.player:num_human_players() == 2
end


function IngamePlayerListUI:kick_player(player, t)
...
function IngamePlayerListUI:kick_player_available(player)
...
function IngamePlayerListUI:_show_profile_by_peer_id(peer_id)
...
function IngamePlayerListUI:event_weave_objective_synced()
	self:_setup_weave_display_info()
end


function IngamePlayerListUI:_has_client_duplicate(player_list, peer_id)
	local occurences = 0
	...
	if duplicate_found then
		...
		do return true, duplicate_index end
	else
		return false
	end
end