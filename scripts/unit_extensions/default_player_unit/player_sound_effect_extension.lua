...
local aggro_breed_ranges_sq = { skaven_storm_vermin_champion = 100, chaos_exalted_champion_warcamp = 100, chaos_exalted_sorcerer = 400, chaos_warrior = 100, skaven_storm_vermin_warlord = 100, skaven_rat_ogre = 100, chaos_exalted_champion_norsca = 100, beastmen_minotaur = 100, chaos_troll = 100, chaos_spawn = 100, skaven_stormfiend_boss = 100, chaos_spawn_exalted_champion_norsca = 100, skaven_stormfiend = 100 }
...
function PlayerSoundEffectExtension:init(extension_init_context, unit, extension_init_data)
	self._unit = unit
...
function PlayerSoundEffectExtension:extensions_ready(world, unit)
...
function PlayerSoundEffectExtension:update(unit, input, dt, context, t)
...
function PlayerSoundEffectExtension:destroy()
	return end


function PlayerSoundEffectExtension:_update_recent_hits(dt)
...
function PlayerSoundEffectExtension:_update_recent_kills(dt)
...
function PlayerSoundEffectExtension:_update_aggro_ranges(dt)
...
function PlayerSoundEffectExtension:_set_hit_count(num_hits)
...
function PlayerSoundEffectExtension:_set_kill_count(num_kills)
...
function PlayerSoundEffectExtension:_update_camera_look_angle()
...
function PlayerSoundEffectExtension:_update_specials_proximity(dt)
	...
	if self._broadphase_update_timer <= 0 then
		...
		local state = nil
		for i = 1, num_nearby_ai_units do repeat
				...
				state = Vector3.distance_squared(own_position, ai_position) <= BROADPHASE_NEAR_RANGE ^ 2 and
				"near" or state or
...
function PlayerSoundEffectExtension:add_hit()
...
function PlayerSoundEffectExtension:add_kill()
...
function PlayerSoundEffectExtension:dodge()
	if self._first_person_unit then
		Unit.flow_event(self._first_person_unit, "lua_dodge")
	end
end

function PlayerSoundEffectExtension:dodged_attack()
	if self._first_person_unit then
		Unit.flow_event(self._first_person_unit, "lua_dodged_attack")
	end
end

function PlayerSoundEffectExtension:melee_kill()
	if self._first_person_unit then
		Unit.flow_event(self._first_person_unit, "lua_melee_kill")
	end
end

function PlayerSoundEffectExtension:aggro_unit_changed(enemy_unit, has_aggro)
...
function PlayerSoundEffectExtension:get_music_aggro_state()
