...
InputFilters.virtual_axis = {
	...
	edit_types = { { "up", "keymap", "soft_button", "input_mappings" },
		{ "down", "keymap", "soft_button", "input_mappings" },
		{ "left", "keymap", "soft_button", "input_mappings" },
		{ "right", "keymap", "soft_button", "input_mappings" },
		{ "forward", "keymap", "soft_button", "input_mappings" },
		{ "back", "keymap", "soft_button", "input_mappings" } } }
...
InputFilters.scale_vector3_xy = {
	...
	edit_types = { { "multiplier_x", "number" },
		{ "multiplier_y", "number" } } }
...
InputFilters.scale_vector3_xy_accelerated_x_inverted = {
	...
	update = function (filter_data, input_service)
		...
		local time = Application.time_since_launch() if filter_data.turnaround_threshold and filter_data.turnaround_threshold <= math.abs(val.x) and math.sign(val.x) ~= filter_data.input_x_turnaround then
			filter_data.input_x_turnaround = math.sign(val.x)
			filter_data.input_x_turnaround_t = time
		elseif filter_data.threshold <= math.abs(val.x) and math.sign(val.x) ~= filter_data.input_x then
			filter_data.input_x = math.sign(val.x)
