...
function HeroWindowCharacterSelectionConsole:on_enter(params, offset)
...
function HeroWindowCharacterSelectionConsole:_select_hero(profile_index, career_index, initial_selection)
	...
	if not initial_selection then
		if spawn_character then
			Managers.state.event:trigger("respawn_hero", { hero_name = hero_name, career_index = career_index })
		else
...
function HeroWindowCharacterSelectionConsole:_update_selectable(selectable, dlc_name)
...
function HeroWindowCharacterSelectionConsole:_set_hero_icon_selected(index)
	for icon_index, widget in ipairs(self._hero_icon_widgets) do
		widget.content.selected = icon_index == index
	end
end

function HeroWindowCharacterSelectionConsole:_set_hero_info(hero_name, career_name, level)
...
function HeroWindowCharacterSelectionConsole:_start_transition_animation(key, animation_name)
	local params = {
...
function HeroWindowCharacterSelectionConsole:_create_ui_elements(params, offset)
...
function HeroWindowCharacterSelectionConsole:_setup_hero_selection_widgets()
	local hero_widgets = { }
...
function HeroWindowCharacterSelectionConsole:on_exit(params)
...
function HeroWindowCharacterSelectionConsole:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function HeroWindowCharacterSelectionConsole:post_update(dt, t)

	return end

function HeroWindowCharacterSelectionConsole:_update_animations(dt)
...
function HeroWindowCharacterSelectionConsole:_update_input(dt)
	...
	elseif
	confirm_pressed and select_button.content.dlc_name then
		self:_play_sound("play_gui_start_menu_button_click")
...
function HeroWindowCharacterSelectionConsole:_handle_mouse_selection()
	local hero_widgets = self._hero_widgets
	local num_max_rows = self._num_max_hero_rows
...
function HeroWindowCharacterSelectionConsole:_handle_gamepad_selection(input_service)
	local selected_row = self._selected_hero_row
	local selected_column = self._selected_hero_column
...
function HeroWindowCharacterSelectionConsole:set_focus(focused)
	self._focused = focused
end

function HeroWindowCharacterSelectionConsole:_exit(selected_level)
	self.exit = true
	self.exit_level_id = selected_level
end

function HeroWindowCharacterSelectionConsole:_draw(dt)
...
function HeroWindowCharacterSelectionConsole:_play_sound(event)
