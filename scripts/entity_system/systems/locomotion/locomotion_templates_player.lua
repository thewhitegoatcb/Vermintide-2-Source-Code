...
if not DETAILED_PROFILING then
	...
	function detailed_profiler_start() return end
	function detailed_profiler_stop() return end
end
...
function T.init(data, nav_world)
	data.nav_world = nav_world
	data.all_update_units = { }
	data.all_disabled_units = { }

	if UPDATE_STATISTICS then
		self.drawer = Managers.state.debug:drawer({ mode = "immediate", name = "PlayerUnitLocomotionExtension" })
		GraphHelper.create("PlayerUnitLocomotionExtension", { "move_speed" }, { "move_velocity" })
...
function T.update_average_velocity(data, t, dt)
...
function T.update_movement(data, t, dt)
	...
	for unit, extension in pairs(data.all_update_units) do
		...
		if on_ground then
			...
			local hits, num_hits = PhysicsWorld.immediate_overlap(physics_world,
			"shape", "sphere",
			"position", POSITION_LOOKUP [unit],
			"rotation", rotation,
			"size", size,
			"collision_filter", extension._default_mover_filter)
...
function T.update_network(data, dt)
...
function T.update_statistics(data, t, dt)
...
function T.update_rotation(data, t, dt)
...
function T.update_disabled_units(data, dt)
...
function T.update_debug_anims(data)
