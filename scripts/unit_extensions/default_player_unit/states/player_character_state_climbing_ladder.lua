...
function PlayerCharacterStateClimbingLadder:init(character_state_init_context)
...
function PlayerCharacterStateClimbingLadder:on_enter_animation_event()
...
function PlayerCharacterStateClimbingLadder:on_enter(unit, input, dt, context, t, previous_state, params)
	local unit = self.unit
...
function PlayerCharacterStateClimbingLadder:on_exit(unit, input, dt, context, t, next_state)
	local loc_ext = self.locomotion_extension
...
function PlayerCharacterStateClimbingLadder:update(unit, input, dt, context, t)
	local csm = self.csm
	local unit = self.unit
	local input_extension = self.input_extension
	local status_extension = self.status_extension
...
function PlayerCharacterStateClimbingLadder:_move_on_ladder(first_person_extension, rotation, input_extension, locomotion_extension, unit, speed, ladder_offset)
...
function PlayerCharacterStateClimbingLadder:on_ladder_animation()
	local unit = self.unit
	local movement_settings_table = PlayerUnitMovementSettings.get_movement_settings_table(unit)
	local current_velocity = self.locomotion_extension:current_velocity()
	local current_velocity_z = current_velocity.z
