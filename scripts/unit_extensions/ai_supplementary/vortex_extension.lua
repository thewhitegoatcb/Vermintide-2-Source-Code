...
function VortexExtension:init(extension_init_context, unit, extension_init_data)
...
function VortexExtension:_create_nav_cost_maps(ai_system, position, full_outer_radius, high_cost_type, medium_cost_type)
	local num_volumes = 1
...
function VortexExtension:extensions_ready(world, unit)
	...
	blackboard.vortex_data = { idle_time = 0, height = 5, inner_radius = 2, start_lerp_height = 5, start_lerp_fx_radius = 8, wander_time = 0, start_lerp_inner_radius = 2, current_raycast_rad = 0, num_players_inside = 0, outer_radius = 8,
		ai_units_inside = { },
		players_inside = { },
		players_ejected = { },
		...
		height_ring_buffer = { write_index = 1, buffer = Script.new_array(max_size), max_size = max_size },
		...
		inner_radius_ring_buffer = { write_index = 1, buffer = Script.new_array(max_size), max_size = max_size },
...
function VortexExtension:destroy()
...
function VortexExtension:update(unit, input, dt, context, t)
	local blackboard = self.blackboard
	local vortex_template = self.vortex_template
...
function VortexExtension:_update_nav_cost_map_volumes(position, full_outer_radius, nav_world, ai_system, navigation_extension, locomotion_extension)
...
function VortexExtension:_update_height(unit, t, dt, vortex_template, vortex_data)
...
function VortexExtension:_update_radius(unit, t, dt, nav_world, traverse_logic, vortex_template, vortex_data)
	local unit_position = position_lookup [unit]
...
function VortexExtension:control_size(unit, t, dt, nav_world, traverse_logic, vortex_template, vortex_data)
...
function VortexExtension:_update_attract_players(unit, blackboard, vortex_data, vortex_template, t, center_pos, minimum_height_diff, inner_radius, outer_radius, falloff_radius, allowed_distance)
	local nav_world = blackboard.nav_world
	local physics_world = vortex_data.physics_world
	...
	local land_test_above = 15 local land_test_below = 15
		...
		for i = 1, num_player_and_bots do
			...
			elseif valid_vortex_target and not target_status_extension:is_in_vortex() and player_distance < outer_radius and minimum_height_diff <= height and height < vortex_height then
				if inner_radius < player_distance then
					...
					players_inside [player_unit] = { vortex_eject_height = vortex_eject_height, vortex_eject_time = t + vortex_template.player_in_vortex_max_duration }
					vortex_data.num_players_inside = vortex_data.num_players_inside + 1
				end
			end
		end
...
local ai_units = { }
function VortexExtension:_update_attract_outside_ai(vortex_data, blackboard, vortex_template, center_pos, minimum_height_diff, inner_radius, outer_radius, falloff_radius)
	local vortex_height = vortex_data.height
...
function VortexExtension:_update_attract_inside_ai(blackboard, vortex_data, vortex_template, dt, center_pos, inner_radius, allowed_distance)
...
function VortexExtension:attract(unit, t, dt, blackboard, vortex_template, vortex_data, center_pos, inner_radius, outer_radius)
...
function VortexExtension:is_position_inside(position, min_allowed_distance)
	local blackboard = self.blackboard
	local vortex_data = blackboard.vortex_data
...
function VortexExtension:debug_render_vortex(t, dt, pos, fx_radius, inner_radius, outer_radius, spin_speed, height)
