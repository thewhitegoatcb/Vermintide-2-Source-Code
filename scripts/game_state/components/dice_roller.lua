...
local directions = { { rot = 0,
		up = { 0, 1, 0 } }, {
		up = { 1, 0, 0 }, rot = math.pi / 2 }, {
		up = { 0, 1, 0 }, rot = math.pi / 2 }, {
		up = { 0, 1, 0 }, rot = -math.pi / 2 }, {
		up = { 1, 0, 0 }, rot = -math.pi / 2 }, {
		up = { 0, 1, 0 }, rot = math.pi }, {
		up = { 0, 0, 1 }, rot = math.pi / 2 }, {
		up = { 0, 0, 1 }, rot = -math.pi / 2 }, {
		up = { 0, 0, 1 }, rot = math.pi } }


local rotation_mappings = { { 1, 2, 3, 4, 5, 6 },
...
local dice_types_mapping = { "wood", "metal", "gold", "warpstone" }
...
local dice_type_success_amounts = { gold = 3, metal = 4, warpstone = 1, wood = 5 }
...
local unit_names = { gold = "units/props/dice_bowl/dice_tier_04", metal = "units/props/dice_bowl/dice_tier_02", warpstone = "units/props/dice_bowl/dice_tier_05", wood = "units/props/dice_bowl/dice_tier_01" }
...
local broken_heights = { gold = 1.36, metal = 1.34, warpstone = 1.18, wood = 1.18 }
...
function DiceRoller:init(world, dice_keeper, rewards, hero_name)
	self.world = world
...
function DiceRoller:destroy()
	if not self.post_cleanup_done then
		self:cleanup_post_roll()
	end
end

function DiceRoller:_request_from_backend(hero_name)
...
function DiceRoller:_check_for_achievement(reward_backend_id)
	local backend_items = Managers.backend:get_interface("items")
...
function DiceRoller:poll_for_backend_result()
...
function DiceRoller:is_rolling()
	return self.rolling
end

function DiceRoller:is_completed()
	return self.rolling_finished
end

function DiceRoller:has_rerolls()
	return self.needs_rerolls
end

local dice_visibility_groups = { normal = "lvl1" }
...
function DiceRoller:_add_to_glow_list(unit)
	self._glow_dice [#self._glow_dice + 1] = { time = 0, unit = unit }
	WwiseWorld.trigger_event(self.wwise_world, "hud_dice_game_glow")
end

function DiceRoller:_update_glow(dt)
...
function DiceRoller:_create_success_table(success_list)
	local success_table = { }
	...
	for _, dice_type in ipairs(dice_types_mapping) do
		...
		for i = 1, dice_amount do
			...
			local data = { dice_type = dice_type, success = success }

			success_table [#success_table + 1] = data
			if success then success_count = success_count + 1
			end
		end
	end
...
function DiceRoller:roll_dices()
	fassert(self._got_backend_result, "Trying to roll dice before response from backend")
...
function DiceRoller:simulate_dice_rolls(success_list)
	fassert(self._got_backend_result, "Trying to roll dice before response from backend")
		...
		for j = 1, #initial_positions do
			repeat
				if Vector3.length(initial_position - initial_positions [j]) < 2 then
					...
					valid_pos = true end until valid_pos
		end
	...
	for i = 1, num_dice do
		...
		dice_simulation_settings [i] = { dice_result = 0, unit = dice_unit,
			dice_type = dice_type,
...
function DiceRoller:run_simulation(dice_simulation_settings)
	fassert(self._got_backend_result, "Trying to roll dice before response from backend")
	local world = self.simulation_world
...
function DiceRoller:calculate_results(dice_simulation_settings)
...
function DiceRoller:get_dice_result(unit, dice_type)
	...
	if broken_heights [dice_type] <= Unit.world_position(unit, 0).z then
		dice_result = 0
	end
...
function DiceRoller:alter_rotations(dice_simulation_settings)
	local amount = #dice_simulation_settings

	for i = 1, amount do repeat
			local data = dice_simulation_settings [i]
			...
			for i = 1, num_rotations do
				...
				rotations [i] = QuaternionBox(new_rotation) end until true
	end

end

function DiceRoller:get_dice_results()
...
function DiceRoller:update(dt)
	...
	for dice_unit, data in pairs(dice_units) do
		...
		if data.completed_index <= lower_index and data.success and not data.highlighted then
...
function DiceRoller:cleanup_post_roll()
	local dice_units = self.dice_units
...
if Development.parameter("dice_chance_simulation") then
	local dice_configurations = { { 7, 0, 0, 0 },
		{ 6, 0, 1, 0 },
		{ 6, 1, 0, 0 },
		{ 6, 0, 0, 1 },
		{ 5, 0, 2, 0 },
	...
	local probabilities = { 0.3333333333333333, 0.5, 0.6666666666666666, 1 }
