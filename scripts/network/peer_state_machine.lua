...
function PeerStateMachine.create(server, peer_id, xb1_preconnect)
	local state_data = {
		server = server,
		peer_id = peer_id }
	...
	function state_data.change_state(_, new_state)
		...
		__index = function (self, k)
			...
			if not state_machine_member then
				...
				if not fetched_function then
					...
					do return new_function end
				else
					do return fetched_function end
				end
			else
...
function PeerStateMachine:has_function(function_name)
	return not not self.current_state [function_name]
end

function PeerStateMachine:update(dt)
