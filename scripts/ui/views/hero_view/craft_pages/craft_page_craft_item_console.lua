...
local definitions = local_require("scripts/ui/views/hero_view/craft_pages/definitions/craft_page_craft_item_console_definitions")
...
function CraftPageCraftItemConsole:on_enter(params, settings)
...
function CraftPageCraftItemConsole:setup_recipe_requirements()
...
function CraftPageCraftItemConsole:reset_requirements(num_required_ingredients)
...
function CraftPageCraftItemConsole:create_ui_elements(params)
...
function CraftPageCraftItemConsole:on_exit(params)
...
function CraftPageCraftItemConsole:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function CraftPageCraftItemConsole:post_update(dt, t)

	return end

function CraftPageCraftItemConsole:_update_animations(dt)
...
function CraftPageCraftItemConsole:_is_button_pressed(widget)
...
function CraftPageCraftItemConsole:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageCraftItemConsole:_is_button_held(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageCraftItemConsole:_handle_input(dt, t)
...
function CraftPageCraftItemConsole:_handle_craft_input_progress(progress)
	return self.parent:_set_input_progress(progress)
end

function CraftPageCraftItemConsole:craft_result(result, error, reset_slots)
	if not error then
		self._craft_result = result
	end
end

function CraftPageCraftItemConsole:reset()
...
function CraftPageCraftItemConsole:present_results()
	local item_grid = self._item_grid
	item_grid:clear_locked_items()
	item_grid:update_items_status()
...
function CraftPageCraftItemConsole:on_craft_completed()
...
function CraftPageCraftItemConsole:_update_craft_items()
	local super_parent = self.super_parent
	local item_grid = self._item_grid
...
function CraftPageCraftItemConsole:_remove_craft_item(backend_id, slot_index)
	local craft_items = self._craft_items
...
function CraftPageCraftItemConsole:_add_craft_item(backend_id, slot_index, ignore_sound)
...
function CraftPageCraftItemConsole:_clear_item_grid()
	local craft_items = self._craft_items
	local super_parent = self.super_parent
...
function CraftPageCraftItemConsole:_has_added_item_by_id(backend_id)
	local craft_items = self._craft_items
...
function CraftPageCraftItemConsole:_set_craft_button_disabled(disabled)
...
function CraftPageCraftItemConsole:_exit(selected_level)
	self.exit = true
	self.exit_level_id = selected_level
end

function CraftPageCraftItemConsole:draw(dt)
...
function CraftPageCraftItemConsole:_play_sound(event)
	self.super_parent:play_sound(event)
end

function CraftPageCraftItemConsole:_set_craft_button_text(text, localize)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.craft_button
	widget.content.button_text = localize and Localize(text) or text
end

function CraftPageCraftItemConsole:_add_crafting_material_requirement(index, item_key, amount_text, has_required_amount)
