...
local RPCS = { "rpc_sync_door_state", "rpc_sync_boss_door_state" }




local extensions = { "DoorExtension", "SimpleDoorExtension", "BossDoorExtension", "BigBoyDestructibleExtension", "CrawlSpaceExtension" }
...
function DoorSystem:init(entity_system_creation_context, system_name)
...
function DoorSystem:on_add_extension(world, unit, extension_name, ...)
	...
	if extension_name == "BossDoorExtension" then
		...
		for i = 0, 2 do repeat
				...
				boss_doors_in_section [#boss_doors_in_section + 1] = unit until true
		end
	end
...
function DoorSystem:extensions_ready(world, unit, extension_name)
	if extension_name == "CrawlSpaceExtension" then
...
function DoorSystem:update(context, t)
...
function DoorSystem:get_doors(position, radius, result)
	return Broadphase.query(self._broadphase, position, radius, result)
end


function DoorSystem:get_boss_door_units()
	local boss_doors = self._boss_doors
...
function DoorSystem:on_remove_extension(unit, extension_name)
...
function DoorSystem:close_boss_doors(map_section, group_id, breed_name)
	...
	if boss_doors then
		...
		active_groups_in_section [#active_groups_in_section + 1] = { active = false, group_id = group_id }
	end
end

function DoorSystem:open_boss_doors(map_section)
	local boss_doors = self._boss_doors [map_section]
	local network_manager = Managers.state.network
	local network_transmit = network_manager.network_transmit
...
function DoorSystem:get_crawl_space_tunnel_units(disabled_too)
...
function DoorSystem:get_crawl_space_spawner_units()
	if not self._crawl_spaces_ready then
		return
	end
...
function DoorSystem:rpc_sync_door_state(channel_id, level_object_id, door_state_id)
	local level = LevelHelper:current_level(self.world)
...
function DoorSystem:rpc_sync_boss_door_state(channel_id, level_object_id, door_state_id, breed_id)
	local level = LevelHelper:current_level(self.world)
	local door_unit = Level.unit_by_index(level, level_object_id)
	local door_extension = ScriptUnit.has_extension(door_unit, "door_system")
