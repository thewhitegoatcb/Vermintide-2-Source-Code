...
local projectiles = { }
function ProjectileTest.add_simulated_projectile(p1, p2, projectile_speed, projectile_angle, target_velocity, gravity)
	...
	if projectile_speed then
		...
		if angle then
			...
			projectiles [#projectiles + 1] = { type = "known_speed", last_dot = 0, t = 0, last_pos = Vector3Box(p1), p1 = Vector3Box(p1), p2 = Vector3Box(new_p2), x_vel_0 = x_vel_0, y_vel_0 = y_vel_0, vec_flat = Vector3Box(vec_flat), gravity = gravity }
		end
	elseif projectile_angle then
		...
		if speed then
			...
			projectiles [#projectiles + 1] = { type = "known_angle", last_dot = 0, t = 0, last_pos = Vector3Box(p1), p1 = Vector3Box(p1), p2 = Vector3Box(new_p2), x_vel_0 = x_vel_0, y_vel_0 = y_vel_0, vec_flat = Vector3Box(vec_flat), gravity = gravity }
		end
	end
end

function ProjectileTest.simulate_projectiles(physics_world, dt)
...
function ProjectileTest.run_projectile_test(world, t, dt)
...
function ProjectileTest.trajectory_test(set_speed, set_angle)
	for i = 20, 35 do
		local p1 = Vector3(0, 0, 0)
...
function ProjectileTest.draw_projectile_trajectory(p1, p2, gravity, projectile_speed)
