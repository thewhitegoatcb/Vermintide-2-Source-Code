...
function SkipInputUI:init(parent, context)
	self._parent = parent
	self._ui_renderer = context.ui_renderer
	self._context = context
	self._skip = false
	self._render_settings = { alpha_multiplier = 0, internal_alpha_multiplier = 0, snap_pixel_positions = false }

	self:_create_ui_elements()
end

function SkipInputUI:_create_ui_elements()
...
function SkipInputUI:destroy()

	return end

function SkipInputUI:update(dt, t, input_service, parent_render_settings)
	self:_update_input(dt, t, input_service, parent_render_settings)
	self:_draw(dt, t, input_service, parent_render_settings)
end

function SkipInputUI:_update_input(dt, t, input_service, parent_render_settings)
...
function SkipInputUI:skip()
	self._skip = true
end

function SkipInputUI:skipped()
	local skip = self._skip
	self._skip = false
	return skip
end

function SkipInputUI:_draw(dt, t, input_service, parent_render_settings)
