...
FormationUtils = {
	...
	make_formation = function (formation_template, spacing)
		local formation = { arrangement = { }, formation_template = formation_template, x = formation_template.x, y = formation_template.y }
		...
		for j = 0, num_units_y - 1 do
			for i = 0, num_units_x - 1 do
				k = k + 1
				arrangement [k] = { i * spacing - half_width, j * spacing - half_height }
			end
		end
...
function FormationUtils.make_encampment(encampment_template)


	local encampment = { army_size = 0, encampment_template = encampment_template }
	local army_size = 0
	local size = nil
...
function FormationUtils.draw_encampment(encampment, pos, rot, drawer)
...
function FormationUtils.draw_formation(formation, pos, rot, color, drawer)
...
function FormationUtils.spawn_formation(formation, pos, rot, breed_name, group_template, side_id)
...
function FormationUtils.spawn_encampment(encampment, pos, rot, unit_composition, side_id)
	local group_template = { template = "encampment",
		id = Managers.state.entity:system("ai_group_system"):generate_group_id(),

		size = encampment.army_size,
		group_data = { sneaky = true, idle = true,
