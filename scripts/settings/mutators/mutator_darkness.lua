return { description = "description_mutator_darkness", display_name = "display_name_mutator_darkness", disable_environment_variations = true, icon = "mutator_icon_darkness",
	...
	server_update_function = function (context, data, dt, t)
		...
		for i = 1, num_player_units do
			...
			if not status_extension:is_disabled() then break
			end
		end
		...
		local extension_init_data = {
			pickup_system = { has_physics = true, pickup_name = "mutator_torch", spawn_type = "guaranteed" },
	...
	client_start_function = function (context, data)
		...
		if not level_settings.camera_backlight then
			...
			if light then
				local backlight_settings = { intensity = 0.015, start_falloff = 0, end_falloff = 5,
