...
local RENDER_CAPS = { "d3d12", "dlss_supported", "dlss_g_supported", "reflex_supported", "use_deferred_contexts" }
...
function ImguiRenderCaps:init()
	return end

function ImguiRenderCaps:update()
	return end

local function set_all(t, k, v)
	for i = 1, #k do t [k [i]] = v end
end

function ImguiRenderCaps:draw()
	...
	for i = 1, #RENDER_CAPS do
		...
		if Imgui.radio_button("false##" .. cap, val == false) then val = false end Imgui.same_line()
		if Imgui.radio_button("true##" .. cap, val == true) then val = true end Imgui.same_line(30)
		if Imgui.small_button("Clear##" .. cap) then val = nil end
...
function ImguiRenderCaps:is_persistent()
	return false
end