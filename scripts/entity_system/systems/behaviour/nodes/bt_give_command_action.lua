...
local command_to_query_concept = { clan_rat_attack = "commanding" }
...
function BTGiveCommandAction:init(...)
	BTGiveCommandAction.super.init(self, ...)
end

BTGiveCommandAction.name = "BTGiveCommandAction"

function BTGiveCommandAction:enter(unit, blackboard, t)
...
function BTGiveCommandAction:leave(unit, blackboard, t, reason, destroy)
...
function BTGiveCommandAction:run(unit, blackboard, t, dt)
	...
	if blackboard.anim_cb_stormvermin_voice then
		...
		elseif order == "cheer" then --[[ Nothing --]]
		elseif order == "rally" then --[[ Nothing --]]
