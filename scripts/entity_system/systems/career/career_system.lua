...
local extension_list = { "CareerExtension" }



local RPCS = { "rpc_server_reduce_activated_ability_cooldown", "rpc_server_reduce_activated_ability_cooldown_percent", "rpc_reduce_activated_ability_cooldown", "rpc_reduce_activated_ability_cooldown_percent", "rpc_ability_activated" }
...
function CareerSystem:init(entity_system_creation_context, system_name)
...
function CareerSystem:destroy()
	self.network_event_delegate:unregister(self)
	self.network_event_delegate = nil
end

function CareerSystem:on_add_extension(world, unit, extension_name, extension_init_data)
...
function CareerSystem:on_remove_extension(unit, extension_name)
	self.unit_extensions [unit] = nil
	CareerSystem.super.on_remove_extension(self, unit, extension_name)
end

function CareerSystem:server_reduce_activated_ability_cooldown(units, amount, ability_id, ignore_paused)
	if not units then
		return
	end
...
function CareerSystem:rpc_server_reduce_activated_ability_cooldown(sender, unit_game_object_ids, amount, ability_id, ignore_paused)
...
function CareerSystem:rpc_reduce_activated_ability_cooldown(sender, unit_game_object_id, amount, ability_id, ignore_paused)
...
function CareerSystem:rpc_server_reduce_activated_ability_cooldown_percent(sender, unit_game_object_id, amount, ability_id, ignore_paused)
	local network_transmit = self.network_transmit
	if self.is_server then
		local unit_storage = self.unit_storage
		local player_manager = self.player_manager
...
function CareerSystem:rpc_reduce_activated_ability_cooldown_percent(sender, unit_game_object_id, amount, ability_id, ignore_paused)
	local unit = self.unit_storage:unit(unit_game_object_id)
	local career_extension = self.unit_extensions [unit]
	if career_extension then
...
function CareerSystem:rpc_ability_activated(channel_id, unit_game_object_id, ability_id)
	local unit = self.unit_storage:unit(unit_game_object_id)
