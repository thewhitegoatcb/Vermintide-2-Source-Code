...
function LureWhereaboutsExtension:init(extension_init_context, unit, extension_init_data)
	self._unit = unit
	local nav_world = Managers.state.entity:system("ai_system"):nav_world()
	self._closest_positions = { }
	local pos = Unit.world_position(unit, 0)

	local above = 1 local below = 5
...
function LureWhereaboutsExtension:destroy()
	return end

function LureWhereaboutsExtension:closest_positions_when_outside_navmesh()
