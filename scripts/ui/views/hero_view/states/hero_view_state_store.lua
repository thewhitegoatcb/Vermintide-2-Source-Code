...
local item_widget_size_by_type = {
	...
	bundle = { 800, 220 },
	dlc = { 800, 220 },
	divider_horizontal = { 800, 30 },
	dlc_logo = { 800, 0 },
	dlc_feature_vertical = { 260, 440 },
	dlc_feature_vertical_long = { 800, 440 },
	dlc_feature_horizontal = { 800, 220 },
	dlc_feature_pullet_point = { 800, 40 },
	dlc_header_video = { 800, 450 },
	header_text = { 800, 450 },
	body_text = { 800, 450 },
	spacing = { 800, 40 },
	big_image = { 800, 592 } }

local item_widget_definition_functions = { default = "create_store_item_definition", header_text = "create_store_header_text_definition", item = "create_store_item_definition", dlc = "create_store_item_definition", dlc_feature_pullet_point = "create_store_dlc_feature_pullet_point_definition", dlc_feature_horizontal = "create_store_dlc_feature_horizontal_definition", dlc_feature_vertical_long = "create_store_dlc_feature_vertical_definition", body_text = "create_store_body_text_definition", divider_horizontal = "create_store_list_divider_definition", spacing = "create_store_list_spacing_definition", dlc_feature_vertical = "create_store_dlc_feature_vertical_definition", big_image = "create_store_dlc_feature_horizontal_definition", dlc_header_video = "create_store_header_video_definition", dlc_logo = "create_store_dlc_logo_definition" }
...
function HeroViewStateStore:on_enter(params)
...
function HeroViewStateStore:_apply_global_shader_flag_overrides()
...
function HeroViewStateStore:_remove_global_shader_flag_overrides()
	local global_shader_flag_overrides = StoreLayoutConfig.global_shader_flag_overrides
	if not global_shader_flag_overrides then
		return
	end
...
function HeroViewStateStore:_trigger_welcome_popup(unseen_currency_rewards)
	local reward_data = { }
	...
	local layout = { { type = "body",
			settings = { text = "welcome_currency_popup_intro_description_1", localize = true } } }


	if includes_base_game_reward then
		layout [#layout + 1] = { type = "body", settings = { text = "welcome_currency_popup_intro_description_2_no_dlc", localize = true } }

		if includes_dlc_rewards then
			layout [#layout + 1] = { type = "body", settings = { text = "welcome_currency_popup_intro_description_2", localize = true } }
		end
	elseif includes_dlc_rewards then
		layout [#layout + 1] = { type = "body", settings = { text = "welcome_currency_popup_intro_description_2", localize = true } }
		layout [#layout + 1] = { type = "body", settings = { text = "" } }
	end


	layout [#layout + 1] = { type = "summary_title", settings = { text = "welcome_currency_popup_rewards_title", localize = true, text2 = "welcome_currency_popup_currency_name" } }
	...
	for i = 1, #reward_data do
		...
		if rewarded_from == "base_game" then
			table.insert(layout, starting_line_count + 1, { type = "summary_entry", settings = { text = "welcome_currency_popup_intro_welcome_gift", localize = true, value = value } })
		else
			...
			layout [#layout + 1] = { type = "summary_entry", settings = { localize = true, text = text, value = value } }
		end
...
function HeroViewStateStore:_has_unseen_items_tab_cat()
...
function HeroViewStateStore:_setup_menu_layout()
...
function HeroViewStateStore:get_renderers()
	return self._store_ui_renderer, self._ui_top_renderer
end

function HeroViewStateStore:_create_ui_elements(params)
...
function HeroViewStateStore:_disable_player_world()
...
function HeroViewStateStore:_enable_player_world()
...
function HeroViewStateStore:_disable_store_ui_world()
...
function HeroViewStateStore:_enable_store_ui_world()
...
function HeroViewStateStore:enable_ingame_overlay()
...
function HeroViewStateStore:disable_ingame_overlay()
...
function HeroViewStateStore:_initial_windows_setups(params)
	...
	if start_state then
		...
		self:go_to_store_path({ "featured" })
	end
end

function HeroViewStateStore:window_input_service(force)
...
function HeroViewStateStore:_close_window_at_index(window_index)
...
function HeroViewStateStore:_change_window(window_index, window_name)
	local active_windows = self._active_windows
...
function HeroViewStateStore:set_layout_by_name(name)
...
function HeroViewStateStore:close_on_exit()
	return self._close_on_exit
end

function HeroViewStateStore:set_layout(index)
...
function HeroViewStateStore:set_window_input_focus(window_name)
...
function HeroViewStateStore:get_selected_layout_index()
	return self._selected_layout_index
end

function HeroViewStateStore:get_previous_selected_layout_index()
	return self._previous_selected_layout_index
end

function HeroViewStateStore:_get_layout_setting(index)
	return self._window_layouts [index]
end

function HeroViewStateStore:page_exists(page_name)
	return StoreLayoutConfig.pages [page_name] ~= nil
end


function HeroViewStateStore:go_to_store_path(path_array, keep_global_shader_flags)
...
function HeroViewStateStore:_apply_page_global_shader_flag_overrides(page)
...
function HeroViewStateStore:_remove_page_global_shader_flag_overrides()
	local pages = StoreLayoutConfig.pages
...
function HeroViewStateStore:go_to_product(product_id, optional_path, optional_product_settings, keep_global_shader_flags)
	...
	if dlc_settings then
		product = { type = "dlc", dlc_settings = dlc_settings, product_id = product_id }
		...
		if item_type == "bundle" then
			product = { type = "item", item = item, product_id = product_id, settings = optional_product_settings }
			if not new_path then
				new_path = table.clone(current_store_path)
...
function HeroViewStateStore:get_item_by_key(item_key)
	local backend_store = Managers.backend:get_interface("peddler")
...
function HeroViewStateStore:get_store_path()
	return self._store_path_array
end

function HeroViewStateStore:_step_back_in_path()
...
function HeroViewStateStore:_windows_update(dt, t)
	local active_windows = self._active_windows
	for _, window in pairs(active_windows) do
...
function HeroViewStateStore:_windows_post_update(dt, t)
	local active_windows = self._active_windows
	for _, window in pairs(active_windows) do
...
function HeroViewStateStore:transitioning()
...
function HeroViewStateStore:_wanted_state()
	local new_state = self._parent:wanted_state()
	return new_state
end

function HeroViewStateStore:wanted_menu_state()
	return self._wanted_menu_state
end

function HeroViewStateStore:clear_wanted_menu_state()
	self._wanted_menu_state = nil
end

function HeroViewStateStore:requested_screen_change_by_name(state)
	self._on_close_next_state = state
	self:close_menu()
end

function HeroViewStateStore:on_exit(params)
...
function HeroViewStateStore:_close_active_windows(force_unload)
	local active_windows = self._active_windows
	local params = self._window_params
...
function HeroViewStateStore:_update_transition_timer(dt)
...
function HeroViewStateStore:input_service()
	return self._parent:input_service()
end

function HeroViewStateStore:update(dt, t)

	return end


function HeroViewStateStore:_delayed_update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
	...
	if not self._transition_timer then
		if not transitioning then
			...
			elseif not self._item_purchase_popup and not self._welcome_popup and

			not self._login_rewards_popup then
...
function HeroViewStateStore:_update_dlc_purchases()
	...
	if unseen_rewards then
		for i = 1, #unseen_rewards do
			...
			if item then
				...
				if store_item then
					local product = { type = "item", item = store_item, product_id = item_key }
...
function HeroViewStateStore:can_afford_item(item)
...
function HeroViewStateStore:get_wallet_currency_amount(currency_type)
	local backend_store = Managers.backend:get_interface("peddler")
...
function HeroViewStateStore:is_friends_list_active()
	local friends_component_ui = self._friends_component_ui
...
function HeroViewStateStore:_handle_friend_joining()
	local friends_component_ui = self._friends_component_ui
	if friends_component_ui then
...
function HeroViewStateStore:_has_active_level_vote()
...
function HeroViewStateStore:post_update(dt, t)
...
function HeroViewStateStore:_update_animations(dt)
...
function HeroViewStateStore:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_enter
end

function HeroViewStateStore:_handle_input(dt, t)
	local input_blocked = self._input_blocked
	local window_focused = self._window_focused

	if self._draw_fullscreen_video and
	Managers.input:any_input_released() then
		self:_stop_fullscreen_video()
	end
...
function HeroViewStateStore:close_menu(ignore_sound_on_close_menu)
...
function HeroViewStateStore:_draw(input_service, dt)
...
function HeroViewStateStore:_is_button_pressed(widget)
	local content = widget.content
...
function HeroViewStateStore:play_sound(event)
	self._parent:play_sound(event)
end

function HeroViewStateStore:_start_transition_animation(key, animation_name, optional_widgets)
	local params = {
		wwise_world = self._wwise_world,
...
function HeroViewStateStore:block_input()
	self._parent:set_input_blocked(true)
	self._input_blocked = true
end

function HeroViewStateStore:unblock_input()
	self._parent:set_input_blocked(false)
	self._input_blocked = false
end

function HeroViewStateStore:input_blocked()
	return self._input_blocked
end

function HeroViewStateStore:product_purchase_request(product)
				...
				if result == 1 then
					...
					if bundle_contains then
						for _, subitem_steam_itemdefid in ipairs(bundle_contains) do
							...
							if item then
								local sub_product = { type = "bundle_item", item = { data = sub_item }, product_id = sub_product_id }
...
function HeroViewStateStore:open_login_rewards_popup()
...
function HeroViewStateStore:_show_storepage(url, dlc_name)
	...
	elseif IS_PS4 then
		...
		if dlc_name then
			...
			if product_label then
				Managers.system_dialog:open_commerce_dialog(NpCommerceDialog.MODE_PRODUCT, user_id, { product_label })
			else
				Application.error(string.format("[HeroViewStateStore:_show_storepage] No product_id for dlc: %s", dlc_name))
			end
		else
...
function HeroViewStateStore:_complete_item_purchase_request()
	self:_abort_item_purchase_request()
	self._products_version_id = ( self._products_version_id or 0 ) + 1
	self._items_bought = self._items_bought + 1
end

function HeroViewStateStore:_abort_item_purchase_request()
	self._item_purchase_popup:destroy()
	self._item_purchase_popup = nil
	self:unblock_input()
end

function HeroViewStateStore:products_version_id()
	return self._products_version_id or 0
end


function HeroViewStateStore:create_item_widget(product, scenegraph_id, masked)
	local product_type = product.type
...
function HeroViewStateStore:populate_product_widget(widget, product)
...
function HeroViewStateStore:destroy_product_widget(widget, product, force_unload)
	local content = widget.content
...
function HeroViewStateStore:_calculate_discount_textures(widget, discount)
	local content = widget.content
...
function HeroViewStateStore:_populate_item_widget(widget, product, product_id)
	...
	if package_available then
		...
		return end Application.warning("Icon package not accessable for product_id: (%s) and package_name: (%s)", product_id, package_name) content.icon = "icons_placeholder" end

function HeroViewStateStore:_set_product_price_text_comparison(widget, price_text_now, price_text_before, real_currency, platform_price_data)
	local content = widget.content
	local style = widget.style
...
function HeroViewStateStore:_set_product_price_text(widget, price_text, real_currency, platform_price_data, price_text_var_name)
...
function HeroViewStateStore:_handle_platform_price_data(widget, price_data)
	if IS_PS4 then
...
function HeroViewStateStore:_setup_ps4_price_data(widget, price_data)
	local content = widget.content
	local style = widget.style
	local spacing = 20
	...
	console_secondary_price_stroke_style.texture_size = { console_secondary_price_text_length, 1 }
	...
	console_third_price_stroke_style.texture_size = { console_third_price_text_length, 1 }
end

function HeroViewStateStore:_setup_xb1_price_data(widget, price_data)
	local content = widget.content
	local style = widget.style
	local spacing = 20
	local size = content.size
	...
	console_secondary_price_stroke_style.texture_size = { console_secondary_price_text_length + 10, 1 }

	if content.show_secondary_stroke then
		console_first_price_style.base_color = { 255, 255, 255, 0 }
		console_secondary_price_stroke_style.color = { 255, 90, 90, 90 }
		console_secondary_price_style.text_color = { 255, 90, 90, 90 }
	else
		console_first_price_style.base_color = { 255, 255, 255, 255 }
		console_secondary_price_stroke_style.color = { 255, 255, 255, 255 }
		console_secondary_price_style.text_color = { 255, 255, 255, 255 }
	end
end

function HeroViewStateStore:get_steam_item_price_text(steam_itemdefid, content, item)
...
function HeroViewStateStore:can_use_item(item)
...
function HeroViewStateStore:get_dlc_price_text(dlc_name)
...
function HeroViewStateStore:_destroy_video_player(widget, reference_name)
	local ui_top_renderer = self._ui_top_renderer
	local content = widget.content
...
function HeroViewStateStore:_populate_video_widget(widget, settings, product_id)
	self._reference_id = ( self._reference_id or 0 ) + 1
	local reference_name = product_id .. "_" .. self._reference_id
...
function HeroViewStateStore:_populate_text_widget(widget, settings, product_id)
	local content = widget.content
	local style = widget.style
...
function HeroViewStateStore:_populate_dlc_feature_horizontal_widget(widget, settings, product_id)
	local content = widget.content
	local style = widget.style
	local text = settings.text
	if settings.localize then
	...
	if package_available then
		...
		return end Application.warning("DLC icon package not accessable for product_id: (%s) and package_name: (%s)", product_id, package_name) end

function HeroViewStateStore:_populate_dlc_logo_widget(widget, settings, product_id)
	local content = widget.content
	local style = widget.style
	content.product_id = product_id
...
function HeroViewStateStore:_populate_dlc_feature_vertical_widget(widget, settings, product_id)
	local content = widget.content
	local style = widget.style
	local text = settings.text
	if settings.localize then
...
function HeroViewStateStore:_populate_dlc_widget(widget, settings, product_id, show_old_price)
	local item_rarity_textures = UISettings.item_rarity_textures
	local item_type_store_icons = UISettings.item_type_store_icons
...
function HeroViewStateStore:set_list_details_visibility(visible)
	local animations = self._animations
	local ui_animator = self._ui_animator
...
function HeroViewStateStore:set_list_details_length(length, animation_duration)
	local ui_scenegraph = self._ui_scenegraph
...
function HeroViewStateStore:_update_list_detail_animation(dt)
...
function HeroViewStateStore:_create_material_instance(gui, new_material_name, template_material_name, reference_name)
...
function HeroViewStateStore:_set_material_diffuse(gui, material_name, texture_path)
...
function HeroViewStateStore:_load_texture_package(package_name, reference_name, callback)
...
function HeroViewStateStore:_is_unique_reference_to_material(reference_name)
	local cloned_materials_by_reference = self._cloned_materials_by_reference
...
function HeroViewStateStore:_unload_texture_by_reference(reference_name, force_unload)
	local loaded_package_names = self._loaded_package_names
	local cloned_materials_by_reference = self._cloned_materials_by_reference
...
function HeroViewStateStore:_unload_all_textures()
	local force_unload = true
	local loaded_package_names = self._loaded_package_names
...
function HeroViewStateStore:_clear_unload_list()
...
function HeroViewStateStore:_update_unload()
...
function HeroViewStateStore:start_fullscreen_video(material_name, resource, sound_event)
...
function HeroViewStateStore:_stop_fullscreen_video()
...
function HeroViewStateStore:_setup_fullscreen_video_player(material_name, resource)
	self:_destroy_fullscreen_video_player()
...
function HeroViewStateStore:_destroy_fullscreen_video_player()
	local ui_top_renderer = self._ui_top_renderer
...
function HeroViewStateStore:animate_store_product(widget, dt, optional_hover)
...
function HeroViewStateStore:_animate_dlc_video_button(widget, dt, optional_hover)
	local content = widget.content
	local style = widget.style
	local hotspot = content.button_hotspot
...
function HeroViewStateStore:_animate_item_product(widget, dt, optional_hover)
