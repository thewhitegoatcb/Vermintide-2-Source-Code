...
local scenegraph_definition = {
	...
	pivot = { vertical_alignment = "bottom", parent = "root", horizontal_alignment = "left",
...
function WorldMarkerUI:init(parent, ingame_ui_context)
	self._parent = parent
	self.ui_renderer = ingame_ui_context.ui_renderer
	self.ingame_ui = ingame_ui_context.ingame_ui
	self.input_manager = ingame_ui_context.input_manager

	self._render_settings = { alpha_multiplier = 1, snap_pixel_positions = false }
...
function WorldMarkerUI:destroy()
	local event_manager = Managers.state.event
...
function WorldMarkerUI:_create_ui_elements()
...
function WorldMarkerUI:event_remove_world_marker(id)
...
function WorldMarkerUI:event_add_world_marker_unit(marker_type, unit, callback)
	...
	if on_enter then on_enter(widget)
	end
	if callback then callback(id, widget) end
end



function WorldMarkerUI:event_add_world_marker_position(marker_type, world_position, callback)
	local widget = self:_create_widget_by_type(marker_type)
	local marker = {
		type = marker_type,
...
function WorldMarkerUI:on_spectator_target_changed(spectated_player_unit)
...
function WorldMarkerUI:_register_marker(marker)
	local markers = self._markers
...
function WorldMarkerUI:_unregister_marker(marker)
	local markers = self._markers
	local markers_by_id = self._markers_by_id
	local markers_by_type = self._markers_by_type
...
function WorldMarkerUI:_create_widget_by_type(widget_type)
...
function WorldMarkerUI:update(dt, t)

	return end


function WorldMarkerUI:post_update(dt, t)
...
function WorldMarkerUI:_raycast_marker(marker)
	local widget = marker.widget
	local content = widget.content
	...
	physics_world,
	camera_position,
	Vector3.normalize(marker_position - camera_position),
	distance,
	"closest",
	"collision_filter",
	"filter_physics_projectile")
end

function WorldMarkerUI:_get_scale(settings, distance)
...
function WorldMarkerUI:_apply_scale(widget, scale)
...
function WorldMarkerUI:_convert_world_to_screen_position(camera, world_position)
	if camera then
...
function WorldMarkerUI:_normal_clamp_to_screen(x, y, screen_margins, is_behind, is_under, world_position, camera_position_center, camera_position_left, camera_position_right, camera_position_up, camera_position_down)
...
function WorldMarkerUI:_is_clamped(x, y)
	local root_size = UISceneGraph.get_size_scaled(self.ui_scenegraph, "root")
...
function WorldMarkerUI:_tutorial_clamp_to_screen(x, y, forward_dot, right_dot, settings)
...
local INDEX_ACTOR = 4
function WorldMarkerUI:_test_raycast()
	local player_unit = self.local_player.player_unit
...
function WorldMarkerUI:_get_raycast_position(unit, from, to, physics_world, collision_filter)
