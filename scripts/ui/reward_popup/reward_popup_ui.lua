...
local function speed_up_popup_pressed(input_service)
	...
	return input_service:get("toggle_menu", true) or
	input_service:get("back", true) or
	input_service:get("skip_pressed", true) or
	input_service:get("left_press")

end

function RewardPopupUI:init(level_end_view_context)
...
function RewardPopupUI:create_ui_elements()
...
function RewardPopupUI:set_input_manager(input_manager)
	self._input_manager = input_manager
	self:_setup_input()
end

function RewardPopupUI:destroy()
	self:_release_input()
...
function RewardPopupUI:set_visible(visible)
	self._is_visible = visible
end

function RewardPopupUI:update(dt)
...
function RewardPopupUI:_update_animations(dt)
...
function RewardPopupUI:draw(dt)
...
function RewardPopupUI:display_presentation(data)
...
function RewardPopupUI:is_presentation_active()
	return self._animation_presentation_data ~= nil
end

function RewardPopupUI:is_presentation_complete()
	return self._presentation_complete
end

function RewardPopupUI:on_presentation_complete()
...
function RewardPopupUI:start_presentation_animation(animation_name, widgets)
	local params = { wwise_world = self._wwise_world }
	widgets = widgets or {
...
function RewardPopupUI:_hacky_get_tooltip_size(widget)
	local ui_top_renderer = self._ui_top_renderer
	local render_settings = self._render_settings
...
function RewardPopupUI:_setup_entry_widget(entry_data, index)
	local widget_definitions = definitions.widget_definitions
	...
	elseif widget_type == "deus_icon" then
		...
		local profile_index = player:profile_index() local career_index = player:career_index()
...
function RewardPopupUI:_setup_presentation(presentation_data)
	local amount = #presentation_data

	local entries = { }
	local animation_data = { end_animation = "close", start_animation = "open", started = false, animations_played = 0, entry_play_index = 1,
	...
	animation_wait_time =
	animation_wait_time or animation_data.claim_button and 0 or 2
	...
	for i = 1, #presentation_data do
		...
		local widgets_data = { }
		local data = { enter_animation = "entry_enter", exit_animation = "entry_exit",
			index = i,
		...
		for j = 1, #presentation_entries do
			...
			widgets_data [j] = { alpha_multiplier = 0,
				widget = widget,
				height = height,
...
function RewardPopupUI:_align_entry_widgets(entry)
	local ui_scenegraph = self._ui_scenegraph
...
function RewardPopupUI:_play_animation(data, anim_name, anim_data)
...
function RewardPopupUI:_update_presentation_animation(dt)
	local animation_data = self._animation_presentation_data
...
function RewardPopupUI:_handle_input(entry)
	local input_service = self:input_service()

	entry.claimed = entry.claimed or
	input_service:get("skip_pressed", true) or
	input_service:get("confirm_press", true) or
	UIUtils.is_button_pressed(self.claim_button_widget)
...
function RewardPopupUI:set_fullscreen_effect_enable_state(enabled, progress)
...
function RewardPopupUI:_setup_input()
...
function RewardPopupUI:_acquire_input()
...
function RewardPopupUI:_release_input()
...
function RewardPopupUI:input_service()
