...
MechanismSettings = {
	adventure = { server_port = 27015, display_name = "game_mode_adventure", default_inventory = true, server_universe = "carousel", tobii_available = true, max_members = 4, vote_switch_mechanism_background = "vote_switch_mechanism_adventure_background", vote_switch_mechanism_text = "vote_switch_mechanism_adventure_description", check_matchmaking_hero_availability = true, class_name = "AdventureMechanism",



		states = { "inn", "ingame", "tutorial", "weave" },





		venture_end_states_in = { "inn" },
		venture_end_states_out = { "inn" },
		...
		gamemode_lookup = { default = "adventure", keep = "inn" } },





	weave = { disable_difficulty_check = true, display_name = "game_mode_adventure", default_inventory = true, server_port = 27015, tobii_available = true, check_matchmaking_hero_availability = true, class_name = "AdventureMechanism", server_universe = "carousel", vote_switch_mechanism_background = "vote_switch_mechanism_adventure_background", vote_switch_mechanism_text = "vote_switch_mechanism_adventure_description", max_members = 4, required_dlc = "scorpion",
		...
		gamemode_lookup = { default = "weave", keep = "inn" } } }
...
local rpcs = { "rpc_set_current_mechanism_state", "rpc_level_load_started", "rpc_carousel_set_local_match", "rpc_carousel_update_set_count", "rpc_carousel_set_private_lobby", "rpc_dedicated_or_player_hosted_search", "rpc_reserved_slots_count", "rpc_party_slots_status", "rpc_force_start_dedicated_server", "rpc_switch_level_dedicated_server", "rpc_sync_players_session_score" }
...
function GameMechanismManager:init()
...
function GameMechanismManager:handle_level_load(done_again_during_loading)
...
function GameMechanismManager:create_level_seed()
	return LevelTransitionHandler.create_level_seed()
end


function GameMechanismManager:generate_level_seed()
...
function GameMechanismManager:get_level_seed(optional_system)
...
function GameMechanismManager:get_end_of_level_rewards_arguments(game_won, quickplay, statistics_db, stats_id)
...
function GameMechanismManager:get_end_of_level_extra_mission_results()
...
function GameMechanismManager:sync_players_session_score(statistics_db, profile_synchronizer, players_session_score)
	local peer_ids = { }
...
function GameMechanismManager:network_server()
	return self._network_server
end

function GameMechanismManager:network_client()
	return self._network_client
end

function GameMechanismManager:network_handler()
	return self._network_server or self._network_client
end

function GameMechanismManager:set_level_seed(seed)
	print("GameMechanismManager setting level seed:", seed)
	self._level_seed = seed
end

function GameMechanismManager:generate_locked_director_functions(level_key)
...
function GameMechanismManager:can_spawn_pickup(spawner_unit, pickup_name)
...
function GameMechanismManager:uses_random_directors()
	local game_mechanism = self._game_mechanism
...
function GameMechanismManager:destroy()
...
function GameMechanismManager:set_profile_synchronizer(profile_synchronizer)
	self._profile_synchronizer = profile_synchronizer
end

function GameMechanismManager:get_level_end_view()
	return self._game_mechanism and self._game_mechanism.get_level_end_view and self._game_mechanism:get_level_end_view()
end

function GameMechanismManager:handle_ingame_enter(game_mode)
	local game_mechanism = self._game_mechanism
...
function GameMechanismManager:handle_ingame_exit(exit_type)
	local game_mechanism = self._game_mechanism
...
function GameMechanismManager:can_resync_loadout()
	local game_mechanism = self._game_mechanism
...
function GameMechanismManager:update_loadout()
	local game_mechanism = self._game_mechanism
...
function GameMechanismManager:network_context_created(lobby, server_peer_id, own_peer_id, is_server, network_handler)
...
function GameMechanismManager:server_peer_id()
	return self._server_peer_id
end

function GameMechanismManager:is_server()
	return self._is_server
end


function GameMechanismManager:network_context_destroyed(level_key)
...
function GameMechanismManager:register_rpcs(network_event_delegate)
...
function GameMechanismManager:_register_mechanism_rpcs()
...
function GameMechanismManager:unregister_rpcs()
	self:_unregister_mechanism_rpcs()
...
function GameMechanismManager:_init_mechanism()
	...
	if not self._game_mechanism or switching_mechanism then
		if self._game_mechanism and
		self._game_mechanism.left_mechanism_due_to_switch then
...
function GameMechanismManager:rpc_level_load_started(channel_id, session_id)
	local level_transition_handler = Managers.level_transition_handler
...
function GameMechanismManager:create_host_migration_info(gm_event_end_conditions_met, gm_event_end_reason)
...
function GameMechanismManager:get_current_level_key()
	return Managers.level_transition_handler:get_current_level_key()
end


function GameMechanismManager:get_current_level_keys()
	return Managers.level_transition_handler:get_current_level_key()
end

function GameMechanismManager:game_mechanism()
	return self._game_mechanism
end

function GameMechanismManager:current_mechanism_name()
	return self._mechanism_key
end

function GameMechanismManager:get_last_mechanism_switch()
	return self._last_mechanism_switch
end

function GameMechanismManager:mechanism_setting(setting_name)
...
function GameMechanismManager:max_members()
	fassert(self._mechanism_key, "No mechanism set yet.")
...
function GameMechanismManager:server_universe()
	fassert(self._mechanism_key, "No mechanism set yet.")
	return MechanismSettings [self._mechanism_key].server_universe
end

function GameMechanismManager:is_packages_loaded()
...
function GameMechanismManager:load_packages()
	if self._game_mechanism.load_packages then
		self._game_mechanism:load_packages()
	end
end

function GameMechanismManager:client_left(peer_id)
	if self._game_mechanism.client_left then
		self._game_mechanism:client_left(peer_id)
	end
end

function GameMechanismManager:profile_available(profile_name, career_name)
...
function GameMechanismManager:profile_available_for_peer(peer_id, local_player_id, profile_name, career_name)
	if self._game_mechanism then
...
function GameMechanismManager:profile_changed(peer_id, local_player_id, profile_index, career_index)
...
function GameMechanismManager:profile_synchronizer()
	return self._profile_synchronizer
end

function GameMechanismManager:get_players_session_score(statistics_db, profile_synchronizer, saved_scoreboard_stats)
	...
	scoreboard or scoreboard or ScoreboardHelper.get_grouped_topic_statistics(statistics_db, profile_synchronizer, saved_scoreboard_stats)
...
function GameMechanismManager:get_prior_state()
	return self._game_mechanism:get_prior_state()
end

function GameMechanismManager:choose_next_state(state)
	self._game_mechanism:choose_next_state(state)
end

function GameMechanismManager:reset_choose_next_state()
	self._game_mechanism:reset_choose_next_state()
end

function GameMechanismManager:setting(key)
	return MechanismSettings [self._mechanism_key] [key]
end

function GameMechanismManager:progress_state()
...
function GameMechanismManager:get_starting_level()
...
function GameMechanismManager:default_level_key()
...
function GameMechanismManager:get_loading_tip()
...
function GameMechanismManager:set_lobby_max_members(max_members)
	self._lobby:set_max_members(max_members)
end

function GameMechanismManager:backend_profiles_loaded()
...
function GameMechanismManager:try_reserve_game_server_slots(reserver, peers)
...
function GameMechanismManager:game_server_slot_reservation_expired(peer_id)
...
function GameMechanismManager:debug_load_level(level_name, environment_variation_id)
	if not self._is_server then
		return
	end
...
function GameMechanismManager:_on_venture_start()
...
function GameMechanismManager:_on_venture_end()
...
function GameMechanismManager:check_venture_start()
	if not self._venture_started then
		self:_on_venture_start()
	end
end

function GameMechanismManager:check_venture_end(left_lobby)
	...
	if left_lobby or self._venture_ended_manually or
	self._venture_started and (
	table.contains(venture_end_states_in, state) or
	table.contains(venture_end_states_out, prior_state)) then
...
function GameMechanismManager:manual_end_venture()
	self._venture_ended_manually = true
end

function GameMechanismManager:is_venture_over()
...
function GameMechanismManager:rpc_set_current_mechanism_state(channel_id, state_id)
...
function GameMechanismManager:rpc_carousel_update_set_count(channel_id, set, last_set)
...
function GameMechanismManager:rpc_carousel_set_local_match(channel_id, local_match)
	local peer_id = CHANNEL_TO_PEER_ID [channel_id]
...
function GameMechanismManager:rpc_carousel_set_private_lobby(channel_id, private_lobby)
	local peer_id = CHANNEL_TO_PEER_ID [channel_id]
	if peer_id ~= self._server_peer_id then
		return
	end
...
function GameMechanismManager:rpc_dedicated_or_player_hosted_search(channel_id, use_dedicated_servers, use_dedicated_aws_servers, use_player_hosted)
	local peer_id = CHANNEL_TO_PEER_ID [channel_id]
	if peer_id ~= self._server_peer_id then
		return
	end
...
function GameMechanismManager:rpc_reserved_slots_count(channel_id, num_reserved_slots, num_slots_total)
...
function GameMechanismManager:rpc_party_slots_status(channel_id, server_name, party_id, party_members, member_states)
...
function GameMechanismManager:rpc_force_start_dedicated_server(channel_id)
...
function GameMechanismManager:rpc_switch_level_dedicated_server(channel_id, level_id)
...
function GameMechanismManager:rpc_sync_players_session_score(channel_id, peer_ids, local_player_ids, players_session_score)
	...
	unsynced_players_session_score =
	unsynced_players_session_score or ScoreboardHelper.get_grouped_topic_statistics(statistics_db, self._profile_synchronizer)


	if ScoreboardHelper.num_stats_per_player ~= num_stats_per_player then
		Crashify.print_exception(
		"GameMechanismManager",
		"rpc_sync_players_session_score received with mismatching stats_per_player count, probably the host was modded. Ignoring the host score and using client's.")
...
function GameMechanismManager:dedicated_server_peer_id()
	return self._dedicated_server_peer_id
end

function GameMechanismManager:reset_dedicated_server_peer_id()
	self._dedicated_server_peer_id = nil
end

function GameMechanismManager:send_rpc_clients(rpc_name, ...)
...
function GameMechanismManager:setup_mechanism_parties()
	local settings = MechanismSettings [self._mechanism_key]
	local party_data = settings.party_data
	Managers.party:register_parties(party_data)
end

function GameMechanismManager:get_level_dialogue_context()
...
function GameMechanismManager:override_hub_level(new_hub_level_key)
...
function GameMechanismManager:get_player_level_fallback(player)
...
function GameMechanismManager:get_slot_reservation_handler()
