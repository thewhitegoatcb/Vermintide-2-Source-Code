...
function ImguiAISpawnLog:init()
	...
	self._event_type_names = { "queued", "canceled", "spawned" }
...
function ImguiAISpawnLog:register_events()
...
function ImguiAISpawnLog:unregister_events()
	local event_manager = Managers.state.event
	if event_manager then
...
function ImguiAISpawnLog:update()
...
function ImguiAISpawnLog:is_persistent()
	return self._keep_on_screen
end

function ImguiAISpawnLog:draw()
	local game_time = Managers.time:time("game")
	if not game_time then
		return
	end
...
function ImguiAISpawnLog:log_queue(...)
	self:_log_event(1, ...)
end

function ImguiAISpawnLog:log_queue_cancel(...)
	self:_log_event(2, ...)
end

function ImguiAISpawnLog:log_spawn(...)
	self:_log_event(3, ...)
end

function ImguiAISpawnLog:_log_event(event_type, boxed_location, breed, spawn_category, spawn_type, spawn_queue_id)
...
function ImguiAISpawnLog:_clear()
	self._log = { }
	self._player_positions = { }
	self._totals = { }
end

function ImguiAISpawnLog:_reset()
	self._drawer = nil
	self._hero_side = nil
	self:_clear()
end

function ImguiAISpawnLog:_init_session()
	local state = Managers.state
	if state then
		self:register_events()
		self._drawer = state.debug:drawer({ mode = "immediate", name = "ImguiAISpawnLog" })
...
function ImguiAISpawnLog:_log_player_positions(game_time)
	...
	if hero_side then
		...
		for i = 1, #hero_units do
			...
			if not self._player_positions [unit] then
				self._player_positions [unit] = { game_time, position.x, position.y, position.z }
			else
				local recorded_player_pos = self._player_positions [unit]
				local last_pos_id = #recorded_player_pos
				local x = recorded_player_pos [last_pos_id - 2] local y = recorded_player_pos [last_pos_id - 1] local z = recorded_player_pos [last_pos_id]
...
function ImguiAISpawnLog:_visualise_player_pos()
	local segment_start = self._timeline_end - self._timeline_slice_size
	local segment_end = self._timeline_end
...
function ImguiAISpawnLog:_export_recap_data()
...
function ImguiAISpawnLog:_export_log_data()
	local type_names = self._event_type_names
