...
function DemoCharacterPreviewer:init(world, profile_name, career_index, position, rotation, zoom_offset)
	self._world = world
...
function DemoCharacterPreviewer:reset_state()
	self._is_hover = nil
	self._is_pressed = nil
	self:outline_unit(false)
end

function DemoCharacterPreviewer:_spawn_character(position)
...
function DemoCharacterPreviewer:_color_from_table(color_table)
	return Color(color_table [1], color_table [2], color_table [3], color_table [4])
end

function DemoCharacterPreviewer:outline_unit(enable_outline, outline_settings, extra_time)
...
function DemoCharacterPreviewer:is_outlined()
	return self._outlined
end

function DemoCharacterPreviewer:_reset_hero()
...
function DemoCharacterPreviewer:_spawn_hero_unit(profile_name, career_index)
...
function DemoCharacterPreviewer:cb_spawn_hero_unit(profile, career, skin_data)
...
function DemoCharacterPreviewer:update(activated, dt, t)
...
function DemoCharacterPreviewer:_update_hover()
...
function DemoCharacterPreviewer:_update_pressed(dt, t)
...
function DemoCharacterPreviewer:cb_on_select_animation_complete()
	local node = "j_neck"
	local unit = self._character_unit
...
function DemoCharacterPreviewer:pressed_pose()
	local viewport = ScriptWorld.viewport(self._world, "title_screen_viewport")
	local camera = ScriptViewport.camera(viewport)
...
function DemoCharacterPreviewer:_update_aim_constraint(dt, t)
	if not Unit.alive(self._character_unit) then
		return
	end
...
function DemoCharacterPreviewer:is_hover()
	return self._is_hover
end

function DemoCharacterPreviewer:is_pressed()
	return self._is_pressed
end

function DemoCharacterPreviewer:was_pressed_this_frame()
	local was_pressed_this_frame = self._was_pressed_this_frame
	self._was_pressed_this_frame = nil
	return was_pressed_this_frame
end

function DemoCharacterPreviewer:profile_information()
	return self._profile_name, self._career_index
end

function DemoCharacterPreviewer:_spawn_inventory(career)
...
function DemoCharacterPreviewer:character_spawned()
	return self._character_spawned
end

function DemoCharacterPreviewer:wield_weapon_slot(slot_type)
...
function DemoCharacterPreviewer:play_character_animation(animation_event)
	local character_unit = self._character_unit
...
function DemoCharacterPreviewer:_equip_item(item_name, slot)
	...
	if item_slot_type == "melee" or item_slot_type == "ranged" then
		...
		if left_hand_unit then
			...
			units_to_spawn_data [#units_to_spawn_data + 1] = { left_hand = true,
		...
		if right_hand_unit then
			...
			units_to_spawn_data [#units_to_spawn_data + 1] = { right_hand = true,
				despawn_both_hands_units = despawn_both_hands_units,
...
function DemoCharacterPreviewer:load_package(package_names, item_name)
...
function DemoCharacterPreviewer:on_load_complete(package_name, item_name)
...
function DemoCharacterPreviewer:_spawn_item(item_name)
	local world = self._world
	local character_unit = self._character_unit
...
function DemoCharacterPreviewer:equip_item_unit(unit, item_slot_type, item_template, unit_attachment_node_linking, scene_graph_links, material_settings)
	local world = self._world
	local character_unit = self._character_unit
...
function DemoCharacterPreviewer:destroy()
	self:_reset_hero()
