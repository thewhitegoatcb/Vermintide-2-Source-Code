...
function DeusRunStatsUi:init(ingame_ui_context, parent)
...
function DeusRunStatsUi:show_info_message(rewards)
	for i = 1, #rewards do
...
function DeusRunStatsUi:_start_animation(animation_name, info_type)
	local params = { }
...
function DeusRunStatsUi:_create_ui_elements()
...
function DeusRunStatsUi:update(dt, t)
...
function DeusRunStatsUi:_update_animations(dt, t)
...
function DeusRunStatsUi:lock(lock, show_fullscreen_fade)
...
function DeusRunStatsUi:locked()
	return self._locked
end

function DeusRunStatsUi:set_active(active)
...
function DeusRunStatsUi:active()
	return self._active
end

function DeusRunStatsUi:_handle_input(dt, t)
...
function DeusRunStatsUi:_handle_gamepad_input(dt, t)
	local gamepad_active = Managers.input:is_device_active("gamepad")
...
function DeusRunStatsUi:_populate_power_up(power_up_name, power_up_rarity, power_up_description_widget, extend_left)
...
function DeusRunStatsUi:_draw_reminder(dt, t)
...
function DeusRunStatsUi:_draw(dt, t)
	if not self._active then
		return
	end
...
function DeusRunStatsUi:_play_sound(event)
	WwiseWorld.trigger_event(self._wwise_world, event)
end

function DeusRunStatsUi:destroy()
	self:lock(false)
end

function DeusRunStatsUi:update_dynamic_values(data)
...
function DeusRunStatsUi:_update_blessings(blessings)
	...
	if has_blessings then
		...
		for i = 1, num_blessings do
			...
			local widget_definition = UIWidgets.create_framed_info_box(
			scenegraph_id,
			widget_data.title_frame_name,
			widget_data.info_frame_name,
			nil, nil,

			icon,
			widget_data.icon_size,
			widget_data.icon_frame_name,
			title_text,
			info_text,

			widget_data.bottom_panel_size)
			local widget = UIWidget.init(widget_definition)
...
function DeusRunStatsUi:_update_power_ups(party_power_ups, power_ups, profile_index, career_index)
	...
	if has_power_ups then
		...
		for i = 1, num_power_ups do
			...
			local widget_definition = UIWidgets.create_icon_info_box(
			scenegraph_id,
			icon,
			widget_data.icon_size,
			widget_data.icon_offset,
			widget_data.background_icon,
			widget_data.background_icon_size,
			widget_data.background_icon_offset,
			sub_text,
			title_text,
			text_color,
			widget_data.width,
...
function DeusRunStatsUi:set_loadout(melee, ranged, healing_slot, potion_slot, grenade_slot)
...
function DeusRunStatsUi:_create_player_portrait(portrait_frame, portrait_image, player_level_text)
...
function DeusRunStatsUi:_set_widget_text(widget_name, text)
