...
function PlayFabMirrorAdventure:init(signin_result)
	...
	self._verify_slot_keys_per_affiliation = {
		heroes = { "slot_ranged", "slot_melee", "slot_hat", "slot_skin", "slot_necklace", "slot_trinket_1", "slot_ring", "slot_frame" } }
...
function PlayFabMirrorAdventure:request_characters(mechanism_key)
...
function PlayFabMirrorAdventure:_set_inital_career_data_weaves(career_name, loadout, slots_to_verify)
	local broken_slots = { }
...
function PlayFabMirrorAdventure:_check_weaves_loadout()
	...
	for character_name, character_data in pairs(characters_data) do
		for career_name, career_data in pairs(character_data.careers) do
			...
			local broken_slots = self:_set_inital_career_data_weaves(career_name, weave_loadout, { "slot_melee", "slot_ranged" })
...
function PlayFabMirrorAdventure:fix_weaves_career_data_request_cb(result)
