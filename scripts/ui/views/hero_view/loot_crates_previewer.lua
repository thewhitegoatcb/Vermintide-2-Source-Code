...
function LootCratesPreviewer:init(rewards, units, spawn_positions, end_positions, background_world, background_viewport)
...
function LootCratesPreviewer:destroy()
	self:_destroy_units()
end

function LootCratesPreviewer:_destroy_units()
...
function LootCratesPreviewer:update(dt, t)
	return end

function LootCratesPreviewer:post_update(dt, t)
	if not self._entry_animation_complete then
		self:_animate_entry_positions(dt, t)
	end
end

function LootCratesPreviewer:_animate_entry_positions(dt, t)
...
function LootCratesPreviewer:_trigger_unit_flow_event(unit, event_name)
	if unit and Unit.alive(unit) then
		Unit.flow_event(unit, event_name)
	end
end

function LootCratesPreviewer:_get_world()
	return self.background_world, self.background_viewport
end

function LootCratesPreviewer:_get_camera_position()
...
function LootCratesPreviewer:_get_camera_rotation()
	local background_viewport = self.background_viewport
	local camera = ScriptViewport.camera(background_viewport)
	return ScriptCamera.rotation(camera)
end

function LootCratesPreviewer:get_units()
	return self._spawned_units
end

function LootCratesPreviewer:has_units()
	local units_left = self._spawned_units and #self._spawned_units > 0
	return units_left
end

function LootCratesPreviewer:get_item_key_by_unit(unit)
	return self._item_key_by_unit [unit]
end

function LootCratesPreviewer:delete_unit(unit)
	local world = self.background_world
	local spawned_units = self._spawned_units
...
function LootCratesPreviewer:spawn_units(units_name)
	local units = { }
	local spawn_positions = self.spawn_positions
...
function LootCratesPreviewer:_enable_units_visibility()
	local spawned_units = self._spawned_units
	for _, unit in ipairs(spawned_units) do
		if unit and Unit.alive(unit) then
			Unit.set_unit_visibility(unit, true)
