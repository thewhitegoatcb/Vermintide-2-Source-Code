...
local _tracked_weapon_kill_stats = {
	...
	we_1h_axe = { "holly" },
	bw_1h_crowbill = { "holly" },
	wh_dual_wield_axe_falchion = { "holly" },
	dr_dual_wield_hammers = { "holly" },
	es_dual_wield_hammer_sword = { "holly" },
	bw_1h_flail_flaming = { "scorpion" },
	dr_1h_throwing_axes = { "scorpion" },
	we_1h_spears_shield = { "scorpion" },
	es_2h_heavy_spear = { "scorpion" },
	wh_2h_billhook = { "scorpion" } }


DLCUtils.dofile_list("statistics_util")

function StatisticsUtil.generate_weapon_kill_stats_dlc(stat_player, dlc_name, template)
...
local _tracked_levels_complted_w_weapons_levels = {
	...
	warcamp = { "scorpion" },
	skaven_stronghold = { "scorpion" },
	ground_zero = { "scorpion" },
	skittergate = { "scorpion" } }
...
function StatisticsUtil.generate_level_complete_with_weapon_stats_dlc(stat_player, dlc_name, template)
...
function StatisticsUtil.register_kill(victim_unit, damage_data, statistics_db, is_server)
	...
	if attacker_player and attacker_player ~= victim_player then
		...
		if breed_killed then
			...
			if master_list_item then
				...
				if attack_type then
					if attack_type == "heavy_attack" or attack_type == "light_attack" then slot_type = "melee" else slot_type = "ranged"
					end
				end
...
function StatisticsUtil.register_knockdown(victim_unit, damage_data, statistics_db, is_server)
	local victim_health_extension = ScriptUnit.has_extension(victim_unit, "health_system")
	local victim_damage_data = victim_health_extension.last_damage_data
	if not victim_damage_data then
		return
...
function StatisticsUtil.check_save(savior_unit, enemy_unit)
...
function StatisticsUtil.register_pull_up(puller_up_unit, pulled_up_unit, statistics_db)
	local player_manager = Managers.player
...
function StatisticsUtil.register_assisted_respawn(reviver_unit, revivee_unit, statistics_db)
	local player_manager = Managers.player
...
function StatisticsUtil.register_revive(reviver_unit, revivee_unit, statistics_db)
	local player_manager = Managers.player
	local player1 = player_manager:owner(reviver_unit)
	if player1 then
...
function StatisticsUtil.register_heal(healer_unit, healed_unit, statistics_db)
	local player_manager = Managers.player
...
function StatisticsUtil.register_damage(victim_unit, damage_data, statistics_db)
		...
		if target_breed then
			...
			if current_health > 0 then
				...
				if target_breed.is_player then
					...
					if is_enemy and attacker_side.show_damage_feedback and
					HEALTH_ALIVE [victim_unit] then
...
function StatisticsUtil.won_games(statistics_db)
...
function StatisticsUtil.register_collected_grimoires(collected_grimoires, statistics_db)
	local local_player = Managers.player:local_player()
	local stats_id = local_player:stats_id()
...
function StatisticsUtil.register_collected_tomes(collected_tomes, statistics_db)
	local local_player = Managers.player:local_player()
	local stats_id = local_player:stats_id()
...
function StatisticsUtil.register_collected_dice(collected_dice, statistics_db)
	local local_player = Managers.player:local_player()
	local stats_id = local_player:stats_id()
...
function StatisticsUtil.register_complete_level(statistics_db)
	local level_settings = LevelHelper:current_level_settings()
	local level_id = level_settings.level_id
	if not table.find(UnlockableLevels, level_id) then
		return
	...
	if Managers.unlock:is_dlc_unlocked("holly") then
		...
		if above_legend_difficulty and is_lord_level then
			local weapon_names = { "we_1h_axe", "bw_1h_crowbill", "wh_dual_wield_axe_falchion", "dr_dual_wield_hammers", "es_dual_wield_hammer_sword" }
...
function StatisticsUtil.register_weave_complete(statistics_db, player, is_quick_game, difficulty_key)
	local stats_id = player:stats_id()
...
function StatisticsUtil._register_mutator_challenges(statistics_db, stats_id, wind)
	if ScorpionSeasonalSettings.current_season_id == 1 or not IS_WINDOWS then
		if wind == "life" then
...
function StatisticsUtil.register_journey_complete(statistics_db, player, journey_name, dominant_god, difficulty_name)
	StatisticsUtil._register_completed_journey_difficulty(statistics_db, player, journey_name, dominant_god, difficulty_name)
end

function StatisticsUtil.register_complete_tutorial(statistics_db)
	local level_settings = LevelHelper:current_level_settings()
	local local_player = Managers.player:local_player()
	local stats_id = local_player:stats_id()
	local level_id = level_settings.level_id
...
function StatisticsUtil.register_played_quickplay_level(statistics_db, player, level_key)
...
function StatisticsUtil.register_played_weekly_event_level(statistics_db, player, level_key, difficulty_key)
	if not table.find(UnlockableLevels, level_key) then
		return
	end
...
function StatisticsUtil.register_last_played_level_id(statistics_db, player, level_key)
...
function StatisticsUtil.get_game_progress(statistics_db)
	local local_player = Managers.player:local_player()
	local stats_id = local_player:stats_id()
...
function StatisticsUtil._register_completed_level_difficulty(statistics_db, level_id, career_name, difficulty_name)
	local local_player = Managers.player:local_player()
	local stats_id = local_player:stats_id()
...
function StatisticsUtil._register_completed_journey_difficulty(statistics_db, player, journey_name, dominant_god, difficulty_name)
	local stats_id = player:stats_id()
	local profile_index = player:profile_index()
	...
	if current_completed_difficulty < difficulty_index then
		if difficulty_index > #DefaultDifficulties then
			ferror("This shouldn't happen. \ndifficulties: %s\ndifficulty_name: %s\ndifficulty_index: %s\nDefaultDifficulties: %s\ncurrent_completed_difficulty: %s",
			table.tostring(difficulties),
			difficulty_name,
			difficulty_index,
			table.tostring(DefaultDifficulties),

			current_completed_difficulty)
		end
	...
	if current_completed_journey_dominant_god_difficulty < difficulty_index then
		if difficulty_index > #DefaultDifficulties then
			ferror("This shouldn't happen. \ndifficulties: %s\ndifficulty_name: %s\ndifficulty_index: %s\nDefaultDifficulties: %s\ncurrent_completed_journey_dominant_god_difficulty: %s",
			table.tostring(difficulties),
			difficulty_name,
			difficulty_index,
			table.tostring(DefaultDifficulties),

			current_completed_journey_dominant_god_difficulty)
		end
	...
	if current_completed_hero_journey_difficulty < difficulty_index then
		if difficulty_index > #DefaultDifficulties then
			ferror("This shouldn't happen. \ndifficulties: %s\ndifficulty_name: %s\ndifficulty_index: %s\nDefaultDifficulties: %s\ncurrent_completed_hero_journey_difficulty: %s",
			table.tostring(difficulties),
			difficulty_name,
			difficulty_index,
			table.tostring(DefaultDifficulties),

			current_completed_hero_journey_difficulty)
		end
		statistics_db:set_stat(stats_id, "completed_hero_journey_difficulty", profile_abbreviation, journey_db_name, difficulty_index)
	end
end

function StatisticsUtil.unlock_lorebook_page(page_id, statistics_db)
...
function StatisticsUtil.get_survival_stat(statistics_db, level_id, difficulty, stat_name, stats_id)
...
function StatisticsUtil._set_survival_stat(statistics_db, level_id, difficulty, stat_name, value)
	local stat = survival_stat_name(level_id, difficulty, stat_name)
	local player_manager = Managers.player
	local local_player = player_manager:local_player()
	local stats_id = local_player:stats_id()
...
function StatisticsUtil.reset_mission_streak(player, statistics_db, stats_id)
	local profile_index = player:profile_index()
	local profile = SPProfiles [profile_index]
	local career_index = player:career_index()
	local career_data = profile.careers [career_index]
...
function StatisticsUtil._modify_survival_stat(statistics_db, level_id, difficulty, stat_name, value)
	local stat = survival_stat_name(level_id, difficulty, stat_name)
	local player_manager = Managers.player
	local local_player = player_manager:local_player()
	local stats_id = local_player:stats_id()
...
function StatisticsUtil.register_complete_survival_level(statistics_db)
	local mission_system = Managers.state.entity:system("mission_system")
