...
local survival_tip_list = { "dlc1_2_survival_tip_01", "dlc1_2_survival_tip_02", "dlc1_2_survival_tip_03", "dlc1_2_survival_tip_04", "dlc1_2_survival_tip_05", "dlc1_2_survival_tip_06" }
...
local tip_type_prefix_list = { npcs = "loading_screen_npcs", kerillian = "loading_screen_kerillian", lore = "loading_screen_lore", khazalid = "loading_screen_khazalid", rotbloods = "loading_screen_rotbloods", okri = "loading_screen_okri", tip = "loading_screen_tip" }
...
local tip_type_max_range = { npcs = 3, kerillian = 10, lore = 55, khazalid = 47, rotbloods = 9, okri = 1, tip = 89 }
...
local tip_type_list = { "tip", "lore", "rotbloods", "khazalid", "npcs", "kerillian", "okri" }
...
function LoadingView:init(ui_context)
	...
	self.ui_renderer = UIRenderer.create(self.world,
	"material", "materials/ui/loading_screens/" .. self.default_loading_screen,
...
function LoadingView:_create_hdr_gui()
...
function LoadingView:texture_resource_loaded(level_key, act_progression_index, game_difficulty, optional_loading_ui_package_name, optional_loading_screen_material_name, weave_data)
	...
	if IS_XB1 then
		local gui = World.create_screen_gui(self.world, "immediate",
		"material", "materials/ui/loading_screens/" .. self.default_loading_screen,
		...
		local gui_retained = World.create_screen_gui(self.world,
		"material", "materials/ui/loading_screens/" .. self.default_loading_screen,
		"material", bg_material,
		"material", "materials/fonts/gw_fonts",
		"material", "materials/ui/ui_1080p_common",
...
function LoadingView:deactivate()
	self.active = false
end

function LoadingView:activate()
	self.active = true
end

function LoadingView:showing_press_to_continue()
	return self._show_press_to_continue
end

function LoadingView:show_press_to_continue(show)
	self._show_press_to_continue = show
end

function LoadingView:create_ui_elements()
...
function LoadingView:subtitle_gui()
	return self.subtitle_timed_gui
end

function LoadingView:trigger_subtitles(wwise_event, t)
...
function LoadingView:trigger_weave_subtitles(wwise_events, t)
...
function LoadingView:reset_tip_text()
...
function LoadingView:fit_title()
	...
	repeat
		...
		if text_width <= 260 or style.font_size <= 1 then
			...
			style.font_size = style.font_size - 1 end until not continue


end

local DEFAULT_SECOND_ICON_DATA = { }
function LoadingView:_find_second_input_texture(suffix_text, macro_replacement, input_action, font, scaled_font_size)
...
local DEFAULT_ICON_SIZE_TABLE = { 0, 0 }
function LoadingView:setup_tip_text(act_progression_index, game_mode, tip_localization_key)
	self:fit_title()
...
function LoadingView:setup_act_text(level_key)
	if level_key then
		local level_settings = LevelSettings [level_key]
...
function LoadingView:setup_level_text(level_key)
	if level_key then
		local level_settings = LevelSettings [level_key]
...
function LoadingView:setup_difficulty_text(game_difficulty)
...
function LoadingView:setup_news_ticker(text)
...
function LoadingView:update(dt)
	if DO_RELOAD then
		print("reload")
		self:create_ui_elements()
		DO_RELOAD = false
...
function LoadingView:is_done() return true end