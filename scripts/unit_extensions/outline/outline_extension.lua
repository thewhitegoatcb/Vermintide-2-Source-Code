...
function OutlineExtension:init()
	self._unique_id = 0
...
function OutlineExtension:add_outline(settings)
	...
	if settings_bucket [insert_index] then
		...
		settings_bucket [insert_index] = { settings }
	end
...
function OutlineExtension:remove_outline(unique_id)
...
function OutlineExtension:update_outline(settings, unique_id)
	if not unique_id or unique_id < 0 then
		return
	end
...
function OutlineExtension:reapply_outline()
	self.reapply = true
end

function OutlineExtension:_refresh_current_outline(reapply)
...
function OutlineExtension:on_freeze()
	self.method = "never"
	table.clear(self.outline_settings)
	self.outline_settings [1] = { self._default_settings }
end

function OutlineExtension:on_unfreeze()
	self:_refresh_current_outline()
end