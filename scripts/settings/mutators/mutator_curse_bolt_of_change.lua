...
local breed_override = {
	chaos_warrior = { chance = 0.3, breed = "chaos_spawn" },



	beastmen_bestigor = { chance = 0.3, breed = "chaos_spawn" },



	skaven_slave = { chance = 0.005, breed = "critter_rat" } }
...
local max_spawn_amounts = { chaos_spawn = 1 }
...
local unchangeable_breeds = { chaos_troll = true, chaos_spawn_exalted_champion_warcamp = true, chaos_exalted_champion_warcamp = true, chaos_exalted_sorcerer = true, skaven_storm_vermin_warlord = true, pet_rat = true, chaos_exalted_champion_norsca = true, beastmen_minotaur = true, skaven_stormfiend = true, skaven_grey_seer = true, skaven_rat_ogre = true, pet_pig = true, chaos_spawn = true, chaos_exalted_sorcerer_drachenfels = true, skaven_stormfiend_boss = true, pet_wolf = true, skaven_storm_vermin_champion = true }
...
local excluded_random_breeds = { chaos_spawn_exalted_champion_warcamp = true, chaos_exalted_champion_warcamp = true, chaos_exalted_sorcerer = true, beastmen_ungor = true, skaven_storm_vermin_warlord = true, pet_pig = true, pet_rat = true, chaos_exalted_champion_norsca = true, beastmen_minotaur = true, chaos_fanatic = true, skaven_slave = true, skaven_stormfiend = true, skaven_grey_seer = true, skaven_rat_ogre = true, chaos_troll = true, chaos_spawn = true, chaos_exalted_sorcerer_drachenfels = true, skaven_stormfiend_boss = true, pet_wolf = true, skaven_storm_vermin_champion = true }
...
function bolt_of_change.server_start_function(context, data)
	...
	data.extension_init_data = {
		area_damage_system = { explosion_template_name = "bolt_of_change" } }
...
function bolt_of_change.server_stop_function(context, data)
...
function bolt_of_change.spawn_lightning_strike_unit(data)
...
function bolt_of_change.server_players_left_safe_zone(context, data)
	data.has_left_safe_zone = true
end

function bolt_of_change.server_update_function(context, data, dt, t)
...
function bolt_of_change.modify_player_base_damage(context, data, damaged_unit, attacker_unit, damage, damage_type)
	local player = Managers.player:owner(damaged_unit)
	local is_bot = player and player.bot_player
	if is_bot then
		do return damage * BOT_DAMAGE_MODIFIER end
	else
		return damage
	end
end

function bolt_of_change.populate_available_breeds(context, data)
...
function bolt_of_change.cb_on_explode(template, data, explosion_template_name, position)
...
function bolt_of_change.get_overridden_breed(data, maxed_out_breeds, breed_name)
...
function bolt_of_change.spawn_new_breed(data, ai_unit, new_breed)
...
function bolt_of_change.change_ai(data, ai_unit)
	local time = Managers.time:time("game")
...
function bolt_of_change.server_player_hit_function(context, data, hit_unit, attacker_unit, hit_data)
