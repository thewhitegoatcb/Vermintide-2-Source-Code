...
local layout_name_by_index = { "equipment", "talents", "forge", "cosmetics", "system" }
...
function HeroWindowPanelConsole:on_enter(params, offset)
...
function HeroWindowPanelConsole:_start_transition_animation(animation_name)
	local params = {
...
function HeroWindowPanelConsole:create_ui_elements(params, offset)
...
function HeroWindowPanelConsole:on_exit(params)
	print("[HeroViewWindow] Exit Substate HeroWindowPanelConsole")
	self.ui_animator = nil
end

function HeroWindowPanelConsole:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function HeroWindowPanelConsole:post_update(dt, t)
	self:_handle_input(dt, t)
end

function HeroWindowPanelConsole:_handle_bot_warning()
...
function HeroWindowPanelConsole:_set_bot_information(current_profile_index, current_career_index)
...
function HeroWindowPanelConsole:_update_animations(dt)
...
function HeroWindowPanelConsole:_is_button_pressed(widget)
...
function HeroWindowPanelConsole:_is_stepper_button_pressed(widget)
	local content = widget.content
...
function HeroWindowPanelConsole:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_enter
end

function HeroWindowPanelConsole:_is_button_hover_exit(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_exit
end

function HeroWindowPanelConsole:_is_button_selected(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.is_selected
end

function HeroWindowPanelConsole:_handle_input(dt, t)
	...
	if not self.force_ingame_menu then
		...
		if
		not close_on_exit and not input_made and self:_is_button_pressed(back_button) then
	...
	if self.is_in_inn and not self.force_ingame_menu then
		...
		if not input_made and self._present_purchase_add and
		input_service:get(INPUT_ACTION_PURCHASE) and
		IS_XB1 then
			input_made = true
...
function HeroWindowPanelConsole:_on_panel_button_selected(index)
	local parent = self.parent
...
function HeroWindowPanelConsole:_set_selected_option(index)
	local system_button = self._widgets_by_name.system_button
...
function HeroWindowPanelConsole:_update_selected_option()
	local parent = self.parent
	local selected_layout_name = parent:get_layout_name()
...
function HeroWindowPanelConsole:draw(dt)
...
function HeroWindowPanelConsole:_play_sound(event)
	self.parent:play_sound(event)
end

function HeroWindowPanelConsole:_sync_news(dt, t)
...
function HeroWindowPanelConsole:_setup_input_buttons()
	local input_service = self.parent:window_input_service()
...
function HeroWindowPanelConsole:_handle_back_button_visibility()
...
function HeroWindowPanelConsole:_reset_back_button()
	local back_button = self._widgets_by_name.back_button
...
function HeroWindowPanelConsole:_set_text_button_size(widget, width)
	local ui_scenegraph = self.ui_scenegraph
...
function HeroWindowPanelConsole:_animate_purchase_add(dt)
	local widgets_by_name = self._widgets_by_name
...
function HeroWindowPanelConsole:_set_text_button_horizontal_position(widget, x_position)
	local ui_scenegraph = self.ui_scenegraph
	local scenegraph_id = widget.scenegraph_id
	ui_scenegraph [scenegraph_id].local_position [1] = x_position
end

function HeroWindowPanelConsole:_validate_product_owner()
...
function HeroWindowPanelConsole:_open_marketplace_xb1()
...
function HeroWindowPanelConsole:_set_purchase_add_visibility(visible)
	local widgets_by_name = self._widgets_by_name
...
function HeroWindowPanelConsole:_animate_title_entry(widget, dt)
	local content = widget.content
	local style = widget.style
	local hotspot = content.button_hotspot
...
function HeroWindowPanelConsole:_animate_back_button(widget, dt)
	local content = widget.content
	local style = widget.style
	local hotspot = content.button_hotspot
