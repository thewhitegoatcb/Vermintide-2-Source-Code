...
function SpecialsPacing:init(world, nav_world, nav_tag_volume_handler, specials_side)
...
function SpecialsPacing:remove_unwanted_breeds()
...
local mutator_bosses_spawn = { "chaos_spawn", "skaven_rat_ogre", "skaven_stormfiend", "chaos_troll" }
...
SpecialsPacing.setup_functions = {
	specials_by_slots = function (t, slots, method_data, state_data)
		...
		local override_time = nil local trickle_time = 2
		...
		for i = 1, specials_settings.max_specials do
			...
			slots [i] = { state = "waiting", breed = breed_name, time = time, health_modifier = health_modifier, special_spawn_stinger = special_spawn_stinger, special_spawn_stinger_at_t = special_spawn_stinger_at_t }
		end
	end }
...
SpecialsPacing.select_breed_functions = {
	...
	get_random_breed = function (slots, specials_settings, method_data, state_data)
		...
		repeat
			...
			i = i + 1 until
		not count [breed] or count [breed] < method_data.max_of_same or max_tries <= i
	...
	get_chance_of_boss_breed = function (slots, specials_settings, method_data, state_data)
		...
		if will_spawn_boss then
			...
			do return breed, health_modifier end
		else
			local count = FrameTable.alloc_table()
...
function SpecialsPacing:set_next_coordinated_attack(t, specials_settings, method_data, slots, spawn_queue)
...
function SpecialsPacing:specials_by_slots(t, specials_settings, method_data, slots, spawn_queue)
...
function SpecialsPacing:specials_by_time_window(t, specials_settings, method_data, slots, spawn_queue, alive_specials)
...
function SpecialsPacing:enable(state)
	self._disabled = not state
end

function SpecialsPacing:is_disabled()
...
function SpecialsPacing:update(t, alive_specials, specials_population, player_positions)
	local specials_settings = CurrentSpecialsSettings
...
function SpecialsPacing:_play_stinger(stinger_name, slot)
	local audio_system = Managers.state.entity:system("audio_system")
	audio_system:play_2d_audio_event(stinger_name)
	slot.has_played_special_stinger = true
end

function SpecialsPacing:delay_spawning(t, delay, per_unit_delay, ignore_coordinated_attack)
	...
	for i = 1, #specials_spawn_queue do
		specials_spawn_queue [i] = nil
	end
end

function SpecialsPacing:debug_spawn()
...
function SpecialsPacing:get_special_spawn_pos(spawning_rule)
...
function SpecialsPacing:request_rushing_intervention(t, player_unit, main_path_info, main_path_player_info, disable_rush_intervention)
...
function SpecialsPacing:request_speed_running_intervention(t, player_unit, main_path_player_info)
	if script_data.ai_specials_spawning_disabled then
		return false, "specials spawning disabled"
	end
...
function SpecialsPacing:get_relative_main_path_pos(main_paths, player_info, extra_distance)
...
function SpecialsPacing:debug(t, alive_specials, specials_population, slots)
