...
function BTVomitAction:enter(unit, blackboard, t)
...
function BTVomitAction:_position_on_navmesh(position, blackboard)
...
function BTVomitAction:_get_vomit_position(unit, blackboard)
...
function BTVomitAction:init_attack(unit, blackboard, action, t)
...
function BTVomitAction:leave(unit, blackboard, t, reason, destroy)
...
function BTVomitAction:_calculate_oobb_collision(bot_threat, self_pos, self_rot)
...
function BTVomitAction:run(unit, blackboard, t, dt)
	...
	if t < blackboard.anim_locked then
		...
		elseif t < blackboard.rotation_time and (not target_unit_status_extension or not target_unit_status_extension:get_is_dodging() and not target_unit_status_extension:is_invisible()) and
		blackboard.update_puke_pos_at_t < t then
			local puke_position, puke_distance_sq, puke_direction = self:_get_vomit_position(unit, blackboard)
...
function BTVomitAction.player_vomit_hit_check(unit, puke_pos, physics_world, blackboard)
	local troll_head_node = Unit.node(unit, "j_head")
	local troll_head_pos = Unit.world_position(unit, troll_head_node)
...
function BTVomitAction:create_aoe(unit, blackboard, action)
...
function BTVomitAction:anim_cb_vomit(unit, blackboard)
