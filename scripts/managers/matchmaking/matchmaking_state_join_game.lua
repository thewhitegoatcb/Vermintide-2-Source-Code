...
function MatchmakingStateJoinGame:init(params)
...
function MatchmakingStateJoinGame:destroy()
	return end

function MatchmakingStateJoinGame:on_enter(state_context)
...
function MatchmakingStateJoinGame:on_exit()
...
function MatchmakingStateJoinGame:update(dt, t)
	local ui_manager = Managers.ui
	...
	if self._exit_to_search_game then
		...
		if join_by_lobby_browser then
			...
			do return MatchmakingStateIdle, self.state_context end
...
function MatchmakingStateJoinGame:_update_lobby_data(dt, t)
...
function MatchmakingStateJoinGame:_handle_popup_result(result, t)
...
function MatchmakingStateJoinGame:_update_profiles_data(profile_array, player_id_array)
...
function MatchmakingStateJoinGame:get_transition()
...
function MatchmakingStateJoinGame:_spawn_join_popup(dt, t)
...
function MatchmakingStateJoinGame:_update_popup_timeout(dt, t)
...
function MatchmakingStateJoinGame:_request_profile_from_host(hero_index)
	local lobby_client = self.lobby_client
...
function MatchmakingStateJoinGame:rpc_matchmaking_request_profile_reply(channel_id, profile, reply)
...
function MatchmakingStateJoinGame:_current_hero()
	local peer_id = Network.peer_id()
	local player = Managers.player:player_from_peer_id(peer_id)
	local profile_index = player:profile_index()
...
function MatchmakingStateJoinGame:_level_started()
	local lobby_client = self.lobby_client
	local selected_mission_id = lobby_client:lobby_data("selected_mission_id")
...
function MatchmakingStateJoinGame:loading_context()
	return self._matchmaking_loading_context
end

function MatchmakingStateJoinGame:rpc_matchmaking_join_game(channel_id)
	mm_printf_force("Transition from join due to rpc_matchmaking_join_game")
	self:_set_state_to_start_lobby()
end

function MatchmakingStateJoinGame:active_lobby()
	return self.lobby_client
end

function MatchmakingStateJoinGame:_set_state_to_start_lobby()
