...
function CraftPageRollTraitConsole:on_enter(params, settings)
...
function CraftPageRollTraitConsole:setup_recipe_requirements()
...
function CraftPageRollTraitConsole:reset_requirements(num_required_ingredients)
...
function CraftPageRollTraitConsole:create_ui_elements(params)
...
function CraftPageRollTraitConsole:on_exit(params)
...
function CraftPageRollTraitConsole:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function CraftPageRollTraitConsole:post_update(dt, t)

	return end

function CraftPageRollTraitConsole:_update_animations(dt)
...
function CraftPageRollTraitConsole:_is_button_pressed(widget)
...
function CraftPageRollTraitConsole:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageRollTraitConsole:_is_button_held(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageRollTraitConsole:_handle_input(dt, t)
...
function CraftPageRollTraitConsole:_handle_craft_input_progress(progress)
	return self.parent:_set_input_progress(progress)
end

function CraftPageRollTraitConsole:craft_result(result, error, reset_slots)
	if not error then
		self._craft_result = result
	end
end

function CraftPageRollTraitConsole:reset()
...
function CraftPageRollTraitConsole:present_results()
	local item_grid = self._item_grid
	item_grid:clear_locked_items()
	item_grid:update_items_status()
...
function CraftPageRollTraitConsole:on_craft_completed()
...
function CraftPageRollTraitConsole:_update_craft_items()
	local super_parent = self.super_parent
	local item_grid = self._item_grid
...
function CraftPageRollTraitConsole:_remove_craft_item(backend_id, slot_index)
	local craft_items = self._craft_items
...
function CraftPageRollTraitConsole:_add_craft_item(backend_id, slot_index, ignore_sound)
...
function CraftPageRollTraitConsole:_set_craft_button_disabled(disabled)
...
function CraftPageRollTraitConsole:_exit(selected_level)
	self.exit = true
	self.exit_level_id = selected_level
end

function CraftPageRollTraitConsole:draw(dt)
...
function CraftPageRollTraitConsole:_play_sound(event)
	self.super_parent:play_sound(event)
end

function CraftPageRollTraitConsole:_set_craft_button_text(text, localize)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.craft_button
	widget.content.button_text = localize and Localize(text) or text
end

function CraftPageRollTraitConsole:_add_crafting_material_requirement(index, item_key, amount_text, has_required_amount)
...
function CraftPageRollTraitConsole:_clear_item_grid()
	local craft_items = self._craft_items
	local super_parent = self.super_parent
...
function CraftPageRollTraitConsole:_has_added_item_by_id(backend_id)
	local craft_items = self._craft_items
