...
local RPCS = { "rpc_client_disrupt_ritual_update" }



function DisruptRitualExtension:init(extension_init_context, unit, extension_init_data)
...
function DisruptRitualExtension:start_disrupt_ritual(unit, volume_name, sub_type, num_progression_events, tick_length, damage_per_tick, heal_per_tick)
	if unit ~= self._unit then return
	end
	self._active = true
	...
	if not self._is_server then return
	end
	...
	self._volume_system:register_volume(volume_name, "trigger_volume", { sub_type = sub_type })


	local checkpoints = { }
	for i = 0, 100 do
		local checkpoint = Unit.get_data(self._unit, "checkpoints", i)
		if not checkpoint then break
		end
	...
	local progression_event_thresholds = { [1.0] = 0, [num_progression_events] = max_damage }
...
function DisruptRitualExtension:update(t)
	if not self._active or t < self._next_tick then return end
...
function DisruptRitualExtension:server_heal(current_damage, checkpoints, current_checkpoint)
...
function DisruptRitualExtension:server_apply_damage(current_damage, checkpoints, current_checkpoint, num_checkpoints)
	...
	if current_checkpoint == num_checkpoints then return
	end
...
function DisruptRitualExtension:server_update_progression_status(progression_event_thresholds, current_progression_event, num_progression_events, current_damage)
...
function DisruptRitualExtension:server_send_rpc_update_clients(damage, checkpoint, progression_event, volume_name)
	self._network_transmit:send_rpc_clients("rpc_client_disrupt_ritual_update", damage, checkpoint, progression_event, volume_name)
end



function DisruptRitualExtension:rpc_client_disrupt_ritual_update(sender, damage, checkpoint, progression_event, volume_name)
	if not self._active or self._volume_name ~= volume_name then return
	end
