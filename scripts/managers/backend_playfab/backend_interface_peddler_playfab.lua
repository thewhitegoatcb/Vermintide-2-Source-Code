...
local NON_FATAL_ERROR_CODES = { [1052.0] = true, [1053.0] = true, [1047.0] = true, [1059.0] = true }
...
function BackendInterfacePeddlerPlayFab:init(backend_mirror)
...
function BackendInterfacePeddlerPlayFab:ready()

	return self._login_rewards and
	self._stock_ready and
	self._steam_stock_ready and
	self._chips_ready and
	self._app_prices_ready

end

function BackendInterfacePeddlerPlayFab:destroy()
...
function BackendInterfacePeddlerPlayFab:get_peddler_stock()
	return self._peddler_stock
end

local empty_params = { }

function BackendInterfacePeddlerPlayFab:get_filtered_items(filter, params)
...
function BackendInterfacePeddlerPlayFab:get_chips(chip_type)
	return self._chips [chip_type]
end

function BackendInterfacePeddlerPlayFab:get_app_price(app_id)
	return self._app_prices [app_id]
end

function BackendInterfacePeddlerPlayFab:get_steam_item_price(steam_itemdefid)
	return self._steam_item_prices [steam_itemdefid], self._steam_item_currency
end

function BackendInterfacePeddlerPlayFab:is_purchaseable(steam_itemdefid)
...
function BackendInterfacePeddlerPlayFab:get_unseen_currency_rewards()
...
function BackendInterfacePeddlerPlayFab:refresh_stock(external_cb)
...
function BackendInterfacePeddlerPlayFab:_refresh_stock_cb(external_cb, result)
...
function BackendInterfacePeddlerPlayFab:set_chips(chip_type, chip_amount)
	self._chips [chip_type] = chip_amount
end

function BackendInterfacePeddlerPlayFab:refresh_chips(external_cb)
	local request = { FunctionName = "getUserChips",
...
function BackendInterfacePeddlerPlayFab:_refresh_chips_cb(external_cb, result)
...
function BackendInterfacePeddlerPlayFab:refresh_layout_override(use_mirrored_title_data, external_cb)
	local mirror = self._backend_mirror
	...
	if use_mirrored_title_data then
		...
		local request = { Keys = { "store_layout_override" } }
...
function BackendInterfacePeddlerPlayFab:_refresh_layout_override_cb(external_cb, result)
...
function BackendInterfacePeddlerPlayFab:store_display_items()
	local mirror = self._backend_mirror
	local title_data = mirror:get_title_data()
...
function BackendInterfacePeddlerPlayFab:refresh_platform_item_prices(external_cb)
	if HAS_STEAM then
...
function BackendInterfacePeddlerPlayFab:_read_bundle_from_steam(steam_itemdefid)
...
function BackendInterfacePeddlerPlayFab:_refresh_steam_item_prices_cb(external_cb, price_list, currency)
	...
	for i = 1, #price_list, 2 do
		...
		if item_key then
			...
			if not master_item.steam_store_hidden and

			verify_stock_item(master_item) then
				...
				peddler_stock [steam_stock_index] = { type = "item",
...
function BackendInterfacePeddlerPlayFab:refresh_app_prices(external_cb)
...
function BackendInterfacePeddlerPlayFab:_refresh_app_prices_steam(external_cb)
	local request = { FunctionName = "getSteamAppPriceInfo",
...
function BackendInterfacePeddlerPlayFab:_refresh_app_prices_steam_cb(external_cb, result)
	local function_result = result.FunctionResult
	local success = true
...
function BackendInterfacePeddlerPlayFab:_refresh_app_prices_psn(external_cb)
...
function BackendInterfacePeddlerPlayFab:_refresh_app_prices_psn_cb(external_cb, product_label_lookup, result_json)
	print("")
...
function BackendInterfacePeddlerPlayFab:_refresh_app_prices_xboxlive(external_cb)
...
function BackendInterfacePeddlerPlayFab:_refresh_app_prices_xboxlive_cb(external_cb, product_id_lookup, result)
...
function BackendInterfacePeddlerPlayFab:exchange_chips(item_id, chip_type, expected_chip_amount, external_cb)
	local request = {
		StoreId = PEDDLER_ID,
		ItemId = item_id,
...
function BackendInterfacePeddlerPlayFab:_exchange_chips_success_cb(external_cb, result)
	...
	local request = { FunctionName = "storePurchaseMade",
...
function BackendInterfacePeddlerPlayFab:_exchange_chips_error_cb(external_cb, result, reenable_queue_function)
...
function BackendInterfacePeddlerPlayFab:_store_purchase_made_cb(items, result)
	local function_result = result.FunctionResult
	...
	if function_result.new_cosmetics then
		for i = 1, #function_result.new_cosmetics do
			...
			self._backend_mirror:add_item(item and item.ItemInstanceId, { ItemId = function_result.new_cosmetics [i] })
		end
	end

	if function_result.new_weapon_skins then
		for i = 1, #function_result.new_weapon_skins do
			self._backend_mirror:add_item(nil, { ItemId = function_result.new_weapon_skins [i] })
		end
	end
end

function BackendInterfacePeddlerPlayFab:_refresh_on_error(external_cb)
	self:refresh_stock(callback(self, "_refresh_stock_on_error_cb", external_cb))
end

function BackendInterfacePeddlerPlayFab:_refresh_stock_on_error_cb(external_cb)
	self:refresh_chips(callback(self, "_refresh_chips_on_error_cb", external_cb))
end

function BackendInterfacePeddlerPlayFab:_refresh_chips_on_error_cb(external_cb)
	self:refresh_layout_override(false, callback(self, "_refresh_layout_override_on_error_cb", external_cb))
end

function BackendInterfacePeddlerPlayFab:_refresh_layout_override_on_error_cb(external_cb)
...
function BackendInterfacePeddlerPlayFab:refresh_login_rewards(external_cb)
	local request = { FunctionName = "getStoreRewards" }
...
function BackendInterfacePeddlerPlayFab:_refresh_login_rewards_cb(external_cb, result)
...
function BackendInterfacePeddlerPlayFab:get_login_rewards()
	return self._login_rewards
end

function BackendInterfacePeddlerPlayFab:done_claiming_login_rewards()
