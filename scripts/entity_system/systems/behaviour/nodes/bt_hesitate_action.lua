...
function BTHesitateAction:init(...)
...
function BTHesitateAction:enter(unit, blackboard, t)
...
function BTHesitateAction:leave(unit, blackboard, t, reason, destroy)
...
function BTHesitateAction:anim_cb_hesitate_finished(unit, blackboard)
	blackboard.hesitate_finished = true
end

function BTHesitateAction:set_unit_wall_hesitation(unit, blackboard, current_pos)
...
function BTHesitateAction:wall_check(unit, blackboard, current_pos, direction)
	...
	if result and (not DO_DOT_CHECK_FOR_RE_RAYCAST or Vector3.dot(normal, -direction) < RE_RAYCAST_DOT) then
		local result2, hit_pos2, hit_distance2, normal2 = PhysicsWorld.immediate_raycast(
		physics_world,
		raycast_pos,
		-normal,
		distance,
...
function BTHesitateAction:calculate_outnumber_multiplier(unit, blackboard, t, dt, current_pos, target_pos)
...
function BTHesitateAction:start_move_animation(unit, blackboard, target_pos)
...
function BTHesitateAction:run(unit, blackboard, t, dt)
...
function BTHesitateAction:_select_new_hesitate_anim(unit, blackboard)
	local anim = nil
	if not blackboard.do_wall_check then
		anim = "hesitate"
	elseif blackboard.last_hesitate_anim == "hesitate_bwd" then
		if Math.random() > 0.3333333333333333 then anim = "hesitate" else anim = "hesitate_bwd"
		end
	elseif Math.random() > 0.3333333333333333 then anim = "hesitate_bwd" else anim = "hesitate"
	end
