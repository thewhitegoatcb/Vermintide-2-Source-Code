...
local STORY_STATES = { "stopped_beginning", "moving_forward", "moving_backward", "stopped_end" }
...
function LinkerTransportationExtension:init(extension_init_context, unit, extension_init_data)
	self.unit = unit
...
function LinkerTransportationExtension:extensions_ready()

	return end

function LinkerTransportationExtension:movement_delta()
	return self._movement_delta:unbox()
end

function LinkerTransportationExtension:register_navmesh_units(start_unit, end_unit)
...
function LinkerTransportationExtension:interacted_with(interactor_unit)
...
function LinkerTransportationExtension:hot_join_sync(peer)
...
function LinkerTransportationExtension:rpc_hot_join_sync_linker_transporting(interactor_unit_id)
	local interactor_unit = Managers.state.network.unit_storage:unit(interactor_unit_id)

	self:interacted_with(interactor_unit)
end

function LinkerTransportationExtension:rpc_hot_join_sync_linker_transport_state(state_id, story_time)
	self.story_state = STORY_STATES [state_id]
	self.current_story_time = story_time
	self:update_nav_obstacles()
end

function LinkerTransportationExtension:_link_all_transported_units(interactor_unit)
...
function LinkerTransportationExtension:_is_inside_transportation_unit(unit, size_modifier)
...
function LinkerTransportationExtension:_is_bot(player)
	if self.is_server then
		do return player.bot_player end
...
function LinkerTransportationExtension:update_units_inside_oobb()
...
function LinkerTransportationExtension:update_nav_obstacles()
...
function LinkerTransportationExtension:update(unit, input, dt, context, t)
	...
	local units_inside_oobb = self.units_inside_oobb
	if
	units_inside_oobb and self.oobb_next_update <= t then
...
function LinkerTransportationExtension:post_update(unit, input, dt, context, t)
...
function LinkerTransportationExtension:_update_local_player_position()
	local player_manager = Managers.player

	local transported_units = self.transported_units
	local num_transported_units = #transported_units

	for i = 1, num_transported_units do repeat
			...
			if not is_inside_transportation_unit then
				...
				locomotion_extension:teleport_to(position, current_rotation) end until true
	end

end

function LinkerTransportationExtension:is_stationary()
	return self.story_state == "stopped_beginning" or self.story_state == "stopped_end"
end

function LinkerTransportationExtension:can_interact(interactor_unit)
...
function LinkerTransportationExtension:destroy()
...
function LinkerTransportationExtension:_unlink_all_transported_units()
...
function LinkerTransportationExtension:_unlink_transported_unit(unit_to_unlink)
	local unit = self.unit
...
function LinkerTransportationExtension:_get_position_from_index(index)
	local unit = self.unit
...
function LinkerTransportationExtension:_link_transported_unit(unit_to_link, teleport_on_enter)
	local unit = self.unit
...
function LinkerTransportationExtension:assign_position_to_bot()
	return Unit.world_position(self.unit, 0)
end


local AI_SLOT_SIZE = 0.5
function LinkerTransportationExtension:get_ai_slot(slot_id)
	local unit = self.unit
...
function LinkerTransportationExtension:add_transporting_ai_unit(unit, slot_id)
...
function LinkerTransportationExtension:remove_transporting_ai_unit(unit)
	if self.is_server then
		local blackboard = BLACKBOARDS [unit]
		if blackboard then
...
function LinkerTransportationExtension:_update_transported_ai_positions()
