...
function ImguiProfiler:on_show()
	CALCULATE_AVERAGE = true
end

function ImguiProfiler:on_hide()
	CALCULATE_AVERAGE = false
end

function ImguiProfiler:update(t, dt)

	return end

function ImguiProfiler:draw()
...
function ImguiProfiler:post_draw()
...
function ImguiProfiler:_draw_lookup_table(in_scope, is_top_scope)
...
function ImguiProfiler:_apply_filter(in_scope)
	local parent = in_scope.name
	if in_scope.frame_index and in_scope.frame_index < CURRENT_FRAME_INDEX then
		return
	end
	...
	INDEX = INDEX + 1
	if
	parent and string.find(string.lower(parent), string.lower(self._filter)) ~= nil then
...
function ImguiProfiler:post_update(t, dt)
