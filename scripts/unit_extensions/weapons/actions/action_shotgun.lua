...
function ActionShotgun:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionShotgun:client_owner_start_action(new_action, t, chain_action_data, power_level)
...
function ActionShotgun:_use_ammo()
...
function ActionShotgun:_add_overcharge()
	local current_action = self.current_action
...
function ActionShotgun:_start_shooting()
	local owner_unit = self.owner_unit
	local current_action = self.current_action
...
function ActionShotgun:_shooting(t, final_frame)
...
function ActionShotgun:_shoot(num_shots_total, num_shots_this_frame)
	local current_action = self.current_action
...
function ActionShotgun:client_owner_post_update(dt, t, world, can_damage)
	local owner_unit = self.owner_unit
...
function ActionShotgun:reload(current_action)
	local ammo_extension = self.ammo_extension
...
function ActionShotgun:finish(reason)
	if self.state == "start_shooting" then
		self:_start_shooting()
	end
...
function ActionShotgun:_get_spread_rotation(num_shots_total, current_rotation, num_layers_spread, bullseye, spread_pitch)
