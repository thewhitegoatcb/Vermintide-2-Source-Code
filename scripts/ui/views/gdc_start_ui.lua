local scenegraph = {
	root = { is_root = true,
		size = { 1920, 1080 },
		position = { 0, 0, UILayer.gdc_intro } },


	gdc_logo = { vertical_alignment = "top", parent = "root", horizontal_alignment = "center",
	...
	input_root = { parent = "root",

		position = { 960, 320, 0 },
		size = { 1, 1 } },

	input = { vertical_alignment = "bottom", parent = "input_root",

		position = { 0, 0, 1 },
		size = { 200, 40 } },


	input_text = { vertical_alignment = "center", parent = "input",


		size = { 600, 62 },
		position = { 0, 0, 2 } },

	input_prefix_text = { vertical_alignment = "center", parent = "input_icon", horizontal_alignment = "left",
	...
	input_icon = { vertical_alignment = "center", parent = "input", horizontal_alignment = "left",
...
local widget_definitions = {
	input = { scenegraph_id = "input",
		element = {
			passes = { { texture_id = "icon_textures", style_id = "icon_styles", pass_type = "multi_texture" },
				...
				{ style_id = "prefix_text", pass_type = "text", text_id = "prefix_text",
		...
		content = { text = "input_text", prefix_text = "", button_text = "",
			icon_textures = { "pc_button_icon_left" } },
		...
		style = {
			prefix_text = { scenegraph_id = "input_prefix_text", font_size = 36, word_wrap = true, pixel_perfect = true, horizontal_alignment = "right", vertical_alignment = "center", dynamic_font = true, font_type = "hell_shark",
			...
			text = { scenegraph_id = "input_text", font_size = 36, word_wrap = true, pixel_perfect = true, horizontal_alignment = "left", vertical_alignment = "center", dynamic_font = true, font_type = "hell_shark",
			...
			button_text = { font_size = 24, scenegraph_id = "input_icon", horizontal_alignment = "center", pixel_perfect = true, vertical_alignment = "center", dynamic_font = true, font_type = "hell_shark",
			...
			icon_styles = { scenegraph_id = "input_icon",
				texture_sizes = { { 20, 36 } },
	...
	logo = { scenegraph_id = "gdc_logo",
		element = {
			passes = { { pass_type = "texture", texture_id = "logo" } } },





		content = { logo = "vermintide_logo_transparent" },
...
function GDCStartUI:init(ingame_ui_context)
...
function GDCStartUI:create_ui_elements()
	...
	self.ui_animations.button_text_pulse = UIAnimation.init(UIAnimation.pulse_animation,
	input_widget_style.button_text.text_color, 1, 100, 255, 2)

	self.ui_animations.button_texture_pulse = UIAnimation.init(UIAnimation.pulse_animation,
	input_widget_style.icon_styles.color, 1, 100, 255, 2)
end

function GDCStartUI:update(dt)
...
function GDCStartUI:start_gdc_intro()
	self.draw_intro = true
end

function GDCStartUI:end_gdc_intro()
	self.draw_intro = nil
	self.intro_complete = true
end

function GDCStartUI:rpc_on_skip_gdc_intro(channel_id)
...
function GDCStartUI:check_start_input(input_service)
	...
	if ignore_player_count then expected_num_of_players = 1
	end
	...
	if
	input_service and (expected_num_of_players <= num_of_human_players and input_service:get("gdc_skip") or input_service:has("gdc_debug_skip") and input_service:get("gdc_debug_skip")) then
		if Managers.player.is_server then
...
function GDCStartUI:set_input_text(optinal_text)
	local ui_renderer = self.ui_renderer
	local ui_scenegraph = self.ui_scenegraph
	local widget = self.input_widget
	local widget_content = widget.content
	local widget_style = widget.style
	local text = "" local prefix_text = "" local button_text = "" local button_texture_data = nil
	...
	if button_texture_data then
		if button_texture_data.texture then
			...
			widget_content.icon_textures = { button_texture_data.texture }
			widget_style.icon_styles.texture_sizes = { button_texture_data.size }
			...
			for i = 1, #button_texture_data do
				...
				if texture_size_y < sizes [i] [2] then texture_size_y = sizes [i] [2] or texture_size_y
				end
			end
...
function GDCStartUI:get_text_width(text_style, text)
