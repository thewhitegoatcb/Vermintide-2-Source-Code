...
function Development.init_application_parameters(args, do_pretty_print_args)
	...
	local function first_char(s) return s:sub(1, 1) end
	local function is_parameter(s) return first_char(s) == "-" end
	local function parameter(s) return s:sub(2) end

	local num_args = #args
	local i = 1

	local function has_more_args() return i <= num_args end
	local function has_more_args_after_current() return num_args >= i + 1 end
	local function step_to_next_arg() i = i + 1 end
	local function current_arg() return args [i] end
	local function next_arg() return args [i + 1] end
	...
	local function warn_multiple_definitions(parameter_name, old)
		...
		if type(value) ~= "table" or not value then local t = { value } end
		printf("[parse_application_parameters] multiple defintions of '%s' using [%s]. old value [%s]",
		parameter_name, table.tostring(t), table.tostring(old))
	end
	...
	while has_more_args() do
		...
		if not is_parameter(arg) then
			...
			local no_value_exists_for_param = has_more_args_after_current() and next_is_parameter() or
			not has_more_args_after_current()
			...
			if no_value_exists_for_param then
				...
				while has_more_args_after_current() and not next_is_parameter() do
					...
					elseif type(application_parameters [param]) == "table" then
						...
						local value_table = { current_value, value }
	...
	script_data ["eac-untrusted"] =
	application_parameters ["eac-untrusted"] ~= nil or
	application_parameters.eac_untrusted ~= nil
