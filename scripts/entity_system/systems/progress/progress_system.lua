...
local extensions = { "PlayerInZoneExtension" }



local RPCS = { "rpc_player_in_zone_set_active", "rpc_player_in_zone_end_event" }




function ProgressSystem:init(entity_system_creation_context, system_name)
...
function ProgressSystem:on_add_extension(world, unit, extension_name, extension_init_data)
	local extension = ProgressSystem.super.on_add_extension(self, world, unit, extension_name)
	self._existing_units [unit] = extension
	return extension
end

function ProgressSystem:on_remove_extension(unit, extension_name)
	self._existing_units [unit] = nil
	ProgressSystem.super.on_remove_extension(self, unit, extension_name)
end

function ProgressSystem:destroy()
	self._network_event_delegate:unregister(self)
end

function ProgressSystem:rpc_player_in_zone_end_event(channel_id, unit_id)
...
function ProgressSystem:rpc_player_in_zone_set_active(channel_id, unit_index)
