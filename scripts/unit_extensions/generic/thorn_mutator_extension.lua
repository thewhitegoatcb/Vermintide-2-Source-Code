...
function ThornMutatorExtension:init(extension_init_context, unit, extension_init_data)
...
function ThornMutatorExtension:current_progress()
	return self._spawn_timer
end

function ThornMutatorExtension:get_spawn_time()
	return self.spawn_time
end

function ThornMutatorExtension:setup_rpc_sync(spawn_time, progress)
	self.spawn_time = spawn_time
	self._spawn_timer = progress
end

function ThornMutatorExtension:update(unit, input, dt, context, t)
	...
	if spawn_timer < 1 then
		...
		if
		spawn_timer == 1 and self._is_server then
	...
	if self._spawn_timer == 1 and self._life_timer < 1 then
		...
		if not self._despawning then
			...
			if
			life_timer == 1 and self._is_server then
				local network_manager = Managers.state.network
				local unit_id = network_manager:unit_game_object_id(self._unit)
...
function ThornMutatorExtension:trigger_area_damage()
...
function ThornMutatorExtension:despawn()
...
function ThornMutatorExtension:_check_for_deletion(t)
