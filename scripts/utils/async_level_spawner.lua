...
function AsyncLevelSpawner:init(world_name, level_name, spawned_object_sets, frame_time_budget)
...
function AsyncLevelSpawner:update()
	...
	if done then
		...
		self._world = world world = self._world
		self._level = level level = self._level
...
function AsyncLevelSpawner:_setup_world(world_name)
	local layer = 1


	local flags = { Application.ENABLE_UMBRA, Application.ENABLE_VOLUMETRICS }
