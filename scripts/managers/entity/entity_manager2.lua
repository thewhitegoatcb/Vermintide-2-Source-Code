...
function EntityManager2:set_extension_extractor_function(extension_extractor_function)
	self.extension_extractor_function = extension_extractor_function
end

function EntityManager2:register_system(system, system_name, extension_list)
...
function EntityManager2:add_unit_extensions(world, unit, unit_template_name, all_extension_init_data)
	...
	local self_units = self._units local self_extensions = self._extensions local self_systems = self._systems
	...
	if unit_template_name and self.system_to_extension_per_unit_type_map [extensions_list] == nil then
		...
		for i = 1, num_extensions do repeat
				...
				if system_name then
					reverse_lookup [system_name] = extension_name end until true
		end
	...
	for i = 1, num_extensions do repeat
			...
			assert(extension ~= EMPTY_TABLE) until true
	end
	...
	for i = 1, num_extensions do repeat
			...
			if system.extensions_ready ~= nil then
				system:extensions_ready(world, unit, extension_name) end until true
	end
...
function EntityManager2:sync_unit_extensions(unit, go_id)
...
function EntityManager2:hot_join_sync(unit)
...
local TEMP_TABLE = { }
function EntityManager2:register_unit(world, unit, maybe_init_data, ...)
...
function EntityManager2:add_and_register_units(world, unit_list, num_units)
...
function EntityManager2:register_units_extensions(unit_list, num_units)

	local self_units = self._units
	local self_extensions = self._extensions

	for i = 1, num_units do repeat
			local unit = unit_list [i]
			...
			for extension_name, extension in pairs(unit_extensions) do
				...
				self_extensions [extension_name] [unit] = extension end until true
	end


end

function EntityManager2:remove_extensions_from_unit(unit, extensions_to_remove)
	local unit_extensions_list = self._unit_extensions_list
	local self_extensions = self._extensions
...
function EntityManager2:freeze_extensions(unit, extensions_to_freeze, freeze_reason)
...
function EntityManager2:unregister_units(units, num_units)
	local networked_flow_state = self._networked_flow_state

	local self_units = self._units local self_extensions = self._extensions
	...
	for i = 1, num_units do repeat
			...
			self_units [unit] = nil
			unit_extensions_list [unit] = nil until true
	end


end

function EntityManager2:game_object_unit_destroyed(unit)
	local unit_extensions_list = self._unit_extensions_list
	local extensions_list = unit_extensions_list [unit]
...
function EntityManager2:add_ignore_extensions(ignore_extensions)
	local ignore_extensions_list = self._ignore_extensions_list
...
function EntityManager2:unregister_unit(unit)
