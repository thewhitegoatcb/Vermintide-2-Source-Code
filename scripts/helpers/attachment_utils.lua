...
function AttachmentUtils.create_attachment(world, owner_unit, attachments, slot_name, item_data, show)
...
function AttachmentUtils.create_weapon_visual_attachment(world, owner_unit, unit_to_spawn, attachment_node_linking)
	local unit_spawner = Managers.state.unit_spawner
...
function AttachmentUtils.destroy_attachment(world, owner_unit, slot_data)
...
function AttachmentUtils.link(world, source, target, node_linking)
...
function AttachmentUtils.hot_join_sync(peer_id, unit, slots, synced_buffs)
	...
	for slot_name, slot_data in pairs(slots) do repeat
			...
			if slot_synced_buffs then
				...
				RPC.rpc_add_attachment_buffs(channel_id, unit_go_id, slot_id, num_buffs, buff_ids, buff_value_type_ids, buff_values) end until true
	end


end

function AttachmentUtils.get_syncable_buff_params(synced_buffs)
	...
	local params = {
		buff_1_id, buff_data_type_1_id, buff_value_1,
		buff_2_id, buff_data_type_2_id, buff_value_2,
		buff_3_id, buff_data_type_3_id, buff_value_3,
		buff_4_id, buff_data_type_4_id,
		buff_value_4 }
