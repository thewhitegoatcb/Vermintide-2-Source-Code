...
function BTRatlingGunnerMoveToShootAction:leave(unit, blackboard, t, reason, destroy)
	if reason ~= "done" then
		blackboard.move_pos = nil
	end
...
function BTRatlingGunnerMoveToShootAction:run(unit, blackboard, t, dt)
...
function BTRatlingGunnerMoveToShootAction:move_to(position, unit, blackboard)
	local navigation_extension = blackboard.navigation_extension
	navigation_extension:move_to(position)
	blackboard.move_pos = Vector3Box(position)
end

function BTRatlingGunnerMoveToShootAction:calculate_move_position(unit, blackboard)
	local action = blackboard.action
