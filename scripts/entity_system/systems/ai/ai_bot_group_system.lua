...
local extensions = { "AIBotGroupExtension", "BotBreakableExtension" }
...
function AIBotGroupExtension:init()
	return end

function AIBotGroupExtension:destroy()
	return end

local HOLD_POSITION_EPSILON = 0.05
function AIBotGroupExtension:set_hold_position(hold_position, max_allowed_distance)
...
function AIBotGroupExtension:get_hold_position()
	local data = self.data
	if data.hold_position then
		local hold_position = data.hold_position:unbox() local distance_sq = data.hold_position_max_distance_sq
		do return hold_position, distance_sq end
	else
		return nil, nil
	end
end
...
function AIBotGroupSystem:init(context, system_name)
	...
	if self._is_server then
		...
		self._disallowed_tag_layers = { bot_poison_wind = true, barrel_explosion = true }
		...
		local rpcs = { "rpc_bot_create_threat_oobb" }
...
function AIBotGroupSystem:destroy()
	if self._is_server then
		self.network_event_delegate:unregister(self)
	end
end

function AIBotGroupSystem:on_add_extension(world, unit, extension_name, extension_init_data)
	if extension_name == "BotBreakableExtension" then
		...
		local data = {
			...
			aoe_threat = { expires = -math.huge, escape_to = Vector3Box() },
		...
		if item_data then
			...
			if Weapons [template_name].is_grimoire then
				...
				data.pickup_orders [slot_name] = { pickup_name = pickup_name }
			end
		end
...
function AIBotGroupSystem:extensions_ready(world, unit, extension_name)
	if extension_name ~= "BotBreakableExtension" then
...
function AIBotGroupSystem:on_remove_extension(unit, extension_name)
...
function AIBotGroupSystem:hot_join_sync(peer_id, player)
	return end

function AIBotGroupSystem:set_in_carry_event(enable, side)
	local side_id = side.side_id
	self._in_carry_event [side_id] = enable
end

function AIBotGroupSystem:update(context, t)
...
AIBotGroupSystem.bot_orders = {
	pickup = { rpc_type = "rpc_bot_unit_order", function_name = "_order_pickup" },
	drop = { rpc_type = "rpc_bot_lookup_order", lookup = "pickup_names", function_name = "_order_drop" } }


function AIBotGroupSystem:order(order_type, bot_unit, order_target, ordering_player)
	...
	if self._is_server then
		...
		if Managers.state.network:game() then
			...
			self._network_transmit:send_rpc_server(rpc_type,
			order_type_id,
			bot_unit_id,
			target_id,
			ordering_player:network_id(),
			ordering_player:local_player_id())
		end
	end
end

function AIBotGroupSystem:get_pickup_order(bot_unit, slot_name)
	local bot_data = self._bot_ai_data_lookup [bot_unit]
	local order = bot_data.pickup_orders [slot_name]
	return order
end

function AIBotGroupSystem:get_ammo_pickup_order_unit(bot_unit)
	local bot_data = self._bot_ai_data_lookup [bot_unit]
	local pickup_unit = bot_data.ammo_pickup_order_unit
	return pickup_unit
end

function AIBotGroupSystem:has_pending_pickup_order(bot_unit)
	local bot_data = self._bot_ai_data_lookup [bot_unit]
...
function AIBotGroupSystem:rpc_bot_unit_order(channel_id, order_type_id, bot_unit_id, order_target_id, ordering_player_peer, ordering_local_player_id)
...
function AIBotGroupSystem:rpc_bot_lookup_order(channel_id, order_type_id, bot_unit_id, order_target_id, ordering_player_peer, ordering_local_player_id)
	local order_type = NetworkLookup.bot_orders [order_type_id]
	local bot_unit = self._unit_storage:unit(bot_unit_id)
...
function AIBotGroupSystem:queue_aoe_threat(position, shape, size, rotation, duration)
...
function AIBotGroupSystem:rpc_bot_create_threat_oobb(channel_id, threat_position, threat_rotation, threat_size, threat_duration)
	self:queue_aoe_threat(threat_position, "oobb", threat_size, threat_rotation, threat_duration)
end

function AIBotGroupSystem:_order_ammo_pickup(bot_unit, pickup_unit, ordering_player)
	local bot_data = self._bot_ai_data_lookup [bot_unit]
	if bot_data then
...
function AIBotGroupSystem:_order_pickup(bot_unit, pickup_unit, ordering_player)
	if self._is_server then
		...
		elseif slot_name then
			...
			if bot_data then
				...
				if slot_data and not can_hold_more then
					...
					has_similar_item_already = pickup_ext.pickup_name == "grimoire" and
					current_item_template.is_grimoire or

					current_item_template.pickup_data and current_item_template.pickup_data.pickup_name == pickup_ext.pickup_name
				...
				bot_data.pickup_orders [slot_name] = { unit = pickup_unit, pickup_name = pickup_ext.pickup_name }
...
function AIBotGroupSystem:_order_drop(bot_unit, pickup_name, ordering_player)
	if self._is_server then
		local bot_data = self._bot_ai_data_lookup [bot_unit]
		if bot_data then
...
function AIBotGroupSystem:_update_existence_checks(dt, t)
...
function AIBotGroupSystem:_update_move_targets(dt, t)
	local side_manager = Managers.state.side
	...
	for side_id = 1, #bot_ai_data do repeat
			...
			table.clear(TEMP_PLAYER_UNITS)
			table.clear(TEMP_DISABLED_PLAYER_UNITS) until true
	end
end

function AIBotGroupSystem:_selected_unit_is_in_disallowed_nav_tag_volume(nav_world, selected_unit_pos)
...
function AIBotGroupSystem:_find_closest_move_target(targets, last_target, position)
...
function AIBotGroupSystem:_find_least_lonely_move_target(targets, last_target)
	local num_targets = #targets
	for i = 1, num_targets do
		local unit = targets [i]
...
function AIBotGroupSystem:_find_most_lonely_move_target(targets, origin_unit)
	local num_targets = #targets
	for i = 1, num_targets do
		local unit = targets [i]
		TEMP_PLAYER_POSITIONS [i] = POSITION_LOOKUP [unit]
...
function AIBotGroupSystem:_find_origin(nav_world, selected_unit)
	local unit_pos = POSITION_LOOKUP [selected_unit]
...
function AIBotGroupSystem:_find_cluster_position(nav_world, selected_unit)
...
local function find_permutation(current_index, units, tested_points, current_solution, utility, data, best_utility, best_solution)
	...
	if current_index > num_units then
		if best_utility < utility then
			...
			do return utility end
		else
			do return best_utility end
		end
	else
...
function AIBotGroupSystem:_assign_destination_points(bot_ai_data, points, follow_unit, follow_unit_table)
...
function AIBotGroupSystem:_calculate_center_of_volume(volume_mapping)
...
function AIBotGroupSystem:_find_destination_points_outside_volume(nav_world, selected_unit_pos, volume_mapping, origin_point, needed_points)
...
function AIBotGroupSystem:_find_destination_points(nav_world, origin_point, rotation, needed_points)
	local range = 3
...
function AIBotGroupSystem:_find_points(nav_world, origin_point, rotation, left_vectors, right_vectors, space_per_player, range, needed_points)
...
function AIBotGroupSystem:_raycast(nav_world, point, vector, range)
...
function AIBotGroupSystem:_update_priority_targets(dt, t)
...
function AIBotGroupSystem:_update_urgent_targets(dt, t)
	local conflict_director = Managers.state.conflict
...
local URGENT_TARGET_REVIVE_MIN_DISTANCE_SQ = { skaven_ratling_gunner = 25, skaven_pack_master = 49, chaos_corruptor_sorcerer = 100, skaven_poison_wind_globadier = 25, skaven_warpfire_thrower = 100, chaos_plague_sorcerer = 25 }
...
function AIBotGroupSystem:_can_revive_with_urgent_target(bot_unit, self_position, blackboard, urgent_target, t)
	...
	elseif breed_name == "skaven_ratling_gunner" then
		...
		do return ( not hit_by_projectile or t > hit_by_projectile + 1 ) and revive_min_distance_sq < distance_squared end
	else
...
function AIBotGroupSystem:_update_opportunity_targets(dt, t)
	local conflict_director = Managers.state.conflict
...
function AIBotGroupSystem:_calculate_opportunity_utility(bot_unit, bot_blackboard, self_position, current_target, potential_target, t, force_seen, use_difficulty_reaction_times)
	...
	if prox_ext and not prox_ext.has_been_seen and not force_seen then
		do return -math.huge, math.huge end
...
function AIBotGroupSystem:_update_pickups(dt, t)
...
function AIBotGroupSystem:_update_orders(dt, t)
	local bot_ai_data = self._bot_ai_data
	for side_id = 1, #bot_ai_data do
		local side_bot_data = bot_ai_data [side_id]
		for unit, data in pairs(side_bot_data) do
			...
			for slot_name, order in pairs(orders) do
				...
				if slot_data and not can_hold_more then
					...
					has_picked_up_item = order.pickup_name == "grimoire" and
					current_item_template.is_grimoire or

					current_item_template.pickup_data and current_item_template.pickup_data.pickup_name == order.pickup_name
...
function AIBotGroupSystem:_update_pickups_near_player(player_unit, t)
		...
		if pickup_extension and (not aware_extension or aware_extension.has_been_seen or ScriptUnit.extension(pickup_unit, "ping_system"):pinged()) then
			...
			if pickup_name == "healing_draught" or pickup_name == "first_aid_kit" or pickup_name == "tome" then
				...
				if not hp_pickups [pickup_unit] then
					hp_pickups [pickup_unit] = { template = template, valid_until = valid_until }
				else
				...
				for unit, data in pairs(side_bot_data) do
					...
					if not ammo_pickup_order_unit or bb.ammo_pickup_valid_until <= t then
						...
						if game_mode_key == "survival" then
							...
							allowed_to_take_ammo =
							pickup_ammo_kind == "thrown" and true or

							bb.has_ammo_missing and (not pickup_data.only_once or bb.needs_ammo and all_players_have_ammo)
						end



						local ammo_condition = ( dist < allowed_distance_to_self or follow_pos and Vector3.distance(follow_pos, pickup_pos) < allowed_distance_to_follow_pos ) and (
						not current_pickup or dist - (current_pickup == pickup_unit and ammo_stickiness or 0) < data.ammo_dist)
...
function AIBotGroupSystem:_update_mule_pickups(dt, t)
...
function AIBotGroupSystem:_update_health_pickups(dt, t)
	local Unit_alive = Unit.alive
	...
	for side_id = 1, #bot_ai_data do
		...
		for bot_unit, data in pairs(side_bot_data) do
			...
			if reservation then
				...
				if not pickup_unit then --[[ Nothing --]]
		...
		for unit, data in pairs(side_bot_data) do
			...
			if RESERVED_HEALTH_ITEMS_TEMP [unit] and not has_heal_item then --[[ Nothing --]]
...
function AIBotGroupSystem:_calculate_priority_target_utility(self_position, target, time, current_target)
...
function AIBotGroupSystem:_update_first_person_debug()
...
function AIBotGroupSystem:_update_weapon_debug()
...
function AIBotGroupSystem:_update_order_debug()
	if not script_data.ai_bots_order_debug then
		return
	end

	local debug_colors = { slot_healthkit = Color(255, 0, 0), slot_potion = Color(0, 255, 0), slot_level_event = Color(0, 0, 255), slot_grenade = Color(0, 255, 255) }
...
function AIBotGroupSystem:_update_proximity_bot_breakables_debug()
...
function AIBotGroupSystem:_update_ally_needs_aid_priority()
	local unit_alive = Unit.alive
...
function AIBotGroupSystem:first_person_debug(bot_number)
	...
	if not new_player then return
	end
	local new_unit = new_player.player_unit

	if not Unit.alive(new_unit) then return
	end
	...
	if not Unit.alive(old_unit) then return
	end
...
function AIBotGroupSystem:ranged_attack_started(attacker_unit, victim_unit, attack_type)
...
function AIBotGroupSystem:ranged_attack_ended(attacker_unit, victim_unit, attack_type, optional_cooldown)
	local bot_ai_data = self._bot_ai_data
	for side_id = 1, #bot_ai_data do
		local side_bot_data = bot_ai_data [side_id]
		for unit, _ in pairs(side_bot_data) do
...
function AIBotGroupSystem:enemy_teleported(enemy_unit, teleport_position)
...
function AIBotGroupSystem:register_ally_needs_aid_priority(bot_unit, target_unit)
...
function AIBotGroupSystem:is_prioritized_ally(bot_unit, target_unit)
	return self._ally_needs_aid_priority [target_unit] == bot_unit
end

local BROADPHASE_RESULTS = { }
function AIBotGroupSystem:_update_proximity_bot_breakables(t)
	local nav_world = Managers.state.entity:system("ai_system"):nav_world()
...
function AIBotGroupSystem:set_in_cover(bot_unit, cover_unit)
	self._used_covers [bot_unit] = cover_unit
end

function AIBotGroupSystem:in_cover(cover_unit)
...
function AIBotGroupSystem:aoe_threat_created(position, shape, size, rotation, duration)
	...
	local pos_x = position.x local pos_y = position.y local pos_z = position.z
	local bot_ai_data = self._bot_ai_data
	for side_id = 1, #bot_ai_data do
		local side_bot_data = bot_ai_data [side_id]
		for unit, data in pairs(side_bot_data) do
...
local MESSAGES = {
	abort_pickup_assigned_to_other = {
		default = { "bot_command_generic_abort_pickup_assigned_to_other_01" } },



	acknowledge_pickup = {
		default = { "bot_command_generic_acknowledge_pickup_01" } },



	acknowledge_ammo = {
		default = { "bot_command_generic_acknowledge_ammo_01" } },



	has_full_ammo = {
		default = { "bot_command_generic_has_full_ammo_01" } },



	already_picking_up = {
		default = { "bot_command_generic_already_picking_up_01" } },



	already_have_item = {
		default = { "bot_command_generic_already_have_item_01" } },



	acknowledge_drop = {
		default = { "bot_command_generic_acknowledge_drop_01" } } }





function AIBotGroupSystem:_chat_message(unit, ordering_player, message, ...)
