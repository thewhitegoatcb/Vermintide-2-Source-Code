...
function StartGameWindowMissionSelectionConsole:on_enter(params, offset)
...
function StartGameWindowMissionSelectionConsole:_start_transition_animation(animation_name)
	local params = {
...
function StartGameWindowMissionSelectionConsole:_create_ui_elements(params, offset)
...
function StartGameWindowMissionSelectionConsole:_setup_levels_by_area(area_name)
...
function StartGameWindowMissionSelectionConsole:_setup_level_acts(acts)
	local levels_by_act = { }
...
function StartGameWindowMissionSelectionConsole:_verify_act(act)
	if not act then
		return false
	end
...
function StartGameWindowMissionSelectionConsole:_present_act_levels(act)
...
function StartGameWindowMissionSelectionConsole:_select_level(level_id)
...
function StartGameWindowMissionSelectionConsole:_set_presentation_info(level_id)
	local level_text = ""
...
function StartGameWindowMissionSelectionConsole:_setup_mission_data(level_settings)
...
function StartGameWindowMissionSelectionConsole:_sync_completed_difficulty(mission_count, entry_width, level_settings)
...
function StartGameWindowMissionSelectionConsole:_calculate_paint_scrap_amount(level_settings)
...
local TEMP_TABLE = { }
function StartGameWindowMissionSelectionConsole:_sync_hero_completion(level_settings)
	local level_key = level_settings.level_id
...
function StartGameWindowMissionSelectionConsole:_profile_difficulty_index_completed(profile, level_key)
	local statistics_db = self._statistics_db
	local stats_id = self._stats_id
...
function StartGameWindowMissionSelectionConsole:_sync_missions(mission_settings_data, level_settings)
...
function StartGameWindowMissionSelectionConsole:_setup_grid_navigation()
...
function StartGameWindowMissionSelectionConsole:_find_level_location_in_grid(level_id)
	...
	if not IS_XB1 then --[[ Nothing --]] end
...
function StartGameWindowMissionSelectionConsole:on_exit(params)
	print("[StartGameWindow] Exit Substate StartGameWindowMissionSelectionConsole")
	self._ui_animator = nil
	self._parent:set_input_description(nil)
end

function StartGameWindowMissionSelectionConsole:update(dt, t)
...
function StartGameWindowMissionSelectionConsole:post_update(dt, t)
	return end

function StartGameWindowMissionSelectionConsole:_update_animations(dt)
...
function StartGameWindowMissionSelectionConsole:_is_button_pressed(widget)
	local content = widget.content
...
function StartGameWindowMissionSelectionConsole:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function StartGameWindowMissionSelectionConsole:_update_level_option()
...
function StartGameWindowMissionSelectionConsole:_is_level_presented(level_id)
	local active_node_widgets = self._active_node_widgets
	if active_node_widgets then
		for i = 1, #active_node_widgets do
			local widget = active_node_widgets [i]
...
function StartGameWindowMissionSelectionConsole:_get_first_level_id()
	local area_name = self._parent:get_selected_area_name()
	local area_settings = AreaSettings [area_name]
	local acts = area_settings.acts
...
function StartGameWindowMissionSelectionConsole:_update_selection_from_grid()
...
function StartGameWindowMissionSelectionConsole:_update_grid_row(new_row)
	local navigation_grid = self._navigation_grid
...
function StartGameWindowMissionSelectionConsole:_update_grid_column(new_column)
...
function StartGameWindowMissionSelectionConsole:_update_grid_navigation(row_change, column_change)
...
function StartGameWindowMissionSelectionConsole:_find_row(row_change)
...
function StartGameWindowMissionSelectionConsole:_find_column(column_change)
...
function StartGameWindowMissionSelectionConsole:_level_is_unlocked(level_id)
	local statistics_db = self._statistics_db
	local stats_id = self._stats_id
	return LevelUnlockUtils.level_unlocked(statistics_db, stats_id, level_id)
end

function StartGameWindowMissionSelectionConsole:_handle_input(dt, t)
	...
	if active_node_widgets then
		for i = 1, #active_node_widgets do
			...
			if self:_is_button_hovered(widget) and
			self._selected_level_id ~= level_id then
				self:_play_sound("play_gui_lobby_button_02_mission_act_click")
				self:_select_level(level_id)
			end
...
function StartGameWindowMissionSelectionConsole:_draw(dt)
...
function StartGameWindowMissionSelectionConsole:_play_sound(event)
	self._parent:play_sound(event)
end

function StartGameWindowMissionSelectionConsole:_animate_node_widget(widget, dt)
	local content = widget.content
	local hotspot = content.button_hotspot
