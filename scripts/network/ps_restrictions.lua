...
local default_restrictions = { "network_availability", "playstation_plus", "parental_control" }





local start_funcs = { playstation_plus = "_playstation_plus_start", network_availability = "_network_availability_start", parental_control = "_parental_control_start" }





local callbacks = { playstation_plus = "cb_playstation_plus", network_availability = "cb_network_availability", parental_control = "cb_parental_control" }
...
function PSRestrictions:init()
	self._current_users = { }
end

function PSRestrictions:add_user(user_id)
	self._current_users [user_id] = { restrictions = table.clone(default_restrictions) }
	self:_start_restriction_access_fetched(user_id)
end

function PSRestrictions:_start_restriction_access_fetched(user_id)
	local user = self._current_users [user_id]
	self:_fetch_next_restriction_access(user_id)
end

function PSRestrictions:_fetch_next_restriction_access(user_id)
	...
	if signed_in then
		...
		self [callback_name](self, user_id, restriction, { error = PS4.SCE_NP_ERROR_SIGNED_OUT })
	end
end

function PSRestrictions:has_access(user_id, restriction)
	if fake_restrictions then
		return true
	end
	local user = self._current_users [user_id]
...
function PSRestrictions:has_error(user_id, restriction)
	if fake_restrictions then
		return false
	end
	local user = self._current_users [user_id]
...
function PSRestrictions:restriction_access_fetched(user_id, restriction)
	if fake_restrictions then
		return true
	end
	local user = self._current_users [user_id]
...
function PSRestrictions:refetch_restriction_access(user_id, restrictions)
...
function PSRestrictions:_set_restriction_fetched(user_id, restriction)
	local user = self._current_users [user_id]
	local restrictions = user.restrictions
...
function PSRestrictions:_try_fetch_next_restriction_access(user_id)
	local user = self._current_users [user_id]
	local restrictions = user.restrictions
...
function PSRestrictions:_playstation_plus_start(user_id)
	return NpCheck.check_plus(user_id, NpCheck.REALTIME_MULTIPLAY)
end

function PSRestrictions:_network_availability_start(user_id)
	return NpCheck.check_availability(user_id)
end

function PSRestrictions:_parental_control_start(user_id)
	return NpCheck.parental_control_info(user_id)
end



function PSRestrictions:cb_network_availability(user_id, restriction, info)
	...
	if error then
		...
		self._current_users [user_id] [restriction] = { access = false, error = error }
	else
		local result = NpCheck.result(info.token)
		dprint("(%q) access for user (%d) result (%s)", restriction, user_id, tostring(result))
		self._current_users [user_id] [restriction] = { error = false, access = result }
	end
...
function PSRestrictions:cb_playstation_plus(user_id, restriction, info)
	local error = info.error or NpCheck.error_code(info.token)
	if error then
		error_print("Error (%#x) when checking (%s) access for user (%d)", error, restriction, user_id)
		self._current_users [user_id] [restriction] = { access = false, error = error }
...
function PSRestrictions:cb_parental_control(user_id, restriction, info)
	local error = info.error or NpCheck.error_code(info.token)
	if error then
		error_print("Error (%#x) when checking parental control access for user (%d)", error, user_id)
		self._current_users [user_id].chat = { access = false, error = error }
		self._current_users [user_id].user_generated_content = { access = false, error = error }
	else
		...
		self._current_users [user_id].chat = { error = false, access = chat }
		self._current_users [user_id].user_generated_content = { error = false, access = ugc }
