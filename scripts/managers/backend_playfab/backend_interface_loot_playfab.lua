...
function BackendInterfaceLootPlayfab:init(backend_mirror)
...
function BackendInterfaceLootPlayfab:ready()
	return true
end

function BackendInterfaceLootPlayfab:update(dt)
	return end

function BackendInterfaceLootPlayfab:_new_id()
...
function BackendInterfaceLootPlayfab:open_loot_chest(hero_name, backend_id, game_mode_key, num_chests)
	...
	local generate_loot_chest_rewards_request = { FunctionName = "generateLootChestRewards",
...
function BackendInterfaceLootPlayfab:loot_chest_rewards_request_cb(data, result)
	...
	if new_cosmetics then
		for i = 1, #new_cosmetics do
			local backend_id = backend_mirror:add_item(nil, { ItemId = new_cosmetics [i] })
			if backend_id then
...
function BackendInterfaceLootPlayfab:generate_end_of_level_loot(game_won, quick_play_bonus, difficulty, level_key, hero_name, start_experience, end_experience, loot_profile_name, deed_item_name, deed_backend_id, game_mode_key, game_time, end_of_level_rewards_arguments)
	local id = self:_new_id()
	...
	local generate_end_of_level_loot_request = { FunctionName = "generateEndOfLevelLoot",
...
function BackendInterfaceLootPlayfab:end_of_level_loot_request_cb(data, result)
	...
	for item_type, item_data in pairs(rewards) do
		...
		loot_request [item_type] = { backend_id = backend_id }
	...
	if cosmetic_rewards then
		for reward_key, item in pairs(cosmetic_rewards) do
			local backend_id = backend_mirror:add_item(nil, { ItemId = item })
			if backend_id then
				loot_request [reward_key] = { backend_id = backend_id }
			end
		end
	end
...
function BackendInterfaceLootPlayfab:_get_remote_player_network_ids_and_characters()
...
function BackendInterfaceLootPlayfab:get_achievement_rewards(achievement_id)
...
function BackendInterfaceLootPlayfab:achievement_rewards_claimed(achievement_id)
...
function BackendInterfaceLootPlayfab:can_claim_achievement_rewards(achievement_id)
	local mirror = self._backend_mirror
	local claimed_achievements = mirror:get_claimed_achievements()
...
function BackendInterfaceLootPlayfab:claim_achievement_rewards(achievement_id, poll_id)
	...
	local generate_achievement_rewards_request = { FunctionName = "generateAchievementRewards",
...
function BackendInterfaceLootPlayfab:achievement_rewards_request_cb(data, result)
	local function_result = result.FunctionResult
	local id = data.id
	...
	if items then
		for i = 1, #items do
			...
			loot [#loot + 1] = { type = "item", backend_id = backend_id, amount = amount }
		end
	end
	...
	if new_cosmetics then
		...
		for i = 1, #new_cosmetics do
			...
			if backend_id then
				loot [#loot + 1] = { type = item.slot_type, backend_id = backend_id }
			end
		end
	end
...
function BackendInterfaceLootPlayfab:can_claim_all_achievement_rewards(achievement_ids)
	...
	if table.is_empty(claimable_achievements) then
		do return false, nil, unclaimable_achievements end
	else
...
function BackendInterfaceLootPlayfab:claim_multiple_achievement_rewards(achievement_ids, poll_id, start_index, end_index)
	self._reward_poll_id = true
...
function BackendInterfaceLootPlayfab:claim_multiple_achievement_rewards_request_cb(data, id, start_index, end_index, achievement_ids, result)
...
function BackendInterfaceLootPlayfab:polling_reward()
	return self._reward_poll_id
end

function BackendInterfaceLootPlayfab:is_loot_generated(id)
...
function BackendInterfaceLootPlayfab:get_loot(id)
	return self._loot_requests [id]
end

function BackendInterfaceLootPlayfab:generate_reward_loot_id()
	return self:_new_id()
end