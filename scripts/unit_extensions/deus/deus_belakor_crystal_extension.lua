...
function DeusBelakorCrystalExtension:init(extension_init_context, unit, extension_init_data)
	self._unit = unit
...
function DeusBelakorCrystalExtension:game_object_initialized(unit, go_id)
	self._go_id = go_id
end

function DeusBelakorCrystalExtension:extensions_ready(world, unit)
	if not self._is_server then
		return
	end
...
function DeusBelakorCrystalExtension:destroy()
	if not self._is_server then
		return
	end
...
function DeusBelakorCrystalExtension:update(unit, input, dt, context, t)
	if not self._is_server then
		return
	end
	...
	if self._running_astar then
		if GwNavAStar.processing_finished(self._astar) then
			...
			if not GwNavAStar.path_found(self._astar) then
				...
				self._nav_world,
				RESPAWN_LOCUS_MIN_DISTANCE,
				RESPAWN_LOCUS_MAX_DISTANCE,
				1,
				nil, nil, nil, nil, nil,
				...
				RESPAWN_LOCUS_BELOW_MAX_DISTANCE,

				RESPAWN_LOCUS_ABOVE_MAX_DISTANCE)
...
function DeusBelakorCrystalExtension:_find_nearest_locus()
