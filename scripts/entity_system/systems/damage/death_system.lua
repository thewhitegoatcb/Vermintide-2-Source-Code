...
local RPCS = { "rpc_forced_kill" }



local extensions = { "GenericDeathExtension" }
...
function DeathSystem:init(entity_system_creation_context, system_name)
	...
	self.active_reactions = { unit = { }, husk = { } }
end

function DeathSystem:destroy()
	self.network_event_delegate:unregister(self)
end

function DeathSystem:on_add_extension(world, unit, extension_name, extension_init_data)
...
function DeathSystem:extensions_ready(world, unit, extension_name)
	local extension = self.unit_extensions [unit]
	local health_extension = ScriptUnit.extension(unit, "health_system")
	extension.health_extension = health_extension
end

function DeathSystem:on_remove_extension(unit, extension_name)
	self.frozen_unit_extensions [unit] = nil
	self:_cleanup_extension(unit, extension_name)
	ScriptUnit.remove_extension(unit, self.NAME)
end

function DeathSystem:on_freeze_extension(unit, extension_name)
	ferror("Shouldn't get called, should run during death until unspawned/frozen.")
end

function DeathSystem:_cleanup_extension(unit, extension_name)
	local extension = self.unit_extensions [unit]
...
function DeathSystem:freeze(unit, extension_name, reason)
...
function DeathSystem:unfreeze(unit, extension_name)
...
function DeathSystem:hot_join_sync(sender)
	return end

function DeathSystem:set_death_reaction_template(unit, template_name)
	local extension = self.unit_extensions [unit]
...
local function start_death_reaction(unit, death_extension, killing_blow, active_reactions, t, context, is_server)
	...
	if breed and breed.name == "skaven_poison_wind_globadier" then
		...
		Unit.get_data(unit, "globadier_43348"),
		death_reaction_template,
		network_type,
		table.tostring(killing_blow))
	end
...
function DeathSystem:update(context, t)
	local dt = context.dt
...
function DeathSystem:kill_unit(unit, killing_blow)
	local extension = self.unit_extensions [unit]
	...
	if breed and breed.name == "skaven_poison_wind_globadier" then
		printf("[HON-43348] Globadier (%s) pre-starting death reaction. template: '%s', network_type: '%s'",
		Unit.get_data(unit, "globadier_43348"),
		death_reaction_template,
...
function DeathSystem:_create_dummy_killing_blow(unit, damage_type)
...
function DeathSystem:forced_kill(unit, damage_type)
...
function DeathSystem:rpc_forced_kill(channel_id, unit_go_id, damage_type_id)
...
function DeathSystem:get_dead(fill_table)
	local sum = 0
	local active_reactions = self.active_reactions
	for network_type, templates in pairs(active_reactions) do
		for template, units in pairs(templates) do
