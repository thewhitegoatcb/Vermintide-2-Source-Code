...
function BaseEndScreenUI:init(ingame_ui_context, input_service, definitions, params)
	...
	self._draw_flags = { alpha_multiplier = 0 }
...
function BaseEndScreenUI:_setup_rewards(params)
...
function BaseEndScreenUI:destroy()
	self._rewards_popup:destroy()

	self:_destroy()
end

function BaseEndScreenUI:on_fade_in()
	self:_on_fade_in()
end


function BaseEndScreenUI:_on_fade_in()
	return end


function BaseEndScreenUI:_start()
	return end


function BaseEndScreenUI:_update(dt)
	return end


function BaseEndScreenUI:_destroy()
	return end


function BaseEndScreenUI:_draw_widgets(render_settings)
	return end


function BaseEndScreenUI:_on_completed()
	self._completed = true
end

function BaseEndScreenUI:completed()
	return self._completed and self._rewards_popup:all_presentations_done()
end

function BaseEndScreenUI:_play_sound(event)
	WwiseWorld.trigger_event(self._wwise_world, event)
end

function BaseEndScreenUI:_create_ui_elements(definitions)
...
function BaseEndScreenUI:start()
	self._started = true

	self:_start()
end

function BaseEndScreenUI:started()
	return self._started
end

function BaseEndScreenUI:update(dt, t)
	if not self._started then return
	end
...
function BaseEndScreenUI:draw(dt)
	if not self._started then return
	end
