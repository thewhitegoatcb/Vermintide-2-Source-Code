...
local RPCS = { "rpc_sync_anim_state_1", "rpc_sync_anim_state_2", "rpc_sync_anim_state_3", "rpc_sync_anim_state_4", "rpc_sync_anim_state_5", "rpc_sync_anim_state_6", "rpc_sync_anim_state_7", "rpc_sync_anim_state_8", "rpc_sync_anim_state_9", "rpc_sync_anim_state_10", "rpc_sync_anim_state_11", "rpc_sync_anim_state_12", "rpc_anim_event", "rpc_anim_event_variable_float", "rpc_anim_set_variable_float", "rpc_anim_set_variable_int", "rpc_link_unit", "rpc_anim_set_variable_by_distance", "rpc_anim_set_variable_by_time", "rpc_update_anim_variable_done" }
...
function AnimationSystem:init(entity_system_creation_context, system_name)
...
function AnimationSystem:animation_callback(unit, callback, param)
	local cb = nil
...
function AnimationSystem:update(context, t)
	self:update_anim_variables(t)
	self:_update_networked_anim_variables(context.dt, t)
end

function AnimationSystem:update_anim_variables(t)
	local position_lookup = position_lookup
...
function AnimationSystem:anim_event(unit, event_name, skip_sync)
...
function AnimationSystem:anim_event_with_variable_float(unit, event_name, variable_name, variable_value, skip_sync)
	if not skip_sync and Managers.state.network:game() then
		local go_id = self.unit_storage:go_id(unit)
		fassert(go_id, "Unit storage does not have a game object id for %q", unit)
		local event_id = NetworkLookup.anims [event_name]
...
if LEVEL_EDITOR_TEST then
	function AnimationSystem:anim_event(unit, event_name)
		self:_init_networked_variables(unit, event_name)
		Unit.animation_event(unit, event_name)
	end

	function AnimationSystem:anim_event_with_variable_float(unit, event_name, variable_name, variable_value)
		self:_init_networked_variables(unit, event_name)
		local variable_index = Unit.animation_find_variable(unit, variable_name)
		Unit.animation_set_variable(unit, variable_index, variable_value)
		Unit.animation_event(unit, event_name)
...
function AnimationSystem:_init_networked_variables(unit, event_name)
	...
	for variable_name, data in pairs(variable_datas) do
		...
		if not networked_anim_vars [unit] then local unit_vars = { updates = { } } end
...
function AnimationSystem:_remove_networked_variables(unit)
	local networked_anim_vars = self._networked_animation_variables
...
function AnimationSystem:_update_networked_anim_variables(dt, t)
...
function AnimationSystem:rpc_sync_anim_state(channel_id, go_id, ...)
...
function AnimationSystem:rpc_anim_event_variable_float(channel_id, anim_id, go_id, variable_id, variable_value)
	local unit = self.unit_storage:unit(go_id)
...
function AnimationSystem:rpc_anim_set_variable_float(channel_id, go_id, variable_id, variable_value)
	local unit = self.unit_storage:unit(go_id)
	if not unit or not Unit.alive(unit) then
		return
	end
...
function AnimationSystem:rpc_anim_set_variable_int(channel_id, go_id, variable_id, variable_value)
	local unit = self.unit_storage:unit(go_id)
	if not unit or not Unit.alive(unit) then
		return
	end
...
function AnimationSystem:rpc_anim_event(channel_id, anim_id, go_id)
	local unit = self.unit_storage:unit(go_id)
...
function AnimationSystem:rpc_link_unit(channel_id, child_unit_id, child_node, parent_unit_id, parent_node)
...
function AnimationSystem:rpc_anim_set_variable_by_distance(channel_id, unit_id, anim_variable_index, goal_pos, scale, flat_distance)
	local unit = self.unit_storage:unit(unit_id)
	self:_set_variable_by_distance(unit, anim_variable_index, goal_pos, scale, flat_distance)
end

function AnimationSystem:_set_variable_by_distance(unit, anim_variable_index, goal_pos, scale, flat_distance)
	local pos = position_lookup [unit]
	...
	if data then
		...
		self.anim_variable_update_list [unit] = { unit = unit, goal_pos = Vector3Box(goal_pos), anim_variable_index = anim_variable_index, initial_distance = initial_distance, scale = scale, flat_distance = flat_distance }
	end
end


function AnimationSystem:rpc_anim_set_variable_by_time(channel_id, unit_id, anim_variable_index, int_16bit_duration, scale)
	local unit = self.unit_storage:unit(unit_id)
	local duration = int_16bit_duration * 0.00390625
	self:_set_variable_by_time(unit, anim_variable_index, duration, scale)
end

function AnimationSystem:_set_variable_by_time(unit, anim_variable_index, duration, scale)
	local data = self.anim_variable_update_list [unit]
	...
	if data then
		...
		self.anim_variable_update_list [unit] = { unit = unit, start_time = t, duration = duration, anim_variable_index = anim_variable_index, scale = scale }
	end
end


function AnimationSystem:rpc_update_anim_variable_done(channel_id, unit_id)
	local unit = self.unit_storage:unit(unit_id)
...
function AnimationSystem:set_update_anim_variable_done(unit)
...
function AnimationSystem:start_anim_variable_update_by_distance(unit, anim_variable_index, goal_pos, scale, flat_distance)
...
function AnimationSystem:start_anim_variable_update_by_time(unit, anim_variable_index, duration, scale)
