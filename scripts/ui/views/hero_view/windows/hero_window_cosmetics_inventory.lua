...
function HeroWindowCosmeticsInventory:on_enter(params, offset)
...
function HeroWindowCosmeticsInventory:create_ui_elements(params, offset)
...
function HeroWindowCosmeticsInventory:_assign_tab_icons()
...
function HeroWindowCosmeticsInventory:on_exit(params)
...
function HeroWindowCosmeticsInventory:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function HeroWindowCosmeticsInventory:post_update(dt, t)

	return end

function HeroWindowCosmeticsInventory:_update_animations(dt)
...
function HeroWindowCosmeticsInventory:_is_button_pressed(widget)
...
function HeroWindowCosmeticsInventory:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function HeroWindowCosmeticsInventory:_is_inventory_tab_hovered()
	local widget = self._widgets_by_name.item_tabs
	local widget_content = widget.content
	local amount = widget_content.amount
	for i = 1, amount do
...
function HeroWindowCosmeticsInventory:_is_inventory_tab_pressed()
	local widget = self._widgets_by_name.item_tabs
	local widget_content = widget.content
	local amount = widget_content.amount
	for i = 1, amount do
...
function HeroWindowCosmeticsInventory:_select_tab_by_category_index(index)
	local widget = self._widgets_by_name.item_tabs
	local widget_content = widget.content
	local amount = widget_content.amount
	for i = 1, amount do
...
function HeroWindowCosmeticsInventory:_handle_input(dt, t)
	local widgets_by_name = self._widgets_by_name
...
function HeroWindowCosmeticsInventory:_update_page_info()
...
function HeroWindowCosmeticsInventory:_update_selected_cosmetic_slot_index()
...
function HeroWindowCosmeticsInventory:_update_loadout_sync()
	local item_grid = self._item_grid
	local parent = self.parent
...
function HeroWindowCosmeticsInventory:_exit(selected_level)
	self.exit = true
	self.exit_level_id = selected_level
end

function HeroWindowCosmeticsInventory:draw(dt)
...
function HeroWindowCosmeticsInventory:_play_sound(event)
	self.parent:play_sound(event)
end

function HeroWindowCosmeticsInventory:_change_category_by_index(index, force_update)
