...
function KeystrokeHelper.parse_strokes(text, index, mode, keystrokes, optional_text_length_cap)
...
function KeystrokeHelper._build_utf8_table(text, external_table)
...
KeystrokeHelper [Keyboard.F9] = function (text_table, index, mode, max_length)
	...
	if max_length then n = math.min(n, max_length - #text_table) end
	for i = 1, n do
