return { max_spawns = 3, display_name = "lightning_strike_mutator_name", description = "lightning_strike_mutator_desc", spawn_rate = 11, icon = "mutator_icon_heavens_lightning",
	...
	server_start_function = function (context, data)
		...
		data.extension_init_data = {
			area_damage_system = { explosion_template_name = "lightning_strike_twitch" } }
	...
	server_update_function = function (context, data, dt, t)
		...
		if #data.units > 0 then
			...
			elseif t < last_spawn_time + data.follow_time + data.time_to_explode then
				if t > last_spawn_time + data.follow_time + data.time_to_explode - 3 and
				not data.bots_alerted then
			...
			elseif not data.hit_played and
			t > last_spawn_time + data.follow_time + data.time_to_explode then
