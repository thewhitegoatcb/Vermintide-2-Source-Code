...
local settings = {
	tome = { item_name = "wpn_side_objective_tome_01", mission_name = "tome_bonus_mission", total_amount = 3, texture = "loot_objective_icon_02" },





	grimoire = { item_name = "wpn_grimoire_01", mission_name = "grimoire_hidden_mission", total_amount = 2, texture = "loot_objective_icon_01" } }
...
function LootObjectiveUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function LootObjectiveUI:create_ui_elements()
...
function LootObjectiveUI:destroy()
	GarbageLeakDetector.register_object(self, "loot_objective_ui")
end

local customizer_data = { root_scenegraph_id = "background", label = "Books", registry_key = "books", drag_scenegraph_id = "background" }
...
function LootObjectiveUI:update(dt, t)
	if DO_RELOAD then
		self:create_ui_elements()
	end
...
function LootObjectiveUI:_sync_missions(initialize)
...
function LootObjectiveUI:_assign_amount_to_widget(widget, amount)
	widget.content.draw_count = amount
end

function LootObjectiveUI:_get_item_amount_by_mission_name(mission_name)
...
function LootObjectiveUI:_add_presentation_event(widget, previous_amount, amount)
	local event_queue = self._event_queue
	local stack_count = #event_queue
	event_queue [#event_queue + 1] = { amount = amount, previous_amount = previous_amount, widget = widget }
end

function LootObjectiveUI:_update_active_presentation(dt, t)
	local event_queue = self._event_queue
	local stack_count = #event_queue
...
function LootObjectiveUI:_animate_in(widget, previous_draw_count)
	...
	for i = 1, math.min(amount, largest_amount) do
		...
		if i == largest_amount then
			if amount_increased then
				local anim = UIAnimation.init(UIAnimation.wait, duration + 0.2,
				func, color, target, from, 255, duration, easing)
				animations ["icon_textures_" .. i] = anim
			else
				local anim = UIAnimation.init(UIAnimation.wait, duration + 0.5,
				func, color, target, 255, 0, duration, easing)
...
function LootObjectiveUI:_animate_out(widget)
	local animations = self._animations
...
function LootObjectiveUI:_update_animations(dt)
	local animations = self._animations
...
function LootObjectiveUI:draw(dt)
