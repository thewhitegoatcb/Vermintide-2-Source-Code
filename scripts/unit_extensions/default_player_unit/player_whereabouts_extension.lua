...
function PlayerWhereaboutsExtension:init(extension_init_context, unit, extension_init_data)
	self.unit = unit
...
function PlayerWhereaboutsExtension:_setup(nav_world, unit)
...
function PlayerWhereaboutsExtension:destroy()
	return end

function PlayerWhereaboutsExtension:set_is_onground()
	self._input.is_onground = true
end

function PlayerWhereaboutsExtension:set_fell(player_state)
	self._input.fell = true
	self._input.player_state = player_state
end

function PlayerWhereaboutsExtension:set_jumped()
	self._input.jumped = true
end

function PlayerWhereaboutsExtension:set_landed()
	self._input.landed = true
end

function PlayerWhereaboutsExtension:set_no_landing()
	self._input.no_landing = true
end

function PlayerWhereaboutsExtension:update(unit, input, dt, context, t)
...
function PlayerWhereaboutsExtension:last_position_on_navmesh()
	local pos = self._last_pos_on_nav_mesh:unbox()
	return Vector3.is_valid(pos) and pos or nil
end


function PlayerWhereaboutsExtension:last_position_onground_on_navmesh()
	local pos = self._last_onground_pos_on_nav_mesh:unbox()
	return Vector3.is_valid(pos) and pos or nil
end

local EPSILON = 0.0001
function PlayerWhereaboutsExtension:_find_start_position(current_position, perform_distance_check)
...
function PlayerWhereaboutsExtension:_check_bot_nav_transition(nav_world, input, current_position)
...
function PlayerWhereaboutsExtension:_get_closest_positions(pos, is_onground, point_list)
	local nav_world = self._nav_world
...
function PlayerWhereaboutsExtension:closest_positions_when_outside_navmesh()
	return self.closest_positions, self.player_on_nav_mesh
end

function PlayerWhereaboutsExtension:set_new_hang_ledge_position(hang_ledge_position)
	self.hang_ledge_position = Vector3Box(hang_ledge_position)
end

function PlayerWhereaboutsExtension:_calculate_hang_ledge_spawn_position(hang_ledge_position)
	local nav_world = self._nav_world
...
function PlayerWhereaboutsExtension:get_hang_ledge_spawn_position()
	return self.hang_ledge_spawn_position:unbox()
end


function PlayerWhereaboutsExtension:_debug_draw(unit_position, point_list, t)
