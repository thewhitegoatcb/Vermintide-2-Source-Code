...
function CharacterSelectionStateCharacter:on_enter(params)
...
function CharacterSelectionStateCharacter:_update_video_player_settings()
...
function CharacterSelectionStateCharacter:_setup_video_player(material_name, resource)
	self:_destroy_video_player()
...
function CharacterSelectionStateCharacter:_destroy_video_player()
	local ui_top_renderer = self.ui_top_renderer
...
function CharacterSelectionStateCharacter:_inject_additional_scenegraph_definitions(scenegraph_definition)
...
function CharacterSelectionStateCharacter:create_ui_elements(params)
...
function CharacterSelectionStateCharacter:_set_hero_icon_selected(index)
	for icon_index, widget in ipairs(self._hero_icon_widgets) do
		widget.content.selected = icon_index == index
	end
end

function CharacterSelectionStateCharacter:_setup_hero_selection_widgets()
	local hero_widgets = { }
...
function CharacterSelectionStateCharacter:_update_available_profiles()
...
function CharacterSelectionStateCharacter:_handle_mouse_selection()
	local hero_widgets = self._hero_widgets
...
function CharacterSelectionStateCharacter:_update_equipped_bots()
...
function CharacterSelectionStateCharacter:_exit_bot_selection()
...
function CharacterSelectionStateCharacter:_enter_bot_selection(selected_row)
...
function CharacterSelectionStateCharacter:_handle_gamepad_selection(input_service)
	local selected_row = self._selected_hero_row
	local selected_column = self._selected_hero_column
...
function CharacterSelectionStateCharacter:_is_selected_hero_unlocked()
	local selected_row = self._selected_hero_row
	local selected_column = self._selected_hero_column
	local num_max_rows = self._num_max_hero_rows
...
function CharacterSelectionStateCharacter:_handle_gamepad_bot_selection(input_service)
...
function CharacterSelectionStateCharacter:_update_bot_order(new_row, reset_x_offset)
...
function CharacterSelectionStateCharacter:_set_bot_selection(index)
...
function CharacterSelectionStateCharacter:_reset_bot_selection(select_hero)
	self._current_selected_bot_index = nil
	self._current_selected_row = nil
	self._x_offset = nil
	self._base_y_offset = nil
...
function CharacterSelectionStateCharacter:_handle_mouse_bot_selection(input_service)
	...
	if not self.parent:input_blocked() then
		...
		for index, hero_icon_widget in ipairs(self._hero_icon_widgets) do
			...
			elseif UIUtils.is_left_button_released(hero_icon_widget, "bot_change_order_hotspot") and
			self._current_selected_row then
...
function CharacterSelectionStateCharacter:_select_hero(profile_index, career_index, ignore_sound, disable_hero_spawn)
...
function CharacterSelectionStateCharacter:_get_skin_item_data(index, career_index)
...
function CharacterSelectionStateCharacter:_wanted_state()
	local new_state = self.parent:wanted_state()
	return new_state
end

function CharacterSelectionStateCharacter:on_exit(params)
...
function CharacterSelectionStateCharacter:_respawn_player()
...
function CharacterSelectionStateCharacter:_update_transition_timer(dt)
...
function CharacterSelectionStateCharacter:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
	...
	if not self._transition_timer and not self:pending_profile_request() and (
...
function CharacterSelectionStateCharacter:post_update(dt, t)
	...
	if self._despawning_player_unit_career_change and
	not Unit.alive(self._despawning_player_unit_career_change) and
	profile_synchronizer:all_ingame_synced_for_peer(peer_id, local_player_id) then
...
function CharacterSelectionStateCharacter:draw(dt, t)
	local ui_top_renderer = self.ui_top_renderer
...
function CharacterSelectionStateCharacter:_update_animations(dt)
...
function CharacterSelectionStateCharacter:_spawn_hero_unit(hero_name)
...
function CharacterSelectionStateCharacter:cb_hero_unit_spawned(hero_name)
	local world_previewer = self.world_previewer
	local career_index = self._selected_career_index
...
function CharacterSelectionStateCharacter:_is_button_pressed(widget)
	local content = widget.content
...
function CharacterSelectionStateCharacter:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_enter
end

function CharacterSelectionStateCharacter:_is_button_hover_exit(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_exit
end

function CharacterSelectionStateCharacter:_populate_career_info(profile_index, career_index)
	local ui_scenegraph = self._ui_scenegraph
	local ui_top_renderer = self.ui_top_renderer
	local widgets_by_name = self._widgets_by_name
...
function CharacterSelectionStateCharacter:_setup_additional_career_info(career_settings)
	if career_settings.additional_ui_info_file then
...
function CharacterSelectionStateCharacter:_handle_input(dt, t)
	local input_service = self:input_service()
	local gamepad_active = Managers.input:is_device_active("gamepad")
...
function CharacterSelectionStateCharacter:_set_hero_info(hero_name, career_name, level)
	local widgets_by_name = self._widgets_by_name
...
function CharacterSelectionStateCharacter:_play_sound(event)
	self.parent:play_sound(event)
end

function CharacterSelectionStateCharacter:get_camera_position()
	local world, viewport = self.parent:get_background_world()
	local camera = ScriptViewport.camera(viewport)
	return ScriptCamera.position(camera)
end

function CharacterSelectionStateCharacter:get_camera_rotation()
	local world, viewport = self.parent:get_background_world()
	local camera = ScriptViewport.camera(viewport)
	return ScriptCamera.rotation(camera)
end

function CharacterSelectionStateCharacter:trigger_unit_flow_event(unit, event_name)
	if unit and Unit.alive(unit) then
		Unit.flow_event(unit, event_name)
	end
end

function CharacterSelectionStateCharacter:_start_transition_animation(key, animation_name)
	local params = {
...
function CharacterSelectionStateCharacter:_change_profile(profile_index, career_index)
...
function CharacterSelectionStateCharacter:_change_career(profile_index, career_index)
...
function CharacterSelectionStateCharacter:pending_profile_request()
	return self._pending_profile_request
end

function CharacterSelectionStateCharacter:_save_selected_profile(profile_index)
...
function CharacterSelectionStateCharacter:_update_profile_request()
	if self._pending_profile_request then
		...
		if result == "success" then
			...
			local profile_index = self._requested_profile_index local career_index = self._requested_career_index
...
function CharacterSelectionStateCharacter:_on_option_button_hover(widget, style_id)
...
function CharacterSelectionStateCharacter:_on_option_button_dehover(widget, style_id)
	local ui_animations = self._ui_animations
	local animation_name = "option_button_" .. style_id
...
function CharacterSelectionStateCharacter:play_sound(event)

	return end

function CharacterSelectionStateCharacter:_animate_element_by_time(target, target_index, from, to, time)
	local new_animation = UIAnimation.init(UIAnimation.function_by_time, target, target_index,
	from, to, time, math.ease_out_quad)
	return new_animation
end

function CharacterSelectionStateCharacter:_animate_element_by_catmullrom(target, target_index, target_value, p0, p1, p2, p3, time)
	local new_animation = UIAnimation.init(UIAnimation.catmullrom, target, target_index, target_value,
	p0, p1, p2, p3, time)
	return new_animation
end

function CharacterSelectionStateCharacter:input_service()
