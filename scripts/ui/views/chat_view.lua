...
local WELCOME_MESSAGE = { "--------------------------------------------------------------------------", "    WELCOME TO VERMINTIDE 2 GLOBAL CHAT                    ", "                                                           ", "    type '/'  or click the '?' to get a list of commands   ", "                                                           ", "    Current channel: %s                                    ", "--------------------------------------------------------------------------", " ", " " }
...
local COMMANDS = { { command = "/join", description_text = "<channel_name> - Join a Channel", parameter = "#",
		color = Colors.get_table("red") }, { command = "/leave", description_text = "<channel_name> - Leave a Channel",
		color = Colors.get_table("red") }, { command = "/msg", description_text = "<user_name> <message> - Send Message to Another User",
		color = Colors.get_table("red") }, { command = "/reply", description_text = "<message> - Replies to the Person You Last Spoke To",
		color = Colors.get_table("red") }, { command = "/invite", description_text = "<description> - Send an Invite to Your Game",
		color = Colors.get_table("red") }, { command = "/clear", description_text = "- Clears the chat",
		color = Colors.get_table("red") } }




function ChatView:init(ingame_ui_context)
	...
	local block_reasons = { keybind = true, channels_list = true, debug_screen = true, free_flight = true }
	...
	local block_reasons = { keybind = true, debug_screen = true, free_flight = true }
...
function ChatView:_strip_identifier_from_user_name(user_name)
	return string.sub(user_name, 1, -11)
end

function ChatView:cb_private_message(key, message_type, username, message, parameter)
...
function ChatView:_find_end_index(match_str, start_index)
...
function ChatView:cb_channel_message(key, message_type, username, message, parameter)
	local chat_output_widget = self._widgets.chat_output_widget
	local chat_output_content = chat_output_widget.content
...
function ChatView:cb_join_updated(key, message_type, username, message, parameter)
...
function ChatView:cb_meta_updated(key, message_type, username, parameter, user_data)
	self:_update_members()
end

function ChatView:cb_list_updated(key, message_type, username, parameter, user_data)
...
function ChatView:cb_list_end(key, message_type, username, parameter, user_data)
...
function ChatView:_change_channel(channel_name)
	local chat_output_widget = self._widgets.chat_output_widget
	local chat_output_content = chat_output_widget.content
...
local TO_REMOVE = { }
function ChatView:_verify_channel_tabs()
	local channels = Managers.irc:get_channels()
...
function ChatView:_change_to_private(user_name)
	local chat_output_widget = self._widgets.chat_output_widget
	local chat_output_content = chat_output_widget.content
...
function ChatView:_list_private_messages(user_name)
	local chat_output_widget = self._widgets.chat_output_widget
	local chat_output_content = chat_output_widget.content
...
function ChatView:cb_leave_updated(key, message_type, username, message, parameter)
	local my_user_name = Managers.irc:user_name()
	if username == my_user_name then
...
function ChatView:cb_members_updated(key, message_type, username, message, parameter)
	self:_update_members()
end

function ChatView:_create_ui_elements()
...
function ChatView:on_enter()
	self:set_active(true)
end

local DO_RELOAD = false
function ChatView:update(dt, t, is_sub_menu)
...
function ChatView:_update_filter(filter)
...
function ChatView:_play_sound(event)
	WwiseWorld.trigger_event(self._wwise_world, event)
end

function ChatView:_update_recent_channels_input(dt, t)
	...
	if widget_hotspot.is_hover then
		...
		elseif join_button_hotspot.on_pressed then
			if recent_channels_window_content.selected_channel ~= nil then
				local localize = false local localize_parameters = false
...
function ChatView:_create_recent_channels_window()
	local widget_definitions = definitions.widget_definitions
...
function ChatView:cb_populate_recent_channels(channel_list, channel_list_lookup)
	if table.is_empty(self._recent_channels_widgets) then
		return
	end
	...
	for index, channel_name in ipairs(recent_irc_channels) do
		...
		channel_entry_style.icon.texture_size = { scenegraph_entry_size [2] - channels_width_spacing * 2, scenegraph_entry_size [2] - channels_height_spacing * 2 }
...
function ChatView:_destroy_recent_channels_window(go_back_to_main_window)
...
function ChatView:_create_invite_window()
	local widget_definitions = definitions.widget_definitions
...
function ChatView:_destroy_send_invite_window()
...
function ChatView:_update_send_invite_input(dt, t)
	...
	elseif send_invite_window_content.text_field_active then
		...
		elseif input_service:get("chat_backspace_pressed") then
			local keystrokes = { Keyboard.BACKSPACE }
			send_invite_window_content.chat_text_id, send_invite_window_content.caret_index = KeystrokeHelper.parse_strokes(send_invite_window_content.chat_text_id, send_invite_window_content.caret_index, "insert", keystrokes)
		end
	end
end

function ChatView:_create_create_channels_window()
	local widget_definitions = definitions.widget_definitions
	local scenegraph_definition = definitions.scenegraph_definition
...
function ChatView:_destroy_create_channel_window(go_back_to_main_window)
...
function ChatView:_update_create_channel_input(dt, t)
...
function ChatView:_create_channels_list()
	local widget_definitions = definitions.widget_definitions
	local scenegraph_definition = definitions.scenegraph_definition
	...
	channels_list_settings.channels_entry_size = { entry_width, entry_height }
	channel_entry_icon_style.texture_size = { entry_height - channels_width_spacing * 2, entry_height - channels_height_spacing * 2 }
...
function ChatView:_destroy_channels_list()
...
function ChatView:cb_populate_channels_list(search_text, channel_list, channel_list_lookup)
...
function ChatView:_handle_and_draw_channels_list(ui_renderer, ui_scenegraph, dt, t)
	local channels_list_settings = definitions.channels_list_settings
...
function ChatView:_update_channels_list_input(dt, t)
	if table.is_empty(self._channels_list_widgets) then
		return
	end
...
function ChatView:_update_input(dt, t)
...
function ChatView:_handle_command_list()
	local frame_widget = self._widgets.frame_widget
	local frame_widget_content = frame_widget.content
...
function ChatView:_create_channel_list()
	local channels = Managers.irc:get_channels()
	local widget_definitions = definitions.widget_definitions
...
function ChatView:_destroy_emoji_list()
	table.clear(self._emoji_widgets)
	self._base_offset = 0
end

function ChatView:_create_emoji_list()
	local widget_definitions = definitions.widget_definitions
	...
	scenegraph_entry.size = { emoji_list_settings.emoji_size [1] * emoji_list_settings.emojis_per_row + ( emoji_list_settings.emojis_per_row - 1 ) * emoji_list_settings.emoji_width_spacing + emoji_list_settings.emoji_offset [1] * 2 + 20, height + emoji_list_settings.emoji_offset [2] * 2 }
	emoji_frame_widget_style.mask_rect.offset [2] = emoji_list_settings.emoji_size [2] + emoji_list_settings.emoji_height_spacing * 3
	emoji_frame_widget_style.mask_rect.size = { scenegraph_entry.size [1], scenegraph_entry.size [2] - (emoji_list_settings.emoji_size [2] + emoji_list_settings.emoji_height_spacing * 3) }
...
function ChatView:_handle_and_draw_emoji_list_input(dt)
	local ui_renderer = self._ui_renderer
	local ui_scenegraph = self._ui_scenegraph
	local input_service = self._input_manager:get_service("chat_view")
	local render_settings = self._render_settings
...
function ChatView:_create_private_list()
	local chat_output_widget = self._widgets.chat_output_widget
...
function ChatView:_create_filtered_user_list(filter)
...
function ChatView:_destroy_channel_list()
	table.clear(self._channel_list_widgets)
end

function ChatView:_destroy_private_list()
	table.clear(self._private_list_widgets)
end

function ChatView:_destroy_command_list()
	table.clear(self._commands_list_widgets)
	local commands_button = self._widgets.commands_widget
	local commands_button_content = commands_button.content
	local commands_button_hotspot = commands_button_content.button_hotspot
...
function ChatView:_destroy_filtered_user_names_list()
	table.clear(self._filtered_user_names_list_widgets)
end

function ChatView:_handle_channel_list_input()
...
function ChatView:_handle_private_list_input()
...
function ChatView:_handle_command_list_input()
...
function ChatView:_handle_filtered_user_names_list_input()
...
function ChatView:_handle_user_list_scroll_input(input_service)
...
function ChatView:_handle_link_presses()
...
function ChatView:_send_message(content)
...
function ChatView:_show_welcome_message()
	local chat_output_widget = self._widgets.chat_output_widget
	local chat_output_content = chat_output_widget.content
	local channel_messages_table = chat_output_content.channel_messages_table
...
function ChatView:_send_channel_message(content, emojis)
	local localize = false local localize_parameters = false
...
function ChatView:_send_private_message(content, emojis)
...
function ChatView:set_active(active)
	self._active = active
...
function ChatView:_update_animations(dt, t)
...
function ChatView:_update_channel_tabs(dt, t)
...
function ChatView:_populate_user_widgets(users, read_index)
	local user_entry_widgets = self._user_entry_widgets
...
function ChatView:_update_members(read_index)
	read_index =
	read_index or self._user_list_read_index or 1
