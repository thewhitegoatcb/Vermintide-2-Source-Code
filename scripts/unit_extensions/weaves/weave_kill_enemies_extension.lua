...
local BASE_SCORE_MULTIPLIER = { hardest = 0.7, hard = 0.9, harder = 0.8, cataclysm_2 = 0.5, cataclysm = 0.6, cataclysm_3 = 0.4, normal = 1 }
...
function WeaveKillEnemiesExtension:init(extension_init_context, unit, extension_init_data)
	...
	if self._kills_required > 0 then
		...
		if type(self._breed_score_multipliers) == "number" then
			self._breed_score_multipliers = { default = self._breed_score_multipliers }
		else
	...
	if self._breeds_allowed and #self._breeds_allowed == 0 then self._breeds_allowed = nil end
	if self._races_allowed and #self._races_allowed == 0 then self._races_allowed = nil end
	if self._hit_zones_allowed and #self._hit_zones_allowed == 0 then self._hit_zones_allowed = nil end
	if self._attacks_allowed and #self._attacks_allowed == 0 then self._attacks_allowed = nil end
	if self._damage_types_allowed and #self._damage_types_allowed == 0 then self._damage_types_allowed = nil end
end

function WeaveKillEnemiesExtension:get_objective_settings()
	return WeaveObjectiveSettings [WeaveKillEnemiesExtension.NAME]
end

function WeaveKillEnemiesExtension:score()
	return self._score
end

function WeaveKillEnemiesExtension:activate(game_object_id, objective_data)
...
function WeaveKillEnemiesExtension:complete()
...
function WeaveKillEnemiesExtension:deactivate()
	if self._is_server then
...
function WeaveKillEnemiesExtension:objective_name()
	return self._objective_name
end

function WeaveKillEnemiesExtension:cb_game_session_disconnect()

	return end

function WeaveKillEnemiesExtension:update(dt, t)
...
function WeaveKillEnemiesExtension:_server_update(dt, t)
	return end

function WeaveKillEnemiesExtension:_client_update(dt, t)
	return end

function WeaveKillEnemiesExtension:is_done()
...
function WeaveKillEnemiesExtension:get_percentage_done()
	if self._method == "score" then
		return 0
	end
...
function WeaveKillEnemiesExtension:get_score()
	return self._score
end

function WeaveKillEnemiesExtension:on_ai_killed(killed_unit, killer_unit, death_data, killing_blow)
	...
	if
	breeds_allowed and table.contains(breeds_allowed, breed_name) then
		enemy_found = true
	end
