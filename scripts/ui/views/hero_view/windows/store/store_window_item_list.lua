...
function StoreWindowItemList:on_enter(params, offset)
...
function StoreWindowItemList:_start_transition_animation(animation_name)
	local params = {
...
function StoreWindowItemList:_create_ui_elements(params, offset)
...
function StoreWindowItemList:on_exit(params, force_unload)
...
function StoreWindowItemList:update(dt, t)
...
function StoreWindowItemList:post_update(dt, t)
	if self._initialized then
		self:_handle_input(dt, t)
	end
end

function StoreWindowItemList:_sync_products_version()
...
function StoreWindowItemList:_update_animations(dt)
...
function StoreWindowItemList:_draw(dt)
...
function StoreWindowItemList:_play_sound(event)
	self._parent:play_sound(event)
end

function StoreWindowItemList:_handle_gamepad_activity()
...
function StoreWindowItemList:_list_index_pressed()
	local list_widgets = self._list_widgets
	if list_widgets then
...
function StoreWindowItemList:_animate_list_entries(dt)
	local parent = self._parent
...
function StoreWindowItemList:_get_items_by_filter(item_filter)
...
function StoreWindowItemList:_get_all_items()
	local backend_store = Managers.backend:get_interface("peddler")
...
function StoreWindowItemList:_update_item_list()
	if self._initialized then
	...
	elseif product_type == "dlc" then
		for index, dlc_name in ipairs(page_content) do
			...
			if dlc_settings then
				layout [#layout + 1] = { dlc_settings = dlc_settings, type = product_type, product_id = dlc_settings.dlc_name }
			end
		end
	elseif product_type == "bundle_items" then
		...
		for _, item_key in pairs(bundle_contains) do
			...
			layout [#layout + 1] = { type = "item", item = { dlc_name = current_page.dlc_name, data = item }, product_id = item.key, settings = { hide_price = true, hide_new = true } }
		end
	end
...
local RARITY_SORTING = { common = 2, promo = 7, magic = 5, plentiful = 1, exotic = 4, rare = 3, unique = 6 }
...
function StoreWindowItemList:_sort_peddler_items_by_type(items)
	local TEMP_DATA = { }
...
function StoreWindowItemList:_sort_peddler_items_by_price(items)
	local function sort_func(a, b)
...
function StoreWindowItemList:_get_list_index_by_product_id(product_id)
...
function StoreWindowItemList:_on_list_index_selected(index, scrollbar_animation_percentage)
	local layout = self._layout
...
function StoreWindowItemList:_create_product_widgets(layout)
	local widgets = { }
...
function StoreWindowItemList:_destroy_product_widgets(force_unload)
	local parent = self._parent
...
function StoreWindowItemList:_align_item_widgets()
...
function StoreWindowItemList:_handle_gamepad_grid_selection(input_service)
...
function StoreWindowItemList:_find_closest_neighbour(column_index_list, current_index)
	local list_widgets = self._list_widgets
...
function StoreWindowItemList:_initialize_scrollbar()
...
function StoreWindowItemList:_update_scroll_position()
	local scrollbar_logic = self._scrollbar_logic
...
function StoreWindowItemList:_update_visible_list_entries()
	local scrollbar_logic = self._scrollbar_logic
...
function StoreWindowItemList:_scroll_to_list_index(index)
	local scrollbar_logic = self._scrollbar_logic
	local enabled = scrollbar_logic:enabled()
...
function StoreWindowItemList:_get_scrollbar_percentage_by_index(index)
	local scrollbar_logic = self._scrollbar_logic
	local enabled = scrollbar_logic:enabled()
