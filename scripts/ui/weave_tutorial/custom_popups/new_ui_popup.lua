...
function NewUIPopup:init(ui_context, parent)
...
function NewUIPopup:destroy()
	self:_destroy_video()
end

function NewUIPopup:_create_ui_elements()
	self:_destroy_video()
...
function NewUIPopup:_clear_page_data()
	self:_destroy_video()
	self._current_page_widgets = { }
end

function NewUIPopup:_change_page(index)
...
function NewUIPopup:_create_video(video_data)
...
function NewUIPopup:_destroy_video()
...
function NewUIPopup:update(dt, input_service)
...
function NewUIPopup:_update_animations(dt)
...
function NewUIPopup:_handle_input(dt, input_service)
	if not self._video_exanded then
		...
		if self._page_index < max_page_num then
			...
			if widget.content.visible and
			UIUtils.is_button_pressed(self._widgets_by_name.next_button, nil, confirm_pressed) then
			...
			if widget.content.visible and
			UIUtils.is_button_pressed(self._widgets_by_name.ok_button, nil, confirm_pressed) then
				self._parent:hide()
			end
		end
		...
		if self._page_index > 1 then
			...
			if widget.content.visible and
			UIUtils.is_button_pressed(self._widgets_by_name.prev_button, nil, confirm_pressed) then
...
function NewUIPopup:_handle_keyboard_input(dt, input_service)
	local button_widgets = { }
	local widgets_by_name = self._widgets_by_name
...
function NewUIPopup:_draw(dt, input_service)
	local ui_top_renderer = self._ui_top_renderer
...
function NewUIPopup:_draw_video(renderer)
...
function NewUIPopup:_handle_expand_video(input_service)
	if not self._video_exanded then
...
function NewUIPopup:start_transition_animation(key, animation_name)
	local params = {
...
function NewUIPopup:_play_sound(event_name)
