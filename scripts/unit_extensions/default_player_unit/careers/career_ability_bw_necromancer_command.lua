local RPCS = { "rpc_necromancer_command_sacrifice", "rpc_necromancer_command_charge" }




local CommandSyncTypes = table.mirror_array({ "pet", "player", "enemy" })

CareerAbilityBWNecromancerCommand = class(CareerAbilityBWNecromancerCommand)

function CareerAbilityBWNecromancerCommand:init(extension_init_context, unit, extension_init_data)
	self._owner_unit = unit
	self._player = extension_init_data.player
	self._is_local = extension_init_data.player.local_player
	self._is_server = extension_init_context.is_server
	self._command_explosion_params = { source_attacker_unit = unit }
...
function CareerAbilityBWNecromancerCommand:extensions_ready(world, unit)
...
function CareerAbilityBWNecromancerCommand:_on_talents_changed(unit, talent_extension)
...
function CareerAbilityBWNecromancerCommand:update(dt, t)
...
function CareerAbilityBWNecromancerCommand:destroy()
...
function CareerAbilityBWNecromancerCommand:_update_outlines(t)
...
function CareerAbilityBWNecromancerCommand:_command_sacrifice_pet(pet_unit)
...
function CareerAbilityBWNecromancerCommand:_trigger_charge_sound()
	self._fp_extension:play_hud_sound_event("Play_career_necro_skeleton_charge")
end

function CareerAbilityBWNecromancerCommand:_start_charge_cooldown()
...
function CareerAbilityBWNecromancerCommand:_add_talent_buffs(unit)
	if self._has_charge then
		self._buff_extension:add_buff("sienna_necromancer_6_3_available_charge")
	end
end

function CareerAbilityBWNecromancerCommand:_cleanup_talent_buffs(unit)
	local buff_extension = self._buff_extension
...
function CareerAbilityBWNecromancerCommand:_add_outline(target_unit, command_sync_type)
	local outline_extension = ScriptUnit.has_extension(target_unit, "outline_system")
	if outline_extension then
		local data = self._outline_data
		if
		data and ALIVE [data.unit] then
			data.extension:remove_outline(data.id)
		end
...
function CareerAbilityBWNecromancerCommand:command_attack_enemy(target_unit, should_charge, t)
...
function CareerAbilityBWNecromancerCommand:rpc_necromancer_command_charge(channel_id, target_unit_id)
...
function CareerAbilityBWNecromancerCommand:command_sacrifice(target_unit)
	if self._is_local then
...
function CareerAbilityBWNecromancerCommand:rpc_necromancer_command_sacrifice(channel_id, target_unit_id)
	local peer_id = CHANNEL_TO_PEER_ID [channel_id]
	if peer_id ~= self._player.peer_id then
		return
	end
...
function CareerAbilityBWNecromancerCommand:_update_vent_command_target(t)
...
function CareerAbilityBWNecromancerCommand:vent_command_target()
	return self._vent_command_target
end

function CareerAbilityBWNecromancerCommand:command_stand_ground(position, fallback_rotation)
...
function CareerAbilityBWNecromancerCommand:_play_command_sound()
