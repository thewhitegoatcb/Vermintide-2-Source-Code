...
function AILineOfSightExtension:init(extension_init_context, unit, extension_init_data)
	self.unit = unit
	self._offsets = { }
end

function AILineOfSightExtension:extensions_ready(world, unit)
	self._physics_world = World.physics_world(world)
end

function AILineOfSightExtension:destroy()
	return end

function AILineOfSightExtension:reset()
	return end
...
function AILineOfSightExtension:has_line_of_sight(unit, blackboard, override_target, override_distance)

	if blackboard.pause_line_of_sight_t then
		local t = Managers.time:time("game")
		if t < blackboard.pause_line_of_sight_t then
			do return false, 1 end
		else
			...
			if dist_sq < max_distance then
				...
				if math.abs(target_offset.x) < BENEATH_ABOVE_EPSILON and math.abs(target_offset.y) < BENEATH_ABOVE_EPSILON then
					...
					if RAY_DISTANCE_EPSILON < dist then
						...
						local hit, _, _, _, hit_actor = PhysicsWorld.raycast(physics_world,
						from,
						dir,
						dist,
						"closest",
