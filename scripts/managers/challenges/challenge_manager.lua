...
function ChallengeManager:init(statistics_db, is_server)
...
function ChallengeManager:destroy()
...
function ChallengeManager:on_round_start(network_event_delegate, event_manager)
...
function ChallengeManager:on_round_end()
...
function ChallengeManager:update(t, dt)
	local all_challenges = self._all_challenges
...
function ChallengeManager:add_challenge(challenge_template, is_repeatable, category, reward, owner_unique_id, custom_amount, auto_resume)
	if self._is_server then
...
function ChallengeManager:remove_challenge(challenge)
	if self._is_server and challenge then
		challenge:cancel()
	end
end

function ChallengeManager:get_challenge_from_unique_id(unique_id)
	local all_challenges = self._all_challenges
	for i = 1, #all_challenges do
...
function ChallengeManager:remove_filtered_challenges(category, owner_unique_id)
...
function ChallengeManager:get_all_challenges()
	return self._all_challenges
end



function ChallengeManager:get_challenges_filtered(results, category, owner_unique_id)
...
function ChallengeManager:get_all_completed_challenges()
	return self._completed_challenges
end

function ChallengeManager:get_completed_challenges_filtered(results, category, owner_unique_id)
	fassert(results, "Missing mandatory table (array) argument 'results'")
...
function ChallengeManager:player_entered_game_session(peer_id, local_player_id, wanted_party_index)
...
function ChallengeManager:player_left_game_session(peer_id, local_player_id)
...
function ChallengeManager:on_bot_added(bot_player)
	self:player_entered_game_session(bot_player:network_id(), bot_player:local_player_id())
end

function ChallengeManager:on_bot_removed(bot_player)
...
function ChallengeManager:reserve_free_unique_id()
...
function ChallengeManager:_cleanup_orphanated_challenge_ids(num_to_cleanup)
...
function ChallengeManager:_cancel_challenge_instant(challenge)
	challenge:cancel()
...
local rpcs = { "rpc_server_add_ingame_challenge", "rpc_server_remove_ingame_challenge", "rpc_server_update_ingame_challenge", "rpc_server_hot_join_sync_ingame_challenge" }
...
function ChallengeManager:register_rpcs(network_event_delegate)
...
function ChallengeManager:unregister_rpcs()
...
function ChallengeManager:hot_join_sync(peer_id)
	local all_challenges = self._all_challenges
	for i = 1, #all_challenges do
		local challenge = all_challenges [i]
		...
		Managers.state.network.network_transmit:send_rpc("rpc_server_hot_join_sync_ingame_challenge", peer_id,
		unique_id, template_id, is_repeatable, category_id, reward_id, owner_peer_id, owner_local_player_id, progress, max_progress, status_id, result_id)
	end
end

function ChallengeManager:rpc_server_add_ingame_challenge(sender, unique_id, challenge_id, is_repeatable, category_id, reward_id, owner_peer_id, owner_local_player_id, custom_amount)
...
function ChallengeManager:rpc_server_remove_ingame_challenge(sender, unique_id)
...
function ChallengeManager:rpc_server_update_ingame_challenge(sender, unique_id, progress, status_id, result_id)
	local challenge = self:get_challenge_from_unique_id(unique_id)
	if challenge then
...
function ChallengeManager:rpc_server_hot_join_sync_ingame_challenge(sender, unique_id, challenge_id, is_repeatable, category_id, reward_id, owner_peer_id, owner_local_player_id, progress, custom_amount, status_id, result_id)
	local challenge_template = NetworkLookup.challenges [challenge_id]
	local category = NetworkLookup.challenge_categories [category_id]
	local reward = NetworkLookup.challenge_rewards [reward_id]
	local owner_unique_id = PlayerUtils.unique_player_id(owner_peer_id, owner_local_player_id)
