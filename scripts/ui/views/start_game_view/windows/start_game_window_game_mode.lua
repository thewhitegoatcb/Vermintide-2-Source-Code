...
function StartGameWindowGameMode:on_enter(params, offset)
...
function StartGameWindowGameMode:create_ui_elements(params, offset)
...
function StartGameWindowGameMode:on_exit(params)
	print("[StartGameWindow] Exit Substate StartGameWindowGameMode")
	self.ui_animator = nil
end

function StartGameWindowGameMode:update(dt, t)
...
function StartGameWindowGameMode:post_update(dt, t)
	return end

function StartGameWindowGameMode:_update_animations(dt)
...
function StartGameWindowGameMode:_is_button_pressed(widget)
...
function StartGameWindowGameMode:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_enter
end

function StartGameWindowGameMode:_is_button_selected(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.is_selected
end

function StartGameWindowGameMode:_handle_input(dt, t)
...
function StartGameWindowGameMode:_update_game_options_hover_effect(dt)
	local game_mode_widgets = self._game_mode_widgets
	for i = 1, #game_mode_widgets do
		local widget = game_mode_widgets [i]
...
function StartGameWindowGameMode:_set_selected_option(selected_layout_name)
	local game_mode_widgets = self._game_mode_widgets
	for i = 1, #game_mode_widgets do
		local widget = game_mode_widgets [i]
		local layout_name = widget.content.layout_name
...
function StartGameWindowGameMode:_update_selected_option()
...
function StartGameWindowGameMode:draw(dt)
...
function StartGameWindowGameMode:_play_sound(event)
