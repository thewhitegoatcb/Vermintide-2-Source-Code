...
function StartGameWindowDeusQuickplay:on_enter(params, offset)
...
function StartGameWindowDeusQuickplay:_start_transition_animation(animation_name)
	local params = {
...
function StartGameWindowDeusQuickplay:_create_ui_elements(params, offset)
...
function StartGameWindowDeusQuickplay:on_exit(params)
...
function StartGameWindowDeusQuickplay:set_focus(focused)
	self._is_focused = focused
end

function StartGameWindowDeusQuickplay:update(dt, t)
...
function StartGameWindowDeusQuickplay:post_update(dt, t)

	return end

function StartGameWindowDeusQuickplay:_handle_gamepad_activity()
...
function StartGameWindowDeusQuickplay:_update_can_play()
...
function StartGameWindowDeusQuickplay:_handle_input(dt, t)
	...
	if not mouse_active then
		...
		if
		gamepad_confirm_pressed and self._dlc_locked then
			Managers.unlock:open_dlc_page(self._dlc_name)
		end



		if self:_can_play() and
		input_service:get(START_GAME_INPUT) then
		...
		for i = 1, #selector_input_definitions do
			...
			widget_name == "play_button" and self:_can_play() then
...
function StartGameWindowDeusQuickplay:_play_sound(event)
	return self._parent:play_sound(event)
end

function StartGameWindowDeusQuickplay:_can_play()
	local selected_difficulty_key = self._current_difficulty
	return selected_difficulty_key ~= nil and not self._dlc_locked
end

function StartGameWindowDeusQuickplay:_set_info_window(difficulty_key)
...
function StartGameWindowDeusQuickplay:_update_difficulty_option(difficulty_key)
...
function StartGameWindowDeusQuickplay:_option_selected(widget_name, button_name, t)
	if widget_name == "difficulty_stepper" then
...
function StartGameWindowDeusQuickplay:_verify_selection_index(input_index, input_change)
...
function StartGameWindowDeusQuickplay:_gamepad_selector_input_func(input_index, input_change)
	local mouse_active = Managers.input:is_device_active("mouse")
...
function StartGameWindowDeusQuickplay:_handle_new_selection(input_index, input_change)
	local num_inputs = #selector_input_definitions
	input_index = math.clamp(input_index, 1, num_inputs)
...
function StartGameWindowDeusQuickplay:_update_animations(dt)
...
function StartGameWindowDeusQuickplay:_update_button_animations(dt)
	local widgets_by_name = self._widgets_by_name
	UIWidgetUtils.animate_default_button(widgets_by_name.upsell_button, dt)
end

function StartGameWindowDeusQuickplay:_draw(dt)
...
function StartGameWindowDeusQuickplay:_update_difficulty_lock()
	local selected_difficulty_key = self._current_difficulty
...
function StartGameWindowDeusQuickplay:_handle_difficulty_info(show_widget)
	if show_widget then
...
function StartGameWindowDeusQuickplay:_calculate_difficulty_info_widget_size(diff_widget)
	local spacing = 20
...
function StartGameWindowDeusQuickplay:_resize_difficulty_info(new_size, new_offset)
	local difficulty_info_widget = self._widgets_by_name.difficulty_info
...
function StartGameWindowDeusQuickplay:_update_gamemode_info_text(input_service)
...
function StartGameWindowDeusQuickplay:_handle_difficulty_stepper_gamepad(widget, input_service, t)
	local anim_params = { }
...
function StartGameWindowDeusQuickplay:_set_expedition_text_highlight_offset(ui_renderer)
