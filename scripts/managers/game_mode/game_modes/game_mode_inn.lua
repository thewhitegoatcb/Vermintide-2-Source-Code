...
function GameModeInn:init(settings, world, ...)
...
function GameModeInn:register_rpcs(network_event_delegate, network_transmit)
...
function GameModeInn:unregister_rpcs()
...
function GameModeInn:update(t, dt)
	self:_update_objectives()
	self._adventure_spawning:update(t, dt)
end

function GameModeInn:server_update(t, dt)
	self._adventure_spawning:server_update(t, dt)
end

function GameModeInn:evaluate_end_conditions(round_started)
...
function GameModeInn:event_local_player_spawned(is_initial_spawn)
...
function GameModeInn:COMPLETE_LEVEL()
	COMPLETE_LEVEL_VAR = true
end

function GameModeInn:FAIL_LEVEL()
	FAIL_LEVEL_VAR = true
end



function GameModeInn:player_entered_game_session(peer_id, local_player_id, wanted_party_index)
...
function GameModeInn:flow_callback_add_spawn_point(unit)
	self._adventure_spawning:add_spawn_point(unit)
end

function GameModeInn:respawn_unit_spawned(unit)
	self._adventure_spawning:respawn_unit_spawned(unit)
end

function GameModeInn:get_respawn_handler()
	return self._adventure_spawning:get_respawn_handler()
end

function GameModeInn:respawn_gate_unit_spawned(unit)
	self._adventure_spawning:respawn_gate_unit_spawned(unit)
end

function GameModeInn:force_respawn(peer_id, local_player_id)
	local status = Managers.party:get_player_status(peer_id, local_player_id)
...
function GameModeInn:_update_objectives()
...
function GameModeInn:_update_objective_marker()
...
local WAYSTONE_TYPE = { "waystone", "waystone", "waystone_weave" }
function GameModeInn:_state_game_is_matchmaking()
...
local MAP_TYPE = { "map", "map", "wom_tutorial_weave_select" }
function GameModeInn:_state_choose_map()
	local current_objective_id = self._current_objective_id
	local unit_id = MAP_TYPE [self._current_waystone_type]

	if self._is_server and
	self._waystone_is_active then
...
function GameModeInn:_should_show_tutorial()
...
function GameModeInn:_state_tutorial(tutorial_step)
	local current_objective_id = self._current_objective_id
	local unit_id = nil
...
function GameModeInn:_activate_objective_marker(unit_id)
...
function GameModeInn:_deactivate_objective_marker(unit_id)
	local unit = self._objective_markers [unit_id]
	if unit then
		local extension = ScriptUnit.extension(unit, "tutorial_system")
...
function GameModeInn:rpc_waystone_active(channel_id, waystone_type, waystone_is_active, current_waystone_type)
	self._waystone_is_active = waystone_is_active
	self._waystone_type = waystone_type
	self._current_waystone_type = current_waystone_type
end

function GameModeInn:hot_join_sync(peer_id)
	self._waystone_is_active = false
	self._waystone_type = 0
...
function GameModeInn:local_player_ready_to_start(player, loading_context)
...
function GameModeInn:local_player_game_starts(player, loading_context)
...
function GameModeInn:_cb_start_menu_closed()
