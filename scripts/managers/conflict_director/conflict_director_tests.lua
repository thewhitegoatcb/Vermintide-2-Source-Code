...
function ConflictDirectorTests:test_main_path_optimization(t, dt)
...
function _env:test_spawn_pos_ahead_half_sphere()
...
function _env:test_player_path_pos_and_50m_ahead()
	...
	ahead_pos =
	ahead_pos or MainPathUtils.point_on_mainpath(main_paths, total_path_dist - 10)
...
function _env:test_angled_trajectory()
...
function ConflictDirectorTests:setup_reachable_coverpoints_test()
	...
	self._reachable_processing = LevelAnalysis.setup_unreachable_processing(self.nav_world, self.main_path_info.main_paths, point_list, { max_concurrent_astars = 5, line_object = QuickDrawerStay })
	print("Points to test:", num_found)

end

function ConflictDirectorTests:process_reachable_coverpoints_test()
...
function _env:setup_reachable_navgraph_test()
	...
	self._reachable_navgraph_processing = LevelAnalysis.setup_unreachable_processing(self.nav_world, self.main_path_info.main_paths, point_list, { max_concurrent_astars = 5, line_object = QuickDrawerStay, fail_color = Color(212, 48, 0) })
	print("Points to test:", #point_list)

end

function _env:process_reachable_navgraph_test()
...
function ConflictDirectorTests:update_jslots(unit)

	local jslot = { num = 0, slots = { }, units = { }, adjusted_dirs = { } }
...
function ConflictDirectorTests.draw_sparse_grid(pos)
	if not pos then
		return
	end
...
function ConflictDirectorTests.sparse_grid_test(pos, unit)
	...
	if SPARSE_GRID [hash] then
		...
		SPARSE_GRID [hash] = { u = unit, x = xp * cell_size_xy, y = yp * cell_size_xy, z = zp * cell_size_z }
...
function ConflictDirectorTests.lean_slot_test()
	...
	if lean_slots then
		...
		if max_slots < target_index then
			...
			lean_slots [target_index] = { x, y, pos.z }
		end
	else
		...
		lean_slots = { { x, y, pos.z }, lean_dogpile = 1, center_angle = center_angle }

		ConflictDirectorTests.lean_slots = lean_slots
	end
end

function ConflictDirectorTests.lean_slot_test_update(side)
	local lean_slots = ConflictDirectorTests.lean_slots
	if lean_slots then
...
function ConflictDirectorTests.drag_test_start(side)
	...
	if ConflictDirectorTests.drag_test then
		...
		ConflictDirectorTests.drag_test = { pole_length = 2, apos = Vector3Box(start_pos), bpos = Vector3Box(target_pos) }
	end
end

function ConflictDirectorTests.drag_test_update(side)
	if ConflictDirectorTests.drag_test then
...
function ConflictDirectorTests.tentacle_test_start(side, t, dt)
...
function ConflictDirectorTests.tentacle_test_update(side, t, dt)
...
local cut_type = "soft"
function ConflictDirectorTests.spawn_mesh_cut(conflict_director)
...
function ConflictDirectorTests.spawn_liquid_blob(conflict_director, t, dt)
...
function ConflictDirectorTests:test_cover_points(side)
	local player_positions = side.PLAYER_POSITIONS
...
function ConflictDirectorTests:update_kill_tester()
	if not script_data.kill_test then
		return
	end

	local breeds = { "skaven_slave", "skaven_slave", "skaven_slave", "skaven_clan_rat", "chaos_marauder", "chaos_fanatic", "chaos_fanatic", "chaos_fanatic" }
	...
	local optional_data = { ignore_breed_limits = true,
	...
	if size >= 3 then
		...
		if
		health_extension and health_extension:is_alive() then
...
function ConflictDirectorTests:nav_group_astar_test(side)
...
function ConflictDirectorTests:update_group_astar_test(side)
...
local function get_lean_target(enemy_list)
	...
	if best_target_unit then
		do return best_target_unit, true end
	else
		return nil, false
	end
end

function _env:slot_testing()
...
function setup_slot_testing()
	local units_1 = { { lean_dogpile = 0, pos = { -1, 3 } }, { lean_dogpile = 0,
			pos = { 0, 3 } }, { lean_dogpile = 0,
			pos = { 1, 3 } }, { lean_dogpile = 0,
			pos = { 2, 3 } }, { lean_dogpile = 0,
			pos = { 3, 3 } } }
	local units_2 = { { lean_dogpile = 0, pos = { 1, 0 } }, { lean_dogpile = 0,
			pos = { 2, 0 } } }
...
function ConflictDirectorTests.start_test(conflict_director, t, dt, test)
	local side = Managers.state.side:get_side_from_name("heroes") or Managers.state.side:get_side(1)
	...
	elseif test == "spawn_encampment" then
		...
		local event_data = { side_id = conflict_director.debug_spawn_side_id, debug_pos = position, debug_dir = { 0, 1 } }
		TerrorEventMixer.start_event("encampment4", event_data)


		local event_data = { side_id = 1, debug_pos = position + Vector3(0, 8, 0), debug_dir = { 0, -1 } }
		TerrorEventMixer.start_event("encampment4", event_data)
		do return end
...
function ConflictDirectorTests.update(conflict_director, t, dt)
