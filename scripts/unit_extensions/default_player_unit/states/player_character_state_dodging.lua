...
function PlayerCharacterStateDodging:on_enter_animation(unit)
...
function PlayerCharacterStateDodging:on_enter(unit, input, dt, context, t, previous_state, params)
...
function PlayerCharacterStateDodging:on_exit(unit, input, dt, context, t, next_state)
	local movement_settings_table = PlayerUnitMovementSettings.get_movement_settings_table(unit)
...
function PlayerCharacterStateDodging:update(unit, input, dt, context, t)
	local csm = self.csm
	local unit = self.unit
	local movement_settings_table = PlayerUnitMovementSettings.get_movement_settings_table(unit)
	local input_extension = self.input_extension
	...
	if ( input_extension:get("jump") or input_extension:get("jump_only") ) and
	status_extension:can_override_dodge_with_jump(t) and self.locomotion_extension:jump_allowed() then
		local params = self.temp_params
...
local params = { }
function PlayerCharacterStateDodging:update_dodge(unit, dt, t)
	local movement_settings_table = PlayerUnitMovementSettings.get_movement_settings_table(unit)
...
function PlayerCharacterStateDodging:get_is_dodging()
	return self.dodge_timer or self.dodge_stand_still_timer or self.dodge_return_timer
end

function PlayerCharacterStateDodging:start_dodge(unit, t)
	local movement_settings_table = PlayerUnitMovementSettings.get_movement_settings_table(unit)
	local network_manager = Managers.state.network

	if network_manager:game() and
...
function PlayerCharacterStateDodging:calculate_dodge_total_time(unit)
