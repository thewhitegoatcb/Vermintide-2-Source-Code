...
local extensions = { "AISpawner" }
local script_data = script_data

function SpawnerSystem:init(context, system_name)
...
local spawn_list = { "skaven_slave", "skaven_clan_rat", "skaven_slave", "skaven_clan_rat", "skaven_slave", "skaven_clan_rat", "skaven_slave", "skaven_clan_rat", "skaven_slave", "skaven_clan_rat" }



function SpawnerSystem:update_test_all_spawners(t)
...
function SpawnerSystem:running_spawners()
	return self._active_spawners
end

function SpawnerSystem:enabled_spawners()
	return self._enabled_spawners
end

function SpawnerSystem:hidden_spawners_lookup()
	return self._hidden_spawners
end



function SpawnerSystem:register_enabled_spawner(spawner, terror_event_id, hidden)
...
function SpawnerSystem:hibernate_spawner(spawner, hibernate)
...
function SpawnerSystem:_hibernate_spawner(num_enabled_spawners, enabled_spawners, disabled_spawners, spawner)
...
function SpawnerSystem:_hibernate_hidden_spawner(hidden_spawners, disabled_hidden_spawners, spawner)
	hidden_spawners [spawner] = nil
	disabled_hidden_spawners [spawner] = true
end

function SpawnerSystem:_awaken_spawner(enabled_spawners, disabled_spawners, spawner)
	disabled_spawners [spawner] = nil
	enabled_spawners [#enabled_spawners + 1] = spawner
end

function SpawnerSystem:_awaken_hidden_spawner(hidden_spawners, disabled_hidden_spawners, spawner)
	disabled_hidden_spawners [spawner] = nil
	hidden_spawners [spawner] = true
end

function SpawnerSystem:_add_broadphase(spawner)
...
function SpawnerSystem:_remove_broadphase(spawner)
...
function SpawnerSystem:register_raw_spawner(spawner, terror_event_id)
	if terror_event_id then
...
local copy_list = { }
function SpawnerSystem:spawn_horde(spawner, breed_list, side_id, group_template, optional_data)
...
function SpawnerSystem:set_breed_event_horde_spawn_limit(breed_name, limit)
...
table.sort(exchange_order, function (name1, name2) return Breeds [name1].exchange_order < Breeds [name2].exchange_order end)
table.dump(exchange_order)


function SpawnerSystem:_try_spawn_breed(breed_name, spawn_list_per_breed, spawn_list, breed_limits, active_enemies, side_id, group_template)
...
function SpawnerSystem:_fill_spawners(spawn_list, spawners, limit_spawners, side_id, group_template, use_closest_spawners, source_unit, optional_data)
...
function SpawnerSystem:spawn_horde_from_terror_event_ids(event_ids, variant, limit_spawners, group_template, strictly_not_close_to_players, side_id, use_closest_spawners, source_unit, optional_data)
	...
	local count = 0 local hidden_count = 0
...
function SpawnerSystem:change_spawner_id(unit, spawner_id, new_spawner_id)
...
function SpawnerSystem:get_raw_spawner_unit(terror_id)
...
function SpawnerSystem:deactivate_spawner(spawner)
...
function SpawnerSystem:debug_show_spawners(t, spawners)
	local h = 70
...
function SpawnerSystem:set_spawn_list(list)
	self._spawn_list = list
end

function SpawnerSystem:pop_pawn_list()
...
function SpawnerSystem:update(context, t, dt)
...
function SpawnerSystem:show_hidden_spawners(t)
