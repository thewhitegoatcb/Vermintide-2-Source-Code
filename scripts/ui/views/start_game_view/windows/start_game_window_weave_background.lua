...
function StartGameWindowWeaveBackground:on_enter(params, offset)
...
function StartGameWindowWeaveBackground:_create_ui_elements(params, offset)
...
function StartGameWindowWeaveBackground:on_exit(params)
...
function StartGameWindowWeaveBackground:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function StartGameWindowWeaveBackground:post_update(dt, t)

	return end

function StartGameWindowWeaveBackground:_update_animations(dt)
...
function StartGameWindowWeaveBackground:_set_background_wheel_visibility(visible)
...
function StartGameWindowWeaveBackground:_update_background_animations(dt)
	local widgets_by_name = self._widgets_by_name
	for i = 1, 2 do
		local wheel_ring_1 = widgets_by_name ["wheel_ring_" .. i .. "_1"]
		local wheel_ring_2 = widgets_by_name ["wheel_ring_" .. i .. "_2"]
...
function StartGameWindowWeaveBackground:_set_background_bloom_intensity(fraction, is_game_matchmaking)
	local min = 1.39
...
function StartGameWindowWeaveBackground:_play_sound(event)
	self._parent:play_sound(event)
end

function StartGameWindowWeaveBackground:_exit(selected_level)
	self.exit = true
	self.exit_level_id = selected_level
end

function StartGameWindowWeaveBackground:draw(dt)
...
function StartGameWindowWeaveBackground:_animate_pulse(target, target_index, from, to, speed)
	local new_animation = UIAnimation.init(UIAnimation.pulse_animation, target, target_index,
	from, to, speed)
	return new_animation
end

function StartGameWindowWeaveBackground:_animate_element_by_time(target, target_index, from, to, time)
	local new_animation = UIAnimation.init(UIAnimation.function_by_time, target, target_index,
	from, to, time, math.ease_out_quad)
	return new_animation
end

function StartGameWindowWeaveBackground:_animate_element_by_catmullrom(target, target_index, target_value, p0, p1, p2, p3, time)
	local new_animation = UIAnimation.init(UIAnimation.catmullrom, target, target_index, target_value,
	p0, p1, p2, p3, time)
	return new_animation
end

function StartGameWindowWeaveBackground:_animate_wheel_position(dt)
