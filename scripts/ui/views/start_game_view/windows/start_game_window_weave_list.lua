...
function StartGameWindowWeaveList:on_enter(params, offset)
...
function StartGameWindowWeaveList:_setup_definitions(params)
...
function StartGameWindowWeaveList:_start_transition_animation(animation_name)
	local params = {
...
function StartGameWindowWeaveList:_create_ui_elements(params, offset)
	DO_RELOAD = false
...
function StartGameWindowWeaveList:_can_play()
	local can_play = self._selected_weave_name ~= nil
	return can_play
end

function StartGameWindowWeaveList:_can_set_next_weave()
...
function StartGameWindowWeaveList:_update_can_play()
...
function StartGameWindowWeaveList:post_update(dt, t)

	return end

function StartGameWindowWeaveList:_update_animations(dt)
...
function StartGameWindowWeaveList:_on_list_index_selected(index)
...
function StartGameWindowWeaveList:_handle_gamepad_input(dt, t)
	...
	elseif input_service:get("move_down") or self._hold_down_timer > 0.5 then
		...
		if is_matchmaking then
			...
			input_service:get("refresh_press") or input_service:get("skip_pressed")) then
				self._parent:play_sound("Play_hud_hover")
				Managers.matchmaking:cancel_matchmaking()
			end

		elseif self:_can_play() and (
		input_service:get("refresh_press") or input_service:get("skip_pressed")) then
...
function StartGameWindowWeaveList:_handle_gamepad_scrollbar(dt, t)
	local num_entries = #self._weave_entry_widgets
...
function StartGameWindowWeaveList:_handle_input(dt, t)
...
function StartGameWindowWeaveList:_is_list_hovered()
	local widgets_by_name = self._widgets_by_name
...
function StartGameWindowWeaveList:_on_weave_widget_pressed(widget, ignore_sound)
	local widgets = self._weave_entry_widgets
...
function StartGameWindowWeaveList:_draw(dt)
...
function StartGameWindowWeaveList:_calculate_first_widget_to_draw()
...
function StartGameWindowWeaveList:_play_sound(event)
	self._parent:play_sound(event)
end

function StartGameWindowWeaveList:_populate_list()
...
function StartGameWindowWeaveList:_setup_scrollbar()
	local num_weave_widgets = #self._weave_entry_widgets
...
function StartGameWindowWeaveList:_next_weave_widget_pressed()
	local next_weave_widget_content = self._next_weave_widget.content
...
function StartGameWindowWeaveList:_next_weave_widget_hover_enter()
	local next_weave_widget_content = self._next_weave_widget.content
	local hotspot = next_weave_widget_content.button_hotspot
...
function StartGameWindowWeaveList:_weave_widget_pressed()
	local widgets = self._weave_entry_widgets
...
function StartGameWindowWeaveList:_weave_widget_hover_enter()
	local widgets = self._weave_entry_widgets
	for idx, widget in pairs(widgets) do
		local content = widget.content
		if content then
...
function StartGameWindowWeaveList:_update_mouse_scroll_input()
	local widgets_by_name = self._widgets_by_name
...
function StartGameWindowWeaveList:_set_scrollbar_value(value)
	local current_scroll_value = self._scroll_value
	if value then
		local widgets_by_name = self._widgets_by_name
		local widget = widgets_by_name.list_scrollbar
...
function StartGameWindowWeaveList:_animate_list_entries(list_hovered, dt)
	local widgets = self._weave_entry_widgets
	for _, widget in pairs(widgets) do
		local content = widget.content
...
function StartGameWindowWeaveList:_animate_list_entry(content, style, dt, optional_hover)
	local mouse_active = Managers.input:is_device_active("mouse")
