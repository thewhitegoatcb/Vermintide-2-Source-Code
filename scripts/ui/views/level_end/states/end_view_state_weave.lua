...
function EndViewStateWeave:on_enter(params)
...
function EndViewStateWeave:exit(direction)
...
function EndViewStateWeave:_wanted_state()
	local new_state = self.parent:wanted_menu_state()
	return new_state
end

function EndViewStateWeave:set_input_manager(input_manager)
	self.input_manager = input_manager
end

function EndViewStateWeave:on_exit(params)
	print("[PlayState] Exit Substate EndViewStateWeave")
	self.ui_animator = nil
end

function EndViewStateWeave:_update_transition_timer(dt)
...
function EndViewStateWeave:update(dt, t)
...
function EndViewStateWeave:_update_ready(dt, t)
...
function EndViewStateWeave:_handle_input(dt, t)
...
function EndViewStateWeave:_handle_gamepad_input(dt, t)
...
function EndViewStateWeave:_show_profile_by_peer_id(peer_id)
...
function EndViewStateWeave:_update_animations(dt)
...
function EndViewStateWeave:draw(input_service, dt)
...
function EndViewStateWeave:_start_transition_animation(key, animation_name)
	local params = {
...
function EndViewStateWeave:_start_score_count_animation(key, animation_name, params)
	local widgets = { }
...
function EndViewStateWeave:_animate_element_by_time(target, target_index, from, to, time)
	local new_animation = UIAnimation.init(UIAnimation.function_by_time, target, target_index,
	from, to, time, math.ease_out_quad)
	return new_animation
end

function EndViewStateWeave:_animate_element_by_catmullrom(target, target_index, target_value, p0, p1, p2, p3, time)
...
function EndViewStateWeave:_setup_team_results(players_session_scores)
...
function EndViewStateWeave:_move_profile_selector(selection_index)
	...
	profile_selector_widget.offset = { x_offset, 0, 0 }
...
function EndViewStateWeave:_fill_portrait(slot, portrait_frame, level_text, portrait_image, player_name)
	local hero_frame_count = self._player_count
	...
	hero_widget.offset = { x_offset, 0, 0 }
...
function EndViewStateWeave:_setup_score_panel()
...
function EndViewStateWeave:_play_sound(event)
