...
function Geheimnisnacht2021AltarExtension:init(extension_init_context, unit, extension_init_data)
	self._unit = unit
...
function Geheimnisnacht2021AltarExtension:destroy()
	self:unregister_events()
end

function Geheimnisnacht2021AltarExtension:assign_cultist_group_id(group_id)
	self._cultist_group_id = group_id
end

function Geheimnisnacht2021AltarExtension:get_current_state()
	return self._state
end

function Geheimnisnacht2021AltarExtension:can_interact()
	return self._state == STATE_INTERACTABLE
end

function Geheimnisnacht2021AltarExtension:on_interact(server_interact, success)
...
function Geheimnisnacht2021AltarExtension:on_interact_start(server_interact)
	if not server_interact then
		Unit.animation_event(self._unit, ANIM_STATE_INTERACT_START)
	end
end

function Geheimnisnacht2021AltarExtension:update(unit, input, dt, context, t)
...
function Geheimnisnacht2021AltarExtension:die()
	if self._is_server then
...
function Geheimnisnacht2021AltarExtension:register_events()
...
function Geheimnisnacht2021AltarExtension:unregister_events()
	local event_manager = Managers.state.event
...
function Geheimnisnacht2021AltarExtension:on_cultists_killed(group_id)
...
function Geheimnisnacht2021AltarExtension:on_cultists_aggroed(group_id)
	if group_id == self._cultist_group_id then
		self:stop_relevant_faction_sound()
...
function Geheimnisnacht2021AltarExtension:stop_relevant_faction_sound()
...
function Geheimnisnacht2021AltarExtension:play_relevant_faction_sound()
	local faction = self._faction
	if faction then
		local unit = self._unit
		if not ALIVE [unit] then
...
function Geheimnisnacht2021AltarExtension:set_ritual_sound(activate)
	local audio_system = self._audio_system
	local unit = self._unit
	if activate then
...
function Geheimnisnacht2021AltarExtension:setup_faction(faction)
	if faction then
		self._faction = faction
	end
end

function Geheimnisnacht2021AltarExtension:change_state(new_state)
...
function Geheimnisnacht2021AltarExtension:_init_state()
...
function Geheimnisnacht2021AltarExtension:_increment_state(new_state)
...
function Geheimnisnacht2021AltarExtension:_mark_aggroed()
	Unit.animation_event(self._unit, ANIM_STATE_AGGROED)
end

function Geheimnisnacht2021AltarExtension:_mark_interactable()

	self:unregister_events()
	Unit.animation_event(self._unit, ANIM_STATE_INTERACTABLE)
end

function Geheimnisnacht2021AltarExtension:_mark_destructible()
...
function Geheimnisnacht2021AltarExtension:nurglings_flee()
...
function Geheimnisnacht2021AltarExtension:spawn_nurglings()
	...
	local group_data = { template = "critter_nurglings",
