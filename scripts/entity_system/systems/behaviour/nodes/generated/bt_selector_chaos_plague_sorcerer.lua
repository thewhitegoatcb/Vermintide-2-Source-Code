...
BTSelector_chaos_plague_sorcerer = class(BTSelector_chaos_plague_sorcerer, BTNode)
BTSelector_chaos_plague_sorcerer.name = "BTSelector_chaos_plague_sorcerer"

function BTSelector_chaos_plague_sorcerer:init(...)
	BTSelector_chaos_plague_sorcerer.super.init(self, ...)
	self._children = { }
end

function BTSelector_chaos_plague_sorcerer:leave(unit, blackboard, t, reason)
	self:set_running_child(unit, blackboard, t, nil, reason)
end

function BTSelector_chaos_plague_sorcerer:run(unit, blackboard, t, dt)
	...
	local node_quick_teleport = children [6]
	...
	local node_spawn_plague_wave = children [7]
	local condition_result = blackboard.ready_to_summon and (blackboard.summoning or Unit.alive(blackboard.target_unit))

	if condition_result then
		self:set_running_child(unit, blackboard, t, node_spawn_plague_wave, "aborted")


		local result, evaluate = node_spawn_plague_wave:run(unit, blackboard, t, dt)
	...
	elseif node_spawn_plague_wave == child_running then
		self:set_running_child(unit, blackboard, t, nil, "failed")
	end
...
function BTSelector_chaos_plague_sorcerer:add_child(node)
