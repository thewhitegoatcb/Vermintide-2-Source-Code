...
local RPCS = { "rpc_enemy_has_target" }



SoundSectorSystem = class(SoundSectorSystem, ExtensionSystemBase)

SoundSectorSystem.system_extensions = { "SoundSectorExtension" }

function SoundSectorSystem:init(context, system_name)
	...
	self._events = { ai_unit_deactivated = "event_ai_unit_deactivated", ai_unit_activated = "event_ai_unit_activated" }
...
function SoundSectorSystem:destroy()
...
function SoundSectorSystem:on_add_extension(world, unit, extension_name)
	local extension = { }
...
function SoundSectorSystem:extensions_ready(world, unit, extension_name)
	if extension_name == "SoundSectorExtension" then
...
function SoundSectorSystem:on_remove_extension(unit, extension_name)
	self._frozen_extensions [unit] = nil
	self:_cleanup_extension(unit, extension_name)
	ScriptUnit.remove_extension(unit, self.NAME)
end

function SoundSectorSystem:on_freeze_extension(unit, extension_name)
	local extension = self._extensions [unit]
...
function SoundSectorSystem:_cleanup_extension(unit, extension_name)
	local extension = self._extensions [unit]
...
function SoundSectorSystem:freeze(unit, extension_name, reason)
...
function SoundSectorSystem:unfreeze(unit)
...
function SoundSectorSystem:update(context, t, dt)
...
function SoundSectorSystem:_update_sectors(camera_position)
...
function SoundSectorSystem:_play_sector_sound_event(sector_index, sound_id, num_of_units_in_sector, units_center, sound_event)
...
function SoundSectorSystem:_stop_sector_sound_event(sector_index, sound_id, sound_event)
	local wwise_world = self.wwise_world
...
function SoundSectorSystem:_calc_unit_sector(camera_position, unit)
...
function SoundSectorSystem:hot_join_sync(peer_id)
...
function SoundSectorSystem:local_player_created(player)
	self.camera_unit = player.camera_follow_unit
end


function SoundSectorSystem:event_ai_unit_activated(unit, breed_name, event_spawned)
	local go_id = self.unit_storage:go_id(unit)
...
function SoundSectorSystem:event_ai_unit_deactivated(unit, breed_name, event_spawned)
	local go_id = self.unit_storage:go_id(unit)
	local sound_sector_extension = self._extensions [unit]
	if sound_sector_extension then
