		...
		if item_1_power_level == item_2_power_level then
			...
			if item_1_rarity_order == item_2_rarity_order then
				...
				if item_type_1 == item_type_2 then
					...
					do return item_name_1 < item_name_2 end
				else
...
function HeroWindowLoadoutInventoryConsole:on_enter(params, offset)
...
function HeroWindowLoadoutInventoryConsole:_create_item_categories(profile_index, career_index)
	...
	for slot_name, slot_types in pairs(item_slot_types_by_slot_name) do
		...
		if ui_slot_index then
			...
			local category = { hero_specific_filter = true,
				name = slot_name,
...
function HeroWindowLoadoutInventoryConsole:_start_transition_animation(animation_name)
	local params = {
...
function HeroWindowLoadoutInventoryConsole:create_ui_elements(params, offset)
...
function HeroWindowLoadoutInventoryConsole:set_focus(focused)
...
function HeroWindowLoadoutInventoryConsole:on_exit(params)
...
function HeroWindowLoadoutInventoryConsole:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function HeroWindowLoadoutInventoryConsole:post_update(dt, t)

	return end

function HeroWindowLoadoutInventoryConsole:_update_input_description()
...
function HeroWindowLoadoutInventoryConsole:_set_item_compare_enable_state(enabled)
	self._widgets_by_name.item_tooltip_compare.content.visible = enabled
	self._draw_item_compare = enabled
end

function HeroWindowLoadoutInventoryConsole:_update_equipped_item_tooltip()
...
function HeroWindowLoadoutInventoryConsole:_update_selected_item_tooltip()
...
function HeroWindowLoadoutInventoryConsole:_update_animations(dt)
...
function HeroWindowLoadoutInventoryConsole:_is_button_pressed(widget)
...
function HeroWindowLoadoutInventoryConsole:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.button_hotspot or content.hotspot
...
function HeroWindowLoadoutInventoryConsole:_handle_gamepad_input(dt, t)
...
function HeroWindowLoadoutInventoryConsole:_handle_input(dt, t)
	local widgets_by_name = self._widgets_by_name
	local parent = self.parent
	local item_grid = self._item_grid
...
function HeroWindowLoadoutInventoryConsole:_update_page_info()
...
function HeroWindowLoadoutInventoryConsole:_update_selected_loadout_slot_index()
...
function HeroWindowLoadoutInventoryConsole:_get_category_slot_index(index)
	local categories = self._categories
	local category = categories [index]
	return category.slot_index
end

function HeroWindowLoadoutInventoryConsole:_get_category_index_by_slot_index(slot_index)
	local categories = self._categories
...
function HeroWindowLoadoutInventoryConsole:_update_loadout_sync()
	local item_grid = self._item_grid
	local parent = self.parent
...
function HeroWindowLoadoutInventoryConsole:_exit(selected_level)
	self.exit = true
	self.exit_level_id = selected_level
end

function HeroWindowLoadoutInventoryConsole:draw(dt)
...
function HeroWindowLoadoutInventoryConsole:_input_service()
	local parent = self.parent
...
function HeroWindowLoadoutInventoryConsole:_play_sound(event)
	self.parent:play_sound(event)
end

function HeroWindowLoadoutInventoryConsole:_change_category_by_index(index)
	local categories = self._categories
	local category = categories [index]
...
function HeroWindowLoadoutInventoryConsole:_setup_input_buttons()
...
function HeroWindowLoadoutInventoryConsole:_set_gamepad_input_buttons_visibility(visible)
	local widgets_by_name = self._widgets_by_name
...
function HeroWindowLoadoutInventoryConsole:_handle_gamepad_activity()
	local mouse_active = Managers.input:is_device_active("mouse")
