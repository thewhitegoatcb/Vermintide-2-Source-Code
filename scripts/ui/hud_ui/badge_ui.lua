...
function BadgeUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function BadgeUI:_create_ui_elements()
...
function BadgeUI:_draw(dt)
	if not self._has_active_badge then return
	end
...
function BadgeUI:_get_badge(badge_id)
...
function BadgeUI:event_add_local_badge(badge_id)
	self:add_badge(
	self._local_unique_id .. "_" .. badge_id,
	self:_get_badge(badge_id))

end

function BadgeUI:event_add_remote_player_badge(remote_player_peer_id, badge_id)
	self:add_badge(
	remote_player_peer_id .. "_" .. badge_id,
	self:_get_badge(badge_id))

end

function BadgeUI:_add_to_queue(hash, badge)
	...
	badges_queue [#badges_queue + 1] = { amount = 1,
		hash = hash,
		badge = badge }
...
function BadgeUI:add_badge(hash, badge, add_to_queue, num_badges)
	if add_to_queue == nil then add_to_queue = true end
	if num_badges == nil then num_badges = 1
	end
	...
	Gui.material(gui, "versus_badge_icon"),
	"diffuse_map",
	"gui/1080p/single_textures/carousel/badge_icons/" .. badge.texture_id .. "_icon")

	Material.set_texture(
	Gui.material(gui, "versus_badge_glow"),
	"diffuse_map",
	"gui/1080p/single_textures/carousel/badge_icons/" .. badge.texture_id .. "_glow")
...
function BadgeUI:_start_animation(animation_name, event_id, widget)
	local params = {
...
function BadgeUI:_remove_active_badge(event_id)
	self._animations [event_id] = nil
	self._has_active_badge = false
end

function BadgeUI:_add_badge_from_queue()
	if self._has_active_badge then return
	end
	local badge_in_queue = table.remove(self._badges_queue, 1)
	if not badge_in_queue then return
	end
