...
function LevelAnalysis:init(nav_world, using_editor, level_name, level_seed)
	...
	local terror_spawners = {
		event_boss = { spawners = { }, level_sections = { } },
		event_patrol = { spawners = { }, level_sections = { } } }
...
function LevelAnalysis:_setup_level_data(level_name, level_seed)
...
function LevelAnalysis:set_random_seed(checkpoint_data, override_seed)
	local seed = nil

	seed =
	checkpoint_data and checkpoint_data.seed or override_seed or
...
function LevelAnalysis:create_checkpoint_data()
	return { seed = self.starting_seed }
end

function LevelAnalysis:_random(...)
	local seed, value = Math.next_random(self.seed, ...)
	self.seed = seed
	return value
end

function LevelAnalysis:_random_float_interval(a, b)
	local seed, value = Math.next_random(self.seed)
	value = a + ( b - a ) * value
	self.seed = seed
	return value
...
function LevelAnalysis:destroy()
	self:reset()
...
function LevelAnalysis:set_enemy_recycler(enemy_recycler)
	self.enemy_recycler = enemy_recycler
end

function LevelAnalysis:get_start_and_finish()
	return self.start, self.finish
end

function LevelAnalysis:get_path_markers()
	return self.path_markers
end

function LevelAnalysis:_add_path_marker_data(pos, marker_type, order, main_path_index, crossroads_string, roaming_set_string, mutators_string, peak_boolean, nav_world, path_markers)
	local kind = nil
...
function LevelAnalysis:_initialize_path_markers_from_editor(path_markers, drawer)
...
function LevelAnalysis:_initialize_path_markers_from_ingame(path_markers, level_name)
...
function LevelAnalysis:generate_main_path(level_name, path_markers, in_editor, drawer, use_level_reference)
	...
	table.sort(path_markers, function (a, b) return a.order < b.order end)
	...
	for i = 1, #path_markers do
		...
		if path_marker.crossroads and path_marker.crossroads ~= "" then
			...
			if not crossroad then
				crossroad = { num_roads = 0, main_path_index = main_path_index, roads = { } }
...
function LevelAnalysis:start_main_path_generation(num_main_paths)
	...
	local layer_costs = { bot_ladders = 20, ledges_with_fence = 20, jumps = 20, ledges = 20, bot_jumps = 20, bot_drops = 20 }
	...
	for i = 1, #path_markers - 1 do
		...
		if path_markers [i].marker_type == "break" or path_markers [i].marker_type == "crossroad_break" then
			...
			self.astar_list [j] = { GwNavAStar.create(), sub_index, path_markers [i].main_path_index, i }
	...
	for i = 1, num_main_paths do
		self.main_paths [i] = { path_length = 0, nodes = { }, astar_paths = { }, path_markers = { } }
	end
	printf("[LevelAnalysis] main path generation - found %d main paths, total of %d sub-paths.", num_main_paths, #self.astar_list)
end

function LevelAnalysis:initialize_cost_table(navtag_layer_cost_table, layer_costs)
...
function LevelAnalysis.boxify_pos_array(array)
	for i = 1, #array do
		array [i] = Vector3Box(array [i])
	end
end

function LevelAnalysis.boxify_table_pos_array(source_array)
	local array = { }
...
function LevelAnalysis:update_main_path_generation()
	...
	while size >= a_star_index do
		...
		if result then
			if GwNavAStar_path_found(a_star) then
				...
				main_path.astar_paths [sub_index] = { dist, cost, node_list, main_path_index, path_marker_index }
...
function LevelAnalysis:calc_dists_to_start()
	local main_paths = self.main_paths
	local dist_from_start = 0
	for i = 1, #main_paths do
		local main_path = main_paths [i]
...
function LevelAnalysis:boss_gizmo_spawned(unit)
	...
	if encampment_id and encampment_id > 0 then
		...
		if EncampmentTemplates [encampment_id] then
			spawner_list [#spawner_list + 1] = { unit, travel_dist, map_section_index, encampment_id }
	...
	for terror_event_kind, data in pairs(terror_spawners) do
		...
		if can_spawn then
			...
			spawners [#spawners + 1] = { unit, travel_dist, map_section_index }
		end
	end
...
function LevelAnalysis:generic_ai_node_spawned(unit)
...
function LevelAnalysis:group_spawners(spawners, level_sections)

	table.sort(spawners, function (a, b) return a [3] < b [3] end)
	...
	for i = 1, #spawners do
		...
		if map_section_index ~= last_map_section_index then
			...
			elseif map_section_index < last_map_section_index then --[[ Nothing --]]
...
function LevelAnalysis:boxify_waypoint_table(waypoint_table)
	local waypoints = { }
...
function LevelAnalysis:print_boss_waypoints()
...
function LevelAnalysis:get_boss_spline_travel_distance(waypoints_table)
...
function LevelAnalysis:get_possible_events()
	local events = { }
	local boss_waypoints = self.boss_waypoints
	...
	for _, section_table in ipairs(boss_waypoints) do
		for _, waypoints_table in ipairs(section_table) do
			...
			events [#events + 1] = { kind = "event_patrol",
	...
	for _, spawner in ipairs(spawners) do
		...
		events [#events + 1] = { kind = "event_boss",
	...
	table.sort(events, function (a, b) return a.travel_dist < b.travel_dist end)

	return events
end

function LevelAnalysis:pick_boss_spline(map_section, padding, last_travel_dist)
	local boss_waypoints = self.boss_waypoints
	...
	local event_data = { spline_type = "patrol", event_kind = "event_spline_patrol", spline_id = waypoints_table.id, spline_way_points = spline_waypoints, one_directional = waypoints_table.one_directional }
	local travel_dist = self:get_boss_spline_travel_distance(waypoints_table)

	return true, fail_message, spline_waypoints [1], event_data, travel_dist
end

function LevelAnalysis:spawn_all_boss_spline_patrols(optional_id)
	local boss_waypoints = self.boss_waypoints
	if not boss_waypoints then
	...
	for i = 1, #boss_waypoints do
		...
		for j = 1, #section_waypoints do
			...
			if not optional_id or waypoints_table.id == optional_id then
				local spline_waypoints = self:boxify_waypoint_table(waypoints_table.waypoints)
				local event_data = { spline_type = "patrol", event_kind = "event_spline_patrol", spline_id = waypoints_table.id, spline_way_points = spline_waypoints }
...
function LevelAnalysis:inject_all_bosses_into_main_path()
	local boss_waypoints = self.boss_waypoints
	if not boss_waypoints then
		return false
	end
...
function LevelAnalysis:_give_events(main_paths, terror_spawners, generated_event_list, terror_event_list, conflict_director_section_list, terror_event_category)
	...
	for i = map_start_section, #conflict_director_section_list do
		...
		if terror_event_kind == "event_boss" or terror_event_kind == "event_patrol" then
			...
			if terror_event_kind == "event_patrol" then
				...
				if forbidden_dist > 0 then
					...
					if new_start_index then
						...
						if pos then
							...
							event_data = { event_kind = terror_event_kind }
						else
				...
				if not boxed_pos then
					...
					event_data = { gizmo_unit = gizmo_unit, event_kind = terror_event_kind }
				end
			end
		elseif terror_event_kind == "encampment" then
			...
			event_data = { encampment_id = encampment_id, unit_compositions_id = unit_compositions_id, gizmo_unit = gizmo_unit, event_kind = terror_event_kind }
		else
		...
		terror_event_list [#terror_event_list + 1] = { boxed_pos, terror_event_name, spawn_distance, debug_color_name }
	end
...
function LevelAnalysis:_override_generated_event_list(generated_event_list, conflict_director_section_list, terror_event_category)
	local override_spawners = self.override_spawners
...
function LevelAnalysis:_generate_event_name_list(conflict_director_section_list, max_events_of_this_kind, terror_event_category)
...
function LevelAnalysis:_hand_placed_terror_creation(main_paths, terror_event_list, terror_event_category)
	...
	local max_events_of_this_kind = level_overrides and level_overrides.max_events_of_this_kind or { event_boss = 2 }
...
function LevelAnalysis:_automatic_terror_creation(main_paths, total_main_path_dist, terror_event_list, terror_event_category, event_every_x_meter, safe_distance)
	terror_event_list [#terror_event_list + 1] = { Vector3Box(0, 0, 0), "safe-dist", safe_distance, "deep_pink" }
	...
	local padding = 100
	local spawn_distance = 0
	local wanted_distances = { } local conflict_director_section_list = { }
	...
	for i = 1, #event_list do
		...
		terror_event_list [#terror_event_list + 1] = { boxed_pos, terror_event_name, wanted_distance, debug_color_name }
	end
...
function LevelAnalysis:debug_spawn_boss_from_closest_spawner_to_player(draw_only)
...
function LevelAnalysis:generate_boss_paths()
	self.boss_event_list = { }
...
function LevelAnalysis:get_main_paths()
	return self.main_paths
end

function LevelAnalysis:get_crossroads()
...
function LevelAnalysis:_make_waypoint_lookup()
...
function LevelAnalysis:_remove_short_routes(routes, patrol_type)
	if not routes then
		return
	end
	...
	for j = #routes, 1, -1 do
		...
		local length = 0
		local p = waypoints [1]
		local p1 = Vector3(p [1], p [2], p [3]) local p2 = nil
...
function LevelAnalysis:store_patrol_waypoints(boss_waypoints, patrol_waypoints, event_waypoints)
...
function LevelAnalysis:draw_patrol_route(route_data, drawer, col)
...
function LevelAnalysis:draw_patrol_routes()
	local drawer = QuickDrawerStay
	local section_colors = { Color(0, 255, 40), Color(0, 75, 255), Color(200, 25, 40), Color(255, 0, 255), Color(0, 0, 255), Color(0, 200, 0), Color(220, 200, 0) }
...
function LevelAnalysis:draw_patrol_start_position(route_data, drawer, col, map_section)
	local h = Vector3(0, 0, 1)
	local waypoints = route_data.waypoints
	local wp = waypoints [1]
	local p1 = Vector3(wp [1], wp [2], wp [3]) + h
...
function LevelAnalysis:draw_patrol_start_positions()
	local drawer = QuickDrawerStay
...
function LevelAnalysis:debug_get_closest_boss_patrol_spawn(player_position)
	local boss_waypoints = self.boss_waypoints
...
function LevelAnalysis:get_waypoint_spline(spline_id)
...
function LevelAnalysis:get_closest_waypoint_spline(pos)
	...
	local min_dist = math.huge local best_id = nil
...
function LevelAnalysis:get_closest_pos_to_waypoint_list(waypoints, pos)
...
function LevelAnalysis:get_closest_roaming_spline(pos, exact)
...
function LevelAnalysis:store_main_paths(main_paths)
	...
	self.main_path_data = { collapsed_path = collapsed_path, collapsed_travel_dists = collapsed_travel_dists, breaks_lookup = breaks_lookup, breaks_order = breaks_order, total_dist = total_dist }
end

function LevelAnalysis:remove_crossroads_extra_path_branches(main_paths, crossroads, total_main_path_length_unmodified, zones, num_main_zones, path_markers)
	...
	if main_path_was_changed then
		...
		do return true, altered_amount_num_main_zones end
	else
...
function LevelAnalysis:remove_terror_spawners_due_to_crossroads(removed_path_distances)
	local to_remove = { }
	local terror_spawners = self.terror_spawners
...
function LevelAnalysis:brute_force_calc_zone_distances(zones, num_main_zones, spawn_pos_lookup)
...
function LevelAnalysis:store_path_markers(path_markers)
	self.path_markers = path_markers
	self.start = path_markers [1].pos
	self.finish = path_markers [#path_markers].pos
end

function LevelAnalysis:main_path(index)
	local main_path = self.main_paths [index]
	return main_path.nodes, main_path.path_length
end


function LevelAnalysis.get_path_point(path_list, path_length, move_percent)
	local travel_dist = 0
...
function LevelAnalysis:update(t)
...
function LevelAnalysis.get_main_and_sub_zone_index_from_pos(nav_world, zones, lookup, pos, zone_index_lookup)
...
function LevelAnalysis:get_zone_from_unique_id(zones, unique_zone_id)
	for i = 1, #zones do
		local zone = zones [i]
...
function LevelAnalysis.get_zone_segment_from_travel_dist(travel_dist, zones, num_main_zones)
...
function LevelAnalysis.setup_unreachable_processing(nav_world, main_paths, point_list, optional)
	local setup = { investigated_points = 0, num_points_started = 0,
...
function LevelAnalysis.process_unreachable(work_data)
	local point_list = work_data.point_list
	local delete_failed_points = work_data.delete_failed_points
	local path_found_func = work_data.path_found_func or function () return end
	local path_not_found_func = work_data.path_not_found_func or function () return end
	local get_pos_func = work_data.get_pos_func or function (point_list, index) return point_list [index]:unbox() end
	local get_pos_func2 = work_data.get_pos_func2 or function () return end
	...
	if max_running_astars > #running_astars - #free_astars then
		...
		while j < total_points and i < max_running_astars do
			...
			elseif total_astars < max_running_astars then
				...
				data = { astar = astar, point_index = j + 1 }
...
function LevelAnalysis.setup_main_path_breaks_check(nav_world, main_paths, traverse_logic, path_markers, drawer)
	local setup_data = { max_running_astars = 50,
		nav_world = nav_world,
	...
	for i = num_main_paths, 1, -1 do
		...
		for j = i - 1, 1, -1 do
			...
			nodes_to_check [#nodes_to_check + 1] = { from_node_box = from_node, to_node_box = to_node, from_main_path_index = i, to_main_path_index = j, is_crossroad = is_crossroad, has_crossroad_break = has_crossroad_break, shares_crossroad_id = shares_crossroad_id, shares_road_id = shares_road_id }
		end
			...
			if crossroads_id then
				for j = i + 1, num_main_paths do
					...
					if not to_main_path_crossroad_id or to_main_path_crossroad_id == crossroads_id then
						nodes_to_check [#nodes_to_check + 1] = { crossroad_break_check = true, from_node_box = last_from_node, to_node_box = to_node, from_main_path_index = i, to_main_path_index = j }
				...
				for j = i + 1, num_main_paths do
					...
					if to_main_path_crossroad_id then
						chosen_crossroad_id =
						chosen_crossroad_id or to_main_path_crossroad_id
...
function LevelAnalysis.process_main_path_breaks_check(work_data)
	...
	while #nodes_to_check > 0 and max_running_astars > #running_astar_list do
		...
		if #free_astar_list > 0 then
			...
			astar_data = { astar = GwNavAStar_create() }
		end
	...
	local i = 1 local num_running_astars = #running_astar_list
		...
		if GwNavAStar_processing_finished(current_astar) then
			...
			if GwNavAStar_path_found(current_astar) then
				...
				elseif not is_crossroad or not has_crossroad_break and (not shares_crossroad_id or shares_road_id) then
					...
					failed_data [to_main_path_index] = { node = to_node_box, is_crossroad = is_crossroad }
			...
			elseif crossroad_break_check then
				local failed_data = failed_main_path_breaks [from_main_path_index] or { }
				failed_data [to_main_path_index] = { node = to_node_box, optional_failed_messages = optional_failed_messages }
				failed_main_path_breaks [from_main_path_index] = failed_data
	...
	if #nodes_to_check == 0 and #running_astar_list == 0 then
		...
		do return true, error_string end
	else
		return false
	end
end
...
function LevelAnalysis:check_splines_integrity()
