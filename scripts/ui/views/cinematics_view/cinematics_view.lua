...
local VIDEO_PACKAGES = { "resource_packages/menu_cinematics_videos" }



function CinematicsView:init(ingame_ui_context)
	self._ui_renderer = ingame_ui_context.ui_renderer
	self._ui_top_renderer = ingame_ui_context.ui_top_renderer
	self._ingame_ui = ingame_ui_context.ingame_ui
	self._input_manager = ingame_ui_context.input_manager
	self._ingame_ui_context = ingame_ui_context
	self._render_settings = { alpha_multiplier = 0, snap_pixel_positions = false }
...
function CinematicsView:_packages_loaded()
...
function CinematicsView:_load_packages()
	local package_manager = Managers.package
	for _, name in ipairs(VIDEO_PACKAGES) do
		if not package_manager:has_loaded(name, "cinematics_view") then
...
function CinematicsView:_unload_packages()
	local package_manager = Managers.package
	for _, name in ipairs(VIDEO_PACKAGES) do
...
function CinematicsView:_reset()
...
function CinematicsView:_create_ui_elements()
...
function CinematicsView:_create_scrollbar()
...
function CinematicsView:_destroy_video_players()
...
function CinematicsView:input_service()
	return self._input_manager:get_service("cinematics_view")
end

function CinematicsView:on_enter()
...
function CinematicsView:_show_loading_icon()
	Managers.transition:show_loading_icon(false)
end

function CinematicsView:_hide_loading_icon()
	Managers.transition:hide_loading_icon()
end

function CinematicsView:_create_video_renderer()
	local materials = { "material", "materials/ui/ui_1080p_menu_atlas_textures", "material", "materials/ui/ui_1080p_menu_single_textures", "material", "materials/ui/ui_1080p_hud_single_textures", "material", "materials/fonts/gw_fonts", "material", "materials/ui/ui_1080p_common" }
...
function CinematicsView:_destroy_video_renderer()
	local world = self._ui_top_renderer.world
	UIRenderer.destroy(self._ui_video_renderer, world)
end

function CinematicsView:initialized()
	return self._initialized
end

function CinematicsView:_init_view()
...
function CinematicsView:_start_animation(animation_name, callback)
	self._render_settings = self._render_settings or { alpha_multiplier = 0, snap_pixel_positions = false }
	local params = {
		render_settings = self._render_settings }


	self._animations [animation_name] = self._ui_animator:start_animation(
	animation_name,
	nil,
	self._ui_scenegraph,
	params,
...
function CinematicsView:_enable_viewport(enable)
	if IS_WINDOWS then
...
function CinematicsView:_create_skip_widget()
	local context = {
...
function CinematicsView:on_exit()
...
function CinematicsView:do_exit(return_to_game)
...
function CinematicsView:update(dt, t)
...
function CinematicsView:current_gamepad_selection()
	return self._current_gamepad_selection_index
end

local EMPTY_TABLE = { }
function CinematicsView:_update_input(dt, t)
...
function CinematicsView:_update_animations(dt, t)
...
function CinematicsView:_update_scrollbar(dt, t, input_service, gamepad_active)
...
function CinematicsView:_update_video()
...
function CinematicsView:deactivate_video()
...
function CinematicsView:_play_sound(sound_event)
	if IS_WINDOWS then
...
function CinematicsView:_start_video_sound(sound_start, sound_stop)
	if IS_WINDOWS then
...
function CinematicsView:_reset_sound()
	local current_video_content = self._current_video_content
...
function CinematicsView:activate_video(video_content, index)
	if self._exiting then
...
function CinematicsView:_setup_subtitles(subtitle_template_settings)
	self._cutscene_overlay_ui = nil
...
function CinematicsView:is_video_active(reference_name)
	local current_video_content = self._current_video_content or EMPTY_TABLE
	local current_reference_name = current_video_content.video_player_reference
...
function CinematicsView:_draw(dt, t)
