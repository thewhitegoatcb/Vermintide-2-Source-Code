...
function NavBoxObstacle.get_navboxstacle(unit)
	return _navboxstacles [unit]
end

function NavBoxObstacle:init(navworld, unit)
	self.unit = unit
	self.navworld = navworld

	local half_extents = Vector3(NavHelpers.unit_script_data(unit, 0.2, "GwNavBoxObstacle", "half_extent", "x"), NavHelpers.unit_script_data(unit, 1, "GwNavBoxObstacle", "half_extent", "y"),
	NavHelpers.unit_script_data(unit, 2, "GwNavBoxObstacle", "half_extent", "z"))
	local local_center = Vector3(NavHelpers.unit_script_data(unit, 0, "GwNavBoxObstacle", "offset", "x"), NavHelpers.unit_script_data(unit, 0, "GwNavBoxObstacle", "offset", "y"),
	NavHelpers.unit_script_data(unit, 0, "GwNavBoxObstacle", "offset", "z"))
...
function NavBoxObstacle:set_does_trigger_tagvolume(does_trigger_tag_volume)
	GwNavBoxObstacle.set_does_trigger_tagvolume(self.nav_boxobstacle, does_trigger_tag_volume)
end

function NavBoxObstacle:set_rotation_mode_around_yaw(rotation_mode_around_yaw_only)
	GwNavBoxObstacle.set_rotation_mode_around_yaw_only(self.nav_boxobstacle, rotation_mode_around_yaw_only)
end

function NavBoxObstacle:set_next_update_config(transform, linear_velocity, angular_velocity)
...
function NavBoxObstacle:update(dt)
...
function NavBoxObstacle:shutdown()
...
function NavBoxObstacle:add_to_world()
	GwNavBoxObstacle.add_to_world(self.nav_boxobstacle)
end

function NavBoxObstacle:remove_from_world()
