...
function WeaveSpawner:start_terror_event_from_template(event_template_name, spawner_id)
...
function WeaveSpawner:_update_terror_event_trickle(t, dt, terror_event_trickle)
...
function WeaveSpawner:_update_main_path_spawning(t, dt, main_path_spawning_settings)
...
function WeaveSpawner:set_seed(seed)
	fassert(seed and type(seed) == "number", "Bad seed input!")
	self.seed = seed
	self.original_seed = seed
end

function WeaveSpawner:_random(...)
...
function WeaveSpawner:get_hidden_spawn_pos_from_position_seeded(epicenter)
