...
function ChatGui:init(ui_context)
...
function ChatGui:set_profile_synchronizer(profile_synchronizer)
	self.profile_synchronizer = profile_synchronizer
end

function ChatGui:set_wwise_world(wwise_world)
	self.wwise_world = wwise_world
end

function ChatGui:set_input_manager(input_manager)
	if input_manager then

		local block_reasons = { keybind = true, irc_chat = true, debug_screen = true, rcon = true, twitch = true, popup = true, free_flight = true }
...
function ChatGui:create_ui_elements()
...
local customizer_data = { registry_key = "chat_gui", drag_scenegraph_id = "root_dragger", root_scenegraph_id = "root", label = "Chat", use_plain_rects = true }
...
function ChatGui:update(dt, menu_active, menu_input_service, no_unblock, chat_enabled)
	if RELOAD_CHAT_GUI then
		self:create_ui_elements()
	end
	...
	if show_new_messages and not menu_active then
		...
		closed = false
		if
		not chat_focused and not self.keep_chat_visible then
...
function ChatGui:_update_chat_messages()
...
function ChatGui:menu_close()
	self:clear_current_transition()
	self.closing = true
	self.transition_timer = 0
end

function ChatGui:set_menu_transition_fraction(fraction)
	local ui_scenegraph = self.ui_scenegraph
	local scenegraph_definition = definitions.scenegraph_definition
...
function ChatGui:update_transition(dt)
...
function ChatGui:clear_current_transition()
	self.opening = nil
	self.closing = nil
	self.transition_timer = nil
end

function ChatGui:block_input()
	self.input_manager:capture_input({ "keyboard", "gamepad", "mouse" }, 1, "chat_input", "ChatGui")
	self:_show_cursor()
	Window.set_ime_enabled(true)
end

function ChatGui:unblock_input()
	Window.set_ime_enabled(false)
	if self.input_manager then
		self.input_manager:release_input({ "keyboard", "gamepad", "mouse" }, 1, "chat_input", "ChatGui")
	end
	self:_hide_cursor()
end

function ChatGui:_show_cursor()
...
function ChatGui:block_chat_input_for_one_frame()
	self._block_keystrokes = true
end

function ChatGui:_update_input(input_service, menu_input_service, dt, no_unblock, chat_enabled)
	local input_manager = self.input_manager
	...
	local block_chat_activation = self.block_chat_activation_hack < 0.2 or
	not Managers.chat:is_chat_enabled() or
	self._block_keystrokes
	...
	if chat_closed then
		...
		if GameSettingsDevelopment.use_global_chat then
			if enlarge_hotspot.on_release then
				self:unblock_input()
				Managers.ui:handle_transition("chat_view_force", { use_fade = true })
...
function ChatGui:_draw_widgets(dt, input_service, chat_enabled)
...
function ChatGui:_update_hud_scale()
...
function ChatGui:_apply_hud_scale()
	self:_update_hud_scale()
...
function ChatGui:_abort_hud_scale()
	local scale_modified = self._scale_modified
	local resolution_modified = self._resolution_modified
	local force_update = scale_modified or resolution_modified
...
function ChatGui:_set_chat_window_alpha(progress)
	local ui_settings = UISettings.chat
	local window_widget = self.chat_window_widget
	local input_widget = self.chat_input_widget
	local output_widget = self.chat_output_widget
...
function ChatGui:_apply_color_values(color, to_color)
	color [2] = to_color [2]
	color [3] = to_color [3]
	color [4] = to_color [4]
end