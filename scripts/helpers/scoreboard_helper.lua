...
ScoreboardHelper.scoreboard_topic_stats = { { name = "kills_elites", display_text = "scoreboard_topic_kills_elites",


		stat_types = { { "kills_per_breed", "skaven_storm_vermin" },
		...
		sort_function = function (a, b) return b.score < a.score end },
	{ name = "kills_specials", display_text = "scoreboard_topic_kills_specials",


		stat_types = { { "kills_per_breed", "skaven_gutter_runner" },
		...
		sort_function = function (a, b) return b.score < a.score end },
	{ name = "kills_total", stat_type = "kills_total", display_text = "scoreboard_topic_kills_total",




		sort_function = function (a, b) return b.score < a.score end },
	{ name = "kills_melee", stat_type = "kills_melee", display_text = "scoreboard_topic_kills_melee",




		sort_function = function (a, b) return b.score < a.score end },
	{ name = "kills_ranged", stat_type = "kills_ranged", display_text = "scoreboard_topic_kills_ranged",




		sort_function = function (a, b) return b.score < a.score end },
	{ name = "damage_taken", stat_type = "damage_taken", display_text = "scoreboard_topic_damage_taken",




		sort_function = function (a, b) return a.score < b.score end },
	{ name = "damage_dealt", stat_type = "damage_dealt", display_text = "scoreboard_topic_damage_dealt",




		sort_function = function (a, b) return b.score < a.score end },
	{ name = "damage_dealt_bosses", display_text = "scoreboard_topic_damage_dealt_bosses",


		stat_types = { { "damage_dealt_per_breed", "skaven_rat_ogre" },
		...
		sort_function = function (a, b) return b.score < a.score end },
	{ name = "headshots", stat_type = "headshots", display_text = "scoreboard_topic_headshots",




		sort_function = function (a, b) return b.score < a.score end },
	{ name = "saves", stat_type = "saves", display_text = "scoreboard_topic_saves",




		sort_function = function (a, b) return b.score < a.score end },
	{ name = "revives", stat_type = "revives", display_text = "scoreboard_topic_revives",
...
ScoreboardHelper.scoreboard_grouped_topic_stats = { { group_name = "offense",


		stats = { "kills_total", "kills_specials", "kills_elites", "kills_ranged", "kills_melee", "damage_dealt", "damage_dealt_bosses", "damage_taken", "headshots", "saves", "revives" } },
...
local function get_score_by_name(statistics_db, stats_id, stat_name, saved_scoreboard_data)
	...
	return { score = score_amount, stat_name = stat_name, display_text = topic.display_text }
end

function ScoreboardHelper.get_weave_stats(statistics_db, profile_synchronizer)
...
function ScoreboardHelper.get_grouped_topic_statistics(statistics_db, profile_synchronizer, saved_scoreboard_data)
	assert(statistics_db, "Missing statistics_database reference.")
	assert(profile_synchronizer, "Missing profile_synchronizer reference.")
...
function ScoreboardHelper.get_current_players()
	...
	if table.size(human_and_bot_players) <= max_members then
		do return human_and_bot_players end
	else
		table.clear(PLAYERS)
...
function ScoreboardHelper.debug_get_grouped_topic_statistics()
	...
	for i = 1, 4 do
		player_list [i] = { career_index = 1, portrait_frame = "default", player_level = 1,
	...
	for stats_id, player_data in pairs(player_list) do
		...
		for _, grouped_stat_data in ipairs(ScoreboardHelper.scoreboard_grouped_topic_stats) do
			...
			for _, stat_name in pairs(stats) do
				local score_data = { score = 10, display_text = "display_text!", stat_name = "stat_name_" .. tostring(_) }
				group_scores [#group_scores + 1] = score_data
			end
		end
