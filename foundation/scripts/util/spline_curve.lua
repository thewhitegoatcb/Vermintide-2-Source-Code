...
function SplineCurve:init(points, class_name, movement_class, name, subdivisions, cached_spline, ...)
	self._t = 0
	self._name = name
...
function SplineCurve:recalc_splines(points)
...
function SplineCurve:_build_splines(splines, points, spline_class)
	...
	while index do
		local spline_points = { spline_class.spline_points(points, index) }

		for index, point in ipairs(spline_points) do
			spline_points [index] = Vector3Box(point)
		end

		splines [spline_index] = { points = spline_points }
...
function SplineCurve:get_point_at_distance(dist)
	...
	for i = 1, #spline_points do
		...
		if dist < travel_dist + segment_length then
			...
			local s = data.points
			local p0 = s [1]:unbox() local p1 = s [2]:unbox() local p2 = s [3]:unbox() local p3 = s [4]:unbox()
...
function SplineMovementMetered:recalc_splines()
...
function SplineMovementHermiteInterpolatedMetered:init(spline_curve, splines, spline_class, subdivisions, cached_spline)
...
function SplineMovementHermiteInterpolatedMetered:_build_subdivisions(subdivisions, splines, spline_class)
	...
	for index, spline in ipairs(splines) do
		...
		for sub_index = 1, subdivisions do
			local sub = { }
			local p0 = points [point_index - 1] local p1 = points [point_index] local p2 = points [point_index + 1] local p3 = points [point_index + 2]
			sub.points = { Vector3Box(p0), Vector3Box(p1), Vector3Box(p2), Vector3Box(p3) }
...
function SplineMovementHermiteInterpolatedMetered:reset_to_end()
	...
	local from_spline_index = 1 local from_subdivision_index = 1 local from_t = 0
	local to_spline_index = self._current_spline_index local to_subdivision_index = self._current_subdivision_index local to_t = self._t
