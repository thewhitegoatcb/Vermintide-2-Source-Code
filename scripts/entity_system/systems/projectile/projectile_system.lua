...
local RPCS = { "rpc_spawn_pickup_projectile", "rpc_spawn_pickup_projectile_limited", "rpc_spawn_explosive_pickup_projectile", "rpc_spawn_explosive_pickup_projectile_limited", "rpc_projectile_stopped", "rpc_drop_projectile", "rpc_generic_impact_projectile_impact", "rpc_generic_impact_projectile_force_impact", "rpc_player_projectile_impact_level", "rpc_player_projectile_impact_dynamic", "rpc_client_spawn_light_weight_projectile", "rpc_client_despawn_light_weight_projectile", "rpc_client_create_aoe", "rpc_spawn_globadier_globe", "rpc_spawn_globadier_globe_fixed_impact", "rpc_clients_continuous_shoot_start", "rpc_clients_continuous_shoot_stop", "rpc_projectile_event" }
...
local extensions = { "GenericImpactProjectileUnitExtension", "PlayerProjectileUnitExtension", "PlayerProjectileHuskExtension" }
...
function ProjectileSystem:init(entity_system_creation_context, system_name)
	...
	function self.projectile_owner_destroy_callback(destroyed_projectile_owner_unit)
	...
	self._light_weight = {
		husk_list = { },
		own_data = { is_owner = true, current_index = 0,
...
function ProjectileSystem:on_add_extension(world, unit, extension_name, ...)
...
function ProjectileSystem:on_remove_extension(unit, extension_name)
...
function ProjectileSystem:update(context, t)
...
function ProjectileSystem:_get_projectile_units_names(projectile_info, owner_unit)
	...
	if projectile_info.use_weapon_skin then
		...
		if inventory_extension then
			...
			if slot_data then projectile_units_template = slot_data.projectile_units_template or projectile_units_template
			end
		end end
...
function ProjectileSystem:spawn_player_projectile(owner_unit, position, rotation, scale, angle, target_vector, speed, item_name, item_template_name, action_name, sub_action_name, fast_forward_time, is_critical_strike, power_level, gaze_settings, charge_level)
	...
	local extension_init_data = {
		projectile_locomotion_system = {
			...
			data = { owner_unit, position, rotation, scale, angle, target_vector, speed, item_name, item_template_name, action_name, sub_action_name },
...
function ProjectileSystem:spawn_globadier_globe(position, target_vector, angle, speed, initial_radius, radius, duration, owner_unit, damage_source, aoe_dot_damage, aoe_init_damage, aoe_dot_damage_interval, create_nav_tag_volume, instant_explosion, fixed_impact_data)
	if self.is_server then
		...
		if instant_explosion then
			local extension_init_data = {
				area_damage_system = { area_damage_template = "globadier_area_dot_damage", invisible_unit = true, player_screen_effect_name = "fx/screenspace_poison_globe_impact", area_ai_random_death_template = "area_poison_ai_random_death", dot_effect_name = "fx/wpnfx_poison_wind_globe_impact", extra_dot_effect_name = "fx/chr_gutter_death", damage_players = true,
			...
			local extension_init_data = {
				projectile_locomotion_system = { trajectory_template_name = "throw_trajectory",
					angle = angle,
					speed = speed,
					target_vector = target_vector,
					initial_position = position },
				...
				area_damage_system = { area_damage_template = "globadier_area_dot_damage", invisible_unit = false, threat_duration = 0.5, player_screen_effect_name = "fx/screenspace_poison_globe_impact", area_ai_random_death_template = "area_poison_ai_random_death", dot_effect_name = "fx/wpnfx_poison_wind_globe_impact", damage_players = true,
					aoe_dot_damage = aoe_dot_damage,
					aoe_init_damage = aoe_init_damage,
					aoe_dot_damage_interval = aoe_dot_damage_interval,
					radius = radius,
			...
			if fixed_impact_data then
				...
				extension_init_data.projectile_impact_system = { server_side_raycast = true, collision_filter = "filter_enemy_ray_projectile",
...
function ProjectileSystem:rpc_spawn_globadier_globe(channel_id, position, target_vector, angle, speed, initial_radius, radius, duration, owner_unit_id, damage_source_id, aoe_dot_damage, aoe_init_damage, aoe_dot_damage_interval, create_nav_tag_volume, instant_explosion)
...
function ProjectileSystem:rpc_spawn_globadier_globe_fixed_impact(channel_id, position, target_vector, angle, speed, initial_radius, radius, duration, owner_unit_id, damage_source_id, aoe_dot_damage, aoe_init_damage, aoe_dot_damage_interval, create_nav_tag_volume, instant_explosion, impact_hit_unit_id, impact_position, impact_direction, impact_normal, impact_actor_index, time)
	fassert(self.is_server, "Have to be server")
...
function ProjectileSystem:rpc_projectile_stopped(channel_id, go_id)
	local unit = self.unit_storage:unit(go_id)
	local locomotion_extension = ScriptUnit.extension(unit, "projectile_locomotion_system")
	locomotion_extension:stop()
end

function ProjectileSystem:rpc_drop_projectile(channel_id, go_id)
	local unit = self.unit_storage:unit(go_id)
	local locomotion_extension = ScriptUnit.extension(unit, "projectile_locomotion_system")
...
function ProjectileSystem:rpc_projectile_event(channel_id, go_id, event_id)
	local unit = self.unit_storage:unit(go_id)
...
function ProjectileSystem:rpc_spawn_pickup_projectile(channel_id, projectile_unit_name_id, projectile_unit_template_name_id, network_position, network_rotation, network_velocity, network_angular_velocity, pickup_name_id, pickup_spawn_type_id, spawn_limit)
	...
	local extension_init_data = {
		...
		pickup_system = { has_physics = true,
...
function ProjectileSystem:rpc_spawn_pickup_projectile_limited(channel_id, projectile_unit_name_id, projectile_unit_template_name_id, network_position, network_rotation, network_velocity, network_angular_velocity, pickup_name_id, spawner_unit_id, limited_item_id, pickup_spawn_type_id)
	if not Managers.state.network:game() then
		return
	end
...
function ProjectileSystem:rpc_spawn_explosive_pickup_projectile(channel_id, projectile_unit_name_id, projectile_unit_template_name_id, network_position, network_rotation, network_velocity, network_angular_velocity, pickup_name_id, damage, explode_time, fuse_time, attacker_unit_id, item_name_id, pickup_spawn_type_id)
	...
	if explode_time ~= 0 then
		explosion_data = { explode_time = explode_time, fuse_time = fuse_time, attacker_unit_id = attacker_unit_id }
	end
...
function ProjectileSystem:rpc_spawn_explosive_pickup_projectile_limited(channel_id, projectile_unit_name_id, projectile_unit_template_name_id, network_position, network_rotation, network_velocity, network_angular_velocity, pickup_name_id, spawner_unit_id, limited_item_id, damage, explode_time, fuse_time, attacker_unit_id, item_name_id, pickup_spawn_type_id)
...
function ProjectileSystem:spawn_true_flight_projectile(owner_unit, target_unit, true_flight_template_name, position, rotation, angle, target_vector, speed, item_name, item_template_name, action_name, sub_action_name, scale, is_critical_strike, power_level)
	local action = Weapons [item_template_name].actions [action_name] [sub_action_name]
	local projectile_info = action.projectile_info
	local gravity_settings = projectile_info.gravity_settings
	local trajectory_template_name = projectile_info.trajectory_template_name
...
function ProjectileSystem:spawn_ai_true_flight_projectile(owner_unit, target_unit, true_flight_template_name, position, rotation, angle, target_vector, speed, projectile_info, impact_template_name, scale, is_critical_strike, power_level)
	local gravity_settings = projectile_info.gravity_settings
	local trajectory_template_name = projectile_info.trajectory_template_name
...
function ProjectileSystem:_add_player_projectile_reference(owner_unit, projectile_unit, projectile_info)
...
function ProjectileSystem:_remove_player_projectile_reference(projectile_unit, owner_unit)
...
function ProjectileSystem:get_indexed_projectile_count(owner_unit)
	local indexed_projectiles = self.indexed_player_projectile_units [owner_unit]
...
function ProjectileSystem:get_and_delete_indexed_projectile(owner_unit, index, skip_mark_delete)
	local indexed_projectiles = self.indexed_player_projectile_units [owner_unit]
	if not indexed_projectiles then
		return nil
	end
...
function ProjectileSystem:delete_indexed_projectiles(owner_unit)
	local indexed_projectiles = self.indexed_player_projectile_units [owner_unit]
	if not indexed_projectiles then
		return
	end
...
function ProjectileSystem:rpc_generic_impact_projectile_impact(channel_id, unit_id, hit_go_unit_id, hit_level_unit_id, position, direction, normal, actor_index)
	if self.is_server then
		local peer_id = CHANNEL_TO_PEER_ID [channel_id]
...
function ProjectileSystem:rpc_generic_impact_projectile_force_impact(channel_id, unit_id, position)
	if self.is_server then
		local peer_id = CHANNEL_TO_PEER_ID [channel_id]
...
function ProjectileSystem:rpc_player_projectile_impact_level(channel_id, unit_id, hit_unit_id, position, direction, normal, actor_index)
	if self.is_server then
		local peer_id = CHANNEL_TO_PEER_ID [channel_id]
...
function ProjectileSystem:rpc_player_projectile_impact_dynamic(channel_id, unit_id, hit_unit_id, position, direction, normal, actor_index)
	if self.is_server then
		local peer_id = CHANNEL_TO_PEER_ID [channel_id]
...
function ProjectileSystem:create_light_weight_projectile(damage_source, owner_unit, position, direction, speed, gravity, flat_speed, range, collision_filter, action_data, effect_name, owner_peer_id, is_husk, skip_rpc, husk_projectile, projectile_list, id)
	...
	elseif owning_projectile then
		...
		if not husk_data then
			local max_index = NetworkConstants.light_weight_projectile_index.max
			husk_data = { is_owner = false, current_index = 0,
				projectiles = Script.new_array(max_index),
...
function ProjectileSystem:hot_join_sync(joining_client)
...
function ProjectileSystem:rpc_clients_continuous_shoot_start(channel_id, owner_unit_id, owner_is_level_unit, breed_id, breed_action_id, shoot_duration, owner_peer_id)
	...
	husk_shoot_list [owner_unit_id] = { shots_fired = 0,
		owner_unit = owner_unit,
		...
		owner_peer_id = owner_peer_id,
		breed = breed,
		projectile_list = { is_owner = false, current_index = 0,
			max_index = max_index,
			projectiles = Script.new_array(max_index),
...
function ProjectileSystem:_update_shooting(dt, t, husk_shooting_data)
...
function ProjectileSystem:_fire_from_position_direction(ratling_gun_unit, go_id)
...
function ProjectileSystem:_shoot(owner_peer_id, data, t, dt)
...
function ProjectileSystem:rpc_clients_continuous_shoot_stop(channel_id, owner_unit_id)
	local husk_shoot_list = self._light_weight.husk_shoot_list
...
function ProjectileSystem:rpc_client_spawn_light_weight_projectile(channel_id, damage_source_id, owner_unit_id, position, direction, speed, gravity, flat_speed, effect_id, owner_is_level_unit, owner_peer_id, id)
...
function ProjectileSystem:rpc_client_despawn_light_weight_projectile(channel_id, owner_peer_id, index, id)
...
function ProjectileSystem:rpc_client_create_aoe(channel_id, owner_unit_id, position, damage_source_id, explosion_template_id, radius)
	local world = self.world
	local owner_unit = self.unit_storage:unit(owner_unit_id)
	local damage_source = NetworkLookup.damage_sources [damage_source_id]
...
function ProjectileSystem:_remove_light_weight_projectile(data, index)
	local world = self.world
...
function ProjectileSystem:physics_cb_light_weight_projectile_hit(projectile_data, hits)
	if not hits then
		return
	end
	...
	elseif projectile_data.husk_projectile then
		...
		local hit_data = DamageUtils.process_projectile_hit(
		self.world,
		projectile_data.damage_source,
		projectile_data.owner_unit,
		self.is_server,
		hits,
		action_data,
		projectile_data.direction:unbox(),
		false,
		nil, nil,

		false,

		action_data.power_level)
...
function ProjectileSystem:_redirect_shield_linking(hit_unit, node_index, link_position, depth_position_offset)
...
function ProjectileSystem:_link_projectile(hit_data, projectile_linker_data)
	local hit_unit = hit_data.hit_unit
...
function ProjectileSystem:_update_light_weight_projectiles(dt, t, data)
...
function ProjectileSystem:_print_debug()
...
local REMOVE_LIST = { }
function ProjectileSystem:_server_update_light_weight_projectiles(dt, t, data)
	local projectiles = data.projectiles
...
function ProjectileSystem:_client_update_light_weight_projectiles(dt, t, data)
	local projectiles = data.projectiles
	local index = data.current_index
	local world = self.world
...
function ProjectileSystem:_move_light_weight_projectile(dt, world, projectile, debug)
