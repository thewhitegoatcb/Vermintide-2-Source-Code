...
function DebugScreen.setup(world, settings, callbacks)


	local DebugScreen = DebugScreen
	DebugScreen.world = world

	DebugScreen.gui = World.create_screen_gui(world,
	"material", "materials/fonts/gw_fonts",
	"material", "materials/menu/debug_screen",
	"immediate")
	...
	DebugScreen.shortcuts = Development.setting("debug_shortcuts") or { "numpad 0", "debug_weapons" }
	for i = 1, #DebugScreen.shortcuts do
		local shortcut_input = DebugScreen.shortcuts [i] local shortcut_setting = DebugScreen.shortcuts [i + 1]
		i = i + 1
	...
	DebugScreen.active_shortcut_data = { time = 0 }
...
function DebugScreen.set_blocked(is_blocked)
	DebugScreen.is_blocked = is_blocked
end

DebugScreen.accelerate_factor = DebugScreen.accelerate_factor or 1
function DebugScreen.update(dt, t, input_service, input_manager)
	local DebugScreen = DebugScreen
	...
	if DebugScreen.hot_id == nil then --[[ Nothing --]] end local current_hot_cs = DebugScreen.filtered_console_settings [DebugScreen.hot_id]
	...
	if indicator_offset > 10 then indicator_offset = 0 end
				...
				elseif is_hot_sub then
					if ( input_service:get("right_key") or input_service:has("exclusive_right_key") and input_service:get("exclusive_right_key") ) and not opened_this_frame then
						...
						if current_selected_option_position then
							DebugScreen.text_effects [#DebugScreen.text_effects + 1] = { time = 0,
								start_position = { option_x, pos_y, base_text_layer },
								end_position = { current_selected_option_position.x, current_selected_option_position.y, current_selected_option_position.z },
...
function DebugScreen.reset_settings()
	...
	for i = 1, #DebugScreen.console_settings do
		...
		elseif cs.item_source then --[[ Nothing --]]
		elseif cs.bitmap then --[[ Nothing --]]
		elseif cs.func then --[[ Nothing --]]
		end
	end
	...
	if all_false then
		for i = 1, #DebugScreen.console_settings do
			local cs = DebugScreen.console_settings [i]
			if cs.is_boolean and
			cs.selected_id == 2 then
				all_false = false
...
function DebugScreen.set_texture_quality(value)
...
function DebugScreen.update_search(input_manager, input_service, gui, t, dt)
