...
local RPCS = { "rpc_terror_event_trigger_flow" }
...
function ConflictDirector:init(world, level_key, network_event_delegate, level_seed, is_server, conflict_settings)
	self._world = world
	self._time = 0
	...
	for side_id, _ in pairs(sides) do
		data_by_side [side_id] = { num_spawned_ai_event = 0, num_spawned_ai = 0,
			spawned = { },
			spawned_lookup = { },
	...
	self.main_path_info = { current_path_index = 1, behind_percent = 1, ahead_percent = 0, ahead_travel_dist = 0, main_path_player_info_index = 0, ahead_unit = ahead_unit, behind_unit = behind_unit, player_info_by_travel_distance = { } }
	...
	self.rushing_intervention_data = { ahead_dist = 0, loneliness_value = 0 }

	self._next_speed_running_intervention_time = 5.1
	self.speed_running_intervention_data = { next_travel_dist_check_t = 10, player_travel_distances = { }, total_travel_distances = { } }
...
function ConflictDirector:_setup_sides_to_update_recycler()
...
function ConflictDirector:rpc_terror_event_trigger_flow(channel_id, event_id)
...
function ConflictDirector:alive_specials_count()
	local count = 0
...
function ConflictDirector:alive_specials(optional_table)
...
function ConflictDirector:alive_bosses()
	return self._alive_bosses
end

function ConflictDirector:alive_standards()
	return self._alive_standards
end

function ConflictDirector:reset_queued_spawn_by_breed()
...
function ConflictDirector:destroy()
...
function ConflictDirector:get_player_unit_segment(player_unit)
	local player_info = self.main_path_player_info [player_unit]
	return player_info and player_info.path_index or nil
end

function ConflictDirector:get_player_unit_travel_distance(player_unit)
	local player_info = self.main_path_player_info [player_unit]
	return player_info and player_info.travel_dist or nil
end


function ConflictDirector:stop_rush_check(t)
	self._next_rush_check = math.huge
	table.clear(self._rushing_checks)
end

function ConflictDirector:init_rush_check(t)
	self.players_speeding_dist = 0
	self._next_rush_check = 0
end

function ConflictDirector:are_players_rushing(t)
	...
	if self._next_rush_check < t then
		...
		if unit then
			...
			if not start_data then
				start_data = { start_pos = Vector3Box(path_data.path_pos:unbox()), start_dist = path_data.travel_dist }
...
function ConflictDirector:main_path_completion(unit)
...
function ConflictDirector:sort_player_info_by_travel_distance(main_path_info, main_path_player_info)
	...
	if i > 0 then
		table.sort(player_info_by_travel_distance, function (info_a, info_b) return info_a.travel_dist < info_b.travel_dist end)
...
function ConflictDirector:main_path_player_far_away_check(data, travel_dist, path_pos, pos, t)
...
function ConflictDirector:update_main_path_player_info(side, t)
	...
	if unit then
		...
		if not nav_group then
			local above = 1 local below = 3 local horizontal = 3 local distance_from_obstacle = 0.1
		...
		if not data then
			data = { astar_timer = 0, path_pos = Vector3Box(), unit = unit, total_path_dist = MainPathUtils.total_path_dist(), travel_dist = travel_dist }
		...
		if not far_away then
			data.path_index = path_index data.sub_index = sub_index data.move_percent = move_percent data.travel_dist = travel_dist
			...
			if main_path_info.ahead_percent <= move_percent or main_path_info.ahead_unit == unit then
				...
				if not script_data.override_conflict_settings and zone and
				self.current_conflict_settings ~= zone.conflict_setting.name and not self.level_settings.ignore_zone_conflict_settings then
...
function ConflictDirector:get_main_path_player_data(unit)
	return self.main_path_player_info [unit]
end


function ConflictDirector:get_cluster_and_loneliness(min_dist, position_list, unit_list)
...
function ConflictDirector:update_player_areas(sides)
...
function ConflictDirector:add_horde(amount, event_breed_name)
...
function ConflictDirector:set_master_event_running(event_name)
...
function ConflictDirector:spawned_during_event(side_id)
	return self._conflict_data_by_side [side_id].num_spawned_ai_event
end

function ConflictDirector:enemies_spawned_during_event()
	return self._conflict_data_by_side [self.default_enemy_side_id].num_spawned_ai_event
end

function ConflictDirector:horde_size()
	return self._living_horde, self._horde_ends_at
end

function ConflictDirector:has_horde()
	if self.horde_spawner then
		return self.horde_spawner:running_horde()
	end
end

function ConflictDirector:mini_patrol(t, terror_event_id, side_id, composition_type, group_template, optional_data)
...
function ConflictDirector:mini_patrol_killed(id)
	print("Mini patrol killed!", id)
end

function ConflictDirector:event_horde(t, terror_event_type, side_id, composition_type, limit_spawners, silent, group_template, sound_settings, optional_data)
...
function ConflictDirector:check_updated_settings(new_conflict_setting)
	...
	if should_update_settings then
		...
		conflict_settings =
		conflict_settings or level_conflict_settings or "default"
...
function ConflictDirector:check_update_mutators(new_mutators)
	...
	if to_activate then
		for _, mutator_name in ipairs(to_activate) do
			Managers.state.game_mode._mutator_handler:initialize_mutators({ mutator_name })
...
function ConflictDirector:set_updated_settings(conflict_settings_name)
...
function ConflictDirector:refresh_conflict_director_patches()
...
function ConflictDirector:update_horde_pacing(t, dt)
	...
	if self._next_horde_time < t and not self.delay_horde then
		...
		if not horde_type then
			if horde_settings.mix_paced_hordes then
				if self.horde_spawner.num_paced_hordes % 2 == 0 then
					if math.random() < horde_settings.chance_of_vector then horde_type = "vector" else horde_type = "ambush"
					end
				elseif self.horde_spawner.last_paced_horde_type == "vector" then horde_type = "ambush" else horde_type = "vector"
				end

			elseif math.random() < horde_settings.chance_of_vector then horde_type = "vector" else horde_type = "ambush"
			end
		...
		local extra_data = { multiple_horde_count = self._multiple_horde_count, horde_wave = wave, optional_wave_composition = optional_wave_composition }
...
function ConflictDirector:horde_killed(wave)
	if not wave then
		return
	end
...
function ConflictDirector:going_to_relax_state()
	self._multiple_horde_count = nil
end

function ConflictDirector:get_horde_data()
	return self._next_horde_time, self.horde_spawner.hordes, self._multiple_horde_count
end

function ConflictDirector:get_horde_timer()
	return self._next_horde_time, self.delay_horde
end

function ConflictDirector:start_terror_event(event_name, optional_seed, origin_unit, origin_position)
	local seed = optional_seed or 0
	return TerrorEventMixer.add_to_start_event_list(event_name, seed, origin_unit, origin_position)
end

function ConflictDirector:terror_event_finished(event_name)
	return table.contains(TerrorEventMixer.finished_events, event_name)
end

function ConflictDirector:start_terror_event_from_template(event_template_name, spawner_id, optional_seed)
...
function ConflictDirector:handle_speed_runners(t)
...
function ConflictDirector:handle_alone_player(t, enemy_side)
	...
	if ahead_unit then
		...
		if ahead_unit == loneliest_player_unit or #player_and_bot_units == 2 then
			...
			if rush_intervention.loneliness_value_for_special < loneliness_value then
				...
				if ( not disable_rush_intervention or not disable_rush_intervention.horde ) and
				rush_intervention.loneliness_value_for_ambush_horde < loneliness_value and
				Math.random() < rush_intervention.chance_of_ambush_horde then
					...
					if type(composition) == "table" then
						...
						extra_data = { optional_wave_composition = optional_wave_composition }
					end
...
function ConflictDirector:respawn_level(seed)
...
function ConflictDirector:create_debug_list()
	self._debug_list = { "none",
...
function ConflictDirector:update_mini_patrol(t, dt)
	...
	elseif timer < t then
		local enemy_data = self._conflict_data_by_side [self.default_enemy_side_id]
		local mini_patrol_ok = pacing.total_intensity <= settings.only_spawn_below_intensity and
		settings.only_spawn_above_intensity <= pacing.total_intensity and
		RecycleSettings.max_grunts - #enemy_data.spawned >= 0 and
		not self.delay_mini_patrol
...
function ConflictDirector:reset_data()
	self._cluster_and_loneliness = FrameTable.alloc_table()
end

function ConflictDirector:update(dt, t)
	self._time = t
	...
	if self.in_safe_zone then
		...
		if not conflict_settings.specials.disabled and
		self.specials_pacing and not script_data.ai_specials_spawning_disabled and not Managers.state.game_mode:setting("ai_specials_spawning_disabled") then
		...
		if script_data.recycler_in_freeflight then
			if script_data.recycler_in_cutscene then
				...
				if freeflight_pos then
					recycler_positions = self._recycler_extra_pos and { freeflight_pos, self._recycler_extra_pos:unbox() } or { freeflight_pos }
...
function ConflictDirector:pre_update()
	if self.breed_freezer and not script_data.disable_breed_freeze_opt then
		self.breed_freezer:commit_freezes()
	end
end

function ConflictDirector:post_update()
	if self.breed_freezer and not script_data.disable_breed_freeze_opt then
		self.breed_freezer:commit_freezes()
	end
end

function ConflictDirector:set_recycler_extra_pos(boxed_pos, ends_at)
	self._recycler_extra_pos = boxed_pos
	self._recycler_extra_end_time = ends_at
end

function ConflictDirector:get_free_flight_pos()
	local position = nil
...
function ConflictDirector:spawn_queued_unit(breed, boxed_spawn_pos, boxed_spawn_rot, spawn_category, spawn_animation, spawn_type, optional_data, group_data, unit_data)
	...
	if data then
		...
		data = { breed, boxed_spawn_pos, boxed_spawn_rot, spawn_category, spawn_animation, spawn_type, optional_data, group_data, unit_data, self.spawn_queue_id }
...
function ConflictDirector:remove_queued_unit(queue_id)
	local spawn_queue = self.spawn_queue
...
function ConflictDirector:update_spawn_queue(t)
	local enemy_package_loader = self.enemy_package_loader
...
local dialogue_system_init_data = { faction = "enemy" }
function ConflictDirector:_spawn_unit(breed, spawn_pos, spawn_rot, spawn_category, spawn_animation, spawn_type, optional_data, group_data, spawn_index)
	...
	if breed.has_inventory then
		...
		inventory_init_data = { optional_spawn_data = optional_data, inventory_template = breed_inventory_template, inventory_configuration_name = optional_data.inventory_configuration_name }
	end

	local aim_init_data = nil
	if breed.aim_template ~= nil then
		aim_init_data = { husk = false, template = breed.aim_template }
	end

	local animation_movement_init_data = nil
	if breed.animation_movement_template ~= nil then
		animation_movement_init_data = { husk = false, template = breed.animation_movement_template }
	end
	...
	local extension_init_data = {
		...
		health_system = { health = health, optional_data = optional_data },
		ai_system = { size_variation = 1, size_variation_normalized = 1, breed = breed, nav_world = nav_world, spawn_type = spawn_type, spawn_category = spawn_category, optional_spawn_data = optional_data, side_id = side_id },
		locomotion_system = { nav_world = nav_world, breed = breed },
		ai_navigation_system = { nav_world = nav_world },
		death_system = { is_husk = false, death_reaction_template = breed.death_reaction, disable_second_hit_ragdoll = breed.disable_second_hit_ragdoll },
		hit_reaction_system = { is_husk = false, hit_reaction_template = breed.hit_reaction, hit_effect_template = breed.hit_effect_template },
		...
		proximity_system = { breed = breed },
		buff_system = { breed = breed },
...
function ConflictDirector:_post_spawn_unit(ai_unit, go_id, breed, spawn_pos, spawn_category, spawn_animation, optional_data, spawn_type)
...
function ConflictDirector:set_disabled(state)
	self.disabled = state
end

function ConflictDirector:spawned_units_by_side(side_id)
	return self._conflict_data_by_side [side_id].spawned
end

function ConflictDirector:spawned_enemies()
	return self._conflict_data_by_side [self.default_enemy_side_id].spawned
end

function ConflictDirector:count_units_by_breed(breed_name, side_id)
	side_id = side_id or self.default_enemy_side_id
...
function ConflictDirector:spawned_units_by_breed(breed_name, side_id)
	side_id = side_id or self.default_enemy_side_id
	return self._conflict_data_by_side [side_id].spawned_units_by_breed [breed_name]
end

function ConflictDirector:spawned_units_by_breed_table(side_id)
	side_id = side_id or self.default_enemy_side_id
	return self._conflict_data_by_side [side_id].spawned_units_by_breed
end

function ConflictDirector:count_units_by_breed_during_event(breed_name, side_id)
	side_id = side_id or self.default_enemy_side_id
...
function ConflictDirector:all_spawned_units()
	return self._all_spawned_units, self._num_spawned_ai
end

function ConflictDirector:total_num_ai_spawned()
	return self._num_spawned_ai
end

function ConflictDirector:add_unit_to_bosses(unit)
	self._alive_bosses [#self._alive_bosses + 1] = unit
end

function ConflictDirector:remove_unit_from_bosses(unit)
	remove_element_from_array(self._alive_bosses, unit)
end

function ConflictDirector:add_unit_to_standards(unit)
	self._alive_standards [#self._alive_standards + 1] = unit
end

function ConflictDirector:remove_unit_from_standards(unit)
	remove_element_from_array(self._alive_standards, unit)
end

function ConflictDirector:_remove_unit_from_spawned(unit, blackboard, do_not_trigger_despawn_event)
...
function ConflictDirector:get_threat_value()
	return self.threat_value, self.num_aggroed
end

function ConflictDirector:get_num_aggroed_enemies()
	return self.num_aggroed
end

function ConflictDirector:set_threat_value(breed_name, value)
	threat_values [breed_name] = value
end

function ConflictDirector:calculate_threat_value()
...
function ConflictDirector:check_pacing_event_delay()
...
function ConflictDirector:register_unit_killed(unit, blackboard, killer_unit, killing_blow)
...
function ConflictDirector:register_unit_destroyed(unit, blackboard, reason)
	local breed = blackboard.breed
	local breed_name = breed.name
...
function ConflictDirector:event_delay_pacing(should_delay)
...
function ConflictDirector:destroy_unit(unit, blackboard, reason)
	if ALIVE [unit] then
...
function ConflictDirector:destroy_all_units(except_immune)
...
function ConflictDirector:destroy_close_units(position, except_unit, dist_squared)
	local network_manager = Managers.state.network
	if not network_manager:game() then
		return
	end
...
function ConflictDirector:destroy_specials()
...
function ConflictDirector:debug_spawn_breed(breed_name, delayed, override_pos, extra_data)
	local breed = Breeds [breed_name]
	local item = extra_data
	local text = item [1] local func = item [2]

	if
	func and self [func] then
...
function ConflictDirector:debug_spawn_all_breeds(except_these_breeds, use_except_list_as_spawn_list)
	...
	for i = 1, amount do
		...
		for j = 1, num_attempts do
			...
			if spawn_pos then
				local optional_data = { ignore_breed_limits = true, side_id = self.debug_spawn_side_id }
...
function ConflictDirector:debug_spawn_breed_at_hidden_spawner(breed_name)
	local breed = Breeds [breed_name]
...
function ConflictDirector:rpc_debug_conflict_director_command(channel_id, command_name, breed_name, position, enhancements_string, extra_data)
...
function ConflictDirector:debug_spawn_group(breed_name)
	local breed = Breeds [breed_name]
	print("Spawning group: " .. breed_name)
	self:aim_spawning_group(breed, true)
end

function ConflictDirector:debug_spawn_group_at_main_path(main_path_index, sub_node_index)
	...
	local data = { wanted_size = 5, group_type = "main_path_patrol", breed = breed }
	self:spawn_group(patrol_template, sub_node:unbox(), data)
end

function ConflictDirector:debug_spawn_horde()
	if self.in_safe_zone then
	...
	local side_id = self.default_enemy_side_id
	local extra_data = { horde_wave = "single" }

	if not kind or kind == "random" then
		local random_table = { "vector", "vector_blob", "ambush" }
...
function ConflictDirector:debug_speed_running_intervention(t)
...
local function player_aim_direction()
	...
	if in_free_flight then
		...
		do return position, raycast_dir end
	else
		local player_manager = Managers.player
		local player = player_manager:local_player(1)
...
function ConflictDirector:player_aim_raycast(world, only_breed, filter)
	...
	if result then
		...
		for i = 1, num_hits do
			...
			if not attack_hit_self then
				...
				if only_breed then
					if breed then
						do return breed, hit [1], hit [2], hit [3], hit [4], raycast_dir end
					end
				else
...
function ConflictDirector:debug_spawn_tentacle_blob(breed, only_breed, optional_delayed, optional_override_pos)
	...
	function optional_data.spawned_func(unit, breed, optional_data)
...
function ConflictDirector:aim_spawning_surface(breed, on_navmesh, optional_delayed, optional_override_pos)
...
function ConflictDirector:set_debug_spawn_side(side_id)
	self.debug_spawn_side_id = side_id
end

function ConflictDirector:cycle_debug_spawn_side()
	self.debug_spawn_side_id = self.debug_spawn_side_id % #Managers.state.side:sides() + 1
end

function ConflictDirector:aim_spawning_air(breed, on_navmesh, optional_delayed, optional_override_pos)
	local position, distance, normal, actor = self:player_aim_raycast(self._world, false, "filter_ray_horde_spawn")
...
function ConflictDirector:aim_spawning(breed, on_navmesh, optional_delayed, optional_override_pos, additional_data)
...
function ConflictDirector:debug_spawn_roaming_patrol(position)
	if not position then
		return
	end
	...
	if level_has_roaming_splines then
		...
		local spline_way_points = { Vector3Box(28, -243, 0), Vector3Box(37, -143, 0) }
		local data = { spawn_all_at_same_position = false, group_type = "roaming_patrol", formation = formation, spline_name = spline_name, spline_way_points = spline_way_points }
		self:spawn_spline_group("spline_patrol", position, data)
	end
end

function ConflictDirector:debug_spawn_spline_patrol_closest_spawner()
	local local_player = Managers.player:local_player()
	local position = self:get_free_flight_pos()

	position =
	position or POSITION_LOOKUP [local_player.player_unit]


	local route, waypoints = Managers.state.conflict.level_analysis:debug_get_closest_boss_patrol_spawn(position)
	local event_data = { spline_type = "patrol", event_kind = "event_spline_patrol", spline_id = route.id, spline_way_points = waypoints, one_directional = route.one_directional }

	TerrorEventMixer.start_event("boss_event_chaos_spline_patrol", event_data)
end

function ConflictDirector:aim_patrol_spawning(formation_name)
	self:aim_spawning_group(nil, true, formation_name)
	return true
end

function ConflictDirector:inject_event_patrol()
...
function ConflictDirector:aim_spawning_group(breed, on_navmesh, formation_name)
	local position, distance, normal, actor = self:player_aim_raycast(self._world, false, "filter_ray_horde_spawn")
	...
	if not formation_name then
		local data = { wanted_size = 50, group_type = "grid", breed = breed }
		...
		if spline_type then
			...
			if spline_name then
				waypoints = { Vector3Box(110, 0, 0), Vector3Box(1, -220, 0) }
			else
			...
			local data = { spawn_all_at_same_position = false, group_type = "spline_patrol", formation = formation, spline_name = spline_name, spline_way_points = waypoints, despawn_at_end = despawn_at_end }
			self:spawn_spline_group("spline_patrol", position, data)
		else
			local data = { group_type = "main_path_patrol", breed = breed or Breeds.skaven_storm_vermin }
			self:spawn_group("spline_patrol", position, data)
		end
	end
end
...
function ConflictDirector:spawn_group(patrol_template_name, position, data)
	...
	local optional_data = { ignore_breed_limits = true, side_id = self.debug_spawn_side_id or data.side_id }
...
function ConflictDirector:spawn_spline_group(patrol_template_name, position, data)
	local group_type = data.group_type
...
function ConflictDirector:_check_hi_data_override(breed, breed_count, zone_data)
...
function ConflictDirector:set_breed_override_lookup(breed_override_lookup)
	self._breed_override_lookup = breed_override_lookup
end

function ConflictDirector:_spawn_spline_group(base_group_data, spline)
	local spawn_category = "patrol"
	...
	local zone_data = base_group_data.zone_data local breed_count = nil
	if zone_data then
	...
	for row, columns in ipairs(formation_data) do
		for column, data in ipairs(columns) do repeat
				...
				group_data.group_position = { row = row, column = column }
				...
				self:spawn_queued_unit(spawn_breed, Vector3Box(spawn_pos), QuaternionBox(rotation), spawn_category, nil, nil, optional_data, group_data) until true
		end
	end

end

function ConflictDirector:spawn_one(breed, optional_pos, group_data, optional_data, optional_rotation)
	...
	if spawn_pos then
		...
		rot =
		rot or Quaternion(Vector3.up(), math.degrees_to_radians(Math.random(1, 360)))
...
function ConflictDirector:spawn_at_raw_spawner(breed, spawner_id, optional_data, side_id)
	...
	if spawner_unit then
		...
		if optional_data.spawned_func then
			...
			function optional_data.spawned_func(ai_unit, breed, optional_data)
...
function ConflictDirector:debug_spawn_at_raw(breed, optional_data, side_id)
...
function ConflictDirector:generate_spawns()
	...
	if self.spawn_zone_baker.spawn_zones_available and not self.level_settings.skip_generate_spawns then
		...
		do return pos_list, pack_sizes, pack_rotations, pack_members, zone_data_list end
	else
...
function ConflictDirector:register_main_path_obstacle(position, radius_sq)
	local obstacles = self._main_path_obstacles
	obstacles [#obstacles + 1] = { position = position, radius_sq = radius_sq }
end

function ConflictDirector:ai_ready(level_seed)
	...
	if USE_ENGINE_SLOID_SYSTEM then
		...
		EngineOptimized.init_sloid_system(self._world, self.sloid_broadphase, unit_broadphase,
		self.nav_world,
		traverse_logic)
	...
	if self.spawn_zone_baker:loaded_spawn_zones_available() then --[[ Nothing --]]
	else
	...
	local start = { self.level_analysis:get_start_and_finish() }
...
function ConflictDirector:ai_nav_groups_ready(level_seed)
...
function ConflictDirector:a_star_area_pos_search(p1, p2)
...
function ConflictDirector:freeze_intensity_decay(freeze_time)
	self.frozen_intensity_decay_until = self._time + freeze_time
end

function ConflictDirector:intensity_decay_frozen(freeze_time)
	return self._time < self.frozen_intensity_decay_until
end

function ConflictDirector:boss_event_running(side_id)
	side_id = side_id or self.default_enemy_side_id
...
function ConflictDirector:angry_boss()
	return self._num_angry_bosses > 0
end


function ConflictDirector:add_angry_boss(amount, blackboard)
...
function ConflictDirector:level_flow_event(event_name)
	LevelHelper:flow_event(self._world, event_name)
end

function ConflictDirector:jslots(unit, attacker_unit)
...
function ConflictDirector:update_server_debug(t, dt)
	local side = self._hero_side
	...
	if DebugKeyHandler.key_pressed("o", "draw spawn zones", "ai", "left shift") then
		...
		d = d == nil and "all" or
		d == "all" and "last" or
		d == "last" and "last_naive" or
		d == "last_naive" and nil

		if d == "all" then
			self.spawn_zone_baker:draw_zones(self.nav_world)
		elseif d == "last_naive" then --[[ Nothing --]]
		elseif d ~= "last" then
...
function ConflictDirector:client_ready()
	local level_settings = LevelSettings [self._level_key]
	if not level_settings.load_no_enemies then
...
function ConflictDirector:update_client(dt, t)
...
function ConflictDirector:hot_join_sync(peer_id)
	if self.breed_freezer then
		self.breed_freezer:hot_join_sync(peer_id)
	end
end

function ConflictDirector:set_peaks(peaks)
	self._peak_delayer:set_peaks(peaks)
end

function ConflictDirector:get_peaks()
	return self._peak_delayer:get_peaks()
end

function ConflictDirector:is_near_or_in_a_peak()
	return self._peak_delayer:is_near_or_in_a_peak()
end

function ConflictDirector:spawn_breed_func(breed_name)
...
function ConflictDirector:debug_spawn_variant(breed_name, enhancement_set, num_enhancements)
	local breed = Breeds [breed_name]
	...
	if enhancement_set then
		...
		local additional_data = { enhancements = enhancements }
...
function ConflictDirector:spawn_encampment(encampment_id)
...
function ConflictDirector:pick_enhancement(enhancement_name)
