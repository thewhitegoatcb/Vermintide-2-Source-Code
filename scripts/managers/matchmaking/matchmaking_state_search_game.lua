...
function MatchmakingStateSearchGame:init(params)
...
function MatchmakingStateSearchGame:destroy()
	return end

function MatchmakingStateSearchGame:on_enter(state_context)
...
function MatchmakingStateSearchGame:_start_searching_for_games()
	local search_config = self.search_config

	local current_filters = {
		difficulty = { comparison = "equal",
	...
	if not quick_game then
		...
		if mission_id then
			current_filters.selected_mission_id = { comparison = "equal",
				value = mission_id }
		end
		...
		if act_key then
			current_filters.act_key = { comparison = "equal",
				value = act_key }
		end
	end
	...
	if matchmaking_type then
		...
		if matchmaking_type == "standard" then
			...
			current_filters.matchmaking_type = { comparison = "less_or_equal",
				value = matchmaking_type_index }
		else


			local matchmaking_type_index = NetworkLookup.matchmaking_types [matchmaking_type]
			current_filters.matchmaking_type = { comparison = "equal",
				value = matchmaking_type_index }
		end
	end
	...
	current_filters.eac_authorized = { comparison = "equal",
		value = eac_authorized and "true" or "false" }



	current_filters.mechanism = { comparison = "equal",
	...
	local current_near_filters = { { key = "power_level",
...
function MatchmakingStateSearchGame:on_exit()
	return end

function MatchmakingStateSearchGame:update(dt, t)
...
function MatchmakingStateSearchGame:_search_for_game(dt)
	...
	if any_level then
		preferred_levels = { "any" }
	elseif mission_id then
		preferred_levels = { mission_id }
...
function MatchmakingStateSearchGame:_get_server_lobbies()
...
function MatchmakingStateSearchGame:_get_lobbies()
	return self._lobby_finder:lobbies()
end

function MatchmakingStateSearchGame:_get_servers()
	return self._game_server_finder:servers()
end

function MatchmakingStateSearchGame:_times_party_completed_level(level_key)
...
function MatchmakingStateSearchGame:_compare_first_prio_lobbies(current_lobby, new_lobby)
...
function MatchmakingStateSearchGame:_compare_secondary_prio_lobbies(current_lobby, new_lobby)
	if current_lobby == nil then
		return new_lobby
	end
...
function MatchmakingStateSearchGame:_find_suitable_lobby(lobbies, search_config, wanted_profile_id, preferred_levels)
	...
	for _, level_key in pairs(preferred_levels) do
		...
		if current_first_prio_lobby then break end
		for _, lobby_data in ipairs(lobbies) do
			...
			if lobby_match then
				...
				if
				not discard and using_strict_matchmaking and lobby_data.selected_mission_id ~= selected_mission_id then
					discard = true
					discard_reason = "strict matchmaking"
				end
