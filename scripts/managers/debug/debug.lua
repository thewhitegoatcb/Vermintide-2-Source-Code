...
function Debug.setup(world, world_name)
...
function Debug.create_line_object(name)
...
function Debug.update(t, dt)
	...
	local max = 100
	if num_debug_texts > max then --[[ Nothing --]]
	end
...
function Debug.cond_text(c, ...)
	if c then Debug.text(...) end
end
...
function Debug.colored_text(color, ...)
	if not Debug.active or script_data and script_data.disable_debug_draw then
		return
	end
	local text_table = FrameTable.alloc_table()
...
function Debug.world_text(pos, text, color_name)
	if not Debug.active or script_data and script_data.disable_debug_draw then
		return
	end
	...
	if wt [index] then
		...
		wt [index] = { text, pos [1], pos [3], pos [2], color [1], color [2], color [3] }
	end
end


function Debug.world_sticky_text(pos, text, color_name)
	if not Debug.active or script_data and script_data.disable_debug_draw then
		return
	end
...
function Debug.sticky_text(...)
	...
	table.insert(Debug.sticky_texts, { string.format(...), Managers.time:time("game") + delay })
end

function Debug.drawer(name, disabled)
	name = name or "default"
	local lo = Debug.line_objects [name]
	lo =
	lo or Debug.create_line_object(name)

	return DebugDrawer:new(lo, to_boolean(not disabled))
end

function Debug.draw_text(text, text_pos, opt_font_size, opt_color)
	local gui = Debug.gui
...
function Debug.draw_rect(pos, size, color)
	local gui = Debug.gui
...
function Debug.animation_log_specific_profile(profile, enable)
...
function Debug.spawn_hero(hero_name)
...
function Debug.load_level(level_name, environment_variation_id, debug_environment_level_flow_event)
	Managers.mechanism:debug_load_level(level_name, environment_variation_id)

	if debug_environment_level_flow_event ~= nil then
		StateIngame._level_flow_events = { debug_environment_level_flow_event }
	else
		StateIngame._level_flow_events = nil
	end
end

function Debug.level_loaded(level_name)
...
function Debug.visualize_level_unit(level_unit_id)
...
function Debug.test_spawn_unit(profile_name, career_index)
...
function Debug.test_despawn_unit(profile_name, career_index)
	local world = Managers.state.spawn.world
...
function Debug.hook(obj, method, handler)
...
function Debug.unhook(obj, method, silent)
	local data_for_obj = Debug._hook_data [obj]
	if not data_for_obj then
