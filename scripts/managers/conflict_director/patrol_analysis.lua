...
local NAVTAG_LAYERS_STANDARD = { jumps = 1.5, ledges_with_fence = 1.5, doors = 1.5, teleporters = 5, ledges = 1.5 }
...
local NAVTAG_LAYERS_ROAMING = { jumps = 20, ledges_with_fence = 20, doors = 20, teleporters = 5, ledges = 20 }
...
local function make_colors(fill_table, f, g)
	local s = #fill_table
	fill_table [s + 1] = { 255, 0, f, 0 } fill_table [s + 2] = { 255, 0, 0, f } fill_table [s + 3] = { 255, f, 0, 0 }
	fill_table [s + 4] = { 255, f, f, 0 } fill_table [s + 5] = { 255, 0, f, f } fill_table [s + 6] = { 255, f, 0, f }
	fill_table [s + 7] = { 255, g, g, f } fill_table [s + 8] = { 255, f, g, g } fill_table [s + 9] = { 255, g, f, g }
	fill_table [s + 10] = { 255, f, g, 0 } fill_table [s + 11] = { 255, f, 0, g } fill_table [s + 12] = { 255, 0, g, f }
	fill_table [s + 1] = { 255, g, 0, f }
end
...
function PatrolAnalysis:init(nav_world, using_editor, drawer)
	...
	self.free_navbots_lists = { standard = { }, roaming = { } }
	self.navbot_setups = {
		standard = { nav_cost_map_cost_table = nav_cost_map_cost_table_standard, nav_cost_table = cost_table_standard },
		roaming = { nav_cost_map_cost_table = nav_cost_map_cost_table_roaming, nav_cost_table = cost_table_roaming } }

end

function PatrolAnalysis:destroy()
...
function PatrolAnalysis:setup_nav(navtag_layer, cost_table)
...
function PatrolAnalysis:generate_patrol_splines(level_name, main_paths, drawer)
...
function PatrolAnalysis:_finilize_splines(main_paths, drawer)
...
function PatrolAnalysis:_generate_patrol_spline(level_name, main_paths, gizmo_unit_name, patrol_type, drawer, navbot_kind)
	local patrol_waypoints = self.patrol_waypoints
	...
	for object_id, object in pairs(level_objects) do
		...
		if Unit.alive(unit) and Unit.is_a(unit, gizmo_unit_name) then
			...
			if not spline then
				spline = { wp_index = 2, id = patrol_id, patrol_type = patrol_type, map_section = map_section, navbot_kind = navbot_kind, index = self._spline_counter, one_directional = one_directional }
...
function PatrolAnalysis:create_navbot(nav_world, pos, navbot_kind)
	local speed = 5
	local height = 1.6
	...
	if use_nav_bot_splines then
		...
		GwNavBot.set_channel_computer_configuration(navbot,
		CHANNEL_RADIUS,
		TURN_SAMPLING_ANGLE,
		...
		GwNavBot.set_spline_trajectory_configuration(navbot,
		SCRIPT_DRIVEN,
...
function PatrolAnalysis:inject_spline_path(spline, line_drawer)
	local navbot = spline.navbot
...
function PatrolAnalysis:compute_spline_path(spline_name, spline_way_points, navbot_kind)
	local spline = { unique_navbot = true, wp_index = 2, id = spline_name, navbot_kind = navbot_kind }
	for i, spline_way_point in ipairs(spline_way_points) do
		table.insert(spline, i, { pos = spline_way_point, order = i })
	end

	self.patrol_waypoints [spline_name] = spline
end

function PatrolAnalysis:spline(spline_name)
...
function PatrolAnalysis:draw_raw_spline(spline_name)
...
function PatrolAnalysis:draw_astar_spline(points)
...
function PatrolAnalysis:get_path_point(points, path_length, move_percent)
...
function PatrolAnalysis:get_path_length(points)
	local total_len = 0
	local p1 = points [1]:unbox()
	for j = 2, #points do
		local p2 = points [j]:unbox()
		...
		if path_computing_done then
			...
			if wp_index < #spline and not spline.failed then
				...
				if not spline.spline_points then
					...
					spline.spline_points = { Vector3Box(p1), Vector3Box(p1 + direction) }
