...
function TeamPreviewer:init(ingame_ui_context, world, viewport)
...
function TeamPreviewer:setup_team(hero_data, hero_arrangement)
...
function TeamPreviewer:on_enter()
	return end

function TeamPreviewer:update(dt, t)
	local previewers = self.hero_previewers
...
function TeamPreviewer:post_update(dt, t)
	local previewers = self.hero_previewers
	for i = 1, #previewers do
...
function TeamPreviewer:on_exit()
	self:destroy_previewers()
end

function TeamPreviewer:clear_team()
	local previewers = self.hero_previewers
	for i = 1, #previewers do
...
function TeamPreviewer:destroy_previewers()
	local previewers = self.hero_previewers
	for i = 1, #previewers do
		local hero_previewer = previewers [i]
		if hero_previewer then
...
function TeamPreviewer:_spawn_hero(hero_previewer, hero_data)
...
function TeamPreviewer:cb_hero_unit_spawned_skin_preview(hero_previewer, hero_data)
...
function TeamPreviewer:update_hero_arrangement(hero_arrangement, lookat_target, orientate_towards_camera)
...
function TeamPreviewer:set_camera_orientation(position, lookat_target)
...
function TeamPreviewer:set_camera_fov(fov)
	Camera.set_vertical_fov(self.camera, math.degrees_to_radians(fov))
end

function TeamPreviewer:get_hero_previewer(index)
