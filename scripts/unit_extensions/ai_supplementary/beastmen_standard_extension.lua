...
function BeastmenStandardExtension:init(extension_init_context, unit, extension_init_data)
	...
	if self.is_server then
		...
		local astar_to_players_allowed_layers = { ledges = 1, ledges_with_fence = 1, doors = 1, bot_poison_wind = 1, planks = 1, bot_ratling_gun_fire = 1, fire_grenade = 1 }
...
function BeastmenStandardExtension:destroy()
...
function BeastmenStandardExtension:on_death(killer_unit)
	if self.is_server then
...
function BeastmenStandardExtension:update(unit, input, dt, context, t)
	if self.dead then
		return
	end
	...
	if self.is_server and standard_template.apply_buff_to_ai and self.next_apply_buff_t <= t then
		...
		for ai_unit, buff_id in pairs(ai_units_inside) do
			...
			if not is_inside or not HEALTH_ALIVE [ai_unit] then
				if Unit.alive(ai_unit) and
				buff_system:has_server_controlled_buff(ai_unit, buff_id) then
					buff_system:remove_server_controlled_buff(ai_unit, buff_id)
				end
...
function BeastmenStandardExtension:_update_self_destruction(unit, dt, t)
