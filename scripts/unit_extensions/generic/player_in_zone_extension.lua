...
function PlayerInZoneExtension:init(extension_init_context, unit)
	self._unit = unit
...
function PlayerInZoneExtension:_get_script_data()
...
function PlayerInZoneExtension:_create_game_object()
	local level = LevelHelper:current_level(self._world)
	self._level_unit_index = Level.unit_index(level, self._unit)

	local go_data_table = { progress_time = 0,
...
function PlayerInZoneExtension:cb_game_session_disconnect()
	self._go_id = nil
end


function PlayerInZoneExtension:on_game_object_created(go_id)
	self._go_id = go_id
end


function PlayerInZoneExtension:on_game_object_destroyed()
	self._go_id = nil
end

function PlayerInZoneExtension:extensions_ready()
...
function PlayerInZoneExtension:activated()
	return self._activated
end

function PlayerInZoneExtension:_current_time()
	return self._state_data.end_progression_timer
end

function PlayerInZoneExtension:should_progress_count_down()
	return self._progress_bar_countdown
end

function PlayerInZoneExtension:progress_bar_personal()
	return self._show_progress_bar_personal
end

function PlayerInZoneExtension:progress_bar_global()
	return self._show_progress_bar_global
end

function PlayerInZoneExtension:player_been_in_zone()
	return self._has_been_in_zone
end

function PlayerInZoneExtension:progress()
	return self._state_data.end_progression_timer
end

function PlayerInZoneExtension:update(unit, input, dt, context, t)
...
function PlayerInZoneExtension:_update_client(dt)
...
function PlayerInZoneExtension:_client_unit_inactive(progress_time)
	self._state_data.end_progression_timer = progress_time
...
function PlayerInZoneExtension:_client_progress(counting_up, progress_time, dt)
	if counting_up then
...
function PlayerInZoneExtension:set_active()
...
function PlayerInZoneExtension:set_active_rpc()
	self._activated = true

end

function PlayerInZoneExtension:_update_state(dt, t)
	self [self._state](self, dt, t, self._state_data)
end

function PlayerInZoneExtension:hot_join_sync(peer_id)
	if self._activated then
		local network_manager = Managers.state.network
		local unit_index = LevelHelper:unit_index(self._world, self._unit)
		if unit_index then
...
function PlayerInZoneExtension:destroy()
...
function PlayerInZoneExtension:_update_distances()
...
function PlayerInZoneExtension:_idle(dt, t)
...
function PlayerInZoneExtension:_fulfill_in_zone_check()
...
function PlayerInZoneExtension:_local_player_in_zone()
...
function PlayerInZoneExtension:_progress_frozen()
	...
	local game = self._game
	if
	go_id and not GameSession.game_object_field(game, go_id, "progress_is_frozen") then
		GameSession.set_game_object_field(game, go_id, "progress_is_frozen", true)
	end

end

function PlayerInZoneExtension:_progress_check(dt, t)
	local go_id = self._go_id
	local game = self._game
	local new_state = nil
	...
	if fulfill_in_zone_check then
		...
		if self._state_data.end_progression_timer == 0 then
			if
			go_id and GameSession.game_object_field(game, go_id, "unit_in_progress") then
...
function PlayerInZoneExtension:_progress_finished()
	local network_manager = Managers.state.network
...
function PlayerInZoneExtension:_count_up(dt, num_players)
...
function PlayerInZoneExtension:_count_down(dt)
	local timer = self._timer
	return math.clamp(self:_current_time() - dt / timer, 0, 1)
end

function PlayerInZoneExtension:_reset()
	self._activated = false
...
function PlayerInZoneExtension:end_event()
	self:_reset()
	Unit.flow_event(self._unit, "lua_start_end_event")
end

function PlayerInZoneExtension:_register_count_up(is_counting_up)
	local go_id = self._go_id
	local game = self._game
	GameSession.set_game_object_field(game, go_id, "counting_up", is_counting_up)
end

function PlayerInZoneExtension:_debug_drawer(current_debug_state)
	if current_debug_state == "counting" then
		self._drawer = self._drawer or Managers.state.debug:drawer({ mode = "immediate" })
		self._drawer:reset()
