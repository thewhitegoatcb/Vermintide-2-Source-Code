...
local RPCS = { "rpc_set_equipped_frame", "rpc_server_request_emote", "rpc_server_cancel_emote" }
...
local extension_list = { "PlayerUnitCosmeticExtension" }



function CosmeticSystem:init(entity_system_creation_context, system_name)
...
function CosmeticSystem:destroy()
	self._network_event_delegate:unregister(self)
	self._network_event_delegate = nil
end

function CosmeticSystem:on_add_extension(world, unit, extension_name, extension_init_data)
...
function CosmeticSystem:get_equipped_frame(unit)
...
function CosmeticSystem:set_equipped_frame(unit, frame_name)
...
function CosmeticSystem:rpc_set_equipped_frame(channel_id, unit_id, frame_name_id)
	if self.is_server then
...
function CosmeticSystem:rpc_server_request_emote(channel_id, unit_id, anim_event_id, hide_weapons)
	...
	if unit and ALIVE [unit] then
		...
		self._emote_states [unit_id] = { anim_event = anim_event, hide_weapons = hide_weapons }
...
function CosmeticSystem:rpc_server_cancel_emote(channel_id, unit_id)
...
function CosmeticSystem:hot_join_sync(peer_id)
