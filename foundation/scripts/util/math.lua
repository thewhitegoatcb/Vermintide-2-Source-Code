...
local math = math
local math_sqrt = math.sqrt local math_cos = math.cos local math_sin = math.sin local math_random = math.random
local math_max = math.max local math_abs = math.abs local math_acos = math.acos
local pi = math.pi
...
function math.lerp_clamped(a, b, v)
...
function math.inv_lerp(a, b, v)
	return ( v - a ) / (b - a)
end



function math.remap(imin, imax, omin, omax, v)
...
function math.round_with_precision(value, precision)
...
function math.round_to_closest_multiple(value, multiple)
...
function math.next_random_range(seed, min, max)
...
function math.point_is_inside_2d_box(pos, lower_left_corner, size)
	...
	return lower_left_corner [1] < pos [1] and pos [1] < lower_left_corner [1] + size [1] and
	lower_left_corner [2] < pos [2] and pos [2] < lower_left_corner [2] + size [2]

end

function math.box_overlap_box(a_pos, a_size, b_pos, b_size)

	return b_pos [1] <= a_pos [1] + a_size [1] and a_pos [1] <= b_pos [1] + b_size [1] and
	b_pos [2] <= a_pos [2] + a_size [2] and a_pos [2] <= b_pos [2] + b_size [2]

end

function math.point_is_inside_aabb(pos, aabb_pos, aabb_half_extents)

	return pos [1] >= aabb_pos [1] - aabb_half_extents [1] and
	pos [1] <= aabb_pos [1] + aabb_half_extents [1] and
	pos [2] >= aabb_pos [2] - aabb_half_extents [2] and
	pos [2] <= aabb_pos [2] + aabb_half_extents [2] and
	pos [3] >= aabb_pos [3] - aabb_half_extents [3] and
	pos [3] <= aabb_pos [3] + aabb_half_extents [3]

end

function math.point_is_inside_box(pos, box_pose, box_half_extents)
...
function math.point_is_inside_oobb(pos, oobb_pose, oobb_radius)
	...
	if local_pos.x > -oobb_radius [1] and local_pos.x < oobb_radius [1] and
	local_pos.y > -oobb_radius [2] and local_pos.y < oobb_radius [2] and
	local_pos.z > -oobb_radius [3] and local_pos.z < oobb_radius [3] then
		do return true end
	else
		return false
	end
...
function math.point_is_inside_view(pos, view_position, view_rotation, vertical_fov_rad, horizontal_fov_rad)
...
function math.circular_to_square_coordinates(pos)
	local x = pos.x local y = pos.y
	local w = x * x - y * y
	local k = 4 * math.inverse_sqrt_2
	local u = x * k local v = y * k
	return Vector2(
	0.5 * (math_sqrt(math_max(2 + u + w, 0)) - math_sqrt(math_max(2 - u + w, 0))),
	0.5 * (math_sqrt(math_max(2 + v - w, 0)) - math_sqrt(math_max(2 - v - w, 0))))

end

function math.polar_to_cartesian(radius, theta_degrees)
...
function Geometry.convex_hull(points, hull)
...
function Geometry.convex_hull_tracking(points, hull, hull_indexes)
	local num_points = #points
	if num_points == 0 then
		return hull, 0
	end
...
function Geometry.concave_hull(points, hull)
	local num_points = #points
	if num_points == 0 then
		return hull, 0
	end
...
function Intersect.ray_segment(ray_from, ray_direction, segment_start, segment_end)
	...
	if is_intersection_inside_segment then
		do return distance_along_ray, normalized_distance_along_line end
	else
		return nil, nil
	end
end

function Intersect.ray_circle(ray_from, ray_direction, circle_position, circle_radius)
...
function math.ease_in_quart(t)
	return t * t * t * t
end

function math.ease_out_quart(t)
	return 1 - math.pow(1 - t, 4)
end

function math.ease_in_out_quart(t)
...
function math.ease_in_circ(t)
	return 1 - math.sqrt(1 - t ^ 2)
end

function math.ease_out_circ(t)
	return math.sqrt(1 - ( t - 1 ) ^ 2)
end


function math.ease_in_back(t)
	local c1 = 1.70158
	local c3 = c1 + 1
...
function math.ease_out_back(t)
	c1 = 1.70158
	c3 = c1 + 1

	return 1 + c3 * ( t - 1 ) ^ 3 + c1 * ( t - 1 ) ^ 2
end
function math.ease_in_out_back(t)
	local c1 = 1.70158
	local c2 = c1 * 1.525

	return t < 0.5 and
	( 2 * t ) ^ 2 * (( c2 + 1 ) * 2 * t - c2) / 2 or
	( ( 2 * t - 2 ) ^ 2 * (( c2 + 1 ) * (t * 2 - 2) + c2) + 2 ) / 2
end



function math.easeOutQuint(t)
	return 1 - ( 1 - t ) ^ 5
end
...
function math.rand_utf8_string(string_length, ignore_chars)
...
function math.uuid()
	...
	return string.format("%08x-%04x-4%03x-%x%03x-%012x",
	random(0, 4294967295.0),
	random(0, 65535),
	random(0, 4095),
	random(0, 11),
	random(0, 4095),
	random(0, 281474976710655.0))

end


function math.get_uniformly_random_point_inside_sector(radius1, radius2, angle1, angle2)
...
function math.get_uniformly_random_point_inside_sector_seeded(seed, radius1, radius2, angle1, angle2)
	local radius1_squared = radius1 * radius1
	local radius2_squared = radius2 * radius2
	local rnd1, rnd2 = nil
...
function math.get_random_point_inside_box_seeded(seed, box_pose, bounds)
...
function math.diststance_3d(x1, y1, z1, x2, y2, z2)
...
function math.wrap_index_between(index, from, to)
	if to < from then
		to = from from = to
	end
...
function math.value_inside_range(value, min, max)
	...
	for rank_i = 1, min_rank_n do
		...
		if not left_rank or not right_rank then
			...
			for remaining_i = rank_i, #rank_lookup do
				...
				table.sort(other_rank, function (a, b) return sort_lookup [b] < sort_lookup [a] end) end break
		end
		...
		local smaller_n = left_n local larger_n = right_n
		...
		if left_n <= right_n then
			...
			larger_n = smaller_n smaller_n = larger_n
		...
		while smaller_n < larger_n do
			...
			if larger_n >= smaller_n + next_rank_n then
				...
				table.sort(next_rank, function (a, b) return smaller_sort_lookup [b] < smaller_sort_lookup [a] end)
		...
		table.sort(left_rank, function (a, b) return left_sort_lookup [b] < left_sort_lookup [a] end)
		table.sort(right_rank, function (a, b) return right_sort_lookup [b] < right_sort_lookup [a] end)
	end
end



function math.distributed_point_matching(source_positions, target_positions, out_indices, use_rows)
