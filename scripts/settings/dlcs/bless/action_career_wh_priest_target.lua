...
local crosshair_lookup = { target_self = "wh_priest_self", target_ally = "wh_priest_ally" }




function ActionCareerWHPriestTarget:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionCareerWHPriestTarget:client_owner_start_action(new_action, t, chain_action_data, power_level, action_init_data)
...
function ActionCareerWHPriestTarget:_start_charge_sound()
...
function ActionCareerWHPriestTarget:_stop_charge_sound()
	local current_action = self.current_action
	local owner_unit = self.owner_unit
	local wwise_world = self.wwise_world
	local is_bot = self.is_bot
...
function ActionCareerWHPriestTarget:client_owner_post_update(dt, t, world, can_damage)
	local current_action = self.current_action
	local owner_unit = self.owner_unit
...
function ActionCareerWHPriestTarget:_mark_target(new_target)
	if self.is_bot then
		return
	end
...
function ActionCareerWHPriestTarget:_target_ally_from_crosshair()
	...
	for i = 1, num_friendly_units do
		...
		if friendly_unit ~= self.owner_unit and
		HEALTH_ALIVE [friendly_unit] then
...
function ActionCareerWHPriestTarget:_check_cone_from_crosshair(player_position, player_direction, target, range_sq, dot_threshold)
...
function ActionCareerWHPriestTarget:finish(reason, data)
	local is_bot = self.is_bot
	...
	local chain_action_data = { target = aimed_target }
	local current_action = self.current_action
