...
function BTCombatIdleAction:init(...)
...
function BTCombatIdleAction:enter(unit, blackboard, t)
	self:_check_if_should_idle(unit, blackboard)
	blackboard.navigation_extension:set_enabled(true)
end

function BTCombatIdleAction:leave(unit, blackboard, t, reason, destroy)
	blackboard.combat_idling = nil
end

local EPSILON_SQ = 0.0001
function BTCombatIdleAction:_check_if_should_idle(unit, blackboard)
...
function BTCombatIdleAction:_init_idle_anim(unit, blackboard)
...
function BTCombatIdleAction:run(unit, blackboard, t, dt)
	self:_check_if_should_idle(unit, blackboard)
