...
function NetworkState:init(is_server, network_handler, server_peer_id, own_peer_id)
	self._shared_state = SharedState:new(
	"network_state_" .. server_peer_id,
	shared_state_spec,
	is_server,
	network_handler,
	server_peer_id,
	...
	if is_server then
		self._shared_state:set_server(key, { server_peer_id })
	else
		self._shared_state:set_server(key, { server_peer_id, own_peer_id })
	end
end

function NetworkState:full_sync()
	self._shared_state:full_sync()
end

function NetworkState:register_rpcs(network_event_delegate, network_transmit)
	self._shared_state:register_rpcs(

	network_event_delegate)
end

function NetworkState:unregister_network_events()
...
function NetworkState:get_revision()
	return self._shared_state:get_revision()
end

function NetworkState:is_peer_fully_synced(peer_id)
	return self._shared_state:is_peer_fully_synced(peer_id)
end

function NetworkState:is_server()
	return self._is_server
end

function NetworkState:get_server_peer_id()
	return self._server_peer_id
end

function NetworkState:get_own_peer_id()
	return self._own_peer_id
end

function NetworkState:get_peers()
	local key = self._shared_state:get_key("peers")
	return self._shared_state:get_server(key)
end

function NetworkState:add_peer(peer_id)
	local key = self._shared_state:get_key("peers")
...
function NetworkState:remove_peer(peer_id)
	local key = self._shared_state:get_key("peers")
	local peers = self._shared_state:get_server(key)
...
function NetworkState:set_level_data(level_key, environment_variation_id, level_seed, mechanism, game_mode, conflict_director, locked_director_functions, difficulty, difficulty_tweak, level_session_id, level_transition_type, extra_packages)
...
function NetworkState:is_peer_ingame(peer_id)
	local key = self._shared_state:get_key("peer_ingame", peer_id)
	return self._shared_state:get_server(key)
end

function NetworkState:set_peer_ingame(peer_id, ingame)
	local key = self._shared_state:get_key("peer_ingame", peer_id)
	self._shared_state:set_server(key, ingame)
end

function NetworkState:get_loaded_or_loading_packages()
	return self._loaded_or_loading_packages
end

function NetworkState:set_loaded_or_loading_packages(loaded_or_loading_packages)
	self._loaded_or_loading_packages = loaded_or_loading_packages
end

function NetworkState:get_profile_index_reservation(profile_index)
...
function NetworkState:set_profile_index_reservation(profile_index, peer_id)
	local key = self._shared_state:get_key("profile_index_reservation", nil, nil, profile_index)
	self._shared_state:set_server(key, peer_id or "")
end

function NetworkState:get_peers_with_full_profiles()
	local key = self._shared_state:get_key("full_profile_peers")

	return self._shared_state:get_server(key)
end

function NetworkState:get_profile(peer_id, local_player_id)
	local key = self._shared_state:get_key("full_profile_peers")
...
function NetworkState:set_profile(peer_id, local_player_id, profile_index, career_index, is_bot)
...
function NetworkState:get_inventory_data(peer_id, local_player_id)
	local key = self._shared_state:get_key("inventory_list", nil, local_player_id)
	return self._shared_state:get_peer(peer_id, key)
end

function NetworkState:set_inventory_data(peer_id, local_player_id, inventory)
...
function NetworkState:get_loaded_inventory_id(peer_id, for_peer_id, for_local_player_id)
...
function NetworkState:set_own_loaded_inventory_id(for_peer_id, for_local_player_id, inventory_id)
	local loaded_inventory_key = self._shared_state:get_key("loaded_inventory_id", for_peer_id, for_local_player_id)
	self._shared_state:set_own(loaded_inventory_key, inventory_id)
end


function NetworkState:delete_profile_data(peer_id, local_player_id)
...
function NetworkState:get_actually_ingame(peer_id)
	local key = self._shared_state:get_key("actually_ingame", peer_id)
	return self._shared_state:get_peer(peer_id, key)
end

function NetworkState:set_own_actually_ingame(actually_ingame)
