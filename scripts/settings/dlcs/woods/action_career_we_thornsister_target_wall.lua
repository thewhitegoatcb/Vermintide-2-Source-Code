...
function ActionCareerWEThornsisterTargetWall:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
	...
	self._segment_positions = { { num_segments = 0 }, { num_segments = 0 } }
...
function ActionCareerWEThornsisterTargetWall:client_owner_start_action(new_action, t, chain_action_data, power_level, action_init_data)
...
function ActionCareerWEThornsisterTargetWall:client_owner_post_update(dt, t, world, can_damage, current_time_in_action)
	self:_update_targeting()
end

function ActionCareerWEThornsisterTargetWall:_update_targeting()
...
function ActionCareerWEThornsisterTargetWall:finish(reason)
	if self._decal_unit then
...
function ActionCareerWEThornsisterTargetWall:_check_wall_linear(center, wall_rotation, width, thickness)
...
function ActionCareerWEThornsisterTargetWall:_check_wall_radial(center, wall_rotation, width, thickness)
	local game_session = Network.game_session()
	if game_session then
		local half_width = width / 2
		local half_thickness = thickness / 2
...
function ActionCareerWEThornsisterTargetWall:_check_segment(prev_position, position, forward)
	if position then
		...
		local result, hit_position, length = PhysicsWorld.immediate_raycast(physics_world,
		from,
		...
		if result then
			...
			if
			prev_position and WALL_MAX_HEIGHT_OFFSET < math.abs(hit_position.z - prev_position.z) then
			...
			if prev_position then
				...
				local sweep_results = PhysicsWorld.linear_obb_sweep(
				physics_world,
				sweep_from,
				overlap_pos,
				overlap_size,
				overlap_rot,
				5,
				"collision_filter", "filter_player_mover",
				"types", "statics",
				...
				hit_actors, actor_count = PhysicsWorld.immediate_overlap(physics_world,
				"position", overlap_pos,
				"rotation", overlap_rot,
				"size", overlap_size,
				"shape", "oobb",
				"types", "statics",
				"collision_filter",
