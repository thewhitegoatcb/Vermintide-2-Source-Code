...
if not UrlLoader then
	...
	UrlLoader = {
		init = function (loader) return end,
		load_texture = function (loader, url) return 0 end,
		unload = function (loader, job) return end,
		done = function (loader, job) return false end,
		success = function (loader, job) return false end,
		texture = function (loader, job) return nil end,
		update = function (loader) return end,
		destroy = function (loader) return end,
		is_stub = true }
end
...
function UrlLoaderManager:load_resource(reference_name, url, callback, cache_key, cache_version, texture_category)
...
function UrlLoaderManager:unload_resource(reference_name)
...
function UrlLoaderManager:_on_job_complete(job, success)
	local url_loader = self._url_loader
...
function UrlLoaderManager:update(dt)
	local url_loader = self._url_loader
...
function UrlLoaderManager:post_render()
