...
function BackendInterfaceCraftingBase:init()
...
function BackendInterfaceCraftingBase:get_recipes()
	return self._crafting_recipes
end
function BackendInterfaceCraftingBase:get_recipe_by_name(name)
	return self._crafting_recipes_by_name [name]
end

function BackendInterfaceCraftingBase:get_recipes_lookup()
	return self._crafting_recipes_lookup
end


function BackendInterfaceCraftingBase:_get_valid_recipe(item_backend_ids, recipe_override)
...
function BackendInterfaceCraftingBase:salvage_validation_func(recipe, item_backend_ids)
	...
	for i = 1, #item_backend_ids do
		...
		if masterlist_data then
			valid_item_ids [#valid_item_ids + 1] = { amount = 1, backend_id = backend_id }
		end
	end
...
function BackendInterfaceCraftingBase:craft_validation_func(recipe, item_backend_ids)
...
function BackendInterfaceCraftingBase:_validate_ingredient(ingredient, item_backend_ids)
	local backend_items = Managers.backend:get_interface("items")
	...
	for i = 1, #item_backend_ids do repeat
			...
			if item_name then if ingredient_name and ingredient_name ~= item_name then
					break
				end
				...
				if
				has_variable and not item_data [has_variable] then
					break
				end
				...
				ingredient_ids [#ingredient_ids + 1] = { backend_id = item_backend_id, amount = amount_from_item }
				...
				if total_found_ingredients == amount then
					...
					return true, ingredient_ids end end until true
	end
...
function BackendInterfaceCraftingBase:weapon_skin_application_validation_func(recipe, item_backend_ids)
	local ingredients = recipe.ingredients
	local backend_items = Managers.backend:get_interface("items")
	local cloned_backend_ids = table.clone(item_backend_ids)
	...
	for i = 1, #cloned_backend_ids do
		...
		if table.find(CraftingData.weapon_skin_slot_types, item_slot_type) then
			...
			valid_item_ids [#valid_item_ids + 1] = { skin_name = skin_name }
		end
		...
		if item_slot_type == "crafting_material" then
			for _, ingredient in ipairs(ingredients) do
				if ingredient.name and ingredient.amount and ingredient.name == item.ItemId then
					valid_item_ids [#valid_item_ids + 1] = { backend_id = backend_id, amount = ingredient.amount }
				end
			end
		end
	end
...
function BackendInterfaceCraftingBase:check_same_item_func(item_backend_ids)
	local backend_items = Managers.backend:get_interface("items")
	local name = nil
...
function BackendInterfaceCraftingBase:check_has_skin(item_backend_ids)
	local backend_items = Managers.backend:get_interface("items")
