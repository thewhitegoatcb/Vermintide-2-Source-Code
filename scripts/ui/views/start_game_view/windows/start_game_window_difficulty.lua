...
function StartGameWindowDifficulty:on_enter(params, offset)
...
function StartGameWindowDifficulty:create_ui_elements(params, offset)
...
function StartGameWindowDifficulty:_setup_difficulties()
...
function StartGameWindowDifficulty:_get_difficulty_options()
	return Managers.state.difficulty:get_level_difficulties()
end

function StartGameWindowDifficulty:on_exit(params)
...
function StartGameWindowDifficulty:update(dt, t)
...
function StartGameWindowDifficulty:post_update(dt, t)
	return end

function StartGameWindowDifficulty:_update_animations(dt)
...
function StartGameWindowDifficulty:_is_button_pressed(widget)
	local content = widget.content
...
function StartGameWindowDifficulty:_is_button_released(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function StartGameWindowDifficulty:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_enter and not hotspot.is_selected
end

function StartGameWindowDifficulty:_handle_input(dt, t)
	local difficulty_widgets = self._difficulty_widgets
	for i = 1, #difficulty_widgets do
		local widget = difficulty_widgets [i]
...
function StartGameWindowDifficulty:_set_selected_difficulty_option(new_difficulty_key)
	local difficulty_widgets = self._difficulty_widgets
	for i = 1, #difficulty_widgets do
		local widget = difficulty_widgets [i]
		local content = widget.content
...
function StartGameWindowDifficulty:_set_info_window(difficulty_key)
	local difficulty_settings = DifficultySettings [difficulty_key]
...
function StartGameWindowDifficulty:_update_difficulty_lock()
	local widgets_by_name = self._widgets_by_name
	local select_button = widgets_by_name.select_button
	local buy_button = widgets_by_name.buy_button
...
function StartGameWindowDifficulty:_update_selected_difficulty_option(difficulty_key)
...
function StartGameWindowDifficulty:draw(dt)
...
function StartGameWindowDifficulty:_play_sound(event)
	self.parent:play_sound(event)
end

function StartGameWindowDifficulty:_animate_difficulty_option_button(widget, dt)
	local content = widget.content
...
function StartGameWindowDifficulty:_show_storepage(store_page_url)
