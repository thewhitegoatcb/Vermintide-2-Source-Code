...
function BTBotShootAction:init(...)
...
local DEFAULT_AIM_DATA = { min_radius_pseudo_random_c = 0.0557, max_radius_pseudo_random_c = 0.01475, min_radius = math.pi / 72, max_radius = math.pi / 16 }
...
function BTBotShootAction:enter(unit, blackboard, t)
	...
	blackboard.shoot = { num_aim_rolls = 0, charging_shot = false, disengage_update_time = 0, obstructed = true,
...
function BTBotShootAction:_update_collision_filter(target_unit, shoot_blackboard, priority_target_enemy, target_ally_unit, target_ally_needs_aid, need_type)
	...
	shoot_blackboard.collision_filter = ignore_enemies_for_obstruction and ignore_hitting_allies and "filter_bot_ranged_line_of_sight_no_allies_no_enemies" or
	ignore_hitting_allies and "filter_bot_ranged_line_of_sight_no_allies" or
	ignore_enemies_for_obstruction and "filter_bot_ranged_line_of_sight_no_enemies" or "filter_bot_ranged_line_of_sight"

	shoot_blackboard.collision_filter_charged = ignore_enemies_for_obstruction_charged and ignore_hitting_allies_charged and "filter_bot_ranged_line_of_sight_no_allies_no_enemies" or
	ignore_hitting_allies_charged and "filter_bot_ranged_line_of_sight_no_allies" or
	ignore_enemies_for_obstruction_charged and "filter_bot_ranged_line_of_sight_no_enemies" or "filter_bot_ranged_line_of_sight"
end

function BTBotShootAction:leave(unit, blackboard, t, reason, destroy)
	local input_ext = blackboard.input_extension
...
function BTBotShootAction:run(unit, blackboard, t, dt)
	THIS_UNIT = unit
	local done, evaluate = self:_aim(unit, blackboard, dt, t)

	if done then
		do return "done", "evaluate" end
	else
		return "running", evaluate and "evaluate" or nil
	end
end

function BTBotShootAction:_set_new_aim_target(self_unit, t, shoot_blackboard, target_unit, first_person_ext)
...
function BTBotShootAction:_wanted_aim_rotation(self_unit, target_unit, current_position, projectile_info, projectile_speed, aim_at_node)
...
function BTBotShootAction:_aim_position(dt, t, self_unit, current_position, current_rotation, target_unit, shoot_blackboard)
...
function BTBotShootAction:_calculate_aim_speed(self_unit, dt, current_yaw, current_pitch, wanted_yaw, wanted_pitch, current_yaw_speed, current_pitch_speed)
	local pi = math.pi
...
function BTBotShootAction:_may_attack(unit, enemy_unit, shoot_blackboard, range_squared, t)
...
function BTBotShootAction:_aim(unit, blackboard, dt, t)
	local target_unit = blackboard.target_unit
	local shoot_bb = blackboard.shoot
	...
	if shoot_bb.reevaluate_obstruction_time <= t then
		if self:_reevaluate_obstruction(unit, shoot_bb, action_data, t, World.get_data(blackboard.world, "physics_world"), camera_position, wanted_aim_rotation, unit, target_unit, actual_aim_position, blackboard.priority_target_enemy, blackboard.target_ally_unit, blackboard.target_ally_needs_aid, blackboard.target_ally_need_type) then
			if not blackboard.ranged_obstruction_by_static then
				blackboard.ranged_obstruction_by_static = { unit = target_unit, timer = t }
			else
	...
	if shoot_bb.fired and shoot_bb.hold_fire_condition and
	shoot_bb.hold_fire_condition(t, blackboard) then
		self:_fire_shot(shoot_bb, action_data, input_ext, t)
		done_firing = false
	end
...
function BTBotShootAction:_aim_good_enough(dt, t, shoot_blackboard, yaw_offset, pitch_offset)
...
function BTBotShootAction:_should_charge(shoot_blackboard, range_squared, target_unit, t)
...
function BTBotShootAction:_fire_shot(shoot_blackboard, action_data, input_extension, t)
...
function BTBotShootAction:_charge_shot(shoot_blackboard, action_data, input_extension, t)
...
function BTBotShootAction:_update_disengage_position(blackboard, t, breed_distance_override)
	local first_person_ext = blackboard.first_person_extension
...
function BTBotShootAction:_reevaluate_obstruction(unit, shoot_blackboard, action_data, t, physics_world, ray_from, wanted_aim_rotation, self_unit, target_unit, actual_aim_position, priority_target_enemy, target_ally_unit, target_ally_needs_aid, target_ally_need_type)
...
function BTBotShootAction:_is_shot_obstructed(physics_world, from, direction, self_unit, target_unit, actual_aim_position, collision_filter)
	...
	local raycast_hits = PhysicsWorld.immediate_raycast(
	physics_world,
	from,
	direction,
	max_distance,
	...
	if not raycast_hits then return false
	end
