...
local FLOW_STATE_TYPES = {
	boolean = { rpcs = { change = "rpc_flow_state_bool_changed" } },
	number = { network_constant = "number", rpcs = { change = "rpc_flow_state_number_changed" } } }


local LOOP_MODE_LOOKUP = { "none", "loop", "ping_pong" }
...
local RPCS = { "rpc_flow_state_story_played", "rpc_flow_state_story_stopped" }
...
function NetworkedFlowStateManager:create_checkpoint_data()
	local object_states = { }
	...
	local checkpoint_data = { object_states = object_states, playing_stories = playing_stories }

	return checkpoint_data
end

function NetworkedFlowStateManager:load_checkpoint_data(checkpoint_data)
	...
	for client_call_event_name, checkpoint_story_data in pairs(checkpoint_data.playing_stories) do
		...
		if story_data.stopped then
			...
			self._client_call_data = { stop_out = true }
			Level.trigger_event(self._level, client_call_event_name)
			self._client_call_data = { play_out = true, time_out = stop_time }
			Level.trigger_event(self._level, client_call_event_name)
			self._client_call_data = { stop_out = true }
			Level.trigger_event(self._level, client_call_event_name)
		else
			local start_time = story_data.current_time
			self._client_call_data = { play_out = true, time_out = start_time }
...
function NetworkedFlowStateManager:flow_cb_create_story(params)
...
function NetworkedFlowStateManager:flow_cb_play_networked_story(params)
	if self._is_client then
		return nil
	end
	...
	return { play_out = true, time_out = start_time }
end

function NetworkedFlowStateManager:rpc_flow_state_story_played(channel_id, client_call_event_name_id, start_time)
...
function NetworkedFlowStateManager:flow_cb_networked_story_client_call(params)
...
function NetworkedFlowStateManager:flow_cb_stop_networked_story(params)
	if self._is_client then
		return nil
	end
	...
	return { stop_out = true }
end

function NetworkedFlowStateManager:rpc_flow_state_story_stopped(channel_id, client_call_event_name_id, stop_time)
	local client_call_event_name = self._story_lookup [client_call_event_name_id]
...
function NetworkedFlowStateManager:flow_cb_has_stopped_networked_story(params)
	if self._is_client then
		return nil
	end
...
function NetworkedFlowStateManager:flow_cb_has_played_networked_story(params)
	if self._is_client then
		return nil
	end
...
function NetworkedFlowStateManager:hot_join_sync(peer)
	self:_sync_states(peer)
	self:_sync_stories(peer)
end

function NetworkedFlowStateManager:_sync_stories(peer)
...
function NetworkedFlowStateManager:_sync_states(peer)
...
function NetworkedFlowStateManager:set_level(level)
	self._level = level
end
...
function NetworkedFlowStateManager:flow_cb_create_state(unit, state_name, default_value, client_data_changed_event, hot_join_sync_event, is_game_object)
	...
	if not states [unit] then local unit_states = { lookup = { }, states = { } }
	end
...
function NetworkedFlowStateManager:clear_object_state(unit)
