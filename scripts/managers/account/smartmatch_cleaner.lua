...
function SmartMatchCleaner:init()
	self:reset()
end

function SmartMatchCleaner:reset()
	self._sessions_to_clean = { }
end

function SmartMatchCleaner:ready()
	return #self._sessions_to_clean == 0
end

function SmartMatchCleaner:add_session(session_data)
	self._sessions_to_clean [#self._sessions_to_clean + 1] = session_data
end

local ENTRIES_TO_REMOVE = ENTRIES_TO_REMOVE or { }
function SmartMatchCleaner:update(dt)
	self:_update_cleanup(dt)
	self:_update_remove(dt)
end

function SmartMatchCleaner:_update_cleanup(dt)
...
function SmartMatchCleaner:_update_remove(dt)
...
function SmartMatchCleaner:_change_state(session_data, state)
...
function SmartMatchCleaner:_cleanup_ticket(dt, index, session_data)
	local session_id = session_data.session_id
	local session_name = session_data.session_name
...
function SmartMatchCleaner:_cleanup_session(dt, index, session_data)
	local session_id = session_data.session_id
	local session_name = session_data.session_name
	local session_name = session_data.session_name
	local hopper_name = session_data.hopper_name
...
function SmartMatchCleaner:_free_session(dt, index, session_data)
	local session_id = session_data.session_id
	local session_name = session_data.session_name
...
function SmartMatchCleaner:_do_remove() return end