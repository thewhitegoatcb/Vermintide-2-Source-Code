...
local extensions = { "GameObjectExtension" }



GameObjectSystem = class(GameObjectSystem, ExtensionSystemBase)

function GameObjectSystem:init(entity_system_creation_context, system_name)
...
function GameObjectSystem:destroy()
	return end

local dummy_input = { }
function GameObjectSystem:on_add_extension(world, unit, extension_name, extension_init_data)
	local extension = { }
...
function GameObjectSystem:extensions_ready(world, unit, extension_name)
	if extension_name == "GameObjectExtension" then
...
function GameObjectSystem:on_remove_extension(unit, extension_name)
	local extension = self.unit_extension_data [unit]
...
function GameObjectSystem:game_object_created(game_object_id, owner_id, go_template)
	local game_session = Managers.state.network:game()
...
function GameObjectSystem:update(context, t)
	return end

function GameObjectSystem:hot_join_sync(peer_id)
	return end