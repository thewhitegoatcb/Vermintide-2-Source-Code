...
function ActionThrowGrimoire:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionThrowGrimoire:client_owner_start_action(new_action, t)
...
function ActionThrowGrimoire:client_owner_post_update(dt, t, world, can_damage)
	return end

function ActionThrowGrimoire:finish(reason)
	...
	if self.is_server then
		Managers.state.network.network_transmit:send_rpc_clients("rpc_coop_feedback",
		player:network_id(),
		player:local_player_id(),
		NetworkLookup.coop_feedback [predicate],
		player:network_id(),
		player:local_player_id())
	else
		Managers.state.network.network_transmit:send_rpc_server("rpc_coop_feedback",
		player:network_id(),
		player:local_player_id(),
		NetworkLookup.coop_feedback [predicate],
		player:network_id(),
