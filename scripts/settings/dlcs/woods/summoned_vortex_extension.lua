...
local sot_landing_breeds = { chaos_marauder_with_shield = true, chaos_raider = true, chaos_fanatic = true, skaven_slave = true, chaos_berzerker = true, skaven_clan_rat_with_shield = true, skaven_clan_rat = true, chaos_marauder = true }
...
function SummonedVortexExtension:init(extension_init_context, unit, extension_init_data)
...
function SummonedVortexExtension:_create_nav_cost_maps(ai_system, position, full_outer_radius, high_cost_type, medium_cost_type)
...
function SummonedVortexExtension:extensions_ready(world, unit)
	...
	self.vortex_data = { idle_time = 0, height = 5, start_lerp_height = 5, start_lerp_inner_radius = 2, start_lerp_fx_radius = 8, current_raycast_rad = 0, num_players_inside = 0,
		ai_units_inside = { },
...
function SummonedVortexExtension:refresh_duration()
...
function SummonedVortexExtension:destroy()
	local vortex_data = self.vortex_data
	local ai_units_inside = vortex_data.ai_units_inside
	local BLACKBOARDS = BLACKBOARDS
...
function SummonedVortexExtension:update(unit, input, dt, context, t)
	local vortex_template = self.vortex_template
...
function SummonedVortexExtension:_update_nav_cost_map_volumes(position, full_outer_radius, nav_world, ai_system, navigation_extension, locomotion_extension)
...
function SummonedVortexExtension:_update_height(unit, t, dt, vortex_template, vortex_data)
...
local ai_units = { }
function SummonedVortexExtension:_update_attract_outside_ai(vortex_data, vortex_template, center_pos, minimum_height_diff, inner_radius, outer_radius, falloff_radius)
...
function SummonedVortexExtension:_update_attract_inside_ai(vortex_data, vortex_template, dt, center_pos, inner_radius, allowed_distance)
...
function SummonedVortexExtension:attract(unit, t, dt, vortex_template, vortex_data, center_pos, inner_radius, outer_radius)
...
function SummonedVortexExtension:is_position_inside(position, min_allowed_distance)
	local vortex_data = self.vortex_data
	local outer_radius = vortex_data.outer_radius
...
function SummonedVortexExtension:debug_render_vortex(t, dt, pos, fx_radius, inner_radius, outer_radius, spin_speed, height)
