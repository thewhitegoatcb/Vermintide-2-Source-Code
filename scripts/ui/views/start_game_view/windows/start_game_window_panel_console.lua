...
function StartGameWindowPanelConsole:on_enter(params, offset)
...
function StartGameWindowPanelConsole:_create_ui_elements(definitions, params, offset)
...
function StartGameWindowPanelConsole:_setup_text_buttons_width_and_position()
...
function StartGameWindowPanelConsole:_set_text_button_size(widget, width)
...
function StartGameWindowPanelConsole:on_exit(params)
	print("[HeroViewWindow] Exit Substate StartGameWindowPanelConsole")
	self.ui_animator = nil
end

function StartGameWindowPanelConsole:update(dt, t)
...
function StartGameWindowPanelConsole:_update_selected_option()
	local parent = self.parent
...
function StartGameWindowPanelConsole:_update_title_buttons_disable_status()
	local title_button_widgets = self._title_button_widgets
...
function StartGameWindowPanelConsole:post_update(dt, t)
	self:_handle_input(dt, t)
end

function StartGameWindowPanelConsole:_update_animations(dt, t)
...
function StartGameWindowPanelConsole:_find_next_layout_name(direction)
...
function StartGameWindowPanelConsole:_handle_input(dt, t)
	...
	if not close_on_exit and not input_made and UIUtils.is_button_pressed(back_button) then
		...
		return_layout_name =
		return_layout_name or parent:get_previous_selected_layout_name()
...
function StartGameWindowPanelConsole:_set_selected_option(selected_layout_name)
	local title_button_widgets = self._title_button_widgets
	for i = 1, #title_button_widgets do
		local widget = title_button_widgets [i]
...
function StartGameWindowPanelConsole:draw(dt)
...
function StartGameWindowPanelConsole:_play_sound(event)
	self.parent:play_sound(event)
end

function StartGameWindowPanelConsole:_setup_input_buttons()
	local input_service = self.parent:window_input_service()
...
function StartGameWindowPanelConsole:_handle_back_button_visibility()
...
function StartGameWindowPanelConsole:_reset_back_button()
	local back_button = self._widgets_by_name.back_button
...
function StartGameWindowPanelConsole:_is_supported_with_twitch(game_mode)
...
function StartGameWindowPanelConsole:_animate_title_entry(widget, dt, t)
	local content = widget.content
...
function StartGameWindowPanelConsole:_animate_back_button(widget, dt)
	local content = widget.content
	local style = widget.style
	local hotspot = content.button_hotspot
