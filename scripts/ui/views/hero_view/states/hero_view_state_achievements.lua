...
function HeroViewStateAchievements:on_enter(params)
	...
	self._current_gamepad_input_selection = { 1, 1 }
...
function HeroViewStateAchievements:_create_filter_input_service()
	local input_manager = Managers.input
	input_manager:create_input_service("achievement_filter", "IngameMenuKeymaps", "IngameMenuFilters", { hero_view = false })
	input_manager:map_device_to_service("achievement_filter", "gamepad")
end

function HeroViewStateAchievements:get_filter_input_service()
	return Managers.input:get_service("achievement_filter")
end

function HeroViewStateAchievements:_update_buttons_new_status()
...
function HeroViewStateAchievements:_update_summary_quest_timers(dt)
...
function HeroViewStateAchievements:create_ui_elements(params)
...
function HeroViewStateAchievements:_setup_achievement_progress_overview()
	...
	for i, category in ipairs(achievement_outline.categories) do
		if category.present_progression then
			local category_progress_data = { display_name = category.name }
...
function HeroViewStateAchievements:_handle_layout_buttons_hovered()
...
function HeroViewStateAchievements:_on_layout_button_pressed(widget, window_widget, layout_type, tab_widget_index)
	local widgets_by_name = self._widgets_by_name
...
function HeroViewStateAchievements:_reset_tabs()
	for _, widget in ipairs(self._category_tab_widgets) do
		self:_reset_tab(widget)
	end
end

function HeroViewStateAchievements:_setup_layout(layout_type)
...
function HeroViewStateAchievements:_setup_tab_widget(tab_widget, data)
	if data then
...
function HeroViewStateAchievements:_get_layout(layout_type)
	if layout_type == "achievements" then
...
function HeroViewStateAchievements:_reset_tab(widget)
	...
	if entries then
		for i = 1, #entries do
			...
			if data.completed and not data.claimed then
				...
				local is_unlocked = ( not required_dlc or unlock_manager:is_dlc_unlocked(required_dlc) ) and (
				not required_dlc_extra or unlock_manager:is_dlc_unlocked(required_dlc_extra))
...
function HeroViewStateAchievements:_has_any_unclaimed_completed_challenge_in_category(base_data)
...
function HeroViewStateAchievements:_populate_tab(widget, categories)
	local content = widget.content
...
function HeroViewStateAchievements:_create_entries(entries, entry_type, entry_subtype)
	local quest_manager = self._quest_manager
	local achievement_manager = self._achievement_manager
	self._claimable_challenge_widgets = { }
	...
	for i = 1, #entries do
		...
		needle ~= nil and not SearchUtils.simple_search(needle, entry_data.name) and
		not SearchUtils.simple_search(needle, entry_data.desc) then --[[ Nothing --]]
			...
			if query.completed ~= nil and query.completed == not completed then --[[ Nothing --]]
				...
				if query.claimed == nil or query.claimed ~= not claimed then
					...
					if query.locked ~= nil and query.locked == unlocked then --[[ Nothing --]]
						...
						if reward then
							if type(reward) == "string" then
								...
								local fake_item = { data = item_template }
							...
							elseif type(reward) == "table" then
								...
								elseif reward_type == "keep_decoration_painting" then
									...
									local fake_item = {
										data = { item_type = "keep_decoration_painting", slot_type = "keep_decoration_painting", information_text = "information_text_painting", matching_item_key = "keep_decoration_painting",
								...
								elseif reward_type == "weapon_skin" then
									...
									local fake_item = {
										data = { item_type = "weapon_skin", slot_type = "weapon_skin", information_text = "information_weapon_skin",
								...
								elseif reward_type == "currency" then
									local fake_item = { data = BackendUtils.get_fake_currency_item(reward.currency_code, reward.amount) }
	...
	if #unclaimable_achievement_widgets > 1 then
		table.sort(unclaimable_achievement_widgets, function (a, b)
			...
			if a_content.claimed == b_content.claimed then
				do return a_content.original_order_index < b_content.original_order_index end
			else
...
function HeroViewStateAchievements:_show_empty_entries_warning()
...
function HeroViewStateAchievements:_hide_empty_entries_warning()
	local additional_type_widgets_by_name = self._additional_type_widgets_by_name
	local overlay_widget = additional_type_widgets_by_name.overlay
	local overlay_text_widget = additional_type_widgets_by_name.overlay_text
	local overlay_fade_widget = additional_type_widgets_by_name.overlay_fade
...
function HeroViewStateAchievements:_set_widget_bar_progress(widget, current, required)
	local content = widget.content
	local style = widget.style
...
function HeroViewStateAchievements:_set_requirements(widget, requirements)
	local content = widget.content
	local style = widget.style
...
function HeroViewStateAchievements:_set_achievement_expand_height(widget, height)
	local content = widget.content
	local style = widget.style
...
function HeroViewStateAchievements:_update_achievements_scroll_height(optional_scroll_value)
...
function HeroViewStateAchievements:_update_categories_scroll_height(optional_scroll_value)
...
function HeroViewStateAchievements:_get_achievement_entries_height(start_index)
...
function HeroViewStateAchievements:_get_category_entries_height()
...
function HeroViewStateAchievements:_get_active_tabs_height()
...
function HeroViewStateAchievements:_get_active_category_height()
...
function HeroViewStateAchievements:_setup_scrollbar(height, optional_value)
...
function HeroViewStateAchievements:_update_mouse_scroll_input()
...
function HeroViewStateAchievements:_set_scrollbar_value(value)
	local current_scroll_value = self.scroll_value
	if value then
		local widgets_by_name = self._widgets_by_name
		local widget = widgets_by_name.achievement_scrollbar
...
function HeroViewStateAchievements:_update_achievement_read_index(fraction)
	local achievement_widgets = self._achievement_widgets
...
function HeroViewStateAchievements:_update_category_scroll_position()
	local scrollbar_logic = self._category_scrollbar
...
function HeroViewStateAchievements:_on_achievement_pressed(widget)
	if self._claim_all then return end
	local content = widget.content
	local style = widget.style
...
function HeroViewStateAchievements:_claim_reward(widget)
	local content = widget.content
	local id = content.id
...
function HeroViewStateAchievements:_claim_multiple_rewards(challenge_widgets)
...
function HeroViewStateAchievements:_claim_quest_reward(id)
	local quest_manager = self._quest_manager
...
function HeroViewStateAchievements:_claim_multiple_quest_rewards(keys)
	local quest_manager = self._quest_manager
...
function HeroViewStateAchievements:_claim_achievement_reward(id)
	local achievement_manager = self._achievement_manager
...
function HeroViewStateAchievements:_claim_multiple_achievement_rewards(achievements_ids)
	local achievement_manager = self._achievement_manager
...
function HeroViewStateAchievements:_is_polling()
	return self._reward_poll_id or self._quest_refresh_poll_id or self._reward_poll_claim_all_id
end

function HeroViewStateAchievements:_poll_quest_refresh(dt)
...
function HeroViewStateAchievements:_poll_rewards(dt)
	local reward_poll_id = self._reward_poll_id
	if not reward_poll_id then
		return
	end

	local is_polling_reward = nil local polling_type = self._reward_poll_type
...
function HeroViewStateAchievements:_poll_all_rewards(dt)
...
function HeroViewStateAchievements:_on_reward_claimed(reward_poll_id, polling_type)
...
function HeroViewStateAchievements:_on_all_rewards_claimed(reward_poll_id, polling_type)
...
function HeroViewStateAchievements:_update_new_status_for_current_tab()
	if self._achievement_layout_type == "achievements" then
		...
		return end if self._achievement_layout_type == "quest" then local layout = self:_get_layout(self._achievement_layout_type) local categories = layout.categories local category_tab_widgets = self._category_tab_widgets local num_tab_widgets = #category_tab_widgets for i = 1, num_tab_widgets do local new_data = categories [i] local widget = category_tab_widgets [i] self:_setup_tab_widget(widget, new_data) end end end

function HeroViewStateAchievements:_setup_reward_presentation(reward_poll_id, polling_type)
	...
	if num_rewards > 0 then
		...
		for _, data in ipairs(rewards) do
			...
			if reward_type == "item" or CosmeticUtils.is_cosmetic_item(reward_type) then
				...
				entry [#entry + 1] = { widget_type = "description",
					value = description }



				entry [#entry + 1] = { widget_type = "item",
					value = reward_item }
			...
			elseif reward_type == "keep_decoration_painting" then
				...
				entry [#entry + 1] = { widget_type = "icon",
					value = icon }
			...
			elseif reward_type == "weapon_skin" then
				...
				entry [#entry + 1] = { widget_type = "weapon_skin",
					value = { icon = icon, rarity = rarity } }



				presentation_data [#presentation_data + 1] = entry
			elseif reward_type == "currency" then
				local fake_item = { data = BackendUtils.get_fake_currency_item(data.currency_code, data.amount) }
				local description = { }
				...
				local entry = {
					[#entry + 1] = { widget_type = "description",
						value = description },


					[#entry + 1] = { widget_type = "icon",
...
function HeroViewStateAchievements:_align_achievement_entries()
	local total_height = 0
...
function HeroViewStateAchievements:_setup_achievement_entries_animations()
...
function HeroViewStateAchievements:transitioning()
...
function HeroViewStateAchievements:_wanted_state()
	local new_state = self.parent:wanted_state()
	return new_state
end

function HeroViewStateAchievements:wanted_menu_state()
	return self._wanted_menu_state
end

function HeroViewStateAchievements:clear_wanted_menu_state()
	self._wanted_menu_state = nil
end

function HeroViewStateAchievements:on_exit(params)
...
function HeroViewStateAchievements:_update_transition_timer(dt)
...
function HeroViewStateAchievements:input_service()
	return self.parent:input_service()
end

function HeroViewStateAchievements:update(dt, t)
...
function HeroViewStateAchievements:_has_active_level_vote()
...
function HeroViewStateAchievements:post_update(dt, t)
	self.ui_animator:update(dt)
	self:_update_animations(dt)
end

function HeroViewStateAchievements:_update_animations(dt)
...
function HeroViewStateAchievements:_set_button_force_hover(widget, forced)
	local content = widget.content
	local hotspot = content.button_hotspot or content.hotspot
	hotspot.force_hover = forced
end

function HeroViewStateAchievements:_handle_gamepad_filter_input(dt, t)
	...
	elseif input_service:get("confirm") then
		...
		if current_row ~= self._current_gamepad_input_selection [2] or current_column ~= self._current_gamepad_input_selection [1] then
			...
			filter_content.gamepad_button_index = { current_column, current_row }
...
function HeroViewStateAchievements:_enable_gamepad_filters(enable)
...
function HeroViewStateAchievements:_handle_input(dt, t)
...
function HeroViewStateAchievements:_on_tab_list_pressed(list_index, ignore_sound)
...
function HeroViewStateAchievements:_tab_pressed(widget, index, tab_list_index, ignore_sound)
...
function HeroViewStateAchievements:_activate_tab(widget, index, tab_list_index, ignore_sound)
...
function HeroViewStateAchievements:_deactivate_active_tab()
...
function HeroViewStateAchievements:close_menu(ignore_sound_on_close_menu)
...
function HeroViewStateAchievements:draw(input_service, dt)
...
function HeroViewStateAchievements:play_sound(event)
	self.parent:play_sound(event)
end

function HeroViewStateAchievements:_start_transition_animation(key, animation_name)
	local params = {
		wwise_world = self.wwise_world,
...
function HeroViewStateAchievements:set_fullscreen_effect_enable_state(enabled)
...
function HeroViewStateAchievements:block_input()
	self._input_blocked = true
end

function HeroViewStateAchievements:unblock_input()
	self._input_blocked = false
end

function HeroViewStateAchievements:input_blocked()
	return self._input_blocked
end

function HeroViewStateAchievements:_set_summary_achievement_categories_progress(progress_data)
	local summary_widgets_by_name = self._summary_widgets_by_name
...
function HeroViewStateAchievements:_present_reward(data)
...
function HeroViewStateAchievements:_handle_queued_presentations()
...
function HeroViewStateAchievements:_displaying_reward_presentation()
	return self.reward_popup:is_presentation_active()
end

function HeroViewStateAchievements:_is_reward_presentation_complete()
	return self.reward_popup:is_presentation_complete()
end

function HeroViewStateAchievements:_reward_presentation_done()
	return not self._reward_presentation_active
end

function HeroViewStateAchievements:_setup_quest_summary_progress()
	local layout_type = "quest"
	local layout = self:_get_layout(layout_type)
	local categories = layout.categories
	...
	local disabled_title_color = { 255, 80, 80, 80 }
...
function HeroViewStateAchievements:_animate_window_button(widget, dt)
	local content = widget.content
	local style = widget.style
...
function HeroViewStateAchievements:_set_uvs_scale_progress(scenegraph_id, uvs, progress)
...
function HeroViewStateAchievements:_handle_input_desc()
	local query = self._search_widgets_by_name.filters.content.query
	local input_desc = nil
	if self._achievement_layout_type ~= "summary" then
		if self._gamepad_filter_active then --[[ Nothing --]]


		elseif not table.is_empty(query) then input_desc = "filter_available" else input_desc = "filter_unavailable"
		end
	end
...
function HeroViewStateAchievements:_handle_search_input(dt, t, input_service)
	local input_content = self._search_widgets_by_name.input.content
	local filters_content = self._search_widgets_by_name.filters.content


	if input_content.clear_hotspot.on_pressed then
		input_content.text_index = 1 input_content.caret_index = 1 input_content.search_query = ""
		self:_do_search(input_content.search_query)
		return true
	end
...
function HeroViewStateAchievements:_do_search(search_query)
...
function HeroViewStateAchievements:_set_input_blocked(blocked)
	local input_manager = Managers.input
	if blocked then
...
function HeroViewStateAchievements:_handle_claim_all_challenges()
	local achievement_layout_type = self._achievement_layout_type
	local active_tab_index = self._active_tab_index
	local active_list_index = self._active_list_index
...
function HeroViewStateAchievements:_animate_claim_button(widget, within_display_range, dt, t)
	local visible = widget.content.visible
...
function HeroViewStateAchievements:_handle_gamepad_activity()
	local gamepad_active = Managers.input:is_device_active("gamepad")
