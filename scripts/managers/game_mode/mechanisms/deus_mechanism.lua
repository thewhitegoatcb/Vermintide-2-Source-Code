...
local vote_requests = {
	deus_quickplay = function (request_type, params)
		local vote_data = { mechanism = "deus",
...
function DeusMechanism:network_context_created(lobby, server_peer_id, own_peer_id, is_server, network_handler)
...
function DeusMechanism:network_context_destroyed()
	return end

function DeusMechanism:handle_ingame_enter(game_mode)
	if self._deus_run_controller then
...
function DeusMechanism:handle_ingame_exit(exit_type)
...
function DeusMechanism:create_host_migration_info(gm_event_end_conditions_met, gm_event_end_reason)
	...
	matchmaking_type = IS_PS4 and
	"n/a" or


	deus_run_controller and NetworkLookup.matchmaking_types ["n/a"] or

	network_handler.lobby_client:lobby_data("matchmaking_type") or NetworkLookup.matchmaking_types ["n/a"]
...
function DeusMechanism:register_rpcs(network_event_delegate)
...
function DeusMechanism:unregister_rpcs()
...
function DeusMechanism:can_resync_loadout()
	local level_transition_handler = Managers.level_transition_handler
	local game_mode = level_transition_handler:get_current_game_mode()

	if game_mode == "deus" then

		do return self._deus_run_controller ~= nil end
	else
		return true
	end
end

function DeusMechanism:update_loadout()
	local deus_run_controller = self._deus_run_controller
...
function DeusMechanism:_update_career_loadout(local_player_id, career_name)
	local deus_run_controller = self._deus_run_controller
	if not deus_run_controller or deus_run_controller:get_run_ended() then
		return
	end
...
function DeusMechanism:choose_next_state(next_state)

	self._next_state = next_state
end


function DeusMechanism:reset_choose_next_state()
	self._next_state = nil
end


function DeusMechanism:progress_state()
...
function DeusMechanism:get_prior_state()
...
function DeusMechanism:set_current_state(state)
	self._prior_state = self._state
	self._state = state
end

function DeusMechanism:get_hub_level_key()
	return HUB_LEVEL_NAME
end

function DeusMechanism:get_end_of_level_rewards_arguments(game_won, quickplay, statistics_db, stats_id)
...
function DeusMechanism:get_end_of_level_extra_mission_results()
	return self._deus_run_controller:get_mission_results()
end

function DeusMechanism:get_players_session_score(statistics_db, profile_synchronizer, saved_scoreboard_stats)
	return self._deus_run_controller and self._deus_run_controller:get_scoreboard()
end

function DeusMechanism:on_final_round_won(statistics_db, stats_id)
...
function DeusMechanism:request_vote(params)
...
function DeusMechanism:get_deus_run_controller()
	return self._deus_run_controller
end


function DeusMechanism:is_final_round()
	return self._final_round
end


function DeusMechanism:is_venture_over()
...
function DeusMechanism:game_round_ended(t, dt, reason, reason_data)
...
function DeusMechanism:_transition_next_node(next_node_key)
	local deus_run_controller = self._deus_run_controller
	local level_transition_handler = Managers.level_transition_handler
...
function DeusMechanism:_transition_map()
	local deus_run_controller = self._deus_run_controller
	local level_transition_handler = Managers.level_transition_handler
...
function DeusMechanism:_transition_to_inn()
	local level_transition_handler = Managers.level_transition_handler
...
function DeusMechanism:should_run_tutorial()
	return true, "tutorial"
end

function DeusMechanism:get_level_end_view()
	return "LevelEndViewDeus"
end


function DeusMechanism:_get_next_game_mode_key()
...
function DeusMechanism:start_next_round()
	local deus_run_controller = self._deus_run_controller
...
function DeusMechanism:_build_side_compositions(state)
	local available_profiles = self._hero_profiles
	local party_manager = Managers.party
	local side_compositions = { { name = "heroes",
			...
			add_these_settings = { using_grims_and_tomes = true, show_damage_feedback = false, using_enemy_recycler = true,
...
function DeusMechanism:profile_available(profile_synchronizer, profile_name, career_name)
...
function DeusMechanism:get_state()
	return self._state
end

function DeusMechanism:generate_level_seed()
	local deus_run_controller = self._deus_run_controller
...
function DeusMechanism:get_current_node_curse()
	local deus_run_controller = self._deus_run_controller
	local current_node = deus_run_controller and deus_run_controller:get_current_node()
	return current_node and current_node.curse or nil
end

function DeusMechanism:get_current_node_theme()
	local deus_run_controller = self._deus_run_controller
	local current_node = deus_run_controller and deus_run_controller:get_current_node()
	return current_node and current_node.theme or nil
end

function DeusMechanism:get_level_seed(level_seed, optional_system)
	local deus_run_controller = self._deus_run_controller
	local current_node = deus_run_controller and deus_run_controller:get_current_node()
...
function DeusMechanism:can_spawn_pickup(spawner_unit, pickup_name)
	local can_spawn = nil
...
function DeusMechanism:uses_random_directors()

	return false
end

function DeusMechanism:profile_available_for_peer(profile_synchronizer, peer_id, local_player_id, profile_name, career_name)
	local profile_index = FindProfileIndex(profile_name)
...
function DeusMechanism:profile_changed(peer_id, local_player_id, profile_index, career_index)
...
function DeusMechanism:allocate_slot(sender, profile_index)
...
function DeusMechanism:sync_mechanism_data(peer_id, mechanism_newly_initialized)
...
function DeusMechanism:client_left(peer_id)
	return end

function DeusMechanism:_send_level_started_tracking_data()
...
function DeusMechanism:_send_level_ended_tracking_data(game_won)
	local statistics_db = Managers.player:statistics_db()
	local bots = Managers.player:bots()
	local num_bots = #bots
...
function DeusMechanism:_send_run_tracking_data(game_won)
...
function DeusMechanism:debug_load_level(level_name)
	...
	if level_settings and level_settings.hub_level then
		...
		local run_seed = "DEBUG_SPECIFIC_NODE" .. (
		script_data.deus_force_load_run_progress or 0) ..
		"_" ..
		level_name ..
		"SEED" ..
		0 ..
...
function DeusMechanism:debug_load_deus_level(level_name, difficulty, progress, level_seed, with_belakor)
	local run_seed = "DEBUG_SPECIFIC_NODE" ..
...
function DeusMechanism:_debug_load_seed(run_seed, difficulty, with_belakor)
	local run_id = string.sub(tostring(math.random_seed()), 0, 8)
...
function DeusMechanism:_setup_run(run_id, run_seed, is_server, server_peer_id, difficulty, journey_name, dominant_god, with_belakor)
	...
	for _, career in ipairs(DEUS_CAREERS) do
		...
		for slot, backend_id in pairs(slots) do
			if slot == "slot_melee" or slot == "slot_ranged" then
				...
				local item = DeusWeaponGeneration.generate_item_from_item_key(
				deus_item_key,
				difficulty,
				0,
				"plentiful",
	...
	self._deus_run_controller = DeusRunController:new(
	run_id,
	is_server,
	network_handler,
	server_peer_id,
	...
	self._deus_run_controller:setup_run(run_seed,
	difficulty,
	journey_name,
	dominant_god,
	rolled_over_coins,
...
function DeusMechanism:_update_own_avatar_info()
...
function DeusMechanism:rpc_deus_setup_run(channel_id, run_id, run_seed, difficulty_id, journey_name_id, dominant_god_id, with_belakor)
...
function DeusMechanism:should_play_level_introduction()
	return false
end

function DeusMechanism:set_vote_data(data)
	self._vote_data = data
end

function DeusMechanism:_get_vote_data()
	return self._vote_data
end

function DeusMechanism:get_loading_tip()
...
function DeusMechanism:post_match()
	return self._post_match
end

function DeusMechanism:get_level_dialogue_context()
...
function DeusMechanism:_update_current_state()
	local new_state = nil
...
function DeusMechanism:get_player_level_fallback(player)
