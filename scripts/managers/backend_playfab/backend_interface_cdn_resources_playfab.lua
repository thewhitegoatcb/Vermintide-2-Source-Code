...
local LOCALIZATION_STATUSES = { failed = 2, loaded = 1 }





function BackendInterfaceCdnResourcesPlayFab:init(backend_mirror)
	self._backend_mirror = backend_mirror
	self._url_cache = { }
	self._localization_status = { }
end

function BackendInterfaceCdnResourcesPlayFab:ready()
	return true
end

function BackendInterfaceCdnResourcesPlayFab:load_backend_localizations(language_id, external_cb)
...
function BackendInterfaceCdnResourcesPlayFab:_cb_localization_urls_loaded(localizations_to_load, external_cb, result)
...
function BackendInterfaceCdnResourcesPlayFab:_cb_localization_loaded(key, external_cb, success, code, headers, data)
	if success then
...
function BackendInterfaceCdnResourcesPlayFab:has_localization_loaded(key)
	return self._localization_status [key] == LOCALIZATION_STATUSES.loaded
end

function BackendInterfaceCdnResourcesPlayFab:has_localization_failed(key)
	return self._localization_status [key] == LOCALIZATION_STATUSES.failed
end

function BackendInterfaceCdnResourcesPlayFab:get_resource_urls(resource_ids, external_cb)
	local result_table = { }
...
function BackendInterfaceCdnResourcesPlayFab:_request_resource_urls(resource_id_chunks, result_table, external_cb)
	local resource_ids = table.remove(resource_id_chunks)

	local request = { FunctionName = "getResourceURL",
...
function BackendInterfaceCdnResourcesPlayFab:_request_resource_urls_cb(resource_id_chunks, result_table, external_cb, response)
...
function BackendInterfaceCdnResourcesPlayFab:_add_url_to_cache(resource_id, url, seconds_to_live)
	local expire_time = nil
	if seconds_to_live then
		expire_time = os.time() + seconds_to_live
	end
	self._url_cache [resource_id] = { url = url, expire_time = expire_time }
end

function BackendInterfaceCdnResourcesPlayFab:_get_url_from_cache(resource_id)
