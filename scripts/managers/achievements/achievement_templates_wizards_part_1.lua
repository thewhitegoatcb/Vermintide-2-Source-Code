...
local XB1_ACHIEVEMENT_ID = { trail_sleigher = 104, trail_shatterer = 102, trail_beacons_are_lit = 105, onions_complete_trail_legend = 106, trail_cog_strike = 103 }
...
local PS4_ACHIEVEMENT_ID = { trail_beacons_are_lit = "087" }




local all_difficulties = { }
local portals = { LevelSettings.dlc_wizards_trail }
...
achievements.trail_cog_strike = { name = "achv_onions_cog_strike_name", display_completion_ui = true, icon = "achievement_wizards_trail_push_enemies_with_cog", desc = "achv_onions_cog_strike_desc",




	events = { "on_trail_cog_strike", "on_trail_cog_reset_stat" },
...
achievements.trail_shatterer = { name = "achv_onions_icicles_name", display_completion_ui = true, icon = "achievement_wizards_trail_break_icicles", desc = "achv_onions_icicles_desc",
...
achievements.trail_sleigher = { name = "achv_onions_sleigh_kills_name", display_completion_ui = true, icon = "achievement_wizards_trail_kill_enemies_with_sleigh", desc = "achv_onions_sleigh_kills_desc",
...
achievements.trail_beacons_are_lit = { name = "achv_onions_light_beacons_name", display_completion_ui = true, icon = "achievement_wizards_trail_light_bonfires", desc = "achv_onions_light_beacons_desc",
	...
	progress = function (statistics_db, stats_id, template_data)
		...
		if watch_tower_beacon > 1 then watch_tower_beacon = 1 end
		if river_path_beacon > 1 then river_path_beacon = 1 end
		if look_out_beacon > 1 then look_out_beacon = 1
		end
		local number_of_beacons = 0
		number_of_beacons = watch_tower_beacon + river_path_beacon + look_out_beacon
		return { number_of_beacons, 3 }
	end,
	completed = function (statistics_db, stats_id, template_data)
		...
		if watch_tower_beacon >= 1 and
		river_path_beacon >= 1 and
		look_out_beacon >= 1 then
			return true
		end
	end }
