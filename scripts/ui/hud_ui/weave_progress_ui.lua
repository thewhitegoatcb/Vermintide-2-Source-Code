...
function WeaveProgressUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function WeaveProgressUI:destroy() return end


function WeaveProgressUI:_create_ui_elements()
...
function WeaveProgressUI:_sync_weave_objectives()
...
function WeaveProgressUI:update(dt, t)


	if not self._initiated then self:_sync_weave_objectives() end
...
local function objective_sort_func(a, b) return a.sort_index < b.sort_index end

local TEMP_TABLE = { }
local OBJECTIVES_TO_ADD = { }
local STACKS_ADDED = { }
function WeaveProgressUI:_update_bonus_objectives(dt, t)
	...
	if weave_objective_system then
		...
		for _, objective in pairs(current_objectives) do
			...
			if not bonus_objective_lookup [objective_name] then
				...
				if objective_settings then
					...
					OBJECTIVES_TO_ADD [#OBJECTIVES_TO_ADD + 1] = { sort_index = sort_index, objective = objective }
				end
			end
		end
		...
		for objective_name, widget in pairs(bonus_objective_lookup) do
			...
			if not TEMP_TABLE [objective_name] and not widget.content:is_done_func(objective_name) and
			self:_handle_stacks(widget, objective_name) then
...
function WeaveProgressUI:_handle_stacks(widget, objective_name)
...
function WeaveProgressUI:_update_animations(dt)
...
function WeaveProgressUI:_update_bar(dt, t)
	...
	if weave_name then
		...
		if not old_progress or old_progress < self._progress then
			...
			function self._animation_callbacks.update_bar_glow()
...
function WeaveProgressUI:_draw(dt, t)
