...
local RPCS = { "rpc_trigger_local_afk_system_message", "rpc_follow_to_lobby" }
...
function StateInGameRunning:on_enter(params)
	...
	if self.is_server and Managers.state.room and
	not Managers.state.room:has_room(peer_id) then
	...
	if
	not quickplay_bonus and self.game_mode_key == "weave" then
...
function StateInGameRunning:_setup_end_of_level_UI()
	...
	elseif not Managers.state.game_mode:setting("skip_level_end_view") then
		...
		if not self._booted_eac_untrusted then
			...
			level_end_view_context.rewards = {
				...
				level_start = { level, start_experience, start_experience_pool },
...
function StateInGameRunning:level_end_view_wrapper()
	return self._level_end_view_wrapper
end

function StateInGameRunning:handle_end_conditions()
	local game_mode_manager = Managers.state.game_mode
	if game_mode_manager and game_mode_manager:is_game_mode_ended() and
	game_mode_manager:is_game_mode_ended() then --[[ Nothing --]]
	end
...
MOOD_BLACKBOARD = { }
function StateInGameRunning:setup_mood_blackboard()
	for mood, _ in pairs(MoodSettings) do
		MOOD_BLACKBOARD [mood] = false
	end
end

function StateInGameRunning:check_invites()
	...
	if invite_data then
		...
		elseif not self.popup_id then
			Managers.matchmaking:request_join_lobby(invite_data, { friend_join = true })
		end
	end

end

function StateInGameRunning:wanted_transition()
	...
	if
	not wanted_transition and self.afk_kick then
	...
	wanted_transition or Managers.state.game_mode:wanted_transition()


	if wanted_transition and IS_XB1 and

	not self.is_in_inn and not self.is_in_tutorial then if Development.parameter("auto-host-level") ~= nil then --[[ Nothing --]]
...
function StateInGameRunning:event_end_screen_ui_complete()
...
function StateInGameRunning:gm_event_end_conditions_met(reason, checkpoint_available, percentages_completed)
	...
	elseif game_won then
		...
		is_final_objective and Managers.mechanism.on_final_round_won then
	...
	local function callback(status)
		if is_game_mode_weave and
		not is_booted_unstrusted and game_won and is_final_objective and is_server and not self.is_quickplay then
...
function StateInGameRunning:_award_end_of_level_rewards(statistics_db, stats_id, game_won, difficulty_key)
...
function StateInGameRunning:_get_weave_scores()
	local weave_manager = Managers.weave
	local weave_tier = weave_manager:get_weave_tier()
	local score = weave_manager:get_score()
	local num_players = weave_manager:get_num_players()
...
function StateInGameRunning:_submit_weave_scores(weave_tier, score, num_players)
	local weave_interface = Managers.backend:get_interface("weaves")
	weave_interface:submit_scores(weave_tier, score, num_players)
end

function StateInGameRunning:on_checkpoint_vote_cancelled()
...
function StateInGameRunning:update(dt, t)
	...
	if ingame_ui then
		...
		local rewards_ready = self._booted_eac_untrusted or

		self.rewards:rewards_generated() and
		not self.rewards:consuming_deed() and
		self.chests_package_name and Managers.package:has_loaded(self.chests_package_name, "global")
...
function StateInGameRunning:check_for_new_quests_or_contracts(dt)
...
function StateInGameRunning:disable_ui()
	Managers.ui:set_ingame_ui_enabled(false)
end
...
function StateInGameRunning:event_realtime_multiplay(active)
	if
	active and (self.is_in_tutorial or self.is_in_inn) then
		return
	end
...
function StateInGameRunning:cb_loading_view_fade_in_done()
	Managers.transition:fade_out(GameSettings.transition_fade_out_speed, nil)
	self.show_loading_view = false
end

function StateInGameRunning:update_mood(dt, t)
...
function StateInGameRunning:trigger_xbox_multiplayer_round_end_events()
...
function StateInGameRunning:event_game_started()
...
if IS_XB1 then
	function StateInGameRunning:event_trigger_xbox_round_end()
		self:_xbone_end_of_round_events(self.statistics_db)
	end

	function StateInGameRunning:_xbone_round_start_events()
		...
		if not self._xbox_event_init_triggered then
			...
			local multiplayer_round_start_table = {
				...
				Managers.account:round_id(), 0,

				Managers.account:player_session_id(),
				MultiplayerSession.multiplayer_correlation_id(session_id), 0, 0, 0 }





			Managers.transition:set_multiplayer_values("start", { xuid = Managers.account:xbox_user_id(), round_id = Managers.account:round_id(), player_session_id = Managers.account:player_session_id(), correlation_id = MultiplayerSession.multiplayer_correlation_id(session_id) }, string.format("[StateInGameRunning] Writing MultiplayerRoundStart. CorrelationID: %s. RoundID: %s", tostring(MultiplayerSession.multiplayer_correlation_id(session_id)), tostring(Managers.account:round_id())))
	...
	function StateInGameRunning:_xbone_end_of_round_events(statistics_db)
		if self.is_in_inn or self.is_in_tutorial or Development.parameter("auto-host-level") ~= nil or not Managers.account:is_online() then
			return
		end
		...
		if not self._xbox_event_end_triggered then
			...
			local multiplayer_round_end_table = {
				...
				math.floor(Managers.time:time("game")), 0 }



			Managers.transition:set_multiplayer_values("end", { xuid = Managers.account:xbox_user_id(), round_id = Managers.account:round_id(), player_session_id = Managers.account:player_session_id(), correlation_id = MultiplayerSession.multiplayer_correlation_id(session_id), time = Managers.time:time("game") }, string.format("[StateInGameRunning] Writing MultiplayerRoundEnd. CorrelationID: %s. RoundID: %s", tostring(MultiplayerSession.multiplayer_correlation_id(session_id)), tostring(Managers.account:round_id())))
...
function StateInGameRunning:_check_black_screen_transition_requirements()
...
function StateInGameRunning:event_conflict_director_setup_done()
	self._conflict_directory_is_ready = true
end



function StateInGameRunning:_catchup_framerate_before_starting()
...
function StateInGameRunning:_update_catchup_framerate_before_starting()
	if self._frame_catchup_counter == nil then
		return
	end
...
function StateInGameRunning:_game_actually_starts()
...
function StateInGameRunning:update_player_afk_check(dt, t)
	do return end
...
function StateInGameRunning:_show_afk_warning()
...
function StateInGameRunning:rpc_trigger_local_afk_system_message(channel_id, message_id, peer_id)
	if self.is_server then
...
function StateInGameRunning:_cancel_afk_warning()
	if self.afk_popup_id then
...
function StateInGameRunning:_handle_afk_warning_result()
	if self.afk_popup_id then
...
function StateInGameRunning:_kick_afk_player()
	self:_cancel_afk_warning()
	local player = Managers.player:local_player(1)
...
function StateInGameRunning:transitioned_from_black_screen()
	return self._transitioned_from_black_screen
end

function StateInGameRunning:rpc_follow_to_lobby(channel_id, lobby_type, lobby_to_join)
	...
	local lobby_join_data = { join_method = "party" }
	...
	local state_context_params = { friend_join = true }
