...
local NAV_TAG_ALLOWED_LAYERS = { planks = 10, ledges_with_fence = 10, doors = 10, jumps = 10, ledges = 10, bot_poison_wind = 15, fire_grenade = 15, bot_ratling_gun_fire = 15 }
...
local NAV_COST_MAP_ALLOWED_LAYERS = { plague_wave = 15, troll_bile = 15, lamp_oil_fire = 15, warpfire_thrower_warpfire = 15, stormfiend_warpfire = 20 }
...
AIGroupTemplates.spline_patrol = { in_patrol = true,
	...
	update = function (world, nav_world, group, t, dt)
		...
		if state == "find_path_entry" then --[[ Nothing --]]
		...
		elseif state == "in_combat" then --[[ Nothing --]]
		end
...
local function find_patrol_spline(world, group)
	...
	local node_data = { forward_list = { }, reversed_list = { } }
...
local function detect_jump_points(nav_world, group)
	...
	local spline_curve = anchor.spline
	local movement = spline_curve:movement()
	local start_index = 2 local end_index = 3
	...
	for i = 1, num_splines do
		...
		local start_point = points [start_index]:unbox() local end_point = points [end_index]:unbox()
		...
		if not success then
			...
			jump_points [i] = { forward = { next_t = 1, start_subdivision_index = 1, next_subdivsion_index = num_subdivisions },
				reversed = { next_t = 0, next_subdivsion_index = 1, start_subdivision_index = num_subdivisions } }
		end
	end
...
function init_group(nav_world, group, world, t)
	...
	for i = 1, num_anchors do
		...
		for c = 1, num_columns do
			...
			anchor_start_direction =
			anchor_start_direction or data.start_direction:unbox()
		end
		...
		anchors [i].wanted_offset = { anchor_offset_y, anchor_offset_y }
	end
...
function debug_draw_formation(group)
	local drawer = Managers.state.debug:drawer({ mode = "retained", name = "patrol_retained" })
...
local function get_spline_distance_between_anchors(from_anchor, to_anchor)
	local from_spline = from_anchor.spline local to_spline = to_anchor.spline
	local from_spline_movement = from_anchor.spline:movement() local to_spline_movement = to_anchor.spline:movement()
...
function update_units(nav_world, group, t, dt)
	...
	for i = 1, num_indexed_members do repeat
			local unit = indexed_members [i]
			local blackboard = BLACKBOARDS [unit]
			...
			if success then
				navigation_extension:move_to(destination) end until true
	end
	...
	for i = 1, num_anchors do repeat
			...
			if status == "end" then
				...
				elseif despawn_at_end then
					...
					for j, unit in pairs(anchor_units) do
						local blackboard = BLACKBOARDS [unit]
						conflict_director:destroy_unit(unit, blackboard, "patrol_finished") end end end until true
	end
...
function update_anchor_positions(nav_world, group)
	local anchor_offset = group.formation_settings.offsets.ANCHOR_OFFSET
	local check1_up = 0.6 local check1_down = 1
	local check2_up = 1.2 local check2_down = 1
	local check2_side = 1 local check2_obstacle_distance = 1
