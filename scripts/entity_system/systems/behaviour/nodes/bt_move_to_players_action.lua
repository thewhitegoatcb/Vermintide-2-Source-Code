...
function BTMoveToPlayersAction:init(...)
	BTMoveToPlayersAction.super.init(self, ...)
end

BTMoveToPlayersAction.name = "BTMoveToPlayersAction"

function BTMoveToPlayersAction:enter(unit, blackboard, t)
...
function BTMoveToPlayersAction:_init_targets(data, t, unit, blackboard)
	data.index = 0
...
function BTMoveToPlayersAction:leave(unit, blackboard, t, reason, destroy)
...
function BTMoveToPlayersAction:run(unit, blackboard, t, dt)
...
function BTMoveToPlayersAction:_evalute_targets(unit, blackboard, data, t)
	...
	repeat
		...
		next_target_unit = data.target_units [next_index] until
	next_target_unit == nil or Unit.alive(next_target_unit)
...
function BTMoveToPlayersAction:_find_target_globadier(unit, blackboard, action, next_target_unit, t)

	local throw_globe_data = blackboard.throw_globe_data
	if throw_globe_data and throw_globe_data.next_throw_at and
	blackboard.target_dist < 4 then
	...
	if self:_valid_globadier_target(next_target_unit, blackboard, blackboard.target_dist, action) and self:_has_line_of_sight(unit, next_target_unit, blackboard.world, t) then
		...
		if has_trajectory then
			...
			if not blackboard.throw_globe_data then local throw_data = { throw_pos = Vector3Box(), target_direction = Vector3Box() } end
...
function BTMoveToPlayersAction:_find_target_ratling_gunner(unit, blackboard, action, next_target_unit, t)
...
function BTMoveToPlayersAction:_update_move_to_players_position(blackboard, navigation_extension, wanted_position, data)
...
function BTMoveToPlayersAction:_calculate_trajectory_to_target(unit, world, target_unit, attack_throw_offset, max_speed)
...
function BTMoveToPlayersAction:_valid_globadier_target(target_unit, blackboard, target_distance, action)
	local side = blackboard.side
	return side.VALID_ENEMY_TARGETS_PLAYERS_AND_BOTS [target_unit] and target_distance < action.attack_distance
end

function BTMoveToPlayersAction:_has_line_of_sight(unit, target_unit, world, t)
	...
	local hit, position1, distance1, normal, actor1 = PhysicsWorld.immediate_raycast(
...
function BTMoveToPlayersAction:start_idle_animation(unit, blackboard)
	local network_manager = Managers.state.network
	network_manager:anim_event(unit, "idle")
	blackboard.move_state = "idle"
end

function BTMoveToPlayersAction:start_move_animation(unit, blackboard)
	local network_manager = Managers.state.network
