...
function PassiveAbilityEngineer:init(extension_init_context, unit, extension_init_data, ability_init_data)
...
function PassiveAbilityEngineer:extensions_ready(world, unit)
...
function PassiveAbilityEngineer:destroy()
	self:_unregister_events()
	self._game_object_id = nil
end

function PassiveAbilityEngineer:_register_events()
...
function PassiveAbilityEngineer:update(dt, t)
...
function PassiveAbilityEngineer:_update_career_weapon_particles(inventory_extension)
...
function PassiveAbilityEngineer:_update_career_weapon(weapon_unit)
...
function PassiveAbilityEngineer:_update_weapon_anim_variables(dt)
...
function PassiveAbilityEngineer:on_engineer_weapon_fire(heat_to_add)
...
function PassiveAbilityEngineer:on_engineer_weapon_spin_up(custom_progress, ignore_anim_fixup)
	local t = Managers.time:time("game")
...
function PassiveAbilityEngineer:on_level_start_local_player_spawned(initial_spawn)
	if self._is_local_player and not self._game_object_id then
		self:create_game_object()
	end
end

function PassiveAbilityEngineer:create_game_object()
...
function PassiveAbilityEngineer:on_talents_changed(unit, talent_extension, is_spawning)
...
function PassiveAbilityEngineer:_add_5_2_bombs(is_spawning)
...
function PassiveAbilityEngineer:set_career_game_object_id(go_id)
	self._game_object_id = go_id
end

function PassiveAbilityEngineer:cb_game_session_disconnect()
	self._game_object_id = nil
end