...
AchievementTemplateHelper.rarity_index = { common = 2, plentiful = 1, exotic = 4, rare = 3, unique = 5 }
...
function AchievementTemplateHelper.check_level(statistics_db, stats_id, level_id)
...
function AchievementTemplateHelper.check_level_list(statistics_db, stats_id, levels_to_complete)
	...
	for i = 1, #levels_to_complete do
		...
		local level_stat = statistics_db:get_persistent_stat(stats_id, "completed_levels", level_id)
		if not level_stat or level_stat == 0 then return false
		end
	end
	return true
end

function AchievementTemplateHelper.rpc_increment_stat(unit, stat_name)
...
function AchievementTemplateHelper.rpc_modify_stat(unit, stat_name, amount)
	local player = Managers.player:unit_owner(unit)
	if player and not player.bot_player then
		local peer_id = player:network_id()
		local network_manager = Managers.state.network
...
function AchievementTemplateHelper.check_level_difficulty(statistics_db, stats_id, level_id, difficulty_rank, career, streak)
...
function AchievementTemplateHelper.check_level_table_difficulty(statistics_db, stats_id, level_to_complete, difficulty_rank, career)
...
function AchievementTemplateHelper.check_level_list_difficulty(statistics_db, stats_id, levels_to_complete, difficulty_rank, career)
	assert(type(levels_to_complete) == "table" and #levels_to_complete > 0, "levels_to_complete needs to be a list of levels with at least 1 element")
...
function AchievementTemplateHelper.hero_level(hero_name)
	local experience = ExperienceSettings.get_experience(hero_name)
	return ExperienceSettings.get_level(experience)
end


local equipment_slots = { "melee", "ranged", "necklace", "ring", "trinket" }
...
function AchievementTemplateHelper.equipped_items_of_rarity(statistics_db, stats_id, required_rarity)
...
function AchievementTemplateHelper.add_stat_count_challenge(achievements, id, stat_name, count, career, icon, dlc, id_xb1, id_ps4)
	achievements [id] = { display_completion_ui = true,
		name = "achv_" .. id .. "_name",
		desc = function ()
		...
		completed = function (statistics_db, stats_id)
			if career then
				do return count <= statistics_db:get_persistent_stat(stats_id, stat_name, career) end
			else
...
function AchievementTemplateHelper.add_health_challenge(achievements, id, career, threshold, icon, dlc, id_xb1, id_ps4)
	achievements [id] = { display_completion_ui = true,
		name = "achv_" .. id .. "_name",
		desc = "achv_" .. id .. "_desc",
		icon = icon or "achievement_trophy_" .. id,
...
function AchievementTemplateHelper.add_weapon_kills_per_breeds_challenge(achievements, id, weapons, breeds_to_kill, count, icon, dlc, show_complete, id_xb1, id_ps4)
...
function AchievementTemplateHelper.add_career_mission_count_challenge(achievements, id, stat_name, career_name, difficulty_ranks, count, min_health, icon, dlc, id_xb1, id_ps4)
	achievements [id .. "_" .. career_name] = { display_completion_ui = true,
...
function AchievementTemplateHelper.add_multi_stat_count_challenge(achievements, id, stat_names, count, icon, dlc, id_xb1, id_ps4)
	achievements [id] = { display_completion_ui = true,
		name = "achv_" .. id .. "_name",
		desc = "achv_" .. id .. "_desc",
		icon = icon or "achievement_trophy_" .. id,
...
function AchievementTemplateHelper.add_weapon_kill_challenge(achievements, id, weapon, count, icon, dlc, id_xb1, id_ps4)
...
function AchievementTemplateHelper.add_weapon_levels_challenge(achievements, id, weapon, levels, difficuty, icon, dlc, id_xb1, id_ps4)
	local stat_names = { }
	...
	local template = {
		...
		requirements = function (statistics_db, stats_id)
			...
			for i = 1, num_levels do
				...
				local entry = { name = level_display_name, completed = rank <= statistics_db:get_persistent_stat(stats_id, stat_names [i]) }
...
function AchievementTemplateHelper.add_event_challenge(achievements, id, icon, description_args, dlc, id_xb1, id_ps4)
	local template = { display_completion_ui = true,
		name = "achv_" .. id .. "_name",
		icon = icon or "achievement_trophy_" .. id,
	...
	if description_args then
		function template.desc() return string.format(Localize(desc_id), unpack(description_args)) end
	else
...
function AchievementTemplateHelper.add_levels_complete_challenge(achievements, id, levels, difficulty_rank, icon, dlc, id_xb1, id_ps4)
	local num_levels = levels and #levels or 0
	local template = {
		name = "achv_" .. id .. "_name",
		desc = "achv_" .. id .. "_desc",
	...
	if num_levels > 1 then
		function template.progress(statistics_db, stats_id)
			local count = 0
			for i = 1, num_levels do
				if AchievementTemplateHelper.check_level_table_difficulty(statistics_db, stats_id, levels [i], difficulty_rank) then
					count = count + 1
		...
		function template.requirements(statistics_db, stats_id)
			local out_table = { }
			for i = 1, num_levels do
				local entry = { name = levels [i].display_name, completed = AchievementTemplateHelper.check_level_table_difficulty(statistics_db, stats_id, levels [i], difficulty_rank) }
				table.insert(out_table, entry)
			end
			return out_table
		end
...
function AchievementTemplateHelper.add_levels_complete_per_hero_challenge(achievements, id, levels, difficulty_rank, career_name, streak, icon, dlc, id_xb1, id_ps4)
	...
	if num_levels > 1 then
		function template.progress(statistics_db, stats_id)
			...
			for i = 1, num_levels do
				if AchievementTemplateHelper.check_level_list_difficulty(statistics_db, stats_id, { levels [i] }, difficulty_rank, career_name, streak) then
					num_completed = num_completed + 1
				end
			end
			return { num_completed, num_levels }
		end
		...
		function template.requirements(statistics_db, stats_id)
			...
			local out_table = { }
			for i = 1, num_levels do
				local entry = { name = LevelSettings [levels [i]].display_name, completed = AchievementTemplateHelper.check_level_list_difficulty(statistics_db, stats_id, { levels [i] }, difficulty_rank, career_name, streak) }
				table.insert(out_table, entry)
			end
			return out_table
		end
...
function AchievementTemplateHelper.add_meta_challenge(achievements, id, achievement_ids, icon, dlc, id_xb1, id_ps4)
	local template = { display_completion_ui = true,
		name = "achv_" .. id .. "_name",
		desc = "achv_" .. id .. "_desc",
		icon = icon or "achievement_trophy_" .. id,
...
function AchievementTemplateHelper.add_console_achievements(xb1_achievements, ps4_achievements)
