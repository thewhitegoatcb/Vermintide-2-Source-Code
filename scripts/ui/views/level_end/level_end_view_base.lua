...
local RPCS = { "rpc_signal_end_of_level_done", "rpc_notify_lobby_joined" }
...
function LevelEndViewBase:init(context)
	...
	self.render_settings = { alpha_multiplier = 0, snap_pixel_positions = true }
	...
	self._state_machine_params = { parent = self, context = context, game_won = game_won, game_mode_key = self.game_mode_key }
...
function LevelEndViewBase:register_rpcs(network_event_delegate)
	self._registered_rpcs = true
	network_event_delegate:register(self, unpack(RPCS))
	self._network_event_delegate = network_event_delegate
end

function LevelEndViewBase:unregister_rpcs()
	self._network_event_delegate:unregister(self)
	self._network_event_delegate = nil
	self._registered_rpcs = false
end

function LevelEndViewBase:enable_chat()
	return true
end

function LevelEndViewBase:start()
...
function LevelEndViewBase:on_enter()
	self._state_speed_mult = 1
end

function LevelEndViewBase:on_exit()
...
function LevelEndViewBase:_vote_to_leave_game()
	local voting_manager = Managers.state.voting
	voting_manager:vote(1)
	self._voted = true
end

function LevelEndViewBase:exit_to_game()

	self._exit_timer = 2
	self._started_exit = true
end

function LevelEndViewBase:done()
	return self._wants_to_exit_to_game
end

function LevelEndViewBase:setup_pages(game_won, rewards)
	local index_by_state_name = { }
	return index_by_state_name
end

function LevelEndViewBase:create_ui_elements()
	return end

function LevelEndViewBase:_activate_viewport()
	local world, viewport = self:get_viewport_world()
	ScriptWorld.activate_viewport(world, viewport)
end


function LevelEndViewBase:get_world_link_unit()
...
function LevelEndViewBase:get_viewport_world()
	return self._world, self._world_viewport
end

function LevelEndViewBase:post_update()
	return end

function LevelEndViewBase:update(dt, t)
...
function LevelEndViewBase:transitioning()
	return self.exiting
end

function LevelEndViewBase:left_lobby()
...
function LevelEndViewBase:destroy()
...
function LevelEndViewBase:play_sound(event)
	WwiseWorld.trigger_event(self.wwise_world, event)
end

function LevelEndViewBase:_is_button_pressed(widget)
...
function LevelEndViewBase:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_enter
end

function LevelEndViewBase:_is_page_selector_pressed()
...
function LevelEndViewBase:_set_page_selector_selection(index)
	local widget = self._page_selector_widget
	local content = widget.content
	local amount = content.amount
...
function LevelEndViewBase:_update_exit(dt)
...
function LevelEndViewBase:do_retry()
	return false
end
...
function LevelEndViewBase:_get_level_up_rewards()
...
function LevelEndViewBase:_get_win_track_rewards()
	local end_of_level_rewards = self.context.rewards.end_of_level_rewards
...
function LevelEndViewBase:_get_deed_rewards()
	local end_of_level_rewards = self.context.rewards.end_of_level_rewards
	local deed_rewards = { }
...
function LevelEndViewBase:_get_event_rewards()
	local end_of_level_rewards = self.context.rewards.end_of_level_rewards
	local event_rewards = { }
...
function LevelEndViewBase:_get_deus_rewards()
	local end_of_level_rewards = self.context.rewards.end_of_level_rewards
	local deus_rewards = { }
...
function LevelEndViewBase:_get_keep_decoration_rewards()
	local end_of_level_rewards = self.context.rewards.end_of_level_rewards
...
function LevelEndViewBase:_present_reward(data)
...
function LevelEndViewBase:_handle_queued_presentations()
...
function LevelEndViewBase:displaying_reward_presentation()
	return self.reward_popup:is_presentation_active()
end

function LevelEndViewBase:_is_reward_presentation_complete()
	return self.reward_popup:is_presentation_complete()
end

function LevelEndViewBase:reward_presentation_done()
	return self._reward_presentation_done
end

function LevelEndViewBase:present_level_up(hero_name, hero_level)
		...
		for index, template in ipairs(level_unlocks) do
			...
			if title and description then
				entry [#entry + 1] = { widget_type = "description",
					value = { Localize(title), Localize(description) } }


			elseif title then
				entry [#entry + 1] = { widget_type = "title",
...
function LevelEndViewBase:present_win_track_reward(idx)
	local item_interface = Managers.backend:get_interface("items")
...
function LevelEndViewBase:present_additional_rewards()
...
function LevelEndViewBase:present_chest_rewards()
	local end_of_level_rewards = self.context.rewards.end_of_level_rewards
	local item_interface = Managers.backend:get_interface("items")
	...
	if chest then
		...
		local presentation_data = { {
				{ widget_type = "description",
					...
					value = { Localize(display_name), Localize("end_screen_chest_received") } },
...
function LevelEndViewBase:wanted_menu_state()
	return self._wanted_menu_state
end

function LevelEndViewBase:clear_wanted_menu_state()
	self._wanted_menu_state = nil
end

function LevelEndViewBase:_request_state_change(state_name)
...
function LevelEndViewBase:_handle_state_exit()
	local state_machine = self._machine
	if not state_machine then
		return
	end
...
function LevelEndViewBase:_setup_state_machine(optional_start_state_name, initial)
	if self._machine then
		self._machine:destroy()
		self._machine = nil
	end
...
function LevelEndViewBase:_handle_state_auto_change()
	local state_machine = self._machine
	if not state_machine then
		return
	end
...
function LevelEndViewBase:_proceed_to_next_auto_state(index, num_states)
...
function LevelEndViewBase:rpc_signal_end_of_level_done(channel_id, peer_id, do_reload)
...
function LevelEndViewBase:signal_done(do_reload)
...
function LevelEndViewBase:peer_signaled_done(peer_id, do_reload)
...
function LevelEndViewBase:rpc_notify_lobby_joined(channel_id)
	if self.is_server then
...
function LevelEndViewBase:start_force_shutdown()
...
function LevelEndViewBase:get_force_shutdown_time()
	return self._force_shutdown_timer, self._force_shutdown_timer_start
end

function LevelEndViewBase:is_force_shutdown_active()
	return self._started_force_shutdown
end

function LevelEndViewBase:update_force_shutdown(dt)
...
local cam_shake_settings = { persistance = 1, fade_out = 0.5, amplitude = 0.9, seed = 0, duration = 0.5, fade_in = 0.1, octaves = 7 }
...
function LevelEndViewBase:setup_camera()
...
function LevelEndViewBase:add_camera_shake(settings, start_time, scale)
	local data = { }
...
function LevelEndViewBase:_apply_shake_event(settings, t)
	local active_camera_shakes = self._active_camera_shakes
...
function LevelEndViewBase:_calculate_perlin_value(x, settings)
	local total = 0
...
function LevelEndViewBase:_interpolated_noise(x, settings)
...
function LevelEndViewBase:_smoothed_noise(x, settings)
	return self:_noise(x, settings) / 2 + self:_noise(x - 1, settings) / 4 + self:_noise(x + 1, settings) / 4
end

function LevelEndViewBase:_noise(x, settings)
...
function LevelEndViewBase:set_camera_position(position)
	local _, viewport = self:get_viewport_world()
	local camera = ScriptViewport.camera(viewport)
	return ScriptCamera.set_local_position(camera, position)
end

function LevelEndViewBase:set_camera_rotation(rotation)
	local _, viewport = self:get_viewport_world()
	local camera = ScriptViewport.camera(viewport)
	return ScriptCamera.set_local_rotation(camera, rotation)
end

function LevelEndViewBase:get_camera_position()
	local _, viewport = self:get_viewport_world()
	local camera = ScriptViewport.camera(viewport)
	return ScriptCamera.position(camera)
end

function LevelEndViewBase:get_camera_rotation()
	local _, viewport = self:get_viewport_world()
	local camera = ScriptViewport.camera(viewport)
	return ScriptCamera.rotation(camera)
end

function LevelEndViewBase:position_camera(optional_pose, fov)
	local world, viewport = self:get_viewport_world()
...
function LevelEndViewBase:set_camera_zoom(progress)
...
function LevelEndViewBase:_setup_viewport_camera()
	local world, viewport = self:get_viewport_world()
...
function LevelEndViewBase:_push_mouse_cursor()
...
function LevelEndViewBase:_pop_mouse_cursor()
...
function LevelEndViewBase:set_input_manager(input_manager)
...
function LevelEndViewBase:setup_world(context)
...
function LevelEndViewBase:destroy_world()
...
function LevelEndViewBase:get_world_flags()
	local flags = { Application.DISABLE_SOUND, Application.DISABLE_ESRAM, Application.ENABLE_VOLUMETRICS }
...
function LevelEndViewBase:create_world(context)
...
function LevelEndViewBase:create_viewport(context, world)
...
function LevelEndViewBase:spawn_level(context, world)
	local level_name = "levels/end_screen/world"
	local object_sets = { }
...
function LevelEndViewBase:create_ui_renderer(context, world, top_world)
	local materials = { "material", "materials/ui/ui_1080p_hud_atlas_textures", "material", "materials/ui/ui_1080p_hud_single_textures", "material", "materials/ui/ui_1080p_menu_atlas_textures", "material", "materials/ui/ui_1080p_menu_single_textures", "material", "materials/ui/ui_1080p_common", "material", "materials/fonts/gw_fonts" }
