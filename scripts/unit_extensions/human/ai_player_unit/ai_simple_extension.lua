...
function AISimpleExtension:init(extension_init_context, unit, extension_init_data)
...
function AISimpleExtension:unit_removed_from_game()
	Managers.state.side:remove_unit_from_side(self._unit)
	self._side_id = nil
end

function AISimpleExtension:destroy()
	local blackboard = self._blackboard
	AiUtils.special_dead_cleanup(self._unit, self._blackboard)
	self._brain:destroy()
end

STATIC_BLACKBOARD_KEYS = STATIC_BLACKBOARD_KEYS or { target_dist = true, stagger_count = true, node_data = true, is_in_attack_cooldown = true, next_lean_index = true, health_extension = true, stagger_count_reset_at = true, lean_dogpile = true, next_smart_object_data = true, navigation_extension = true, locomotion_extension = true, move_orders = true, lean_slots = true, unit = true, optional_spawn_data = true, level = true, system_api = true, override_targets = true, spawn_type = true, running_nodes = true, lean_unit_list = true, group_blackboard = true, attack_cooldown_at = true, stuck_check_time = true, world = true, inventory_extension = true, is_passive = true, breed = true, nav_world = true }
...
function AISimpleExtension:freeze()
	self._brain:exit_last_action()
	self._side_id = nil
end

function AISimpleExtension:unfreeze(unit, data)
	local blackboard = self._blackboard
...
function AISimpleExtension:extensions_ready(world, unit)
	local blackboard = self._blackboard
...
function AISimpleExtension:get_overlap_context()
	if self._overlap_context then
		self._overlap_context.num_hits = 0
	else
		self._overlap_context = { has_gotten_callback = false, spine_node = false, num_hits = 0,
...
function AISimpleExtension:set_properties(params)
...
function AISimpleExtension:_parse_properties()
...
function AISimpleExtension:init_perception(breed, is_horde)
...
function AISimpleExtension:set_perception(perception_func_name, target_selection_func_name)
...
function AISimpleExtension:_init_brain(behavior, is_horde)
	self._brain = AIBrain:new(self._world, self._unit, self._blackboard, self._breed, behavior)
end

function AISimpleExtension:_set_size_variation(size_variation, size_variation_normalized)
	self._size_variation = size_variation or 1
	self._size_variation_normalized = size_variation_normalized or 1
end

function AISimpleExtension:locomotion()
	return self._locomotion
end

function AISimpleExtension:navigation()
	return self._navigation
end

function AISimpleExtension:brain()
	return self._brain
end

function AISimpleExtension:breed()
	return self._breed
end

function AISimpleExtension:blackboard()
	return self._blackboard
end

function AISimpleExtension:size_variation()
	return self._size_variation, self._size_variation_normalized
end


function AISimpleExtension:force_enemy_detection(t)
...
function AISimpleExtension:current_action_name()
	local blackboard = self._blackboard
	return blackboard.action and blackboard.action.name or "n/a"
end


function AISimpleExtension:die(killer_unit, killing_blow)
	local blackboard = self._blackboard
...
function AISimpleExtension:attacked(attacker_unit, t, damage_hit)
...
function AISimpleExtension:enemy_aggro(alerting_unit, enemy_unit)
...
function AISimpleExtension:enemy_alert(alerting_unit, enemy_unit)
...
function AISimpleExtension:increase_stagger_count()
	local blackboard = self._blackboard
	local breed = self._breed
...
function AISimpleExtension:reset_stagger_count()
	local blackboard = self._blackboard
	blackboard.stagger_count_reset_at = 0
	blackboard.stagger_count = 0
end

function AISimpleExtension:update_stagger_count()
	local blackboard = self._blackboard
