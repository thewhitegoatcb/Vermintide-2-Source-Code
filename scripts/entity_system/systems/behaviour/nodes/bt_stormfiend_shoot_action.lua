...
function BTStormfiendShootAction:init(...)
...
function BTStormfiendShootAction:enter(unit, blackboard, t)
...
function BTStormfiendShootAction:set_global_environment_intensity(unit, group_blackboard, action)
...
function BTStormfiendShootAction:_calculate_attack_animation(right_vector, forward_vector, dir, attack_anims)
...
function BTStormfiendShootAction:_calculate_aim(unit, unit_position, attack_arm, forward_vector, rotation_needed, blackboard, action, target_unit, target_position, target_direction)
	...
	local arm_collides = PhysicsWorld.immediate_raycast(physics_world,
	shoulder_pos,
	shoulder_to_muzzle_direction,
	shoulder_to_muzzle_length,
	"any",
	...
	if ray_can_go_start_to_target then
		...
		local result = PhysicsWorld.linear_sphere_sweep(physics_world,
		muzzle_pos,
		target_head_position,
		SPHERE_CAST_RADIUS,
		SPHERE_CAST_MAX_NUM_HITS,
...
function BTStormfiendShootAction:init_attack(unit, blackboard, action, t)
...
function BTStormfiendShootAction:leave(unit, blackboard, t, reason, destroy)
...
function BTStormfiendShootAction:run(unit, blackboard, t, dt)
...
function BTStormfiendShootAction:_calculate_oobb_collision(bot_threat, bot_threat_range, self_pos, self_rot)
...
function BTStormfiendShootAction:_create_bot_aoe_threat(unit, attack_rotation, bot_threat, bot_threat_range)
...
function BTStormfiendShootAction:constrain_aim(unit, blackboard, called_by_me)
	local data = blackboard.shoot_data
...
function BTStormfiendShootAction:create_firewall(unit, blackboard, data)
	...
	local extension_init_data = {
		area_damage_system = { liquid_template = "stormfiend_firewall",
			flow_dir = direction,
...
function BTStormfiendShootAction:shoot_hit_check(unit, blackboard)
	local action = blackboard.action
	local data = blackboard.shoot_data
	local attack_arm = data.attack_arm
...
function BTStormfiendShootAction:_stop_beam_sfx(unit, blackboard, shoot_data)
	local action = blackboard.action
...
function BTStormfiendShootAction:initiate_firing_warpfire_thrower(unit, blackboard)
	local action = blackboard.action
	local data = blackboard.shoot_data
...
function BTStormfiendShootAction:_fire_from_position_direction(unit, blackboard, data, dt)
...
function BTStormfiendShootAction:_update_ratling_gun(unit, blackboard, t, dt)
	local data = blackboard.shoot_data
...
function BTStormfiendShootAction:_shoot_ratling_gun(unit, blackboard, t, dt)
	local action = blackboard.action
	local data = blackboard.shoot_data
...
function BTStormfiendShootAction:initiate_firing_ratling_gun(blackboard)
	local action = blackboard.action
	local data = blackboard.shoot_data
...
function BTStormfiendShootAction:_debug_firewall(minimum_length, start_position, wanted_end_pos, projected_start_pos, projected_end_pos)
	if script_data.debug_stormfiend then
		local drawer = Managers.state.debug:drawer({ mode = "retained", name = "BTStormfiendShootAction" })
		local success_color = Colors.get("green") local neutral_color = Colors.get("yellow") local fail_color = Colors.get("red")
...
function BTStormfiendShootAction:_debug_fire_beam(start_position, end_position, success, stop_hit_index, hits, draw_mode)
	if script_data.debug_stormfiend then
		local drawer, success_color, fail_color, normal_color, stop_color, disabled_color = nil
		if draw_mode == "retained" then
			fail_color = Colors.get("red") success_color = Colors.get("green")
			disabled_color = Colors.get("gray") stop_color = Colors.get("dark_orange") normal_color = Colors.get("gold")
			drawer = Managers.state.debug:drawer({ mode = "retained", name = "BTStormfiendShootAction" })
		else
			fail_color = Colors.get("indian_red") success_color = Colors.get("light_green")
			disabled_color = Colors.get("light_gray") stop_color = Colors.get("orange") normal_color = Colors.get("yellow")
			drawer = QuickDrawer
		end
...
function BTStormfiendShootAction:_debug_colliding_arm(shoulder_position, muzzle_position)
	if script_data.debug_stormfiend then
		local drawer = Managers.state.debug:drawer({ mode = "retained", name = "BTStormfiendShootAction" })
