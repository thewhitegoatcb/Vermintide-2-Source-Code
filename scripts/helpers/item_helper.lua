...
local weapon_attack_stats_order = { speed = 2, range = 5, damage = 1, targets = 3, stagger = 4 }
...
local stats_localization_keys = { burn = "item_compare_burn", range = "item_compare_range", armor_penetration = "item_compare_armor_penetration", damage = "item_compare_damage", head_shot = "item_compare_head_shot", poison = "item_compare_poison", speed = "item_compare_attack_speed", targets = "item_compare_targets", stagger = "item_compare_stagger" }
...
function ItemHelper.get_template_by_item_name(name)
...
function ItemHelper.get_slot_type(slot)
...
function ItemHelper.mark_sign_in_reward_as_new(reward_id, item_backend_id)
...
function ItemHelper.unmark_sign_in_reward_as_new(reward_id)
...
function ItemHelper.has_new_sign_in_reward(reward_id)
	if reward_id then
		local new_sign_in_rewards = PlayerData.new_sign_in_rewards
		local reward_items = new_sign_in_rewards [reward_id]
		do return reward_items and true or false end
	else
...
function ItemHelper.mark_backend_id_as_new(backend_id, item, skip_autosave)
	...
	if skip_autosave then return
	end
	Managers.save:auto_save(SaveFileName, SaveData, nil)
end

function ItemHelper.unmark_backend_id_as_new(backend_id, skip_autosave)
...
function ItemHelper.is_new_backend_id(backend_id)
	local new_item_ids = PlayerData.new_item_ids
	return new_item_ids and new_item_ids [backend_id]
end

function ItemHelper.has_new_backend_ids_by_career_name_and_slot_type(career_name, slot_type_name, rarities_to_ignore)
	local new_item_ids_by_career = PlayerData.new_item_ids_by_career
...
function ItemHelper.has_new_backend_ids_by_slot_type(slot_type_name, rarities_to_ignore)
	local new_item_ids_by_career = PlayerData.new_item_ids_by_career
...
function ItemHelper.has_new_backend_ids_by_career_name(career_name, rarities_to_ignore)
	local new_item_ids_by_career = PlayerData.new_item_ids_by_career
...
function ItemHelper.retrieve_weapon_item_statistics(item_data, backend_id)
	local stats_data = { }
...
function ItemHelper._retrieve_weapon_attack_data(stats_data, data_store_table)
	for key, value in pairs(stats_data) do
		...
		weapon_data [#weapon_data + 1] = { key = key, title = Localize(localization_key), value = value }
		data_store_table [key] = weapon_data
	end
end

function ItemHelper.weapon_stat_order_by_type(stat_type)
	return weapon_attack_stats_order [stat_type]
end

function ItemHelper.on_inventory_item_added(item)
...
function ItemHelper.mark_backend_id_as_favorite(backend_id, item, save)
	if not item then
		local item_interface = Managers.backend:get_interface("items")
...
function ItemHelper.unmark_backend_id_as_favorite(backend_id, item)
	if not item then
		local item_interface = Managers.backend:get_interface("items")
...
function ItemHelper.is_favorite_backend_id(backend_id, item)
	if not item then
		local item_interface = Managers.backend:get_interface("items")
		item = item_interface:get_item_from_id(backend_id)
	end
...
function ItemHelper.is_equiped_backend_id(backend_id, career)
	local item_interface = Managers.backend:get_interface("items")
...
function ItemHelper.get_equipped_slots(backend_id, career_name)
	local slots = { }
	local slots_n = 0
	local item_interface = Managers.backend:get_interface("items")
...
function ItemHelper.mark_keep_decoration_as_new(keep_decoration_id)
...
function ItemHelper.unmark_keep_decoration_as_new(keep_decoration_id)
...
function ItemHelper.is_new_keep_decoration_id(keep_decoration_id)
	local new_keep_decoration_ids = PlayerData.new_keep_decoration_ids
	return new_keep_decoration_ids and new_keep_decoration_ids [keep_decoration_id]
end


ItemHelper.tab_conversions = { dlc = "dlc", weapon_skin = "cosmetics", hat = "cosmetics", bundle = "bundles", featured = "featured", skin = "cosmetics" }
...
local skip_items = { skin = true, weapon_skin = true, hat = true }
function ItemHelper.create_tab_unseen_item_stars(tab_cat)
	...
	for dlc_name, dlc_settings in pairs(StoreDlcSettingsByName) do
		...
		if
		not seen_dlc and tab_cat.dlc ~= nil then
...
function ItemHelper.update_featured_unseen(featured_peddler_items, tab_cat)
	local seen_items = PlayerData.seen_shop_items
	tab_cat.featured = 0
	for i = 1, #featured_peddler_items do
		...
		local seen_item = seen_items [peddler_item.key]
		if
		not seen_item and tab_cat.featured ~= nil then
...
function ItemHelper.set_shop_item_seen(item_key, item_type, tab_cat, optional_tab_name)
...
function ItemHelper.set_all_shop_item_seen(tab_cat)
...
function ItemHelper.is_fake_item(item_type)
