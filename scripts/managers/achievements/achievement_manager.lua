...
function AchievementManager:init(world, statistics_db)
	...
	for _, template in pairs(AchievementTemplates.achievements) do
		...
		if self.platform == "steam" and template.ID_STEAM or
		IS_PS4 and template.ID_PS4 or
		IS_XB1 and template.ID_XB1 or
		self.platform == "debug" then
...
function AchievementManager:trigger_event(event_name, ...)
...
function AchievementManager:register_timed_event(achievement_id, event_name, delay, data)
	...
	local handle = { achievement_id, event_name, delay, data, valid = true }
	self._timed_events [handle] = t + (delay or 0)
	return handle
end

function AchievementManager:cancel_timed_event(handle)
...
function AchievementManager:get_registered_timed_event(handle)
	return self._timed_events [handle]
end

function AchievementManager:reset_timed_event(handle)
...
function AchievementManager:_update_timed_events(dt, t)
...
function AchievementManager:event_enable_achievements(enable)
	self._enabled = enable
end

function AchievementManager:is_enabled()
	return self._enabled
end

function AchievementManager:update(dt, t)




	if not self._enabled or
	not self:_check_version_number() or
	not self:_check_initialized_achievements() or
	not self:_verify_platform_achievements() or
	script_data ["eac-untrusted"] then
	...
	local should_process = not unlocked_achievements [template_id] and
	not unlock_tasks [template_id]
	...
	if should_process then
		...
		if token then
			...
			unlock_tasks [template_id] = { token = token, achievement_completed = achievement_completed }
...
function AchievementManager:outline()
...
function AchievementManager:_search_sub_categories(categories, in_category_id)
	...
	for i = 1, #categories do
		...
		if category_id == in_category_id then
			do return category.entries end
		else
...
function AchievementManager:get_entries_from_category(in_category_id)
	return self:_search_sub_categories(outline.categories, in_category_id)
end


function AchievementManager:get_data_by_id(achievement_id)
...
function AchievementManager:setup_achievement_data()
...
function AchievementManager:setup_achievement_data_from_list(achievement_ids)
	if not self._enabled then
		return
	end
...
function AchievementManager:can_claim_achievement_rewards(achievement_id)
	if not self._enabled then
...
function AchievementManager:can_claim_all_achievement_rewards(achievement_ids)
	if not self._enabled then
...
function AchievementManager:claim_reward(achievement_id)
	local backend_interface_loot = self._backend_interface_loot
...
function AchievementManager:claim_multiple_rewards(achievement_ids)
	local backend_interface_loot = self._backend_interface_loot
	local reward_poll_id = backend_interface_loot:generate_reward_loot_id()
...
function AchievementManager:polling_reward()
	local backend_interface_loot = self._backend_interface_loot
	return backend_interface_loot:polling_reward()
end


function AchievementManager:has_any_unclaimed_achievement()
...
function AchievementManager:evaluate_end_of_level_achievements(statistics_db, stats_id, level_key, difficulty_key)
...
function AchievementManager:_check_version_number()
...
function AchievementManager:_check_initialized_achievements()
...
function AchievementManager:_display_completion_ui(achievement_id)
...
function AchievementManager:_check_for_completed_achievements()
...
function AchievementManager:setup_incompleted_achievements()
	if not self._enabled then
		return
	end
	local template_count = 0
...
function AchievementManager:_setup_achievement_data(achievement_id)
...
function AchievementManager:_add_achievement_to_platform_unlock_verification(achievement_id)
...
function AchievementManager:_verify_platform_achievements()
	local platform_functions = self._platform_functions
...
function AchievementManager:debug_draw()
	if not script_data.achievement_debug then return
	end
