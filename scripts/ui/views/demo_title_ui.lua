...
function DemoTitleUI:init(world, viewport, parent)
	self._world = world
...
function DemoTitleUI:menu_input_enabled() return true end

function DemoTitleUI:_setup_gui()
...
function DemoTitleUI:_setup_world_gui()
...
function DemoTitleUI:_setup_level()
...
function DemoTitleUI:shading_callback(world, shading_env, viewport)
...
function DemoTitleUI:_collect_cameras()
	self._camera_poses = { }
...
function DemoTitleUI:_position_camera()
...
function DemoTitleUI:_create_ui_elements()
	...
	local text_style = { vertical_alignment = "center", word_wrap = true, horizontal_alignment = "center", font_size = 18, font_type = "hell_shark",
...
function DemoTitleUI:_populate_career_page(profile_name, career_index)
...
function DemoTitleUI:_set_career_widget_text(widget_name, text)
	local widget = self._career_widgets [widget_name]
	widget.content.text = text
end

function DemoTitleUI:_destroy_career_video_player()
...
function DemoTitleUI:_setup_video_player(material_name, resource)
...
function DemoTitleUI:_setup_input()
...
function DemoTitleUI:_setup_characters()
	self._character_previewers = { }
...
function DemoTitleUI:_play_sound(event)
	return Managers.music:trigger_event(event)
end

function DemoTitleUI:get_ui_renderer()
	return self._ui_renderer
end

function DemoTitleUI:in_transition()
	return self._camera_transition
end

function DemoTitleUI:_recreate_characters()
...
function DemoTitleUI:update(dt, t)
...
function DemoTitleUI:_update_scatter(dt, t)
...
function DemoTitleUI:_update_input(dt, t)
...
function DemoTitleUI:_update_character_previewers(dt, t)
...
function DemoTitleUI:_update_start_game(dt, t)
...
function DemoTitleUI:_update_back(dt, t)
	if self._camera_transition and self._selected_profile then
		return
	end
...
function DemoTitleUI:_update_career_information(dt, t)
	...
	if not is_selected and self._selected_profile and
	self._displayed_profile ~= self._selected_profile then
		local character_previewer = self._character_previewers [self._selected_profile]
		local profile_name, career_index = character_previewer:profile_information()
		self:_populate_career_page(profile_name, career_index)
	end
	...
	if not self._ui_animations.animate_out and not self._ui_animations.delay and not is_selected and not self._selected_profile then
		...
		self._ui_animations.delay = UIAnimation.init(UIAnimation.function_by_time, { 0, 0, 0 }, 1, 0, 0, 0, math.easeInCubic)
		self._ui_animation_cb.delay = animation_cb
	end
end

function DemoTitleUI:_update_animation(dt, t)
...
function DemoTitleUI:_update_camera(dt, t)
...
function DemoTitleUI:_draw_3d_logo(dt, t)
...
function DemoTitleUI:_draw(dt, t)
	...
	elseif not self._entering and
	not self:in_transition() then
...
function DemoTitleUI:_draw_fps(dt, t)
...
function DemoTitleUI:activate_career_ui(activate)
...
function DemoTitleUI:animate_to_camera(camera_name, pose, cb, time)
	local camera = ScriptViewport.camera(self._viewport)
	local camera_pose = ScriptCamera.pose(camera)
...
function DemoTitleUI:should_start()
	return self._start_pressed
end

function DemoTitleUI:selected_profile()
	return self._selected_profile
end

function DemoTitleUI:set_start_pressed(pressed)
	self._entering = pressed
end
function DemoTitleUI:clear_playgo_status() return end
function DemoTitleUI:set_playgo_status() return end
function DemoTitleUI:show_menu() return end
function DemoTitleUI:clear_user_name() return end
function DemoTitleUI:current_menu_index() return end
function DemoTitleUI:active_menu_selection() return end
function DemoTitleUI:set_menu_item_enable_state_by_index() return end

function DemoTitleUI:destroy()
	for profile_name, character_previewer in pairs(self._character_previewers) do
		character_previewer:destroy()
	end
	self._character_previewers = { }
...
function DemoTitleUI:set_information_text(optinal_text) return end

function DemoTitleUI:set_user_name(username)
...
function DemoTitleUI:clear_user_name()
	self._draw_gamertag = nil
	self._switch_profile_blocked = nil
end

function DemoTitleUI:set_update_offline_data_enabled(enable) return end
function DemoTitleUI:disable_input(disable) return end
function DemoTitleUI:set_game_type(game_type) return end