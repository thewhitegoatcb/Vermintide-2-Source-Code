...
local camera_entry_shake_settings = { persistance = 0.9, fade_out = 0.3, amplitude = 1, duration = 0.3, fade_in = 0.1, octaves = 5.5 }
...
local camera_default_shake_settings = { persistance = 1, fade_out = 0.5, amplitude = 0.9, seed = 0, duration = 0.5, fade_in = 0.1, octaves = 7 }
...
local glow_rarity_colors = {
	default = {
		front = { 255, 173, 155, 99 },
		back = { 255, 255, 223, 154 },
		center = { 255, 255, 223, 154 } },
	...
	common = {
		front = { 255, 255, 223, 154 },
		back = { 255, 38, 254, 18 },
		center = { 150, 38, 254, 18 } },

	rare = {
		front = { 255, 154, 255, 219 },
		back = { 255, 30, 171, 255 },
		center = { 255, 30, 171, 255 } },

	exotic = {
		back = { 255, 255, 106, 6 },
		front = { 255, 245, 255, 154 },
		center = { 255, 255, 106, 6 } },


	unique = {
		front = { 255, 255, 210, 179 },
		back = { 255, 254, 25, 18 },
		center = { 255, 254, 25, 18 } },

	promo = {
		back = { 255, 119, 18, 254 },
		front = { 255, 255, 223, 154 },
		center = { 255, 119, 18, 254 } } }
...
function HeroViewStateLoot:on_enter(params, optional_ignore_item_population)
...
function HeroViewStateLoot:post_update_on_enter()
...
function HeroViewStateLoot:_setup_input_buttons()
	...
	icon_style_input_1.texture_size = { input_1_texture_data.size [1], input_1_texture_data.size [2] }
	...
	icon_style_input_2.texture_size = { input_2_texture_data.size [1], input_2_texture_data.size [2] }
	input_2_widget.content.texture_id = input_2_texture_data.texture
end

function HeroViewStateLoot:_set_gamepad_input_buttons_visibility(visible)
	local widgets_by_name = self._widgets_by_name
...
function HeroViewStateLoot:disable_player_world()
...
function HeroViewStateLoot:enable_player_world()
...
function HeroViewStateLoot:populate_items()
	local widgets_by_name = self._widgets_by_name
	...
	local function sort_func(a, b)
		...
		if a_chest_sort_order == b_chest_sort_order then
			if a_chest_tier == b_chest_tier then
				do return a.backend_id < b.backend_id end
			else
...
function HeroViewStateLoot:_get_items_by_filter(item_filter)
...
function HeroViewStateLoot:get_background_world()
	return self.parent:get_background_world()
end

function HeroViewStateLoot:transitioning()
...
function HeroViewStateLoot:wanted_menu_state()
	return self._wanted_menu_state
end

function HeroViewStateLoot:clear_wanted_menu_state()
	self._wanted_menu_state = nil
end

function HeroViewStateLoot:_wanted_state()
	local new_state = self.parent:wanted_state()
	return new_state
end

function HeroViewStateLoot:create_ui_elements()
...
function HeroViewStateLoot:_setup_reward_option_widgets()
...
function HeroViewStateLoot:_setup_camera()
...
function HeroViewStateLoot:set_camera_position(position)
	local _, viewport = self:get_viewport_world()
	local camera = ScriptViewport.camera(viewport)
	return ScriptCamera.set_local_position(camera, position)
end

function HeroViewStateLoot:set_camera_rotation(rotation)
	local _, viewport = self:get_viewport_world()
	local camera = ScriptViewport.camera(viewport)
	return ScriptCamera.set_local_rotation(camera, rotation)
end

function HeroViewStateLoot:get_camera_position()
	local _, viewport = self:get_viewport_world()
	local camera = ScriptViewport.camera(viewport)
	return ScriptCamera.position(camera)
end

function HeroViewStateLoot:get_camera_rotation()
	local _, viewport = self:get_viewport_world()
	local camera = ScriptViewport.camera(viewport)
	return ScriptCamera.rotation(camera)
end

function HeroViewStateLoot:get_viewport_world()
...
function HeroViewStateLoot:_position_camera(optional_pose)
...
function HeroViewStateLoot:on_exit(params)
...
function HeroViewStateLoot:_update_transition_timer(dt)
...
function HeroViewStateLoot:update(dt, t)
...
function HeroViewStateLoot:post_update(dt, t)
	if self.waiting_for_post_update_enter then
...
function HeroViewStateLoot:_update_animations(dt)
...
function HeroViewStateLoot:draw(dt)
	local loot_ui_renderer = self.loot_ui_renderer
...
function HeroViewStateLoot:_activate_widget_viewport(preview_widget, activate)
...
function HeroViewStateLoot:_set_debug_buttons_disable_state(is_disabled)
...
function HeroViewStateLoot:_is_button_pressed(widget)
	local content = widget.content
	local hotspot = content.hotspot or content.button_hotspot
...
function HeroViewStateLoot:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.hotspot or content.button_hotspot
...
function HeroViewStateLoot:_is_option_tab_selected()
...
function HeroViewStateLoot:_select_option_tab_by_index(index)
	local widget = self._widgets_by_name.inventory_tabs
	local widget_content = widget.content
	local amount = widget_content.amount
	for i = 1, amount do
...
function HeroViewStateLoot:_has_grid_item(item)
	local item_grid = self._item_grid
	return item_grid:has_item(item)
end

function HeroViewStateLoot:_select_grid_item(item, t, reset_num_chests)
	local widgets_by_name = self._widgets_by_name
	local item_grid = self._item_grid
...
function HeroViewStateLoot:_play_sound(sound_event)
	WwiseWorld.trigger_event(self.wwise_world, sound_event)
end

function HeroViewStateLoot:_handle_gamepad_input(dt, t)
	local mouse_active = Managers.input:is_device_active("mouse")
	...
	elseif self._reward_option_animation_complete then
		...
		elseif input_service:get(INPUT_ACTION_PREVIOUS) then
			local page_change = -1
			self:_change_chest_page(page_change)
		elseif not self._rewards_presented and
		not input_service:get("special_1_hold") and not self._ui_animations.page_cycle then
...
function HeroViewStateLoot:_find_console_selection_index(next_index)
	local active_reward_options = self._active_reward_options
...
function HeroViewStateLoot:_handle_page_selection(dt)
...
function HeroViewStateLoot:_change_chest_page(page_change)
...
function HeroViewStateLoot:_set_last_pressed(mode)
...
function HeroViewStateLoot:_handle_input(dt, t)
	local widgets_by_name = self._widgets_by_name
	local input_manager = self.input_manager
	local input_service = input_manager:get_service("hero_view")
	...
	elseif not self._opening_chest then
		...
		if
		item and (not self._selected_item or self._selected_item.backend_id ~= item.backend_id) then
			local reset_num_chests = true
...
function HeroViewStateLoot:_update_page_info()
...
local rarity_sound_mapping = { common = "play_hud_rewards_tier1", exotic = "play_hud_rewards_tier3", rare = "play_hud_rewards_tier2", unique = "play_hud_rewards_tier4" }
...
function HeroViewStateLoot:open_reward_option(index)
	local active_reward_options = self._active_reward_options
...
function HeroViewStateLoot:_setup_gamepad_tooltip(index, reward_option)
...
function HeroViewStateLoot:_reset_gamepad_tooltips()
...
function HeroViewStateLoot:_setup_rewards(rewards)
	local backend_items = Managers.backend:get_interface("items")
	local reward_options = self._reward_options
	...
	for index, data in ipairs(reward_options) do
		...
		if backend_id then
			...
			if slot_type == "melee" or slot_type == "ranged" or slot_type == "weapon_skin" then
				...
				local preview_position = { 0, 0, -0.2 }
...
function HeroViewStateLoot:_update_active_viewports(force_update)
	if not USE_DELAYED_SPAWN then
		return
	end
...
function HeroViewStateLoot:_get_hero_wield_info_by_item(item)
	local item_data = item.data
...
function HeroViewStateLoot:_apply_color_to_glow_style(style, color)
...
function HeroViewStateLoot:_spawn_hero_skin(world_previewer, hero_name, career_index, optional_skin)
...
function HeroViewStateLoot:_spawn_hero_with_hat(world_previewer, hero_name, career_index, optional_skin, item_name)
	local callback = callback(self, "cb_hero_unit_spawned_hat_preview", world_previewer, hero_name, career_index, item_name)

	world_previewer:request_spawn_hero_unit(hero_name, career_index, false, callback, 1, nil, optional_skin)
end

function HeroViewStateLoot:cb_hero_unit_spawned_skin_preview(world_previewer, hero_name, career_index)
...
function HeroViewStateLoot:cb_hero_unit_spawned_hat_preview(world_previewer, hero_name, career_index, hat_item_name)
	local profile_index = FindProfileIndex(hero_name)
	local profile = SPProfiles [profile_index]
	local careers = profile.careers
	local career_settings = careers [career_index]
...
function HeroViewStateLoot:_create_player_portrait(scenegraph_id, portrait_frame, portrait_image, player_level_text, optional_scale)
...
function HeroViewStateLoot:_set_background_blur_progress(progress)
	local world, viewport = self:get_viewport_world()
...
function HeroViewStateLoot:play_sound(event)
	self.parent:play_sound(event)
end

function HeroViewStateLoot:_start_transition_animation(key, animation_name)
	local params = {
		wwise_world = self.wwise_world,
		render_settings = self.render_settings }
...
function HeroViewStateLoot:_start_animation(key, animation_name, widget, optional_params)
	if not optional_params then local params = { wwise_world = self.wwise_world }
	end
...
function HeroViewStateLoot:_open_chest(selected_item, num_chests)
	self:_reset_camera()
...
function HeroViewStateLoot:loot_chest_opened(loot)
...
function HeroViewStateLoot:_start_reward_presentation(loot)
	local ui_scenegraph = self.ui_scenegraph
...
function HeroViewStateLoot:_animate_reward_options_entry(dt)
...
function HeroViewStateLoot:set_reward_options_height_progress(progress)
	local w = RESOLUTION_LOOKUP.res_w local h = RESOLUTION_LOOKUP.res_h
...
function HeroViewStateLoot:_unload_loaded_packages()
...
function HeroViewStateLoot:_destroy_chest_unit()
	if self._chest_unit then
...
function HeroViewStateLoot:_load_package(package_name)
	self:_destroy_chest_unit()
	self:_unload_loaded_packages()
...
function HeroViewStateLoot:_on_load_complete(package_name)
...
function HeroViewStateLoot:_unload_package(package_name)
	local reference_name = "HeroViewStateLoot"
	local package_manager = Managers.package
	package_manager:unload(package_name, reference_name)
end

function HeroViewStateLoot:_spawn_chest_unit(unit_name, instant_spawn, t)
	local world = self:get_viewport_world()
...
function HeroViewStateLoot:get_world_link_unit()
	local level_name = viewport_widget_definition.style.viewport.level_name
	local previewer_pass_data = self.viewport_widget.element.pass_data [1]
	local world = previewer_pass_data.world
...
function HeroViewStateLoot:set_camera_zoom(progress)
...
function HeroViewStateLoot:set_grid_animation_progress(progress)
	local ui_scenegraph = self.ui_scenegraph
...
function HeroViewStateLoot:set_continue_button_animation_progress(progress)
	local ui_scenegraph = self.ui_scenegraph
...
function HeroViewStateLoot:set_chest_title_alpha_progress(progress)
	local widgets_by_name = self._widgets_by_name
...
function HeroViewStateLoot:_update_enter_animation_time(dt, t)
...
function HeroViewStateLoot:_update_continue_button_animation_time(dt, t)
...
function HeroViewStateLoot:_update_camera_look_up_time(dt, t)
...
function HeroViewStateLoot:_update_camera_look_down_time(dt, t)
...
function HeroViewStateLoot:_reset_camera()
	self._camera_look_down_duration = nil
	self._camera_look_up_progress = 0
...
function HeroViewStateLoot:_update_chest_open_wait_time(dt, t)
...
function HeroViewStateLoot:_update_chest_zoom_in_time(dt, t)
...
function HeroViewStateLoot:_update_chest_zoom_out_time(dt, t)
...
function HeroViewStateLoot:_update_camera_shake_chest_spawn_time(dt, t)
...
function HeroViewStateLoot:add_camera_shake(settings, start_time, scale)
	local data = { }
	...
	self._active_camera_shakes = { [data] = true }
end

function HeroViewStateLoot:_apply_shake_event(settings, t)
...
function HeroViewStateLoot:_calculate_perlin_value(x, settings)
	local total = 0
...
function HeroViewStateLoot:_interpolated_noise(x, settings)
...
function HeroViewStateLoot:_smoothed_noise(x, settings)
	return self:_noise(x, settings) / 2 + self:_noise(x - 1, settings) / 4 + self:_noise(x + 1, settings) / 4
end

function HeroViewStateLoot:_noise(x, settings)
...
function HeroViewStateLoot:_get_card_spawn_position()
...
function HeroViewStateLoot:_create_portrait_frame_widget(frame_settings_name, portrait_texture, player_level_text)
...
function HeroViewStateLoot:_setup_info_window()
	local hero_name = self.hero_name
	local career_index = self.career_index
...
function HeroViewStateLoot:_handle_gamepad_activity()
	local gamepad_active = Managers.input:is_device_active("gamepad")
