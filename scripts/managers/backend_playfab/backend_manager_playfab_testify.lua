...
function BackendManagerPlayFabTestify.clear_backend_inventory(backend_manager_playfab)
...
function BackendManagerPlayFabTestify.request_magic_weapons_for_career(backend_manager_playfab, career_name)
...
function BackendManagerPlayFabTestify.request_non_magic_weapons_for_career(backend_manager_playfab, career_name)
	local items = backend_manager_playfab:get_interface("items"):get_all_backend_items()
	return table.filter(items, function (item)
		local is_weapon = item.data.slot_type == "melee" or item.data.slot_type == "ranged"
		local is_magic = item.data.rarity == "magic"
...
function BackendManagerPlayFabTestify.wait_for_playfab_response(backend_manager_playfab, cloudscript_function)
