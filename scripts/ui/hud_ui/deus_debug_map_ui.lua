...
local color_map_for_label = { [0] =
	ColorBox(Colors.get("black")),
...
function DeusDebugMapUI:init(parent, ingame_ui_context)
...
function DeusDebugMapUI:destroy()
	World.destroy_gui(self._world, self._gui)
	self._gui = nil
end

function DeusDebugMapUI:update(dt, t)
...
function DeusDebugMapUI:_draw_base_graph(graph, dt, t)
...
function DeusDebugMapUI:_draw_final_graph(graph, dt, t)
...
function DeusDebugMapUI:_draw_edges(graph)
	local width, height = Gui.resolution()
...
function DeusDebugMapUI:_draw_edge(from_x, from_y, to_x, to_y)
	local dx = to_x - from_x local dy = to_y - from_y
	...
	if dx ~= 0 or dy ~= 0 then
		...
		local dot_delta_x = dx / dot_count local dot_delta_y = dy / dot_count

		local current_dot_x = from_x local current_dot_y = from_y
		for i = 1, dot_count do
			Gui.rect(
			self._gui,
			Vector2(current_dot_x, current_dot_y),
			Vector2(2 + 5 * i / dot_count, 2 + 5 * i / dot_count),
			Color(128, 0, 0, 0))
