...
function PlayerUnitHealthExtension:init(extension_init_context, unit, extension_init_data)
...
function PlayerUnitHealthExtension:hot_join_sync(sender)
	return end

function PlayerUnitHealthExtension:cb_game_session_disconnect()
	self.health_game_object_id = nil
end

function PlayerUnitHealthExtension:set_health_game_object_id(go_id)
	self.health_game_object_id = go_id
end

function PlayerUnitHealthExtension:create_health_game_object()
	...
	local game_object_data_table = { current_temporary_health = 0,
...
function PlayerUnitHealthExtension:sync_health_state()
...
function PlayerUnitHealthExtension:_get_base_max_health()
...
function PlayerUnitHealthExtension:_calculate_buffed_max_health()
...
function PlayerUnitHealthExtension:get_buffed_max_health()
	local max_health = self:_calculate_buffed_max_health()
	return max_health
end

function PlayerUnitHealthExtension:_calculate_max_health()
	local buff_extension = self.buff_extension
	local health_state = self.state
	local modifier = 1
...
function PlayerUnitHealthExtension:knock_down(unit)
...
function PlayerUnitHealthExtension:_revive(unit, t)
	self.state = "alive"
...
function PlayerUnitHealthExtension:update(dt, context, t)
	...
	if self.is_server then
		...
		elseif self.state == "knocked_down" and
		self:_is_alive() and status_extension:is_revived() then
			self:_revive(unit, t)
		end
...
function PlayerUnitHealthExtension:_update_outline_color(t, dt)
	...
	if not self._outline_id then
		self._outline_id = outline_ext:add_outline({ priority = 2, method = "always",
	...
	elseif self._current_outline_color ~= new_outline_color then
		outline_ext:update_outline({ outline_color = new_outline_color }, self._outline_id)
	end
	self._current_outline_color = new_outline_color
end


local FORCED_PERMANENT_DAMAGE_TYPES = { death_explosion = true }




function PlayerUnitHealthExtension:apply_client_predicted_damage(predicted_damage)

	return end

function PlayerUnitHealthExtension:add_damage(attacker_unit, damage_amount, hit_zone_name, damage_type, hit_position, damage_direction, damage_source_name, hit_ragdoll_actor, source_attacker_unit, hit_react_type, is_critical_strike, added_dot, first_hit, total_hits, attack_type, backstab_multiplier)
	if DamageUtils.is_in_inn then return end
	local status_extension = self.status_extension
	if status_extension:is_ready_for_assisted_respawn() then return
	end
	...
	if damage_source_name == "ground_impact" and not breed.is_hero then return
	end
	...
	if controller_features_manager then
		...
		if player.local_player and damage_amount > 0 and damage_type ~= "temporary_health_degen" then
			controller_features_manager:add_effect("hit_rumble", { damage_amount = damage_amount, unit = unit })
		end
	end
	...
	if damage_source_name ~= "dot_debuff" and damage_type ~= "temporary_health_degen" and damage_type ~= "overcharge" then
		...
		if self.player.local_player and (
		buff_extension:has_buff_type("bardin_ironbreaker_activated_ability") or buff_extension:has_buff_type("bardin_ironbreaker_activated_ability_taunt_range_and_duration")) then
...
function PlayerUnitHealthExtension:add_heal(healer_unit, heal_amount, heal_source_name, heal_type)
	local unit = self.unit
	local status_extension = self.status_extension
	...
	if self.is_server then
		...
		if game and game_object_id then
			...
			if
			heal_type ~= "career_passive" and not status_extension:is_wounded() then
...
function PlayerUnitHealthExtension:die(damage_type)
...
function PlayerUnitHealthExtension:entered_kill_volume(t)
...
function PlayerUnitHealthExtension:destroy()
	if self.is_server and
	self.health_game_object_id then
...
function PlayerUnitHealthExtension:reset()
	if self.is_server then
		self.state = "alive"
...
function PlayerUnitHealthExtension:_is_alive()
	local game = self.game
	local game_object_id = self.health_game_object_id
...
function PlayerUnitHealthExtension:current_permanent_health_percent()
	local game = self.game
	local game_object_id = self.health_game_object_id
...
function PlayerUnitHealthExtension:current_temporary_health_percent()
	local game = self.game
	local game_object_id = self.health_game_object_id
...
function PlayerUnitHealthExtension:current_permanent_health()
	local game = self.game
	local game_object_id = self.health_game_object_id
...
function PlayerUnitHealthExtension:current_temporary_health()
	local game = self.game
	local game_object_id = self.health_game_object_id
...
function PlayerUnitHealthExtension:get_max_health()
	local game = self.game
	local game_object_id = self.health_game_object_id
...
function PlayerUnitHealthExtension:get_base_max_health()
	local base_health = self:_get_base_max_health()
	return base_health
end

function PlayerUnitHealthExtension:get_uncursed_max_health()
	local game = self.game
	local game_object_id = self.health_game_object_id
...
function PlayerUnitHealthExtension:get_damage_taken(max_health_go_field)
	local game = self.game
	local game_object_id = self.health_game_object_id
...
function PlayerUnitHealthExtension:convert_permanent_to_temporary_health()
	local game = self.game
	local game_object_id = self.health_game_object_id
...
function PlayerUnitHealthExtension:convert_temporary_to_permanent_health()
	local game = self.game
	local game_object_id = self.health_game_object_id
...
function PlayerUnitHealthExtension:convert_to_temp(amount)
...
function PlayerUnitHealthExtension:switch_permanent_and_temporary_health()
	local game = self.game
	local game_object_id = self.health_game_object_id
...
function PlayerUnitHealthExtension:shield(shield_amount)
...
function PlayerUnitHealthExtension:has_assist_shield()
	return self._shield_duration_left > 0 and self._shield_amount > 0, self._shield_amount
end

function PlayerUnitHealthExtension:remove_assist_shield(end_reason)
	self._shield_duration_left = 0
	self._shield_amount = 0
	self._end_reason = end_reason
end

function PlayerUnitHealthExtension:previous_shield_end_reason()
	return self._end_reason
end

function PlayerUnitHealthExtension:set_dead()
	self.state = "dead"
...
function PlayerUnitHealthExtension:set_max_health(health)

	return end

function PlayerUnitHealthExtension:set_current_damage(damage)
