...
function NavWorld.get_navworld(level)
	return _navworlds [level]
end

function NavWorld:init(world, level)
	self.world = world
	self.level = level
...
function NavWorld:add_navdata(resource_name)
	self.navdata = GwNavWorld.add_navdata(self.gwnavworld, resource_name)
end

function NavWorld:init_bot(unit)
...
function NavWorld:init_bot_from_unit(unit, configuration_unit)
...
function NavWorld:get_navbot(unit)
	return self.bots [unit]
end

function NavWorld:init_navmarker(unit)
	self.markers [#self.markers + 1] = unit
end

function NavWorld:set_smartobject_cost_multiplier(smartobject_id, cost_multiplier, smartobject_type)
...
function NavWorld:unset_smartobject(smartobject_id)
	GwNavWorld.unset_smartobject(self.gwnavworld, smartobject_id)
end

function NavWorld:allow_smartobject(smartobject_id)
	GwNavWorld.allow_smartobject(self.gwnavworld, smartobject_id)
end

function NavWorld:forbid_smartobject(smartobject_id)
	GwNavWorld.forbid_smartobject(self.gwnavworld, smartobject_id)
end

function NavWorld:get_smartobject_type(smartobject_id)
	return self.smartobject_types [smartobject_id]
end

function NavWorld:set_dynamicnavmesh_budget(budget)
	GwNavWorld.set_dynamicnavmesh_budget(self.gwnavworld, budget)
end

function NavWorld:set_pathfinder_budget_in_ms(budget)
	GwNavWorld.set_pathfinder_budget(self.gwnavworld, budget)
end

function NavWorld:init_fromnavworldunit(unit)
...
function NavWorld:init_bot_configuration(unit)
...
function NavWorld:init_graph_connector(unit)
...
function NavWorld:init_tagbox(unit)
	...
	local local_center = Vector3(NavHelpers.unit_script_data(unit, 0, "GwNavTagBox", "offset", "x"),
	NavHelpers.unit_script_data(unit, 0, "GwNavTagBox", "offset", "y"), NavHelpers.unit_script_data(unit, 0, "GwNavTagBox", "offset", "z"))
	...
	local point_table = { unitPos + forward * extent_x - right * extent_y, unitPos + forward * extent_x + right * extent_y, unitPos - forward * extent_x + right * extent_y, unitPos - forward * extent_x - right * extent_y }
...
function NavWorld:add_bot(bot)
	self.bots [bot.unit] = bot
end

function NavWorld:remove_bot(bot)
	self.bots [bot.unit] = nil
end

function NavWorld:force_all_bots_to_repath()
	for kb, bot in pairs(self.bots) do
		bot:force_repath()
	end
end

function NavWorld:update(dt)
...
function NavWorld:clear_bot_configuration()
...
function NavWorld:clear_navgraphs()
...
function NavWorld:clear_tagboxes()
...
function NavWorld:clear_boxobstacles()
	for kb, box in pairs(self.navboxobstacles) do
...
function NavWorld:clear_cylinderobstacles()
	for kc, cylinder in pairs(self.navcylinderobstacles) do
...
function NavWorld:debug_draw(gui, line_object)
	if self.render_mesh == false then return end
...
function NavWorld:visual_debug_camera(camera)
