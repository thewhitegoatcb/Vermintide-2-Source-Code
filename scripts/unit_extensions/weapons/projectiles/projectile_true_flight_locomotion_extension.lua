...
function ProjectileTrueFlightLocomotionExtension:init(extension_init_context, unit, extension_init_data)
	...
	self._lerp_modifier_func = template.lerp_modifier_func or function (distance) return distance < 5 and 1 or 5 / distance end
...
local function valid_position(position)
	...
	local pmin = NetworkConstants.position.min local pmax = NetworkConstants.position.max
	for i = 1, 3 do
...
function ProjectileTrueFlightLocomotionExtension:_do_forced_impact(unit, current_position)
...
function ProjectileTrueFlightLocomotionExtension:bounce(hit_position, hit_direction, hit_normal)
...
function ProjectileTrueFlightLocomotionExtension:update(unit, input, dt, context, t)
	self.dt = t - self.t
	self.t = t
	self.moved = false
...
function ProjectileTrueFlightLocomotionExtension:_check_target_valid(target, current_position, template)
...
function ProjectileTrueFlightLocomotionExtension:set_projectile_state(state_id)
...
function ProjectileTrueFlightLocomotionExtension:update_towards_slow_bomb_target(position, t, dt)
...
function ProjectileTrueFlightLocomotionExtension:update_towards_strike_missile_target(position, t, dt)
	local target_unit = self.target_unit
	local current_direction = self.current_direction:unbox()
...
function ProjectileTrueFlightLocomotionExtension:update_towards_position_target(position, t, dt)
	local current_direction = self.current_direction:unbox()
...
function ProjectileTrueFlightLocomotionExtension:update_towards_target(position, t, dt)
	local target_unit = self.target_unit
	local current_direction = self.current_direction:unbox()
...
function ProjectileTrueFlightLocomotionExtension:update_seeking_target(position, dt, t, seeking)
	...
	local self_dt = self.dt local speed = self.speed * speed_multiplier local angle = self.radians local gravity = self.gravity
...
function ProjectileTrueFlightLocomotionExtension:find_new_target(position, true_flight_template, t, dt)
...
function ProjectileTrueFlightLocomotionExtension:find_player_target(position)
	...
	if players_n > 0 then
		...
		for i = start_index, start_index + players_n do
			...
			if HEALTH_ALIVE [unit] and not self.hit_units [unit] and
			self:_legitimate_target_func(unit, position) then
				return unit
			end
		end
	end
...
local ai_units = { }
function ProjectileTrueFlightLocomotionExtension:find_broadphase_target(position)
	...
	if ai_units_n > 0 then
		...
		for i = 1, ai_units_n do
			...
			if ScriptUnit.has_extension(unit, "health_system") and
			HEALTH_ALIVE [unit] and not self.hit_units [unit] and
			self:_legitimate_target_func(unit, position) then
				return unit
			end
		end
...
function ProjectileTrueFlightLocomotionExtension:find_closest_highest_value_target(position)
	local template = TrueFlightTemplates [self.true_flight_template_name]
	...
	if ai_units_n > 0 then
		...
		while ai_units_n >= i do
			...
			if not breed or breed.no_autoaim or
			not ScriptUnit.has_extension(unit, "health_system") or
			not HEALTH_ALIVE [unit] or
			self.hit_units [unit] or
			not self:_legitimate_target_func(unit, position) then
...
function ProjectileTrueFlightLocomotionExtension:legitimate_always(unit, position)
	return true
end

function ProjectileTrueFlightLocomotionExtension:legitimate_never(unit, position)
	return false
end

function ProjectileTrueFlightLocomotionExtension:legitimate_only_dot_check(unit, position)
...
function ProjectileTrueFlightLocomotionExtension:legitimate_target(unit, position)
...
function ProjectileTrueFlightLocomotionExtension:legitimate_target_keep_target(unit, position)
	local target_position = get_target_head_node_position(unit, "c_head")
...
function ProjectileTrueFlightLocomotionExtension:legitimate_player_target(unit, position)
...
function ProjectileTrueFlightLocomotionExtension:_unit_set_position_rotation(unit, position, rotation)
...
function ProjectileTrueFlightLocomotionExtension:moved_this_frame()
	return self.moved
end

function ProjectileTrueFlightLocomotionExtension:current_velocity()
	return self.velocity:unbox()
end

function ProjectileTrueFlightLocomotionExtension:current_position()
	return self._current_position:unbox()
end

function ProjectileTrueFlightLocomotionExtension:destroy()
...
function ProjectileTrueFlightLocomotionExtension:notify_hit_enemy(hit_unit)
	self.hit_units [hit_unit] = true
	self.raycast_timer = 0

end

function ProjectileTrueFlightLocomotionExtension:update_bot_threat(target_unit, distance)
	local template = self.true_flight_template
...
function ProjectileTrueFlightLocomotionExtension:stop()
