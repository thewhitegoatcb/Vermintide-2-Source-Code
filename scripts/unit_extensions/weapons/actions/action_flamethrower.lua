...
local NODES = { "j_leftshoulder", "j_rightshoulder", "j_spine1" }
...
function ActionFlamethrower:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionFlamethrower:client_owner_start_action(new_action, t, chain_action_data, power_level)
...
local INDEX_ACTOR = 4
function ActionFlamethrower:client_owner_post_update(dt, t, world, can_damage)
	...
	if self.state == "shooting" and t < self.max_flame_time then
		...
		if not Managers.player:owner(owner_unit).bot_player and not self._rumble_effect_id then
			self._rumble_effect_id = Managers.state.controller_features:add_effect("persistent_rumble", { rumble_effect = "reload_start" })
		end
				...
				for i = 1, #targets do
					...
					if Unit.alive(current_target) then
						...
						if result then
							...
							if data.buffs_checked then check_buffs =
								check_buffs and false
							end
							processed_hit = true
						end
					end
...
function ActionFlamethrower:_stop_fx()
...
function ActionFlamethrower:finish(reason)
	self:_clear_targets()
...
function ActionFlamethrower:destroy()
	if self._flamethrower_effect then
...
function ActionFlamethrower:_clear_targets()
...
function ActionFlamethrower:_select_targets(world, show_outline)
	local owner_unit = self.owner_unit
	local first_person_extension = ScriptUnit.extension(owner_unit, "first_person_system")
	...
	if ai_units_n > 0 then
		...
		for i = 1, ai_units_n do
			...
			if targets [hit_unit] == nil then
				...
				if
				( is_enemy or not ignore_hitting_allies ) and self:_is_infront_player(player_position, player_direction, hit_position) and self:_check_within_cone(start_point, player_direction, hit_unit, is_enemy) then
...
function ActionFlamethrower:_check_within_cone(player_position, player_direction, target, is_enemy)
...
function ActionFlamethrower:_is_infront_player(player_position, player_direction, hit_position)
...
function ActionFlamethrower:raycast_to_target(world, from_position, direction, target)
	local physics_world = World.get_data(world, "physics_world")
...
function ActionFlamethrower:_check_critical_strike(t)
	local owner_unit = self.owner_unit
	local current_action = self.current_action
	local is_critical_strike = ActionUtils.is_critical_strike(owner_unit, current_action, t)
