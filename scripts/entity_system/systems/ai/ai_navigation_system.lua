...
local extensions = { "AINavigationExtension", "PlayerBotNavigation" }
...
function AINavigationSystem:init(entity_system_creation_context, system_name)
...
function AINavigationSystem:destroy()
	AINavigationSystem.super.destroy(self)
end

function AINavigationSystem:on_add_extension(world, unit, extension_name, extension_init_data)
...
function AINavigationSystem:on_remove_extension(unit, extension_name)
	self.frozen_unit_extension_data [unit] = nil
	self:_cleanup_extension(unit, extension_name)
	AINavigationSystem.super.on_remove_extension(self, unit, extension_name)
end

function AINavigationSystem:on_freeze_extension(unit, extension_name)
...
function AINavigationSystem:_cleanup_extension(unit, extension_name)
...
function AINavigationSystem:freeze(unit, extension_name, reason)
...
function AINavigationSystem:unfreeze(unit)
...
function AINavigationSystem:simulate_dummy_target(t)
...
function AINavigationSystem:update(context, t)
	local dt = context.dt
...
function AINavigationSystem:add_safe_navigation_callback(callback)
	self._nav_safe_callbacks [#self._nav_safe_callbacks + 1] = callback
end

function AINavigationSystem:post_update(context, t)
	local dt = context.dt
...
function AINavigationSystem:add_navbot_to_release(unit)
	local extension = self.unit_extension_data [unit] or self.delayed_units [unit]
	self.navbots_to_release [unit] = extension
end

function AINavigationSystem:update_navbots_to_release()
...
function AINavigationSystem:update_enabled()
...
function AINavigationSystem:update_destination(t)
...
function AINavigationSystem:update_delayed_units(t)
...
function AINavigationSystem:setup_far_astar(p1, p2, extension, blackboard, nav_bot)
...
function AINavigationSystem:far_astar(p1, p2)
	local navigation_group_manager = Managers.state.conflict.navigation_group_manager
...
function AINavigationSystem:update_desired_velocity(t, dt)
...
function AINavigationSystem:update_next_smart_object(t, dt)
...
function AINavigationSystem:update_dispersion(t, dt)

	for unit, extension in pairs(self.enabled_units) do
		local action = GwNavBot.update_logic_for_crowd_dispersion(extension._nav_bot)
		if action == 1 then --[[ Nothing --]]

		elseif action == 2 then --[[ Nothing --]]
		end
	end
...
function AINavigationSystem:update_position(t, dt)
...
function AINavigationSystem:update_debug_draw(t)

	local drawer = Managers.state.debug:drawer({ mode = "immediate", name = "AINavigationExtension" })
...
function AINavigationSystem:_debug_draw_text(debug_unit, blackboard, navigation_extension, t)
...
function AINavigationSystem:_debug_draw_nav_path(drawer, navigation_extension)
...
function AINavigationSystem:_debug_draw_far_path(drawer, debug_unit, blackboard, navigation_extension)
	local far_path = blackboard.far_path
	if far_path then
...
function AINavigationSystem:override_nav_funcs()

	local patch_list = { "GwNavWorld", "GwNavBot", "GwNavQueries", "GwNavSmartObjectInterval", "GwNavTagLayerCostTable", "GwNavTraverseLogic", "GwNavBoxObstacle", "GwNavAStar", "GwNavCylinderObstacle", "GwNavTagVolume" }
