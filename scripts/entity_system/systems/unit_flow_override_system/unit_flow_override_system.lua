...
if not UNIT_FLOW_EVENT_OVERRIDDEN then
	function Unit.flow_event(unit, event_name, params)
...
local extensions = { "UnitFlowOverrideExtension" }



function UnitFlowOverrideSystem:init(entity_system_creation_context, system_name)
...
function UnitFlowOverrideSystem:on_add_extension(world, unit, extension_name)
	local extension = { }
...
function UnitFlowOverrideSystem:handle_flow_event(unit, event_name, params)
...
function UnitFlowOverrideSystem:_add_dynamic_event_data(unit, event_name, event_data)
...
local EMPTY_TABLE = { }
function UnitFlowOverrideSystem:destroy_data(unit, event_name)
...
function UnitFlowOverrideSystem:update(context, t)
	local dynamic_events = self._dynamic_events
...
function UnitFlowOverrideSystem:on_remove_extension(unit, extension_name)
	self._frozen_unit_extensions [unit] = nil
	self:_cleanup_extension(unit, extension_name)
	ScriptUnit.remove_extension(unit, self.NAME)
end

function UnitFlowOverrideSystem:on_freeze_extension(unit, extension_name)
...
function UnitFlowOverrideSystem:freeze(unit, extension_name, reason)
...
function UnitFlowOverrideSystem:unfreeze(unit)
...
function UnitFlowOverrideSystem:_cleanup_extension(unit, extension_name)
	local extension = self._unit_extensions [unit]
