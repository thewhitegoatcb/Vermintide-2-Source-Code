...
function PlayerProjectileUnitExtension:init(extension_init_context, unit, extension_init_data)
	...
	if impact_data.grenade and owner_buff_extension and
	owner_buff_extension:has_buff_perk("frag_fire_grenades") then
...
function PlayerProjectileUnitExtension:extensions_ready(world, unit)
...
function PlayerProjectileUnitExtension:initialize_projectile(projectile_info, impact_data)
...
function PlayerProjectileUnitExtension:_handle_critical_strike(unit, is_critical_strike)
	if self._is_critical_strike then
		Unit.flow_event(unit, "vfx_critical_strike")
	end
end

function PlayerProjectileUnitExtension:mark_for_deletion()
...
function PlayerProjectileUnitExtension:stop(hit_unit, hit_zone_name, hit_normal)
...
function PlayerProjectileUnitExtension:_stop_by_life_time()
	self:mark_for_deletion()
...
function PlayerProjectileUnitExtension:update(unit, input, _, context, t)
...
function PlayerProjectileUnitExtension:handle_timed_events(t)
...
function PlayerProjectileUnitExtension:destroy()
	if self._projectile_unit and self._active then
		self:stop()
	end
end

function PlayerProjectileUnitExtension:validate_position(position, min, max)
	for i = 1, 3 do
...
function PlayerProjectileUnitExtension:_alert_enemy(hit_unit, owner_unit)
...
function PlayerProjectileUnitExtension:handle_impacts(impacts, num_impacts, time)
	...
	local pos_min = NetworkConstants.position.min local pos_max = NetworkConstants.position.max
	...
	for i = 1, num_impacts / ProjectileImpactDataIndex.STRIDE do repeat
			if self._stop_impacts then
				return
			end
			...
			if not hit_self and valid_position and not hit_units [hit_unit] then
				...
				elseif not is_level_unit then
					self:hit_non_level_unit(impact_data, hit_unit, hit_position, hit_direction, hit_normal, hit_actor, has_ranged_boost, ranged_boost_curve_multiplier) end end until true
	end
...
function PlayerProjectileUnitExtension:_activate_life_time(game_time)
	local timed_data = self._timed_data
...
function PlayerProjectileUnitExtension:hit_enemy(impact_data, hit_unit, hit_position, hit_direction, hit_normal, hit_actor, breed, has_ranged_boost, ranged_boost_curve_multiplier)
...
function PlayerProjectileUnitExtension:hit_enemy_damage(damage_profile, hit_unit, hit_position, hit_direction, hit_normal, hit_actor, breed, has_ranged_boost, ranged_boost_curve_multiplier)
	local network_manager = Managers.state.network
	...
	if
	( multiplier_type == "headshot" or multiplier_type == "weakspot" and not shield_blocked ) and not action.no_headshot_sound and HEALTH_ALIVE [hit_unit] then
	...
	local predicted_damage, invulnerable = DamageUtils.calculate_damage(DamageOutput,
	hit_unit,
	owner_unit,
	hit_zone_name,
	power_level,
	...
	"first_hit", self._num_targets_hit == 1)
...
function PlayerProjectileUnitExtension:hit_player(impact_data, hit_unit, hit_position, hit_direction, hit_normal, hit_actor, has_ranged_boost, ranged_boost_curve_multiplier)
	...
	if hit then
		...
		if not impact_data.no_stop_on_friendly_fire and
		self._max_mass <= self._amount_of_mass_hit then
			if self._num_additional_penetrations > 0 then
				forced_penetration = true
			else
...
function PlayerProjectileUnitExtension:hit_player_damage(damage_profile, hit_unit, hit_position, hit_direction, hit_normal, hit_actor, has_ranged_boost, ranged_boost_curve_multiplier)
	local owner_unit = self._owner_unit
	local network_manager = Managers.state.network
	local attacker_unit_id = network_manager:unit_game_object_id(owner_unit)
	local hit_unit_id = network_manager:unit_game_object_id(hit_unit)
...
function PlayerProjectileUnitExtension:hit_level_unit(impact_data, hit_unit, hit_position, hit_direction, hit_normal, hit_actor, level_index, has_ranged_boost, ranged_boost_curve_multiplier)
...
function PlayerProjectileUnitExtension:hit_damagable_prop(damage_profile, hit_unit, hit_position, hit_direction, hit_normal, hit_actor, level_index, has_ranged_boost, ranged_boost_curve_multiplier)
	self._amount_of_mass_hit = self._amount_of_mass_hit + 1
...
function PlayerProjectileUnitExtension:hit_non_level_unit(impact_data, hit_unit, hit_position, hit_direction, hit_normal, hit_actor, has_ranged_boost, ranged_boost_curve_multiplier)
	local damage_profile_name = impact_data.damage_profile_prop or impact_data.damage_profile or "default"
	local damage_profile = DamageProfileTemplates [damage_profile_name]
...
function PlayerProjectileUnitExtension:hit_non_level_damagable_unit(damage_profile, hit_unit, hit_position, hit_direction, hit_normal, hit_actor, has_ranged_boost, ranged_boost_curve_multiplier)
	local network_manager = Managers.state.network
...
function PlayerProjectileUnitExtension:_get_projectile_units_names(projectile_info)
...
function PlayerProjectileUnitExtension:_get_weapon_unit()
	local inventory_extension = ScriptUnit.has_extension(self._owner_unit, "inventory_system")
	if inventory_extension then
		local slot_name = "slot_ranged"
		local slot_data = inventory_extension:get_slot_data(slot_name)
...
function PlayerProjectileUnitExtension:_handle_linking(impact_data, hit_unit, hit_position, hit_direction, hit_normal, hit_actor, damage_amount, allow_link, shield_blocked, hit_enemy_or_player)
...
function PlayerProjectileUnitExtension:_redirect_shield_linking(hit_unit, node_index, link_position, depth_position_offset)
	local breed = AiUtils.unit_breed(hit_unit)
...
function PlayerProjectileUnitExtension:_link_projectile(hit_unit, hit_actor, linker_unit_name, hit_position, hit_direction, depth, shield_blocked, flow_event_on_init, flow_event_on_walls)
...
function PlayerProjectileUnitExtension:_spawn_linked_pickup_projectile(pickup_name, hit_unit, hit_actor, hit_position, hit_direction, hit_normal, hit_unit_id, is_level_unit, depth, shield_blocked)
	local normalized_direction = Vector3.normalize(hit_direction)
	local depth_position_offset = normalized_direction * depth
	local link_position = hit_position + depth_position_offset
...
function PlayerProjectileUnitExtension:_spawn_pickup_projectile(pickup_name, hit_position, hit_direction, hit_normal, hit_enemy_or_player)
	local spawn_limit = 1
	local weapon_unit = self:_get_weapon_unit()
	if weapon_unit then
		local ammo_extension = ScriptUnit.has_extension(weapon_unit, "ammo_system")
...
function PlayerProjectileUnitExtension:do_aoe(aoe_data, position, network_sync)
	local world = self._world
	local unit = self._projectile_unit
	local owner_unit = self._owner_unit
...
function PlayerProjectileUnitExtension:spawn_liquid_area(unit, pos, dir, data)
	local start_pos = pos
	local flow_dir = dir
...
function PlayerProjectileUnitExtension:are_impacts_stopped()
	return self._stop_impacts
end


function PlayerProjectileUnitExtension:trigger_external_event(event_name, network_sync)
...
function PlayerProjectileUnitExtension:queue_delayed_external_event(event_name, event_t, network_sync)
	if not self._delayed_external_events then
		self._delayed_external_events = { }
	end
	local num_entries = #self._delayed_external_events
	self._delayed_external_events [num_entries + 1] = { event_t, event_name, network_sync }
end

function PlayerProjectileUnitExtension:_update_delayed_external_event(t)
