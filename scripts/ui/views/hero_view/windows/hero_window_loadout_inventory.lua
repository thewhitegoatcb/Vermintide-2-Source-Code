...
function HeroWindowLoadoutInventory:on_enter(params, offset)
...
function HeroWindowLoadoutInventory:_create_item_categories(profile_index, career_index)
...
function HeroWindowLoadoutInventory:create_ui_elements(params, offset)
...
function HeroWindowLoadoutInventory:_setup_tab_widget()
	local categories = self._categories
	local num_tabs = #categories

	local widgets = self._widgets
	local widgets_by_name = self._widgets_by_name
	local item_tabs_segments = UIWidget.init(UIWidgets.create_simple_centered_texture_amount("menu_frame_09_divider_vertical", { 5, 35 }, "item_tabs_segments", num_tabs - 1))
	local item_tabs_segments_top = UIWidget.init(UIWidgets.create_simple_centered_texture_amount("menu_frame_09_divider_top", { 17, 9 }, "item_tabs_segments_top", num_tabs - 1))
	local item_tabs_segments_bottom = UIWidget.init(UIWidgets.create_simple_centered_texture_amount("menu_frame_09_divider_bottom", { 17, 9 }, "item_tabs_segments_bottom", num_tabs - 1))
...
function HeroWindowLoadoutInventory:on_exit(params)
...
function HeroWindowLoadoutInventory:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function HeroWindowLoadoutInventory:post_update(dt, t)

	return end

function HeroWindowLoadoutInventory:_update_animations(dt)
...
function HeroWindowLoadoutInventory:_is_button_pressed(widget)
...
function HeroWindowLoadoutInventory:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function HeroWindowLoadoutInventory:_is_inventory_tab_hovered()
...
function HeroWindowLoadoutInventory:_is_inventory_tab_pressed()
	local widget = self._widgets_by_name.item_tabs
	local widget_content = widget.content
	local amount = widget_content.amount
	for i = 1, amount do
...
function HeroWindowLoadoutInventory:_select_tab_by_slot_index(index)
	local widget = self._widgets_by_name.item_tabs
	local widget_content = widget.content
	local amount = widget_content.amount
	for i = 1, amount do
...
function HeroWindowLoadoutInventory:_handle_input(dt, t)
	local widgets_by_name = self._widgets_by_name
...
function HeroWindowLoadoutInventory:_update_page_info()
...
function HeroWindowLoadoutInventory:_get_category_slot_index(index)
	local categories = self._categories
	local category = categories [index]
	return category.slot_index
end

function HeroWindowLoadoutInventory:_get_category_index_by_slot_index(slot_index)
	local categories = self._categories
...
function HeroWindowLoadoutInventory:_update_selected_loadout_slot_index()
...
function HeroWindowLoadoutInventory:_update_loadout_sync()
	local item_grid = self._item_grid
	local parent = self.parent
...
function HeroWindowLoadoutInventory:_exit(selected_level)
	self.exit = true
	self.exit_level_id = selected_level
end

function HeroWindowLoadoutInventory:draw(dt)
...
function HeroWindowLoadoutInventory:_play_sound(event)
	self.parent:play_sound(event)
end

function HeroWindowLoadoutInventory:_change_category_by_index(index)
