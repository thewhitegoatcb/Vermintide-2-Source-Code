...
function ActionBeam:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionBeam:client_owner_start_action(new_action, t, chain_action_data, power_level)
...
function ActionBeam:_start_charge_sound()
...
function ActionBeam:_stop_charge_sound()
	local current_action = self.current_action
	local owner_unit = self.owner_unit
	local owner_player = self.owner_player
	local is_bot = owner_player and owner_player.bot_player
...
function ActionBeam:client_owner_post_update(dt, t, world, can_damage)
	local owner_unit = self.owner_unit
	local current_action = self.current_action
				...
				if health_extension then
					...
					if self.damage_timer == 0 then
						...
						if health_extension then
							...
							if not self.charge_damage_profiles and
							self.consecutive_hits < 3 then
...
function ActionBeam:_stop_client_vfx()
...
function ActionBeam:finish(reason)
	if self.do_zoom then
