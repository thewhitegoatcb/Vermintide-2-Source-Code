...
local Color = Color local Vector2 = Vector2 local Vector3 = Vector3
local Gui_bitmap_uv = Gui.bitmap_uv local Gui_bitmap = Gui.bitmap
local Gui_update_bitmap_uv = Gui.update_bitmap_uv local Gui_update_bitmap = Gui.update_bitmap
...
function UIRenderer.script_draw_bitmap(gui, render_settings, material, gui_position, gui_size, color, masked, saturated, retained_id, point_sample, viewport_mask)
...
function UIRenderer.script_draw_bitmap_uv(gui, render_settings, material, uvs, gui_position, gui_size, color, masked, saturated, retained_id, point_sample, viewport_mask)
	...
	if texture_settings then
		...
		local new_uvs1 = new_uvs [1] local new_uvs2 = new_uvs [2]
...
local Gui_update_bitmap_3d_uv = Gui.update_bitmap_3d_uv local Gui_bitmap_3d_uv = Gui.bitmap_3d_uv local Gui_update_bitmap_3d = Gui.update_bitmap_3d local Gui_bitmap_3d = Gui.bitmap_3d
function UIRenderer.script_draw_bitmap_3d(gui, render_settings, material, tm, gui_layer, gui_size, color, optional_uvs, masked, retained_id)
	...
	if texture_settings then
		...
		if optional_uvs then
			...
			uv11 = texture_settings.uv11 uv00 = texture_settings.uv00
		end
	...
	elseif optional_uvs then
		local new_uvs1 = optional_uvs [1] local new_uvs2 = optional_uvs [2]
		local uv00, uv11 = nil
		uv00 = Vector2(new_uvs1 [1], new_uvs1 [2])
		uv11 = Vector2(new_uvs2 [1], new_uvs2 [2])
...
function UIRenderer.create(world, ...)
...
local UI_RENDERER_INTERFACE = table.set({ "gui", "gui_retained", "ui_scenegraph", "scenegraph_queue", "input_service", "dt", "video_players", "world", "wwise_world", "render_settings", "debug_startpoint" })
...
function UIRenderer.create_ui_renderer(world, gui, gui_retained)
...
function UIRenderer:begin_pass(ui_scenegraph, input_service, dt, parent_scenegraph_id, render_settings)
...
local DUMMY = { alpha_multiplier = 1 }
function UIRenderer:draw_all_widgets(widget_list)
...
function UIRenderer:draw_widget(widget)
	...
	for i = 1, #passes do
		...
		if ui_pass.update then
			...
			ui_pass.update(self,
			pass_data, ui_scenegraph, pass,
			pass_style, pass_content,
			input_service, dt,
		...
		if pass.retained_mode then --[[ Nothing --]]
		...
		elseif visible then
			...
			local pass_pos_x = pos_x local pass_pos_y = pos_y local pass_pos_z = pos_z
			...
			if pass_scenegraph_id then
				...
				pass_pos_z = pass_world_pos [3] pass_pos_y = pass_world_pos [2] pass_pos_x = pass_world_pos [1]
			end
			...
			pass_data, ui_scenegraph, pass,
			pass_style, pass_content,
			Vector3(pass_pos_x, pass_pos_y, pass_pos_z), pass_size,
			input_service,
			dt)
		end
	end
...
function UIRenderer:set_element_visible(ui_element, visible)
...
function UIRenderer:draw_rect(lower_left_corner, size, color, retained_id)
...
function UIRenderer:draw_triangle(lower_left_corner, size, ui_style, retained_id)
	local render_settings = self.render_settings
	local alpha_multiplier = render_settings and render_settings.alpha_multiplier or 1
...
local function debug_draw_texture(self, pos, size, texture)
	...
	local position = { pos [1], pos [2], 990 }
	local color = { 64, 255, 0, 0 }
	local frame_color = { 192, 255, 0, 0 }

	UIRenderer.draw_rect(self, position, { size [1], 1 }, frame_color)
	UIRenderer.draw_rect(self, position, { 1, size [2] }, frame_color)
	UIRenderer.draw_rect(self, { position [1] + size [1], position [2] + size [2], position [3] }, { -size [1], 1 }, frame_color)
	UIRenderer.draw_rect(self, { position [1] + size [1], position [2] + size [2], position [3] }, { 1, -size [2] }, frame_color)
	...
	if math.point_is_inside_2d_box(inv_scale * Mouse.axis(2), pos, size) then
		...
		if position [1] + text_w > 1920 then position [1] = position [1] - text_w + size [1] end
		if position [2] < 0 then position [2] = position [2] + size [2] end
		UIRenderer.draw_rect(self, position, { text_w, text_h }, frame_color)
		UIRenderer.draw_text(self, text, DEBUG_FONT_MTRL, 12, DEBUG_FONT_NAME, { position [1], position [2] + 6, position [3] })
	end
end

local uvs_draw_texture_flip_horizontal = { { 1, 0 },
	{ 0, 1 } }


function UIRenderer:draw_texture_flip_horizontal(material, lower_left_corner, size, color, masked, saturated)
...
function UIRenderer:draw_texture(material, position, size, color, masked, saturated, retained_id, point_sample, viewport_mask)
	local gui = self.gui
	if retained_id then
	...
	return UIRenderer.script_draw_bitmap(gui, self.render_settings, material,
	Vector3(position [1] * scale, position [2] * scale, position [3] or 0),
	Vector3(size [1] * scale, size [2] * scale, size [3] or 0),
	color, masked, saturated, retained_id, point_sample,
	viewport_mask)
end

function UIRenderer:draw_texture_uv(material, lower_left_corner, size, uvs, color, masked, saturated, retained_id, point_sample, viewport_mask)
	if script_data.ui_debug_draw_texture and Keyboard.button(Keyboard.button_index("v")) > 0 then
		debug_draw_texture(self, lower_left_corner, size, material)
	end
	local gui_position = UIScaleVectorToResolution(lower_left_corner)
...
function UIRenderer:draw_gradient_mask_texture(material, lower_left_corner, size, color, masked, gradient_threshold, retained_id)
	if script_data.ui_debug_draw_texture and Keyboard.button(Keyboard.button_index("v")) > 0 then
		debug_draw_texture(self, lower_left_corner, size, material)
	end
	local gui = self.gui
...
function UIRenderer:draw_multi_texture(materials, lower_left_corner, texture_size, texture_sizes, texture_offsets, tile_sizes, axis, spacing, direction, draw_count, texture_colors, color, masked, texture_saturation, saturated, retained_ids)
	...
	for i = 1, num_draws do
		...
		if texture_sizes then texture_size = texture_sizes [i] or texture_size
		end
...
local uvs_draw_tiled_texture = { { 0, 0 }, { 1, 1 } }
function UIRenderer:draw_tiled_texture(material, position, total_size, texture_size, color, masked, saturated, retained_ids)
...
function UIRenderer:draw_centered_texture_amount(material, lower_left_corner, size, texture_size, texture_amount, axis, spacing, color, texture_colors, masked, retained_ids)
	local position = UIScaleVectorToResolution(lower_left_corner)
...
function UIRenderer:draw_texture_rotated(material, size, position, angle, pivot, color, optional_uvs, masked, retained_id)
	size = UIScaleVectorToResolution(size)
	local scaled_pivot = UIScaleVectorToResolution(pivot)
	local tm = Rotation2D(Vector3.zero(), angle, Vector2(scaled_pivot [1], scaled_pivot [2]))
	local translation = Matrix4x4.translation(tm)
...
function UIRenderer:draw_text(text, font_material, font_size, font_name, position, color, retained_id, color_override)
...
function UIRenderer:draw_justified_text(text, font_material, font_size, font_name, position, color, retained_id, justify_width, ...)
...
function UIRenderer:word_wrap(text, font_material, size, width, option, optional_font_name)

	local whitespace = " 。，" local soft_dividers = " -+&/*" local return_dividers = "\n"
...
function UIRenderer:text_size(text, font_material, font_size, ...)
...
function UIRenderer:text_alignment_size(text, font_material, font_size, ...)
	local min, max = Gui.text_extents(self.gui, text, font_material, font_size, Gui.FormatDirectives, ...)
	local inv_scaling = RESOLUTION_LOOKUP.inv_scale
...
function UIRenderer:draw_video(material_name, position, size, color, video_player_reference, optional_video_player)
	if script_data.disable_video_player then
		return true
	end
...
function UIRenderer:draw_splash_video(material_name, position, size, color, video_player_reference, optional_video_player)
	if script_data.disable_video_player then
		return true
	end
...
function UIRenderer:draw_rounded_rect(position, size, radius, color)
	local scale = RESOLUTION_LOOKUP.scale
	local Gui_triangle = Gui.triangle
...
function UIRenderer.scaled_cursor_position_by_scenegraph(input_service, scenegraph, scenegraph_id, ignore_scale)
...
local crop_suffix = "..."
function UIRenderer:crop_text_width(text, max_width, style)
	...
	if max_width < text_width then
		repeat
			...
			text_width = math.floor(UIRenderer.text_size(self, text, font [1], scaled_font_size)) until text_width <= max_width
...
function UIRenderer:scaled_font_size_by_area(text, area_size, style)
	...
	for font_size = style.font_size, 1, -0.5 do
		...
		if text_height < area_height then return font_size
		end end
	return 1
end

function UIRenderer:scaled_font_size_by_width(text, max_width, style)
	local font, scaled_font_size = UIFontByResolution(style)
	local text_width = UIRenderer.text_size(self, text, font [1], scaled_font_size)
...
function UIRenderer:draw_texture_frame(position, size, texture_id, texture_size, texture_sizes, color, masked, saturated, only_corners, use_tiling, mirrored_tiling, skip_background, retained_ids)
	local gui = self.gui
	local gui_retained = self.gui_retained
	...
	local corner_size_x = corner_size_vec [1] local corner_size_y = corner_size_vec [2]

	local x_pos = position.x local y_pos = position.y
	local texture_size_x = texture_size [1] local texture_size_y = texture_size [2]
	local x_size = size.x local y_size = size.y
	...
	local corner_uv_size_x = corner_size_x / texture_size_x local corner_uv_size_y = corner_size_y / texture_size_y
	uvs [1] [1] = 0
	...
	if use_tiling then
		...
		local tile_vertical_size_x = tile_vertical_size_vec [1] local tile_vertical_size_y = tile_vertical_size_vec [2]
		...
		local tile_horizontal_size_x = tile_horizontal_size_vec [1] local tile_horizontal_size_y = tile_horizontal_size_vec [2]
