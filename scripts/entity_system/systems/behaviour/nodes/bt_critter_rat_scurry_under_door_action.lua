...
function BTCritterRatScurryUnderDoorAction:init(...)
...
function BTCritterRatScurryUnderDoorAction:enter(unit, blackboard, t)
...
function BTCritterRatScurryUnderDoorAction:leave(unit, blackboard, t, reason, destroy)
	...
	if navigation_extension:is_using_smart_object() then
		...
		if
		not success and not blackboard.exit_last_action then
...
function BTCritterRatScurryUnderDoorAction:run(unit, blackboard, t, dt)
	...
	if blackboard.scurry_state == "moving_to_door" and
	not self:_moving_to_door_update(unit, blackboard) then
		return "failed"
	end
...
function BTCritterRatScurryUnderDoorAction:_moving_to_door_update(unit, blackboard)
	local unit_position = POSITION_LOOKUP [unit]
...
function BTCritterRatScurryUnderDoorAction:_move_towards_smartobject_entrance_update(unit, blackboard, dt)
	local unit_position = POSITION_LOOKUP [unit]
	local entrance_pos = blackboard.scurry_under_entrance_pos:unbox()
...
function BTCritterRatScurryUnderDoorAction:_waiting_to_reach_end_update(unit, blackboard)
