...
function WeaponHelper.speed_to_hit_moving_target(p1, p2, projectile_angle, target_velocity, gravity, acceptable_accuracy)
...
function WeaponHelper.angle_to_hit_moving_target(p1, p2, projectile_speed, target_velocity, gravity, acceptable_accuracy, use_greatest_angle)
...
function WeaponHelper.test_angled_trajectory(physics_world, p1, p2, gravity, projectile_speed, angle, segment_list, sections, collision_filter, always_complete)
...
function WeaponHelper.ray_segmented_test(physics_world, segment_list, delta)
...
function WeaponHelper.multi_ray_test(physics_world, p1, p2, relative_positions)
...
function WeaponHelper.draw_ball_at_time(physics_world, p1, vec_flat, gravity, x_vel_0, y_vel_0, t, color)
...
function WeaponHelper:calculate_trajectory(world, initial_position, target_position, gravity, max_speed)
	local drawer = Managers.state.debug:drawer({ mode = "retained", name = "trajectory_vectors" })
	drawer:reset()
...
function WeaponHelper:_trajectory_hits_target(world, radians, speed, gravity, initial_position, target_position, normalized_target_vector, drawer)
	local points = { }
...
function WeaponHelper:position_on_trajectory(initial_position, normalized_target_vector, projectile_speed, radians, projectile_gravity, t)
...
function WeaponHelper:debug_draw_trajectory_hit(position, hit_target, drawer)
...
function WeaponHelper:ground_target(physics_world, fitting_unit, origin, velocity, gravity, collision_filter)
	...
	for i = 1, GROUND_TARGET_MAX_STEPS do
		...
		if result then
				...
				for j = 1, GROUND_TARGET_MAX_STEPS do
					...
					if new_result then
						...
						if fits_at_landing_pos then
							...
							do return true, new_hit_position end
						else
			...
			do return true, hit_position end
		end
...
function WeaponHelper:ballistic_raycast(physics_world, max_steps, max_time, position, velocity, gravity, collision_filter, visualize)
...
function WeaponHelper:look_at_enemy_or_static_position(physics_world, position, direction, side, hit_radius, length)
	local static_hit, static_hit_pos, ray_length = PhysicsWorld.immediate_raycast(physics_world,
	position,
	direction,
	length, "closest",
	"collision_filter",
	...
	local results = PhysicsWorld.linear_sphere_sweep(physics_world,
	position + direction * hit_radius / 2,
	position + direction * max_length,
	hit_radius,
	100,
	"types", "both",
