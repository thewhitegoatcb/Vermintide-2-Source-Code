...
local debug_id = 1
function AIEnemySlotExtension:init(extension_init_context, unit, extension_init_data)
	self.unit = unit
	self.target = nil
...
function AIEnemySlotExtension:extensions_ready(world, unit)
...
function AIEnemySlotExtension:cleanup_extension(unit, update_slots_ai_units, update_slots_ai_units_n)
...
function AIEnemySlotExtension:_improve_slot_position(self_unit, t, nav_world)
...
function AIEnemySlotExtension:_improve_ai_slot_position(self_unit, t, nav_world, target_unit)
	local ai_unit_position = POSITION_LOOKUP [self_unit]
...
function AIEnemySlotExtension:freeze(unit)
	self:_detach_from_slot()
	self:_detach_from_ai_slot("freeze")
end

function AIEnemySlotExtension:unfreeze(unit)
	self.target = nil
	self.improve_wait_slot_position_t = 0
end

function AIEnemySlotExtension:update(self_unit, all_slot_extensions, nav_world, t, traverse_logic, system)
...
function AIEnemySlotExtension:ai_has_slot(target_unit)
...
function AIEnemySlotExtension:on_unit_blocked_attack(ai_unit, system)
...
function AIEnemySlotExtension:ai_unit_staggered(ai_unit, system)
	if self.waiting_on_slot then
		return
	end
...
function AIEnemySlotExtension:_detach_from_slot()
...
function AIEnemySlotExtension:_detach_from_ai_slot(reason)
...
function AIEnemySlotExtension:_update_target(target_unit)
...
function AIEnemySlotExtension:_update_ai_target(target_unit)
...
function AIEnemySlotExtension:_engine_update_ai_target(target_unit)
...
function AIEnemySlotExtension:on_slot_lost()
	local slot = self.slot
...
function AIEnemySlotExtension:on_slot_gained(slot_provider_ext, slot)
...
function AIEnemySlotExtension:on_entered_slot_queue(slot_provider_ext, slot)
	local queue_slot = self.waiting_on_slot
	local previous_slot = self.slot
...
function AIEnemySlotExtension:get_current_slot()
	return self.slot or self.waiting_on_slot, self.waiting_on_slot ~= nil
end


function AIEnemySlotExtension:get_preferred_slot_type()
	return self.use_slot_type or DEFAULT_SLOT_TYPE
end

function AIEnemySlotExtension:on_ai_slot_gained(defender_unit, system)
	...
	if USE_ENGINE_SLOID_SYSTEM then
		...
		if not dogpiled_attackers then
			Managers.state.conflict.dogpiled_attackers_on_unit [defender_unit] = { [unit] = self.sloid_id }
		else
...
function AIEnemySlotExtension:on_ai_slot_lost(target_unit)
	if USE_ENGINE_SLOID_SYSTEM then
...
function AIEnemySlotExtension:free_slot(slot_consumer_ext)
	local unit = self.unit
	local blackboard = BLACKBOARDS [unit]
	if blackboard then
		blackboard.lean_dogpile = blackboard.lean_dogpile - 1
