...
local STATES = { OPEN = 3, HOTJOIN_OPEN = 4, WAITING = 1, INITIALIZING = 0, RUNNING = 2 }
...
local SOUND_EVENTS = { trigger_cursed_chest = "trigger_cursed_chest", finish_cursed_chest = "finish_cursed_chest" }
...
function DeusCursedChestExtension:init(extension_init_context, unit, extension_init_data)
	self._unit = unit
	self._is_server = Managers.player.is_server
	self:register_rpcs(extension_init_context.network_transmit.network_event_delegate)
end

function DeusCursedChestExtension:game_object_initialized(unit, go_id)
	self:_set_state(STATES.WAITING)
end

function DeusCursedChestExtension:destroy()
...
function DeusCursedChestExtension:register_rpcs(network_event_delegate)
	network_event_delegate:register(self, unpack(RPCS))
	self._network_event_delegate = network_event_delegate
end

function DeusCursedChestExtension:unregister_rpcs()
	self._network_event_delegate:unregister(self)
	self._network_event_delegate = nil
end

function DeusCursedChestExtension:update(unit, input, dt, context, t)
	...
	elseif current_state == STATES.RUNNING and

	self._is_server and
	not TerrorEventMixer.find_event("cursed_chest_prototype") then
	...
	if self._objective_unit then
		...
		elseif self._objective_unit_running_astar then
			...
			if local_player_unit then
				...
				GwNavAStar.start_with_propagation_box(
				self._objective_unit_astar,
				nav_world,
				player_position,
				chest_position,
				30,
...
function DeusCursedChestExtension:on_reward_collected()
	if self._objective_unit then
		self:_clear_objective_unit()
	end
...
function DeusCursedChestExtension:_clear_objective_unit()
...
function DeusCursedChestExtension:_set_objective_unit_activate(active)
	if self._objective_unit then
...
function DeusCursedChestExtension:get_interaction_action()
	local state = self:_get_state()
	if state == STATES.OPEN then
		do return "deus_cursed_chest_get_reward_hud_desc" end
	else
...
function DeusCursedChestExtension:on_server_interact(world, interactor_unit, interactable_unit, data, config, t, result)
	local state = self:_get_state()
	if state == STATES.WAITING then
...
function DeusCursedChestExtension:on_client_interact(world, interactor_unit, interactable_unit, data, config, t, result)
	local state = self:_get_state()
	if state == STATES.OPEN then
...
function DeusCursedChestExtension:_get_state()
...
function DeusCursedChestExtension:_set_state(state)
...
function DeusCursedChestExtension:rpc_deus_chest_looted(channel_id, go_id)
