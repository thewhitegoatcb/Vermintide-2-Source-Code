...
function PlayerUnitSmartTargetingExtension:update_opt2(unit, input, dt, context, t)
	if DISABLED then
		return
	end
	...
	local aim_screen_pos_x = res_w / 2 local aim_screen_pos_y = res_h / 2
	...
	local num_nearby_ai_units = EngineOptimized.smart_targeting_query(
	ai_broadphase,
	own_position,
	look_dir,
	1.5,
	...
	self._target_broadphase_categories)

	local highest_score = 0
	local target_unit = nil local aim_score = 0
	...
	for i = 1, num_nearby_ai_units do repeat
			...
			local aim_scalar = EngineOptimized.smart_targeting_optimized(
			camera,
			target_pos,
			look_right,
			distance,
			effective_max_range,
			range_scalar_at_effective_max_range,
			max_range,
			...
			smart_targeting_outer_width,
			smart_targeting_height_multiplier,

			( projectile_speed or 0 ) * 0.01,
			drop_multiplier,
			locomotion_velocity,
			...
			if score > 0 then
				score_modifiers [unit] = math_min(score_modifier + dt * 2, 1) end until true
	end
...
function PlayerUnitSmartTargetingExtension:update(unit, input, dt, context, t)
	...
	for i = 1, num_nearby_ai_units do repeat
			...
			if score > 0 then
				score_modifiers [unit] = math.min(score_modifier + dt * 2, 1) end until true
	end
...
function PlayerUnitSmartTargetingExtension:_get_player_camera()
...
function PlayerUnitSmartTargetingExtension:get_target_visibility_and_aim_position(target_unit, own_position, aim_assist_settings)
...
function PlayerUnitSmartTargetingExtension:get_targeting_data()
