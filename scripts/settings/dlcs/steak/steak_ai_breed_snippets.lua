...
function AiBreedSnippets.on_beastmen_minotaur_spawn(unit, blackboard)
	...
	if breed.use_charge_nav_layers then
		local allowed_layers = { planks = 1, bot_ratling_gun_fire = 1, doors = 1, destructible_wall = 0, bot_poison_wind = 1, temporary_wall = 0, fire_grenade = 1 }
...
function AiBreedSnippets.on_beastmen_minotaur_update(unit, blackboard, t)
...
function AiBreedSnippets.on_beastmen_minotaur_death(unit, blackboard, t)
	print("minotaur died!")
...
function AiBreedSnippets.on_beastmen_minotaur_despawn(unit, blackboard, t)
	local astar_data = blackboard.charge_astar_data
	if astar_data and astar_data.astar then
		local astar = astar_data.astar
		if not GwNavAStar.processing_finished(astar) then
