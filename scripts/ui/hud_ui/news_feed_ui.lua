...
function NewsFeedUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function NewsFeedUI:_create_ui_elements()
	...
	self._active_news = { }
	self.conditions_params = { rarities_to_ignore = table.enum_safe("magic") }
...
function NewsFeedUI:_sync_news(dt, t)

	if not self.is_in_inn then return
	end
...
function NewsFeedUI:_add_entry(template)
	local template_name = template.name
	...
	if template.added_func then template.added_func()
	end
	return true
end

function NewsFeedUI:_update_alignment_duration()
...
function NewsFeedUI:_update_entries_expire_time(dt, t)
...
function NewsFeedUI:_mark_entry_for_removal(index)
	local active_news = self._active_news
...
function NewsFeedUI:_remove_entry(index)
	local active_news = self._active_news
...
function NewsFeedUI:_update_alignment(dt)
...
function NewsFeedUI:_update_state_animations(dt)
...
function NewsFeedUI:_animate_widget(widget, state, progress)
...
function NewsFeedUI:set_position(x, y)
	local position = self.ui_scenegraph.pivot.local_position
	position [1] = x
	position [2] = y
end

function NewsFeedUI:destroy()
	self:set_visible(false)
end

function NewsFeedUI:set_visible(visible)
	self._is_visible = visible
	local ui_renderer = self.ui_renderer
end

function NewsFeedUI:update(dt, t)
...
function NewsFeedUI:_handle_resolution_modified()
	if RESOLUTION_LOOKUP.modified then
		self:_on_resolution_modified()
	end
end

function NewsFeedUI:_on_resolution_modified()
	return end

function NewsFeedUI:draw(dt)
	local ui_renderer = self.ui_renderer
