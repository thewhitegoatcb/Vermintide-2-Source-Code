...
local legal_texts = { "fatshark_legal_1", "gw_legal_1", "gw_legal_2", "gw_legal_3", "gw_legal_4" }
...
function TitleMainUI:init(world)
	self._world = world
	...
	local materials = { }
	if IS_WINDOWS then
		materials = { "material", "materials/ui/ui_1080p_title_screen", "material", "materials/ui/ui_1080p_start_screen", "material", "materials/ui/ui_1080p_menu_atlas_textures", "material", "materials/ui/ui_1080p_menu_single_textures", "material", "materials/fonts/gw_fonts", "material", "materials/ui/ui_1080p_common", "material",
			...
			attract_mode_video.video_name, "material",
			menu_videos.main.video_name, "material",
			menu_videos.main_menu.video_name }
	else

		materials = { "material", "materials/ui/ui_1080p_title_screen", "material", "materials/ui/ui_1080p_start_screen", "material", "materials/ui/ui_1080p_menu_atlas_textures", "material", "materials/ui/ui_1080p_menu_single_textures", "material", "materials/ui/ui_1080p_hud_single_textures", "material", "materials/ui/ui_1080p_news_splash", "material", "materials/fonts/gw_fonts", "material", "materials/ui/ui_1080p_common", "material",
...
function TitleMainUI:set_update_offline_data_enabled(enable)
	self._update_offline_data_enabled = enable
end

function TitleMainUI:_play_sound(event)
	return Managers.music:trigger_event(event)
end

function TitleMainUI:get_ui_renderer()
	return self._ui_renderer
end

function TitleMainUI:_init_animations()
...
function TitleMainUI:_create_ui_elements()
...
function TitleMainUI:_reset_menu_buttons()
	self._menu_item_animations = { }
...
function TitleMainUI:_update_animations(dt)
...
local TO_DELETE = { }
function TitleMainUI:update(dt, t, render_background_only)
	if DO_RELOAD then
		self._attract_mode_active = false
		self._game_type = nil
		self:_create_ui_elements()
...
function TitleMainUI:_destroy_video_players()
...
function TitleMainUI:_change_video(video_widget_name)
...
function TitleMainUI:_create_videos()
	if script_data.disable_video_player then
		return
	end
...
function TitleMainUI:_update_video(dt, t)
	if script_data.disable_video_player then
		return
	end
...
function TitleMainUI:_handle_menu_input(dt, t)
	...
	if navigation_allowed then
		...
		elseif not menu_item_content.disabled and input_service:get("start", true) and (
		Managers.input:is_device_active("gamepad") or menu_item_content.button_text.is_hover) then
			self:_reset_menu_buttons()
			self._menu_widgets = self._sub_menu_widgets
...
function TitleMainUI:menu_option_activated(activated)
	if activated then
...
function TitleMainUI:_set_info_text(current_index)
...
function TitleMainUI:set_game_type(is_online)
...
function TitleMainUI:game_type()
	return self._game_type
end

function TitleMainUI:offline_data_available()
	return self._game_type == "offline" and self._update_offline_data_enabled
end

function TitleMainUI:current_menu_index()
...
function TitleMainUI:active_menu_selection()
	if self._show_menu then
		if Managers.input:is_device_active("gamepad") then
			do return self._current_menu_index ~= nil end
...
function TitleMainUI:_draw(dt, t, render_background_only)
	local ui_renderer = self._ui_renderer
	local ui_scenegraph = self._ui_scenegraph
	local input_service = self.input_manager:get_service("main_menu")
	...
	if not render_background_only then
		...
		if self._show_menu then
			local render_settings = { alpha_multiplier = self._alpha_multiplier }
...
function TitleMainUI:show_menu(show, force)
	...
	if not show then
		...
		if force then
			...
			function self._ui_animation_callbacks.sidebar_fade_bg()
				self._ui_scenegraph.sidebar_fade_bg.position [1] = 544
			end
		end
	end
...
function TitleMainUI:set_start_pressed(pressed)
	if self._start_pressed ~= pressed then
		if pressed then
			self._ui_animations.legal_text_fade = UIAnimation.init(UIAnimation.function_by_time,
			self._legal_text.style.text.text_color, 1, 255, 0, 0.2, math.easeCubic)

			self._ui_animations.information_text_fade = UIAnimation.init(UIAnimation.function_by_time,
			self._information_text.style.text.text_color, 1, 0, 255, 0.5, math.easeCubic)
		else
			self._ui_animations.legal_text_fade = UIAnimation.init(UIAnimation.function_by_time,
			self._legal_text.style.text.text_color, 1, 0, 255, 0.5, math.easeCubic)

			self._ui_animations.information_text_fade = UIAnimation.init(UIAnimation.function_by_time,
			self._information_text.style.text.text_color, 1, 255, 0, 0.2, math.easeCubic)
			self._draw_information_text = nil
		end
	end
...
function TitleMainUI:anim_select_button(animation_data, index, dt)
...
function TitleMainUI:anim_deselect_button(animation_data, index, dt, optional_progress)
...
function TitleMainUI:_get_text_size(localized_text, text_style)
...
function TitleMainUI:_get_word_wrap_size(localized_text, text_style, text_area_width)
	local font, scaled_font_size = UIFontByResolution(text_style)
...
function TitleMainUI:_add_menu_item_animation(index, func, widgets)
...
function TitleMainUI:set_information_text(optinal_text)
...
function TitleMainUI:set_user_name(username)
...
function TitleMainUI:clear_user_name()
	self._draw_gamertag = nil
	self._switch_profile_blocked = nil
end

function TitleMainUI:set_playgo_status(status_text)
	self._draw_playgo = true
	self._playgo_status_widget.content.text = status_text
end

function TitleMainUI:clear_playgo_status()
	self._draw_playgo = nil
end

function TitleMainUI:set_menu_item_enable_state_by_index(item_lookup_name, enabled, sub_menu, reason)
	...
	if not enabled then
		self._disabled_buttons [item_lookup_name] = { enabled = enabled, sub_menu = sub_menu, reason = reason }
	else
		self._disabled_buttons [item_lookup_name] = nil
	end
end

function TitleMainUI:menu_input_enabled()
...
function TitleMainUI:disable_input(disable)
