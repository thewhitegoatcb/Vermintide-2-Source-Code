...
local IGNORED_BREEDS = { critter_rat = true }
...
function ActionCareerBWNecromancerCommandAttack.pre_calculate_target(owner_unit)
	...
	local num_hit_ai_units = EngineOptimized.smart_targeting_query(ai_broadphase, own_position, look_direction,
	0,
	50,
	0.1,
	0.2,
	0.8,
	5,
	hit_ai_units, dummy_table, dummy_table)
	...
	for i = 1, num_hit_ai_units do
		...
		if IGNORED_BREEDS [breed_name] then --[[ Nothing --]]
...
function ActionCareerBWNecromancerCommandAttack:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionCareerBWNecromancerCommandAttack:client_owner_start_action(new_action, t, chain_action_data, power_level)
...
function ActionCareerBWNecromancerCommandAttack:client_owner_post_update(dt, t, world, can_damage)
	return end

function ActionCareerBWNecromancerCommandAttack:destroy()
	return end

function ActionCareerBWNecromancerCommandAttack:_select_target()

	return end

function ActionCareerBWNecromancerCommandAttack:_is_charge_off_cooldown()
	return self._buff_extension:get_buff_type("sienna_necromancer_6_3_available_charge")
end

function ActionCareerBWNecromancerCommandAttack:_has_armored_pet()
