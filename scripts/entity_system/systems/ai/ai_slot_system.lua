...
local extensions = { "AIEnemySlotExtension", "AIPlayerSlotExtension", "AIAggroableSlotExtension" }
...
function AISlotSystem:init(context, system_name)
...
function AISlotSystem:hot_join_sync(peer_id, player)
	return end
...
function AISlotSystem:do_slot_search(ai_unit, set)
...
function AISlotSystem:improve_slot_position(ai_unit, t)
...
function AISlotSystem:ai_unit_have_slot(ai_unit)
	local ai_unit_extension = self.unit_extension_data [ai_unit]
	if not ai_unit_extension then
		return false
	end
...
function AISlotSystem:ai_unit_have_wait_slot(ai_unit)
	local ai_unit_extension = self.unit_extension_data [ai_unit]
	if not ai_unit_extension then
		return false
	end
...
function AISlotSystem:ai_unit_wait_slot_distance(ai_unit)
	local ai_unit_extension = self.unit_extension_data [ai_unit]
	if not ai_unit_extension then
		return math.huge
	end
...
function AISlotSystem:ai_unit_slot_position(ai_unit)
	local ai_unit_extension = self.unit_extension_data [ai_unit]
	if not ai_unit_extension then
		return nil
	end
...
function AISlotSystem:ai_unit_blocked_attack(ai_unit)
	local ai_unit_extension = self.unit_extension_data [ai_unit]
...
function AISlotSystem:ai_unit_staggered(ai_unit)
	local ai_unit_extension = self.unit_extension_data [ai_unit]
	if not ai_unit_extension or ai_unit_extension.waiting_on_slot then
		return nil
	end
...
function AISlotSystem:get_target_unit_slot_data(target_unit, slot_type)
...
function AISlotSystem:slots_count(unit, slot_type)
...
function AISlotSystem:total_slots_count(unit, slot_type)
	local unit_extension = self.unit_extension_data [unit]
	local slot_type = slot_type or DEFAULT_SLOT_TYPE
	local slot_data = unit_extension.all_slots [slot_type]
	local total_slots_count = slot_data.total_slots_count
...
function AISlotSystem:disabled_slots_count(unit, slot_type)
	local unit_extension = self.unit_extension_data [unit]
	local slot_type = slot_type or DEFAULT_SLOT_TYPE
	local slot_data = unit_extension.all_slots [slot_type]
...
function AISlotSystem:set_release_slot_lock(unit, release_slot_lock)
	local unit_extension = self.unit_extension_data [unit]
...
local function get_best_slot(target_unit, target_units, ai_unit, unit_extension_data, nav_world, t, skip_slots_behind_target)
	...
	for i = 1, total_slots_count do repeat
			...
			if slot_score < best_score then
				best_slot = slot
				best_score = slot_score end until true
	end


	if best_slot then repeat
			...
			update_anchor_weights(target_unit, unit_extension_data) until true
	end
...
function AISlotSystem:update(context, t, dt)
...
function AISlotSystem:physics_async_update(context, t)
	self.t = t
...
function AISlotSystem:update_ai_unit_slot(ai_unit, target_units, unit_extension_data, nav_world, t)
...
function AISlotSystem:update_target_slots(t, target_unit, target_units, unit_extension_data, target_unit_extension, nav_world, traverse_logic)
	local dist_sq = 0
...
function AISlotSystem:update_total_slots_count(t)
	local target_units = self.target_units
	local target_units_n = #target_units
...
function AISlotSystem:register_prioritized_ai_unit_update(unit)
	self.update_slots_ai_units_prioritized [unit] = true
end

function AISlotSystem:prioritize_queued_units_on_slot(slot)
...
local dummy_input = { }
function AISlotSystem:on_add_extension(world, unit, extension_name, extension_init_data)
	local extension = { }
...
function AISlotSystem:extensions_ready(world, unit, extension_name)
	if extension_name == "AIEnemySlotExtension" then
...
function AISlotSystem:on_remove_extension(unit, extension_name)
	self.frozen_unit_extension_data [unit] = nil
	self:_cleanup_extension(unit, extension_name)
	ScriptUnit.remove_extension(unit, self.NAME)
end

function AISlotSystem:on_freeze_extension(unit, extension_name)
	local extension = self.unit_extension_data [unit]
...
function AISlotSystem:_cleanup_extension(unit, extension_name)
	local extension = self.unit_extension_data [unit]
...
function AISlotSystem:freeze(unit, extension_name, reason)
...
function AISlotSystem:unfreeze(unit)
...
function debug_draw_slots(target_units, unit_extension_data, nav_world, t)
	local drawer = Managers.state.debug:drawer({ mode = "immediate", name = "AISlotSystem_immediate" })
	...
	for j = 1, #sides do
		...
		for i_target, target_unit in pairs(targets) do repeat
				...
				for slot_type, slot_data in pairs(all_slots) do
					...
					for i = 1, target_slots_n do repeat
							...
							if slot.absolute_position then
								...
								local category = "wait_slot_index_" .. slot_type .. "_" .. slot.index .. "_" .. i
								Managers.state.debug_text:clear_world_text(category) end until true end end until true
		end
	end
...
function AISlotSystem:set_allowed_layer(layer_name, allowed)
...
SLOT_COLORS = { { "aqua_marine", "cadet_blue", "corn_flower_blue", "dodger_blue", "sky_blue", "midnight_blue", "medium_purple", "blue_violet", "dark_slate_blue" },
