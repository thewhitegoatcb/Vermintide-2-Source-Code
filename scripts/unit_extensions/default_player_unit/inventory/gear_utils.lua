...
function GearUtils.create_equipment(world, slot_name, item_data, unit_1p, unit_3p, is_bot, unit_template, extra_extension_data, ammo_percent, override_item_template, override_item_units, career_name)
...
function GearUtils.apply_properties_to_item_template(template, backend_id)
...
function GearUtils.apply_material_settings(unit, material_settings)
...
function GearUtils.spawn_inventory_unit(world, hand, item_template, item_units, slot_name, item_data, owner_unit_1p, owner_unit_3p, unit_template, extra_extension_data, ammo_percent, material_settings)
	...
	if item_template.uses_weapon_system_on_3p and not owner_unit_1p then
		extension_init_data_3p = {
			weapon_system = { item_template = item_template } }
	else
	...
	if owner_unit_1p then
		...
		if ammo_data and ammo_hand == hand then
			...
			if default_spread_template then unit_template_1p = "weapon_unit_ammo_spread" else unit_template_1p = "weapon_unit_ammo"
			end
		elseif default_spread_template then unit_template_1p = "weapon_unit_spread" else unit_template_1p = "weapon_unit"
		end
...
function GearUtils._attach_ammo_unit(world, ammo_unit_name, attachment_node_linking, target_unit)
...
function GearUtils.link(world, attachment_node_linking, scene_graph_links, user_unit, unit)
	table.clear(scene_graph_links)
	GearUtils.link_units(world, attachment_node_linking, scene_graph_links, user_unit, unit)
end

function GearUtils.link_units(world, attachment_node_linking, link_table, source, target)
	for i, attachment_nodes in ipairs(attachment_node_linking) do
		...
		link_table [#link_table + 1] = { unit = target, i = target_node_index, parent = Unit.scene_graph_parent(target, target_node_index), local_pose = Matrix4x4Box(Unit.local_pose(target, target_node_index)) }

		World.link_unit(world, target, target_node_index, source, source_node_index)
	end
end

function GearUtils.restore_scene_graph(scene_graph_links)
...
function GearUtils.destroy_wielded(world, wielded_unit)
...
function GearUtils.get_ammo_extension(right_hand_unit, left_hand_unit)
...
function GearUtils.destroy_equipment(world, equipment)
	local unit_spawner = Managers.state.unit_spawner
...
function GearUtils.destroy_slot(world, unit, slot_data, equipment, allow_destroy_weapon)
...
local temp_table = { }
function GearUtils.hot_join_sync(peer_id, unit, equipment, additional_items)
	local slots = equipment.slots
	local unit_object_id = Managers.state.unit_storage:go_id(unit)

	local channel_id = PEER_ID_TO_CHANNEL [peer_id]
	for slot_name, slot_data in pairs(slots) do repeat
			...
			RPC.rpc_add_equipment(channel_id, unit_object_id, slot_id, item_id, weapon_skin_id) until true
	end
...
function GearUtils._setup_extension_init_data_type_impact(item_template, item_name)
...
function GearUtils._setup_extension_init_data_type_dot(item_template, item_name)
...
function GearUtils.create_grenade_extension_init_data(owner_unit, item_name, current_action, projectile_data, explode_time)
...
function GearUtils.get_property_and_trait_buffs(backend_items, backend_id, buffs_table, only_no_wield_required)
		...
		for property_key, property_value in pairs(properties) do
			...
			if BuffTemplates [buff_name] then
				if only_no_wield_required and no_wield_required then
					buffs_table [buffer] [buff_name] = { variable_value = property_value }
	...
	if traits then
		...
		for _, trait_key in pairs(traits) do
			...
			if BuffTemplates [buff_name] then
				if only_no_wield_required and no_wield_required then
					buffs_table [buffer] [buff_name] = { variable_value = 1 }
				elseif not only_no_wield_required and not no_wield_required then
					buffs_table [buffer] [buff_name] = { variable_value = 1 }
				end
			end
...
function GearUtils.create_attached_particles(world, particle_fx, equipment, unit_3p, unit_1p, is_first_person)
...
function GearUtils.destroy_attached_particles(world, fx_ids)
