...
function WeaveTimerUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function WeaveTimerUI:destroy()
	return end

function WeaveTimerUI:_create_ui_elements()
...
function WeaveTimerUI:update(dt, t)
...
function WeaveTimerUI:_play_sound(sound_event)
	WwiseWorld.trigger_event(self._wwise_world, sound_event)
end

function WeaveTimerUI:_update_timer(dt, t)
...
function WeaveTimerUI:_update_bar(dt, t)
	local weave_manager = Managers.weave
	if weave_manager:get_active_weave() then
		local remaining_time_in_seconds = weave_manager:get_time_left()
...
function WeaveTimerUI:_draw(dt, t)
