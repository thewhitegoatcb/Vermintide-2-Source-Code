...
local network_options = { project_hash = "bulldozer", config_file_name = "global",
...
local GAME_MODE_LOOKUP_STRINGS = { weave = "lb_game_type_weave", deed = "lb_game_type_deed", event = "lb_game_type_event", custom = "lb_game_type_custom", demo = "lb_game_type_none", adventure = "lb_game_type_quick_play", tutorial = "lb_game_type_prologue", twitch = "lb_game_type_twitch", ["n/a"] = "lb_game_type_none", any = "lobby_browser_mission" }
...
function StartGameWindowLobbyBrowserConsole:on_enter(params, offset)
...
function StartGameWindowLobbyBrowserConsole:get_selected_game_mode_index()
	local game_modes = self._game_mode_data.game_modes
	return self._selected_game_mode_index or game_modes.adventure
end

local EMPTY_DATA = { }
function StartGameWindowLobbyBrowserConsole:cb_friends_collected(friend_data)
...
function StartGameWindowLobbyBrowserConsole:change_generic_actions(input_actions)
	self._parent:change_generic_actions(input_actions)
end

function StartGameWindowLobbyBrowserConsole:set_input_description(input_actions)
	self._parent:set_input_description(input_actions)
end

function StartGameWindowLobbyBrowserConsole:on_exit(params)
...
function StartGameWindowLobbyBrowserConsole:disable_input(input_name)
	return input_name == "show_gamercard"
end

function StartGameWindowLobbyBrowserConsole:update(dt, t)
...
function StartGameWindowLobbyBrowserConsole:post_update(dt, t)
	return end


function StartGameWindowLobbyBrowserConsole:_is_refreshing()
...
function StartGameWindowLobbyBrowserConsole:play_sound(event)
...
function StartGameWindowLobbyBrowserConsole:cancel_join_lobby(status_message)
...
function StartGameWindowLobbyBrowserConsole:_populate_lobby_list(auto_update)
...
function StartGameWindowLobbyBrowserConsole:get_lobbies()
...
local REQUIRED_DLCS = { }
function StartGameWindowLobbyBrowserConsole:_valid_lobby(lobby_data)
	...
	if not is_server then
		...
		if mechanism == "weave" then
			...
			if weave_template then level_key = weave_template.objectives [1].level_id or level_key
			end
		end
...
function StartGameWindowLobbyBrowserConsole:_is_friend_lobby(lobby_data)
	local name = lobby_data.name
	print(name, self._friend_names [name])
	return self._friend_names [name] ~= nil
end

function StartGameWindowLobbyBrowserConsole:input_service()
	return self._parent:window_input_service()
end

function StartGameWindowLobbyBrowserConsole:dirty()
	local dirty = self._dirty
	self._dirty = false
	return dirty
end

function StartGameWindowLobbyBrowserConsole:_update_auto_refresh(dt)
	local is_refreshing = self:_is_refreshing()
...
function StartGameWindowLobbyBrowserConsole:reset_filters(selected_game_mode, selected_level, selected_difficulty, selected_filter, selected_distance)
...
function StartGameWindowLobbyBrowserConsole:_create_filter_requirements()
	local lobby_finder = self._lobby_finder
...
function StartGameWindowLobbyBrowserConsole:_join(lobby_data, join_params)
	Managers.matchmaking:request_join_lobby(lobby_data, join_params)
	self.join_lobby_data_id = lobby_data.id
end

function StartGameWindowLobbyBrowserConsole:_search(skip_populate)
...
function StartGameWindowLobbyBrowserConsole:_get_game_modes()
...
function StartGameWindowLobbyBrowserConsole:_get_levels()
	local game_mode_data = self._game_mode_data
	local game_modes = game_mode_data.game_modes
...
function StartGameWindowLobbyBrowserConsole:_get_difficulties()
	local game_mode_data = self._game_mode_data
	local game_modes = game_mode_data.game_modes
	local game_mode_index = self._selected_game_mode_index or game_modes.adventure
...
function StartGameWindowLobbyBrowserConsole:completed_level_difficulty_index(lobby_data)
...
function StartGameWindowLobbyBrowserConsole:refresh()
	if not self:_is_refreshing() then
		self:_search()
	end
end

function StartGameWindowLobbyBrowserConsole:set_game_mode(game_mode)
...
function StartGameWindowLobbyBrowserConsole:set_level(level)
	local levels_table = self:_get_levels()
...
function StartGameWindowLobbyBrowserConsole:set_difficulty(difficulty)
...
function StartGameWindowLobbyBrowserConsole:set_lobby_filter(lobby_filter)
...
function StartGameWindowLobbyBrowserConsole:set_distance_filter(distance)
...
function StartGameWindowLobbyBrowserConsole:is_lobby_joinable(lobby_data)
	local mission_id = lobby_data.selected_mission_id or lobby_data.mission_id
	local difficulty = lobby_data.difficulty
	local num_players = tonumber(lobby_data.num_players)
	...
	if mechanism == "weave" then
		if mission_id ~= "false" and not quick_game then
			...
			if not weave_unlocked then
				do return false, "weave_not_unlocked" end
			end
		end
		...
		if not level_unlocked then
			...
			if dlc_name then
				...
				if not is_unlocked then
					do return false, "dlc1_2_dlc_level_locked_tooltip" end
				else
					do return false, "start_game_level_locked" end
				end
			else
				return false, "start_game_level_locked"
			end
