...
function CameraStateObserver:init(camera_state_init_context)
...
function CameraStateObserver:on_enter(unit, input, dt, context, t, previous_state, params)
...
function CameraStateObserver:on_exit(unit, input, dt, context, t, next_state)
...
function CameraStateObserver:update(unit, input, dt, context, t)
	local csm = self.csm
...
function CameraStateObserver:_get_valid_players_to_observe()
...
function CameraStateObserver:follow_next_unit()
...
function CameraStateObserver:follow_previous_unit()
	local players = self:_get_valid_players_to_observe()
	if not players then
		return false
	end
	...
	repeat
		...
		elseif current_player_id then
			...
			if Unit.alive(player_unit) then
				...
				previous_player_unit = player_unit end end until current_player_id == nil
...
function CameraStateObserver:_set_follow_unit(observed_player_id, follow_unit)
