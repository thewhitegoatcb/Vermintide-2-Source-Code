...
local SETTINGS = { damage_entry = 40, random_direction = true, anticipation_delay = 5, random_start_rotation = true, damage_tick = 15, rotation_speed = 10, damage_tick_rate = 0.5,



	buffs_on_entry = { "wall_slow_debuff" },
	buffs_on_tick = { "wall_slow_debuff" },

	starting_state = STATE_STOPPED,



	areas = { { angle_offset = 0, height_min = -1, length = 35, height_max = 5, flow_name = "first", width = 10, length_offset = 1 },
		...
		{ height_min = -1, length = 35, height_max = 5, flow_name = "second", width = 10, length_offset = 1,
	...
	init_func = function ()
		return end }
function SETTINGS.update_func(world, unit, settings, state, is_activating, area_settings, area_data, angle, rotation_direction, t)
...
function RotatingHazardExtension:init(extension_init_context, unit, extension_init_data)
	...
	for i = 1, self._num_areas do
		...
		self._area_data [i] = { overlapping_units = { } }
	end
end

function RotatingHazardExtension:hot_join_sync(peer_id)
...
function RotatingHazardExtension:network_sync(start_t, pause_t, state, seed)
...
function RotatingHazardExtension:destroy()
	return end

function RotatingHazardExtension:start(reset_rotation)
...
function RotatingHazardExtension:_update_random_settings_from_seed(seed)
	local new_seed = seed
...
function RotatingHazardExtension:pause()
...
function RotatingHazardExtension:stop()
...
function RotatingHazardExtension:update(unit, input, dt, context, t)
	...
	if state == STATE_RUNNING and
	self._is_activating ~= is_activating then
...
function RotatingHazardExtension:_update_damage(area_settings, area_data, angle, dt, t)
