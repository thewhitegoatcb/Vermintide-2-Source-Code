...
function BTVortexWanderAction:init(...)
	BTVortexWanderAction.super.init(self, ...)
end

BTVortexWanderAction.name = "BTVortexWanderAction"

function BTVortexWanderAction:enter(unit, blackboard, t)
	local action = self._tree_node.action_data
	blackboard.action = action
end

function BTVortexWanderAction:leave(unit, blackboard, t, reason, destroy)
	return end

function BTVortexWanderAction:run(unit, blackboard, t, dt)
	local vortex_data = blackboard.vortex_data
	self:_wander_around(unit, t, dt, blackboard, vortex_data)
	return "running"
end

function BTVortexWanderAction:_wander_around(unit, t, dt, blackboard, vortex_data)
	...
	elseif wander_state ~= "standing_still" and wander_state == "forced_standing_still" then --[[ Nothing --]]
	end
