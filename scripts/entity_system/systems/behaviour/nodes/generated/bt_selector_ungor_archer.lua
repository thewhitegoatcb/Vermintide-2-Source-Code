...
function BTSelector_ungor_archer:init(...)
	BTSelector_ungor_archer.super.init(self, ...)
	self._children = { }
end

function BTSelector_ungor_archer:leave(unit, blackboard, t, reason)
	self:set_running_child(unit, blackboard, t, nil, reason)
end

function BTSelector_ungor_archer:run(unit, blackboard, t, dt)
	...
	local node_switch_weapons = children [6]
	local condition_result = BTConditions.ungor_archer_enter_melee_combat(blackboard) and not blackboard.has_switched_weapons
...
function BTSelector_ungor_archer:add_child(node)
