...
local function get_possible_skins(item_key, rarity)
	...
	if deus_item_data.fixed_skin then
		skins = { deus_item_data.fixed_skin }
...
function DeusWeaponGeneration.serialize_weapon(item)
	local string_array = { }
...
function DeusWeaponGeneration.deserialize_weapon(item_string)
...
function DeusWeaponGeneration.create_weapon(item_key, properties, traits, skin, powerlevel, rarity)
	return create_item(item_key, properties, traits, skin, powerlevel, rarity)
end

function DeusWeaponGeneration.get_possibilities_for_item_key(item_key, difficulty, run_progress, rarity)
	local archetypes = get_possible_archetypes(item_key)
	local skins = get_possible_skins(item_key, rarity)
	local powerlevel = get_power_level(rarity, difficulty, run_progress)
	...
	trait_combinations,
	skins and #skins > 0 and skins or nil
end

function DeusWeaponGeneration.generate_item_from_item_key(item_key, difficulty, run_progress, rarity, seed)
...
function DeusWeaponGeneration.get_random_rarity(difficulty, run_progress, seed)
	local random_generator = DeusGenUtils.create_random_generator(seed)
	return get_random_rarity(random_generator, difficulty, run_progress)
end

function DeusWeaponGeneration.upgrade_item(item, difficulty, run_progress, target_rarity, seed)
	local random_generator = DeusGenUtils.create_random_generator(seed)
	return upgrade_item(item, difficulty, run_progress, target_rarity, random_generator)
end

function DeusWeaponGeneration.generate_weapon(difficulty, run_progress, rarity, seed, weapon_pool, slot_chance_melee, slot_chance_ranged)
	local random_generator = DeusGenUtils.create_random_generator(seed)
...
function DeusWeaponGeneration.generate_weapon_for_slot(difficulty, run_progress, rarity, seed, weapon_pool, slot)
	local random_generator = DeusGenUtils.create_random_generator(seed)
	local weapon_key = get_random_weapon_key(slot, rarity, weapon_pool, random_generator)
	if not weapon_key then
		return
...
function DeusWeaponGeneration.generate_weapon_pool(career, weapon_group_whitelist)
	local weapon_pool = { }
...
function DeusWeaponGeneration.get_weapon_pool_slot_amounts(base_weapon_pool, weapon_pool)
	local slot_amounts = { }
	local deus_weapon_groups = DeusWeaponGroups
