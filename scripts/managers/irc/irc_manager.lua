...
function IRCManager:init()
	self:_reset()
end

function IRCManager:_reset()
	self._state = "none"
...
function IRCManager:connect(user_name, optional_password, settings, cb)
...
function IRCManager:home_channel()
	return self._home_channel
end

function IRCManager:set_user_name(user_name)
...
function IRCManager:register_message_callback(key, message_type, callback)
...
function IRCManager:unregister_message_callback(key, optional_message_type)
...
function IRCManager:user_name()
	return self._user_name
end

function IRCManager:force_disconnect()
	Irc.disconnect()
end

function IRCManager:send_message(message, channel_or_user)
	if self._allow_send then
		...
		if channel_or_user == self._user_name then
			...
			MESSAGES_TO_SEND [#MESSAGES_TO_SEND + 1] = { message = message, channel_or_user = channel_or_user }
			do return true end
		end
	else
...
function IRCManager:join_channel(channel)
	debug_print("Joining Channel: %s", tostring(channel))
	Irc.join_channel(channel)
end

function IRCManager:leave_channel(channel)
	debug_print("Leaving Channel: %s", tostring(channel))
	Irc.leave_channel(channel)
end

function IRCManager:who(channel)
	Irc.who(channel)
end

function IRCManager:destroy()
	Irc.disconnect()
end

function IRCManager:_handle_irc_message(message_type, username, message, parameter)
...
function IRCManager:_handle_connections(message_type, username, message, parameter)
	...
	if message_type == Irc.NAMES_MSG then
		...
		for _, name in ipairs(new_members) do
			if not members [name] then
				members [name] = { icon_id = 0, info = "", level = "n/a", name = name, time = Managers.time:time("main") }
			end
		end
	...
	elseif message_type == Irc.JOIN_MSG then
		...
		if username == self._user_name then
			...
			user_data = { name = username, time = Managers.time:time("main"), icon_id = icon_id, level = level, info = info }
...
function IRCManager:_handle_meta(message_type, username, message, parameter)
...
function IRCManager:_create_metadata_table(username, icon_id, level, info)
	local message = "$META;"
...
function IRCManager:parse_metadata(meta_data, username, parameter)
...
function IRCManager:_update_meta_data(username, channel, user_data)
...
function IRCManager:get_channel_members(channel_name)
	if channel_name and self._channel_members [channel_name] then
		do return self._channel_members [channel_name] end
	else
		return { }
	end
end

function IRCManager:get_channels()
	return self._channels
end

function IRCManager:_parse_names_list(channel, username, members, names_list_parameter)
...
function IRCManager:cb_connect_token_received(data)
...
function IRCManager:_change_state(state)
...
function IRCManager:_notify_connected(connected)
...
function IRCStates.none(irc, dt) return end

function IRCStates.initialize(irc_manager, dt)
	if IS_PS4 then
...
function IRCStates.connect(irc_manager, dt)
...
function IRCStates.join_channel(irc_manager, dt)
...
function IRCStates.connected(irc_manager, dt)
	local is_connected = Irc.is_connected()
	if is_connected then
...
function IRCStates.disconnect(irc_manager, dt)
	local is_connected = Irc.is_connected()
	if is_connected then
		Irc.disconnect()
	end
...
function IRCStates.verify_connection(irc_manager, dt)
	local is_connected = Irc.is_connected()
	if is_connected then
...
function IRCStates.wait_for_connection(irc_manager, dt)
