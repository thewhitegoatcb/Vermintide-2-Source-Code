...
function HeroWindowOptions:on_enter(params, offset)
...
function HeroWindowOptions:create_ui_elements(params, offset)
...
function HeroWindowOptions:on_exit(params)
	print("[HeroViewWindow] Exit Substate HeroWindowOptions")
	self.ui_animator = nil
end

function HeroWindowOptions:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function HeroWindowOptions:post_update(dt, t)
	self:_handle_input(dt, t)
end

function HeroWindowOptions:_update_animations(dt)
...
function HeroWindowOptions:_is_button_pressed(widget)
...
function HeroWindowOptions:_is_stepper_button_pressed(widget)
	local content = widget.content
...
function HeroWindowOptions:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_enter and not hotspot.is_selected
end

function HeroWindowOptions:_is_button_hover_exit(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_exit and not hotspot.is_selected
end

function HeroWindowOptions:_is_button_selected(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.is_selected
end

function HeroWindowOptions:_handle_input(dt, t)
	local widgets_by_name = self._widgets_by_name
...
function HeroWindowOptions:_update_game_options_hover_effect(dt)
	local widgets_by_name = self._widgets_by_name
...
function HeroWindowOptions:_set_selected_option(index)
	local widgets_by_name = self._widgets_by_name
	local widget_prefix = "game_option_"
	for i = 1, 4 do
		local widget_name = widget_prefix .. i
...
function HeroWindowOptions:_update_selected_option()
...
function HeroWindowOptions:_update_loadout_sync()
	local parent = self.parent
...
function HeroWindowOptions:_has_hero_level_changed()
...
function HeroWindowOptions:_update_experience_presentation()
	local widgets_by_name = self._widgets_by_name
...
function HeroWindowOptions:_calculate_power_level()
	local hero_name = self.hero_name
	local career_index = self.career_index
...
function HeroWindowOptions:_update_hero_power_effect(dt)
...
function HeroWindowOptions:_update_hero_portrait_frame()
	local career_index = self.career_index
...
function HeroWindowOptions:draw(dt)
...
function HeroWindowOptions:_play_sound(event)
	self.parent:play_sound(event)
end

function HeroWindowOptions:_create_portrait_frame_widget(frame_settings_name, portrait_texture, level_text)
...
function HeroWindowOptions:_get_portrait_frame()
	local profile_index = self.profile_index
	local career_index = self.career_index
	local hero_name = self.hero_name
...
function HeroWindowOptions:_create_style_animation_enter(widget, target_value, style_id, widget_index, instant)
	local ui_animations = self._ui_animations
...
function HeroWindowOptions:_create_style_animation_exit(widget, target_value, style_id, widget_index, instant)
	local ui_animations = self._ui_animations
	local animation_name = "game_option_" .. style_id
...
function HeroWindowOptions:_animate_element_by_time(target, target_index, from, to, time)
	local new_animation = UIAnimation.init(UIAnimation.function_by_time, target, target_index,
	from, to, time, math.ease_out_quad)
	return new_animation
end

function HeroWindowOptions:_animate_element_by_catmullrom(target, target_index, target_value, p0, p1, p2, p3, time)
	local new_animation = UIAnimation.init(UIAnimation.catmullrom, target, target_index, target_value,
	p0, p1, p2, p3, time)
	return new_animation
end

function HeroWindowOptions:_sync_news(dt, t)
