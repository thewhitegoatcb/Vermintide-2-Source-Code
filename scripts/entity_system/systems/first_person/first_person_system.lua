...
local RPCS = { "rpc_play_hud_sound_event", "rpc_play_first_person_sound", "rpc_play_husk_sound_event", "rpc_play_husk_unit_sound_event", "rpc_first_person_flow_event" }
...
local EXTENSIONS = { "PlayerUnitFirstPerson", "PlayerBotUnitFirstPerson" }




function FirstPersonSystem:init(entity_system_creation_context, system_name)
...
function FirstPersonSystem:destroy()
	self.network_event_delegate:unregister(self)
	self.network_event_delegate = nil
end

function FirstPersonSystem:rpc_play_first_person_sound(channel_id, unit_id, sound_id, position)
...
function FirstPersonSystem:rpc_play_hud_sound_event(channel_id, unit_id, event_id)
	local unit = self.unit_storage:unit(unit_id)
	if not unit then
		printf("unit from game_object_id %d is nil", unit_id)
		return
...
function FirstPersonSystem:rpc_play_husk_sound_event(channel_id, unit_id, event_id)
...
function FirstPersonSystem:rpc_play_husk_unit_sound_event(channel_id, unit_id, node_id, event_id)
	if self.is_server then
		local peer_id = CHANNEL_TO_PEER_ID [channel_id]
...
function FirstPersonSystem:rpc_first_person_flow_event(channel_id, unit_id, event_id)
	local unit = self.unit_storage:unit(unit_id)
	if not unit then
		printf("unit from game_object_id %d is nil", unit_id)
		return
