...
local EMPTY_TABLE = { }
function BTAttackAction:enter(unit, blackboard, t)
...
function BTAttackAction:_select_attack(action, unit, target_unit, blackboard, target_unit_status_extension)
	...
	if target_exception_attack then
		do return target_exception_attack end
	else
		...
		elseif knocked_down_attack and z_offset < knocked_down_attack.z_threshold and target_unit_status_extension and target_unit_status_extension:is_knocked_down() then
			do return knocked_down_attack end
		elseif

		step_attack_with_callback and (blackboard.target_speed_away > (step_attack_with_callback.step_speed_moving or 1) and
		flat_distance > (step_attack_with_callback.step_distance_moving or 1.5) or

		flat_distance > (step_attack_with_callback.step_distance_stationary or 2.5)) then
		...
		step_attack and (blackboard.target_speed_away > (step_attack.step_speed_moving or 1) and
		flat_distance > (step_attack.step_distance_moving or 1.5) or

		flat_distance > (step_attack.step_distance_stationary or 2.5)) then
...
function BTAttackAction:leave(unit, blackboard, t, reason, destroy)
...
function BTAttackAction:clear_blackboard(unit, blackboard, t)
...
function BTAttackAction:run(unit, blackboard, t, dt)
...
function BTAttackAction:attack_cooldown(unit, blackboard)
...
function BTAttackAction:attack_success(unit, blackboard)
	local breed = blackboard.breed
...
function BTAttackAction:attack_blocked(unit, blackboard, direction)
...
function BTAttackAction:_attack(unit, t, dt, blackboard)
	local bb = blackboard
...
function BTAttackAction:_handle_movement(unit, t, dt, blackboard)
	local bb = blackboard
...
function BTAttackAction:_get_attack_cooldown_finished_at(unit, blackboard, t)
...
function BTAttackAction:anim_cb_attack_vce(unit, blackboard)
	local network_manager = Managers.state.network

	local game = network_manager:game()
	if
	game and blackboard.target_unit_status_extension then
		DialogueSystem:trigger_attack(blackboard, blackboard.target_unit, unit, false, false)
	end

end

function BTAttackAction:anim_cb_attack_vce_long(unit, blackboard)
	local network_manager = Managers.state.network
...
function BTAttackAction:anim_cb_running_attack_start(unit, blackboard)
	local network_manager = Managers.state.network
...
function BTAttackAction:anim_cb_attack_finished(unit, blackboard)
