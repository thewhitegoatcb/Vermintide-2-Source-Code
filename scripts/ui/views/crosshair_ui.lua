...
local CROSSHAIR_STYLE_FUNCTIONS = { default = "draw_default_style_crosshair", circle = "draw_circle_style_crosshair", wh_priest = "draw_wh_priest_style_crosshair", shotgun = "draw_shotgun_style_crosshair", dot = "draw_dot_style_crosshair", arrows = "draw_arrows_style_crosshair", projectile = "draw_projectile_style_crosshair" }
...
function CrosshairUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function CrosshairUI:create_ui_elements()
...
function CrosshairUI:update(dt, t, player)
...
local crosshairs = { }
function CrosshairUI:update_enabled_crosshair_styles()
...
function CrosshairUI:update_crosshair_style(equipment)
...
function CrosshairUI:_apply_crosshair_position(x, y)
	local position = self.ui_scenegraph.pivot.local_position
	position [1] = x
	position [2] = y
end

function CrosshairUI:update_hit_markers(dt)
...
function CrosshairUI:set_hit_marker_animation(hit_markers, hit_markers_n, hit_marker_animations, hit_marker_data)
	for i = 1, hit_markers_n do
...
function CrosshairUI:configure_hit_marker_color_and_size(hit_marker, hit_marker_data)
...
function CrosshairUI:update_hit_marker_animation(hit_markers, hit_markers_n, hit_marker_animations, hud_extension, dt)
	for i = 1, hit_markers_n do
...
function CrosshairUI:update_spread(dt, t, equipment)

	local wielded_item_data = equipment.wielded
	local item_template = BackendUtils.get_item_template(wielded_item_data)
	local pitch = 0 local yaw = 0
...
function CrosshairUI:draw(dt, t, pitch_percentage, yaw_percentage)
...
function CrosshairUI:draw_default_style_crosshair(ui_renderer, pitch_percentage, yaw_percentage)
...
function CrosshairUI:draw_arrows_style_crosshair(ui_renderer, pitch_percentage, yaw_percentage)
	UIRenderer.draw_widget(ui_renderer, self.crosshair_dot)
...
function CrosshairUI:draw_shotgun_style_crosshair(ui_renderer, pitch_percentage, yaw_percentage)
	UIRenderer.draw_widget(ui_renderer, self.crosshair_dot)
...
function CrosshairUI:draw_projectile_style_crosshair(ui_renderer, pitch_percentage, yaw_percentage)
	UIRenderer.draw_widget(ui_renderer, self.crosshair_dot)
...
function CrosshairUI:draw_dot_style_crosshair(ui_renderer, pitch_percentage, yaw_percentage)
	UIRenderer.draw_widget(ui_renderer, self.crosshair_dot)
end

function CrosshairUI:draw_circle_style_crosshair(ui_renderer, pitch_percentage, yaw_percentage)
	UIRenderer.draw_widget(ui_renderer, self.crosshair_circle)
end

function CrosshairUI:draw_wh_priest_style_crosshair(ui_renderer, pitch_percentage, yaw_percentage)
	UIRenderer.draw_widget(ui_renderer, self.wh_priest)
end

function CrosshairUI:_set_widget_point_offset(widget, point_index, max_points, pitch_percentage, yaw_percentage, start_degrees, pitch_offset, yaw_offset)
...
function CrosshairUI:_get_point_offset(point_index, max_points, pitch_percentage, yaw_percentage, start_degrees)
...
function CrosshairUI:_set_crosshair_target_info(portrait, state)
...
function CrosshairUI:_update_self_to_ally_transition()
	local content = self.wh_priest.content
...
function CrosshairUI:_update_animations(dt)
	local ui_animator = self._ui_animator
	ui_animator:update(dt)
end

function CrosshairUI:destroy()
...
function CrosshairUI:update_game_options()
...
function CrosshairUI:_register_kill_confirm(killing_blow, breed_killed, ai_unit)
	...
	if kill_confirm_type then
		...
		if self._kill_confirm_enabled_groups [kill_confirm_enemy_type] and (
		not self._current_kill_confirm_type or not self._current_kill_confirm_widget or
		kill_confirm_enemy_prio [self._current_kill_confirm_type] <= kill_confirm_enemy_prio [kill_confirm_enemy_type] or
		self._current_kill_confirm_widget.style.color [1] <= kill_confirm_cutoff_alpha) then
...
function CrosshairUI:_draw_kill_confirm(dt, t, ui_renderer)
