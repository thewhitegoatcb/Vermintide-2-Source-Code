...
function BTTentacleAttackAction:init(...)
	BTTentacleAttackAction.super.init(self, ...)
end

function BTTentacleAttackAction:enter(unit, blackboard, t)
...
function BTTentacleAttackAction:sync_state_to_clients(unit, blackboard, template_name, reach_dist, t)
...
function BTTentacleAttackAction:leave(unit, blackboard, t, reason, destroy)

	blackboard.tentacle_satisfied = true
end

local Unit_alive = Unit.alive
function BTTentacleAttackAction:run(unit, blackboard, t, dt)
...
function BTTentacleAttackAction:update_tentacle(unit, blackboard, t, dt)
...
function BTTentacleAttackAction:dist_sqr_to_tentacle_tip(unit, tentacle_data, target_unit)
...
function BTTentacleAttackAction:target_evade_through_dodge_check(action, tentacle_data, target_unit, dist_to_tip_sqr)
	local target_status_extension = ScriptUnit.has_extension(target_unit, "status_system")
	if
	target_status_extension and target_status_extension.is_dodging and
	action.dodge_mitigation_radius_squared < dist_to_tip_sqr then
		return true
	end
...
function BTTentacleAttackAction:target_tentacle_status_check(target_unit, status)
	local target_status_extension = ScriptUnit.has_extension(target_unit, "status_system")
	if
	target_status_extension and target_status_extension.grabbed_by_tentacle_status == status then
		return true
	end
