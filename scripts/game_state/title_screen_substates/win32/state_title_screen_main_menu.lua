...
function StateTitleScreenMainMenu:update(dt, t)
	...
	if active_view then
		...
		self._title_start_ui:should_start() and not self._input_disabled then
		...
		if input_service:get("back") or Managers.time:get_demo_transition() then
			...
			if script_data.honduras_demo and
			not self._title_start_ui:in_transition() then
...
function StateTitleScreenMainMenu:cb_fade_in_done(level_key, profile_name)
