...
function CraftPageSalvageConsole:on_enter(params, settings)
...
function CraftPageSalvageConsole:_start_transition_animation(animation_name)
	local params = {
...
function CraftPageSalvageConsole:create_ui_elements(params)
...
function CraftPageSalvageConsole:on_exit(params)
...
function CraftPageSalvageConsole:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function CraftPageSalvageConsole:post_update(dt, t)

	return end

function CraftPageSalvageConsole:_update_animations(dt)
...
function CraftPageSalvageConsole:_handle_input(dt, t)
	...
	if unblocked then
		if UIUtils.is_button_pressed(widgets_by_name.auto_fill_plentiful) then auto_fill_rarity = "plentiful" end
		if UIUtils.is_button_pressed(widgets_by_name.auto_fill_common) then auto_fill_rarity = "common" end
		if UIUtils.is_button_pressed(widgets_by_name.auto_fill_rare) then auto_fill_rarity = "rare" end
		if UIUtils.is_button_pressed(widgets_by_name.auto_fill_exotic) then auto_fill_rarity = "exotic"
		end
...
function CraftPageSalvageConsole:_handle_craft_input_progress(progress)
	return self.parent:_set_input_progress(progress)
end

function CraftPageSalvageConsole:craft_result(result, error, reset_slots)
	if not error then
		self._craft_result = result
	end
end

function CraftPageSalvageConsole:reset()
...
function CraftPageSalvageConsole:present_results()
	self.super_parent:clear_disabled_backend_ids()
	self.super_parent:update_inventory_items()
end

function CraftPageSalvageConsole:on_craft_completed()
...
function CraftPageSalvageConsole:_update_craft_items()
	local super_parent = self.super_parent
...
function CraftPageSalvageConsole:_remove_craft_item(backend_id)
	local craft_items = self._craft_items
...
function CraftPageSalvageConsole:_add_craft_item(backend_id, ignore_sound, specific_amount)
...
function CraftPageSalvageConsole:_set_craft_counter_text(counter_text, visible)
	if self._presenting_rewards then
		return
	end
...
function CraftPageSalvageConsole:_set_craft_button_disabled(disabled)
...
function CraftPageSalvageConsole:_exit(selected_level)
	self.exit = true
	self.exit_level_id = selected_level
end

function CraftPageSalvageConsole:draw(dt)
...
function CraftPageSalvageConsole:_play_sound(event)
	self.super_parent:play_sound(event)
end

function CraftPageSalvageConsole:_set_craft_button_text(text, localize)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.craft_button
	widget.content.button_text = localize and Localize(text) or text
end

function CraftPageSalvageConsole:_has_added_item_by_id(backend_id)
	return self._craft_items [backend_id]
end

function CraftPageSalvageConsole:_update_reward_material_fade_out(dt)
...
function CraftPageSalvageConsole:_on_craft_material_fade_complete()
...
function CraftPageSalvageConsole:_set_reward_material_alpha_fraction(fraction)
...
function CraftPageSalvageConsole:_reset_reward_materials(visible)
...
function CraftPageSalvageConsole:_set_material_enabled_state(index, enabled)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name ["material_text_" .. index]
...
function CraftPageSalvageConsole:_set_reward_material_by_index(backend_id, amount_text)
	local material_presentation_order = UISettings.crafting_material_order_by_item_key
