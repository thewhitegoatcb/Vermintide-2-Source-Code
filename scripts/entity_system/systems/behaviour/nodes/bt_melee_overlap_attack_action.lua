...
function BTMeleeOverlapAttackAction:init(...)
...
function BTMeleeOverlapAttackAction:enter(unit, blackboard, t)
...
local function incremental_randomize(event, blackboard)
	if type(event) == "table" then
		...
		do return event [blackboard.attack_random_index] end
	else
		return event
	end
end

function BTMeleeOverlapAttackAction:_init_attack(unit, target_unit, blackboard, action, t, start_attack_index)
...
function BTMeleeOverlapAttackAction:leave(unit, blackboard, t, reason, destroy)
	local locomotion_extension = blackboard.locomotion_extension
...
function BTMeleeOverlapAttackAction:_attack_finished(unit, blackboard, t, dt)
...
function BTMeleeOverlapAttackAction:_calculate_cylinder_collision(attack, bot_threat, self_pos, self_rot)
...
function BTMeleeOverlapAttackAction:_calculate_oobb_collision(attack, bot_threat, self_pos, self_rot)
...
function BTMeleeOverlapAttackAction:_create_bot_aoe_threat(unit, attack_rotation, attack, bot_threat, bot_threat_duration)
...
function BTMeleeOverlapAttackAction:_check_wall_collision(unit, blackboard, check_range, dt)
...
function BTMeleeOverlapAttackAction:run(unit, blackboard, t, dt)
...
function BTMeleeOverlapAttackAction:push_player(unit, hit_unit, push_speed, push_speed_z, catapult_player)
...
function BTMeleeOverlapAttackAction:hit_player(unit, blackboard, hit_unit, action, attack)
...
function BTMeleeOverlapAttackAction:hit_ai(unit, hit_unit, action, attack, blackboard, t)
...
function BTMeleeOverlapAttackAction:anim_cb_frenzy_damage(unit, blackboard)
	self:anim_cb_damage(unit, blackboard)
end


function BTMeleeOverlapAttackAction:anim_cb_damage(unit, blackboard)
...
function BTMeleeOverlapAttackAction:push_close_units(unit, blackboard, t, data)
...
function BTMeleeOverlapAttackAction:weapon_sweep_overlap(unit, blackboard, action, attack, data, physics_world, t, dt)
...
local debug_drawer_info = { mode = "retained", name = "BTMeleeOverlapAttackAction" }

function BTMeleeOverlapAttackAction:overlap_checks(unit, blackboard, physics_world, t, action, attack, oobb_pos, box_rot, box_size, hit_units, overlap_update_radius)
	if blackboard.is_illusion then
		return 0
	end
	...
	local hit_actors, num_hit_actors = PhysicsWorld.immediate_overlap(physics_world,
	"position", oobb_pos,
	"rotation", box_rot,
	"size", box_size,
	"shape", "oobb",
	"types", "dynamics",
	"collision_filter", filter_name)
...
function BTMeleeOverlapAttackAction:anim_cb_attack_overlap_done(unit, blackboard)
	local overlap_data = blackboard.continous_overlap_data
	overlap_data.perform_overlap = nil
end

function BTMeleeOverlapAttackAction:anim_cb_attack_grabbed_smash(unit, blackboard)
