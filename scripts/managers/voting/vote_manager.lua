...
local VOTING_RPCS = { "rpc_server_request_start_vote_peer_id", "rpc_server_request_start_vote_lookup", "rpc_server_request_start_vote_deed", "rpc_client_start_vote_peer_id", "rpc_client_start_vote_lookup", "rpc_client_start_vote_deed", "rpc_client_add_vote", "rpc_vote", "rpc_client_complete_vote", "rpc_client_vote_kick_enabled", "rpc_update_voters_list", "rpc_client_check_dlc", "rpc_server_check_dlc_reply", "rpc_requirement_failed" }
...
function VoteManager:init(ingame_context)
...
function VoteManager:_gather_dlc_dependencies(vote_data)
...
function VoteManager:request_vote(name, vote_data, voter_peer_id, ignore_dlc_check)
...
function VoteManager:_server_abort_active_vote()
...
function VoteManager:_trigger_can_vote_fail_reply(vote_name, vote_data, message)
...
function VoteManager:can_start_vote(name, vote_data)
	local vote_template = VoteTemplates [name]
...
function VoteManager:vote(vote)
...
function VoteManager:_number_of_votes()
	local active_voting = self.active_voting
	if active_voting then
...
function VoteManager:has_voted(peer_id)
	local active_voting = self.active_voting
	return active_voting and active_voting.votes [peer_id] ~= nil
end

function VoteManager:vote_in_progress()
	local active_voting = self.active_voting
	if active_voting then
...
function VoteManager:active_vote_template()
	return self.active_voting.template
end

function VoteManager:active_vote_data()
	return self.active_voting.data
end

function VoteManager:previous_vote_info()
	return self.previous_voting_info
end

function VoteManager:is_ingame_vote()
	return self.active_voting.template.ingame_vote
end

function VoteManager:is_mission_vote()
	return self.active_voting.template.mission_vote
end

function VoteManager:allow_vote_input(enable)
	self._allow_vote_input = enable
end

function VoteManager:vote_time_left()
...
function VoteManager:_handle_popup_result(result)
	self._popup_id = nil
end

function VoteManager:update(dt)
...
function VoteManager:_time_ended(t)
...
function VoteManager:_vote_result(vote_time_ended)
	local active_voting = self.active_voting
...
function VoteManager:hot_join_sync(peer_id)
	local channel_id = PEER_ID_TO_CHANNEL [peer_id]
...
function VoteManager:_server_start_vote(name, ignore_peer_list, data)
	local vote_template = VoteTemplates [name]
	local network_time = Managers.state.network:network_time()
	self.active_voting = {
		name = name,
...
function VoteManager:_get_voter_start_list(ignore_list)
	local ignore_peers = { }
	if ignore_list then
...
local removed_peers = { }
function VoteManager:_update_voter_list_by_active_peers(active_peers, voter_list, votes)
...
function VoteManager:rpc_vote(channel_id, vote_cast)
	local active_voting = self.active_voting
	if active_voting then
		local peer_id = nil
...
function VoteManager:_server_add_vote(peer_id, vote_option)
	self.active_voting.votes [peer_id] = vote_option
end

function VoteManager:_handle_requirement_results(requirement_check_data)
	local is_done = true
	local success = true
...
function VoteManager:_server_handle_requirement_check(dt, t)
	local requirement_check_data = self._requirement_check_data
...
function VoteManager:_server_update(dt, t)
	local requirement_check_data = self._requirement_check_data
	if requirement_check_data then
...
function VoteManager:_handle_undecided_votes(active_voting)
...
function VoteManager:rpc_server_request_start_vote_base(channel_id, vote_type_id, sync_data)
...
function VoteManager:rpc_server_request_start_vote_peer_id(channel_id, vote_type_id, sync_data)
	self:rpc_server_request_start_vote_base(channel_id, vote_type_id, sync_data)
end

function VoteManager:rpc_server_request_start_vote_lookup(channel_id, vote_type_id, sync_data)
	self:rpc_server_request_start_vote_base(channel_id, vote_type_id, sync_data)
end

function VoteManager:rpc_server_request_start_vote_deed(channel_id, vote_type_id, sync_data)
	self:rpc_server_request_start_vote_base(channel_id, vote_type_id, sync_data)
end


function VoteManager:_start_vote_base(peer_id, vote_type_id, sync_data, voters)
	local vote_type_name = NetworkLookup.voting_types [vote_type_id]
	local vote_template = VoteTemplates [vote_type_name]
...
function VoteManager:rpc_client_start_vote_peer_id(channel_id, vote_type_id, sync_data, voters)
	self:_start_vote_base(channel_id, vote_type_id, sync_data, voters)
end

function VoteManager:rpc_client_start_vote_lookup(channel_id, vote_type_id, sync_data, voters)
	self:_start_vote_base(channel_id, vote_type_id, sync_data, voters)
end

function VoteManager:rpc_client_start_vote_deed(channel_id, vote_type_id, sync_data, voters)
	self:_start_vote_base(channel_id, vote_type_id, sync_data, voters)
end

function VoteManager:rpc_client_add_vote(channel_id, peer_id, vote_option)
	local active_voting = self.active_voting
	if active_voting then
...
function VoteManager:rpc_client_complete_vote(channel_id, vote_result)
	if self.active_voting then
		local number_of_votes, vote_results = self:_number_of_votes()
		self.previous_voting_info = { text = self.active_voting.text, number_of_votes = number_of_votes, vote_results = vote_results, vote_result = vote_result, votes = self.active_voting.votes }
...
function VoteManager:rpc_client_vote_kick_enabled(channel_id, is_enabled)
	self._vote_kick_enabled = is_enabled
end

function VoteManager:rpc_update_voters_list(channel_id, voters)
	local active_voting = self.active_voting
...
function VoteManager:rpc_client_check_dlc(channel_id, dlc_name_ids)
	local owns_dlc = true
...
function VoteManager:rpc_server_check_dlc_reply(channel_id, success)
	local requirement_check_data = self._requirement_check_data
	local peer_id = CHANNEL_TO_PEER_ID [channel_id]
	requirement_check_data.results [peer_id] = success
end

function VoteManager:rpc_requirement_failed(channel_id, vote_id, message)
...
function VoteManager:_client_update(dt, t)
	return end

function VoteManager:set_vote_kick_enabled(is_enabled)
	if self.is_server then
		self._vote_kick_enabled = is_enabled
...
function VoteManager:vote_kick_enabled()
...
function VoteManager:play_sound(event)

	WwiseWorld.trigger_event(self.wwise_world, event)
end

local current_voters = { }
function VoteManager:get_current_voters()
...
local peers_local = { }
function VoteManager:_active_peers()
