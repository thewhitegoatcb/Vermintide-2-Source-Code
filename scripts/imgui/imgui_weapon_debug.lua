...
function ImguiWeaponDebug:init()
	...
	self._hit_zone_names = { "head", "neck", "torso", "left_arm", "right_arm", "left_leg", "right_leg", "full" }
...
function ImguiWeaponDebug:update()
	...
	if self._current_unit and not Unit.alive(self._current_unit) or
	self._weapon_unit_left and not Unit.alive(self._weapon_unit_left) or
	self._weapon_unit_right and not Unit.alive(self._weapon_unit_right) or
	self._weapon_unit_left == nil and self._weapon_unit_right == nil or slot_changed then
...
function ImguiWeaponDebug:is_persistent()
	return self._draw_chain_action_data or self._draw_hit_box
end

function ImguiWeaponDebug:draw()
	...
	for name, weapon in pairs(self._weapon_extensions) do
		...
		if name == "any" then weapon = nil end
...
function ImguiWeaponDebug:_refresh_unit_list()
	self._unit_names = { }
	self._units = { }
	self._selected_unit = -1
...
function ImguiWeaponDebug:_initialize_unit(unit)
	...
	if unit then
		...
		if extension then
			...
			local armor_modifier_values = { 0, 0, 0, 0, 0, 0 }
			local mod_count = 0
					...
					for sub_name, sub_action in pairs(action) do
						...
						if left or right then
							...
							if left then
								...
								if not armor_mod_per_charge [charge_value] then local charge_data = { count = 0, values = { } } end
							...
							if right then
								...
								if not armor_mod_per_charge [charge_value] then local charge_data = { count = 0, values = { } }
								end
						...
						if inner or outer then
							self._combat_push_actions [sub_name] = { inner = inner, outer = outer }
						end
					end
...
function ImguiWeaponDebug:_draw_basic_info()
...
function ImguiWeaponDebug:_draw_damage_info()
	Imgui.separator()
	...
	for breed_table_name, breed_table in pairs(self._breed_table) do
		if
		difficulty_level and hit_zone_name and Imgui.tree_node(breed_table_name) then
			Imgui.separator()
...
function ImguiWeaponDebug:_update_combat_settings()
...
function ImguiWeaponDebug:_draw_faction_combat_info(breed_table, difficulty_level, hit_zone_name, power_level, stagger_level, is_critical_strike, backstab_multiplier, has_power_boost)
	...
	for breed_name, _ in pairs(breed_table) do
		...
		if Imgui.tree_node(node_name) then
			...
			Imgui.text("Type") Imgui.next_column()
			Imgui.text("Duration") Imgui.next_column()
			Imgui.text("Distance") Imgui.next_column()
			Imgui.text("Value") Imgui.next_column()
			Imgui.text("Strength")
			Imgui.separator()
...
function ImguiWeaponDebug:_get_current_weapon()
	if not self._display_current_action then
	...
	if self._current_weapon_extension then
		do return self._current_weapon_extension end
	else
		for name, extension in pairs(self._weapon_extensions) do
...
function ImguiWeaponDebug:_get_current_action()
	if not self._display_current_action then
		local selected_action_name = self._selected_action >= 0 and self._action_list [self._selected_action]
		local selected_sub_actions = selected_action_name and self._sub_action_list [selected_action_name] or { }
	...
	if self._current_weapon_extension then
		do return self._current_weapon_extension.current_action_settings end
	else
		for name, extension in pairs(self._weapon_extensions) do
			if extension and extension.current_action_settings then
...
function ImguiWeaponDebug:debug_draw_chain_data(current_time_in_action, action, owner_unit, weapon_extension)
	local gui = Debug.gui
	...
	for i = 1, #allowed_chain_actions do
		...
		local action_color = clamped_time_in_action < chain_start_time and Colors.get("orange") or
		chain_end_time <= clamped_time_in_action and Colors.get("red") or
		Colors.get("green")
...
function ImguiWeaponDebug:get_breed_health(difficulty_level, breed)
...
function ImguiWeaponDebug:get_breed_stagger(difficulty_level, breed)
...
function ImguiWeaponDebug:get_damage(sub_action, power_level, difficulty_level, hit_zone_name, breed, target_index, stagger_level, is_critical_strike, backstab_multiplier, has_power_boost)
	local unit = self._current_unit
...
function ImguiWeaponDebug:get_ai_stagger(damage_profile, power_level, difficulty_level, hit_zone_name, breed, target_index, is_critical_strike, has_power_boost)
	local unit = self._current_unit
	local item = self._combat_current_weapon
...
function ImguiWeaponDebug:_calculate_damage(unit, item, damage_profile, difficulty_settings, power_level, difficulty_level, hit_zone_name, breed, target_index, stagger_level, is_critical_strike, backstab_multiplier, has_power_boost)
	...
	local damage = DamageUtils.custom_calculate_damage(unit, damage_source, power_level, damage_profile, target_index, range_scalar_multiplier,
	is_critical_strike, backstab_multiplier, has_power_boost, boost_damage_multiplier, breed, hit_zone_name, stagger_level, difficulty_level)

	damage = DamageUtils.networkify_damage(damage)
	return damage
end

function ImguiWeaponDebug:_calculate_ai_stagger(unit, item, damage_profile, power_level, difficulty_level, hit_zone_name, breed, target_index, is_critical_strike, has_power_boost)
	local blocked = false
	local damage_source = item.name
	local range_scalar_multiplier = 0
...
function ImguiWeaponDebug:_get_damage_profile_for_action(sub_action)
	if sub_action then
		local action_hand = sub_action.weapon_action_hand
		if action_hand == "both" and self._selected_weapon_extenstion_name ~= "any" then
			action_hand = self._selected_weapon_extenstion_name
...
function ImguiWeaponDebug:_verify_crits()
	...
	local damage_profile_exclusion_list = table.mirror_array_inplace({ "tutorial_longbow_charged" })


	local breed_exclusion_list = table.mirror_array_inplace({ "chaos_raider", "chaos_raider_tutorial", "chaos_dummy_sorcerer", "chaos_dummy_exalted_sorcerer_drachenfels", "skaven_stormfiend", "skaven_stormfiend_demo", "skaven_stormfiend_boss" })




	local hit_zones = { "head", "torso" }
...
function ImguiWeaponDebug:_dump_weapon_performance()
...
function ImguiWeaponDebug:_check_missing_unused_actions()
	local ignored_unused_actions = {
		weapon_reload = { auto_reload_on_empty = true },


		action_two = { give_item = true } }
	...
	for name, template in pairs(Weapons) do
		...
		for action_name, action in pairs(template.actions) do
			...
			if not current_actions then
				...
				for sub_action_name, _ in pairs(action) do
					if ( not ignored_unused_actions [action_name] or not ignored_unused_actions [action_name] [sub_action_name] ) and
					not current_actions [sub_action_name] then
