...
local respawn_colors_by_score = { { 255, 0, 0 },
	{ 255, 128, 0 },
	{ 255, 255, 0 },
	{ 0, 255, 0 } }
...
function ImguiLevelDebug:is_persistent()
	return true
end

function ImguiLevelDebug:_load_flow_events()
	...
	if next(lookup) == nil then
		do return "No events found in the level." end
	else
...
function ImguiLevelDebug:update()
	return end

function ImguiLevelDebug:draw()
...
function ImguiLevelDebug:draw_flow_debug()
...
function ImguiLevelDebug:draw_unit_finder()
...
function ImguiLevelDebug:draw_respawn_debug(respawn_handler)
