...
function BTCrazyJumpAction:init(...)
...
function BTCrazyJumpAction:enter(unit, blackboard, t)
...
function BTCrazyJumpAction:leave(unit, blackboard, t, reason, destroy)
...
function BTCrazyJumpAction:run(unit, blackboard, t, dt)
	...
	if data.start_jump < t then
		...
		elseif data.state == "in_air" then
			...
			if not data.snap_failed and

			distance < enter_snap_state_distance then
			...
			if data.start_check_obstacles < t then
				...
				elseif Mover.collides_down(mover) and
				t - blackboard.last_jump > 0.1 then
		...
		elseif data.state == "snapping" then
			...
			if dot < 0 then
				...
				local mover = Unit.mover(unit)
				if Mover.collides_down(mover) and
				t - blackboard.last_jump > 0.1 then
...
function BTCrazyJumpAction:create_bot_threat(unit, blackboard, t)
...
function BTCrazyJumpAction:check_colliding_players(unit, blackboard, pos)
...
function BTCrazyJumpAction:setup_jump(unit, blackboard, data)
...
function BTCrazyJumpAction:update_anim_variable_done(unit, data)
...
local INDEX_ACTOR = 4
local hit_units = { }
function BTCrazyJumpAction.ray_cast(from, to, blackboard, ignore_unit)
...
function BTCrazyJumpAction:debug(unit, blackboard, data, t)
