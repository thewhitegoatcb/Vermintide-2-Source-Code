...
function CharacterStateHelper.get_movement_input(input_extension)
...
function CharacterStateHelper.get_square_movement_input(input_extension)
	local move_input = input_extension:get("move") or Vector3(0, 0, 0)
	local move_input_controller = input_extension:get("move_controller") or Vector3(0, 0, 0)
...
function CharacterStateHelper.get_look_input(input_extension, status_extension, inventory_extension, is_3p)
...
function CharacterStateHelper.apply_motion_controls(look_delta, input_extension)
...
function CharacterStateHelper.update_dodge_lock(unit, input_extension, status_extension)
...
function CharacterStateHelper.check_to_start_dodge(unit, input_extension, status_extension, t)
		...
		for input, dir in pairs(DOUBLE_TAP_DODGES) do
			if input_extension:get(input) then
				...
				if was_double_tap then
					start_dodge = true
					dodge_direction = dir:unbox() break
				end
...
function CharacterStateHelper.move_on_ground(first_person_extension, input_extension, locomotion_extension, local_move_direction, speed, unit, strafe_speed_mult)
...
function CharacterStateHelper.packmaster_move_on_ground(t, first_person_extension, input_extension, locomotion_extension, local_move_direction, speed, unit, player, pole_dir, dragged_unit, idle_anim_played, is_pushing)
	local unit_rotation = first_person_extension:current_rotation()
	local flat_unit_rotation = Quaternion.look(Vector3.flat(Quaternion.forward(unit_rotation)), Vector3.up())
	local move_direction = Quaternion.rotate(flat_unit_rotation, local_move_direction)
	...
	if is_pushing then
		...
		local hit, hit_position, dist, hit_normal, actor = PhysicsWorld.immediate_raycast(physics_world,
		dragged_unit_pos + Vector3(0, 0, 0.5),
		pole_dir,
		ray_dist,
		"closest",
		"types", "both",
...
function CharacterStateHelper.update_soft_collision_movement(first_person_extension, status_extension, locomotion_extension, unit, world, current_animation, side)
	local movement_settings_table = PlayerUnitMovementSettings.get_movement_settings_table(unit)
...
function CharacterStateHelper.do_common_state_transitions(status_extension, csm, in_state)
	...
	if is_catapulted then
		local params = { sound_event = "Play_hit_by_ratogre", direction = direction }
	...
	if status_extension.in_hanging_cage then
		...
		local params = { animations = animations, cage_unit = cage_unit }
...
function CharacterStateHelper.is_colliding_with_gameplay_collision_box(world, unit, collision_filter, params)
	local physics_world = World.get_data(world, "physics_world")
...
function CharacterStateHelper.move_in_air(first_person_extension, input_extension, locomotion_extension, speed, unit, wait_timer_force_backwards_movement, wait_timer_force_forward_movement)
...
function CharacterStateHelper.move_in_air_pactsworn(first_person_extension, input_extension, locomotion_extension, speed, unit, wait_timer_force_backwards_movement, wait_timer_force_forward_movement, dt)
	local movement = CharacterStateHelper.get_movement_input(input_extension)
	local force_y_movement = 0
...
function CharacterStateHelper.looking_up(first_person_extension, threshold)
...
function CharacterStateHelper.looking_down(first_person_extension, threshold)
	local first_person_unit = first_person_extension:get_first_person_unit()
...
function CharacterStateHelper.look(input_extension, viewport_name, first_person_extension, status_extension, inventory_extension, override_sens, override_delta)
...
function CharacterStateHelper.look_limited_rotation_freedom(input_extension, viewport_name, first_person_extension, unit, rotation, max_radians_yaw, max_radians_pitch, status_extension, inventory_extension, override_sens)
	local camera_manager = Managers.state.camera
...
function CharacterStateHelper.lerp_player_rotation_radian(player_radian, target_radian, original_diference_radian, percentage_in_lerp)
...
function CharacterStateHelper.lerp_player_pitch_rotation(player_start_pitch, first_person_extension, percentage_in_lerp, unit)
...
function CharacterStateHelper.lerp_player_yaw_rotation(player_yaw, target_yaw, original_diference_yaw, first_person_extension, percentage_in_lerp, unit)
	local first_person_unit = first_person_extension:get_first_person_unit()
...
function CharacterStateHelper.time_in_ladder_move_animation(unit, ladder_base_height)
...
function CharacterStateHelper.show_inventory_3p(unit, show_inventory_3p, include_local_player, is_server, inventory_extension)
...
function CharacterStateHelper.set_is_on_ladder(ladder_unit, unit, on_ladder, is_server, status_extension)
	local network_manager = Managers.state.network
	local unit_id = network_manager:unit_game_object_id(unit)
...
function CharacterStateHelper.set_is_on_ledge(ledge_unit, unit, on_ledge, is_server, status_extension)
	local network_manager = Managers.state.network
	local unit_id = network_manager:unit_game_object_id(unit)
	...
	if Managers.state.network:game() and
	not LEVEL_EDITOR_TEST then
...
function CharacterStateHelper.get_buffered_input(input_id, input_extension, no_buffer, doubleclick_window, softbutton_threshold, is_melee_slot)
...
function CharacterStateHelper._check_cooldown(weapon_extension, action, t)
...
function CharacterStateHelper.wield_input(input_extension, inventory_extension, action_name, verify)
	...
	if CharacterStateHelper.get_buffered_input("wield_switch", input_extension, nil, nil, nil, wielded_slot_name == "slot_melee") then
		if current_slot.name ~= "slot_melee" then slot_to_wield = "slot_melee" else slot_to_wield = "slot_ranged"
		end
	end
	if not slot_to_wield then
		for index, slot in ipairs(wieldable_slots) do
			...
			if wield_input_alt and CharacterStateHelper.get_buffered_input(wield_input_alt, input_extension, nil, nil, nil, wielded_slot_name == "slot_melee") and (
			slot ~= current_slot or inventory_extension:can_swap_from_storage(slot.name, SwapFromStorageType.LowestUnwieldPrio)) then
		...
		repeat
			...
			if not slot_data then
				...
				elseif slot_to_wield_index < 1 then
					...
					slot_to_wield_index = slot_to_wield_index + scroll_dir end end until slot_data
	...
	scroll_value, swap_from_storage_type
end

local empty_table = { }

function CharacterStateHelper.get_item_data_and_weapon_extensions(inventory_extension)
	local equipment = inventory_extension:equipment()
...
function CharacterStateHelper.get_current_action_data(left_hand_weapon_extension, right_hand_weapon_extension)
...
function CharacterStateHelper._check_chain_action(wield_input, action_data, item_template, current_action_extension, input_extension, inventory_extension, unit, t, ammo_extension)
	...
	if not input then
		local action = action_data.action local sub_action = action_data.sub_action
...
local career_chain_action = { sub_action = "default", start_time = 0, action = "N/A", input = "action_career" }
function CharacterStateHelper._get_chain_action_data(item_template, current_action_extension, current_action_settings, input_extension, inventory_extension, unit, t, ammo_extension)
					...
					if on_wield then
						...
						if chain_actions then
							for chain_action_idx = 1, #chain_actions do
								...
								if chain_action_data.input == "action_wield" and
								current_action_extension:is_chain_action_available(chain_action_data, t) then
...
local function validate_action(unit, action_name, sub_action_name, action_settings, input_extension, inventory_extension, only_check_condition, ammo_extension, current_action_extension, t)
	...
	local has_input =
	only_check_condition or input_extension:get(input_id) or
	input_extension:get_buffer(input_id) or
	input_extension:get(action_settings.attack_hold_input) or
	not allow_toggle and input_extension:get(hold_input) or
	action_settings.kind == "block" and input_extension:is_input_blocked()
	...
	if has_input or wield_input then
		...
		if ( not condition_func or condition_func(unit, input_extension, ammo_extension, current_action_extension) ) and
		not CharacterStateHelper._check_cooldown(current_action_extension, action_name, t) then
			...
			do return action_name, sub_action_name end
		else
...
function CharacterStateHelper.validate_action(unit, action_name, sub_action_name, action_settings, input_extension, inventory_extension, only_check_condition, ammo_extension, current_action_extension, t)
...
local weapon_action_interrupt_damage_types = { cutting_berserker = true, cutting = true }
...
function CharacterStateHelper.update_weapon_actions(t, unit, input_extension, inventory_extension, health_extension)
	...
	if new_action and new_sub_action then
		...
		if weapon_action_hand == "both" then
			...
			local left_action_init_data = next_action_init_data and table.merge(next_action_init_data, { action_hand = "left" }) or { action_hand = "left" }
			local right_action_init_data = next_action_init_data and table.merge(next_action_init_data, { action_hand = "right" }) or { action_hand = "right" }
...
function CharacterStateHelper.stop_weapon_actions(inventory_extension, reason)
	local equipment = inventory_extension:equipment()
...
function CharacterStateHelper.stop_career_abilities(career_extension, reason)
	career_extension:stop_ability(reason)
end

function CharacterStateHelper.check_crouch(unit, input_extension, status_extension, toggle_crouch, first_person_extension, t)
	...
	if
	gamepad_active and Managers.matchmaking and Managers.matchmaking:is_matchmaking_in_inn() then
		toggle_input = false
...
function CharacterStateHelper.can_uncrouch(unit)
	local mover = Unit.mover(unit)
	local position = Mover.position(mover)
	return Unit.mover_fits_at(unit, "standing", position)
end

function CharacterStateHelper.crouch(unit, t, first_person_extension, status_extension)
...
function CharacterStateHelper.uncrouch(unit, t, first_person_extension, status_extension)
...
function CharacterStateHelper.get_move_animation(locomotion_extension, input_extension, status_extension)
...
function CharacterStateHelper.is_colliding_down(unit)
	local mover = Unit.mover(unit)
	return Mover.collides_down(mover)
end

function CharacterStateHelper.is_colliding_sides(unit)
	local mover = Unit.mover(unit)
	return Mover.collides_sides(mover)
end

function CharacterStateHelper.has_move_input(input_extension)
	local movement = CharacterStateHelper.get_movement_input(input_extension)
	return Vector3.length(movement) > 0
end

function CharacterStateHelper.is_moving(locomotion_extension)
...
function CharacterStateHelper.is_moving_backwards(locomotion_extension, first_person_extension)
...
function CharacterStateHelper.is_knocked_down(status_extension)
	return status_extension:is_knocked_down()
end

function CharacterStateHelper.is_staggered(status_extension)
	return status_extension:is_staggered()
end

function CharacterStateHelper.is_pounced_down(status_extension)
	return status_extension:is_pounced_down()
end

function CharacterStateHelper.is_catapulted(status_extension)
	local is_catapulted, direction = status_extension:is_catapulted()
	return is_catapulted, direction
end

function CharacterStateHelper.is_grabbed_by_pack_master(status_extension)
	return status_extension:is_grabbed_by_pack_master()
end

function CharacterStateHelper.is_grabbed_by_tentacle(status_extension)
	return status_extension.grabbed_by_tentacle
end

function CharacterStateHelper.is_in_vortex(status_extension)
	return status_extension.in_vortex
end

function CharacterStateHelper.is_overcharge_exploding(status_extension)
	return status_extension:is_overcharge_exploding()
end

function CharacterStateHelper.pack_master_status(status_extension)
	return status_extension.pack_master_status
end

function CharacterStateHelper.corruptor_status(status_extension)
	return status_extension.corruptor_status
end

function CharacterStateHelper.grabbed_by_tentacle_status(status_extension)
	return status_extension.grabbed_by_tentacle_status
end

function CharacterStateHelper.grabbed_by_chaos_spawn_status(status_extension)
	return status_extension.grabbed_by_chaos_spawn_status, status_extension.grabbed_by_chaos_spawn_status_count
end

function CharacterStateHelper.is_waiting_for_assisted_respawn(status_extension)
	return status_extension:is_ready_for_assisted_respawn()
end

function CharacterStateHelper.is_assisted_respawning(status_extension)
	return status_extension:is_assisted_respawning()
end

function CharacterStateHelper.is_pushed(status_extension)
	return status_extension:is_pushed()
end

function CharacterStateHelper.is_charged(status_extension)
	return status_extension:is_charged()
end

function CharacterStateHelper.is_block_broken(status_extension)
	return status_extension:is_block_broken()
end

function CharacterStateHelper.is_dead(status_extension)
	return status_extension:is_dead()
end

function CharacterStateHelper.is_using_transport(status_extension)
	return status_extension:is_using_transport()
end

function CharacterStateHelper.is_zooming(status_extension)
	return status_extension:is_zooming()
end

function CharacterStateHelper.is_crouching(status_extension)
	return status_extension:is_crouching()
end

function CharacterStateHelper.is_starting_interaction(input_extension, interactor_extension)
...
function CharacterStateHelper.is_interacting(interactor_extension)
	return interactor_extension:is_interacting()
end

function CharacterStateHelper.is_waiting_for_interaction_approval(interactor_extension)
	return interactor_extension:is_waiting_for_interaction_approval()
end

function CharacterStateHelper.interact(input_extension, interactor_extension)
...
function CharacterStateHelper.will_be_ledge_hanging(world, unit, params)
...
local INDEX_ACTOR = 4
function CharacterStateHelper.is_raycasting_to_gameplay_collision_box(world, unit, collision_filter, params)
	local physics_world = World.get_data(world, "physics_world")
...
function CharacterStateHelper.is_ledge_hanging(world, unit, params)
	if not script_data.ledge_hanging_turned_off then
...
function CharacterStateHelper.recently_left_ladder(status_extension, t)
	return status_extension:has_recently_left_ladder(t)
end

function CharacterStateHelper.change_camera_state(player, state, params)
...
function CharacterStateHelper.change_camera_state_delayed(player, state, params, t)
	if player.bot_player then
		return
	end
...
function CharacterStateHelper.play_animation_event(unit, anim_event)
	Managers.state.network:anim_event(unit, anim_event)
end

function CharacterStateHelper.play_animation_event_first_person(first_person_extension, anim_event)
	first_person_extension:animation_event(anim_event)
end

function CharacterStateHelper.set_animation_var_first_person(first_person_extension, var_name, value)
	first_person_extension:animation_set_variable(var_name, value)
end

function CharacterStateHelper.play_animation_event_with_variable_float(unit, anim_event, variable_name, variable_value)
	Managers.state.network:anim_event_with_variable_float(unit, anim_event, variable_name, variable_value)
end


function CharacterStateHelper.set_animation_variable_float(unit, variable_name, variable_value)
	Managers.state.network:anim_set_variable_float(unit, variable_name, variable_value)
end


function CharacterStateHelper.is_enemy_character(unit)
...
function CharacterStateHelper.is_viable_stab_target(unit, target_unit, target_status_extension)
...
function CharacterStateHelper.ghost_mode(ghost_mode_extension, input_extension)
...
function CharacterStateHelper.handle_bot_ledge_hanging_failsafe(unit, is_bot)
