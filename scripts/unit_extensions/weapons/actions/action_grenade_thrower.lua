...
function ActionGrenadeThrower:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionGrenadeThrower:client_owner_start_action(new_action, t, chain_action_data, power_level)
...
function ActionGrenadeThrower:client_owner_post_update(dt, t, world, can_damage)
	...
	if self.state == "shooting" then
		...
		if not Managers.player:owner(self.owner_unit).bot_player then
			Managers.state.controller_features:add_effect("rumble", { rumble_effect = "crossbow_fire" })
		end
...
function ActionGrenadeThrower:finish(reason)
	local ammo_extension = self.ammo_extension
	local current_action = self.current_action
	local owner_unit = self.owner_unit
