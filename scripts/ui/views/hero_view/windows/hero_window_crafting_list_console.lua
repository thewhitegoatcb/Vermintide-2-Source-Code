...
function HeroWindowCraftingListConsole:on_enter(params, offset)
	...
	self._animation_settings = { entry_alignment_progress = 0 }
...
function HeroWindowCraftingListConsole:_start_transition_animation(animation_name)
	local params = {
...
function HeroWindowCraftingListConsole:create_ui_elements(params, offset)
...
function HeroWindowCraftingListConsole:on_exit(params)
...
function HeroWindowCraftingListConsole:_input_service()
...
function HeroWindowCraftingListConsole:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function HeroWindowCraftingListConsole:post_update(dt, t)
	self:_handle_input(dt, t)
end

function HeroWindowCraftingListConsole:_update_animations(dt)
...
function HeroWindowCraftingListConsole:_is_button_pressed(widget)
...
function HeroWindowCraftingListConsole:_is_stepper_button_pressed(widget)
	local content = widget.content
...
function HeroWindowCraftingListConsole:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_enter and not hotspot.is_selected
end

function HeroWindowCraftingListConsole:_is_button_hover_exit(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_exit and not hotspot.is_selected
end

function HeroWindowCraftingListConsole:_is_button_selected(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.is_selected
end

function HeroWindowCraftingListConsole:_handle_input(dt, t)
	local parent = self.parent
...
function HeroWindowCraftingListConsole:_open_recipe_page(index)
	self._params.recipe_index = index
	self.parent:set_layout_by_name("crafting_recipe")
end

function HeroWindowCraftingListConsole:_selected_button_index()
	local title_button_widgets = self._title_button_widgets
	for i, widget in ipairs(title_button_widgets) do
...
function HeroWindowCraftingListConsole:_on_button_selected(index, ignore_sound)
	local title_button_widgets = self._title_button_widgets
	for i, widget in ipairs(title_button_widgets) do
...
function HeroWindowCraftingListConsole:draw(dt)
...
function HeroWindowCraftingListConsole:_play_sound(event)
	self.parent:play_sound(event)
end

function HeroWindowCraftingListConsole:_populate_buttons(page_settings)
	local title_button_widgets = self._title_button_widgets
...
function HeroWindowCraftingListConsole:_set_alignment_progress(progress)
	local title_button_widgets = self._title_button_widgets
...
function HeroWindowCraftingListConsole:_setup_text_button_size(widget)
...
function HeroWindowCraftingListConsole:_set_text_button_horizontal_position(widget, x_position)
	local ui_scenegraph = self.ui_scenegraph
	local scenegraph_id = widget.scenegraph_id
	ui_scenegraph [scenegraph_id].local_position [1] = x_position
end

function HeroWindowCraftingListConsole:_animate_entry(widget, dt)
	local content = widget.content
	local style = widget.style
	local hotspot = content.button_hotspot
