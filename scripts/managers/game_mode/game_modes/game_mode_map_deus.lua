...
"scripts/ui/views/deus_menu/deus_shop_view" or "scripts/ui/views/deus_menu/deus_shop_view_v2")



local UI_RENDERER_MATERIALS = { "material", "materials/ui/ui_1080p_hud_atlas_textures", "material", "materials/ui/ui_1080p_hud_single_textures", "material", "materials/ui/ui_1080p_menu_atlas_textures", "material", "materials/ui/ui_1080p_menu_single_textures", "material", "materials/ui/ui_1080p_common", "material", "materials/fonts/gw_fonts", "material", "materials/ui/ui_1080p_morris_single_textures", "material", "materials/ui/ui_1080p_belakor_atlas" }
...
local states = { WAITING_FOR_PLAYERS_AFTER_SHOP = 4, MAP_DECISION = 1, SHOP = 3, FINISHING = 5, WAITING_FOR_PLAYERS_AFTER_MAP_DECISION = 2 }
...
local shared_state_spec = {
	server = {
		state = { default_value = 0, type = "number",
...
function GameModeMapDeus:init(settings, world, network_server, is_server, profile_synchronizer, level_key, statistics_db, game_mode_settings)
	...
	"deus_game_mode_map_" .. self._deus_run_controller:get_run_id(),
	shared_state_spec,
	is_server,
	network_server,
	server_peer_id,
...
function GameModeMapDeus:register_rpcs(network_event_delegate, network_transmit)
...
function GameModeMapDeus:unregister_rpcs()
...
function GameModeMapDeus:ended(reason)
...
function GameModeMapDeus:local_player_ready_to_start(player)
...
function GameModeMapDeus:local_player_game_starts(player, loading_context)
...
function GameModeMapDeus:profile_changed(peer_id, local_player_id, profile_index, career_index)
...
local EMPTY_TABLE = { }
function GameModeMapDeus:mutators()
	return EMPTY_TABLE
end

function GameModeMapDeus:update(t, dt)
	...
	if own_state ~= server_state then
		...
		if server_state == states.MAP_DECISION then
			...
			Managers.ui:handle_transition("close_active", { use_fade = true,
		...
		elseif server_state == states.WAITING_FOR_PLAYERS_AFTER_MAP_DECISION then --[[ Nothing --]]
		...
		elseif server_state == states.WAITING_FOR_PLAYERS_AFTER_SHOP then --[[ Nothing --]]

		elseif server_state == states.FINISHING then --[[ Nothing --]]
		end
...
function GameModeMapDeus:post_update(dt, t)
	local own_state = self._shared_state:get_own(self._shared_state:get_key("state"))
...
function GameModeMapDeus:destroy()
	if self._map_decision_view then
...
function GameModeMapDeus:server_update(t, dt)
	...
	elseif current_state == states.FINISHING and
	self:_are_all_peers_in_same_state() then
		self._final_node_selected = self._node_decided
	end

end

function GameModeMapDeus:_are_all_peers_in_same_state()
	local server_state = self._shared_state:get_server(self._shared_state:get_key("state"))
...
function GameModeMapDeus:player_entered_game_session(peer_id, local_player_id)
...
function GameModeMapDeus:evaluate_end_conditions(round_started)
