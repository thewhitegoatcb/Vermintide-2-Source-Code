...
local extensions = { "PlayerVolumeExtension", "BotVolumeExtension", "AIVolumeExtension", "PickupProjectileVolumeExtension", "LocalPlayerVolumeExtension" }
...
function VolumeSystem:init(context, name)
...
local dummy_table = { }
function VolumeSystem:on_add_extension(world, unit, extension_name, extension_init_data)
...
function VolumeSystem:on_remove_extension(unit, extension_name)
	self:_cleanup_extension(unit, extension_name)
end

function VolumeSystem:on_freeze_extension(unit, extension_name)
	self:_cleanup_extension(unit, extension_name)
end

function VolumeSystem:freeze(unit, extension_name, reason)
	self:_cleanup_extension(unit, extension_name)
end

function VolumeSystem:unfreeze(unit, extension_name)
	EngineOptimizedExtensions.volume_on_add_extension(self._volume_system, unit, extension_name)
	ScriptUnit.set_extension(unit, self.name, dummy_table)
end

function VolumeSystem:_cleanup_extension(unit, extension_name)
...
function VolumeSystem:update(context, t)
	EngineOptimizedExtensions.volume_update(self._volume_system, t, context.dt)
end

function VolumeSystem:register_volume(volume_name, volume_type, params)
	...
	for _, extension_name in ipairs(extensions) do
		...
		if settings then
			...
			if GenericVolumeTemplates.functions and GenericVolumeTemplates.functions [volume.volume_type] and
			GenericVolumeTemplates.functions [volume.volume_type] [volume.params.sub_type] then
			...
			EngineOptimizedExtensions.volume_register_volume(
			self._volume_system,
			level,
			volume_name,
			extension_name,
			params.invert_volume,
			volume,
			on_enter,
			on_exit,
			filter)
...
function VolumeSystem:unregister_volume(volume_name)
	local level = LevelHelper:current_level(self.world)
	fassert(Level.has_volume(level, volume_name), "No volume named %q exists in current level", volume_name)
...
function VolumeSystem:ai_ready()
...
function VolumeSystem:create_nav_tag_volume_from_data(pos, size, layer_name)
...
function VolumeSystem:get_volume_mapping_from_lookup_id(lookup_id)
	local nav_tag_volume_handler = self.nav_tag_volume_handler
	return self.nav_tag_volume_handler:get_mapping_from_lookup_id(lookup_id)
end

function VolumeSystem:destroy_nav_tag_volume(volume_name)
	local nav_tag_volume_handler = self.nav_tag_volume_handler
	nav_tag_volume_handler:destroy_nav_tag_volume(volume_name)
end

function VolumeSystem:create_nav_tag_volume(volume_name, layer_name, layer_costs)
	local level_settings = LevelHelper:current_level_settings()
	if level_settings.no_bots_allowed then
		return
	end
...
function VolumeSystem:volume_has_units_inside(volume_name)
	return EngineOptimizedExtensions.volume_has_any_units_inside(self._volume_system, volume_name)
end

function VolumeSystem:any_alive_human_players_inside(volume_name)
	...
	for _, player_unit in ipairs(PLAYER_UNITS) do
		...
		if
		status_ext and not status_ext:is_disabled() and
		EngineOptimizedExtensions.volume_has_all_units_inside(self._volume_system, volume_name, player_unit) then
			return true
		end
	end
...
function VolumeSystem:all_alive_human_players_inside(volume_name)
	local side = Managers.state.side:get_side_from_name("heroes")
	local PLAYER_UNITS = side.PLAYER_UNITS
	...
	for _, player_unit in ipairs(PLAYER_UNITS) do
		local status_ext = Unit.alive(player_unit) and ScriptUnit.has_extension(player_unit, "status_system")
		if
		status_ext and not status_ext:is_disabled() then
...
function VolumeSystem:all_alive_or_respawned_human_players_inside(volume_name)
	local side = Managers.state.side:get_side_from_name("heroes")
	local PLAYER_UNITS = side.PLAYER_UNITS
	local to_test_count = 0
	local to_test = { }
	for _, player_unit in ipairs(PLAYER_UNITS) do
		local status_ext = Unit.alive(player_unit) and ScriptUnit.has_extension(player_unit, "status_system")
		if
		status_ext and (not status_ext:is_disabled() or status_ext:is_disabled() and not status_ext:is_ready_for_assisted_respawn()) then
			to_test_count = to_test_count + 1
			to_test [to_test_count] = player_unit
		end
	end
...
function VolumeSystem:all_human_players_inside_disabled(volume_name)
...
function VolumeSystem:player_inside(volume_name, unit)
