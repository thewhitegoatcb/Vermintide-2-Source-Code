...
function PlayerCharacterStateLeavingLadderTop:init(character_state_init_context)
...
function PlayerCharacterStateLeavingLadderTop:on_enter_animation_event(speed)
...
function PlayerCharacterStateLeavingLadderTop:on_enter(unit, input, dt, context, t, previous_state, params)
	local unit = self.unit
...
function PlayerCharacterStateLeavingLadderTop:on_exit(unit, input, dt, context, t, next_state)
...
function PlayerCharacterStateLeavingLadderTop:update(unit, input, dt, context, t)
	local csm = self.csm
	local unit = self.unit
	local input_extension = self.input_extension
	...
	CharacterStateHelper.look_limited_rotation_freedom(
	input_extension,
	self.player.viewport_name,
	self.first_person_extension,
	unit,
	ladder_rotation,
	max_radians,
	nil,
	status_extension,
	self.inventory_extension)
end