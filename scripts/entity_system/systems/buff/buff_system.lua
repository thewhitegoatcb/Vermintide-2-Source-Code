...
local RPCS = { "rpc_add_buff", "rpc_add_volume_buff_multiplier", "rpc_remove_volume_buff", "rpc_add_group_buff", "rpc_remove_group_buff", "rpc_buff_on_attack", "rpc_remove_server_controlled_buff", "rpc_proc_event", "rpc_remove_gromril_armour", "rpc_add_buff_synced", "rpc_add_buff_synced_params", "rpc_add_buff_synced_relay", "rpc_add_buff_synced_relay_params", "rpc_add_buff_synced_response", "rpc_remove_buff_synced" }
...
local extensions = { "BuffExtension" }



function BuffSystem:init(entity_system_creation_context, system_name)
...
function BuffSystem:on_add_extension(world, unit, extension_name, extension_init_data)
...
function BuffSystem:hot_join_sync(peer_id)
	if self.is_server then
...
function BuffSystem:_clean_up_server_controller_buffs(unit)
...
function BuffSystem:on_remove_extension(unit, extension_name)
...
function BuffSystem:on_freeze_extension(unit, extension_name)

	self:freeze(unit, extension_name)
end

function BuffSystem:freeze(unit, extension_name, reason)
	local frozen_extensions = self.frozen_unit_extension_data
	if frozen_extensions [unit] then return end
	local extension = self.unit_extension_data [unit]
...
function BuffSystem:unfreeze(unit)
...
local dummy_input = { }
function BuffSystem:update(context, t)
...
function BuffSystem:get_player_group_buffs()
...
function BuffSystem:_next_free_server_buff_id()
...
local params = { }
function BuffSystem:_add_buff_helper_function(unit, template_name, attacker_unit, server_buff_id, power_level, source_attacker_unit)
	...
	if server_buff_id > 0 then
		...
		if not self.server_controlled_buffs [unit] [server_buff_id] then
			...
			self.server_controlled_buffs [unit] [server_buff_id] = { local_buff_id = local_buff_id, template_name = template_name, attacker_unit = attacker_unit, source_attacker_unit = source_attacker_unit }
		end
	else
...
function BuffSystem:add_buff(unit, template_name, attacker_unit, is_server_controlled, power_level, source_attacker_unit)
...
function BuffSystem:remove_server_controlled_buff(unit, server_buff_id)
...
function BuffSystem:has_server_controlled_buff(unit, server_buff_id)
...
function BuffSystem:add_volume_buff_multiplier(unit, buff_template_name, multiplier)
...
function BuffSystem:add_volume_buff(unit, buff_template_name, multiplier)
...
function BuffSystem:remove_volume_buff_multiplier(unit, buff_template_name)
...
function BuffSystem:remove_volume_buff(unit, buff_template_name)
	if not Unit.alive(unit) then
		return
	end
...
function BuffSystem:rpc_remove_server_controlled_buff(channel_id, unit_id, server_buff_id)
	local unit = self.unit_storage:unit(unit_id)
...
function BuffSystem:rpc_add_volume_buff_multiplier(channel_id, unit_id, buff_template_name_id, multiplier)
	local unit = self.unit_storage:unit(unit_id)
	local buff_template_name = NetworkLookup.buff_templates [buff_template_name_id]
...
function BuffSystem:rpc_remove_volume_buff(channel_id, unit_id, buff_template_name_id)
	local unit = self.unit_storage:unit(unit_id)
	local buff_template_name = NetworkLookup.buff_templates [buff_template_name_id]
...
function BuffSystem:rpc_add_group_buff(channel_id, group_buff_template_id, num_instances)
	if self.is_server then
	...
	for i = 1, num_instances do
		local group_buff_data = { group_buff_template_name = group_buff_template_name, recipients = { } }
...
function BuffSystem:rpc_remove_group_buff(channel_id, group_buff_template_id, num_instances)
	local group_buff_template_name = NetworkLookup.group_buff_templates [group_buff_template_id]
...
function BuffSystem:rpc_buff_on_attack(channel_id, attacking_unit_id, hit_unit_id, attack_type_id, is_critical, hit_zone_id, target_number, buff_type_id)
...
function BuffSystem:rpc_proc_event(channel_id, peer_id, local_player_id, event_id)
...
function BuffSystem:rpc_remove_gromril_armour(channel_id, unit_id)
	local unit = self.unit_storage:unit(unit_id)
	if not Unit.alive(unit) then
		return
	end
...
function BuffSystem:set_buff_ext_active(unit, is_active)
	if is_active then
...
local function buff_param_pack_float(input) return math.floor(input * 100 + 32768) end
local function buff_param_unpack_float(input) return ( input - 32768 ) / 100 end
local function buff_param_pack_t(input) return math.floor(input * 10) end
local function buff_param_unpack_t(input) return input / 10 end
local function buff_param_raw(input) return input end
local function buff_param_damage_source(input) return NetworkLookup.damage_sources [input] end
local function buff_param_pack_unit(input, ctx) return ctx.network_manager:unit_game_object_id(input) or NetworkConstants.invalid_game_object_id end
local function buff_param_unpack_unit(input, ctx) return ctx.unit_storage:unit(input) end
...
local packed_param_flags = { "refresh_duration_only" }
...
function BuffSystem:_pack_buff_params(buff_params, dest_param_ids, dest_param_vals)
...
function BuffSystem:_unpack_buff_params(dest_table, param_ids, param_vals)
...
function BuffSystem:_prepare_sync(target_unit, template_name, sync_type, params)
	local network_manager = Managers.state.network
...
function BuffSystem:add_buff_synced(target_unit, template_name, sync_type, params, optional_peer_id)
...
function BuffSystem:remove_buff_synced(target_unit, buff_id)
	local buff_extension = self.unit_extension_data [target_unit]
...
function BuffSystem:rpc_add_buff_synced(channel_id, target_unit_id, template_name_id, remote_sync_id, sync_type_id)
...
function BuffSystem:rpc_add_buff_synced_relay(channel_id, target_unit_id, template_name_id, server_sync_id, sync_type_id)
	local target_unit = self.unit_storage:unit(target_unit_id)
	local buff_extension = self.unit_extension_data [target_unit]
	if buff_extension then
		local template_name = NetworkLookup.buff_templates [template_name_id]
...
function BuffSystem:rpc_add_buff_synced_params(channel_id, target_unit_id, template_name_id, remote_sync_id, sync_type_id, param_ids, param_vals)
	local target_unit = self.unit_storage:unit(target_unit_id)
	local buff_extension = self.unit_extension_data [target_unit]
	if buff_extension then
		local template_name = NetworkLookup.buff_templates [template_name_id]
...
function BuffSystem:rpc_add_buff_synced_relay_params(channel_id, target_unit_id, template_name_id, server_sync_id, sync_type_id, param_ids, param_vals)
	local target_unit = self.unit_storage:unit(target_unit_id)
	local buff_extension = self.unit_extension_data [target_unit]
	if buff_extension then
		local template_name = NetworkLookup.buff_templates [template_name_id]
...
function BuffSystem:rpc_add_buff_synced_response(channel_id, target_unit_id, local_sync_id, server_sync_id)
	local target_unit = self.unit_storage:unit(target_unit_id)
	local buff_extension = self.unit_extension_data [target_unit]
	if

	buff_extension and not buff_extension:apply_sync_id(local_sync_id, server_sync_id) then
		local network_manager = Managers.state.network
...
function BuffSystem:rpc_remove_buff_synced(channel_id, target_unit_id, server_sync_id)
...
function BuffSystem:_hot_join_sync_synced_buffs(peer_id)
