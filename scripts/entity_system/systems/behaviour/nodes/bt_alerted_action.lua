...
function BTAlertedAction:init(...)
	BTAlertedAction.super.init(self, ...)
end

BTAlertedAction.name = "BTAlertedAction"

function BTAlertedAction:enter(unit, blackboard, t)
...
function BTAlertedAction:init_alerted(unit, blackboard, t)
	...
	if has_line_of_sight and not blackboard.action.no_hesitation and
	target_dist < 12 then
...
function BTAlertedAction:decide_deadline(unit, blackboard, t)
...
function BTAlertedAction:should_hesitate(unit, blackboard, action)
...
function BTAlertedAction:leave(unit, blackboard, t, reason, destroy)
	local network_manager = Managers.state.network
...
function BTAlertedAction:check_if_should_start_moving(unit, blackboard)
	local action = blackboard.action
	local target_unit = blackboard.target_unit
	local target_pos = POSITION_LOOKUP [target_unit]
...
function BTAlertedAction:run(unit, blackboard, t, dt)
	local action = blackboard.action
	local target_unit = blackboard.target_unit
