...
function ActionBulletSpray:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionBulletSpray:client_owner_start_action(new_action, t, chain_action_data, power_level)
...
function ActionBulletSpray:client_owner_post_update(dt, t, world, can_damage)
	local current_action = self.current_action

	if self.use_ammo_time and
	not self.used_ammo and self.use_ammo_time <= t then
		self.used_ammo = true
...
function ActionBulletSpray:finish(reason)
	self:_clear_targets()
...
function ActionBulletSpray:_clear_targets()
...
function ActionBulletSpray:_select_targets(world, show_outline)
	...
	SPRAY_RADIUS,
	100,
	"collision_filter", "filter_character_trigger", "report_initial_overlap")
	...
	if result then
		...
		for i = 1, num_hits do
			...
			if not hit_units [hit_unit] then
				...
				elseif
				breed and self:_is_infront_player(player_position, player_direction, hit_position) and self:_check_within_cone(player_position, player_direction, hit_unit) then
					targets [#targets + 1] = hit_unit
					hit_units [hit_unit] = true
...
function ActionBulletSpray:_check_within_cone(player_position, player_direction, target, player)
...
function ActionBulletSpray:_is_infront_player(player_position, player_direction, hit_position)
...
function ActionBulletSpray:raycast_to_target(world, from_position, direction, target)
	local physics_world = World.get_data(world, "physics_world")
