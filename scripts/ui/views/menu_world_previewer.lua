...
local camera_position_by_character = {
	witch_hunter = { z = 0.4, x = 0, y = 0.8 },




	bright_wizard = { z = 0.2, x = 0, y = 0.4 },




	dwarf_ranger = { z = 0, x = 0, y = 0 },




	wood_elf = { z = 0.16, x = 0, y = 0.45 },




	empire_soldier = { z = 0.4, x = 0, y = 1 },




	empire_soldier_tutorial = { z = 0.4, x = 0, y = 1 },




	default = { z = 0.4, x = 0, y = 1 } }
...
function MenuWorldPreviewer:init(ingame_ui_context, optional_camera_character_positions, unique_id, delayed_spawn)
	...
	self._camera_default_position = { z = 0.9, x = 0, y = 2.8 }
...
function MenuWorldPreviewer:set_default_position(default_position)
	self._camera_default_position = default_position
end

function MenuWorldPreviewer:set_lookat_target(lookat_target)
	self._lookat_target = lookat_target
end

function MenuWorldPreviewer:destroy()
...
function MenuWorldPreviewer:on_enter(viewport_widget, hero_name)
	MenuWorldPreviewer.super.on_enter(self)

	self:setup_viewport(viewport_widget, hero_name)
end

function MenuWorldPreviewer:setup_viewport(viewport_widget, hero_name)
	...
	self.character_look_current = { 0, 3, 1 }
	self.character_look_target = { 0, 3, 1 }
...
local EMPTY_TABLE = { }
function MenuWorldPreviewer:activate(activate, viewport_widget, hero_name)
...
function MenuWorldPreviewer:trigger_level_event(event_name)
	Level.trigger_event(self.level, event_name)
end

function MenuWorldPreviewer:show_level_units(unit_indices, visibility)
...
function MenuWorldPreviewer:has_units_spawned()
	return self.character_unit ~= nil
end

function MenuWorldPreviewer:on_exit()
	MenuWorldPreviewer.super.on_exit(self)
	Renderer.set_automatic_streaming(true)
end

function MenuWorldPreviewer:update(dt, t, input_disabled)
	self._requested_unit_spawn_queue = self._delayed_unit_spawn_queue or EMPTY_TABLE
...
function MenuWorldPreviewer:post_update(dt)
	self:_handle_unit_spawn_request()
	MenuWorldPreviewer.super.post_update(self, dt)
end

function MenuWorldPreviewer:force_stream_highest_mip_levels()
	self._use_highest_mip_levels = true
end

function MenuWorldPreviewer:force_hide_character()
	self._force_hide_character = true
end

function MenuWorldPreviewer:force_unhide_character()
	self._force_hide_character = false
end

function MenuWorldPreviewer:_update_units_visibility(dt)
...
function MenuWorldPreviewer:_set_character_visibility(visible, camera_move_duration, disable_camera_position_update)
...
function MenuWorldPreviewer:_update_camera_animation_data(animation_data, dt)
...
function MenuWorldPreviewer:set_camera_axis_offset(axis, value, animation_time, func_ptr, fixed_position)
...
function MenuWorldPreviewer:set_camera_gamepad_offset(value)
	self._camera_gamepad_offset_data = value
end

function MenuWorldPreviewer:set_camera_rotation_axis_offset(axis, value, animation_time, func_ptr)
...
function MenuWorldPreviewer:set_character_axis_offset(axis, value, animation_time, func_ptr)
...
function MenuWorldPreviewer:handle_mouse_input(input_service, dt)
	local character_unit = self.character_unit
...
function MenuWorldPreviewer:handle_controller_input(input_service, dt)
	local character_unit = self.character_unit
	if character_unit == nil then return
	end
...
function MenuWorldPreviewer:start_character_rotation(direction)
	if direction then
		self.rotation_direction = direction
	end
end

function MenuWorldPreviewer:end_character_rotation()
	print("end_character_rotation", self.rotation_direction)
end

function MenuWorldPreviewer:request_spawn_hero_unit(profile_name, career_index, state_character, callback, optional_scale, camera_move_duration, optional_skin, reset_camera)
	self._requested_hero_spawn_data = { frame_delay = 1,
...
function MenuWorldPreviewer:request_spawn_unit(unit_name, unit_type, cb)
	local queue = self._requested_unit_spawn_queue
	queue [#queue + 1] = { frame_delay = 1,
...
function MenuWorldPreviewer:_handle_hero_spawn_request()
...
function MenuWorldPreviewer:_handle_unit_spawn_request()
...
function MenuWorldPreviewer:_load_hero_unit(profile_name, career_index, state_character, callback, optional_scale, camera_move_duration, optional_skin)
	self.camera_xy_angle_target = DEFAULT_ANGLE
	...
	local data = { num_loaded_packages = 0,
...
function MenuWorldPreviewer:_spawn_hero_unit(skin_data, optional_scale, career_index)
...
function MenuWorldPreviewer:respawn_hero_unit(profile_name, career_index, state_character, callback, camera_move_duration)
	local reset_camera = true
...
function MenuWorldPreviewer:_spawn_item(item_name, spawn_data)
	local hero_material_changed = MenuWorldPreviewer.super._spawn_item(self, item_name, spawn_data)
	if
	hero_material_changed and (self._use_highest_mip_levels or UISettings.wait_for_mip_streaming_character) then
		self:_request_mip_streaming_for_unit(self.character_unit)
	end

end

function MenuWorldPreviewer:_spawn_item_unit(unit, item_slot_type, item_template, unit_attachment_node_linking, scene_graph_links, material_settings)
...
function MenuWorldPreviewer:_spawn_unit(unit_name, item_data)
...
function MenuWorldPreviewer:set_unit_location(unit, location)
	if location and unit and Unit.alive(unit) then
		Unit.set_local_position(unit, 0, Vector3Aux.unbox(location))
	end
end

function MenuWorldPreviewer:_destroy_item_units_by_slot(slot_type)
...
function MenuWorldPreviewer:_reference_name()
...
function MenuWorldPreviewer:clear_units(reset_camera)
...
function MenuWorldPreviewer:hide_character()
	self._draw_character = false
end

function MenuWorldPreviewer:trigger_unit_flow_event(unit, event_name)
	if unit and Unit.alive(unit) then
		Unit.flow_event(unit, event_name)
	end
end

function MenuWorldPreviewer:trigger_level_flow_event(event_name)
