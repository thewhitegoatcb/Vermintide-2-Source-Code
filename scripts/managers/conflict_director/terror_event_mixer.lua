...
local function add_spawned_counting(event, optional_data, spawn_counter_category)
	...
	if not optional_data.despawned_func then
		...
		function optional_data.despawned_func(unit, breed, optional_data)
...
TerrorEventMixer.init_functions = {
	...
	spawn_around_origin_unit = function (event, element, t)
		...
		ConflictUtils.find_positions_around_position(
		center_position,
		spawn_positions,
		nav_world,
		min_distance,
	...
	start_event = function (event, element, t)
		...
		start_events [#start_events + 1] = { name = element.start_event_name, data = { },
			id = id }
	...
	play_stinger = function (event, element, t)
		...
		if optional_pos then
			...
			optional_pos and WwiseUtils.trigger_position_event(world, stinger_name, optional_pos) or
			WwiseWorld.trigger_event(wwise_world, stinger_name) local wwise_source_id = nil
	...
	create_boss_door_group = function (event, element, t)
		...
		local group_data = { template = "boss_door_closers",
	...
	do_volume_challenge = function (event, element, t, dt)
		...
		optional_data [volume_name] = { time_inside = 0,
	...
	activate_mutator = function (event, element, t, dt)
		...
		if Managers.state.game_mode then
			...
			if not mutator_handler:has_activated_mutator(name) then
				mutator_handler:initialize_mutators({ name })
	...
	event_horde = function (event, element, t, dt)
		...
		if horde_data then
			if horde_data.started then
				...
				do return "waiting to start..." end
			end
		else
...
function TerrorEventMixer.reset()
...
function TerrorEventMixer.add_to_start_event_list(event_name, seed, origin_unit, origin_position)
	local start_events = TerrorEventMixer.start_event_list
	local id = TerrorEventMixer.incrementing_id
	TerrorEventMixer.incrementing_id = TerrorEventMixer.incrementing_id + 1
	start_events [#start_events + 1] = { name = event_name, data = { seed = seed, origin_unit = origin_unit,
...
function TerrorEventMixer.start_random_event(event_chunk_name)
...
local function is_element_available(element, data)
	...
	if element.minimum_difficulty_tweak and
	current_difficulty_tweak < element.minimum_difficulty_tweak then
	...
	elseif element.only_on_difficulty and
	current_difficulty ~= element.only_on_difficulty then
	...
	elseif element [1] == "one_of" then
		for _, possible_element in ipairs(element [2]) do
			if is_element_available(possible_element, data) then
				process_terror_event_recursive_element(
				possible_element,
				data,
				processed_elements,
				base_event_name,
...
function TerrorEventMixer.start_event(event_name, data, id)
	...
	if #elements > 0 then
		local new_event = { index = 1, ends_at = 0,
			name = event_name,
			elements = elements,
			data = data,
...
function TerrorEventMixer.stop_event(event_name)
...
function TerrorEventMixer.find_event(event_name)
	local active_events = TerrorEventMixer.active_events
	local num_events = #active_events
	for i = 1, num_events do
		local event = active_events [i]
...
function TerrorEventMixer.is_event_id_active_or_pending(id)
	local active_events = TerrorEventMixer.active_events
	local num_events = #active_events
	for i = 1, num_events do
		local event = active_events [i]
...
function TerrorEventMixer.update(t, dt, gui)
	local active_events = TerrorEventMixer.active_events
	local num_events = #active_events
...
function TerrorEventMixer.run_event(event, t, dt)
...
function TerrorEventMixer.debug(gui, active_events, t, dt)
...
function TerrorEventMixer.debug_event(gui, event, t, dt, x1, y1, panning_x, render_master)
	local elements = event.elements
	local index = event.index
	local element = elements [index]
	local func_name = element [1]
	local borderx = 20 + panning_x local bordery = 280
	x1 = x1 + borderx + 20
	y1 = y1 + bordery + 40
	local y2 = y1
	local layer = 200
