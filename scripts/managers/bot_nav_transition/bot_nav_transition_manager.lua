...
BotNavTransitionManager.TRANSITION_LAYERS = { end_zone = 1, bot_poison_wind = 20, fire_grenade = 30, barrel_explosion = 50, bot_damage_drops = 10, bot_jumps = 1, temporary_wall = 1, bot_leap_of_faith = 3, bot_ratling_gun_fire = 30, doors = 0.1, planks = 0.1, bot_ladders = 5, bot_drops = 1 }
...
BotNavTransitionManager.NAV_COST_MAP_LAYERS = { plague_wave = 30, mutator_heavens_zone = 50, lamp_oil_fire = 30, warpfire_thrower_warpfire = 30, vortex_near = 50, stormfiend_warpfire = 50, vortex_danger_zone = 75, troll_bile = 30 }
...
function BotNavTransitionManager:init(world, physics_world, nav_world, is_server, network_event_delegate, using_editor)
	self._world = world
...
function BotNavTransitionManager:update(dt, t)
...
function BotNavTransitionManager:clear_transitions()
...
function BotNavTransitionManager:_find_matching_layer(from, to, player_jumped)
	local diff = to - from
	...
	elseif height < -min_fall_damage_height then
		do return "bot_damage_drops" end
...
function BotNavTransitionManager:rpc_create_bot_nav_transition(channel_id, from, via, to, player_jumped)
	self:create_transition(from, via, to, player_jumped)
end

function BotNavTransitionManager:create_transition(from, via, wanted_to, player_jumped, make_permanent, drawer)
	...
	local above = 0.3 local beneath = 0.3
	...
	transitions [index] = { graph = graph, from = Vector3Box(from), waypoint = Vector3Box(waypoint), to = Vector3Box(to), unit = unit, type = layer_name, permanent = make_permanent or false }
	self._bot_nav_transition_lookup [unit] = index


	local next_index = index
	repeat
		next_index = ( next_index - self._index_offset ) % self._max_amount + 1 + self._index_offset until
	not transitions [next_index] or not transitions [next_index].permanent
...
function BotNavTransitionManager:unregister_transition(unit)
...
function BotNavTransitionManager:transition_data(unit)
	local ladder_transition = self._ladder_transitions [unit]
	if ladder_transition then
		do return "ladder", ladder_transition.from:unbox(), ladder_transition.to:unbox() end
	else
		local index = self._bot_nav_transition_lookup [unit]
...
function BotNavTransitionManager:register_ladder(unit, index_offset, drawer)
	local data = { }
	...
	local hit, hit_position = PhysicsWorld.immediate_raycast(
	ph_world,
	ray_from,
	down,
	ray_length,
	"closest",
	"collision_filter", "filter_bot_nav_transition_ladder_ray")
	...
	if not data.failed then
		...
		local graph = GwNavGraph.create(
		nav_world,
		ladder_is_bidirectional, {
			to, align_pos, hit_position + back * 0.2, from },
		Colors.get("blue"),
		layer_id,
...
function BotNavTransitionManager:get_ladder_coordinates(unit)
	local data = self._ladder_transitions [unit]

	return data.from:unbox(), data.to:unbox(), data.failed
end

function BotNavTransitionManager:debug_refresh_ladders()
...
function BotNavTransitionManager:clear_ladder_transitions()
...
function BotNavTransitionManager:allow_layer(layer_name, layer_allowed)
	local layer_id = LAYER_ID_MAPPING [layer_name]
...
function BotNavTransitionManager:set_layer_cost(layer_name, layer_cost)
	local layer_id = LAYER_ID_MAPPING [layer_name]
