...
if not IS_WINDOWS then
	function menu_functions.console_friends_menu(this)
...
function StartGameView:init(ingame_ui_context)
...
function StartGameView:_init_menu_views()
...
function StartGameView:_activate_view(new_view)
...
function StartGameView:active_view()
	return self._active_view
end

function StartGameView:exit_current_view()
...
function StartGameView:initial_profile_view()
	return self.ingame_ui.initial_profile_view
end

function StartGameView:_setup_state_machine(state_machine_params, optional_start_state, optional_start_sub_state, optional_params)
...
function StartGameView:wanted_state()
	return self._wanted_state
end

function StartGameView:clear_wanted_state()
	self._wanted_state = nil
end

function StartGameView:input_service()
	return self._draw_loading and FAKE_INPUT_SERVICE or self.input_manager:get_service("start_game_view")
end

function StartGameView:set_input_blocked(blocked)
	self._input_blocked = blocked
end

function StartGameView:input_blocked()
	return self._input_blocked
end

function StartGameView:play_sound(event)
	WwiseWorld.trigger_event(self.wwise_world, event)
end

function StartGameView:play_mechanism_sound(event_setting_name, default_event)
...
function StartGameView:create_ui_elements()
...
function StartGameView:draw(dt, input_service)
...
function StartGameView:post_update(dt, t)
	if self._machine then
		self._machine:post_update(dt, t)
	end
end

function StartGameView:update(dt, t)
...
function StartGameView:on_enter(params)
...
function StartGameView:_handle_new_ui_disclaimer()
	local mechanism_name = Managers.mechanism:current_mechanism_name()
	local global_disclaimer_states = {
		deus = { play = false, default = false },



		adventure = { default = true, leaderboard = false },



		default = { default = true, leaderboard = false } }
...
function StartGameView:on_enter_sub_state()
	return self._on_enter_sub_state
end

function StartGameView:set_current_hero(profile_index)
...
function StartGameView:_get_sorted_players()
...
function StartGameView:_handle_mouse_input(dt, t, input_service)
	return end

function StartGameView:_handle_input(dt, t)
...
function StartGameView:_is_selection_widget_pressed(widget)
...
function StartGameView:hotkey_allowed(input, mapping_data)
...
function StartGameView:_get_screen_settings_by_state_name(state_name)
...
function StartGameView:requested_screen_change_by_name(screen_name, sub_screen_name)
...
function StartGameView:_change_screen_by_name(screen_name, sub_screen_name, optional_params)
...
function StartGameView:_change_screen_by_index(index)
...
function StartGameView:post_update_on_enter()
...
function StartGameView:post_update_on_exit()
	return end

function StartGameView:on_exit()
...
function StartGameView:exit(return_to_game, ignore_sound)
...
function StartGameView:transitioning()
...
function StartGameView:suspend()
	self.input_manager:device_unblock_all_services("keyboard", 1)
	self.input_manager:device_unblock_all_services("mouse", 1)
	self.input_manager:device_unblock_all_services("gamepad", 1)
	self.suspended = true
...
function StartGameView:unsuspend()
...
function StartGameView:close_menu(return_to_main_screen, ignore_sound)
	local return_to_game = not return_to_main_screen
	self:exit(return_to_game, ignore_sound)
end

function StartGameView:destroy()
...
function StartGameView:_is_button_pressed(widget)
...
function StartGameView:_has_active_level_vote()
...
function StartGameView:_set_loading_overlay_enabled(enabled, message)
...
function StartGameView:number_of_players()
	local player_manager = Managers.player
	return player_manager:num_human_players()
end

function StartGameView:start_game(params)
...
function StartGameView:cancel_matchmaking()
