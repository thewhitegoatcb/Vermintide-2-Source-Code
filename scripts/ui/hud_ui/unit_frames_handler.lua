...
local allowed_consumable_slots = { slot_healthkit = true, slot_grenade = true, slot_potion = true }





local allowed_weapon_slots = { slot_ranged = true, slot_melee = true }
...
function UnitFramesHandler:init(parent, ingame_ui_context)
	self._parent = parent
...
function UnitFramesHandler:add_respawn_counter_event(player, is_local_player, spawn_timer)
...
function UnitFramesHandler:on_spectator_target_changed(spectated_player_unit)
...
function UnitFramesHandler:unit_frame_amount()
	return #self._unit_frames
end

function UnitFramesHandler:get_unit_widget(index)
...
function UnitFramesHandler:_create_player_unit_frame()
	...
	unit_frame =
	unit_frame or self:_create_unit_frame_by_type("player")
...
function UnitFramesHandler:_create_party_members_unit_frames()
	local unit_frames = self._unit_frames
...
function UnitFramesHandler:_create_enemy_party_members_unit_frames()
	local unit_frames = self._unit_frames
...
function UnitFramesHandler:_create_unit_frame_by_type(frame_type, frame_index)
...
function UnitFramesHandler:_get_unused_unit_frame()
	local unit_frames = self._unit_frames
	for i = 1, #unit_frames do
		local unit_frame = unit_frames [i]
...
function UnitFramesHandler:_get_unit_frame_by_connecting_peer_id(peer_id)
	local unit_frames = self._unit_frames
	for i = 1, #unit_frames do
		local unit_frame = unit_frames [i]
...
function UnitFramesHandler:_reset_unit_frame(unit_frame)
	local widget = unit_frame.widget
	widget:reset()
...
function UnitFramesHandler:_handle_unit_frame_assigning()
	...
	if
	mechanism_name == "adventure" and self:_handle_connecting_peers(temp_active_peer_ids, unit_frames_used_by_players) then
		frames_changed = true
	end
...
function UnitFramesHandler:_handle_connecting_peers(active_peer_ids, num_unit_frames_used)
				...
				if not active_peer_ids [peer_id] then
					...
					if not unit_frame then
						...
						if avaiable_unit_frame then
							...
							avaiable_unit_frame.player_data = { connecting_peer_id = peer_id }
...
function UnitFramesHandler:_cleanup_unused_unit_frames(active_ui_ids, connecting_peer_ids)
...
function UnitFramesHandler:_align_party_member_frames()
	...
	for i = 2, #unit_frames do
		...
		if ( peer_id or connecting_peer_id ) and is_visible then
			local position_x, position_y = nil
...
function UnitFramesHandler:_set_player_extensions(player_data, player_unit)
	local extensions = {
...
function UnitFramesHandler:_sync_player_stats(unit_frame)
	...
	if ( not player_unit or not Unit.alive(player_unit) ) and
	player_data.extensions then
	...
	local equipment, career_index, inventory_extension = nil
	...
	local update_portrait_status = false local update_health_bar_status = false
		...
		for i = 1, #inventory_slots do
			...
			if update_weapons and allowed_weapon_slots [slot_name] and item_data then
				...
				if item_template.ammo_data and slot_data then
					...
					if
					slot_name == "slot_ranged" and stored_slot_data.overcharge_fraction then
...
function UnitFramesHandler:set_visible(visible)
...
function UnitFramesHandler:on_gamepad_activated()
...
function UnitFramesHandler:on_gamepad_deactivated()
	local my_unit_frame = self._unit_frames [1]
...
function UnitFramesHandler:update(dt, t)
...
function UnitFramesHandler:resolution_modified()
	if not self._is_visible then
...
function UnitFramesHandler:_draw(dt)
	if not self._is_visible then
		return
	end
...
function UnitFramesHandler:_update_numeric_ui()
