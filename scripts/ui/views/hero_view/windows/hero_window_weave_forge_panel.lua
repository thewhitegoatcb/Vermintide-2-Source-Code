...
function HeroWindowWeaveForgePanel:on_enter(params, offset)
...
function HeroWindowWeaveForgePanel:_start_transition_animation(key, animation_name)
	local params = {
		wwise_world = self._wwise_world,
...
function HeroWindowWeaveForgePanel:_setup_definitions()
...
function HeroWindowWeaveForgePanel:create_ui_elements(params, offset)
...
function HeroWindowWeaveForgePanel:_setup_essence_tooltip()
	...
	local content_passes = { "weave_progression_slot_titles" }
	local content_data = {
...
function HeroWindowWeaveForgePanel:_set_essence_tooltip_amounts(total_essence, maximum_essence)
	local widgets_by_name = self._widgets_by_name
...
function HeroWindowWeaveForgePanel:on_exit(params)
...
function HeroWindowWeaveForgePanel:_set_loadout_power(power)
	local widgets_by_name = self._widgets_by_name
	local widget_text = widgets_by_name.loadout_power_text
	widget_text.content.text = power
end

function HeroWindowWeaveForgePanel:_set_essence_amount(essence_amount)
	local widgets_by_name = self._widgets_by_name
...
function HeroWindowWeaveForgePanel:_sync_backend_loadout()
...
function HeroWindowWeaveForgePanel:_is_button_pressed(widget)
	local content = widget.content
...
function HeroWindowWeaveForgePanel:_handle_input(dt, t)
	return end

function HeroWindowWeaveForgePanel:_play_sound(event)
	self._parent:play_sound(event)
end

function HeroWindowWeaveForgePanel:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function HeroWindowWeaveForgePanel:_sync_component_visibilty_by_layout(layout_name)
...
function HeroWindowWeaveForgePanel:post_update(dt, t)
	return end

function HeroWindowWeaveForgePanel:_update_animations(dt)
...
function HeroWindowWeaveForgePanel:_draw(dt)
	local parent = self._parent
...
function HeroWindowWeaveForgePanel:_set_background_bloom_intensity(fraction)
	local min = 1.39
...
function HeroWindowWeaveForgePanel:_set_background_wheel_visibility(visible)
	local widgets_by_name = self._widgets_by_name
...
function HeroWindowWeaveForgePanel:_update_background_animations(dt)
	local widgets_by_name = self._widgets_by_name
	for i = 1, 2 do
		local wheel_ring_1 = widgets_by_name ["wheel_ring_" .. i .. "_1"]
		local wheel_ring_2 = widgets_by_name ["wheel_ring_" .. i .. "_2"]
