...
function AdventureSpawning:init(profile_synchronizer, side, is_server, network_server, saved_game_mode_data)
...
function AdventureSpawning:_setup_game_mode_data(side, saved_game_mode_data)
...
function AdventureSpawning:get_saved_game_mode_data()
	return self._saved_game_mode_data
end

function AdventureSpawning:register_rpcs(network_event_delegate, network_transmit)
	self._respawn_handler:register_rpcs(network_event_delegate, network_transmit)
end

function AdventureSpawning:unregister_rpcs()
	self._respawn_handler:unregister_rpcs()
end

function AdventureSpawning:_assign_data_to_slot(slot, data)
...
function AdventureSpawning:_unassign_data_from_slot(slot, data)
	local health_state = data.health_state
...
function AdventureSpawning:player_entered_game_session(peer_id, local_player_id, wanted_party_index)
...
function AdventureSpawning:player_joined_party(peer_id, local_player_id, party_id, slot_id)
...
function AdventureSpawning:player_left_party(peer_id, local_player_id, party_id, slot_id)
	local party = self._side.party
	local side_party_id = party.party_id
	if side_party_id ~= party_id then
		return
...
function AdventureSpawning:update(t, dt)
	if Managers.state.network:game() then
		self._respawn_handler:update(dt, t)
	end
end

function AdventureSpawning:server_update(t, dt)
...
function AdventureSpawning:_update_player_status(t, dt, occupied_slots)
	...
	for i = 1, #occupied_slots do
		...
		if peer_id and local_player_id then
			...
			if player then
				...
				elseif
				( spawn_state == "despawned" or spawn_state == "not_spawned" ) and player.player_unit then
					data.spawn_state = "spawned"
				end
			end
		end
...
function AdventureSpawning:_update_spawning(dt, t, occupied_slots, party_id)
...
function AdventureSpawning:add_delayed_client(peer_id, local_player_id)
...
function AdventureSpawning:remove_delayed_client(peer_id, local_player_id)
...
function AdventureSpawning:_update_joining_clients(dt, t)
...
function AdventureSpawning:_add_client_to_party(peer_id, local_player_id)
...
function AdventureSpawning:_spawn_player(status)
	local data = status.game_mode_data
...
function AdventureSpawning:_spawn_bot(status)
	local data = status.game_mode_data
...
function AdventureSpawning:_find_spawn_point(status)
...
function AdventureSpawning:force_update_spawn_positions(safe_position, safe_rotation)
...
function AdventureSpawning:set_respawning_enabled(enabled)
	fassert(self._respawns_enabled ~= enabled, "Respawns already enabled=%s", tostring(enabled))
	self._respawns_enabled = enabled
end

function AdventureSpawning:set_spawning_disabled(disabled)
	self._spawning = not disabled
end

function AdventureSpawning:add_spawn_point(unit)
...
function AdventureSpawning:get_spawn_point()
...
function AdventureSpawning:respawn_unit_spawned(unit)
	self._respawn_handler:respawn_unit_spawned(unit)
end

function AdventureSpawning:respawn_gate_unit_spawned(unit)
	self._respawn_handler:respawn_gate_unit_spawned(unit)
end

function AdventureSpawning:remove_respawn_units_due_to_crossroads(removed_path_distances, total_main_path_length)
	self._respawn_handler:remove_respawn_units_due_to_crossroads(removed_path_distances, total_main_path_length)
end

function AdventureSpawning:recalc_respawner_dist_due_to_crossroads()
	self._respawn_handler:recalc_respawner_dist_due_to_crossroads()
end

function AdventureSpawning:teleport_despawned_players(position)
	local party = self._side.party
	local occupied_slots = party.occupied_slots
	local player_manager = Managers.player
	for i = 1, #occupied_slots do
...
function AdventureSpawning:force_respawn(peer_id, local_player_id)
	local status = Managers.party:get_player_status(peer_id, local_player_id)
	local data = status.game_mode_data
	data.spawn_state = "force_respawn"
end

function AdventureSpawning:force_respawn_dead_players()
	local party = self._side.party
	self._respawn_handler:force_respawn_dead_players(party)
end

function AdventureSpawning:set_override_respawn_group(respawn_group_name, active)
	self._respawn_handler:set_override_respawn_group(respawn_group_name, active)
end

function AdventureSpawning:set_respawn_group_enabled(respawn_group_name, enabled)
	self._respawn_handler:set_respawn_group_enabled(respawn_group_name, enabled)
end

function AdventureSpawning:set_respawn_gate_enabled(respawn_gate_unit, enabled)
	self._respawn_handler:set_respawn_gate_enabled(respawn_gate_unit, enabled)
end

function AdventureSpawning:get_active_respawn_units()
	return self._respawn_handler:get_active_respawn_units()
end

function AdventureSpawning:set_move_dead_players_to_next_respawn(value)
	self._respawn_handler:set_move_dead_players_to_next_respawn(value)
end

function AdventureSpawning:get_respawn_handler()
...
function AdventureSpawning:add_spawn_point_to_spawn_group(unit)
...
function AdventureSpawning:get_spawn_point_from_spawn_group(group_id)
