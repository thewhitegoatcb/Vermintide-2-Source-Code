...
function GameModeDeus:init(settings, world, network_server, is_server, profile_synchronizer, level_key, statistics_db, game_mode_settings)
...
function GameModeDeus:on_round_end()
...
function GameModeDeus:destroy()
	return end

function GameModeDeus:cleanup_game_mode_units()
	local update_safe = false
	self:_clear_bots(update_safe)
end

function GameModeDeus:register_rpcs(network_event_delegate, network_transmit)
...
function GameModeDeus:unregister_rpcs()
	self._deus_spawning:unregister_rpcs()
	GameModeDeus.super.unregister_rpcs(self)
end

function GameModeDeus:event_local_player_spawned(is_initial_spawn)
	self._local_player_spawned = true
	self._is_initial_spawn = is_initial_spawn
end

function GameModeDeus:update(t, dt)
	self._deus_spawning:update(t, dt)
end

function GameModeDeus:server_update(t, dt)
...
function GameModeDeus:evaluate_end_conditions(round_started, dt, t, mutator_handler)
...
function GameModeDeus:gm_event_end_conditions_met(reason, checkpoint_available, percentages_completed)
...
function GameModeDeus:player_entered_game_session(peer_id, local_player_id)
...
function GameModeDeus:player_left_game_session(peer_id, local_player_id)
...
function GameModeDeus:remove_bot(peer_id, local_player_id, update_safe)
...
function GameModeDeus:get_end_screen_config(game_won, game_lost, player)
	if Managers.mechanism:is_final_round() or game_lost then
		...
		do return game_won and "deus_victory" or "defeat", {
			journey_name = journey_name,
			profile_index = profile_index,
			previous_completed_difficulty_index = previous_completed_difficulty_index } end
	else
		...
		if granted_power_ups then
			for i = 1, #granted_power_ups do
				...
				local reward = { type = "deus_power_up_end_of_level", sounds = { "hud_morris_weapon_chest_unlock", "morris_reliquarys_get_boon" }, power_up = granted_power_up }
				rewards [#rewards + 1] = reward
			end
		end
		return "none", { }, { rewards = rewards }
	end
end


function GameModeDeus:ended(reason)
...
function GameModeDeus:local_player_ready_to_start(player)
...
function GameModeDeus:local_player_game_starts(player, loading_context)
	local deus_run_controller = self._deus_run_controller
	...
	if self._is_server and has_volume and theme == DEUS_THEME_TYPES.BELAKOR then
		...
		local volume_system = Managers.state.entity:system("volume_system")
		volume_system:register_volume(INTRO_VO_VOLUME_ID, "trigger_volume", { sub_type = "players_inside",
...
function GameModeDeus:disable_player_spawning()
	self._deus_spawning:set_spawning_disabled(true)
end

function GameModeDeus:enable_player_spawning(safe_position, safe_rotation)
...
function GameModeDeus:teleport_despawned_players(position)
	self._deus_spawning:teleport_despawned_players(position)
end

function GameModeDeus:flow_callback_add_spawn_point(unit)
	self._deus_spawning:add_spawn_point(unit)
end

function GameModeDeus:set_override_respawn_group(respawn_group_name, active)
	self._deus_spawning:set_override_respawn_group(respawn_group_name, active)
end

function GameModeDeus:set_respawn_group_enabled(respawn_group_name, active)
	self._deus_spawning:set_respawn_group_enabled(respawn_group_name, active)
end

function GameModeDeus:set_respawn_gate_enabled(respawn_gate_unit, enabled)
	self._deus_spawning:set_respawn_gate_enabled(respawn_gate_unit, enabled)
end

function GameModeDeus:respawn_unit_spawned(unit)
	self._deus_spawning:respawn_unit_spawned(unit)
end

function GameModeDeus:get_respawn_handler()
	return self._deus_spawning:get_respawn_handler()
end

function GameModeDeus:respawn_gate_unit_spawned(unit)
	self._deus_spawning:respawn_gate_unit_spawned(unit)
end

function GameModeDeus:set_respawning_enabled(enabled)
	self._deus_spawning:set_respawning_enabled(enabled)
end

function GameModeDeus:remove_respawn_units_due_to_crossroads(removed_path_distances, total_main_path_length)
	self._deus_spawning:remove_respawn_units_due_to_crossroads(removed_path_distances, total_main_path_length)
end

function GameModeDeus:recalc_respawner_dist_due_to_crossroads()
	self._deus_spawning:recalc_respawner_dist_due_to_crossroads()
end

function GameModeDeus:profile_changed(peer_id, local_player_id, profile_index, career_index)
	self._deus_spawning:profile_changed(peer_id, local_player_id, profile_index, career_index)
end

function GameModeDeus:force_respawn(peer_id, local_player_id)
...
function GameModeDeus:force_respawn_dead_players()
	self._deus_spawning:force_respawn_dead_players()
end

function GameModeDeus:_get_first_available_bot_profile()
...
function GameModeDeus:_setup_bot_spawn_priority_lookup()
...
function GameModeDeus:_handle_bots(t, dt)
...
function GameModeDeus:_add_bot(bot_players)
	local party_id = 1
...
function GameModeDeus:_remove_bot(bot_players, index, update_safe)
...
function GameModeDeus:_remove_bot_by_profile(bot_players, profile_index, update_safe)
	local bot_index = nil
...
function GameModeDeus:_clear_bots(update_safe)
	local bot_players = self._bot_players
	local num_bot_players = #bot_players
...
function GameModeDeus:get_active_respawn_units()
	return self._deus_spawning:get_active_respawn_units()
end

function GameModeDeus:get_player_wounds(profile)
...
function GameModeDeus:mutators()
	return self._mutators
end

function GameModeDeus:on_picked_up_soft_currency(interactable_unit, interactor_unit)
	local deus_run_controller = self._deus_run_controller
...
function GameModeDeus:get_boss_loot_pickup()
	return "deus_soft_currency"
end

function GameModeDeus:_get_coins_amount_and_type(interactable_unit)
...
function GameModeDeus:players_left_safe_zone()
...
function GameModeDeus:_update_morris_music_intensity()
