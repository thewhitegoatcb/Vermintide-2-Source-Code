...
function BTSpawningAction:init(...)
...
function BTSpawningAction:enter(unit, blackboard, t)
	...
	if
	( is_horde or wield_inventory_on_spawn ) and ScriptUnit.has_extension(unit, "ai_inventory_system") then
...
function BTSpawningAction:leave(unit, blackboard, t, reason, destroy)
...
function BTSpawningAction:run(unit, blackboard, t, dt)
	local breed = blackboard.breed
	...
	elseif blackboard.spawn_landing_state == "landing" and (
	blackboard.jump_climb_finished or blackboard.fallback_landing_t < t) then
		return "done"
	end
...
local variation_type_to_rpc = { int = "rpc_anim_set_variable_int", float = "rpc_anim_set_variable_float" }




function BTSpawningAction:_apply_anim_varations(unit)
	local action = self._tree_node.action_data
	if action then
