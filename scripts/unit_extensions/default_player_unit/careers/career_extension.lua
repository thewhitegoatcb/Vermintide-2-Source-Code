...
function CareerExtension:init(extension_init_context, unit, extension_init_data)
	self._unit = unit
	...
	for ability_id = 1, num_abilities do
		...
		self._abilities [ability_id] = { is_ready = false, cooldown_paused = false, cooldown_anim_started = false,
...
function CareerExtension:ability_id(ability_name)
...
function CareerExtension:ability_was_triggered(ability_id)
	local ability = self._abilities [ability_id].activated_ability
	return ability:was_triggered()
end

function CareerExtension:ability_by_id(ability_id)
	local ability = self._abilities [ability_id].activated_ability
	return ability
end

function CareerExtension:ability_name_by_id(ability_id)
	local ability = self._abilities [ability_id]
	return ability.name
end

function CareerExtension:extensions_ready(world, unit)
...
function CareerExtension:game_object_initialized(unit, go_id)
	local passive_abilities = self._passive_abilities
	for i = 1, self._num_passive_abilities do
		local ability = passive_abilities [i]
...
function CareerExtension:force_trigger_active_ability()
	local player = self.player
	local abilities = self._abilities
...
function CareerExtension:update(unit, input, dt, context, t)
	local abilities = self._abilities
...
function CareerExtension:stop_ability(reason, ability_id)
...
function CareerExtension:_update_game_object_field(unit)
...
function CareerExtension:destroy()
	local passive_abilities = self._passive_abilities
	for i = 1, self._num_passive_abilities do
		local ability = passive_abilities [i]
		ability:destroy()
...
function CareerExtension:get_activated_ability_data(ability_id)
	ability_id = ability_id or 1
...
function CareerExtension:start_activated_ability_cooldown(ability_id, refund_percent, modified_cost, ignore_ability_readiness)
	ability_id = ability_id or 1
	local ability = self._abilities [ability_id]
...
function CareerExtension:reduce_activated_ability_cooldown_percent(amount, ability_id, ignore_paused)
	ability_id = ability_id or 1
	local ability = self._abilities [ability_id]
	self:reduce_activated_ability_cooldown(ability.max_cooldown * amount, ability_id, ignore_paused)
end

function CareerExtension:reduce_activated_ability_cooldown(amount, ability_id, ignore_paused)
	ability_id = ability_id or 1
	local ability = self._abilities [ability_id]
...
function CareerExtension:modify_max_cooldown(ability_id, bonus, multiplier)
	ability_id = ability_id or 1
	bonus = bonus or 0
...
function CareerExtension:uses_cooldown(ability_id)
	ability_id = ability_id or 1
	local ability = self._abilities [ability_id]
...
function CareerExtension:set_activated_ability_cooldown_paused(ability_id)
	ability_id = ability_id or 1
	local ability = self._abilities [ability_id]
	ability.cooldown_paused = true
end
function CareerExtension:set_activated_ability_cooldown_unpaused(ability_id)
	ability_id = ability_id or 1
	local ability = self._abilities [ability_id]
	ability.cooldown_paused = false
end

function CareerExtension:abilities_always_usable()
	return self._abilities_always_usable
end

function CareerExtension:set_abilities_always_usable(value, reason)
	if value then
...
function CareerExtension:has_abilities_always_usable_reason(reason)
	return self._abilities_always_usable_reasons [reason] ~= nil
end

function CareerExtension:modify_extra_ability_uses(num)
...
function CareerExtension:get_extra_ability_uses()
	return self._extra_ability_uses, self._extra_ability_uses_max
end

function CareerExtension:get_extra_ability_charge()
	return self._extra_ability_use_charge, self._extra_ability_use_required_charge
end

function CareerExtension:modify_extra_ability_charge(amount)
...
function CareerExtension:update_extra_ability_charge(required_charge)
	self._extra_ability_use_required_charge = required_charge
end

function CareerExtension:setup_extra_ability_uses(current_charge, max_charge, current_extra_uses, max_extra_uses)
...
function CareerExtension:update_extra_ability_uses_max(max_extra_uses)
...
function CareerExtension:reset_cooldown(ability_id)
	ability_id = ability_id or 1
	local ability = self._abilities [ability_id]
	ability.cooldown = ability.max_cooldown
end

function CareerExtension:can_use_activated_ability(ability_id)
	local network_manager = Managers.state.network
	local game = network_manager:game()
	if not game then
		return false
...
function CareerExtension:current_ability_cooldown(ability_id)
	ability_id = ability_id or 1
	local ability = self._abilities [ability_id]
	return ability.cooldown, ability.max_cooldown > 0 and ability.max_cooldown or 1
end

function CareerExtension:current_ability_cooldown_percentage(ability_id)
	local network_manager = Managers.state.network
...
function CareerExtension:get_max_ability_cooldown(ability_id)
	ability_id = ability_id or 1
	local ability = self._abilities [ability_id]
	return ability.max_cooldown
end

function CareerExtension:current_ability_paused(ability_id)
	ability_id = ability_id or 1
	local ability = self._abilities [ability_id]
	return ability.cooldown_paused
end

function CareerExtension:profile_index()
	return self._profile_index
end

function CareerExtension:career_index()
	return self._career_index
end

function CareerExtension:career_name()
	return self._career_name
end

function CareerExtension:career_settings()
	return self._career_data
end

function CareerExtension:career_skill_weapon_name(ability_id, weapon_index)
	ability_id = ability_id or 1
	local ability = self._abilities [ability_id]
...
function CareerExtension:get_base_critical_strike_chance()
	return self._career_data.attributes.base_critical_strike_chance or 0
end

function CareerExtension:has_melee_boost()
...
function CareerExtension:has_ranged_boost()
	local buff_extension = self._buff_extension
...
function CareerExtension:get_career_power_level()
	local player = self.player
...
function CareerExtension:set_state(state)
	self._state = state
end

function CareerExtension:get_state()
	return self._state or "default"
end

function CareerExtension:get_breed()
	return self._breed
end

function CareerExtension:ability_amount()
	return self._num_abilities
end

function CareerExtension:_run_ability_ready_feedback(ability_id, t)
	local ability = self._abilities [ability_id]
...
function CareerExtension:check_cooldown_anim(ability_id)
	local ability = self._abilities [ability_id]

	if not ability.cooldown_anim_started and ability.cooldown_anim_time and
	ability.cooldown - ability.cooldown_anim_time < 0 then
...
function CareerExtension:should_reload_career_weapon()
	return self._career_data.should_reload_career_weapon
end


function CareerExtension:set_career_game_object_id(go_id)
	local passive_abilities = self._passive_abilities
	for i = 1, self._num_passive_abilities do
		local ability = passive_abilities [i]
...
function CareerExtension:get_passive_ability(ability_id)
	local passive_abilities = self._passive_abilities
	return passive_abilities and passive_abilities [ability_id or 1]
end


function CareerExtension:get_passive_ability_by_name(ability_name)
