...
function UnitFrameUI:init(ingame_ui_context, definitions, data, frame_index, player_data, frame_type)
	...
	self.render_settings = { alpha_multiplier = 1, snap_pixel_positions = true }
...
function UnitFrameUI:_create_ui_elements(frame_index)
...
function UnitFrameUI:_widget_by_name(name)
	return self._widgets [name]
end

function UnitFrameUI:_widget_by_feature(feature_name, list_name)
	local widget_name = self.widget_name_by_feature [list_name] [feature_name]
	return self:_widget_by_name(widget_name)
end

function UnitFrameUI:set_position(x, y)
...
function UnitFrameUI:destroy()
	self:set_visible(false)
end

function UnitFrameUI:is_visible()
	return self._is_visible
end

function UnitFrameUI:set_visible(visible)
...
function UnitFrameUI:set_alpha(alpha)
...
function UnitFrameUI:set_default_alpha(alpha)
...
function UnitFrameUI:set_portrait_alpha(alpha)
...
function UnitFrameUI:set_equipment_alpha(alpha)
...
function UnitFrameUI:set_health_alpha(alpha)
...
function UnitFrameUI:set_ability_alpha(alpha)
...
function UnitFrameUI:set_respawn_alpha(alpha)
...
function UnitFrameUI:update(dt, t)
	...
	update_weapons and self:_update_overcharge_animation(dt, t) then
		dirty = true
	end
	...
	update_equipment and self:_update_slot_equip_animations(dt, t) then
		dirty = true
	end
...
function UnitFrameUI:on_resolution_modified()
...
local customizer_data_player_portrait = { root_scenegraph_id = "portrait_pivot", label = "You", registry_key = "player_portrait", drag_scenegraph_id = "portrait_pivot_dragger" }





local customizer_data_player_status = { root_scenegraph_id = "player_status", is_child = true, registry_key = "player_status" }




local customizer_data_teammate_status = { root_scenegraph_id = "pivot", label = "Team", registry_key = "teammate_portrait", drag_scenegraph_id = "pivot_dragger" }
...
function UnitFrameUI:draw(dt)
	...
	elseif self._frame_type == "team" and
	HudCustomizer.run(self.ui_renderer, self.ui_scenegraph, customizer_data_teammate_status) then
		UIUtils.mark_dirty(self._portrait_widgets)
		UIUtils.mark_dirty(self._default_widgets)
		UIUtils.mark_dirty(self._health_widgets)
		UIUtils.mark_dirty(self._ability_widgets)
...
function UnitFrameUI:set_dirty()
	self._dirty = true
end

function UnitFrameUI:_set_widget_dirty(widget)
	widget.element.dirty = true
end

function UnitFrameUI:reset()
	...
	local show_health_bar = true local is_knocked_down = false local is_wounded = false
...
function UnitFrameUI:set_portrait_frame(frame_settings_name, level_text)
...
function UnitFrameUI:set_portrait(portrait_texture)
...
function UnitFrameUI:set_host_status(is_host)
	local widget = self:_widget_by_feature("default", "static")
	local widget_content = widget.content
...
function UnitFrameUI:set_talking(is_talking)
...
function UnitFrameUI:set_status_icon(icon_texture, alpha)
...
function UnitFrameUI:set_connecting_status(is_connecting)
	local widget = self:_widget_by_feature("default", "dynamic")
	local widget_content = widget.content
...
function UnitFrameUI:set_icon_visibility(show_icon)
	local widget = self:_widget_by_feature("status_icon", "dynamic")
	local widget_content = widget.content
...
function UnitFrameUI:set_portrait_status(is_knocked_down, needs_help, is_dead, assisted_respawn)
	local widget = self:_widget_by_feature("default", "static")
...
function UnitFrameUI:set_player_name(name_text)
...
local item_count_slot_names = { "item_count_1", "item_count_2", "item_count_3" }




function UnitFrameUI:set_inventory_slot_data(slot_name, slot_visible, item_data, item_count)
...
function UnitFrameUI:set_equipped_weapon_info(slot_name, wielded, item_name, hud_icon)
...
local ammo_prefix = " "
function UnitFrameUI:set_ammo_for_slot(slot_name, ammo_count, remaining_ammo, using_single_clip)
	local widget = self:_widget_by_feature("weapons", "dynamic")
	local widget_content = widget.content
...
function UnitFrameUI:set_ammo_percentage(ammo_percent)
...
function UnitFrameUI:set_ability_percentage(ability_percent)
...
function UnitFrameUI:set_overcharge_percentage(has_overcharge, overcharge_percent)
	local widget = self:_widget_by_feature("weapons", "dynamic")
	local widget_content = widget.content
...
function UnitFrameUI:set_active_percentage(active_percentage)
...
function UnitFrameUI:set_health_percentage(health_percentage, health_multiplier)
	local widget = self:_widget_by_feature("health", "dynamic")
	local widget_content = widget.content
...
function UnitFrameUI:set_total_health_percentage(total_health_percentage, health_multiplier)
	local widget = self:_widget_by_feature("health", "dynamic")
	local widget_content = widget.content
...
function UnitFrameUI:set_health_bar_status(show_health_bar, is_knocked_down, is_wounded)
	local widget = self:_widget_by_feature("health", "dynamic")
	local widget_style = widget.style
	local widget_content = widget.content
...
function UnitFrameUI:set_health_bar_divider_amount(health_bar_divider_count)
	local widget = self:_widget_by_feature("health", "dynamic")
	local widget_style = widget.style
...
function UnitFrameUI:_update_portrait_opacity(is_dead, is_knocked_down, needs_help, assisted_respawn)
	local alpha = nil
	local widget = self:_widget_by_feature("default", "static")
...
function UnitFrameUI:_reset_voice_animation()
	local widget = self:_widget_by_feature("default", "dynamic")
...
function UnitFrameUI:_update_voice_animation(dt, t, is_talking)
	local widget = self:_widget_by_feature("default", "dynamic")
	local style = widget.style
...
function UnitFrameUI:_update_health_bar_animation(dt, t)
	local widget = self:_widget_by_feature("health", "dynamic")
	local widget_content = widget.content
...
function UnitFrameUI:_update_total_health_bar_animation(dt, t)
	local widget = self:_widget_by_feature("health", "dynamic")
	local widget_content = widget.content
...
function UnitFrameUI:show_respawn_ui()
	return self._show_respawn_ui
end

function UnitFrameUI:show_respawn_countdown(player, is_local_player, spawn_timer)
...
function UnitFrameUI:update_respawn_countdown(dt, t)
...
function UnitFrameUI:_update_overcharge_animation(dt, t)
	local widget = self:_widget_by_feature("weapons", "dynamic")
	local widget_content = widget.content
	local widget_style = widget.style
...
function UnitFrameUI:_on_num_grimoires_changed(name, widget, health_debuff_percent)
...
function UnitFrameUI:_on_overcharge_changed(name, widget, overcharge_percent)
	if not self.bar_animations then
		self.bar_animations = { }
	end
...
function UnitFrameUI:_on_player_ammo_changed(name, widget, ammo_percent)
	local bar_animation = self.bar_animations [name] or { }
	self.bar_animations [name] = bar_animation
...
function UnitFrameUI:_on_player_ability_changed(name, widget, ability_percent)
	local bar_animation = self.bar_animations [name] or { }
	self.bar_animations [name] = bar_animation
...
function UnitFrameUI:_on_player_health_changed(name, widget, health_percent)
	local bar_animation = self.bar_animations [name] or { }
	self.bar_animations [name] = bar_animation
...
function UnitFrameUI:_on_player_total_health_changed(name, widget, total_health_percent)
	local bar_animation = self.bar_animations [name] or { }
	self.bar_animations [name] = bar_animation
...
function UnitFrameUI:_update_bar_animations(dt)
	local dirty = false
...
function UnitFrameUI:_update_bar_flash(widget, style, time, dt)
...
function UnitFrameUI:_update_damage_highlight(widget, time, dt)
...
function UnitFrameUI:_update_player_bar_animation(content, style, time, total_time, anim_start_value, anim_end_value, dt)
	time = time + dt
	if total_time > 0 then
		local progress = math.min(time / total_time, 1)
		local weight = 7
...
function UnitFrameUI:_add_slot_equip_animation(name, widget, style)
...
function UnitFrameUI:_animate_slot_equip(animation_data, dt)
	local style = animation_data.style
	local total_time = animation_data.total_time
	local time = animation_data.time
...
function UnitFrameUI:_update_slot_equip_animations(dt)
	local animations = self.slot_equip_animations
	local dirty = false
...
function UnitFrameUI:_update_connection_animation(dt)
	if not self._is_visible then
		return false
	end
...
function UnitFrameUI:update_numeric_ui_health(player_data)
	local widget = self:_widget_by_feature("health", "dynamic")
	...
	if not widget.content.numeric_health then return end
...
function UnitFrameUI:update_numeric_ui_ammo(player_data)
	...
	if not inventory_extension then return
	end
	if self._frame_type == "team" then
		...
		if slot_name == "slot_ranged" then
			...
			if not slot_data then widget.content.has_ranged_weapon = false return end
			local item_template = slot_data.item_template
			if not item_template then return end
			local ammo_data = item_template.ammo_data


			if ammo_data and ammo_data.hide_ammo_ui then return
			end
...
function UnitFrameUI:update_numeric_ui_career_ability(game, go_id, player_data)
	local widget = self:_widget_by_name("default_dynamic")
	local player = player_data.player
	local player_unit = player and player.player_unit
	if not ALIVE [player_unit] then
	...
	if not widget.content.ability_cooldown then return end
	local extensions = player_data.extensions
	local career_extension = extensions and extensions.career
	if not career_extension then return
	end
	...
	if not on_cooldown then return
	end
