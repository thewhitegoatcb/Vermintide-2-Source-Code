...
function ActionShieldSlam:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionShieldSlam:client_owner_start_action(new_action, t, chain_action_data, power_level, action_init_data)
	...
	if
	ammo_extension and ammo_extension:is_reloading() then
	...
	if results then
		...
		for i = 1, num_results do
			...
			if not melee_friendly_fire and side_manager:is_ally(owner_unit, hit_unit) then --[[ Nothing --]]
...
function ActionShieldSlam:client_owner_post_update(dt, t, world, can_damage)
	local current_action = self.current_action
	local owner_unit = self.owner_unit

	if self.state == "waiting_to_hit" and
	self.time_to_hit <= t then
		self.state = "hitting"
	end
...
function ActionShieldSlam:_hit(world, can_damage, owner_unit, current_action)
	...
	for i = 1, actors_n do repeat
			...
			if breed then
				...
				if self:_is_infront_player(self_pos, unit_forward, hit_position) then
					...
					if distance_to_inner_position_sq <= inner_radius_sq then
						...
						hit_position,
						attack_direction,
						self.damage_profile_aoe_id,
						"power_level", power_level,
						"hit_target_index", target_index,
						"blocking", shield_blocked,
						"shield_break_procced", false,
						"boost_curve_multiplier", self.melee_boost_curve_multiplier,
			...
			elseif not hit_self and ScriptUnit.has_extension(hit_unit, "health_system") then
				...
				if is_level_unit then
					...
					"is_critical_strike", self._is_critical_strike,
					"can_damage", true,
					"can_stagger", true) end end until true
	end
...
function ActionShieldSlam:finish(reason)
...
function ActionShieldSlam:_is_infront_player(player_position, player_direction, hit_position, push_dot)
...
function ActionShieldSlam:destroy()
