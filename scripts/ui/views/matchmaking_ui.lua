...
function MatchmakingUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function MatchmakingUI:create_ui_elements()
	...
	self._input_to_widget_mapping [#self._input_to_widget_mapping + 1] = { input_action = "cancel_matchmaking",
		widgets = {
...
function MatchmakingUI:_get_widget(name)
	if self._active_mechanism == "deus" then
		do return self._widgets_deus_by_name [name] end
	else
...
function MatchmakingUI:_get_detail_widget(name)
	if self._active_mechanism == "deus" then
		do return self._detail_widgets_deus_by_name [name] end
	else
...
function MatchmakingUI:_get_widgets()
	if self._active_mechanism == "deus" then
		do return self._widgets_deus, self._detail_widgets_deus end
	else
...
function MatchmakingUI:is_in_inn()
	return self._is_in_inn
end

function MatchmakingUI:update(dt, t)
...
function MatchmakingUI:_update_background(is_matchmaking, has_mission_vote)
...
function MatchmakingUI:_update_status(dt)
...
function MatchmakingUI:_update_mission_vote_status()
	local voting_manager = self.voting_manager
...
function MatchmakingUI:_update_mission_vote_player_status()
	local voting_manager = self.voting_manager
...
function MatchmakingUI:_update_mission_timer()
	local voting_manager = self.voting_manager
...
function MatchmakingUI:_update_show_timer(has_mission_vote)
	local alpha = nil
...
function MatchmakingUI:update_debug()
...
function MatchmakingUI:destroy()
	return end
...
function MatchmakingUI:_update_button_prompts()
...
function MatchmakingUI:_update_portraits(has_mission_vote)
	if self._active_mechanism == "versus" then
		return
	end
...
function MatchmakingUI:_get_portrait_index(peer_id)
	local portrait_index_table = self.portrait_index_table
	for i = 1, self._max_number_of_players do
...
function MatchmakingUI:_get_first_free_portrait_index()
	local portrait_index_table = self.portrait_index_table
	for i = 1, self._max_number_of_players do
		local player_peer_id = portrait_index_table [i]
...
function MatchmakingUI:large_window_set_player_portrait(index, peer_id)
	...
	portrait_texture =
	portrait_texture or "small_unit_frame_portrait_default"

	content.portrait = portrait_texture
end

function MatchmakingUI:_get_party_slot_index_by_peer_id(peer_id)
	for i = 1, self._max_number_of_players do
		local widget_name = "party_slot_" .. i
		local widget = self:_get_detail_widget(widget_name)
		local content = widget.content
...
function MatchmakingUI:_sync_players_ready_state(dt)
	if self._active_mechanism == "versus" then
		return
	end
...
function MatchmakingUI:_set_player_ready_state(index, is_ready)
	local widget = self:_get_detail_widget("party_slot_" .. index)
	local status_widget = self:_get_widget("player_status_" .. index)
...
function MatchmakingUI:large_window_set_player_connecting(index, is_connecting)
	local widget = self:_get_detail_widget("party_slot_" .. index)
	local status_widget = self:_get_widget("player_status_" .. index)
...
function MatchmakingUI:_set_player_is_voting(index, is_voting)
	local widget = self:_get_detail_widget("party_slot_" .. index)
	widget.content.is_voting = is_voting
end

function MatchmakingUI:_set_player_voted_yes(index, voted_yes)
	local widget = self:_get_detail_widget("party_slot_" .. index)
	if not widget then
		return
	end
...
function MatchmakingUI:_set_detail_difficulty_text(text, optional_color, disable_localization)
	local widget = self:_get_detail_widget("difficulty_text")
...
function MatchmakingUI:_set_detail_level_text(text, localize)
	local widget = self:_get_detail_widget("title_text")
	widget.content.text = localize and Localize(text) or text
end

function MatchmakingUI:_set_status_text(text)
	local widget = self:_get_widget("status_text")
	widget.content.text = Localize(text)
end

function MatchmakingUI:_set_vote_time_progress(progress)
