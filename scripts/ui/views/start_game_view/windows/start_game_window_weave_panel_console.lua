...
function StartGameWindowWeavePanelConsole:on_enter(params, offset)
...
function StartGameWindowWeavePanelConsole:_create_ui_elements(params, offset)
	...
	local temp_text_style = { upper_case = true, localize = true, dynamic_font_size = true, word_wrap = false, font_type = "hell_shark_header",
	...
	for index, settings in ipairs(window_layouts) do
		if settings.panel_sorting and parent:can_add_layout(settings) then
			...
			local optional_offset = { total_length, 0, 0 }
...
function StartGameWindowWeavePanelConsole:on_exit(params)
	print("[StartGameWindow] Exit Substate StartGameWindowWeavePanelConsole")
	self._ui_animator = nil
end

function StartGameWindowWeavePanelConsole:update(dt, t)
	if DO_RELOAD then
...
function StartGameWindowWeavePanelConsole:post_update(dt, t)
	self:_handle_input(dt, t)
end

function StartGameWindowWeavePanelConsole:_update_animations(dt)
...
function StartGameWindowWeavePanelConsole:_is_button_pressed(widget)
	local content = widget.content
...
function StartGameWindowWeavePanelConsole:_is_stepper_button_pressed(widget)
	local content = widget.content
...
function StartGameWindowWeavePanelConsole:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_enter
end

function StartGameWindowWeavePanelConsole:_is_button_hover_exit(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_exit
end

function StartGameWindowWeavePanelConsole:_is_button_selected(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.is_selected
end

function StartGameWindowWeavePanelConsole:_handle_input(dt, t)
	local parent = self._parent
...
function StartGameWindowWeavePanelConsole:_on_panel_button_selected(index)
	local parent = self._parent
...
function StartGameWindowWeavePanelConsole:_set_selected_option(index)
...
function StartGameWindowWeavePanelConsole:_update_selected_option()
	local parent = self._parent
	...
	if selected_layout_name then
		...
		for i, widget in ipairs(title_button_widgets) do
			local layout_name = widget.content.layout_name
			if
			layout_name == selected_layout_name and i ~= self._selected_index then
...
function StartGameWindowWeavePanelConsole:_draw(dt)
...
function StartGameWindowWeavePanelConsole:_play_sound(event)
	self._parent:play_sound(event)
end

function StartGameWindowWeavePanelConsole:_setup_input_buttons()
	local input_service = self._parent:window_input_service()
...
function StartGameWindowWeavePanelConsole:_handle_gamepad_activity()
...
function StartGameWindowWeavePanelConsole:_set_text_button_size(widget, width)
	local ui_scenegraph = self._ui_scenegraph
...
function StartGameWindowWeavePanelConsole:_get_text_width(text_style, text)
...
function StartGameWindowWeavePanelConsole:_set_text_button_horizontal_position(widget, x_position)
	local offset = widget.offset
	offset [1] = x_position
end

function StartGameWindowWeavePanelConsole:_animate_title_entry(widget, dt)
	local content = widget.content
	local style = widget.style
	local hotspot = content.button_hotspot
...
function StartGameWindowWeavePanelConsole:_start_panel_selection_animation(previous_selected_index, new_selected_index)
	local widgets_by_name = self._widgets_by_name
...
function StartGameWindowWeavePanelConsole:_update_panel_selection_animation(dt)
