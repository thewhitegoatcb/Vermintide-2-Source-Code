...
function TutorialTooltipUI:init(ingame_ui_context)
...
function TutorialTooltipUI:destroy()

	return end

function TutorialTooltipUI:create_ui_elements()
...
function TutorialTooltipUI:button_texture_data_by_input_action(input_action, alt_button_name)
...
function TutorialTooltipUI:update(tooltip_tutorial, player_unit, dt)
...
function TutorialTooltipUI:draw(dt, t)
	local ui_renderer = self.ui_renderer
	local ui_scenegraph = self.ui_scenegraph
...
function TutorialTooltipUI:set_dirty()
	self.tutorial_tooltip_widget.element.dirty = true
...
function TutorialTooltipUI:hide()
	local ui_renderer = self.ui_renderer
...
local FADE_TIME = 0.1
function TutorialTooltipUI:fade_in()
	self:_fade(0, 255, FADE_TIME)
end

function TutorialTooltipUI:fade_out()
	self:_fade(255, 0, FADE_TIME)
end

function TutorialTooltipUI:_fade(from_alpha, to_alpha, duration)
	local widget_style = self.tutorial_tooltip_widget.style
...
function TutorialTooltipUI:has_completed_fade()
	if next(self.tutorial_tooltip_animations) ~= nil then
		return false
	end
	return true
...
function TutorialTooltipUI:set_visible(visible)
