...
local input_actions_by_slot = { slot_healthkit = "wield_3", slot_grenade = "wield_5", slot_potion = "wield_4", slot_melee = "wield_1", slot_ranged = "wield_2" }
...
function GamePadEquipmentUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function GamePadEquipmentUI:_create_ui_elements()
	...
	widgets_by_name.overcharge_background.style.texture_id.color = { 100, 150, 150, 150 }
...
function GamePadEquipmentUI:event_swap_equipment_from_storage(slot_name, additional_items)


	if slot_name ~= "slot_grenade" then return
	end
...
function GamePadEquipmentUI:event_input_changed()
...
function GamePadEquipmentUI:_set_switch_input(widget, input_action)
...
function GamePadEquipmentUI:_set_slot_input(widget, input_action)
	local texture_data, input_text, prefix_text = self:_get_input_texture_data(input_action)
	local text_length = input_text and UTF8Utils.string_length(input_text) or 0
	local max_length = 40
...
function GamePadEquipmentUI:_get_input_texture_data(input_action)
	...
	if device_type == "keyboard" then
		...
		do return nil, button_name, prefix_text end
	elseif device_type == "mouse" then
		if is_button_unassigned then button_name = "" else button_name = Mouse.button_name(key_index) end
		do return nil, button_name, prefix_text end
	elseif device_type == "gamepad" then
		if is_button_unassigned then button_name = "" else button_name = Pad1.button_name(key_index) end
...
function GamePadEquipmentUI:_update_widgets()
...
function GamePadEquipmentUI:_get_wield_scroll_input()
...
function GamePadEquipmentUI:_set_wielded_item(item_name, force_select)
...
local allowed_equipment_slots = { slot_grenade = true, slot_healthkit = true, slot_potion = true, slot_melee = false, slot_ranged = false }
...
function GamePadEquipmentUI:_update_equipment_lookup(equipment, inventory_extension)
...
function GamePadEquipmentUI:_check_equipment_changed(equipment, inventory_extension)
	...
	local grenade_slot_data = equipment_slots.slot_grenade
	if grenade_slot_data and grenade_slot_data.item_data and
	inventory_extension:has_additional_item_slots("slot_grenade") then
		local item_count = inventory_extension:get_total_item_count("slot_grenade")
...
function GamePadEquipmentUI:_sync_player_equipment()
				...
				if widget then
					...
					if additional_items then
						...
						if next_additional_item then
							...
							local default_background_color = { 255, 0, 0, 0 }
							local default_glow_color = { 255, 255, 255, 255 }
							...
							if slot_background_color then
								...
								style.secondary_texture_icon_glow.color = { 255, 0, 0, 0 }
							else
...
function GamePadEquipmentUI:_update_ammo_count(item_data, slot_data, player_unit)
...
function GamePadEquipmentUI:_animate_ammo_counter(dt)
...
function GamePadEquipmentUI:_set_ammo_counter_alpha(alpha)
	local ammo_widgets_by_name = self._ammo_widgets_by_name
...
function GamePadEquipmentUI:_set_ammo_counter_color(color)
...
function GamePadEquipmentUI:_set_ammo_text_focus(focus)
	if self._draw_overheat and
	self._overcharge_fraction ~= nil then
		local multiplier = 1
...
function GamePadEquipmentUI:_get_ammunition_count(left_hand_wielded_unit, right_hand_wielded_unit, item_template)
...
function GamePadEquipmentUI:_get_overcharge_amount(player_unit)
...
function GamePadEquipmentUI:_add_animation(name, widget, style, func_name, animation_duration)
...
function GamePadEquipmentUI:_update_animations(dt, t)
	...
	if t_until_fade then
		...
		if progress == 0 then self._time_fade_storage_slots = nil
		end
	end
	local animations = self._animations
	local dirty = false
...
function GamePadEquipmentUI:_animate_weapon_wield(animation_data, dt)
	local widget = animation_data.widget
...
function GamePadEquipmentUI:_animate_weapon_unwield(animation_data, dt)
	local widget = animation_data.widget
	local total_time = animation_data.total_time
	local time = animation_data.time
...
function GamePadEquipmentUI:_animate_slot_wield(animation_data, dt)
	local widget = animation_data.widget
	local total_time = animation_data.total_time
	local time = animation_data.time
...
function GamePadEquipmentUI:_animate_slot_unwield(animation_data, dt)
	local widget = animation_data.widget
	local total_time = animation_data.total_time
	local time = animation_data.time
...
function GamePadEquipmentUI:_add_item(slot_data, data)
...
function GamePadEquipmentUI:_remove_item(index)
	local num_added_items = self._num_added_items or 0
...
function GamePadEquipmentUI:set_position(x, y)
...
function GamePadEquipmentUI:destroy()
	local event_manager = Managers.state.event
...
function GamePadEquipmentUI:set_visible(visible)
	self._is_visible = visible

	self:_set_elements_visible(visible)
end

function GamePadEquipmentUI:_set_elements_visible(visible)
	local ui_renderer = self.ui_renderer
...
function GamePadEquipmentUI:update(dt, t)
...
function GamePadEquipmentUI:_handle_career_change()
...
function GamePadEquipmentUI:_handle_resolution_modified()
	if RESOLUTION_LOOKUP.modified then
		self:_on_resolution_modified()
	end
end

function GamePadEquipmentUI:_on_resolution_modified()
	for _, widget in ipairs(self._widgets) do
		self:_set_widget_dirty(widget)
	end
	for _, widget in ipairs(self._static_widgets) do
...
function GamePadEquipmentUI:_handle_gamepad_activity()
	local gamepad_active = Managers.input:is_device_active("gamepad")
...
function GamePadEquipmentUI:_set_game_options_dirty()
	self._game_options_dirty = true
end


function GamePadEquipmentUI:_update_game_options()
...
function GamePadEquipmentUI:_update_gamepad_input_button()
...
function GamePadEquipmentUI:_handle_gamepad()
...
function GamePadEquipmentUI:draw(dt)
...
function GamePadEquipmentUI:_set_color(color, new_color, ignore_alpha)
...
function GamePadEquipmentUI:set_dirty()
	self._dirty = true
...
function GamePadEquipmentUI:_set_widget_dirty(widget)
...
function GamePadEquipmentUI:_set_overheat_fraction(fraction)
...
function GamePadEquipmentUI:_show_overheat_meter(visible)
	local widgets_by_name = self._widgets_by_name
	local ammo_widgets_by_name = self._ammo_widgets_by_name
...
function GamePadEquipmentUI:_set_widget_visibility(widget, visible)
	widget.content.visible = visible
	self:_set_widget_dirty(widget)
end

function GamePadEquipmentUI:set_alpha(alpha)
...
function GamePadEquipmentUI:set_ammo_alpha(alpha)
...
function GamePadEquipmentUI:set_frame_alpha(alpha)
...
function GamePadEquipmentUI:set_panel_alpha(alpha)
...
function GamePadEquipmentUI:_apply_crosshair_position(x, y)
...
function GamePadEquipmentUI:_show_hold_to_reload(t)
	local gamepad_active = Managers.input:is_device_active("gamepad")
	if ( not gamepad_active or UISettings.use_gamepad_hud_layout == "never" ) and UISettings.use_gamepad_hud_layout ~= "always" then
		return
	end
	...
	if not item_data or not slot_data or not item_template then return
	end
	local ammo_count, remaining_ammo, using_single_clip = self:_get_ammunition_count(slot_data.left_unit_1p, slot_data.right_unit_1p, item_template)
...
function GamePadEquipmentUI:_update_reload_ui_state(t, item_template)
	local reload_tip_widget = self._ammo_widgets_by_name.reload_tip_text
