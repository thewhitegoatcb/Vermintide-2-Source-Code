...
function GameNetworkManager:init(world, lobby, is_server, event_delegate)
	...
	event_delegate:register(self,
	"rpc_play_particle_effect_no_rotation",
	"rpc_play_particle_effect",
	"rpc_play_particle_effect_with_variable",
	"rpc_play_particle_effect_spline",
	...
	"rpc_coop_feedback",
	"rpc_ladder_shake",
	"rpc_request_spawn_network_unit",


	"rpc_flow_event")
end

function GameNetworkManager:lobby()
	return self._lobby
end

function GameNetworkManager:session_id()
	return self._session_id
end

function GameNetworkManager:ping_by_peer(peer_id)
	fassert(self.is_server, "tried to fetch ping by peer id as a client")
	return self._lobby:ping_by_peer(peer_id)
end

function GameNetworkManager:set_small_network_packets(enable)
	if enable then
...
function GameNetworkManager:set_entity_system(entity_system)
	self.entity_system = entity_system
end

function GameNetworkManager:post_init(context)
...
function GameNetworkManager:set_unit_storage(unit_storage)
	self.unit_storage = unit_storage
end
function GameNetworkManager:set_unit_spawner(unit_spawner)
	self.unit_spawner = unit_spawner
end

function GameNetworkManager:in_game_session()
...
function GameNetworkManager:update_receive(dt)
...
function GameNetworkManager:update_transmit(dt)
...
function GameNetworkManager:update(dt)
	if self.is_server and self:in_game_session() then
		...
		local min_ping = NetworkConstants.ping.min local max_ping = NetworkConstants.ping.max
...
function GameNetworkManager:network_time()
...
function GameNetworkManager:is_leaving_game()
...
function GameNetworkManager:game_object_or_level_unit(unit_id, is_level_unit)
	if is_level_unit then
		local level = LevelHelper:current_level(self._world)
		local level_unit = Level.unit_by_index(level, unit_id)
		do return level_unit end
	else
...
function GameNetworkManager:game_object_or_level_id(unit)
...
function GameNetworkManager:unit_game_object_id(unit)
	...
	if go_id then return go_id end
end

function GameNetworkManager:game_object_template(go_type)
	return game_object_templates [go_type]
end

function GameNetworkManager:request_profile(local_player_id, profile_name, career_name, force_respawn)
	if self.network_server then self.network_server:request_profile(local_player_id, profile_name, career_name, force_respawn) end
	if self.network_client then self.network_client:request_profile(local_player_id, profile_name, career_name, force_respawn) end
end

function GameNetworkManager:create_game_object(object_template, data_table, session_disconnect_callback)
...
function GameNetworkManager:create_player_game_object(profile, data_table, session_disconnect_callback)
...
function GameNetworkManager:cb_spawn_point_game_object_created(go_id, owner_id)
...
function GameNetworkManager:game_object_created_player(go_id, owner_peer_id)
...
function GameNetworkManager:game_object_destroyed_player(go_id, owner_peer_id)
	local peer_id = GameSession.game_object_field(self.game_session, go_id, "network_id")
	local local_player_id = GameSession.game_object_field(self.game_session, go_id, "local_player_id")
...
function GameNetworkManager:game_object_created_player_unit_health(go_id, owner_peer_id)
...
function GameNetworkManager:game_object_destroyed_player_unit_health(go_id, owner_peer_id)
	local health_extension = self:_health_extension(go_id)
...
function GameNetworkManager:game_object_created_player_sync_data(go_id, owner_peer_id)
	local peer_id = GameSession.game_object_field(self.game_session, go_id, "network_id")
	local local_player_id = GameSession.game_object_field(self.game_session, go_id, "local_player_id")
...
function GameNetworkManager:game_object_destroyed_player_sync_data(go_id, owner_peer_id)
	local peer_id = GameSession.game_object_field(self.game_session, go_id, "network_id")
	local local_player_id = GameSession.game_object_field(self.game_session, go_id, "local_player_id")
...
function GameNetworkManager:_health_extension(go_id)
...
function GameNetworkManager:_career_extension(go_id)
	local player_unit_id = GameSession.game_object_field(self.game_session, go_id, "unit_game_object_id")
	local player_unit = self.unit_storage:unit(player_unit_id)
	if player_unit == nil then
		return nil
...
function GameNetworkManager:game_object_created(go_id, owner_id)
...
function GameNetworkManager:game_object_created_network_unit(game_object_id, owner_id, go_template)
	self.unit_spawner:spawn_unit_from_game_object(game_object_id, owner_id, go_template)
end

function GameNetworkManager:game_object_created_music_states(game_object_id, owner_id, go_template)
	Managers.music:game_object_created(game_object_id, owner_id, go_template)
end

function GameNetworkManager:game_object_created_keep_decoration(game_object_id, owner_id, go_template)
...
function GameNetworkManager:game_object_destroyed_keep_decoration(game_object_id, owner_id, go_template)
	local unit_level_index = GameSession.game_object_field(self.game_session, game_object_id, "level_unit_index")
	local level = LevelHelper:current_level(self._world)
	local unit = Level.unit_by_index(level, unit_level_index)
	local keep_decoration_extension = ScriptUnit.extension(unit, "keep_decoration_system")
...
function GameNetworkManager:game_object_created_progress_timer(game_object_id, owner_id, go_template)
	local unit_level_index = GameSession.game_object_field(self.game_session, game_object_id, "level_unit_index")
	local level = LevelHelper:current_level(self._world)
	local unit = Level.unit_by_index(level, unit_level_index)
...
function GameNetworkManager:game_object_destroyed_progress_timer(game_object_id, owner_id, go_template)
	local unit_level_index = GameSession.game_object_field(self.game_session, game_object_id, "level_unit_index")
	local level = LevelHelper:current_level(self._world)
	local unit = Level.unit_by_index(level, unit_level_index)
	local progress_extension = ScriptUnit.extension(unit, "progress_system")
...
function GameNetworkManager:game_object_created_game_mode_data(game_object_id, owner_id, go_template)
	Managers.state.game_mode:on_game_mode_data_created(self.game_session, game_object_id)
end

function GameNetworkManager:game_object_destroyed_game_mode_data(game_object_id, owner_id, go_template)
	Managers.state.game_mode:on_game_mode_data_destroyed()
end

function GameNetworkManager:game_object_created_weave(game_object_id, owner_id, go_template)
	Managers.weave:game_object_created(game_object_id)
end

function GameNetworkManager:game_object_destroyed_weave(game_object_id)
	Managers.weave:game_object_destroyed(game_object_id)
end

function GameNetworkManager:game_object_created_weave_objective(game_object_id, owner_id, go_template)
...
function GameNetworkManager:game_object_destroyed_weave_objective(game_object_id)
	local weave_objective_system = Managers.state.entity:system("weave_objective_system")
	weave_objective_system:game_object_destroyed(game_object_id)
end

function GameNetworkManager:game_object_created_horde_surge(game_object_id, owner_id, go_template)
	local game_mode = Managers.state.game_mode:game_mode()
	game_mode._horde_surge_handler._game_object_id = game_object_id
end

function GameNetworkManager:game_object_destroyed_horde_surge(game_object_id)
	local game_mode = Managers.state.game_mode:game_mode()
	game_mode._horde_surge_handler._game_object_id = nil
end

function GameNetworkManager:game_object_created_versus_objective(game_object_id, owner_id, go_template)
...
function GameNetworkManager:game_object_destroyed_versus_objective(game_object_id, owner_id, go_template)
	local versus_objective_system = Managers.state.entity:system("versus_objective_system")
	versus_objective_system:game_object_destroyed(game_object_id)
end

function GameNetworkManager:destroy_game_object(go_id)
...
function GameNetworkManager:game_object_destroyed(go_id, owner_id)
	local go_type_id = GameSession.game_object_field(self.game_session, go_id, "go_type")
	local go_type = NetworkLookup.go_types [go_type_id]
...
function GameNetworkManager:game_object_created_player_unit(go_id, owner_id, go_template)
	if self.is_server then
...
function GameNetworkManager:game_object_destroyed_player_unit(go_id, owner_id, go_template)
	...
	if DEDICATED_SERVER then return false end
...
function GameNetworkManager:game_object_destroyed_network_unit(go_id, owner_id, go_template)
	self.unit_spawner:destroy_game_object_unit(go_id, owner_id, go_template)
end

function GameNetworkManager:game_object_destroyed_music_states(go_id, owner_id, go_template)
	debug_print("MUSIC object destroyed")
	Managers.music:game_object_destroyed(go_id, owner_id, go_template)
end

function GameNetworkManager:game_object_migrated_away(go_id, new_peer_id)
	assert(false, "Not implemented.")
end

function GameNetworkManager:game_object_migrated_to_me(go_id, old_peer_id)
	assert(false, "Not implemented.")
end
...
function GameNetworkManager:game_session_disconnect_music_states(game_object_id)
	Managers.music:client_game_session_disconnect_music_states(game_object_id)
end

function GameNetworkManager:game_object_destroyed_do_nothing()
	return end

function GameNetworkManager:game_object_created_sync_unit(game_object_id, owner_id, go_template)
	Managers.state.entity:system("game_object_system"):game_object_created(game_object_id, owner_id, go_template)
end

function GameNetworkManager:game_object_destroyed_sync_unit(go_id, owner_id, go_template)

	return end

function GameNetworkManager:game_object_created_payload(game_object_id, owner_id, go_template)
	local unit_level_index = GameSession.game_object_field(self.game_session, game_object_id, "level_unit_index")
	local level = LevelHelper:current_level(self._world)
	local unit = Level.unit_by_index(level, unit_level_index)
...
function GameNetworkManager:game_object_destroyed_payload(game_object_id)
	return end

function GameNetworkManager:game_object_created_twitch_vote(game_object_id, owner_id, go_template)
	Managers.twitch:add_game_object_id(game_object_id)
end

function GameNetworkManager:game_object_destroyed_twitch_vote(game_object_id)
	Managers.twitch:remove_game_object_id(game_object_id)
end

function GameNetworkManager:game_object_created_versus_character_selection_unit(game_object_id, owner_id, go_template)
	local mechanism = Managers.mechanism:game_mechanism()
	mechanism._party_selection_logic._go_id = game_object_id
end

function GameNetworkManager:game_object_destroyed_versus_character_selection_unit(game_object_id)
	local mechanism = Managers.mechanism:game_mechanism()
	mechanism._party_selection_logic._go_id = nil
end

function GameNetworkManager:game_object_created_career_data(go_id, owner_peer_id)
...
function GameNetworkManager:game_object_destroyed_career_data(go_id, owner_peer_id)
	local career_extension = self:_career_extension(go_id)
...
function GameNetworkManager:remove_peer(peer_id)
...
function GameNetworkManager:set_peer_synchronizing(peer_id)
	self._object_synchronizing_clients [peer_id] = true
	self.network_transmit:add_peer_ignore(peer_id)
end

function GameNetworkManager:_hot_join_sync(peer_id)
...
function GameNetworkManager:rpc_play_particle_effect(channel_id, effect_id, go_id, node_id, offset, rotation_offset, linked)
	if self.is_server then
...
function GameNetworkManager:rpc_play_particle_effect_no_rotation(sender, effect_id, go_id, node_id, offset, linked)
	if self.is_server then
...
function GameNetworkManager:rpc_play_particle_effect_with_variable(sender, effect_id, position, rotation, variable, value)
	if self.is_server then
...
function GameNetworkManager:rpc_play_particle_effect_spline(sender, fx_name_id, variable_ids, spline_points)
	if self.is_server then
...
function GameNetworkManager:_pack_percentages_completed_arrays(percentages_completed)
	local peer_ids = { }
...
function GameNetworkManager:_unpack_percentages_completed_arrays(peer_ids, local_player_ids, percentages)
...
function GameNetworkManager:gm_event_end_conditions_met(reason, checkpoint_available, percentages_completed)
...
function GameNetworkManager:rpc_gm_event_end_conditions_met(channel_id, reason_id, checkpoint_available, peer_ids, local_player_ids, percentages)
...
function GameNetworkManager:gm_event_round_started()
	self.network_transmit:send_rpc_clients("rpc_gm_event_round_started")
end

function GameNetworkManager:rpc_gm_event_round_started(channel_id)
	Managers.state.game_mode:trigger_event("round_started")
end

function GameNetworkManager:gm_event_initial_peers_spawned()
	self.network_transmit:send_rpc_clients("rpc_gm_event_initial_peers_spawned")
end

function GameNetworkManager:rpc_gm_event_initial_peers_spawned(channel_id)
	Managers.state.game_mode:trigger_event("initial_peers_spawned")
end

function GameNetworkManager:rpc_play_melee_hit_effects(channel_id, sound_event_id, hit_position, sound_type_id, unit_game_object_id)
...
function GameNetworkManager:rpc_request_spawn_network_unit(channel_id, request_spawn_template_id, position, rotation, source_unit_go_id, state_int)
...
function GameNetworkManager:rpc_surface_mtr_fx(channel_id, effect_name_id, unit_game_object_id, position, rotation, normal, actor_index)
...
function GameNetworkManager:rpc_surface_mtr_fx_lvl_unit(channel_id, effect_name_id, unit_level_index, position, rotation, normal, actor_index)
	local level = LevelHelper:current_level(self._world)
	local unit = Level.unit_by_index(level, unit_level_index)
...
function GameNetworkManager:rpc_skinned_surface_mtr_fx(channel_id, effect_name_id, position, rotation, normal)
	if self.is_server then
		local peer_id = CHANNEL_TO_PEER_ID [channel_id]
...
function GameNetworkManager:game_session_host()
	return self.game_session and (self._game_session_host or GameSession.game_session_host(self.game_session))
end

function GameNetworkManager:rpc_enemy_is_alerted(channel_id, unit_id, is_alerted)
...
function GameNetworkManager:rpc_ladder_shake(channel_id, unit_index)
...
function GameNetworkManager:rpc_assist(channel_id, savior_player_id, savior_local_player_id, saved_player_id, saved_local_player_id, predicate_id, enemy_unit_id)
	local player_manager = Managers.player
...
function GameNetworkManager:rpc_coop_feedback(channel_id, player1_peer_id, player1_local_player_id, predicate_id, player2_peer_id, player2_local_player_id)
	if self.is_server then
		local peer_id = CHANNEL_TO_PEER_ID [channel_id]
		Managers.state.network.network_transmit:send_rpc_clients_except("rpc_coop_feedback", peer_id,
		player1_peer_id,
		player1_local_player_id,
		predicate_id,
		player2_peer_id,
		player2_local_player_id)
	end
...
function GameNetworkManager:rpc_flow_event(channel_id, unit_id, event_id)
	local unit = self.unit_storage:unit(unit_id)
	if not unit then
...
function GameNetworkManager:anim_event(unit, event)
	local anim_system = Managers.state.entity:system("animation_system")
	anim_system:anim_event(unit, event)
end

function GameNetworkManager:anim_event_with_variable_float(unit, event, variable_name, variable_value)
	local anim_system = Managers.state.entity:system("animation_system")
	anim_system:anim_event_with_variable_float(unit, event, variable_name, variable_value)
end

function GameNetworkManager:anim_set_variable_float(unit, variable_name, variable_value)
	local go_id = self.unit_storage:go_id(unit)
