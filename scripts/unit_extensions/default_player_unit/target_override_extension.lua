...
function TargetOverrideExtension:init(extension_init_context, unit, extension_init_data)
	self._unit = unit
	self._result_table = { }
...
function TargetOverrideExtension:destroy()
	return end

function TargetOverrideExtension:taunt(radius, duration, do_stagger, taunt_bosses)
	...
	local num_ai_units = AiUtils.broadphase_query(position, radius,
	result_table,
	self._broadphase_categories)
...
function TargetOverrideExtension:update(unit, input, dt, context, t)
...
function TargetOverrideExtension:add_to_override_targets(ai_unit, target_unit, ai_unit_blackboard, t)
	local override_time = t + OVERRIDE_LIFETIME
