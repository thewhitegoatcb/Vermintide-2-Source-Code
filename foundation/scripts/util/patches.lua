	...
	return rawset(_G, library_name, setmetatable(library, {
		__index = function (self, key)
			...
			if not script_data.disable_auto_patch_missing_methods then
				...
				library_name, key, Script.callstack())
...
if not _G.FOUNDATION_patches_applied and (IS_CONSOLE or DEDICATED_SERVER) then
	...
	function DisplayAdapter.mode() return 1, 1 end
...
if not EAC then
	...
	function EAC.initialization_error() return false, "No error" end
	EAC.before_join = NOP
	EAC.after_leave = NOP
	function EAC.state() return "untrusted", "No Steam", "", "not_running" end
	EAC.set_host = NOP
...
if not EACServer then
	...
	function EACServer.state() return "untrusted", "No Steam" end
end
...
if not STRING_FORMAT then
	...
	function string.format(str, ...)
		...
		if not success then
			...
			do return "<Invalid string format>" end
		else
