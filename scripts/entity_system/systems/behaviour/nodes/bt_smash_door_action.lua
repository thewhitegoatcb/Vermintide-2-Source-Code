...
function BTSmashDoorAction:init(...)
	BTSmashDoorAction.super.init(self, ...)
end

BTSmashDoorAction.name = "BTSmashDoorAction"

function BTSmashDoorAction:enter(unit, blackboard, t)
...
function BTSmashDoorAction:leave(unit, blackboard, t, reason, destroy)
	if not destroy then
		local locomotion_extension = blackboard.locomotion_extension
...
function BTSmashDoorAction:run(unit, blackboard, t, dt)
...
function BTSmashDoorAction.StateInit:on_enter(params)
	self.blackboard = params.blackboard
	self.unit = params.unit
	self.entrance_pos = params.entrance_pos
end

function BTSmashDoorAction.StateInit:update(dt, t)
	...
	if blackboard.is_in_smartobject_range then
		...
		if navigation_extension:use_smart_object(true) then
			...
			do return BTSmashDoorAction.StateMovingToSmartObjectEntrance end
		else
...
function BTSmashDoorAction.StateMovingToSmartObjectEntrance:on_enter(params)
	self.blackboard = params.blackboard
	self.unit = params.unit
...
function BTSmashDoorAction.StateMovingToSmartObjectEntrance:update(dt, t)
...
function BTSmashDoorAction.StateOpening:on_enter(params)
...
function BTSmashDoorAction.StateOpening:update(dt, t)
	local blackboard = self.blackboard
	local target_unit = self.target_unit
	...
	if door_extension:is_open() and not door_extension:is_opening() then
		do return BTSmashDoorAction.StateMovingToSmartObjectExit end
...
function BTSmashDoorAction.StateAttacking:on_enter(params)
...
function BTSmashDoorAction.StateAttacking:update(dt, t)
	local blackboard = self.blackboard
	local target_unit = self.target_unit
	local door_extension = ScriptUnit.extension(target_unit, "door_system")
...
function BTSmashDoorAction.StateAttacking:on_exit()
	local target_unit = self.target_unit
	local door_extension = ScriptUnit.extension(target_unit, "door_system")
	door_extension.num_attackers = door_extension.num_attackers - 1
end

function BTSmashDoorAction.StateAttacking:attack()
	local target_unit = self.target_unit
	local unit = self.unit
	local blackboard = self.blackboard
...
function BTSmashDoorAction.StateMovingToSmartObjectExit:on_enter(params)
	self.blackboard = params.blackboard
	self.unit = params.unit
...
function BTSmashDoorAction.StateMovingToSmartObjectExit:update(dt, t)
	local unit = self.unit
	local blackboard = self.blackboard
...
function BTSmashDoorAction:anim_cb_damage(unit, blackboard)
...
function BTSmashDoorAction:anim_cb_attack_overlap_done(unit, blackboard)
	if blackboard.smash_door.target_unit then
		local action = blackboard.action
		AiUtils.damage_target(blackboard.smash_door.target_unit, unit, action, action.damage)
	end
