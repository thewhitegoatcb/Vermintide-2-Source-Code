...
function CareerAbilityWHCaptain:init(extension_init_context, unit, extension_init_data)
...
function CareerAbilityWHCaptain:extensions_ready(world, unit)
...
function CareerAbilityWHCaptain:destroy()
	return end

function CareerAbilityWHCaptain:update(unit, input, dt, context, t)
...
function CareerAbilityWHCaptain:stop(reason)
	if
...
function CareerAbilityWHCaptain:_update_priming(dt)
...
function CareerAbilityWHCaptain:_stop_priming()
	local effect_id = self._priming_fx_id
	if effect_id then
		local world = self._world
...
function CareerAbilityWHCaptain:_run_ability(new_initial_speed)
	self:_stop_priming()
	local career_extension = self._career_extension
	...
	if not talent_extension:has_talent("victor_witchhunter_activated_ability_guaranteed_crit_self_buff") then
		...
		for _, player_unit in pairs(nearby_player_units) do
			...
			not side_manager:is_enemy(owner_unit, player_unit) then
...
function CareerAbilityWHCaptain:_play_vo()
	local owner_unit = self._owner_unit
...
function CareerAbilityWHCaptain:_play_vfx()
	local owner_unit = self._owner_unit
