...
function HordeSpawner:init(world, cover_points_broadphase)
...
function HordeSpawner:horde(horde_type, extra_data, side_id, no_fallback)
...
function HordeSpawner:execute_fallback(horde_type, side_id, fallback, reason, extra_data)
...
function HordeSpawner:execute_event_horde(t, terror_event_type, side_id, composition_type, limit_spawners, silent, group_template, strictly_not_close_to_players, sound_settings, use_closest_spawners, source_unit, optional_data)
...
function HordeSpawner:_execute_event_horde(t, side_id, composition_type, limit_spawners, silent, group_template, strictly_not_close_to_players, sound_settings, use_closest_spawners, source_unit, optional_data)
	...
	local horde = { composition_type = composition_type,
...
function HordeSpawner:max_composition_size(composition_name)
...
function HordeSpawner:running_horde()
...
function random_array_insert(array, size, data)
	...
	array [size + 1] = array [swap] array [swap] = data
end


local ok_spawner_breeds = { skaven_clan_rat = true, skaven_slave = true }
local spawn_list_a = { }
local spawn_list_b = { }
function HordeSpawner:compose_horde_spawn_list(variant)
...
function HordeSpawner:compose_blob_horde_spawn_list(composition_type)
...
function HordeSpawner:pop_random_any_breed()
...
function HordeSpawner:pop_random_horde_breed_only()
	local breed = pop_array(spawn_list_a)
	return breed
end

function HordeSpawner:execute_ambush_horde(extra_data, side_id, fallback, override_epicenter_pos, optional_data)
	...
	local group_template = { template = "horde",
		id = group_id,
	...
	local horde = { horde_type = "ambush", spawned = 0, num_to_spawn = num_to_spawn,
	...
	local spawner_count = 0 local last_spawner_count = -1
	...
	while last_spawner_count ~= spawner_count and spawner_count < num_to_spawn do
		...
		for i = 1, num_sectors do
			...
			if spawner then
				...
				if breed_name then
					horde.horde_spawns [#horde.horde_spawns + 1] = { num_to_spawn = 1, spawner = spawner, spawn_list = { breed_name }, hidden = hidden }
			...
			if cover_point_unit then
				...
				if breed_name then
					horde.cover_spawns [#horde.cover_spawns + 1] = { num_to_spawn = 1, next_spawn_time = spawn_time, cover_point_unit = cover_point_unit, spawn_list = { breed_name } }
					spawner_count = spawner_count + 1
...
function HordeSpawner:replace_hidden_spawners(hidden_spawners, spawner_in_sight, offending_player_pos)
	...
	table.clear_array(found_cover_points, #found_cover_points)
	self:hidden_cover_points(self.cover_broadphase, epicenter_pos, { epicenter_pos }, found_cover_points, hidden_min_dist, hidden_max_dist, main_target_pos)
...
function HordeSpawner:find_vector_horde_spawners(epicenter_pos, main_target_pos)
...
function HordeSpawner:find_good_vector_horde_pos(main_target_pos, distance, check_reachable)
...
function HordeSpawner:execute_vector_horde(extra_data, side_id, fallback)
	local settings = CurrentHordeSettings.vector
	local max_spawners = settings.max_spawners
	...
	local horde = { horde_type = "vector", spawned = 0, num_to_spawn = num_to_spawn,
	...
	local spawn_counter = 0 local last_spawn_counter = nil
	...
	for i = 1, n_horde_spawners do
		...
		horde_spawns [#horde_spawns + 1] = { num_to_spawn = 1, spawner = spawner, spawn_list = { breed_name }, hidden = hidden }
	...
	for i = 1, n_cover_spawners do
		...
		local breed_name = self:pop_random_any_breed()
		cover_spawns [#cover_spawns + 1] = { num_to_spawn = 1, next_spawn_time = spawn_time, cover_point_unit = spawner, spawn_list = { breed_name } }
		spawn_counter = spawn_counter + 1
		spawn_time = spawn_time + 0.1
	end
...
function HordeSpawner:execute_custom_horde(spawn_list, only_ahead, side_id)
	...
	for i = 1, num_to_spawn do
		...
		for j = 1, max_attempts do
			...
			if spawn_pos then
				...
				local optional_data = { side_id = side_id }
...
function HordeSpawner:get_pos_ahead_or_behind_players_on_mainpath(check_ahead, dist, raw_dist, side_id)
	...
	if hidden and is_within_raw_distance then
		do return true, wanted_pos, to_player_dir end
	else
		return false
	end
end

function HordeSpawner:execute_vector_blob_horde(extra_data, side_id, fallback)
	local settings = CurrentHordeSettings.vector_blob
	local roll = math.random()
	local spawn_horde_ahead = roll <= settings.main_path_chance_spawning_ahead
	print("wants to spawn " .. (spawn_horde_ahead and "ahead" or "behind") .. " within distance: ", settings.main_path_dist_from_players)
	...
	local t = Managers.time:time("game")
	local sound_settings = composition.sound_settings
	local horde = { horde_type = "vector_blob", spawned = 0, num_to_spawn = num_to_spawn,
...
function HordeSpawner:spawn_unit(hidden_spawn, breed_name, goal_pos, horde)
...
function HordeSpawner:play_sound(stinger_name, pos)
...
function HordeSpawner:create_event_horde_no_horde_spawners(horde, variant, t, side_id)
	local conflict_director = Managers.state.conflict
	local main_path_info = conflict_director.main_path_info
	...
	if behind_unit then
		...
		if num_found > 0 then
			...
			horde.cover_spawns = { num_to_spawn = 0, next_spawn_time = t, cover_point_unit = cover_point_unit, spawn_list = spawn_list }
...
function HordeSpawner:update_event_horde_no_horde_spawners(horde, t)
...
function HordeSpawner:update_event_horde(horde, t)
	if not horde.started then
		if horde.start_time < t then
...
function HordeSpawner:spawner_in_view_of_players(spawner, side_id, spawner_pos)
	...
	for i = 1, #player_and_bot_positions do
		...
		if distance < 30 then
			...
			local hit, _, _, _, _ = PhysicsWorld.immediate_raycast(
			self.physics_world,
			spawner_pos + up_vector,
			direction,
			distance,
...
function HordeSpawner:update_horde(horde, t)
...
function HordeSpawner:update(t, dt)
	local hordes = self.hordes
...
function HordeSpawner:set_horde_has_spawned(group_id)
	local hordes = self.hordes
	for i = 1, #hordes do
		local horde = hordes [i]
...
function HordeSpawner:set_horde_is_done(group_id)
	local hordes = self.hordes
	for i = 1, #hordes do
		local horde = hordes [i]
		local horde_group_id = horde.group_id
...
function HordeSpawner:debug_hordes(t)
...
local found_units = { }
function HordeSpawner:hidden_cover_points(broadphase, epicenter_pos, player_pos_list, found_cover_points, min_rad, max_rad, main_target_pos)
...
function HordeSpawner:calc_sectors(center_pos, list, sectors)
	local unit_local_position = Unit.local_position
	local vector3_normalize = Vector3.normalize
...
function HordeSpawner:render_sectors(sectors)
	local unit_local_position = Unit.local_position
	local sector_colors = { Color(255, 255, 0, 0), Color(255, 255, 128, 0),
		Color(255, 0, 255, 0), Color(255, 128, 255, 0),
		Color(255, 0, 0, 255), Color(255, 0, 128, 255),
...
function HordeSpawner:reset_sectors(sectors)
	for i = 1, num_sectors do
		local sector = sectors [i]
		for j = 1, #sector do
			sector [j] = nil
...
function HordeSpawner:filter_dist(center_pos, list, min_rad_squared, max_rad_squared)
	local distance_squared = Vector3.distance_squared
	local size = #list
	local i = 1
	while size >= i do
...
function HordeSpawner:filter_angle(center_pos, pos_list, dot_angle)
...
function HordeSpawner:get_point_on_main_path(pos, distance, confirm_with_far_astar)
