...
if not IS_WINDOWS or not has_ffi or not has_jutil then return
end
local C = ffi.C
local bit = bit local debug = debug local math = math local string = string
local pairs = pairs local tonumber = tonumber local type = type
...
local itypename = { [0] = "nil", "false", "true", "lightud", "str", "upval", "thread", "proto", "func", "trace", "cdata", "tab", "udata", "numx" }
...
function LuaJIT.itype(o)
	...
	if it % 4294967296.0 <= 4294901759.0 then tag = 13
	elseif bit.arshift(it, 15) == -2 then tag = 3
	end
...
local function sizeof(lut, o, to)
	...
	elseif to == "userdata" then
		...
		if it == "lightud" then return 0 end
		local ptr = toptr("uint32_t*", o)
		do return 16 + ptr [-3] end
