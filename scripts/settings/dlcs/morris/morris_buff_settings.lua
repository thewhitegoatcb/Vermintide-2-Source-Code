...
local function find_pickup_buff_settings(buff, pickup_settings)
	...
	local buff_settings = template.pickup_names and template.pickup_names [pickup_settings.pickup_name] or
	template.pickup_slot_names and template.pickup_slot_names [pickup_settings.slot_name] or
	template.pickup_types and template.pickup_types [pickup_settings.type]
...
local function spawn_barrel(item_name, position, rotation, velocity, explode_time, fuse_time, random_explosion_delay)
	...
	local extension_init_data = {
		...
		pickup_system = { has_physics = true, spawn_type = "loot",
...
dlc_settings.buff_function_templates = {
	...
	start_armor_breaker = function (unit, buff, params, world)
		...
		if unit == local_player_unit then
			...
			local armor_breaker_power_level = math.max(
			get_armor_breaker_power_level(career_name, "slot_melee"),
			get_armor_breaker_power_level(career_name, "slot_ranged"))
	...
	curse_khorne_champions_unit_link_unit = function (unit, buff, params, world)
		...
		Managers.state.unit_spawner:create_unit_extensions(
		Unit.world(spawned_unit),
		spawned_unit,
	...
	deus_knockdown_damage_immunity_aura_func = function (owner_unit, buff, params)
		...
		for i = 1, num_units do
			...
			if Unit.alive(unit) and unit ~= owner_unit then
				...
				if
				distance_squared < range_squared and knocked_down and not buff_extension:has_buff_type(buff_to_add) then
	...
	money_magnet_start = function (unit, buff, params)
		if not is_local(unit) then return
		end
	...
	detect_weakness_unit_entered_range = function (unit, buffer_unit, buff, params, world)
		if not is_local(buffer_unit) then return
		end
	...
	detect_weakness_unit_left_range = function (unit, user_data, buffer_unit, buff, params, world)
		...
		if
		marked_enemy == unit and HEALTH_ALIVE [marked_enemy] then
			local buff_extension = ScriptUnit.extension(unit, "buff_system")
	...
	hidden_escape_remove = function (unit, buff, params, world)
		if is_local(unit) then
			...
			local buff_extension = ScriptUnit.has_extension(unit, "buff_system")
			buff_extension:add_buff(cooldown_buff, { attacker_unit = unit })
	...
	update_bad_breath = function (unit, buff, params)
		...
		if buff.rescue_timer and buff.rescue_timer < time then
			...
			local area_damage_system = Managers.state.entity:system("area_damage_system")
			area_damage_system:create_explosion(
			unit,
			hit_position,
			rotation,
			explosion_template,
...
dlc_settings.proc_functions = {
	...
	stagger_aoe_on_hit = function (owner_unit, buff, params)
		if not is_server() then return end
		if not ALIVE [owner_unit] then return
		end
	...
	apply_mark_of_nurgle_dot = function (owner_unit, buff, params, world, param_order)
		...
		if ALIVE [hit_unit] and damage_source ~= "dot_debuff" then
			...
			local params = { attacker_unit = attacker_unit }
	...
	mark_of_nurgle_explosion = function (owner_unit, buff, params)
		...
		if projectile_system then
			...
			local extension_init_data = {
				area_damage_system = { area_damage_template = "globadier_area_dot_damage", invisible_unit = true, nav_tag_volume_layer = "bot_poison_wind", create_nav_tag_volume = true, damage_source = "poison_dot", player_screen_effect_name = "fx/screenspace_poison_globe_impact", area_ai_random_death_template = "area_poison_ai_random_death", dot_effect_name = "fx/wpnfx_poison_wind_globe_impact", explosion_template_name = "corrupted_flesh_explosion", extra_dot_effect_name = "fx/chr_gutter_death", damage_players = true,
		...
		if ALIVE [owner_unit] then
			...
			if pickup_specific_settings then
				...
				for i = 1, #player_and_bot_units do
					...
					if ALIVE [ally_unit] and ally_unit ~= owner_unit and
					Vector3.distance_squared(player_pos, POSITION_LOOKUP [ally_unit]) <= range_sq then
	...
	drop_item_on_ability_use = function (owner_unit, buff, params)
		...
		if inventory_extension then
			local items = { }
			local item_slots = { "slot_healthkit", "slot_potion", "slot_grenade" }
	...
	deus_push_charge = function (owner_unit, buff, params, world)
		if ALIVE [owner_unit] then
			...
			status_extension.do_lunge = { animation_end_event = "dodge_bwd", allow_rotation = false, first_person_animation_end_event = "dodge_bwd", first_person_hit_animation_event = "charge_react", dodge = true, first_person_animation_event = "dodge_bwd", first_person_animation_end_event_hit = "dodge_bwd", noclip = true, animation_event = "dodge_bwd",
	...
	melee_wave_effect = function (owner_unit, buff, params)
		...
		if not server_buff_ids then return
		end
			...
			for i = 1, stacks do
				...
				if current_stacks < max_stacks then
					...
					if not server_buff_ids then
						parent_buff_shared_table.server_buff_ids = { server_buff_id }
					else
	...
	cluster_barrel_on_barrel_exploded = function (owner_unit, buff, params)
		...
		for i = 1, barrel_count do
			local randomized_direction = Vector3(
			math.random() * 2 - 1,
			math.random() * 2 - 1,
			math.random() * 2 - 1)
			...
			math.random() * max_horizontal_velocity * 2 - max_horizontal_velocity,
			math.random() * max_horizontal_velocity * 2 - max_horizontal_velocity,
	...
	lucky_on_non_crit = function (owner_unit, buff, params)
		...
		local id = buff_extension:add_buff(buff_name, { attacker_unit = owner_unit })
	...
	start_bad_breath_timer = function (owner_unit, buff, params)
		if not is_server() then return false
		end
		local template = buff.template
	...
	static_blade_on_timed_block = function (owner_unit, buff, params)
		...
		local damage = DamageUtils.calculate_damage(DamageOutput, attacking_unit, owner_unit, hit_zone_name, power_level, boost_curve,
		...
		buff_extension:add_buff(cooldown_buff, { attacker_unit = owner_unit })
...
dlc_settings.explosion_templates = {
	...
	armor_breaker = { name = "armor_breaker",

		explosion = { use_attacker_power_level = true, radius = 4, hit_sound_event = "Play_wind_metal_gameplay_mutator_wind_hit", damage_profile = "armor_breaker", no_friendly_fire = true } },
	...
	bolt_of_change = { time_to_explode = 3, follow_time = 6,
		...
		explosion = { trigger_on_server_only = true, radius = 4, alert_enemies_radius = 20, attack_template = "grenade", alert_enemies = true, always_hurt_players = true, different_power_levels_for_players = true, buildup_effect_time = 1.5, sound_event_name = "Play_mutator_enemy_split_large", damage_profile = "bolt_of_change", power_level = 250, buildup_effect_name = "fx/deus_lightning_strike_02", effect_name = "fx/deus_lightning_strike_01",
			...
			camera_effect = { near_distance = 5, near_scale = 1, shake_name = "lightning_strike", far_scale = 0.15, far_distance = 20 } } },
	...
	magma = {
		aoe = { dot_template_name = "burning_magma_dot", nav_tag_volume_layer = "fire_grenade", dot_balefire_variant = true, create_nav_tag_volume = true, attack_template = "wizard_staff_geiser", sound_event_name = "player_combat_weapon_fire_bw_deus_01_impact", damage_interval = 0.5, duration = 10, area_damage_template = "explosion_template_aoe",
			...
			nav_mesh_effect = { particle_radius = 2, particle_name = "fx/wpnfx_bw_deus_geyser_01_remap", particle_spacing = 0.9 } } },
	...
	bots_avoid_curse = {
		aoe = { duration = 5, radius = 5, create_nav_tag_volume = true, nav_tag_volume_layer = "bot_poison_wind" } },
	...
	corrupted_flesh_explosion = {
		aoe = { start_aoe_sound_event_name = "Play_curse_corrupted_flesh_explosion", stop_aoe_sound_event_name = "Stop_curse_corrupted_flesh_explosion" } },
	...
	holy_hand_grenade = { is_grenade = true,

		explosion = { dont_rotate_fx = true, radius = 10, max_damage_radius = 6, alert_enemies_radius = 20, sound_event_name = "Play_blessing_morris_grenade_explosion", attack_template = "grenade", damage_profile_glance = "holy_hand_grenade", alert_enemies = true, damage_profile = "holy_hand_grenade", effect_name = "fx/wpnfx_holy_handgrenade_explosion_01",



			difficulty_power_level = {
				easy = { power_level_glance = 4000, power_level = 8000 },
				normal = { power_level_glance = 8000, power_level = 16000 },
				hard = { power_level_glance = 12000, power_level = 24000 },
				harder = { power_level_glance = 16000, power_level = 32000 },
				hardest = { power_level_glance = 20000, power_level = 40000 },
				cataclysm = { power_level_glance = 12000, power_level = 24000 },
				cataclysm_2 = { power_level_glance = 16000, power_level = 32000 },
				cataclysm_3 = { power_level_glance = 20000, power_level = 40000 } },
			...
			camera_effect = { near_distance = 10, near_scale = 1, shake_name = "holy_hand_grenade_explosion", far_scale = 0.5, far_distance = 40 } } },
	...
	curse_skulls_of_fury_explosion = { time_to_explode = 3,
		...
		explosion = { trigger_on_server_only = true, radius = 4, alert_enemies = true, buildup_effect_name = "fx/deus_curse_skulls_of_fury_timer_01", buildup_effect_time = 3, deletion_timer = 0, alert_enemies_radius = 20, attack_template = "skulls_of_fury", different_power_levels_for_players = true, sound_event_name = "Play_curse_skulls_of_fury_explosion", effect_name = "fx/magic_wind_fire_explosion_01", always_hurt_players = true, max_damage_radius = 4, unit_scale = 1, damage_profile_glance = "curse_skulls_of_fury_explosion_glance", damage_profile = "curse_skulls_of_fury_explosion",
			...
			difficulty_power_level = {
				easy = { power_level_glance = 50, power_level = 100 },
				normal = { power_level_glance = 100, power_level = 200 },
				hard = { power_level_glance = 150, power_level = 300 },
				harder = { power_level_glance = 200, power_level = 400 },
				hardest = { power_level_glance = 250, power_level = 500 },
				cataclysm = { power_level_glance = 300, power_level = 600 },
				cataclysm_2 = { power_level_glance = 350, power_level = 700 },
				cataclysm_3 = { power_level_glance = 400, power_level = 800 } },
	...
	we_deus_01_small = {
		explosion = { use_attacker_power_level = true, radius_min = 0.5, radius_max = 1, attacker_power_level_offset = 0.25, max_damage_radius_min = 0.1, damage_profile_glance = "we_deus_01_small_explosion_glance", max_damage_radius_max = 0.75, sound_event_name = "we_deus_01_big_hit", damage_profile = "we_deus_01_small_explosion", effect_name = "fx/wpnfx_we_deus_01_impact" } },
	...
	we_deus_01_large = {
		explosion = { use_attacker_power_level = true, radius_min = 1.25, sound_event_name = "we_deus_01_big_hit", radius_max = 3, attacker_power_level_offset = 0.25, max_damage_radius_min = 0.5, alert_enemies_radius = 10, damage_profile_glance = "we_deus_01_large_explosion_glance", max_damage_radius_max = 2, alert_enemies = true, damage_profile = "we_deus_01_large_explosion", effect_name = "fx/wpnfx_we_deus_01_explosion" } },
	...
	deus_relic_small = {
		explosion = { use_attacker_power_level = true, radius_min = 0.5, no_friendly_fire = true, radius_max = 1, attacker_power_level_offset = 0.25, max_damage_radius_min = 0.1, damage_profile_glance = "deus_relic_small_explosion_glance", max_damage_radius_max = 0.75, sound_event_name = "we_deus_01_big_hit", damage_profile = "deus_relic_small_explosion", effect_name = "fx/wpnfx_we_deus_01_impact" } },
	...
	deus_relic_large = {
		explosion = { use_attacker_power_level = true, radius_min = 1.25, sound_event_name = "we_deus_01_big_hit", radius_max = 3, no_friendly_fire = true, attacker_power_level_offset = 0.25, max_damage_radius_min = 0.5, alert_enemies_radius = 10, damage_profile_glance = "deus_relic_large_explosion_glance", max_damage_radius_max = 2, alert_enemies = true, damage_profile = "deus_relic_large_explosion", effect_name = "fx/wpnfx_we_deus_01_explosion" } },
	...
	dr_deus_01 = {
		explosion = { use_attacker_power_level = true, dont_rotate_fx = true, radius = 4, max_damage_radius = 1, alert_enemies_radius = 20, attacker_power_level_offset = 2, attack_type = "grenade", attack_template = "grenade", sound_event_name = "player_combat_weapon_dr_deus_01_explosion", damage_profile_glance = "dr_deus_01_glance", alert_enemies = true, damage_profile = "dr_deus_01_explosion", effect_name = "fx/wpnfx_frag_grenade_impact",
			...
			camera_effect = { near_distance = 5, near_scale = 1, shake_name = "frag_grenade_explosion", far_scale = 0.15, far_distance = 20 } } },
	...
	buff_explosion = {
		explosion = { use_attacker_power_level = true, radius = 3, max_damage_radius = 1.5, alert_enemies_radius = 10, attacker_power_level_offset = 0.5, effect_name = "fx/cw_enemy_explosion", attack_template = "grenade", sound_event_name = "fireball_big_hit", damage_profile_glance = "frag_grenade_glance", alert_enemies = true, damage_profile = "frag_grenade", no_friendly_fire = true,
	...
	deus_ranged_crit_explosion = {
		explosion = { radius = 3, alert_enemies = true, max_damage_radius = 2.5, no_friendly_fire = true, alert_enemies_radius = 15, sound_event_name = "Play_enemy_combat_warpfire_backpack_explode", damage_profile = "frag_grenade", effect_name = "fx/cw_enemy_explosion",


			difficulty_power_level = {
				easy = { power_level_glance = 100, power_level = 200 },
				normal = { power_level_glance = 100, power_level = 100 },
				hard = { power_level_glance = 200, power_level = 200 },
				harder = { power_level_glance = 300, power_level = 300 },
				hardest = { power_level_glance = 400, power_level = 400 },
				cataclysm = { power_level_glance = 300, power_level = 600 },
				cataclysm_2 = { power_level_glance = 400, power_level = 800 },
				cataclysm_3 = { power_level_glance = 500, power_level = 1000 } } } },
	...
	shield_splinters = { name = "shield_splinters",

		explosion = { use_attacker_power_level = true, radius = 4, no_friendly_fire = true, hit_sound_event = "Play_wind_metal_gameplay_mutator_wind_hit", damage_profile = "armor_breaker", sound_event_name = "boon_shield_of_splinters", effect_name = "fx/wpnfx_flaming_flail_hit_01" } },
	...
	blazing_revenge = { name = "blazing_revenge",

		aoe = { dot_template_name = "burning_dot_fire_grenade", radius = 4, nav_tag_volume_layer = "fire_grenade", create_nav_tag_volume = true, attack_template = "fire_grenade_dot", friendly_fire = false, damage_interval = 1, area_damage_template = "explosion_template_aoe",
			...
			nav_mesh_effect = { particle_radius = 2, particle_name = "fx/wpnfx_fire_grenade_impact_remains", particle_spacing = 0.9 } } },
	...
	thorn_skin = { name = "thorn_skin",

		explosion = { use_attacker_power_level = true, radius = 2, hit_sound_event = "Play_wind_metal_gameplay_mutator_wind_hit", damage_profile = "thorn_skin", no_friendly_fire = true } },
	...
	static_blade = { name = "static_blade",

		explosion = { use_attacker_power_level = true, radius = 1, no_friendly_fire = true, max_damage_radius = 0, damage_profile = "markus_knight_charge", no_prop_damage = true, attack_template = "markus_knight_charge" } } }
...
dlc_settings.buff_templates = {
	liquid_bravado_potion = {
		buffs = { { name = "liquid_bravado_potion", stat_buff = "power_level", max_stacks = 1, icon = "potion_liquid_bravado", remove_buff_func = "remove_deus_potion_buff", refresh_durations = true,
	...
	liquid_bravado_potion_increased = {
		buffs = { { name = "liquid_bravado_potion_increased", stat_buff = "power_level", max_stacks = 1, icon = "potion_liquid_bravado", remove_buff_func = "remove_deus_potion_buff", refresh_durations = true,
	...
	vampiric_draught_potion = {
		buffs = { { name = "vampiric_draught_potion_heal", remove_buff_func = "remove_deus_potion_buff", buff_func = "vampiric_heal", event = "on_damage_dealt", refresh_durations = true, max_stacks = 1, icon = "potion_vampiric_draught",
	...
	vampiric_draught_potion_increased = {
		buffs = { { name = "vampiric_draught_potion_heal_increased", remove_buff_func = "remove_deus_potion_buff", buff_func = "vampiric_heal", event = "on_damage_dealt", refresh_durations = true, max_stacks = 1, icon = "potion_vampiric_draught",
	...
	moot_milk_potion = {
		...
		buffs = { { buff_to_add = "moot_milk_strength", name = "moot_milk_potion", remove_buff_func = "add_buff", refresh_durations = true, max_stacks = 1,
	...
	moot_milk_strength = {
		buffs = { { apply_buff_func = "apply_movement_buff", name = "moot_milk_increase_dodge_distance", icon = "potion_moot_milk", refresh_durations = true, remove_buff_func = "remove_movement_buff", max_stacks = 1,
			...
			{ name = "moot_milk_increase_dodge_speed", max_stacks = 1, remove_buff_func = "remove_movement_buff", apply_buff_func = "apply_movement_buff", refresh_durations = true,
			...
			{ remove_buff_func = "remove_deus_potion_buff", name = "moot_milk_sound", refresh_durations = true,
	...
	moot_milk_potion_increased = {
		...
		buffs = { { buff_to_add = "moot_milk_strength_increased", name = "moot_milk_strength_increased", refresh_durations = true, max_stacks = 1, remove_buff_func = "add_buff",
	...
	moot_milk_strength_increased = {
		buffs = { { apply_buff_func = "apply_movement_buff", name = "moot_milk_increase_dodge_distance_increased", icon = "potion_moot_milk", refresh_durations = true, remove_buff_func = "remove_movement_buff", max_stacks = 1,
			...
			{ name = "moot_milk_increase_dodge_speed_increased", max_stacks = 1, remove_buff_func = "remove_movement_buff", apply_buff_func = "apply_movement_buff", refresh_durations = true,
			...
			{ remove_buff_func = "remove_deus_potion_buff", name = "moot_milk_sound_increased", refresh_durations = true,
	...
	friendly_murderer_potion = {
		buffs = { { name = "friendly_murderer_potion", buff_func = "friendly_murder", event = "on_damage_dealt", remove_buff_func = "remove_deus_potion_buff", refresh_durations = true, max_stacks = 1, icon = "potion_friendly_murderer",
	...
	friendly_murderer_potion_increased = {
		buffs = { { name = "friendly_murderer_potion_increased", buff_func = "friendly_murder", event = "on_damage_dealt", remove_buff_func = "remove_deus_potion_buff", refresh_durations = true, max_stacks = 1, icon = "potion_friendly_murderer",
	...
	killer_in_the_shadows_potion = {
		buffs = { { remove_buff_func = "remove_killer_in_the_shadows_buff", name = "killer_in_the_shadows_potion", icon = "potion_killer_in_the_shadows", refresh_durations = true, max_stacks = 1, apply_buff_func = "apply_killer_in_the_shadows_buff",
	...
	killer_in_the_shadows_potion_increased = {
		buffs = { { remove_buff_func = "remove_killer_in_the_shadows_buff", name = "killer_in_the_shadows_potion_increased", icon = "potion_killer_in_the_shadows", refresh_durations = true, max_stacks = 1, apply_buff_func = "apply_killer_in_the_shadows_buff",
	...
	pockets_full_of_bombs_potion = {
		buffs = { { update_func = "update_pockets_full_of_bombs_buff", name = "pockets_full_of_bombs_potion", remove_buff_func = "remove_deus_potion_buff", icon = "potion_pockets_full_of_bombs", apply_buff_func = "apply_pockets_full_of_bombs_buff",
				...
				perks = { buff_perks.disable_interactions, buff_perks.free_grenade, buff_perks.rewield_grenade_on_throw } },
			{ remove_buff_func = "remove_movement_buff", name = "pockets_full_of_bombs_potion_movement_speed", apply_buff_func = "apply_movement_buff",
	...
	pockets_full_of_bombs_potion_increased = {
		buffs = { { update_func = "update_pockets_full_of_bombs_buff", name = "pockets_full_of_bombs_potion_increased", remove_buff_func = "remove_deus_potion_buff", icon = "potion_pockets_full_of_bombs", apply_buff_func = "apply_pockets_full_of_bombs_buff",
				...
				perks = { buff_perks.disable_interactions, buff_perks.free_grenade, buff_perks.rewield_grenade_on_throw } },
			{ name = "pockets_full_of_bombs_potion_movement_speed_increased", remove_buff_func = "remove_movement_buff", apply_buff_func = "apply_movement_buff",
	...
	hold_my_beer_potion = {
		buffs = { { activation_effect = "fx/screenspace_drink_01", name = "hold_my_beer_potion", icon = "potion_hold_my_beer", continuous_effect = "fx/screenspace_drink_looping", max_stacks = 1, remove_buff_func = "remove_deus_potion_buff", refresh_durations = true,
			...
			{ remove_buff_func = "remove_movement_buff", name = "hold_my_beer_potion_movement_speed", max_stacks = 1, apply_buff_func = "apply_movement_buff", refresh_durations = true,
			...
			{ name = "hold_my_beer_potion_damage_increase", stat_buff = "increased_weapon_damage", refresh_durations = true, max_stacks = 1,
	...
	hold_my_beer_potion_increased = {
		buffs = { { activation_effect = "fx/screenspace_drink_01", name = "hold_my_beer_potion_increased", icon = "potion_hold_my_beer", continuous_effect = "fx/screenspace_drink_looping", max_stacks = 1, remove_buff_func = "remove_deus_potion_buff", refresh_durations = true,
			...
			{ remove_buff_func = "remove_movement_buff", name = "hold_my_beer_potion_movement_speed_increased", max_stacks = 1, apply_buff_func = "apply_movement_buff", refresh_durations = true,
			...
			{ name = "hold_my_beer_potion_damage_increase_increased", stat_buff = "increased_weapon_damage", refresh_durations = true, max_stacks = 1,
	...
	poison_proof_potion = {
		buffs = { { name = "poison_proof_potion", remove_buff_func = "remove_deus_potion_buff", max_stacks = 1, icon = "potion_poison_proof", refresh_durations = true,
	...
	poison_proof_potion_increased = {
		buffs = { { name = "poison_proof_potion_increased ", remove_buff_func = "remove_deus_potion_buff", max_stacks = 1, icon = "potion_poison_proof", refresh_durations = true,
	...
	mark_of_nurgle = {
		buffs = { { start_sound_event_name = "Play_curse_corrupted_flesh_loop", name = "mark_of_nurgle", mark_particle = "fx/deus_corrupted_flesh_01", buff_func = "remove_mark_of_nurgle", event = "on_death", remove_buff_func = "remove_mark_of_nurgle", apply_buff_func = "apply_mark_of_nurgle", stop_sound_event_name = "Stop_curse_corrupted_flesh_loop" },
			...
			{ event = "on_damage_dealt", name = "mark_of_nurgle_dot_attack", buff_func = "apply_mark_of_nurgle_dot" },




			{ name = "mark_of_nurgle_death_explosion", radius = 5, cloud_life_time = 4, buff_func = "mark_of_nurgle_explosion", event = "on_death", initial_radius = 1, aoe_dot_damage_interval = 1,
				...
				aoe_init_difficulty_damage = { 5, 5, 5, 5, 5 },
				aoe_dot_difficulty_damage = { 10, 10, 10, 10, 10 } },
			{ remove_on_proc = true, name = "mark_of_nurgle_pingable", buff_func = "curse_khorne_champions_leader_death", event = "on_death", remove_buff_func = "remove_make_pingable", apply_buff_func = "apply_make_pingable" } } },
	...
	curse_mark_of_nurgle_dot = {
		buffs = { { duration = 3, name = "curse_mark_of_nurgle_dot", damage_profile = "curse_mark_of_nurgle_dot", refresh_durations = true, apply_buff_func = "start_dot_damage", update_start_delay = 1, time_between_dot_damages = 1, max_stacks = 1, update_func = "apply_dot_damage" } } },
	...
	curse_khorne_champions_aoe = {
		buffs = { { particle_fx = "fx/deus_curse_khorne_champions_leader", name = "curse_khorne_champions_leader", buff_func = "curse_khorne_champions_leader_death", event = "on_death", remove_buff_func = "remove_curse_khorne_champions_aoe", apply_buff_func = "apply_curse_khorne_champions_aoe", remove_on_proc = true, update_func = "update_curse_khorne_champions_aoe", in_range_units_buff_name = "curse_khorne_champions_buff",
				...
				range_check = { unit_left_range_func = "unit_left_range_champions_aoe", radius = 8, update_rate = 1, unit_entered_range_func = "unit_entered_range_champions_aoe" } },
			...
			{ remove_on_proc = true, name = "curse_khorne_champions_aoe_pingable", buff_func = "curse_khorne_champions_leader_death", event = "on_death", remove_buff_func = "remove_make_pingable", apply_buff_func = "apply_make_pingable" },
			...
			{ unit_name = "units/props/deus_bloodgod_curse/deus_bloodgod_curse_01", name = "curse_khorne_champions_unit", buff_func = "remove_linked_unit", event = "on_death", remove_buff_func = "remove_linked_unit", apply_buff_func = "curse_khorne_champions_unit_link_unit",
				...
				z_offset = { default = 2, chaos_raider = 2, beastmen_bestigor = 1.9, chaos_warrior = 2.4, skaven_storm_vermin_commander = 1.9, skaven_storm_vermin = 1.9, skaven_storm_vermin_with_shield = 1.9, skaven_storm_vermin_champion = 1.9 } } } },
	...
	curse_khorne_champions_buff = {
		buffs = { { remove_buff_func = "remove_max_health_buff_for_ai", name = "curse_khorne_champions_max_health", apply_buff_func = "apply_max_health_buff_for_ai", multiplier = 1 },





			{ remove_buff_func = "remove_attach_particle", name = "curse_khorne_champions_particle", apply_buff_func = "apply_attach_particle", particle_fx = "fx/deus_curse_khorne_champions_buff" } } },
	...
	curse_skulls_of_fury = {
		buffs = { { name = "curse_skulls_of_fury", apply_buff_func = "trigger_skulls_of_fury_sound_event", sound_event_name = "Play_curse_skulls_of_fury_activated" },




			{ decal = "units/decals/deus_decal_bloodstorm_outer", name = "curse_skulls_of_fury_decal", decal_z_offset = -2, decal_scale = 5, remove_buff_func = "remove_generic_decal", apply_buff_func = "apply_generic_decal" } } },
	...
	curse_blood_storm_dot = {
		buffs = { { duration = 1, name = "curse_blood_storm_dot", max_stacks = 1, refresh_durations = true, apply_buff_func = "start_dot_damage", update_start_delay = 0.5, time_between_dot_damages = 0.5, damage_profile = "blood_storm", update_func = "apply_dot_damage", reapply_buff_func = "reapply_dot_damage",
	...
	curse_blood_storm_dot_bots = {
		buffs = { { duration = 1, name = "curse_blood_storm_dot", max_stacks = 1, refresh_durations = true, apply_buff_func = "start_dot_damage", update_start_delay = 0.5, time_between_dot_damages = 0.5, damage_profile = "blood_storm_bots", update_func = "apply_dot_damage", reapply_buff_func = "reapply_dot_damage",
	...
	curse_abundance_of_life = {
		buffs = { { damage_percentage = 0.01, name = "curse_abundance_of_life_dot", time_between_dot_damages = 2, custom_dot_tick_func = "curse_abundance_of_life_custom_dot_tick", update_func = "apply_dot_damage", apply_buff_func = "start_dot_damage", update_start_delay = 2 },
			...
			{ event = "on_potion_consumed", name = "curse_abundance_of_life_heal_on_potions", bonus = 100, buff_func = "all_potions_heal_func" },





			{ event = "on_potion_consumed", name = "curse_abundance_of_life_vo", dialogue_event = "curse_positive_effect_happened", buff_func = "trigger_dialogue_event" } } },
	...
	blessing_of_grimnir_boss_buff = {
		buffs = { { multiplier = 0.5, name = "blessing_of_grimnir_boss_health_buff", stat_buff = "max_health", remove_buff_func = "remove_max_health_buff_for_ai", apply_buff_func = "apply_max_health_buff_for_ai" },
			...
			{ multiplier = 0.5, name = "blessing_of_grimnir_boss_damage_buff", stat_buff = "damage_dealt" } } },
	...
	blessing_of_grimnir_player_buff = {
		buffs = { { name = "blessing_of_grimnir_player_buff", multiplier = 0.2, stat_buff = "max_health", is_persistent = true, icon = "bardin_ironbreaker_regen_stamina_on_block_broken" } } },
	...
	curse_rotten_miasma = {
		buffs = { { update_func = "update_curse_rotten_miasma", name = "curse_rotten_miasma", buff_exposure_tick_rate = 1.3, remove_buff_func = "remove_curse_rotten_miasma", apply_buff_func = "apply_curse_rotten_miasma", miasma_stack_limit = 35,
				...
				safe_area_radius = { 8, 8, 8, 8, 8 } } } },



	curse_rotten_miasma_debuff = {
		buffs = { { icon = "buff_icon_mutator_icon_slayer_curse", name = "curse_rotten_miasma_debuff", debuff = true,




				perks = { buff_perks.slayer_curse } },
			{ activation_effect = "fx/screenspace_deus_miasma", name = "curse_rotten_miasma_effect", continuous_effect = "fx/screenspace_deus_miasma", max_stacks = 1, remove_buff_func = "remove_curse_rotten_miasma_debuff", apply_buff_func = "apply_curse_rotten_miasma_debuff" } } },
	...
	curse_greed_pinata_drops = {
		buffs = { { name = "curse_greed_pinata_drops", buff_func = "curse_greed_pinata_death", event = "on_death", update_func = "update_curse_greed_pinata_drops", apply_buff_func = "apply_curse_greed_pinata_drops",
	...
	curse_greed_pinata_spawner = {
		buffs = { { particle_fx = "fx/deus_curse_khorne_champions_leader", name = "curse_greed_pinata_spawner", buff_func = "spawn_greed_pinata", event = "on_death", remove_buff_func = "remove_attach_particle", apply_buff_func = "apply_attach_particle" } } },
	...
	blessing_of_shallya_buff = {
		buffs = { { name = "blessing_of_shallya_buff",


				perks = { buff_perks.temp_to_permanent_health } } } },



	stockpile_refresh_ammo_buffs = {
		buffs = { { event = "on_inventory_post_apply_buffs", name = "stockpile_refresh_ammo_buffs", buff_func = "stockpile_refresh_ammo_buffs" } } },
	...
	deus_rally_flag_aoe_buff = {
		buffs = { { remove_buff_func = "remove_deus_rally_flag", name = "deus_rally_flag_lifetime", duration = 120 },




			{ name = "deus_rally_flag_aoe_buff", update_func = "update_generic_aoe", remove_buff_func = "remove_generic_aoe", apply_buff_func = "apply_generic_aoe", in_range_units_buff_name = "deus_rally_flag_buff",





				range_check = { radius = 5, update_rate = 0.01, only_players = true, unit_left_range_func = "unit_left_range_generic_buff", unit_entered_range_func = "unit_entered_range_generic_buff" } },
			...
			{ decal = "units/decals/decal_deus_rally_flag_01", name = "deus_rally_flag_aoe_decal", decal_scale = 5, remove_buff_func = "remove_generic_decal", apply_buff_func = "apply_generic_decal" } } },
	...
	deus_rally_flag_buff = {
		buffs = { { icon = "markus_questing_knight_buff_health_regen", name = "deus_rally_flag_health_buff", stat_buff = "max_health", multiplier = 0.2 },





			{ heal = 5, name = "deus_rally_flag_health_regen_buff", heal_type = "health_regen", time_between_heal = 1, update_func = "health_regen_update", apply_buff_func = "health_regen_start" } } },
	...
	blessing_of_isha_invincibility = {
		buffs = { { name = "blessing_of_isha_invincibility",


				perks = { buff_perks.ignore_death } } } },



	blessing_of_ranald_damage_taken = {
		buffs = { { multiplier = 0.2, name = "blessing_of_ranald_damage_taken", stat_buff = "damage_taken" } } },
	...
	blessing_of_ranald_coins_greed = {
		buffs = { { multiplier = 0.5, name = "blessing_of_ranald_coins_greed", stat_buff = "deus_coins_greed" } } },
	...
	objective_unit = {
		buffs = { { name = "objective_unit", buff_func = "remove_objective_unit", event = "on_death", remove_buff_func = "remove_objective_unit", apply_buff_func = "apply_objective_unit" } } },
	...
	cursed_chest_objective_unit = {
		buffs = { { apply_buff_func = "apply_cursed_chest_init", name = "cursed_chest_init" },



			{ name = "cursed_chest_objective_unit", buff_func = "remove_objective_unit", event = "on_death", remove_buff_func = "remove_objective_unit", apply_buff_func = "apply_objective_unit" } } },
	...
	curse_empathy_shared_health_pool = {
		buffs = { { name = "shared_health_pool",
				...
				perks = { buff_perks.shared_health_pool_damage_only } } } },



	we_deus_01_kerillian_critical_bleed_dot_disable = {
		buffs = { { name = "we_deus_01_kerillian_critical_bleed_dot_disable",
	...
	wh_deus_01_victor_witchhunter_bleed_on_critical_hit_disable = {
		buffs = { { name = "wh_deus_01_victor_witchhunter_bleed_on_critical_hit_disable",


				perks = { buff_perks.victor_witchhunter_bleed_on_critical_hit_disable } } } },



	we_deus_01_dot = {
		buffs = { { duration = 2, name = "we_deus_01_dot", apply_buff_func = "start_dot_damage", update_start_delay = 0.75, time_between_dot_damages = 0.75, damage_type = "burninating", damage_profile = "we_deus_01_dot", update_func = "apply_dot_damage",
				...
				perks = { buff_perks.burning_elven_magic } } } },




	we_deus_01_dot_fast = {
		buffs = { { name = "we_deus_01_dot_fast", ticks = 2, apply_buff_func = "start_dot_damage", update_start_delay = 0.75, time_between_dot_damages = 0.75, damage_type = "burninating", damage_profile = "we_deus_01_dot", update_func = "apply_dot_damage",
	...
	we_deus_01_dot_special_charged = {
		buffs = { { name = "we_deus_01_dot_special_charged", ticks = 4, apply_buff_func = "start_dot_damage", update_start_delay = 0.75, time_between_dot_damages = 0.75, damage_type = "burninating", damage_profile = "we_deus_01_dot", update_func = "apply_dot_damage",
	...
	we_deus_01_dot_charged = {
		buffs = { { name = "we_deus_01_dot_charged", ticks = 6, apply_buff_func = "start_dot_damage", update_start_delay = 0.75, time_between_dot_damages = 0.75, damage_type = "burninating", damage_profile = "we_deus_01_dot", update_func = "apply_dot_damage",
	...
	health_bar = {
		buffs = { { name = "health_bar", buff_func = "remove_health_bar", event = "on_death", remove_buff_func = "remove_health_bar", apply_buff_func = "apply_health_bar" } } },
	...
	deus_difficulty_tweak_boss_buff = {
		buffs = { { name = "deus_difficulty_tweak_boss_buff", apply_buff_func = "apply_max_health_buff_for_ai", remove_buff_func = "remove_max_health_buff_for_ai",




				variable_multiplier = { -0.25, 0.25 } } } },



	ledge_rescue = {
		buffs = { { rescue_delay = 0.5, name = "ledge_rescue", buff_func = "start_ledge_rescue_timer", event = "on_ledge_hang_start", update_func = "update_ledge_rescue", pull_up_duration = 1,
	...
	disable_rescue = {
		buffs = { { name = "disable_rescue", rescue_delay = 0.5, buff_func = "start_disable_rescue_timer", event = "on_player_disabled", update_func = "update_disable_rescue", explosion_template = "player_disabled_stagger",
	...
	melee_wave_buff = {
		buffs = { { max_stacks = 3, name = "melee_wave_buff", icon = "deus_icon_melee_wave" } } },
	...
	speed_over_stamina_buff = {
		buffs = { { name = "speed_over_stamina", stat_buff = "attack_speed", refresh_durations = true, max_stacks = 1, icon = "deus_icon_speed_over_stamina",
	...
	missing_health_power_up_buff = {
		buffs = { { name = "missing_health_power_up_buff", stat_buff = "damage_taken", icon = "deus_icon_missing_health_power_up",
	...
	detect_weakness_marked_enemy = {
		buffs = { { unit_name = "units/props/blk/blk_kill_the_marked", name = "detect_weakness_marked_enemy", buff_func = "remove_linked_unit", event = "on_death", remove_buff_func = "remove_linked_unit", apply_buff_func = "detect_weakness_link_unit",
				...
				z_offset = { default = 2.2, chaos_raider = 2.2, skaven_storm_vermin_with_shield = 2.1, beastmen_bestigor = 2.2, chaos_berzerker = 2.2, skaven_clan_rat_with_shield = 2, chaos_marauder = 2.2, skaven_plague_monk = 2.1, chaos_marauder_with_shield = 2.2, chaos_fanatic = 2.2, skaven_slave = 1.9, skaven_clan_rat = 2, beastmen_ungor = 2.2, chaos_warrior = 2.6, skaven_storm_vermin_commander = 2.1, skaven_storm_vermin = 2.1, beastmen_gor = 2.2, skaven_storm_vermin_champion = 2.1 } } } },
	...
	detect_weakness_buff = {
		buffs = { { name = "detect_weakness_buff", stat_buff = "power_level", refresh_durations = true, max_stacks = 1, priority_buff = true, icon = "deus_icon_kill_the_marked",
	...
	squats_build_up_buff = {
		buffs = { { name = "squats_build_up_buff", refresh_durations = true,
	...
	squats_buff = {
		buffs = { { name = "squats_buff", stat_buff = "power_level", icon = "deus_icon_squats", max_stacks = 1, priority_buff = true,
	...
	guaranteed_crit_buff = {
		buffs = { { max_stacks = 1, name = "guaranteed_crit_buff", buff_func = "dummy_function", event = "on_critical_action", icon = "bardin_ranger_linesman_unbalance", remove_on_proc = true,
	...
	follow_up_guaranteed_crit_buff = {
		buffs = { { max_stacks = 1, name = "follow_up_guaranteed_crit_buff", buff_func = "dummy_function", event = "on_critical_action", icon = "deus_icon_buff_follow_up", priority_buff = true, remove_on_proc = true,
	...
	wolfpack_buff = {
		buffs = { { name = "wolfpack_buff", stat_buff = "power_level", max_stacks = 4, icon = "deus_icon_wolfpack",
	...
	comradery_buff = {
		buffs = { { name = "comradery_buff", stat_buff = "power_level_melee", max_stacks = 4, icon = "deus_icon_comradery",
	...
	invigorating_strike_cooldown = {
		buffs = { { icon = "deus_icon_invigorating_strike", name = "invigorating_strike_cooldown", max_stacks = 1, is_cooldown = true,
	...
	staggering_force_buff = {
		buffs = { { name = "staggering_force_buff", stat_buff = "power_level", refresh_durations = true, max_stacks = 1, icon = "deus_icon_staggering_force",
	...
	crescendo_strike_buff = {
		buffs = { { refresh_durations = true, name = "crescendo_strike_buff", stat_buff = "critical_strike_chance", icon = "deus_icon_buff_crescendo_strike",
	...
	lucky_buff = {
		buffs = { { name = "lucky_buff", stat_buff = "critical_strike_chance", max_stacks = 20, icon = "deus_icon_lucky",
	...
	hidden_escape_buff = {
		buffs = { { apply_buff_func = "hidden_escape_apply", name = "hidden_escape_buff", icon = "deus_icon_hidden_escape", remove_buff_func = "hidden_escape_remove", cooldown_buff = "hidden_escape_cooldown_buff",
			...
			{ event = "on_hit", name = "hidden_escape_on_hit", buff_func = "hidden_escape_on_hit" } } },
	...
	hidden_escape_cooldown_buff = {
		buffs = { { is_cooldown = true, name = "hidden_escape_cooldown_buff", icon = "deus_icon_hidden_escape",
	...
	curative_empowerment_buff = {
		buffs = { { name = "curative_empowerment_buff", stat_buff = "power_level", refresh_durations = true, max_stacks = 5, icon = "deus_icon_curative_empowerment",
	...
	pent_up_anger_buff = {
		buffs = { { reset_on_max_stacks = true, name = "pent_up_anger_buff", on_max_stacks_overflow_func = "add_remove_buffs", on_max_stacks_func = "add_remove_buffs", icon = "deus_icon_pent_up_anger",


				max_stacks = MorrisBuffTweakData.pent_up_anger_buff.max_stacks,
				max_stack_data = {
					buffs_to_add = { "pent_up_anger_guaranteed_crit_buff" } } } } },
	...
	pent_up_anger_guaranteed_crit_buff = {
		buffs = { { max_stacks = 1, name = "pent_up_anger_guaranteed_crit_buff", buff_func = "dummy_function", event = "on_critical_action", icon = "deus_icon_pent_up_anger", priority_buff = true, remove_on_proc = true,
	...
	surprise_strike_guaranteed_crit_buff = {
		buffs = { { icon = "deus_icon_surprise_strike", name = "surprise_strike_guaranteed_crit_buff",
	...
	bad_breath_cooldown_buff = {
		buffs = { { is_cooldown = true, name = "bad_breath_cooldown_buff", icon = "deus_icon_bad_breath",
	...
	boulder_bro_buff = {
		buffs = { { buff_to_add = "boulder_bro_cooldown_buff", rescue_delay = 0.5, name = "boulder_bro_perk", buff_func = "start_boulder_bro_timer", event = "on_ledge_hang_start", remove_buff_func = "boulder_bro_add_buff", update_func = "update_boulder_bro", pull_up_duration = 1,
	...
	boulder_bro_cooldown_buff = {
		buffs = { { buff_to_add = "boulder_bro_buff", name = "boulder_bro_cooldown_buff", icon = "deus_icon_boulder_bro", is_cooldown = true, remove_buff_func = "boulder_bro_add_buff",
	...
	static_blade_cooldown_buff = {
		buffs = { { is_cooldown = true, name = "static_blade_cooldown_buff", icon = "deus_icon_static_blade",
	...
	home_run = {
		buffs = { { multiplier = 10, name = "home_run", stat_buff = "hit_force" },




			{ multiplier = 0.5, name = "home_run_hit_mass_reduction", stat_buff = "applied_stagger_distance" },




			{ multiplier = 0.4, name = "home_run_impact", stat_buff = "power_level_impact" },




			{ sound_event = "boon_homerun", name = "home_run_sound", cooldown = 0.25, buff_func = "home_run_sound", event = "on_body_pushed" } } },
	...
	shield_splinters = {
		buffs = { { event = "on_broke_shield", name = "shield_splinters", explosion_template = "shield_splinters", buff_func = "shield_splinters_explosion" } } },
	...
	refilling_shot = { create_parent_buff_shared_table = true,

		buffs = { { event = "on_start_action", name = "refilling_shot", buff_func = "refilling_shot_on_start_action" },




			{ event = "on_ammo_used", name = "refilling_shot_on_ammo_used", buff_func = "refilling_shot_on_ammo_used" },




			{ event = "on_critical_hit", name = "refilling_shot_critical_hit_ammo_reload", buff_func = "refilling_shot_on_critical_hit" } } },
	...
	piercing_projectiles = {
		buffs = { { name = "piercing_projectiles", stat_buff = "ranged_additional_penetrations",
	...
	serrated_blade = {
		buffs = { { name = "serrated_blade",


				perks = { buff_perks.generic_melee_bleed } } } },



	crescendo_strike = {
		buffs = { { event = "on_critical_hit", name = "crescendo_strike", buff_to_add = "crescendo_strike_buff", buff_func = "crescendo_strike_on_crit" } } },
	...
	follow_up = {
		buffs = { { name = "follow_up", blocker_buff = "follow_up_cooldown", buff_func = "add_buffs_on_melee_headshot", event = "on_hit",




				buffs_to_add = { "follow_up_guaranteed_crit_buff", "follow_up_cooldown" } } } },




	follow_up_cooldown = {
		buffs = { { name = "follow_up_cooldown",
	...
	deus_extra_shot = {
		buffs = { { name = "deus_extra_shot", stat_buff = "extra_shot",
	...
	always_blocking = {
		buffs = { { buff_to_add = "deus_always_blocking_buff", name = "always_blocking", update_func = "always_blocking_update", remove_buff_func = "always_blocking_remove", apply_buff_func = "always_blocking_init" },
			...
			{ event = "on_block_broken", name = "block_broken_remove_buff", buff_func = "always_blocking_temporarily_remove" } } },
	...
	deus_big_swing_stagger = {
		buffs = { { buff_to_add = "deus_big_swing_stagger_buff", name = "deus_big_swing_stagger", buff_func = "deus_big_swing_stagger_on_hit", event = "on_hit",
	...
	deus_always_blocking_buff = {
		buffs = { { remove_buff_func = "remove_always_blocking", name = "deus_always_blocking_buff", apply_buff_func = "apply_always_blocking" } } },
	...
	deus_always_blocking_lock_out = {
		buffs = { { refresh_durations = true, name = "deus_always_blocking_lock_out", icon = "deus_icon_always_blocking_01", debuff = true, max_stacks = 1, duration = 10 } } },
	...
	deus_big_swing_stagger_buff = {
		buffs = { { refresh_durations = true, name = "deus_big_swing_stagger_buff", stat_buff = "power_level_impact", icon = "deus_icon_big_swing_stagger", max_stacks = 1,
	...
	deus_ammo_pickup_reload_speed_buff = {
		buffs = { { name = "deus_ammo_pickup_reload_speed_buff", stat_buff = "reload_speed", refresh_durations = true, remove_buff_func = "remove_ammo_reload_speed_buff", apply_buff_func = "apply_ammo_reload_speed_buff", max_stacks = 1, icon = "deus_icon_ammo_pickup_reload_speed",
	...
	deus_ammo_pickup_reload_speed = {
		buffs = { { name = "deus_ammo_pickup_reload_speed", authority = "client", buff_func = "add_buff_on_pickup", event = "on_consumable_picked_up",




				pickup_types = {
					ammo = { "deus_ammo_pickup_reload_speed_buff" } } } } },
	...
	deus_crit_chain_lightning = {
		buffs = { { sound_event = "morris_power_ups_lightning_strike", name = "deus_crit_chain_lightning", authority = "server", buff_func = "chain_lightning", event = "on_player_damage_dealt", damage_profile = "beam_shot", particle_name = "", damage_source = "buff",
	...
	deus_ranged_crit_explosion = {
		buffs = { { sound_event = "morris_power_ups_ammo_explosion", name = "deus_ranged_crit_explosion", authority = "server", buff_func = "deus_ranged_crit_explosion_on_damage_dealt", event = "on_hit", cooldown_buff = "deus_ranged_crit_explosion_cooldown", explosion_template = "deus_ranged_crit_explosion",



				valid_attack_types = { instant_projectile = true, heavy_instant_projectile = true, projectile = true },
	...
	deus_ranged_crit_explosion_cooldown = {
		buffs = { { name = "deus_ranged_crit_explosion_cooldown", icon = "deus_ranged_crit_explosion",
	...
	deus_collateral_damage_on_melee_killing_blow = {
		buffs = { { name = "deus_collateral_damage_on_melee_killing_blow", authority = "server", buff_func = "deus_collateral_damage_on_melee_killing_blow_func", event = "on_kill", sound_event = "morris_power_ups_extra_damage",
	...
	health_orb = {
		buffs = { { name = "health_orb", apply_buff_func = "health_orb_apply_func", duration = 0,
	...
	static_charge = {
		buffs = { { activation_effect = "fx/screenspace_potion_01", name = "static_charge", update_func = "update_static_charge", refresh_durations = true, remove_buff_func = "remove_static_charge", apply_buff_func = "start_static_charge", explosion_template = "static_charge", icon = "twitch_icon_heavens_lightning", tick_every_t = 1,
	...
	protection_orb = {
		buffs = { { name = "protection_orb", stat_buff = "damage_taken", icon = "deus_icon_protection", max_stacks = 1,
	...
	focused_accuracy_cooldown = {
		buffs = { { name = "focused_accuracy_cooldown", debuff = true, is_cooldown = true, icon = "deus_icon_focussed_accuracy",
	...
	ability_cooldown_reduction_orb = {
		buffs = { { name = "ability_cooldown_reduction_orb", stat_buff = "cooldown_regen", refresh_durations = true, max_stacks = 1, icon = "deus_icon_focussed_accuracy",
	...
	resolve_cooldown_buff = {
		buffs = { { is_cooldown = true, name = "resolve_cooldown_buff", icon = "deus_icon_resolve",
	...
	resolve_buff = {
		buffs = { { name = "resolve_buff",


				perks = { buff_perks.full_health_revive } } } } }
