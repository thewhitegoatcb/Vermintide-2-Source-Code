...
local PlayFabHttps_LuaSec = {
	...
	MakePlayFabApiCall = function (urlPath, request, authKey, authValue, onSuccess, onError)
		...
		local requestHeaders = { ["X-ReportErrorAsSuccess"] = "true", ["Content-Type"] = "application/json",
		...
		local body, code, headers, status = https.request({ method = "POST",
		...
		if code == 200 then
			...
			elseif response and onError then
				onError(response)
			elseif onError then
				onError({ errorCode = 1123, error = "ServiceUnavailable",
					code = code,
					status = status,
