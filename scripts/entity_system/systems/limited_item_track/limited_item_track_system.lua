...
local extensions = { "LimitedItemTrackSpawner", "HeldLimitedItemExtension", "LimitedItemExtension", "WeaveLimitedItemTrackSpawner" }
...
function LimitedItemTrackSystem:init(entity_system_creation_context, system_name)
	...
	self.marked_items = { }
	function self.mark_item_for_transformation(extension)
		local unit = extension.unit
		self.marked_items [unit] = extension.id
	end

	function self.enable_spawner(extension)
		local unit = extension.unit
	...
	function self.disable_spawner(extension)
		local unit = extension.unit
...
function LimitedItemTrackSystem:register_group(group_name, pool_size)
...
function LimitedItemTrackSystem:register_weave_group(group_name, pool_size)
	fassert(self.groups [group_name] == nil, "Limited Item Group with name %q, is already registered", group_name)
...
function LimitedItemTrackSystem:decrease_group_pool_size(group_name)
...
function LimitedItemTrackSystem:activate_group(group_name, pool_size)
...
function LimitedItemTrackSystem:weave_activate_spawner(unit, group_name)
...
function LimitedItemTrackSystem:deactivate_group(group_name)
	local active_groups = self.active_groups
	local active_groups_n = self.active_groups_n
	for i = 1, active_groups_n do
		local active_group_name = active_groups [i]
...
function LimitedItemTrackSystem:find_active_spawner_id(unit)
...
function LimitedItemTrackSystem:destroy()
...
function LimitedItemTrackSystem:on_add_extension(world, unit, extension_name, extension_init_data)
	if next(extension_init_data) == nil then extension_init_data = temp_extension_init_data or extension_init_data end
...
function LimitedItemTrackSystem:on_remove_extension(unit, extension_name)
...
function LimitedItemTrackSystem:spawn_batch(group)
	local spawners = group.spawners
...
function LimitedItemTrackSystem:update(context, t)
	local active_groups_n = self.active_groups_n
...
function LimitedItemTrackSystem:held_limited_item_destroyed(spawner_unit, id)
