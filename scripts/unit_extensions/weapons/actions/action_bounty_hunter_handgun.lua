...
function ActionBountyHunterHandgun:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionBountyHunterHandgun:client_owner_start_action(new_action, t, chain_action_data, power_level, action_init_data)
...
function ActionBountyHunterHandgun:client_owner_post_update(dt, t, world, can_damage, time_in_action)
	if not self.upper_shot_done and
	self.time_to_shoot_upper <= t then
		self:upper_shoot_function()
		self.upper_shot_done = true
	end


	if not self.lower_shot_done and
	self.time_to_shoot_lower <= t then
		self:lower_shoot_function()
		self.lower_shot_done = true
	end


	if not self.aoe_done and
	self.time_to_aoe <= t then
		self:_do_aoe()
		self.aoe_done = true
	end
...
function ActionBountyHunterHandgun:_railgun_shoot()
	local owner_unit = self.owner_unit
	...
	self.is_critical_strike,
	self.power_level)

	first_person_extension:reset_aim_assist_multiplier()
end

function ActionBountyHunterHandgun:_shotgun_shoot()
...
function ActionBountyHunterHandgun:_do_aoe()
	local world = self.world
	local owner_unit = self.owner_unit
	local current_action = self.current_action
	...
	for i = 1, actors_n do repeat
			...
			if breed and not hit_units [hit_unit] and not breed.is_player then
				...
				"can_stagger", can_stagger) end until true
	end

end

function ActionBountyHunterHandgun:finish(reason)
	local current_action = self.current_action
