...
local possible_grudge_marks = {
	chaos_warrior = { chance = 0.15, base_grudgemark_name = "elite_base",

		names = { "shockwave", "ignore_death_aura" } } }
...
local mutator = { description = "description_mutator_geheimnisnacht_2021_hard_mode", display_name = "display_name_mutator_geheimnisnacht_2021_hard_mode", icon = "mutator_icon_geheimnisnacht_2021_difficulty" }





function mutator.server_ai_spawned_function(context, data, spawned_unit)
	data.enemies_to_be_buffed [#data.enemies_to_be_buffed + 1] = spawned_unit
end

function mutator.server_stop_function(context, data, is_destroy)
...
function mutator.client_start_function(context, data)
	local pop_chat = true
...
function mutator.client_stop_function(context, data, is_destroy)
	if not is_destroy then
		local pop_chat = true
...
function mutator.server_start_function(context, data)
...
function mutator.server_update_function(context, data, dt, t)
...
function mutator.post_ai_spawned_function(mutator_context, mutator_data, breed, optional_data)
