...
function CraftPageRollTrait:on_enter(params, settings)
...
function CraftPageRollTrait:setup_recipe_requirements()
...
function CraftPageRollTrait:create_ui_elements(params)
...
function CraftPageRollTrait:on_exit(params)
...
function CraftPageRollTrait:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function CraftPageRollTrait:post_update(dt, t)

	return end

function CraftPageRollTrait:_update_animations(dt)
...
function CraftPageRollTrait:_is_button_pressed(widget)
...
function CraftPageRollTrait:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageRollTrait:_is_button_held(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageRollTrait:_handle_input(dt, t)
...
function CraftPageRollTrait:_handle_craft_input_progress(progress)
...
function CraftPageRollTrait:craft_result(result, error, reset_slots)
	if not error then
		self._craft_result = result
	end
end

function CraftPageRollTrait:reset()
	local item_grid = self._item_grid
	item_grid:clear_locked_items()
	item_grid:update_items_status()
end

function CraftPageRollTrait:on_craft_completed()
...
function CraftPageRollTrait:_update_craft_items()
	local super_parent = self.super_parent
	local item_grid = self._item_grid
...
function CraftPageRollTrait:_remove_craft_item(backend_id, slot_index, ignore_sound)
	local craft_items = self._craft_items
...
function CraftPageRollTrait:_add_craft_item(backend_id, slot_index, ignore_sound)
	if self._num_craft_items == 0 then
...
function CraftPageRollTrait:_set_craft_button_disabled(disabled)
	self._widgets_by_name.craft_button.content.button_hotspot.disable_button = disabled
end

function CraftPageRollTrait:_exit(selected_level)
	self.exit = true
	self.exit_level_id = selected_level
end

function CraftPageRollTrait:draw(dt)
...
function CraftPageRollTrait:_play_sound(event)
	self.super_parent:play_sound(event)
end

function CraftPageRollTrait:_set_craft_button_text(text, localize)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.craft_button
