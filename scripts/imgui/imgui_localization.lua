...
local LOCALES = { "br-pt", "de", "en", "es", "fr", "it", "pl", "ru", "zh" }
...
function ImguiLocalization:init()
	self._text = ""
	self._cached_localizations = { }
	self._action_queue = { n = 0 }
end



function ImguiLocalization:update()
	local action = table.remove(self._action_queue)
	if action then action() end
end

function ImguiLocalization:action_push(thunk)
	local queue = self._action_queue
	queue.n = queue.n + 1
	table.insert(queue, 1, thunk)
end

function ImguiLocalization:draw()
...
function ImguiLocalization:is_persistent()
	return false
end