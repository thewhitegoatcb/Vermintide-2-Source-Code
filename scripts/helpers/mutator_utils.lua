...
local function modify_time_table(time_table, modifier, dprint_string)
	local tt_1 = time_table [1] local tt_2 = time_table [2]
...
function MutatorUtils.apply_buff_to_alive_player_units(context, data, buff_name)
	...
	for i = 1, num_current_player_units do
		...
		if player_units [unit] == nil and HEALTH_ALIVE [unit] then
			...
			local params = { attacker_unit = unit }
...
function MutatorUtils.store_breed_and_action_settings(context, data)
...
function MutatorUtils.restore_breed_and_action_settings(context, data)
...
function MutatorUtils.update_conflict_settings_horde_size_modifier(multipller)
...
function MutatorUtils.update_conflict_settings_horde_frequency(horde_frequency_modifier, horde_startup_time_modifier, relax_duration_modifier, max_delay_modifier)
	local pacing_settings = CurrentPacing
...
function MutatorUtils.tweak_pack_spawning_settings_convert_breeds(pack_spawning_settings, conversion_table)
...
function MutatorUtils.tweak_pack_spawning_settings_density_multiplier(pack_spawning_settings, density_multiplier)
...
function MutatorUtils.tweak_pack_spawning_settings_override_chance(pack_spawning_settings, min, max)
...
function MutatorUtils.update_conflict_settings_specials_frequency(max_specials_multiplier, spawn_time_reduction_multiplier)
