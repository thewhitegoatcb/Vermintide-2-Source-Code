...
function FatigueUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function FatigueUI:create_ui_elements()
...
function FatigueUI:destroy()
	return end

function FatigueUI:shield_state(shield_number, living_shields)
	local leftover = living_shields - shield_number
	if leftover >= 0 then
		do return "state_1" end
	elseif leftover == -0.5 then
		do return "state_2" end
	else
		return "state_3"
	end
end

function FatigueUI:setup_hud(status_extension)
...
function FatigueUI:start_fade_in()
...
function FatigueUI:start_fade_out()
	local active_shields = self.active_shields
	local shields = self.shields
	for i = 1, active_shields do
		local shield = shields [i]
...
local customizer_data = { root_scenegraph_id = "background", label = "Stamina", registry_key = "fatigue", drag_scenegraph_id = "background_dragger" }
...
function FatigueUI:check_active(status_extension)
	local active = status_extension:is_blocking() or status_extension.show_fatigue_gui

	return active
end

function FatigueUI:update_shields(status_extension, dt)
	...
	for i = 1, active_shields do
		...
		if current_state ~= new_state then
			...
			if state_animations and state_animations [new_state] then
				...
				UIWidget.animate(shield,
				UIAnimation.init(UIAnimation.picture_sequence, shield.content, "texture_id", animation_data.pictures,
				animation_data.time))
			else
