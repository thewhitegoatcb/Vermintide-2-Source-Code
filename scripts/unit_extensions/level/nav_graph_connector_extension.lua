...
function NavGraphConnectorExtension:init(extension_init_context, unit, extension_init_data)
	...
	if ledge_id and ledges [ledge_id] then
		...
		local drawer = Managers.state.debug:drawer({ mode = "retained", name = "NavGraphConnectorExtension" })
...
function NavGraphConnectorExtension:init_nav_graphs(ledge_id)
...
function NavGraphConnectorExtension:extensions_ready()

	return end

function NavGraphConnectorExtension:destroy()
...
function NavGraphConnectorExtension:update(unit, input, dt, context, t)
	return end