...
function UnlockManager:enable_update_unlocks(enable)
	self._update_unlocks = enable
end

function UnlockManager:_init_unlocks()
	local unlocks = { }
...
function UnlockManager:update(dt, t)
	if IS_XB1 then
...
function UnlockManager:_handle_popups()
...
function UnlockManager:_handle_popup_results(result)
...
function UnlockManager:_check_ps4_dlc_status()
...
function UnlockManager:_reinitialize_backend_dlc()
	self._state = "query_unlocked"
	self._query_unlocked_index = 0
end

function UnlockManager:_check_licenses()
...
function UnlockManager:dlc_status_changed()
	return self._dlc_status_changed
end

function UnlockManager:_update_console_backend_unlocks()
	...
	elseif self._state == "evaluate_restart" and
	table.is_empty(self._popup_ids) then
...
function UnlockManager:cb_reward_claimed(unlock, success, rewarded_items, presentation_text)
	if not success then
...
function UnlockManager:cb_reward_removed(unlock, success)
	if not success then
...
local type_sort_order = { ranged = 2, weapon_skin = 3, hat = 4, frame = 6, melee = 1, skin = 5, deed = 8, keep_decoration_painting = 7 }
...
function UnlockManager:_add_reward(items, presentation_text)
	...
	self._reward_queue [#self._reward_queue + 1] = { items = items, presentation_text = presentation_text }
end

function UnlockManager:poll_rewards()
	if #self._reward_queue <= self._reward_queue_id then
		return
	end
...
function UnlockManager:get_unlocked_dlcs()
...
function UnlockManager:get_installed_dlcs()
	local unlocks = self._unlocks
...
function UnlockManager:get_dlcs()
	return self._unlocks
end

function UnlockManager:get_dlc(name)
	return self._unlocks [name]
end

function UnlockManager:dlc_requires_restart(dlc_name)
	local unlock = self._unlocks [dlc_name]
	return unlock:requires_restart()
end

function UnlockManager:is_dlc_unlocked(name)
...
function UnlockManager:is_dlc_cosmetic(name)
	local unlock = self._unlocks [name]
	if not IS_WINDOWS and not IS_LINUX and not unlock then
		return true
	end
...
function UnlockManager:dlc_exists(name)
	return self._unlocks [name] ~= nil
end

function UnlockManager:dlc_id(name)
	local unlock = self._unlocks [name]
	fassert(unlock, "No such unlock %q", name or "nil")
...
function UnlockManager:dlc_name_from_id(id)
...
function UnlockManager:open_dlc_page(dlc_name)
...
function UnlockManager:ps4_dlc_product_label(name)
...
function UnlockManager:debug_add_console_dlc_reward(reward_id)
...
function UnlockManager:debug_remove_console_dlc_reward(reward_id)
	local unlock, index = nil
	for i, unlocks in ipairs(self._unlocks_indexed) do
		for _, u in pairs(unlocks) do
			local backend_reward_id = u:backend_reward_id()
...
function UnlockManager:_update_backend_unlocks(t)
...
function UnlockManager:_handle_unseen_rewards()
	local backend_manager = Managers.backend
	...
	for i = 1, #unseen_rewards do
		...
		elseif reward.reward_type == "keep_decoration_painting" then
			...
			item = {
				...
				data = { slot_type = "keep_decoration_painting", information_text = "information_text_painting", item_type = "keep_decoration_painting", matching_item_key = "keep_decoration_painting",
...
function UnlockManager:set_excluded_dlcs(dlcs_to_exclude)
...
function UnlockManager:_has_new_dlc()
