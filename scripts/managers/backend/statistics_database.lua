...
function StatisticsDatabase:init()
	self.statistics = { }
	self.categories = { }
	self.local_statistics = { }
end

function StatisticsDatabase:destroy()
...
local RPCS = { "rpc_sync_statistics_number", "rpc_increment_stat", "rpc_increment_stat_group", "rpc_set_local_player_stat", "rpc_modify_stat", "rpc_increment_stat_party" }
function StatisticsDatabase:register_network_event_delegate(network_event_delegate)
	self.network_event_delegate = network_event_delegate
	network_event_delegate:register(self, unpack(RPCS))
end

function StatisticsDatabase:unregister_network_event_delegate()
...
function StatisticsDatabase:register(id, category, backend_stats)
...
function StatisticsDatabase:hot_join_sync(peer_id)
	for stat_id, category in pairs(self.categories) do
		...
		elseif category == "session" then --[[ Nothing --]]
		end
	end
...
function StatisticsDatabase:reset_session_stats()
...
function StatisticsDatabase:generate_backend_stats(id, backend_stats)
	local stats = self.statistics [id]
...
function StatisticsDatabase:increment_stat(id, ...)
...
function StatisticsDatabase:decrement_stat(id, ...)
	local stat = self.statistics [id]
	local arg_n = select("#", ...)
	for i = 1, arg_n do
		local arg_value = select(i, ...)
...
function StatisticsDatabase:increment_stat_and_sync_to_clients(stat_name)
...
function StatisticsDatabase:modify_stat_by_amount(id, ...)
	local stat = self.statistics [id]
...
function StatisticsDatabase:get_array_stat(id, ...)
...
function StatisticsDatabase:get_persistent_array_stat(id, ...)
	local array_stat = self.statistics [id]
	local arg_n = select("#", ...)
	for i = 1, arg_n - 1 do
		local arg_value = select(i, ...)
...
function StatisticsDatabase:set_array_stat(id, ...)
	local array_stat = self.statistics [id]
	local arg_n = select("#", ...)
	for i = 1, arg_n - 2 do
		local arg_value = select(i, ...)
...
function StatisticsDatabase:set_stat(id, ...)
	local stat = self.statistics [id]
	local arg_n = select("#", ...)
	for i = 1, arg_n - 1 do
		local arg_value = select(i, ...)
...
function StatisticsDatabase:set_non_persistent_stat(id, ...)
	local stat = self.statistics [id]
	local arg_n = select("#", ...)
	for i = 1, arg_n - 1 do
		local arg_value = select(i, ...)
...
function StatisticsDatabase:get_stat(id, ...)
	local stat = self.statistics [id]
	local arg_n = select("#", ...)
	for i = 1, arg_n do
		local arg_value = select(i, ...)
...
function StatisticsDatabase:has_stat(id, ...)
	local stat = self.statistics [id]
	if not stat then
		return false
	end
...
function StatisticsDatabase:get_persistent_stat(id, ...)
	local stat = self.statistics [id]
	local arg_n = select("#", ...)
	for i = 1, arg_n do
		local arg_value = select(i, ...)
...
function StatisticsDatabase:sync_stats_to_server(stat_id, peer_id, local_player_id, network_transmit)
	local stats = self.statistics [stat_id]
...
function StatisticsDatabase:debug_draw()
...
function StatisticsDatabase:rpc_increment_stat(channel_id, stat_id)
...
function StatisticsDatabase:rpc_increment_stat_group(channel_id, group_id, stat_id)
...
function StatisticsDatabase:rpc_increment_stat_party(channel_id, stat_id)
...
function StatisticsDatabase:rpc_set_local_player_stat(channel_id, stat_id, amount)
	local stat = NetworkLookup.statistics [stat_id]
	local player = Managers.player:local_player()
	if not player then
		return
...
function StatisticsDatabase:rpc_sync_statistics_number(channel_id, peer_id, local_player_id, statistics_path_names, value, persistent_value)
...
function StatisticsDatabase:rpc_modify_stat(channel_id, stat_id, amount)
	local stat = NetworkLookup.statistics [stat_id]
	local player = Managers.player:local_player()
	if not player then
		return
...
function StatisticsDatabase:get_all_stats(id)
	return self.statistics [id]
end

function StatisticsDatabase:get_local_stat(stat_name)
	return self.local_statistics [stat_name]
end

function StatisticsDatabase:set_local_stat(stat_name, value)
	self.local_statistics [stat_name] = value
end

function StatisticsDatabase:increment_local_stat(stat_name)
...
function StatisticsDatabase:apply_persistant_stats()
...
function StatisticsDatabase:reset_persistant_stats()
...
if DB_UNIT_TEST then
	...
	local backend_stats = { kills_total = 10, lorebook_unlocks = "6F" }
	...
	script_data.statistics_debug = old_debug end