...
function AreaDamageExtension:init(extension_init_context, unit, extension_init_data)
...
function AreaDamageExtension:destroy()
	...
	area_damage.server.destroy then
...
function AreaDamageExtension:enable(enable)
	if enable then
		self.enabled = true
...
function AreaDamageExtension:start()
	self.area_damage_started = true
	local area_damage = AreaDamageTemplates.get_template(self.area_damage_template)


	if self.is_server and
	self.aoe_init_damage then
	...
	local particle_var_table = { { particle_variable = "pool_size",
...
function AreaDamageExtension:update(unit, input, dt, context, t)
...
function AreaDamageExtension:_update_damage_buffer()
...
function AreaDamageExtension:_add_to_damage_buffer(temp_damage_buffer)
	local damage_buffer = self._damage_buffer
...
function AreaDamageExtension:hot_join_sync(peer_id)
