...
function BTNinjaHighGroundAction:init(...)
	BTNinjaHighGroundAction.super.init(self, ...)
end

BTNinjaHighGroundAction.name = "BTNinjaHighGroundAction"

function BTNinjaHighGroundAction:enter(unit, blackboard, t)
	...
	if ALIVE [blackboard.target_unit] then
		...
		if smart_data.smart_object_type == "ledges_with_fence" and blackboard.breed.allow_fence_jumping then
			...
			print("fence jumping")
		elseif smart_data.smart_object_type == "ledges" and pos2.z < pos1.z and

		self:try_jump(unit, blackboard, t, pos1) then
...
function BTNinjaHighGroundAction:leave(unit, blackboard, t, reason, destroy)
...
function BTNinjaHighGroundAction:run(unit, blackboard, t, dt)

	if blackboard.high_ground_opportunity then
		do return "running" end
	else
		if blackboard.fence_jumping and
		blackboard.climb_state == "waiting_for_finished_climb_anim" and blackboard.jump_climb_finished then
...
function BTNinjaHighGroundAction:try_jump(unit, blackboard, t, pos1, force_idle)
