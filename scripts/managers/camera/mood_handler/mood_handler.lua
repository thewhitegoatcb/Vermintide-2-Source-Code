...
function MoodHandler:init(world)
	self.world = world
...
function MoodHandler:parse_environment_settings(environment)
		...
		if env_setting_name ~= "default" then
			...
			for var_name, var_value in pairs(env_setting.variables) do
				...
				if weight == 1 then
					...
					if var_type then
						variables [env_setting_name] [var_n] = { name = var_name, value = var_value }
...
function MoodHandler:set_mood(next_mood)
...
function MoodHandler:add_mood_blend(current_mood, next_mood)
	if Development.parameter("screen_space_player_camera_reactions") == false then
		return
	end
...
function MoodHandler:handle_particles(current_mood, next_mood)
	local playing_particles = self.playing_particles
	local world = self.world
...
function MoodHandler:update(dt)
...
function MoodHandler:update_mood_blends(dt)
...
function MoodHandler:set_mood_weights(dt, blend, mood_weights, weight)
	if blend then
...
function MoodHandler:update_environment_variables()
		...
		if mood_name ~= "default" then
			...
			for i = 1, #variables do
				...
				elseif var_type == "vector4" then
					value_to_set = value_to_set or { 0, 0, 0, 0 }
...
function MoodHandler:apply_environment_variables(shading_environment)
