...
function MatchmakingStateRequestJoinGame:init(params)
...
function MatchmakingStateRequestJoinGame:destroy()
...
function MatchmakingStateRequestJoinGame:on_enter(state_context)
...
function MatchmakingStateRequestJoinGame:on_exit()
	return end

function MatchmakingStateRequestJoinGame:_run_pre_connection_verification(join_lobby_data)
...
function MatchmakingStateRequestJoinGame:_setup_lobby_connection(join_lobby_data, password)
...
function MatchmakingStateRequestJoinGame:update(dt, t)
	...
	elseif state == "waiting_for_password" then
		...
		if self._password_request then
			...
			password = "" user_data = self._user_data action = "join"
		end
		if action ~= nil then
			if action == "join" then
...
function MatchmakingStateRequestJoinGame:_gather_dlc_ids()
	local unlocked_dlcs = { }
...
function MatchmakingStateRequestJoinGame:_join_game_success(t)
	...
	if join_method == "party" then
		do return MatchmakingStatePartyJoins, self.state_context end
	else
...
function MatchmakingStateRequestJoinGame:_join_fail_popup(fail_text)
...
function MatchmakingStateRequestJoinGame:_join_game_failed(reason, t, is_bad_connection, reason_variable, disable_chat_message)
...
function MatchmakingStateRequestJoinGame:rpc_matchmaking_request_join_lobby_reply(channel_id, reply_id, reply_variable)

	self._game_reply = NetworkLookup.game_ping_reply [reply_id]
	self._game_reply_variable = reply_variable
end

function MatchmakingStateRequestJoinGame:rpc_notify_connected(channel_id)
