...
StateLoading.LoadoutResyncStates = { NEEDS_RESYNC = "needs_resync", WAIT_FOR_LEVEL_LOAD = "wait_for_level_load", IDLE = "idle", DONE = "done", CHECK_RESYNC = "check_resync", RESYNCING = "resyncing" }
...
local LOADING_SCREEN_MUSIC_BY_GAME_MECHANISM = { survival = "Play_loading_screen_music", weave = "Play_loading_screen_music", quad = "Play_loading_screen_music", inn = "Play_loading_screen_music", inn_vs = "Play_loading_screen_music", demo = "Play_loading_screen_music", adventure = "Play_loading_screen_music", tutorial = "Play_loading_screen_music", versus = "Play_loading_screen_music", deus = "Play_loading_screen_music_morris" }
...
function StateLoading:on_enter(param_block)
	...
	if self._network_client then self._network_client.voip:set_input_manager(self._input_manager) end
	if self._network_server then self._network_server.voip:set_input_manager(self._input_manager)
	end
...
function StateLoading:set_loadout_resync_state(state)
...
function StateLoading:loadout_resync_state()
	return self._loadout_resync_state
end

function StateLoading:_setup_input()
...
function StateLoading:_parse_loading_context()
	local loading_context = self.parent.loading_context
...
function StateLoading:_setup_garbage_collection()
...
function StateLoading:_setup_world()
...
function StateLoading:_setup_init_network_view()
...
function StateLoading:_setup_end_of_level_ui()
...
function StateLoading:_setup_first_time_ui()
	local loading_context = self.parent.loading_context
	if ( loading_context.first_time or loading_context.gamma_correct or loading_context.play_trailer ) and
	not GameSettingsDevelopment.disable_intro_trailer and not script_data.skip_intro_trailer then
...
function StateLoading:_unmute_all_world_sounds()
	Managers.music:trigger_event("unmute_all_world_sounds")
end

function StateLoading:_get_game_difficulty()
	local level_transition_handler = Managers.level_transition_handler
	return level_transition_handler:get_current_difficulty()
end

function StateLoading:_create_loading_view(level_key, act_progression_index)
	if DEDICATED_SERVER then
		return
	end
...
function StateLoading:_trigger_loading_view(level_key, act_progression_index)
	level_key = level_key or Managers.mechanism:default_level_key()

	if not self._loading_music_triggered then
		if Managers.mechanism:game_mechanism():should_play_level_introduction() and
		not Development.parameter("gdc") then
...
function StateLoading:setup_loading_view(level_key)
	...
	if not DEDICATED_SERVER then
		...
		if level_settings.game_mode == "weave" then
			...
			if not weave_name or weave_name == "false" or not WeaveSettings.templates [weave_name] then
				if IS_XB1 and
				not self._lobby_client:is_updating_lobby_data() then
...
function StateLoading:loading_view_setup_done()
	return self._loading_view_setup_is_done
end

function StateLoading:setup_menu_assets()
...
function StateLoading:menu_assets_setup_done()
	return self._menu_setup_done
end

function StateLoading:cb_loading_screen_loaded(level_key, act_progression_index, skip_fade)
...
function StateLoading:cb_loading_screen_change_fade(level_key, act_progression_index, skip_fade)
	local game_difficulty = self:_get_game_difficulty()
	self._loading_view:texture_resource_loaded(level_key, act_progression_index, game_difficulty, self._loading_material_path, self._loading_material_name, self._weave_data)
...
function StateLoading:_trigger_sound_events(level_key)
	local level_settings = LevelSettings [level_key]
		...
		if profile_synchronizer then
			...
			for i = 1, #peers_with_full_profiles do
				...
				if loading_screen_wwise_events [career_name] then
					career_specific_events =
					career_specific_events or { }
...
function StateLoading:_trigger_weave_sound_events(weave_template, level_key, is_arena)
...
function StateLoading:_setup_state_machine()
	local params = {
		world = self._world,
...
function StateLoading:_handle_do_reload()
...
function StateLoading:set_invitation_error()
	self._has_invitation_error = true
end

function StateLoading:update(dt, t)
	...
	local level_transition_handler = Managers.level_transition_handler
	if IS_PS4 and not self._popup_id and not self._handled_psn_client_error and level_transition_handler:all_packages_loaded() and
	level_transition_handler.enemy_package_loader:loading_completed() and
	level_transition_handler.transient_package_loader:loading_completed() and
	Managers.backend:profiles_loaded() and self:global_packages_loaded() then
	...
	local menu_active = false local menu_input_service = nil
	if self._level_end_view_wrappers then
...
function StateLoading:_update_network(dt)
	...
	elseif self._network_client then
		...
		if self._network_client:is_in_post_game() and not self._in_post_game_popup_id and not self._in_post_game_popup_shown then
			...
			Managers.popup:activate_timer(self._in_post_game_popup_id, 200, "timeout", "center", false, function (timer) return string.format(Localize("timer_max_time") .. ": %.2d:%.2d", timer / 60 % 60, timer % 60) end, 28)
...
function StateLoading:_get_lost_connection_text_id()
	local text_id = nil
	if IS_WINDOWS or IS_LINUX then
...
function StateLoading:_update_lobbies(dt, t)
	...
	if IS_XB1 and self._waiting_for_cleanup and
	Managers.account:all_lobbies_freed() and
	self._cleanup_wait_time < t then
...
function StateLoading:_verify_joined_lobby(dt, t)
	if IS_XB1 and
	not self:_update_xbox_lobby_data(dt, t) then
		return
	end
	...
	if ready_to_compare_data then
		...
		if not script_data.unlock_all_levels and
		mechanism_settings.extra_requirements_function and not mechanism_settings.extra_requirements_function() then
		...
		if not Development.parameter("unlock_all_difficulties") and
		not mechanism_settings.disable_difficulty_check then
...
function StateLoading:_update_xbox_lobby_data(dt, t)
	...
	elseif
	state == "WAIT_FOR_LOBBY_UPDATE" and not self._lobby_client:is_updating_lobby_data() then
		state = "DONE"
	end
...
function StateLoading:lobby_verified()
	return self._lobby_verified
end

function StateLoading:_destroy_lobby_client()
...
function StateLoading:_destroy_lobby_host()
...
function StateLoading:_update_lobby_join(dt, t)
...
function StateLoading:_update_server_lobby_join(dt, t)
...
function StateLoading:_try_next_state(dt)
	...
	Managers.time:get_demo_transition() then
		self._teardown_network = true
...
function StateLoading:_handle_afk_timer(dt)
	if Managers.account:leaving_game() then
		return
	end
	...
	if Managers.account:has_popup() or self._popup_id then
		...
		if self._afk_timer < time and (
		not self._ui_package_name or self._ui_package_name and Managers.package:has_loaded(self._ui_package_name)) then
			if self._first_time_view then
				self._first_time_view:destroy()
				self._first_time_view = nil
			end
...
function StateLoading:_level_end_view_done()
	local level_end_view_wrapper = self._level_end_view_wrappers [1]
	return level_end_view_wrapper and level_end_view_wrapper:done()
end

function StateLoading:_handle_popup()
...
function StateLoading:_handle_join_popup()
...
function StateLoading:_handle_in_post_game_popup()
...
function StateLoading:_backend_broken()
...
function StateLoading:on_exit(application_shutdown)
	...
	if application_shutdown or self._teardown_network then
		...
		self._ingame_world_object = loading_context.ingame_world_object loading_context.ingame_world_object = self._ingame_world_object
		self._ingame_level_object = loading_context.ingame_level_object loading_context.ingame_level_object = self._ingame_level_object
		...
		if self._do_reload then
			...
			level_transition_handler:set_next_level(
			level_key,
	...
	local package_manager = Managers.package
	if self._ui_package_name and (package_manager:has_loaded(self._ui_package_name, "global_loading_screens") or package_manager:is_loading(self._ui_package_name)) and
	package_manager:can_unload(self._ui_package_name) then
		package_manager:unload(self._ui_package_name, "global_loading_screens")
	end
...
function StateLoading:global_packages_loaded()
	if not GlobalResources.loaded then
		self:_load_global_packages()
	end
...
function StateLoading:_packages_loaded()
	local level_transition_handler = Managers.level_transition_handler
	if level_transition_handler:all_packages_loaded() and Managers.backend:profiles_loaded() then
		...
		if self._ui_loading_package_path and self._ui_loading_package_reference_name and
		not package_manager:has_loaded(self._ui_loading_package_path, self._ui_loading_package_reference_name) then
			return false
		end
...
function StateLoading:load_current_level()
...
function StateLoading:_update_loadout_resync()
	...
	if state == states.WAIT_FOR_LEVEL_LOAD then --[[ Nothing --]]
	end


	if state == states.CHECK_RESYNC and
	self:has_joined() and Managers.mechanism:can_resync_loadout() then
		local level_transition_handler = Managers.level_transition_handler
		local level_key = level_transition_handler:get_current_level_key()
...
function StateLoading:_destroy_network(application_shutdown)
	PartyManager.reset()
	if Managers.matchmaking then
		Managers.matchmaking:destroy()
		Managers.matchmaking = nil
...
function StateLoading:_tear_down_level_end_view_wrappers()
	local level_end_view_wrappers = self._level_end_view_wrappers
...
function StateLoading:set_matchmaking(matchmaking)
	self._joined_matchmaking_lobby = matchmaking
end

function StateLoading:has_registered_rpcs()
	return self._registered_rpcs
end

function StateLoading:register_rpcs()
...
function StateLoading:_unregister_rpcs()
...
function StateLoading:waiting_for_cleanup()
	return self._waiting_for_cleanup
end

function StateLoading:setup_join_lobby(optional_wait_time, setup_voip)
	if IS_XB1 and (
	not Managers.account:all_lobbies_freed() or optional_wait_time) then
	...
	if setup_voip then
		local voip_params = { is_server = false,
...
function StateLoading:setup_lobby_finder(lobby_joined_callback, lobby_to_join, host_to_join, lobby_is_server)
...
function StateLoading:setup_lobby_host(wait_for_joined_callback, platform_lobby, xbox_lobby_session_name, xbox_session_template_name)
	if IS_XB1 and
	not Managers.account:all_lobbies_freed() then
		self._waiting_for_cleanup = true
...
function StateLoading:host_joined()
	self:_create_network_server()
...
function StateLoading:_create_network_server()
...
function StateLoading:setup_chat_manager(lobby, host_peer_id, my_peer_id, is_server)
	...
	local function member_func()
		if DEDICATED_SERVER and

		Managers.level_transition_handler:in_hub_level() then
			local mechanism = Managers.mechanism:game_mechanism()
...
function StateLoading:setup_deed_manager(lobby, host_peer_id, my_peer_id, is_server, network_handler)
	Managers.deed:network_context_created(lobby, host_peer_id, my_peer_id, is_server, network_handler)
end

function StateLoading:setup_enemy_package_loader(lobby, host_peer_id, my_peer_id)
...
function StateLoading:setup_global_managers(lobby, host_peer_id, my_peer_id, is_server, network_handler)
...
function StateLoading:setup_network_transmit(network_handler)
...
function StateLoading:create_popup(error, header, action, right_button, ...)
	if Managers.account:leaving_game() then
		return
	end
...
function StateLoading:create_join_popup(host_name)
	if Managers.account:leaving_game() then
		return
	end
...
function StateLoading:clear_network_loading_context()
	local loading_context = self.parent.loading_context
...
function StateLoading:setup_network_client(clear_peer_state, lobby_client)
...
function StateLoading:get_current_level_keys()
	return Managers.level_transition_handler:get_current_level_keys()
end

function StateLoading:set_lobby_host_data(level_key)
	if self._lobby_host then
		local lobby_host = self._lobby_host
...
function StateLoading:start_matchmaking()
...
function StateLoading:get_lobby()
	local lobby = self._lobby_host or self._lobby_client
	return lobby
end

function StateLoading:has_joined()
	local lobby = self._lobby_host or self._lobby_client
