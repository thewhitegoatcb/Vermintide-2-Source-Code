...
function SpawnZoneBaker:init(world, nav_world, level_analyzer, level_seed)
	self.world = world
...
function SpawnZoneBaker:_random(...)
	local seed, value = Math.next_random(self.seed, ...)
	self.seed = seed
	return value
end

function SpawnZoneBaker:_random_dice_roll(prob, alias)
	local seed, value = LoadedDice.roll_seeded(prob, alias, self.seed)
	self.seed = seed
	return value
end

function SpawnZoneBaker:set_seed(seed)
	fassert(seed and type(seed) == "number", "Bad seed input!")
	self.seed = seed
	self._initial_seed = seed
end

function SpawnZoneBaker:_random_interval(numbers)
...
local all = { }
function SpawnZoneBaker:_get_random_array_indices(size, num_picks)
...
function SpawnZoneBaker:loaded_spawn_zones_available()
	return self.spawn_zones_available
end

function SpawnZoneBaker:create_cover_points(cover_points, broad_phase)
...
function SpawnZoneBaker:periodical(hi, dist_data)
...
function SpawnZoneBaker:generate_spawns(spawn_cycle_length, goal_density, area_density_coefficient, length_density_coefficient, conflict_director_name, mutator_list)
	if not InterestPointUnitsLookup then
	...
	local spawns = { } local pack_sizes = { } local pack_rotations = { } local pack_members = { } local zone_data_list = { }
	...
	for i = 1, num_great_cycles do
		...
		local sum_density = 0 local num_hi = 0
		local zone = nil
		...
		local average_goal_density = 0 local pack_spawning_setting, goal_density = nil
		for j = 1, num_cycle_zones do
	...
	for i = num_main_zones + 1, #zones do
		...
		if parent_zone_index then local parent_zone = self.level_analyzer:get_zone_from_unique_id(zone_convert, parent_zone_index) end
		if parent_zone then
			...
			if zone.on_roof then slot35 = BreedPacks [pack_type].roof_spawning_allowed if BreedPacks [pack_type].roof_spawning_allowed then slot35 = false end else local ok_to_spawn = true end
			if ok_to_spawn then
				...
				for j = 1, #sub_zones do
					...
					local island_zone = { total_area = 0, nodes = nodes, area = area, outer = { }, pack_type = pack_type, pack_spawning_setting = pack_spawning_setting, conflict_setting = conflict_setting, unique_zone_id = zone.unique_zone_id,
						period_length = 1,
						hi = false,
						island = true,
						density = density,
...
function SpawnZoneBaker:inject_special_packs(total_peaks, cycle_zones)
		...
		if period_length and zone.hi then
			...
			if lookup [peak_index] then
				...
				if breed_packs_override then
					...
					local prob = loaded_dice [1] local alias = loaded_dice [2]
...
function SpawnZoneBaker:create_hi_data(zone, pack_type_name)
	local hi_data = nil
	local breed_packs = BreedPacks [pack_type_name]
	local zone_checks = breed_packs.zone_checks
	if zone_checks then
		self._count_up = self._count_up + 1
		hi_data = { id = self._count_up }
		zone.hi_data = hi_data

		if not zone.hi or not zone_checks.clamp_breeds_hi then local clamp_breeds = zone_checks.clamp_breeds_low
		end
		...
		if clamp_breeds then
			...
			if difficulty_overrides then
				...
				for i = 1, #difficulty_overrides do
					...
					breed_count [check_breed_name] = { switch_count = 0, count = 0, max_amount = max_amount, switch_breed = switch_breed, hi = zone.hi }
				end
...
function SpawnZoneBaker:populate_spawns_by_rats(global_pack_spawning_setting, spawns, pack_sizes, pack_rotations, pack_members, zone_data_list, zone_list, global_area_density_coefficient, global_length_density_coefficient, pack_type_override, is_main_zone, parent_zone)
	...
	for j = 1, num_zones do
		...
		if not zone.pack_spawning_setting then local pack_spawning_setting = global_pack_spawning_setting end
		local basics = pack_spawning_setting.basics
		if ( not is_main_zone or not zone.density_coefficient ) and not pack_spawning_setting.area_density_coefficient then local area_density_coefficient = global_area_density_coefficient end
		if not is_main_zone or not basics.length_density_coefficient then local length_density_coefficient = global_length_density_coefficient end
		if is_main_zone then local clamp_area = basics.clamp_main_path_zone_area end
		local nodes = zone.nodes

		if nodes then
			if not is_main_zone or zone.area >= clamp_area or not clamp_area then local area = zone.area end
			...
			if num_wanted_rats > 0 then
				local a, b, c = Script.temp_count()
				slot38 = self local num_spawns = self.spawn_amount_rats slot39 = spawns slot40 = pack_sizes slot41 = pack_rotations slot42 = pack_members slot43 = zone_data_list slot44 = nodes slot45 = num_wanted_rats if not pack_type_override then slot46 = zone.pack_type end slot47 = area if not parent_zone then slot48 = zone end local num_spawns = slot37(slot38, slot39, slot40, slot41, slot42, slot43, slot44, slot45, slot46, slot47, slot48)
...
function SpawnZoneBaker:_generate_pack_members(pack_type, pack_size, zone, ip_unit_name, center_position)
	local pack_type_by_size = BreedPacksBySize [pack_type]
	local pack_by_size = pack_type_by_size [pack_size]
	local prob = pack_by_size.prob local alias = pack_by_size.alias
	...
	if zone then local hi_data = zone.hi_data end
...
function SpawnZoneBaker:spawn_amount_rats(spawns, pack_sizes, pack_rotations, pack_members, zone_data_list, nodes, num_wanted_rats, pack_type, area, zone)
	...
	while num_wanted_rats > 0 do
		...
		if num_variants == 1 then slot25 = 1 else local ip_unit_index = self:_random(num_variants) end
...
function SpawnZoneBaker:get_zone_segment_from_travel_dist(travel_dist)
	local zones = self.zones
	...
	for i = 1, num_zones do
		...
		if travel_dist < zone_travel_dist then
			if i <= 1 or not (i - 1) then local zone_index = i end
...
function SpawnZoneBaker:draw_zones(nav_world, draw_only_one_zone_index)
	local show_spawn_density = false local show_zone_text = true
	if self.gui then
	...
	if not draw_only_one_zone_index then slot12 = 1 end local start = math.clamp(1, 1, #zones)
	for i = start, #zones do
		local zone = zones [i]
		local sub_zones = zone.sub
		local colors, density = nil
		if show_spawn_density then
			slot20 = math.floor if not zone_convert [i] or not zone_convert [i].density then local g = 1 end density = math.floor(g * 16)
		else
			local f = 92 + 63 * (i % 3)
			local g = f / 2
			colors = { Color(alpha, 0, f, 0), Color(alpha, 0, 0, f), Color(alpha, f, 0, 0),
				Color(alpha, f, f, 0), Color(alpha, 0, f, f), Color(alpha, f, 0, f),
				Color(alpha, f, g, 0), Color(alpha, f, 0, g), Color(alpha, 0, g, f),
				Color(alpha, g, 0, f) }
		end
		...
		if show_zone_text then
			...
			local text = string.format slot23 = "%d %.1f <- %.1f" local j = i if not zone.travel_dist then slot25 = 0 end if not zone.old_travel_dist then slot26 = 0 end local text = slot22("%d %.1f <- %.1f", j, 0, 0)
				...
				for k = 1, #sub_zone do
					...
					if triangle then
						...
						if show_spawn_density then
							...
							if not colors [j] then local col = Colors.get_indexed(( i * 7 + j + 5 ) % 32 + 1) end
...
function SpawnZoneBaker:show_debug(show)
	if show then
...
function SpawnZoneBaker:execute_debug()
...
function print_zone_list(cycle_zones)
	for j = 1, #cycle_zones do
		...
		if zone.hi_data then
			...
			slot8 = print slot9 = string.format("Zone: %d, hi: %s, hi-id: %d, Density: %.1f, Area: %.1f", j, tostring(zone.hi), hi_data_id, zone.density, area) slot10 = "con:" slot11 = zone.conflict_setting.name local k = "period_len:" if not zone.period_length then slot13 = "--" end slot14 = "data:" if zone.hi_data then slot15 = "Y" else slot15 = "N" end slot8(slot9, slot10, slot11, k, slot13, slot14, slot15, string.format("Director / Packtype: %s / %s ", zone.conflict_setting.name, zone.pack_type))
			...
			slot7 = print slot8 = string.format("Zone: %d, hi: %s, hi-id: --, Density: %.1f, Area: %.1f", j, tostring(zone.hi), zone.density, area) slot9 = "con:" slot10 = zone.conflict_setting.name slot11 = "period_len:" if not zone.period_length then slot12 = "--" end slot13 = "data:" if zone.hi_data then slot14 = "Y" else slot14 = "N" end slot7(slot8, slot9, slot10, slot11, slot12, slot13, slot14, string.format("Director / Packtype: %s / %s ", zone.conflict_setting.name, zone.pack_type))
		end
	end
end
...
function SpawnZoneBaker:debug_print_hi_data()
	...
	for i = 1, #great_cycles do
		...
		for j = 1, #cycle_zones do
			...
			if hi_data ~= old_hi_data then
				...
				if zone.hi then slot15 = "Hi" else local period = "Low" end
	...
	for i = 1, #self._all_hi_data do
		...
		if breed_count then
			for check_breed_name, data in pairs(breed_count) do
				slot14 = print slot15 = "Hidata id:" slot16 = hi_data.id if data.hi then slot17 = "HI" else slot17 = "LOW" end slot14(slot15, slot16, slot17, "count:", data.count, "switched:", data.switch_count, "max:", data.max_amount, "breed:", check_breed_name, "switched to:", data.switch_breed.name)
			end
		else
...
function SpawnZoneBaker:draw_pack_density_graph()
	if not self.graph then
		self.graph = Managers.state.debug.graph_drawer:create_graph("spawn density", { "distance", "density" })
	...
	if num_great_cycles > 0 then
		...
		for i = 1, num_great_cycles do
			...
			local cycle_zones = cycle.zones
			if i > 1 then
				self.graph:add_annotation({ text = "Cycle", live = true, y = 105, color = "green", x = dist })
			end
			...
			for j = 1, #cycle_zones do
				...
				if current_pack_type ~= zone.pack_type then
					self.graph:add_annotation({ text = "O", live = true, color = "lawn_green", x = dist, y = density * 100 })
				elseif zone.hi then
					self.graph:add_annotation({ text = "H", live = true, color = "lawn_green", x = dist, y = density * 100 })
				end
				...
				if current_conflict_setting ~= zone.conflict_setting then
					...
					self.graph:add_annotation({ live = true, y = 110, color = "orange", x = dist, text = current_conflict_setting.name })
				end
	...
	for k = 1, #rares do
		...
		self.graph:add_annotation({ live = true, x = path_dist, y = y, text = event_name, color = color })
		y = y + 7
		if y > 70 then
			y = 30
		end
	...
	local player_annotation = { text = "PLAYER", live = true, y = 50, color = "green", x = 0 }
	self.graph:add_annotation(player_annotation)
	self.player_annotation = player_annotation
end

function SpawnZoneBaker:draw_player_in_density_graph(travel_dist)
	if self.graph then
...
function SpawnZoneBaker:draw_func1(zone, zone_list_index, row, left, row_y)
	if zone.island then slot6 = "ISLAND" else local zone_type = "MAIN" end
	local row_text = string.format if not zone.pack_spawning_setting.name then slot12 = "?" end local row_text = slot7("%d %s: %d %s", row, zone_type, zone_list_index, "?")
	Gui.text(self._gui, row_text, "materials/fonts/arial", 14, "materials/fonts/arial", Vector3(left + 200, row_y, 1000))
end

function SpawnZoneBaker:_draw_zone(zone, color)
...
function SpawnZoneBaker:draw_func2(zone, zone_list_index, row, left, row_y, is_closest)
	...
	if ( not is_closest or not Color(200, 200, 0) ) and (not zone.hi or not Color(255, 255, 255)) then local color = Color(175, 175, 175)
	end
	...
	if zone.island then slot13 = "ISLAND" else local zone_type = "MAIN" end
...
function SpawnZoneBaker:_draw_legend(legend, x, y)
...
function SpawnZoneBaker:draw_zone_info_on_screen()
	...
	local alpha = 60
	local win_x1 = 640 local win_y1 = 40
	local win_x2 = w - 100 local win_y2 = h - 100
	...
	if zone_index <= mid_row then slot19 = 1 else local zone_list_index = zone_index - mid_row end
	local row_y = top - 40
	local i = 1
	while max_rows_shown > i and zone_list_index <= num_zones do
		local zone = plain_zone_list [zone_list_index]
		if zone then
			slot24 = self slot23 = self.draw_func2 slot25 = zone slot26 = zone_list_index slot27 = i slot28 = left slot29 = row_y if near_zone ~= zone then slot30 = false else slot30 = true end slot23(slot24, slot25, slot26, slot27, slot28, slot29, slot30)
			row_y = row_y - 20
			i = i + 1
		end
...
function SpawnZoneBaker:_debug_draw_baker_data(hi_data, data, breed_name, position)
	if data.max_amount < data.count then
		if not Colors.distinct_colors_lookup [hi_data.id] then local c = Colors.distinct_colors_lookup [1] end
