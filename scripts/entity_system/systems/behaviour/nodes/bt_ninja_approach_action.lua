...
function BTNinjaApproachAction:init(...)
...
function BTNinjaApproachAction:enter(unit, blackboard, t)
...
function BTNinjaApproachAction:leave(unit, blackboard, t, reason, destroy)
	if reason == "aborted" then --[[ Nothing --]]
	end
...
local close_range = 8
function BTNinjaApproachAction:run(unit, blackboard, t, dt)
	local locomotion = blackboard.locomotion_extension
...
local segment_list = { }
local relative_positions = { 0.4, 0.5, -0.4, 0.5, 0, 1.5 }
function BTNinjaApproachAction:check_free_los(unit, blackboard, data)
...
function BTNinjaApproachAction:try_dodge_pos(unit, blackboard, pos, dodge_pos)
...
function BTNinjaApproachAction:dodge(unit, blackboard, dodge_vec, aim_vec)
...
function BTNinjaApproachAction:in_crosshairs(unit, blackboard, t, data)
...
function BTNinjaApproachAction:get_fallback_goal(unit, blackboard)
...
function BTNinjaApproachAction:set_goal_at_target(unit, blackboard)
...
function BTNinjaApproachAction:get_straight_at_goal(unit, blackboard)
...
function BTNinjaApproachAction:check_high_point_on_line(nav_world, from_pos, dir, dist_left_to_rat, base_z, height_advantage, above, below)
	...
	for i = 2, dist_left_to_rat + 1 do
		...
		if success and height_advantage < z - base_z then
			...
			do return check_height_pos, i end
		else
...
function BTNinjaApproachAction:get_new_goal(unit, blackboard, t)
...
function BTNinjaApproachAction:anim_cb_dodge_finished(unit, params)
	blackboard.anim_cb_dodge_finished = nil
end


function BTNinjaApproachAction:debug(unit, blackboard)
	if blackboard.skulk_pos then
		local pos = blackboard.skulk_pos:unbox()
