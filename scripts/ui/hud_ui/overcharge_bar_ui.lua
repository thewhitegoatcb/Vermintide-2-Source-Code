...
local accepted_slots = { slot_ranged = true, slot_melee = true }




local DEFAULT_UI_DATA = { material = "overcharge_bar",

	color_normal = { 255, 255, 255, 255 },
	color_medium = { 255, 255, 165, 0 },
	color_high = { 255, 255, 0, 0 } }


local KEEP_AT_0_DURATION = 0.5

function OverchargeBarUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function OverchargeBarUI:create_ui_elements()
...
function OverchargeBarUI:update(dt, t, player)
...
function OverchargeBarUI:destroy()
	Managers.state.event:unregister("on_spectator_target_changed", self)
end

function OverchargeBarUI:set_alpha(alpha)
	self.render_settings.alpha_multiplier = alpha
end

function OverchargeBarUI:_apply_crosshair_position(x, y)
