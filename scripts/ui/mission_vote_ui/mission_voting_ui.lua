...
function MissionVotingUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function MissionVotingUI:create_ui_elements()
	self._ui_animations = { }
	...
	local twitch_widgets = { } local twitch_widgets_by_name = { }
...
function MissionVotingUI:destroy()
	if self.vote_started then
		self:on_vote_ended()
	end
end

function MissionVotingUI:get_chrome_widgets()
	if self._active_mechanism == "deus" then
		do return self._widgets_deus_by_name, self._widgets_deus end
	else
		return self._widgets_by_name, self._widgets
	end
end

function MissionVotingUI:is_active()
	return self.vote_started and not self.has_voted
end

function MissionVotingUI:setup_option_input(option_widget, option)
...
function MissionVotingUI:start_vote(active_voting)
...
function MissionVotingUI:_setup_gamepad_input_desc(vote_template)
...
function MissionVotingUI:_check_initial_votes()
...
function MissionVotingUI:on_vote_casted(voted_yes)
	self.has_voted = true
...
function MissionVotingUI:on_vote_ended()
...
function MissionVotingUI:_set_weave_quickplay_presentation(difficulty)
...
function MissionVotingUI:_set_adventure_presentation(difficulty)
	local difficulty_settings = DifficultySettings [difficulty]
	local difficulty_display_name = difficulty_settings.display_name
	local difficulty_display_image = difficulty_settings.display_image
	local difficulty_frame_texture = difficulty_settings.completed_frame_texture or "map_frame_00"
...
function MissionVotingUI:_set_game_mode_presentation(mechanism_key)
...
function MissionVotingUI:_set_switch_mechanism_presentation(vote_data)
...
function MissionVotingUI:_set_custom_game_presentation(difficulty, level_key, private_game, always_host, strict_matchmaking)
	local difficulty_settings = DifficultySettings [difficulty]
	local difficulty_display_name = difficulty_settings.display_name
	local difficulty_display_image = difficulty_settings.display_image
	local difficulty_frame_texture = difficulty_settings.completed_frame_texture or "map_frame_00"
...
function MissionVotingUI:_set_deed_presentation(item_key, level_key, difficulty)
	local item_data = ItemMasterList [item_key]
	local item = { data = item_data, difficulty = difficulty, level_key = level_key }
...
function MissionVotingUI:_set_event_game_presentation(difficulty, level_key, mutators)
	local difficulty_settings = DifficultySettings [difficulty]
	local difficulty_display_name = difficulty_settings.display_name
	local difficulty_display_image = difficulty_settings.display_image
	local difficulty_frame_texture = difficulty_settings.completed_frame_texture or "map_frame_00"
	...
	event_summary.content.item = { level_key = level_key, mutators = mutators }

	self._presentation_type = "event"
end

function MissionVotingUI:_set_weave_presentation(difficulty, mission_id, private_game)
...
function MissionVotingUI:_assign_objective(index, text, icon, spacing)
...
function MissionVotingUI:_set_deus_quickplay_presentation(difficulty)
	local difficulty_settings = DifficultySettings [difficulty]
	local difficulty_display_name = difficulty_settings.display_name
	local difficulty_display_image = difficulty_settings.display_image
	local difficulty_frame_texture = difficulty_settings.completed_frame_texture or "map_frame_00"
...
function MissionVotingUI:_set_deus_custom_game_presentation(difficulty, journey_name, private_game, always_host, strict_matchmaking, theme)
...
function MissionVotingUI:_update_vote_timer()
...
function MissionVotingUI:_set_vote_time_progress(progress)
	local widgets_by_name = self:get_chrome_widgets()
...
function MissionVotingUI:_update_animations(dt, t)
...
function MissionVotingUI:update(dt, t)
	local parent = self._parent
	local ingame_ui = parent:parent()

	local menu_active = ingame_ui.menu_active or
	ingame_ui.current_view or
	ingame_ui._transition_fade_data
	...
	local is_mission_vote_in_progress = voting_manager:vote_in_progress() and
	voting_manager:is_mission_vote() and
	not voting_manager:has_voted(Network.peer_id())
...
function MissionVotingUI:draw(dt)
...
function MissionVotingUI:_update_pulse_animations(dt)
...
function MissionVotingUI:_acquire_input(ignore_cursor_stack)
	self:_release_input(true)
	local input_manager = self.input_manager
	input_manager:capture_input({ "keyboard", "gamepad", "mouse" }, 1, "mission_voting", "MissionVotingUI")
	if not ignore_cursor_stack then
		ShowCursorStack.push()
	end
end

function MissionVotingUI:_release_input(ignore_cursor_stack)
	local input_manager = self.input_manager
	input_manager:release_input({ "keyboard", "gamepad", "mouse" }, 1, "mission_voting", "MissionVotingUI")
	if not ignore_cursor_stack then
		ShowCursorStack.pop()
	end
end

function MissionVotingUI:active_input_service()
	local input_manager = self.input_manager
...
function MissionVotingUI:_play_sound(sound_event)
