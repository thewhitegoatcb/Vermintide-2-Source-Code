...
local RPCS = { "rpc_alert_enemies_within_range", "rpc_set_allowed_nav_layer", "rpc_change_tentacle_state", "rpc_sync_tentacle_path", "rpc_set_ward_state", "rpc_set_hit_reaction_template", "rpc_set_corruptor_beam_state", "rpc_check_trigger_backstab_sfx", "rpc_set_attribute_bool", "rpc_set_attribute_int", "rpc_remove_attribute" }
...
local extensions = { "AISimpleExtension", "AiHuskBaseExtension", "PlayerBotBase" }
...
function AISystem:init(context, name)
	...
	self.group_blackboard = { rats_currently_moving_to_ip = 0,
		special_targets = { },
		disabled_by_special = { },
	...
	self._nav_cost_map_id_data = { size = 0, current_id = 1, ids = Script.new_array(NAV_COST_MAP_MAX_COST_MAPS), max_size = NAV_COST_MAP_MAX_COST_MAPS }
	self._nav_cost_map_volume_id_data = { size = 0, current_id = 1, ids = Script.new_array(NAV_COST_MAP_MAX_VOLUMES), max_size = NAV_COST_MAP_MAX_VOLUMES }
...
function AISystem:get_nav_cost_maps_data()
	return self._nav_cost_maps_data, NAV_COST_MAP_MAX_COST_MAPS
end

function AISystem:create_nav_cost_map(cost_layer, num_volumes_guess)
	...
	self._nav_cost_maps_data [cost_map_id] = { recompute = false, cost_map = GwNavCostMap.create(nav_world, cost_map_layer_id), volumes = Script.new_map(num_volumes_guess) }
...
function AISystem:destroy_nav_cost_map(cost_map_id)
	local cost_map_id_data = self._nav_cost_map_id_data
	local cost_map_ids_size = cost_map_id_data.size
	local cost_map_ids = cost_map_id_data.ids
...
function AISystem:add_nav_cost_map_box_volume(transform, scale_vector, cost_map_id)
...
function AISystem:add_nav_cost_map_sphere_volume(position, radius, cost_map_id)
	local volume_id_data = self._nav_cost_map_volume_id_data
	local volume_id = volume_id_data.current_id
	local volume_ids = volume_id_data.ids
	local volume_ids_size = volume_id_data.size
...
function AISystem:set_nav_cost_map_volume_transform(volume_id, cost_map_id, transform)
	local volume_id_data = self._nav_cost_map_volume_id_data
	local volume_ids = volume_id_data.ids
...
function AISystem:set_nav_cost_map_volume_scale(volume_id, cost_map_id, scale)
	local volume_id_data = self._nav_cost_map_volume_id_data
	local volume_ids = volume_id_data.ids
...
function AISystem:remove_nav_cost_map_volume(volume_id, cost_map_id)
	local volume_id_data = self._nav_cost_map_volume_id_data
...
function AISystem:_recompute_nav_cost_maps()
...
function AISystem:_initialize_client_traverse_logic(nav_world)
	local nav_tag_layer_costs = { bot_poison_wind = 1, bot_ratling_gun_fire = 1, fire_grenade = 1 }
...
function AISystem:on_add_extension(world, unit, extension_name, extension_init_data)
...
function AISystem:use_perception_continuous(unit, set)
...
function AISystem:set_default_blackboard_values(unit, blackboard)
...
function AISystem:on_remove_extension(unit, extension_name)
...
function AISystem:on_freeze_extension(unit, extension_name)
...
function AISystem:_cleanup_extension(unit, extension_name)
	if self.unit_extension_data [unit] == nil then return end
	local extension = self.unit_extension_data [unit]
...
function AISystem:freeze(unit, extension_name, reason)
...
function AISystem:register_prioritized_perception_unit_update(unit, ai_extension)
	self.ai_units_perception_prioritized [unit] = ai_extension
end

function AISystem:update(context, t)
	local dt = context.dt
...
function AISystem:physics_async_update(context, t)
	local dt = context.dt
...
function AISystem:update_alive()
...
function AISystem:_update_taunt(t, blackboard)
...
function AISystem:update_perception(t, dt)
...
function AISystem:update_brains(t, dt)
...
function AISystem:update_game_objects()
...
function AISystem:update_broadphase()
...
function AISystem:update_debug_unit(t)
...
function AISystem:update_debug_draw(t)
	...
	if script_data.debug_stagger then
		for unit, extension in pairs(self.ai_units_alive) do repeat
				local blackboard = extension._blackboard
				...
				if health_threshold < health_percent then
					...
					if not stagger_immune then
						...
						index = index + 1 end end until true
		end
	end
		...
		for unit, extension in pairs(self.ai_units_alive) do repeat
				...
				if spine_node then
					...
					elseif have_slot > 0 then
						...
						QuickDrawer:sphere(position, 0.25, Colors.get("gray")) end end until true
		end
	end
...
local function update_blackboard(unit, blackboard, t, dt)
	...
	if breed.wake_up_push and target_alive and

	blackboard.stagger > 0 then
	...
	if target_alive and is_valid_attacking_target then
		...
		do return inside_priority_distance end
...
function AISystem:update_ai_blackboards_prioritized(t, dt)
...
function AISystem:update_ai_blackboards(t, dt)
	...
	if ai_blackboard_updates_n < index then index = 1
	end
...
function AISystem:nav_world()
	return self._nav_world
end


function AISystem:client_traverse_logic()
	return self._traverse_logic
end

function AISystem:get_tri_on_navmesh(pos)
	return GwNavQueries.triangle_from_position(self._nav_world, pos, 30, 30)
end

function AISystem:set_allowed_layer(layer_name, allowed)
...
function AISystem:alert_enemies_within_range(unit, position, radius)
...
function AISystem:rpc_alert_enemies_within_range(channel_id, unit_id, position, radius)
	local unit = Managers.state.unit_storage:unit(unit_id)
	self:alert_enemies_within_range(unit, position, radius)
end

function AISystem:rpc_set_ward_state(channel_id, unit_id, state)
	local unit = Managers.state.unit_storage:unit(unit_id)
	AiUtils.stormvermin_champion_set_ward_state(unit, state, false)
end

function AISystem:rpc_set_hit_reaction_template(channel_id, unit_id, hit_effect_template)
	local unit = Managers.state.unit_storage:unit(unit_id)
	local hit_reaction_extension = ScriptUnit.extension(unit, "hit_reaction_system")
	hit_reaction_extension:set_hit_effect_template_id(hit_effect_template)
end

function AISystem:rpc_change_tentacle_state(channel_id, tentacle_unit_id, target_unit_id, tentacle_template_id, tentacle_reach_dist, server_time)
...
function AISystem:rpc_sync_tentacle_path(channel_id, tentacle_unit_id, point_array)
	local tentacle_unit = Managers.state.unit_storage:unit(tentacle_unit_id)
...
function AISystem:rpc_set_corruptor_beam_state(channel_id, unit_id, state, target_unit_id)
	local unit = Managers.state.unit_storage:unit(unit_id)
	local target_unit = Managers.state.unit_storage:unit(target_unit_id)
...
function AISystem:rpc_set_allowed_nav_layer(channel_id, layer_id, allowed)
...
function AISystem:rpc_check_trigger_backstab_sfx(channel_id, unit_id)
	if DEDICATED_SERVER then return end
...
function AISystem:set_attribute(unit, attribute_name, category_name, value, skip_sync)
...
function AISystem:get_attributes(unit)
	local extension = self.unit_extension_data [unit]
	return extension and extension.attributes or dummy_table
end

function AISystem:rpc_set_attribute_bool(channel_id, unit_id, attribute_id, category_id, value)
...
function AISystem:rpc_set_attribute_int(channel_id, unit_id, attribute_id, category_id, value)
...
function AISystem:rpc_remove_attribute(channel_id, unit_id, attribute_id, category_id)
...
function AISystem:hot_join_sync(peer_id)
...
function AISystem:create_all_trees()
...
function AISystem:behavior_tree(tree_name)
	return self._behavior_trees [tree_name]
end

function AISystem:register_unit_for_destruction(unit)
