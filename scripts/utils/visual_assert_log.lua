...
function VisualAssertLog.setup(world)
...
function VisualAssertLog.update()
	if not script_data.visual_assert_log_enabled then return end
		...
		if VisualAssertLog.display_asserts then
			...
			if DEDICATED_SERVER then
				...
				if Keyboard.pressed(VisualAssertLog.console_page_up_key) then
					...
					if n_asserts < current_visualized_assert then current_visualized_assert = 1
					end end
				if Keyboard.pressed(VisualAssertLog.console_page_down_key) then
					current_visualized_assert = current_visualized_assert - 1
					if current_visualized_assert <= 0 then current_visualized_assert = n_asserts or current_visualized_assert
					end end
