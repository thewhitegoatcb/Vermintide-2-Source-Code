...
function ActionBase:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
	self.world = world
...
function ActionBase:client_owner_start_action(new_action, t, chain_action_data, power_level, action_init_data)
...
function ActionBase:_handle_critical_strike(is_critical_strike, buff_extension, hud_extension, first_person_extension, proc_type, hud_sound_event)
...
function ActionBase:_do_critical_strike_fx(hud_extension, first_person_extension, hud_sound_event)
...
function ActionBase:_do_critical_strike_procs(buff_extension, proc_type)
	if buff_extension and proc_type then
		buff_extension:trigger_procs(proc_type)
	end
end

function ActionBase:_update_extra_shots(buff_extension, shots_to_consume, override)
...
function ActionBase:_handle_fatigue(buff_extension, status_extension, new_action, check_buffs)
	local procced = nil
...
function ActionBase:_proc_spell_used(buff_extension)
	local current_action = self.current_action
...
function ActionBase:_play_additional_animation(anim_data)
	if
	anim_data and anim_data.variable_name and anim_data.variable_value then
...
function ActionBase:finish(reason)
	return end