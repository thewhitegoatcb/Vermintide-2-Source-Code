...
local extensions = { "AIInterestPointExtension", "AIInterestPointHuskExtension" }
...
function AIInterestPointSystem:init(context, name)
	...
	local layer_costs = { ledges = 1, ledges_with_fence = 1, jumps = 1 }
...
function AIInterestPointSystem:_random(...)
	local seed, value = Math.next_random(self._seed, ...)
	self._seed = seed
	return value
end

function AIInterestPointSystem:set_seed(seed)
	fassert(seed and type(seed) == "number", "Bad seed input!")
	self._seed = seed
end

function AIInterestPointSystem:destroy()
...
local dummy_input = { }
function AIInterestPointSystem:on_add_extension(world, unit, extension_name, extension_init_data)
	local extension = { }
	...
	if extension_name == "AIInterestPointExtension" then
		...
		if enabled then
			...
			while Unit.has_data(unit, "interest_point", "points", point_i) do
				...
				local animations = { }
				local anim_i = 0
				local x = point_position.x local y = point_position.y local z = point_position.z
...
function AIInterestPointSystem:on_remove_extension(unit, extension_name)
...
function AIInterestPointSystem:update(context, t)
...
function AIInterestPointSystem.breed_spawned_callback(ai_unit, breed, optional_data)
	local point = optional_data.dead_breed_data
	BREED_DIE_LOOKUP [ai_unit] = { AIInterestPointSystem.cleanup_dead_breed, point }
end


function AIInterestPointSystem.cleanup_dead_breed(ai_unit, point)
	point [1] = false
...
function AIInterestPointSystem:spawn_interest_points()
	...
	while num_to_spawn > 0 and interest_point_unit ~= nil do
		...
		for i = 1, points_n do
			...
			if point.is_position_on_navmesh then
				...
				local optional_data = { ignore_event_counter = true }
				...
				local id = conflict:spawn_queued_unit(
				breed,
				Vector3Box(spawn_position),
				point.rotation,
				spawn_category,
				spawn_animation,
				spawn_type,
...
function AIInterestPointSystem:release_obsolete_requests(t)
	...
	release_claim = not HEALTH_ALIVE [claim_unit] and
	true or

	blackboard.confirmed_player_sighting
...
function AIInterestPointSystem:_update_astar_result(current_request_point_unit, request, best_unit, astar)
	self.processing_astar = false
	self.processing_best_point = nil
	self.processing_best_ip_unit = nil
	self.processing_best_point_extension = nil
...
function AIInterestPointSystem:_start_astar_query(astar, start_position, end_position, nav_world, traverse_logic, best_unit, best_point, best_point_extension)
...
function AIInterestPointSystem:resolve_requests()
	...
	if request ~= nil then
		...
		local path_check_done = false local path_found = false
...
function AIInterestPointSystem:debug_draw(t, dt)
...
function AIInterestPointSystem:api_start_async_claim_request(claim_unit, position, min_range, max_range, current_request_id)
...
function AIInterestPointSystem:api_get_claim(request_id)
	fassert(request_id, "Tried to get claim with no request_id")
	return self.requests [request_id]
end

function AIInterestPointSystem:api_release_claim(request_id)
	local request = self.requests [request_id]
	assert(request)
...
function AIInterestPointSystem:rpc_interest_point_chatter_update(channel_id, go_id, is_level_unit, percent_claimed)
...
function AIInterestPointSystem:set_breed_override_lookup(breed_override_lookup)
