...
function UnitSpawner:init(world, entity_manager, is_server)
	self.world = world
...
function UnitSpawner:set_gameobject_initializer_data(initializer_function_table, extraction_function_table, gameobject_context)
...
function UnitSpawner:set_unit_storage(unit_storage)
	self.unit_storage = unit_storage
end


function UnitSpawner:set_gameobject_to_unit_creator_function(func)
...
function UnitSpawner:set_unit_template_lookup_table(template_lut)
	self.unit_template_lut = template_lut
end

function UnitSpawner:push_unit_to_death_watch_list(unit, t, data)
	self:freeze_unit_extensions(unit, t, data)

	self.unit_death_watch_list_n = self.unit_death_watch_list_n + 1
	self.unit_death_watch_list [self.unit_death_watch_list_n] = { unit = unit, t = t, data = data }
	self.unit_death_watch_lookup [unit] = self.unit_death_watch_list [self.unit_death_watch_list_n]
end

function UnitSpawner:freeze_unit_extensions(unit, t, data)
...
function UnitSpawner:prioritize_death_watch_unit(unit, t)
...
function UnitSpawner:breed_in_death_watch(breed_name)
...
local function death_watch_list_sort(a, b) return a.t < b.t end

function UnitSpawner:update_death_watch_list()
...
function UnitSpawner:mark_for_deletion(unit)
...
function UnitSpawner:is_marked_for_deletion(unit)
...
function UnitSpawner:commit_and_remove_pending_units()

	local n_commited = 0 local n_removed = 0
	repeat
		n_commited = self:commit_pending_unit_system_registrations()
		n_removed = self:remove_units_marked_for_deletion() until
	n_commited + n_removed == 0
end

function UnitSpawner:commit_pending_unit_system_registrations()
	...
	local pending_extension_adds_map = self.pending_extension_adds_map local pending_extension_adds_list = self.pending_extension_adds_list
	local count = 0
...
function UnitSpawner:remove_units_marked_for_deletion()
	fassert(not self.locked)
	...
	if Unit_alive(unit) then
		...
		if pending_extension_adds_list_n > 0 and
		pending_extension_adds_map [unit] then
			pending_extension_adds_map [unit] = nil
...
function UnitSpawner:spawn_local_unit(unit_name, position, rotation, material)
...
function UnitSpawner:create_unit_extensions(world, unit, unit_template_name, extension_init_data)
...
function UnitSpawner:spawn_local_unit_with_extensions(unit_name, unit_template_name, extension_init_data, position, rotation, material)
...
function UnitSpawner:spawn_network_unit(unit_name, unit_template_name, extension_init_data, position, rotation, material)
...
function UnitSpawner:request_spawn_network_unit(template_name, position, rotation, source_unit, state_int, group_spawn_index)
...
function UnitSpawner:world_delete_units(world, units_list, units_list_n)
	local game_session = self.game_session local unit_storage = self.unit_storage
...
function UnitSpawner:spawn_unit_from_game_object(go_id, owner_id, go_template)
...
function UnitSpawner:destroy_game_object_unit(go_id, owner_id)
...
function UnitSpawner:add_destroy_listener(unit, identifier, callback, post_cleanup_listener)
...
function UnitSpawner:remove_destroy_listener(unit, identifier, post_cleanup_listener)
	local destroy_listeners = post_cleanup_listener and self.unit_destroy_listeners_post_cleanup or self.unit_destroy_listeners
	local listeners = destroy_listeners [unit]
	if listeners then
		listeners [identifier] = nil
	else
		printf("[UnitSpawner] [%s] failed to remove listener [%s] from unit [%s]",
		self.identifier_tag, tostring(identifier), tostring(unit))
	end
end