...
local relevant_bosses = { "skaven_rat_ogre", "skaven_stormfiend", "chaos_spawn", "beastmen_minotaur", "chaos_troll" }
...
for career, _ in pairs(CareerSettings) do
	if career ~= "empire_soldier_tutorial" then
		...
		if career_breed and career_breed.is_hero then
			...
			player.grudge_mark_kills [career] = { value = 0, source = "player_data", database_name = database_name }
			...
			for i = 1, #relevant_bosses do
				...
				player.grudge_marks_kills_per_career_per_monster [career] [breed_name] = { value = 0, source = "player_data", database_name = database_name }
			end
			...
			for i = 1, #expeditions do
				...
				player.grudge_marks_kills_per_career_per_expedition [career] [expedition_name] = { value = 0, source = "player_data", database_name = database_name } end end end end