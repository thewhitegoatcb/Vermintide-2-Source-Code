...
function ModManager:init(boot_gui)
	self._mods = { }
	self._num_mods = nil
	self._state = "not_loaded"
	self._settings = Application.user_setting("mod_settings") or { toposort = false, log_level = 1, developer_mode = false }
...
function ModManager:developer_mode_enabled()
	return self._settings.developer_mode
end



function ModManager:_draw_state_to_gui(gui, dt)
...
function ModManager:remove_gui()
	assert(self._gui, "Trying to remove gui without setting gui first.")
	self._gui = nil
end


function ModManager:_has_enabled_mods(in_modded_realm)
...
function ModManager:all_mods_loaded()
...
function ModManager:_run_callback(mod, callback_name, ...)
...
function ModManager:_fetch_mod_metadata()
	local url = "http://cdn.fatsharkgames.se/mod_metadata.txt"
	local headers = { ["User-Agent"] = "Warhammer: Vermintide 2" }
...
function ModManager:_cb_mod_metadata(success, return_code, headers, data, userdata)
...
function ModManager:_build_mod_table(mod_handles)
	...
	for i, mod_data in ipairs(user_settings_mod_list) do
		...
		self._mods [i] = { state = "not_loaded", callbacks_disabled = false,
			id = id,
			name = mod_data.name,
			enabled = enabled,
			handle = handle,
...
function ModManager:_load_mod(index)
	self._ui_time = 0
...
function ModManager:_load_package(mod, index)
	mod.package_index = index
	local package_name = mod.data.packages [index]
	if not package_name then return
	end
...
function ModManager:unload_all_mods()
	if self._state ~= "done" then
...
function ModManager:unload_mod(index)
...
function ModManager:_reload_mods()
...
function ModManager:_topologically_sorted(mod_list)
	local visited = { } local sorted = { }
...
function ModManager:_visit(mod_list, visited, sorted, mod_data)
...
local LOG_LEVELS = { spew = 4, info = 3, warning = 2, error = 1 }
...
function ModManager:print(level, str, ...)
...
function ModManager:network_bind(port, callback)
...
function ModManager:network_unbind(port)
...
function ModManager:network_is_occupied(port)
...
function ModManager:network_send(destination_peer_id, port, payload)
...
function ModManager:rpc_mod_user_data(relay_channel_id, source_peer_id, destination_peer_id, port, payload)


	if destination_peer_id == self._my_peer_id then
		local cb = self._network_callbacks [port]
		if cb then cb(source_peer_id, payload)
		end
...
function ModManager:register_network_event_delegate(network_event_delegate)
...
function ModManager:unregister_network_event_delegate()
...
function ModManager:network_context_created(host_peer_id, my_peer_id, is_server)
