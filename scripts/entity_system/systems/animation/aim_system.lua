...
local extensions = { "GenericUnitAimExtension" }



AimSystem = class(AimSystem, ExtensionSystemBase)

function AimSystem:init(context, system_name)
...
function AimSystem:on_add_extension(world, unit, extension_name, extension_init_data)
...
function AimSystem:on_remove_extension(unit, extension_name)
	self._frozen_extensions [unit] = nil
	self._extensions [unit] = nil
	ScriptUnit.remove_extension(unit, self.NAME)
end

function AimSystem:on_freeze_extension(unit, extension_name)
...
function AimSystem:freeze(unit, extension_name, reason)
...
function AimSystem:unfreeze(unit)
...
function AimSystem:update(context, t)
	local dt = context.dt
