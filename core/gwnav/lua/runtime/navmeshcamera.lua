...
local Window = nil
if stingray.Window then Window = stingray.Window end
...
function NavMeshCamera:init(camera, unit, nav_world)
	self.camera = camera
	self.unit = unit
...
function NavMeshCamera:update(dt)
	local input = { }
	...
	if Application.platform() == "win32" or Application.platform() == "macosx" then
		...
		Keyboard.button(Keyboard.button_id("e")) - Keyboard.button(Keyboard.button_id("q")))
	else
