...
function StoreLoginRewardsPopup:init(parent, params)
	self._parent = parent
...
function StoreLoginRewardsPopup:destroy()
...
function StoreLoginRewardsPopup:_create_ui_elements()
...
function StoreLoginRewardsPopup:_has_claimed_reward(claimed_rewards, day_index)
...
function StoreLoginRewardsPopup:_setup_rewards_data(login_rewards)
	...
	local reward_index = login_rewards.total_claims or 0
...
function StoreLoginRewardsPopup:_claim_rewards()
...
function StoreLoginRewardsPopup:_refresh_login_rewards_cb()
	self._waiting_for_refresh = false
end


function StoreLoginRewardsPopup:update(input_service, dt, t)
	...
	elseif state == STATE.wait_for_backend then
		if backend_store:done_claiming_login_rewards() then
			...
			local rewards = login_rewards.rewards
			local total_claims = login_rewards.total_claims or 1
			local reward_index = total_claims == 0 and #rewards or total_claims
	...
	elseif state == STATE.exiting and
	not animations.on_exit then
...
function StoreLoginRewardsPopup:_present_rewards(rewards)
	...
	for i = 1, num_rewards do
		...
		if reward_type == "item" or reward_type == "crafting_material" then
			...
			presentation_data [#presentation_data + 1] = { { widget_type = "description",
...
function StoreLoginRewardsPopup:_update_timer(cooldown, expiry)
	local widgets_by_name = self._widgets_by_name
...
function StoreLoginRewardsPopup:_play_animation(name, widgets)
...
function StoreLoginRewardsPopup:_update_animations(dt)
...
function StoreLoginRewardsPopup:_handle_input(input_service, dt, t)
	local widgets_by_name = self._widgets_by_name
...
function StoreLoginRewardsPopup:_handle_gamepad_input(input_service)
	local gamepad_active = Managers.input:is_device_active("gamepad")
	self._gamepad_active = gamepad_active
	...
	if modified then
		...
		for i = 1, #reward_widgets do
			...
			local is_selected = content.day_index == cursor_x and
			content.item_index == cursor_y
...
function StoreLoginRewardsPopup:_draw(state, input_service, dt, t)
...
function StoreLoginRewardsPopup:is_complete()
	return self._state == STATE.exited
end


function StoreLoginRewardsPopup:_play_sound(event)
	return self._parent:play_sound(event)
end

function StoreLoginRewardsPopup:has_claimed_rewards()
