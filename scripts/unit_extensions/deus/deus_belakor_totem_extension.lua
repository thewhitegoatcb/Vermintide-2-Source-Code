...
local STATE = { INITIAL = 0, COOLDOWN_FROM_SPAWN = 2, WAITING_TO_SPAWN_ENEMIES = 1, SPAWNING_ENEMIES = 4, DESPAWNED = 5, DECAL_SPAWNED = 3 }
...
function DeusBelakorTotemExtension:init(extension_init_context, unit, extension_init_data)
	self._unit = unit
	self.spawn_count = 0
...
function DeusBelakorTotemExtension:game_object_initialized(unit, go_id)
...
function DeusBelakorTotemExtension:extensions_ready(world, unit)
	self._health_ext = ScriptUnit.extension(unit, "health_system")
end

function DeusBelakorTotemExtension:destroy()
...
function DeusBelakorTotemExtension:is_despawned()
	return self._current_state == STATE.DESPAWNED
end

function DeusBelakorTotemExtension:update(unit, input, dt, context, t)
