...
function WeaveTutorialPopupUI:init(ui_context)
...
function WeaveTutorialPopupUI:_setup_input()
...
function WeaveTutorialPopupUI:_create_ui_elements()
...
function WeaveTutorialPopupUI:show(title, sub_title, body, optional_button_2, optional_button_2_func, optional_button_2_input_actions, disable_body_localization, data)
...
function WeaveTutorialPopupUI:show_custom_popup(popup_data)
...
function WeaveTutorialPopupUI:hide()
...
function WeaveTutorialPopupUI:_destroy_custom_popup()
...
function WeaveTutorialPopupUI:destroy()
	self:hide()
	self.widgets = nil
...
function WeaveTutorialPopupUI:update(dt)
	...
	if self._custom_popup then
		...
		if self._optional_button_2_func and (
		UIUtils.is_button_pressed(self.button_2) or input_service:get("special_1_press", true)) then
...
function WeaveTutorialPopupUI:_update_animations(dt)
...
function WeaveTutorialPopupUI:_draw(dt, input_service)
	if self._custom_popup then
		return
	end
...
function WeaveTutorialPopupUI:populate_message(title_text, sub_title_text, body_text, optional_button_2, disable_body_localization)
...
function WeaveTutorialPopupUI:resize_to_fit()
...
function WeaveTutorialPopupUI:calculate_base_window_height()
...
function WeaveTutorialPopupUI:break_paragraphs(text)
	local paragraphs = { }
	local i = 1
...
function WeaveTutorialPopupUI:draw_body(renderer)
...
function WeaveTutorialPopupUI:start_transition_animation(key, animation_name)
	local params = {
	...
	local widgets = { self.widgets_by_name.screen_background }

	local anim_id = self.ui_animator:start_animation(animation_name, widgets, scenegraph_definition, params)
	self._animations [key] = anim_id
end

function WeaveTutorialPopupUI:_acquire_input()
	local input_manager = self.input_manager
	input_manager:capture_input({ "keyboard", "gamepad", "mouse" }, 1, "weave_tutorial", "WeaveTutorialPopupUI")
	ShowCursorStack.push()
end

function WeaveTutorialPopupUI:_release_input()
	local input_manager = self.input_manager
	input_manager:release_input({ "keyboard", "gamepad", "mouse" }, 1, "weave_tutorial", "WeaveTutorialPopupUI")
	ShowCursorStack.pop()
end