...
local spline_window_size = { x = 300, y = 300 }
...
local tool_pos = { x = 250, y = resy - spline_window_size.y - 200 }
local windows_x = 3
local windows_y = 2
...
for j = 0, windows_y - 1 do
	for i = 0, windows_x - 1 do
		...
		window_list [k] = { x = xpos, y = ypos }
		drag_point_list [k] = { value = 0, index = k, x = xpos + spline_window_size.x - half_row_height / 2, y = ypos - row_height / 2 }
		k = k + 1
	end
end
...
action_list_layout = { size_x = 200, x = 30, y = resy - action_list_height }
...
function EditAiUtility:init(world)
	self.world = world
...
function EditAiUtility:activate()
	ShowCursorStack.push()
end

function EditAiUtility:deactivate()
	ShowCursorStack.pop()
end

function EditAiUtility:use_breed(breed)

	return end

function EditAiUtility:update(unit, t, dt, input_service, blackboard)
	...
	if not status.selected_drag_point then
		...
		local spline = status.hover_win_name and considerations [status.hover_win_name].spline local spline_id = nil
		...
		if not status.hover_point and
		status.hover_win_name and DebugKeyHandler.key_pressed("a", "insert spline point", "ai editor", "left ctrl") then
	...
	elseif status.selected_drag_point then
		...
		if status.left_pressed then
			...
			local xd, yd = EditAiUtility:drag_point_distance(t, point, mouse_pos)
			if math.abs(yd) < safe_drag_lane and

			point.max_value then
	...
	for name, data in pairs(considerations) do
		if type(data) == "table" and not data.is_condition then
			...
			if blackboard then
				...
				for i, breed_action in pairs(breed_actions) do repeat
						...
						for unit_data_name, unit_data in pairs(unit_considerations) do
							...
							if unit_data_name == data_name then
								...
								unit_data.max_value = data.max_value end end until true
				end
			end
		end
	...
	if status.hover_action_window and
	status.left_pressed and status.hover_action then
...
function EditAiUtility:insert_spline_point(spline, win_pos, win_size, mouse_pos)
...
function EditAiUtility:remove_spline_point(spline, point_index)
...
function EditAiUtility:hover_win(t, mouse_pos, window_list, win_size)
...
function EditAiUtility:move_spline_point(t, spline, win_pos, win_size, point_index, new_pos)
	...
	first_point_index < point_index and point_index < last_point_index and spline [point_index - 2] < x and x < spline [point_index + 2] then
...
local hover_dist = 20
function EditAiUtility:hover_spline_point(t, spline, win_pos, win_size, mouse_pos)
...
function EditAiUtility:drag_point_distance(t, point, mouse_pos)
	local x = mouse_pos.x
	local y = mouse_pos.y
	local safe_zone = 10

	local x_dist = x - point.x
	x_dist = math.abs(x_dist) < safe_zone and
	0 or

	x_dist - (x_dist > 0 and safe_zone or -safe_zone)


	local y_dist = y - point.y
	y_dist = math.abs(y_dist) < safe_zone and
	0 or

	y_dist - (y_dist > 0 and safe_zone or -safe_zone)
...
function EditAiUtility:hover_drag_points(t, point_list, mouse_pos)
...
function EditAiUtility:draw_mouse_selection(t, spline, win_pos, win_size, point_index, selected, max_value)
...
function EditAiUtility.draw_square(gui, t, pos, width, color, thickness)
...
function EditAiUtility:hover_action(t, layout, action_list, mouse_pos)
...
function EditAiUtility:draw_action_list(unit, t, name, layout, action_list, bk_color, selected_action, blackboard)
...
function EditAiUtility:draw_safe_drag_lane(point, safe_distance)
...
function EditAiUtility.draw_realtime_utility(gui, action_name, consideration, pos, win_size, blackboard)
...
function EditAiUtility.draw_utility_sum(gui, name, pos, size)

	return end


function EditAiUtility:draw_utility_ruler(gui, consideration_data, pos, size)
...
function EditAiUtility.draw_utility_info(gui, consideration_data, temp_max_value, name, pos, size, fade_factor, tiny)
...
function EditAiUtility.draw_utility_spline(gui, t, consideration_data, temp_max_value, name, pos, size, bk_color, fade_factor, thickness)
...
function EditAiUtility.draw_utility_condition(gui, action_name, consideration, pos, win_size, blackboard, bk_color)
...
function EditAiUtility:save_considerations()
