...
function ActionGeiser:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionGeiser:client_owner_start_action(new_action, t, chain_action_data, power_level)
...
function ActionGeiser:client_owner_post_update(dt, t, world, can_damage)
...
function ActionGeiser:finish(reason)
...
function ActionGeiser:fire(reason)
	local current_action = self.current_action
	local owner_unit = self.owner_unit
		...
		for i = 1, num_actors do
			...
			if not hit_units [hit_unit] then
				...
				if should_hit then
					...
					local damage_data = { hit_zone_name = "torso",
						hit_unit = hit_unit,
						damage_profile_name = damage_profile_name,
						target_index = target_index, allow_critical_proc = target_index == 1,
...
function ActionGeiser:_update_damage(current_action)
	local damage_buffer = self._damage_buffer
	...
	for i = damage_buffer_index, num_units do repeat
			...
			"is_critical_strike", is_critical_strike) until true
	end
