...
local RPCS = { "rpc_status_change_bool", "rpc_status_change_int", "rpc_status_change_int_and_unit", "rpc_set_catapulted", "rpc_leap_start", "rpc_leap_finished", "rpc_set_blocking", "rpc_set_charge_blocking", "rpc_player_blocked_attack", "rpc_set_wounded", "rpc_hooked_sync", "rpc_hot_join_sync_health_status", "rpc_replenish_fatigue", "rpc_replenish_fatigue_other_players", "rpc_set_stagger", "rpc_set_action_data", "rpc_set_override_blocking" }
...
local extensions = { "GenericStatusExtension" }
...
function StatusSystem:init(entity_system_creation_context, system_name)
...
function StatusSystem:destroy()
	self.network_event_delegate:unregister(self)
end

function StatusSystem:rpc_hooked_sync(channel_id, status_id, game_object_id, time_left)
...
function StatusSystem:rpc_set_action_data(channel_id, game_object_id, breed_id, breed_action_id)
	local unit = self.unit_storage:unit(game_object_id)
...
function StatusSystem:rpc_status_change_bool(channel_id, status_id, status_bool, game_object_id, other_object_id)
	local unit = self.unit_storage:unit(game_object_id)
	if not unit or not Unit.alive(unit) then return end
...
function StatusSystem:rpc_status_change_int(channel_id, status_id, status_int, game_object_id)
	local unit = self.unit_storage:unit(game_object_id)
	if not unit or not Unit.alive(unit) then return end
	local status_ext = ScriptUnit.extension(unit, "status_system")
	local status = NetworkLookup.statuses [status_id]
...
function StatusSystem:rpc_status_change_int_and_unit(channel_id, status_id, status_int, game_object_id, other_object_id)
	local unit = self.unit_storage:unit(game_object_id)
	if not unit or not Unit.alive(unit) then return end
	local other_unit = self.unit_storage:unit(other_object_id)
	local status_ext = ScriptUnit.extension(unit, "status_system")
...
function StatusSystem:rpc_set_wounded(channel_id, game_object_id, wounded, reason_id)
	local unit = self.unit_storage:unit(game_object_id)

	if not unit or not Unit.alive(unit) then return
	end
...
function StatusSystem:rpc_set_catapulted(channel_id, unit_id, catapulted, velocity)
...
function StatusSystem:rpc_set_override_blocking(channel_id, unit_id, blocking)
	local unit = self.unit_storage:unit(unit_id)
	if not unit or not Unit.alive(unit) then return end
	local status_extension = ScriptUnit.extension(unit, "status_system")
...
function StatusSystem:rpc_leap_start(channel_id, unit_id)
	local unit = self.unit_storage:unit(unit_id)
	if not unit or not Unit.alive(unit) then return end
	local status_extension = ScriptUnit.extension(unit, "status_system")
...
function StatusSystem:rpc_leap_finished(channel_id, unit_id)
	local unit = self.unit_storage:unit(unit_id)
	if not unit or not Unit.alive(unit) then return end
	local status_extension = ScriptUnit.extension(unit, "status_system")
...
function StatusSystem:rpc_set_blocking(channel_id, game_object_id, blocking)
	local unit = self.unit_storage:unit(game_object_id)
	if not unit or not Unit.alive(unit) then return
	end
	local status_extension = ScriptUnit.extension(unit, "status_system")
...
function StatusSystem:rpc_set_charge_blocking(channel_id, game_object_id, charge_blocking)
	local unit = self.unit_storage:unit(game_object_id)
	if not unit or not Unit.alive(unit) then return
	end
	local status_extension = ScriptUnit.extension(unit, "status_system")
...
function StatusSystem:rpc_player_blocked_attack(sender, game_object_id, fatigue_type_id, attacking_unit_id, fatigue_point_costs_multiplier, improved_block, attack_direction, attacker_is_level_unit)
	local unit = self.unit_storage:unit(game_object_id)
...
function StatusSystem:rpc_hot_join_sync_health_status(channel_id, game_object_id, wounds, ready_for_assisted_respawn, respawn_unit_game_object_id)
	local unit = self.unit_storage:unit(game_object_id)
	local status_extension = ScriptUnit.extension(unit, "status_system")
...
function StatusSystem:rpc_replenish_fatigue(channel_id, game_object_id, fatigue_type_id)
...
function StatusSystem:rpc_replenish_fatigue_other_players(channel_id, fatigue_type_id)
	if self.is_server then
		local peer_id = CHANNEL_TO_PEER_ID [channel_id]
...
function StatusSystem:rpc_set_stagger(channel_id, game_object_id, stagger_type, stagger_direction, stagger_length, accumulated_stagger, stagger_time, stagger_animation_scale, always_stagger_suffered)
	local unit = self.unit_storage:unit(game_object_id)
	local status_extension = ScriptUnit.extension(unit, "status_system")
