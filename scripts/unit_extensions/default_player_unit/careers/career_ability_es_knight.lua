...
function CareerAbilityESKnight:init(extension_init_context, unit, extension_init_data)
...
function CareerAbilityESKnight:extensions_ready(world, unit)
...
function CareerAbilityESKnight:destroy()
	return end

function CareerAbilityESKnight:update(unit, input, dt, context, t)
...
function CareerAbilityESKnight:stop(reason)
	if
...
function CareerAbilityESKnight:_update_priming(dt)
...
function CareerAbilityESKnight:_stop_priming()
	if self._decal_unit then
		local unit_spawner = Managers.state.unit_spawner
...
function CareerAbilityESKnight:_run_ability()
	self:_stop_priming()
	...
	status_extension.do_lunge = { animation_end_event = "foot_knight_ability_charge_hit", allow_rotation = false, falloff_to_speed = 5, ledge_falloff_immunity = 0.5, dodge = true, first_person_animation_event = "foot_knight_ability_charge_start", first_person_animation_end_event = "foot_knight_ability_charge_hit", first_person_hit_animation_event = "charge_react", damage_start_time = 0.3, duration = 1.5, initial_speed = 20, animation_event = "foot_knight_ability_charge_start",
		...
		damage = { offset_forward = 2.4, height = 1.8, depth_padding = 0.6, hit_zone_hit_name = "full", ignore_shield = false, collision_filter = "filter_explosion_overlap_no_player", interrupt_on_max_hit_mass = true, power_level_multiplier = 1, interrupt_on_first_hit = false, damage_profile = "markus_knight_charge", width = 2, allow_backstab = false,
			...
			stagger_angles = { max = 80, min = 25 },




			on_interrupt_blast = { allow_backstab = false, radius = 3, power_level_multiplier = 1, hit_zone_hit_name = "full", damage_profile = "markus_knight_charge_blast", ignore_shield = false, collision_filter = "filter_explosion_overlap_no_player" } } }
...
function CareerAbilityESKnight:_play_vo()
	local owner_unit = self._owner_unit
