...
TestCases = {
	...
	smoke = function ()
		Testify:run_case(
		function (dt, t)
			TestifySnippets.load_level({ level_key = "inn_level" })
		end)
	...
	load_level = function (level_key, skip_cinematic, wait_for_player_to_spawn)
		Testify:run_case(
		function (dt, t)
			TestifySnippets.load_level({ level_key = level_key })
	...
	equip_weapons = function (is_magic)
		Testify:run_case(function (dt, t)
			if is_magic then
				...
				TestifySnippets.load_level({ level_key = "military" })
				Testify:make_request("wait_for_cutscene_to_finish")
			end
			...
			TestifySnippets.set_script_data({ ai_bots_disabled = true, allow_same_bots = true })
...
function TestCases.load_weave(weave_number)
...
function TestCases.run_through_level(case_settings, skip_cinematic)
	Testify:run_case(
	function (dt, t)
		local result = ""
		TestifySnippets.set_script_data({ power_level_override = 1600, ai_bots_disabled = false })
		...
		for i = 1, 3 do
			bots_stuck_data [i] = { Vector3Box(Vector3(-999, -999, -999)), os.time() }
		end
		local bot_teleportation_data = { main_path_point = 0, bots_blocked_time_before_teleportation = 15, bots_blocked_distance = 2,
		...
		while not Testify:make_request("level_end_screen_displayed") and main_path_point < total_main_path_distance - 10 do
			...
			if main_path_point < player_point then main_path_point = player_point or main_path_point end
...
function TestCases.run_through_weave(case_settings)
	Testify:run_case(
	function (dt, t)
		local result = ""
		TestifySnippets.set_script_data({ power_level_override = 1600, ai_bots_disabled = false })
		...
		while not Testify:make_request("is_end_zone_activated", end_zone_name) do
			...
			if end_screen_displayed then
				if Testify:make_request("weave_remaining_time") == 0 then
					result = result .. "Out of time, Phase 1" break
				end
				result = result .. "Defeated Phase 1"
...
function TestCases.load_level_environment_variations(level_key)
	Testify:run_case(
	function ()
...
function TestCases.measure_performance(level_key, skip_cinematic)
	Testify:run_case(
	function ()
...
function TestCases.measure_deus_performance(level_key)
	TestCases.measure_performance(level_key, true)
end

function TestCases.run_through_deus_level(case_settings)
	TestCases.run_through_level(case_settings, true)
end

function TestCases.run_through_deus_level_terror_event(level_key, terror_event_name, peak_offset)
	Testify:run_case(
	function (dt, t)
		...
		TestifySnippets.set_script_data({ insta_death = true, disable_external_velocity = true, disable_vortex_attraction = true, disable_catapulting = true, ai_terror_events_disabled = true, debug_terror = true, ai_bots_disabled = false, infinite_ammo = true, power_level_override = 1600,
		...
		local bot_teleportation_data = { bots_blocked_time_before_teleportation = 15, bots_blocked_distance = 2,
			bots_stuck_data = bots_stuck_data,
		...
		Testify:make_request("add_buffs_to_heroes", { "ledge_rescue", "disable_rescue" })
...
function TestCases.run_through_pvp_level(case_settings)
	Testify:run_case(
	function (dt, t)
		local result = ""
		TestifySnippets.set_script_data({ power_level_override = 1600, ai_bots_disabled = false })
...
function TestCases.spawn_all_enemies(case_settings)
	Testify:run_case(function (dt, t)
		local result = ""
		...
		TestifySnippets.load_level({ level_key = "plaza" })
		Testify:make_request("wait_for_cutscene_to_finish")
		...
		local spawn_position_offset = { z = 1, x = 8, y = -1 }
		local boxed_spawn_position = Vector3Box(
		player_current_position.x + spawn_position_offset.x,
		player_current_position.y + spawn_position_offset.y,
		player_current_position.z + spawn_position_offset.z)
		...
		for breed_name, breed_data in pairs(breeds) do
			...
			if spawn_simultaneously then
				...
				while kill_timer > os.clock() - minion_time_of_spawn do
					...
					if not is_minion_alive then break
					end
				end
...
function TestCases.equip_deus_power_ups(case_settings)
	Testify:run_case(
	function (dt, t)
		...
		TestifySnippets.set_script_data({ disable_catapulting = true, disable_external_velocity = true, disable_vortex_attraction = true, ai_terror_events_disabled = true, debug_terror = true, ai_bots_disabled = false, infinite_ammo = true, power_level_override = 1600,
		...
		for rarity, powerup_tests_for_rarity in pairs(power_up_tests) do
			for power_up_name, power_up_test_function in pairs(powerup_tests_for_rarity) do
				...
				Testify:make_request("add_buffs_to_heroes", { "ledge_rescue", "disable_rescue", "blessing_of_isha_invincibility" })
...
function TestCases.write_morris_levels_to_file()
	Testify:run_case(function (dt, t)
		...
		local suite_definition = { "suites_definition:", "  - suite_id: load_all_deus_levels", "    name: Load All Deus Levels", "    abort_if_n_first_cases_failed: 3", "    sessions:", "      - project_config_type: game_host", "        extra_session_args: -skip-splash -disable-intro-trailer -debug-testify -use-local-backend -mechanism deus" }
		...
		local case_definition = { "      - case_definition_id: load_level", nil, nil, "        script_args:", nil, "          - true" }
