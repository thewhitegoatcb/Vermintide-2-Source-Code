...
function ActiveReloadAmmoUserExtension:init(extension_init_context, unit, extension_init_data)
...
function ActiveReloadAmmoUserExtension:extensions_ready(world, unit)
	return end

function ActiveReloadAmmoUserExtension:destroy()
	return end

function ActiveReloadAmmoUserExtension:reset()
	self.current_ammo = self.ammo_per_clip
	self.available_ammo = self.start_ammo - self.current_ammo
	self.shots_fired = 0
end

function ActiveReloadAmmoUserExtension:update(unit, input, dt, context, t)
...
function ActiveReloadAmmoUserExtension:_update_active_reload(dt, t)
...
function ActiveReloadAmmoUserExtension:_debug_draw(dt, t)
	local gui = self._gui
...
function ActiveReloadAmmoUserExtension:_setup_indicator_area()
...
function ActiveReloadAmmoUserExtension:reload_start_time()
	assert(self.next_reload_time)
	return self.next_reload_time - self.reload_time
end

function ActiveReloadAmmoUserExtension:add_ammo(ammo_amount)
	self.available_ammo = math.min(self.available_ammo + ammo_amount, self.max_ammo - (self.current_ammo - self.shots_fired))
end

function ActiveReloadAmmoUserExtension:use_ammo(ammo_used)
	self.shots_fired = self.shots_fired + ammo_used
	assert(self:ammo_count() >= 0)
end

function ActiveReloadAmmoUserExtension:start_reload(play_reload_animation)
	assert(self:can_reload())
	assert(self.next_reload_time == nil)
	self.start_reloading = true
	self.next_reload_time = 0
end
function ActiveReloadAmmoUserExtension:abort_reload()
	assert(self:is_reloading())
	self.start_reloading = nil
	self.next_reload_time = nil
end

function ActiveReloadAmmoUserExtension:ammo_count()
	return self.current_ammo - self.shots_fired
end

function ActiveReloadAmmoUserExtension:clip_size()
	return self.ammo_per_clip
end

function ActiveReloadAmmoUserExtension:remaining_ammo()
	return self.available_ammo
end

function ActiveReloadAmmoUserExtension:can_reload()
	if self:is_reloading() then return false end
	if self:ammo_count() == self.ammo_per_clip then return false end
	if script_data.infinite_ammo then return true
	end
	return self.available_ammo > 0
end

function ActiveReloadAmmoUserExtension:is_reloading()
