...
function StartGameWindowWeaveBackgroundConsole:on_enter(params, offset)
...
function StartGameWindowWeaveBackgroundConsole:_create_ui_elements(params, offset)
...
function StartGameWindowWeaveBackgroundConsole:on_exit(params)
...
function StartGameWindowWeaveBackgroundConsole:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function StartGameWindowWeaveBackgroundConsole:post_update(dt, t)

	return end

function StartGameWindowWeaveBackgroundConsole:_update_animations(dt, t)
...
function StartGameWindowWeaveBackgroundConsole:_set_background_wheel_visibility(visible)
...
function StartGameWindowWeaveBackgroundConsole:_update_background_animations(dt)
	local widgets_by_name = self._widgets_by_name
	for i = 1, 2 do
		local wheel_ring_1 = widgets_by_name ["wheel_ring_" .. i .. "_1"]
		local wheel_ring_2 = widgets_by_name ["wheel_ring_" .. i .. "_2"]
...
function StartGameWindowWeaveBackgroundConsole:_set_background_bloom_intensity(fraction, is_game_matchmaking)
	local min = 1.39
...
function StartGameWindowWeaveBackgroundConsole:_play_sound(event)
	self._parent:play_sound(event)
end

function StartGameWindowWeaveBackgroundConsole:_exit(selected_level)
	self.exit = true
	self.exit_level_id = selected_level
end

function StartGameWindowWeaveBackgroundConsole:draw(dt)
...
function StartGameWindowWeaveBackgroundConsole:_animate_pulse(target, target_index, from, to, speed)
	local new_animation = UIAnimation.init(UIAnimation.pulse_animation, target, target_index,
	from, to, speed)
	return new_animation
end

function StartGameWindowWeaveBackgroundConsole:_animate_element_by_time(target, target_index, from, to, time)
	local new_animation = UIAnimation.init(UIAnimation.function_by_time, target, target_index,
	from, to, time, math.ease_out_quad)
	return new_animation
end

function StartGameWindowWeaveBackgroundConsole:_animate_element_by_catmullrom(target, target_index, target_value, p0, p1, p2, p3, time)
	local new_animation = UIAnimation.init(UIAnimation.catmullrom, target, target_index, target_value,
	p0, p1, p2, p3, time)
	return new_animation
end

function StartGameWindowWeaveBackgroundConsole:_animate_wheel_position(dt)
	local parent = self._parent
	local selected_layout_name = parent:get_selected_layout_name()
	self._current_offset = self._current_offset or { 0, 0 }
	self._starting_point = self._starting_point or { 0, 0 }
	self._offset_destinations = self._offset_destinations or {
		weave_ranked = { 300, 120 },



		weave_quickplay = { 0, 220 },
...
function StartGameWindowWeaveBackgroundConsole:_animate_background_color(dt, t)
	local parent = self._parent
	local selected_layout_name = parent:get_selected_layout_name()
	self._current_alpha = self._current_alpha or 0
	self._starting_alpha = self._starting_alpha or 0
	self._alpha_destinations = { lobby_browser_weave = 0.3, default = 0.1 }
