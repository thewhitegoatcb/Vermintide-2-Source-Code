...
local extensions = { "GlobalObserverExtension", "LookatTargetExtension", "SurroundingObserverExtension", "SurroundingObserverHuskExtension" }
...
local GLOBAL_CONCEPT_NAMES = { heard_speak = true }



SurroundingAwareSystem = class(SurroundingAwareSystem, ExtensionSystemBase)

function SurroundingAwareSystem:init(entity_system_creation_context, system_name)
...
function SurroundingAwareSystem:destroy()
...
function SurroundingAwareSystem.add_event(unit, event_name, distance, ...)
...
function SurroundingAwareSystem:add_system_event(unit, event_name, distance, ...)
	distance = distance or DialogueSettings.default_hear_distance
...
local dummy_input = { }
function SurroundingAwareSystem:on_add_extension(world, unit, extension_name)
	local extension = {
...
function SurroundingAwareSystem:extensions_ready(world, unit, extension_name)
	if extension_name ~= "SurroundingObserverExtension" then if extension_name == "SurroundingObserverHuskExtension" then --[[ Nothing --]]
...
function SurroundingAwareSystem:on_remove_extension(unit, extension_name)
...
function SurroundingAwareSystem:update(context, t)
...
local function check_raycast_center(physics_world, unit, target, ray_position, ray_direction, ray_length, collision_filter)
	...
	"all",
	"types", "both",
	"collision_filter", collision_filter or "filter_lookat_object_ray")
	if hits then
		local num_hits = #hits
		for i = 1, num_hits do
...
local found_units = { }
function SurroundingAwareSystem:update_lookat(context, t)
	...
	local previous_seen_observer = seen_observers [unit]
	local closest_observer_utility = math.huge local closest_observer_unit = nil
...
function SurroundingAwareSystem:update_debug(context, t)
	if not script_data.dialogue_debug_lookat then return
	end
	local game = self.game
	local player = Managers.player:local_player()
	if not player or not player.player_unit or not game then return
	end
	...
	for i = 1, num_nearby do
		...
		if target ~= player_unit then
			...
			if lookat_target_ext.is_lookat_object or is_server and observers [target] then
				...
				if angle then debug_text = string.format(debug_text .. "| ANGLE: %.2f/%.2f", math.radians_to_degrees(angle), math.radians_to_degrees(max_angle))
				end
				if in_range and not darkness_system:is_in_darkness(target_center) then
					local observer_to_target_length = Vector3.length(observer_to_target_vector)
					local collision_filter = lookat_target_ext.collision_filter
...
function SurroundingAwareSystem:update_events(context, t)
	...
	while num_event_data >= i do
		...
		if Unit.alive(unit) then
			...
			for j = 1, n_targets do
				...
				found_units [j] = nil
				if
				target ~= unit and ScriptUnit.has_extension(target, "dialogue_system") then
...
function SurroundingAwareSystem:update_seen_recently(context, t)
	local seen_recently = self.seen_recently
...
function SurroundingAwareSystem:hot_join_sync(sender)
	return end