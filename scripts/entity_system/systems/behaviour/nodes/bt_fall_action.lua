...
function BTFallAction:init(...)
	BTFallAction.super.init(self, ...)
end

BTFallAction.name = "BTFallAction"

function BTFallAction:enter(unit, blackboard, t)
...
function BTFallAction:leave(unit, blackboard, t, reason, destroy)
...
function BTFallAction:run(unit, blackboard, t, dt)
	...
	elseif blackboard.fall_state == "waiting_to_land" and
	blackboard.jump_climb_finished then
		return "done"
	end
