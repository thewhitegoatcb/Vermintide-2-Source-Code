...
function NavigationUtils.create_exclusive_box_obstacle_from_unit_data(nav_world, unit)
	...
	return NavigationUtils.create_exclusive_box_obstacle_from_mesh(nav_world, unit, is_exclusive, color, has_layer, layer_idx,
	has_smartobject, smartobject_idx, mesh_name, padding_x, padding_y, padding_z)
end

function NavigationUtils.create_exclusive_box_obstacle_from_mesh(nav_world, unit, is_exclusive, color, has_layer, layer_idx, has_smartobject, smartobject_idx, mesh_name, padding_x, padding_y, padding_z)
	...
	local obstacle = GwNavBoxObstacle.create(nav_world, position, local_center, size,
	is_exclusive, color, layer_idx, smartobject_idx)

	return obstacle, transform
end

function NavigationUtils.debug_draw_nav_mesh(nav_world, nav_cost_maps_data, nav_cost_maps_count, world, line_object)
...
function NavigationUtils.get_closest_index_on_spline(spline_curve, position)
	local splines = spline_curve:splines()
	local smallest_distance_sq = math.huge local point = nil local best_index = 1
...
function NavigationUtils.get_position_on_interpolated_spline(spline_curve, position)
	local Vector3_distance_squared = Vector3.distance_squared
	local splines = spline_curve:splines()
	local num_splines = #splines
	local best_distance_sq = math.huge local best_spline_index, best_subdivision_index = nil
