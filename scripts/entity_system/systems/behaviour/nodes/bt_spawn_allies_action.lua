...
function BTSpawnAllies:enter(unit, blackboard, t)
...
function BTSpawnAllies:leave(unit, blackboard, t, reason)
	local nav_ext = blackboard.navigation_extension
...
function BTSpawnAllies:_activate_ward(unit, blackboard)
...
function BTSpawnAllies.find_spawn_point(unit, blackboard, action, data, override_spawn_group)
	...
	if num_players > 0 then
		...
		local pos1 = SPAWN_POS_TEMP [best_index] local pos2 = SPAWN_POS_TEMP [best_other_index]
		...
		local above = 0.25 local below = 10
		local success, z = nil
...
function BTSpawnAllies:_spawn(unit, data, blackboard, t)
	...
	if action.difficulty_spawn_list or action.spawn_list then
		...
		for i = 1, #spawn_list do
			...
			spawner_system:spawn_horde(unit, { spawn_list [i] }, side_id)
		end
	end
	...
	if action.difficulty_spawn or action.spawn then
		...
		local group_template = { size = 0, template = "horde",
...
function BTSpawnAllies:run(unit, blackboard, t, dt)
...
function BTSpawnAllies:_release_animation_lock(unit, blackboard)
