...
function StateDedicatedServerInit:on_enter(params)
	self:_init_network()
end

function StateDedicatedServerInit:_init_network()
...
function StateDedicatedServerInit:_load_save_data()
...
function StateDedicatedServerInit:cb_save_data_loaded(info)
	if info.error then
...
function StateDedicatedServerInit:update(dt, t)
	...
	elseif state == "wait_for_connect" then
		...
		if server_state == "connected" then
			...
			do return StateDedicatedServerRunning end
