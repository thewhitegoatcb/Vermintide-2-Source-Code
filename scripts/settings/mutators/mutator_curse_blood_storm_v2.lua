...
local DIFFICULTY_POWER_LEVEL = { harder = 60, hard = 45, normal = 30, hardest = 80, cataclysm = 100, cataclysm_3 = 130, cataclysm_2 = 110, easy = 20 }
...
local STORM_STATES = { COOLDOWN = "COOLDOWN", ACTIVE = "ACTIVE", READY = "READY" }
...
function Storm:init(vortex_template_name, inner_decal_unit_name, outer_decal_unit_name, min_cooldown, max_cooldown, logging_prefix)
...
function Storm:update(dt, t)
	...
	elseif self._state == STORM_STATES.READY then --[[ Nothing --]]
...
function Storm:spawn(spawn_position)
...
function Storm:get_state()
	return self._state
end

function Storm:get_position()
...
return { description = "curse_blood_storm_desc", curse_package_name = "resource_packages/mutators/mutator_curse_blood_storm", display_name = "curse_blood_storm_name", icon = "deus_curse_khorne_01",
		...
		for storm_index = 1, #storms do
			...
			if state == STORM_STATES.READY then
				...
				if random_player then
					...
					MAX_DISTANCE,
					1,
					forbidden_position_list,

					DISTANCE_TO_FORBIDDEN_POSITION_LIST)
