...
function BuffUtils.apply_buff_tweak_data(buffs, tweak_data)
...
function BuffUtils.copy_talent_buff_names(buffs)
	for name, buff_data in pairs(buffs) do
...
function BuffUtils.get_max_stacks(buff_name, buff_index)
...
function BuffUtils.remove_stacked_buffs(buffed_unit, stacked_buff_ids)
...
function BuffUtils.buffs_from_rpc_params(num_buffs, buff_ids, buff_data_type_ids, buff_values)
	...
	for i = 1, num_buffs do
		...
		buffs [name] = { [data_type] = value }
	end

	return buffs
end

function BuffUtils.buffs_to_rpc_params(buffs)
	local lookup_templates = NetworkLookup.buff_templates
	local lookup_data_types = NetworkLookup.buff_data_types
	...
	return { num_buffs, buff_ids, buff_data_type_ids, buff_values }
end
...
function BuffUtils.create_attached_particles(world, particle_fx, unit, is_first_person)
...
function BuffUtils.destroy_attached_particles(world, fx_ids)
...
function BuffUtils.create_liquid_forward(unit, buff)
	if ALIVE [unit] then
...
function BuffUtils.generate_balefire_burn_variants(buff_templates)
...
function BuffUtils.generate_infinite_burn_variants(buff_templates)
	for template_name, template in pairs(buff_templates) do
