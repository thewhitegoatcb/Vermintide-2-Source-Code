...
for _, name in ipairs(AvailableJourneyOrder) do
	...
	local completed_journeys_difficulty_definition = { value = 0, source = "player_data", sync_to_host = true, database_name = journey_difficulty_name }
...
for _, name in pairs(DEUS_GOD_TYPES) do
	...
	local completed_journey_dominant_god_difficulty_definition = { value = 0, source = "player_data", sync_to_host = true, database_name = god_difficulty_name }
...
for _, hero in ipairs(SPProfilesAbbreviation) do
	...
	for _, name in ipairs(AvailableJourneyOrder) do
		...
		local completed_hero_journey_difficulty_definition = { value = 0, source = "player_data", sync_to_host = true, database_name = database_name }
...
for _, chest_type in pairs(DEUS_CHEST_TYPES) do
	...
	player.opened_shrines [chest_type] = { value = 0, source = "player_data", database_name = database_name } end