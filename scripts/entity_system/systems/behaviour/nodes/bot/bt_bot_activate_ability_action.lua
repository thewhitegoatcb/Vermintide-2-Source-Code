...
function BTBotActivateAbilityAction:init(...)
...
function BTBotActivateAbilityAction:enter(unit, blackboard, t)
...
function BTBotActivateAbilityAction:leave(unit, blackboard, t, reason, destroy)
	local activate_ability_data = blackboard.activate_ability_data
	if activate_ability_data.activation.action == "aim_at_target" then
		local input_extension = blackboard.input_extension
...
function BTBotActivateAbilityAction:_start_ability(activate_ability_data, blackboard, t)
	local started = false
	...
	elseif activate_ability_data.is_weapon_ability then
		...
		if wielded_slot_data.id ~= "slot_career_skill_weapon" then
			started = false do_start_input = true
		else
			started = true do_start_input = false
		end
	else
		started = true do_start_input = false
	end
...
function BTBotActivateAbilityAction:_cancel_ability(activate_ability_data, blackboard, t)
	local input_extension = blackboard.input_extension
	input_extension:cancel_ability()
end

function BTBotActivateAbilityAction:_perform_wait_action(activate_ability_data, unit, blackboard)
...
function BTBotActivateAbilityAction:_evaluate_end_condition(activate_ability_data, unit, blackboard, t)
...
function BTBotActivateAbilityAction:run(unit, blackboard, t, dt)
