...
local extensions = { "LightSourceExtension", "PlayerUnitDarknessExtension", "ShadowFlareExtension" }





local RPCS = { "rpc_shadow_flare_done" }
...
function DarknessSystem:init(entity_system_creation_context, system_name)
...
function DarknessSystem:set_player_light_intensity(intensity)
...
function DarknessSystem:on_add_extension(world, unit, extension_name, extension_init_data)
	...
	local extension = { intensity = extension_init_data and extension_init_data.intensity or script_data_intensity or 1 }
...
function DarknessSystem:update(context, t)
	local dt = context.dt
...
local IN = nil LIGHT_LIGHT_VALUE = 0.05
...
function DarknessSystem:_update_darkness_fx(dt, t)
...
function DarknessSystem:calculate_light_value(position, player_units)
	local light_value = 0
...
function DarknessSystem:_update_shadow_flare_extensions(dt, t)
...
function DarknessSystem:remove_mutator_torches()
...
function DarknessSystem:shadow_flares_on_ground()
	return Managers.state.entity:get_entities("ShadowFlareExtension")
end

function DarknessSystem:rpc_shadow_flare_done(channel_id, unit_id)
