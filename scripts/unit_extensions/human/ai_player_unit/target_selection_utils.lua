...
local raycast_points = { "j_hips", "j_leftforearm", "j_rightforearm", "j_head" }
...
local function _line_of_sight_from_random_point(from_pos, target_unit)
	...
	if has_node then
		...
		if HEAR_DISTANCE < distance then
			...
			local result, pos = PhysicsWorld.immediate_raycast(
			physics_world,
			from_pos,
			direction,
			distance,
...
function PerceptionUtils.pick_closest_target(ai_unit, blackboard, breed)
...
function PerceptionUtils.pick_closest_target_with_filter(ai_unit, blackboard, breed)
	local ai_pos = POSITION_LOOKUP [ai_unit]
	local closest_enemy = nil
	local closest_dist = math.huge
...
function PerceptionUtils.pick_closest_vortex_target(ai_unit, blackboard, breed)
	local ai_pos = POSITION_LOOKUP [ai_unit]
	local closest_enemy = nil
	local closest_dist = math.huge
...
function PerceptionUtils.pick_boss_sorcerer_target(ai_unit, blackboard, breed)
	local ai_pos = POSITION_LOOKUP [ai_unit]
	local closest_enemy = nil
	local closest_dist = math.huge
...
function PerceptionUtils.pick_closest_target_infinte_range(ai_unit, blackboard, breed)
	local ai_pos = POSITION_LOOKUP [ai_unit]
	local closest_enemy = nil
	local closest_dist = math.huge
...
function PerceptionUtils.healthy_players(unit, blackboard, breed)
...
function PerceptionUtils.pick_ninja_skulking_target(unit, blackboard, breed)
	return PerceptionUtils.pick_solitary_target(unit, blackboard, breed)
end



function PerceptionUtils.pick_ninja_approach_target(unit, blackboard, breed)
...
function PerceptionUtils.pick_solitary_target(unit, blackboard, breed)
	local pos = POSITION_LOOKUP [unit]
	local closest_enemy = nil
	local closest_dist = math.huge
	local urgency_to_engage = 0
...
local dogpile_score = { [0] = 0, 4, 9, 16, 25, 36 }
...
local function get_lean_target(blackboard, position, side, ai_unit, check_for_walls, t, ignored_breed_filter)
	...
	local num_ai_units, target_unit = nil



	local lean_unit_list = blackboard.lean_unit_list

	local unit_index, time_index, score_index = nil
	...
	if HEALTH_ALIVE [target_unit] then
		...
		if target_unit == best_target_unit then --[[ Nothing --]]

		else

			target_blackboard = BLACKBOARDS [target_unit]
			local target_breed_name = target_blackboard.breed.name
			if ignored_breed_filter [target_breed_name] then --[[ Nothing --]]

			else

				local dogpile = target_blackboard.lean_dogpile
				if target_blackboard.crowded_slots <= dogpile then --[[ Nothing --]]
	...
	if best_target_unit then
		do return best_target_unit, best_score * 0.95 end
	else
...
function PerceptionUtils.horde_pick_closest_target_with_spillover(ai_unit, blackboard, breed, t)
	...
	for target_unit, end_of_override_t in pairs(override_targets) do
		...
		local is_valid = nil
		is_valid =
		is_player and valid_players [target_unit] or

		enemy_units [target_unit] and HEALTH_ALIVE [target_unit]
...
function PerceptionUtils.pick_closest_target_near_detection_source_position(ai_unit, blackboard, breed, t)
	local side = blackboard.side
...
function PerceptionUtils.pick_best_target_near_commander_target(ai_unit, blackboard, breed, t)
...
function PerceptionUtils.attack_commander_target_with_fallback(ai_unit, blackboard, breed, t)
	local commander_target = blackboard.commander_target
	if not HEALTH_ALIVE [commander_target] then
		blackboard.override_target_selection_name = nil
		return
	...
	if bb.crowded_slots <= dogpile then
...
function PerceptionUtils.pick_closest_target_with_spillover(ai_unit, blackboard, breed, t)
	fassert(ScriptUnit.has_extension(ai_unit, "ai_slot_system"), "Error! Trying to use slot_system perception for non-slot system unit!")
...
function PerceptionUtils.patrol_passive_target_selection(ai_unit, blackboard, breed, t)
	...
	blackboard_target and HEALTH_ALIVE [blackboard_target] and (not VALID_ENEMY_PLAYERS_AND_BOTS [blackboard_target] or VALID_ENEMY_TARGETS_PLAYERS_AND_BOTS [blackboard_target]) then
		group_targets [blackboard_target] = true


		local attacker_pos = POSITION_LOOKUP [blackboard_target]
		for i = 1, num_global_targets do repeat
				...
				if distance_sq < detection_radius_sqr then
					group_targets [target_unit] = true end until true
		end
	end
			...
			if distance_sq < detection_radius_sqr then
				...
				if view_cone_dot < dot and not is_bot or distance_sq < breed.panic_close_detection_radius_sq then
					...
					if see_you then
						...
						best_target_unit = target_unit
						distance_to_target_sq = distance_sq end end end until true
	end
	...
	local detection_radius = nil return detection_radius
end

function PerceptionUtils.storm_patrol_death_squad_target_selection(ai_unit, blackboard, breed, t)
	fassert(ScriptUnit.has_extension(ai_unit, "ai_slot_system"), "Error! Trying to use slot_system perception for non-slot system unit!")
	...
	for target_unit, _ in pairs(group_targets) do repeat
			local status_extension = ScriptUnit.has_extension(target_unit, "status_system")
			local is_player = status_extension
			...
			if
			is_player and not valid_players_and_bots [target_unit] then
				group_targets [target_unit] = nil



			elseif status_extension.using_transport or status_extension.spawn_grace then
				group_targets [target_unit] = nil
			else
				...
				if status_extension:is_knocked_down() and 5 or 0 or
				not HEALTH_ALIVE [target_unit] then
					group_targets [target_unit] = nil
				else
					...
					if best_score > score then
						best_score = score
						best_target_unit = target_unit end end end until true
	end
...
function PerceptionUtils.pick_encampment_target_idle(unit, blackboard, breed, t)
...
function PerceptionUtils.pick_closest_target_with_spillover_wakeup_group(unit, blackboard, breed, t)
	local attacker = blackboard.previous_attacker
...
function PerceptionUtils.pick_player_controller_allied(unit, blackboard, breed, t)
...
function PerceptionUtils.pick_rat_ogre_target_idle(unit, blackboard, breed, t)
...
local score_table = { }
function PerceptionUtils.pick_rat_ogre_target_with_weights(unit, blackboard, breed, t)
	local vector3_distance = Vector3.distance
	...
	if best_score < infighting.trigger_minion_target_search then
		...
		if infight_wanted_enemy then
			wanted_dist = vector3_distance(pos, POSITION_LOOKUP [infight_wanted_enemy]) wanted_enemy = infight_wanted_enemy
		end
	end
...
function PerceptionUtils.pick_bestigor_target_with_weights(unit, blackboard, breed, t)
	local vector3_distance = Vector3.distance
	local wanted_enemy = nil
	local wanted_dist = math.huge
	local is_valid_target_func = blackboard.valid_target_func or GenericStatusExtension.is_ogre_target
			...
			if VALID_ENEMY_TARGETS_PLAYERS_AND_BOTS [enemy_unit] and is_valid_target_func(status_extension) then
				...
				if not breed.ignore_targets_outside_detection_radius or blackboard.target_unit or distance_valid_target then
					...
					if best_score < score then
						wanted_enemy = enemy_unit
						wanted_dist = dist
						best_score = score end end end until true
	end
...
function PerceptionUtils.pick_chaos_troll_target_with_weights(unit, blackboard, breed, t)
...
function PerceptionUtils.debug_ai_perception(unit, ai_ext, blackboard, t, gui, x1, y1)
	...
	local borderx = 20 local bordery = y1 + 20
...
function PerceptionUtils.debug_rat_ogre_perception(gui, t, x1, y1, blackboard)
	local tiny_font_size = 16
	local tiny_font = "arial"
	local tiny_font_mtrl = "materials/fonts/" .. tiny_font
	local resx = RESOLUTION_LOOKUP.res_w local resy = RESOLUTION_LOOKUP.res_h
	local borderx = 20 local bordery = y1 + 10
...
function PerceptionUtils.pick_pack_master_target(unit, blackboard, breed)
...
function PerceptionUtils.pick_mutator_sorcerer_target(unit, blackboard, breed)
...
function PerceptionUtils.pick_corruptor_target(unit, blackboard, breed)
...
function double_raycast(blackboard, from, cast_template, enemy_unit, physics_world)
	...
	local is_hit, hit_pos = PhysicsWorld.immediate_raycast(
	physics_world,
	from,
	...
	if is_hit then
		...
		is_hit, hit_pos = PhysicsWorld.immediate_raycast(
		physics_world,
...
function PerceptionUtils.pick_ratling_gun_target(unit, blackboard, old_target, view_cone_dot_product, forward_direction)
...
function PerceptionUtils.pick_warpfire_thrower_target(unit, blackboard, old_target)
	local world = blackboard.world
	local physics_world = World.get_data(world, "physics_world")
	local target_unit = blackboard.target_unit
...
function PerceptionUtils.raycast_spine_to_spine(from_unit, to_unit, physics_world)
	...
	local result, hit_position, _, _, actor = PhysicsWorld.immediate_raycast(
	physics_world,
	unit_position,
	normalized_direction,
	distance,
	"closest",
	"collision_filter", "filter_ai_line_of_sight_check")
	...
	if actor then
		...
		do return hit_unit == to_unit end
	else
		return true
	end
end
...
function PerceptionUtils.is_position_in_line_of_sight(unit, from_position, target_position, physics_world, collision_filter)
	...
	local result, hit_position, hit_distance, normal, actor = PhysicsWorld.immediate_raycast(
	physics_world,
	from_position,
	direction,
	distance,
...
function PerceptionUtils.has_line_of_sight_to_any_player(unit, optional_z_offset)
...
function PerceptionUtils.position_has_line_of_sight_to_any_player(position)
...
function PerceptionUtils.pick_area_target(unit, blackboard, breed, radius, max_distance)
	local target = nil
...
function PerceptionUtils._find_circles(unit, target_positions, radius, max_distance)
		...
		for j = 2, #positions do
			if i < j then
				...
				if distance < radius * 2 then
					...
					circles [#circles + 1] = { targets = 0, pos = n1, distance = Vector3.distance(n1, unit_position) }
					circles [#circles + 1] = { targets = 0, pos = n2, distance = Vector3.distance(n2, unit_position) }
				end
			end
		end
	end
...
function PerceptionUtils._pick_best_circle(circles, positions, radius)
...
function PerceptionUtils.debug_draw_pick_area_target(circles, target, radius, positions)
	local drawer = Managers.state.debug:drawer({ mode = "retained", name = "pick_area_target" })
...
function PerceptionUtils.debug_draw_throw_trajectory(points)
	local drawer = Managers.state.debug:drawer({ mode = "retained", name = "projectile_unit_debug" })
