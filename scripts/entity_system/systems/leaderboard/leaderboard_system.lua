...
local RPCS = { "rpc_client_leaderboard_register_score" }
...
if Leaderboard then local EXTRA_DATA_TEMPLATE = { Leaderboard.UINT(32), Leaderboard.UINT(32), Leaderboard.UINT(32), Leaderboard.UINT(4), Leaderboard.UINT(4), Leaderboard.UINT(4), Leaderboard.UINT(4) } end
...
function LeaderboardSystem:init(entity_system_creation_context, system_name)
...
function LeaderboardSystem:update(context, t)
	local tokens = self.transaction_tokens
	for token, data in pairs(tokens) do
...
function LeaderboardSystem:round_started(score_type, data)
...
function LeaderboardSystem:debug_simulate_wave_score_enty(wave, time, nr_players)

	local wave_score = calculate_wave_score(wave, time)
	local human_players = { }

	local net_ids = { "1", "2", "3", "4" }
	local hero_ids = { 1, 2, 3, 4 }
...
function LeaderboardSystem:round_completed()
	if not self.is_server or not STEAM_AVAILABLE then
		return
	end
	...
	local data = { completed_time = end_t, nr_waves_completed = nr_waves_completed }
	...
	local player_data = { 0, 0, 0, 0, 0, 0, 0, 0 }
...
function LeaderboardSystem:register_score(level_key, difficulty_name, score, human_players)
	...
	local extra_data = { 0, 0, 0, 0, 0, 0, 0 }
	...
	self.transaction_tokens [token] = { name = "register_token" }
...
function LeaderboardSystem:get_ranking_range(level_key, difficulty_name, callback, start_range, num_ranks)
	...
	self.transaction_tokens [token] = { name = "ranking_range_token", callback = callback }
end

function LeaderboardSystem:get_ranking_around_self(level_key, difficulty_name, callback, ranks_before, ranks_after)
	if not STEAM_AVAILABLE then
		return
	end
	...
	self.transaction_tokens [token] = { name = "ranking_around_self_token", callback = callback }
end

function LeaderboardSystem:get_ranking_for_friends(level_key, difficulty_name, callback)
	if not STEAM_AVAILABLE then
		return
	end
	...
	self.transaction_tokens [token] = { name = "ranking_for_friends", callback = callback }
end
...
function LeaderboardSystem:rpc_client_leaderboard_register_score(channel_id, level_key_id, difficulty_id, score, ...)
