...
local RPCS = { "rpc_create_attachment", "rpc_remove_attachment", "rpc_add_attachment_buffs" }





local extension_list = { "PlayerUnitAttachmentExtension", "PlayerHuskAttachmentExtension" }




function AttachmentSystem:init(entity_system_creation_context, system_name)
...
function AttachmentSystem:destroy()
	self.network_event_delegate:unregister(self)
	self.network_event_delegate = nil
end

function AttachmentSystem:on_add_extension(world, unit, extension_name, extension_init_data)
...
function AttachmentSystem:create_attachment(unit, slot_id, item_name_id)
...
function AttachmentSystem:remove_attachment(unit, slot_id)
	local attachment_extension = ScriptUnit.extension(unit, "attachment_system")
	local slot_name = NetworkLookup.equipment_slots [slot_id]
	attachment_extension:remove_attachment(slot_name)
end

function AttachmentSystem:add_attachment_buffs(unit, slot_id, num_buffs, buff_ids, buff_value_type_ids, buff_values)
	local slot_name = NetworkLookup.equipment_slots [slot_id]
...
function AttachmentSystem:rpc_create_attachment(channel_id, unit_go_id, slot_id, item_name_id)
...
function AttachmentSystem:rpc_remove_attachment(channel_id, unit_go_id, slot_id)
	if self.is_server then
		local peer_id = CHANNEL_TO_PEER_ID [channel_id]
...
function AttachmentSystem:rpc_add_attachment_buffs(channel_id, go_id, slot_id, num_buffs, buff_ids, buff_value_type_ids, buff_values)
	if self.is_server then
		local peer_id = CHANNEL_TO_PEER_ID [channel_id]
