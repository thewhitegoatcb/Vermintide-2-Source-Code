...
StatisticsTemplates.multikill = {
	config = { kills_to_get = 1, time_window = 10 },



	init = function ()
		local data = { kills_total_last = 0, kill_times_n = 0,
	...
	update = function (unit, extension, context, t)
		...
		if kills_total <= kills_total_last then return
		end
...
for name, cat in pairs(StatisticsTemplateCategories) do
	assert(StatisticsTemplates [name] == nil, "Statistics templates: Can't have category with the same name as a template") end