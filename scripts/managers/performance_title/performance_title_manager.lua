...
local RPCS = { "rpc_sync_performance_titles" }
...
local function networkify_number(number)
	...
	local min = uint_16.min local max = uint_16.max

	return math.clamp(number, min, max)
end

function PerformanceTitleManager:init(network_transmit, statistics_db, is_server)
...
function PerformanceTitleManager:register_rpcs(network_event_delegate)
	network_event_delegate:register(self, unpack(RPCS))
	self._network_event_delegate = network_event_delegate
end

function PerformanceTitleManager:unregister_rpcs()
	self._network_event_delegate:unregister(self)
	self._network_event_delegate = nil
end

function PerformanceTitleManager:destroy()
	self._statistics_db = nil

	self._network_transmit = nil
end

function PerformanceTitleManager:assigned_titles()
	return self._assigned_titles
end


function PerformanceTitleManager:_evaluate_player_titles(player, eligible_titles)
...
function PerformanceTitleManager:_get_title_list_from_player_titles(player_titles)
	local title_list = { }
...
function PerformanceTitleManager:_find_individually_achieved_title(player_titles, title_name)
...
function PerformanceTitleManager:_assign_title(assigned_titles, player_titles, player, title_name)
...
function PerformanceTitleManager:_remove_title_from_player_titles(player_titles, title_name)
	for _, titles in pairs(player_titles) do
...
function PerformanceTitleManager:_assign_individual_titles(player_titles, assigned_titles)
...
function PerformanceTitleManager:_assign_compared_titles(player_titles, assigned_titles)
	local title_list = self:_get_title_list_from_player_titles(player_titles)
	...
	for _, title_name in ipairs(title_list) do
		local best_amount = 0 local winner_player = nil
...
function PerformanceTitleManager:_sync_assigned_titles(assigned_titles)
	local peer_ids = { }
...
function PerformanceTitleManager:evaluate_titles(players)
...
function PerformanceTitleManager:_translate_title_assignment(peer_id, local_player_id, title_id, amount)
...
function PerformanceTitleManager:rpc_sync_performance_titles(channel_id, peer_ids, local_player_ids, title_ids, amounts)
	local assigned_titles = { }
