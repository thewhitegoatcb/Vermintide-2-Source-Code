...
function StoreWindowCategoryList:on_enter(params, offset)
...
function StoreWindowCategoryList:_start_transition_animation(animation_name)
	local params = {
...
function StoreWindowCategoryList:_create_ui_elements(params, offset)
...
function StoreWindowCategoryList:on_exit(params)
	print("[HeroViewWindow] Exit Substate StoreWindowCategoryList")
	self._ui_animator = nil
end

function StoreWindowCategoryList:update(dt, t)
	self:_update_animations(dt)
	self:_draw(dt)
end

function StoreWindowCategoryList:post_update(dt, t)
...
function StoreWindowCategoryList:_update_animations(dt)
...
function StoreWindowCategoryList:_is_list_hovered()
	local list_mask = self._widgets_by_name.list
	return list_mask.content.list_hotspot.is_hover or false
end

function StoreWindowCategoryList:_handle_input(dt, t)
...
function StoreWindowCategoryList:_draw(dt)
...
function StoreWindowCategoryList:_play_sound(event)
	self._parent:play_sound(event)
end

function StoreWindowCategoryList:_update_gamepad_focus()
...
function StoreWindowCategoryList:_handle_gamepad_activity()
...
function StoreWindowCategoryList:_get_items_by_filter(item_filter)
...
function StoreWindowCategoryList:_get_all_items()
	local backend_store = Managers.backend:get_interface("peddler")
...
function StoreWindowCategoryList:_get_items_by_path(path)
	local item_filter = ""
...
function StoreWindowCategoryList:_populate_list(layout)
	local widgets = { }
...
function StoreWindowCategoryList:_align_entry_widgets()
...
function StoreWindowCategoryList:_list_index_pressed()
	local list_widgets = self._list_widgets
	if list_widgets then
...
function StoreWindowCategoryList:_animate_list_entries(dt)
	local parent = self._parent
...
function StoreWindowCategoryList:_animate_list_entry(content, style, dt, list_hovered)
	local hotspot = content.button_hotspot or content.hotspot
...
function StoreWindowCategoryList:_setup_list_elements()
	local parent = self._parent
...
function StoreWindowCategoryList:_on_list_index_pressed(index)
...
function StoreWindowCategoryList:_on_list_index_selected(index, scrollbar_animation_percentage)
	local layout = self._layout
	local entry = layout [index]
...
function StoreWindowCategoryList:_handle_gamepad_grid_selection(input_service)
...
function StoreWindowCategoryList:_find_closest_neighbour(column_index_list, current_index)
	local list_widgets = self._list_widgets
...
function StoreWindowCategoryList:_initialize_scrollbar()
...
function StoreWindowCategoryList:_update_scroll_position()
	local scrollbar_logic = self._scrollbar_logic
...
function StoreWindowCategoryList:_update_visible_list_entries()
	local scrollbar_logic = self._scrollbar_logic
...
function StoreWindowCategoryList:_scroll_to_list_index(index)
	local scrollbar_logic = self._scrollbar_logic
	local enabled = scrollbar_logic:enabled()
...
function StoreWindowCategoryList:_get_scrollbar_percentage_by_index(index)
	local scrollbar_logic = self._scrollbar_logic
	local enabled = scrollbar_logic:enabled()
