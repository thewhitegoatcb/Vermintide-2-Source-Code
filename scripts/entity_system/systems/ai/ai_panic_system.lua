...
local extensions = { "AIPanicExtension", "AIFearExtension" }
...
function AIPanicSystem:init(context, system_name)
...
function AIPanicSystem:destroy()
	return end

local dummy_input = { }
function AIPanicSystem:on_add_extension(world, unit, extension_name, extension_init_data)
	local extension = { }
...
function AIPanicSystem:on_remove_extension(unit, extension_name)
...
function AIPanicSystem:hot_join_sync(peer_id, player)
	return end

function AIPanicSystem:activate_fear(unit)
	local extension = self.unit_extension_data [unit]
...
function AIPanicSystem:register_panic_zone(position, radius)
	local panic_zone = { position = Vector3Box(position), radius_squared = radius * radius, radius = radius }
...
function AIPanicSystem:deregister_panic_zone(panic_zone_to_remove)
	local panic_zones = self.panic_zones
	local panic_zones_n = #panic_zones
...
function AIPanicSystem:set_panic_zone_position(panic_zone, position)
	panic_zone.position:store(position)
end

function AIPanicSystem:inside_panic_zone(position)
	local panic_zones = self.panic_zones
	local panic_zones_n = #panic_zones

	for i = 1, panic_zones_n do repeat
			local panic_zone = panic_zones [i]
			...
			if distance_squared <= radius_squared then
				return panic_zone end until true
	end
...
function AIPanicSystem:update_fear_units()
	local fear_units = self.fear_units
	local fear_units_n = #fear_units
	...
	for i = start_index, end_index do repeat
			...
			local panic_zone = extension.panic_zone
			local position = POSITION_LOOKUP [unit]
			self:set_panic_zone_position(panic_zone, position) until true
	end
...
function AIPanicSystem:update_panic_units()
	local panic_units = self.panic_units
	local panic_units_n = #panic_units
...
function AIPanicSystem:update(context, t, dt)
...
function AIPanicSystem:debug_draw_panic_zones()
	local drawer = Managers.state.debug:drawer({ mode = "immediate", name = "AIPanicSystem" })
	local panic_zones = self.panic_zones
