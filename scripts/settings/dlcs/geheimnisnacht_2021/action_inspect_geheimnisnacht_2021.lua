...
function ActionInspectGeheimnisnacht2021:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionInspectGeheimnisnacht2021:client_owner_start_action(new_action, t)
...
function ActionInspectGeheimnisnacht2021:client_owner_post_update(dt, t, world, can_damage)
	...
	if old_influnece < 0.7 and self._influence_str >= 0.7 then
		...
		self._first_person_extension:set_weapon_sway_settings({ recentering_lerp_speed = 250, lerp_speed = 3, sway_range = 1, camera_look_sensitivity = 0.03, look_sensitivity = 8 })
	end
	...
	if old_influnece > 0.7 and self._influence_str <= 0.7 then
		...
		self._first_person_extension:set_weapon_sway_settings({ recentering_lerp_speed = 0, lerp_speed = 10, sway_range = 1, camera_look_sensitivity = 1, look_sensitivity = 1.5 })
	end
	...
	if old_influnece < 0.9 and self._influence_str >= 0.9 then
		...
		self._first_person_extension:set_weapon_sway_settings({ recentering_lerp_speed = 10, lerp_speed = 10, sway_range = 1, camera_look_sensitivity = 1, look_sensitivity = 1.5 })
	...
	if self._take_curse_damage and
	self._next_curse_time_t <= t then
...
function ActionInspectGeheimnisnacht2021:finish(reason)
	ActionInspectGeheimnisnacht2021.super.finish(self, reason)
	self:_destroy_screen_particles()
end

function ActionInspectGeheimnisnacht2021:_create_screen_particles()
...
function ActionInspectGeheimnisnacht2021:_destroy_screen_particles()
	if self._screen_fx_id then
