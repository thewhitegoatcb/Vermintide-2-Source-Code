...
function ChaosTrollHuskHealthExtension:init(extension_init_context, unit, ...)
...
function ChaosTrollHuskHealthExtension:set_max_health(value, set_only_current_max_health)
...
function ChaosTrollHuskHealthExtension:_setup_initial_health_variables(new_max_health, from_init)
...
function ChaosTrollHuskHealthExtension:current_max_health_percent()
	return self.health / self.current_max_health
end

function ChaosTrollHuskHealthExtension:update(dt, context, t)
...
function ChaosTrollHuskHealthExtension:apply_client_predicted_damage(predicted_damage)

	return end

function ChaosTrollHuskHealthExtension:add_damage(attacker_unit, damage_amount, hit_zone_name, damage_type, hit_position, damage_direction, damage_source_name, hit_ragdoll_actor, source_attacker_unit, hit_react_type, is_critical_strike, added_dot, first_hit, total_hits, attack_type)
...
function ChaosTrollHuskHealthExtension:add_heal(healer_unit, heal_amount, heal_source_name, heal_type)
	local unit = self.unit
...
function ChaosTrollHuskHealthExtension:sync_damage_taken(damage, set_max_health, state)
...
function ChaosTrollHuskHealthExtension:rpc_sync_current_max_health(channel_id, go_id, new_max_health)
	local game_object_id = self.game_object_id or Managers.state.unit_storage:go_id(self.unit)
	if game_object_id ~= go_id then return end
	self.current_max_health = DamageUtils.networkify_health(new_max_health)
end

function ChaosTrollHuskHealthExtension:destroy()
