...
function BTSwarmAction:init(...)
	BTSwarmAction.super.init(self, ...)
end

function BTSwarmAction:enter(unit, blackboard, t)
...
function BTSwarmAction:_calculate_swarm_targets(unit, blackboard)
...
function BTSwarmAction:leave(unit, blackboard, t, reason, destroy)
...
function BTSwarmAction:anim_cb_damage(unit, blackboard)
...
function BTSwarmAction:anim_cb_attack_finished(unit, blackboard)
	blackboard.attack_finished = true
end

function BTSwarmAction:run(unit, blackboard, t, dt)
	...
	if not valid_target then
		local valid_action = self:_calculate_swarm_targets(unit, blackboard)
		if not valid_action then return "done" end
