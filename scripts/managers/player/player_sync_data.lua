...
PrivacyLevels = table.mirror_array_inplace({ "private", "friends", "public" })
...
function PlayerSyncData:init(player, network_manager)
	self._player = player
	...
	if player.local_player or player.bot_player and player.is_server then
		...
		local game_object_data_table = { best_aquired_power_level = 0, power_level = 0,
...
function PlayerSyncData:_on_game_options_changed()
	self:set_data("playerlist_build_privacy", Application.user_setting("playerlist_build_privacy"))
end

function PlayerSyncData:_calc_highest_unlocked_difficulty()
...
function PlayerSyncData:reevaluate_highest_difficulty()
...
function PlayerSyncData:cb_game_session_disconnect()
	self._game_object_id = nil
end

function PlayerSyncData:set_game_object_id(go_id)
	self._game_object_id = go_id
end

function PlayerSyncData:active()
	return self._game_object_id ~= nil
end

function PlayerSyncData:destroy()
...
function PlayerSyncData:set_data(key, value)
	if not self._game_object_id then
		return
	end
...
function PlayerSyncData:get_data(key)
	if not self._game_object_id then
