...
local PRIORITY_REASONS = { proximity = 1, lord = 2, ping = 4, damage_taken = 3, damage_done = 4 }
...
function BossHealthUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function BossHealthUI:create_ui_elements()
...
function BossHealthUI:_set_portrait_and_title(marked, breed_name, title)
...
local skull_dividers = { "skull_divider1", "skull_divider2", false, "skull_divider3", "skull_divider4", false }
...
local small_style = { font_size = 16, upper_case = true, font_type = "hell_shark", divider_icon_width = 22 }





local large_style = { upper_case = true, divider_icon_width = 22, font_size = 20, font_type = "hell_shark",
...
function BossHealthUI:_generate_attributes(attributes, widget, current_style, max_row_width)
...
function BossHealthUI:_update_enemy_portrait_name_and_attributes(unit, breed_name)
...
local customizer_data = { root_scenegraph_id = "pivot", label = "Boss health", registry_key = "boss_health", drag_scenegraph_id = "pivot_dragger" }
...
function BossHealthUI:update(dt, t)
...
function BossHealthUI:_update_targeted_boss(dt, t)
...
function BossHealthUI:_update_animations(dt, t)
...
function BossHealthUI:_update_timed_prioritization(dt, t)
...
function BossHealthUI:_draw(dt, t)
...
function BossHealthUI:_show_boss_health_bar(unit)
	if not ALIVE [unit] then
		return
	end
...
function BossHealthUI:on_spectator_target_changed(spectated_player_unit)
	self._spectated_player_unit = spectated_player_unit
	self._spectated_player = Managers.player:owner(spectated_player_unit)
	self._is_spectator = true
end

function BossHealthUI:event_set_prioritized_unit(unit, reason)
...
function BossHealthUI:event_clear_prioritized_unit(reason)
...
function BossHealthUI:_sync_boss_health(dt, t)
...
function BossHealthUI:_set_bar_progress(progress, max_health_fraction, instant, dt, t)
...
function BossHealthUI:_set_healing_amount(start_progress, end_progress, time)
	local widget = self._widgets_by_name.bar
	local content = widget.content
	local style = widget.style
...
function BossHealthUI:_update_healing_bar(dt, t, freeze_healing)
...
function BossHealthUI:_set_health_edge_texture_position_progress(progress)
	local widget = self._widgets_by_name.bar
	local content = widget.content
	local style = widget.style
...
function BossHealthUI:_update_healing_effect(dt, t)
...
function BossHealthUI:_set_health_effect_alpha(alpha)
	local widget = self._widgets_by_name.bar
	local content = widget.content
	local style = widget.style
