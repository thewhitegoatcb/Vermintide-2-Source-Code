...
function RoomManagerServer:init(world)
	self._peer_rooms = { }
	self._room_order = { }
...
function RoomManagerServer:setup_level_anchor_points(level)
	self._room_handler:setup_level_anchor_points(level)
end

function RoomManagerServer:create_room(peer_id, local_player_id)
	...
	self._peer_rooms [peer_id] = { room_id = room_id, profile_index = profile_index }
...
function RoomManagerServer:get_spawn_point_by_peer(peer_id)
	return self._peer_rooms [peer_id].room_id
end

function RoomManagerServer:has_room(peer_id)
	return self._peer_rooms [peer_id] and true or false
end

function RoomManagerServer:destroy_room(peer_id, move_other_players_from_room)
...
function RoomManagerServer:move_players_from_room(room_id)
	...
	for _, player in pairs(players) do repeat
			...
			if Level.is_point_inside_volume(level, "room_volume", position) then
				...
				if player.local_player then
					...
					RPC.rpc_teleport_unit_to(channel_id, unit_id, spawn_pos, spawn_rot) end end until true
	end


end

function RoomManagerServer:hot_join_sync(sender)
...
function RoomManagerServer:destroy()
