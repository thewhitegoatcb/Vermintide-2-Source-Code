...
function BTWarpfireThrowerShootAction:init(...)
...
function BTWarpfireThrowerShootAction:enter(unit, blackboard, t)
...
function BTWarpfireThrowerShootAction:_init_attack(unit, target_unit, blackboard, action)
...
function BTWarpfireThrowerShootAction:_abort_shooting(t, warpfire_data)
	warpfire_data.blob_extension:stop_placing_blobs(t)
	warpfire_data.is_firing = false
end

function BTWarpfireThrowerShootAction:leave(unit, blackboard, t, reason, destroy)
...
function BTWarpfireThrowerShootAction:run(unit, blackboard, t, dt)
...
function BTWarpfireThrowerShootAction:_move_warpfire_blob(unit, warpfire_data, blackboard, action, dt)
...
function BTWarpfireThrowerShootAction:_end_align_towards_target(unit, warpfire_data, attack_pattern_data, blackboard, t)
...
function BTWarpfireThrowerShootAction:_start_align_towards_target(unit, blackboard, data, target_unit)
	data.state = "align"
	data.align_speed = 0
...
function BTWarpfireThrowerShootAction:_remaining_angle(from, to)
...
function BTWarpfireThrowerShootAction:_angle_to_speed(speed, angle_left)
...
function BTWarpfireThrowerShootAction:_update_align_towards_target(unit, blackboard, attack_pattern_data, target_unit, dt)
...
function BTWarpfireThrowerShootAction:_close_range_attack_check(blackboard, action, t)
...
function BTWarpfireThrowerShootAction:_close_range_attack(unit, attack_pattern_data, blackboard, action, t)
	...
	radius,
	max_hits,
	"collision_filter", "filter_character_trigger",
					...
					if buff_extension then
						...
						if is_player_unit then
							...
							if target_power_block_perk and target_blocking and shield_block then
								if
								is_valid_target and not DamageUtils.check_ranged_block(unit, hit_unit, dot, "blocked_berzerker") then
...
function BTWarpfireThrowerShootAction:_aim_at_target(unit, target_unit, attack_pattern_data, blackboard, action, t, dt)
	...
	elseif blackboard.line_of_sight_raycast_timer < t then
		...
		local is_hit, _ = PhysicsWorld.immediate_raycast(
		physics_world,
		self_pos + Vector3.up(),
		Vector3.normalize(self_to_target),
		Vector3.length(self_to_target),
		"closest",
...
function BTWarpfireThrowerShootAction:_rotate_from_to(from, to, max_angle_speed, dt)
...
function BTWarpfireThrowerShootAction:_calculate_wanted_target_position(unit, target_unit)
...
function BTWarpfireThrowerShootAction:_calculate_align_animation(right_vector, forward_vector, dir, attack_anims)
...
function BTWarpfireThrowerShootAction:_attack_fire(unit, warpfire_data, action, blackboard, t)
...
function BTWarpfireThrowerShootAction:_create_warpfire_blob(unit, data, action, blackboard, t)
	local attack_pattern_data = blackboard.attack_pattern_data
	local warpfire_data = blackboard.warpfire_data
	local warpfire_unit = attack_pattern_data.warpfire_gun_unit
	local target_unit = blackboard.target_unit
	...
	local extension_init_data = {
		area_damage_system = { damage_blob_template_name = "warpfire",
...
function BTWarpfireThrowerShootAction:_calculate_cylinder_collision(action, self_pos, self_rot)
...
function BTWarpfireThrowerShootAction:_create_bot_aoe_threat(unit, action)
	local unit_position = POSITION_LOOKUP [unit]
