...
local RPCS = { "rpc_deus_shop_heal_player", "rpc_deus_shop_blessing_selected", "rpc_deus_shop_power_up_bought", "rpc_deus_save_loadout", "rpc_deus_add_power_ups", "rpc_deus_set_initial_soft_currency", "rpc_deus_set_initial_setup", "rpc_deus_chest_unlocked", "rpc_deus_soft_currency_picked_up", "rpc_deus_grant_end_of_level_power_ups" }
...
local stats_to_save = { { "kills_per_breed", "skaven_storm_vermin" },
	{ "kills_per_breed", "skaven_storm_vermin_commander" },
	{ "kills_per_breed", "skaven_storm_vermin_with_shield" },
	{ "kills_per_breed", "skaven_plague_monk" },
	{ "kills_per_breed", "chaos_warrior" },
	{ "kills_per_breed", "chaos_berzerker" },
	{ "kills_per_breed", "chaos_raider" },
	{ "kills_per_breed", "beastmen_bestigor" },
	{ "kills_per_breed", "skaven_gutter_runner" },
	{ "kills_per_breed", "skaven_poison_wind_globadier" },
	{ "kills_per_breed", "skaven_pack_master" },
	{ "kills_per_breed", "skaven_ratling_gunner" },
	{ "kills_per_breed", "skaven_warpfire_thrower" },
	{ "kills_per_breed", "chaos_corruptor_sorcerer" },
	{ "kills_per_breed", "chaos_vortex_sorcerer" },
	{ "kills_per_breed", "beastmen_standard_bearer" },
	{ "kills_total" },
	{ "kills_melee" },
	{ "kills_ranged" },
	{ "damage_taken" },
	{ "damage_dealt" },
	{ "damage_dealt_per_breed", "skaven_rat_ogre" },
	{ "damage_dealt_per_breed", "skaven_stormfiend" },
	{ "damage_dealt_per_breed", "chaos_spawn" },
	{ "damage_dealt_per_breed", "chaos_troll" },
	{ "damage_dealt_per_breed", "beastmen_minotaur" },
	{ "headshots" },
	{ "saves" },
	{ "revives" } }
...
local loot_chest_rewards = {
	normal = { "loot_chest_01_06", 2 },
	hard = { "loot_chest_02_06", 2 },
	harder = { "loot_chest_03_06", 2 },
	hardest = { "loot_chest_04_06", 2 },
	cataclysm = { "loot_chest_04_06", 2 } }
...
function DeusRunController:init(run_id, is_server, network_handler, server_peer_id, own_peer_id, own_initial_loadout, own_initial_talents, weapon_group_whitelist)
...
function DeusRunController:network_context_created(lobby, server_peer_id, own_peer_id, is_server, network_handler)
...
function DeusRunController:register_rpcs(network_event_delegate)
...
function DeusRunController:unregister_rpcs()
...
function DeusRunController:full_sync()
	self._run_state:full_sync()
end

function DeusRunController:is_server()
	return self._run_state:is_server()
end

function DeusRunController:get_server_peer_id()
	return self._run_state:server_peer_id()
end

function DeusRunController:get_own_peer_id()
	return self._run_state:own_peer_id()
end

function DeusRunController:destroy()
...
function DeusRunController:get_run_ended()
	return self._run_state:get_run_ended()
end

function DeusRunController:handle_run_ended()
	self._run_state:set_run_ended(true)
end

function DeusRunController:setup_run(run_seed, difficulty, journey_name, dominant_god, initial_own_soft_currency, telemetry_id, with_belakor)
	...
	self._path_graph = deus_generate_graph(
	run_seed,
	journey_name,
	dominant_god,
	populate_config,
...
function DeusRunController:get_state_revision()
	return self._run_state:get_revision()
end


function DeusRunController:rpc_deus_set_initial_soft_currency(sender_channel_id, initial_own_soft_currency)
...
function DeusRunController:rpc_deus_set_initial_setup(sender_channel_id, profile_index, career_index, initial_talents_for_career, melee_item_string, ranged_item_string)
	local sender = CHANNEL_TO_PEER_ID [sender_channel_id]
...
function DeusRunController:rpc_deus_grant_end_of_level_power_ups(sender_channel_id, node_key)
	local path_graph = self:_get_graph_data()
	...
	_, new_power_ups = DeusPowerUpUtils.generate_random_power_ups(
	power_up_seed,
	granted_random_power_up_count,
	existing_power_ups,
	self._run_state:get_run_difficulty(),
	run_progress,
...
function DeusRunController:profile_changed(peer_id, local_player_id, profile_index, career_index)
...
function DeusRunController:_add_initial_talents_as_power_ups(peer_id, local_player_id, profile_index, career_index, initial_talents_for_career)
...
function DeusRunController:_add_initial_weapons_to_loadout(peer_id, local_player_id, profile_index, career_index, melee_item_string, ranged_item_string)
...
function DeusRunController:get_run_id()
	return self._run_state:get_run_id()
end

function DeusRunController:get_run_seed()
	return self._run_state:get_run_seed()
end

function DeusRunController:get_run_difficulty()
	return self._run_state:get_run_difficulty()
end

function DeusRunController:get_journey_name()
	return self._run_state:get_journey_name()
end

function DeusRunController:get_dominant_god()
...
function DeusRunController:handle_level_won()
...
function DeusRunController:handle_map_exited()
...
function DeusRunController:get_belakor_enabled()
	return self._run_state:get_belakor_enabled()
end

function DeusRunController:has_completed_current_node()
	local current_node_key = self._run_state:get_current_node_key()
...
function DeusRunController:handle_shrine_entered(shrine_node_key)
...
function DeusRunController:get_traversed_nodes()
	return self._run_state:get_traversed_nodes() or { }
end

function DeusRunController:get_unreachable_nodes()
	local current_node_key = self:get_current_node_key()

	local reachable_nodes = { [current_node_key] = true }
...
function DeusRunController:get_visited_nodes()
...
function DeusRunController:get_completed_level_count()
	return self._run_state:get_completed_level_count()
end

function DeusRunController:get_map_visibility()
...
function DeusRunController:get_end_of_level_rewards_arguments(game_won, quickplay)
	local completed_level_count = self._run_state:get_completed_level_count()
...
function DeusRunController:record_cursed_chest_purified()
	local peers = self._network_handler:get_peers()
...
function DeusRunController:save_persisted_score(statistics_db, peer_id, local_player_id)
...
function DeusRunController:restore_persisted_score(statistics_db, peer_id, local_player_id)
	local stats_id = PlayerUtils.unique_player_id(peer_id, local_player_id)
...
function DeusRunController:save_scoreboard(scoreboard)
	self._run_state:set_scoreboard(scoreboard)
end

function DeusRunController:get_scoreboard()
...
function DeusRunController:get_peers()
	return self._network_handler:get_peers()
end

function DeusRunController:set_own_player_avatar_info(level, name, frame)
...
function DeusRunController:get_own_loadout()
	local local_peer_id = self._run_state:get_own_peer_id()
	local profile_index, career_index = self._run_state:get_player_profile(local_peer_id, REAL_PLAYER_LOCAL_ID)
	...
	if self._destroyed then
		...
		do return melee_item, ranged_item end
	else
...
function DeusRunController:get_loadout(peer_id, local_player_id, profile_index, career_index)
...
function DeusRunController:save_loadout(item, slot_name)
	local local_peer_id = self._run_state:get_own_peer_id()
...
function DeusRunController:rpc_deus_save_loadout(sender_channel_id, slot_name, item_string)
	local sender = CHANNEL_TO_PEER_ID [sender_channel_id]
...
function DeusRunController:on_soft_currency_picked_up(amount, type)
	local own_peer_id = self._run_state:get_own_peer_id()
...
function DeusRunController:get_player_soft_currency(peer_id)
	return self._run_state:get_player_soft_currency(peer_id, REAL_PLAYER_LOCAL_ID)
end

function DeusRunController:rpc_deus_soft_currency_picked_up(sender_channel_id, amount, type)
	local sender = CHANNEL_TO_PEER_ID [sender_channel_id]
	self:_add_soft_currency_to_peer(sender, amount)
end

function DeusRunController:_add_soft_currency_to_peer(peer_id, amount)
...
function DeusRunController:get_shop_heal_data()
	local local_peer_id = self._run_state:get_own_peer_id()
...
function DeusRunController:shop_buy_health()
	local local_peer_id = self._run_state:get_own_peer_id()
	...
	success and not self._run_state:is_server() then
		local server_peer_id = self._run_state:get_server_peer_id()
		local server_channel_id = PEER_ID_TO_CHANNEL [server_peer_id]
...
function DeusRunController:shop_buy_blessing(blessing_name)
...
function DeusRunController:shop_buy_power_up(power_up, discount)
	...
	if success and not self._run_state:is_server() then
		local server_peer_id = self._run_state:get_server_peer_id()
		local server_channel_id = PEER_ID_TO_CHANNEL [server_peer_id]
		RPC.rpc_deus_shop_power_up_bought(
		server_channel_id,
		power_up.rarity,
		power_up.name,
		power_up.client_id,
		math.round(discount * DISCOUNT_CONVERSION_EPSILON))
	end

end

function DeusRunController:_try_buy_health(peer_id)
...
function DeusRunController:rpc_deus_shop_heal_player(sender_channel_id)
	local sender = CHANNEL_TO_PEER_ID [sender_channel_id]
...
function DeusRunController:rpc_deus_shop_blessing_selected(sender_channel_id, blessing_name)
	local sender = CHANNEL_TO_PEER_ID [sender_channel_id]
...
function DeusRunController:rpc_deus_shop_power_up_bought(sender_channel_id, rarity, power_up_name, power_up_client_id, discount)
	local sender = CHANNEL_TO_PEER_ID [sender_channel_id]
...
function DeusRunController:get_player_power_ups(peer_id, local_player_id)
...
function DeusRunController:get_power_ups(peer_id, local_player_id, profile_index, career_index)
	local power_ups = self._run_state:get_player_power_ups(peer_id, local_player_id, profile_index, career_index)

	return power_ups
end

function DeusRunController:get_party_power_ups()
	local power_ups = self._run_state:get_party_power_ups()

	return power_ups
end

function DeusRunController:generate_random_power_ups(count, availability_type, extra_seed)
	...
	local new_power_ups = nil
	power_up_seed, new_power_ups = DeusPowerUpUtils.generate_random_power_ups(
	power_up_seed,
	count,
	existing_power_ups,
	self._run_state:get_run_difficulty(),
...
function DeusRunController:add_power_ups(new_power_ups, local_player_id)
...
function DeusRunController:try_grant_end_of_level_deus_power_ups()
	local current_node_key = self:get_current_node_key()
	local path_graph = self:_get_graph_data()
	local current_node = path_graph [current_node_key]
	...
	if granted_random_power_up_count then
		...
		local _, new_power_ups = DeusPowerUpUtils.generate_random_power_ups(
		power_up_seed,
		granted_random_power_up_count,
		existing_power_ups,
		self._run_state:get_run_difficulty(),
...
function DeusRunController:get_arena_belakor_node()
	return self._run_state:get_arena_belakor_node()
end

function DeusRunController:has_own_seen_arena_belakor_node()
	local local_peer_id = self._run_state:get_own_peer_id()
...
function DeusRunController:rpc_deus_add_power_ups(sender_channel_id, power_ups_string, granted_by_end_of_level_node_key)
	local sender = CHANNEL_TO_PEER_ID [sender_channel_id]
...
function DeusRunController:get_blessings()
	return self._run_state:get_blessings()
end

function DeusRunController:get_blessings_with_buyer()
	return self._run_state:get_blessings_with_buyer()
end

function DeusRunController:has_blessing(blessing_name)
	local blessings = self._run_state:get_blessings()
	return table.contains(blessings, blessing_name)
end

function DeusRunController:generate_random_blessing_name()
...
function DeusRunController:remove_blessing(blessing_to_remove)
	local blessings_with_buyer = self._run_state:get_blessings_with_buyer()
...
function DeusRunController:has_power_up(peer_id, power_up_client_id)
	local profile_index, career_index = self._run_state:get_player_profile(peer_id, REAL_PLAYER_LOCAL_ID)
...
function DeusRunController:reached_max_power_ups(peer_id, power_up_name)
	local profile_index, career_index = self._run_state:get_player_profile(peer_id, REAL_PLAYER_LOCAL_ID)
...
function DeusRunController:_blessings_handle_level_won()
	local blessings_with_buyer = self._run_state:get_blessings_with_buyer()
...
function DeusRunController:_try_buy_blessing(buyer, blessing_name)
	local blessing_data = DeusBlessingSettings [blessing_name]
	...
	if blessing_data.grant_item then
		...
		local function can_add_additional_items()
			...
			if additional_item_slots then
				...
				if slot_count then
					...
					do return slot_count > current_item_count end
				else
					do return false end
				end
			else
		...
		local function add_to_additional_items()
			...
			if not additional_items [slot_name] then local slot_additional_items = {
					items = { } }
			end
...
function DeusRunController:_try_buy_power_up(buyer, power_up, discount)
...
function DeusRunController:grant_party_power_up(power_up_name, power_up_rarity)
	if not self._run_state:is_server() then
...
function DeusRunController:get_player_profile(peer_id, local_player_id)
	local profile_index, career_index = self._run_state:get_player_profile(peer_id, local_player_id)
	return profile_index, career_index
end

function DeusRunController:get_player_level(peer_id)
	return self._run_state:get_player_level(peer_id)
end

function DeusRunController:get_player_name(peer_id)
	return self._run_state:get_player_name(peer_id)
end

function DeusRunController:get_player_frame(peer_id)
...
function DeusRunController:get_player_health_state(peer_id, local_player_id)
	local profile_index, career_index = self._run_state:get_player_profile(peer_id, local_player_id)
	return self._run_state:get_player_health_state(peer_id, local_player_id, profile_index, career_index)
end

function DeusRunController:get_player_health_percentage(peer_id, local_player_id)
	local profile_index, career_index = self._run_state:get_player_profile(peer_id, local_player_id)
	return self._run_state:get_player_health_percentage(peer_id, local_player_id, profile_index, career_index)
end

function DeusRunController:get_player_melee_ammo(peer_id, local_player_id)
	local profile_index, career_index = self._run_state:get_player_profile(peer_id, local_player_id)
	return self._run_state:get_player_melee_ammo(peer_id, local_player_id, profile_index, career_index)
end

function DeusRunController:get_player_ranged_ammo(peer_id, local_player_id)
	local profile_index, career_index = self._run_state:get_player_profile(peer_id, local_player_id)
...
function DeusRunController:get_player_consumable_healthkit_slot(peer_id, local_player_id)
	local profile_index, career_index = self._run_state:get_player_profile(peer_id, local_player_id)
	return self._run_state:get_player_consumable_healthkit_slot(peer_id, local_player_id, profile_index, career_index)
end

function DeusRunController:get_player_consumable_potion_slot(peer_id, local_player_id)
	local profile_index, career_index = self._run_state:get_player_profile(peer_id, local_player_id)
	return self._run_state:get_player_consumable_potion_slot(peer_id, local_player_id, profile_index, career_index)
end

function DeusRunController:get_player_consumable_grenade_slot(peer_id, local_player_id)
	local profile_index, career_index = self._run_state:get_player_profile(peer_id, local_player_id)
	return self._run_state:get_player_consumable_grenade_slot(peer_id, local_player_id, profile_index, career_index)
end

function DeusRunController:get_player_persistent_buffs(peer_id, local_player_id)
	local profile_index, career_index = self._run_state:get_player_profile(peer_id, local_player_id)
	return self._run_state:get_player_persistent_buffs(peer_id, local_player_id, profile_index, career_index)
end

function DeusRunController:restore_game_mode_data(peer_id, local_player_id, profile_index, career_index)
	local data = { }
	...
	if not spawned_once then
		...
		data.ammo = { slot_ranged = 1, slot_melee = 1 }
...
function DeusRunController:save_game_mode_data(peer_id, local_player_id, profile_index, career_index, data)
...
function DeusRunController:save_persistent_buffs(peer_id, local_player_id, profile_index, career_index, persistent_buffs)
...
function DeusRunController:get_graph_data()
	return self:_get_graph_data()
end
function DeusRunController:_get_graph_data()
...
function DeusRunController:set_current_node_key(node_key)
	self._run_state:set_current_node_key(node_key)
end

function DeusRunController:get_current_node_key()
	return self._run_state:get_current_node_key()
end

function DeusRunController:get_coins_spent()
	return self._run_state:get_coins_spent()
end

function DeusRunController:get_cursed_chests_purified(peer_id)
	return self._run_state:get_cursed_chests_purified(peer_id)
end

function DeusRunController:get_current_node()
	local current_node_key = self:get_current_node_key()
	local path_graph = self:_get_graph_data()
	return path_graph [current_node_key]
end

function DeusRunController:get_node(node_key)
	local path_graph = self:_get_graph_data()
	return path_graph [node_key]
end

function DeusRunController:can_spawn_belakor_locus()
	local path_graph = self:_get_graph_data()
	local current_node_key = self._run_state:get_current_node_key()
...
function DeusRunController:unlock_arena_belakor()
	if not self._run_state:is_server() then
...
function DeusRunController:get_weapon_pool()
...
function DeusRunController:get_slot_chances()
...
function DeusRunController:get_own_weapon_pool_excludes()
	return self._run_state:get_own_weapon_pool_excludes()
end

function DeusRunController:get_base_weapon_pool()
...
function DeusRunController:purchase_chest(rarity, chest_type, cost)
	local own_peer_id = self._run_state:get_own_peer_id()
...
function DeusRunController:remove_weapon_from_pool(rarity, weapon_key)
	self:_remove_weapon_from_pool(rarity, weapon_key)
end

function DeusRunController:_remove_weapon_from_pool(rarity, weapon_key)
...
function DeusRunController:rpc_deus_chest_unlocked(sender_channel_id, cost, rarity_lookup, chest_type_lookup)
	local sender = CHANNEL_TO_PEER_ID [sender_channel_id]
...
function DeusRunController:_record_chest_purchased_for_tracking(rarity, chest_type)
	if chest_type == DEUS_CHEST_TYPES.power_up then
...
function DeusRunController:set_twitch_level_vote(node_key)
	self._run_state:set_twitch_level_vote(node_key)
end

function DeusRunController:get_twitch_level_vote()
	return self._run_state:get_twitch_level_vote()
end


function DeusRunController:request_standard_twitch_level_vote(twitch_manager)
...
function DeusRunController:map_finished_voting()
...
function DeusRunController:_add_coin_tracking_entry(peer_id, local_player_id, coins, description)
...
function DeusRunController:handle_level_start()
	self._level_start_time = os.time()
end

function DeusRunController:handle_start_next_round()
	self._deus_weapon_chest_distribution = nil
end

function DeusRunController:get_deus_weapon_chest_type()
...
function DeusRunController:get_level_ended_tracking_data(statistics_db, game_won, num_bots)
...
function DeusRunController:get_level_started_tracking_data(statistics_db, num_bots)
	local run_state = self._run_state
	local current_node_key = run_state:get_current_node_key()
	local path_graph = self:_get_graph_data()
	local current_node = path_graph [current_node_key]
...
function DeusRunController:get_run_tracking_data(game_won)
