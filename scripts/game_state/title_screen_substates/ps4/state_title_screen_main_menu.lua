...
local game_types = { HOST_PLAY_TOGETHER = "host_play_together", INVITATION = "invitation", OFFLINE = "offline", ONLINE = "online" }
...
function StateTitleScreenMainMenu:on_enter(params)
...
function StateTitleScreenMainMenu:_setup_sound()
...
function StateTitleScreenMainMenu:cb_camera_animation_complete()
	ShowCursorStack.push()
	self._title_start_ui:activate_career_ui(true)
end

function StateTitleScreenMainMenu:cb_camera_animation_complete_back()
	self._new_state = StateTitleScreenMain
end

function StateTitleScreenMainMenu:_setup_input()
	local input_manager = Managers.input
	self.input_manager = input_manager
end

function StateTitleScreenMainMenu:_init_menu_views()
...
function StateTitleScreenMainMenu:_update_chat_ignore_list()
	Managers.chat:update_ignore_list()
end

function StateTitleScreenMainMenu:_try_activate_splash()
	local additional_content_view = self._views.additional_content_view
...
function StateTitleScreenMainMenu:_update_network(dt, t)
...
function StateTitleScreenMainMenu:_start_game(game_type, level_key, disable_trailer, profile_name)
...
function StateTitleScreenMainMenu:update(dt, t)
...
function StateTitleScreenMainMenu:_check_popup()
...
function StateTitleScreenMainMenu:cb_offline_data_updated(success)
	if success then
...
function StateTitleScreenMainMenu:_close_menu()
	self.parent:show_menu(false)
	self._title_start_ui:set_start_pressed(false)
	self._title_start_ui:disable_input(false)
...
function StateTitleScreenMainMenu:_next_state()
...
function StateTitleScreenMainMenu:_update_input(dt, t)
...
function StateTitleScreenMainMenu:_update_demo_input(dt, t)
	...
	if Managers.time:get_demo_transition() and
	not demo_ui:in_transition() then
	...
	if not self._input_disabled and not has_popup and not user_detached and not self._popup_id and
	input_service:get("back") and
	not demo_ui:in_transition() then
		demo_ui:animate_to_camera(DemoSettings.starting_camera_name, nil, callback(self, "cb_camera_animation_complete_back"))
		demo_ui:activate_career_ui(false)
...
function StateTitleScreenMainMenu:_update_play_go(dt, t)
	if self._is_installed then
		return
	end
...
function StateTitleScreenMainMenu:on_exit()
...
function StateTitleScreenMainMenu:cb_fade_in_done()
...
function StateTitleScreenMainMenu:activate_view(new_view)
...
function StateTitleScreenMainMenu:exit_current_view()
	local active_view = self._active_view
	local views = self._views
	assert(active_view)
...
function StateTitleScreenMainMenu:_check_restrictions()
	Managers.account:add_restriction_user(Managers.account:user_id())
	self._state = "check_restrictions_network"
end

function StateTitleScreenMainMenu:_check_restrictions_network()
...
function StateTitleScreenMainMenu:_check_restrictions_ps_plus()
...
function StateTitleScreenMainMenu:_check_restrictions_chat()
...
function StateTitleScreenMainMenu:_setup_chat_restriction_dialog()
...
function StateTitleScreenMainMenu:_update_chat_restriction_dialog()
...
function StateTitleScreenMainMenu:_setup_playstation_plus_dialog()
	NpCommerceDialog.initialize()

	self._state = "update_ps_plus_dialog"
end

function StateTitleScreenMainMenu:_update_ps_plus_dialog()
	...
	elseif status == NpCommerceDialog.FINISHED then
		...
		if result == true then
			Managers.account:refetch_restriction_access(nil, { "playstation_plus" })
			self._state = "check_restrictions_ps_plus"
		else
			self:_close_menu()
		end
...
function StateTitleScreenMainMenu:_show_error_dialog(error_code)
...
function StateTitleScreenMainMenu:cb_error_dialog_done()
	self:_close_menu()
end

function StateTitleScreenMainMenu:_request_np_auth_data()
...
function StateTitleScreenMainMenu:cb_np_auth_data_received(result)
...
function StateTitleScreenMainMenu:_signin_to_backend()
...
function StateTitleScreenMainMenu:_waiting_for_backend_signin()
