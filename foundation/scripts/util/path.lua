...
function Path.path_from_string(string_path)
	...
	while index ~= nil do
		...
		if next_slash_index == nil or next_slash_index == string_path_len then break end
...
function Path.change_dir_up(path)

	assert(path.size > 0)
	path.size = path.size - 1
end

function Path.add_path_part(path, path_part)
	path.size = path.size + 1
	path [path.size] = path_part
end

function Path.join(path1, path2, result)
	result = result or { }
	result.size = 0
...
function Path.tostring(path, separator)
...
if UNIT_TEST then
	...
	assert(p6_string_path == "C:/trunk/lols") end