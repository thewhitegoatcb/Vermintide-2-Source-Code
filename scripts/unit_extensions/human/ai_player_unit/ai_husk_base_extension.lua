...
function AiHuskBaseExtension:init(extension_init_context, unit, extension_init_data)
	self.is_husk = true
	self.unit = unit
...
function AiHuskBaseExtension:extensions_ready(world, unit)
...
function AiHuskBaseExtension:freeze()
	self._side_id = nil
end

function AiHuskBaseExtension:unfreeze(unit, data)
...
function AiHuskBaseExtension:current_action_name()
...
function AiHuskBaseExtension:breed()
	return self._breed
end

function AiHuskBaseExtension:update(unit, input, dt, context)
	if self.broadphase_id and
...
function AiHuskBaseExtension:unit_removed_from_game()
	Managers.state.side:remove_unit_from_side(self.unit)
	self._side_id = nil
end

function AiHuskBaseExtension:destroy(unit, input)
	return end