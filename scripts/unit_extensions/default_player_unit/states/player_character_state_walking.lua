...
function PlayerCharacterStateWalking:on_enter(unit, input, dt, context, t, previous_state, params)
...
function PlayerCharacterStateWalking:on_exit(unit, input, dt, context, t, next_state)
...
function PlayerCharacterStateWalking:_handle_ladder_collision(t, movement_settings_table)
	...
	if colliding_with_ladder then
		...
		if facing_correctly and not recently_left_ladder and close_enough then
			...
			if above_align_cube then
				do return "enter_ladder_top" end
			else
...
function PlayerCharacterStateWalking:update(unit, input, dt, context, t)
	local csm = self.csm
	...
	if CharacterStateHelper.is_ledge_hanging(world, unit, self.temp_params) and
	not CharacterStateHelper.handle_bot_ledge_hanging_failsafe(unit, self.is_bot) then
	...
	if not csm.state_next and (
	input_extension:get("jump") or input_extension:get("jump_only")) and (
	not is_crouching or CharacterStateHelper.can_uncrouch(unit)) and
	locomotion_extension:jump_allowed() then
	...
	elseif is_moving then current_movement_speed_scale = 1 else current_movement_speed_scale = 0
	end
