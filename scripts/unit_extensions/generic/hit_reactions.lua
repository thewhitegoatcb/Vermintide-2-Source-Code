...
local ignored_damage_types = { temporary_health_degen = true, kinetic = true, buff_shared_medpack = true, buff = true, buff_shared_medpack_temp_health = true, push = true, health_degen = true, life_tap = true, wounded_dot = true, heal = true, knockdown_bleed = true, life_drain = true }
	...
	player = {
		...
		unit = function (unit, dt, context, t, hit)
			...
			if not ignored_damage_types [damage_type] then
				...
				if hit [DamageDataIndex.DAMAGE_AMOUNT] > 0 and
				Development.parameter("screen_space_player_camera_reactions") ~= false then
...
function HitReactions.get_reaction(hit_reaction_template, is_husk)
