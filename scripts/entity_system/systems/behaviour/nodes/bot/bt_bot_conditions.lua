...
BTConditions.ability_check_categories = {
	activate_ability = { dr_ranger = true, es_huntsman = true, es_mercenary = true, wh_captain = true, we_maidenguard = true, dr_slayer = true, wh_zealot = true, bw_adept = true, es_knight = true, dr_ironbreaker = true, we_shade = true, bw_unchained = true },
	...
	shoot_ability = { bw_scholar = true, we_waywatcher = true, wh_bountyhunter = true } }
...
function BTConditions.can_activate.dr_ironbreaker(blackboard)
...
function BTConditions.can_activate.dr_slayer(blackboard)
...
function BTConditions.can_activate.dr_ranger(blackboard)
	local self_unit = blackboard.unit
	local target_ally_unit = blackboard.target_ally_unit
	local ai_bot_group_system = Managers.state.entity:system("ai_bot_group_system")
	local is_prioritized = ai_bot_group_system:is_prioritized_ally(self_unit, target_ally_unit)
...
function BTConditions.can_activate.es_mercenary(blackboard)
	local self_unit = blackboard.unit
	local self_position = POSITION_LOOKUP [self_unit]
	...
	near_players_percentage =
	num_players_except_self == 0 and 0.5 or

	num_players_within_range / num_players_except_self
...
function BTConditions.can_activate.es_huntsman(blackboard)
	local proximite_enemies = blackboard.proximite_enemies
	local num_proximite_enemies = #proximite_enemies
	local target_unit = blackboard.target_unit
...
function BTConditions.can_activate.es_knight(blackboard)
	local self_unit = blackboard.unit
	local self_position = POSITION_LOOKUP [self_unit]
	local target_unit = blackboard.target_unit
	local target_blackboard = BLACKBOARDS [target_unit]
...
function BTConditions.can_activate.we_waywatcher(blackboard)
	local target = blackboard.target_unit
	if not ALIVE [target] then return false end
	if not BLACKBOARDS [target] then return false
	end
	local max_distance = 30
	local is_range_ok = target == blackboard.priority_target_enemy and blackboard.priority_target_distance <= max_distance or
	target == blackboard.urgent_target_enemy and blackboard.urgent_target_distance <= max_distance or
	target == blackboard.opportunity_target_enemy and blackboard.opportunity_target_distance <= max_distance
...
function BTConditions.can_activate.we_maidenguard(blackboard)
	local self_unit = blackboard.unit
	local self_position = POSITION_LOOKUP [self_unit]
	local target_unit = blackboard.target_unit
	local target_blackboard = BLACKBOARDS [target_unit]
...
function BTConditions.can_activate.we_shade(blackboard)
	local proximite_enemies = blackboard.proximite_enemies
	local num_proximite_enemies = #proximite_enemies
	local target_unit = blackboard.target_unit
	if num_proximite_enemies == 0 and target_unit == nil then
...
function BTConditions.can_activate.wh_captain(blackboard)
	local self_unit = blackboard.unit
	local self_position = POSITION_LOOKUP [self_unit]
...
function BTConditions.can_activate.wh_bountyhunter(blackboard)
	local target = blackboard.target_unit
	if not ALIVE [target] then return false end
	if not BLACKBOARDS [target] then return false
	end
...
function BTConditions.can_activate.wh_zealot(blackboard)
	local self_unit = blackboard.unit
	local self_position = POSITION_LOOKUP [self_unit]
	local target_unit = blackboard.target_unit
	local target_blackboard = BLACKBOARDS [target_unit]
...
function BTConditions.can_activate.bw_adept(blackboard)
	local self_unit = blackboard.unit
	local self_position = POSITION_LOOKUP [self_unit]
	local target_unit = blackboard.target_unit
	local target_blackboard = BLACKBOARDS [target_unit]
...
function BTConditions.can_activate.bw_scholar(blackboard)
	local target = blackboard.target_unit
	if not ALIVE [target] then return false end
	if not BLACKBOARDS [target] then return false
	end
...
function BTConditions.can_activate.bw_unchained(blackboard)
...
function BTConditions.can_activate_ability(blackboard, args)
...
function BTConditions.should_reload_ability_weapon(blackboard, args)
...
function BTConditions.is_disabled(blackboard)
...
function BTConditions.can_revive(blackboard)
	local target_ally_unit = blackboard.target_ally_unit
...
function BTConditions.is_there_threat_to_aid(self_unit, proximite_enemies, force_aid)
	return is_there_threat_to_aid(self_unit, proximite_enemies, force_aid)
end

function BTConditions.can_heal_player(blackboard)
	local target_ally_unit = blackboard.target_ally_unit
...
function BTConditions.can_help_in_need_player(blackboard, args)
...
function BTConditions.can_rescue_hanging_from_hook(blackboard)
	local target_ally_unit = blackboard.target_ally_unit
...
function BTConditions.can_rescue_ledge_hanging(blackboard)
	local target_ally_unit = blackboard.target_ally_unit
...
function BTConditions.can_loot(blackboard)
...
function BTConditions.bot_should_heal(blackboard)
	local self_unit = blackboard.unit
	...
	local target_unit = blackboard.target_unit
	local is_safe =
	not target_unit or ( template.fast_heal or blackboard.is_healing_self ) and #blackboard.proximite_enemies == 0 or
	target_unit ~= blackboard.priority_target_enemy and target_unit ~= blackboard.urgent_target_enemy and target_unit ~= blackboard.proximity_target_enemy and target_unit ~= blackboard.slot_target_enemy
...
function BTConditions.is_slot_not_wielded(blackboard, args)
...
function BTConditions.is_wanted_slot_not_wielded(blackboard, args)
	local wielded_slot = blackboard.inventory_extension:equipment().wielded_slot
...
function BTConditions.has_double_weapon_slots(blackboard, args)
	return blackboard.double_weapons == args [1]
end

function BTConditions.has_better_alt_weapon(blackboard, args)
...
function BTConditions.needs_weapon_swap(blackboard, args)


	if BTConditions.has_double_weapon_slots(blackboard, args) and
	BTConditions.has_better_alt_weapon(blackboard, args) then
		return BTConditions.is_slot_not_wielded(blackboard, { args [2] })
	end

	return BTConditions.is_slot_not_wielded(blackboard, { args [1] })
end

function BTConditions.has_priority_or_opportunity_target(blackboard)
	local target = blackboard.target_unit
	if not ALIVE [target] then return false end
	local dist = 40
...
function BTConditions.bot_in_melee_range(blackboard)
	local target_unit = blackboard.target_unit
	if not ALIVE [target_unit] then return false
	end
...
function BTConditions.has_target_and_ammo_greater_than(blackboard, args)
	local target_unit = blackboard.target_unit
...
function BTConditions.should_vent_overcharge(blackboard, args)
	local overcharge_extension = blackboard.overcharge_extension
	local overcharge_limit_type = args.overcharge_limit_type
	local current_oc, threshold_oc, max_oc = overcharge_extension:current_overcharge_status()
...
function BTConditions.should_recall_unique_ammo(blackboard, args)
	local inventory_extension = blackboard.inventory_extension
...
function BTConditions.should_reload_weapon(blackboard, args)
	local inventory_extension = blackboard.inventory_extension
...
function BTConditions.wants_to_reload_weapon(blackboard, args)
	return blackboard.wanted_slot_to_reload ~= nil
end

function BTConditions.can_open_door(blackboard)
...
function BTConditions.bot_at_breakable(blackboard)
	local navigation_extension = blackboard.navigation_extension
	return navigation_extension:is_in_transition() and navigation_extension:transition_type() == "planks"
end

function BTConditions.cant_reach_ally(blackboard)
...
function BTConditions.should_teleport(blackboard)
	local follow_unit = blackboard.ai_bot_group_extension.data.follow_unit
	if not ALIVE [follow_unit] or blackboard.has_teleported then
		return false
	end
...
function BTConditions.should_drop_grimoire(blackboard)
	local inventory_extension = blackboard.inventory_extension
