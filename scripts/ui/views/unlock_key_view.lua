...
function UnlockKeyView:init(ingame_ui_context)
...
function UnlockKeyView:input_service()
	return self.input_manager:get_service("unlock_key_menu")
end

function UnlockKeyView:destroy()
...
function UnlockKeyView:create_ui_elements()
...
function UnlockKeyView:on_enter()
	...
	Managers.transition:fade_in(10, function () self.fade_in_done = true end)
	self.ui_animations.entry_animation = UIAnimation.init(
	UIAnimation.function_by_time,
	self.ui_scenegraph.root.local_position,
	2, 1080, 1080, 0.01, math.easeInCubic,
	UIAnimation.wait,
	0.1,
	UIAnimation.function_by_time,
	self.ui_scenegraph.root.local_position,
	2, 1080, 0, 0.01, math.easeInCubic)
...
function UnlockKeyView:on_exit()

	return end

function UnlockKeyView:suspend()
	self.suspended = true
...
function UnlockKeyView:unsuspend()
	self.input_manager:block_device_except_service("unlock_key_menu", "keyboard", 1)
	self.input_manager:block_device_except_service("unlock_key_menu", "mouse", 1)
	self.input_manager:block_device_except_service("unlock_key_menu", "gamepad", 1)
	self.suspended = nil
...
function UnlockKeyView:update(dt, t)
...
function UnlockKeyView:draw_widgets(dt, t)
	local ui_renderer = self.ui_renderer
	local ui_scenegraph = self.ui_scenegraph
	local input_service = self.input_manager:get_service("unlock_key_menu")
...
function UnlockKeyView:handle_input(input_service)
...
function UnlockKeyView:handle_controller_input(input_service, dt)
	...
	elseif self.confirm_gamepad_button_widget.content.gamepad_button.is_clicked ~= 0 then if self.confirm_gamepad_button_widget.content.button_hotspot.is_clicked == 0 then --[[ Nothing --]]
...
function UnlockKeyView:on_reset()

	return end

function UnlockKeyView:on_apply()

	return end

function UnlockKeyView:on_menu_close()

	return end

if rawget(_G, "my_global_ass_pointer") then
	my_global_ass_pointer:create_ui_elements() end