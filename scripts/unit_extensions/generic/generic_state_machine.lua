...
function GenericStateMachine:init(extension_init_context, unit, states, start_state)
	self.unit = unit

	self.debugging = false
end

function GenericStateMachine:post_init(states, start_state)
	self.states = states

	self.dummy_params = setmetatable({ }, no_write_meta)
	self.dummy_state = setmetatable({ name = "dummy", update = function () return end, on_exit = function () return end }, no_write_meta)
...
function GenericStateMachine:update(unit, input, dt, context, t)
...
function GenericStateMachine:change_state(state_next, state_next_params)
...
function GenericStateMachine:exit_current_state()
...
function GenericStateMachine:current_state()
