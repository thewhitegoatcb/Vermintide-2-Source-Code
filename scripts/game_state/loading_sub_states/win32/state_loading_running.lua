...
function StateLoadingRunning:on_enter(params)
...
function StateLoadingRunning:_init_network()
	local loading_context = self.parent.parent.loading_context
...
function StateLoadingRunning:update(dt)
	...
	self.parent._network_server and level_transition_handler:needs_level_load() or
	self.parent._network_client and self.parent._network_client:is_fully_synced() and level_transition_handler:needs_level_load()) then
	...
	not self.parent:loading_view_setup_done() then
		local level_key = level_transition_handler:get_current_level_key()
		self.parent:setup_loading_view(level_key)
	end
...
function StateLoadingRunning:on_exit(application_shutdown) return end