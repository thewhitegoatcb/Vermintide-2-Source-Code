...
function ContractPresentationScreenUI:init(ingame_ui_context)
...
function ContractPresentationScreenUI:on_enter(ignore_input_blocking)
...
function ContractPresentationScreenUI:input_service()
	return self.input_manager:get_service("contract_presentation_screen_ui")
end

function ContractPresentationScreenUI:destroy()
	self.ui_animator = nil
end

function ContractPresentationScreenUI:update(dt, t)
	if DO_RELOAD then
		self:_create_ui_elements()
		DO_RELOAD = false
	end
...
function ContractPresentationScreenUI:_update_continue_timer(dt)
...
function ContractPresentationScreenUI:_draw(dt)
...
function ContractPresentationScreenUI:_create_ui_elements()
...
function ContractPresentationScreenUI:_initialize_active_contracts()
	...
	for contract_id, contract_data in pairs(all_contracts) do
		if self.quest_manager:get_session_progress_by_contract_id(contract_id) and
		contract_data.active and not contract_data.turned_in then
...
function ContractPresentationScreenUI:_set_contract_start_info_by_contract_id(widget, contract_id)
	...
	local task_data = { }
	local tasks_total_end_values = 0 local tasks_total_start_values = 0 local tasks_total_session_values = 0
...
function ContractPresentationScreenUI:_set_widget_task_amount(widget, amount)
...
function ContractPresentationScreenUI:_sync_contracts_task_progress()
	local widgets = self._widgets
...
function ContractPresentationScreenUI:_sync_contract_task_progress(contract_id)
...
function ContractPresentationScreenUI:_set_widget_contract_progress(widget, progress)
...
function ContractPresentationScreenUI:_set_widget_task_info(widget, index, text, value_text)
	local widget_content = widget.content
	local widget_style = widget.style
...
function ContractPresentationScreenUI:_get_text_size(text_style, text)
	local ui_renderer = self.ui_renderer

	local size = text_style.size

	local font, size_of_font = UIFontByResolution(text_style, nil)
	local font_material = font [1] local _ = font [2] local font_name = font [3]
...
function ContractPresentationScreenUI:_handle_animations()
	...
	if num_active_contract_widget > 0 then
		...
		do return all_entries_completed end
	else
		return true
	end
end

function ContractPresentationScreenUI:_start_contract_animation(contract_id, animation_name, task_index)
