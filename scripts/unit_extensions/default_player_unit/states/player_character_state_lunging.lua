...
function PlayerCharacterStateLunging:_on_enter_animation(unit, anim_event, variable_name, variable_value, first_person_anim_event)
...
function PlayerCharacterStateLunging:on_enter(unit, input, dt, context, t, previous_state, params)
...
function PlayerCharacterStateLunging:on_exit(unit, input, dt, context, t, next_state)
...
function PlayerCharacterStateLunging:update(unit, input, dt, context, t)
	local csm = self.csm
	...
	if not self._stop then
		...
		if
		move_state == "stop" and not self._stop then
			local position = POSITION_LOOKUP [unit]
			local forward_direction = Quaternion.forward(first_person_extension:current_rotation())
...
function PlayerCharacterStateLunging:_update_movement(unit, dt, t, lunge_data)
...
function PlayerCharacterStateLunging:_check_ledge_hang(unit, dt, t, move_direction, speed, z_offset)
...
function PlayerCharacterStateLunging:_move_on_ground(unit, dt, t, lunge_data)
...
function PlayerCharacterStateLunging:_move_in_air(unit, dt, t, lunge_data)
	local locomotion_extension = self.locomotion_extension
	local first_person_extension = self.first_person_extension
...
function PlayerCharacterStateLunging:_parse_attack_data(damage_settings)
...
function PlayerCharacterStateLunging:_update_damage(unit, dt, t, damage_data)
...
local hit_units = { }
function PlayerCharacterStateLunging:_do_blast(new_pos, forward_direction)
	self._hit = true
