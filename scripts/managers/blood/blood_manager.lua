...
function BloodManager:update_blood_enabled(blood_enabled)
...
function BloodManager:get_blood_enabled()
	return self._blood_active
end

function BloodManager:update_num_blood_decals(num_blood_decals)
	BloodSettings.blood_decals.num_decals = num_blood_decals
end

function BloodManager:update_screen_blood_enabled(screen_blood_enabled)
	BloodSettings.screen_space.enabled = screen_blood_enabled
end

function BloodManager:update_dismemberment_enabled(dismemberment_enabled)
	BloodSettings.dismemberment.enabled = dismemberment_enabled
end

function BloodManager:update_ragdoll_enabled(ragdoll_enabled)
	BloodSettings.ragdoll_push.enabled = ragdoll_enabled
end

function BloodManager:_init_settings()
...
function BloodManager:clear_blood_decals()
	Managers.state.decal:clear_all_of_type("blood_decals")
end

function BloodManager:clear_unit_decals(unit)
	Unit.set_vector4_for_materials(unit, "hit_position", Color(0, 0, 0, 0))
end

function BloodManager:_update_blood_effects()
...
function BloodManager:_set_weapon_blood_intensity(attacker_unit, weapon, amount)
...
function BloodManager:clear_weapon_blood(attacker, weapon)
...
function BloodManager:_update_blood_ball_buffer()
...
function BloodManager:_create_blood_ball_buffer()
	...
	self._blood_ball_ring_buffer = { write_index = 1, read_index = 1, size = 0, buffer = Script.new_array(buffer_size), max_size = buffer_size }
	for index = 1, buffer_size do
		self._blood_ball_ring_buffer.buffer [index] = { velocity = 0,
			position = Vector3Box(),
...
function BloodManager:_spawn_blood_ball(blood_ball_data)
	...
	EngineOptimizedExtensions.blood_spawn_blood_ball(
	self._blood_system,
	"units/decals/blood_ball",
	position,
	rotation,
	direction,
...
function BloodManager:despawn_blood_ball(unit)

	EngineOptimizedExtensions.blood_despawn_blood_ball(self._blood_system, unit)

end

function BloodManager:_add_blood_ball_data_to_buffer(position, direction, damage_type)
...
function BloodManager:add_blood_ball(position, direction, damage_type, hit_unit)
...
function BloodManager:_get_blood_effect_data(unit, effect_nodes)
...
function BloodManager:_spawn_effects(hit_unit, breed, health_ext)
...
function BloodManager:_update_blood_intensity(hit_unit, breed, health_ext)
...
function BloodManager:add_weapon_blood(attacker, damage_type)
	if BloodSettings.weapon_blood.enabled then
		local player = self:_is_player(attacker)
		if
		player and self:_is_melee_weapon(attacker) then
...
function BloodManager:add_enemy_blood(position, unit)
	if BloodSettings.enemy_blood.enabled and
...
function BloodManager:play_screen_space_blood(particle_name, position, optional_offset, option_rotation_offset, optional_scale)
...
function BloodManager:_is_melee_weapon(attacker)
...
function BloodManager:_is_player(attacker)
