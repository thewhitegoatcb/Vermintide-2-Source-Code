...
function ActionCatch:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
	ActionCatch.super.init(self, world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
end

function ActionCatch:client_owner_start_action(new_action, t, chain_action_data, power_level, action_init_data)
...
function ActionCatch:client_owner_post_update(dt, t, world, can_damage)
	if self._state == "waiting_to_catch" and
	self._catch_time <= t then
		self:_add_ammo()
		self._state = "caught"
	end
...
function ActionCatch:_remove_pickup()
...
function ActionCatch:_add_ammo()
	local inventory_extension = self._inventory_extension
...
function ActionCatch:finish(reason)
	return end