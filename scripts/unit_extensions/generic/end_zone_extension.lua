...
function EndZoneExtension:init(extension_init_context, unit)
	self._unit = unit
...
function EndZoneExtension:_trigger_vo(category, event)
...
function EndZoneExtension:activated()
	return self._activated
end

function EndZoneExtension:_set_light_intensity(intensity)
...
function EndZoneExtension:activation_allowed(allowed)
	self._activation_allowed = allowed
end

function EndZoneExtension:_activate(activate)
...
function EndZoneExtension:_get_wind_name()
...
function EndZoneExtension:_activate_volume()
...
function EndZoneExtension:_deactivate_volume()
...
function EndZoneExtension:_reset_distances()
	self._closest_player = math.huge
	table.clear(self._player_distances)
end

function EndZoneExtension:_check_proximity()
...
function EndZoneExtension:_update_state(dt, t)
...
function EndZoneExtension:hot_join_sync(sender)
...
function EndZoneExtension:_open(dt, t)
...
function EndZoneExtension:_close(dt, t)
	if self._activated and (self._always_activated or self._closest_player <= EndZoneSettings.activate_size ^ 2) then
		self._state = "_open"
		Unit.flow_event(self._unit, "opening_end_zone")
	else
...
function EndZoneExtension:_check_end_mission_all_inside(dt, all_inside)
	...
	if all_inside then
		...
		if self:end_time_left() <= 0 and
		not self._disable_complete_level then
...
function EndZoneExtension:_check_end_mission_any_inside(dt, all_inside, any_inside, players_outside_portal)
	local game_mode_key = Managers.state.game_mode:game_mode_key()
...
function EndZoneExtension:_end_mission_check(dt, t)
	if self._activated and (self._always_activated or self._closest_player <= EndZoneSettings.activate_size ^ 2) then
...
function EndZoneExtension:_all_players_joined()
