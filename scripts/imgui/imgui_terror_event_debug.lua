...
function ImguiTerrorEventDebug:init()
...
function ImguiTerrorEventDebug:update()
	return end

function ImguiTerrorEventDebug:is_persistent()
	return true
end

function ImguiTerrorEventDebug:draw(is_open)
	...
	if Imgui.button("Start Level Specific Terror Event") and
	Managers.state.conflict then
	...
	if Imgui.button("Start Generic Terror Event") and
	Managers.state.conflict then
		script_data.terror_event_difficulty = difficulties [self._difficulty_index]
		script_data.terror_event_difficulty_tweak = difficulty_tweaks [self._difficulty_tweak_index]
	...
	if Imgui.button("Stop active terror events") and (
	Managers.player.is_server or LEVEL_EDITOR_TEST) then
