...
function BTInterestPointChooseAction:init(...)
	BTInterestPointChooseAction.super.init(self, ...)
end

function BTInterestPointChooseAction:enter(unit, blackboard, t)
...
function BTInterestPointChooseAction:leave(unit, blackboard, t, reason, destroy)
	if


	( reason == "failed" or reason == "aborted" ) and HEALTH_ALIVE [unit] then
		local interest_point_system_api = blackboard.system_api.ai_interest_point_system
...
function BTInterestPointChooseAction:run(unit, blackboard, t, dt)
