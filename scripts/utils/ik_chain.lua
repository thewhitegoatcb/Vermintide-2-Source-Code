...
function IkChain:init(joints, start_pos, target_pos, tolerance, use_max_joint_angle)
...
function IkChain:set_origin_pos(pos)
	self.origin_pos:store(pos)
end

function IkChain:set_target_pos(target_pos, acceleration)
	self.target_pos:store(target_pos)
	self.acc = acceleration or 1
end

function IkChain:set_whip(angle_velocity)
	self.whip_angle_velocity = angle_velocity
end

function IkChain:update_whip(joints, angular_velocity, t, dt)
	local n1 = 1
	local n2 = 2
...
function IkChain:debug_draw(joints, num_joints)
...
function IkChain:backward(joints, lengths, num_joints, target_pos)
...
function IkChain:forward(joints, lengths, num_joints, start_pos)
...
function IkChain:forward_constrained(joints, lengths, num_joints, start_pos)
...
function IkChain:solve_dragging(t, dt)
	local target_pos = self.target_pos:unbox()
