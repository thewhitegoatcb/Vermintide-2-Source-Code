...
local passive_ui_data = { material = "overcharge_bar_warrior_priest",

	color = { 255, 144, 54, 36 } }



local passive_feedback_values = { detail_bar_passive_active = 0.2, glow_brightness_min = 0.1, detail_bar_passive_inactive = -0.4, glow_brightness_max = 0.8 }
...
function PriestResourceBarUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function PriestResourceBarUI:create_ui_elements()
...
local customizer_data = { root_scenegraph_id = "screen_bottom_pivot_parent", label = "Overcharge", registry_key = "overcharge", drag_scenegraph_id = "charge_bar" }
...
function PriestResourceBarUI:update(dt, t, player)
...
function PriestResourceBarUI:set_charge_bar_fraction(player, overcharge_fraction, min_threshold_fraction, max_threshold_fraction, anim_blend_overcharge, dt)
...
function PriestResourceBarUI:destroy()
...
function PriestResourceBarUI:set_alpha(alpha)
	self.render_settings.alpha_multiplier = alpha
end

function PriestResourceBarUI:_apply_crosshair_position(x, y)
...
function PriestResourceBarUI:glow_feedback()
...
function PriestResourceBarUI:handle_glow_feedback(widget, dt)
	if not self._play_glow_feedback then
		return
	end
...
function PriestResourceBarUI:active_passive_feedback(active)
	self._active_passive = active
	if active then
		self._animations.fade_in = UIAnimation.init(UIAnimation.function_by_time, self.charge_bar.style.bar_active.color,
		1, 0, 255, 0.3, math.ease_in_exp)
	else
		self._animations.fade_in = UIAnimation.init(UIAnimation.function_by_time, self.charge_bar.style.bar_active.color,
		1, 255, 0, 0.3, math.ease_in_exp)
	end
end

function PriestResourceBarUI:handle_active_passive_feedback(value)
	local widget = self.charge_bar
	local material_name = widget.content.bar_detail
	local material = Gui.material(self._gui, material_name)
	Material.set_scalar(material, "detail_offset", value)
