...
function ActionPotion:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionPotion:client_owner_start_action(new_action, t)
	ActionPotion.super.client_owner_start_action(self, new_action, t)
	self.current_action = new_action
end

function ActionPotion:client_owner_post_update(dt, t, world, can_damage)

	return end

function ActionPotion:finish(reason)
	...
	if not buff_extension:has_buff_type("trait_ring_all_potions") and not buff_extension:has_buff_type("weave_trait_ring_all_potions") then
		...
		local additional_potion_buffs = { "speed_boost_potion_reduced", "damage_boost_potion_reduced", "cooldown_reduction_potion_reduced" }
		if cooldown_reduction_override and career_extension then
