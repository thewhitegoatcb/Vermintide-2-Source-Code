...
function PlayerCharacterStateLedgeHanging:on_enter_animation()
...
function PlayerCharacterStateLedgeHanging:change_to_third_person_camera()
...
function PlayerCharacterStateLedgeHanging:on_enter(unit, input, dt, context, t, previous_state, params)
	local unit = self.unit
...
function PlayerCharacterStateLedgeHanging:on_exit(unit, input, dt, context, t, next_state)
...
function PlayerCharacterStateLedgeHanging:update(unit, input, dt, context, t)
	local csm = self.csm
	local unit = self.unit
...
function PlayerCharacterStateLedgeHanging:calculate_start_position()
	local unit = self.unit
...
function PlayerCharacterStateLedgeHanging:calculate_and_start_rotation_to_ledge()
	local unit = self.unit
	local ledge_unit = self.ledge_unit
	local movement_settings_table = PlayerUnitMovementSettings.get_movement_settings_table(unit)
...
function PlayerCharacterStateLedgeHanging:calculate_offset_rotation()
	local unit = self.unit
	local ledge_unit = self.ledge_unit
	...
	if not ray_1_succeded then
		...
		elseif script_data.debug_hang_ledges then --[[ Nothing --]]
		end
	end
