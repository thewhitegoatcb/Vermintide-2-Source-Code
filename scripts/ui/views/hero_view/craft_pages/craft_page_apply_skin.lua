...
function CraftPageApplySkin:on_enter(params, settings)
...
function CraftPageApplySkin:setup_recipe_requirements()
...
function CraftPageApplySkin:create_ui_elements(params)
...
function CraftPageApplySkin:_weapon_slot_updated()
	local item_interface = Managers.backend:get_interface("items")
...
function CraftPageApplySkin:on_exit(params)
	self.parent.parent:set_craft_optional_item_filter(nil)
...
function CraftPageApplySkin:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function CraftPageApplySkin:post_update(dt, t)

	return end

function CraftPageApplySkin:_update_animations(dt)
...
function CraftPageApplySkin:_is_button_pressed(widget)
...
function CraftPageApplySkin:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageApplySkin:_is_button_held(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageApplySkin:_handle_input(dt, t)
...
function CraftPageApplySkin:_handle_craft_input_progress(progress)
...
function CraftPageApplySkin:craft_result(result, error, reset_slots)
	if not error then
		self._craft_result = result
	end
end

function CraftPageApplySkin:reset()
...
function CraftPageApplySkin:on_craft_completed()
...
function CraftPageApplySkin:_update_craft_items()
	local super_parent = self.super_parent
	local item_grid = self._item_grid
	local item_grid_2 = self._item_grid_2
...
function CraftPageApplySkin:_remove_item(item_grid, backend_id, ignore_sound)
...
function CraftPageApplySkin:_add_item(item_grid, backend_id, ignore_sound)
	item_grid:clear_item_grid()
...
function CraftPageApplySkin:_set_craft_button_disabled(disabled)
	self._widgets_by_name.craft_button.content.button_hotspot.disable_button = disabled
end

function CraftPageApplySkin:_exit(selected_level)
	self.exit = true
	self.exit_level_id = selected_level
end

function CraftPageApplySkin:draw(dt)
...
function CraftPageApplySkin:_play_sound(event)
	self.super_parent:play_sound(event)
end

function CraftPageApplySkin:_set_craft_button_text(text, localize)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.craft_button
