...
local string = string local math = math
...
UIPasses.texture_uv_dynamic_color_uvs_size_offset = {
	...
	draw = function (ui_renderer, pass_data, ui_scenegraph, pass_definition, ui_style, ui_content, position, widget_size, input_service, dt)
		if pass_definition.content_id then ui_content = ui_content [pass_definition.content_id] or ui_content end
		if pass_definition.style_id then ui_style = ui_style [pass_definition.style_id] or ui_style end
...
local element_alignment_position = { 0, 0, 0 }
UIPasses.list_pass = {
	init = function (pass_definition)
	...
	draw = function (ui_renderer, pass_data, ui_scenegraph, pass_definition, ui_style, ui_content, position, size, input_service, dt)
		...
		local index = 0 local row_count = 0
		if IS_PS4 then
			row_count = math.max(start_index - 1, 0) index = 0
		end
		local change_row = nil
		for i = start_index, stop_index do
			...
			for j = 1, num_passes do
				...
				if
				content_check_function and not content_check_function(pass_element_content, pass_element_style, i) then
					draw = false
				end
	...
	draw = function (ui_renderer, pass_data, ui_scenegraph, pass_definition, ui_style, ui_content, position, size, input_service, dt)
		...
		if ui_style then
			...
			if frame_margins then
				gui_position = Vector3(
				position [1] + frame_margins [1],
				position [2] + frame_margins [2],
				...
				if gui_size then
					...
					gui_size = Vector2(
					size [1] - frame_margins [1] * 2,
					size [2] - frame_margins [2] * 2)
				end
			end


			gui_position =
			gui_position or position
...
UIPasses.scrollbar_hotspot = {
	init = function (pass_definition)
		return {
			...
			scrollbar_size = { 0, 0 },
			scrollbar_position = { 0, 0, 0 },

			hotspot_size = { 0, 0 },
			hotspot_position = { 0, 0, 0 },

			scroll_area_position = { 0, 0, 0 },

			start_move_pos = { math.huge, math.huge } }
	...
	draw = function (ui_renderer, pass_data, ui_scenegraph, pass_definition, ui_style, ui_content, position, size, input_service, dt)
		...
		if script_data.ui_debug_hover then
			if is_hover then
				UIRenderer.draw_rect(ui_renderer, Vector3(hotspot_position [1], hotspot_position [2], 999), hotspot_size, { 128, 0, 255, 0 })
			else
				UIRenderer.draw_rect(ui_renderer, Vector3(hotspot_position [1], hotspot_position [2], hotspot_position [3] + 1), hotspot_size, { 60, 255, 0, 0 })
			end

			if is_hover_scrollbar then
				UIRenderer.draw_rect(ui_renderer, Vector3(scrollbar_position [1], scrollbar_position [2], 999), scrollbar_size, { 128, 0, 255, 0 })
			else
				UIRenderer.draw_rect(ui_renderer, Vector3(scrollbar_position [1], scrollbar_position [2], scrollbar_position [3] + 1), scrollbar_size, { 60, 255, 0, 0 })
			end

			if is_hover_scroll_area then
				UIRenderer.draw_rect(ui_renderer, Vector3(scroll_area_position [1], scroll_area_position [2], 999), scroll_area_size, { 128, 0, 255, 0 })
			else
				UIRenderer.draw_rect(ui_renderer, Vector3(scroll_area_position [1], scroll_area_position [2], scroll_area_position [3] + 1), scroll_area_size, { 60, 255, 0, 0 })
			end
		end
	end }
...
UIPasses.border = {
	...
	draw = function (ui_renderer, pass_data, ui_scenegraph, pass_definition, ui_style, ui_content, position, size, input_service, dt)
		...
		UIRenderer.draw_rect(ui_renderer, lower_left_corner + Vector3(size.x - thickness, 0, 0),
		Vector3(thickness, size.y, 0), ui_style.color)
...
UIPasses.text_area_chat = {
	...
	draw = function (ui_renderer, pass_data, ui_scenegraph, pass_definition, ui_style, ui_content, position, size, input_service, dt)
		...
		if ui_style.font_type then
			...
			font_name = ui_style.font_type font_size = size_of_font font_material = font [1]
		end
		...
		for i = 1, #ui_content.message_tables do
			...
			if formatted ~= current_format then
				...
				if channel_string ~= "" then
					local channel_color = is_enemy and { 255, 237, 48, 48 } or { 255, 53, 161, 212 }
				...
				default_color = default_color and type(default_color) == "table" and
				string.format(format_directive_color_string, default_color [2], default_color [3], default_color [4]) or default_color or

				string.format(format_directive_color_string, color_text [2], color_text [3], color_text [4])
					...
					for _, emoji_data in ipairs(emojis) do
						for row, split_msg in ipairs(formatted_message_array) do
							...
							if index then
								...
								formatted_emojis [row] [#formatted_emojis [row] + 1] = { data = emoji_data, offset_x = width, offset_y = -ui_style.emoji_size [2] * 0.3, size = ui_style.emoji_size }
...
local function extract_button_data_from_text(ui_renderer, ui_style, input_text, prepared_text)
	...
	if INPUT_ACTIONS [1] then
		...
		for i = 1, #INPUT_ACTIONS do
			...
			local font, size_of_font = UIFontByResolution(ui_style)
			local font_material = font [1] local font_size = size_of_font
...
local BUTTON_COLOR = { 0, 255, 255, 255 }
		...
		if start_idx then
			...
			if not ui_style.skip_button_rendering then
				...
				elseif not keymap_binding or not unassigned and keymap_binding [1] == "mouse" then
					...
					UIRenderer_draw_texture(ui_renderer, left_part.texture, texture_pos, Vector2(left_part.size [1] * size_multiplier, left_part.size [2] * size_multiplier), { ui_style.text_color [1], 255, 255, 255 }, ui_style.masked, ui_style.saturated)
					texture_pos [1] = texture_pos [1] + left_part.size [1] * size_multiplier


					UIRenderer_draw_texture(ui_renderer, middle_part.texture, texture_pos, Vector2(button_text_length, middle_part.size [2] * size_multiplier), { ui_style.text_color [1], 255, 255, 255 }, ui_style.masked, ui_style.saturated)
					...
					UIRenderer_draw_texture_uv(ui_renderer, left_part.texture, texture_pos, Vector2(left_part.size [1] * size_multiplier, left_part.size [2] * size_multiplier), { { 1, 0 }, { 0, 1 } }, { ui_style.text_color [1], 255, 255, 255 }, ui_style.masked, ui_style.saturated)
				end
			end
...
UIPasses.text = {
	...
	draw = function (ui_renderer, pass_data, ui_scenegraph, pass_definition, ui_style, ui_content, position, size, input_service, dt)
		...
		if ui_style.word_wrap and ui_style.dynamic_font_size_word_wrap then
			...
			dynamic_wrap_font_size =
			recalculate and UIRenderer.scaled_font_size_by_area(ui_renderer, text, ui_style.area_size or size, ui_style) or

			ui_style._dynamic_wrap_font_size
		...
		if ui_style.word_wrap then
			...
			elseif ui_style.vertical_alignment == "center" then
				...
				math.max(num_texts - 1, 0) * 0.5 * full_font_height
			else
			...
			for i = 1, num_texts do
				...
				if ui_style.draw_text_rect then
					...
					if max_text_width < width then max_text_width = width or max_text_width
					end
				end
...
UIPasses.lorebook_multiple_texts = {
	...
	draw = function (ui_renderer, pass_data, ui_scenegraph, pass_definition, ui_style, ui_content, position, size, input_service, dt)
		...
		if ui_style.font_type then
			local font, size_of_font = UIFontByResolution(ui_style)
			font_name = font [3] font_size = size_of_font font_material = font [1]
		else
			local font = ui_style.font
			font_name = font [3] font_size = font [2] font_material = font [1]
...
UIPasses.multiple_texts = {
	...
	draw = function (ui_renderer, pass_data, ui_scenegraph, pass_definition, ui_style, ui_content, position, size, input_service, dt)
		...
		local font_material = font [1] local font_size = calculated_font_size local font_name = font [3]
...
UIPasses.viewport = {
	init = function (pass_definition, widget_content, widget_style)
		...
		if not style.world_flags then local world_flags = { Application.DISABLE_SOUND, Application.DISABLE_ESRAM }
		end
		...
		if style.enable_sub_gui then
			ui_renderer = UIRenderer.create(world,
	...
	raycast_at_screen_position = function (pass_data, screen_position, result_type, range, collision_filter)
		if pass_data.viewport_rect_pos_x == nil then return nil
		end
		local resx = RESOLUTION_LOOKUP.res_w local resy = RESOLUTION_LOOKUP.res_h
...
local drag_position_table = { 0, 0, 0 }
...
UIPasses.drag = {
	...
	draw = function (ui_renderer, pass_data, ui_scenegraph, pass_definition, ui_style, ui_content, position, size, input_service, dt)
		...
		if script_data.ui_debug_drag then
			UIRenderer.draw_rect(ui_renderer, position + Vector3(0, 0, 1), size, ui_content.is_dragging and { 128, 0, 100, 100 } or { 0, 0, 0, 255 })
		end
	end }


local cursor_position_table = { 0, 0, 0 }
UIPasses.gamepad_cursor = {
	...
	draw = function (ui_renderer, pass_data, ui_scenegraph, pass_definition, ui_style, ui_content, position, size, input_service, dt)
		...
		local offset = ui_style.offset or { 0, 0 }
		...
		if script_data.ui_debug_hover then
			...
			UIRenderer.draw_rect(ui_renderer, { pos [1], pos [2], pos [3] }, { cursor_size [1], cursor_size [2] }, { 128, 255, 255, 255 })
		end
...
UIPasses.hover = {
	...
	draw = function (ui_renderer, pass_data, ui_scenegraph, pass_definition, ui_style, ui_content, position, size, input_service, dt)
		...
		if ui_content.hover_type == "circle" then
			...
			if script_data.ui_debug_hover then
				UIRenderer.draw_rect(ui_renderer, position + Vector3(0, 0, 1), size, ui_content.is_hover and { 128, 0, 255, 0 } or { 128, 255, 0, 0 })
			end
		end
...
UIPasses.additional_option_tooltip = {
	init = function (pass_definition, ui_content, ui_style)
		...
		local pass_definitions = pass_definition.content_passes or { "additional_option_info" }
...
UIPasses.item_tooltip = {
	init = function (pass_definition, ui_content, ui_style)
		...
		local pass_definitions = pass_definition.content_passes or { "equipped_item_title", "item_titles", "skin_applied", "deed_mission", "deed_difficulty", "mutators", "deed_rewards", "ammunition", "fatigue", "item_power_level", "properties", "traits", "weapon_skin_title", "item_information_text", "loot_chest_difficulty", "loot_chest_power_range", "unwieldable", "keywords", "special_action_tooltip", "other_equipped_careers_tooltip", "item_description", "light_attack_stats", "heavy_attack_stats", "detailed_stats_light", "detailed_stats_heavy", "detailed_stats_push", "detailed_stats_ranged_light", "detailed_stats_ranged_heavy" }
		...
		pass_data.items_alpha_progress = disable_fade_in and { 1, 1, 1, 1 } or { 0, 0, 0, 0 }
		local tooltip_wait_duration = disable_fade_in and 0 or UISettings.tooltip_wait_duration
		pass_data.alpha_wait_times = { tooltip_wait_duration, tooltip_wait_duration * 2, tooltip_wait_duration * 2, tooltip_wait_duration * 2 }
...
local tooltip_size = { 0, 0 }
local temp_cursor_pos = { 0, 0 }
local tooltip_background_color = { 220, 3, 3, 3 }
	...
	draw = function (ui_renderer, pass_data, ui_scenegraph, pass_definition, ui_style, ui_content, position, size, input_service, dt)
		...
		if script_data.ui_debug_hover then
			if ui_content.is_hover then
				UIRenderer.draw_rect(ui_renderer, Vector3(position [1], position [2], 999), size, { 128, 0, 255, 0 })
			else
				UIRenderer.draw_rect(ui_renderer, position + Vector3(0, 0, 1), size, { 60, 255, 0, 0 })
			end
		end
...
UIPasses.item_presentation = {
	init = function (pass_definition, ui_content, ui_style)
		...
		local pass_definitions = pass_definition.content_passes or { "item_titles", "deed_mission", "deed_difficulty", "mutators", "deed_rewards", "ammunition", "fatigue", "item_power_level", "properties", "traits" }
...
local function run_content_functions_of_auto_layout_sub_pass(sub_pass, index)
	...
	if
	content_check_function and not content_check_function(sub_pass.content, sub_pass.style, index) then
...
UIPasses.auto_layout = {
	init = function (pass_definition, ui_content, ui_style)
		...
		for _, sub_pass_definition in ipairs(sub_pass_definitions) do
			...
			if sub_pass_style.render_random_debug_color then
				debug_color = { 64, math.random(0, 255), math.random(0, 255), math.random(0, 255) }
			end

			passes [#passes + 1] = { visible = true,
		...
		if background_definitions then
			for _, background_definition in ipairs(background_definitions) do
				...
				background_passes [#background_passes + 1] = { visible = true,
		...
		pass_data._size_table = { 0, 0 }
		return pass_data
	end,
	update = function (ui_renderer, pass_data, ui_scenegraph, pass_definition, ui_style, ui_content, input_service, dt, visible)
		local min_x = 0 local min_y = 0 local max_x = 0 local max_y = 0

		local layout_delta_x = ui_style and ui_style.layout_delta_x or 0 local layout_delta_y = ui_style and ui_style.layout_delta_y or 1

		local x_correction = layout_delta_x < 0 and 1 or 0
		local y_correction = layout_delta_y < 0 and 1 or 0

		local current_pos_x = 0 local current_pos_y = 0
		...
		for i, sub_pass in ipairs(pass_data.passes) do
			...
			if sub_pass.visible then
				...
				if sub_pass.style and sub_pass.style.size then
					height = sub_pass.style.size [2] width = sub_pass.style.size [1]
				end
	...
	draw = function (ui_renderer, pass_data, ui_scenegraph, pass_definition, ui_style, ui_content, position, parent_size, input_service, dt)
		...
		local base_pos_x, base_pos_y = nil
		...
		if screen_padding then
			local inv_scale = RESOLUTION_LOOKUP.inv_scale
			local screen_width = RESOLUTION_LOOKUP.res_w * inv_scale local screen_height = RESOLUTION_LOOKUP.res_h * inv_scale
