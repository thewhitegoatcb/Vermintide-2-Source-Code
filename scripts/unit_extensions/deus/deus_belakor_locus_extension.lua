...
local STATE = { INITIAL = 0, DONE = 4, ACTIVATED = 3, WAITING_TO_SPAWN_CULTISTS = 1, WAITING_FOR_ACTIVATION = 2 }
...
local SHOW_RUNE_FLOW_EVENTS = { "SHOW_RUNE_01", "SHOW_RUNE_02", "SHOW_RUNE_03" }
...
local LIEUTENANT_TERROR_EVENTS = { "belakor_altar_shadow_lieutenant_spawn_01", "belakor_altar_shadow_lieutenant_spawn_02", "belakor_altar_shadow_lieutenant_spawn_03" }





local DECAL_PER_LOCUS_TYPE = { "units/decals/decal_belakor_arena_01", "units/decals/decal_belakor_arena_02", "units/decals/decal_belakor_arena_03" }
...
local function check_if_should_play_vo(world, locus_unit, locus_position, players_and_bots)
	...
	if DISTANCE_TO_TRIGGER_DIALOGUE < closest_distance then
		return false
	end
...
function DeusBelakorLocusExtension:init(extension_init_context, unit, extension_init_data)
	self._unit = unit
...
function DeusBelakorLocusExtension:game_object_initialized(unit, go_id)
	self:_set_state(STATE.WAITING_TO_SPAWN_CULTISTS)
end

function DeusBelakorLocusExtension:extensions_ready(world, unit)
	self._interactable_extension = ScriptUnit.extension(unit, "interactable_system")
end

function DeusBelakorLocusExtension:destroy()
...
function DeusBelakorLocusExtension:connect_to_statue(statue_unit, decal_pose)
	...
	if current_state ~= STATE.DONE then
		...
		self._statue_decal = Managers.state.unit_spawner:spawn_local_unit(
		DECAL_PER_LOCUS_TYPE [self._locus_type],
		decal_pos,
		decal_rotation,

		"units/materials/d/decal/decal_belakor_arena_01")
	end
end

function DeusBelakorLocusExtension:update(unit, input, dt, context, t)
	...
	if current_state ~= prev_state then
		if current_state == STATE.WAITING_TO_SPAWN_CULTISTS then --[[ Nothing --]]
...
function DeusBelakorLocusExtension:activate()
	self._paused = false
end

function DeusBelakorLocusExtension:deactivate()
	self._paused = true
end

function DeusBelakorLocusExtension:is_complete()
	local state = self:_get_state()
	return state == STATE.DONE
end

function DeusBelakorLocusExtension:_get_state()
...
function DeusBelakorLocusExtension:_set_state(state)
...
function DeusBelakorLocusExtension:can_interact_validate(player_unit)
...
function DeusBelakorLocusExtension:can_interact()
	local state = self:_get_state()
...
function DeusBelakorLocusExtension:get_interaction_length()
	local state = self:_get_state()
...
function DeusBelakorLocusExtension:get_interaction_action()
	local state = self:_get_state()
...
function DeusBelakorLocusExtension:on_server_interact(world, interactor_unit, interactable_unit, data, config, t, result)
	local state = self:_get_state()
	if state == STATE.WAITING_FOR_ACTIVATION then
...
function DeusBelakorLocusExtension:on_client_interact(world, interactor_unit, interactable_unit, data, config, t, result)
	local state = self:_get_state()
	if state == STATE.ACTIVATED then
...
function DeusBelakorLocusExtension:on_server_start_interact(world, interactor_unit, interactable_unit, data, config, t)
