...
function AbilityUI:init(parent, ingame_ui_context)
...
function AbilityUI:_create_ui_elements()
...
function AbilityUI:_get_player_unit()
...
function AbilityUI:_update_ability_widget(dt, t)
...
function AbilityUI:set_visible(visible)
	self._is_visible = visible

	self:_set_elements_visible(visible)
end

function AbilityUI:_set_elements_visible(visible)
...
function AbilityUI:_smudge()
	UIUtils.mark_dirty(self._widgets)
	self._dirty = true
end

function AbilityUI:_handle_gamepad()
...
local customizer_data = { root_scenegraph_id = "ability_root", is_child = true, registry_key = "player_status" }





function AbilityUI:update(dt, t)
...
function AbilityUI:set_alpha(alpha)
	self._render_settings.alpha_multiplier = alpha
	self:_smudge()
end

function AbilityUI:_get_input_texture_data(input_action)
...
function AbilityUI:event_input_changed()
	local gamepad_active = Managers.input:is_device_active("gamepad")
...
function AbilityUI:on_spectator_target_changed(spectated_player_unit)
...
function AbilityUI:_update_numeric_ui_ability_cooldown()
