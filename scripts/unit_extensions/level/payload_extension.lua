...
function PayloadExtension:init(extension_init_context, unit, extension_init_data)
	...
	self._last_synched_spline_values = { last_synch_time = 0, error_compensation_speed = 0, spline_index = 1, subdivision_index = 1, spline_t = 0 }
	...
	if not DEDICATED_SERVER then
		...
		if unit_hazard_type == "sled" then if statistics_db:get_persistent_stat(stats_id, "trail_sleigher") <= 50 then
...
function PayloadExtension:activate()
	self._activated = true
end

function PayloadExtension:deactivate(stop)
	self._activated = false
	self._stop_command_given = stop
end

function PayloadExtension:destroy()
	Managers.state.event:unregister("on_killed", self)
end

function PayloadExtension:extensions_ready()
	return end

function PayloadExtension:hot_join_sync(sender)
	return end

function PayloadExtension:init_payload(payload_gizmos)
...
function PayloadExtension:_push_player(player_unit, abs_speed)
	local unit = self._unit
...
local RESULT_TABLE = { }
local STAGGERED = { }
function PayloadExtension:_hit_enemies(abs_speed, t)
	...
	local num_hits = AiUtils.broadphase_query(payload_position, radius,
	RESULT_TABLE,
	self._enemy_broadphase_categories)
...
function PayloadExtension:update(unit, input, dt, context, t)
...
function PayloadExtension:payload_flow_event(spline_index)
	local spline_curve = self._spline_curve
...
function PayloadExtension:_players_in_proximity()
...
function PayloadExtension:_error_speed_calculation(dt, t, game, id, movement)
...
function PayloadExtension:set_game_object_id(game_object_id)
...
function PayloadExtension:_init_movement_spline(world, unit, payload_gizmos)
	...
	if payload_gizmos then
		for i = 1, #payload_gizmos do
			...
			local smallest_distance = math.huge local point = nil
	...
	for index, spline in ipairs(splines) do
		...
		if gizmo_unit then
			...
			if unit_speed_setting ~= "" then speed_setting = unit_speed_setting or speed_setting end
			flow_event = unit_flow_event ~= "" and unit_flow_event
		end

		local speed_settings = PayloadSpeedSettings [speed_setting]
		local metadata = { speed_settings = speed_settings, flow_event_data = { event_thrown = false, flow_event = flow_event } }
...
function PayloadExtension:_create_game_object()
	local unit = self._unit
	local movement = self._spline_curve:movement()
...
function PayloadExtension:cb_game_session_disconnect()
	self._game = nil
end

function PayloadExtension:started()
	return self._started
end

function PayloadExtension:finished()
	return self._previous_status == "end"
end


function PayloadExtension:increment_kill_stat(killing_blow, breed_killed, breed_attacker, attacker_unit, ai_unit)
