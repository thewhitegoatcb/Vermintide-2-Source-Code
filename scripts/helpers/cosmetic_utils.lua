...
function CosmeticUtils.color_tint_unit(unit, hero_name, gradient_variation, gradient_value)
...
local cosmetic_slots = { slot_frame = true, slot_hat = true, slot_skin = true }
...
local cosmetic_and_weapon_slots = { "slot_ranged", "slot_melee", "slot_skin", "slot_hat", "slot_frame" }
...
local cosmetic_and_weapon_slots_lookup = { slot_hat = true, slot_skin = true, slot_frame = true, slot_melee = true, slot_ranged = true }
...
function CosmeticUtils.is_cosmetic_slot(slot_name)
	return cosmetic_slots [slot_name] ~= nil
end


function CosmeticUtils.is_cosmetic_item(item_type)
	return cosmetic_items [item_type] ~= nil
end



function CosmeticUtils.update_cosmetic_slot(player, slot, item_name, skin_name)
...
function CosmeticUtils.get_cosmetic_slot(player, slot)
...
function CosmeticUtils.is_weapon_slot(slot)
	return slot == "slot_melee" or slot == "slot_ranged"
end

function CosmeticUtils.is_valid(item_data)
	return item_data and item_data.item_name
end


function CosmeticUtils.get_default_cosmetic_slot(career_settings, slot_name)
	...
	if slot_name == "slot_skin" then
		do return { item_name = career_settings.base_skin } end
...
function CosmeticUtils.sync_local_player_cosmetics(player, profile_index, career_index)
	if not player then
