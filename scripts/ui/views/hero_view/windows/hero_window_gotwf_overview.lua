...
local ITEM_SIZE = { 260 * icon_scale, 220 * icon_scale }
...
function HeroWindowGotwfOverview:on_enter(params, offset)
...
function HeroWindowGotwfOverview:_reset_current_item()
...
function HeroWindowGotwfOverview:_sync_backend_gotwf()
	self._synced = false
...
function HeroWindowGotwfOverview:gotwf_data_cb(login_rewards)
...
function HeroWindowGotwfOverview:_start_transition_animation(animation_name)
	local params = {
		parent = self._parent,
...
function HeroWindowGotwfOverview:_start_item_rotation_animation(item_widget, reward_index)
	local params = {
		parent = self._parent,
		render_settings = self._render_settings,
...
function HeroWindowGotwfOverview:_init_scenegraph()
	self._ui_scenegraph = UISceneGraph.init_scenegraph(scenegraph_definition)
end

function HeroWindowGotwfOverview:_create_background_ui_elements(params)
...
function HeroWindowGotwfOverview:_create_ui_elements(params)
...
function HeroWindowGotwfOverview:_create_reward_popup()
	local reward_params = {
...
function HeroWindowGotwfOverview:_create_ui_animator()
	self._ui_animator = UIAnimator:new(self._ui_scenegraph, animation_definitions)
end

function HeroWindowGotwfOverview:_create_scrollbar()
...
function HeroWindowGotwfOverview:_select_current_reward()
	local login_rewards = self._login_rewards
...
function HeroWindowGotwfOverview:_calculate_duration()
	local login_rewards = self._login_rewards
...
function HeroWindowGotwfOverview:_create_claim_button_widget(reward_index)
	local login_rewards = self._login_rewards
	local rewards = login_rewards.rewards
...
function HeroWindowGotwfOverview:_create_reward_widget(reward_index)
...
function HeroWindowGotwfOverview:_update_claim_button_visibility()
...
function HeroWindowGotwfOverview:_animate_list_entries(dt)
...
function HeroWindowGotwfOverview:_animate_item_product(widget, dt, optional_hover)
...
function HeroWindowGotwfOverview:_populate_painting_data(widget, index, item, reward_index)
	...
	local function callback()
		...
		if painting_data.orientation == "horizontal" then
			...
			style ["painting_" .. reward_index].texture_size = { base_size, base_size * (1 - 2 * padding) }

			style ["painting_frame_" .. reward_index].area_size = { base_size, base_size * (1 - 2 * padding) }
			...
			style ["painting_" .. reward_index].texture_size = { base_size * (1 - 2 * padding), base_size }

			style ["painting_frame_" .. reward_index].area_size = { base_size * (1 - 2 * padding), base_size }
...
function HeroWindowGotwfOverview:_populate_item_widget(widget, index, reward)
...
function HeroWindowGotwfOverview:_populate_item_data(widget, index, item, reward_index)
	...
	if item_type == "frame" then
		...
		local scale = 1
		local spacing = 20
		local offset = { 10 + ( index - 1 ) * (ITEM_SIZE [1] + spacing), 20, 0 }
		local masked = true
		...
		if package_available then
			...
			return end Application.warning("Icon package not accessable for product_id: (%s) and texture_name: (%s)", item_name, texture_name) end end

function HeroWindowGotwfOverview:_create_material_instance(gui, new_material_name, template_material_name, reference_name)
...
function HeroWindowGotwfOverview:_set_material_diffuse(gui, material_name, texture_path)
...
function HeroWindowGotwfOverview:_load_texture_package(package_name, reference_name, callback)
...
function HeroWindowGotwfOverview:_is_unique_reference_to_material(reference_name)
	local cloned_materials_by_reference = self._cloned_materials_by_reference
...
function HeroWindowGotwfOverview:_unload_texture_by_reference(reference_name)
	local loaded_package_names = self._loaded_package_names
	local cloned_materials_by_reference = self._cloned_materials_by_reference
...
function HeroWindowGotwfOverview:_play_sound(event)
	self._parent:play_sound(event)
end

function HeroWindowGotwfOverview:on_exit(params)
...
function HeroWindowGotwfOverview:update(dt, t)
...
function HeroWindowGotwfOverview:_handle_reward_popup(dt, t)
	self._reward_popup:update(dt)
end

function HeroWindowGotwfOverview:_check_ready()
...
function HeroWindowGotwfOverview:_handle_popup()
...
function HeroWindowGotwfOverview:_claim_daily_reward(reward_offset)
	local num_allowed_old_segments_to_claim = self._login_rewards.num_allowed_old_segments_to_claim
...
function HeroWindowGotwfOverview:_claim_reward_result_cb(reward_index, login_rewards)
	if login_rewards.event_type ~= "calendar" then
		self._awaiting_result = false
		self._parent:unblock_input()
		Managers.ui:handle_transition("close_active", { fade_out_speed = 1, use_fade = true, fade_in_speed = 1 })
		return
	end
	...
	function self._ui_animations_callbacks.reveal()
...
function HeroWindowGotwfOverview:_trigger_replacement_rewards()
...
function HeroWindowGotwfOverview:_gather_replacement_presentation_data(reward_index)
	local login_rewards = self._login_rewards
	...
	local fake_item = { data = BackendUtils.get_fake_currency_item(data.code or "SM", data.amount) }
	local description = { }
	...
	PRESENTATION_DATA.offset = { 0, 190, 1 }

	return PRESENTATION_DATA
end

function HeroWindowGotwfOverview:_update_daily_rewards(reward_index)
...
function HeroWindowGotwfOverview:_update_selected_reward(item_widget)
	local login_rewards = self._login_rewards
	local rewards = login_rewards.rewards
	local claimed_rewards = login_rewards.claimed_rewards
...
function HeroWindowGotwfOverview:post_update(dt, t)
	if self._ready then
...
function HeroWindowGotwfOverview:_animate_buttons(dt, t)
...
function HeroWindowGotwfOverview:_animate_button(widget, dt)
	local content = widget.content
	local style = widget.style
...
function HeroWindowGotwfOverview:_handle_arrow_visibility(dt, t)
...
function HeroWindowGotwfOverview:_update_animations(dt)
...
function HeroWindowGotwfOverview:_handle_input(dt, t)
	local parent = self._parent
	...
	if not self._awaiting_result and not force_update then
		local hold_left_timer = 0 local hold_right_timer = 0
...
function HeroWindowGotwfOverview:_handle_input_descriptions(dt, t)
	local skip_mark_all_as_seen = true
	local current_reward_index = #self._login_rewards.rewards
...
function HeroWindowGotwfOverview:_draw(dt, t)
	local parent = self._parent
...
function HeroWindowGotwfOverview:_draw_background(dt, t)
	local parent = self._parent
	local ui_top_renderer = self._ui_top_renderer
	local ui_scenegraph = self._ui_scenegraph
	local parent = self._parent
