...
function HeroWindowPrestige:on_enter(params, offset)
...
function HeroWindowPrestige:on_exit(params)
	print("[HeroViewWindow] Exit Substate HeroWindowPrestige")
	self.ui_animator = nil
end

function HeroWindowPrestige:create_ui_elements(params, offset)
...
function HeroWindowPrestige:_setup_prestige_reward()
	...
	if not is_max_prestiged then
		...
		if item_type == "hat" then --[[ Nothing --]]

		elseif item_type == "frame" then
			reward_portrait_frame = item_data.name
		elseif item_type == "skin" then --[[ Nothing --]]
		end
...
function HeroWindowPrestige:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function HeroWindowPrestige:post_update(dt, t)

	return end


function HeroWindowPrestige:_update_animations(dt)
...
function HeroWindowPrestige:_is_button_pressed(widget)
...
function HeroWindowPrestige:_is_button_released(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function HeroWindowPrestige:_is_stepper_button_pressed(widget)
	local content = widget.content
...
function HeroWindowPrestige:_handle_input(dt, t)
...
function HeroWindowPrestige:draw(dt)
...
function HeroWindowPrestige:_play_sound(event)
	self.parent:play_sound(event)
end

function HeroWindowPrestige:_set_prestige_reward_portrait_frame(frame_settings_name)
