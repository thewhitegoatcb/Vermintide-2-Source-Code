...
local function store_scene_graph_data(item_unit, attachment_node_linking)
	...
	for i, attachment_nodes in ipairs(node_linking_data) do
		...
		if target_node ~= 0 then
			...
			scene_graph_data [#scene_graph_data + 1] = { i = target_node_index, parent = Unit.scene_graph_parent(item_unit, target_node_index), local_pose = Matrix4x4Box(Unit.local_pose(item_unit, target_node_index)) }
		end
	end
...
function AIInventoryExtension:_setup_configuration(unit, start_n, inventory_configuration, item_extension_init_data)
	local items = inventory_configuration.items local items_n = inventory_configuration.items_n
...
function AIInventoryExtension:init(unit, extension_init_data)
	...
	local item_extension_init_data = { ai_inventory_item_system = { wielding_unit = unit } }
	...
	if multiple_configurations then
		...
		for i = 1, #multiple_configurations do
			local item_set = { start_index = items_n + 1 }
...
function AIInventoryExtension:destroy()
	local unit_spawner = Managers.state.unit_spawner
...
function AIInventoryExtension:destroy_dropped_items(inventory_item_index)
...
function AIInventoryExtension:freeze()
...
function AIInventoryExtension:unfreeze()
...
function AIInventoryExtension:show_single_item(item_inventory_index, show)
...
function AIInventoryExtension:get_unit(category)
	return self.inventory_item_units_by_category [category]
end

function AIInventoryExtension:get_item_inventory_index(item_unit)
...
function AIInventoryExtension:drop_single_item(item_inventory_index, reason, optional_drop_direction)
	if script_data.ai_debug_inventory then
	...
	if item_extension and not item_extension.dropped and item.drop_reasons [reason] and item_unit_template_name ~= "ai_helmet_unit" and item_unit_template_name ~= "ai_outfit_unit" then
		...
		do return true, item_unit end
	else
		return false
	end
end

function AIInventoryExtension:disable_inventory_item(item, item_unit)
...
function AIInventoryExtension:_drop_unit(drop_unit_name, item_unit, item, item_inventory_index, reason, drop_multiple, optional_drop_direction)
...
function AIInventoryExtension:wield_item_set(item_set_index, ignore_animation_event)
	local unit = self.unit
...
function AIInventoryExtension:unwield_set(item_set_index)
	local item_set = self.item_sets [item_set_index]
...
function AIInventoryExtension:play_hit_sound(victim_unit, damage_type)
...
function AIInventoryExtension:hot_join_sync(peer_id)

	local channel_id = PEER_ID_TO_CHANNEL [peer_id]
	if self.hidden_item_index and
	ALIVE [self.unit] then
...
function AIInventoryExtension:add_additional_hit_sfx(additional_sfx_name)
	...
	if not override_ids then
		...
		additional_hit_sounds_ids [additional_sfx_name] = { unique_id }
...
function AIInventoryExtension:remove_additioanl_hit_sfx(override_id)
	local additional_hit_sounds_ids = self._additional_hit_sounds_ids
