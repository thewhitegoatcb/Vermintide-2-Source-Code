...
VoteTemplates.deus_settings_vote = { client_start_vote_rpc = "rpc_server_request_start_vote_lookup", ingame_vote = false, mission_vote = true, gamepad_support = true, text = "deus_settings_vote", minimum_voter_percent = 1, success_percent = 1, server_start_vote_rpc = "rpc_client_start_vote_lookup", duration = 30, priority = 110, min_required_voters = 1, gamepad_input_desc = "default_voting", timeout_vote_option = 2,
	...
	on_complete = function (vote_result, ingame_context, data)
		if vote_result == 1 then
			...
			local search_config = { any_level = true, dedicated_servers = false, dedicated_server = false, mechanism = "deus", join_method = "solo",
	...
	extract_sync_data = function (sync_data)
		...
		local data = {
			...
			act_key = act_key,
			difficulty = difficulty,
			dominant_god = dominant_god, quick_game = quick_game_id == 1, private_game = private_game_id == 1, always_host = always_host_id == 1, strict_matchmaking = strict_matchmaking_id == 1,




			matchmaking_type = matchmaking_type, twitch_enabled = twitch_enabled_id == 1,
