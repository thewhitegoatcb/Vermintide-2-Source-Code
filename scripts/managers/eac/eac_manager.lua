...
function EacManager:init()
	...
	self._file_related_violations = { hash_catalogue_error = true, hash_catalogue_file_not_found = true, unknown_game_file_version = true, required_game_file_not_found = true }
...
function EacManager:update(dt, t)
	local popup_manager = Managers.popup


	if self._popup_id ~= nil and
	popup_manager:query_result(self._popup_id) == "quit" then
		self._popup_id = nil
		Application.quit()
	end
	...
	( state == "untrusted" or state == "banned" ) and self._file_related_violations [violation] then
...
function EacManager:draw_panel(gui, dt)
...
function EacManager:_draw_indicator(gui, dt, state_text, violation_text, cause_text, explanation_text, reason_text)
	local margin = 8
