return { beam_material_name = "cloud_1", min_damage_distance = 4, display_name = "display_name_mutator_leash", beam_material_min_intensity = 0.5, player_effect_name = "fx/leash_beam_player_01", beam_effect_name = "fx/leash_beam_01", damage_percentage_per_interval = 0.02, max_damage_interval = 0.15, center_sound_event = "Play_mutator_leash_center", max_damage_distance = 7, damage_type = "damage_over_time", icon = "mutator_icon_leash", min_damage_interval = 1, description = "description_mutator_leash", stop_damage_sound_event = "Stop_mutator_leash_loop", beam_material_max_intensity = 5, start_damage_sound_event = "Play_mutator_leash_loop", damage_sound_global_parameter = "leash_distance", center_effect_name = "fx/leash_beam_center_01",
	...
	server_update_function = function (context, data)
		...
		for i = 1, #PLAYER_UNITS do
			...
			if HEALTH_ALIVE [player_unit] then
				...
				if template.min_damage_distance <= distance then
		...
		local min_damage_interval = template.min_damage_interval local max_damage_interval = template.max_damage_interval
		local min_damage_distance = template.min_damage_distance local max_damage_distance = template.max_damage_distance
	...
	client_update_function = function (context, data)
		...
		local start_damage_sound_event = template.start_damage_sound_event local stop_damage_sound_event = template.stop_damage_sound_event
		...
		if num_alive_players > 1 then
			...
			local center_effect_name = template.center_effect_name local center_sound_event = template.center_sound_event
			local player_effect_name = template.player_effect_name
			local beam_effect_name = template.beam_effect_name local beam_material_name = template.beam_material_name
			local beam_min_intensity = template.beam_material_min_intensity local beam_max_intensity = template.beam_material_max_intensity
			local beam_min_beam_distance = 0.5 local beam_max_beam_distance = template.max_damage_distance
			local beam_start_variable_id = data.beam_start_variable_id local beam_end_variable_id = data.beam_end_variable_id
			local min_damage_distance = template.min_damage_distance local max_damage_distance = template.max_damage_distance
			...
			if data.center_sound == nil then
				...
				data.center_sound = { source_id = source_id, event_id = event_id }
			end
			...
			for i = 1, #PLAYER_UNITS do
				...
				if HEALTH_ALIVE [player_unit] then
					if not beam_effects [player_unit] then
						...
						beam_effects [player_unit] = { beam_effect_id = beam_effect_id, player_effect_id = player_effect_id }
					end

					local player_effect_position = nil
