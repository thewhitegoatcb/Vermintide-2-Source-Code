...
local FALLBACK_FORMATION = { alternating = true, lead_dist_min = 2, lead_dist_max = 2, commander_avoid_radius = 1.2, dist = 4, formation_type = "circle",
...
function AICommanderExtension:init(extension_init_context, unit, extension_init_data)
	self._unit = unit
	...
	if self._is_server then
		...
		self._fallback_position_data = { n = 0, cell_width = 2, grid_width = 0 }
...
function AICommanderExtension:extensions_ready(world, unit)
...
function AICommanderExtension:destroy()
	return end

function AICommanderExtension:_claim_follow_index(unit)
	local next_index = #self._follow_indices + 1
	self._follow_indices [next_index] = unit
	self._follow_datas [unit] = { undergoing_avoidance = false,
...
function AICommanderExtension:_free_follow_index(unit)
...
function AICommanderExtension:register_follow_node_update(unit)
...
function AICommanderExtension:unregister_follow_node_update(unit)
	self._follow_units [unit] = nil
	self._units_to_recalculate [unit] = nil
end

function AICommanderExtension:follow_node_pending(blackboard)
	return blackboard.waiting_for_follow_node
end


function AICommanderExtension:follow_node_position(unit)
	local follow_data = self._follow_datas [unit]
	return follow_data and follow_data.lerped_follow_position
end

function AICommanderExtension:update(unit, input, dt, context, t)
	if self._is_server then
...
function AICommanderExtension:_on_controlled_unit_destroyed(controlled_unit)
	self:remove_controlled_unit(controlled_unit)
end

function AICommanderExtension:set_controlled_unit_template(controlled_unit, template_name, re_initialize, optional_t)
...
function AICommanderExtension:controlled_unit_template(controlled_unit)
	local data = self._controlled_units [controlled_unit]
	return data and data.template
end

function AICommanderExtension:add_controlled_unit(controlled_unit, template_name, t, skip_sync)
...
function AICommanderExtension:remove_controlled_unit(controlled_unit, skip_sync)
...
function AICommanderExtension:get_controlled_units()
	return self._controlled_units
end

function AICommanderExtension:get_controlled_units_count()
...
function AICommanderExtension:_update_follow(dt, t)
...
function AICommanderExtension:_update_follow_nodes(dt, t)
	...
	for controlled_unit in pairs(self._units_to_recalculate) do repeat
			...
			if use_fallback_formation then
				...
				if alternating then step_index = math.floor(step_index * 0.5) or step_index end
			...
			if Vector3.distance_squared(node_position:unbox(), follow_pos) > 0.09 then
				...
				if follow_pos then
					follow_data.target_follow_position:store(follow_pos) end end until true
	end


end

function AICommanderExtension:_lerp_follow_positions(dt)
	local nav_world = self._nav_world
...
function AICommanderExtension:_unit_arrived_at_follow_node(controlled_unit)
...
function AICommanderExtension:_avoid_unit(avoid_unit, controlled_unit, follow_pos, avoid_radius, blackboard, nav_world, nav_above, nav_below, dt, t)
...
function AICommanderExtension:_pair_best_follow_nodes()
	local follow_indices = self._follow_indices
...
function AICommanderExtension:_navify_follow_pos(follow_pos, nav_world, commander_unit_pos, lead_distance, blackboard)
...
function AICommanderExtension:_update_units(dt, t)
...
function AICommanderExtension:pet_ui_data(unit)
	...
	if pet_ui_type == "health" then
		...
		if health_extension then
			...
			do return template, current_health, max_health end
		end
...
function AICommanderExtension:controlled_units_in_combat()
	return self._combat_units
end

function AICommanderExtension:set_in_combat(controlled_unit, is_in_combat)
...
function AICommanderExtension:_calculate_hovered_friendly_unit()
	local radius = 1
	...
	for pet_unit in pairs(pets) do repeat
				...
				if angle_to_pet < best_angle then
					...
					if angle_to_pet <= max_angle_rad then
						...
						if self:command_state(pet_unit) ~= CommandStates.Following then
							commanded_unit = pet_unit end end end end until true
	end
...
function AICommanderExtension:_set_command_state(controlled_unit, command_state)
...
function AICommanderExtension:command_state(controlled_unit)
	local data = self._controlled_units [controlled_unit]
	return data and data.command_state
end

function AICommanderExtension:cancel_current_command(controlled_unit, ignore_attacking_units)
...
function AICommanderExtension:command_attack(controlled_unit, target_unit)
	if self._is_server then
...
function AICommanderExtension:command_stand_ground(controlled_unit, position, fallback_rotation)
	if self._is_server then
		self:cancel_current_command(controlled_unit)
...
function AICommanderExtension:hovered_friendly_unit()
...
function AICommanderExtension:hovered_commanded_unit()
...
function AICommanderExtension:_update_command_stand_ground()
...
function AICommanderExtension:_update_commands()
	if self._is_local then
...
function AICommanderExtension:command_stand_ground_group(units, target_position, fallback_rotation)
...
function AICommanderExtension:_command_stand_ground_group(units, target_position, fallback_rotation)
	table.array_remove_if(units, function (unit) return not self._controlled_units [unit] end)
...
function AICommanderExtension:_commander_is_on_navmesh()
...
function AICommanderExtension:_cleanup_command_buffs(controlled_unit, only_remove_on_command)
...
function AICommanderExtension:_add_command_buffs(controlled_unit, command_state)
...
function AICommanderExtension:_get_fallback_position(controlled_unit)
...
function AICommanderExtension:_store_fallback_position(controlled_unit)
	local fallback_positions = self._fallback_positions
	if fallback_positions [controlled_unit] then
		local stored_positions = self._stored_fallback_positions
		local fallback_data = self._fallback_position_data
