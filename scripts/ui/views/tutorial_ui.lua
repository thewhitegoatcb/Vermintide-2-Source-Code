...
local INFO_SLATES = { "mission_goal", "mission_objective", "side_mission", "tutorial", side_mission = 3, mission_objective = 2, tutorial = 4, mission_goal = 1 }
...
local function convert_current_screen_resolution_position_to_target_resolution(pos_width, pos_height, resolution_width, resolution_height)
	local current_width = RESOLUTION_LOOKUP.res_w local current_height = RESOLUTION_LOOKUP.res_h
...
function TutorialUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function TutorialUI:create_ui_elements()
...
function TutorialUI:get_player_first_person_extension()
	if self._first_person_extension then
		do return self._first_person_extension end
	else
...
function TutorialUI:update(dt, t)
...
function TutorialUI:post_update(dt, t)
...
local center_position = { definitions.scenegraph.root.size [1] * 0.5, definitions.scenegraph.root.size [2] * 0.5 }
function TutorialUI:update_mission_tooltip(tooltip_tutorial, player_unit, dt)
	local ui_scenegraph = self.floating_icons_ui_scene_graph
...
function TutorialUI:update_objective_tooltip(objective_tooltips, player_unit, dt)
	...
	for i = 1, units_n do repeat
			...
			if widget_holder_id then
				...
				new_units [new_units_n] = objective_unit end until true
	end
...
function TutorialUI:setup_objective_tooltip_widget(widget_holder, objective_tooltips, player_unit, dt)
...
function TutorialUI:_floating_icon_overlap(widget_holder, x, y, scale)
	local lookup = self._objective_tooltip_position_lookup
	local overlap = nil

	for _, pos in pairs(lookup) do
		if ( pos [1] <= x and x <= pos [1] + 200 * scale or x <= pos [1] and pos [1] <= x + 200 * scale ) and (
		pos [2] <= y and y <= pos [2] + definitions.FLOATING_ICON_SIZE [2] * scale or y <= pos [2] and pos [2] <= y + definitions.FLOATING_ICON_SIZE [2] * scale) then
			if y < pos [2] then
				overlap = -definitions.FLOATING_ICON_SIZE [2] * 0.75 * scale break
			end
	...
	lookup [widget_holder.scenegraph_id] = { x, y }

	return overlap
end

local objective_tooltip_settings = UISettings.tutorial.objective_tooltip
function TutorialUI:update_objective_tooltip_widget(widget_holder, player_unit, dt)
	local viewport_name = "player_1"
	local camera = nil
	if self.camera_manager:has_viewport(viewport_name) then
		local world_name = "level_world"
...
function TutorialUI:get_floating_icon_position(screen_pos_x, screen_pos_y, forward_dot, right_dot, tooltip_settings)
	...
	local screen_width = RESOLUTION_LOOKUP.res_w local screen_height = RESOLUTION_LOOKUP.res_h
...
function TutorialUI:floating_icon_animations(widget, animation_container, is_behind, is_clamped, tooltip_settings)
	...
	if is_clamped or is_behind then
		...
		if not animation_container.out_of_view and icon_style.color [1] ~= alpha_fade_out_value then
			...
			animation_container.out_of_view = UIAnimation.init(UIAnimation.function_by_time, icon_style.color,
			1, icon_style.color [1], alpha_fade_out_value, fade_out_time, math.easeInCubic)
		...
		if not animation_container.in_of_view and icon_style.color [1] ~= 255 then
			...
			animation_container.in_of_view = UIAnimation.init(UIAnimation.function_by_time, icon_style.color,
			1, icon_style.color [1], 255, fade_in_time, math.easeInCubic)
		end
		if not animation_container.in_of_view_text and text_style.text_color [1] ~= 255 then
			local fade_in_time = tooltip_settings.fade_in_time
			animation_container.in_of_view_text = UIAnimation.init(UIAnimation.function_by_time, text_style.text_color,
			1, text_style.text_color [1], 255, fade_in_time, math.easeInCubic)
			animation_container.in_of_view_text_shadow = UIAnimation.init(UIAnimation.function_by_time, text_shadow_style.text_color,
			1, text_shadow_style.text_color [1], 255, fade_in_time, math.easeInCubic)
		end
	end
end

function TutorialUI:get_arrow_angle_and_offset(forward_dot, right_dot, arrow_size, icon_size, height_from_center)
	local static_angle_value = 1.57079633
	local offset_x = 0 local offset_y = 0 local offset_z = 0
...
function TutorialUI:get_icon_size(position, player_position, current_size, original_size, tooltip_settings, scale)
...
function TutorialUI:icon_scale_by_distance(current_distance, max_distance)
...
function TutorialUI:distance_between_screen_positions(position_a, position_b)
	local width = position_a [1] - position_b [1]
	local height = position_a [2] - position_b [2]
	if width < 0 then width = -1 * width end
	if height < 0 then height = -1 * height end
	return { width, height }
end

function TutorialUI:convert_world_to_screen_position(camera, world_position)
	if camera then
...
function TutorialUI:animate_in_mission_tooltip(time, render_text, dt, widget, size_target)
	local icon_style = widget.style.texture_id
	local text_style = widget.style.text
...
function TutorialUI:add_info_slate_entries()
	for i = 1, definitions.NUMBER_OF_INFO_SLATE_ENTRIES do
		local widget = self.info_slate_widgets [i]
...
function TutorialUI:queue_info_slate_entry(info_slate_type, text, icon_texture, update_sound, template, unit, raycast_unit)
...
function TutorialUI:clear_tutorials()
...
function TutorialUI:complete_mission_info_slate(info_slate_type, entry_id)
...
function TutorialUI:update_info_slate_entry_text(info_slate_type, entry_id, text)
	local queue = self.queued_info_slate_entries [info_slate_type]
	for index, entry in pairs(queue) do
		if entry.entry_id == entry_id then
...
local anim_params = {
	slot_1 = { end_id = "info_slate_slot1_end", start_id = "info_slate_slot1_start" },
	slot_2 = { end_id = "info_slate_slot2_end", start_id = "info_slate_slot2_start" } }


function TutorialUI:update_info_slate_entries(dt, t)
	...
	if info_slate_entries then
		...
		elseif self.mission_goal_state == "animating_out" and
		ui_animator:is_animation_completed(self.mission_goal_anim_id) then
		...
		elseif self.mission_objective_state == "animating_out" and
		ui_animator:is_animation_completed(self.mission_objective_anim_id) then
			UIRenderer.set_element_visible(self.ui_renderer, widget.element, false)
		...
		elseif self.side_mission_state == "animating_out" and
		ui_animator:is_animation_completed(self.side_mission_anim_id) then
			UIRenderer.set_element_visible(self.ui_renderer, widget.element, false)
		...
		elseif self.tutorial_state == "animating_out" and
		ui_animator:is_animation_completed(self.tutorial_anim_id) then
			UIRenderer.set_element_visible(self.ui_renderer, widget.element, false)
...
function TutorialUI:_get_next_verified(queue, t)
...
function TutorialUI:info_slate_text_height(text, text_style)
...
function TutorialUI:play_sound(event)
...
function TutorialUI:add_health_bar(unit)
	...
	for i = 1, definitions.NUMBER_OF_HEALTH_BARS do
		if self.health_bars [i] == nil then
			...
			self.health_bars [i] = { health_percent = 1, init_position = true, visible_time = 0, damage_time = 0, visible = true, active = false,
				unit = unit,
...
function TutorialUI:remove_health_bar(unit)
	for i = 1, definitions.NUMBER_OF_HEALTH_BARS do
...
function TutorialUI:_get_health_bar_by_unit(unit)
	for i = 1, definitions.NUMBER_OF_HEALTH_BARS do
		local health_bar = self.health_bars [i]
		if health_bar and health_bar.unit == unit then
			return health_bar
...
function TutorialUI:show_health_bar(unit, visible)
...
function TutorialUI:update_health_bars(dt, player_unit)
...
function TutorialUI:set_visible(visible)
