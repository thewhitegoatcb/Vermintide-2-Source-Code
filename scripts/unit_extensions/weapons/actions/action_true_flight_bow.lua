...
function ActionTrueFlightBow:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionTrueFlightBow:client_owner_start_action(new_action, t, chain_action_data, power_level, action_init_data)
	...
	if chain_action_data then
		...
		if not self.targets then
			self.targets = { chain_action_data.target }
		end
	end
	...
	if action_init_data then
		...
		if not self.targets then
			self.targets = { action_init_data.target }
		end
	end
...
function ActionTrueFlightBow:client_owner_post_update(dt, t, world, can_damage)
...
function ActionTrueFlightBow:finish(reason, data)
...
function ActionTrueFlightBow:fire(current_action)
	local owner_unit = self.owner_unit
	...
	for i = 1, num_projectiles do
		...
		local scale = 1
		ActionUtils.spawn_true_flight_projectile(
		owner_unit,
		target_unit,
		self.true_flight_template_id,
		position,
		fire_rotation,
		angle,
		target_vector,
