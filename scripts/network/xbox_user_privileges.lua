...
function XboxUserPrivileges:init()
	self:reset()
	self:_setup_lookup_tables()
end

function XboxUserPrivileges:reset()
	self._current_users = { }
...
function XboxUserPrivileges:add_user(user_id)
	self:reset()
...
function XboxUserPrivileges:get_privilege_async(user_id, privilege, attempt_resolution, external_cb)
...
function XboxUserPrivileges:update_privilege(privilege, cb)
...
function XboxUserPrivileges:cb_user_privilege_done(user_id, privilege, external_cb, info)
	if info.error then
...
function XboxUserPrivileges:has_privilege(user_id, privilege)
	if user_id and self._current_users [user_id] then
		do return self._current_users [user_id] [privilege] end
	else
		return false
	end
end
...
function XboxUserPrivileges:is_initialized()
...
function XboxUserPrivileges:has_error()
	return self._has_error
end

function XboxUserPrivileges:_setup_lookup_tables()
