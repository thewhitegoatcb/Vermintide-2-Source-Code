...
function EnemyPackageLoader:init()
...
local rpcs = { "rpc_from_server_load_breed_package", "rpc_from_server_load_breeds_by_bitmask", "rpc_from_server_unload_breed_package", "rpc_from_client_loading_breed_package_done", "rpc_from_client_sync_packages_load_done" }
...
function EnemyPackageLoader:register_rpcs(network_event_delegate)
	self.network_event_delegate = network_event_delegate
	network_event_delegate:register(self, unpack(rpcs))
end

function EnemyPackageLoader:unregister_rpcs()
	self.network_event_delegate:unregister(self)
	self.network_event_delegate = nil
end

function EnemyPackageLoader:set_unit_spawner(unit_spawner)
	self._unit_spawner = unit_spawner
end


function EnemyPackageLoader:network_context_created(lobby, server_peer_id, own_peer_id)
...
function EnemyPackageLoader:network_context_destroyed()
...
function EnemyPackageLoader:_pick_breed_from_processed_breeds(breeds, limit)
...
function EnemyPackageLoader:request_breed(breed_name, ignore_breed_limits, spawn_category)
	...
	if not ignore_breed_limits and package_limit <= current_packages then
		...
		if unused_breed_name then
			...
			if replacement_breed_override_func then
				...
				do return false, replacement_breed_name end
			else
...
function EnemyPackageLoader:_find_patrol_replacement()
...
function EnemyPackageLoader:_set_breed_processed(breed_name, processed)
	local breed_processed = self.breed_processed
	local aliases = BREED_TO_ALIASES [breed_name]
	if aliases then
		local num_aliases = #aliases
...
function EnemyPackageLoader:_set_breed_loaded_on_all_peers(breed_name, loaded)
...
function EnemyPackageLoader:_set_breed_package_lock(breed_name, locked)
...
function EnemyPackageLoader:update_breeds_loading_status()
...
function EnemyPackageLoader:lock_breed_package(breed_name)
	self:_set_breed_package_lock(breed_name, true)
end

function EnemyPackageLoader:unlock_breed_package(breed_name)
	self:_set_breed_package_lock(breed_name, false)
end

function EnemyPackageLoader:_start_loading_package(breed_name, callback_name)
...
function EnemyPackageLoader:_load_package(breed_name, breed_category_data)
...
function EnemyPackageLoader:_unload_package(breed_name, breed_category_data)
...
function EnemyPackageLoader:update()
...
function EnemyPackageLoader:load_sync_done_for_peer(peer_id)
...
function EnemyPackageLoader:report_if_synced_to_server()
...
function EnemyPackageLoader:cb_breed_package_loaded(breed_name)
	local breed_id = NetworkLookup.breeds [breed_name]
	local is_server = self._is_server
	if is_server then
...
function EnemyPackageLoader:cb_breed_package_loaded_client_startup(breed_name)
	self._package_state [breed_name] = "loaded"
	self:report_if_synced_to_server()
end

function EnemyPackageLoader:cb_startup_breed_package_loaded(breed_name)
	self._package_state [breed_name] = "loaded"
	self:_set_breed_loaded_on_all_peers(breed_name, true)
	self:report_if_synced_to_server()
end
...
function EnemyPackageLoader:_add_breed_package_name(breed_list)
	local breed_to_package_name = self._breed_to_package_name
...
function EnemyPackageLoader:_add_missing_breeds_to_dynamic()
...
function EnemyPackageLoader:_create_breed_category_lookup(breed_list, category, limit, replacement_breed_override_funcs)
	...
	if breed_category_loaded_packages [category] == nil then
		breed_category_loaded_packages [category] = { current = 0, name = category, limit = limit, loaded_breeds = { }, breeds = breed_list, replacement_breed_override_funcs = replacement_breed_override_funcs }
	end
...
function EnemyPackageLoader:_reset_dynamic_breed_lookups()
...
function EnemyPackageLoader:_remove_locked_directors(director_list, failed_locked_functions)
...
function EnemyPackageLoader:_get_directors_from_breed_budget(spawn_breed_hash, num_needed_directors, director_list, breed_cap, difficulty_name, non_random_conflict_directors, seed)
		...
		while #director_list > 0 do
			...
			if success then
				...
				if new_breeds > 0 then
					print_breed_hash(new_breed_list, "\t--> Added these breeds: ") break
				end
...
function EnemyPackageLoader:_remove_directors_by_breed_budget(director_names_list, breeds_in_use, difficulty_name, breed_cap)
	local new_breed_list = { }
...
function EnemyPackageLoader:_get_factions_from_directors(director_names_list)
	local factions = { }
...
function EnemyPackageLoader:_make_faction_list(available_factions, mandatory_factions, faction_weights, seed, preferred_num_faction)
...
function EnemyPackageLoader:_remove_directors_not_in_factions(director_names_list, faction_list)
...
function EnemyPackageLoader:_get_startup_breeds(level_key, level_seed, failed_locked_functions, use_random_directors, conflict_director_name, difficulty, difficulty_tweak)
	...
	local crossroads = spawn_zone_data.crossroads local main_paths = spawn_zone_data.main_paths local zones = spawn_zone_data.zones local num_main_zones = spawn_zone_data.num_main_zones local path_markers = spawn_zone_data.path_markers
...
function EnemyPackageLoader:setup_startup_enemies(level_key, level_seed, failed_locked_functions, use_random_directors, conflict_director_name, difficulty, difficulty_tweak)
	...
	if not breeds_to_load_at_startup.loaded then
		...
		print(
		"[EnemyPackageLoader] setup_startup_enemies - level_key:", level_key,
		"- level_seed:", level_seed,
		"- use_random_directors:", use_random_directors,
...
function EnemyPackageLoader:_load_startup_enemy_packages()
	local breeds_to_load_at_startup = self._breeds_to_load_at_startup
	...
	local async = true local prioritize = false
	local package_manager = Managers.package
...
function EnemyPackageLoader:loading_completed()
...
function EnemyPackageLoader:unload_enemy_packages(force_unload_startup_packages)
...
function EnemyPackageLoader:_sync_dynamic_to_client(peer_id, connection_key)
	local package_state = self._package_state
	local breeds_to_load_at_startup = self._breeds_to_load_at_startup
...
function EnemyPackageLoader:rpc_from_server_load_breeds_by_bitmask(channel_id, connection_key, bitmasks)
...
function EnemyPackageLoader:rpc_from_server_load_breed_package(channel_id, connection_key, breed_id)
...
function EnemyPackageLoader:rpc_from_server_unload_breed_package(channel_id, connection_key, breed_id)
	local server_peer_id = self._server_peer_id
	local unique_connection_key = self._unique_connection_key
	local peer_id = CHANNEL_TO_PEER_ID [channel_id]
	if server_peer_id ~= peer_id then
...
function EnemyPackageLoader:rpc_from_client_loading_breed_package_done(channel_id, connection_key, breed_id)
	local peer_id = CHANNEL_TO_PEER_ID [channel_id]
...
function EnemyPackageLoader:rpc_from_client_sync_packages_load_done(channel_id, connection_key)
	local peer_id = CHANNEL_TO_PEER_ID [channel_id]
	local client_key = self._unique_connections [peer_id]
	if client_key ~= connection_key then
...
function EnemyPackageLoader:_send_rpc(rpc_name, peer_id, ...)
...
function EnemyPackageLoader:_send_rpc_to_server(rpc_name, ...)
	if not self._lobby then
		return
	end
...
function EnemyPackageLoader:_send_rpc_to_clients(rpc_name, ...)
	local rpc = RPC [rpc_name]
	local server_peer_id = self._server_peer_id
...
function EnemyPackageLoader:_send_rpc_to_clients_except(rpc_name, except, ...)
	local rpc = RPC [rpc_name]
	local server_peer_id = self._server_peer_id
	local unique_connections = self._unique_connections
...
function EnemyPackageLoader:get_startup_breeds()
	return self._breeds_to_load_at_startup
end


function EnemyPackageLoader:client_connected(peer_id)
...
function EnemyPackageLoader:client_disconnected(peer_id)
