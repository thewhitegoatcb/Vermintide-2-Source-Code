...
function TalentExtension:init(extension_init_context, unit, extension_init_data)
	self._unit = unit
...
function TalentExtension:extensions_ready(world, unit)
...
function TalentExtension:game_object_initialized(unit, unit_go_id)
	local talent_ids = self:get_talent_ids()
	self:_send_rpc_sync_talents(talent_ids)
end

function TalentExtension:talents_changed()
	local talent_ids = self:get_talent_ids()
...
function TalentExtension:_send_rpc_sync_talents(talent_ids)
...
function TalentExtension:apply_buffs_from_talents(talent_ids)
					...
					for j = 1, num_buffs do
						...
						if sub_buffs then
							for k = 1, sub_buffs.num_buffs do
								buff_extension:add_buff(sub_buffs.buff_name, { attacker_unit = player.player_unit })
							end
						end
...
function TalentExtension:update_talent_weapon_index(talent_ids)
	if Managers.state.game_mode:has_activated_mutator("whiterun") then
		return
	end
...
function TalentExtension:get_talent_career_skill_index()
	return self.talent_career_skill_index
end

function TalentExtension:get_talent_career_weapon_index()
	return self.talent_career_weapon_index
end

function TalentExtension:_clear_buffs_from_talents()
	local buff_extension = self.buff_extension
	local talent_buff_ids = self._talent_buff_ids
...
function TalentExtension:has_talent(talent_name)
	if Managers.state.game_mode:has_activated_mutator("whiterun") then
		return false
	end
...
function TalentExtension:get_talent_ids()
...
function TalentExtension:has_talent_perk(perk)
	local hero_name = self._hero_name
...
function TalentExtension:get_talent_names()
	local talent_ids = self:get_talent_ids()
	local talent_names = { }
...
function TalentExtension:_broadcast_talents_changed(is_spawning)
...
function TalentExtension:destroy()
	return end

function TalentExtension:initial_talent_synced()
	return true
end


function TalentExtension:_check_talent_package_dendencies(talent_ids, initial_setup)
