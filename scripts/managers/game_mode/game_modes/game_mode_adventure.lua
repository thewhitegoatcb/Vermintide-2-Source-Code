...
function GameModeAdventure:init(settings, world, ...)
...
function GameModeAdventure:destroy()
	return end

function GameModeAdventure:cleanup_game_mode_units()
	local update_safe = false
	self:_clear_bots(update_safe)
end

function GameModeAdventure:register_rpcs(network_event_delegate, network_transmit)
...
function GameModeAdventure:player_entered_game_session(peer_id, local_player_id, wanted_party_index)
...
function GameModeAdventure:player_left_game_session(peer_id, local_player_id)
...
function GameModeAdventure:remove_bot(peer_id, local_player_id, update_safe)
...
function GameModeAdventure:get_end_screen_config(game_won, game_lost, player)
	...
	if game_won then
		...
		screen_config = { show_act_presentation = true,
...
function GameModeAdventure:ended(reason)
...
function GameModeAdventure:local_player_ready_to_start(player)
...
function GameModeAdventure:local_player_game_starts(player, loading_context)
...
function GameModeAdventure:disable_player_spawning()
	self._adventure_spawning:set_spawning_disabled(true)
end

function GameModeAdventure:enable_player_spawning(safe_position, safe_rotation)
...
function GameModeAdventure:teleport_despawned_players(position)
	self._adventure_spawning:teleport_despawned_players(position)
end

function GameModeAdventure:flow_callback_add_spawn_point(unit)
	self._adventure_spawning:add_spawn_point(unit)
end

function GameModeAdventure:set_override_respawn_group(respawn_group_name, active)
	self._adventure_spawning:set_override_respawn_group(respawn_group_name, active)
end

function GameModeAdventure:set_respawn_group_enabled(respawn_group_name, enabled)
	self._adventure_spawning:set_respawn_group_enabled(respawn_group_name, enabled)
end

function GameModeAdventure:set_respawn_gate_enabled(respawn_gate_unit, enabled)
	self._adventure_spawning:set_respawn_gate_enabled(respawn_gate_unit, enabled)
end

function GameModeAdventure:respawn_unit_spawned(unit)
	self._adventure_spawning:respawn_unit_spawned(unit)
end

function GameModeAdventure:respawn_gate_unit_spawned(unit)
	self._adventure_spawning:respawn_gate_unit_spawned(unit)
end

function GameModeAdventure:get_respawn_handler()
	return self._adventure_spawning:get_respawn_handler()
end

function GameModeAdventure:set_respawning_enabled(enabled)
	self._adventure_spawning:set_respawning_enabled(enabled)
end

function GameModeAdventure:remove_respawn_units_due_to_crossroads(removed_path_distances, total_main_path_length)
	self._adventure_spawning:remove_respawn_units_due_to_crossroads(removed_path_distances, total_main_path_length)
end

function GameModeAdventure:recalc_respawner_dist_due_to_crossroads()
	self._adventure_spawning:recalc_respawner_dist_due_to_crossroads()
end

function GameModeAdventure:force_respawn(peer_id, local_player_id)
	local status = Managers.party:get_player_status(peer_id, local_player_id)
...
function GameModeAdventure:force_respawn_dead_players()
	self._adventure_spawning:force_respawn_dead_players()
end

function GameModeAdventure:_get_first_available_bot_profile()
...
function GameModeAdventure:_setup_bot_spawn_priority_lookup()
...
function GameModeAdventure:_handle_bots(t, dt)
...
function GameModeAdventure:_add_bot(bot_players)
	local party_id = 1
...
function GameModeAdventure:_remove_bot(bot_players, index, update_safe)
...
function GameModeAdventure:_remove_bot_by_profile(bot_players, profile_index, update_safe)
	local bot_index = nil
...
function GameModeAdventure:_clear_bots(update_safe)
	local bot_players = self._bot_players
	local num_bot_players = #bot_players
...
function GameModeAdventure:get_active_respawn_units()
	return self._adventure_spawning:get_active_respawn_units()
end

function GameModeAdventure:get_player_wounds(profile)
