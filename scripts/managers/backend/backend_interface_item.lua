...
local CLEARABLE_SLOTS = { slot_trinket_2 = true, slot_trinket_3 = true, slot_trinket_1 = true }





local MUST_HAVE_SLOTS = { slot_hat = true, slot_skin = true, slot_frame = true, slot_melee = true, slot_ranged = true }
...
local COSMETIC_ITEMS = { frame = true, skin = true, hat = true }
...
local STARTING_ITEMS = { dr_shield_axe_0001 = true, dr_crossbow_0001 = true, we_shortbow_0001 = true, dr_helmet_0001 = true, bw_skullstaff_fireball_0001 = true, es_blunderbuss_0001 = true, ww_hood_0001 = true, bw_gate_0001 = true, bw_sword_0001 = true, we_dual_wield_daggers_0001 = true, es_2h_hammer_0001 = true, es_hat_0001 = true, wh_hat_0001 = true, wh_brace_of_pistols_0001 = true, wh_fencing_sword_0001 = true }
...
local function clean_inventory(items, loadout, whitelist)
	...
	for profile_name, slots in pairs(loadout) do
		for slot, backend_id in pairs(slots) do
			if slot == "backend_id" then --[[ Nothing --]]
			...
			elseif not items [backend_id] then
				...
				if MUST_HAVE_SLOTS [slot] then
					empty_must_have_slots [#empty_must_have_slots + 1] = { slot = slot, profile_name = profile_name }
				end
...
function Items:set_item_whitelist(item_keys)
...
function Items:_refresh_entities_if_needed()
	if self._dirty then
...
function Items:get_all_backend_items()
...
function Items:get_filtered_items(filter, params)
...
function Items:set_error(error_data)
	self._error_data = error_data
end

function Items:check_for_errors()
...
function Items:update(dt)
	...
	if self._dice_game_data then
		...
		elseif self._dice_game_data.time_out < Managers.time:time("main") then
			self._dice_game_data = nil
			self:set_error({ reason = BACKEND_LUA_ERRORS.ERR_DICE_TIMEOUT1 })
		end
	elseif self._upgrades_failed_game_data then
		...
		elseif self._upgrades_failed_game_data.time_out < Managers.time:time("main") then
			self._upgrades_failed_game_data = nil
			self:set_error({ reason = BACKEND_LUA_ERRORS.ERR_UPGRADES_TIMEOUT })
		end
	end
end

function Items:reset_dice_game_item()
	self._dice_item = nil
end

function Items:dice_game_item()
	return self._dice_item
end

function Items:poll_upgrades()
...
function Items:get_loadout()
	self:_refresh_entities_if_needed()
	return self._loadout
end

function Items:generate_item_server_loot(dice, difficulty, start_level, end_level, hero_name, dlc_name)
	...
	local parameters = { "param_dice", tostring(dice), "param_difficulty", difficulty, "param_start_level", start_level, "param_end_level", end_level }
	if hero_name then
	...
	self._dice_game_data = { time_out = time_out, parameters = parameters }
end

function Items:upgrades_failed_game(start_level, end_level)
	fassert(not self._dice_game_data and not self._upgrades_failed_game_data, "Trying to do two item server scripts at once. DiceGame: %s, UpgradesFailedGame: %s", self._dice_game_data and "true", self._upgrades_failed_game_data and "true")
	local time_out = Managers.time:time("main") + 20
	self._upgrades_failed_game_data = { time_out = time_out, start_level = start_level, end_level = end_level }
end

function Items:num_current_item_server_requests()
	return self._queue:num_current_requests()
end

function Items:make_dirty()
	self._dirty = true
end

function Items:set_data_server_queue(queue)
	self._queue = queue
end

function Items:data_server_queue()
	return self._queue
end

BackendInterfaceItem = class(BackendInterfaceItem)

function BackendInterfaceItem:init()
	self._backend_items = Items:new()
	self._modified_templates = { }
end

function BackendInterfaceItem:type()
	return "backend"
end

function BackendInterfaceItem:update()

	self._backend_items:update()

end

function BackendInterfaceItem:refresh_entities()
...
function BackendInterfaceItem:check_for_errors()
	return self._backend_items:check_for_errors()
end

function BackendInterfaceItem:num_current_item_server_requests()
	return self._backend_items:num_current_item_server_requests()
end

function BackendInterfaceItem:set_properties_serialized(backend_id, properties)
	local error_code = BackendItem.set_traits(backend_id, properties)
end

function BackendInterfaceItem:get_properties(backend_id)
...
function BackendInterfaceItem:get_traits(backend_id)
...
function BackendInterfaceItem:set_runes(backend_id, runes)
...
function BackendInterfaceItem:get_runes(backend_id)
	local rune_interface = Managers.backend:get_interface("runes")
	local runes = rune_interface:get(backend_id)
	return runes
end

function BackendInterfaceItem:get_key(backend_id)
...
function BackendInterfaceItem:get_item_from_id(backend_id)
...
function BackendInterfaceItem:get_all_backend_items()
	return self._backend_items:get_all_backend_items()
end

function BackendInterfaceItem:get_loadout()
	return self._backend_items:get_loadout()
end

function BackendInterfaceItem:get_loadout_item_id(career_name, slot)
...
function BackendInterfaceItem:get_filtered_items(filter)
	local items = self._backend_items:get_filtered_items(filter)

	return items
end

function BackendInterfaceItem:set_loadout_item(item_id, profile, slot)
	local items = self._backend_items:get_all_backend_items()
	if item_id then
...
function BackendInterfaceItem:remove_item(backend_id, ignore_equipped)
...
function BackendInterfaceItem:award_item(item_key)
	BackendItem.award_item(item_key)
	self._backend_items:make_dirty()
end

function BackendInterfaceItem:data_server_script(script_name, ...)
	local queue = self._backend_items:data_server_queue()
	local request = queue:add_item(script_name, ...)
	return request
end

function BackendInterfaceItem:upgrades_failed_game(level_start, level_end)
	self._backend_items:upgrades_failed_game(level_start, level_end)
end

function BackendInterfaceItem:generate_item_server_loot(dice, difficulty, start_level, end_level, hero_name, dlc_name)
	local dice_string = ""
...
function BackendInterfaceItem:check_for_loot()
...
function BackendInterfaceItem:equipped_by(backend_id)
...
function BackendInterfaceItem:is_equipped(backend_id, profile_name)
	local loadout = self._backend_items:get_loadout()
	for hero, slots in pairs(loadout) do
...
local SalvageableSlotTypes = { ranged = true, melee = true, hat = true, trinket = true }
...
local SalvageableRarities = { common = true, plentiful = true, exotic = true, rare = true, unique = true }
...
function BackendInterfaceItem:is_salvageable(backend_id)
...
local FuseableSlotTypes = { melee = true, ranged = true }




local FuseableRarities = { common = true, plentiful = true, rare = true }





function BackendInterfaceItem:is_fuseable(backend_id)
	local unequipped = not self:is_equipped(backend_id)
	local items = self._backend_items:get_all_backend_items()
	local item = items [backend_id]
	local item_config = ItemMasterList [item.key]
...
function BackendInterfaceItem:set_data_server_queue(queue)
...
function BackendInterfaceItem:__dirtify()
	self._backend_items:make_dirty()
end

function BackendInterfaceItem:has_item(item_key)
	local items, loadout = BackendItem.get_items()
...
function BackendInterfaceItem:clean_inventory_for_prestige(profile_index, unit)
	local items, loadout = BackendItem.get_items()
	local missing_items = nil
...
function BackendInterfaceItem:get_runes(item_id)
...
function BackendInterfaceItem:_slot_item_rune(item_data, rune_to_insert)
	return end



function BackendInterfaceItem:get_item_template(item_data, backend_id)
...
function BackendInterfaceItem:_command_item_whitelist(data)
