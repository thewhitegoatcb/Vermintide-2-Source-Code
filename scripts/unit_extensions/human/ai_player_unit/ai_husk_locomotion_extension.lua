...
function AiHuskLocomotionExtension:init(extension_init_context, unit, extension_init_data)
	self._unit = unit
	...
	self.constrain_min = { 0, 0, 0 }
	self.constrain_max = { 0, 0, 0 }
...
function AiHuskLocomotionExtension:destroy()


	self:_cleanup()
end

function AiHuskLocomotionExtension:freeze()
	self:_cleanup()
end

function AiHuskLocomotionExtension:_cleanup()
...
function AiHuskLocomotionExtension:unfreeze()
	local unit = self._unit
	Unit.set_animation_merge_options(unit)
...
function AiHuskLocomotionExtension:set_animation_translation_scale(animation_translation_scale)
...
function AiHuskLocomotionExtension:set_animation_rotation_scale(animation_rotation_scale)
...
function AiHuskLocomotionExtension:set_affected_by_gravity(affected)
...
function AiHuskLocomotionExtension:set_animation_driven(is_animation_driven, is_affected_by_gravity, has_script_driven_rotation, is_on_transport)
...
function AiHuskLocomotionExtension:set_mover_disable_reason(reason, state)
	MoverHelper.set_disable_reason(self._unit, self._mover_state, reason, state)
end

function AiHuskLocomotionExtension:set_constrained(constrain, min, max)
	if not self._engine_extension_id then
		return
	end
...
function AiHuskLocomotionExtension:teleport_to(position, rotation, velocity, dontseparate)
	if not self._engine_extension_id then
		return
	end
...
function AiHuskLocomotionExtension:set_collision_disabled(reason, state)
	if self._collision_state then
		MoverHelper.set_collision_disable_reason(self._unit, self._collision_state, reason, state)
	end
end

function AiHuskLocomotionExtension:current_velocity()
	return self._velocity:unbox()
end

function AiHuskLocomotionExtension:traverse_logic()
	return self._traverse_logic
end

function AiHuskLocomotionExtension:hot_join_sync(sender)
