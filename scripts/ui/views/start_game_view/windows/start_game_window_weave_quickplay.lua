...
function StartGameWindowWeaveQuickplay:on_enter(params, offset)
...
function StartGameWindowWeaveQuickplay:_start_transition_animation(animation_name)
	local params = {
...
function StartGameWindowWeaveQuickplay:_create_ui_elements(params, offset)
...
function StartGameWindowWeaveQuickplay:_setup_input_buttons()
...
function StartGameWindowWeaveQuickplay:on_exit(params)
	print("[StartGameWindow] Exit Substate StartGameWindowWeaveQuickplay")
	self._ui_animator = nil
end

function StartGameWindowWeaveQuickplay:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function StartGameWindowWeaveQuickplay:_handle_gamepad_activity()
...
function StartGameWindowWeaveQuickplay:_update_can_play()
	local widgets_by_name = self._widgets_by_name
...
function StartGameWindowWeaveQuickplay:_sync_selected_difficulty()
...
function StartGameWindowWeaveQuickplay:_handle_input(dt, t)
	local parent = self._parent
	local widgets_by_name = self._widgets_by_name
...
function StartGameWindowWeaveQuickplay:_is_button_released(widget)
	local content = widget.content
...
function StartGameWindowWeaveQuickplay:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot or content.hotspot
	return hotspot.on_hover_enter
end

function StartGameWindowWeaveQuickplay:post_update(dt, t)
	return end

function StartGameWindowWeaveQuickplay:_update_animations(dt)
	local difficulty_widgets = self._difficulty_widgets
...
function StartGameWindowWeaveQuickplay:_play_sound(event)
	self._parent:play_sound(event)
end

function StartGameWindowWeaveQuickplay:_exit(selected_level)
	self.exit = true
	self.exit_level_id = selected_level
end

function StartGameWindowWeaveQuickplay:draw(dt)
...
function StartGameWindowWeaveQuickplay:_animate_pulse(target, target_index, from, to, speed)
	local new_animation = UIAnimation.init(UIAnimation.pulse_animation, target, target_index,
	from, to, speed)
	return new_animation
end

function StartGameWindowWeaveQuickplay:_animate_element_by_time(target, target_index, from, to, time)
	local new_animation = UIAnimation.init(UIAnimation.function_by_time, target, target_index,
	from, to, time, math.ease_out_quad)
	return new_animation
end

function StartGameWindowWeaveQuickplay:_animate_element_by_catmullrom(target, target_index, target_value, p0, p1, p2, p3, time)
	local new_animation = UIAnimation.init(UIAnimation.catmullrom, target, target_index, target_value,
	p0, p1, p2, p3, time)
	return new_animation
end

function StartGameWindowWeaveQuickplay:_set_difficulty_selected(difficulty_key)
	local widgets_by_name = self._widgets_by_name
...
function StartGameWindowWeaveQuickplay:_setup_difficulties()
...
function StartGameWindowWeaveQuickplay:_animate_difficulty_button(widget, dt)
	local content = widget.content
