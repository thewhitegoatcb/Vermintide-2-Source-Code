	...
	if table.contains(UnlockableLevels, level_name) then
		...
		for j = 1, scrap_count_level_num do
			...
			AchievementTemplates.achievements [id] = {
				...
				desc = function () return string.format(Localize("achv_" .. id .. "_desc"), QuestSettings.scrap_count_level [j]) end,
				...
				completed = function (statistics_db, stats_id)
					return QuestSettings.scrap_count_level [j] <= statistics_db:get_persistent_stat(stats_id, statistics_id, level_name)
				end,
				progress = function (statistics_db, stats_id)
					...
					return { progression_comparison, QuestSettings.scrap_count_level [j] }
				end }
		end
	end
end
...
for i = 1, scrap_count_generic_num do
	...
	AchievementTemplates.achievements [id] = { icon = "achievement_trophy_gecko_scraps_generic",
		name = "achv_" .. id .. "_name",

		desc = function () return string.format(Localize("achv_" .. id .. "_desc"), QuestSettings.scrap_count_generic [i]) end,
		completed = function (statistics_db, stats_id)
			local total_scrap_amount = nil
			...
			return total_scrap_amount and QuestSettings.scrap_count_generic [i] <= total_scrap_amount
		end,
		progress = function (statistics_db, stats_id)
			local total_scrap_amount = nil
			local statistics_id = "collected_painting_scraps_generic"
			...
			return { progression_comparison, QuestSettings.scrap_count_generic [i] }
		end } end