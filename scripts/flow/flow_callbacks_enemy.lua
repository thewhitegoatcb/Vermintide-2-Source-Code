...
function flow_callback_enemy_dissolve_data(params)
	...
	return { dissovle_time = enemy_dissovle_time, darken_time = enemy_darken_time, darken_to = enemy_darken_to_value }
end

function flow_callback_enemy_dissolve_darken_vector(params)


	return { darken_vector = Vector3(1, enemy_darken_to_value, enemy_darken_time) }
end
...
local function enemy_variation_tint_part(unit, outfit_units, variation, material_result)
	...
	for i = 1, #variation.materials do
		for j = 1, #variation.variables do
			table.insert(material_result, { material = variation.materials [i], variable = variation.variables [j], value = variable_value, meshes = variation.meshes })
		end
	end
	return { }
end
...
local function enemy_dismember_get_helmet_units(unit, unit_inventory_extension)
	local helmet_units = { }
	helmet_units =
	unit_inventory_extension ~= nil and unit_inventory_extension.inventory_item_helmet_units or

	Unit.get_data(unit, "helmet_items") or { }
...
function enemy_explode(params)
	...
	if ( BloodSettings == nil or BloodSettings.enemy_blood.enabled ) and
	explodesettings.vfx_align_node ~= nil then
