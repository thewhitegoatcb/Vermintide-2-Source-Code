...
local function spawn_graph_units(world, level_ref_values, graph)
	...
	for key, node in pairs(graph) do
		...
		for _, next in pairs(node.next) do
			...
			local distance_to_edge_squared = math.lerp(
			distance_to_edge_a_squared,
			distance_to_edge_b_squared,
...
local function setup_fog_plane(world, level_ref_values, graph_data, visibility_data, debug_drawer)
	...
	Gui.bitmap(
	gui,
	"default_deus_map_fog_mask_clear",
	Vector3(0, 0, 0),
	Vector2(w, h),
	Color(255, 0, 0, 0))
	...
	local function draw_triangle(material, from_multiplier, to_multiplier, x1, y1, x2, y2, x3, y3, u1, v1, u2, v2, u3, v3)
		...
		Vector3(x3, 0, y3),
		0,
		Color(255, from_multiplier * 255, to_multiplier * 255, 255),
		material,
		Vector2(u1, v1),
		Vector2(u2, v2),
		Vector2(u3, v3))
	...
	local function draw_edge(start_node_key, end_node_key)
		...
		draw_quad(
		"default_deus_map_fog_mask_edge",
		start_alpha,
		end_alpha,
		end_a_x, end_a_y,
		start_a_x, start_a_y,
	...
	local function draw_node(node_key)
		...
		draw_quad(
		"default_deus_map_fog_mask_node",
		alpha,
		alpha,
		end_a_x, end_a_y,
		start_a_x, start_a_y,
	...
	local ab2_x = 0 local ab2_y = 1
	local ab1_x = 0 local ab1_y = 1 - BORDER_INSET_V
	local ab4_x = start_layout_x local ab4_y = 1
	local ab3_x = start_layout_x local ab3_y = 1 - BORDER_INSET_V

	local aa2_x = 0 local aa2_y = BORDER_INSET_V
	local aa1_x = 0 local aa1_y = 0
	local aa4_x = start_layout_x local aa4_y = BORDER_INSET_V
	local aa3_x = start_layout_x local aa3_y = 0

	local ba2_x = 1 - BORDER_INSET_U local ba2_y = BORDER_INSET_V
	local ba1_x = 1 - BORDER_INSET_U local ba1_y = 0
	local ba4_x = 1 local ba4_y = BORDER_INSET_V
	local ba3_x = 1 local ba3_y = 0

	local bb2_x = 1 - BORDER_INSET_U local bb2_y = 1
	local bb1_x = 1 - BORDER_INSET_U local bb1_y = 1 - BORDER_INSET_V
	local bb4_x = 1 local bb4_y = 1
	local bb3_x = 1 local bb3_y = 1 - BORDER_INSET_V



	draw_quad(
	"default_deus_map_fog_mask_border",
	1, 0,
	ab1_x * w, ab1_y * h,
	aa2_x * w, aa2_y * h,
	aa4_x * w, aa4_y * h,
...
local function get_interpolated_camera_pose(bottom_left_pose, top_right_pose, x, y)
	...
	local new_translation = Vector3(
	math.lerp(translation_bottom_left [1], translation_top_right [1], x),
	math.lerp(translation_bottom_left [2], translation_top_right [2], y),
	math.lerp(translation_bottom_left [3], translation_top_right [3], lerp_z))
...
local STATES = { paused = "paused", active = "active", initialized = "initialized" }





function DeusMapScene:init()
	self._state = STATES.initialized
end

function DeusMapScene:on_enter(graph_data, input_service, node_pressed_cb, node_hovered_cb, node_unhovered_cb)
...
function DeusMapScene:on_finish()
...
function DeusMapScene:update(dt, t, gamepad_active)
	...
	should_regenerate_fog and self._last_visibility_data then
	...
	if self._state == STATES.active and
	self._cursor_update_enabled then
...
function DeusMapScene:post_update(dt, t)
	if self._state ~= STATES.initialized then
		self:_update_camera(t)
	end
end

function DeusMapScene:destroy()
	self:_clear()
...
function DeusMapScene:_update_camera(t)
	...
	camera,
	self._fov,
	self._camera_source_pose:unbox(),
	self._camera_target_pose:unbox(),
	self._camera_animation_start_time,
	self._camera_animation_end_time,
...
function DeusMapScene:_update_cursor(gamepad_active)
	...
	local raycast, _, _, _, actor = raycast_screen(
	self._camera,
	self._physics_world,
	cursor_position,
	"closest",
	3,
	"filter_deus_map_node_click",

	self._debug_drawer_stay)
	...
	if node_key_under_cursor then
		if self._selectables and (self._input_service:get("confirm_press") or self._input_service:get("left_press")) and
		table.contains(self._selectables, node_key_under_cursor) then
...
function DeusMapScene:setup_fog(visibility_data)
...
function DeusMapScene:_on_game_options_changed()
	self._game_options_changed = true
end

function DeusMapScene:animate_camera_to(x, y, duration)
...
function DeusMapScene:zoom_camera_to(x, y, duration)
	self._started_once = true
	self._camera_animation_duration = duration
	self._camera_animation_start_time = nil
	self._camera_animation_end_time = nil
...
function DeusMapScene:set_zoomed_camera_to(x, y)
	local bottom_left_pose = self._level_ref_values.camera_zoom_bottom_left_pose:unbox()
	local top_right_pose = self._level_ref_values.camera_zoom_top_right_pose:unbox()
...
function DeusMapScene:place_token(profile_index, slot, node_key)
	self:_place_token(profile_index, slot, node_key)
end

function DeusMapScene:hide_token(profile_index)
	self:_hide_token(profile_index)
end

function DeusMapScene:set_own_hero_name(hero_name)
...
function DeusMapScene:undiscover_node(node_key)
...
function DeusMapScene:discover_node(node_key)
	local unit = self._nodes_to_units [node_key]
...
function DeusMapScene:selectable_node(node_key)
	local unit = self._nodes_to_units [node_key]
...
function DeusMapScene:unselectable_node(node_key)
	local unit = self._nodes_to_units [node_key]
...
function DeusMapScene:untraversed_node(node_key)
	local unit = self._nodes_to_units [node_key]
...
function DeusMapScene:traversed_node(node_key)
	local unit = self._nodes_to_units [node_key]
...
function DeusMapScene:unreachable_node(node_key)
	local unit = self._nodes_to_units [node_key]
...
function DeusMapScene:select_node(node_key, sound_event)
	local unit = self._nodes_to_units [node_key]
	...
	if
	sound_event and Managers.state.network:game() then
...
function DeusMapScene:unselect_node(node_key)
	local unit = self._nodes_to_units [node_key]
...
function DeusMapScene:set_final_node(node_key)
	local unit = self._nodes_to_units [node_key]
...
function DeusMapScene:highlight_edge(from, to)
...
function DeusMapScene:unhighlight_edge(from, to)
	local unit = self._edges_to_units [from] [to]
...
function DeusMapScene:hover_node(node)
...
function DeusMapScene:unhover_node(node)
	local unit = self._nodes_to_units [node]
...
function DeusMapScene:get_screen_pos_of_node(node_key)
	local unit = self._nodes_to_units [node_key]

	return Camera.world_to_screen(self._camera, Unit.local_position(unit, 0))
end

function DeusMapScene:animate_arena_belakor_node(node_key)
	local unit = self._nodes_to_units [node_key]

	Unit.flow_event(unit, "first_time_seeing_arena_belakor_node")
end

function DeusMapScene:_place_token(profile_index, slot, node_key)
...
function DeusMapScene:_hide_token(profile_index)
	local token = self._profile_index_to_token [profile_index]
