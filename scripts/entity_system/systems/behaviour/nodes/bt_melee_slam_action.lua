...
function BTMeleeSlamAction:init(...)
...
function BTMeleeSlamAction:enter(unit, blackboard, t)
...
function BTMeleeSlamAction:init_attack(unit, blackboard, action, t)
...
function BTMeleeSlamAction:leave(unit, blackboard, t, reason, destroy)
...
function BTMeleeSlamAction:_calculate_collision(action, self_pos, forward_direction)
...
function BTMeleeSlamAction:_calculate_cylinder_collision(action, bot_threat, self_pos, self_rot)
...
function BTMeleeSlamAction:_create_bot_aoe_threat(unit, attack_rotation, action, bot_threat)
...
function BTMeleeSlamAction:anim_cb_damage(unit, blackboard)
	...
	local hit_actors, num_actors = PhysicsWorld.immediate_overlap(
	physics_world,
	"shape", shape,
	"position", pos,
	"rotation", rotation,
	"size", size,
	"types", "both",
	"collision_filter", "filter_rat_ogre_melee_slam")
	...
	for i = 1, num_actors do
		...
		if hit_unit ~= unit and not hit_units [hit_unit] then
			...
			if target_status_extension then
				...
				if target_status_extension.is_dodging and
				action.dodge_mitigation_radius_squared < Vector3.length_squared(to_target) then
					dodge = true
				end
...
function BTMeleeSlamAction:run(unit, blackboard, t, dt)
