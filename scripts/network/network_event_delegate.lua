...
local function empty_function() return end
local function empty_deny_function() return false end
...
function NetworkEventDelegate:register(object, ...)
	for i = 1, select("#", ...) do
...
function NetworkEventDelegate:register_with_return(object, callback_name)
...
function NetworkEventDelegate:unregister(object)
...
function NetworkEventDelegate:unregister_callback(callback_name)
	self._registered_objects [callback_name] = nil
end

function NetworkEventDelegate:_cleanup()
	for callback_name, registered_objects in pairs(self._registered_objects) do
		local num_registered_objects = #registered_objects
