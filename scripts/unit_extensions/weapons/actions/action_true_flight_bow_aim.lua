...
function ActionTrueFlightBowAim:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionTrueFlightBowAim:client_owner_start_action(new_action, t, chain_action_data)
...
function ActionTrueFlightBowAim:_start_charge_sound()
...
function ActionTrueFlightBowAim:_stop_charge_sound()
	local current_action = self.current_action
	local owner_unit = self.owner_unit
	local owner_player = self.owner_player
	local is_bot = owner_player and owner_player.bot_player
...
local EMPTY_TABLE = { }
function ActionTrueFlightBowAim:client_owner_post_update(dt, t, world, can_damage)
	local current_action = self.current_action
	local owner_unit = self.owner_unit
	...
	if required_aim_time <= self.aim_timer and (not current_target or aim_sticky_time <= self.aim_sticky_timer) then
		...
		if current_action.aim_obstructed_by_walls then
			results, num_results = PhysicsWorld.immediate_raycast_actors(physics_world,
			player_position,
			direction,
			"dynamic_collision_filter", "filter_ray_true_flight_ai_only",
			"dynamic_collision_filter", "filter_ray_true_flight_hitbox_only",
				...
				if hit_actor then
					...
					if HEALTH_ALIVE [unit] then
						...
						if not hit_unit_side or side_manager:is_enemy_by_side(side, hit_unit_side) then
							...
							if hit_zone and hit_zone.name ~= "afro" and
							not breed.no_autoaim and (not ignore_bosses or not breed.boss) then
								local priority = prio_breeds [breed.name] or -1
								if priority > 0 and higest_priority < priority then
									hit_unit = unit
									higest_priority = priority else
									hit_unit =
									hit_unit or unit
								end
							end
...
function ActionTrueFlightBowAim:_get_visible_targets(aimed_target, num_targets, is_bot)
	local first_person_extension = self.first_person_extension
	local range = 50
...
function ActionTrueFlightBowAim:finish(reason, data)
	local current_action = self.current_action
	local owner_unit = self.owner_unit
...
function ActionTrueFlightBowAim:_mark_target(unit)
	if self.is_bot then
		return
	end
