...
__INTERCEPT_CALLS__ = setmetatable(__INTERCEPT_CALLS__ or { }, {
	...
	__call = function (self, obj, method, enabled)
		...
		local log = { hits = 0, buffer = 50,


			enabled = enabled == nil or not not enabled }

		local function decorator(...)
			if not log.enabled then return func(...) end
			log.hits = log.hits + 1
			...
			log [#log + 1] = entry
			while log.buffer < #log do table.remove(log, 1) end
...
function ImguiCallInterceptor:init()
	self._is_persistent = false
	self._obj_name = ""
	self._method_name = ""
end

function ImguiCallInterceptor:update()
	return end
...
function ImguiCallInterceptor:draw()
...
function ImguiCallInterceptor:is_persistent()
