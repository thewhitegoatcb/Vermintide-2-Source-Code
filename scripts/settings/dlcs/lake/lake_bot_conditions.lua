...
table.merge_recursive(BTConditions.ability_check_categories, {
	activate_ability = { es_questingknight = true } })
...
function BTConditions.can_activate.es_questingknight(blackboard)
	local target = blackboard.target_unit
	if not ALIVE [target] then return false
	end
	local target_blackboard = BLACKBOARDS [target]
	if not target_blackboard then return false
	end

	if target == blackboard.priority_target_enemy and blackboard.priority_target_distance <= QK_MAX_DISTANCE or
	target == blackboard.urgent_target_enemy and blackboard.urgent_target_distance <= QK_MAX_DISTANCE or
	target == blackboard.opportunity_target_enemy and blackboard.opportunity_target_distance <= QK_MAX_DISTANCE then
		return true
	end
