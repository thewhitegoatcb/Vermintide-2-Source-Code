...
function BTLootRatFleeAction:init(...)
...
function BTLootRatFleeAction:enter(unit, blackboard, t)
	...
	if not blackboard.flee_node_data then
		...
		blackboard.flee_node_data = { direction = "fwd",
			nodes = {
	...
	if not blackboard.flee_astar_data then
		...
		blackboard.flee_astar_data = { doing_astar = false,
			astar = astar,
...
function BTLootRatFleeAction:run(unit, blackboard, t, dt)
...
function BTLootRatFleeAction:leave(unit, blackboard, t, reason, destroy)
...
function BTLootRatFleeAction:enter_state_moving_to_level_end(unit, blackboard)
...
function BTLootRatFleeAction:update_state_moving_to_level_end(unit, blackboard, t)
	local astar_data = blackboard.flee_astar_data
...
function BTLootRatFleeAction:move_to_main_path_node(blackboard, node_index)
	local node_data = blackboard.flee_node_data
	local nodes = node_data.nodes [node_data.direction]
...
function BTLootRatFleeAction:do_astar_to_between_main_path_nodes(blackboard, from_node_index)
	local node_data = blackboard.flee_node_data
	local nodes = node_data.nodes [node_data.direction]
	...
	GwNavAStar.start_with_propagation_box(astar_data.astar, blackboard.nav_world,
	from_position, to_position, BREAK_NODE_ASTAR_BOX_EXTENTS, astar_data.traverse_logic)
end

function BTLootRatFleeAction:has_escaped_players(unit, blackboard)
...
function BTLootRatFleeAction:despawn(unit, blackboard, reason)
	local conflict = Managers.state.conflict
	conflict:destroy_unit(unit, blackboard, reason)
end

function BTLootRatFleeAction:set_state(blackboard, new_state)
	blackboard.flee_state = new_state
end

function BTLootRatFleeAction:toggle_start_move_animation_lock(unit, should_lock_ani, blackboard)
...
function BTLootRatFleeAction:debug_draw_path_nodes(nav_world, nodes, break_nodes, target_node_index, t)
	for i = 1, #nodes do
		local node = nodes [i]
