...
function QuestManager:init(statistics_db)
...
function QuestManager:event_stat_incremented(stats_id, ...)
...
function QuestManager:on_achievement_event(event_name, event_data)
	local quests = self._backend_interface_quests:get_quests()
	local event_quests = quests.event
...
function QuestManager:_increment_quest_stats(quests, stats_id, ...)
	local templates = quest_templates.quests
	local statistics_db = self._statistics_db
	local arg_n = select("#", ...)
...
function QuestManager:update(dt, t)
...
function QuestManager:get_quest_outline()
	local quests = self._backend_interface_quests:get_quests()
	...
	for quest_list_name, quest_list in pairs(quests) do
		...
		for backend_quest_key, quest_data in pairs(quest_list) do
			...
			if
			not quest_exists and not table.contains(quest_exist_warning, quest_key) then
			...
			if quest_exists then
				if category_name then
					...
					if not quest_type_category then
						quest_type_category = { type = "quest",
							entries = { },
							name = category_name }
...
function QuestManager:get_data_by_id(quest_id)
	...
	if
	backend_quest_data and backend_quest_data.reward then
		reward = backend_quest_data.reward
	end


	local evaluated_quest = { claimed = false,
		id = quest_id,
		name = name,
		desc = desc,
		icon = icon,
...
function QuestManager:has_any_unclaimed_quests()
...
function QuestManager:can_refresh_daily_quest()
	local backend_interface_quests = self._backend_interface_quests
...
function QuestManager:refresh_daily_quest(quest_id)
	local backend_interface_quests = self._backend_interface_quests
	local quest_key = backend_interface_quests:get_quest_key(quest_id)
...
function QuestManager:polling_quest_refresh()
	return self._refresh_poll_id and true or false
end

function QuestManager:claim_reward(quest_id)
	if self._reward_poll_id or self._refresh_poll_id then
		return nil, "Polling in progress."
	end
...
function QuestManager:claim_multiple_quest_rewards(quest_ids)
	if self._reward_poll_id or self._refresh_poll_id then
		return nil, "Polling in progress."
	end
...
function QuestManager:polling_quest_reward()
	return self._reward_poll_id and true or false
end

function QuestManager:can_claim_quest_rewards(quest_id)
	local backend_interface_quests = self._backend_interface_quests
	local quest_key = backend_interface_quests:get_quest_key(quest_id)
...
function QuestManager:can_claim_multiple_quest_rewards(quest_ids)
	local backend_interface_quests = self._backend_interface_quests
	local claimable_quest_keys = { }
	for i = 1, #quest_ids do
		local quest_id = quest_ids [i]
...
function QuestManager:time_until_new_daily_quest()
	local backend_interface_quests = self._backend_interface_quests
...
function QuestManager:time_until_new_weekly_quest()
	local backend_interface_quests = self._backend_interface_quests
...
function QuestManager:time_left_on_event_quest()
	local backend_interface_quests = self._backend_interface_quests
...
function QuestManager:update_quests()
	local backend_interface_quests = self._backend_interface_quests
...
local EMPTY_TABLE = { }
function QuestManager:on_quests_updated()
