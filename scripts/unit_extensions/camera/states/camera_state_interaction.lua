...
function CameraStateInteraction:init(camera_state_init_context)
	CameraState.init(self, camera_state_init_context, "camera_state_interaction")
end

function CameraStateInteraction:on_enter(unit, input, dt, context, t, previous_state, params)
	...
	if world_manager:has_world("level_world") then
		...
		if level then
			...
			for i, level_unit in ipairs(units) do
				if Unit.has_data(level_unit, "camera_interaction_name") and
				Unit.get_data(level_unit, "camera_interaction_name") == camera_interaction_name then
...
function CameraStateInteraction:on_exit(unit, input, dt, context, t, next_state)
	self.camera_target_unit = nil
end

function CameraStateInteraction:update(unit, input, dt, context, t)
	local csm = self.csm
