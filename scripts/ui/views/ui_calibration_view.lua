...
local scenegraph_definition = {
	...
	top_left_reticule = { vertical_alignment = "top", parent = "root", horizontal_alignment = "left",

		position = { -RETICULE_SIZE / 2, RETICULE_SIZE / 2, 1 },
		size = { RETICULE_SIZE, RETICULE_SIZE } },



	bottom_right_reticule = { vertical_alignment = "bottom", parent = "root", horizontal_alignment = "right",
	...
	reset_button = { vertical_alignment = "center", parent = "root", horizontal_alignment = "center",
...
local widget_definitions = {
	top_left_reticule = { scenegraph_id = "top_left_reticule",
		element = {
			passes = { { pass_type = "hotspot" },

				{ pass_type = "rect", style_id = "horizontal" },


				{ pass_type = "rect", style_id = "vertical" } } },
		...
		style = {
			horizontal = {
				color = { 255, 0, 255, 0 },
				size = { RETICULE_SIZE, RETICULE_THICKNESS },
				offset = { 0, RETICULE_SIZE / 2 - RETICULE_THICKNESS / 2 } },

			vertical = {
				color = { 255, 0, 255, 0 },
				size = { RETICULE_THICKNESS, RETICULE_SIZE },
				offset = { RETICULE_SIZE / 2 - RETICULE_THICKNESS / 2, 0 } } } },




	bottom_right_reticule = { scenegraph_id = "bottom_right_reticule",
		element = {
			passes = { { pass_type = "hotspot" },
...
local button_navigation = { "reset" }



local button_definitions = { { scenegraph_id = "reset_button",

		element = UIElements.Button3States,
		content = { texture_click_id = "small_button_selected", texture_id = "small_button_normal", texture_hover_id = "small_button_hover",
...
function UICalibrationView:init()
...
function UICalibrationView:destroy()

	return end

function UICalibrationView:update(ui_renderer, input_service, dt)
	...
	if top_left_reticule_content.on_pressed then
		...
		self.cursor_start_pos = { cursor.x, cursor.y }
	...
	for i, button in ipairs(self.buttons) do
		if button.content.button_hotspot.on_release and
		button_navigation [i] == "reset" then
...
function UICalibrationView:reset_root_scale()
	UISettings.root_scale [1] = 1
	UISettings.root_scale [2] = 1
	self:save_new_root_scale(UISettings.root_scale)
end

function UICalibrationView:evaluate_new_root_scale(root_scale)
...
function UICalibrationView:save_new_root_scale(root_scale)
