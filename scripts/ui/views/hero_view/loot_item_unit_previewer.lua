...
function LootItemUnitPreviewer:init(item, spawn_position, background_world, background_viewport, unique_id, invert_start_rotation, display_unit_key, use_highest_mip_levels, delayed_spawn, career_name_override)
...
function LootItemUnitPreviewer:activate(activate, background_world, background_viewport, force_present)
	if not self._delayed_spawn then
		return
	end
...
function LootItemUnitPreviewer:activate_auto_spin()
	self._auto_spin_random_seed = math.random(5, 30000)
end

function LootItemUnitPreviewer:register_spawn_callback(callback)
	self._spawn_callback = callback
end

function LootItemUnitPreviewer:destroy()
	self:_destroy_units()
...
function LootItemUnitPreviewer:_destroy_units()
...
function LootItemUnitPreviewer:update(dt, t, input_service)
...
function LootItemUnitPreviewer:set_zoom_fraction(fraction)
	self._zoom_fraction = math.clamp(fraction, 0, 1)
	self._zoom_dirty = true
end

function LootItemUnitPreviewer:zoom_fraction()
	return self._zoom_fraction or 0
end

function LootItemUnitPreviewer:_auto_spin_values(dt, t)
...
function LootItemUnitPreviewer:_handle_mouse_input(input_service, dt)
...
function LootItemUnitPreviewer:_handle_controller_input(input_service, dt)
...
function LootItemUnitPreviewer:post_update(dt, t)
	if not self._activated then
		return
	end
...
function LootItemUnitPreviewer:_load_item_units(item)
	if not item then
		return
	end
	...
	elseif item_type == "weapon_skin" then
		...
		item_skin =
		item_skin or item_key
	end


	item_template =
	item_template or ItemHelper.get_template_by_item_name(item_key)
...
function LootItemUnitPreviewer:_trigger_unit_flow_event(unit, event_name)
	if unit and Unit.alive(unit) then
		Unit.flow_event(unit, event_name)
	end
end

function LootItemUnitPreviewer:_get_world()
	return self._background_world, self._background_viewport
end

function LootItemUnitPreviewer:_get_camera_position()
...
function LootItemUnitPreviewer:_get_camera_rotation()
	local background_viewport = self._background_viewport
	local camera = ScriptViewport.camera(background_viewport)
	return ScriptCamera.rotation(camera)
end

function LootItemUnitPreviewer:_packages_loaded()
...
function LootItemUnitPreviewer:load_package(package_name)
...
function LootItemUnitPreviewer:_on_load_complete(package_name)
	self._loaded_packages [package_name] = true
	self._packages_to_load [package_name] = false
end

function LootItemUnitPreviewer:_unload_packages()
	local reference_name = "LootItemUnitPreviewer"
	if self._unique_id then
		reference_name = reference_name .. tostring(self._unique_id)
	end
...
function LootItemUnitPreviewer:_spawn_link_unit(item)
	local item_data = item.data
	...
	if item_type ~= "rune" and item_type ~= "material" and item_type ~= "ring" and item_type == "necklace" then --[[ Nothing --]]
	end
...
function LootItemUnitPreviewer:_spawn_items()
...
function LootItemUnitPreviewer:spawn_units(spawn_data)
	local units = { }
...
function LootItemUnitPreviewer:present_item(item_key, ignore_spin)
	if self._use_highest_mip_levels and not self:_update_manual_mip_streaming() then
		self._request_show_settings = { item_key = item_key, ignore_spin = ignore_spin }
	else
		self:_enable_item_units_visibility(item_key, ignore_spin, true)
	end
end

function LootItemUnitPreviewer:_enable_item_units_visibility(item_key, ignore_spin, visible)
	local spawned_units = self._spawned_units
	if spawned_units then
		local link_unit = self._link_unit
		for _, unit in ipairs(spawned_units) do
...
function LootItemUnitPreviewer:_request_all_mips_for_unit(unit)
...
function LootItemUnitPreviewer:_update_manual_mip_streaming()
