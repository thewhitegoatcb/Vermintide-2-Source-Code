...
function CareerAbilityDRIronbreaker:init(extension_init_context, unit, extension_init_data)
...
function CareerAbilityDRIronbreaker:extensions_ready(world, unit)
...
function CareerAbilityDRIronbreaker:destroy()
	return end

function CareerAbilityDRIronbreaker:update(unit, input, dt, context, t)
...
function CareerAbilityDRIronbreaker:stop(reason)
	if
...
function CareerAbilityDRIronbreaker:_ability_available()
	local career_extension = self._career_extension
	local status_extension = self._status_extension
	return career_extension:can_use_activated_ability() and not status_extension:is_disabled()
end

function CareerAbilityDRIronbreaker:_start_priming()
...
function CareerAbilityDRIronbreaker:_update_priming(dt)
...
function CareerAbilityDRIronbreaker:_stop_priming()
	local effect_id = self._priming_fx_id
	if effect_id then
		local world = self._world
...
function CareerAbilityDRIronbreaker:_run_ability()
	self:_stop_priming()
	...
	local buffs = { "bardin_ironbreaker_activated_ability", "bardin_ironbreaker_activated_ability_block_cost", "bardin_ironbreaker_activated_ability_attack_intensity_decay_increase" }
	...
	if talent_extension:has_talent("bardin_ironbreaker_activated_ability_taunt_range_and_duration") then
		table.clear(buffs)
		buffs = { "bardin_ironbreaker_activated_ability_taunt_range_and_duration", "bardin_ironbreaker_activated_ability_taunt_range_and_duration_block_cost", "bardin_ironbreaker_activated_ability_taunt_range_and_duration_attack_intensity_decay_increase" }
	end
	...
	for i = 1, num_targets do
		...
		for j, buff_name in ipairs(buffs) do
			...
			if is_server then
				target_buff_extension:add_buff(buff_name, { attacker_unit = owner_unit })
				network_transmit:send_rpc_clients("rpc_add_buff", target_unit_object_id, buff_template_name_id, owner_unit_id, 0, false)
			else
				network_transmit:send_rpc_server("rpc_add_buff", target_unit_object_id, buff_template_name_id, owner_unit_id, 0, true)
			end
...
function CareerAbilityDRIronbreaker:_play_vo()
	local owner_unit = self._owner_unit
...
function CareerAbilityDRIronbreaker:_play_vfx()
	local owner_unit = self._owner_unit
