...
local amulet_slot_layout = { { amount_per_layer = 6, name = "talent", degrees = 180, max_amount = 6, radius = 178 },
	...
	{ amount_per_layer = 3, name = "trait", degrees = 90, max_amount = 3, radius = 370 },
	...
	{ amount_per_layer = 10, name = "property", degrees = 180, max_amount = 30, radius = 100, inherent_parent = "trait", start_angle = 2.827433385,
		...
		layer_settings = { { radius = 100, degrees = 324, amount_per_layer = 10 },




			{ radius = 100, degrees = 324, amount_per_layer = 10 },
...
local weapon_slot_layout = { { amount_per_layer = 1, name = "trait", degrees = 90, max_amount = 1, radius = 370 },
	...
	{ amount_per_layer = 10, name = "property", degrees = 324, max_amount = 10, radius = 100, inherent_parent = "trait", start_angle = 2.827433385 } }
...
function HeroWindowWeaveProperties:on_enter(params, offset)
	...
	if self.upgrage_tutorial then
		...
		self._ui_animations.upgrade_button_pulse = UIAnimation.init(UIAnimation.pulse_animation,
		upgrade_button.style.texture_highlight.color, 1, 100, 255, 2)
	end
end

function HeroWindowWeaveProperties:_start_transition_animation(animation_name)
	local params = {
		parent = self._parent,
...
function HeroWindowWeaveProperties:_selected_item()
	local params = self._params
	return params.selected_item
end

function HeroWindowWeaveProperties:_selected_unit_name()
	local params = self._params
	return params.selected_unit_name
end

function HeroWindowWeaveProperties:_setup_definitions()
...
function HeroWindowWeaveProperties:create_ui_elements(params, offset)
...
function HeroWindowWeaveProperties:_setup_menu_options(career_name, slots_progression)
	local menu_options = { }
	local masked = true
	...
	if progression_traits then
		...
		for _, slot_unlock in ipairs(progression_traits) do
			...
			menu_option [category] = { root_scenegraph_id = "option_trait", scrolled_length = 0,
				...
				scroll_content = { scroll_progress = 0 } }
		end
	end
	...
	if progression_talents then
		...
		for _, slot_unlock in ipairs(progression_talents) do
			...
			menu_option [category] = { root_scenegraph_id = "option_talent", scrolled_length = 0,
				draw_count = draw_count,
				spacing = spacing,
				total_length = total_length,
				scroll_length = scroll_length,
	...
	if progression_properties then
		...
		for _, slot_unlock in ipairs(progression_properties) do
			...
			menu_option [category] = { root_scenegraph_id = "option_propery", scrolled_length = 0,
				draw_count = draw_count,
				spacing = spacing,
				total_length = total_length,
				scroll_length = scroll_length,
...
function HeroWindowWeaveProperties:_populate_menu_widgets()
...
function HeroWindowWeaveProperties:_populate_menu_option_widget(entry_data, menu_option)
...
local mastery_price_colors = {
	affordable = { 255, 121, 193, 229 },
	unaffordable = { 255, 200, 0, 0 } }


function HeroWindowWeaveProperties:_align_menu_option_price_text(style, price_icon, price_text, value_text, can_afford, locked)
...
function HeroWindowWeaveProperties:_create_viewport_definition()
	local shading_environment = "environment/ui_weave_forge_preview"
	return { scenegraph_id = "viewport",
		element = UIElements.Viewport,
		style = {
			viewport = { layer = 840, world_name = "item_preview", viewport_type = "default_forward", viewport_name = "item_preview_viewport", enable_sub_gui = false, fov = 20,
...
function HeroWindowWeaveProperties:_create_cluster_background(index, position_x, position_y)
	local widgets_by_name = self._widgets_by_name
	...
	local widget_definition_default = UIWidgets.create_simple_texture(texture_background, scenegraph_id, nil, nil, { 150, 255, 255, 255 })
	local widget_definition_effect = UIWidgets.create_simple_texture(texture_background_effect, scenegraph_id, nil, nil, { 255, 138, 0, 147 }, 1)
...
function HeroWindowWeaveProperties:_create_slot_grid(slot_layout, slots_progression)
	local function add_entry(self, slots_progression, slot_table, layout, layer_settings, parent_x, parent_y, layer_index)
		...
		if name == "trait" then
			widget_definition = create_trait_slot_definition("slot_root", { 80, 80 })
			progression_table = slots_progression.traits
		elseif name == "talent" then
			widget_definition = create_talent_slot_definition("slot_root", { 100, 100 })
			progression_table = slots_progression.talents
		elseif name == "property" then
			widget_definition = create_property_slot_definition("slot_root", { 50, 50 })
		...
		for i = 1, layers do
			...
			if settings then amount_per_layer = settings.amount_per_layer or amount_per_layer end
			if settings then degrees = settings.degrees or degrees end
			if settings then radius = settings.radius or radius
			end
...
function HeroWindowWeaveProperties:on_exit(params)
...
function HeroWindowWeaveProperties:update(dt, t)
...
function HeroWindowWeaveProperties:post_update(dt, t)
...
function HeroWindowWeaveProperties:_update_background_animations(dt)
	local widgets_by_name = self._widgets_by_name
...
function HeroWindowWeaveProperties:_set_background_bloom_intensity(fraction)
	local min = 1.39
...
function HeroWindowWeaveProperties:_update_animations(dt)
	local params = self._params
...
function HeroWindowWeaveProperties:_is_button_pressed(widget)
	local content = widget.content
...
function HeroWindowWeaveProperties:_is_button_hover(widget)
	local content = widget.content
	local hotspot = content.button_hotspot or content.hotspot
	return hotspot.is_hover
end

function HeroWindowWeaveProperties:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_enter and not hotspot.is_selected
end

function HeroWindowWeaveProperties:_is_button_hover_exit(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_exit and not hotspot.is_selected
end

function HeroWindowWeaveProperties:_is_button_selected(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.is_selected
end

function HeroWindowWeaveProperties:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.button_hotspot or content.hotspot
	return hotspot.is_hover or false
end

function HeroWindowWeaveProperties:_handle_input(dt, t)
	local widgets_by_name = self._widgets_by_name
	local gamepad_active = Managers.input:is_device_active("gamepad")
...
function HeroWindowWeaveProperties:_upgrade_magic_level()
...
function HeroWindowWeaveProperties:_upgrade_magic_level_cb(success)
	print("_upgrade_magic_level_cb")
	self._upgrade_magic_level_response = success
end

function HeroWindowWeaveProperties:_upgrade_magic_level_done(success)
...
function HeroWindowWeaveProperties:_magic_level()
	return self._current_magic_level or 0
end

function HeroWindowWeaveProperties:_sync_backend_loadout(upgraded)
	local career_name = self._career_name
	local item = self:_selected_item()
	local item_backend_id = item and item.backend_id
		...
		for slot_index, slot in ipairs(type_slots) do
			...
			if property_key then
				if not slot.presentation_data then
					slot.presentation_data = { animation_duration = 0.4 }
				end
...
function HeroWindowWeaveProperties:_setup_upgrade_tooltip(magic_level, max_magic_level, item)
	local widgets_by_name = self._widgets_by_name
	local upgrade_button = widgets_by_name.upgrade_button
...
function HeroWindowWeaveProperties:_set_essence_upgrade_cost(essence_amount, can_afford, magic_cap_reached)
	local widgets_by_name = self._widgets_by_name
...
function HeroWindowWeaveProperties:_draw_slots(ui_renderer, dt)
	local slots = self._slots
...
function HeroWindowWeaveProperties:_set_grid_slot_selected(selected_slot)
	local slots = self._slots
	for type_index, type_slots in pairs(slots) do
		local num_slots = #type_slots
...
function HeroWindowWeaveProperties:_set_locked_slot_description(widget_type, category, required_level)
...
function HeroWindowWeaveProperties:_disable_locked_slot_description()
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.locked_slot_description
	widget.content.visible = false
end

function HeroWindowWeaveProperties:_animate_menu_widgets(dt)
	local widgets_by_name = self._widgets_by_name
...
function HeroWindowWeaveProperties:_animate_menu_option_entry(entry, menu_option, hover_approved, dt)
...
function HeroWindowWeaveProperties:_animate_slot(slot, dt)
	local active_menu_option = self._active_menu_option
	local active_menu_category = self._active_menu_category
...
function HeroWindowWeaveProperties:_animate_slot_fill(slot, progress)
	local presentation_data = slot.presentation_data
	local widget = slot.widget
	local widget_type = slot.widget_type
...
function HeroWindowWeaveProperties:_enable_menu_options(enabled, widget_type, category)
	local menu_options = self._menu_options
	local options = menu_options [widget_type]
...
function HeroWindowWeaveProperties:_set_overlay_alpha(alpha)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.window_overlay
	widget.style.rect.color [1] = alpha
end

function HeroWindowWeaveProperties:_options_mask_height()
	local mask_size = scenegraph_definition.options_scroll_field.size
	return mask_size [2]
end

local draw_offset = { 0, 0, 0 }
function HeroWindowWeaveProperties:_draw_menu_selection(ui_renderer, render_settings, dt)
	local active_menu_option = self._active_menu_option
	local active_menu_category = self._active_menu_category
...
function HeroWindowWeaveProperties:_highlight_slots_by_key(key, menu_option)
	local slots = self._slots
...
function HeroWindowWeaveProperties:_find_slot_by_key(key, menu_option)
	local loadout = self._loadout
...
function HeroWindowWeaveProperties:_find_next_available_slot(menu_option, menu_category, allow_occupied_slots)
...
function HeroWindowWeaveProperties:_set_clear_button_enabled_state(enabled)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.clear_button
	widget.content.button_hotspot.disable_button = not enabled
end

function HeroWindowWeaveProperties:_can_clear_slots(active_menu_option, active_category)
	local loadout = self._loadout
...
function HeroWindowWeaveProperties:_clear_slots(menu_option, category)
...
function HeroWindowWeaveProperties:_clear_slot(slot, key, menu_option)
	local career_name = self._career_name
	local backend_manger = Managers.backend
	local backend_interface_weaves = backend_manger:get_interface("weaves")
...
function HeroWindowWeaveProperties:_add_key_to_slot(slot, key, menu_option)
	local career_name = self._career_name
	local backend_manger = Managers.backend
	local backend_interface_weaves = backend_manger:get_interface("weaves")
...
function HeroWindowWeaveProperties:_calculate_list_index_offset(total_length, draw_length, entry_height, spacing, scrolled_length)
...
function HeroWindowWeaveProperties:_draw_selection_menu_widget(ui_renderer, widget_size, spacing, menu_option, entries, index_offset, draw_count, draw_offset, curve_multiplier, scrolled_length, anim_progress)
	local input_service = self._parent:window_input_service()
...
function HeroWindowWeaveProperties:_draw(dt)
	local parent = self._parent
...
function HeroWindowWeaveProperties:_calculate_background_fraction(size)
...
function HeroWindowWeaveProperties:_play_sound(event)
	self._parent:play_sound(event)
end

function HeroWindowWeaveProperties:_animate_element_by_time(target, target_index, from, to, time)
	local new_animation = UIAnimation.init(UIAnimation.function_by_time, target, target_index,
	from, to, time, math.ease_out_quad)
	return new_animation
end

function HeroWindowWeaveProperties:_animate_element_by_catmullrom(target, target_index, target_value, p0, p1, p2, p3, time)
	local new_animation = UIAnimation.init(UIAnimation.catmullrom, target, target_index, target_value,
	p0, p1, p2, p3, time)
	return new_animation
end

function HeroWindowWeaveProperties:_get_hero_wield_info_by_item(item)
	local item_data = item.data
...
function HeroWindowWeaveProperties:_destroy_previewers()
	local item_previewer = self._item_previewer
	if item_previewer then
...
function HeroWindowWeaveProperties:_set_options_title(title_text, sub_title_text)
	local widgets_by_name = self._widgets_by_name
	widgets_by_name.title_text.content.text = title_text
	widgets_by_name.sub_title_text.content.text = sub_title_text
end

function HeroWindowWeaveProperties:_set_mastery_amount(initial_mastery, current_mastery, play_sound)
...
function HeroWindowWeaveProperties:_set_title_text(title_text, sub_title_text)
	local widgets_by_name = self._widgets_by_name
	widgets_by_name.viewport_title.content.text = title_text
	widgets_by_name.viewport_sub_title.content.text = sub_title_text
end

function HeroWindowWeaveProperties:_animate_mastery_amount(dt)
...
function HeroWindowWeaveProperties:_create_item_previewer(viewport_widget, item)
	local item_data = item.data
	...
	local preview_position = { -0.85, 3, 0 }
...
function HeroWindowWeaveProperties:_create_unit_previewer(viewport_widget, unit_name, package_name)
	local viewport_pass_data = viewport_widget.element.pass_data [1]
	local viewport = viewport_pass_data.viewport
	local world = viewport_pass_data.world

	local preview_position = { -0.8, 2.5, 0.05 }
...
function HeroWindowWeaveProperties:cb_unit_spawned_unit_preview(unit_previewer, unit_name)
	print("cb_unit_spawned_unit_preview", unit_previewer, unit_name)
end

function HeroWindowWeaveProperties:_spawn_hero_skin(world_previewer, hero_name, career_index, optional_skin)
...
function HeroWindowWeaveProperties:_spawn_hero_with_hat(world_previewer, hero_name, career_index, optional_skin, item_name)
	local callback = callback(self, "cb_hero_unit_spawned_hat_preview", world_previewer, hero_name, career_index, item_name)
	world_previewer:request_spawn_hero_unit(hero_name, career_index, false, callback, 1, nil, optional_skin)
end

function HeroWindowWeaveProperties:cb_hero_unit_spawned_skin_preview(world_previewer, hero_name, career_index)
...
function HeroWindowWeaveProperties:cb_hero_unit_spawned_hat_preview(world_previewer, hero_name, career_index, hat_item_name)
	local profile_index = FindProfileIndex(hero_name)
	local profile = SPProfiles [profile_index]
	local careers = profile.careers
	local career_settings = careers [career_index]
...
function HeroWindowWeaveProperties:cb_unit_spawned_item_preview(item_previewer, item_key)
	local ignore_spin = true
	item_previewer:present_item(item_key, ignore_spin)
	self._fadeout_loading_overlay = true
end

function HeroWindowWeaveProperties:_initialize_scrollbar(scrollbar_widget, content_length, list_scenegraph_id, spacing)
...
function HeroWindowWeaveProperties:_update_menu_option_scrollbar_position(menu_option)
	local ui_scenegraph = self._ui_scenegraph
