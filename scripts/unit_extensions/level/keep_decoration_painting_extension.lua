...
function KeepDecorationPaintingExtension:init(extension_init_context, unit, extension_init_data)
...
function KeepDecorationPaintingExtension:interacted_with()

	return end

function KeepDecorationPaintingExtension:destroy()
...
function KeepDecorationPaintingExtension:extensions_ready()
...
function KeepDecorationPaintingExtension:get_settings()
	return self._paintings_lookup
end

function KeepDecorationPaintingExtension:can_interact()
	if not DLCSettings.gecko then
		return false
	end
...
function KeepDecorationPaintingExtension:decoration_selected(current_painting)
	self:_load_painting(current_painting, nil)
end

function KeepDecorationPaintingExtension:reset_selection()
...
function KeepDecorationPaintingExtension:unequip_decoration(new_painting)
	local painting = new_painting or "hor_none"
	self:_load_painting(painting)
	self:sync_decoration()
end

function KeepDecorationPaintingExtension:confirm_selection()
	local current_preview_painting = self._current_preview_painting
...
function KeepDecorationPaintingExtension:sync_decoration()
	local current_preview_painting = self._current_preview_painting
...
function KeepDecorationPaintingExtension:hot_join_sync(sender)
	return end

function KeepDecorationPaintingExtension:distributed_update()
...
function KeepDecorationPaintingExtension:set_client_painting(painting)
	self:_load_painting(painting)
...
function KeepDecorationPaintingExtension:get_selected_decoration()
	if self._is_leader then
		...
		do return selected_painting end
	else
		return self._currently_set_painting
	end
end

function KeepDecorationPaintingExtension:_set_selected_painting(painting)
	local backend_key = self._backend_key
...
function KeepDecorationPaintingExtension:_load_painting(painting, callback)
	painting =
	painting or "hor_none"
...
function KeepDecorationPaintingExtension:_load_painting_frame(data)
...
function KeepDecorationPaintingExtension:_load_painting_material(name, cb_done)
...
function KeepDecorationPaintingExtension:_apply_material_by_sub_path(subpath)
...
function KeepDecorationPaintingExtension:_unload_painting_material(package_name)
	local reference_name = self._decoration_settings_key
...
function KeepDecorationPaintingExtension:_create_game_object(painting)
	local go_data_table = {
...
function KeepDecorationPaintingExtension:cb_game_session_disconnect()
	self._go_id = nil
end


function KeepDecorationPaintingExtension:on_game_object_created(go_id)
	local game = Managers.state.network:game()
	local painting_index = GameSession.game_object_field(game, go_id, "painting_index")
	local painting = self._paintings_lookup [painting_index]
	self:_load_painting(painting, nil)
...
function KeepDecorationPaintingExtension:on_game_object_destroyed()
	self._go_id = nil
end