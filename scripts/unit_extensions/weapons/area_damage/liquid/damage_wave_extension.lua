...
local DEFAULT_STAGGER_REFRESH_TIME = { math.huge, math.huge, math.huge, math.huge, math.huge, math.huge }
local impact_hit_units = { }
function DamageWaveExtension:init(extension_init_context, unit, extension_init_data)
	...
	self._buff_params = { attacker_unit = unit, source_attacker_unit = extension_init_data.source_unit }
...
function DamageWaveExtension:set_update_func(update_func, init_func, t)
...
function DamageWaveExtension:_calculate_oobb_collision(width, range, height, offset_forward, offset_up, self_pos, self_rot)
...
function DamageWaveExtension:launch_wave(target_unit, optional_target_pos, optional_data)
	...
	target_pos =
	target_pos or position_lookup [target_unit]
	...
	if create_bot_aoe_threat then
		...
		local offset_forward = 0 local offset_up = 0
...
function DamageWaveExtension:destroy()
	local unit = self.unit
...
function DamageWaveExtension:abort()
...
function DamageWaveExtension:move_wave(unit, t, dt, total_dist, grow)
...
function DamageWaveExtension.on_hit_by_wave(hit_unit, unit, parent)
...
function DamageWaveExtension:wave_arrived(t, unit)
...
function DamageWaveExtension:wavefront_impact(t, impact_position, radius, ai_push_data, attacker_unit, wave_dir, dt)
	...
	local num_ai_units = AiUtils.broadphase_query(impact_position, radius,
	ai_units,
...
function DamageWaveExtension:check_overlap(unit, target_unit, wave_radius, p1, p2, buff_system, num_blobs)
	local player_units_inside = self.player_units_inside
...
function DamageWaveExtension:update(unit, input, dt, context, t)
...
function DamageWaveExtension:insert_blob(position, radius, rotation, nav_world)
	...
	blobs [blob_index] = { position [1], position [2], position [3], radius, { }, nav_cost_map_volume_id, blob_index }
...
function DamageWaveExtension:insert_fx(position, rotation, fx_idx)
	...
	if fx_idx == 0 or config.spawn_type == "effect" then
		...
		local fx_list = self.fx_list
		fx_list [#fx_list + 1] = { id = unit_or_id, position = Vector3Box(position), rotation = QuaternionBox(rotation), index = fx_idx, name_index = name_idx }
...
function DamageWaveExtension:update_blob_overlaps()
	local blobs = self.blobs
	local num_blobs = #blobs
	if num_blobs < 1 then
		return
	...
	local buff_system = self.buff_system
	local first_blob = blobs [1] local last_blob = blobs [num_blobs]
	...
	while amount > 0 do
		...
		local num_ai_units = AiUtils.broadphase_query(blob_pos, blob_radius,
		ai_units,
		broadphase_categories)
		for i = 1, num_ai_units do
...
function DamageWaveExtension:get_rim_nodes()
	return self.rim_nodes, true
end

function DamageWaveExtension:is_position_inside(position, nav_cost_map_table, is_player)
	local blobs = self.blobs
	local num_blobs = #blobs
	if num_blobs == 0 then
		return false
...
function DamageWaveExtension:is_position_inside_blob(position, blob)
	local blob_pos = Vector3(blob [1], blob [2], blob [3])
...
function DamageWaveExtension:hot_join_sync(peer_id)
...
local wave_length = 1
function DamageWaveExtension:debug_render_wave(t, dt, pos, travel_dir, height)
	local k = 0
...
function DamageWaveExtension:debug_render_blobs()
	local blobs = self.blobs
	for i = 1, #blobs do
		local blob = blobs [i]
		local blob_pos = Vector3(blob [1], blob [2], blob [3])
...
function DamageWaveExtension:_update_running_spawn_datas(t)
