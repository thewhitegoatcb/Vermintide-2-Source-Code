...
function HeroWindowCharacterSummary:on_enter(params, offset)
...
function HeroWindowCharacterSummary:_start_transition_animation(animation_name)
	local params = {
...
function HeroWindowCharacterSummary:_create_ui_elements(params)
...
function HeroWindowCharacterSummary:on_exit(params)
...
function HeroWindowCharacterSummary:_input_service()
...
function HeroWindowCharacterSummary:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function HeroWindowCharacterSummary:post_update(dt, t)
	return end

function HeroWindowCharacterSummary:_handle_input(input_service, dt, t)
...
function HeroWindowCharacterSummary:_handle_gamepad_selection(input_service)
...
function HeroWindowCharacterSummary:_set_career_selection_state(enabled, career_applied)
...
function HeroWindowCharacterSummary:_change_carrer(career_settings)
	local parent = self._parent
...
function HeroWindowCharacterSummary:_set_selected_hero_by_coordinates(row, column)
	local hero_widgets = self._hero_widgets
...
function HeroWindowCharacterSummary:_set_selected_hero_index(index)
	local hero_widgets = self._hero_widgets
	for i, widget in ipairs(hero_widgets) do
		local content = widget.content
...
function HeroWindowCharacterSummary:_selected_hero_career()
...
function HeroWindowCharacterSummary:_on_talent_pressed(row, column)
...
function HeroWindowCharacterSummary:_set_talent_selected(row, column)
	local talent_slot_widgets = self._talent_slot_widgets
	local talent_widgets = self._talent_widgets
...
function HeroWindowCharacterSummary:_on_talent_slot_pressed(index)
...
function HeroWindowCharacterSummary:_enable_talent_row(row_index)
	local talent_widgets = self._talent_widgets
...
function HeroWindowCharacterSummary:_toggle_statistics(enabled)
...
function HeroWindowCharacterSummary:_update_hero_sync()
	local parent = self._parent
...
function HeroWindowCharacterSummary:_commit_talent_changes()
...
function HeroWindowCharacterSummary:_update_animations(dt)
...
function HeroWindowCharacterSummary:_is_button_pressed(widget)
	local content = widget.content
...
function HeroWindowCharacterSummary:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.button_hotspot or content.hotspot
...
function HeroWindowCharacterSummary:_is_button_hover(widget)
	local content = widget.content
	local hotspot = content.button_hotspot or content.hotspot
	return hotspot.is_hover
end

function HeroWindowCharacterSummary:_exit()
	self.exit = true
end

function HeroWindowCharacterSummary:_draw(dt)
...
function HeroWindowCharacterSummary:_play_sound(event)
	self._parent:play_sound(event)
end

function HeroWindowCharacterSummary:_sync_statistics()
...
function HeroWindowCharacterSummary:_populate_talents(hero_name, career_name)
	local ui_renderer = self._ui_renderer
...
function HeroWindowCharacterSummary:_populate_statistics(layout)
	local ui_renderer = self._ui_renderer
	local widgets = { }
...
function HeroWindowCharacterSummary:_align_list_widgets(widgets, spacing)
...
function HeroWindowCharacterSummary:_initialize_scrollbar()
...
function HeroWindowCharacterSummary:_update_scroll_position()
	local scrollbar_logic = self._scrollbar_logic
...
function HeroWindowCharacterSummary:_update_visible_list_entries()
	local scrollbar_logic = self._scrollbar_logic
...
function HeroWindowCharacterSummary:_get_scrollbar_percentage_by_index(index)
	local scrollbar_logic = self._scrollbar_logic
	local enabled = scrollbar_logic:enabled()
...
function HeroWindowCharacterSummary:_setup_title_texts()
	local widgets_by_name = self._widgets_by_name
...
function HeroWindowCharacterSummary:_animate_title_button(dt)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.summary_title
	local content = widget.content
	local style = widget.style
...
function HeroWindowCharacterSummary:_populate_career_info(career_name)
	local ui_renderer = self._ui_renderer
	local ui_scenegraph = self._ui_scenegraph
	...
	local career_color = Colors.color_definitions [career_name] and Colors.get_color_table_with_alpha(career_name, 255) or { 255, 255, 255, 255 }
...
function HeroWindowCharacterSummary:_animate_talent_widget(widget, dt)
	local content = widget.content
	local style = widget.style
	local hotspot = content.button_hotspot or content.hotspot
...
function HeroWindowCharacterSummary:_update_talent_position_animation(dt)
	local selected_talent_index = self._selected_talent_index
...
function HeroWindowCharacterSummary:_get_timer_progress(time, max_time, dt)
...
function HeroWindowCharacterSummary:_set_talent_list_animation_progress(widgets, progress)
...
function HeroWindowCharacterSummary:_setup_hero_selection_widgets()
	local hero_widgets = { }
...
function HeroWindowCharacterSummary:_animate_hero_widget(widget, dt)
	local content = widget.content
	local style = widget.style
	local hotspot = content.button_hotspot or content.hotspot
...
function HeroWindowCharacterSummary:_animate_hero_icon_widget(widget, highlight, dt)
	local content = widget.content
	local style = widget.style
