...
function ActionPushStagger:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionPushStagger:client_owner_start_action(new_action, t, chain_action_data, power_level, action_init_data)
...
local callback_context = { has_gotten_callback = false,
...
function ActionPushStagger:client_owner_post_update(dt, t, world, can_damage)
	...
	elseif self.waiting_for_callback and callback_context.has_gotten_callback then
		...
		for hit_unit, info in pairs(push_units) do repeat
				...
				self.owner.local_player and not self.has_played_rumble_effect then
					Managers.state.controller_features:add_effect("rumble", { rumble_effect = "push_hit" })
				...
				not player_manager.is_server then
		...
		if
		hit_once and not self.bot_player then
...
function ActionPushStagger:finish(reason)
