...
function LadderExtension:init(extension_init_context, unit, extension_init_data)
...
function LadderExtension:ladder_extents()
	return self._bottom_position:unbox(), self._top_position:unbox()
end

LadderExtension.perlin_shake = { persistance = 1, magnitude = 0.1, frequency_multiplier = 4, duration = 1, min_value = 0, octaves = 6 }
...
function LadderExtension:update_enabled(unit, input, dt, context, t)
...
function LadderExtension:is_shaking()
	return self._shaking and true or false
end

function LadderExtension:shake()
...
function LadderExtension:destroy()
	if self._is_server then
