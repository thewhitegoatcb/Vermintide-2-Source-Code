...
function CareerAbilityWEShadeDash:init(extension_init_context, unit, extension_init_data)
	...
	self._lunge_events = {
		...
		start = function (this)
			...
			if local_player or is_server and bot_player then
				...
				local events = { "Play_career_ability_kerillian_shade_enter", "Play_career_ability_kerillian_shade_loop_husk" }
...
function CareerAbilityWEShadeDash:extensions_ready(world, unit)
...
function CareerAbilityWEShadeDash:destroy()
	return end

function CareerAbilityWEShadeDash:update(unit, input, dt, context, t)
...
function CareerAbilityWEShadeDash:stop(reason)
	if
...
function CareerAbilityWEShadeDash:_ability_available()
	local career_extension = self._career_extension
	local status_extension = self._status_extension
	local talent_extension = ScriptUnit.extension(self._owner_unit, "talent_system")
...
function CareerAbilityWEShadeDash:_start_priming()
...
function CareerAbilityWEShadeDash:_update_priming()
...
function CareerAbilityWEShadeDash:_stop_priming()
	if self._decal_unit then
		local unit_spawner = Managers.state.unit_spawner
...
function CareerAbilityWEShadeDash:_run_ability()
	self:_stop_priming()
	...
	status_extension.do_lunge = { animation_end_event = "maiden_guard_active_ability_charge_hit", allow_rotation = false, falloff_to_speed = 5, first_person_animation_end_event = "dodge_bwd", first_person_hit_animation_event = "charge_react", dodge = true, first_person_animation_event = "shade_stealth_ability", first_person_animation_end_event_hit = "dodge_bwd", duration = 0.65, initial_speed = 25, animation_event = "maiden_guard_active_ability_charge_start",
...
function CareerAbilityWEShadeDash:_play_vo()
	local owner_unit = self._owner_unit
