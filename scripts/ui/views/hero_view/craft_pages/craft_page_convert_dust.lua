...
function CraftPageConvertDust:on_enter(params, settings)
...
function CraftPageConvertDust:_has_required_item_amount(backend_id)
...
function CraftPageConvertDust:_get_recipe_by_backend_id(backend_id)
	local item_interface = Managers.backend:get_interface("items")
...
function CraftPageConvertDust:setup_recipe_requirements()
...
function CraftPageConvertDust:reset_requirements(num_required_ingredients)
...
function CraftPageConvertDust:_add_crafting_material_requirement(index, item_key, amount_text, has_required_amount)
...
function CraftPageConvertDust:create_ui_elements(params)
...
function CraftPageConvertDust:on_exit(params)
...
function CraftPageConvertDust:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function CraftPageConvertDust:post_update(dt, t)

	return end

function CraftPageConvertDust:_update_animations(dt)
...
function CraftPageConvertDust:_is_button_pressed(widget)
	local content = widget.content
...
function CraftPageConvertDust:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageConvertDust:_is_button_held(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageConvertDust:_handle_input(dt, t)
...
function CraftPageConvertDust:_handle_craft_input_progress(progress)
...
function CraftPageConvertDust:craft_result(result, error, reset_slots)
	if not error then
		self._craft_result = result
	end
end

function CraftPageConvertDust:reset()
	local item_grid = self._item_grid
	item_grid:clear_locked_items()
	item_grid:update_items_status()
end

function CraftPageConvertDust:on_craft_completed()
...
function CraftPageConvertDust:_update_craft_items()
	local super_parent = self.super_parent
	local item_grid = self._item_grid
...
function CraftPageConvertDust:_remove_craft_item(backend_id, slot_index)
	local craft_items = self._craft_items
...
function CraftPageConvertDust:_add_craft_item(backend_id, slot_index, ignore_sound)
	if self._num_craft_items == 0 then
...
function CraftPageConvertDust:_set_craft_button_disabled(disabled)
	self._widgets_by_name.craft_button.content.button_hotspot.disable_button = disabled
end

function CraftPageConvertDust:_exit(selected_level)
	self.exit = true
	self.exit_level_id = selected_level
end

function CraftPageConvertDust:draw(dt)
...
function CraftPageConvertDust:_play_sound(event)
	self.super_parent:play_sound(event)
end

function CraftPageConvertDust:_set_craft_button_text(text, localize)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.craft_button
