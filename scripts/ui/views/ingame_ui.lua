...
for _, settings in pairs(PopupSettings) do
	if settings.file then require(settings.file)
	end
end
...
function IngameUI:init(ingame_ui_context)
...
function IngameUI:is_local_player_ready_for_game()
...
function IngameUI:can_view_lobby_browser()
...
function IngameUI:is_lorebook_enabled()
...
function IngameUI:register_rpcs(network_event_delegate)
	self.network_event_delegate = network_event_delegate
	network_event_delegate:register(self, unpack(rpcs))
end

function IngameUI:unregister_rpcs()
	self.network_event_delegate:unregister(self)
	self.network_event_delegate = nil
end

function IngameUI:is_in_view_state(state_name)
...
function IngameUI:destroy()
	...
	if not menu_active and current_view then --[[ Nothing --]]
	end
...
function IngameUI:weaves_requirements_fulfilled()
	...
	for _, level_key in pairs(MainGameLevels) do
		...
		if level_settings.game_mode == "adventure" and
		statistics_db:get_persistent_stat(stats_id, "completed_levels", level_key) < 1 then
			return false
		end
	end
...
function IngameUI:not_in_modded()
	return not script_data ["eac-untrusted"]
end

local hotkeys_blocked_during_vote = { "hotkey_map" }



function IngameUI:handle_menu_hotkeys(dt, input_service, hotkeys_enabled, menu_active)
				...
				if can_interact then
					if transition_not_allowed then
						...
						if error_message then
							self:add_local_system_message(error_message) end
						break
					end
					local transition = menu_active and mapping_data.in_transition_menu or mapping_data.in_transition
					local transition_params = { menu_state_name = mapping_data.transition_state, menu_sub_state_name = mapping_data.transition_sub_state }
...
function IngameUI:event_dlc_status_changed()
...
function IngameUI:update_loading_subtitle_gui(loading_subtitle_gui, dt)
	loading_subtitle_gui:update(self.ui_top_renderer, dt)
end

function IngameUI:update(dt, t, disable_ingame_ui, end_of_level_ui)
		...
		if allowed_to_access_menu and not player_list_active and not disable_toggle_menu and not self:pending_transition() and not fade_active and not self:end_screen_active() and not self.menu_active and not self.leave_game and not self.return_to_title_screen and not self:get_active_popup("profile_picker") and input_service:get("toggle_menu", true) then
			...
			if use_gamepad_layout then
				...
				if is_in_inn and gamepad_active then
					...
					local transition_params = { menu_state_name = menu_state_name, menu_sub_state_name = menu_sub_state_name }
					self:transition_with_fade("hero_view_force", transition_params)
				else
					local menu_sub_state_name = "system"
					local transition_params = { menu_state_name = menu_state_name, menu_sub_state_name = menu_sub_state_name, force_ingame_menu = IS_WINDOWS }
...
function IngameUI:disable_ingame_ui()
	return self._disable_ingame_ui
end

function IngameUI:_holly_dlc_intro_closed()
	PlayerData.viewed_dialogues.dlc_holly = true
	Managers.save:auto_save(SaveFileName, SaveData)
end

function IngameUI:post_update(dt, t)
...
function IngameUI:_handle_resolution_changes()
	...
	if res_x ~= self.last_resolution_x or res_y ~= self.last_resolution_y then
		self.last_resolution_y = res_y self.last_resolution_x = res_x
	end
end

function IngameUI:_update_menu_blocking_information(dt, t, input_service, end_of_level_ui)
...
function IngameUI:_menu_blocking_information(input_service, end_of_level_ui)
	local ingame_hud = self.ingame_hud
	...
	elseif active_cutscene then
		...
		do return in_view, cutscene_input_service, false end
	elseif end_screen_active then
		local end_screen_input_service = self.end_screen:input_service()
		do return in_view, end_screen_input_service, false end
	else
		return in_view, nil, false
	end
end

function IngameUI:_render_debug_ui(dt, t)
...
function IngameUI:_show_text(text, pos)
...
function IngameUI:_update_system_message_cooldown(dt)
...
function IngameUI:add_local_system_message(message)
...
function IngameUI:is_transition_allowed(transition)
...
function IngameUI:_post_handle_transition()
...
function IngameUI:handle_transition(new_transition, params)
...
function IngameUI:transition_with_fade(new_transition, params, fade_in_speed, fade_out_speed)
	local blocked_transitions = self.blocked_transitions
	if blocked_transitions and blocked_transitions [new_transition] then
		return
	end
...
function IngameUI:_update_fade_transition()
...
function IngameUI:pending_transition()
	return self._transition_fade_data ~= nil or self.new_transition_old_view ~= nil
end

function IngameUI:get_transition()
	if self.leave_game then
		if Managers.play_go:installed() then
			do return "leave_game" end
		else
			do return "finish_tutorial" end
		end
	elseif self.return_to_pc_menu then
		do return "return_to_pc_menu" end
	elseif self.return_to_title_screen then
		do return "return_to_title_screen" end
	elseif self.return_to_demo_title_screen then
		do return "return_to_demo_title_screen" end
	elseif self.restart_demo then
		do return "restart_demo" end
	elseif self.join_lobby then
		do return "join_lobby", self.join_lobby end
	elseif self.restart_game then
		do return "restart_game" end
	elseif self.quit_game then
		return "quit_game"
	end
end

function IngameUI:suspend_active_view()
	local current_view = self.current_view
...
function IngameUI:activate_end_screen_ui(screen_name, screen_config, screen_params)
	self.end_screen:on_enter(screen_name, screen_config, screen_params)
end

function IngameUI:deactivate_end_screen_ui()
	local end_screen = self.end_screen
...
function IngameUI:end_screen_active()
	local end_screen = self.end_screen
	return end_screen and end_screen.is_active
end

function IngameUI:end_screen_completed()
	local end_screen = self.end_screen
	return end_screen and end_screen.is_complete
end

function IngameUI:end_screen_fade_in_complete()
	local end_screen = self.end_screen
	return end_screen and end_screen:fade_in_complete()
end

function IngameUI:update_map_enable_state()
	if self.is_in_inn then
...
function IngameUI:play_sound(event)
...
function IngameUI:_render_version_info()
	local ui_top_renderer = self.ui_top_renderer
	local res_width = 1920 local res_height = 1080
	local text_size = 18
...
function IngameUI:_render_fps(dt)
	...
	local res_width = RESOLUTION_LOOKUP.res_w local res_height = RESOLUTION_LOOKUP.res_h
	local text_size = 24
...
function IngameUI:open_popup(popup_name, ...)
	local popups_by_name = self.popups_by_name
...
function IngameUI:close_popup(popup_name)
	local popups_by_name = self.popups_by_name
...
function IngameUI:respawn()
	...
	local peer_id = self.peer_id local local_player_id = self.local_player_id
...
function IngameUI:_update_respawning()
...
function IngameUI:_cancel_popup()
	if self.popup_id then
		Managers.popup:cancel_popup(self.popup_id)
	end
end

function IngameUI:get_hud_component(hud_component_name)
