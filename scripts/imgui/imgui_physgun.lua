...
function ImguiPhysgun:is_persistent()
	return true
end

function ImguiPhysgun:destroy_gui()
...
function ImguiPhysgun:get_player_pos_rot()
...
local function hexchr(c) return string.format("\\x%02x", string.byte(c)) end
local function hexstr(s) return string.gsub(s, ".", hexchr) end
function ImguiPhysgun:show_unit_info(unit)
	local Unit = Unit
...
local BINDING_DEFINITIONS = {
	...
	wheel = function (_method) return mouse.wheel("axis").y end,
...
function ImguiPhysgun:set_camera_lock(bool)
...
function ImguiPhysgun:can_grab(actor)
	if not actor then return false, "actor is nil"
	end
...
function ImguiPhysgun:grab_begin(pos, rot, grab_actor, grab_pos)
...
function ImguiPhysgun:grab_end()
...
function ImguiPhysgun:grab_update(dt, pos, rot)
	...
	local color = is_rotating and (input_get("arcball", "held") and Color(255, 0, 0) or Color(0, 255, 0)) or
	Color(255, 255, 0)
...
function ImguiPhysgun:laser_update(rot)
	return


end

function ImguiPhysgun:do_grab(dt, pos, rot, grab_actor, grab_pos)
...
function ImguiPhysgun:update(t, dt, is_open)
	if DO_RELOAD then
		DO_RELOAD = self:init()
	end
end

function ImguiPhysgun:draw(is_open, t, dt)
	...
	if pos then
		...
		local hit, hit_pos, hit_distance, hit_normal, hit_actor = PhysicsWorld.raycast(
		self._physics_world,
		pos,
		dir,
		raycast_length,
		...
		if hit and not self._physgun_unit then
			...
			if input_get("delete_unit") then
				...
				hit_actor = nil
			elseif input_get("spawn_cylinder") then --[[ Nothing --]]
			end
		end
