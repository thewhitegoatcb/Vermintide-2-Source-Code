...
local DEFAULT_WEAPON_SWAY_SETTINGS = { recentering_lerp_speed = 2, camera_look_sensitivity = 1, sway_range = 1, look_sensitivity = 0.6,
...
function PlayerUnitFirstPerson:init(extension_init_context, unit, extension_init_data)
	...
	self.drawer = Managers.state.debug:drawer({ mode = "immediate", name = "PlayerUnitFirstPerson" })
...
function PlayerUnitFirstPerson:reset()
	return end

function PlayerUnitFirstPerson:extensions_ready()
...
function PlayerUnitFirstPerson:destroy()
...
function PlayerUnitFirstPerson:set_state_machine(new_state_machine)
...
function PlayerUnitFirstPerson:_set_game_options_dirty()
	self._game_options_dirty = true
end

function PlayerUnitFirstPerson:update_game_options()
...
function PlayerUnitFirstPerson:check_for_jumps(unit, t)
	...
	local best_id, best_pos = nil local best_val = 0
	for i = 1, num_hits do
...
function PlayerUnitFirstPerson:_reset_jump_indicator()
...
function PlayerUnitFirstPerson:_draw_smart_objects(jump_data, smart_object_pos, dist, max_dist)
	if not jump_data then return
	end
	...
	local drawer = Managers.state.debug:drawer({ mode = "immediate", name = "DarkPactPlayerJumpDrawer" })
...
function PlayerUnitFirstPerson:get_valid_jump_id()
	return self._valid_jump_id, self._valid_jump_data
end


function PlayerUnitFirstPerson:update(unit, input, dt, context, t)
	...
	if self._want_to_show_first_person_ammo ~= nil then
		...
		elseif can_show and not want_to_show then
			...
			self._want_to_show_first_person_ammo = nil
		elseif not can_show and want_to_show then --[[ Nothing --]]
...
function PlayerUnitFirstPerson:update_aim_assist_multiplier(dt)
	if self._gamepad_auto_aim_enabled then
		...
		aim_assist_settings = action_settings and action_settings.aim_assist_settings and
		action_settings.aim_assist_settings or

		weapon_template and weapon_template.aim_assist_settings
		...
		if ( not aim_assist_settings or not aim_assist_settings.always_auto_aim ) and
		Vector3.length(look_raw) < 0.01 then
...
function PlayerUnitFirstPerson:increase_aim_assist_multiplier(value, max_value, delay)
...
function PlayerUnitFirstPerson:reset_aim_assist_multiplier()
	self.aim_assist_ramp_multiplier = 0
	self.aim_assist_ramp_multiplier_timer = 0
end
...
function PlayerUnitFirstPerson:update_player_height(t)
...
function PlayerUnitFirstPerson:set_rotation(new_rotation)
...
function PlayerUnitFirstPerson:force_look_rotation(rot, total_lerp_time)
	self.forced_look_rotation = QuaternionBox(rot)
	self.forced_lerp_timer = 0
	self.forced_total_lerp_time = total_lerp_time
end

function PlayerUnitFirstPerson:stop_force_look_rotation()
	self.forced_look_rotation = nil
	self.forced_lerp_timer = 0
	self.forced_lerp_time = nil
end

function PlayerUnitFirstPerson:update_rotation(t, dt)
...
function PlayerUnitFirstPerson:tutorial_restrict_camera_rotation(restrict, angle)
	if restrict then
...
function PlayerUnitFirstPerson:calculate_look_rotation(current_rotation, look_delta)
...
function PlayerUnitFirstPerson:calculate_aim_assisted_rotation(look_rotation, aim_assist_data, look_delta, dt)
	local aim_assist_unit = aim_assist_data.unit
...
function PlayerUnitFirstPerson:update_position()
...
function PlayerUnitFirstPerson:is_in_view(position)
	local player = Managers.player:owner(self.unit)
	local viewport_name = player.viewport_name
...
function PlayerUnitFirstPerson:is_infront(position, cos_angle_limit)
	local player = Managers.player:owner(self.unit)
	local viewport_name = player.viewport_name
	local viewport = ScriptWorld.viewport(self.world, viewport_name)
	local camera = ScriptViewport.camera(viewport)
...
function PlayerUnitFirstPerson:is_within_custom_view(position, camera_position, camera_rotation, vertical_fov_rad, horizontal_fov_rad)
	return math.point_is_inside_view(position, camera_position, camera_rotation, vertical_fov_rad, horizontal_fov_rad)
end



function PlayerUnitFirstPerson:is_within_default_view(position)
...
function PlayerUnitFirstPerson:apply_recoil(factor)
	local player = Managers.player:owner(self.unit)
	local viewport_name = player.viewport_name
	local viewport = ScriptWorld.viewport(self.world, viewport_name)
	local camera = ScriptViewport.camera(viewport)
...
function PlayerUnitFirstPerson:get_first_person_unit()
	return self.first_person_unit
end

function PlayerUnitFirstPerson:get_first_person_mesh_unit()
	return self.first_person_attachment_unit
end

function PlayerUnitFirstPerson:set_look_delta(look_delta)
...
function PlayerUnitFirstPerson:set_weapon_sway_settings(weapon_sway_settings)
	self._weapon_sway_settings = weapon_sway_settings
end

function PlayerUnitFirstPerson:play_animation_event(anim_event)
	Unit_animation_event(self.first_person_unit, anim_event)
end

function PlayerUnitFirstPerson:set_aim_constraint_target(id, target)
...
function PlayerUnitFirstPerson:current_rotation()
	return Unit.local_rotation(self.first_person_unit, 0)
end

function PlayerUnitFirstPerson:current_position()
	return Unit.local_position(self.first_person_unit, 0)
end

function PlayerUnitFirstPerson:camera_position_rotation()
	local player = Managers.player:owner(self.unit)
	local viewport_name = player.viewport_name
	local viewport = ScriptWorld.viewport(self.world, viewport_name)
	local camera = ScriptViewport.camera(viewport)
...
function PlayerUnitFirstPerson:get_projectile_start_position_rotation()
	local position = nil
...
function PlayerUnitFirstPerson:set_wanted_player_height(state, t, time_to_change)
...
function PlayerUnitFirstPerson:_player_height_from_name(name)
	local profile = self.profile

	return profile.first_person_heights [name]
end

function PlayerUnitFirstPerson:toggle_visibility(delay)
...
function PlayerUnitFirstPerson:set_first_person_mode(active, override, unarmed)
...
function PlayerUnitFirstPerson:show_third_person_units(show)
...
function PlayerUnitFirstPerson:first_person_mode_active()
	return self.first_person_mode
end

function PlayerUnitFirstPerson:abort_toggle_visibility_timer()
	self.toggle_visibility_timer = nil
end

function PlayerUnitFirstPerson:first_person_units_visible()
	return self._show_first_person_units
end

function PlayerUnitFirstPerson:abort_first_person_units_visibility_timer()
	self._first_person_units_visibility_timer = nil
	self._first_person_units_visibility_reason = nil
end

function PlayerUnitFirstPerson:toggle_first_person_units_visibility(reason, delay)
	if delay then
		local t = Managers.time:time("game")
...
function PlayerUnitFirstPerson:tutorial_show_first_person_units(show)
	Unit.set_unit_visibility(self.first_person_attachment_unit, show)
...
function PlayerUnitFirstPerson:debug_set_first_person_mode(active, override)
...
function PlayerUnitFirstPerson:hide_weapons(reason, hide_lights)
...
function PlayerUnitFirstPerson:unhide_weapons(reason)
...
function PlayerUnitFirstPerson:show_first_person_ammo(show)
...
function PlayerUnitFirstPerson:animation_set_variable(variable_name, value, strict)
...
function PlayerUnitFirstPerson:animation_event(event)
	Unit_animation_event(self.first_person_unit, event)
end

function PlayerUnitFirstPerson:create_screen_particles(name, pos, ...)
...
function PlayerUnitFirstPerson:stop_spawning_screen_particles(id)
...
function PlayerUnitFirstPerson:destroy_screen_particles(id)
...
function PlayerUnitFirstPerson:play_hud_sound_event(event, wwise_source_id, play_on_husk)
	...
	if wwise_source_id then
		...
		do return wwise_playing_id, new_wwise_source_id end
	else
...
function PlayerUnitFirstPerson:play_remote_hud_sound_event(event)
...
function PlayerUnitFirstPerson:play_sound_event(event, position)
...
function PlayerUnitFirstPerson:play_unit_sound_event(event, unit, node_id, play_on_husk)
	if play_on_husk then
...
function PlayerUnitFirstPerson:play_remote_unit_sound_event(event, unit, node_id)
	local event_id = NetworkLookup.sound_events [event]
	local network_manager = Managers.state.network
...
function PlayerUnitFirstPerson:play_camera_effect_sequence(event, t)
	Managers.state.camera:camera_effect_sequence_event(event, t)
end

function PlayerUnitFirstPerson:set_aim_assist(assist_type)
	if assist_type == "" then
		self.aim_assist_type = assist_type
	end
end

function PlayerUnitFirstPerson:enable_rig_movement()
	if script_data.disable_aim_lead_rig_motion then
		self:disable_rig_movement()
		return
	end
...
function PlayerUnitFirstPerson:disable_rig_movement()
...
function PlayerUnitFirstPerson:enable_rig_offset()
	if script_data.disable_aim_lead_rig_motion then
...
function PlayerUnitFirstPerson:disable_rig_offset()
	if self._rig_offset_enabled then
		self._rig_offset_enabled = false
	end
end

function PlayerUnitFirstPerson:update_rig_movement(look_delta)
	if script_data.disable_aim_lead_rig_motion then
		return
	end
...
function PlayerUnitFirstPerson:change_state(state)
	self._state = state
end

function PlayerUnitFirstPerson:play_camera_recoil(settings, t)
...
local weapon_sway_lerp_variables = {
	vs_packmaster = { 0.05, 0.05, 0.05 },
	vs_gutter_runner = { 0.05, 0.05, 0.05 },
	vs_poison_wind_globadier = { 0.05, 0.05, 0.05 },
	vs_warpfire_thrower = { 0.05, 0.05, 0.05 },
	vs_ratling_gunner = { 0.05, 0.05, 0.05 } }
...
function PlayerUnitFirstPerson:_update_state_machine_variables(dt, t)
	local weapon_sway_settings = self._weapon_sway_settings or DEFAULT_WEAPON_SWAY_SETTINGS
	local input_extension = self.input_extension
	...
	local lerp_move_x = 0.05 local lerp_move_y = 0.05 local lerp_move_z = 0.05
