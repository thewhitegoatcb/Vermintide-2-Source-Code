...
function ActionCareerBWNecromancerSingleTarget:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionCareerBWNecromancerSingleTarget:client_owner_start_action(new_action, t, chain_action_data, power_level, action_init_data)
	...
	if chain_action_data then
		...
		if self._talent_extension:has_talent("sienna_necromancer_5_1") then --[[ Nothing --]]
		end
...
function ActionCareerBWNecromancerSingleTarget:client_owner_post_update(dt, t, world, can_damage, current_time_in_action)
	return end

function ActionCareerBWNecromancerSingleTarget:finish(reason)
	self._inventory_extension:wield_previous_non_level_slot()
end

function ActionCareerBWNecromancerSingleTarget:_play_vo()
