...
local LOADOUT_INTERFACE_OVERRIDES = { slot_hat = "items", slot_skin = "items", slot_frame = "items", slot_melee = "deus", slot_ranged = "deus" }
...
function BackendInterfaceDeusBase:init()
...
function BackendInterfaceDeusBase:set_deus_loadout(loadout)
	self._loadouts = loadout
end

function BackendInterfaceDeusBase:reset_deus_inventory()
	self._loadouts = nil
	table.clear(self._extra_deus_inventory)
end

function BackendInterfaceDeusBase:ready()
	return true
end

function BackendInterfaceDeusBase:has_loadout_item_id(career_name, item_id)
...
function BackendInterfaceDeusBase:refresh_deus_weapons_in_items_backend()
	Managers.backend:get_interface("items"):refresh_game_mode_specific_items()
end


function BackendInterfaceDeusBase:get_talent_tree(career_name)
	return nil
end


function BackendInterfaceDeusBase:get_talents(career_name)
	return nil
end


function BackendInterfaceDeusBase:get_talent_ids(career_name)
	local talent_ids = self._talent_ids [career_name]
	return talent_ids and table.clone(talent_ids) or { }
end

function BackendInterfaceDeusBase:set_deus_talent_ids(career_name, talent_ids)
	self._talent_ids [career_name] = talent_ids
end


function BackendInterfaceDeusBase:grant_deus_weapon(item)
...
function BackendInterfaceDeusBase:get_loadout_item_id(career_name, slot_name)
...
function BackendInterfaceDeusBase:set_loadout_item(item_backend_id, career_name, slot_name)
...
function BackendInterfaceDeusBase:get_total_power_level(profile_name, career_name)
...
function BackendInterfaceDeusBase:get_rolled_over_soft_currency()
	ferror("must be implemented by subclass")
end

function BackendInterfaceDeusBase:deus_run_started()
	ferror("must be implemented by subclass")
end

function BackendInterfaceDeusBase:get_journey_cycle()
	ferror("must be implemented by subclass")
end

function BackendInterfaceDeusBase:refresh_belakor_cycle()
	ferror("must be implemented by subclass")
end

function BackendInterfaceDeusBase:has_loaded_belakor_data()
	ferror("must be implemented by subclass")
end

function BackendInterfaceDeusBase:set_has_loaded_belakor_data(value)
	ferror("must be implemented by subclass")
end


function BackendInterfaceDeusBase:_generate_journey_cycle(current_time, remaining_time, cycle)
...
function BackendInterfaceDeusBase:_generate_belakor_curse_cycle(current_time, remaining_time, cycle)
	ferror("must be implemented by subclass")
end

function BackendInterfaceDeusBase:debug_clear_meta_progression()
	self:_debug_clear_meta_progression()
	Managers.backend:commit()
end

function BackendInterfaceDeusBase:write_player_event(event_name, data)
	ferror("must be implemented by subclass")
end