...
function HeroView:init(ingame_ui_context)
...
function HeroView:initial_profile_view()
	return self.ingame_ui.initial_profile_view
end

function HeroView:_setup_state_machine(state_machine_params, optional_start_state, optional_start_sub_state, optional_params)
...
function HeroView:wanted_state()
	return self._wanted_state
end

function HeroView:clear_wanted_state()
	self._wanted_state = nil
end

function HeroView:input_service()
	return self._draw_loading and FAKE_INPUT_SERVICE or self.input_manager:get_service("hero_view")
end

function HeroView:set_input_blocked(blocked)
	self._input_blocked = blocked
end

function HeroView:input_blocked()
	return self._input_blocked
end

function HeroView:play_sound(event)
	WwiseWorld.trigger_event(self.wwise_world, event)
end

function HeroView:create_ui_elements()
...
function HeroView:_setup_hdr_gui()
	if self.is_in_inn then
		...
		if default_hdr_name then
			...
			hdr_gui_data.bottom = { renderer = renderer, world = world, viewport_name = viewport_name }
		end
		...
		if top_hdr_name then
			...
			hdr_gui_data.top = { renderer = renderer, world = world, viewport_name = viewport_name }
		end
		self._hdr_gui_data = hdr_gui_data
	end
end

function HeroView:_setup_hdr_renderer(name, layer)
...
function HeroView:post_update(dt, t)
	self._machine:post_update(dt, t)
end

function HeroView:update(dt, t)
...
function HeroView:on_enter(params)
...
function HeroView:_handle_new_ui_disclaimer()
	local mechanism_name = Managers.mechanism:current_mechanism_name()
	local global_disclaimer_states = {
		deus = { store = false, default = true, loot = false, system = false, achievements = false, keep_decorations = false },
		...
		adventure = { store = false, default = true, loot = false, system = false, achievements = false, keep_decorations = false },
		...
		default = { store = false, default = true, loot = false, system = false, achievements = false, keep_decorations = false } }
	...
	if disclaimer_states [menu_sub_state_name] ~= nil then menu_state_name = menu_sub_state_name or menu_state_name end
	Managers.ui:handle_new_ui_disclaimer(disclaimer_states, menu_state_name)
end

function HeroView:set_current_hero(profile_index)
...
function HeroView:_get_sorted_players()
	local human_players = self.player_manager:human_players()
	local player_order = { }
	for _, player in pairs(human_players) do
		player_order [#player_order + 1] = player
	end
	table.sort(player_order, function (a, b) return a.local_player and not b.local_player end)

	return player_order
end

function HeroView:_handle_mouse_input(dt, t, input_service)
	return end

function HeroView:_is_selection_widget_pressed(widget)
...
function HeroView:hotkey_allowed(input, mapping_data)
...
function HeroView:_get_screen_settings_by_state_name(state_name)
...
function HeroView:requested_screen_change_by_name(screen_name, sub_screen_name)
...
function HeroView:_change_screen_by_name(screen_name, sub_screen_name, optional_params)
...
function HeroView:_change_screen_by_index(index)
...
function HeroView:post_update_on_enter()
...
function HeroView:post_update_on_exit()
	if self._machine then
		self._machine:destroy()
		self._machine = nil
	end
...
function HeroView:_handle_view_popups()
...
function HeroView:exit(return_to_game, ignore_sound)
...
function HeroView:transitioning()
...
function HeroView:_handle_exit(input_service)
	return end

function HeroView:suspend()
	self.input_manager:device_unblock_all_services("keyboard", 1)
	self.input_manager:device_unblock_all_services("mouse", 1)
	self.input_manager:device_unblock_all_services("gamepad", 1)
	self.suspended = true
...
function HeroView:close_menu(return_to_main_screen, ignore_sound)
...
function HeroView:destroy_hdr_gui()
	local hdr_gui_data = self._hdr_gui_data
...
function HeroView:_is_button_pressed(widget)
...
function HeroView:_set_loading_overlay_enabled(enabled, message)
...
function HeroView:current_state()
