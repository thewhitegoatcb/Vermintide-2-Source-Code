...
local show_blackboard_data = {
	BTFallAction = { "is_falling", "fall_done", "fall_state" },
	BTMoveToGoalAction = { "is_passive" },
	BTBossFollowAction = { "move_state" },
	BTMeleeSlamAction = { "move_state", "attack_anim", "attack_anim_driven" },
	BTTargetUnreachableAction = { "move_state", "target_dist" },
	BTCrazyJumpAction = { "jump_data" },
	BTSkulkAroundAction = { "in_los", "skulk_pos", "debug_state" },
	BTCirclePreyAction = { "move_state" },
	BTAttackAction = { "attacks_done", "target_dist", "slot_layer" },
	BTClanRatFollowAction = { "move_state", "using_smart_object" },
	BTCombatShoutAction = { "nav_target_dist_sq", "slot_layer" },
	BTClimbAction = { "is_in_smartobject_range", "is_climbing", "climb_state" },
	BTSkulkAroundAction = { "skulk_jump_tries" },
	BTPackMasterSkulkAroundAction = { "skulk_in_los", "skulk_dogpile", "skulk_time_left", "skulk_debug_state" },
	BTPackMasterDragAction = { "drag_check_index", "drag_check_time_debug" },
	BTSkulkApproachAction = { "target_dist" },
	BTSkulkIdleAction = { "skulk_data" },
	BTNinjaApproachAction = { "skulk_pos_is_jump_off_point" },
	BTTrollDownedAction = { "downed_state" },
	BTRatlingGunnerShootAction = { rshoot_action_debug },
	BTTentacleAttackAction = { tentacle_action_debug },
	BTChaosSorcererSkulkApproachAction = { chaos_sorc_skulk_action_debug },
	BTVortexWanderAction = { "vortex_data" },
	BTInVortexAction = { "in_vortex_state" },
	BTChaosExaltedSorcererSkulkAction = { chaos_sorc_exalt_skulk_action_debug },
	BTJumpSlamAction = { rat_ogre_jump_slam_action_debug } }
...
local function present_perception(gui, x, y, blackboard)
	local x1 = x local y1 = y local y2 = y
	local i = 1
...
local function longest_text_length_check(text, longest_text, longest_text_length)
	...
	if longest_text_length < text_length then
		do return text, text_length end
	else
...
local function draw_node(gui, node, text, running, x1, y1, node_width, extra_height, dt, tcolor)
	...
	local bottom_y = y1 + NODE_HEIGHT + extra_height local box_height = nil
...
local function draw_node_children(bt, gui, node, node_children, blackboard, row, x1, y1, node_width, extra_node_height, total_width, extra_utility_height, t, dt)
	...
	local cx = start_x local cy = start_y
...
function DrawAiBehaviour.tree_width(gui, node)
...
function DrawAiBehaviour.draw_tree(bt, gui, node, blackboard, row, t, dt, x, y, draw_utility, extra_info)
	local identifier = node._identifier
	local node_children = node._children
