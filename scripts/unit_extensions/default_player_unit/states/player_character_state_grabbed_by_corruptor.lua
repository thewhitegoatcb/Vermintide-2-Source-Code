...
function PlayerCharacterStateGrabbedByCorruptor:init(character_state_init_context)
	PlayerCharacterState.init(self, character_state_init_context, "grabbed_by_corruptor")
	self.next_hanging_damage_time = 0
end

function PlayerCharacterStateGrabbedByCorruptor:on_enter(unit, input, dt, context, t, previous_state, params)
...
function PlayerCharacterStateGrabbedByCorruptor:on_exit(unit, input, dt, context, t, next_state)
	local first_person_extension = self.first_person_extension
	local status_extension = self.status_extension
...
function PlayerCharacterStateGrabbedByCorruptor:update(unit, input, dt, context, t)
	local csm = self.csm
