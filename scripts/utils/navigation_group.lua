...
function NavigationGroup:init(nav_world, poly_hash, poly, poly_center, poly_area, group_number)
...
function NavigationGroup:make_string_of_group(write_string)
...
function NavigationGroup:add_polygon(poly, poly_center, poly_area, nav_world)
...
function NavigationGroup:add_neighbour_group(group, connected_by_ledge)
...
function NavigationGroup:remove_neighbour_group(group)
...
function NavigationGroup:set_main_path_index(index)
	self._main_path_index = index
end

function NavigationGroup:set_distance_from_finish(distance_from_finish)
	self._distance_from_finish = distance_from_finish
end

function NavigationGroup:get_group_neighbours()
	return self._group_neighbours
end

function NavigationGroup:get_group_ledge_neighbours()
	return self._group_ledge_neighbours
end

function NavigationGroup:get_main_path_index()
	return self._main_path_index
end

function NavigationGroup:calculate_group_center(poly_center, poly_hash, nav_world)
	...
	if new_center_hash then
		...
		if not group_polygons [new_center_hash] then
			...
			if new_center == nil then
				...
				new_center_hash = poly_hash new_center = poly_center
			end
		end
...
function NavigationGroup:get_group_polygons_centers(list, nav_world)
	error("not used?")
...
function NavigationGroup:get_poly_center_from_hash(poly_hash)
	return self._group_polygons [poly_hash]
end

function NavigationGroup:get_group_center_poly()
	return self._center_poly
end

function NavigationGroup:get_group_center()
	return self._group_center
end

function NavigationGroup:get_group_area()
	return self._area
end

function NavigationGroup:get_group_polygons()
	return self._group_polygons
end

function NavigationGroup:get_group_size()
	return self._group_size
end

function NavigationGroup:get_distance_from_finish()
	return self._distance_from_finish
end

function NavigationGroup:destroy()
	self._group_neighbours = { }
	self._group_polygons = { }
	self._group_size = 0
end
...
function NavigationGroup:calc_polygon_center(poly, nav_world)
...
function NavigationGroup:get_poly_hash(poly, nav_world)
	local a, b, c = Script.temp_count()
...
function NavigationGroup:breadth_first_find_nearest_group_triangle(root_triangle, nav_world)
...
function NavigationGroup:print_group(world, nav_world, line_object, line_drawer, debug_world_gui)
...
function NavigationGroup:draw_poly_lines(poly, color, nav_world, line_object, debug_world_gui)
	local a, b, c = Script.temp_count()
