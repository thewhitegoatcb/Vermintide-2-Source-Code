...
local challenge_settings = {
	default = {
		possible_challenges = { { reward = "markus_questing_knight_passive_power_level", type = "kill_elites",
				amount = { 1, 15, 15, 20, 20, 30, 30, 30 } }, { reward = "markus_questing_knight_passive_attack_speed", type = "kill_specials",
				amount = { 1, 10, 10, 15, 15, 20, 20, 20 } }, { reward = "markus_questing_knight_passive_cooldown_reduction", type = "kill_monsters",
				amount = { 1, 1, 1, 1, 1, 1, 1, 1 } }, { reward = "markus_questing_knight_passive_health_regen", type = "find_grimoire",
				amount = { 1, 1, 1, 1, 1, 1, 1, 1 }, condition = only_when_grims_allowed_and_there_from_the_start }, { reward = "markus_questing_knight_passive_damage_taken", type = "find_tome",
				amount = { 1, 1, 1, 1, 1, 1, 1, 1 }, condition = only_when_tomes_allowed_and_there_from_the_start } },

		side_quest_challenge = { reward = "markus_questing_knight_passive_strength_potion", type = "kill_enemies",


			amount = { 1, 100, 125, 150, 175, 200, 200, 200 } } } }
...
function PassiveAbilityQuestingKnight:init(extension_init_context, unit, extension_init_data, ability_init_data)
...
function PassiveAbilityQuestingKnight:extensions_ready(world, unit)
...
function PassiveAbilityQuestingKnight:_create_quests()
...
function PassiveAbilityQuestingKnight:_generate_quest_pool()
	local challenge_list = table.clone(self:_get_possible_challenges())
	return challenge_list
end

function PassiveAbilityQuestingKnight:_get_possible_challenges()
...
function PassiveAbilityQuestingKnight:_get_side_quest_challenge()
	local mechanism_name = Managers.mechanism:current_mechanism_name()
	local settings = challenge_settings [mechanism_name] or challenge_settings.default
...
function PassiveAbilityQuestingKnight:_always_reset_quest_pool()
	local mechanism_name = Managers.mechanism:current_mechanism_name()
	local settings = challenge_settings [mechanism_name] or challenge_settings.default
	return settings.always_reset_quest_pool or false
end

function PassiveAbilityQuestingKnight:_start_quest_from_pool(quest_pool, num_to_start)
	local challenge_manager = Managers.venture.challenge
...
function PassiveAbilityQuestingKnight:on_initial_talents_synced(talent_extension)
...
function PassiveAbilityQuestingKnight:destroy()
	self:_unregister_events()
end

function PassiveAbilityQuestingKnight:_register_events()
	Managers.state.event:register(self, "on_initial_talents_synced", "on_initial_talents_synced")
end

function PassiveAbilityQuestingKnight:_unregister_events()
