ActionChargedProjectileUtility = {
	prepare_charged_projectile = function (action_data, owner_unit, weapon_unit, item_name, charge_level, power_level)
		...
		local projectile_context = { first_shot = true,
	...
	fire_charged_projectile = function (projectile_context, is_critical_strike, last_projectile, position, rotation, gaze_settings)
		...
		if projectile_context.is_grenade then
			...
			if last_projectile then
				...
				if projectile_context.rewield_grenade then
					trigger_wield = "rewield_wielded_weapon"
				else
					trigger_wield = "wield_previous_weapon"
				end
			end
		end
		...
		if action_data.fire_at_gaze_setting and action_data.throw_up_this_much_in_target_direction and
		ScriptUnit.has_extension(owner_unit, "eyetracking_system") then
...
function ActionChargedProjectile:init(world, item_name, is_server, owner_unit, damage_unit, first_person_unit, weapon_unit, weapon_system)
...
function ActionChargedProjectile:client_owner_start_action(new_action, t, chain_action_data, power_level)
...
function ActionChargedProjectile:client_owner_post_update(dt, t, world, can_damage)
...
function ActionChargedProjectile:_update_extra_shots(buff_extension, shots_to_consume)
...
function ActionChargedProjectile:_shoot(t)
	local projectile_context = self._projectile_context
	local action_data = projectile_context.action_data
	local owner_unit = self.owner_unit
	...
	if action_data.fire_pos_rot then
		position, rotation = action_data:fire_pos_rot(first_person_unit, self.weapon_unit, owner_unit, self.world)
	else
...
function ActionChargedProjectile:finish(reason)
