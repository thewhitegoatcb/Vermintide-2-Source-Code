local TOTEM_STATES = { COOLDOWN = "COOLDOWN", ACTIVE = "ACTIVE", READY = "READY" }
...
function Totem:init(logging_prefix)
...
function Totem:update(dt, t)
	...
	elseif self._state == TOTEM_STATES.READY then --[[ Nothing --]]
...
function Totem:spawn(spawn_position)
	...
	local queue_id = Managers.state.conflict:spawn_queued_unit(
	Breeds.shadow_totem,
...
function Totem:get_state()
	return self._state
end

function Totem:get_position()
...
function Totem:get_unit()
	local active_totem_data = self._active_totem_data
	return active_totem_data and active_totem_data.unit
end

function Totem:_clear_active_totem()
	local active_totem_data = self._active_totem_data
...
local TOTEM_COUNT = 1
return { description = "curse_belakor_totems_desc", curse_package_name = "resource_packages/mutators/mutator_curse_belakor_totems", display_name = "curse_belakor_totems_name", icon = "deus_curse_belakor_01",
