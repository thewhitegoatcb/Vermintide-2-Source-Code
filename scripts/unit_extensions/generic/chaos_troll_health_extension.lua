...
function ChaosTrollHealthExtension:init(extension_init_context, unit, ...)
...
function ChaosTrollHealthExtension:extensions_ready(world, unit, extension_name)
...
function ChaosTrollHealthExtension:current_max_health_percent()
	return self.health / self.current_max_health
end

function ChaosTrollHealthExtension:set_max_health(value)
...
function ChaosTrollHealthExtension:_setup_initial_health_variables(new_max_health)
...
function ChaosTrollHealthExtension:hot_join_sync(peer_id)
	local go_id = self._game_object_id or Managers.state.unit_storage:go_id(self.unit)
	if go_id then
...
function ChaosTrollHealthExtension:update_regen_effect(t, dt, regen_pulse_interval, intensity)
...
function ChaosTrollHealthExtension:update(dt, context, t)
...
function ChaosTrollHealthExtension:_should_die()
	return self.state == "wounded" and self.health <= self.damage
end

function ChaosTrollHealthExtension:apply_client_predicted_damage(predicted_damage)

	return end

function ChaosTrollHealthExtension:add_damage(attacker_unit, damage_amount, hit_zone_name, damage_type, hit_position, damage_direction, damage_source_name, hit_ragdoll_actor, source_attacker_unit, hit_react_type, is_critical_strike, added_dot, first_hit, total_hits, attack_type)
...
function ChaosTrollHealthExtension:set_downed_finished()
	if self.state == "down" then
...
function ChaosTrollHealthExtension:die(damage_type)
...
function ChaosTrollHealthExtension:sync_health_to_clients(set_max_health, reason)
...
function ChaosTrollHealthExtension:min_health_reached()
	return self.health - self.damage <= self.respawn_hp_min
end


function ChaosTrollHealthExtension:force_set_wounded()
	self.wounded = true
	self.state = "wounded"
end