...
function GamePadAbilityUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function GamePadAbilityUI:_create_ui_elements()
...
function GamePadAbilityUI:_setup_activated_ability()
...
function GamePadAbilityUI:_sync_ability_cooldown()
	local player = self._player
	local player_unit = player.player_unit
...
function GamePadAbilityUI:_update_thornsister_passive()
	local player = self._player
	local player_unit = player.player_unit
	if not player_unit then
		return
...
function GamePadAbilityUI:_set_ability_activated(activated)
...
function GamePadAbilityUI:_set_ability_cooldown_state(cooldown_fraction, initialize)
...
function GamePadAbilityUI:destroy()
	local event_manager = Managers.state.event
...
function GamePadAbilityUI:set_visible(visible)
	self._is_visible = visible

	self:_set_elements_visible(visible)
end

function GamePadAbilityUI:_set_elements_visible(visible)
...
function GamePadAbilityUI:_handle_gamepad_activity()
...
function GamePadAbilityUI:_handle_gamepad()
...
function GamePadAbilityUI:update(dt, t)
...
function GamePadAbilityUI:_handle_active_ability()
...
function GamePadAbilityUI:_handle_resolution_modified()
...
function GamePadAbilityUI:draw(dt)
...
function GamePadAbilityUI:set_dirty()
	self._dirty = true
end

function GamePadAbilityUI:_set_widget_dirty(widget)
	widget.element.dirty = true
end

function GamePadAbilityUI:event_input_changed()
...
function GamePadAbilityUI:_set_input(widget, input_action)
...
function GamePadAbilityUI:_get_input_texture_data(input_action)
	local input_manager = self._input_manager
	...
	if key_index ~= UNASSIGNED_KEY then
		if device_type == "keyboard" then
			if type(key_index) == "number" then
				...
				do return nil, Localize(key_index), prefix_text end
			end
		elseif device_type == "mouse" then
			local button_texture_data = ButtonTextureByName(device_type .. "_" .. key_index, platform)
			do return button_texture_data, Mouse.button_name(key_index), prefix_text end
...
function GamePadAbilityUI:_update_ability_animations(dt, t)
	if not self._is_visible then
		return false
	end
...
function GamePadAbilityUI:set_alpha(alpha)
...
function GamePadAbilityUI:_update_muneric_ui_ability_cooldown()
	local player = Managers.player:local_player()
	...
	if can_use_ability then return
	end
	local ability_cooldown, max_cooldown = career_extension:current_ability_cooldown()
