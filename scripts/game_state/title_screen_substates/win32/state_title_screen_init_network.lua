...
function StateTitleScreenInitNetwork:on_enter(params)
...
function StateTitleScreenInitNetwork:_load_save_data()
...
function StateTitleScreenInitNetwork:cb_save_data_loaded(info)
	if info.error then
...
function StateTitleScreenInitNetwork:update(dt, t)
...
function StateTitleScreenInitNetwork:_connected_to_steam()
...
function StateTitleScreenInitNetwork:_next_state()
	local eac_initialized = EAC.is_initialized()
	local eac_init_error, eac_error = EAC.initialization_error()
	local ready_to_exit = Managers.backend:profiles_loaded() and
	not Managers.backend:is_waiting_for_user_input() and
	eac_initialized
	if ready_to_exit then
		...
		if GameSettingsDevelopment.skip_start_screen then
			do return StateTitleScreenLoadSave end
		else
...
function StateTitleScreenInitNetwork:on_exit(application_shutdown) return end

function StateTitleScreenInitNetwork:create_popup(error)
...
function StateTitleScreenInitNetwork:_create_eac_error_popup(localized_error)
...
function StateTitleScreenInitNetwork:_handle_popup()
