...
local SCREENSPACE_DEATH_EFFECTS = { heavy = "fx/screenspace_blood_drops_heavy", blunt = "fx/screenspace_blood_drops" }
...
local function handle_military_event_achievement(damage_type, breed_name, statistics_db)
	if damage_type == "military_finish" and breed_name == "chaos_warrior" then
		local stat_names = { "military_statue_kill_chaos_warriors", "military_statue_kill_chaos_warriors_cata" }
	...
	if Unit.alive(target_unit) then
		...
		if tentacle_data.unit then
			...
			if current_length > 0 or t < tentacle_data.wait_for_release then
				do return DeathReactions.IS_NOT_DONE end
			else
...
local function update_wall_nail(unit, dt, t, data)
	...
	for hit_ragdoll_actor, nail_data in pairs(data.wall_nail_data) do
		...
		if actor and Actor.is_physical(actor) then
			...
			local hit, hit_position, hit_distance, _, _ = PhysicsWorld.immediate_raycast(
			World.get_data(world, "physics_world"),
			position,
			dir,
			data.nailed and math.min(ray_dist, 0.4) or ray_dist,
			"closest",
...
local function trigger_player_killing_blow_ai_buffs(ai_unit, killing_blow)
	...
	if not side_manager:is_enemy(attacker_unit, ai_unit) then return
	end
...
DeathReactions.templates = {
		...
		unit = {
			...
			start = function (unit, context, t, killing_blow, is_server)
				...
				if blackboard.suicide_run ~= nil and blackboard.suicide_run.explosion_started then
					...
					do return nil, DeathReactions.IS_DONE end
				else
					local data, result = ai_default_unit_start(unit, context, t, killing_blow, is_server)
					data.blackboard = blackboard
					trigger_unit_dialogue_death_event(unit, killing_blow [DamageDataIndex.ATTACKER], killing_blow [DamageDataIndex.HIT_ZONE], killing_blow [DamageDataIndex.DAMAGE_TYPE])
		...
		unit = {
			...
			start = function (unit, context, t, killing_blow, is_server)
				...
				if killing_blow [DamageDataIndex.HIT_ZONE] == "aux" then
					...
					do return data, DeathReactions.IS_NOT_DONE end
				else
					data.blackboard = blackboard
					return data, result
				end
		...
		unit = {
			...
			start = function (unit, context, t, killing_blow, is_server)
				...
				if math.random() <= chance_to_spawn_ammmo then
					...
					local pickup_settings = AllPickups [pickup_name]
					local extension_init_data = {
						pickup_system = { has_physics = false, spawn_type = "loot",
							pickup_name = pickup_name } }
			...
			start = function (unit, context, t, killing_blow, is_server)
				if not is_hot_join_sync(killing_blow) then
					...
					if ScriptUnit.has_extension(unit, "dialogue_system") then
						SurroundingAwareSystem.add_event(unit, "player_death", DialogueSettings.death_discover_distance,
	...
	explosive_barrel = {
		...
		unit = {
			...
			start = function (unit, context, t, killing_blow, is_server)
				...
				local attacker_unit = killing_blow [DamageDataIndex.ATTACKER]
				local data = { explode_time = explode_time, killer_unit = attacker_unit }
		...
		husk = {
			...
			start = function (unit, context, t, killing_blow, is_server)
				local network_time = Managers.state.network:network_time()
				local explode_time = network_time
				local data = { explode_time = explode_time, killer_unit = killing_blow [DamageDataIndex.ATTACKER] }
	...
	nurgle_liquid_blob = {
		...
		unit = {
			...
			start = function (unit, context, t, killing_blow, is_server, death_extension)
				...
				local data = { start_time = network_time, shrink_and_despawn_time = shrink_and_despawn_time }
				...
				if network_time >= start_time + delaytime then
					...
					if not data.destroyed then
						...
						if not position_on_navmesh then
							...
							local extension_init_data = {
								area_damage_system = { liquid_template = "nurgle_liquid",
									flow_dir = direction,
					...
					elseif data.destroyed and network_time >= start_time + 0.5 then
						if data.shrink_and_despawn_time then
							...
							elseif
							shrinking_state == "shrinking" and scale_unit_extension:scaling_complete() then
								Managers.state.unit_spawner:mark_for_deletion(unit)
								result = DeathReactions.IS_DONE
							end
						else
	...
	lamp_oil = {
		...
		unit = {
			...
			start = function (unit, context, t, killing_blow, is_server)
				local network_time = Managers.state.network:network_time()
				local data = { killer_unit = killing_blow [DamageDataIndex.ATTACKER], start_time = network_time }
			...
			update = function (unit, dt, context, t, data)
				...
				if not data.exploded then
					...
					if not position_on_navmesh then
						...
						local extension_init_data = {
							area_damage_system = { liquid_template = "lamp_oil_fire",
								flow_dir = direction,
...
function DeathReactions.templates.shadow_skull.unit.start(unit, context, t, killing_blow, is_server)
...
function DeathReactions.templates.shadow_skull.husk.start(unit, context, t, killing_blow, is_server)
...
function DeathReactions.templates.tower_homing_skull.unit.start(unit, context, t, killing_blow, is_server)
	local data, result = DeathReactions.templates.ai_default.unit.start(unit, context, t, killing_blow, is_server)
...
function DeathReactions.templates.tower_homing_skull.unit.update(unit, dt, context, t, data)
...
function DeathReactions.templates.tower_homing_skull.husk.start(unit, context, t, killing_blow, is_server)
	local data, result = DeathReactions.templates.ai_default.husk.start(unit, context, t, killing_blow, is_server)
	local projectile_extension = ScriptUnit.extension(unit, "projectile_system")
	projectile_extension:destroy()
...
end) DeathReactions.templates.destructible_ward = { unit = { pre_start = function (unit, context, t, killing_blow) return end, start = function (unit, context, t, killing_blow, is_server) Managers.state.game_mode:level_object_killed(unit, killing_blow) Unit.set_flow_variable(unit, "current_health", 0) Unit.flow_event(unit, "lua_on_death") Managers.state.entity:remove_extensions_from_unit(unit, { "WardExtension" }) end, update = function (unit, dt, context, t, data) return end }, husk = { pre_start = function (unit, context, t, killing_blow) return end, start = function (unit, context, t, killing_blow, is_server) Managers.state.game_mode:level_object_killed(unit, killing_blow) Unit.flow_event(unit, "lua_on_death") Managers.state.entity:remove_extensions_from_unit(unit, { "WardExtension" }) end, update = function (unit, dt, context, t, data) return end } }

function DeathReactions.get_reaction(death_reaction_template, is_husk)
...
function DeathReactions._add_ai_killed_by_player_telemetry(victim_unit, breed_name, player_unit, player, damage_type, weapon_name, death_hit_zone)
	local network_manager = Managers.state.network
	local is_server = network_manager.is_server
	local player_position = POSITION_LOOKUP [player_unit]
	local victim_position = POSITION_LOOKUP [victim_unit]

	Managers.telemetry_events:player_killed_ai(
	player,
	player_position,
	victim_position,
	breed_name,
