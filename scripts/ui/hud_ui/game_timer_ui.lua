...
function GameTimerUI:init(parent, ingame_ui_context)
	self._gui = ingame_ui_context.ui_renderer.gui
	self._visible = true

	self._enabled = Application.make_hash(Application.user_setting("enable_ingame_timer")) == "473df4ed7fa71691" and
	not Development.parameter("disable_ingame_timer")

	Managers.state.event:register(self, "start_game_time", "event_start_game_time")
end


function GameTimerUI:destroy()
	Managers.state.event:unregister("start_game_time", self)
end


function GameTimerUI:event_start_game_time(start_time)
	self._start_time = start_time
end


function GameTimerUI:set_visible(visible)
...
function GameTimerUI:update()
	...
	if start_time then
		...
		local font = "materials/fonts/arial" local font_size = 28 * scale
