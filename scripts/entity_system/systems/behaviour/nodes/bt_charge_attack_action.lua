...
function BTChargeAttackAction:init(...)
...
function BTChargeAttackAction:enter(unit, blackboard, t)
...
function BTChargeAttackAction:leave(unit, blackboard, t, reason, destroy)
...
function BTChargeAttackAction:run(unit, blackboard, t, dt)
...
function BTChargeAttackAction:_start_charging(unit, blackboard)
...
function BTChargeAttackAction:_start_approaching(unit, blackboard)
	local action = blackboard.action
...
function BTChargeAttackAction:_start_lunge(unit, blackboard, lunge_data, distance_to_target, t)
	local action = blackboard.action
...
function BTChargeAttackAction:_start_impact(unit, blackboard, hit_target, hit_wall, hit_target_blocked, target_avoided_attack)
	local action = blackboard.action
...
function BTChargeAttackAction:_start_align_to_target(unit, blackboard)
	local action = blackboard.action
...
function BTChargeAttackAction:_cancel_charge(unit, blackboard)
...
function BTChargeAttackAction:_check_unit_and_wall_collision(unit, blackboard, dt, ignore_wall_collision)
	local action = blackboard.action
...
function BTChargeAttackAction:_check_overlap(unit, blackboard, action)
	...
	local num_results = Broadphase.query(broadphase, self_pos, hit_ai_radius,
	broadphase_query_result,
	...
	for i = 1, num_results do
		...
		if dot > 0 and hit_unit ~= unit and
		not hit_units [hit_unit] then
...
function BTChargeAttackAction:_charged_at_player(unit, hit_unit, blackboard, action)
...
function BTChargeAttackAction:_push_player(unit, hit_unit, blackboard, action, blocked)
	local to_hit_unit = POSITION_LOOKUP [hit_unit] - POSITION_LOOKUP [unit]
...
function BTChargeAttackAction:_hit_player(unit, blackboard, hit_unit, action, attack_direction)
	local hit_attacking_target = hit_unit == blackboard.attacking_target
...
function BTChargeAttackAction:_hit_ai(unit, hit_unit, action, blackboard, t)
	...
	if
	immune_breeds and immune_breeds [breed_name] then
		return
	end
...
function BTChargeAttackAction:_check_wall_collision(unit, blackboard, dt)
...
function BTChargeAttackAction:_is_at_edge(unit, blackboard, from, direction)
	...
	local hit = PhysicsWorld.raycast(pw,
	line_of_sight_start,
	...
	local hit2 = PhysicsWorld.raycast(pw,
	downwards_start,
	downwards_direction,
	downwards_dist,
	"closest",
...
function BTChargeAttackAction:_check_smartobjects(unit, blackboard)
...
function BTChargeAttackAction:_run_starting(unit, blackboard)
...
function BTChargeAttackAction:_run_approaching(unit, blackboard, t, dt)
	local navigation_extension = blackboard.navigation_extension
...
function BTChargeAttackAction:_run_charging(unit, blackboard, t, dt)
	...
	local lunge_data = blackboard.lunge_data
	if
	lunge_data and distance_to_target <= lunge_data.enter_thresholds.far then
...
function BTChargeAttackAction:_run_lunge(unit, blackboard, lunge_data, t, dt)
	local locomotion_extension = blackboard.locomotion_extension
	local action = blackboard.action
...
function BTChargeAttackAction:_run_impact(unit, blackboard, t, dt)
...
function BTChargeAttackAction:_run_align_to_target(unit, blackboard, t, dt)
...
function BTChargeAttackAction:_run_cancel(unit, blackboard, t, dt)
...
function BTChargeAttackAction:_select_charging_animation_and_duration(action, unit, target_unit, blackboard)
...
function BTChargeAttackAction:_pick_distance_identifier(distance_threshold_table, distance)
	...
	wanted_distance_identifier =
	wanted_distance_identifier or previous_distance_identifier
...
function BTChargeAttackAction:_slow_down(unit, blackboard, slow_down_speed, t, dt)
	local locomotion_extension = blackboard.locomotion_extension
	local current_velocity = locomotion_extension:current_velocity()
...
function BTChargeAttackAction:_set_leaning_enabled(unit, blackboard, enable)
...
function BTChargeAttackAction:_update_leaning_position(unit, blackboard, dt, position)

	blackboard.lean_target_position_boxed:store(position)
end

function BTChargeAttackAction:_update_animation_movement_speed(unit, blackboard, dt)
	local locomotion_extension = blackboard.locomotion_extension
	local current_velocity = locomotion_extension:current_velocity()
...
function BTChargeAttackAction:_get_turn_slowdown_percentage(unit, blackboard, dt, direction)
	local action = blackboard.action
	local rotation = Unit.local_rotation(unit, 0)
...
function BTChargeAttackAction:anim_cb_charge_start_finished(unit, blackboard)
	if blackboard.ray_can_go_to_target then
		self:_start_charging(unit, blackboard)
	else
		self:_start_approaching(unit, blackboard)
...
function BTChargeAttackAction:anim_cb_charge_charging_finished(unit, blackboard)
...
function BTChargeAttackAction:anim_cb_charge_impact_finished(unit, blackboard)
...
function BTChargeAttackAction:anim_cb_disable_charge_collision(unit, blackboard)
