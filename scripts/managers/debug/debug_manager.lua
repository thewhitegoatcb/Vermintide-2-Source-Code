...
local RPCS = { "rpc_debug_command" }



GLOBAL_TIME_SCALE = GLOBAL_TIME_SCALE or 1
local time_scale_list = { 1e-05, 0.0001, 0.001, 0.01, 0.1, 1, 5, 10, 20, 30, 50, 75, 100, 125, 150, 175, 200, 250, 300, 500, 750, 1000, 5000, 10000 }
local speed_scale_list = { 10, 20, 30, 40, 50, 75, 100, 150, 200, 250, 300, 500, 750, 1000, 2000, 3000, 5000 }
local NOT_USED = 0

function DebugManager:init(world, free_flight_manager, input_manager, network_event_delegate, is_server)
	self._world = world
	self._drawers = { }
	...
	QuickDrawer = self:drawer({ name = "quick_debug", mode = "immediate" })
	QuickDrawerStay = self:drawer({ name = "quick_debug_stay", mode = "retained" })
...
function DebugManager:reset_drawer(drawer_name)
	if self._drawers [drawer_name] then
		self._drawers [drawer_name]:reset()
	end
end

function DebugManager:update(dt, t)
	...
	if script_data.debug_unit and
	self._is_server and script_data.debug_behaviour_trees then
...
function DebugManager:_clear_debug_draws()
...
function DebugManager:register_update(name, func)
	self._debug_updates [name] = func
end

function DebugManager:unregister_update(name)
	self._debug_updates [name] = nil
end

function DebugManager:update_time_scale(dt)
...
function DebugManager:_adjust_player_speed()
...
function DebugManager:_adjust_gamepad_player_speed()
...
function DebugManager:_update_actor_draw(dt)
	...
	for _, data in pairs(self._actor_draw) do
		PhysicsWorld.overlap(physics_world, function (...) self:_actor_draw_overlap_callback(data, ...) end, "shape", "sphere", "size", data.range, "pose", pose, "types", "both", "collision_filter", data.collision_filter)
		if data.actors then
...
function DebugManager:enable_actor_draw(collision_filter, color, range)
	local world = self._world
	...
	self._actor_drawer = self:drawer({ mode = "immediate", name = "_actor_drawer" })
	self._actor_draw [collision_filter] = { color = QuaternionBox(color), range = range, collision_filter = collision_filter }
end
...
function DebugManager:color(unit, alpha)
...
function DebugManager:_get_next_color_index()
...
function DebugManager:_color_index_in_use(index)
	for unit, color_index in pairs(self._unit_color_list) do
...
function DebugManager:_toggle_debug_mouse_cursor(set)
	...
	if set then
		...
		function self.free_flight_manager._update_global_free_flight() return end
	else
...
function DebugManager:_update_paused_game(input, dt)
...
function DebugManager:_update_visuals()

	local drawer = Managers.state.debug:drawer({ name = "mouse_ray_hit", mode = "immediate" })
...
function DebugManager:selected_unit()
	return self._selected_unit
end

function DebugManager:_create_screen_gui()
	self._screen_gui = World.create_screen_gui(self._world, "material", "materials/fonts/gw_fonts", "immediate")
end

function DebugManager:draw_screen_rect(x, y, z, w, h, color)
...
function DebugManager:draw_screen_text(x, y, z, text, size, color, font)
	if not self._screen_gui then
		self:_create_screen_gui()
	end
	local font_type = font or "hell_shark"
	local font_by_resolution = UIFontByResolution({ dynamic_font = true, font_type = font_type, font_size = size })
...
function DebugManager:screen_text_extents(text, size)
	if not self._screen_gui then
		self:_create_screen_gui()
	end
...
function DebugManager:set_time_scale(time_scale_index)
...
function DebugManager:set_time_paused()
...
function DebugManager:_cycle_patched_items()
	local patched_items_list = self._patched_items_list
...
function DebugManager:rpc_debug_command(channel_id, debug_command_lookup, optional_parameter)
...
function DebugManager:_load_patched_items_into_backend()
	local backend_manager = Managers.backend
	local is_local_backend = backend_manager:is_local()
	if not is_local_backend then
		Debug.sticky_text("patching of ItemMasterList only works with local backend")
	...
	for name, data in pairs(item_master_list_debug_patch) do repeat
			...
			if already_exists then
				...
				printf("awarded %s: to player", name) end until true
	end
	return added_items
end

function DebugManager:_load_resource(unit_name)
...
function DebugManager:send_conflict_director_command(method, breed_name, position, extra_data)
	breed_name =
	breed_name or ""
	...
	if picked_enhancements and next(picked_enhancements) then
		enhancements_string = table.concat(table.keys_if(picked_enhancements, { }, function (key, val) return val == true end), ",")
	end
...
function DebugManager:_update_unit_spawning(dt, t)
