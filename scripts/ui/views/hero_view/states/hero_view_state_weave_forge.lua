...
function HeroViewStateWeaveForge:on_enter(params)
...
function HeroViewStateWeaveForge:gamepad_style_active()
	return self._gamepad_style_active
end

function HeroViewStateWeaveForge:get_ui_renderer()
	if self._gamepad_style_active then
...
function HeroViewStateWeaveForge:get_ui_top_renderer()
	return self.ui_top_renderer
end

function HeroViewStateWeaveForge:hdr_renderer()
	return self.parent:hdr_renderer()
end

function HeroViewStateWeaveForge:hdr_top_renderer()
	return self.parent:hdr_top_renderer()
end

function HeroViewStateWeaveForge:_setup_gamepad_gui()
	if self.is_in_inn then
		local gui_data = { }
...
function HeroViewStateWeaveForge:_setup_gamepad_renderer(name, layer, shading_environment)
...
function HeroViewStateWeaveForge:_destroy_gamepad_gui()
...
function HeroViewStateWeaveForge:_setup_menu_layout()
...
function HeroViewStateWeaveForge:create_ui_elements(params)
...
function HeroViewStateWeaveForge:set_input_description(input_description)
...
function HeroViewStateWeaveForge:disable_player_world()
...
function HeroViewStateWeaveForge:enable_player_world()
...
function HeroViewStateWeaveForge:enable_ingame_overlay()
...
function HeroViewStateWeaveForge:disable_ingame_overlay()
...
function HeroViewStateWeaveForge:_initial_windows_setups(params)
...
function HeroViewStateWeaveForge:window_input_service()
	return self._input_blocked and FAKE_INPUT_SERVICE or self:input_service()
end

function HeroViewStateWeaveForge:_close_window_at_index(window_index)
...
function HeroViewStateWeaveForge:_change_window(window_index, window_name)
	local active_windows = self._active_windows
...
function HeroViewStateWeaveForge:get_layout_name()
...
function HeroViewStateWeaveForge:set_layout_by_name(name)
...
function HeroViewStateWeaveForge:close_on_exit()
	return self._close_on_exit
end

function HeroViewStateWeaveForge:set_layout(index, ignore_layout_history)
...
function HeroViewStateWeaveForge:set_window_input_focus(window_name)
...
function HeroViewStateWeaveForge:get_selected_layout_name()
	return self._selected_layout_name
end

function HeroViewStateWeaveForge:get_selected_layout_index()
	return self._selected_layout_index
end

function HeroViewStateWeaveForge:get_previous_selected_layout_index()
	return self._previous_selected_layout_index
end

function HeroViewStateWeaveForge:_get_layout_setting(index)
	return self._window_layouts [index]
end

function HeroViewStateWeaveForge:_windows_update(dt, t)
	local active_windows = self._active_windows
	for _, window in pairs(active_windows) do
		window:update(dt, t)
	end
...
function HeroViewStateWeaveForge:_windows_post_update(dt, t)
	local active_windows = self._active_windows
	for _, window in pairs(active_windows) do
...
function HeroViewStateWeaveForge:enable_widget(active_window_index, widget_name, enable)
	local active_windows = self._active_windows
...
function HeroViewStateWeaveForge:transitioning()
...
function HeroViewStateWeaveForge:_wanted_state()
	local new_state = self.parent:wanted_state()
	return new_state
end

function HeroViewStateWeaveForge:wanted_menu_state()
	return self._wanted_menu_state
end

function HeroViewStateWeaveForge:clear_wanted_menu_state()
	self._wanted_menu_state = nil
end

function HeroViewStateWeaveForge:requested_screen_change_by_name(state)
	self._on_close_next_state = state
	self:close_menu()
end

function HeroViewStateWeaveForge:on_exit(params)
...
function HeroViewStateWeaveForge:_close_active_windows()
	local active_windows = self._active_windows
	local params = self._window_params
...
function HeroViewStateWeaveForge:_update_transition_timer(dt)
...
function HeroViewStateWeaveForge:input_service()
	return self.parent:input_service()
end

function HeroViewStateWeaveForge:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
	...
	if not self._transition_timer then
		if
		not transitioning and self:_has_active_level_vote() then
...
function HeroViewStateWeaveForge:is_friends_list_active()
	local friends_component_ui = self._friends_component_ui
...
function HeroViewStateWeaveForge:_handle_friend_joining()
	local friends_component_ui = self._friends_component_ui
	if friends_component_ui then
...
function HeroViewStateWeaveForge:_has_active_level_vote()
...
function HeroViewStateWeaveForge:post_update(dt, t)
...
function HeroViewStateWeaveForge:_update_animations(dt)
...
function HeroViewStateWeaveForge:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_enter
end

function HeroViewStateWeaveForge:_handle_input(dt, t)
...
function HeroViewStateWeaveForge:close_menu(ignore_sound_on_close_menu)
...
function HeroViewStateWeaveForge:draw_gamepad_cursor(input_service, dt)
...
function HeroViewStateWeaveForge:draw(input_service, dt)
...
function HeroViewStateWeaveForge:_is_button_pressed(widget)
	local content = widget.content
...
function HeroViewStateWeaveForge:play_sound(event)
	self.parent:play_sound(event)
end

function HeroViewStateWeaveForge:_start_transition_animation(key, animation_name)
	local params = {
		wwise_world = self.wwise_world,
...
function HeroViewStateWeaveForge:set_fullscreen_effect_enable_state(enabled)
...
function HeroViewStateWeaveForge:block_input(start_loading_icon)
...
function HeroViewStateWeaveForge:unblock_input()
...
function HeroViewStateWeaveForge:input_blocked()
