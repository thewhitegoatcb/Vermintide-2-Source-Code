...
function DeusSwapWeaponInteractionUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function DeusSwapWeaponInteractionUI:destroy()
	Managers.state.event:unregister("chest_unlock_failed", self)
end

function DeusSwapWeaponInteractionUI:chest_unlock_failed(chest_type)
	if chest_type == DeusSwapWeaponInteractionUI.TYPE then
		self:_start_animation("chest_unlock_failed")
	end
end

function DeusSwapWeaponInteractionUI:_create_ui_elements()
...
function DeusSwapWeaponInteractionUI:_evaluate_interactable(player_unit)
...
function DeusSwapWeaponInteractionUI:_start_animation(animation_name)
	self._render_settings = self._render_settings or { alpha_multiplier = 0 }
	local params = {
...
function DeusSwapWeaponInteractionUI:_populate_widget(interactable_unit, wielded_slot_name)
	local mechanism = Managers.mechanism:game_mechanism()
	...
	chest_info_widget.style.cost_text.text_color = cost <= soft_currency_amount and { 255, 255, 255, 255 } or { 255, 255, 0, 0 }
...
function DeusSwapWeaponInteractionUI:update(player_unit, dt, t)
...
function DeusSwapWeaponInteractionUI:_update_offset(dt, t)
...
function DeusSwapWeaponInteractionUI:_update_animations(dt, t)
...
function DeusSwapWeaponInteractionUI:_draw(dt, t)
