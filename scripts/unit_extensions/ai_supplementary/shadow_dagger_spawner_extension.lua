...
local function spawn_dagger(spawner_unit, from_position, direction)
	...
	local extension_init_data = {
		projectile_locomotion_system = { rotate_around_forward = true, rotation_speed = 10,
			angle = angle,
			speed = speed,
...
local function check_if_can_spawn(from, to, physics_world, collision_filter)
	...
	local max_hits = 1
	local result = PhysicsWorld.linear_sphere_sweep(
	physics_world,
	from,
	to,
	radius,
...
function ShadowDaggerSpawnerExtension:init(extension_init_context, unit, extension_init_data)
...
function ShadowDaggerSpawnerExtension:destroy()
	return end

function ShadowDaggerSpawnerExtension:on_remove_extension(unit, extension_name)
	return end

function ShadowDaggerSpawnerExtension:update(unit, input, dt, context, t)
	if self._done then
		return
	end
