...
local extensions = { "PlayerProximityExtension", "AIProximityExtension" }

function ProximitySystem:init(context, system_name)
...
function ProximitySystem:on_spectator_target_changed(spectated_player_unit)
	self._spectated_player_unit = spectated_player_unit
	self._spectated_player = Managers.player:owner(spectated_player_unit)
	self._is_spectator = true
end

function ProximitySystem:on_add_extension(world, unit, extension_name, extension_init_data)
	local extension = { last_num_friends_nearby = 0, last_num_enemies_nearby = 0,
	...
	if extension_name == "PlayerProximityExtension" then
		...
		extension.proximity_types = {
			friends_close = { num = 0, distance = DialogueSettings.friends_close_distance, check = self.player_unit_extensions_map, broadphase = self.player_units_broadphase },
			friends_distant = { num = 0, distance = DialogueSettings.friends_distant_distance, check = self.player_unit_extensions_map, broadphase = self.player_units_broadphase },
			enemies_close = { num = 0, distance = DialogueSettings.enemies_close_distance, check = self.ai_unit_extensions_map, broadphase = self.enemy_broadphase },
			enemies_distant = { num = 0, distance = DialogueSettings.enemies_distant_distance, check = self.ai_unit_extensions_map, broadphase = self.enemy_broadphase } }
...
function ProximitySystem:extensions_ready(world, unit, extension_name)
	if extension_name == "PlayerProximityExtension" then
...
function ProximitySystem:on_remove_extension(unit, extension_name)
	self.frozen_unit_extension_data [unit] = nil
	self:_cleanup_extension(unit, extension_name)
	ScriptUnit.remove_extension(unit, self.NAME)
end

function ProximitySystem:on_freeze_extension(unit, extension_name)
...
function ProximitySystem:_cleanup_extension(unit, extension_name)
	local extension = self.unit_extension_data [unit]
...
function ProximitySystem:freeze(unit, extension_name, reason)
...
function ProximitySystem:unfreeze(unit, extension_name)
...
local function check_raycast_center(physics_world, unit, target)
	...
	local hits = PhysicsWorld.immediate_raycast(
	physics_world,
	ray_position,
	ray_direction,
	ray_length,
	"all",
	"types", "both",
	"collision_filter", "filter_lookat_object_ray")
	if hits then
...
local near_lookup = { witch_hunter = "witch_hunter_is_near", empire_soldier = "empire_soldier_is_near", dwarf_ranger = "dwarf_ranger_is_near", empire_soldier_tutorial = "empire_soldier_is_near", wood_elf = "wood_elf_is_near", bright_wizard = "bright_wizard_is_near" }
...
local nearby_units = { }
function ProximitySystem:update(context, t)
	local player_unit_extensions_map = self.player_unit_extensions_map local player_units_broadphase = self.player_units_broadphase
...
function ProximitySystem:physics_async_update(context, t)
	...
	for unit, extension in pairs(player_unit_extensions_map) do repeat
			local position = POSITION_LOOKUP [unit]


			if not position then break
			end
			...
			unit_forwards [unit] = nil until true
	end
...
function ProximitySystem:_update_nearby_boss()
...
function ProximitySystem:_update_nearby_enemies()
	if DEDICATED_SERVER then
		return
	end
	...
	local local_players = self._is_spectator and { self._spectated_player } or Managers.player:players_at_peer(Network.peer_id())
...
function ProximitySystem:_nearby_enemies_debug(list, new_nearby, new_enabled_fx)
...
function ProximitySystem:_clear_old_enabled_fx(old_enabled_fx)
...
function ProximitySystem:post_update(context, t)
	local POSITION_LOOKUP = POSITION_LOOKUP
	local enemy_check_raycasts = self.enemy_check_raycasts
...
function ProximitySystem:hot_join_sync(sender)
	return end