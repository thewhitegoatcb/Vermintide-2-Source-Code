...
local node_type_list = { "SIGNATURE", "ARENA", "TRAVEL", "DUMMY", "SHOP", "START" }
...
local CONNECTION_TYPES = { FINAL = "FINAL", NEW = "NEW", EXISTING = "EXISTING" }
...
local function get_random_node_type_list(random_generator)
	...
	for ii = #node_type_list_copy, 2, -1 do
		...
		node_type_list_copy [ii] = node_type_list_copy [swap] node_type_list_copy [swap] = node_type_list_copy [ii]
	end
...
local function get_random_key_list(table1, random_generator)
	...
	for ii = #keys, 2, -1 do
		local swap = random_generator(1, ii)
		keys [ii] = keys [swap] keys [swap] = keys [ii]
	end
...
local function shuffle_array(table, random_generator)
	...
	for ii = #table, 2, -1 do
		local swap = random_generator(1, ii)
		table [ii] = table [swap] table [swap] = table [ii]
	end
...
local function get_visible_nodes(nodes, node_key, depth)
	...
	if depth > 1 then
		...
		do return all_visible_nodes end
	else
		local all_visible_nodes = { }
		for _, descendant in ipairs(descendants) do
			all_visible_nodes [descendant] = nodes [descendant]
...
function create_connection_to_existing_action(context, nodes, node_key, to_node_key)
	...
	local function executor()
		...
		if validate_node_type(context.config, context.indent, nodes, to_node_key, to_node.type) and
		validate_until_the_end(context.config, context.indent, nodes, to_node_key) then
...
function create_connect_with_type_action(context, nodes, node_key, connection_type)
	...
	local function executor()
		...
		if connection_type == CONNECTION_TYPES.NEW then
			...
			do return true, next_actions end
		elseif connection_type == CONNECTION_TYPES.EXISTING then
			...
			while #possible_connections > 0 do
				...
				possible_connection ~= "final" and validate_connection_to_existing_node(context.config, context.indent, nodes, node_key, possible_connection) then
...
function create_connect_action(context, nodes, node_key)
	...
	local function executor()
		...
		while #random_connection_count > 0 do
			...
			if ( not last_attempt or new_connection_count < last_attempt ) and
			validate_connection_count(context.config, context.indent, nodes, node_key, new_connection_count) then
...
function deus_base_graph_generator(seed, config)
	...
	local nodes = {
		start = { layout_x = 1, name = "start", layout_y = 1,
