...
function EnvironmentBlendVolume:init(data)
...
function EnvironmentBlendVolume:particle_light_intensity()
	return self._data.particle_light_intensity
end

function EnvironmentBlendVolume:event_force_blend_environment_volume()
	if self._enabled then
		self._force_blend = true
	end
end

function EnvironmentBlendVolume:event_enable_environment_volume(volume_name, enable)
	if self._volume_name == volume_name then
		self._enabled = enable
	end
end

function EnvironmentBlendVolume:environment()
	return self._environment
end

function EnvironmentBlendVolume:level_key()
	return self._level_key
end

function EnvironmentBlendVolume:value()
	return self._value
end

function EnvironmentBlendVolume:is_inside()
	return self._is_inside
end

function EnvironmentBlendVolume:override_settings()
	return self._override_values
end

function EnvironmentBlendVolume:update(dt)
...
function EnvironmentBlendVolume:destroy()
