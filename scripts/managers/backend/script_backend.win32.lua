...
local LOG_LEVELS = { off = 0, verbose = 2, normal = 1 }
...
local function print_result(result, action)
	if result and result.reason ~= Backend.ERR_OK then
		...
		return { reason = result.reason, details = result.details }
	end
end

function ScriptBackend:update()
...
function ScriptBackend:_update_state()
	...
	if state ~= self._state and not allowed_changes [state] then
		...
		result = { reason = reason, details = error_message }
	end
...
function ScriptBackend:update_state()
	return self:_update_state()
end

function ScriptBackend:update_signin()
...
function ScriptBackend:authenticated()
	return Backend.state() == Backend.CONNECTION_ENTITIES_LOADED
end

function ScriptBackend:_refresh_stats()
...
function ScriptBackend:get_stats()
	self:_refresh_stats()
	return self._nice_stats
end

function ScriptBackend:set_stats(nice_stats)
	self:_refresh_stats()
...
function ScriptBackend:check_for_errors()
	...
	if self._commit_error then
		commit_error = { reason = Backend.ERR_COMMIT }
		self._commit_error = nil
	end
	return backend_error or session_error or commit_error
end

function ScriptBackend:_new_id()
	self._last_id = self._last_id + 1
	return self._last_id
end

function ScriptBackend:_check_current_commit()
...
function ScriptBackend:_commit_internal(queued_id)
...
function ScriptBackend:_queue_commit()
...
function ScriptBackend:commit(internal)
...
function ScriptBackend:commit_status(commit_id)
	...
	if commit_data.result ~= Backend.COMMIT_WAITING then
		do return commit_data.result end
...
function ScriptBackend:destroy()
	print("[Backend] ScriptBackend destroy")
	Backend.destroy()
end

function ScriptBackend:backend_object()
	error("no backend object in lua anymore")
	return self._backend
end

function ScriptBackend:refresh_log_level()
...
function ScriptBackend:wait_for_shutdown(timeout)
