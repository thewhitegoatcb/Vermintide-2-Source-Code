...
function HeroViewStateKeepDecorations:on_enter(params)
	...
	if player then
		...
		local params = { camera_interaction_name = camera_interaction_name }
...
function HeroViewStateKeepDecorations:_disable_list_widgets()
...
function HeroViewStateKeepDecorations:_initialize_simple_decoration_preview()
...
function HeroViewStateKeepDecorations:on_exit(params)
...
function HeroViewStateKeepDecorations:_create_ui_elements(params)
...
function HeroViewStateKeepDecorations:_set_color_alpha_intensity(color, fraction)
	self:_set_color_values(color, color [1] * fraction)
end

function HeroViewStateKeepDecorations:_set_color_intensity(color, fraction)
	self:_set_color_values(color, nil, color [2] * fraction, color [3] * fraction, color [4] * fraction)
end

function HeroViewStateKeepDecorations:_set_color_values(color, p1, p2, p3, p4)
...
function HeroViewStateKeepDecorations:transitioning()
...
function HeroViewStateKeepDecorations:_wanted_state()
	local new_state = self.parent:wanted_state()
	return new_state
end

function HeroViewStateKeepDecorations:wanted_menu_state()
	return self._wanted_menu_state
end

function HeroViewStateKeepDecorations:clear_wanted_menu_state()
	self._wanted_menu_state = nil
end

function HeroViewStateKeepDecorations:_update_transition_timer(dt)
...
function HeroViewStateKeepDecorations:input_service()
	return self.parent:input_service()
end

function HeroViewStateKeepDecorations:_is_list_hovered()
	local widget = self._widgets_by_name.list_mask
	return widget.content.hotspot.is_hover or false
end

function HeroViewStateKeepDecorations:update(dt, t)
...
function HeroViewStateKeepDecorations:_update_client_paintings(dt)
...
function HeroViewStateKeepDecorations:_has_active_level_vote()
...
function HeroViewStateKeepDecorations:post_update(dt, t)
	self.ui_animator:update(dt)
	self:_update_animations(dt)
end

function HeroViewStateKeepDecorations:_update_animations(dt)
...
function HeroViewStateKeepDecorations:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot or content.hotspot
	return hotspot.on_hover_enter
end

function HeroViewStateKeepDecorations:_is_button_hover_exit(widget)
	local content = widget.content
	local hotspot = content.button_hotspot or content.hotspot
	return hotspot.on_hover_exit
end

function HeroViewStateKeepDecorations:_is_button_hover(widget)
	local content = widget.content
	local hotspot = content.button_hotspot or content.hotspot
	return hotspot.is_hover
end

function HeroViewStateKeepDecorations:_handle_input(dt, t)
	local input_service = self._input_blocked and FAKE_INPUT_SERVICE or self:input_service()
...
function HeroViewStateKeepDecorations:_verify_decoration_selection()
...
function HeroViewStateKeepDecorations:close_menu(ignore_sound_on_close_menu)
	ignore_sound_on_close_menu = true
	self.parent:close_menu(nil, ignore_sound_on_close_menu)
end

function HeroViewStateKeepDecorations:draw(input_service, dt)
...
function HeroViewStateKeepDecorations:_is_button_pressed(widget)
	local content = widget.content
	local hotspot = content.button_hotspot or content.hotspot
...
function HeroViewStateKeepDecorations:_play_sound(event)
	self.parent:play_sound(event)
end

function HeroViewStateKeepDecorations:_start_transition_animation(key, animation_name)
	local params = {
...
function HeroViewStateKeepDecorations:set_fullscreen_effect_enable_state(enabled)
...
function HeroViewStateKeepDecorations:block_input()
	self._input_blocked = true
end

function HeroViewStateKeepDecorations:unblock_input()
	self._input_blocked = false
end

function HeroViewStateKeepDecorations:input_blocked()
	return self._input_blocked
end

function HeroViewStateKeepDecorations:_set_info_by_decoration_key(key, locked)
...
function HeroViewStateKeepDecorations:_update_sound_trigger_delay(dt)
...
function HeroViewStateKeepDecorations:_update_confirm_button()
...
function HeroViewStateKeepDecorations:_on_list_index_selected(index, scrollbar_animation_percentage)
	local interactable_unit = self._interactable_unit
	local keep_decoration_extension = ScriptUnit.extension(interactable_unit, "keep_decoration_system")
	local equipped_decoration = keep_decoration_extension:get_selected_decoration()
...
function HeroViewStateKeepDecorations:_update_scrollbar_progress_animation(dt, t)
...
function HeroViewStateKeepDecorations:_set_info_texts(title_text, description_text, artist_text)
...
function HeroViewStateKeepDecorations:_set_selected_title(title_text)
...
function HeroViewStateKeepDecorations:_set_selected_description(description_text)
...
function HeroViewStateKeepDecorations:_set_selected_artist(artist_text)
...
function HeroViewStateKeepDecorations:_update_equipped_widget()
	local interactable_unit = self._interactable_unit
	local keep_decoration_extension = ScriptUnit.extension(interactable_unit, "keep_decoration_system")
	local equipped_decoration = keep_decoration_extension:get_selected_decoration()
...
function HeroViewStateKeepDecorations:_align_list_widgets()
...
function HeroViewStateKeepDecorations:_handle_gamepad_list_selection(input_service)
...
function HeroViewStateKeepDecorations:_find_closest_neighbour(column_index_list, current_index)
	local list_widgets = self._list_widgets
...
function HeroViewStateKeepDecorations:_initialize_scrollbar()
...
function HeroViewStateKeepDecorations:_update_scroll_position()
	local scrollbar_logic = self._scrollbar_logic
...
function HeroViewStateKeepDecorations:_update_visible_list_entries()
	local scrollbar_logic = self._scrollbar_logic
...
function HeroViewStateKeepDecorations:_get_scrollbar_percentage_by_index(index)
	local scrollbar_logic = self._scrollbar_logic
	local enabled = scrollbar_logic:enabled()
...
function HeroViewStateKeepDecorations:_list_index_pressed()
	local list_widgets = self._list_widgets
	if list_widgets then
...
function HeroViewStateKeepDecorations:_setup_decorations_list()
...
function HeroViewStateKeepDecorations:_animate_list_entries(dt, is_list_hovered)
	local widgets = self._list_widgets
	if not widgets then
		return
	end
...
function HeroViewStateKeepDecorations:_animate_list_widget(widget, dt, optional_hover)
	local offset = widget.offset
	local content = widget.content
	local style = widget.style
	local hotspot = content.button_hotspot or content.hotspot
...
function HeroViewStateKeepDecorations:_handle_gamepad_activity()
	local mouse_active = Managers.input:is_device_active("mouse")
