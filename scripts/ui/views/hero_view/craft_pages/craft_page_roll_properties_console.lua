...
local definitions = local_require("scripts/ui/views/hero_view/craft_pages/definitions/craft_page_roll_properties_console_definitions")
...
function CraftPageRollPropertiesConsole:on_enter(params, settings)
...
function CraftPageRollPropertiesConsole:setup_recipe_requirements()
...
function CraftPageRollPropertiesConsole:reset_requirements(num_required_ingredients)
...
function CraftPageRollPropertiesConsole:create_ui_elements(params)
...
function CraftPageRollPropertiesConsole:on_exit(params)
...
function CraftPageRollPropertiesConsole:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function CraftPageRollPropertiesConsole:post_update(dt, t)

	return end

function CraftPageRollPropertiesConsole:_update_animations(dt)
...
function CraftPageRollPropertiesConsole:_is_button_pressed(widget)
...
function CraftPageRollPropertiesConsole:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageRollPropertiesConsole:_is_button_held(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageRollPropertiesConsole:_handle_input(dt, t)
...
function CraftPageRollPropertiesConsole:_handle_craft_input_progress(progress)
	return self.parent:_set_input_progress(progress)
end

function CraftPageRollPropertiesConsole:craft_result(result, error, reset_slots)
	if not error then
		self._craft_result = result
	end
end

function CraftPageRollPropertiesConsole:reset()
...
function CraftPageRollPropertiesConsole:present_results()
	local item_grid = self._item_grid
	item_grid:clear_locked_items()
	item_grid:update_items_status()
...
function CraftPageRollPropertiesConsole:on_craft_completed()
...
function CraftPageRollPropertiesConsole:_update_craft_items()
	local super_parent = self.super_parent
	local item_grid = self._item_grid
...
function CraftPageRollPropertiesConsole:_remove_craft_item(backend_id, slot_index, ignore_sound)
	local craft_items = self._craft_items
...
function CraftPageRollPropertiesConsole:_add_craft_item(backend_id, slot_index, ignore_sound)
...
function CraftPageRollPropertiesConsole:_set_craft_button_disabled(disabled)
...
function CraftPageRollPropertiesConsole:_exit(selected_level)
	self.exit = true
	self.exit_level_id = selected_level
end

function CraftPageRollPropertiesConsole:draw(dt)
...
function CraftPageRollPropertiesConsole:_play_sound(event)
	self.super_parent:play_sound(event)
end

function CraftPageRollPropertiesConsole:_set_craft_button_text(text, localize)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.craft_button
	widget.content.button_text = localize and Localize(text) or text
end

function CraftPageRollPropertiesConsole:_add_crafting_material_requirement(index, item_key, amount_text, has_required_amount)
...
function CraftPageRollPropertiesConsole:_clear_item_grid()
	local craft_items = self._craft_items
	local super_parent = self.super_parent
...
function CraftPageRollPropertiesConsole:_has_added_item_by_id(backend_id)
	local craft_items = self._craft_items
