...
function PassiveAbilityWarriorPriest:init(extension_init_context, unit, extension_init_data, ability_init_data)
...
function PassiveAbilityWarriorPriest:extensions_ready(world, unit)
...
function PassiveAbilityWarriorPriest:destroy()
	self:_unregister_events()
end

function PassiveAbilityWarriorPriest:_register_events()
...
function PassiveAbilityWarriorPriest:_unregister_events()
...
function PassiveAbilityWarriorPriest:update(dt, t)
...
function PassiveAbilityWarriorPriest:on_player_killed_enemy(killing_blow, breed_killed, ai_unit)
...
function PassiveAbilityWarriorPriest:on_hit(hit_unit, attack_type, hit_zone_name, target_number, buff_type, is_critical, unmodified, unit)
...
function PassiveAbilityWarriorPriest:buff_on_damage_taken(attacker_unit, damage_amount, damage_type)
	self:modify_resource(damage_amount)
end


function PassiveAbilityWarriorPriest:modify_resource(amount, ignore_difficulty)
	...
	if amount > 0 then
		...
		if difficulty then
			...
			local difficulty_tweak = { 1, 1.5, 1.2, 1, 1, 1, 1, 0.7 }
...
function PassiveAbilityWarriorPriest:modify_resource_percent(amount_percent)
	local amount = self._max_resource * amount_percent
	return self:modify_resource(amount, true)
end

function PassiveAbilityWarriorPriest:get_resource_fraction()
	local current_resource = self._current_resource
	local max_resource = self._max_resource
	return current_resource / max_resource
end

function PassiveAbilityWarriorPriest:is_active()
	return self._is_active
end

function PassiveAbilityWarriorPriest:degenerate_resource(dt)
	return self:modify_resource(-resouce_degen_rate * dt)
end

function PassiveAbilityWarriorPriest:set_in_combat()
	self._not_in_combat = false
	local t = Managers.time:time("game")
	self._combat_timer = t + self._time_to_ooc
end

function PassiveAbilityWarriorPriest:combat_timer_update(t)
...
function PassiveAbilityWarriorPriest:activate_buff()
...
function PassiveAbilityWarriorPriest:deactivate_buff()
...
function PassiveAbilityWarriorPriest:_play_vo()
	local owner_unit = self._owner_unit
...
function PassiveAbilityWarriorPriest:_set_fury_glow_enabled(enabled)
	...
	if equipment then
		local left_weapon_3p = equipment.left_hand_wielded_unit_3p local right_weapon_3p = equipment.right_hand_wielded_unit_3p
...
function PassiveAbilityWarriorPriest:on_weapon_wield(equipment)
	self:_set_fury_glow_enabled(self._is_active)
end

function PassiveAbilityWarriorPriest:on_level_start_local_player_spawned(initial_spawn)
	if self._is_local_player and not self._game_object_id then
		self:create_game_object()
	end
end

function PassiveAbilityWarriorPriest:on_talents_changed(unit, talent_extension)
...
function PassiveAbilityWarriorPriest:create_game_object()
...
function PassiveAbilityWarriorPriest:set_career_game_object_id(go_id)
	self._game_object_id = go_id
end

function PassiveAbilityWarriorPriest:cb_game_session_disconnect()
