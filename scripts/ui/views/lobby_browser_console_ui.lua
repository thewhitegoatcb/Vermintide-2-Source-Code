...
local GAME_TYPE_LOOKUP_STRINGS = { deus = "area_selection_morris_name", adventure = "area_selection_campaign", weave = "menu_weave_area_no_wom_title", versus = "vs_ui_versus_tag", any = "lobby_browser_mission" }
...
function LobbyBrowserConsoleUI:init(parent, ingame_ui_context, game_mode_data, show_lobby_data_table, distance_data_table)
...
function LobbyBrowserConsoleUI:_start_transition_animation(animation_name)
	local params = {
...
function LobbyBrowserConsoleUI:_create_ui_elements()
...
function LobbyBrowserConsoleUI:_create_filters()
	...
	self._filter_functions = { { input_function = "_handle_game_type_filter_input", render_function = "_render_game_type_filter_list", input_function_mouse = "_handle_game_type_filter_input_mouse" },




		{ input_function = "_handle_level_filter_input", render_function = "_render_level_filter_list", input_function_mouse = "_handle_level_filter_input_mouse" },




		{ input_function = "_handle_difficulty_filter_input", render_function = "_render_difficulty_filter_list", input_function_mouse = "_handle_difficulty_filter_input_mouse" },




		{ input_function = "_handle_lobby_filter_input", render_function = "_render_lobby_filter_list", input_function_mouse = "_handle_lobby_filter_input_mouse" },




		{ input_function = "_handle_distance_filter_input", render_function = "_render_distance_filter_list", input_function_mouse = "_handle_distance_filter_input_mouse" } }
...
function LobbyBrowserConsoleUI:setup_filter_entries()
...
function LobbyBrowserConsoleUI:update(dt, t, loading)
...
function LobbyBrowserConsoleUI:_update_info_text(dt, t, loading)
...
function LobbyBrowserConsoleUI:_update_animations(dt, t)
...
function LobbyBrowserConsoleUI:_remove_invalid_lobbies(lobbies)
...
function LobbyBrowserConsoleUI:populate_lobby_list(lobbies, ignore_scroll_reset)
...
function LobbyBrowserConsoleUI:_handle_input_description(dt, t)
...
function LobbyBrowserConsoleUI:_handle_input(dt, t, loading)
...
function LobbyBrowserConsoleUI:_handle_mouse_input(dt, t, loading)
	local gamepad_active = Managers.input:is_device_active("gamepad")
...
function LobbyBrowserConsoleUI:_verify_selected_lobby_index()
	...
	if old_selected_lobby_index ~= self._selected_lobby_index then
		...
		if self:_is_nan(progress) then progress = 0 end
...
function LobbyBrowserConsoleUI:_handle_browser_input(input_service, element_settings, dt, t)
...
function LobbyBrowserConsoleUI:_handle_filter_input_mouse(input_service, element_settings, dt, t)

	return end

function LobbyBrowserConsoleUI:_handle_browser_input_mouse(input_service, element_settings, dt, t)
...
function LobbyBrowserConsoleUI:_is_nan(value)
	return type(value) == "number" and value ~= value
end

function LobbyBrowserConsoleUI:_handle_filter_input(input_service, element_settings, dt, t)
	local gamepad_active = Managers.input:is_device_active("gamepad")
...
function LobbyBrowserConsoleUI:_update_filter_index(index_change)
	local frame_widget = self._widgets.filter_frame
	local frame_widget_content = frame_widget.content
...
function LobbyBrowserConsoleUI:_handle_game_type_filter_input(input_service, element_settings, dt, t)
...
function LobbyBrowserConsoleUI:_handle_game_type_filter_input_mouse(input_service, element_settings, dt, t)
...
function LobbyBrowserConsoleUI:_handle_level_filter_input(input_service, element_settings, dt, t)
	local hold_down_timer = 0
	local hold_up_timer = 0
	local old_selected_list_index = self._filter_list_index
	self._filter_list_index = self._filter_list_index or 1
...
function LobbyBrowserConsoleUI:_handle_level_filter_input_mouse(input_service, element_settings, dt, t)
	local length = element_settings.window_height + element_settings.filter_height
	local num_visible_entries = math.ceil(length / (element_settings.filter_height + element_settings.spacing))
	local entry_size_y = element_settings.filter_height + element_settings.spacing
	self._list_base_pos_y = self._list_base_pos_y or scenegraph_definition.filter_level_entry_anchor.position [2]
...
function LobbyBrowserConsoleUI:_handle_difficulty_filter_input(input_service, element_settings, dt, t)
	local old_selected_list_index = self._filter_list_index
	self._filter_list_index = self._filter_list_index or 1
	local hold_down_timer = 0
	local hold_up_timer = 0
...
function LobbyBrowserConsoleUI:_handle_difficulty_filter_input_mouse(input_service, element_settings, dt, t)
	local length = element_settings.window_height + element_settings.filter_height
	local num_visible_entries = math.ceil(length / (element_settings.filter_height + element_settings.spacing))
	local entry_size_y = element_settings.filter_height + element_settings.spacing
...
function LobbyBrowserConsoleUI:_handle_lobby_filter_input(input_service, element_settings, dt, t)
	local old_selected_list_index = self._filter_list_index
	self._filter_list_index = self._filter_list_index or 1
	local hold_down_timer = 0
	local hold_up_timer = 0
...
function LobbyBrowserConsoleUI:_handle_lobby_filter_input_mouse(input_service, element_settings, dt, t)
	local length = element_settings.window_height + element_settings.filter_height
	local num_visible_entries = math.ceil(length / (element_settings.filter_height + element_settings.spacing))
	local entry_size_y = element_settings.filter_height + element_settings.spacing
...
function LobbyBrowserConsoleUI:_handle_distance_filter_input(input_service, element_settings, dt, t)
	local old_selected_list_index = self._filter_list_index
	self._filter_list_index = self._filter_list_index or 1
	local hold_down_timer = 0
	local hold_up_timer = 0
...
function LobbyBrowserConsoleUI:_handle_distance_filter_input_mouse(input_service, element_settings, dt, t)
	local length = element_settings.window_height + element_settings.filter_height
	local num_visible_entries = math.ceil(length / (element_settings.filter_height + element_settings.spacing))
	local entry_size_y = element_settings.filter_height + element_settings.spacing
...
function LobbyBrowserConsoleUI:_select_lobby(old_selected_lobby_index, new_selected_lobby_index, device_selected_lobby_index)
	self._details_filled = false
...
function LobbyBrowserConsoleUI:_update_lobby_data(dt, t)
...
function LobbyBrowserConsoleUI:_update_lobby_details(valid_lobbies_by_id)
...
function LobbyBrowserConsoleUI:_update_lobby_list(valid_current_lobbies_by_id, dt, t)
...
function LobbyBrowserConsoleUI:_update_lobby_entry(index, offset_y, selected, lobby_entry_lobby_data, valid_current_lobbies_by_id, dt, t)
...
function LobbyBrowserConsoleUI:_fill_details(lobby_data)
	...
	if lobby_data then
		local matchmaking_type_lookup = { event = "lb_game_type_event", deed = "lb_game_type_deed", tutorial = "lb_game_type_prologue", weave = "lb_game_type_weave", custom = "lb_game_type_custom", standard = "lb_game_type_quick_play", weave_quick_play = "lb_game_type_weave_quick_play", ["n/a"] = "lb_game_type_none", deus = "area_selection_morris_name" }
...
function LobbyBrowserConsoleUI:_fill_weave_details(lobby_data)
	...
	wind_icon_style.texture_size = { wind_thumbnail_icon_size [1] * 0.6, wind_thumbnail_icon_size [2] * 0.6 }
...
function LobbyBrowserConsoleUI:_gather_unlocked_journeys()
...
function LobbyBrowserConsoleUI:_fill_deus_details(lobby_data)
...
function LobbyBrowserConsoleUI:_assign_objective(index, text, icon, spacing)
...
function LobbyBrowserConsoleUI:set_game_type_filter(game_type_name)
	local widget = self._widgets.filter_frame
	local content = widget.content
	content.game_type_name = game_type_name
end

function LobbyBrowserConsoleUI:set_level_filter(level_display_name)
	local widget = self._widgets.filter_frame
	local content = widget.content
	content.mission_name = level_display_name
end

function LobbyBrowserConsoleUI:set_difficulty_filter(difficulty_display_name)
	local widget = self._widgets.filter_frame
	local content = widget.content
	content.difficulty_name = difficulty_display_name
end

function LobbyBrowserConsoleUI:set_show_lobbies_filter(show_lobbies_text)
	local widget = self._widgets.filter_frame
	local content = widget.content
	content.show_lobbies_name = show_lobbies_text
end

function LobbyBrowserConsoleUI:set_distance_filter(distance_text)
	local widget = self._widgets.filter_frame
	local content = widget.content
	content.distance_name = distance_text
end

function LobbyBrowserConsoleUI:_draw(dt, t)
...
function LobbyBrowserConsoleUI:_render_filter(ui_renderer, ui_scenegraph, input_service, dt, t)
	if self._current_active_filter then
...
function LobbyBrowserConsoleUI:_render_game_type_filter_list(ui_renderer, ui_scenegraph, input_service, dt, t)
	for _, widget in ipairs(self._game_type_filter_widgets) do
		UIRenderer.draw_widget(ui_renderer, widget)
	end
end

function LobbyBrowserConsoleUI:_render_level_filter_list(ui_renderer, ui_scenegraph, input_service, dt, t)
...
function LobbyBrowserConsoleUI:_render_difficulty_filter_list(ui_renderer, ui_scenegraph, input_service, dt, t)
	for _, widget in ipairs(self._difficulty_filter_widgets) do
		UIRenderer.draw_widget(ui_renderer, widget)
	end
end

function LobbyBrowserConsoleUI:_render_lobby_filter_list(ui_renderer, ui_scenegraph, input_service, dt, t)
	for _, widget in ipairs(self._lobby_filter_widgets) do
		UIRenderer.draw_widget(ui_renderer, widget)
	end
end

function LobbyBrowserConsoleUI:_render_distance_filter_list(ui_renderer, ui_scenegraph, input_service, dt, t)
	for _, widget in ipairs(self._distance_filter_widgets) do
		UIRenderer.draw_widget(ui_renderer, widget)
	end
end

function LobbyBrowserConsoleUI:_render_lobby_browser(ui_renderer, ui_scenegraph, input_service, dt, t)
	local element_settings = definitions.element_settings
...
function LobbyBrowserConsoleUI:_is_inside(widget, element_settings, idx)
...
function LobbyBrowserConsoleUI:destroy()
