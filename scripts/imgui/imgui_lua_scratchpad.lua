...
local fallback_color = { 199, 206, 234, 255 }
ImguiLuaScratchpad._TYPE_TO_COLOR = setmetatable({
	["function"] = { 181, 234, 215, 255 },
	string = { 226, 240, 203, 255 },
	number = { 255, 218, 193, 255 },
	boolean = { 255, 183, 178, 255 },
	userdata = { 255, 154, 162, 255 },
	table = { 255, 247, 154, 255 } },
{
	__index = function () return fallback_color end })


local has_util, util = pcall(require, "jit.util")
local magic_mt = { __mode = "kv",
...
function ImguiLuaScratchpad:init()
	self._expr = ""
	self._val = nil self._error = nil self._thunk = nil
	self._func_info_magic = setmetatable({ }, magic_mt)
	self._is_persistent = false
	self._sort_keys = false
end

function ImguiLuaScratchpad:update() return end

function ImguiLuaScratchpad:draw()
	...
	Imgui.same_line()
	if ( self._exec_every_frame or Imgui.button("Execute") ) and
	self:_load_expression() then
		self:_execute_thunk()
	end
...
function ImguiLuaScratchpad:_inspect_pair(k, v)
	k = tostring(k)
	local t = type(v)
	if t == "table" then
...
function ImguiLuaScratchpad:_inspect_function(name, func)
	...
	if has_util and is_open then
		...
		if upvals > 0 and
		Imgui.tree_node("[upvalues]", false) then
			for up = 1, upvals do
		...
		if has_util and info.nconsts and info.nconsts ~= 0 and info.gcconsts ~= 0 and
		Imgui.tree_node("[consts]", false) then
...
function ImguiLuaScratchpad:_inspect_table(name, tab)
	local is_open = Imgui.tree_node(name, false)
	...
	if is_open then
		if self._sort_keys then
			...
			for _, k in pairs(keys) do self:_inspect_pair(k, tab [k])
			end else
			for k, v in pairs(tab) do self:_inspect_pair(k, v)
			end
		end
		if mt then self:_inspect_table("[metatable]", mt)
		end
		Imgui.tree_pop()
	end
end

function ImguiLuaScratchpad:_load_expression()
	self._thunk, self._error = loadstring("return " .. self._expr, "Input")
	if not self._thunk then self._thunk, self._error = loadstring(self._expr, "Input") end
...
local function traceback_table(err)
	...
	for i = 2, 9999 do
		...
		stack [i - 1] = { name = info.name, info = info, slots = slots, ups = ups }
	end
	return { error = err or "?", stack = stack }
end

function ImguiLuaScratchpad:_execute_thunk()
	local ok, val = xpcall(self._thunk, traceback_table)
	if ok then self._error = nil self._val = val else self._error = "Runtime error" self._val = val end
end

function ImguiLuaScratchpad:is_persistent()
