...
function ItemTooltipHelper.format_return_string(format_type, values)
...
function ItemTooltipHelper.get_damage(unit, item, armor_type, primary_armor_type, damage_profile, power_level, difficulty_level)
...
function ItemTooltipHelper.get_stagger_strength(unit, item, damage_profile, power_level, difficulty_level)
	local breed = nil
	local hit_zone_name = "torso"
	local is_critical_strike = false
	local target_index = 1
...
function ItemTooltipHelper.get_next_action_names(action, charge_type)
...
function ItemTooltipHelper.get_action(unit, item, stat_descriptor)
...
function ItemTooltipHelper.get_chain_damages(values, action, unit, item, stat_descriptor)
...
function ItemTooltipHelper.get_chain_max_targets(values, action, unit, item, stat_descriptor)
	local impact_data = action.impact_data
	local career_extension = ScriptUnit.has_extension(unit, "career_system")
	local buff_extension = ScriptUnit.has_extension(unit, "buff_system")
	local power_level = career_extension:get_career_power_level()
	...
	if damage_profile_name then
		...
		elseif action.kind == "geiser" or action.kind == "shield_slam" or action.kind == "push_stagger" then
			values [#values + 1] = { value = -1, type = "single" }
		else
...
function ItemTooltipHelper.get_chain_stagger_strengths(values, action, unit, item, stat_descriptor)
	local impact_data = action.impact_data
	local career_extension = ScriptUnit.has_extension(unit, "career_system")
	local buff_extension = ScriptUnit.has_extension(unit, "buff_system")
	local power_level = career_extension:get_career_power_level()
...
function ItemTooltipHelper.get_chain_critical_hit_chances(values, action, unit, item, stat_descriptor)
	local impact_data = action.impact_data
...
local ranged_actions = { beam = true, crossbow = true, charged_projectile = true, shotgun = true, handgun = true, bow = true, bullet_spray = true, flamethrower = true }
...
function ItemTooltipHelper.get_time_between_damage(values, action, unit, item, stat_descriptor)
	local impact_data = action.impact_data
...
function ItemTooltipHelper.get_chain_boost_coefficients(values, action, unit, item, stat_descriptor)
	local impact_data = action.impact_data
...
function ItemTooltipHelper.get_chain_headshot_boost_coefficients(values, action, unit, item, stat_descriptor)
	local impact_data = action.impact_data
...
function ItemTooltipHelper.get_push_angles(values, action, unit, item, stat_descriptor)
	local item_data = item.data
	local item_template = BackendUtils.get_item_template(item_data)
	local actions = item_template.actions
	...
	if damage_profile_inner_name and damage_profile_outer_name then
		...
		values [#values + 1] = { type = "single", value = inner_push_angle }
		values [#values + 1] = { type = "single", value = outer_push_angle }
	end
end

function ItemTooltipHelper.get_push_strengths(values, action, unit, item, stat_descriptor)
	local career_extension = ScriptUnit.has_extension(unit, "career_system")
	local buff_extension = ScriptUnit.has_extension(unit, "buff_system")
	local power_level = career_extension:get_career_power_level()
	local difficulty_level = Managers.state.difficulty:get_difficulty()
	...
	if damage_profile_inner_name and damage_profile_outer_name then
		...
		values [#values + 1] = { type = "single", value = strength_inner }
		values [#values + 1] = { type = "single", value = strength_outer }
	end
end

local TRAVERSED_ACTION_PAIRS = { }
function ItemTooltipHelper.parse_weapon_chain(values, unit, item, stat_descriptor, chain_value_function)
	local item_data = item.data
	local item_template = BackendUtils.get_item_template(item_data)
	local actions = item_template.actions
	...
	if uses_custom_chain then
		...
		if slot_type == "ranged" then
			traversed_action_pairs [#traversed_action_pairs + 1] = { tooltip_detail.light.action_name, tooltip_detail.light.sub_action_name }
			traversed_action_pairs [#traversed_action_pairs + 1] = { tooltip_detail.heavy.action_name, tooltip_detail.heavy.sub_action_name }
		end

		local chain_complete = false
		while not chain_complete do
			traversed_action_pairs [#traversed_action_pairs + 1] = { current_action_name, current_sub_action_name }
			local action = actions [current_action_name] [current_sub_action_name]
