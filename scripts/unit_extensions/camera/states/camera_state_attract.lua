...
function CameraStateFollowAttract:init(camera_state_init_context)
	CameraState.init(self, camera_state_init_context, "attract")

	self._follow_unit = nil
	self._follow_node = 0
end

function CameraStateFollowAttract:on_enter(unit, input, dt, context, t, previous_state, params)
	...
	Unit.set_data(unit, "camera", "settings_node", "attract_mode")
end

function CameraStateFollowAttract:on_exit(unit, input, dt, context, t, next_state)
	self._follow_unit = nil
end

function CameraStateFollowAttract:refresh_follow_unit(follow_unit, follow_node)
	self._follow_unit = follow_unit
	self._follow_node = follow_node
end

function CameraStateFollowAttract:update(unit, input, dt, context, t)
	local csm = self.csm
