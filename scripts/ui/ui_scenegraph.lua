...
local Vector2 = Vector2 local Vector3 = Vector3
local RESOLUTION_LOOKUP = RESOLUTION_LOOKUP
local Application = Application
local fassert = fassert

local ZERO_VECTOR3 = { 0, 0, 0 }
local function to_vector2_table(t) return { t [1], t [2] } end
local function to_vector3_table(t) return { t [1], t [2], t [3] or 0 } end

UISceneGraph.ZERO_VECTOR3 = ZERO_VECTOR3

local ALIGN_KWORD_MULT = { left = 0, bottom = 0, top = 1, center = 0.5, right = 1 }
...
local NEWINDEX_ERR_MT = { __class_name = "scenegraph",
...
local function scenegraph_visit_node(scenegraph, scenegraph_def, name, node_def)
	...
	if size [1] < 0 then size [1] = size [1] + parent.size [1] end
	if size [2] < 0 then size [2] = size [2] + parent.size [2]
	end
...
function UISceneGraph.init_scenegraph(scenegraph_def)
	local scenegraph = { }
...
function UISceneGraph.update_scenegraph(scenegraph, parent_scenegraph, scenegraph_id)
...
function UISceneGraph.get_size(scenegraph, node_name)
...
function UISceneGraph.get_world_position(scenegraph, node_name)
	local node = scenegraph [node_name]
...
function UISceneGraph.get_local_position(scenegraph, node_name)
	local node = scenegraph [node_name]
...
function UISceneGraph.get_size_scaled(scenegraph, node_name, optional_scale)
	local node = scenegraph [node_name]
	...
	if node.is_root then
		...
		return Vector2(
		w * inverse_scale / 1920 * size [1],
		h * inverse_scale * UISettings.root_scale [2])
	end
...
function UISceneGraph.set_local_position(scenegraph, node_name, new_position)
	local node = scenegraph [node_name]
...
function UISceneGraph.debug_render_scenegraph(ui_renderer, scenegraph, force_draw_depth)
