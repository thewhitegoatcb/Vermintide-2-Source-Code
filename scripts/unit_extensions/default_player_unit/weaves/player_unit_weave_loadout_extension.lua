...
function PlayerUnitWeaveLoadoutExtension:init(extension_init_context, unit, extension_init_data)
	self._unit = unit
...
function PlayerUnitWeaveLoadoutExtension:destroy()
	self._unit = nil
	self._buffs = nil
	self._synced_buff_params = nil
...
function PlayerUnitWeaveLoadoutExtension:extensions_ready(world, unit)
...
function PlayerUnitWeaveLoadoutExtension:game_object_initialized(unit, unit_go_id)
	self:_sync_buffs(unit_go_id)
end

function PlayerUnitWeaveLoadoutExtension:hot_join_sync(sender)
...
function PlayerUnitWeaveLoadoutExtension:_is_in_weave()
...
function PlayerUnitWeaveLoadoutExtension:_get_weave_buffs()
	local buffs = {
		client = { },
		server = { },
		both = { } }
	...
	for property_key, property_upgrades in pairs(properties) do
		...
		buffs [buffer] [buff_name] = { variable_value = varable_value }
	end
	...
	for trait_key, _ in pairs(traits) do
		...
		buffs [buffer] [buff_name] = { variable_value = 1 }
	end

	return buffs
end

function PlayerUnitWeaveLoadoutExtension:_apply_buffs(buffs_by_buffer)
...
function PlayerUnitWeaveLoadoutExtension:_sync_buffs(unit_go_id)
