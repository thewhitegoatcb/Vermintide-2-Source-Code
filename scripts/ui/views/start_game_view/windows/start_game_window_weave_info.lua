...
function StartGameWindowWeaveInfo:on_enter(params, offset)
...
function StartGameWindowWeaveInfo:_start_transition_animation(animation_name)
	local params = {
...
function StartGameWindowWeaveInfo:_create_ui_elements(params, offset)
...
function StartGameWindowWeaveInfo:_setup_input_buttons()
	...
	input_texture_style.texture_size = { start_game_input_data.size [1], start_game_input_data.size [2] }
	play_button_console.content.input_texture = start_game_input_data.texture
end

function StartGameWindowWeaveInfo:on_exit(params)
	print("[StartGameWindow] Exit Substate StartGameWindowWeaveInfo")
	self._ui_animator = nil
end

function StartGameWindowWeaveInfo:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function StartGameWindowWeaveInfo:_handle_gamepad_activity()
...
function StartGameWindowWeaveInfo:_update_can_play()
	local widgets_by_name = self._widgets_by_name
...
function StartGameWindowWeaveInfo:post_update(dt, t)

	return end

function StartGameWindowWeaveInfo:_update_animations(dt)
...
function StartGameWindowWeaveInfo:_is_button_pressed(widget)
...
function StartGameWindowWeaveInfo:_is_button_released(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	if hotspot.on_release then
		hotspot.on_release = false
...
function StartGameWindowWeaveInfo:_is_stepper_button_pressed(widget)
	local content = widget.content
...
function StartGameWindowWeaveInfo:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_enter
end

function StartGameWindowWeaveInfo:_is_button_hover_exit(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_exit
end

function StartGameWindowWeaveInfo:_is_button_selected(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.is_selected
end

function StartGameWindowWeaveInfo:_handle_input(dt, t)
...
function StartGameWindowWeaveInfo:_update_party_status(dt)
...
function StartGameWindowWeaveInfo:_play_sound(event)
	self._parent:play_sound(event)
end

function StartGameWindowWeaveInfo:_update_selected_weave()
...
function StartGameWindowWeaveInfo:_update_wind_icon_animation(dt)
...
local wind_texture_index_by_name = { shadow = 2, fire = 4, beasts = 5, life = 0, death = 3, light = 6, heavens = 1, metal = 7 }
...
function StartGameWindowWeaveInfo:_set_wind_icon_by_name(wind_name)
	local widgets_by_name = self._widgets_by_name
	local widget_icon = widgets_by_name.wind_icon
...
function StartGameWindowWeaveInfo:_set_colors_by_wind(wind_name)
...
function StartGameWindowWeaveInfo:_align_private_checkbox()
	local gamepad_active = Managers.input:is_device_active("gamepad")
	local widgets_by_name = self._widgets_by_name
...
function StartGameWindowWeaveInfo:_apply_color_values(target, source, color_multiplier, include_alpha)
...
function StartGameWindowWeaveInfo:_assign_objective(widget, title_text, text, icon, spacing)
...
function StartGameWindowWeaveInfo:_exit(selected_level)
	self.exit = true
	self.exit_level_id = selected_level
end

function StartGameWindowWeaveInfo:draw(dt)
	local ui_renderer = self._ui_renderer
...
function StartGameWindowWeaveInfo:_animate_pulse(target, target_index, from, to, speed)
	local new_animation = UIAnimation.init(UIAnimation.pulse_animation, target, target_index,
	from, to, speed)
	return new_animation
end

function StartGameWindowWeaveInfo:_animate_element_by_time(target, target_index, from, to, time)
	local new_animation = UIAnimation.init(UIAnimation.function_by_time, target, target_index,
	from, to, time, math.ease_out_quad)
	return new_animation
end

function StartGameWindowWeaveInfo:_animate_element_by_catmullrom(target, target_index, target_value, p0, p1, p2, p3, time)
	local new_animation = UIAnimation.init(UIAnimation.catmullrom, target, target_index, target_value,
	p0, p1, p2, p3, time)
	return new_animation
end

function StartGameWindowWeaveInfo:_format_time(time)
	local floor = math.floor
	local timer_text = string.format("%.2d:%.2d:%.2d", floor(time / 3600), floor(time / 60) % 60, floor(time) % 60)
	return timer_text
end

function StartGameWindowWeaveInfo:_get_save_data_by_weave_name(weave_name)
