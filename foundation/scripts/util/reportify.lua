...
function Reportify:get_data()
	if not self.has_setup then self:setup() end
	local pos, rot = self:_get_location()
	local player_info = self:_get_player_info()
	Application.console_send({ type = "reportify",
...
function Reportify:_get_player_info()
	local ret = { wielded_slot = "", primary_name = "", class_name = "", secondary_name = "" }
	...
	local local_player = self:_get_local_player()
	if not local_player then return ret
	end
...
function Reportify:_get_local_player()
	if not Managers.player then return false end
	if Managers.player:num_players() == 0 then return false end
