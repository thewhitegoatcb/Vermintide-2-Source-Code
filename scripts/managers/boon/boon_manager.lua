...
function BoonManager:destroy()
	self:unregister_rpcs()
end

local queued_target_filter_table = { }
function BoonManager:update(dt, t)
...
function BoonManager:add_boon(owner, reward_id, consume_type, consume_value, reactivation_rule)
	local unique_id = self._unique_id
	self._unique_id = unique_id + 1
	local new_boon = { active = true,
		owner = owner,
		reward_id = reward_id,
...
function BoonManager:_activate_boon(boon, valid_targets)
...
function BoonManager:_deactivate_boon(boon, valid_targets)
	local reward = InGameChallengeRewards [boon.reward_id]
...
function BoonManager:_activate_player_boons(peer_id, local_player_id)
...
function BoonManager:_deactivate_player_boons(peer_id, local_player_id)
	local player_unique_id = PlayerUtils.unique_player_id(peer_id, local_player_id)
...
function BoonManager:_has_been_consumed(boon)
...
function BoonManager:remove_boon(boon_unique_id)
	local boons = self._boons
	for i = 1, #boons do
...
function BoonManager:on_round_start(network_event_delegate, event_manager)
...
function BoonManager:on_round_end()
	self:unregister_rpcs()
...
function BoonManager:on_venture_start()
	return end

function BoonManager:on_venture_end()
	local boons = self._boons
	for i = #boons, 1, -1 do
		local boon = boons [i]
...
function BoonManager:on_player_spawned(player, unit, unique_id)

	self._spawned_players_queue [#self._spawned_players_queue + 1] = unit
end

function BoonManager:on_player_joined_party(peer_id, local_player_id, party_id, slot_id)
	self:_activate_player_boons(peer_id, local_player_id)
end

function BoonManager:on_player_left_party(peer_id, local_player_id, party_id, slot_id_player)
	self:_deactivate_player_boons(peer_id, local_player_id)
end

function BoonManager:on_bot_added(bot_player)
	self:_activate_player_boons(bot_player:network_id(), bot_player:local_player_id())
end

function BoonManager:on_bot_removed(bot_player)
	self:_deactivate_player_boons(bot_player:network_id(), bot_player:local_player_id())
end

function BoonManager:on_clean_up_server_controlled_buffs(unit)
	local boons = self._boons
	for i = 1, #boons do
		local boon = boons [i]
		local reward = InGameChallengeRewards [boon.reward_id]
...
function BoonManager:register_rpcs(network_event_delegate)
...
function BoonManager:unregister_rpcs()
