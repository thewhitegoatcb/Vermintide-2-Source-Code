...
local widget_index_by_consumable_slots = { slot_healthkit = 4, slot_grenade = 1, slot_potion = 2 }
...
local default_slot_textures = {
	slot_grenade = { "default_grenade_icon", "default_grenade_icon_lit" },
	slot_potion = { "default_potion_icon", "default_potion_icon_lit" },
	slot_healthkit = { "default_heal_icon", "default_heal_icon_lit" } }
...
function GamepadConsumableUI:init(ingame_ui_context)
...
function GamepadConsumableUI:_create_ui_elements()
...
function GamepadConsumableUI:destroy()
	self.ui_animator = nil
	self:set_visible(false)
end

function GamepadConsumableUI:set_visible(visible)
...
function GamepadConsumableUI:update(dt, t, inventory_extension)
	local input_manager = self.input_manager
	local gamepad_active = input_manager:is_device_active("gamepad")
...
function GamepadConsumableUI:_draw(dt)
...
function GamepadConsumableUI:_set_dirty()
	self._dirty = true
end

function GamepadConsumableUI:_set_widget_dirty(widget)
	widget.element.dirty = true
end

function GamepadConsumableUI:_update_extension_changes(dt, inventory_extension)
...
function GamepadConsumableUI:_on_slot_selected(widget)
	local ui_renderer = self.ui_renderer
...
function GamepadConsumableUI:_clear_selection()
	local ui_renderer = self.ui_renderer
...
function GamepadConsumableUI:_update_slot_icon(widget, item_data, wielded)
	local dirty = false
...
function GamepadConsumableUI:_update_slot_ammo(widget, slot_data, item_data, wielded)
	local dirty = false
...
function GamepadConsumableUI:_reset_slot_widget(widget, slot_name, index)
	local dirty = false
	local widget_content = widget.content
	...
	if widget_content.has_data then
		...
		if slot_name == "slot_healthkit" then --[[ Nothing --]]
		end
...
function GamepadConsumableUI:_change_heal_other_slot_state(state)
...
function GamepadConsumableUI:_animate_slot_fill(widget, widget_index)
	local params = { }
...
function GamepadConsumableUI:_align_widgets()
	local slot_widgets = self.slot_widgets
...
function GamepadConsumableUI:_update_slot_positions()
	local ui_scenegraph = self.ui_scenegraph
...
function GamepadConsumableUI:on_gamepad_activated()
	self:set_visible(true)
	self:_set_dirty()
end

function GamepadConsumableUI:on_gamepad_deactivated()
	self:set_visible(false)
	self:_set_dirty()
end