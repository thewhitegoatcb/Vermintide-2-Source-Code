...
local RPCS = { "rpc_deus_chest_looted" }
...
function DeusChestExtension:init(extension_init_context, unit, extension_init_data)
...
function DeusChestExtension:extensions_ready(world, unit)
...
function DeusChestExtension:destroy()
	self:unregister_rpcs()
end

function DeusChestExtension:register_rpcs(network_event_delegate)
	network_event_delegate:register(self, unpack(RPCS))
	self._network_event_delegate = network_event_delegate
end

function DeusChestExtension:unregister_rpcs()
	self._network_event_delegate:unregister(self)
	self._network_event_delegate = nil
end

function DeusChestExtension:update(unit, input, dt, context, t)
...
function DeusChestExtension:_update_chest_interaction_time()
...
function DeusChestExtension:update_upgrade_chest_color()
...
function DeusChestExtension:can_interact()
	if self._is_purchased then
		return false
	end
...
function DeusChestExtension:get_interact_hud_description()
	if self._is_purchased then
		do return "deus_weapon_chest_already_picked_up_hud_desc" end
	else
		return "deus_weapon_chest_hud_desc"
	end
end

function DeusChestExtension:get_purchase_cost()
	local chest_type = self._chest_type
	if chest_type == DEUS_CHEST_TYPES.upgrade then
		local wielded_weapon = self:_get_wielded_weapon()
		local equipped_rarity = wielded_weapon.rarity

		do return DeusCostSettings.deus_chest [chest_type] [equipped_rarity] [self._rarity] end
...
function DeusChestExtension:purchase()
...
function DeusChestExtension:_get_wielded_weapon()
	local inventory_extension = self._inventory_extension
...
function DeusChestExtension:on_interact()
	if self._chest_type == DEUS_CHEST_TYPES.upgrade then
...
function DeusChestExtension:_setup_rarity(seed, chest_type)
...
function DeusChestExtension:get_stored_purchase()
...
function DeusChestExtension:get_chest_type()
	return self._chest_type
end

function DeusChestExtension:_generate_stored_power_up(seed)
	local power_ups = self._deus_run_controller:generate_random_power_ups(
	DeusPowerUpSettings.weapon_chest_choice_amount,
...
function DeusChestExtension:_generate_stored_weapon(slots, rarity, go_id, profile_index, career_index)
	local deus_run_controller = self._deus_run_controller
	...
	local weapon_seed = HashUtils.fnv32_hash(string.format("%s_%s_%s_%s_%s",
	profile_index,
	career_index,
	current_node.weapon_pickup_seed,
	go_id,
	...
	local new_weapon = DeusWeaponGeneration.generate_weapon(
	difficulty,
	progress,
	rarity,
	weapon_seed,
...
function DeusChestExtension:_generate_upgraded_weapon(weapon, slot_name, rarity, go_id, profile_index, career_index)
	local deus_run_controller = self._deus_run_controller
	local current_node = deus_run_controller:get_current_node()
	local progress = current_node.run_progress
	local difficulty = deus_run_controller:get_run_difficulty()
	...
	local new_weapon = DeusWeaponGeneration.upgrade_item(
	weapon,
	difficulty,
	progress,
	rarity,
...
function DeusChestExtension:_get_server_chest_type()
	local game_session = Managers.state.network:game()
	local go_id = Managers.state.unit_storage:go_id(self.unit)
...
function DeusChestExtension:_set_server_chest_type(server_chest_type)
	local game_session = Managers.state.network:game()
	local go_id = Managers.state.unit_storage:go_id(self.unit)
...
local sound_events = { unlock_chest = "hud_morris_weapon_chest_unlock", unlock_power_up = "morris_reliquarys_get_boon", close_chest_ui = "hud_morris_weapon_chest_close", button_hover = "hud_morris_hover", exchange_weapon = "hud_morris_weapon_chest_change_weapon", open_chest_ui = "hud_morris_weapon_chest_open",
	...
	unlock_chest_rarity_sounds = { common = "play_hud_rewards_tier1", plentiful = "play_hud_rewards_tier1", exotic = "play_hud_rewards_tier3", rare = "play_hud_rewards_tier2", unique = "play_hud_rewards_tier4" } }
...
function DeusChestExtension:can_be_unlocked()
...
function DeusChestExtension:open_chest()
	local run_controller = self._deus_run_controller
	...
	if self._chest_type == DEUS_CHEST_TYPES.power_up then
		...
		run_controller:add_power_ups({ power_up }, REAL_PLAYER_LOCAL_ID)
...
function DeusChestExtension:_equip_weapon(deus_run_controller, new_weapon)
...
function DeusChestExtension:_get_best_slot_name(stored_weapon, chest_type, career_data, inventory_extension)
	...
	if chest_type == DEUS_CHEST_TYPES.upgrade then
		do return stored_weapon.preferred_slot_name end
	else
...
function DeusChestExtension:_post_chest_unlock(store_purchase)
	...
	if store_purchase then
		if self._chest_type == DEUS_CHEST_TYPES.power_up then
			Managers.state.event:trigger("present_rewards", { { type = "deus_power_up", power_up = store_purchase } })
		else
			Managers.state.event:trigger("present_rewards", { { type = "deus_item_tooltip", backend_id = store_purchase.backend_id } })
		end
	end
...
function DeusChestExtension:_play_sound(event)
	WwiseWorld.trigger_event(self._wwise_world, event)
end

function DeusChestExtension:_update_chest_animation_and_sound_state(chest_unit)
	local player_unit = self._player_unit
...
function DeusChestExtension:rpc_deus_chest_looted(channel_id, go_id)
