...
function EndScreenUI:init(ingame_ui_context)
...
function EndScreenUI:destroy()
	self.ui_animator = nil

	GarbageLeakDetector.register_object(self, "EndScreenUI")
end

function EndScreenUI:create_ui_elements()
	DO_RELOAD = false

	self.draw_flags = { draw_text = false, banner_alpha_multiplier = 0, draw_background = false }
...
function EndScreenUI:input_service()
	return self.input_manager:get_service("end_screen_ui")
end

function EndScreenUI:on_enter(screen_name, screen_context, screen_params)
...
function EndScreenUI:on_complete()
	self.is_complete = true
end

function EndScreenUI:fade_in_complete()
	return self._fade_in_completed
end

function EndScreenUI:_fade_in_background()
	self.background_in_anim_id = self.ui_animator:start_animation("fade_in_background", { self.background_rect_widget }, scenegraph_definition, self.draw_flags)
end

function EndScreenUI:update(dt, t)
	if DO_RELOAD then
		self:create_ui_elements()
	end
	...
	elseif screen:started() and
	screen:completed() and

	not Managers.backend:is_pending_request() then
...
function EndScreenUI:play_sound(event)
