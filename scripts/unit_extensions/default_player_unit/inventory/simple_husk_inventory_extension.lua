...
function SimpleHuskInventoryExtension:init(extension_init_context, unit, extension_init_data)
...
function SimpleHuskInventoryExtension:ammo_percentage()
...
function SimpleHuskInventoryExtension:ammo_status()
	if GameSession.game_object_exists(self._game, self._game_object_id) then
...
function SimpleHuskInventoryExtension:destroy()
...
function SimpleHuskInventoryExtension:get_weapon_unit()
	local equipment = self._equipment
	local weapon_unit = equipment.left_hand_wielded_unit_3p or equipment.right_hand_wielded_unit_3p
	return weapon_unit
end

function SimpleHuskInventoryExtension:get_all_weapon_unit()
	local equipment = self._equipment
	return equipment.left_hand_wielded_unit_3p, equipment.right_hand_wielded_unit_3p
end


function SimpleHuskInventoryExtension:drop_level_event_item(slot_data)
...
function SimpleHuskInventoryExtension:_unlink_unit(unit, reason, attachment_node_linking)
...
function SimpleHuskInventoryExtension:drop_equipped_weapons(reason)
	local equipment = self._equipment
...
function SimpleHuskInventoryExtension:equipment()
	return self._equipment
end

function SimpleHuskInventoryExtension:add_equipment(slot_name, item_name, skin_name)
	...
	self._equipment.slots [slot_name] = { item_data = item_data, id = slot_name, skin = skin_name, item_template = item_template }
end

function SimpleHuskInventoryExtension:add_equipment_limited_item(slot_name, item_name, spawner_unit, limited_item_id)
	local item_data = ItemMasterList [item_name]
...
function SimpleHuskInventoryExtension:evaluate_limited_item_state(slot_data)
...
function SimpleHuskInventoryExtension:_setup_equipment(world, unit_3p, profile)
...
function SimpleHuskInventoryExtension:update()
	return end

local new_linked_units = { }
function SimpleHuskInventoryExtension:_reapply_fade(equipment)
...
function SimpleHuskInventoryExtension:wield(slot_name)
	local equipment = self._equipment
...
function SimpleHuskInventoryExtension:_despawn_attached_units()
...
function SimpleHuskInventoryExtension:_spawn_attached_units(attached_units)
...
function SimpleHuskInventoryExtension:clear_buffs_on_slot(reason, slot_name)
...
function SimpleHuskInventoryExtension:set_buffs_to_slot(reason, slot_name, buffs)
	if slot_name == "slot_ranged" or slot_name == "slot_melee" then
...
function SimpleHuskInventoryExtension:_refresh_buffs(buffs, reason, slot_name)
...
function SimpleHuskInventoryExtension:has_inventory_item(slot_name, item_name)
	local slot_data = self:get_slot_data(slot_name)
...
function SimpleHuskInventoryExtension:show_third_person_inventory(show)
...
function SimpleHuskInventoryExtension:get_item_template(slot_data)
	local item_data = slot_data.item_data
	local item_template = BackendUtils.get_item_template(item_data)
	return item_template
end

function SimpleHuskInventoryExtension:get_wielded_slot_data()
...
function SimpleHuskInventoryExtension:get_wielded_slot_item_template()
...
function SimpleHuskInventoryExtension:hot_join_sync(sender)
	GearUtils.hot_join_sync(sender, self._unit, self._equipment, self._additional_items)
end

function SimpleHuskInventoryExtension:get_wielded_slot_name()
	local equipment = self._equipment
...
function SimpleHuskInventoryExtension:get_slot_data(slot_id)
	local equipment = self._equipment
...
function SimpleHuskInventoryExtension:set_loaded_projectile_override()
	return end

function SimpleHuskInventoryExtension:_override_career_skill_item_template(item_data)
...
function SimpleHuskInventoryExtension:_wield_slot(world, equipment, slot_name, unit_1p, unit_3p)
	...
	if item_units.right_hand_unit then
		right_hand_weapon_unit_3p, right_hand_ammo_unit_3p, right_hand_weapon_unit_1p, right_hand_ammo_unit_1p = GearUtils.spawn_inventory_unit(
		world,
		"right",
		item_template,
		item_units,
		...
		item_units.material_settings)
	end

	if item_units.left_hand_unit then
		left_hand_weapon_unit_3p, left_hand_ammo_unit_3p, left_hand_weapon_unit_1p, left_hand_ammo_unit_1p = GearUtils.spawn_inventory_unit(
		world,
		"left",
		item_template,
		item_units,
...
function SimpleHuskInventoryExtension:is_showing_third_person_inventory()
	return self._show_third_person
end


function SimpleHuskInventoryExtension:start_weapon_fx(fx_name)
	local equipment = self._equipment
...
function SimpleHuskInventoryExtension:stop_weapon_fx(fx_name)
	self._weapon_fx [fx_name] = GearUtils.destroy_attached_particles(self._world, self._weapon_fx [fx_name])
end

function SimpleHuskInventoryExtension:_stop_all_weapon_fx()
	local world = self._world
...
function SimpleHuskInventoryExtension:has_additional_item_slots(slot_name)
	local additional_items_slot = self._additional_items [slot_name]
	return additional_items_slot ~= nil
end

function SimpleHuskInventoryExtension:can_store_additional_item(slot_name)
	local additional_items_slot = self._additional_items [slot_name]
	return additional_items_slot and #additional_items_slot.items < additional_items_slot.max_slots
end

function SimpleHuskInventoryExtension:has_additional_items(slot_name)
	local additional_items_slot = self._additional_items [slot_name]
	return additional_items_slot and #additional_items_slot.items > 0
end

function SimpleHuskInventoryExtension:get_additional_items(slot_name)
	local additional_items_slot = self._additional_items [slot_name]
	return additional_items_slot and additional_items_slot.items
end

function SimpleHuskInventoryExtension:get_additional_items_table()
	return self._additional_items
end

function SimpleHuskInventoryExtension:get_total_item_count(slot_name)
	local count = 0
...
function SimpleHuskInventoryExtension:update_additional_items(slot_name, items)
	local additional_items_slot = self._additional_items [slot_name]
