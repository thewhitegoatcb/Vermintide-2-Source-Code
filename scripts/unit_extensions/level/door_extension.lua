...
function DoorExtension:init(extension_init_context, unit, extension_init_data)
	...
	self.animation_flow_events = {
		closed = { open_backward = "lua_open_backward", open_forward = "lua_open_forward" },



		open_forward = { closed = "lua_close_forward", open_backward = "lua_swing_forward" },



		open_backward = { closed = "lua_close_backward", open_forward = "lua_swing_backward" } }
...
function DoorExtension:extensions_ready()
	self.health_extension = ScriptUnit.extension(self.unit, "health_system")
end

function DoorExtension:update_nav_graphs()
...
function DoorExtension:interacted_with(interacting_unit)
	local unit = self.unit
	local current_state = self.current_state
	local new_state = nil
...
function DoorExtension:set_door_state(new_state)
	local current_state = self.current_state
...
function DoorExtension:get_current_state()
	return self.current_state
end

function DoorExtension:_get_animation_flow_event(current_state, new_state)
...
function DoorExtension:update(unit, input, dt, context, t)
...
function DoorExtension:register_breed_failed_leaving_smart_object(unit)
	if self.breeds_failed_leaving_smart_object == nil then return
	end
	self.breeds_failed_leaving_smart_object [unit] = true
end

function DoorExtension:handle_breeds_failed_leaving_smart_object()
	if self.breeds_failed_leaving_smart_object == nil then return
	end
...
function DoorExtension:hot_join_sync(sender)
...
function DoorExtension:destroy_box_obstacles()
...
function DoorExtension:is_open()
	return self.current_state ~= "closed"
end

function DoorExtension:is_opening()
