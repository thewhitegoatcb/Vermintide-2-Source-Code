	...
	inject_travel_dists = function (main_paths, overrride)
		...
		if not first_path.travel_dist or overrride then
			...
			for i = 1, #main_paths do
				...
				local travel_dist = { total_travel_dist }
				for j = 2, #nodes do
					p1 = nodes [j - 1]:unbox()
					p2 = nodes [j]:unbox()
					total_travel_dist = total_travel_dist + Vector3_distance(p1, p2)
...
function MainPathSpawningGenerator.remove_crossroads_extra_path_branches(crossroads, chosen_crossroads, main_paths, zones, num_main_zones, path_markers, seed)
	if not crossroads or not next(crossroads) then
	...
	for i = #crossroad_main_path_indices, 1, -1 do repeat
			...
			if not chosen_crossroad_paths [next_main_path_index] then
				crossroad_stitch [#crossroad_stitch + 1] = next_main_path_index end until true
	end


	for i = 1, #to_stitch do repeat
			...
			for k = 2, #stitched_indices do
				...
				to_remove [#to_remove + 1] = stitch_index end until true
	end
	...
	for k = #to_remove, 1, -1 do
		...
		if not was_stitched_path then
			removed_path_distances [#removed_path_distances + 1] = { travel_dist [1], travel_dist [#travel_dist] }
		end
...
function MainPathSpawningGenerator.generate_great_cycles(conflict_director, mutator_list, zones, zone_convert, num_main_zones, spawn_cycle_length, level_seed)
	local cycle_length = 0 local cycle_zones = { } local great_cycles = { }
	...
	for i = 1, num_main_zones do
		...
		if zone_layer.mutators then
			...
			for name in string.gmatch(zone_layer.mutators, "([^[%s,]+)%s*,?%s*") do mutators_split [#mutators_split + 1] = name
			end
		...
		for j = 2, #zone_layer.sub do
			...
			outer [#outer + 1] = { nodes = zone_layer.sub [j], area = area }
		end
		...
		if spawn_cycle_length <= cycle_length or i == num_main_zones then
			great_cycles [#great_cycles + 1] = { zones = cycle_zones, length = cycle_length }
...
function MainPathSpawningGenerator.process_conflict_directors_zones(default_conflict_director_name, zones, num_main_zones, level_seed)
