...
function PositiveReinforcementUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function PositiveReinforcementUI:destroy()
...
function PositiveReinforcementUI:create_ui_elements()
...
function PositiveReinforcementUI:remove_event(index)
...
function PositiveReinforcementUI:add_event(hash, is_local_player, color_from, event_type, ...)
	if not script_data.disable_reinforcement_ui then
		...
		if old_event and settings_positive_reinforcement.folding_enabled then
			...
			local event = { text = "", shown_amount = 0, amount = 0,
				full_hash = full_hash,
				widget = widget,
...
local temp_portrait_size = { 96, 112 }
function PositiveReinforcementUI:_assign_portrait_texture(widget, pass_name, texture)
...
function PositiveReinforcementUI:event_add_positive_enforcement(hash, is_local_player, event_type, player1, player2)
...
function PositiveReinforcementUI:event_add_positive_enforcement_kill(hash, is_local_player, event_type, breed_name_attacker, breed_name_killed)
...
function PositiveReinforcementUI:event_add_positive_enforcement_player_knocked_down_or_killed(hash, is_local_player, event_type, profile_index, breed_name)
...
function PositiveReinforcementUI:event_add_lorebook_page_pickup(hash, is_local_player, event_type, page_id)
	self:add_event(hash, is_local_player, event_colors.personal, event_type, page_id)
end

function PositiveReinforcementUI:event_add_interaction_warning(hash, message)
	self:add_event(hash, true, event_colors.kill, "interaction_warning", Localize(message))
end

function PositiveReinforcementUI:_get_hero_portrait(profile_index, career_index)
...
local customizer_data = { root_scenegraph_id = "pivot", label = "Kill feed", registry_key = "kill_feed", drag_scenegraph_id = "pivot_dragger" }
...
function PositiveReinforcementUI:update(dt, t)
