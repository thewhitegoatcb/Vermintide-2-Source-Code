...
local spawn_pack = { }
function PackSpawnerUtils.spawn_predefined_pack(breed_pack, pos, mesh)
		...
		if type(pack) == "table" then
			for i, unit_data in ipairs(pack) do
				...
				if unit_position then
					...
					spawn_pack [size] = { breed, unit_position, rot, animation, inventory_template }
				else
...
function PackSpawnerUtils.spawn_in_circle(breed_pack, pack_size, pos)
	...
	for i = 3, 100, 2 do
		...
		for j = 1, point_cap do
			...
			if unit_position then
				...
				local inventory_template = unit_data.inventory_template or "default"
				spawn_pack [points_found] = { breed, unit_position, rot, animation, inventory_template }
			end
...
function PackSpawnerUtils.spawn_random_pack(breed_pack, pos, mesh)
	...
	for i = 1, random_amount do
		...
		while attempts < 10 do
			...
			if not PackSpawnerUtils.check_unit_overlap(unit_position, spawn_pack, i) then
				...
				spawn_pack [i] = { breed, unit_position, rot, idle_animation }
				break
			end
			attempts = attempts + 1
		end
...
function PackSpawnerUtils.modify_spawn_position(unit_pos, pos)
...
function PackSpawnerUtils.check_unit_overlap(pos, other_units, units_count)
