...
function MissionObjectiveUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function MissionObjectiveUI:create_ui_elements()
...
function MissionObjectiveUI:destroy()
	local event_manager = Managers.state.event
	if event_manager then
...
function MissionObjectiveUI:block_mission_ui(ui_blocked)
	self._ui_blocked = ui_blocked
end

local customizer_data = { root_scenegraph_id = "pivot", label = "Objectives", registry_key = "mission_objective", drag_scenegraph_id = "background" }
...
function MissionObjectiveUI:update(dt)
	if DO_RELOAD then
		self:create_ui_elements()
	end
...
function MissionObjectiveUI:add_mission_objective(mission_name, text, duration_text)
	...
	self.saved_mission_objectives [#self.saved_mission_objectives + 1] = { mission_name = mission_name, text = text, duration_text = duration_text }
end

function MissionObjectiveUI:_clear_animations()
...
function MissionObjectiveUI:complete_mission(mission_name, remove_immediately)
...
function MissionObjectiveUI:_remove_mission_objective(mission_name)
	local index = nil
...
function MissionObjectiveUI:update_mission(mission_name, text, duration_text)
	local index = nil
	for idx, mission_data in ipairs(self.saved_mission_objectives) do
		if mission_data.mission_name == mission_name then
			index = idx
...
function MissionObjectiveUI:next_mission_objective(dt)
...
function MissionObjectiveUI:update_animations(dt)
...
function MissionObjectiveUI:_set_mission_text(text, duration_text, calculate_offsets)
	...
	local max_width = 287.5 local max_height = 40
...
function MissionObjectiveUI:_get_text_size(ui_renderer, text, max_width, max_height, style)
	...
	if max_height < full_font_height then
		repeat
			...
			if style.font_size == style.min_font_size then
				return full_font_height end until full_font_height <= max_height
	end
...
function MissionObjectiveUI:draw(dt)
	local ui_renderer = self.ui_renderer
	local ui_scenegraph = self.ui_scenegraph
...
function MissionObjectiveUI:_start_animation(key, animation_name)
	local params = {
