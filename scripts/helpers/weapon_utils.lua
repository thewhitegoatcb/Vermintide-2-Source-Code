...
function WeaponUtils.add_bot_meta_data_chain_actions(actions, attack_chain_transitions)
...
function WeaponUtils.find_allowed_chain_action(allowed_chain_actions, action_name, sub_action_name, wanted_action_name, wanted_sub_action_name)
...
function WeaponUtils.get_weapon_packages(item_template, item_units, first_person)
	local packages = { }
...
function WeaponUtils.get_used_actions(template)
			...
			for chain_action_id = 1, #chain_actions do
				...
				if chain_action_name and chain_sub_action_name then
					...
					if chain_sub_action then
						if ( not checked_actions [chain_action_name] or not checked_actions [chain_action_name] [chain_sub_action_name] ) and (
						not pending_actions [chain_action_name] or not pending_actions [chain_action_name] [chain_sub_action_name]) then
...
function WeaponUtils.is_valid_weapon_override(source_slot_data, destination_item_data)
