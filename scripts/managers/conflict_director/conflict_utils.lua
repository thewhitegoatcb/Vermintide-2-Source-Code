...
function ConflictUtils.random_interval(numbers)
...
local cluster_work_queue = { false, false, false }
function ConflictUtils.cluster_positions(positions, min_dist)
	...
	for i = 1, 3 do work_queue [i] = nil end
...
local loneliness = { }
local max_cluster_score = { 1, 2, 3, 6 }
function ConflictUtils.cluster_weight_and_loneliness(positions, min_dist)
	local distance_squared = Vector3.distance_squared
	min_dist = min_dist * min_dist
	local num_positions = math.min(#positions, 4)

	if num_positions == 1 then
		do return 1, 1, 100 end
	...
	local utility_sum = 0
	local ab = 0 local ac = 0 local ad = 0 local bc = 0 local bd = 0 local cd = 0
...
function ConflictUtils.average_player_position()
...
function ConflictUtils.hidden_cover_points(center_position, avoid_pos_list, min_rad, max_rad, dot_threshold)
...
function ConflictUtils.debug_is_cover_point_hidden()
	local bp = Managers.state.conflict.level_analysis.cover_points_broadphase
	local num_found_cover_units = Broadphase.query(bp, PLAYER_POSITIONS [1], 20, found_cover_units)

	local red = Colors.get("red") local green = Colors.get("green")
	local unit_local_rotation = Unit.local_rotation
	local unit_local_position = Unit.local_position
	local min_rad = 5
...
function ConflictUtils.is_cover_point_hidden(cover_point_unit, avoid_pos_list, min_rad, long_distance_sqr)
	local vector3_normalize = Vector3.normalize
	local quaternion_forward = Quaternion.forward
	local unit_local_rotation = Unit.local_rotation
	local unit_local_position = Unit.local_position
...
function ConflictUtils.get_random_spawner_with_id(id, except_spawner_unit)
...
function ConflictUtils.get_random_hidden_spawner(center_pos, radius, pick_closest_spawner)
	local spawner_system = Managers.state.entity:system("spawner_system")
...
function ConflictUtils.get_biggest_cluster(clusters_sizes)
	local biggest = 1
...
function ConflictUtils.filter_positions(center_pos, main_target_pos, spawner_units, min_dist, max_dist)
	local list = { }
...
function ConflictUtils.filter_horde_spawners(player_positions, spawner_units, hidden_spawner_units, min_dist, max_dist)
	local list = { } local hidden_list = { }
	max_dist = max_dist * max_dist
	min_dist = min_dist * min_dist
...
function ConflictUtils.filter_horde_spawners_strictly(player_positions, spawner_units, hidden_spawner_units, min_dist, max_dist)
	local list = { } local hidden_list = { }
	max_dist = max_dist * max_dist
	min_dist = min_dist * min_dist
...
function ConflictUtils.get_hidden_pos(world, nav_world, level, nav_tag_volume_handler, check_no_spawn_volumes, center_pos, avoid_positions, radius, radius_spread, avoid_dist_sqr, max_tries, cake_slice_dir, cake_slice_angle_radians)
...
function ConflictUtils.is_position_inside_no_spawn_volume(level, nav_tag_volume_handler, pos)
	return NavTagVolumeUtils.inside_level_volume_layer(level, nav_tag_volume_handler, pos, "NO_SPAWN") or
	NavTagVolumeUtils.inside_level_volume_layer(level, nav_tag_volume_handler, pos, "NO_BOTS_NO_SPAWN")
end


function ConflictUtils.find_center_tri(nav_world, pos, above_max, below_max)
...
function ConflictUtils.simulate_dummy_target(nav_world, center_pos, t)
	local radius = 15
...
function ConflictUtils.test_cake_slice(nav_world, center_pos, t)
...
function ConflictUtils.get_spawn_pos_on_cake_slice(nav_world, center_pos, radius1, radius2, cake_slice_dir, cake_slice_angle_radians, tries, check_no_spawn_volumes, level, nav_tag_volume_handler)
...
function ConflictUtils.get_spawn_pos_on_circle(nav_world, center_pos, dist, spread, tries, check_no_spawn_volumes, level, nav_tag_volume_handler, above_max, below_max)
	local p1, p2, p3 = nil
	for i = 1, tries do
		...
		if
		pos and (not check_no_spawn_volumes or not ConflictUtils.is_position_inside_no_spawn_volume(level, nav_tag_volume_handler, pos)) then
			return pos, p1, p2, p3
		end
	end
...
function ConflictUtils.get_pos_towards_goal(nav_world, center_pos, dist, spread, tries, optional_dir, check_no_spawn_volumes, level, nav_tag_volume_handler)
	tries = tries or 1
...
function ConflictUtils.get_furthest_pos_from_pos_on_circle(nav_world, center_pos, dist, spread, tries, avoid_pos)
	local positions = { }
	for i = 1, tries do
		local add_vec = Vector3(dist + ( math.random() - 0.5 ) * spread, 0, 1)
		local pos = center_pos + Quaternion.rotate(Quaternion(Vector3.up(), math.degrees_to_radians(Math.random(1, 360))), add_vec)
...
function ConflictUtils.get_spawn_pos_on_circle_with_func(nav_world, center_pos, dist, spread, tries, filter_func, filter_data, above_max, below_max)
	for i = 1, tries do
		local add_vec = Vector3(dist + ( math.random() - 0.5 ) * spread, 0, 1)
		local pos = center_pos + Quaternion.rotate(Quaternion(Vector3.up(), math.degrees_to_radians(Math.random(1, 360))), add_vec)
...
function ConflictUtils.get_spawn_pos_on_circle_with_func_range(nav_world, center_pos, min_dist, max_dist, tries, filter_func, filter_data, above_max, below_max)
	for i = 1, tries do
...
function ConflictUtils.draw_stack_of_balls(pos, a, r, g, b)
...
local level_portals = { }
function ConflictUtils.get_teleporter_portals()
	...
	for _, id in ipairs(unit_ind) do
		...
		portals [portal_id] = { boxed_pos, boxed_rot }
	end

	return portals
end

function ConflictUtils.interest_point_outside_nav_mesh(nav_world, unit_name, pos, interest_point_rot)
...
function ConflictUtils.generate_spawn_point_lookup(world)
		...
		if unit_names then
			for k, unit_name in ipairs(unit_names) do
				...
				while Unit.has_data(unit, "interest_point", "points", i) do
					...
					p [#p + 1] = { Vector3Box(point_position), QuaternionBox(point_rotation) }
					i = i + 1
				end
...
function ConflictUtils.display_number_of_breeds(header, num_spawned, num_spawned_by_breed)
...
local count_list = { }
function ConflictUtils.display_number_of_breeds_in_segment(header, units_spawned_by_breed, zone_data)
...
function ConflictUtils.show_where_ai_is(spawned)
...
function ConflictUtils.make_roaming_spawns(nav_world, level_analysis)
	...
	while i < num_triangles do
		...
		local neighbors = { GwNavTraversal.get_neighboring_triangles(triangle) }
...
function ConflictUtils.add_breeds_from_event(event_name, event, difficulty, difficulty_rank, output, terror_event_lookup)
	for i = 1, #event do repeat
			...
			elseif sub_event_name == "event_horde" or sub_event_name == "ambush_horde" then
				...
				add_breeds_from_horde_composition(output, event_composition_type, difficulty) end until true
	end
...
function ConflictUtils.find_conflict_director_breeds(conflict_director, difficulty, output)
...
function ConflictUtils.patch_settings_with_difficulty(source_settings, difficulty, fallback_difficulty)
	...
	if override_settings then
		...
		do return source_settings end
	else
		return source_settings
	end
end

function ConflictUtils.patch_terror_events_with_weaves(level_key, weave_data, objective_index)
...
function ConflictUtils.generate_conflict_director_locked_functions(level_key)
...
function ConflictUtils.teleport_ai_unit(unit, teleport_position, play_sound, play_effect)
...
function ConflictUtils.look_at_position_flat(source_pos, target_pos)
...
function ConflictUtils.get_closest_position(pos, pos_list)
...
function ConflictUtils.override_extension_init_data(main_extension_init_data, optional_data)
...
function ConflictUtils.raise_dead(pos, radius, side_id)
	if pos then
		radius = radius or 10
		local result_table = { }
		local raise_list = { }
		...
		if amount > 0 then
			...
			local group_data = { insert_into_group = true, template = "resurrected", group_type = "resurrected",
				id = group_id,
			...
			for i = 1, amount do
				...
				local optional_data = { ignore_breed_limits = true, side_id = side_id, insert_into_group = resurrected_group_id ~= nil }
...
function ConflictUtils.command_ai_to_move(player, nav_world, pos)
	if pos then
...
function ConflictUtils.find_positions_around_position(center_position, output_position_list, nav_world, min_distance, max_distance, num_of_positions, forbidden_position_list, distance_to_forbidden_position_list, tries, circle_subdivision, row_distance, above_max, below_max, check_line_of_sight, physics_world, line_of_sight_target)
...
local function check_if_above_ground(physics_world, origin_position, distance)
	local hit, _, _, _, _ = PhysicsWorld.raycast(
	physics_world,
	origin_position,
	Vector3(0, 0, -1),
	distance,
...
function ConflictUtils.find_visible_positions_in_sphere_around_player(physics_world, position_count, player_unit, radius, from_pitch, to_pitch, pitch_delta, from_yaw, to_yaw, yaw_delta, forbidden_position_list, distance_to_forbidden_position_list, distance_between_positions, min_distance_from_floor)
	...
	while position_count > #valid_positions do
		...
		if not failed then
			...
			if not failed and check_if_in_line_of_sight(physics_world, player_unit, new_position, center_position) and
			check_if_above_ground(physics_world, new_position, min_distance_from_floor) then
