...
table.merge_recursive(BTConditions.ability_check_categories, {
	activate_ability = { wh_priest = true } })
...
function BTConditions.can_activate.wh_priest(blackboard)
	...
	if ALIVE [self_unit] and ALIVE [target_ally_unit] and blackboard.ally_distance and blackboard.ally_distance < WP_MAX_ASSIST_DIST then
		...
		if ally_status_ext then
			if ally_status_ext:is_pounced_down() or
			ally_status_ext:is_grabbed_by_pack_master() or
			ally_status_ext:is_grabbed_by_corruptor() then
			...
			if not should_target_ally then
				...
				if
				has_revive_talent and ally_status_ext:is_knocked_down() then
					should_target_ally = true
				end
			end
		end
