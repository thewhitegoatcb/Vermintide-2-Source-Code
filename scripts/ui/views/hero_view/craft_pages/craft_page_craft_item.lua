...
function CraftPageCraftItem:on_enter(params, settings)
...
function CraftPageCraftItem:setup_recipe_requirements()
...
function CraftPageCraftItem:create_recipe_grid_by_amount(amount)
...
function CraftPageCraftItem:create_ui_elements(params)
...
function CraftPageCraftItem:on_exit(params)
...
function CraftPageCraftItem:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function CraftPageCraftItem:post_update(dt, t)

	return end

function CraftPageCraftItem:_update_animations(dt)
...
function CraftPageCraftItem:_is_button_pressed(widget)
...
function CraftPageCraftItem:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageCraftItem:_is_button_held(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageCraftItem:_handle_input(dt, t)
...
function CraftPageCraftItem:_handle_craft_input_progress(progress)
...
function CraftPageCraftItem:craft_result(result, error, reset_slots)
	if not error then
		self._craft_result = result
	end
end

function CraftPageCraftItem:reset()
	local item_grid = self._item_grid
...
function CraftPageCraftItem:on_craft_completed()
...
function CraftPageCraftItem:_update_craft_items()
	local super_parent = self.super_parent
	local item_grid = self._item_grid
...
function CraftPageCraftItem:_remove_craft_item(backend_id, slot_index)
	local craft_items = self._craft_items
...
function CraftPageCraftItem:_add_craft_item(backend_id, slot_index, ignore_sound)
...
function CraftPageCraftItem:_set_craft_button_disabled(disabled)
	self._widgets_by_name.craft_button.content.button_hotspot.disable_button = disabled
end

function CraftPageCraftItem:_exit(selected_level)
	self.exit = true
	self.exit_level_id = selected_level
end

function CraftPageCraftItem:draw(dt)
...
function CraftPageCraftItem:_play_sound(event)
	self.super_parent:play_sound(event)
end

function CraftPageCraftItem:_set_craft_button_text(text, localize)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.craft_button
