...
function BTSelector_chaos_vortex_sorcerer:init(...)
	BTSelector_chaos_vortex_sorcerer.super.init(self, ...)
	self._children = { }
end

function BTSelector_chaos_vortex_sorcerer:leave(unit, blackboard, t, reason)
	self:set_running_child(unit, blackboard, t, nil, reason)
end

function BTSelector_chaos_vortex_sorcerer:run(unit, blackboard, t, dt)
	...
	local node_spawn_vortex = children [7]
	local condition_result = blackboard.ready_to_summon and (blackboard.summoning or Unit.alive(blackboard.target_unit))

	if condition_result then
		self:set_running_child(unit, blackboard, t, node_spawn_vortex, "aborted")


		local result, evaluate = node_spawn_vortex:run(unit, blackboard, t, dt)
	...
	local node_skulk_approach = children [8]
	local condition_result = unit_alive(blackboard.target_unit)

	if condition_result then
		self:set_running_child(unit, blackboard, t, node_skulk_approach, "aborted")


		local result, evaluate = node_skulk_approach:run(unit, blackboard, t, dt)
...
function BTSelector_chaos_vortex_sorcerer:add_child(node)
