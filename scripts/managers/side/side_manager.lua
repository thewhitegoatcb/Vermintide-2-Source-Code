...
function SideManager:init(side_compositions)

	side_compositions [0] = { name = "undecided",
...
function SideManager:_create_sides(side_compositions)
	local sides = { }
	local side_lookup = { }
...
function SideManager:_setup_relations(side_compositions, sides, side_lookup)
	for i = 0, #side_compositions do
		local definition = side_compositions [i]
		local side = sides [i]
		...
		side:set_relation("ally", { side })
	end
end

function SideManager:_setup_side_by_party(sides)
	local side_by_party = { }
...
function SideManager:sides()
	return self._sides
end

function SideManager:get_side(side_id)
	return self._sides [side_id]
end



function SideManager:get_side_from_name(name)
	return self._side_lookup [name]
end


function SideManager:get_party_from_side_name(name)
	local side = self._side_lookup [name]
	return side.party
end

function SideManager:versus_is_hero(unit)
	local side = self.side_by_unit [unit]
	return side and side:name() == "heroes"
end

function SideManager:versus_is_dark_pact(unit)
	local side = self.side_by_unit [unit]
	return side and side:name() == "dark_pact"
end

function SideManager:add_unit_to_side(unit, side_id)
...
function SideManager:remove_unit_from_side(unit)
	local side = self.side_by_unit [unit]
	if not side then
		return
	end
...
function SideManager:add_player_unit_to_side(player_unit, side_id)
...
function SideManager:remove_player_unit_from_side(player_unit)
...
function SideManager:is_ally(unit1, unit2)
	local side = self.side_by_unit [unit1]
...
function SideManager:is_player_friendly_fire(unit1, unit2)
...
function SideManager:remove_aggro_unit(side_id, aggro_unit)
	local side = self._sides [side_id]
...
function SideManager:update_frame_tables()
...
function SideManager:_update_frame_tables(side, all_human_and_bot_units)
...
function SideManager:_update_enemy_frame_tables(side)
...
function SideManager:_remove_player_unit_from_lists(player_unit)
...
function SideManager:get_side_from_player_unique_id(unique_id)
