...
DebugKeyHandler = DebugKeyHandler or { num_keys = 0, keys = { } }
local DebugKeyHandler = DebugKeyHandler

function DebugKeyHandler.setup(world, input_manager)
...
function DebugKeyHandler.set_enabled(enabled)
	DebugKeyHandler.enabled = enabled
end

local blocking_modifiers = { "left ctrl", "left shift", "right ctrl", "left alt" }
...
function DebugKeyHandler.key_pressed(key, description, category, key_modifier, input_service_name)
...
function DebugKeyHandler.frame_clear()
...
function DebugKeyHandler.render()
	if not script_data.debug_key_handler_visible then return
	end
	local offset_lerp = 1
	if not DebugKeyHandler.enabled then offset_lerp = 0.3 end
