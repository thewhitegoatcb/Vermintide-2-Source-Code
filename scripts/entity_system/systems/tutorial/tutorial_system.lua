...
local extensions = { "PlayerTutorialExtension", "ObjectiveHealthTutorialExtension", "ObjectivePickupTutorialExtension", "ObjectiveSocketTutorialExtension", "ObjectiveUnitExtension" }
...
function TutorialSystem:init(entity_system_creation_context, system_name)
...
function TutorialSystem:destroy()
...
local dummy_input = { }
function TutorialSystem:on_add_extension(world, unit, extension_name, extension_init_data)
	local extension = { }
	if extension_name == "PlayerTutorialExtension" then
		...
		extension.objective_tooltips = { units_n = 0, active = false, units = { } }
...
function TutorialSystem:on_remove_extension(unit, extension_name)
...
function TutorialSystem:physics_async_update(context, t)
	...
	for unit, extension in pairs(self.player_units) do
		...
		if ( self.pacing == "pacing_peak_fade" or self.pacing == "pacing_relax" ) and
		not script_data.info_slates_disabled then
...
function TutorialSystem:iterate_tooltips(t, unit, extension, raycast_unit, world)
	...
	for i = 1, tooltip_templates_n do repeat
			...
			elseif not in_play_go and template.incompatible_in_game then --[[ Nothing --]]
...
function TutorialSystem:prioritize_objective_tooltip(objective_tooltip_name, reset)
	if reset then
		self._objective_tooltip_prioritized_list = nil
...
function TutorialSystem:iterate_objective_tooltips(t, unit, extension, raycast_unit, world)
	...
	for i = 1, objective_tooltip_templates_n do repeat
...
function TutorialSystem:verify_info_slate(t, unit, raycast_unit, template)
...
function TutorialSystem:iterate_info_slates(t, unit, extension, raycast_unit, world)
	if Application.user_setting("tutorials_enabled") then
		...
		for i = 1, info_slate_templates_n do repeat
				...
				if template.can_show(t, unit, extension.data, raycast_unit, world) then
					...
					Managers.state.event:trigger("tutorial_event_queue_info_slate_entry", text, nil, nil, template, unit, raycast_unit) end until true
		end
	else
...
function TutorialSystem:rpc_tutorial_message(channel_id, template_id, message_id)
...
function TutorialSystem:rpc_pacing_changed(channel_id, pacing_id)
	local pacing = NetworkLookup.pacing [pacing_id]
	self.pacing = pacing
	tutprintf("Changing pacing state to %s", pacing)
end

function TutorialSystem:rpc_objective_unit_set_active(channel_id, level_object_id, is_level_unit, activate)
...
function TutorialSystem:rpc_prioritize_objective_tooltip(channel_id, prioritized_objective_tooltip_id)
...
function TutorialSystem:flow_callback_show_health_bar(unit, show)
	local event_manager = Managers.state.event
	event_manager:trigger("tutorial_event_show_health_bar", unit, show)
end

function TutorialSystem:flow_callback_tutorial_message(template_name, message)
...
function TutorialSystem:hot_join_sync(peer_id)
	local network_manager = Managers.state.network
...
function TutorialSystem:update(context, t)
	if script_data.tutorial_disabled then
		return
	end
