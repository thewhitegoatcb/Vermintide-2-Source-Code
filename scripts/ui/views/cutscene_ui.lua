...
function CutsceneUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function CutsceneUI:_create_ui_elements()
...
function CutsceneUI:handle_event_queue(queue)
...
function CutsceneUI:set_letterbox_enabled(enabled)
	self.letterbox_enabled = enabled
end

function CutsceneUI:set_player_input_enabled(enabled)
	local input_manager = self.input_manager

	if enabled then
		input_manager:release_input({ "keyboard", "gamepad", "mouse" }, 1, "cutscene", "CutsceneUI")
	else
		self.ingame_ui:handle_transition("close_active")
		input_manager:capture_input({ "keyboard", "gamepad", "mouse" }, 1, "cutscene", "CutsceneUI")
	end
end

function CutsceneUI:input_service()
	return self.input_manager:get_service("cutscene")
end

function CutsceneUI:fx_fade(fade_in_time, hold_time, fade_out_time, color)
...
function CutsceneUI:fx_text_popup(fade_in_time, hold_time, fade_out_time, text)
...
function CutsceneUI:check_for_fade()
	local cutscene_system = self.cutscene_system
