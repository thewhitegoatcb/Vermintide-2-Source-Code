...
local RPCS = { "rpc_ping_unit", "rpc_ping_world_position", "rpc_remove_ping", "rpc_social_message" }
...
local extensions = { "ContextAwarePingExtension", "PingTargetExtension" }
...
function PingSystem:init(context, system_name)
	...
	if ping_mode then
		...
		self._outlines_enabled = { item = true, unit = true }
...
function PingSystem:destroy()
	self._network_event_delegate:unregister(self)
end

function PingSystem:freeze(unit)
...
function PingSystem:unfreeze(unit)
	return end

function PingSystem:update(context, t)
...
function PingSystem:_update_server(context, t)
	...
	for pinger_unit, data in pairs(pinged_units) do
		...
		if ALIVE [pinged_unit] then
			...
			if ALIVE [pinger_unit] then
				...
				if
				pinger_unit == pinged_unit and t >= start_time + SELF_PING_DURATION then
...
function PingSystem:_update_client(context, t)
	local pinged_units = self._pinged_units
	for pinger_unit, data in pairs(pinged_units) do
...
function PingSystem:hot_join_sync(sender)
	local pinged_units = self._pinged_units
...
function PingSystem:_handle_ping(ping_type, social_wheel_event_id, sender_player, pinger_unit, pinged_unit, position, flash, parent_pinger_unit)
	...
	self._pinged_units [pinger_unit] = {
		...
		position = position and { Vector3.to_elements(position) },
...
function PingSystem:_handle_chat(ping_type, social_wheel_event_id, sender_player, pinger_unit, pinged_unit, chat_messages)
	...
	if valid_social_wheel_id then
		...
		localization_parameters = nil event_text = text
...
function PingSystem:handle_local_ping(ping_type, social_wheel_event_id, sender_player, pinger_unit, pinged_unit, chat_messages)
	self:_handle_chat(ping_type, social_wheel_event_id, sender_player, pinger_unit, pinged_unit, chat_messages)
end

function PingSystem:is_ping_response(pinged_unit, sender_unique_id, position, sent_ping_type)
		...
		for pinger_unit, data in pairs(self._pinged_units) do
			...
			if pinged_unit == data.pinged_unit then
				if sender_unique_id == data.pinger_unique_id then
					do return sent_ping_is_context and PingTypes.CANCEL or sent_ping_type, nil, pinger_unit end
...
function PingSystem:_get_unit_ping_type(pinged_unit, sender_unique_id, sent_ping_type)
	if not self._world_markers_enabled then
...
function PingSystem:_get_world_position_ping_type(position, sender_unique_id, sent_ping_type)
...
function PingSystem:_add_unit_ping(pinger_unit, pinged_unit, flash, ping_type)
...
function PingSystem:_add_world_marker(pinger_unit, pinged_unit, position, ping_type, social_wheel_event_id)
	...
	if not ping_icon and social_wheel_event_id and social_wheel_event_id ~= NetworkLookup.social_wheel_events ["n/a"] then
		...
		if social_wheel_event_settings.event_text_func and pinged_unit then
			...
			localization_parameters and LocalizeArray(localization_parameters)
...
function PingSystem:remove_ping_from_unit(target_unit)
	if not self._pings_enabled then
		return
	end
...
function PingSystem:_remove_ping(pinger_unit)
...
function PingSystem:_is_outline_enabled(unit)
	if self._outlines_enabled.item and
	ScriptUnit.has_extension(unit, "pickup_system") and ScriptUnit.has_extension(unit, "interactable_system") then
		return true
	end
...
function PingSystem:_play_ping_vo(pinger_unit, pinged_unit, ping_type, social_wheel_event_id)
...
function PingSystem:rpc_ping_unit(channel_id, pinger_unit_id, pinged_unit_id, is_level_unit, flash, ping_type, social_wheel_event_id)
...
function PingSystem:rpc_ping_world_position(channel_id, pinger_unit_id, position, ping_type, social_wheel_event_id)
	local pinger_unit = self._unit_storage:unit(pinger_unit_id)
	local sender_player = Managers.player:unit_owner(pinger_unit)
	local sender_unique_id = sender_player:unique_id()
	...
	if self.is_server then
		...
		if new_position then position = new_position or position
		end
...
function PingSystem:rpc_social_message(channel_id, pinger_unit_id, social_wheel_event_id, pinged_unit_id)
...
function PingSystem:rpc_remove_ping(channel_id, pinger_unit_id)
	if not self._pings_enabled then
		return
	end
...
function PingSystem:_play_sound(event)
