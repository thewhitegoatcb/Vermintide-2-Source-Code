...
PeerState = PeerState or CreateStrictEnumTable(
"Broken",
"Connecting",
"Connected",
"Disconnected",
...
"WaitingForEnter",
"WaitingForGameObjectSync",
"WaitingForSpawnPlayer",
"InGame",
"InPostGame")

NetworkServer = class(NetworkServer)

function NetworkServer:init(player_manager, lobby_host, wanted_profile_index, game_server_manager)
...
function NetworkServer:eac_server()
	return self._eac_server
end

function NetworkServer:server_join()
...
function NetworkServer:num_active_peers()
	local num_peers = 0
...
function NetworkServer:num_joining_peers()
	local num_peers = 0
	for peer_id, statemachine in pairs(self.peer_state_machines) do
		local state = statemachine.current_state
...
function NetworkServer:rpc_notify_connected(channel_id)
...
function NetworkServer:rpc_notify_in_post_game(channel_id, in_post_game)
	local peer_id = CHANNEL_TO_PEER_ID [channel_id]
	if peer_id == self.my_peer_id then
...
function NetworkServer:rpc_game_started(channel_id)
	local peer_id = CHANNEL_TO_PEER_ID [channel_id]
	if peer_id == self.my_peer_id then
...
function NetworkServer:is_in_post_game()
	if DEDICATED_SERVER then
...
function NetworkServer:rpc_to_client_spawn_player(channel_id, ...)
	local peer_id = CHANNEL_TO_PEER_ID [channel_id]
	if peer_id == self.my_peer_id then --[[ Nothing --]]
	end

end

function NetworkServer:on_game_entered(game_network_manager)
...
function NetworkServer:request_profile(local_player_id, profile_name, career_name, force_respawn)
	self._profile_requester:request_profile(Network.peer_id(), local_player_id, profile_name, career_name, force_respawn)
end

function NetworkServer:profile_requester()
	return self._profile_requester
end

function NetworkServer:rpc_is_ingame(channel_id)
	local peer_id = CHANNEL_TO_PEER_ID [channel_id]
	local peer_state_machine = self.peer_state_machines [peer_id]
...
function NetworkServer:rpc_loading_synced(channel_id)

	return end

function NetworkServer:peer_spawned_player(peer_id)
...
function NetworkServer:peer_despawned_player(peer_id)
...
function NetworkServer:peer_respawn_player(peer_id)
...
function NetworkServer:rpc_client_respawn_player(channel_id)
	local peer_id = CHANNEL_TO_PEER_ID [channel_id]
...
function NetworkServer:register_rpcs(network_event_delegate, network_transmit)
	network_event_delegate:register(self,
	"rpc_notify_lobby_joined", "rpc_to_client_spawn_player",
	"rpc_post_game_notified", "rpc_want_to_spawn_player", "rpc_level_load_started",
	"rpc_level_loaded", "rpc_game_started", "rpc_is_ingame", "game_object_sync_done",
	"rpc_notify_connected", "rpc_loading_synced", "rpc_clear_peer_state",
	"rpc_notify_in_post_game", "rpc_client_respawn_player")
...
function NetworkServer:on_level_exit()
...
function NetworkServer:unregister_rpcs()
...
function NetworkServer:has_all_peers_loaded_packages()
	return self.profile_synchronizer:all_synced()
end

function NetworkServer:kick_peer(peer_id)
...
function NetworkServer:update_disconnect_kicked_peers_by_time(dt)
...
function NetworkServer:disconnect_all_peers(reason)
	local reason_id = NetworkLookup.connection_fails [reason]
	local peer_state_machines = self.peer_state_machines
	for peer_id, peer_state_machine in pairs(peer_state_machines) do
		if peer_id ~= Network.peer_id() and
		peer_state_machine.current_state ~= PeerStates.Disconnecting and peer_state_machine.current_state ~= PeerStates.Disconnected then
...
function NetworkServer:disconnect_peer(peer_id, reason)
	local reason_id = NetworkLookup.connection_fails [reason]
	local peer_state_machine = self.peer_state_machines [peer_id]
	local current_state = peer_state_machine.current_state
...
function NetworkServer:force_disconnect_all_client_peers()
	local peer_state_machines = self.peer_state_machines
	for peer_id, peer_state_machine in pairs(peer_state_machines) do
...
function NetworkServer:force_disconnect_client_by_peer_id(peer_id)
	local peer_state_machines = self.peer_state_machines
...
function NetworkServer:rpc_notify_lobby_joined(channel_id, wanted_profile_index, wanted_career_index, wanted_party_index, clan_tag, account_id)
...
function NetworkServer:rpc_post_game_notified(channel_id, in_post_game)
	local remote_peer = CHANNEL_TO_PEER_ID [channel_id]
...
function NetworkServer:rpc_level_load_started(channel_id, level_session_id)
...
function NetworkServer:rpc_level_loaded(channel_id, level_id)
...
function NetworkServer:rpc_want_to_spawn_player(channel_id)
	local remote_peer = CHANNEL_TO_PEER_ID [channel_id]
	local peer_state_machine = self.peer_state_machines [remote_peer]
...
function NetworkServer:game_object_sync_done(peer_id)
...
function NetworkServer:approve_channel(channel_id, peer_id, instance_id)
	...
	local connection = { eac_match_timer = 0,
...
function NetworkServer:close_channel(peer_id)
	local channel_id = PEER_ID_TO_CHANNEL [peer_id]
...
function NetworkServer:_update_connections(peer_state_machines)
	for peer_id, connection in pairs(self._connections) do
...
function NetworkServer:peer_connected(peer_id)
	self._network_state:add_peer(peer_id)
end

function NetworkServer:peer_disconnected(peer_id)
...
function NetworkServer:get_peer_initialized_mechanism(peer_id)
	return self._peer_initialized_mechanisms [peer_id]
end

function NetworkServer:set_peer_initialized_mechanism(peer_id, mechanism_key)
...
function NetworkServer:_handle_peer_left_game(peer_id)
	if peer_id then
...
function NetworkServer:_update_eac_match(dt)
	local peer_state_machines = self.peer_state_machines
	for peer_id, peer_state_machine in pairs(peer_state_machines) do
...
function NetworkServer:eac_check_peer(peer_id)
	...
	match =
	( server_state ~= "banned" and peer_state ~= "banned" or false ) and server_state == peer_state
...
function NetworkServer:_draw_peer_states()
	if DEDICATED_SERVER then
...
function NetworkServer:rpc_clear_peer_state(channel_id)
...
function NetworkServer:_update_reserve_slots(dt)
...
function NetworkServer:add_reserve_slot(peer_id)
	printf("[NetworkServer] Reserved slot for peer (%s) for 15 seconds", peer_id)
	self._reserved_slots [peer_id] = 15
end

function NetworkServer:remove_reserved_slot(peer_id)
	self._reserved_slots [peer_id] = nil
end

function NetworkServer:is_reserved(peer_id)
	return self._reserved_slots [peer_id] ~= nil
end

function NetworkServer:num_reserved_slots()
	return table.size(self._reserved_slots)
end



function NetworkServer:players_past_connecting()
	local peers = FrameTable.alloc_table()
	for peer, sm in pairs(self.peer_state_machines) do
		local is_past_connecting = sm.current_state ~= PeerStates.Connecting and
		sm.current_state ~= PeerStates.Disconnecting and
		sm.current_state ~= PeerStates.Disconnected
...
function NetworkServer:player_is_joining(peer_id)
	local peer_state_machines = self.peer_state_machines
	local peer_state_machine = peer_state_machines [peer_id]
	if not peer_state_machine then
		return false
	...
	local joining = peer_state_machine.current_state == PeerStates.Connecting or
	peer_state_machine.current_state == PeerStates.Loading or
	peer_state_machine.current_state == PeerStates.LoadingProfilePackages or
	peer_state_machine.current_state == PeerStates.WaitingForEnterGame or
	peer_state_machine.current_state == PeerStates.WaitingForGameObjectSync
	return joining
end

local dummy_ignore_map = { }
function NetworkServer:are_all_peers_ingame(ignore_map)
...
function NetworkServer:all_approved_peers_are_connected(ignore_map)
	ignore_map = ignore_map or dummy_ignore_map
	local peer_state_machines = self.peer_state_machines
	for peer_id, peer_state_machine in pairs(peer_state_machines) do
		local current_state = peer_state_machine.current_state
...
function NetworkServer:disconnect_joining_peers()
	local peer_state_machines = self.peer_state_machines
	for peer_id, peer_state_machine in pairs(peer_state_machines) do
		local state_name = peer_state_machine.current_state.state_name
...
function NetworkServer:is_peer_ingame(peer_id)
	return self._network_state:is_peer_ingame(peer_id)
end

function NetworkServer:are_all_peers_ready()
	local peer_state_machines = self.peer_state_machines
	for peer_id, peer_state_machine in pairs(peer_state_machines) do
		local state_name = peer_state_machine.current_state.state_name
...
function NetworkServer:all_client_peers_disconnected()
	local peer_state_machines = self.peer_state_machines
	for peer_id, peer_state_machine in pairs(peer_state_machines) do
		local state_name = peer_state_machine.current_state.state_name
		local client = peer_id ~= self.my_peer_id
...
function NetworkServer:waiting_to_enter_game()
	if DEDICATED_SERVER then
		return true
	end
...
function NetworkServer:disconnected()
	local peer_state_machines = self.peer_state_machines
...
function NetworkServer:peer_wanted_profile(peer_id, local_player_id)
	local peer_state_machine = self.peer_state_machines [peer_id]
	local state_data = peer_state_machine.state_data
	local wanted_profile_index = state_data.wanted_profile_index local wanted_career_index = state_data.wanted_career_index
	return wanted_profile_index, wanted_career_index
end

function NetworkServer:register_shared_state(shared_state)
	self._shared_states [#self._shared_states + 1] = shared_state
end

function NetworkServer:deregister_shared_state(shared_state)
...
function NetworkServer:get_peers()

	return self._network_state and self._network_state:get_peers() or { }
end

function NetworkServer:hot_join_sync_party_and_profiles(peer_id)
