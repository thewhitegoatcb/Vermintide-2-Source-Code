...
function LocomotionUtils.follow_target(unit, blackboard, t, dt)
...
function LocomotionUtils.follow_target_ogre(unit, blackboard, t, dt)
	...
	if blackboard.remembered_threat_pos then
		...
		goal_has_moved = Vector3.distance(Vector3Box.unbox(blackboard.remembered_threat_pos), threat_position) > 1
		if
		not goal_has_moved and blackboard.next_move_check < t then
	...
	if goal_has_moved then
		...
		local above = 2 local below = 2 local horizontal = 3 local distance_from_obstacel = 3
...
local SteeringTweakData = { ROTATION_LERP_LOOK_AT = 20 }



function LocomotionUtils.update_combat_rotation(unit, blackboard, t, dt)
...
function LocomotionUtils.look_at_target_rotation(unit, blackboard, t, dt)
...
function LocomotionUtils.look_at_target_rotation_flat(unit, blackboard, t, dt)
...
function LocomotionUtils.rotation_towards_unit(unit, target_unit)
...
function LocomotionUtils.rotation_towards_unit_flat(unit, target_unit)
	local pos_unit = unit_local_position(unit, 0)
	local pos_target_unit = unit_local_position(target_unit, 0)
...
function LocomotionUtils.look_at_position(unit, position)
...
function LocomotionUtils.look_at_position_flat(unit, position)
	local unit_position = unit_local_position(unit, 0)
...
function LocomotionUtils.get_attack_anim(unit, blackboard, attack_anims)
	if attack_anims then
		...
		if angle > math.pi * 0.95 then
			do return attack_anims.directly_fwd [1], attack_anims.directly_fwd [2] end
		elseif angle > math.pi * 0.75 then
			do return attack_anims.fwd [1], attack_anims.fwd [2] end
		elseif angle < math.pi * 0.25 then
			do return attack_anims.bwd [1], attack_anims.bwd [2] end

		elseif Vector3.cross(my_fwd, to_enemy).z > 0 then
			do return attack_anims.right [1], attack_anims.right [2] end
		else
...
function LocomotionUtils.get_start_anim(unit, blackboard, start_anims)
	if start_anims then
		local target_unit = blackboard.target_unit
		local target_pos = Unit.local_position(target_unit, 0)
		local pos = Unit.local_position(unit, 0)
		...
		if angle > math.pi * 0.75 then
			do return start_anims.fwd end
		elseif angle < math.pi * 0.25 then
			do return start_anims.bwd, true end

		elseif Vector3.cross(my_fwd, to_enemy).z > 0 then
			do return start_anims.right end
		else
...
function LocomotionUtils.constrain_on_clients(unit, constrain, min, max)
...
function LocomotionUtils.set_animation_driven_movement(unit, animation_driven, is_affected_by_gravity, script_driven_rotation, is_on_transport)
...
function LocomotionUtils.set_animation_translation_scale(unit, animation_translation_scale)
	local locomotion_extension = ScriptUnit.extension(unit, "locomotion_system")
...
function LocomotionUtils.set_animation_rotation_scale(unit, animation_rotation_scale)
	local locomotion_extension = ScriptUnit.extension(unit, "locomotion_system")
...
function LocomotionUtils.update_local_animation_driven_movement(unit, dt)
...
function LocomotionUtils.update_local_animation_driven_movement_with_parent(unit, dt, parent)
...
function LocomotionUtils.update_local_animation_driven_movement_with_mover(unit, dt)
	local wanted_pose = Unit.animation_wanted_root_pose(unit)
...
function LocomotionUtils.update_local_animation_driven_movement_plus_mover(unit, dt)
	local mover = Unit.mover(unit)
	local wanted_pose = Unit.animation_wanted_root_pose(unit)
...
function LocomotionUtils.update_local_animation_driven_movement_with_min_z(unit, dt, min_z)
	local wanted_pose = Unit.animation_wanted_root_pose(unit)
...
function LocomotionUtils.new_random_goal(nav_world, blackboard, start_pos, min_dist, max_dist, max_tries, test_points, above, below)
...
function LocomotionUtils.new_random_goal_uniformly_distributed(nav_world, blackboard, start_pos, min_dist, max_dist, max_tries, test_points, above, below)
	local above = above or 30
	local below = below or 30
...
function LocomotionUtils.new_random_goal_uniformly_distributed_with_inside_from_outside_on_last(nav_world, blackboard, start_pos, min_dist, max_dist, max_tries, test_points, above, below, horizontal)
	local above = above or 30
	local below = below or 30
...
function LocomotionUtils.new_random_goal_in_front_of_unit(nav_world, unit, min_dist, max_dist, max_tries, test_points, min_width, max_width, above, below)
	local above = above or 30
	local below = below or 30
...
function LocomotionUtils.new_goal_in_transport(nav_world, unit, ally_unit)
	local tries = 0
...
function LocomotionUtils.outside_goal(nav_world, from_position, target_position, min_distance, max_distance, angle, max_tries, above, below)
	local tries = 0
...
function LocomotionUtils.pick_visible_outside_goal(params)
	...
	local nav_world = params.nav_world local physics_world = params.physics_world
	local from_unit = params.from_unit local to_unit = params.to_unit
	local from_node_name = params.from_node_name local to_node_name = params.to_node_name
	local min_distance = params.min_distance local max_distance = params.max_distance
	local above = params.above local below = params.below
...
function LocomotionUtils.test_pos(nav_world, pos)
	local fail = 0
	local success = 0
	for i = -10, 10 do
		for j = -10, 10 do
...
function LocomotionUtils.get_close_pos_on_mesh(nav_world, pos, searches)
...
function LocomotionUtils.get_close_pos_below_on_mesh(nav_world, pos, searches, above, below)
...
local circle_points = { 1, 0, 0.707, 0.707, 0, 1, -0.707, 0.707, -1, 0, -0.707, -0.707, 0, -1, 0.707, -0.707 }

function LocomotionUtils.mesh_positions_closest_to_outside_pos(nav_world, outside_pos, radius, point_list)
...
function LocomotionUtils.closest_mesh_positions_outward(nav_world, outside_pos, radius, point_list)
...
function LocomotionUtils.pos_on_mesh(nav_world, pos, above, below)
	local above = above or 30
	local below = below or 30
...
function LocomotionUtils.ray_can_go_on_mesh(nav_world, position_start, position_end, traverse_logic, above, below)
...
function LocomotionUtils.raycast_on_navmesh(nav_world, position_start, position_end, traverse_logic, above, below, end_pos_nav_projection)
	local projected_start_pos = LocomotionUtils.pos_on_mesh(nav_world, position_start, above, below)
...
function LocomotionUtils.is_on_flat_ground_raycast(physics_world, unit_position)
...
function LocomotionUtils.navmesh_movement_check(unit_position, unit_velocity, nav_world, physics_world, traverse_logic)
...
function LocomotionUtils.clear_los(physics_world, p1, p2, ignore_unit1, ignore_unit2)
	local to_vec = p2 - p1
	local dist = Vector3.length(to_vec)
	local result, num_hits = PhysicsWorld.immediate_raycast(physics_world,
	p1,
	to_vec,
	dist,
	"all",
...
function LocomotionUtils.target_in_los(unit, blackboard)
	...
	if result then
		...
		for i = 1, num_hits do
			...
			if
			not attack_hit_self and hit_unit ~= blackboard.target_unit then
...
function LocomotionUtils.enable_linked_movement(world, child_unit, parent_unit, parent_node_index, offset)
...
function LocomotionUtils.disable_linked_movement(unit)
	local player_manager = Managers.player
...
function LocomotionUtils.calculate_wanted_lerp_velocity(position_current, position_start, position_end, start_time, end_time, dt, t)
...
function LocomotionUtils.in_crosshairs_dodge(unit, blackboard, t, radius, in_crosshairs_time, min_distance, max_distance)
	...
	for i = 1, #units do
		...
		if
		weapon_template and weapon_template.no_dodge then
			...
			if aim_distance < radius and min_distance < dist and dist < max_distance then
				...
				if dot < -0.3 then
					if in_crosshairs_time then
						...
						do return miss_vec, aim_direction end
					end
...
function LocomotionUtils.separate_mover_fallbacks(mover, seprarate_dist)
...
function LocomotionUtils.on_alerted_dodge(unit, blackboard, alerting_unit, enemy_unit)
	local pos = Unit.world_position(unit, Unit.node(unit, "j_neck"))
...
function LocomotionUtils.get_vortex_spin_velocity(unit_position, center_pos, wanted_radius, up_direction, rotation_speed, radius_change_speed, ascension_speed, dt)
...
function LocomotionUtils.check_start_turning(unit, t, dt, blackboard)
...
function LocomotionUtils.update_leaning(unit, blackboard, target_lean_position)
...
function LocomotionUtils.update_turning(unit, t, dt, blackboard)
	local locomotion_extension = blackboard.locomotion_extension
	local navigation_extension = blackboard.navigation_extension
	local position = POSITION_LOOKUP [unit]
...
function LocomotionUtils.reset_turning(unit, blackboard)
