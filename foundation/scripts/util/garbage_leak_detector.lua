GarbageLeakDetector = GarbageLeakDetector or { enabled = false,
	object_callstack_map = setmetatable({ }, { __mode = "k" }) }



function GarbageLeakDetector.register_object(object, object_name)
...
local function debug_search_stack(what, path, path_n)
	...
	while true do
		...
		if not function_info then break
		end
...
local has_run = nil
function GarbageLeakDetector.run_leak_detection(do_assert)
