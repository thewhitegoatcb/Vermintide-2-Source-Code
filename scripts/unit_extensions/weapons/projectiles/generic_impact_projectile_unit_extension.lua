...
function GenericImpactProjectileUnitExtension:init(extension_init_context, unit, extension_init_data)
...
function GenericImpactProjectileUnitExtension:extensions_ready(world, unit)
...
function GenericImpactProjectileUnitExtension:destroy()
	Unit.flow_event(self.unit, "lua_projectile_end")
end

function GenericImpactProjectileUnitExtension:update(unit, input, _, context, t)
	...
	for i = 1, num_impacts / STRIDE do
		...
		if is_level_unit then
			level_unit_id = unit_id game_object_id = NetworkConstants.game_object_id_max
		elseif unit_id then
			level_unit_id = 0 game_object_id = unit_id
		end
...
function GenericImpactProjectileUnitExtension:_execute_impact(recent_impacts, num_impacts)
...
function GenericImpactProjectileUnitExtension:impact(unit, position, direction, normal, actor_index)
...
local dummy_impact = { }
function GenericImpactProjectileUnitExtension:force_impact(unit, hit_position)
