...
local temp_slot_texture_mapping = { slot_healthkit = "consumables_medpack", slot_grenade = "consumables_frag", slot_potion = "consumables_potion_01" }
...
function PlayerInventoryUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function PlayerInventoryUI:destroy()
...
function PlayerInventoryUI:create_ui_elements()
...
function PlayerInventoryUI:set_visible(visible)
	if visible then
		local gamepad_active = self.input_manager:is_device_active("gamepad")
...
function PlayerInventoryUI:overcharge_amount(unit)
...
function PlayerInventoryUI:update(dt, t, my_player)
...
function PlayerInventoryUI:on_gamepad_activated()
	local gamepad_layout = Application.user_setting("gamepad_layout")
	self:set_visible(false)
end

function PlayerInventoryUI:on_gamepad_deactivated()
	self:set_visible(true)
end

function PlayerInventoryUI:update_inventory_slots(dt, ui_scenegraph, ui_renderer, my_player)
			...
			if not slot_data then
				...
				if ammo_count then
					...
					if ammo_data and not ammo_data.destroy_when_out_of_ammo then
						...
						if picked_up_ammo then
							self.ui_animations [ammo_text_1_flash_lookup_table [slot_name]] = UIAnimation.init(UIAnimation.text_flash,
							widget_style.ammo_text_1.text_color, 1, 255, 200, 5, 0.7)
							self.ui_animations [ammo_text_2_flash_lookup_table [slot_name]] = UIAnimation.init(UIAnimation.text_flash,
							widget_style.ammo_text_2.text_color, 1, 255, 200, 5, 0.7)
							self.ui_animations [ammo_text_1_pulse_lookup_table [slot_name]] = UIAnimation.init(UIAnimation.pulse_animation3,
							widget_style.ammo_text_1, "font_size", 26, 24, 5, 0.7)
							self.ui_animations [ammo_text_2_pulse_lookup_table [slot_name]] = UIAnimation.init(UIAnimation.pulse_animation3,
							widget_style.ammo_text_2, "font_size", 26, 24, 5, 0.7)
						end
					else
						widget_content.ammo_text_1 = ""
						widget_content.ammo_text_2 = ""
					...
					if not self.ui_animations [stance_bar_glow_pulse_lookup_table [slot_name]] and bar_progress >= 1 then
						self.ui_animations [stance_bar_glow_pulse_lookup_table [slot_name]] = UIAnimation.init(UIAnimation.pulse_animation,
						widget_style.stance_bar_glow.color, 1, 0, 255, inventory_hud_settings.bar_lit_pulse_duration)
					...
					elseif self.ui_animations [stance_bar_glow_pulse_lookup_table [slot_name]] and not self.ui_animations [stance_bar_lit_glow_out_lookup_table [slot_name]] and bar_progress < 1 then
						...
						self.ui_animations [stance_bar_glow_fade_out_lookup_table [slot_name]] = UIAnimation.init(UIAnimation.function_by_time, widget_style.stance_bar_glow.color,
						1, widget_style.stance_bar_glow.color [1], 0, inventory_hud_settings.bar_lit_fade_out_duration, math.easeInCubic)
					end
...
function PlayerInventoryUI:update_inventory_slots_positions(dt)
...
function PlayerInventoryUI:on_inventory_selected_slot_changed(new_index)
...
function PlayerInventoryUI:add_animation_for_slot_index(index, selected, optional_duration)
	...
	if animations [scenegraph_id] then
		...
		animations [scenegraph_id] = { time = 0,
...
function PlayerInventoryUI:update_slot_animations(dt)
...
function PlayerInventoryUI:animate_slot_widget(animation_data, dt)
	local ui_scenegraph = self.ui_scenegraph
	local scenegraph_definition = definitions.scenegraph_definition
