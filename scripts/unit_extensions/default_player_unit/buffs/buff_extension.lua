...
local _removed_buff = { removed = true }

function BuffExtension:init(extension_init_context, unit, extension_init_data)
	self._unit = unit
...
function BuffExtension:extensions_ready(world, unit)
...
function BuffExtension:destroy()
	self:clear()
end

function BuffExtension:freeze()
	self:clear()
	self._ai_frozen = true
end

function BuffExtension:unfreeze()
	self._ai_frozen = nil
end

function BuffExtension:clear()
...
function BuffExtension:add_buff(template_name, params)
	local unit = self._unit
	if FROZEN [unit] or self._ai_frozen then return
	end
	local buffs = self._buffs
		...
		if not condition or condition(unit, sub_buff_template, params) then
			...
			if perks and table.find(perks, buff_perk_names.burning_balefire) then
				...
				if source_buff_extension and not Managers.state.side:is_ally(unit, source) then
					...
					ticks =
					ticks and math.floor(ticks * duration_multiplier)
			...
			if
			not is_stacking_buff or self:_add_stacking_buff(sub_buff_template, max_stacks, start_time, duration, end_time, params) then
...
function BuffExtension:_add_stacking_buff(sub_buff_template, max_stacks, start_time, duration, end_time, params)
	local current_buff_stacks = self._stacking_buffs [sub_buff_template.name]
...
function BuffExtension:_refresh_duration(buff, start_time, duration, end_time, params, template)
...
function BuffExtension:_add_stat_buff(sub_buff_template, buff)
	if FROZEN [self._unit] or self._ai_frozen then return
	end
	...
	if application_method == "proc" then
		...
		elseif application_method == "min" then
			...
			if not min_stat_buff.all_values then
				min_stat_buff.all_values = { min_stat_buff.value }
			end
...
function BuffExtension:update(unit, input, dt, context, t)
	local world = self.world
	local buffs = self._buffs
...
function BuffExtension:update_stat_buff(stat_buff_type, difference, index)
	local stat_buffs = self._stat_buffs
	local stat_buff = stat_buffs [stat_buff_type]
	local application_method = StatBuffApplicationMethods [stat_buff_type]
	index = index or 1
...
function BuffExtension:remove_buff(id, skip_net_sync)
	if not id then
		return
	end
...
function BuffExtension:queue_remove_buff(id)
	local queue = self._remove_buff_queue or { }
	queue [#queue + 1] = id
	self._remove_buff_queue = queue
end

function BuffExtension:_remove_sub_buff(buff, index, buff_extension_function_params, free_sync_ids)
	local world = self.world
	local buffs = self._buffs
	local template = buff.template
	...
	if buff_to_remove and remove_buff_func ~= "add_buff" then
		for i = 1, self._num_buffs do
			local buff = buffs [i]
			if buff.buff_type == buff_to_remove and


			not buff.duration then
				buff.duration = 0
				buff.is_stale = true
			end
		end
...
function BuffExtension:_remove_stat_buff(buff)
...
function BuffExtension:get_buff_type(buff_type)
	local buffs = self._buffs
...
function BuffExtension:get_buff_by_id(buff_id)
	if not buff_id then
		return nil
	end
...
function BuffExtension:has_buff_type(buff_type)
	local buffs = self._buffs
...
function BuffExtension:has_buff_perk(perk_name)
	local perk_count = self._perks [perk_name]
	return perk_count and perk_count > 0
end

function BuffExtension:num_buff_perk(perk_name)
	return self._perks [perk_name] or 0
end

function BuffExtension:get_non_stacking_buff(buff_type)
	local buffs = self._buffs
...
function BuffExtension:get_stacking_buff(buff_type)
	return self._stacking_buffs [buff_type]
end

function BuffExtension:num_buff_stacks(buff_type)
	local buff_stacks = self._stacking_buffs [buff_type]
	return buff_stacks and #buff_stacks or 0
end

function BuffExtension:num_buff_type(buff_type)
	local buff_stacks = self._stacking_buffs [buff_type]
	if buff_stacks then
		return #buff_stacks
	end
...
function BuffExtension:has_procced(proc_chance, key)
	local prd_states = self._prd_states local success = nil
...
local function WEIGHTED_PROCS_SORT_FUNC(a, b) return b.proc_weight < a.proc_weight end
...
function BuffExtension:trigger_procs(event, ...)
	local event_buffs = self._event_buffs [event]
	...
	for index, buff in pairs(event_buffs) do
		local proc_chance = buff.proc_chance or 1
		if has_authority(buff, is_server, is_local) and
		current_time > (buff._next_proc_t or 0) and
		self:has_procced(proc_chance, buff) then
			...
			temp_weighted_procs [weigthed_proc_keys] = { buff = buff, proc_weight = proc_weight }
...
function BuffExtension:get_buff_value(stat_buff)
	local stat_buffs = self._stat_buffs [stat_buff]
	local procced = false
...
function BuffExtension:apply_buffs_to_value(value, stat_buff)
	local stat_buffs = self._stat_buffs [stat_buff]
...
function BuffExtension:_play_buff_sound(sound_event, is_synced_3p)
	local unit = self._unit
...
function BuffExtension:_play_screen_effect(effect)
	local unit = self._unit
...
function BuffExtension:_stop_screen_effect(effect_id)
	local unit = self._unit
...
function BuffExtension:active_buffs()
	return self._buffs, self._num_buffs
end

function BuffExtension:initial_buff_names()
	return self._initial_buff_names
end

function BuffExtension:get_persistent_buff_names()
...
function BuffExtension:_activate_initial_buffs()
...
function BuffExtension:set_pending_sync_id(buff_id, local_sync_id, sync_type, orphanated)
...
function BuffExtension:apply_sync_id(local_sync_id, server_sync_id)
...
function BuffExtension:apply_remote_sync_id(buff_id, server_sync_id, sync_type, owner_peer_id)
	if buff_id then
		self:_initalize_sync_tables()
		self._id_to_server_sync [buff_id] = server_sync_id
		self._server_sync_to_id [server_sync_id] = buff_id
...
function BuffExtension:generate_sync_id()
	local sync_id = nil
...
function BuffExtension:claim_buff_id()
	local id = self.id
	self.id = id + 1
	return id
end

function BuffExtension:sync_id_to_id(server_sync_id)
	return self._server_sync_to_id [server_sync_id]
end

function BuffExtension:id_to_sync_id(id)
	return self._id_to_server_sync [id]
end


function BuffExtension:buff_sync_type(id)
	return self._buff_to_sync_type [id]
end

function BuffExtension:_free_sync_id(id)
...
function BuffExtension:_build_free_sync_ids_array()
...
function BuffExtension:_initalize_sync_tables()
...
function BuffExtension:_remove_buff_synced(id)
