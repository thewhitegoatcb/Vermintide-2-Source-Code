...
local RPCS = { "rpc_horde_surge_freeze", "rpc_horde_surge_set_level" }





function HordeSurgeHandler:init(is_server, world, events, seed, activate_on_init)
	...
	if self._is_server then
		local game_object_data_table = { progress = 0,
...
function HordeSurgeHandler:register_rpcs(network_event_delegate, network_transmit)
	network_event_delegate:register(self, unpack(RPCS))
	self._network_event_delegate = network_event_delegate
	self._network_transmit = network_transmit
end

function HordeSurgeHandler:unregister_rpcs()
	self._network_event_delegate:unregister(self)
	self._network_event_delegate = nil
	self._network_transmit = nil
end

function HordeSurgeHandler:destroy()
	if self._is_server then
...
function HordeSurgeHandler:server_update(t, dt)
...
function HordeSurgeHandler:client_update(t, dt)
...
function HordeSurgeHandler:_trigger_event()
	local event_data = { }
...
function HordeSurgeHandler:_next_level(t, game_session)
...
function HordeSurgeHandler:freeze_timer(freeze_time)
	fassert(self._is_server, "This should only be called on the server")
...
function HordeSurgeHandler:activate()
	self._active = true
end

function HordeSurgeHandler:deactivate()
	self._active = false
end

function HordeSurgeHandler:get_progress()
	return self._progress
end

function HordeSurgeHandler:get_freeze_time()
	return self._freeze_time
end

function HordeSurgeHandler:is_frozen()
	return self._frozen
end

function HordeSurgeHandler:get_level()
	return self._level_index
end

function HordeSurgeHandler:rpc_horde_surge_freeze(sender, duration)
	self._freeze_time = duration
	self._frozen = true
end

function HordeSurgeHandler:rpc_horde_surge_set_level(sender, level_index, terror_event_index, duration)
...
function HordeSurgeHandler:hot_join_sync(peer_id)
