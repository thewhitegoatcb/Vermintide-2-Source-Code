...
local function attract(config, attractor, attracted)
	local dx = attracted.pos_x - attractor.pos_x local dy = attracted.pos_y - attractor.pos_y
...
local function repel(config, repeller, reppeled)
	local dx = reppeled.pos_x - repeller.pos_x local dy = reppeled.pos_y - repeller.pos_y
	if dx ~= 0 or dy ~= 0 then
		local distance = math.sqrt(dx * dx + dy * dy)
...
local function setup(config, map)
	...
	for key, node in pairs(map) do
		...
		nodes [key] = { acc_y = 0, acc_x = 0, vel_x = 0, vel_y = 0,
			pos_x = pos_x,
			pos_y = pos_y,
	...
	nodes.start_anchor = { acc_y = 0, acc_x = 0, vel_x = 0, anchor = true, vel_y = 0, pos_y = 0,
	...
	edges [#edges + 1] = { from = "start_anchor", to = "start" }



	nodes.final_anchor = { acc_y = 0, acc_x = 0, vel_x = 0, anchor = true, vel_y = 0, pos_y = 0,
	...
	edges [#edges + 1] = { from = "final", to = "final_anchor" }
