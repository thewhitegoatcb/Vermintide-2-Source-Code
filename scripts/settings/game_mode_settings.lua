...
GameModeSettings.base = { cap_power_level = true, class_name = "GameModeBase", use_level_progress = true, visible = false, use_story_lines = true,
	...
	game_mode_states = { "initial_state" },


	social_wheel_by_side = { heroes = "general" } }
...
GameModeSettings.adventure.object_sets = { adventure = true, gm_sp = true }
...
GameModeSettings.adventure.ping_mode = { world_markers = false,
	outlines = { item = true, unit = true } }
...
GameModeSettings.survival.object_sets = { gm_sp = true }
...
GameModeSettings.survival.ping_mode = { world_markers = false,
	outlines = { item = true, unit = true } }
...
GameModeSettings.tutorial.object_sets = { gm_sp = true }
...
GameModeSettings.demo.object_sets = { gm_sp = true }
...
GameModeSettings.inn.social_wheel_by_side = { heroes = "general" }


GameModeSettings.inn.ping_mode = { world_markers = false,
	outlines = { item = true, unit = true } }
...
GameModeSettings.weave.object_sets = { gm_sp = true }
...
GameModeSettings.weave.difficulties = { "normal", "hard", "harder", "hardest", "cataclysm", "cataclysm_2", "cataclysm_3" }
...
GameModeSettings.weave.ping_mode = { world_markers = false,
	outlines = { item = true, unit = true } }
...
GameModeSettings.weave.hud_ui_settings = { killfeed_offset = true }
...
for table_key, settings in pairs(GameModeSettings) do
	if table_key ~= "base" then
		...
		fassert(settings.display_name, "[GameModeSettings] game mode %q is missing parameter \"display_name\".", table_key) end end