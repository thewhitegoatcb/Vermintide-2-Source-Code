...
function PlayerEyeTrackingExtension:init(extension_init_context, unit, extension_init_data)
	...
	self.extended_view = { pitch = 0, yaw = 0 }
...
function PlayerEyeTrackingExtension:update(unit, input, dt, context, t)
...
function PlayerEyeTrackingExtension:set_eyetracking_options_opened(opened)
	self.eyetracking_options_opened = opened
end

function PlayerEyeTrackingExtension:update_extended_view(dt)
...
function PlayerEyeTrackingExtension:get_extended_view(rotation)
	return self.extended_view.yaw, self.extended_view.pitch
end

function PlayerEyeTrackingExtension:get_direction_without_extended_view(rotation)
...
function PlayerEyeTrackingExtension:update_forward_rayhit()
...
function PlayerEyeTrackingExtension:update_gaze_rayhit()
	local first_person_extension = ScriptUnit.extension(self.unit, "first_person_system")
...
function PlayerEyeTrackingExtension:calc_gaze_forward()
	local first_person_extension = ScriptUnit.extension(self.unit, "first_person_system")
...
function PlayerEyeTrackingExtension:gaze_forward()
	return self.current_gaze_forward:unbox()
end

function PlayerEyeTrackingExtension:gaze_rotation()
	local forward = self:gaze_forward()
	return Quaternion.look(forward, Vector3.up())
end

function PlayerEyeTrackingExtension:get_forward_rayhit()
	return self.forward_rayhit_position and self.forward_rayhit_position:unbox() or nil
end

function PlayerEyeTrackingExtension:get_gaze_rayhit()
	self:update_gaze_rayhit()
	return self.gaze_rayhit_position and self.gaze_rayhit_position:unbox() or nil
end

function PlayerEyeTrackingExtension:get_is_aiming()
	return self.is_aiming
end

function PlayerEyeTrackingExtension:set_is_aiming(is_aiming)
	self.is_aiming = is_aiming
end

function PlayerEyeTrackingExtension:get_aim_at_gaze_cancelled()
	return self.is_aiming_cancelled
end

function PlayerEyeTrackingExtension:set_aim_at_gaze_cancelled(is_cancelled)
	self.is_aiming_cancelled = is_cancelled
end

function PlayerEyeTrackingExtension:get_is_feature_enabled(feature)
...
function PlayerEyeTrackingExtension:get_is_connected()
