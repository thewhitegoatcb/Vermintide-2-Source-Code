...
local curl_info_types = { [0] = "info_text", "info_header_in", "info_header_out", "info_data_in", "info_data_out", "info_ssl_data_in", "info_ssl_data_out" }
...
function Request:OnResponse(data)
	self.data = self.data and self.data .. data or data
end

function Request:OnHeader(data)
...
function CurlManager:update(handle_callbacks)
...
function CurlManager:_num_requests()
	return table.size(self._requests)
end

function CurlManager:add_request(request_type, url, body, headers, user_cb, userdata, options)
...
function CurlManager:get(url, headers, request_cb, userdata, options)
	self:add_request("GET", url, nil, headers, request_cb, userdata, options)
end

function CurlManager:post(url, body, headers, request_cb, userdata, options)
	self:add_request("POST", url, body, headers, request_cb, userdata, options)
end

function CurlManager:put(url, body, headers, request_cb, userdata, options)
	self:add_request("PUT", url, body, headers, request_cb, userdata, options)
end

function CurlManager:delete(url, body, headers, request_cb, userdata, options)
	self:add_request("DELETE", url, body, headers, request_cb, userdata, options)
end

function CurlManager:patch(url, body, headers, request_cb, userdata, options)
...
function CurlManager:upload(url, data, cb)
	local easy = self._curl.easy()
