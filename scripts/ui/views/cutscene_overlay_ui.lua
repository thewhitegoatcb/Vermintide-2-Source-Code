...
function CutsceneOverlayUI:init(parent, context)
	self._parent = parent
...
function CutsceneOverlayUI:force_unregister_event_listener()
	local event_manager = Managers.state.event
...
function CutsceneOverlayUI:_create_ui_elements()
...
function CutsceneOverlayUI:destroy()
	local event_manager = Managers.state.event
	if event_manager and self._registered_event then
		event_manager:unregister("event_start_cutscene_overlay", self)
	end
...
function CutsceneOverlayUI:event_start_function(template_settings)
	self:start(template_settings)
end

function CutsceneOverlayUI:start(template_settings)
...
function CutsceneOverlayUI:_present_template_entry(template_list_name, entry)
...
function CutsceneOverlayUI:_convert_string_timestamp_to_float(string_timestamp)
...
function CutsceneOverlayUI:_has_list_entries(template_list_name)
	local templates = self._templates
	local template_list = templates [template_list_name]
	return #template_list > 0
end

function CutsceneOverlayUI:_get_entry_by_time(template_list_name, time)
	local templates = self._templates
	local template_list = templates [template_list_name]
...
function CutsceneOverlayUI:update(dt)
	...
	for name, template_list_data in pairs(self._active_template_lists) do
		...
		elseif not self:_has_list_entries(name) then
			...
			if
			entry_data and not entry_data.initialized then
...
function CutsceneOverlayUI:_fade(widget, max_alpha, progress)
...
function CutsceneOverlayUI:_draw(widget, dt)
