...
function StartMenuStateOverview:on_enter(params)
...
function StartMenuStateOverview:create_ui_elements(params)
...
function StartMenuStateOverview:_get_skin_item_data(index, career_index)
...
function StartMenuStateOverview:_wanted_state()
	local new_state = self.parent:wanted_state()
	return new_state
end

function StartMenuStateOverview:on_exit(params)
...
function StartMenuStateOverview:_update_transition_timer(dt)
...
function StartMenuStateOverview:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function StartMenuStateOverview:post_update(dt, t)
...
function StartMenuStateOverview:draw(dt)
...
function StartMenuStateOverview:_update_animations(dt)
...
function StartMenuStateOverview:_spawn_hero_unit(hero_name)
...
function StartMenuStateOverview:cb_hero_unit_spawned(hero_name)
	local world_previewer = self.world_previewer
	local career_index = self.career_index
...
function StartMenuStateOverview:_populate_career_page(hero_name, career_index)
	local profile_index = FindProfileIndex(hero_name)
...
function StartMenuStateOverview:_set_select_button_enabled(enabled)
	self._widgets_by_name.select_button.content.button_hotspot.disable_button = not enabled
end

function StartMenuStateOverview:_clear_keyboard_selection(button_grid)
	local widgets_by_name = self._widgets_by_name
...
function StartMenuStateOverview:_handle_keyboard_input()
	local gamepad_active = Managers.input:is_device_active("gamepad")
	local mouse_active = Managers.input:is_device_active("mouse")

	local button_grid = { { "play_button", "options_button", "tutorial_button", "cinematics_button", "credits_button", "quit_button" },
...
function StartMenuStateOverview:_handle_input(dt, t)
	local input_service = self:input_service(true)
	local widgets_by_name = self._widgets_by_name
...
function StartMenuStateOverview:_handle_tobii_button(dt)
	local widgets_by_name = self._widgets_by_name
...
function StartMenuStateOverview:game_popup_active()
	return self._show_play_popup
end

function StartMenuStateOverview:_is_button_pressed(widget)
...
function StartMenuStateOverview:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_enter
end

function StartMenuStateOverview:_is_button_hover_exit(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_exit
end

function StartMenuStateOverview:_play_sound(event)
	self.parent:play_sound(event)
end

function StartMenuStateOverview:get_camera_position()
	local world, viewport = self.parent:get_background_world()
	local camera = ScriptViewport.camera(viewport)
	return ScriptCamera.position(camera)
end

function StartMenuStateOverview:get_camera_rotation()
	local world, viewport = self.parent:get_background_world()
	local camera = ScriptViewport.camera(viewport)
	return ScriptCamera.rotation(camera)
end

function StartMenuStateOverview:trigger_unit_flow_event(unit, event_name)
	if unit and Unit.alive(unit) then
		Unit.flow_event(unit, event_name)
	end
end

function StartMenuStateOverview:_start_transition_animation(key, animation_name)
	local params = {
...
function StartMenuStateOverview:_on_option_button_hover(widget, style_id)
...
function StartMenuStateOverview:_on_option_button_dehover(widget, style_id)
	local ui_animations = self._ui_animations
	local animation_name = "option_button_" .. style_id
...
function StartMenuStateOverview:play_sound(event)

	return end

function StartMenuStateOverview:_animate_element_by_time(target, target_index, from, to, time)
	local new_animation = UIAnimation.init(UIAnimation.function_by_time, target, target_index,
	from, to, time, math.ease_out_quad)
	return new_animation
end

function StartMenuStateOverview:_animate_element_by_catmullrom(target, target_index, target_value, p0, p1, p2, p3, time)
	local new_animation = UIAnimation.init(UIAnimation.catmullrom, target, target_index, target_value,
	p0, p1, p2, p3, time)
	return new_animation
end

function StartMenuStateOverview:_init_menu_views()
...
function StartMenuStateOverview:_activate_view(new_view)
...
function StartMenuStateOverview:exit_current_view()
	local active_view = self._active_view
	local views = self._views
	assert(active_view)
...
function StartMenuStateOverview:input_service(ignore_view_input)
