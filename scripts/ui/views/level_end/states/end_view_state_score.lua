...
function EndViewStateScore:on_enter(params)
...
function EndViewStateScore:exit(direction)
	self._exit_started = true
	self:_start_transition_animation("on_enter", "transition_exit")
end

function EndViewStateScore:exit_done()
	return self._exit_started and self._animations.on_enter == nil
end

function EndViewStateScore:create_ui_elements(params)
...
function EndViewStateScore:_create_gamepad_elements()
	...
	local offset = { -frame_settings.texture_sizes.vertical [1], -frame_settings.texture_sizes.horizontal [2], 0 }
	local size = { scenegraph_size [1] + frame_settings.texture_sizes.vertical [1] * 2, scenegraph_size [2] + frame_settings.texture_sizes.horizontal [2] * 2 }
	local style = {
...
function EndViewStateScore:_wanted_state()
	local new_state = self.parent:wanted_menu_state()
	return new_state
end

function EndViewStateScore:set_input_manager(input_manager)
	self.input_manager = input_manager
end

function EndViewStateScore:on_exit(params)
	print("[PlayState] Exit Substate EndViewStateScore")
	self.ui_animator = nil
end

function EndViewStateScore:done()
	return false
end

function EndViewStateScore:_update_transition_timer(dt)
...
function EndViewStateScore:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function EndViewStateScore:post_update(dt, t)

	return end

function EndViewStateScore:_update_gamepad_input(dt, input_service)
...
function EndViewStateScore:show_gamercard(peer_id)
	if peer_id then
...
function EndViewStateScore:_update_animations(dt)
...
function EndViewStateScore:_update_entry_hover()
...
function EndViewStateScore:_set_entry_hover_index(hover_index)
	local widgets_by_name = self._widgets_by_name
...
function EndViewStateScore:_handle_input(dt, t)
	if Development.parameter("tobii_button") then
		self:_handle_tobii_button(dt)
	end
end

function EndViewStateScore:_handle_tobii_button(dt)
	local widgets_by_name = self._widgets_by_name
...
function EndViewStateScore:_is_button_pressed(widget)
	local content = widget.content
...
function EndViewStateScore:_is_button_hover_enter(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
	return hotspot.on_hover_enter
end

function EndViewStateScore:draw(input_service, dt)
...
function EndViewStateScore:_start_transition_animation(key, animation_name)
	local params = {
...
function EndViewStateScore:_animate_element_by_time(target, target_index, from, to, time)
	local new_animation = UIAnimation.init(UIAnimation.function_by_time, target, target_index,
	from, to, time, math.ease_out_quad)
	return new_animation
end

function EndViewStateScore:_animate_element_by_catmullrom(target, target_index, target_value, p0, p1, p2, p3, time)
	local new_animation = UIAnimation.init(UIAnimation.catmullrom, target, target_index, target_value,
	p0, p1, p2, p3, time)
	return new_animation
end

function EndViewStateScore:_transform_player_session_score(players_session_scores)
	local transformed_player_session_score = { group_scores = { } }
		...
		for group_name, group_data in pairs(player_data.group_scores) do
			...
			for index, score_data in ipairs(group_data) do
				if not transformed_player_session_score.group_scores [group_name] [index] then
					transformed_player_session_score.group_scores [group_name] [index] = { player_scores = { } }
				end
...
function EndViewStateScore:_group_scores_by_player_and_topic(score_panel_scores, player_data, player_index)
	for group_name, group_data in pairs(player_data.group_scores) do
		...
		for index, score_data in ipairs(group_data) do
			if not score_panel_scores [group_name] [index] then
				score_panel_scores [group_name] [index] = { player_scores = { } }
			end
...
function EndViewStateScore:_setup_player_scores(players_session_scores)
...
function EndViewStateScore:_setup_level_widget()
...
local position_textures = { nil, "scoreboard_topic_02", "scoreboard_topic_03", "scoreboard_topic_04" }
...
function EndViewStateScore:_set_topic_data(player_data, widget_index)
...
function EndViewStateScore:_setup_score_panel(score_panel_scores, player_names)
...
function EndViewStateScore:_set_score_topic_by_row(row, text)
...
function EndViewStateScore:_setup_hero_score_tooltip(widget, group_scores)
	...
	for group_name, group_data in pairs(group_scores) do
		...
		text_styles [#text_styles + 1] = { vertical_alignment = "top", horizontal_alignment = "left", font_size = 20, font_type = "hell_shark", word_wrap = true,
			name = group_name,
			...
			value_style = { vertical_alignment = "top", word_wrap = true, horizontal_alignment = "right", font_size = 20, font_type = "hell_shark",
...
function EndViewStateScore:_player_score_data_by_stats_id(stats_id)
	return self._players_list [stats_id]
end

function EndViewStateScore:_get_player_position_in_score_table(stats_id, scores_data)
...
function EndViewStateScore:_start_hero_score_animation(animation_name)
	local params = { wwise_world = self.wwise_world }
	return self.ui_animator:start_animation(animation_name, self._hero_widgets, scenegraph_definition, params)
end

function EndViewStateScore:_play_sound(event)
