...
function EnemyRecycler:init(world, nav_world, pos_list, pack_sizes, pack_rotations, pack_members, zone_data_list, level_seed)
...
function EnemyRecycler:_random(...)
	local seed, value = Math.next_random(self._seed, ...)
	self._seed = seed
	return value
end

function EnemyRecycler:_random_dice_roll(prob, alias)
	local seed, value = LoadedDice.roll_seeded(prob, alias, self._seed)
	self._seed = seed
	return value
end

function EnemyRecycler:set_seed(seed)
	fassert(seed and type(seed) == "number", "Bad seed input!")
	self._seed = seed
end

function EnemyRecycler:setup_forbidden_zones(pos)
...
function EnemyRecycler:add_critters()
...
function EnemyRecycler:boxify_waypoint_table(waypoint_table)
	local waypoints = { }
...
function EnemyRecycler:draw_roaming_splines()
...
function EnemyRecycler:inject_roaming_patrol(area_position, area_rot, pack_type, pack_size_ip_unit_name, zone_data)
	...
	if ptrl and self:_random() < ptrl.patrol_chance then
		...
		local prob = pack_data.prob local alias = pack_data.alias
		...
		local event_data = { spline_type = "roaming", optional_pos = spline_start_position, pack_type = pack_type, spline_name = spline_name, pack = pack, spline_way_points = spline_waypoints, zone_data = zone_data }
		local area = {
			spline_start_position, false, 0, 0, "roaming_patrol",
...
function EnemyRecycler:setup(pos_list, pack_sizes, pack_rotations, pack_members, zone_data_list)
	...
	if not CurrentConflictSettings.roaming.disabled then
		for i = 1, #pos_list do
			...
			if not is_allowed_spawn(level, volume_list, volume_list_size, pos) then
				spawn = false
			elseif NavTagVolumeUtils.inside_level_volume_layer(level, nav_tag_volume_handler, pos, "NO_SPAWN") or
			NavTagVolumeUtils.inside_level_volume_layer(level, nav_tag_volume_handler, pos, "NO_BOTS_NO_SPAWN") then
				spawn = false
...
function EnemyRecycler:reset_areas()
	local areas = self.areas
	for i = 1, #areas do
		local area = areas [i]
...
function EnemyRecycler:update(t, dt, player_positions, threat_population, player_areas, use_player_areas)
...
function EnemyRecycler:update_main_path_events(t)
...
function EnemyRecycler:spawn_interest_point(unit_name, position, do_spawn, angle, pack_members, zone_data)
	local extension_init_data = {
		ai_interest_point_system = { recycler = true, do_spawn = do_spawn, pack_members = pack_members, zone_data = zone_data } }
...
function EnemyRecycler:add_breed(breed_name, boxed_pos, boxed_rot, optional_data)
...
function EnemyRecycler.breed_spawned_callback(ai_unit, breed, optional_data)
	local unit_data = optional_data.dead_breed_data
	BREED_DIE_LOOKUP [ai_unit] = { EnemyRecycler.cleanup_dead_breed, unit_data }
end


function EnemyRecycler.cleanup_dead_breed(ai_unit, unit_data)
	unit_data [U_UNIT] = nil

end

function EnemyRecycler:activate_area(area, threat_population)
	...
	if not units_in_area then
		if area_type == "pack" then
			if threat_population == 0 then
				...
				local units_in_area = {
					{ interest_point_unit, unit_name, position } }
		...
		elseif area_type == "event" then
			...
			if not event_data then local data = { optional_pos = boxed_pos }
			end
...
function EnemyRecycler:deactivate_area(area)
	local units_in_area = area [AREA_UNITS]
	local area_type = area [AREA_TYPE]
...
function EnemyRecycler:_update_roaming_spawning(t, player_positions, threat_population, player_zones, use_player_zones)
	...
	if num_to_remove > 0 then
		local function sort_func(a, b) return b < a end
...
function EnemyRecycler:add_terror_event_in_area(boxed_pos, terror_event_name, event_data)
	local nav_group = self.group_manager:get_group_from_position(boxed_pos:unbox())
	self.areas [#self.areas + 1] = { boxed_pos, nil, 0, 0, terror_event_name, nav_group, "event", event_data or false, [11] = self.unique_area_id }

end

function EnemyRecycler:add_main_path_terror_event(boxed_pos, terror_event_name, activation_dist, event_data, optional_spawn_distance)
	...
	main_path_events [num_events] = { travel_dist, boxed_pos, terror_event_name, event_data }
	...
	if num_events == 1 then
		...
		table.sort(main_path_events, function (a, b) return a [1] < b [1] end)
...
function EnemyRecycler:setup_main_path_events(t)
...
function EnemyRecycler:draw_main_path_events(drawer)
	local events = self.main_path_events
	for i = 1, #events do
...
function EnemyRecycler:draw_debug(player_positions)


	local shutdown = self.shutdown_areas

	local drawer = Managers.state.debug:drawer({ mode = "immediate", name = "ai_recycler" })
...
function EnemyRecycler:far_off_despawn(t, dt, player_positions, spawned)
