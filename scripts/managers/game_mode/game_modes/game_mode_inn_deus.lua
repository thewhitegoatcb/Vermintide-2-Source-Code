...
function GameModeInnDeus:init(settings, world, ...)
...
function GameModeInnDeus:register_rpcs(network_event_delegate, network_transmit)
...
function GameModeInnDeus:unregister_rpcs()
...
function GameModeInnDeus:update(t, dt)
	self:_update_objectives()
	self._adventure_spawning:update(t, dt)
end

function GameModeInnDeus:server_update(t, dt)
	self._adventure_spawning:server_update(t, dt)
end

function GameModeInnDeus:evaluate_end_conditions(round_started)
...
function GameModeInnDeus:event_local_player_spawned(is_initial_spawn)
	self._local_player_spawned = true
	self._is_initial_spawn = is_initial_spawn
end

function GameModeInnDeus:COMPLETE_LEVEL()
	COMPLETE_LEVEL_VAR = true
end

function GameModeInnDeus:FAIL_LEVEL()
	FAIL_LEVEL_VAR = true
end



function GameModeInnDeus:player_entered_game_session(peer_id, local_player_id)
...
function GameModeInnDeus:flow_callback_add_spawn_point(unit)
	self._adventure_spawning:add_spawn_point(unit)
end

function GameModeInnDeus:respawn_unit_spawned(unit)
	self._adventure_spawning:respawn_unit_spawned(unit)
end

function GameModeInnDeus:get_respawn_handler()
	return self._adventure_spawning:get_respawn_handler()
end

function GameModeInnDeus:respawn_gate_unit_spawned(unit)
	self._adventure_spawning:respawn_gate_unit_spawned(unit)
end

function GameModeInnDeus:force_respawn(peer_id, local_player_id)
	local status = Managers.party:get_player_status(peer_id, local_player_id)
...
function GameModeInnDeus:_update_objectives()
...
function GameModeInnDeus:_update_objective_marker()
...
function GameModeInnDeus:_state_game_is_matchmaking()
...
local MAP_TYPE = { "map", "map" }
function GameModeInnDeus:_state_choose_map()
	local current_objective_id = self._current_objective_id
...
function GameModeInnDeus:_activate_objective_marker(unit_id)
...
function GameModeInnDeus:_deactivate_objective_marker(unit_id)
	local unit = self._objective_markers [unit_id]
	if unit then
		local extension = ScriptUnit.extension(unit, "tutorial_system")
...
function GameModeInnDeus:rpc_waystone_active(channel_id, waystone_type, waystone_is_active, current_waystone_type)
	self._waystone_is_active = waystone_is_active
	self._waystone_type = waystone_type
	self._current_waystone_type = current_waystone_type
end

function GameModeInnDeus:hot_join_sync(peer_id)
	self._waystone_is_active = false
	self._waystone_type = 0
...
function GameModeInnDeus:local_player_ready_to_start(player, loading_context)
...
function GameModeInnDeus:local_player_game_starts(player, loading_context)
	...
	if show_profile_on_startup and not LEVEL_EDITOR_TEST and not Development.parameter("skip-start-menu") then
		...
		if IS_CONSOLE then
			Managers.ui:handle_transition("initial_character_selection_force", { menu_state_name = "character" })
		else
