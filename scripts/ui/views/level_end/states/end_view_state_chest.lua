...
local chest_idle_animations = { "loot_chest_jump", "loot_chest_jump_02" }
...
function EndViewStateChest:on_enter(params)
	...
	self.chest_settings = { { text = "Chest Tier 1",
			score_requirement = LootChestData.score_thresholds_per_chest [1], total_score = LootChestData.score_thresholds [1] }, { text = "Chest Tier 2",
			score_requirement = LootChestData.score_thresholds_per_chest [2], total_score = LootChestData.score_thresholds [2] }, { text = "Chest Tier 3",
			score_requirement = LootChestData.score_thresholds_per_chest [3], total_score = LootChestData.score_thresholds [3] }, { text = "Chest Tier 4",
			score_requirement = LootChestData.score_thresholds_per_chest [4], total_score = LootChestData.score_thresholds [4] }, { text = "Chest Tier 5",
			score_requirement = LootChestData.score_thresholds_per_chest [5], total_score = LootChestData.score_thresholds [5] }, { text = "Chest Tier 6",
			score_requirement = LootChestData.score_thresholds_per_chest [6], total_score = LootChestData.score_thresholds [6] } }
...
function EndViewStateChest:exit(direction)
...
function EndViewStateChest:exit_done()
	return self._exit_started and self._animations.on_enter == nil
end

function EndViewStateChest:create_ui_elements(params)
	DO_RELOAD = false
...
function EndViewStateChest:_initialize_score_topics()
...
function EndViewStateChest:_wanted_state()
	local new_state = self.parent:wanted_menu_state()
	return new_state
end

function EndViewStateChest:set_input_manager(input_manager)
	self.input_manager = input_manager
end

function EndViewStateChest:on_exit(params)
	print("[PlayState] Exit Substate EndViewStateChest")
	self.ui_animator = nil
end

function EndViewStateChest:_update_transition_timer(dt)
...
function EndViewStateChest:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
	...
	if self._ready_to_exit and
	not self._exit_timer and not self.parent:displaying_reward_presentation() then
...
function EndViewStateChest:post_update(dt, t)
	return end

function EndViewStateChest:_update_animations(dt)
...
function EndViewStateChest:_handle_input(dt, t)
	local widgets_by_name = self._widgets_by_name
end

function EndViewStateChest:draw(input_service, dt)
...
function EndViewStateChest:_start_transition_animation(key, animation_name)
	local params = {
...
function EndViewStateChest:_animate_element_by_time(target, target_index, from, to, time)
	local new_animation = UIAnimation.init(UIAnimation.function_by_time, target, target_index,
	from, to, time, math.ease_out_quad)
	return new_animation
end

function EndViewStateChest:_animate_element_by_catmullrom(target, target_index, target_value, p0, p1, p2, p3, time)
	local new_animation = UIAnimation.init(UIAnimation.catmullrom, target, target_index, target_value,
	p0, p1, p2, p3, time)
	return new_animation
end

function EndViewStateChest:done()
	return self._score_entry_presentation_done
end

function EndViewStateChest:_set_entry_text_progress(progress)
...
function EndViewStateChest:_display_chest_by_settings_index(index, t, instant_spawn)
	...
	self.ui_animator:start_animation(animation_name, self._widgets_by_name, scenegraph_definition, { wwise_world = self.wwise_world })
end

function EndViewStateChest:_spawn_chest_unit(unit_name, instant_spawn, t)
...
function EndViewStateChest:_update_current_chest_enter(dt, t)
...
function EndViewStateChest:_trigger_unit_flow_event(unit, event_name)
	if unit and Unit.alive(unit) then
		Unit.flow_event(unit, event_name)
	end
end

function EndViewStateChest:_get_viewport_world()
	return self.parent:get_viewport_world()
end

function EndViewStateChest:_start_presentation(t)
	...
	for _, score_topic in ipairs(self._score_topics) do
		...
		if score_part and score_part.score > 0 then
			...
			self:_add_score({ id = entry_id, score = score, name = score_topic, amount = amount })
			entry_id = entry_id + 1
		end
	end
...
function EndViewStateChest:_add_score(data)
	...
	local entry = { entry_animation_completed = false,
...
function EndViewStateChest:_animate_score_entries(dt)
...
function EndViewStateChest:_display_next_score_entry()
	local score_entries = self._score_entries
...
function EndViewStateChest:_start_entry_animation(key)
	local params = {
		wwise_world = self.wwise_world,
		render_settings = self.render_settings }
...
function EndViewStateChest:_animate_score_progress(dt, t)
...
function EndViewStateChest:_get_chest_settings_by_total_score(score)
	for index, settings in ipairs(self.chest_settings) do
...
function EndViewStateChest:_set_presentation_progress(presentation_progress, ignore_sound)
	local widgets_by_name = self._widgets_by_name
...
function EndViewStateChest:_update_chest_zoom_wait_time(dt, t)
...
function EndViewStateChest:_update_chest_zoom_time(dt, t)
...
function EndViewStateChest:_update_chest_bonus_time(dt, t)
...
function EndViewStateChest:_update_chest_exit_time(dt, t)
...
function EndViewStateChest:_set_bar_alpha_by_progress(progress)
	local widgets_by_name = self._widgets_by_name
...
function EndViewStateChest:_play_sound(event)
