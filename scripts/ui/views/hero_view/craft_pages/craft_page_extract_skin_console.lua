...
function CraftPageExtractSkinConsole:on_enter(params, settings)
...
function CraftPageExtractSkinConsole:create_ui_elements(params)
...
function CraftPageExtractSkinConsole:on_exit(params)
...
function CraftPageExtractSkinConsole:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function CraftPageExtractSkinConsole:post_update(dt, t)

	return end

function CraftPageExtractSkinConsole:_update_animations(dt)
...
function CraftPageExtractSkinConsole:_is_button_pressed(widget)
...
function CraftPageExtractSkinConsole:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageExtractSkinConsole:_is_button_held(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageExtractSkinConsole:_handle_input(dt, t)
...
function CraftPageExtractSkinConsole:_handle_craft_input_progress(progress)
	return self.parent:_set_input_progress(progress)
end

function CraftPageExtractSkinConsole:craft_result(result, error, reset_slots)
	if not error then
		self._craft_result = result
	end
end

function CraftPageExtractSkinConsole:reset()
...
function CraftPageExtractSkinConsole:present_results()
	local item_grid = self._item_grid
	item_grid:clear_locked_items()
	item_grid:update_items_status()
...
function CraftPageExtractSkinConsole:on_craft_completed()
...
function CraftPageExtractSkinConsole:_update_craft_items()
	local super_parent = self.super_parent
	local item_grid = self._item_grid
...
function CraftPageExtractSkinConsole:_remove_craft_item(backend_id, slot_index)
	local craft_items = self._craft_items
...
function CraftPageExtractSkinConsole:_add_craft_item(backend_id, slot_index, ignore_sound)
...
function CraftPageExtractSkinConsole:_clear_item_grid()
	local craft_items = self._craft_items
	local super_parent = self.super_parent
...
function CraftPageExtractSkinConsole:_has_added_item_by_id(backend_id)
	local craft_items = self._craft_items
...
function CraftPageExtractSkinConsole:_set_craft_button_disabled(disabled)
...
function CraftPageExtractSkinConsole:_exit(selected_level)
	self.exit = true
	self.exit_level_id = selected_level
end

function CraftPageExtractSkinConsole:draw(dt)
...
function CraftPageExtractSkinConsole:_play_sound(event)
	self.super_parent:play_sound(event)
end

function CraftPageExtractSkinConsole:_set_craft_button_text(text, localize)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.craft_button
