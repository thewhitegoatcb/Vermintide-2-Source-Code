...
function BTChaosExaltedSorcererSkulkAction:init(...)
...
function BTChaosExaltedSorcererSkulkAction:enter(unit, blackboard, t)
...
function BTChaosExaltedSorcererSkulkAction:leave(unit, blackboard, t, reason, destroy)
...
function BTChaosExaltedSorcererSkulkAction:run(unit, blackboard, t, dt)
	...
	if blackboard.done_casting_timer < t and spell and
	spell.search_func(self, unit, blackboard, t, spell) then
		return "done"
	end
...
function BTChaosExaltedSorcererSkulkAction:at_goal(unit, blackboard)
	local skulk_data = blackboard.skulk_data
...
function BTChaosExaltedSorcererSkulkAction:move_to(position, unit, blackboard)
	local ai_navigation = blackboard.navigation_extension
	ai_navigation:move_to(position)
	blackboard.move_pos = Vector3Box(position)
end

function BTChaosExaltedSorcererSkulkAction:idle(unit, blackboard)
	self:anim_event(unit, blackboard, "idle")
	blackboard.move_state = "idle"
end

function BTChaosExaltedSorcererSkulkAction:start_move_animation(unit, blackboard)
	local move_animation = blackboard.action.move_animation
	self:anim_event(unit, blackboard, move_animation)
	blackboard.move_state = "moving"
end

function BTChaosExaltedSorcererSkulkAction:anim_event(unit, blackboard, anim)
	local skulk_data = blackboard.skulk_data
...
local TRIES = 15
function BTChaosExaltedSorcererSkulkAction.get_skulk_target(unit, blackboard, teleporting)
...
function BTChaosExaltedSorcererSkulkAction:debug_show_skulk_circle(unit, blackboard)
	local action = blackboard.action
	local skulk_data = blackboard.skulk_data
...
function BTChaosExaltedSorcererSkulkAction:update_dummy(unit, blackboard, t)
	return false
end
...
function BTChaosExaltedSorcererSkulkAction:update_plague_wave(unit, blackboard, t, plague_wave_data)
...
function BTChaosExaltedSorcererSkulkAction:update_cast_missile(unit, blackboard, t, missile_data)
