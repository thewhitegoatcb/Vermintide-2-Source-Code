...
local STATE = { MOVE_TO_NODE = 2, WAITING_FOR_INTERACTION = 1, IS_ALIGNED = 3 }
...
function TrailUrnAlignmentExtension:init(extension_init_context, unit, extension_init_data)
	self._unit = unit
...
function TrailUrnAlignmentExtension:update_interaction_position(interactor_unit)
	...
	if success then
		...
		do return interaction_position end
	else
		local steps = 10
		local d = 1
		for i = 1, steps do
...
function TrailUrnAlignmentExtension:can_interact()
...
function TrailUrnAlignmentExtension:on_client_start_interaction(interactor_unit, t)
	self._elapsed_time = 0
	self._start_time = t
...
function TrailUrnAlignmentExtension:is_unit_pushed_out_off_range(interactor_unit, interactable_unit)
...
function TrailUrnAlignmentExtension:on_client_move_to_node(interactor_unit, interactable_unit, is_husk, t)
	if self._align_state ~= STATE.MOVE_TO_NODE then return false end
...
function TrailUrnAlignmentExtension:lerp_to_node(interactor_unit, duration, t)
	...
	local new_position = Vector3.lerp(self._start_position:unbox(),
	self._interaction_position:unbox(), lerp_t)
	return new_position
end

function TrailUrnAlignmentExtension:on_client_stop(interaction_result)
...
function TrailUrnAlignmentExtension:is_state_move_to_node()
...
function TrailUrnAlignmentExtension:set_state_waiting_for_interaction()
	self._align_state = STATE.WAITING_FOR_INTERACTION
end
function TrailUrnAlignmentExtension:is_state_aligned()
