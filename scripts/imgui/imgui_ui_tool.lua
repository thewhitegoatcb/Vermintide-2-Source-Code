...
local Gui = Gui local Imgui = Imgui
local format = string.format
local function tab2vec2(t) return Vector2(t [1], t [2]) end
local function tab2col(t) return Color(t [1], t [2], t [3], t [4]) end
local function settab(t, x, y) t [1] = x t [2] = y return t end

local select = select local find = string.find
local function do_search(needle, ...)
	for i = 1, select("#", ...) do
		local ok, i, j = pcall(find, select(i, ...), needle)
		if not ok then do return false end
		elseif i then return i, j
		end
	end
end

function ImguiUITool:init()
	self._active = false
	self._draw_ruler = false
	self._draw_canvas = false
	self._ruler_color = { 64, 255, 0, 0 }
	self._highlight_textures = true
	self._drawing_rect = false
	self._hide_ui = false
	self._disable_localization = not not script_data.disable_localization
	self._rect_y = 0 self._rect_x = 0
	self._data_buffer = { }
	...
	self._tabs = { "Render objects", "Scenegraph", "Atlas browser", "Settings", "Help" }
	self._selected_tab = self._tabs [1]
end

local DO_RELOAD = true
function ImguiUITool:update()
	if DO_RELOAD then self:init() DO_RELOAD = false
	end
	if self._active then
		self._data_back_buffer = self._data_buffer self._data_buffer = self._data_back_buffer
		table.clear(self._data_back_buffer)
	end

	local cursor_vec3 = Mouse.axis(Mouse.axis_id("cursor"))
	local canvas_w = 1920 local canvas_h = 1080
	local w, h = Gui.resolution()
	local scale = math.min(w / canvas_w, h / canvas_h)
	local offset_x = 0.5 * (w - canvas_w * scale) local offset_y = 0.5 * (h - canvas_h * scale)
	...
	if self._selected_tab ~= "Atlas browser" then
		...
		if Mouse.pressed(mouse_button_idx) then
			...
			self._rect_y = cursor [2] self._rect_x = cursor [1]
...
local function inside_2d_box_lenient(cursor, box_pos, box_size)
	...
	return box_pos [1] - BORDER <= cursor [1] and cursor [1] <= box_pos [1] + box_size [1] + BORDER and
	box_pos [2] - BORDER <= cursor [2] and cursor [2] <= box_pos [2] + box_size [2] + BORDER
...
function ImguiUITool:draw_border(pos, size, color, label)
	local gui = self:get_gui()
	if not gui or not self._highlight_textures then return end
	pos = pos + Vector3(0, 0, 1)
	return draw_border(gui, pos, size, BORDER, color)
end

function ImguiUITool:draw_label(label, pos, color)
	local gui = self:get_gui()
	if not gui or not self._highlight_textures then return end
	Gui.text(gui, label, "materials/fonts/arial", 16, nil, pos + Vector2(BORDER + 2, BORDER + 2), color)
end

function ImguiUITool:texture(texture_type, texture, pos, size, color)
...
function ImguiUITool:text(ui_renderer, text, font_material, font_size, pos, color)
...
function ImguiUITool:node(node, file)
	...
	if is_hovered then
		...
		if node.parent then
			...
			buffer [#buffer + 1] = format("%s / %s",
			node.horizontal_alignment or "left",
			node.vertical_alignment or "bottom")
		else
			buffer [#buffer + 1] = "n/a"
			buffer [#buffer + 1] = "n/a"
		end
...
function ImguiUITool:scenegraph(scenegraph, parent_scenegraph, scenegraph_id)
	if parent_scenegraph or scenegraph_id then return
	end
...
function ImguiUITool:on_show()
	Debug.hook(UIRenderer, "script_draw_bitmap", function (func, gui, render_settings, material, gui_position, gui_size, color, masked, saturated, retained_id)
		...
		if self._hide_ui then return end
...
function ImguiUITool:_set_columns(num_columns, border, columns_width)
...
function ImguiUITool:do_render_objects()
	self:_set_columns(6, true)
	Imgui.text("Type") Imgui.next_column()
	Imgui.text("Texture/String") Imgui.next_column()
	Imgui.text("Material/Font") Imgui.next_column()
	Imgui.text("Position") Imgui.next_column()
	Imgui.text("(Font) size") Imgui.next_column()
...
function ImguiUITool:do_scenegraph()
	self:_set_columns(6, true)
	Imgui.text("File") Imgui.next_column()
	Imgui.text("Name") Imgui.next_column()
	Imgui.text("Parent") Imgui.next_column()
	Imgui.text("Alignment") Imgui.next_column()
	Imgui.text("Position") Imgui.next_column()
	Imgui.text("Size") Imgui.next_column()
	Imgui.separator()
	local buffer = self._data_buffer
	for i = 1, #buffer do
		local cell = buffer [i]
...
local function get_texture_name(a) return a.texture_name end
local function get_material_name(a) return a.material_name end
local function get_area(a) local s = a.size return s [1] * s [2] end
local function sorters(data, get, id)
	Imgui.same_line()
	if Imgui.small_button("^##ASC_" .. id) then
		table.sort(data, function (a, b) return get(a) < get(b) end)
		printf("[ImguiUITool] Sorted by %s in ASC order", id)
	end
	Imgui.same_line()
	if Imgui.small_button("v##DESC_" .. id) then
		table.sort(data, function (a, b) return get(b) < get(a) end)
		printf("[ImguiUITool] Sorted by %s in DESC order", id)
	end
end

function ImguiUITool:do_asset_browser()
	...
	Imgui.text("Texture") sorters(texture_registry, get_texture_name, "1") Imgui.next_column()
	Imgui.text("Material") sorters(texture_registry, get_material_name, "2") Imgui.next_column()
	Imgui.text("Size") sorters(texture_registry, get_area, "3") Imgui.next_column()
	Imgui.separator()
...
function ImguiUITool:_setting_checkbox(key, label)
	if do_search(self._search, label) then
		self [key] = Imgui.checkbox(label, self [key] or false)
	end
end

function ImguiUITool:_setting_color(key, label)
	if do_search(self._search, label) then
		local col = self [key] or { 255, 255, 255, 255 }
		Colors.set(col, ImguiX.color_edit_4(label, unpack(col)))
		self [key] = col
	end
end

function ImguiUITool:do_settings()
	Imgui.text("Settings")
	Imgui.separator()
...
function ImguiUITool:do_help()
	local needle = self._search
	local find = string.find local sub = string.sub
...
function ImguiUITool:draw()
	...
	if self._selected_tab == "Render objects" then self:do_render_objects()
	elseif self._selected_tab == "Scenegraph" then self:do_scenegraph()
	elseif self._selected_tab == "Atlas browser" then self:do_asset_browser()
	elseif self._selected_tab == "Settings" then self:do_settings()
	elseif self._selected_tab == "Help" then self:do_help()
	end
...
function ImguiUITool:is_persistent()
	return true
end