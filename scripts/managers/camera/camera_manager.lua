...
CameraManager.NODE_PROPERTY_MAP = { "position", "rotation", "vertical_fov", "near_range", "far_range", "yaw_speed", "pitch_speed", "shading_environment", "fade_to_black", "pitch_offset" }
...
function CameraManager:init(world)
	...
	self._sequence_event_settings = { time_to_recover = 0, end_time = 0, start_time = 0 }
	...
	self._tobii_extended_view = { pitch = 0, yaw = 0 }
...
function CameraManager:register_shadow_lights(set)
	...
	for _, index in pairs(set.units) do
		...
		self._shadow_lights [#self._shadow_lights + 1] = { distance = 0, unit = unit }
...
function CameraManager:_update_shadow_lights(dt, viewport)
	...
	if self._shadow_lights_active and viewport == self._shadow_lights_viewport and not table.is_empty(lights) then
		...
		table.sort(lights, function (light1, light2) return light1.distance < light2.distance end)
...
local EMPTY_TABLE = { }
function CameraManager:shading_callback(world, shading_env, viewport)
...
function CameraManager:set_camera_node(viewport_name, tree_id, node_name)
	...
	if not script_data.camera_debug and script_data.camera_node_debug then --[[ Nothing --]]
	end
	...
	local tree = self._node_trees [viewport_name] [tree_id]
	local next_node = { node = tree.nodes [node_name] }
...
function CameraManager:aim_rotation(viewport_name)
	...
	if pitch_offset then
		...
		do return offset_aim_rotation end
	else
		return aim_rotation
	end
end
...
function CameraManager:update(dt, t, viewport_name)
...
function CameraManager:set_additional_fov_multiplier(multiplier)
	self._additional_fov_multiplier = multiplier
end

function CameraManager:set_additional_fov_multiplier_with_lerp_time(multiplier, lerp_time)
	self._additional_fov_multiplier_data = { current_lerp_time = 0,
...
function CameraManager:_smooth_camera_collision(camera_position, safe_position, smooth_radius, near_radius)
	...
	if script_data.camera_debug then
		drawer = Managers.state.debug:drawer({ name = "Intersection" })
		drawer:reset()
	end

	local hit_actors, num_hits = PhysicsWorld.immediate_overlap(physics_world,
	"shape", "sphere",
	"position", cast_from,
	"size", smooth_radius,
	"types", "statics",
	"collision_filter", "filter_camera_sweep")
	if num_hits > 0 then
		if script_data.camera_debug then
...
function CameraManager:camera_effect_sequence_event(event, start_time)
	if not Application.user_setting("camera_shake") then return
	end
...
function CameraManager:camera_effect_shake_event(event_name, start_time, scale)
	if not Application.user_setting("camera_shake") then return
	end
	local data = { }
	...
	if use_rumble and Managers.state.controller_features then
		Managers.state.controller_features:add_effect("camera_shake", { shake_settings = data, scale = scale or 1, duration = duration, event_name = event_name })
	end
...
function CameraManager:stop_weapon_recoil(id)
...
function CameraManager:_update_additional_fov_multiplier(dt)
...
function CameraManager:_calculate_sequence_event_values_recovery(t)
	local new_values = { yaw = 0, z = 0, roll = 0, y = 0, pitch = 0, x = 0 }
	local sequence_event_settings = self._sequence_event_settings
...
function CameraManager:_apply_shake_event(settings, camera_data, t)
...
function CameraManager:_apply_recoil_event(settings, current_data, dt, t)
...
function CameraManager:_apply_extended_view(current_data)
...
function CameraManager:set_tobii_extended_view(yaw, pitch)
...
function CameraManager:_update_camera_properties(camera, shadow_cull_camera, current_node, camera_data, viewport_name)
	...
	if camera_data.position then
		...
		if script_data.camera_debug and Managers.state.debug then
			local drawer = Managers.state.debug:drawer({ name = "CameraManager" })
