...
local extensions = { "NavGraphConnectorExtension", "LevelUnitSmartObjectExtension", "DynamicUnitSmartObjectExtension", "DarkPactClimbingExtension" }
...
function NavGraphSystem:init(context, system_name)
	...
	elseif level_path then
		...
		if Application.can_get("lua", smart_object_path) then
			...
			version = smart_objects.version self.smart_object_count = smart_objects.smart_object_count self.smart_objects = smart_objects.smart_objects
...
function NavGraphSystem:init_nav_graphs(unit, smart_object_id, extension)
	...
	for i = 1, num_smart_objects_in_unit do
		...
		local navgraph = GwNavGraph.create(
		nav_world,
		is_bidirectional,
		control_points,
		debug_color,
...
function NavGraphSystem:spawn_versus_jump_unit(jump_object_data, swap)
	...
	local unit_jump_data = { jump_object_data = jump_object_data }
...
function NavGraphSystem:level_jump_units()
	return self._level_jumps_ready and self.level_jumps
end

function NavGraphSystem:show_all_jump_units(show)
...
function NavGraphSystem:on_add_extension(world, unit, extension_name, extension_init_data)
	local extension = { navgraphs = { } }
	local input = { }
	...
	if extension_name == "NavGraphConnectorExtension" then
		...
		if smart_object_id and self.smart_objects [smart_object_id] and
		not self.no_nav_mesh and (not Unit.has_data(unit, "enabled_on_spawn") or Unit.get_data(unit, "enabled_on_spawn") == true) then
...
function NavGraphSystem:extensions_ready(world, unit, extension_name)
	if extension_name == "DarkPactClimbingExtension" then
		self._level_jumps_ready = true
	end
end

function NavGraphSystem:_level_unit_smart_object_id(unit)
...
function NavGraphSystem:_dynamic_unit_smart_object_id(unit)
...
function NavGraphSystem:queue_add_nav_graph_from_flow(unit)
...
function NavGraphSystem:queue_remove_nav_graph_from_flow(unit)
	local extension = self.unit_extension_data [unit]
...
function NavGraphSystem:add_nav_graph(unit)
	local extension = self.unit_extension_data [unit]
	fassert(extension, "Tried to add nav graph from flow for a unit without nav graph extension. %s", unit)
...
function NavGraphSystem:remove_nav_graph(unit)
	local extension = self.unit_extension_data [unit]
	fassert(extension, "Tried to remove nav graph from flow for a unit without nav graph extension. %s", unit)
...
function NavGraphSystem:init_nav_graph_from_flow(unit)
	local extension = self.unit_extension_data [unit]
...
function NavGraphSystem:smart_object_from_unit_data(unit, smart_object_id)
	local smart_object = { }
	local i = 0
...
function NavGraphSystem:update(context, t, dt)
...
function NavGraphSystem:hot_join_sync(sender)
	return end

function NavGraphSystem:get_smart_object_type(smart_object_id)
	return self.smart_object_types [smart_object_id]
end

function NavGraphSystem:get_smart_object_data(smart_object_id)
	return self.smart_object_data [smart_object_id]
end

function NavGraphSystem:get_smart_objects(smart_object_id)
	return self.smart_objects [smart_object_id]
end

function NavGraphSystem:get_smart_object_id(unit)
	return self.smart_object_ids [unit]
end

function NavGraphSystem:has_nav_graph(unit)
	local nav_graph_extension = self.unit_extension_data [unit]
	if nav_graph_extension then
		do return true, not nav_graph_extension.nav_graph_removed end
	else
		return false, false
	end
end