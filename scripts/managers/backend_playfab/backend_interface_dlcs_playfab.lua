...
function BackendInterfaceDLCsPlayfab:init(backend_mirror)
	self.is_local = false
...
function BackendInterfaceDLCsPlayfab:ready()
	return true
end

function BackendInterfaceDLCsPlayfab:update(dt)
	return end

function BackendInterfaceDLCsPlayfab:_new_id()
	self._last_id = self._last_id + 1

	return self._last_id
end

function BackendInterfaceDLCsPlayfab:update_dlc_ownership()
...
function BackendInterfaceDLCsPlayfab:_update_owned_dlcs_cb(result)
...
function BackendInterfaceDLCsPlayfab:_execute_dlc_specific_logic()
...
function BackendInterfaceDLCsPlayfab:_execute_dlc_logic_cb(result)
	local function_result = result.FunctionResult
	...
	for i = 1, #new_rewards do
		...
		elseif CosmeticUtils.is_cosmetic_item(item_type) then
			local backend_id = self._backend_mirror:add_item(nil, { ItemId = item_id })
			if backend_id then
				local reward = {
...
function BackendInterfaceDLCsPlayfab:get_owned_dlcs()
	return self._owned_dlcs
end

function BackendInterfaceDLCsPlayfab:get_platform_dlcs()
	return self._platform_dlcs
end

function BackendInterfaceDLCsPlayfab:updating_dlc_ownership()
	return self._updating_dlc_ownership
end

function BackendInterfaceDLCsPlayfab:is_unreleased_career(career_name)
