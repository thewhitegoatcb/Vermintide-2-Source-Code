...
local RPCS = { "rpc_add_controlled_unit", "rpc_remove_controlled_unit", "rpc_cancel_current_command", "rpc_command_stand_ground", "rpc_command_attack", "rpc_set_controlled_unit_template" }
...
local extensions = { "AICommanderExtension" }



AICommanderSystem = class(AICommanderSystem, ExtensionSystemBase)

function AICommanderSystem:init(entity_system_creation_context, system_name)
...
function AICommanderSystem:destroy()
	self._network_event_delegate:unregister(self)
end


function AICommanderSystem:register_commander_unit(commander_unit, controlled_unit)
...
function AICommanderSystem:clear_commander_unit(controlled_unit)
	self._commander_unit_lookup [controlled_unit] = nil
end

function AICommanderSystem:get_commander_unit(controlled_unit)
	return self._commander_unit_lookup [controlled_unit]
end

function AICommanderSystem:on_add_extension(world, unit, extension_name, extension_init_data)
...
function AICommanderSystem:on_remove_extension(unit, extension_name)
...
function AICommanderSystem:_cleanup_extension(extension)
...
function AICommanderSystem:rpc_add_controlled_unit(channel_id, commander_unit_id, controlled_unit_id, template_id)
...
function AICommanderSystem:rpc_remove_controlled_unit(channel_id, commander_unit_id, controlled_unit_id)
	local commander_unit = self._unit_storage:unit(commander_unit_id)
	local controlled_unit = self._unit_storage:unit(controlled_unit_id)
...
function AICommanderSystem:rpc_cancel_current_command(channel_id, controlled_unit_id)
...
function AICommanderSystem:rpc_command_stand_ground(channel_id, controlled_unit_id, position, rotation)
...
function AICommanderSystem:rpc_command_attack(channel_id, controlled_unit_id, target_unit_id)
...
function AICommanderSystem:rpc_set_controlled_unit_template(channel_id, controlled_unit_id, controlled_unit_template_id)
	local controlled_unit = self._unit_storage:unit(controlled_unit_id)
	local commander_unit = self:get_commander_unit(controlled_unit)
	if not commander_unit then
