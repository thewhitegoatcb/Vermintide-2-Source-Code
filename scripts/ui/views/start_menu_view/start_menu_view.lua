...
function StartMenuView:init(ingame_ui_context)
...
function StartMenuView:initial_profile_view()
	return self.ingame_ui.initial_profile_view
end

function StartMenuView:_setup_state_machine(state_machine_params, optional_start_state, optional_start_sub_state, optional_params)
...
function StartMenuView:wanted_state()
	return self._wanted_state
end

function StartMenuView:clear_wanted_state()
	self._wanted_state = nil
end

function StartMenuView:input_service(ignore_state_input)
...
function StartMenuView:set_input_blocked(blocked)
	self._input_blocked = blocked
end

function StartMenuView:input_blocked()
	return self._input_blocked
end

function StartMenuView:play_sound(event)
	WwiseWorld.trigger_event(self.wwise_world, event)
end

function StartMenuView:create_ui_elements()
...
function StartMenuView:get_background_world()
...
function StartMenuView:show_hero_world()
...
function StartMenuView:hide_hero_world()
...
function StartMenuView:draw(dt, input_service)
...
function StartMenuView:post_update(dt, t)
	self._machine:post_update(dt, t)
	self.world_previewer:post_update(dt, t)
end

function StartMenuView:_has_active_level_vote()
...
function StartMenuView:update(dt, t)
...
function StartMenuView:on_enter(params)
...
function StartMenuView:set_current_hero(profile_index)
...
function StartMenuView:_get_sorted_players()
...
function StartMenuView:_handle_mouse_input(dt, t, input_service)
	return end

function StartMenuView:_is_selection_widget_pressed(widget)
...
function StartMenuView:hotkey_allowed(input, mapping_data)
...
function StartMenuView:_get_screen_settings_by_state_name(state_name)
...
function StartMenuView:requested_screen_change_by_name(screen_name, sub_screen_name)
...
function StartMenuView:_change_screen_by_name(screen_name, sub_screen_name, optional_params)
...
function StartMenuView:_change_screen_by_index(index)
...
function StartMenuView:post_update_on_enter()
...
function StartMenuView:post_update_on_exit()
...
function StartMenuView:on_exit()
...
function StartMenuView:exit(return_to_game)
	local initial_profile_view = self:initial_profile_view()
...
function StartMenuView:transitioning()
...
function StartMenuView:suspend()
	self.input_manager:device_unblock_all_services("keyboard", 1)
	self.input_manager:device_unblock_all_services("mouse", 1)
	self.input_manager:device_unblock_all_services("gamepad", 1)
	self.suspended = true
...
function StartMenuView:unsuspend()
...
function StartMenuView:_handle_exit(input_service)
	local initial_profile_view = self:initial_profile_view()
	...
	if not initial_profile_view then
		...
		if ( exit_button_widget.content.button_hotspot.on_release or input_service:get("toggle_menu") ) and
		not self:_game_popup_active() then
			self:play_sound("Play_hud_hover")
...
function StartMenuView:_game_popup_active()
	local state_machine = self._machine
	if state_machine then
		local current_state = state_machine:state()
		local current_state_name = current_state.NAME
...
function StartMenuView:close_menu(return_to_main_screen)
	local state_machine = self._machine
	if state_machine then
		local current_state = state_machine:state()
		local current_state_name = current_state.NAME
...
function StartMenuView:destroy()
	if self.viewport_widget then
		UIWidget.destroy(self.ui_top_renderer, self.viewport_widget)
		self.viewport_widget = nil
	end
...
function StartMenuView:_is_button_pressed(widget)
