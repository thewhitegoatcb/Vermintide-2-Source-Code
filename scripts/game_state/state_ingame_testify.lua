...
function StateInGameTestify.wait_for_bots_to_spawn()
...
function StateInGameTestify.request_profiles(_, affiliation)
	local profiles = { }
...
function StateInGameTestify.set_player_profile(_, profile)
...
function StateInGameTestify.set_bot_profile(_, profile)
...
function StateInGameTestify.enable_bots()
	script_data.ai_bots_disabled = false
end



function StateInGameTestify.disable_bots()
...
function StateInGameTestify.add_all_hats()
...
function StateInGameTestify.add_all_weapon_skins()
	for _, entry in ipairs(DebugScreen.console_settings) do
...
function StateInGameTestify.get_available_deus_talent_power_up_tests()
...
function StateInGameTestify.get_available_deus_generic_power_up_tests()
	local power_up_tests = { }
...
function StateInGameTestify.activate_bots_deus_power_up(_, request_parameter)
	...
	for _, bot in pairs(Managers.player:bots()) do
		...
		deus_run_controller:add_power_ups({ power_up }, local_player_id)
...
function StateInGameTestify.activate_player_deus_power_up(_, request_parameter)
	local power_up_name = request_parameter.power_up_name
	local rarity = request_parameter.rarity
	local power_up = DeusPowerUpUtils.generate_specific_power_up(power_up_name, rarity)
...
function StateInGameTestify.reset_deus_power_ups()
	local mechanism = Managers.mechanism:game_mechanism()
	local deus_run_controller = mechanism:get_deus_run_controller()
...
function StateInGameTestify.set_script_data(_, options)
	table.merge(script_data, options)
end



function StateInGameTestify.wait_for_inventory_to_be_loaded()
	local player = Managers.player:local_player()
...
function StateInGameTestify.wait_for_players_inventory_ready()
...
function StateInGameTestify.player_wield_weapon(_, weapon)
	local player = Managers.player:local_player()
	local inventory = ScriptUnit.extension(player.player_unit, "inventory_system")
...
function StateInGameTestify.bot_wield_weapon(_, weapon)
	for _, bot in pairs(Managers.player:bots()) do
...
function StateInGameTestify.set_game_mode_to_weave(state_ingame)
...
function StateInGameTestify.load_weave(state_ingame, weave_name)
...
function StateInGameTestify.make_game_ready_for_next_weave(state_ingame)
...
function StateInGameTestify.set_camera_to_observe_first_bot()
...
function StateInGameTestify.update_camera_to_follow_first_bot_rotation()
...
function StateInGameTestify.teleport_player_to_main_path_point(_, main_path_point)
...
function StateInGameTestify.closest_travel_distance_to_player()
	local player_unit = Managers.player:local_player().player_unit
...
function StateInGameTestify.teleport_player_to_position(_, position)
	local player_unit = Managers.player:local_player().player_unit
...
function StateInGameTestify.teleport_player_randomly_on_main_path()
	local player_unit = Managers.player:local_player().player_unit
...
function StateInGameTestify.set_player_unit_not_visible()
	local player = Managers.player:local_player()
...
function StateInGameTestify.teleport_bots_forward_on_main_path_if_blocked(_, bots_data)
			...
			if bot_distance_from_stored_position < bots_blocked_distance then
				...
				if bots_blocked_time_before_teleportation < delta_time then
					...
					if tp_pos then
						tp_pos.z = tp_pos.z + 1
						Testify:_print(
						"The bot %s has almost not moved since %ss. Teleporting bot to x:%s, y:%s, z:%s",
						bot_id,
						bots_blocked_time_before_teleportation,
						tp_pos.x,
						tp_pos.y,
...
function StateInGameTestify.are_bots_blocked(_, bots_data)
	local bots_stuck_data = bots_data.bots_stuck_data
	local bots_blocked_time_before_teleportation = bots_data.bots_blocked_time_before_teleportation or 6
...
function StateInGameTestify.make_players_invicible()
	for _, player in pairs(Managers.player:players()) do
...
function StateInGameTestify.make_player_and_two_bots_invicible()
	local bots = Managers.player:bots()
...
function StateInGameTestify.post_telemetry_events()
...
function StateInGameTestify.get_main_path_points(_, nb_points)
...
function StateInGameTestify.set_difficulty(_, difficulty)
...
function StateInGameTestify.get_player_current_position()
...
function StateInGameTestify.is_unit_alive(_, unit)
...
function StateInGameTestify.get_unit_health_values(_, unit)
...
function StateInGameTestify.kill_unit(_, unit)
...
function StateInGameTestify.add_buffs_to_heroes(_, buffs)
...
function StateInGameTestify.fail_test(_, message)
