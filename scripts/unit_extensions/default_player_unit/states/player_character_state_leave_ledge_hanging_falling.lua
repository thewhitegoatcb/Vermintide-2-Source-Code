...
function PlayerCharacterStateLeaveLedgeHangingFalling:init(character_state_init_context)
	PlayerCharacterState.init(self, character_state_init_context, "leave_ledge_hanging_falling")
	local context = character_state_init_context
	self.is_server = Managers.player.is_server
end

function PlayerCharacterStateLeaveLedgeHangingFalling:on_enter_animation()
	CharacterStateHelper.play_animation_event(self.unit, "jump_idle")
end

function PlayerCharacterStateLeaveLedgeHangingFalling:on_enter(unit, input, dt, context, t, previous_state, params)
...
function PlayerCharacterStateLeaveLedgeHangingFalling:on_exit(unit, input, dt, context, t, next_state)
...
function PlayerCharacterStateLeaveLedgeHangingFalling:update(unit, input, dt, context, t)
	local csm = self.csm
	local unit = self.unit
