...
function ImguiSoundDebug:destroy()
	self:unregister_events()
end

function ImguiSoundDebug:register_events()
...
function ImguiSoundDebug:unregister_events()
	local event_manager = Managers.state.event
	if event_manager then
...
function ImguiSoundDebug:is_persistent()
	return self._is_persistent
end

function ImguiSoundDebug:update()
...
function ImguiSoundDebug:_update_music_flags()
...
function ImguiSoundDebug:_update_music_players()
	local music_manager = Managers.music
...
function ImguiSoundDebug:_draw_music_player()
...
function ImguiSoundDebug:_draw_music_flags()
...
function ImguiSoundDebug:_draw_music_players()
...
local keys = { "timestamp", "name", "key", "old_value", "new_value" }
function ImguiSoundDebug:_draw_history()
		...
		for _, data in pairs(self._history) do
			table.sort(self._history, function (a, b)
				if self._sort_direction == "asc" then
					do return a [self._sort_history_by]:lower() < b [self._sort_history_by]:lower() end
				else
...
function ImguiSoundDebug:_draw_sort_button(text)
	local final_text = nil
	final_text = self._sort_history_by == text and
	string.format("%s %s", text, self._sort_direction == "asc" and "/\\" or "\\/") or
...
function ImguiSoundDebug:on_music_flag_change(flag, old_value, new_value)
	local data = { name = "flag",
...
function ImguiSoundDebug:on_music_player_state_change(music_player_name, state_name, old_value, new_value)
	local data = {
		timestamp = format_timestamp(os.time()),
...
function ImguiSoundDebug:_set_columns(num_columns, border, columns_width)
...
function ImguiSoundDebug:_reset_columns()
	self:_set_columns(1)
end

local indent_width = 8
function ImguiSoundDebug:_indent()
	self._indent_counter = self._indent_counter + 1
	Imgui.indent(indent_width)
end

function ImguiSoundDebug:_unindent()
	self._indent_counter = self._indent_counter - 1
	Imgui.unindent(indent_width)
end

function ImguiSoundDebug:_verify_indent()
