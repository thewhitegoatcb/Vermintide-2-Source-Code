...
function CraftPageUpgradeItemConsole:on_enter(params, settings)
...
function CraftPageUpgradeItemConsole:setup_recipe_requirements()
...
function CraftPageUpgradeItemConsole:reset_requirements(num_required_ingredients)
...
function CraftPageUpgradeItemConsole:create_ui_elements(params)
...
function CraftPageUpgradeItemConsole:on_exit(params)
...
function CraftPageUpgradeItemConsole:update(dt, t)
	if DO_RELOAD then
		DO_RELOAD = false
...
function CraftPageUpgradeItemConsole:post_update(dt, t)

	return end

function CraftPageUpgradeItemConsole:_update_animations(dt)
...
function CraftPageUpgradeItemConsole:_is_button_pressed(widget)
...
function CraftPageUpgradeItemConsole:_is_button_hovered(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageUpgradeItemConsole:_is_button_held(widget)
	local content = widget.content
	local hotspot = content.button_hotspot
...
function CraftPageUpgradeItemConsole:_handle_input(dt, t)
...
function CraftPageUpgradeItemConsole:_handle_craft_input_progress(progress)
	return self.parent:_set_input_progress(progress)
end

function CraftPageUpgradeItemConsole:craft_result(result, error, reset_slots)
	if not error then
		self._craft_result = result
	end
end

function CraftPageUpgradeItemConsole:reset()
...
function CraftPageUpgradeItemConsole:present_results()
	local item_grid = self._item_grid
	item_grid:clear_locked_items()
	item_grid:update_items_status()
...
function CraftPageUpgradeItemConsole:on_craft_completed()
...
function CraftPageUpgradeItemConsole:_update_craft_items()
	local super_parent = self.super_parent
	local item_grid = self._item_grid
...
function CraftPageUpgradeItemConsole:_remove_craft_item(backend_id, slot_index)
	local craft_items = self._craft_items
...
function CraftPageUpgradeItemConsole:_add_craft_item(backend_id, slot_index, ignore_sound)
...
function CraftPageUpgradeItemConsole:_set_craft_button_disabled(disabled)
...
function CraftPageUpgradeItemConsole:_exit(selected_level)
	self.exit = true
	self.exit_level_id = selected_level
end

function CraftPageUpgradeItemConsole:draw(dt)
...
function CraftPageUpgradeItemConsole:_play_sound(event)
	self.super_parent:play_sound(event)
end

function CraftPageUpgradeItemConsole:_set_craft_button_text(text, localize)
	local widgets_by_name = self._widgets_by_name
	local widget = widgets_by_name.craft_button
	widget.content.button_text = localize and Localize(text) or text
end

function CraftPageUpgradeItemConsole:_add_crafting_material_requirement(index, item_key, amount_text, has_required_amount)
...
function CraftPageUpgradeItemConsole:_clear_item_grid()
	local craft_items = self._craft_items
	local super_parent = self.super_parent
...
function CraftPageUpgradeItemConsole:_has_added_item_by_id(backend_id)
	local craft_items = self._craft_items
