...
local RPCS = { "rpc_to_client_spawn_player", "rpc_is_ready_for_transition", "rpc_apply_environment_variation", "rpc_change_game_mode_state", "rpc_trigger_round_over", "rpc_trigger_level_event" }
...
function GameModeManager:init(world, lobby_host, lobby_client, network_event_delegate, statistics_db, game_mode_key, network_server, network_transmit, profile_synchronizer, game_mode_settings)
	...
	local max_size = 8192
	self._flow_set_data = { units_per_frame = 150, write_index = 1, read_index = 1, size = 0, ring_buffer = Script.new_array(max_size), max_size = max_size }
...
function GameModeManager:deactivate_mutators(is_destroy)
	self._mutator_handler:deactivate_mutators(is_destroy)
end

function GameModeManager:conflict_director_updated_settings()
...
function GameModeManager:setting(setting_name)
	return GameModeSettings [self._game_mode_key] [setting_name]
end


function GameModeManager:gm_event_end_conditions_met(reason, checkpoint_available, percentages_completed)
...
function GameModeManager:is_game_mode_ended()
	return self._gm_event_end_conditions_met
end

function GameModeManager:setup_done()
	self._game_mode:setup_done()
	self._mutator_handler:activate_mutators()
end

function GameModeManager:deactivate_mutator(mutator_name)
	self._mutator_handler:deactivate_mutator(mutator_name)
end

function GameModeManager:player_entered_game_session(peer_id, local_player_id, wanted_party_index)
	self._game_mode:player_entered_game_session(peer_id, local_player_id, wanted_party_index)
end

function GameModeManager:remove_bot(peer_id, local_player_id, update_safe)
	return self._game_mode:remove_bot(peer_id, local_player_id, update_safe)
end

function GameModeManager:player_left_game_session(peer_id, local_player_id)
	self._game_mode:player_left_game_session(peer_id, local_player_id)
end

function GameModeManager:player_joined_party(peer_id, local_player_id, new_party_id, slot_id)
	self._game_mode:player_joined_party(peer_id, local_player_id, new_party_id, slot_id)
end

function GameModeManager:player_left_party(peer_id, local_player_id, party_id, slot_id)
	self._game_mode:player_left_party(peer_id, local_player_id, party_id, slot_id)
end

function GameModeManager:ai_killed(killed_unit, killer_unit, death_data, killing_blow)
...
function GameModeManager:has_activated_mutator(name)
	return self._mutator_handler:has_activated_mutator(name)
end

function GameModeManager:activated_mutators()
	return self._mutator_handler:activated_mutators()
end

function GameModeManager:has_mutator(mutator_name)
	return self._mutator_handler:has_mutator(mutator_name)
end

function GameModeManager:mutators()
	return self._mutator_handler:mutators()
end

function GameModeManager:evaluate_end_zone_activation_conditions()
	return self._mutator_handler:evaluate_end_zone_activation_conditions()
end

function GameModeManager:post_process_terror_event(elements)
	self._mutator_handler:post_process_terror_event(elements)
end

function GameModeManager:bots_disabled()
...
function GameModeManager:get_saved_game_mode_data()
	if self._game_mode.get_saved_game_mode_data then
		return self._game_mode:get_saved_game_mode_data()
	end
end

function GameModeManager:set_object_set_enabled(set_name, enable)
...
function GameModeManager:_set_flow_object_set_enabled(set, enable, set_name)
...
function GameModeManager:event_camera_teleported()

	self._flush_object_set_enable = 3
end

function GameModeManager:post_update(dt, t)
	if self._game_mode.post_update then
		self._game_mode:post_update(dt, t)
	end
end

function GameModeManager:update_flow_object_set_enable(dt)
	local data = self._flow_set_data
	local size = data.size
...
function GameModeManager:_set_flow_object_set_unit_enabled(level, index)
	...
	if new_state ~= nil then
		...
		if self._game_mode_key == "versus" and
		Unit.is_a(unit, "core/volumetrics/units/fog_volume") then
			if new_state then
...
function GameModeManager:get_end_screen_config(game_won, game_lost, player)
...
function GameModeManager:get_player_wounds(profile)
	return self._game_mode:get_player_wounds(profile)
end

function GameModeManager:get_initial_inventory(healthkit, potion, grenade, additional_items, profile)
	return self._game_mode:get_initial_inventory(healthkit, potion, grenade, additional_items, profile)
end

function GameModeManager:flow_cb_set_flow_object_set_enabled(set_name, enabled)
...
function GameModeManager:_init_game_mode(game_mode_key, game_mode_settings)
...
function GameModeManager:rpc_to_client_spawn_player(channel_id, local_player_id, profile_index, career_index, position, rotation, is_initial_spawn)
...
function GameModeManager:gm_event_round_started()
...
function GameModeManager:is_round_started()
	return self._round_started
end

function GameModeManager:disable_lose_condition()
	self._game_mode:disable_lose_condition()
end

function GameModeManager:complete_level()
...
function GameModeManager:wanted_transition()
	return self._game_mode:wanted_transition()
end

function GameModeManager:fail_level()
	self._game_mode:fail_level()
end

function GameModeManager:retry_level()
...
function GameModeManager:disable_player_spawning(disable, reason, safe_position, safe_rotation)
...
function GameModeManager:start_specific_level(level_key, time_until_start)
...
function GameModeManager:update_timebased_level_start(dt)
...
function GameModeManager:pre_update(t, dt)
	self._mutator_handler:pre_update(dt, t)
	self._game_mode:pre_update(t, dt)
end

function GameModeManager:register_looping_event_timer(timer_name, delay, event_name)
	local os_t = os.clock()
	self._looping_event_timers [timer_name] = { delay = delay, next_trigger_time = os_t + delay, event_name = event_name }
end

function GameModeManager:unregister_looping_event_timer(timer_name)
...
function GameModeManager:local_player_ready_to_start(player)
...
function GameModeManager:local_player_game_starts(player, loading_context)
...
function GameModeManager:_update_initial_join(t, dt)
...
function GameModeManager:evaluate_end_condition_outcome(reason, player)
...
function GameModeManager:_save_last_level_completed(reason)
...
function GameModeManager:rpc_is_ready_for_transition(channel_id)
	local peer_id = CHANNEL_TO_PEER_ID [channel_id]
	self._ready_for_transition [peer_id] = true
end

function GameModeManager:game_won(player)
	local game_won, _ = self:evaluate_end_condition_outcome(self._end_reason, player)
	return game_won
end

function GameModeManager:game_lost(player)
	local _, game_lost = self:evaluate_end_condition_outcome(self._end_reason, player)
	return game_lost
end

function GameModeManager:set_end_reason(end_reason)
	self._end_reason = end_reason
end

function GameModeManager:get_end_reason()
...
function GameModeManager:hot_join_sync(peer_id)
...
function GameModeManager:activate_end_level_area(unit, object, from, to)
	local game_mode = self._game_mode

	game_mode:activate_end_level_area(unit, object, from, to)
end

function GameModeManager:debug_end_level_area(unit, object, from, to)
	local game_mode = self._game_mode

	game_mode:debug_end_level_area(unit, object, from, to)
end

function GameModeManager:disable_end_level_area(unit)
	local game_mode = self._game_mode

	game_mode:disable_end_level_area(unit)
end

function GameModeManager:teleport_despawned_players(position)
	self._game_mode:teleport_despawned_players(position)
end

function GameModeManager:flow_callback_add_spawn_point(unit)
	self._game_mode:flow_callback_add_spawn_point(unit)
end

function GameModeManager:flow_callback_add_game_mode_specific_spawn_point(unit)
	local i = 0
	local sides = { }
...
function GameModeManager:remove_respawn_units_due_to_crossroads(removed_path_distances, total_main_path_length)
...
function GameModeManager:recalc_respawner_dist_due_to_crossroads()
	if self._game_mode.recalc_respawner_dist_due_to_crossroads then
		self._game_mode:recalc_respawner_dist_due_to_crossroads()
	end
end

function GameModeManager:respawn_unit_spawned(unit)
	self._game_mode:respawn_unit_spawned(unit)
end

function GameModeManager:respawn_gate_unit_spawned(unit)
	self._game_mode:respawn_gate_unit_spawned(unit)
end

function GameModeManager:profile_changed(peer_id, local_player_id, profile_index, career_index)
	self._game_mode:profile_changed(peer_id, local_player_id, profile_index, career_index)
end

function GameModeManager:force_respawn(peer_id, local_player_id)
	self._game_mode:force_respawn(peer_id, local_player_id)
end

function GameModeManager:force_respawn_dead_players()
	self._game_mode:force_respawn_dead_players()
end

function GameModeManager:set_respawning_enabled(enabled)
	if self._game_mode.set_respawning_enabled then
		self._game_mode:set_respawning_enabled(enabled)
	end
end

function GameModeManager:on_game_mode_data_created(game_session, game_object_id)
...
function GameModeManager:on_game_mode_data_destroyed()
	self._has_created_game_mode_data = false
	self._game_mode:on_game_mode_data_destroyed()
end

function GameModeManager:_update_end_level_areas()
...
function GameModeManager:on_round_end()
	local game_mode = self._game_mode
...
function GameModeManager:change_game_mode_state(state_name)
...
function GameModeManager:get_boss_loot_pickup()
...
function GameModeManager:get_environment_variation_name()
...
function GameModeManager:apply_environment_variation()
	local environment_variation_name = self:get_environment_variation_name()
	if environment_variation_name then
...
function GameModeManager:rpc_apply_environment_variation()
	self:apply_environment_variation()
end

function GameModeManager:rpc_change_game_mode_state(channel_id, state_name_id)
...
function GameModeManager:rpc_trigger_round_over(channel_id)
	self._game_mode.trigger_round_over = true
end

function GameModeManager:rpc_trigger_level_event(channel_id, event)
	local level = LevelHelper:current_level(self._world)
	if level then
