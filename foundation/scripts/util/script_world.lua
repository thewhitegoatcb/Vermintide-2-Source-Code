...
function ScriptWorld.create_viewport(world, name, template, layer, position, rotation, add_shadow_cull_camera, force_no_scaling)
	...
	if splitscreen and not force_no_scaling then
		Viewport.set_data(viewport, "rect", { SPLITSCREEN_OFFSET_X, SPLITSCREEN_OFFSET_Y, SPLITSCREEN_WIDTH, SPLITSCREEN_HEIGHT })
	else
		Viewport.set_data(viewport, "rect", { 0, 0, 1, 1 })
	end
...
function ScriptWorld.create_global_free_flight_viewport(world, template)
	...
	for key, vp in pairs(viewports) do
		...
		if layer < bottom_layer then
			bottom_layer_vp = vp bottom_layer = layer
		end
	end
...
function ScriptWorld.create_shading_environment(world, shading_environment_name, shading_callback, mood_setting)
	...
	World.set_data(world, "shading_settings", { mood_setting, 1 })

	return shading_env
end


function ScriptWorld.spawn_level(world, name, object_sets, position, rotation, shading_callback, mood_setting, time_sliced_spawn)
	...
	levels [name] = { level = level, nested_levels = nested_levels, spawning = time_sliced_spawn }
...
function ScriptWorld.nested_levels(world, name)
	local levels = World.get_data(world, "levels")
	local level_data = levels [name]
...
function ScriptWorld.destroy_level_from_reference(world, level)
	local levels = World.get_data(world, "levels")
...
function ScriptWorld.destroy_sublevels(world, level)
	local levels = World.get_data(world, "levels")
...
function ScriptWorld.optimize_level_units(world, name)
	local levels = World.get_data(world, "levels")
	local level_data = levels [name]
	local base_level = level_data.level
	local nested_levels = level_data.nested_levels
...
function ScriptWorld.trigger_level_loaded(world, name)
	local levels = World.get_data(world, "levels")
	local level_data = levels [name]
	local base_level = level_data.level
	local nested_levels = level_data.nested_levels
...
function ScriptWorld.trigger_level_shutdown(level)
	local sub_levels = Level.get_data(level, "sub_levels")
	if sub_levels then
		for sublevel_name, sub_level in pairs(sub_levels) do
...
function ScriptWorld.set_material_variable_for_particles(world, particle_id, cloud_name, material_variable, value)
