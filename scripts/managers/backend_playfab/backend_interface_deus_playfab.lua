...
function BackendInterfaceDeusPlayFab:init(backend_mirror)
	self._backend_mirror = backend_mirror
	self._belakor_data = { }
	self.super.init(self)
end

function BackendInterfaceDeusPlayFab:get_journey_cycle()
...
function BackendInterfaceDeusPlayFab:has_loaded_belakor_data()
	return self._backend_mirror:has_loaded_belakor_data()
end

function BackendInterfaceDeusPlayFab:set_has_loaded_belakor_data(value)
	self._backend_mirror:set_has_loaded_belakor_data(value)
end


function BackendInterfaceDeusPlayFab:deus_journey_with_belakor(journey_name)
	if not journey_name then return false
	end
...
function BackendInterfaceDeusPlayFab:get_belakor_cycle()
	local current_time = Managers.time:time("main")
...
function BackendInterfaceDeusPlayFab:_generate_belakor_curse_cycle(current_time, remaining_time, cycle)
...
function BackendInterfaceDeusPlayFab:refresh_belakor_cycle()
	self._backend_mirror:deus_refresh_belakor_data()
end

function BackendInterfaceDeusPlayFab:get_rolled_over_soft_currency()
	return self._backend_mirror:get_deus_rolled_over_soft_currency()
end

function BackendInterfaceDeusPlayFab:deus_run_started()
	local request = { FunctionName = "deusRunStarted",
...
function BackendInterfaceDeusPlayFab:write_player_event(event_name, data)
	local request = {
	...
	local function empty_success_callback(result) return end
