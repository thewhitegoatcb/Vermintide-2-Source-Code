...
function TutorialInputUI:init(parent, ingame_ui_context)
	self._parent = parent
...
function TutorialInputUI:destroy()
...
function TutorialInputUI:event_add_tutorial_input(mission_name, unit)
...
function TutorialInputUI:event_update_tutorial_input(mission_name)

	return end

function TutorialInputUI:event_remove_tutorial_input(mission_name)
...
function TutorialInputUI:event_input_changed()
...
function TutorialInputUI:_create_ui_elements()
	self._tutorial_tooltip_animations = { }
...
function TutorialInputUI:_button_texture_data_by_input_action(input_action, alt_button_name, active_template)
	...
	if alt_button_name then
		...
		do return button_texture_data end
	else
...
function TutorialInputUI:update(dt, t)
	if DO_RELOAD then
		self:_create_ui_elements()
	end
...
function TutorialInputUI:_update_animations(dt, t)
...
function TutorialInputUI:_update_tooltip(dt, t)
	...
	local texture_size_y = 0 local texture_size_x = 0
		...
		for i = 1, num_inputs do
			...
			if button_texture_data then
				...
				if button_texture_data.texture then
					...
					widget_content.icon = { button_texture_data.texture }
					widget_style.icon.texture_sizes = { button_texture_data.size }
					...
					for i = 1, #button_texture_data do
						...
						if button_texture_data [i].tileable then
							tile_sizes [i] = { text_width, sizes [i] [2] }
...
function TutorialInputUI:_draw(dt, t)
	local ui_renderer = self._ui_renderer
	local ui_scenegraph = self._ui_scenegraph
...
function TutorialInputUI:hide()
	self._active_tooltip_name = nil
	self:fade_out()
end

local FADE_TIME = 0.25
function TutorialInputUI:fade_in()
	self:_fade(0, 255, FADE_TIME, false)
end

function TutorialInputUI:fade_out()
	self:_fade(255, 0, FADE_TIME, true)
end

function TutorialInputUI:_fade(from_alpha, to_alpha, duration, completed)
	local widget_style = self._tutorial_tooltip_widget.style
...
function TutorialInputUI:has_completed_fade()
...
function TutorialInputUI:set_visible(visible)
