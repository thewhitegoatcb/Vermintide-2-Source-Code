...
settings.achievement_template_file_names = { "scripts/managers/achievements/achievement_templates_lake" }
...
local function _handle_timing_quest(player, completed)
	...
	if completed < 2 then
		timing_time = t
	elseif completed > 1 and
	timing_time and t < timing_time + completion_margain then
		local peer_id = player:network_id()
