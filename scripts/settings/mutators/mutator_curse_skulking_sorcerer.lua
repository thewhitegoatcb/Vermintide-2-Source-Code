...
function curse_skulking_sorcerer.server_initialize_function(context, data)
	MutatorUtils.store_breed_and_action_settings(context, data)
	Breeds.curse_mutator_sorcerer.max_health = MAX_HEALTH
end

function curse_skulking_sorcerer.server_start_function(context, data)
	...
	data.respawn_times = { respawn_time, respawn_time + 1 }
	data.breed_name = "curse_mutator_sorcerer"
end

function curse_skulking_sorcerer.server_stop_function(context, data)
	MutatorUtils.restore_breed_and_action_settings(context, data)
end

function curse_skulking_sorcerer.server_ai_killed_function(context, data, killed_unit, killer_unit, death_data)
	if death_data.breed.name == "curse_mutator_sorcerer" and

	HEALTH_ALIVE [killer_unit] then
